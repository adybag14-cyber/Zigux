...
test "argvSplit reuses shared blank sentinel views without argc output" {
    ...
    var blank = try argvSplit(fba.allocator(), "");
    var whitespace = try argvSplit(fba.allocator(), " \t\n");
    var whitespace_before_nul = try argvSplit(fba.allocator(), " \t\n\x00ignored tail");
    var leading_nul = try argvSplit(fba.allocator(), "\x00ignored tail");
    defer leading_nul.deinit(fba.allocator());
    defer whitespace_before_nul.deinit(fba.allocator());
    defer whitespace.deinit(fba.allocator());
    defer blank.deinit(fba.allocator());
