...
test "replace4 keeps cross-half carry updates aligned with direct recomputation" {
    var ipv4_header = [_]u8{
        0x45, 0x00, 0x00, 0x3c,
        0x1c, 0x46, 0x40, 0x00,
        0x40, 0x06, 0x00, 0x00,
    ...
    const old_saddr: u32 = 0x00ff_ffff;
    const new_saddr: u32 = 0x0100_0000;

    const checksum_before = compute(&ipv4_header);
    ipv4_header[12] = 0x01;
    ipv4_header[13] = 0x00;
    ipv4_header[14] = 0x00;
    ipv4_header[15] = 0x00;
    const checksum_after_increase = compute(&ipv4_header);
    try std.testing.expectEqual(checksum_after_increase, replace4(checksum_before, old_saddr, new_saddr));

    const checksum_before_reverse = checksum_after_increase;
    ipv4_header[12] = 0x00;
    ipv4_header[13] = 0xff;
    ipv4_header[14] = 0xff;
    ipv4_header[15] = 0xff;
    try std.testing.expectEqual(compute(&ipv4_header), replace4(checksum_before_reverse, new_saddr, old_saddr));
}
...
test "folded pseudo-header helpers match direct pseudo-header plus payload recomputation" {
    ...
    const ipv6_packet = [_]u8{
        ...
        0x20, 0x01, 0x0d, 0xb8,              0x00, 0x00, 0x00, 0x02,
        0xde, 0xad, 0xbe, 0xef,              0xca, 0xfe, 0xba, 0xbf,
        0x00, 0x00, 0x00, @as(u8, ipv6_len), 0x00, 0x00, 0x00, ipv6_proto,
