...
test "mutable wrappers keep write-through aliases with runtime-selected native order comparator pointers" {
    const typed_ascending_compare = compareOrderInt;
    const typed_descending_compare = compareDescendingOrderInt;
    const raw_ascending_compare = compareOpaqueOrderInt;
    const raw_descending_compare = compareOpaqueDescendingOrderInt;
