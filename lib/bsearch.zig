...
test "upper bound wrappers return first post-span elements across typed and raw comparators" {
    const ascending = [_]i32{ 1, 4, 4, 4, 9, 16 };
    const descending = [_]i32{ 16, 9, 4, 4, 4, 1 };

    const typed_hit_key = @as(i32, 4);
    const typed_hit = upperBound(i32, i32, &typed_hit_key, ascending[0..], compareCInt) orelse return error.TestUnexpectedResult;
    try std.testing.expectEqual(@as(i32, 9), typed_hit.*);
    try std.testing.expectEqual(@intFromPtr(&ascending[4]), @intFromPtr(typed_hit));

    const typed_mid_key = @as(i32, 5);
    const typed_mid = upperBound(i32, i32, &typed_mid_key, ascending[0..], compareCInt) orelse return error.TestUnexpectedResult;
    try std.testing.expectEqual(@as(i32, 9), typed_mid.*);
    try std.testing.expectEqual(@intFromPtr(&ascending[4]), @intFromPtr(typed_mid));
    ...
    const raw_hit_key = @as(i32, 4);
    const raw_hit = bsearchUpperBound(&raw_hit_key, @ptrCast(descending[0..].ptr), descending.len, @sizeOf(i32), compareCOpaqueDescendingInt) orelse return error.TestUnexpectedResult;
    const typed_raw_hit: *const i32 = @ptrCast(@alignCast(raw_hit));
    ...
    const raw_mid_key = @as(i32, 5);
    const raw_mid = bsearchUpperBound(&raw_mid_key, @ptrCast(descending[0..].ptr), descending.len, @sizeOf(i32), compareCOpaqueDescendingInt) orelse return error.TestUnexpectedResult;
    const typed_raw_mid: *const i32 = @ptrCast(@alignCast(raw_mid));
    try std.testing.expectEqual(@as(i32, 4), typed_raw_mid.*);
    try std.testing.expectEqual(@intFromPtr(&descending[2]), @intFromPtr(typed_raw_mid));
...
test "mutable wrappers keep write-through aliases" {
    ...
    typed_raw_upper.* = 11;
    try std.testing.expectEqual(@as(i32, 11), raw_duplicates[4]);
}
