...
const allowed_window_policy_flags =
    abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_FORCE_DEFERRED |
    abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_SUPPRESS_DROPPED |
    abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_COALESCE_COOKIE;

fn emptyWindowView() abi.ChrdevNotifyAckWindowView {
    return std.mem.zeroInit(abi.ChrdevNotifyAckWindowView, .{});
}

fn emptySummary() abi.ChrdevNotifyAckWindowPolicySummary {
    return std.mem.zeroInit(abi.ChrdevNotifyAckWindowPolicySummary, .{
...
) abi.ChrdevNotifyAckWindowPolicyView {
    return .{
...
pub fn asChrdevNotifyAckWindowView(view: abi.ChrdevNotifyAckWindowPolicyView) abi.ChrdevNotifyAckWindowView {
...
pub fn isValid(view: abi.ChrdevNotifyAckWindowPolicyView) bool {
    if (view.window_policy_reserved != 0) return false;
    if ((view.window_policy_flags & ~allowed_window_policy_flags) != 0) return false;
    return chrdev_notify_ack_window_plan.isValid(asChrdevNotifyAckWindowView(view));
}

pub fn summarize(view: abi.ChrdevNotifyAckWindowPolicyView) abi.ChrdevNotifyAckWindowPolicySummary {
    if (!isValid(view)) return emptySummary();

    const window_summary = chrdev_notify_ack_window_plan.summarize(asChrdevNotifyAckWindowView(view));
    var summary = emptySummary();
    @memcpy(
        std.mem.asBytes(&summary)[0..@sizeOf(abi.ChrdevNotifyAckWindowSummary)],
        std.mem.asBytes(&window_summary),
    );

    summary.window_policy_flags = view.window_policy_flags;
    summary.effective_window_policy_flags = 0;
    summary.effective_window_cookie = 0;
    summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_NONE;
    summary.policy_window_acked_count = 0;
    summary.policy_window_deferred_count = 0;
    summary.policy_window_suppressed_count = 0;
    summary.policy_window_coalesced_count = 0;
    summary.policy_window_dropped_count = 0;
    summary.policy_window_skipped_count = 0;

    switch (window_summary.window_status) {
        abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_NONE => {},
        abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_SKIPPED => {
            summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_SKIPPED;
            summary.policy_window_skipped_count = 1;
        },
        abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_SUPPRESSED => {
            summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_SUPPRESSED;
            summary.policy_window_suppressed_count = 1;
        },
        abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_DROPPED => {
            if ((view.window_policy_flags & abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_SUPPRESS_DROPPED) != 0) {
                summary.effective_window_policy_flags = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_SUPPRESS_DROPPED;
                summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_SUPPRESSED;
                summary.policy_window_suppressed_count = 1;
            } else {
                summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_DROPPED;
                summary.policy_window_dropped_count = 1;
            }
        },
        abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_DEFERRED => {
            if ((view.window_policy_flags & abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_FORCE_DEFERRED) != 0) {
                summary.effective_window_policy_flags = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_FORCE_DEFERRED;
            }
            summary.effective_window_cookie = window_summary.ack_cookie;
            summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_DEFERRED;
            summary.policy_window_deferred_count = 1;
        },
        abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_ACKED => {
            if ((view.window_policy_flags & abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_COALESCE_COOKIE) != 0 and
                window_summary.ack_cookie != 0 and
                (window_summary.ack_cookie == window_summary.notify_cookie or
                    window_summary.ack_cookie == window_summary.completion_cookie))
            {
                summary.effective_window_policy_flags = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_COALESCE_COOKIE;
                summary.effective_window_cookie = window_summary.ack_cookie;
                summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_COALESCED;
                summary.policy_window_coalesced_count = 1;
            } else if ((view.window_policy_flags & abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_FORCE_DEFERRED) != 0) {
                summary.effective_window_policy_flags = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_FORCE_DEFERRED;
                summary.effective_window_cookie = window_summary.ack_cookie;
                summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_DEFERRED;
                summary.policy_window_deferred_count = 1;
                ...
                summary.effective_window_cookie = window_summary.ack_cookie;
                summary.window_policy_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_ACKED;
