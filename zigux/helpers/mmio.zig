...
fn addressAt(base_addr: usize, offset: usize) usize {
    return std.math.add(usize, base_addr, offset) catch unreachable;
}

fn pointerAt(comptime T: type, base_addr: usize, offset: usize) *align(1) volatile T {
    return @ptrFromInt(addressAt(base_addr, offset));
}
...
pub fn allowsInteropPolicyBytes(unsafe_scope: u8, reserved: u8) bool {
    return narrow.allowsVolatileMmioPolicyBytes(unsafe_scope, reserved);
}
...
pub fn allowsInteropPolicyByte(unsafe_scope: u8) bool {
    return narrow.allowsVolatileMmioByte(unsafe_scope);
}
...
pub fn requireInteropPolicy(policy: abi.InteropPolicy) MmioError!void {
    if (!allowsInteropPolicy(policy)) return error.UnsafeScopeDenied;
}

pub fn requireInteropPolicyByte(unsafe_scope: u8) MmioError!void {
    if (!allowsInteropPolicyByte(unsafe_scope)) return error.UnsafeScopeDenied;
}
...
test "phase3 mmio wrappers keep odd-offset volatile accesses reviewable" {
    ...
    try std.testing.expectEqual(@as(u32, 0xc001_d00d), try read32InteropPolicyBytes(base, 3, mmio_scope, 0));
...
test "phase3 mmio wrappers keep volatile-mmio policy gates reviewable" {
    ...
    try std.testing.expectEqual(@as(u32, 0xc001_d00d), try readInteropPolicyByte(u32, base, 4, @intFromEnum(abi.UnsafeScope.volatile_mmio)));

    try writeInteropPolicyBytes(u64, base, 8, 0x0bad_f00d_dead_beef, @intFromEnum(abi.UnsafeScope.volatile_mmio), 0);
    try std.testing.expectEqual(@as(u64, 0x0bad_f00d_dead_beef), try readInteropPolicyBytes(u64, base, 8, @intFromEnum(abi.UnsafeScope.volatile_mmio), 0));
    ...
    try std.testing.expectEqual(@as(u32, 0xfeed_beef), try read32InteropPolicyByte(base, 18, @intFromEnum(abi.UnsafeScope.volatile_mmio)));

    try write64InteropPolicyBytes(base, 24, 0x0123_4567_89ab_cdef, @intFromEnum(abi.UnsafeScope.volatile_mmio), 0);
    try std.testing.expectEqual(@as(u64, 0x0123_4567_89ab_cdef), try read64InteropPolicyBytes(base, 24, @intFromEnum(abi.UnsafeScope.volatile_mmio), 0));
    ...
    try std.testing.expectError(error.UnsafeScopeDenied, readInteropPolicyBytes(u16, base, 2, @intFromEnum(abi.UnsafeScope.volatile_mmio), 1));
    try std.testing.expectError(error.UnsafeScopeDenied, readInteropPolicyByte(u32, base, 4, @intFromEnum(abi.UnsafeScope.none)));
}
