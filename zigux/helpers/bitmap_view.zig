...
pub const bits_per_long: u32 = @intCast(@bitSizeOf(Word));
...
pub fn lastWordMask(nbits: u32) Word {
    ...
    const rem = nbits % bits_per_long;
    if (rem == 0) return ~@as(Word, 0);

    return ~@as(Word, 0) >> @intCast(bits_per_long - rem);
}

pub fn viewFromWords(backing: []const Word, nbits: u32) abi.BitmapView {
    std.debug.assert(backing.len == wordCount(nbits));
    return .{
        .words_addr = if (backing.len == 0) 0 else narrow.addressOf(&backing[0]),
        .nbits = nbits,
        .word_count = @intCast(backing.len),
    };
}

pub fn isValid(view: abi.BitmapView) bool {
...
pub fn testBit(view: abi.BitmapView, bit: u32) bool {
    if (!isValid(view) or bit >= view.nbits) return false;
    if (view.word_count == 0) return false;

    const slice = wordSlice(view);
    const word_index = bit / bits_per_long;
    const bit_index = bit % bits_per_long;
    return ((slice[word_index] >> @intCast(bit_index)) & 1) != 0;
}

pub fn firstSet(view: abi.BitmapView) u32 {
    if (!isValid(view)) return 0;
    if (view.word_count == 0) return view.nbits;

    const slice = wordSlice(view);
    for (slice, 0..) |raw_word, index| {
        const masked = if (index + 1 == slice.len) raw_word & lastWordMask(view.nbits) else raw_word;
        if (masked != 0) {
...
pub fn firstZero(view: abi.BitmapView) u32 {
    if (!isValid(view)) return 0;
    if (view.word_count == 0) return view.nbits;
    ...
    for (slice, 0..) |raw_word, index| {
        var masked = ~raw_word;
        if (index + 1 == slice.len) masked &= lastWordMask(view.nbits);
        if (masked != 0) {
            const offset: u32 = @intCast(@ctz(masked));
            return @intCast(index * bits_per_long + offset);
        }
...
pub fn weight(view: abi.BitmapView) u32 {
    if (!isValid(view)) return 0;
...
pub fn summarize(view: abi.BitmapView) abi.BitmapSummary {
    if (!isValid(view)) return .{ .first_set = 0, .first_zero = 0, .weight = 0, .reserved = 0 };
    return .{
        .first_set = firstSet(view),
        .first_zero = firstZero(view),
        .weight = weight(view),
        .reserved = 0,
    };
}

test "phase3 bitmap view helpers stay bounded and predictable" {
    var backing = [_]Word{
        (@as(Word, 1) << 1) | (@as(Word, 1) << 5) | (@as(Word, 1) << 63),
        (@as(Word, 1) << 4) | (@as(Word, 1) << 9),
    };
    const view = viewFromWords(backing[0..], bits_per_long + 10);
    const summary = summarize(view);

    try std.testing.expect(isValid(view));
    try std.testing.expectEqual(@as(u32, 2), wordCount(bits_per_long + 10));
    ...
    try std.testing.expect(testBit(view, 63));
...
test "phase3 bitmap view empty sentinel behavior stays explicit" {
