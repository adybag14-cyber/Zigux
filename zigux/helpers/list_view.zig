...
pub const ListView = struct {
    ...
    pub fn isEmpty(self: ListView) bool {
        return self.head.next == @intFromPtr(self.head) and self.head.prev == @intFromPtr(self.head);
    }
...
test "list view does not accept a broken empty sentinel as empty" {
    var head = ListHead{ .next = 0, .prev = 0 };
    var stray = ListHead{ .next = 0, .prev = 0 };
    head.next = @intFromPtr(&head);
    head.prev = @intFromPtr(&stray);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&stray)), breakage.actual_prev);
}
