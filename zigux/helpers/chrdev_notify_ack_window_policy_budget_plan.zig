...
fn emptyWindowPolicyView() abi.ChrdevNotifyAckWindowPolicyView {
    return std.mem.zeroInit(abi.ChrdevNotifyAckWindowPolicyView, .{});
}

fn emptySummary() abi.ChrdevNotifyAckWindowPolicyBudgetSummary {
    return std.mem.zeroInit(abi.ChrdevNotifyAckWindowPolicyBudgetSummary, .{
...
) abi.ChrdevNotifyAckWindowPolicyBudgetView {
    return .{
...
pub fn asChrdevNotifyAckWindowPolicyView(view: abi.ChrdevNotifyAckWindowPolicyBudgetView) abi.ChrdevNotifyAckWindowPolicyView {
    if (view.window_policy_budget_reserved != 0) {
        return emptyWindowPolicyView();
    }
...
pub fn isValid(view: abi.ChrdevNotifyAckWindowPolicyBudgetView) bool {
    if (view.window_policy_budget_reserved != 0) return false;
    return chrdev_notify_ack_window_policy_plan.isValid(asChrdevNotifyAckWindowPolicyView(view));
}

pub fn summarize(view: abi.ChrdevNotifyAckWindowPolicyBudgetView) abi.ChrdevNotifyAckWindowPolicyBudgetSummary {
    if (!isValid(view)) return emptySummary();

    const policy_summary = chrdev_notify_ack_window_policy_plan.summarize(asChrdevNotifyAckWindowPolicyView(view));
    var summary = emptySummary();
    @memcpy(
        std.mem.asBytes(&summary)[0..@sizeOf(abi.ChrdevNotifyAckWindowPolicySummary)],
        std.mem.asBytes(&policy_summary),
    );

    summary.window_policy_budget_flags = 0;
    ...
    summary.deferred_window_policy_budget_before = view.deferred_window_policy_budget;
    summary.deferred_window_policy_budget_after = view.deferred_window_policy_budget;
    summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_NONE;
    summary.budget_window_acked_count = 0;
    summary.budget_window_deferred_count = 0;
    summary.budget_window_suppressed_count = 0;
    summary.budget_window_coalesced_count = 0;
    summary.budget_window_dropped_count = 0;
    summary.budget_window_skipped_count = 0;

    switch (policy_summary.window_policy_status) {
        abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_NONE => {},
        abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_SUPPRESSED => {
            summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_SUPPRESSED;
            summary.budget_window_suppressed_count = 1;
        },
        abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_SKIPPED => {
            summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_SKIPPED;
            summary.budget_window_skipped_count = 1;
        },
        abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_DROPPED => {
            summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_DROPPED;
            summary.budget_window_dropped_count = 1;
        },
        abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_DEFERRED => {
            summary.window_policy_budget_flags |= abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_FLAG_BUDGET_APPLIED;
            if (summary.deferred_window_policy_budget_after > 0) {
                summary.deferred_window_policy_budget_after -= 1;
                summary.window_policy_budget_flags |= abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_FLAG_DEFERRED_WINDOW_POLICY_BUDGET_USED;
                summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_DEFERRED;
                summary.budget_window_deferred_count = 1;
            } else {
                summary.window_policy_budget_flags |= abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_FLAG_DEFERRED_WINDOW_POLICY_BUDGET_EXHAUSTED;
                summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_DROPPED;
                summary.budget_window_dropped_count = 1;
            }
        },
        abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_ACKED,
        abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_COALESCED,
        => {
            summary.window_policy_budget_flags |= abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_FLAG_BUDGET_APPLIED;
            if (summary.window_policy_budget_after > 0) {
                summary.window_policy_budget_after -= 1;
                summary.window_policy_budget_flags |= abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_FLAG_WINDOW_POLICY_BUDGET_USED;
                if (policy_summary.window_policy_status == abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_ACKED) {
                    summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_ACKED;
                    summary.budget_window_acked_count = 1;
                } else {
                    summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_COALESCED;
                    summary.budget_window_coalesced_count = 1;
                }
            } else {
                summary.window_policy_budget_flags |= abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_FLAG_WINDOW_POLICY_BUDGET_EXHAUSTED;
                if (summary.deferred_window_policy_budget_after > 0) {
                    summary.deferred_window_policy_budget_after -= 1;
                    summary.window_policy_budget_flags |= abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_FLAG_DEFERRED_WINDOW_POLICY_BUDGET_USED;
                    summary.window_policy_budget_status = abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_STATUS_DEFERRED;
