...
const errno: isize = -340;
const next_errno: isize = -341;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 340 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 340 raw equation stays between its errno neighbors" {
...
test "errno 340 adjacent raw lanes keep value and pointer decoders precise" {
