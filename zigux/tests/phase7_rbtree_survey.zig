...
test "phase 7 rbtree survey keeps the rematerialized direct-helper packet honest" {
    ...
    try expectSliceContains(manifest.ownership_focus, "build-graph truthfulness must keep the split non-owner evidence explicit: `zigux/Makefile` and `.github/workflows/zigux-bootstrap.yml` are readable, while the roadmap-path port, fixture pair, shared build file, and shared validator still do not directly materialize on current master");
    ...
    try expectContains(direct_anchor_note, "Keep the current Phase 7 rbtree reminder surface tied to the returned tool-root helper, the dedicated slice note, the dedicated replay companion, the returned survey and manifest, and the parity checker");
