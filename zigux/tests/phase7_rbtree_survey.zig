...
test "phase 7 rbtree survey manifest records the parked runtime leaf surface and committed parity fixture" {
    ...
    try expectContains(helper_impl, "test \"rbtree cached helpers keep the leftmost pointer aligned\"");
    try expectContains(helper_impl, "test \"rbtree find helpers return duplicate-key ranges\"");
    ...
    try expectContains(helper_impl, "test \"rbtree eraseInitCached detaches cached nodes and keeps cached roots reusable\"");
    ...
    try expectContains(helper_tests, "phase 7 rbtree cleared detached nodes stop postorder traversal");
