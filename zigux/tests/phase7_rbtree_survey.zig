...
test "phase 7 rbtree survey keeps the rematerialized direct-helper packet honest" {
    ...
    try expectSliceContains(manifest.ownership_focus, "the currently readable same-lane rbtree packet now includes the direct helper at `tools/lib/rbtree.zig`, the dedicated parity checker at `scripts/zigux/check-phase7-rbtree-parity.py`, the dedicated replay at `zigux/tests/phase7_rbtree.zig`, and the direct-anchor note, survey, and manifest, so same-lane truthfulness must keep those returned surfaces explicit while still not presenting the dedicated slice, fixture pair, shared build file, or shared validator as returned on current master");
    ...
    try expectContains(manifest.next_bounded_step, "checker-backed direct-helper packet");
    ...
    try expectContains(manifest.next_bounded_step, "scripts/zigux/validate-phase7.py");
    ...
    try expectContains(direct_anchor_note, "Keep the current Phase 7 rbtree reminder surface tied to the returned tool-root helper, the dedicated replay companion, and the new parity checker");
