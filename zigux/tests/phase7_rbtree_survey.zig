...
test "phase 7 rbtree survey keeps the rematerialized direct-helper packet honest" {
    ...
    try std.testing.expectEqualStrings("direct_helper_test_note_survey_manifest_only", manifest.current_direct_readback_state);
    ...
    try expectSliceContains(manifest.ownership_focus, "the currently readable same-lane rbtree packet now includes the direct helper at `tools/lib/rbtree.zig`, the dedicated replay at `zigux/tests/phase7_rbtree.zig`, and the direct-anchor note, survey, and manifest, so same-lane truthfulness must keep those returned surfaces explicit while still not presenting the dedicated slice, fixture pair, parity checker, shared build file, or shared validator as returned on current master");
    ...
    try expectSliceContains(manifest.ownership_focus, "build-graph truthfulness must keep the split non-owner evidence explicit: `zigux/Makefile` and `.github/workflows/zigux-bootstrap.yml` are readable, while the dedicated rbtree slice, fixture pair, shared build file, and shared validator still do not directly materialize on current master");
    try expectContains(manifest.next_bounded_step, "direct-helper-plus-companion");
    try expectContains(manifest.next_bounded_step, "Documentation/zigux/phase7-rbtree-slice.md");
    try expectContains(manifest.next_bounded_step, "scripts/zigux/check-phase7-rbtree-parity.py");

    try expectContains(direct_anchor_note, "Current direct-readback Phase 7 rbtree helper packet now rematerializes one concrete dedicated helper-local companion surface on current `master`");
    ...
    try expectContains(direct_anchor_note, "Keep the current Phase 7 rbtree reminder surface tied to the returned tool-root helper plus this new dedicated replay companion");
