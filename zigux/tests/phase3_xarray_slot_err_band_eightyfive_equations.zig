...
test "representative 85-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 963, 2005, 3009, 4009 };
...
test "every interior eightyfive window keeps consecutive codes and alternating constructor families" {
...
test "eightyfive windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
