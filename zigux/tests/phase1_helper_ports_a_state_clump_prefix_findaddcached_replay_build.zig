...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_state_clump_prefix_findaddcached_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-state-clump-prefix-findaddcached-replay",
        .root_module = root_module,
    });
    const run = b.addRunArtifact(tests);

    const step = b.step(
        "phase1-helper-ports-a-state-clump-prefix-findaddcached-replay",
        "Run the Lane 06 helper-ports-a state/clump/prefix/findAddCached replay",
    );
