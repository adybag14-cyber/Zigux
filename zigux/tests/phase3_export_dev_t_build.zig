...
pub fn build(b: *std.Build) void {
    ...
    const phase3_dev_t = addDevTTest(b, target, optimize);

    const step = b.step(
        "phase3-export-dev-t-test",
        "Run the shared Phase 3 export/UAPI layout and dev_t binding packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_dev_t.step);
}
