...
const errno: isize = -456;
const next_errno: isize = -457;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 456 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 456 raw equation stays between its errno neighbors" {
...
test "errno 456 adjacent raw lanes keep value and pointer decoders precise" {
