...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("lane17_phase9_phase10_handoff_contract.zig"),
        .target = target,
        .optimize = optimize,
    });
    root_module.addOptions("lane17_phase9_phase10_handoff_options", options);

    const tests = b.addTest(.{
        .name = "lane17-phase9-phase10-handoff-contract",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const contract_step = b.step(
        "lane17-phase9-phase10-handoff-contract",
        "Validate Lane 17 Phase 9 workflow handoff through Phase 7 into Phase 10 routes",
    );
    contract_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run Lane 17 Phase 9 to Phase 10 handoff contract");
