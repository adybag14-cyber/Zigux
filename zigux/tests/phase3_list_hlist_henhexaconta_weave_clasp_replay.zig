...
const staged_break_index = 41;
const route = [_]usize{
    60, 0,  35, 34, 59, 1,  36, 33, 58, 2,  37, 32, 57, 3,
    38, 31, 56, 4,  39, 30, 55, 5,  40, 29, 54, 6,  41, 28,
    53, 7,  42, 27, 52, 8,  43, 26, 51, 9,  44, 25, 50, 10,
    45, 24, 49, 11, 46, 23, 48, 12, 47, 22, 21, 13, 20, 19,
    18, 17, 16, 15, 14,
};
...
test "henhexaconta weave-clasp list route exposes staged backlink repair" {
...
test "henhexaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
