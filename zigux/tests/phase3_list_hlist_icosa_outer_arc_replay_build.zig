...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_icosa_outer_arc_replay.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
    ...
    const step = b.step(
        "phase3-list-hlist-icosa-outer-arc-replay",
        "Run the Lane 28 icosa outer-arc list/hlist replay.",
    );
    step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 28 icosa outer-arc list/hlist replay tests.");
