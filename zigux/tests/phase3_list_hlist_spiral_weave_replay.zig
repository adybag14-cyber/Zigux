...
fn spiralWeaveListForwardOnly(head: *ListHead, nodes: *[6]ListHead) void {
    const order = [_]usize{ 1, 3, 5, 4, 2, 0 };
...
fn spiralWeaveHListForwardOnly(head: *HListHead, nodes: *[6]HListNode) void {
    const order = [_]usize{ 1, 3, 5, 4, 2, 0 };
...
test "list view spiral-weave route exposes forward order before backlink repair" {
    var head = ListHead{ .next = 0, .prev = 0 };
    var nodes = [_]ListHead{.{ .next = 0, .prev = 0 }} ** 6;
    seedCleanList(&head, &nodes);
    spiralWeaveListForwardOnly(&head, &nodes);

    const view = list_view.ListView.init(&head);
    const order = [_]*const ListHead{ &nodes[1], &nodes[3], &nodes[5], &nodes[4], &nodes[2], &nodes[0] };
    ...
    try expectListBreak(view, 1, ptr(&nodes[1]), ptr(&nodes[2]));
    nodes[3].prev = ptr(&nodes[1]);
    try expectListBreak(view, 2, ptr(&nodes[3]), ptr(&nodes[4]));
    nodes[5].prev = ptr(&nodes[3]);
    try expectListBreak(view, 3, ptr(&nodes[5]), ptr(&nodes[3]));
    nodes[4].prev = ptr(&nodes[5]);
    try expectListBreak(view, 4, ptr(&nodes[4]), ptr(&nodes[1]));
    nodes[2].prev = ptr(&nodes[4]);
    try expectListBreak(view, 5, ptr(&nodes[2]), ptr(&head));
    nodes[0].prev = ptr(&nodes[2]);
    try expectListBreak(view, 6, ptr(&nodes[0]), ptr(&nodes[5]));
    head.prev = ptr(&nodes[0]);

    try std.testing.expect(view.hasConsistentBacklinks());
    try std.testing.expect(view.firstBrokenBacklink() == null);
    try std.testing.expectEqual(@as(?*const ListHead, &nodes[0]), view.last());
}

test "hlist view spiral-weave route exposes forward order before prev-link repair" {
    var head = HListHead{ .first = 0 };
    var nodes = [_]HListNode{.{ .next = 0, .pprev = 0 }} ** 6;
    seedCleanHList(&head, &nodes);
    spiralWeaveHListForwardOnly(&head, &nodes);

    const view = hlist_view.HListView.init(&head);
    const order = [_]*const HListNode{ &nodes[1], &nodes[3], &nodes[5], &nodes[4], &nodes[2], &nodes[0] };
    try expectHListOrder(view, &order);
    try std.testing.expectEqual(@as(?*const HListNode, &nodes[1]), view.first());
    try std.testing.expectEqual(@as(?*const HListNode, &nodes[0]), view.last());
    ...
    try std.testing.expect(view.firstPprevMatchesHead());
    try expectHListBreak(view, 1, ptr(&nodes[1].next), ptr(&nodes[2].next));
    nodes[3].pprev = ptr(&nodes[1].next);
    try expectHListBreak(view, 2, ptr(&nodes[3].next), ptr(&nodes[4].next));
    nodes[5].pprev = ptr(&nodes[3].next);
    try expectHListBreak(view, 3, ptr(&nodes[5].next), ptr(&nodes[3].next));
    nodes[4].pprev = ptr(&nodes[5].next);
    try expectHListBreak(view, 4, ptr(&nodes[4].next), ptr(&nodes[1].next));
    nodes[2].pprev = ptr(&nodes[4].next);
    try expectHListBreak(view, 5, ptr(&nodes[2].next), ptr(&head.first));
    nodes[0].pprev = ptr(&nodes[2].next);
