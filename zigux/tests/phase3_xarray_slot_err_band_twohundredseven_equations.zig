...
test "twohundredseven raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 50, 980, 2018, 2998, 3888 };
...
test "twohundredseven constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredseven keeps one centered raw and split payload neighbors" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
