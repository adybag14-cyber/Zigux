...
const errno: isize = -497;
const next_errno: isize = -498;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 497 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 497 raw equation stays between its errno neighbors" {
...
test "errno 497 adjacent raw lanes keep value and pointer decoders precise" {
