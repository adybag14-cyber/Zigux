...
test "list view observes lattice-fold route before staged backlink repair" {
    ...
    const route = [_]*const ListHead{ &six, &three, &five, &one, &four, &two };
...
test "hlist view observes lattice-fold route before staged prev-link repair" {
    ...
    const route = [_]*const HListNode{ &six, &three, &five, &one, &four, &two };
