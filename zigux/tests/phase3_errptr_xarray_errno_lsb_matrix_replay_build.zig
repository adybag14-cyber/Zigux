...
pub fn build(b: *std.Build) void {
    ...
    const replay_root_module = b.createModule(.{
        .root_source_file = b.path("phase3_errptr_xarray_errno_lsb_matrix_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase3-errptr-xarray-errno-lsb-matrix-replay-tests",
    ...
    const named_step = b.step(
        "phase3-errptr-xarray-errno-lsb-matrix-replay",
        "Run the Phase 3 err_ptr/xarray errno LSB matrix replay",
    );
    named_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run the Phase 3 err_ptr/xarray errno LSB matrix replay tests");
