...
const errno: isize = -385;
const next_errno: isize = -386;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 385 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 385 raw equation stays between its errno neighbors" {
...
test "errno 385 adjacent raw lanes keep value and pointer decoders precise" {
