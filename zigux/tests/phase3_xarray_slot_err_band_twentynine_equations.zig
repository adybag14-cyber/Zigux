...
test "representative 29-raw err-band windows keep alias-err spacing exact" {
    ...
    for (left_residues) |left_residue| {
        ...
        try testing.expectEqual(raws[center_index], (center_payload << 1) | xa_value.value_tag_mask);

        for (0..(center_index / 2) + 1) |pair_idx| {
...
test "every interior twentynine window keeps consecutive codes and alternating constructor families" {
    ...
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        for (0..(center_index / 2) + 1) |payload_idx| {
...
test "twentynine windows preserve centered equations around the middle rejected-payload alias" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const center_payload = expectedRejectedPayload(even_residue + center_index);
        ...
        for (0..center_index) |idx| {
            ...
            try testing.expectEqual(
                ...
                expectedCode(even_residue + center_index) * 2,
            );
            try testing.expectEqual(
                @as(u128, rawFromResidue(left_residue)) + @as(u128, rawFromResidue(right_residue)),
                @as(u128, rawFromResidue(even_residue + center_index)) * 2,
            );
        }
