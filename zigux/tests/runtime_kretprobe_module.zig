...
test "runtime kretprobe sample enforces lifecycle transitions and return-probe bookkeeping" {
    ...
    try std.testing.expectEqual(@as(usize, 37), exit_report.last_retval);
    try std.testing.expectEqual(@as(i64, 45), exit_report.last_duration_ns);
...
test "runtime kretprobe sample keeps selftest and outstanding-instance paths explicit" {
    ...
    const recovered_exit_report = try outstanding.exit();
    try std.testing.expectEqualStrings(sample.RuntimeKretprobeSample.default_symbol_name, recovered_exit_report.symbol_name);
    try std.testing.expectEqual(@as(usize, 0), recovered_exit_report.skipped_kernel_threads);
    try std.testing.expectEqual(@as(usize, 0), recovered_exit_report.missed_instances);
    try std.testing.expectEqual(@as(usize, 9), recovered_exit_report.last_retval);
    try std.testing.expectEqual(@as(i64, 60), recovered_exit_report.last_duration_ns);
    try std.testing.expectEqual(@as(usize, 0), recovered_exit_report.selftest_runs);
}

test "runtime kretprobe sample keeps selftest-complete summary explicit across clean exit" {
    var module = sample.RuntimeKretprobeSample{};
    try module.init();
    ...
    try std.testing.expectEqual(@as(usize, 4), selftest_summary.probe_focus.len);
    try std.testing.expectEqual(@as(usize, 1), module.selftest_runs);
    try std.testing.expectEqual(@as(usize, 0), module.exit_runs);
    try std.testing.expectEqualStrings(sample.RuntimeKretprobeSample.default_symbol_name, before_exit.symbol_name);
    ...
    try std.testing.expectEqual(@as(usize, 1), before_exit.skipped_kernel_threads);
    try std.testing.expectEqual(@as(usize, 1), before_exit.nmissed);
    try std.testing.expectEqual(@as(usize, 42), before_exit.last_retval);
    try std.testing.expectEqual(@as(i64, 75), before_exit.last_duration_ns);
    ...
    try std.testing.expectEqual(@as(usize, 1), module.selftest_runs);
    try std.testing.expectEqual(@as(usize, 1), module.exit_runs);
    try std.testing.expectEqualStrings(before_exit.symbol_name, after_exit.symbol_name);
    try std.testing.expectEqual(before_exit.maxactive, after_exit.maxactive);
    try std.testing.expectEqual(before_exit.active_instances, after_exit.active_instances);
    try std.testing.expectEqual(before_exit.skipped_kernel_threads, after_exit.skipped_kernel_threads);
    try std.testing.expectEqual(before_exit.nmissed, after_exit.nmissed);
...
test "runtime kretprobe sample keeps overlapping entry timestamps stable through drain and exit" {
    var module = sample.RuntimeKretprobeSample{};
    try module.init();
    ...
    try std.testing.expectEqual(@as(usize, 0), after_inner.selftest_runs);
    ...
    try std.testing.expectEqual(@as(usize, 0), drained.selftest_runs);
