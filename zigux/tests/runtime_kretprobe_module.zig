...
test "runtime kretprobe sample keeps selftest and outstanding-instance paths explicit" {
    ...
    const before_failed_exit = outstanding.summary();
    ...
    const after_failed_exit = outstanding.summary();
    try std.testing.expectEqual(sample.ModuleStage.initialized, outstanding.stage());
