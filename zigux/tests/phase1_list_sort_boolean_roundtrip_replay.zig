...
test "phase1 list_sort replay keeps boolean descending order when a later pass ties everything" {
    const boolean_cmp = struct {
        fn compare(priv: ?*anyopaque, a: *const list_sort.ListHead, b: *const list_sort.ListHead) i32 {
            const mode: *const SortMode = @ptrCast(@alignCast(priv.?));
            const lhs: *const Entry = @fieldParentPtr("node", a);
    ...
    const count = try collectSorted(&head, &keys, &ordinals);
    try std.testing.expectEqual(@as(usize, entries.len), count);
    try std.testing.expectEqualSlices(i32, &.{ 5, 5, 4, 2, 2, 1, 1 }, keys[0..count]);
    try std.testing.expectEqualSlices(usize, &.{ 1, 4, 5, 0, 2, 3, 6 }, ordinals[0..count]);
    try std.testing.expect(head.next == &entries[1].node);
