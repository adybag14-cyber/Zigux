...
test "triple window masked peer keeps bounded counts and missing slots explicit" {
    const capacity = (bitmap_view.word_bits * 2) + 12;
    const words = [_]usize{
        bit(0) | bit(1) | bit(4) | bit(7),
        bit(2) | bit(5) | bit(8),
        bit(0) | bit(2) | bit(5) | bit(7) | bit(10) | bit(11),
    };
    ...
    try testing.expect(!bitmap.isSet((bitmap_view.word_bits * 2) + 9));
    try testing.expectEqual(@as(usize, 13), bitmap.countSetBits());
...
test "triple window masked peer ignores out-of-range tail bits while preserving subset and overlap checks" {
    const capacity = (bitmap_view.word_bits * 2) + 12;
    ...
    const masked_subset_words = [_]usize{
        bit(1) | bit(7),
        bit(5),
        bit(2) | bit(7) | bit(11) | bit(13) | bit(15),
    };
    ...
    try testing.expectEqual(@as(usize, 6), masked_subset_bitmap.countSetBits());
    ...
    try testing.expect(masked_subset.hasCpu((bitmap_view.word_bits * 2) + 11));
    try testing.expect(!masked_subset.hasCpu((bitmap_view.word_bits * 2) + 10));

    try testing.expectEqual(@as(usize, 3), gap_probe_bitmap.countSetBits());
    try testing.expectEqual(gap_probe_bitmap.countSetBits(), gap_probe.countPresentCpus());
    try testing.expectEqual(@as(?usize, 2), gap_probe_bitmap.firstSetBit());
    ...
    try testing.expect(gap_probe.hasCpu(bitmap_view.word_bits + 1));
    try testing.expect(gap_probe.hasCpu((bitmap_view.word_bits * 2) + 9));
    try testing.expect(!gap_probe.hasCpu((bitmap_view.word_bits * 2) + 10));
}
