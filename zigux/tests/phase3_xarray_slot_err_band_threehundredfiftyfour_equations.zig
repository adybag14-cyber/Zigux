...
const errno: isize = -354;
const next_errno: isize = -355;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 354 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 354 raw equation stays between its errno neighbors" {
...
test "errno 354 adjacent raw lanes keep value and pointer decoders precise" {
