...
const errno: isize = -408;
const next_errno: isize = -409;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 408 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 408 raw equation stays between its errno neighbors" {
...
test "errno 408 adjacent raw lanes keep value and pointer decoders precise" {
