...
pub fn build(b: *std.Build) void {
    ...
    const replay_root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_b_quote_suffix_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-b-quote-suffix-replay-tests",
        .root_module = replay_root_module,
    });
    const run_replay_tests = b.addRunArtifact(replay_tests);

    const replay_step = b.step(
        "phase1-helper-ports-b-quote-suffix-replay",
        "Run Phase 1 helper ports B quote/suffix replay",
    );
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run Phase 1 helper ports B quote/suffix replay");
