...
test "one-hundred-twenty-eighth anchors decode to the expected err band milestones" {
...
test "one-hundred-twenty-eighth anchors keep the err band's symmetric 31 then 32 cadence" {
    ...
    for (0..anchor_count - 1) |index| {
        ...
        const expected_gap: usize = if (index == 0 or index == anchor_count - 2) 31 else 32;
    ...
    try testing.expectEqual(@as(usize, 63), raws[2] - raws[0]);
    try testing.expectEqual(@as(usize, 127), raws[4] - raws[0]);
    try testing.expectEqual(@as(usize, 255), raws[8] - raws[0]);
    try testing.expectEqual(@as(usize, 511), raws[16] - raws[0]);
    try testing.expectEqual(@as(usize, 1023), raws[32] - raws[0]);
    try testing.expectEqual(@as(usize, 2047), raws[64] - raws[0]);
    try testing.expectEqual(@as(usize, 2047), raws[128] - raws[64]);
}

test "interior one-hundred-twenty-eighth anchors stay even err raws bracketed by rejected aliases" {
