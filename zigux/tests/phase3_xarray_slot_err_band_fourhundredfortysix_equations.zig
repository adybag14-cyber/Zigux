...
const errno: isize = -446;
const next_errno: isize = -447;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 446 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 446 raw equation stays between its errno neighbors" {
...
test "errno 446 adjacent raw lanes keep value and pointer decoders precise" {
