...
test "partial tail head gap stays the first missing cpu after full leading words" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    const tail_base = bitmap_view.word_bits * 2;
    const words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        bit(1) | bit(2) | bit(4) | bit(8) | bit(13),
    };
...
test "partial tail head gap keeps subset and overlap blind to invalid extras" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
