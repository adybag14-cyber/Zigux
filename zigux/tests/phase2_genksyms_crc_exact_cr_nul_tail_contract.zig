...
fn partialCrc32One(c: u8, crc: u32) u32 {
    return crctab32[(crc ^ c) & 0xff] ^ (crc >> 8);
}

fn partialCrc32(s: []const u8, crc: u32) u32 {
    var result = crc;
    for (s) |c| result = partialCrc32One(c, result);
    return result;
}

fn crc32(s: []const u8) u32 {
...
fn appendExactCrTerminatedChunk(list: *std.ArrayList(u8), allocator: std.mem.Allocator) !void {
    try list.appendNTimes(allocator, 'a', c_line_payload_len - 2);
    try list.append(allocator, '\r');
    try list.append(allocator, '\r');
}

test "exact payload trims trailing CR before skipping NUL-hidden continuation" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 8);
    defer input.deinit(std.testing.allocator);
    try appendExactCrTerminatedChunk(&input, std.testing.allocator);
    try input.append(std.testing.allocator, 0);
    try input.appendSlice(std.testing.allocator, "bc\nx\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc(input.items, &capture);

    const trimmed_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(input.items[0 .. c_line_payload_len - 2])});
    defer std.testing.allocator.free(trimmed_crc);
    const untrimmed_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(input.items[0..c_line_payload_len])});
    defer std.testing.allocator.free(untrimmed_crc);
    const hidden_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("bc")});
...
test "exact payload plus NUL-hidden EOF continuation emits only the trimmed chunk" {
