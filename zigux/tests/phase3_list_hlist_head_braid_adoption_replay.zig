...
test "detached head braid stays off the visible list route until the head adopts it" {
    ...
    braid_second.next = @intFromPtr(&third);
...
test "list head braid adoption fails closed at the stale rejoin backlink" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var second = list_view.ListHead{ .next = 0, .prev = 0 };
    var third = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    const view = list_view.ListView.init(&head);
    try testing.expectEqual(@as(?*const list_view.ListHead, &braid_first), view.first());
    try testing.expectEqual(@as(?*const list_view.ListHead, &fourth), view.last());
    try testing.expectEqual(@as(usize, 4), view.len());
...
test "detached head braid stays off the visible hlist route until the head adopts it" {
...
test "hlist head braid adoption fails closed at the stale rejoin prev-link" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var second = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var third = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    const view = hlist_view.HListView.init(&head);
    try testing.expectEqual(@as(?*const hlist_view.HListNode, &braid_first), view.first());
    try testing.expectEqual(@as(usize, 4), view.len());
    try testing.expect(view.firstPprevMatchesHead());
    try testing.expect(view.tailNextIsNull());
