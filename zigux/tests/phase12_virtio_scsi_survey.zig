...
const RoadmapGapStatus = struct { required_by_roadmap: bool, status: []const u8, current_surface: []const u8, blocked_by: []const u8 };
const RoadmapGapCheck = struct { dma_safe_abstractions: RoadmapGapStatus, queueing_correctness: RoadmapGapStatus, throughput_and_recovery_parity: RoadmapGapStatus, segmented_rollout: RoadmapGapStatus };
const Gap = struct { id: []const u8, status: []const u8, kind: []const u8, zigux_destination: []const u8, why_now: []const u8 };
const Manifest = struct { lane_key: []const u8, phase: []const u8, surveyed_commit: []const u8, verified_on: []const u8, anchor: []const u8, roadmap_destinations: []const []const u8, survey_summary: SurveySummary, roadmap_gap_check: RoadmapGapCheck, gaps: []const Gap };
const FixtureManifest = struct { lane_key: []const u8, phase: []const u8, surveyed_commit: []const u8, verified_on: []const u8, anchor: []const u8, fixture_kind: []const u8, source_manifest: []const u8, scope: []const u8, required_paths: []const []const u8, expected_absent_paths: []const []const u8, notes: []const []const u8 };
...
test "phase12 virtio scsi fixture manifest keeps rollback-only presence and absence explicit" {
    ...
    try std.testing.expectEqual(@as(usize, 10), fixture.required_paths.len);
    try std.testing.expectEqualStrings("zigux/tests/phase12_virtio_scsi_survey_build.zig", fixture.required_paths[6]);
    try std.testing.expect(fixture.expected_absent_paths.len == 5);
    try std.testing.expect(std.mem.indexOf(u8, fixture.scope, "dedicated survey-build route") != null);
...
test "phase12 virtio scsi fallback catalog keeps archival replay distinct from current-master rollback evidence" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, fallback_catalog, "exact coverage evidence refreshed on `2026-05-23`") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, fallback_catalog, "public raw `master` fallback still serves the older `2026-05-21` archival body for this same path as of `2026-05-23`") != null);
