...
test "phase12 virtio scsi fallback catalog keeps archival replay distinct from current-master rollback evidence" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, fallback_catalog, "authenticated contents and public raw `master` now converge again on this same archival catalog body for the fallback path") != null);
