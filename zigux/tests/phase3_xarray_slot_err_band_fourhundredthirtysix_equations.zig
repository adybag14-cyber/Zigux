...
const errno: isize = -436;
const next_errno: isize = -437;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 436 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 436 raw equation stays between its errno neighbors" {
...
test "errno 436 adjacent raw lanes keep value and pointer decoders precise" {
