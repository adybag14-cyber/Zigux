...
pub fn build(b: *std.Build) void {
    ...
    const packet_step = b.step(
        "phase9-runtime-trace-events-packet-tests",
        "Run the focused Phase 9 runtime trace-events sample-family lifecycle packet.",
    );
    packet_step.dependOn(&run_runtime_trace_events_sample_tests.step);
    packet_step.dependOn(&run_runtime_trace_events_module_tests.step);
    packet_step.dependOn(&run_runtime_trace_events_unregistered_gate_tests.step);
    packet_step.dependOn(&run_runtime_trace_events_exit_rollback_guard_tests.step);
    packet_step.dependOn(&run_runtime_trace_events_registration_reentry_gate_tests.step);
    packet_step.dependOn(&run_runtime_trace_events_reinit_rollback_guard_tests.step);
    packet_step.dependOn(&run_runtime_trace_events_reinit_reexit_guard_tests.step);
