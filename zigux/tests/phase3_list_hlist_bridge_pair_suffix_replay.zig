...
test "list bridge-pair suffix stays off-route until the visible route adopts it" {
    ...
    suffix_left.prev = @intFromPtr(&tail);
    ...
    bridge_right.next = @intFromPtr(&suffix_left);
    ...
    try expectListSequence(
        broken,
        &.{ &entry, &bridge_left, &bridge_right, &suffix_left, &suffix_right, &tail },
    );
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&tail)), breakage.actual_prev);
...
test "hlist bridge-pair suffix stays off-route until the visible route adopts it" {
    ...
    suffix_left.next = @intFromPtr(&suffix_right);
    suffix_left.pprev = @intFromPtr(&tail.next);
    suffix_right.next = @intFromPtr(&tail);
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&tail.next)), breakage.actual_pprev);
