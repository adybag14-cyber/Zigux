...
test "phase14 shared smoke manifest records the bounded study-only packet" {
    ...
    try std.testing.expectEqualStrings("zigux/tests/phase14_ring_buffer_manifest.json", manifest.surfaces[10].path);
    try std.testing.expectEqualStrings("zigux/tests/phase14_rcu_tree_manifest.json", manifest.surfaces[11].path);
