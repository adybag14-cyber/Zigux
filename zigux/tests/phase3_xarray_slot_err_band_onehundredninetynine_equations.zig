...
test "onehundredninetynine raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 34, 964, 2002, 2982, 3896 };
...
test "onehundredninetynine constructor windows keep consecutive codes and reject aliased values" {
...
test "onehundredninetynine keeps one centered raw and split payload center" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
