...
const errno: isize = -477;
const next_errno: isize = -478;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 477 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 477 raw equation stays between its errno neighbors" {
...
test "errno 477 adjacent raw lanes keep value and pointer decoders precise" {
