...
test "list view keeps the live bridge visible over a detached bridge handoff" {
...
test "list view reports the adopted bridge handoff once the reused tail keeps the stale backlink" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var entry = list_view.ListHead{ .next = 0, .prev = 0 };
    var bridge = list_view.ListHead{ .next = 0, .prev = 0 };
    var tail = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    entry.next = @intFromPtr(&bridge);
    entry.prev = @intFromPtr(&head);
    bridge.next = @intFromPtr(&handoff);
    bridge.prev = @intFromPtr(&entry);
    tail.next = @intFromPtr(&head);
    tail.prev = @intFromPtr(&bridge);

    handoff.next = @intFromPtr(&tail);
    handoff.prev = @intFromPtr(&bridge);

    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &entry, &bridge, &handoff, &tail });
...
test "hlist view keeps the live bridge visible over a detached bridge handoff" {
...
test "hlist view reports the adopted bridge handoff once the reused tail keeps the stale prev-link" {
    var head = hlist_view.HListHead{ .first = 0 };
    var entry = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var bridge = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var tail = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    handoff.next = @intFromPtr(&tail);
    handoff.pprev = @intFromPtr(&bridge.next);

    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &entry, &bridge, &handoff, &tail });
