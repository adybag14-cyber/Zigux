...
test "representative 116-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 5, 935, 1977, 2979, 3979 };
...
test "every interior onehundredsixteen window keeps consecutive codes and alternating constructor families" {
...
test "onehundredsixteen windows preserve a dual-center raw seam and a centered payload lane" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
        const seam_code_sum =
