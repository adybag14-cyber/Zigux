...
pub fn build(b: *std.Build) void {
    ...
    const ready_buffer_window_verify_module = b.createModule(.{
        .root_source_file = b.path("../../tools/lib/bpf/zigux_segments/ready_buffer_window_verify.zig"),
        .target = target,
        .optimize = optimize,
    });
    const ready_buffer_window_verify_tests = b.addTest(.{
        .name = "phase8-ready-buffer-window-verify-tests",
        .root_module = ready_buffer_window_verify_module,
    });
    ...
    const run_perf_buffer_ready_window_tests = b.addRunArtifact(perf_buffer_ready_window_tests);
    const run_ready_buffer_window_verify_tests = b.addRunArtifact(ready_buffer_window_verify_tests);
