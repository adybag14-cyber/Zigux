...
test "phase3 list/hlist tail braid replay keeps the live list route visible while a detached tail braid stays off the closure path" {
    ...
    stale_left.prev = @intFromPtr(&tail);
    stale_right.next = @intFromPtr(&head);
...
test "phase3 list/hlist tail braid replay reports the final list closure break when the sentinel tail witness adopts the detached braid first" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_right = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    head.next = @intFromPtr(&first);
    head.prev = @intFromPtr(&stale_right);
    first.next = @intFromPtr(&live_left);
    first.prev = @intFromPtr(&head);
    live_left.next = @intFromPtr(&live_right);
    live_left.prev = @intFromPtr(&first);
...
test "phase3 list/hlist tail braid replay keeps the live hlist route visible while a detached tail braid stays off the closure path" {
    ...
    stale_left.next = @intFromPtr(&stale_right);
    stale_left.pprev = @intFromPtr(&tail.next);
    stale_right.next = 0;
...
test "phase3 list/hlist tail braid replay reports the tail hlist witness when the closing prev-link adopts the detached braid first" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_right = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    live_right.pprev = @intFromPtr(&live_left.next);
    tail.next = 0;
    tail.pprev = @intFromPtr(&stale_right.next);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&live_right.next)), breakage.expected_pprev);
