...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = b.step(
        "phase3-abi-export-pair-test",
        "Run the shared Phase 3 ABI core packet plus export/UAPI layout replay through a standalone paired build shard",
    );
    pair_step.dependOn(&phase3_abi_core_packet.step);
    pair_step.dependOn(&phase3_export_uapi_layout.step);
}
