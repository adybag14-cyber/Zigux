...
test "phase2 genksyms crc boundary newline is consumed without spillover" {
...
test "phase2 genksyms crc NUL before boundary newline truncates only that record" {
    ...
    try input.append(std.testing.allocator, 'z');
    try input.append(std.testing.allocator, '\n');
    ...
    try std.testing.expectEqual(@as(usize, c_line_payload_len - 3), first.len);
    try std.testing.expectEqualStrings("x", second);
    try std.testing.expectEqual(@as(?[]const u8, null), nextCHarnessLineChunk(input.items, &cursor));
    try std.testing.expectEqual(crc32(input.items[0 .. c_line_payload_len - 3]), crc32(first));
    try std.testing.expectEqual(@as(u32, 0x8cdc_1683), crc32(second));
}
