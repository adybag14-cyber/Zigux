...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_b_case_digit_width_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-b-case-digit-width-replay-tests",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase1-helper-ports-b-case-digit-width-replay",
        "Run the Phase 1 helper ports B case-digit-width replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 1 helper ports B case-digit-width replay");
