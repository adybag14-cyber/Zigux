...
test "phase1 hardening gate packet remains wired through closure validator and workflow" {
    ...
    const bench_selftest_marker = "python3 scripts/zigux/check-phase1-bench.py --self-test";
    const bench_check_marker = "python3 scripts/zigux/check-phase1-bench.py";
    if (optionalIndex(workflow.contents, bench_selftest_marker)) |bench_selftest| {
        const search_start = bench_selftest + bench_selftest_marker.len;
        const bench_check_after_selftest = optionalIndex(workflow.contents[search_start..], bench_check_marker);
        try std.testing.expect(bench_check_after_selftest != null);
