...
const staged_break_index = 39;
const route = [_]usize{
    58, 0,  33, 32, 57, 1,  34, 31, 56, 2,  35, 30, 55, 3,
    36, 29, 54, 4,  37, 28, 53, 5,  38, 27, 52, 6,  39, 26,
    51, 7,  40, 25, 50, 8,  41, 24, 49, 9,  42, 23, 48, 10,
    43, 22, 47, 11, 44, 21, 46, 12, 45, 20, 19, 13, 18, 17,
    16, 15, 14,
};
...
test "enneakaipentaconta weave-clasp list route exposes staged backlink repair" {
...
test "enneakaipentaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
