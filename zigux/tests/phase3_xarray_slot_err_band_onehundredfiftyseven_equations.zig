...
test "representative 157-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2950, 3938 };
        ...
        for (0..((window_len + 1) / 2)) |pair_idx| {
            ...
            if (even_idx < center_payload_index) {
                ...
                try testing.expectEqual(
                    ...
                    expectedRejectedPayload(left_residue + center_payload_index) * 2,
                );
            }
        }
    }
...
test "every interior onehundredfiftyseven window keeps consecutive codes and alternating constructor families" {
...
test "onehundredfiftyseven windows preserve a centered raw seam and a centered payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
