...
const center_left_payload_index = center_right_index - 1;
...
test "representative 122-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 4, 934, 1976, 2972, 3972 };
...
test "onehundredtwentytwo windows preserve a split raw seam and adjacent payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
