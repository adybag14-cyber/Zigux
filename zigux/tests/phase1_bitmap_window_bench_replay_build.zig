...
pub fn build(b: *std.Build) void {
    ...
    const replay_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("phase1_bitmap_window_bench_replay.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    const replay_step = b.step("phase1-bitmap-window-bench-replay", "Run the Phase 1 bitmap-window bench replay");
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run the Phase 1 bitmap-window bench replay");
