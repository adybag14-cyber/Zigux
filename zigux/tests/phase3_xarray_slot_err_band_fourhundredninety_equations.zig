...
const errno: isize = -490;
const next_errno: isize = -491;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 490 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 490 raw equation stays between its errno neighbors" {
...
test "errno 490 adjacent raw lanes keep value and pointer decoders precise" {
