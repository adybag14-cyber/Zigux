...
test "lane19 visible control EOF replay preserves leading low control bytes while trimming trailing carriage returns" {
    var split_then_visible_controls_and_trailing_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 8);
    ...
    try gen.runGenksymsCrc(split_then_visible_controls_and_trailing_cr.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_controls_and_trailing_cr.items[0..c_line_payload_len])});
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_controls_and_trailing_cr.items)});
