...
pub fn build(b: *std.Build) void {
    ...
    const replay_mod = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_self_loop_endpoint_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step("phase3-list-hlist-self-loop-endpoint-replay", "Run the Phase 3 list/hlist self-loop endpoint replay");
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 3 list/hlist self-loop endpoint replay");
