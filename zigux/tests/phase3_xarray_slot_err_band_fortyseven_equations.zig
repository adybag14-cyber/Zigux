...
test "representative 47-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1002, 2038, 3048, 4048 };
...
test "every interior fortyseven window keeps consecutive codes and alternating constructor families" {
...
test "fortyseven windows preserve centered equations around the middle err-code slot" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_payload_sum =
