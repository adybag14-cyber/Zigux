...
test "duoquadragintuple trailing empty banks keep early windows aligned and masked tail bits bounded" {
    const capacity = (44 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        bit(0) | bit(5) | bit(9) | bit(12),
        bit(2) | bit(8) | bit(11) | bit(15),
    } ++ [_]usize{0} ** 42 ++ [_]usize{
    ...
    try testing.expect(bitmap.isSet((44 * bitmap_view.word_bits) + 1));
    try testing.expect(bitmap.isSet((44 * bitmap_view.word_bits) + 10));
    try testing.expect(!bitmap.isSet((44 * bitmap_view.word_bits) + 9));
    ...
    try testing.expect(!cpumask.hasCpu((43 * bitmap_view.word_bits) + 7));
    try testing.expect(cpumask.hasCpu((44 * bitmap_view.word_bits) + 6));
}

test "duoquadragintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (44 * bitmap_view.word_bits) + 11;
    ...
    const tail_only_words = [_]usize{0} ** 44 ++ [_]usize{
