...
test "representative 27-raw err-band windows keep alias-err spacing exact" {
...
test "every interior twentyseven window keeps consecutive codes and alternating constructor families" {
...
test "twentyseven windows preserve centered equations around the middle err-code slot" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_payload_sum =
            expectedRejectedPayload(even_residue + center_index - 1) +
            expectedRejectedPayload(even_residue + center_index + 1);
