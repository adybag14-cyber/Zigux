...
test "representative 72-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 978, 2020, 3022, 4022 };
...
test "every interior seventytwo window keeps consecutive codes and alternating constructor families" {
...
test "seventytwo windows preserve dual-center seam equations and a paired payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
