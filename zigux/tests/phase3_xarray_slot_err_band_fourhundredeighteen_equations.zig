...
const errno: isize = -418;
const next_errno: isize = -419;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 418 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 418 raw equation stays between its errno neighbors" {
...
test "errno 418 adjacent raw lanes keep value and pointer decoders precise" {
