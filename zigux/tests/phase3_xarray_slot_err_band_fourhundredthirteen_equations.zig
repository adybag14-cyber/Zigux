...
const errno: isize = -413;
const next_errno: isize = -414;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 413 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 413 raw equation stays between its errno neighbors" {
...
test "errno 413 adjacent raw lanes keep value and pointer decoders precise" {
