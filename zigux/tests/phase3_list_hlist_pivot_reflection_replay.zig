...
test "list view follows a live pivot route instead of a detached reflected window" {
    ...
    var reflected_pivot = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    reflected_right.next = @intFromPtr(&reflected_pivot);
    reflected_right.prev = @intFromPtr(&first);
    reflected_pivot.next = @intFromPtr(&reflected_left);
    reflected_pivot.prev = @intFromPtr(&reflected_right);
    reflected_left.next = @intFromPtr(&tail);
    reflected_left.prev = @intFromPtr(&reflected_pivot);
...
test "list view reports a reflected pivot window once the visible route adopts it" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var left = list_view.ListHead{ .next = 0, .prev = 0 };
    var pivot = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &first, &reflected_right, &reflected_pivot, &reflected_left, &tail });
...
test "hlist view follows a live pivot route instead of a detached reflected window" {
    ...
    var reflected_pivot = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    reflected_pivot.next = @intFromPtr(&reflected_left);
    reflected_pivot.pprev = @intFromPtr(&reflected_right.next);
    reflected_left.next = @intFromPtr(&tail);
    reflected_left.pprev = @intFromPtr(&reflected_pivot.next);
...
test "hlist view reports a reflected pivot window once the visible route adopts it" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var pivot = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &first, &reflected_right, &reflected_pivot, &reflected_left, &tail });
