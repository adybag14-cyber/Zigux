...
const errno: isize = -475;
const next_errno: isize = -476;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 475 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 475 raw equation stays between its errno neighbors" {
...
test "errno 475 adjacent raw lanes keep value and pointer decoders precise" {
