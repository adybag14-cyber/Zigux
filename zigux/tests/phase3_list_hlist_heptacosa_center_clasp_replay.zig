...
const route = [_]usize{ 26, 0, 24, 2, 22, 4, 20, 6, 18, 8, 16, 10, 14, 12, 13, 15, 11, 17, 9, 19, 7, 21, 5, 23, 3, 25, 1 };
const detached_index = 27;

fn resetEntries(entries: *[28]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[28]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[28]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[28]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[28]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[28]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[28]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[28]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "heptacosa center clasp keeps forward list and hlist visibility before repair" {
    var entries: [28]Entry = undefined;
...
test "heptacosa center clasp staged repair crosses the middle hinge" {
    var entries: [28]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 21);
    repairHListPrefix(&hlist_head, &entries, 21);
    ...
    try std.testing.expectEqual(@as(usize, 21), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[20]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 21), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[20]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
