...
test "list view keeps the live tail visible over a detached tail rejoin candidate" {
...
test "list view reports the adopted tail rejoin once the reused tail keeps the stale backlink" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var entry = list_view.ListHead{ .next = 0, .prev = 0 };
    var middle = list_view.ListHead{ .next = 0, .prev = 0 };
    var tail = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    entry.next = @intFromPtr(&middle);
    entry.prev = @intFromPtr(&head);
    middle.next = @intFromPtr(&rejoin);
    middle.prev = @intFromPtr(&entry);
    tail.next = @intFromPtr(&head);
    tail.prev = @intFromPtr(&middle);

    rejoin.next = @intFromPtr(&tail);
    rejoin.prev = @intFromPtr(&middle);

    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &entry, &middle, &rejoin, &tail });
...
test "hlist view keeps the live tail visible over a detached tail rejoin candidate" {
...
test "hlist view reports the adopted tail rejoin once the reused tail keeps the stale prev-link" {
    var head = hlist_view.HListHead{ .first = 0 };
    var entry = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var middle = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var tail = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    rejoin.next = @intFromPtr(&tail);
    rejoin.pprev = @intFromPtr(&middle.next);

    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &entry, &middle, &rejoin, &tail });
