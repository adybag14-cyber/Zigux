...
test "trailing empty bank keeps earlier windows aligned and ignores extra bank noise" {
    const capacity = (2 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        bit(1) | bit(5) | bit(9) | bit(12),
        bit(0) | bit(3) | bit(8) | bit(10) | bit(15),
        0,
    ...
    try testing.expect(!bitmap.isSet((2 * bitmap_view.word_bits) + 10));
...
test "trailing empty bank keeps subset and overlap bounded under tail-masked noise" {
    const capacity = (2 * bitmap_view.word_bits) + 11;
    const base_words = [_]usize{
        bit(1) | bit(5) | bit(9) | bit(12),
        bit(0) | bit(3) | bit(8) | bit(10),
        0,
        std.math.maxInt(usize),
    };
