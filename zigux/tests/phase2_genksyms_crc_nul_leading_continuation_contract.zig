...
test "phase2 genksyms crc skips NUL-led continuation after exact payload" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 16);
    defer input.deinit(std.testing.allocator);
    try input.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try input.append(std.testing.allocator, 0);
    try input.appendSlice(std.testing.allocator, "shadow\nq\n");
    ...
    try std.testing.expectEqual(@as(usize, 1), records.items[1].len);
    try std.testing.expectEqual(crc32("q"), records.items[1].crc);
    try std.testing.expect(!containsCrc(records.items, crc32("shadow")));
    try std.testing.expect(!containsCrc(records.items, crc32(input.items[0 .. c_line_payload_len + "shadow".len + 1])));
}

test "phase2 genksyms crc treats CR after leading continuation NUL as invisible" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 20);
    defer input.deinit(std.testing.allocator);
    try input.appendNTimes(std.testing.allocator, 'b', c_line_payload_len);
    try input.append(std.testing.allocator, 0);
    try input.appendSlice(std.testing.allocator, "\rhidden\r\nkeep\r\n");
    ...
    try std.testing.expectEqual(@as(usize, 2), records.items.len);
    try std.testing.expectEqual(@as(usize, c_line_payload_len), records.items[0].len);
    try std.testing.expectEqual(@as(u8, 'b'), records.items[0].first);
    try std.testing.expectEqual(@as(u8, 'b'), records.items[0].last);
    try std.testing.expectEqual(crc32(input.items[0..c_line_payload_len]), records.items[0].crc);
    try std.testing.expectEqual(@as(usize, 4), records.items[1].len);
    try std.testing.expectEqual(crc32("keep"), records.items[1].crc);
    try std.testing.expect(!containsCrc(records.items, crc32("hidden")));
    try std.testing.expect(!containsCrc(records.items, crc32("\rhidden")));
}
