...
const errno: isize = -392;
const next_errno: isize = -393;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 392 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 392 raw equation stays between its errno neighbors" {
...
test "errno 392 adjacent raw lanes keep value and pointer decoders precise" {
