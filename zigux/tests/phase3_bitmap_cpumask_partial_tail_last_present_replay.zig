...
test "partial tail last-present range keeps only the final valid tail cpu visible" {
...
test "partial tail last-present peers stay equivalent despite invalid extras" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    const last_valid = capacity - 1;
