...
fn replayWords() [tail_word_index + 1]usize {
    ...
    words[0] = (@as(usize, 1) << 1) | (@as(usize, 1) << 9);
...
test "lane27 novemquadragintuple replay keeps bitmap discovery stable across 49 empty banks" {
    ...
    try testing.expectEqual(@as(?usize, 9), view.nextSetBit(2));
    try testing.expectEqual(@as(?usize, tail_base), view.nextSetBit(10));
    ...
    try testing.expectEqual(@as(?usize, 10), view.nextClearBit(10));
    try testing.expectEqual(@as(?usize, tail_base + 1), view.nextClearBit(tail_base + 1));
}

test "lane27 novemquadragintuple replay keeps cpumask traversal aligned with bitmap traversal" {
    const words = replayWords();
    ...
    try testing.expectEqual(bitmap.nextSetBit(10), mask.nextCpu(10));
...
test "lane27 novemquadragintuple replay ignores tail-only noise in subset and overlap checks" {
    const base_words = replayWords();
    var superset_words = replayWords();
    var tail_noise_only_words = [_]usize{0} ** (tail_word_index + 1);
    superset_words[0] |= (@as(usize, 1) << 17);
