...
test "representative 139-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2958, 3956 };
...
test "every interior onehundredthirtynine window keeps consecutive codes and alternating constructor families" {
...
test "onehundredthirtynine windows preserve centered equations around the middle err slot and split rejected-payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + center_left_payload_index - 2) + 1,
            expectedRejectedPayload(even_residue + center_left_payload_index),
        );
        try testing.expectEqual(
