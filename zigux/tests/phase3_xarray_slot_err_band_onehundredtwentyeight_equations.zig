...
test "representative 128-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 8, 934, 1968, 2964, 3964 };
...
test "every interior onehundredtwentyeight window keeps consecutive codes and alternating constructor families" {
...
test "onehundredtwentyeight windows preserve a split raw seam and adjacent payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
