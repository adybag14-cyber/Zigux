...
test "double middle empty banks keep separated outer windows aligned" {
    const capacity = (4 * bitmap_view.word_bits) + 10;
    const words = [_]usize{
        bit(1) | bit(4) | bit(7),
        0,
        0,
        bit(0) | bit(5) | bit(9) | bit(14),
        bit(2) | bit(6) | bit(11) | bit(15),
    };
    ...
    try testing.expect(bitmap.isSet((3 * bitmap_view.word_bits) + 14));
    ...
    try testing.expect(cpumask.hasCpu((3 * bitmap_view.word_bits) + 5));
    try testing.expect(!cpumask.hasCpu((2 * bitmap_view.word_bits) + 12));
    try testing.expect(!cpumask.hasCpu((4 * bitmap_view.word_bits) + 9));
}

test "double middle empty banks keep subset and overlap checks bounded under masked tail noise" {
    const capacity = (4 * bitmap_view.word_bits) + 10;
