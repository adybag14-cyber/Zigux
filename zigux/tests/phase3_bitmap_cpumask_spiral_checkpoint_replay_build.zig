...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_spiral_checkpoint_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-bitmap-cpumask-spiral-checkpoint-replay",
        .root_module = root_module,
    });
    const run = b.addRunArtifact(tests);

    const test_step = b.step(
        "phase3-bitmap-cpumask-spiral-checkpoint-replay",
        "Run the Lane 27 spiral-checkpoint bitmap/cpumask replay",
    );
    test_step.dependOn(&run.step);

    const alias_step = b.step("test", "Run the Lane 27 spiral-checkpoint bitmap/cpumask replay");
