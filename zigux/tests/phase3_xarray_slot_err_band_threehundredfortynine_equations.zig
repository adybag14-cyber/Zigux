...
const errno: isize = -349;
const next_errno: isize = -350;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 349 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 349 raw equation stays between its errno neighbors" {
...
test "errno 349 adjacent raw lanes keep value and pointer decoders precise" {
