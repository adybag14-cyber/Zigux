...
const route = [_]usize{
    43, 0,  21, 22, 1,  42, 2,  41, 20, 23, 3,  40, 19, 24, 4,
    39, 18, 25, 5,  38, 17, 26, 6,  37, 16, 27, 7,  36, 15, 28,
    8,  35, 14, 29, 9,  34, 13, 30, 10, 33, 12, 31, 11, 32,
};
...
test "tetratetraconta braid-lock exposes the same visible route through list and hlist" {
...
test "tetratetraconta braid-lock repairs staged reverse links after the braid lock" {
    var entries = [_]Entry{.{}} ** (visible_count + 1);
    var head = ListHead{ .next = 0, .prev = 0 };
    var hhead = HListHead{ .first = 0 };

    reset(&entries, &head, &hhead);
    const lock_position = 27;
