...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("lane17_phase1_closure_interop_boundary_contract.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "lane17-phase1-closure-interop-boundary-contract",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const contract_step = b.step(
        "lane17-phase1-closure-interop-boundary-contract",
        "Validate Lane 17 Phase 1 closure-to-interop workflow boundary",
    );
    contract_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run Lane 17 Phase 1 closure-to-interop workflow boundary contract");
