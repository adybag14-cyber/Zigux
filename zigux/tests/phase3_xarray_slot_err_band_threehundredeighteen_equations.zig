...
const errno: isize = -318;
const next_errno: isize = -319;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 318 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 318 raw equation stays between its errno neighbors" {
...
test "errno 318 adjacent raw lanes keep value and pointer decoders precise" {
