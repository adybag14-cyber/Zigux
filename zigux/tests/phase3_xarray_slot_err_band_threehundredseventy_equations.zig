...
const errno: isize = -370;
const next_errno: isize = -371;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 370 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 370 raw equation stays between its errno neighbors" {
...
test "errno 370 adjacent raw lanes keep value and pointer decoders precise" {
