...
const errno: isize = -487;
const next_errno: isize = -488;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 487 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 487 raw equation stays between its errno neighbors" {
...
test "errno 487 adjacent raw lanes keep value and pointer decoders precise" {
