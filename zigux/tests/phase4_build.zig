...
pub fn build(b: *std.Build) void {
  ...
  const atomic64_diff_step = b.step(
  "phase4-runtime-atomic64-diff",
  "Run the Phase 4 runtime atomic64 diff tests",
  );

  atomic64_diff_step.dependOn(&run_atomic64_diff_tests.step);

  const runtime_atomic64_diff_survey_step = b.step(
  "phase4-runtime-atomic64-diff-survey",
  "Run the Phase 4 runtime atomic64 diff survey tests",
  );
  ...
  "phase4-bitmap-diff",
  "Run the Phase 4 bitmap diff tests",
  );
  ...
  "Run the Phase 4 bitmap live-helper replay tests",
  );
