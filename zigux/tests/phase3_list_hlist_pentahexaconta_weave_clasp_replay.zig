...
const staged_break_index = 45;
const route = [_]usize{
    64, 0,  39, 38, 63, 1,  40, 37, 62, 2,  41, 36, 61, 3,
    42, 35, 60, 4,  43, 34, 59, 5,  44, 33, 58, 6,  45, 32,
    57, 7,  46, 31, 56, 8,  47, 30, 55, 9,  48, 29, 54, 10,
    49, 28, 53, 11, 50, 27, 52, 12, 51, 26, 25, 13, 24, 23,
...
test "pentahexaconta weave-clasp list route exposes staged backlink repair" {
...
test "pentahexaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
