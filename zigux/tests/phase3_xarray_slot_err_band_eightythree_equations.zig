...
test "representative 83-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 965, 2007, 3011, 4011 };
...
test "every interior eightythree window keeps consecutive codes and alternating constructor families" {
...
test "eightythree windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
