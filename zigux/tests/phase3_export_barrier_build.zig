...
pub fn build(b: *std.Build) void {
    ...
    const phase3_barrier = addBarrierTest(b, target, optimize);

    const step = b.step(
        "phase3-export-barrier-test",
        "Run the shared Phase 3 export/UAPI layout and barrier helper packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_barrier.step);
}
