...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_henhexaconta_weave_clasp_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-list-hlist-henhexaconta-weave-clasp-replay",
        .root_module = root_module,
    });
    const run_unit_tests = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase3-list-hlist-henhexaconta-weave-clasp-replay",
        "Run the Lane 28 Phase 3 list/hlist henhexaconta weave-clasp replay",
    );
