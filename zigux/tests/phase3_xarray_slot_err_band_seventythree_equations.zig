...
test "representative 73-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 975, 2017, 3021, 4021 };
...
test "every interior seventythree window keeps consecutive codes and alternating constructor families" {
...
test "seventythree windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
