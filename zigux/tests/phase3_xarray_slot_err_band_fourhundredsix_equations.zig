...
const errno: isize = -406;
const next_errno: isize = -407;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 406 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 406 raw equation stays between its errno neighbors" {
...
test "errno 406 adjacent raw lanes keep value and pointer decoders precise" {
