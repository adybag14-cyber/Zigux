...
const errno: isize = -423;
const next_errno: isize = -424;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 423 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 423 raw equation stays between its errno neighbors" {
...
test "errno 423 adjacent raw lanes keep value and pointer decoders precise" {
