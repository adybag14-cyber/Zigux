...
test "phase12 virtio_net survey manifest stays aligned with the landed probe starter" {
    ...
    try std.testing.expectEqualStrings("6afa667980bb289b2d811660aa92ea8106fbc043", manifest.surveyed_commit);
    ...
    var saw_header_scatter_followup = false;
    ...
    for (manifest.gaps, 0..) |gap, i| {
        ...
        if (std.mem.eql(u8, gap.id, "phase12-virtio-net-header-scatter-followup")) {
            saw_header_scatter_followup = true;
            try std.testing.expectEqualStrings("drivers/net/virtio_net.zig", gap.zigux_destination);
            try std.testing.expectEqualStrings("starter_landed", gap.status);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "`any_header_sg`") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "`needed_headroom`") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "`VIRTIO_F_ANY_LAYOUT`") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "`VIRTIO_F_VERSION_1`") != null);
        }
    ...
    try std.testing.expect(saw_header_scatter_followup);
    ...
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "header-scatter follow-up") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "`needed_headroom`") != null);
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "`VIRTIO_F_ANY_LAYOUT`") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "surveyed `master` snapshot `6afa667980bb289b2d811660aa92ea8106fbc043`") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "queue-recovery, `hdr_len`, and header-scatter follow-ups") != null);
