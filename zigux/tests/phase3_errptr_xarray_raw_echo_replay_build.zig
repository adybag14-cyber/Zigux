...
pub fn build(b: *std.Build) void {
    ...
    const tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("phase3_errptr_xarray_raw_echo_replay.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    const test_step = b.step(
        "phase3-errptr-xarray-raw-echo-replay",
        "Run the Phase 3 err_ptr/xarray raw echo replay",
    );
    test_step.dependOn(&run_tests.step);

    const default_test_step = b.step("test", "Run the Phase 3 err_ptr/xarray raw echo replay");
