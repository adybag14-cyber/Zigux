...
pub fn build(b: *std.Build) void {
    ...
    const abi_packet = addPhase3AbiPacket(b, target, optimize);

    const phase3_step = b.step(
        "phase3-xarray-helper-abi-test",
        "Run the shared Phase 3 xarray helper cluster and ABI replay",
    );
    phase3_step.dependOn(&xarray_helper_packet.starter.step);
    phase3_step.dependOn(&xarray_helper_packet.slot.step);
    phase3_step.dependOn(&xarray_helper_packet.dump.step);
    phase3_step.dependOn(&abi_packet.step);
}
