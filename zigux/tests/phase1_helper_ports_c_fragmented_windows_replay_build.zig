...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_fragmented_windows_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase1-helper-ports-c-fragmented-windows-replay",
        "Run the Lane 10 Phase 1 helper ports C fragmented/windows replay",
    );
    replay_step.dependOn(&run_unit_tests.step);

    const test_step = b.step("test", "Run the Lane 10 Phase 1 helper ports C fragmented/windows replay");
