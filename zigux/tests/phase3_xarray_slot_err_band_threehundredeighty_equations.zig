...
const errno: isize = -380;
const next_errno: isize = -381;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 380 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 380 raw equation stays between its errno neighbors" {
...
test "errno 380 adjacent raw lanes keep value and pointer decoders precise" {
