...
test "representative 45-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1004, 2038, 3050, 4050 };
...
test "every interior fortyfive window keeps consecutive codes and alternating constructor families" {
...
test "fortyfive windows preserve centered equations and the centered rejected-payload law" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        try testing.expectEqual(
            ...
            (center_payload << 1) | xa_value.value_tag_mask,
        );
