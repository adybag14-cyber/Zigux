...
pub fn build(b: *std.Build) void {
    ...
    parity_module.addImport("runtime_atomic64_sample", runtime_atomic64_sample_module);
    parity_module.addImport("runtime_bitmap_sample", runtime_bitmap_sample_module);
    parity_module.addImport("runtime_loader", runtime_loader_module);

    const tests = b.addTest(.{
        .name = "phase9-first-loadable-loader-handoff-parity-tests",
        .root_module = parity_module,
    });

    const run = b.addRunArtifact(tests);
    const step = b.step(
        "phase9-first-loadable-loader-handoff-parity-tests",
        "Run the Phase 9 first-loadable loader handoff parity tests.",
    );
