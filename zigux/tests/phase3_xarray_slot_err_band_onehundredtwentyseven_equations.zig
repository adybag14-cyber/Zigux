...
test "representative 127-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 8, 934, 1970, 2966, 3966 };
...
test "every interior onehundredtwentyseven window keeps consecutive codes and alternating constructor families" {
...
test "onehundredtwentyseven windows preserve centered equations around the middle err slot and split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
