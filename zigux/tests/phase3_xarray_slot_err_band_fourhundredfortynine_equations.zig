...
const errno: isize = -449;
const next_errno: isize = -450;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 449 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 449 raw equation stays between its errno neighbors" {
...
test "errno 449 adjacent raw lanes keep value and pointer decoders precise" {
