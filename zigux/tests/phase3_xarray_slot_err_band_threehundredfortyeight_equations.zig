...
const errno: isize = -348;
const next_errno: isize = -349;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 348 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 348 raw equation stays between its errno neighbors" {
...
test "errno 348 adjacent raw lanes keep value and pointer decoders precise" {
