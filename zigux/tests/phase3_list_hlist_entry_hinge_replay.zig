...
test "phase3 list/hlist entry hinge replay keeps the live list entry hinge visible while an alternate hinge stays off path" {
    ...
    try expectListSequence(view, &.{ &live_hinge, &first, &second, &tail });
    try std.testing.expect(view.hasConsistentBacklinks());
    try std.testing.expect(view.firstBrokenBacklink() == null);
}

test "phase3 list/hlist entry hinge replay reports the first list entry mismatch when the alternate hinge is adopted too early" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_hinge = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var second = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &alt_hinge, &first, &second, &tail });
...
test "phase3 list/hlist entry hinge replay keeps the live hlist entry hinge visible while an alternate hinge stays off path" {
    ...
    first.next = @intFromPtr(&second);
    first.pprev = @intFromPtr(&live_hinge.next);
    second.next = @intFromPtr(&tail);
    ...
    try expectHListSequence(view, &.{ &live_hinge, &first, &second, &tail });
    try std.testing.expect(view.firstPprevMatchesHead());
    try std.testing.expect(view.hasConsistentPrevLinks());
    try std.testing.expect(view.tailNextIsNull());
}

test "phase3 list/hlist entry hinge replay reports the first hlist entry mismatch when the alternate hinge is adopted too early" {
    var head = hlist_view.HListHead{ .first = 0 };
    var live_hinge = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var second = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &alt_hinge, &first, &second, &tail });
