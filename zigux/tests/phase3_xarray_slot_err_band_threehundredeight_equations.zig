...
const errno: isize = -308;
const next_errno: isize = -309;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 308 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 308 raw equation stays between its errno neighbors" {
...
test "errno 308 adjacent raw lanes keep value and pointer decoders precise" {
