...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_pointer_constructor_rejections.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-xarray-slot-pointer-constructor-rejections-test",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const standalone_step = b.step(
        "phase3-xarray-slot-pointer-constructor-rejections",
        "Run the focused Phase 3 xarray slot pointer constructor rejection replay",
    );
    standalone_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the focused Phase 3 xarray slot pointer constructor rejection replay");
