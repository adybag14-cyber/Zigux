...
const route = [_]usize{ 31, 0, 15, 16, 30, 1, 14, 17, 29, 2, 13, 18, 28, 3, 12, 19, 27, 4, 11, 20, 26, 5, 10, 21, 25, 6, 9, 22, 24, 7, 8, 23 };
const detached_index = 32;
...
test "dotriaconta bridge clasp keeps forward list and hlist visibility before repair" {
...
test "dotriaconta bridge clasp staged repair crosses the bridge hinge" {
    var entries: [entry_count]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 26);
    repairHListPrefix(&hlist_head, &entries, 26);
    ...
    try std.testing.expectEqual(@as(usize, 26), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[25]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_bridge.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 26), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[25]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
