...
test "list parallel rejoin braid stays detached until the live rejoin route adopts it" {
    ...
    braid_right.next = @intFromPtr(&live_join);
    ...
    try expectListSequence(
        broken,
        &.{ &first, &live_left, &braid_left, &braid_mid, &braid_right, &live_join, &live_right, &tail },
    );
...
test "hlist parallel rejoin braid stays detached until the live rejoin route adopts it" {
