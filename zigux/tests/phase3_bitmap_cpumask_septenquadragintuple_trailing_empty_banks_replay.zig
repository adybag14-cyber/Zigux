...
test "septenquadragintuple trailing empty banks keep early windows aligned and masked tail bits bounded" {
    const capacity = (49 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        bit(1) | bit(6) | bit(10) | bit(13),
        bit(3) | bit(8) | bit(12),
    } ++ [_]usize{0} ** 47 ++ [_]usize{
    ...
    try testing.expect(bitmap.isSet((49 * bitmap_view.word_bits) + 0));
    try testing.expect(bitmap.isSet((49 * bitmap_view.word_bits) + 10));
    try testing.expect(!bitmap.isSet((49 * bitmap_view.word_bits) + 9));
    ...
    try testing.expect(!cpumask.hasCpu((48 * bitmap_view.word_bits) + 5));
    try testing.expect(cpumask.hasCpu((49 * bitmap_view.word_bits) + 7));
}

test "septenquadragintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (49 * bitmap_view.word_bits) + 11;
    ...
    const tail_only_words = [_]usize{0} ** 49 ++ [_]usize{
