...
fn crossFoldListForwardOnly(head: *ListHead, nodes: *[6]ListHead) void {
    const order = [_]usize{ 2, 5, 1, 4, 0, 3 };
...
fn crossFoldHListForwardOnly(head: *HListHead, nodes: *[6]HListNode) void {
    const order = [_]usize{ 2, 5, 1, 4, 0, 3 };
...
test "list view cross-fold route exposes forward order before backlink repair" {
    var head = ListHead{ .next = 0, .prev = 0 };
    var nodes = [_]ListHead{.{ .next = 0, .prev = 0 }} ** 6;
    seedCleanList(&head, &nodes);
    crossFoldListForwardOnly(&head, &nodes);

    const view = list_view.ListView.init(&head);
    const order = [_]*const ListHead{ &nodes[2], &nodes[5], &nodes[1], &nodes[4], &nodes[0], &nodes[3] };
    ...
    try expectListBreak(view, 1, ptr(&nodes[2]), ptr(&nodes[4]));
    nodes[5].prev = ptr(&nodes[2]);
    ...
    try expectListBreak(view, 4, ptr(&nodes[4]), ptr(&head));
    nodes[0].prev = ptr(&nodes[4]);
    try expectListBreak(view, 5, ptr(&nodes[0]), ptr(&nodes[2]));
...
test "hlist view cross-fold route exposes forward order before prev-link repair" {
    var head = HListHead{ .first = 0 };
    var nodes = [_]HListNode{.{ .next = 0, .pprev = 0 }} ** 6;
    seedCleanHList(&head, &nodes);
    crossFoldHListForwardOnly(&head, &nodes);

    const view = hlist_view.HListView.init(&head);
    const order = [_]*const HListNode{ &nodes[2], &nodes[5], &nodes[1], &nodes[4], &nodes[0], &nodes[3] };
    ...
    try std.testing.expect(view.firstPprevMatchesHead());
    try expectHListBreak(view, 1, ptr(&nodes[2].next), ptr(&nodes[4].next));
    nodes[5].pprev = ptr(&nodes[2].next);
    ...
    try expectHListBreak(view, 4, ptr(&nodes[4].next), ptr(&head.first));
    nodes[0].pprev = ptr(&nodes[4].next);
    try expectHListBreak(view, 5, ptr(&nodes[0].next), ptr(&nodes[2].next));
