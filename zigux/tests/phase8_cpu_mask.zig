...
test "phase 8 cpu mask starter slice keeps delimiter skipping bounded and rejects malformed ranges" {
    ...
    try std.testing.expectError(error.InvalidCpuRange, cpu_mask.parseCpuMaskString(std.testing.allocator, "0\r\n1"));
}
...
test "phase 8 cpu mask reader interface keeps carriage-return drift out of the landed helper" {
    const ReaderState = struct {
        chunks: []const []const u8,
        index: usize = 0,
