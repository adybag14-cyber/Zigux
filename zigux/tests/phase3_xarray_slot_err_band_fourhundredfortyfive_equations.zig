...
const errno: isize = -445;
const next_errno: isize = -446;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 445 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 445 raw equation stays between its errno neighbors" {
...
test "errno 445 adjacent raw lanes keep value and pointer decoders precise" {
