...
test "list midstream stagger keeps the live middle handoff visible until adoption" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_mid_a = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_mid_b = list_view.ListHead{ .next = 0, .prev = 0 };
    var alt_mid_a = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    alt_mid_a.next = @intFromPtr(&live_mid_b);
    alt_mid_a.prev = @intFromPtr(&first);
...
test "list midstream stagger reports the first stale middle backlink after adoption" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_mid_a = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_mid_b = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    head.next = @intFromPtr(&first);
    head.prev = @intFromPtr(&tail);
    first.next = @intFromPtr(&alt_mid_a);
    first.prev = @intFromPtr(&head);
    live_mid_a.next = @intFromPtr(&live_mid_b);
    live_mid_a.prev = @intFromPtr(&first);
    live_mid_b.next = @intFromPtr(&tail);
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&alt_mid_a)), breakage.expected_prev);
    try testing.expectEqual(@as(usize, @intFromPtr(&live_mid_a)), breakage.actual_prev);
    try testing.expect(!list_view.ListView.init(&head).hasConsistentBacklinks());
}

test "hlist midstream stagger keeps the live middle handoff visible until adoption" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_mid_a = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_mid_b = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var alt_mid_a = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    alt_mid_a.next = @intFromPtr(&live_mid_b);
    alt_mid_a.pprev = @intFromPtr(&first.next);
...
test "hlist midstream stagger reports the first stale middle prev link after adoption" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_mid_a = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_mid_b = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    try testing.expectEqual(@as(usize, 2), breakage.current_index);
    try testing.expectEqual(@as(usize, @intFromPtr(&alt_mid_a.next)), breakage.expected_pprev);
    try testing.expectEqual(@as(usize, @intFromPtr(&live_mid_a.next)), breakage.actual_pprev);
