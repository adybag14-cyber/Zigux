...
pub fn build(b: *std.Build) void {
    ...
    const test_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_err_band_fourhundredfifteen_equations.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named = b.step(
        "phase3-xarray-slot-err-band-fourhundredfifteen-equations",
        "Run the Lane 30 xarray slot errno 415 equation replay",
    );
    named.dependOn(&run.step);

    const test_step = b.step("test", "Run the Lane 30 xarray slot errno 415 equation replay");
