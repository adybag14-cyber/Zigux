...
test "triple leading empty banks keep delayed first cpu and late partial window aligned" {
    const capacity = (5 * bitmap_view.word_bits) + 10;
    const words = [_]usize{
        0,
        0,
        0,
    ...
    try testing.expect(bitmap.isSet((3 * bitmap_view.word_bits) + 1));
    try testing.expect(bitmap.isSet((4 * bitmap_view.word_bits) + 13));
    try testing.expect(bitmap.isSet((5 * bitmap_view.word_bits) + 9));
    try testing.expect(!bitmap.isSet((5 * bitmap_view.word_bits) + 8));
    try testing.expectEqual(@as(usize, 11), bitmap.countSetBits());
    try testing.expectEqual(bitmap.countSetBits(), cpumask.countPresentCpus());
    try testing.expectEqual(@as(?usize, (3 * bitmap_view.word_bits) + 1), bitmap.firstSetBit());
    try testing.expectEqual(bitmap.firstSetBit(), cpumask.firstCpu());
    try testing.expectEqual(@as(?usize, 0), bitmap.firstClearBit());
    try testing.expectEqual(bitmap.firstClearBit(), cpumask.firstMissingCpu());
    try testing.expect(cpumask.hasCpu((4 * bitmap_view.word_bits) + 8));
    try testing.expect(!cpumask.hasCpu((3 * bitmap_view.word_bits) + 2));
    try testing.expect(cpumask.hasCpu((5 * bitmap_view.word_bits) + 6));
}

test "triple leading empty banks keep subset and overlap checks bounded under masked tail noise" {
    const capacity = (5 * bitmap_view.word_bits) + 10;
