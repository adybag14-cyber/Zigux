...
test "phase2 genksyms wrapper replay keeps short option dash-prefixed payloads as data" {
    ...
    switch (outcome) {
        .command => |command| switch (command) {
            .request => |request| {
                ...
                try std.testing.expectEqualStrings("-d", request.reference_files[0]);
...
test "phase2 genksyms wrapper replay preserves version side effects before short option dash-prefixed payloads" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
...
test "phase2 genksyms wrapper replay keeps later flags live after short option dash-prefixed payloads" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    ...
    switch (outcome) {
        .command => |command| switch (command) {
            .request => |request| {
                ...
                try std.testing.expectEqual(@as(usize, 1), request.reference_files.len);
                try std.testing.expectEqualStrings("-d", request.reference_files[0]);
                try std.testing.expectEqualStrings("--quiet", request.dump_types_file.?);
                try std.testing.expectEqualSlices([]const u8, &args, request.rendered_args);
            },
            else => return error.ExpectedRequestCommand,
        },
