...
const errno: isize = -330;
const next_errno: isize = -331;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 330 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 330 raw equation stays between its errno neighbors" {
...
test "errno 330 adjacent raw lanes keep value and pointer decoders precise" {
