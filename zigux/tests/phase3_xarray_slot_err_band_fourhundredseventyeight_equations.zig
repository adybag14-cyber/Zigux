...
const errno: isize = -478;
const next_errno: isize = -479;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 478 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 478 raw equation stays between its errno neighbors" {
...
test "errno 478 adjacent raw lanes keep value and pointer decoders precise" {
