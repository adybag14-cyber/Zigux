...
test "phase3 list/hlist bridge pair alias replay keeps the live list bridge pair visible while a detached alias pair stays off path" {
    ...
    var alias_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var alias_right = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    alias_left.next = @intFromPtr(&alias_right);
    alias_left.prev = @intFromPtr(&left);
    alias_right.next = @intFromPtr(&bridge_right);
    alias_right.prev = @intFromPtr(&alias_left);
...
test "phase3 list/hlist bridge pair alias replay reports the reused right bridge list node when the alias pair takes the route early" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var left = list_view.ListHead{ .next = 0, .prev = 0 };
    var bridge_left = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    first.next = @intFromPtr(&left);
    first.prev = @intFromPtr(&head);
    left.next = @intFromPtr(&alias_left);
    left.prev = @intFromPtr(&first);
    bridge_left.next = @intFromPtr(&bridge_right);
    bridge_left.prev = @intFromPtr(&left);
    bridge_right.next = @intFromPtr(&tail);
    ...
    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &first, &left, &alias_left, &alias_right, &bridge_right, &tail });

    const breakage = view.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 4), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&alias_right)), breakage.expected_prev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&bridge_left)), breakage.actual_prev);
    try std.testing.expect(!view.hasConsistentBacklinks());
}

test "phase3 list/hlist bridge pair alias replay keeps the live hlist bridge pair visible while a detached alias pair stays off path" {
    ...
    var alias_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var alias_right = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    alias_left.next = @intFromPtr(&alias_right);
    alias_left.pprev = @intFromPtr(&left.next);
    alias_right.next = @intFromPtr(&bridge_right);
    alias_right.pprev = @intFromPtr(&alias_left.next);
...
test "phase3 list/hlist bridge pair alias replay reports the reused right bridge hlist node when the alias pair takes the route early" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var bridge_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &first, &left, &alias_left, &alias_right, &bridge_right, &tail });
    try std.testing.expect(view.firstPprevMatchesHead());

    const breakage = view.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 4), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&alias_right.next)), breakage.expected_pprev);
