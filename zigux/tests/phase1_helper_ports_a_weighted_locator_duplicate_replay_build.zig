...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_weighted_locator_duplicate_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-weighted-locator-duplicate-replay",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase1-helper-ports-a-weighted-locator-duplicate-replay",
        "Run the focused Phase 1 helper ports A weighted/locator/duplicate replay",
    );
