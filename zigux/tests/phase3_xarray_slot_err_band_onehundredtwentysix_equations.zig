...
const window_len = 126;
const center_left_index = (window_len / 2) - 1;
const center_right_index = window_len / 2;
const center_left_payload_index = center_left_index;
const center_right_payload_index = center_right_index + 1;
...
test "representative 126-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 8, 934, 1972, 2968, 3968 };
...
test "every interior onehundredtwentysix window keeps consecutive codes and alternating constructor families" {
...
test "onehundredtwentysix windows preserve a split raw seam and a split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
