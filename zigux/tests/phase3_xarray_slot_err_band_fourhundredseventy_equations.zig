...
const errno: isize = -470;
const next_errno: isize = -471;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 470 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 470 raw equation stays between its errno neighbors" {
...
test "errno 470 adjacent raw lanes keep value and pointer decoders precise" {
