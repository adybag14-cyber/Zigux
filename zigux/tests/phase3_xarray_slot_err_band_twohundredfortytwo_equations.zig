...
test "twohundredfortytwo raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 50, 982, 2014, 2994, 3853 };
...
test "twohundredfortytwo constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredfortytwo keeps split raw and payload centers adjacent" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
