...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_masked_last_sysfs_replace_cached_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-masked-last-sysfs-replace-cached-replay-tests",
        .root_module = replay_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase1-helper-ports-a-masked-last-sysfs-replace-cached-replay",
        "Run the Lane 06 masked-last/sysfs/replace-cached replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 masked-last/sysfs/replace-cached replay");
