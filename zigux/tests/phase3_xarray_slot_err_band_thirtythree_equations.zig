...
test "representative 33-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3058, 4062 };
    ...
    for (left_residues) |left_residue| {
        ...
        const centered_payload = expectedRejectedPayload(left_residue + center_index);
        for (0..center_index + 1) |pair_idx| {
            ...
            if (even_idx < center_index) {
                ...
                try testing.expectEqual(payload + mirrored_payload, centered_payload * 2);
            }
        }
    }
}

test "every interior thirtythree window keeps consecutive codes and alternating constructor families" {
...
test "thirtythree windows preserve centered raw and payload equations around the middle rejected slot" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const centered_code = expectedCode(even_residue + center_index);
        const centered_raw = rawFromResidue(even_residue + center_index);
        const centered_payload = expectedRejectedPayload(even_residue + center_index);
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue) + (center_index / 2),
            centered_payload,
        );
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + (window_len - 1)) - (center_index / 2),
            centered_payload,
        );
    }
}
