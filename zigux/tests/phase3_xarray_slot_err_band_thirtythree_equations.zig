...
test "representative 33-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1022, 2044, 3060, 4062 };
...
test "thirtythree windows preserve centered equations around the middle rejected-payload slot" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        try testing.expectEqual(
            center_payload + (center_index / 2),
            expectedRejectedPayload(even_residue + (window_len - 1)),
        );
        try testing.expectEqual(
            expectedCode(even_residue + center_index - 1) + 1,
            center_code,
        );
        try testing.expectEqual(
            center_code + 1,
            expectedCode(even_residue + center_index + 1),
        );
    }
}
