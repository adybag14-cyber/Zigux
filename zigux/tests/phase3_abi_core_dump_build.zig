...
pub fn build(b: *std.Build) void {
    ...
    const abi_core_packet = addPhase3AbiCorePacket(b, target, optimize);
    const abi_dump = addPhase3AbiDump(b, target, optimize);

    const named_step = b.step(
        "phase3-abi-core-dump",
        "Run the Phase 3 ABI core packet and ABI dump together",
    );
    named_step.dependOn(&abi_core_packet.step);
    named_step.dependOn(&abi_dump.step);
