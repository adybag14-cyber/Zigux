...
const errno: isize = -468;
const next_errno: isize = -469;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 468 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 468 raw equation stays between its errno neighbors" {
...
test "errno 468 adjacent raw lanes keep value and pointer decoders precise" {
