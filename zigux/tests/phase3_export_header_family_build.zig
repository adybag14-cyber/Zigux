...
pub fn build(b: *std.Build) void {
    ...
    const phase3_header_family = addHeaderFamilyTest(b, target, optimize);

    const step = b.step(
        "phase3-export-header-family-test",
        "Run the shared Phase 3 export/UAPI layout and header-family binding packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_header_family.step);
}
