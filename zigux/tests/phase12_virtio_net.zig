...
test "phase12 virtio net plans mergeable refill budgets from mtu and header state" {
    ...
    const refill = try lab.planMergeableReceiveRefill(4, 2_048);
    ...
    try std.testing.expectEqual(@as(u32, 2_260), refill.target_room_bytes);
    try std.testing.expectEqual(@as(u16, 20), refill.required_headroom_bytes);
    try std.testing.expectEqual(@as(u32, 2_048), refill.recycled_room_bytes);
    try std.testing.expectEqual(
        virtio_net.ReceiveRefillRoomPath.fresh_mergeable_room,
        refill.room_path,
    );
    try std.testing.expect(!refill.reuses_recycled_room);
    try std.testing.expectEqual(virtio_net.BigPacketReason.none, refill.big_packet_reason);
}

test "phase12 virtio net reuses recycled room when refill headroom already fits" {
    var lab = try virtio_net.VirtioNetProbeLab.init(&.{
        virtio_net.feature_mergeable_rx_buffers,
        virtio_net.feature_hash_report,
        virtio_net.feature_version_1,
    ...
    const refill = try lab.planMergeableReceiveRefill(4, 4_096);
    try std.testing.expectEqual(@as(u32, 2_260), refill.target_room_bytes);
    try std.testing.expectEqual(@as(u32, 4_096), refill.recycled_room_bytes);
    try std.testing.expectEqual(
        virtio_net.ReceiveRefillRoomPath.recycled_room,
        refill.room_path,
    );
    try std.testing.expect(refill.reuses_recycled_room);
}
...
test "phase12 virtio net rejects refill planning without queue entries" {
    ...
    try std.testing.expectError(error.InvalidRxQueueEntries, lab.planMergeableReceiveRefill(0, 0));
}
