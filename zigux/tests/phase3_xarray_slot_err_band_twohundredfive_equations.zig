...
const center_left_payload_index = center_index - 2;
const center_right_payload_index = center_index;
...
test "twohundredfive raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 46, 976, 2014, 2994, 3890 };
...
test "twohundredfive keeps one centered raw and split payload neighbors" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
