...
test "list view ignores a detached tail-slot claimant that still points at the live tail" {
    ...
    // This detached node claims the live tail's predecessor slot and points to
    // the same visible tail, but it never becomes reachable from head.next.
    detached_claim.next = @intFromPtr(&live_tail);
    detached_claim.prev = @intFromPtr(&live_second);
...
test "hlist view ignores a detached tail-slot claimant that still points at the live tail" {
    ...
    detached_claim.next = @intFromPtr(&live_tail);
    detached_claim.pprev = @intFromPtr(&live_second.next);
...
test "tail-slot claim replay keeps the live tail handoff authoritative across both helpers" {
    ...
    list_claim.prev = @intFromPtr(&list_second);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&list_second)), list_claim.prev);
    ...
    hlist_claim.pprev = @intFromPtr(&hlist_second.next);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hlist_second.next)), hlist_claim.pprev);
}
