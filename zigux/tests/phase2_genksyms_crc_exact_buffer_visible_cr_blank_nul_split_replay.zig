...
test "lane19 exact-buffer visible-CR blank NUL split replay skips a blank visible-leading-carriage-return continuation before an embedded NUL and the next line still lands" {
    var exact_then_blank_visible_cr_then_nul_then_line = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 6);
    defer exact_then_blank_visible_cr_then_nul_then_line.deinit(std.testing.allocator);
    try exact_then_blank_visible_cr_then_nul_then_line.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try exact_then_blank_visible_cr_then_nul_then_line.append(std.testing.allocator, '\r');
    try exact_then_blank_visible_cr_then_nul_then_line.append(std.testing.allocator, '\r');
    try exact_then_blank_visible_cr_then_nul_then_line.append(std.testing.allocator, 0);
    try exact_then_blank_visible_cr_then_nul_then_line.append(std.testing.allocator, 'c');
    try exact_then_blank_visible_cr_then_nul_then_line.append(std.testing.allocator, '\n');
    try exact_then_blank_visible_cr_then_nul_then_line.appendSlice(std.testing.allocator, "x\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try gen.runGenksymsCrc(exact_then_blank_visible_cr_then_nul_then_line.items, &capture);

    const exact_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32(exact_then_blank_visible_cr_then_nul_then_line.items[0..c_line_payload_len])},
    );
    ...
    const untruncated_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("\r\r\x00c")});
