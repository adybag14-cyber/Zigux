...
pub fn build(b: *std.Build) void {
    ...
    const tests = b.addTest(.{
        .name = "phase3-list-hlist-bridge-pair-tail-claim-replay",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase3-list-hlist-bridge-pair-tail-claim-replay",
        "Run the focused Phase 3 list/hlist bridge-pair tail-claim replay",
    );
