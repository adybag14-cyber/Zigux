...
pub fn build(b: *std.Build) void {
    ...
    const replay_mod = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_stride_corridor_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-bitmap-cpumask-stride-corridor-replay",
        "Run the Lane 27 stride-corridor bitmap/cpumask replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 27 stride-corridor replay tests");
