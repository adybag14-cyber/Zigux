...
const route = [_]usize{ 27, 0, 25, 2, 23, 4, 21, 6, 19, 8, 17, 10, 15, 12, 14, 13, 11, 16, 9, 18, 7, 20, 5, 22, 3, 24, 1, 26 };
const detached_index = 28;

fn resetEntries(entries: *[29]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[29]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[29]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[29]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[29]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[29]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[29]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[29]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "octacosa center clasp keeps forward list and hlist visibility before repair" {
    var entries: [29]Entry = undefined;
...
test "octacosa center clasp staged repair crosses the middle hinge" {
    var entries: [29]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 22);
    repairHListPrefix(&hlist_head, &entries, 22);
    ...
    try std.testing.expectEqual(@as(usize, 22), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[21]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 22), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[21]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
