...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_baton_loop_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-c-baton-loop-replay-tests",
    ...
    const replay_step = b.step(
        "phase1-helper-ports-c-baton-loop-replay",
        "Run the Lane 10 Phase 1 helper ports C baton-loop replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 10 baton-loop replay tests");
