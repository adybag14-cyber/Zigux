...
const entry_count = 31;
const route = [_]usize{ 29, 0, 27, 2, 25, 4, 23, 6, 21, 8, 19, 10, 17, 12, 15, 14, 16, 13, 18, 11, 20, 9, 22, 7, 24, 5, 26, 3, 28, 1 };
const detached_index = 30;
...
test "triaconta center clasp keeps forward list and hlist visibility before repair" {
...
test "triaconta center clasp staged repair crosses the late hinge" {
    var entries: [entry_count]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 24);
    repairHListPrefix(&hlist_head, &entries, 24);
    ...
    try std.testing.expectEqual(@as(usize, 24), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[23]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 24), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[23]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
