...
test "phase3 list/hlist bridge shadow rejoin replay keeps the live list bridge visible while a shadow rejoin stays off path" {
...
test "phase3 list/hlist bridge shadow rejoin replay reports the first visible list break when the shadow bridge rejoins early" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var left = list_view.ListHead{ .next = 0, .prev = 0 };
    var bridge = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    first.next = @intFromPtr(&left);
    first.prev = @intFromPtr(&head);
    left.next = @intFromPtr(&shadow_bridge);
    left.prev = @intFromPtr(&first);
    shadow_bridge.next = @intFromPtr(&right);
    shadow_bridge.prev = @intFromPtr(&left);
    right.next = @intFromPtr(&tail);
    right.prev = @intFromPtr(&bridge);
    tail.next = @intFromPtr(&head);
    ...
    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &first, &left, &shadow_bridge, &right, &tail });
...
test "phase3 list/hlist bridge shadow rejoin replay keeps the live hlist bridge visible while a shadow rejoin stays off path" {
...
test "phase3 list/hlist bridge shadow rejoin replay reports the first visible hlist break when the shadow bridge rejoins early" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var bridge = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    left.pprev = @intFromPtr(&first.next);
    shadow_bridge.next = @intFromPtr(&right);
    shadow_bridge.pprev = @intFromPtr(&left.next);
    right.next = @intFromPtr(&tail);
    right.pprev = @intFromPtr(&bridge.next);
    tail.next = 0;
    tail.pprev = @intFromPtr(&right.next);
    ...
    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &first, &left, &shadow_bridge, &right, &tail });
