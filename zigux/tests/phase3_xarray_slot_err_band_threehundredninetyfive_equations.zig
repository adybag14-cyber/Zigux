...
const errno: isize = -395;
const next_errno: isize = -396;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 395 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 395 raw equation stays between its errno neighbors" {
...
test "errno 395 adjacent raw lanes keep value and pointer decoders precise" {
