...
test "sixty-fourth anchors decode to the expected err band milestones" {
...
test "sixty-fourth anchors keep the err band's symmetric 63 then 64 cadence" {
    ...
    for (0..anchor_count - 1) |index| {
        ...
        const expected_gap: usize = if (index == 0 or index == anchor_count - 2) 63 else 64;
    ...
    try testing.expectEqual(@as(usize, 127), raws[2] - raws[0]);
    try testing.expectEqual(@as(usize, 255), raws[4] - raws[0]);
    try testing.expectEqual(@as(usize, 511), raws[8] - raws[0]);
    try testing.expectEqual(@as(usize, 1023), raws[16] - raws[0]);
    try testing.expectEqual(@as(usize, 2047), raws[32] - raws[0]);
    try testing.expectEqual(@as(usize, 2047), raws[64] - raws[32]);
}

test "interior sixty-fourth anchors stay even err raws bracketed by rejected aliases" {
