...
const errno: isize = -452;
const next_errno: isize = -453;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 452 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 452 raw equation stays between its errno neighbors" {
...
test "errno 452 adjacent raw lanes keep value and pointer decoders precise" {
