...
pub fn build(b: *std.Build) void {
    ...
    const combined_step = b.step(
        "phase3-abi-low-level-wrappers-test",
        "Run the Phase 3 ABI core packet beside the low-level wrapper packet",
    );
    combined_step.dependOn(&phase3_abi_core_packet.step);
    combined_step.dependOn(&phase3_low_level_wrappers.step);

    const test_step = b.step(
        "test",
        "Run the Phase 3 ABI core packet beside the low-level wrapper packet",
    );
    test_step.dependOn(&phase3_abi_core_packet.step);
    test_step.dependOn(&phase3_low_level_wrappers.step);
}
