...
test "representative err-band novemdecets keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3060, 4076 };
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [19]usize = undefined;
        var slots: [19]xarray_slot_view.SlotView = undefined;
        ...
        const sum_018 = @as(u128, raws[0]) + @as(u128, raws[18]);
        const sum_117 = @as(u128, raws[1]) + @as(u128, raws[17]);
        const sum_216 = @as(u128, raws[2]) + @as(u128, raws[16]);
        const sum_315 = @as(u128, raws[3]) + @as(u128, raws[15]);
        const sum_414 = @as(u128, raws[4]) + @as(u128, raws[14]);
        const sum_513 = @as(u128, raws[5]) + @as(u128, raws[13]);
        const sum_612 = @as(u128, raws[6]) + @as(u128, raws[12]);
        const sum_711 = @as(u128, raws[7]) + @as(u128, raws[11]);
        const sum_810 = @as(u128, raws[8]) + @as(u128, raws[10]);
        try testing.expectEqual(sum_018, sum_117);
        try testing.expectEqual(sum_117, sum_216);
        try testing.expectEqual(sum_216, sum_315);
        try testing.expectEqual(sum_315, sum_414);
        try testing.expectEqual(sum_414, sum_513);
        try testing.expectEqual(sum_513, sum_612);
        try testing.expectEqual(sum_612, sum_711);
        try testing.expectEqual(sum_711, sum_810);
        try testing.expectEqual(sum_810, @as(u128, raws[9]) * 2);
...
test "every interior novemdecet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 18 <= err_band_span) : (even_residue += 2) {
...
test "novemdecet windows preserve centered equations around the middle err-code slot" {
    var even_residue: usize = 0;
    while (even_residue + 18 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[18], codes[1] + codes[17]);
        try testing.expectEqual(codes[1] + codes[17], codes[2] + codes[16]);
        try testing.expectEqual(codes[2] + codes[16], codes[3] + codes[15]);
        try testing.expectEqual(codes[3] + codes[15], codes[4] + codes[14]);
        try testing.expectEqual(codes[4] + codes[14], codes[5] + codes[13]);
        try testing.expectEqual(codes[5] + codes[13], codes[6] + codes[12]);
        try testing.expectEqual(codes[6] + codes[12], codes[7] + codes[11]);
        try testing.expectEqual(codes[7] + codes[11], codes[8] + codes[10]);
        try testing.expectEqual(codes[8] + codes[10], codes[9] * 2);
        try testing.expectEqual(first_payload + nineteenth_payload, third_payload + seventeenth_payload);
        try testing.expectEqual(third_payload + seventeenth_payload, fifth_payload + fifteenth_payload);
        try testing.expectEqual(fifth_payload + fifteenth_payload, seventh_payload + thirteenth_payload);
        try testing.expectEqual(seventh_payload + thirteenth_payload, ninth_payload + eleventh_payload);
        ...
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 18)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 17)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 17)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 16)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 16)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 15)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 15)),
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 14)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 14)),
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 13)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 13)),
            @as(u128, rawFromResidue(even_residue + 6)) + @as(u128, rawFromResidue(even_residue + 12)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 6)) + @as(u128, rawFromResidue(even_residue + 12)),
            @as(u128, rawFromResidue(even_residue + 7)) + @as(u128, rawFromResidue(even_residue + 11)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 7)) + @as(u128, rawFromResidue(even_residue + 11)),
            @as(u128, rawFromResidue(even_residue + 8)) + @as(u128, rawFromResidue(even_residue + 10)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 8)) + @as(u128, rawFromResidue(even_residue + 10)),
            @as(u128, rawFromResidue(even_residue + 9)) * 2,
        );
    }
}
