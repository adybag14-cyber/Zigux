...
const errno: isize = -419;
const next_errno: isize = -420;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 419 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 419 raw equation stays between its errno neighbors" {
...
test "errno 419 adjacent raw lanes keep value and pointer decoders precise" {
