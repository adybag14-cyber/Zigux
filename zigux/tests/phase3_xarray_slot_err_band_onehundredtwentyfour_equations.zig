...
test "representative 124-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 6, 936, 1974, 2970, 3970 };
...
test "every interior onehundredtwentyfour window keeps consecutive codes and alternating constructor families" {
...
test "onehundredtwentyfour windows preserve a split raw seam and adjacent payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
