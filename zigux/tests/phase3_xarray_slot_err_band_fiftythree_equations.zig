...
test "representative 53-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 995, 2037, 3041, 4041 };
...
test "every interior fiftythree window keeps consecutive codes and alternating constructor families" {
...
test "fiftythree windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
        const seam_payload_sum =
            expectedRejectedPayload(odd_residue + center_index - 1) +
            expectedRejectedPayload(odd_residue + center_index + 1);
        ...
        for (0..center_index) |idx| {
            ...
            try testing.expectEqual(
                ...
                expectedCode(odd_residue + center_index) * 2,
            );
            try testing.expectEqual(
                @as(u128, rawFromResidue(left_residue)) + @as(u128, rawFromResidue(right_residue)),
                @as(u128, rawFromResidue(odd_residue + center_index)) * 2,
            );
        }
        ...
        try testing.expectEqual(
            expectedRejectedPayload(odd_residue + center_index - 1) + 1,
            expectedRejectedPayload(odd_residue + center_index + 1),
        );
    }
}
