...
const errno: isize = -432;
const next_errno: isize = -433;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 432 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 432 raw equation stays between its errno neighbors" {
...
test "errno 432 adjacent raw lanes keep value and pointer decoders precise" {
