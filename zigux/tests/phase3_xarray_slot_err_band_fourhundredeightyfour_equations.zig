...
const errno: isize = -484;
const next_errno: isize = -485;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 484 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 484 raw equation stays between its errno neighbors" {
...
test "errno 484 adjacent raw lanes keep value and pointer decoders precise" {
