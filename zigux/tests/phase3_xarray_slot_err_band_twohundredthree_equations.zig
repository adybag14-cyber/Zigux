...
const window_len = 203;
const center_index = window_len / 2;
const center_payload_index = center_index - (center_index & 1);
const center_left_payload_index = center_payload_index - 2;
const center_right_payload_index = center_payload_index + 2;
...
test "twohundredthree raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 42, 972, 2010, 2990, 3892 };
...
test "twohundredthree constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredthree keeps one centered raw and a three-payload center" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
