...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_byte_owner_latch_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const byte_owner_latch_tests = b.addTest(.{
        .name = "phase1-helper-ports-c-byte-owner-latch-replay-tests",
        .root_module = root_module,
    });
    const run_byte_owner_latch_tests = b.addRunArtifact(byte_owner_latch_tests);

    const replay_step = b.step(
        "phase1-helper-ports-c-byte-owner-latch-replay",
        "Run the Lane 10 byte-owner latch replay.",
    );
    replay_step.dependOn(&run_byte_owner_latch_tests.step);

    const test_step = b.step("test", "Run the Lane 10 byte-owner latch replay tests.");
    test_step.dependOn(&run_byte_owner_latch_tests.step);
