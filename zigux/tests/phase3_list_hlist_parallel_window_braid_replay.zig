...
test "list parallel window braid stays detached until the window route adopts it" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_window_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_window_right = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    live_left.next = @intFromPtr(&live_window_left);
    live_left.prev = @intFromPtr(&first);
    live_window_left.next = @intFromPtr(&live_window_right);
    live_window_left.prev = @intFromPtr(&live_left);
    live_window_right.next = @intFromPtr(&live_right);
    live_window_right.prev = @intFromPtr(&live_window_left);
    live_right.next = @intFromPtr(&tail);
    live_right.prev = @intFromPtr(&live_window_right);
    ...
    try expectListSequence(
        stable,
        &.{ &first, &live_left, &live_window_left, &live_window_right, &live_right, &tail },
    );
    ...
    try expectListSequence(
        broken,
        &.{ &first, &live_left, &braid_left, &braid_right, &live_right, &tail },
    );
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&live_window_right)), breakage.actual_prev);
    try testing.expect(!broken.hasConsistentBacklinks());
}

test "hlist parallel window braid stays detached until the window route adopts it" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_window_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_window_right = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    live_window_left.next = @intFromPtr(&live_window_right);
    live_window_left.pprev = @intFromPtr(&live_left.next);
    live_window_right.next = @intFromPtr(&live_right);
    live_window_right.pprev = @intFromPtr(&live_window_left.next);
    live_right.next = @intFromPtr(&tail);
    live_right.pprev = @intFromPtr(&live_window_right.next);
    tail.next = 0;
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&live_window_right.next)), breakage.actual_pprev);
