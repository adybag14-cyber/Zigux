...
test "list bridge-pair claim stays off-route until the visible bridge adopts it" {
    ...
    claim_left.prev = @intFromPtr(&bridge_right);
    claim_right.next = @intFromPtr(&tail);
    ...
    bridge_left.next = @intFromPtr(&claim_left);
    tail.prev = @intFromPtr(&claim_right);
    ...
    try expectListSequence(
        broken,
        &.{ &entry, &bridge_left, &claim_left, &claim_right, &tail },
    );

    const breakage = broken.firstBrokenBacklink().?;
    try testing.expectEqual(@as(usize, 2), breakage.current_index);
    try testing.expectEqual(@as(usize, @intFromPtr(&bridge_left)), breakage.expected_prev);
    try testing.expectEqual(@as(usize, @intFromPtr(&bridge_right)), breakage.actual_prev);
    try testing.expect(!broken.hasConsistentBacklinks());
}

test "hlist bridge-pair claim stays off-route until the visible bridge adopts it" {
    ...
    claim_left.next = @intFromPtr(&claim_right);
    claim_left.pprev = @intFromPtr(&bridge_right.next);
    claim_right.next = @intFromPtr(&tail);
    ...
    bridge_left.next = @intFromPtr(&claim_left);
    tail.pprev = @intFromPtr(&claim_right.next);
    ...
    try testing.expectEqual(@as(usize, 2), breakage.current_index);
    try testing.expectEqual(@as(usize, @intFromPtr(&bridge_left.next)), breakage.expected_pprev);
    try testing.expectEqual(@as(usize, @intFromPtr(&bridge_right.next)), breakage.actual_pprev);
