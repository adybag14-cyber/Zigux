...
const errno: isize = -339;
const next_errno: isize = -340;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 339 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 339 raw equation stays between its errno neighbors" {
...
test "errno 339 adjacent raw lanes keep value and pointer decoders precise" {
