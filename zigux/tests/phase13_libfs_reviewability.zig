...
const expected_surveyed_commit = "37aa7955301349eda9d3127ad6571ed481b5b37d";
...
test "phase13 libfs manifest records the landed close-bookkeeping slice and the blocked deeper cursor follow-up" {
    ...
    for (manifest.gaps, 0..) |gap, i| {
        ...
        if (std.mem.eql(u8, gap.id, "phase13-libfs-dcache-cursor-helpers")) {
            ...
            try std.testing.expectEqualStrings("fs/libfs.zig", gap.zigux_destination);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "sibling-list traversal") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "close-path release planner") != null);
        }
