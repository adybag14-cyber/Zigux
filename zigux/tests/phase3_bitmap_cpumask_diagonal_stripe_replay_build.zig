...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_diagonal_stripe_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-bitmap-cpumask-diagonal-stripe-replay-tests",
    ...
    const named_step = b.step(
        "phase3-bitmap-cpumask-diagonal-stripe-replay",
        "Run the Phase 3 bitmap/cpumask diagonal-stripe replay",
    );
    named_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 3 bitmap/cpumask diagonal-stripe replay");
