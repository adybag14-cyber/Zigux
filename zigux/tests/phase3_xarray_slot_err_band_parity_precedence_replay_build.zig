...
pub fn build(b: *std.Build) void {
    ...
    const replay_mod = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_err_band_parity_precedence_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-xarray-slot-err-band-parity-precedence-replay",
        "Run the Phase 3 xarray slot err-band parity precedence replay",
    );
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run Phase 3 xarray slot err-band parity precedence replay tests");
