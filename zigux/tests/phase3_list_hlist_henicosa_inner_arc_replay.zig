...
const route = [_]usize{ 10, 0, 20, 2, 18, 4, 16, 6, 14, 8, 12, 9, 13, 7, 15, 5, 17, 3, 19, 1, 11 };
const detached_index = 21;

fn resetEntries(entries: *[22]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[22]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[22]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[22]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[22]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[22]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[22]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[22]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "henicosa inner arc keeps forward list and hlist visibility before repair" {
    var entries: [22]Entry = undefined;
...
test "henicosa inner arc staged repair crosses the center bridge" {
    var entries: [22]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 15);
    repairHListPrefix(&hlist_head, &entries, 15);
    ...
    try std.testing.expectEqual(@as(usize, 15), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[14]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_arc.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 15), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[14]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
