...
pub fn build(b: *std.Build) void {
    ...
    const ida_range_starter_packet = addIdaRangeStarterPacket(b, target, optimize);
    const policy_starter_packet = addPhase3PolicyStarterPacket(b, target, optimize);
    const low_level_wrappers = addPhase3LowLevelWrappers(b, target, optimize);

    const combined_step = b.step(
        "phase3-ida-range-policy-low-level-test",
        "Run the Phase 3 IDA range, policy, and low-level wrapper packets together",
    );
    combined_step.dependOn(&ida_range_starter_packet.step);
    combined_step.dependOn(&policy_starter_packet.step);
    combined_step.dependOn(&low_level_wrappers.step);
