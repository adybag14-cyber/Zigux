...
test "representative 125-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 7, 937, 1975, 2969, 3969 };
...
test "every interior onehundredtwentyfive window keeps consecutive codes and alternating constructor families" {
...
test "onehundredtwentyfive windows preserve centered equations around the middle err slot and split payload seam" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
