...
test "representative 114-raw err-band windows keep alias-err spacing exact" {
...
test "every interior onehundredfourteen window keeps consecutive codes and alternating constructor families" {
...
test "onehundredfourteen windows preserve a dual-center raw seam and a centered payload lane" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
