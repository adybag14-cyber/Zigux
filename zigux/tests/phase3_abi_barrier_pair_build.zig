...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = b.step(
        ...
        "Run the Phase 3 ABI replay beside the barrier helper tests",
    );
    pair_step.dependOn(&run_abi_replay_tests.step);
    pair_step.dependOn(&run_barrier_tests.step);

    const test_step = b.step("test", "Run the Phase 3 ABI barrier pair tests");
