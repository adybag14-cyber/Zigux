...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_parallel_window_braid_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-list-hlist-parallel-window-braid-replay",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase3-list-hlist-parallel-window-braid-replay",
        "Run the focused Phase 3 list/hlist parallel window braid replay",
    );
