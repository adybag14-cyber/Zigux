...
test "bench command runs after fail closed expectation and source gates" {
    ...
    try expectContains(checker_source, "root = repo_root(args.repo_root)");
    try expectContains(checker_source, "expectations_file = expectations_path(root)");
    try expectContains(checker_source, "phase1_bench = bench_source_path(root)");
    try expectContains(checker_source, "kind, payload = load_runtime_expectations(expectations_file)");
    try expectContains(checker_source, "kind, payload = load_runtime_bench_source(phase1_bench)");
    try expectContains(checker_source, "zig = find_zig(root, args.zig)");
    try expectContains(checker_source, "[zig, \"build\", \"bench\", \"--build-file\", \"zigux/tests/phase1_bench_build.zig\", \"-Doptimize=ReleaseSafe\"],");
    try expectContains(checker_source, "cwd=str(root)");
    try expectContains(checker_source, "capture_output=True");
    try expectContains(checker_source, "text=True");
