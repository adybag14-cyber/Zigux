...
const window_len = 215;
const center_index = window_len / 2;
const center_payload_index = center_index + 1;
const next_payload_index = center_index + 3;
...
test "twohundredfifteen raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 56, 986, 2024, 3004, 3880 };
...
test "twohundredfifteen constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredfifteen keeps one centered raw and even payload stride" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        try testing.expectEqual(
            rejectedPayload(even_residue + center_payload_index) + 1,
            rejectedPayload(even_residue + next_payload_index),
        );
    }
}
