...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_checkpoint_ring_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-c-checkpoint-ring-replay",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase1-helper-ports-c-checkpoint-ring-replay",
        "Run Lane 10 Phase 1 helper ports C checkpoint ring replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run Lane 10 Phase 1 helper ports C checkpoint ring replay");
