...
test "representative 106-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 4, 944, 1986, 2988, 3988 };
...
test "every interior onehundredsix window keeps consecutive codes and alternating constructor families" {
...
test "onehundredsix windows preserve a dual-center raw seam and a centered payload lane" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
