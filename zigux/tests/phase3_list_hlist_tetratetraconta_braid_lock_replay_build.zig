...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_tetratetraconta_braid_lock_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-list-hlist-tetratetraconta-braid-lock-replay",
        "Run the Lane 28 Phase 3 list/hlist tetratetraconta braid-lock replay.",
    );
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step(
        "test",
        "Run the Lane 28 Phase 3 list/hlist tetratetraconta braid-lock replay tests.",
    );
