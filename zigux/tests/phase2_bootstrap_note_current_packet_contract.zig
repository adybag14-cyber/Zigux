...
test "phase2 bootstrap note keeps current direct packet explicit" {
    ...
    try expectContains(note, "limits archive digests to `x86_64-linux`");
    ...
    for (current_packet_markers) |marker| {
...
test "phase2 bootstrap note keeps workflow replay and follow-through boundary bounded" {
    ...
    try expectAbsent(note, "No current repo-reality gaps remain inside the bounded toolchain, installer, direct cross-route, local-first archive, returned archive-verification and staged-archive helper packet, or returned fixdep packet on current `master`。\n- Treat older validator-first-only Phase 2 names as current repo-reality gaps");
}
