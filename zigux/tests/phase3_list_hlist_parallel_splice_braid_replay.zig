...
test "list parallel splice braid stays detached until the live splice route adopts it" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_gap = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    live_left.next = @intFromPtr(&live_gap);
    live_left.prev = @intFromPtr(&first);
    live_gap.next = @intFromPtr(&live_right);
    live_gap.prev = @intFromPtr(&live_left);
    live_right.next = @intFromPtr(&tail);
    live_right.prev = @intFromPtr(&live_gap);
    ...
    braid_mid.prev = @intFromPtr(&braid_left);
    ...
    try expectListSequence(stable, &.{ &first, &live_left, &live_gap, &live_right, &tail });
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&live_gap)), breakage.actual_prev);
    try testing.expect(!broken.hasConsistentBacklinks());
}

test "hlist parallel splice braid stays detached until the live splice route adopts it" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_gap = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    live_gap.next = @intFromPtr(&live_right);
    live_gap.pprev = @intFromPtr(&live_left.next);
    live_right.next = @intFromPtr(&tail);
    live_right.pprev = @intFromPtr(&live_gap.next);
    tail.next = 0;
    ...
    braid_mid.next = @intFromPtr(&braid_right);
    braid_mid.pprev = @intFromPtr(&braid_left.next);
    braid_right.next = @intFromPtr(&live_right);
    ...
    try expectHListSequence(stable, &.{ &first, &live_left, &live_gap, &live_right, &tail });
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&live_gap.next)), breakage.actual_pprev);
