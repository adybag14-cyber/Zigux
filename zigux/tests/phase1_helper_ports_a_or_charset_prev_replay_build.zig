...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_or_charset_prev_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-or-charset-prev-replay-tests",
        .root_module = replay_module,
    });
    const run_replay_tests = b.addRunArtifact(replay_tests);

    const test_step = b.step("test", "Run the Lane 06 or/charset/prev replay");
    test_step.dependOn(&run_replay_tests.step);

    const replay_step = b.step(
        "phase1-helper-ports-a-or-charset-prev-replay",
        "Run the standalone Lane 06 or/charset/prev replay",
    );
