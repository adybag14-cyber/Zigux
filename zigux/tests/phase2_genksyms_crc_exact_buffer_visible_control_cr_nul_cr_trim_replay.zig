...
const c_line_payload_len = gen.c_line_payload_len;
...
test "lane19 exact-buffer split replay preserves visible low control bytes and leading carriage returns while trimming trailing carriage returns before an embedded NUL" {
    var exact_then_controls_and_leading_cr_then_trailing_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 15);
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"\\b\\f\\u0001\\r\\rb\\r") == null);
