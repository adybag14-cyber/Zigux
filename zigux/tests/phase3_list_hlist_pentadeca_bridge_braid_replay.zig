...
const route = [_]usize{ 14, 0, 12, 2, 10, 4, 8, 6, 7, 5, 9, 3, 11, 1, 13 };
const detached_index = 15;

fn resetEntries(entries: *[16]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[16]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[16]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[16]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[16]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[16]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[16]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[16]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "pentadeca bridge braid keeps forward list and hlist visibility before repair" {
...
test "pentadeca bridge braid staged repair passes the middle bridge" {
    var entries: [16]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 9);
    repairHListPrefix(&hlist_head, &entries, 9);

    const list_after_bridge = list_view.ListView.init(&list_head);
    const hlist_after_bridge = hlist_view.HListView.init(&hlist_head);
    try expectListRoute(list_after_bridge, &entries);
    try expectHListRoute(hlist_after_bridge, &entries);

    const list_break = list_after_bridge.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 9), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[8]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_bridge.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 9), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[8]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
