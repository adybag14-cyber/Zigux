...
const visible_count = 49;
const detached_index = visible_count;
const route = [_]usize{
    48, 0, 25, 23, 47, 1, 26, 22, 46, 2,  27, 21, 45, 3,  28, 20,
    44, 4, 29, 19, 43, 5, 30, 18, 42, 6,  31, 17, 41, 7,  32, 16,
    40, 8, 33, 15, 39, 9, 34, 14, 38, 10, 35, 13, 37, 11, 36, 12,
    24,
};
...
test "enneatetraconta mesh-clasp exposes the same visible route through list and hlist" {
...
test "enneatetraconta mesh-clasp repairs staged reverse links after the center clasp" {
    var entries = [_]Entry{.{}} ** (visible_count + 1);
    var head = ListHead{ .next = 0, .prev = 0 };
    var hhead = HListHead{ .first = 0 };

    reset(&entries, &head, &hhead);
    const gate_position = 38;
