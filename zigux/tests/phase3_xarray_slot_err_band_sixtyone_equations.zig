...
test "representative 61-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 987, 2029, 3033, 4033 };
...
test "every interior sixtyone window keeps consecutive codes and alternating constructor families" {
...
test "sixtyone windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
