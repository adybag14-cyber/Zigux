...
pub fn build(b: *std.Build) void {
    ...
    const starter_packet = addErrPtrXarrayStarterPacket(b, target, optimize);
    const xarray_slot_packet = addXarraySlotStarterPacket(b, target, optimize);
    const dump_packet = addErrPtrXarrayDump(b, target, optimize);

    const combo_step = b.step(
        "phase3-errptr-xarray-slot-dump-test",
        "Run the Phase 3 err_ptr/xarray starter, xarray-slot starter, and dump packets together",
    );
    combo_step.dependOn(&starter_packet.step);
    combo_step.dependOn(&xarray_slot_packet.step);
    combo_step.dependOn(&dump_packet.step);
}
