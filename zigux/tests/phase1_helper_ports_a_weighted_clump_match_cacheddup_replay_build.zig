...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_weighted_clump_match_cacheddup_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-weighted-clump-match-cacheddup-replay",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase1-helper-ports-a-weighted-clump-match-cacheddup-replay",
        "Run the Lane 06 weighted/clump/match/cacheddup replay",
    );
