...
test "phase2 genksyms wrapper replay preserves repeated short request flags and arguments" {
...
test "phase2 genksyms wrapper replay preserves quiet override in long-option request mode" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
...
test "phase2 genksyms wrapper replay renders the normalized request invocation plan" {
