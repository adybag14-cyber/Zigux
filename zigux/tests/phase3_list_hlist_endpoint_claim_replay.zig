...
test "list view ignores detached nodes that imitate both visible endpoints" {
    ...
    // This detached pair imitates both exposed endpoints without joining the
    // head-rooted route.
    detached_first.next = @intFromPtr(&detached_tail);
    detached_first.prev = @intFromPtr(&head);
    detached_tail.next = @intFromPtr(&head);
    detached_tail.prev = @intFromPtr(&detached_first);
...
test "hlist view ignores detached nodes that imitate both visible endpoints" {
    ...
    detached_first.next = @intFromPtr(&detached_tail);
    detached_first.pprev = @intFromPtr(&head.first);
    detached_tail.next = 0;
    detached_tail.pprev = @intFromPtr(&detached_first.next);
...
test "endpoint-claim replay keeps the live route authoritative across both helpers" {
    ...
    var list_detached_tail = ListHead{ .next = 0, .prev = 0 };
    ...
    list_detached_first.next = @intFromPtr(&list_detached_tail);
    list_detached_first.prev = @intFromPtr(&list_head);
    list_detached_tail.next = @intFromPtr(&list_head);
    list_detached_tail.prev = @intFromPtr(&list_detached_first);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&list_head)), list_detached_first.prev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&list_head)), list_detached_tail.next);
    ...
    var hlist_detached_tail = HListNode{ .next = 0, .pprev = 0 };
    ...
    hlist_detached_first.next = @intFromPtr(&hlist_detached_tail);
    hlist_detached_first.pprev = @intFromPtr(&hlist_head.first);
    hlist_detached_tail.next = 0;
    hlist_detached_tail.pprev = @intFromPtr(&hlist_detached_first.next);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hlist_head.first)), hlist_detached_first.pprev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hlist_detached_first.next)), hlist_detached_tail.pprev);
