...
fn addPhase3AbiExportShimPair(
    ...
    const abi_tests = b.addTest(.{
        .name = "phase3_abi_export_shim_pair_abi",
    ...
    const export_shim_tests = b.addTest(.{
        .name = "phase3_abi_export_shim_pair_export_shim",
        .root_module = export_shim,
    });

    const abi_run = b.addRunArtifact(abi_tests);
    const export_shim_run = b.addRunArtifact(export_shim_tests);
    const pair = b.step(
        "phase3-abi-export-shim-pair-test",
        "Run the Phase 3 ABI binding contract beside the kernel export shim packet",
    );
    pair.dependOn(&abi_run.step);
    pair.dependOn(&export_shim_run.step);
    return pair;
}
...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = addPhase3AbiExportShimPair(b, target, optimize);
    const test_step = b.step("test", "Run the Phase 3 ABI plus export shim pair-build shard");
