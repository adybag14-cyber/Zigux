...
const stage_break = 24;

const route = [_]usize{
    41, 0,  39, 2,  37, 4,  35, 6,  33, 8,  31, 10,
    29, 12, 27, 14, 25, 16, 23, 18, 21, 20, 19, 22,
    17, 24, 15, 26, 13, 28, 11, 30, 9,  32, 7,  34,
    5,  36, 3,  38, 1,  40,
};
...
test "dotetraconta outer lattice preserves forward list and hlist visibility before repair" {
    var nodes = initNodes();
...
test "dotetraconta outer lattice repairs list backlinks and hlist pprev past center clasp" {
    var nodes = initNodes();
    var list_head = ListHead{ .next = 0, .prev = 0 };
    var hlist_head = HListHead{ .first = 0 };
    installForwardOnly(&list_head, &hlist_head, &nodes);
