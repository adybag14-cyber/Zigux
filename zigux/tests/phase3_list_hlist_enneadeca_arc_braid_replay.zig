...
const route = [_]usize{ 18, 2, 16, 0, 14, 4, 12, 6, 10, 8, 9, 7, 11, 5, 13, 3, 15, 1, 17 };
const detached_index = 19;

fn resetEntries(entries: *[20]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[20]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[20]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[20]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[20]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[20]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[20]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[20]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "enneadeca arc braid keeps forward list and hlist visibility before repair" {
    var entries: [20]Entry = undefined;
...
test "enneadeca arc braid staged repair crosses the alternating arc" {
    var entries: [20]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 13);
    repairHListPrefix(&hlist_head, &entries, 13);
    ...
    try std.testing.expectEqual(@as(usize, 13), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[12]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_arc.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 13), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[12]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
