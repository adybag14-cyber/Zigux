...
const errno: isize = -390;
const next_errno: isize = -391;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 390 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 390 raw equation stays between its errno neighbors" {
...
test "errno 390 adjacent raw lanes keep value and pointer decoders precise" {
