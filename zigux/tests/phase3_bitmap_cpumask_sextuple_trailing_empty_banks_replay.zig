...
test "sextuple trailing empty banks keep early windows aligned and late banks empty" {
    const capacity = (7 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
    ...
    try testing.expect(!bitmap.isSet((6 * bitmap_view.word_bits) + 8));
    try testing.expect(!bitmap.isSet((7 * bitmap_view.word_bits) + 10));
    ...
    try testing.expect(!cpumask.hasCpu((7 * bitmap_view.word_bits) + 7));
}

test "sextuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (7 * bitmap_view.word_bits) + 11;
