...
test "representative 24-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3060, 4070 };
...
test "every interior twentyfour window keeps consecutive codes and alternating constructor families" {
...
test "twentyfour windows preserve centered equations across the middle seam pair" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
