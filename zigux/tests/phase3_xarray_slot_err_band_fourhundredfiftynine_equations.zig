...
const errno: isize = -459;
const next_errno: isize = -460;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 459 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 459 raw equation stays between its errno neighbors" {
...
test "errno 459 adjacent raw lanes keep value and pointer decoders precise" {
