...
const entry_count = 34;
const route = [_]usize{ 32, 0, 15, 16, 31, 1, 14, 17, 30, 2, 13, 18, 29, 3, 12, 19, 28, 4, 11, 20, 27, 5, 10, 21, 26, 6, 9, 22, 25, 7, 8, 23, 24 };
const detached_index = 33;
...
test "tritriaconta outer bridge keeps forward list and hlist visibility before repair" {
...
test "tritriaconta outer bridge staged repair crosses the outer hinge" {
    var entries: [entry_count]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 27);
    repairHListPrefix(&hlist_head, &entries, 27);
    ...
    try std.testing.expectEqual(@as(usize, 27), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[26]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_bridge.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 27), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[26]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
