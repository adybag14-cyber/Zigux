...
fn fixtureWords() [word_count]Word {
    var words: [word_count]Word = @splat(0);
    ...
    words[tail_word_index] =
        ...
        localBit(tail_base + tail_valid_bits + 6);
    return words;
}

fn supersetWords() [word_count]Word {
    var words = fixtureWords();
    words[tail_word_index] |= localBit(tail_base + 3);
    return words;
}
...
fn disjointWords() [word_count]Word {
    ...
    words[tail_word_index] =
        localBit(tail_base + 4) |
        localBit(tail_base + tail_valid_bits) |
        localBit(tail_base + tail_valid_bits + 10);
    return words;
}

test "bitmap view crosses trioogintuple empty banks before masked tail" {
...
test "cpumask view mirrors bitmap traversal across trioogintuple empty banks" {
    const words = fixtureWords();
    ...
    try std.testing.expect(!mask.hasCpu(tail_base + 3));
...
test "bitmap and cpumask masks ignore trioogintuple tail noise in relations" {
