...
pub fn build(b: *std.Build) void {
    ...
    const phase3_allocator_policy = addAllocatorPolicyTest(b, target, optimize);

    const step = b.step(
        "phase3-export-allocator-policy-test",
        "Run the shared Phase 3 export/UAPI layout and allocator-policy helper packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_allocator_policy.step);
}
