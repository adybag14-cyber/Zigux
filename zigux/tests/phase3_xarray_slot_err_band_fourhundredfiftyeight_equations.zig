...
const errno: isize = -458;
const next_errno: isize = -459;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 458 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 458 raw equation stays between its errno neighbors" {
...
test "errno 458 adjacent raw lanes keep value and pointer decoders precise" {
