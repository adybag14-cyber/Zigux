...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_weightedxor_zero_pad_erasecached_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-weightedxor-zero-pad-erasecached-replay",
        .root_module = root_module,
    });
    const run = b.addRunArtifact(tests);

    const step = b.step(
        "phase1-helper-ports-a-weightedxor-zero-pad-erasecached-replay",
        "Run the Lane 06 helper-ports-a weightedXor/zero/pad/eraseCached replay",
    );
