...
pub fn build(b: *std.Build) void {
    ...
    const ida_range = addIdaRangeStarterPacket(b, target, optimize);
    const export_uapi = addExportUapiLayout(b, target, optimize);

    const focused = b.step(
        "phase3-ida-range-export-test",
        "Run the Phase 3 IDA range starter packet beside the export/UAPI layout replay",
    );
    focused.dependOn(&ida_range.step);
    focused.dependOn(&export_uapi.step);

    const test_step = b.step("test", "Run the Lane 04 IDA range plus export/UAPI layout shard");
