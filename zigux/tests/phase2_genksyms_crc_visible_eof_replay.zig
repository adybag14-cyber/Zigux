...
test "lane19 visible EOF replay preserves leading carriage returns in a split continuation" {
    ...
    try gen.runGenksymsCrc(split_then_visible_leading_cr.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_leading_cr.items[0..c_line_payload_len])});
    defer std.testing.allocator.free(exact_crc);
    const leading_cr_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("\r\rb")});
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_leading_cr.items)});
