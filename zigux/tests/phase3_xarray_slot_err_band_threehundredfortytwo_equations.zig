...
const errno: isize = -342;
const next_errno: isize = -343;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 342 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 342 raw equation stays between its errno neighbors" {
...
test "errno 342 adjacent raw lanes keep value and pointer decoders precise" {
