...
pub fn build(b: *std.Build) void {
    ...
    const replay_mod = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_window_splice_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step("phase1-helper-ports-c-window-splice-replay", "Run the Lane 10 helper ports C window splice replay");
    replay_step.dependOn(&run_replay.step);

    const test_step = b.step("test", "Run the Lane 10 helper ports C window splice replay tests");
