...
test "detached latch bridge pair stays off the visible list route" {
...
test "list latch claim fails closed at the second bridge node" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var second = list_view.ListHead{ .next = 0, .prev = 0 };
    var bridge_a = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    bridge_a.prev = @intFromPtr(&second);
    bridge_b.next = @intFromPtr(&head);
    bridge_b.prev = @intFromPtr(&first);
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&bridge_a)), breakage.expected_prev);
    try testing.expectEqual(@as(usize, @intFromPtr(&first)), breakage.actual_prev);
    try testing.expect(!view.hasConsistentBacklinks());
}

test "detached latch bridge pair stays off the visible hlist route" {
...
test "hlist latch claim fails closed at the second bridge node" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var second = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var bridge_a = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    bridge_a.pprev = @intFromPtr(&second.next);
    bridge_b.next = 0;
    bridge_b.pprev = @intFromPtr(&first.next);
    ...
    try testing.expectEqual(@as(usize, 3), breakage.current_index);
    try testing.expectEqual(@as(usize, @intFromPtr(&bridge_a.next)), breakage.expected_pprev);
