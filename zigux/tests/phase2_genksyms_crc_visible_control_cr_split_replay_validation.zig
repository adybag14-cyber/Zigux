...
test "lane19 visible control split replay preserves low control bytes plus leading carriage returns before the next line" {
    var split_then_visible_controls_and_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 10);
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_controls_and_cr.items[0 .. c_line_payload_len + 6])});
