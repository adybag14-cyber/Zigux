...
const errno: isize = -344;
const next_errno: isize = -345;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 344 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 344 raw equation stays between its errno neighbors" {
...
test "errno 344 adjacent raw lanes keep value and pointer decoders precise" {
