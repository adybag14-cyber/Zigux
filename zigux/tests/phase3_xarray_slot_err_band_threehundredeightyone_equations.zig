...
const errno: isize = -381;
const next_errno: isize = -382;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 381 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 381 raw equation stays between its errno neighbors" {
...
test "errno 381 adjacent raw lanes keep value and pointer decoders precise" {
