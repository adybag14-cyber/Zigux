...
const staged_break_index = 36;
const route = [_]usize{
    55, 0,  30, 29, 54, 1,  31, 28, 53, 2,  32, 27, 52, 3,
    33, 26, 51, 4,  34, 25, 50, 5,  35, 24, 49, 6,  36, 23,
    48, 7,  37, 22, 47, 8,  38, 21, 46, 9,  39, 20, 45, 10,
    40, 19, 44, 11, 41, 18, 43, 12, 42, 17, 13, 16, 15, 14,
};
...
test "hexakaipentaconta weave-clasp list route exposes staged backlink repair" {
...
test "hexakaipentaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
