...
fn addReplayTest(
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_span_suffix_cursor_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-span-suffix-cursor-replay",
...
pub fn build(b: *std.Build) void {
    ...
    const step = b.step(
        "phase1-helper-ports-a-span-suffix-cursor-replay",
        "Run the Phase 1 helper ports A span suffix cursor replay",
    );
    step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 1 helper ports A span suffix cursor replay");
