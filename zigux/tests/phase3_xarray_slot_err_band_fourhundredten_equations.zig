...
const errno: isize = -410;
const next_errno: isize = -411;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 410 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 410 raw equation stays between its errno neighbors" {
...
test "errno 410 adjacent raw lanes keep value and pointer decoders precise" {
