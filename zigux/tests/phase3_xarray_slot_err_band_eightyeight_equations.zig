...
test "representative 88-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 962, 2004, 3006, 4006 };
...
test "every interior eightyeight window keeps consecutive codes and alternating constructor families" {
...
test "eightyeight windows preserve dual-center seam equations and a paired payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
