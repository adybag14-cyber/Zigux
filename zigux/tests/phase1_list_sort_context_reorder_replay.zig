...
test "phase1 list_sort replay reuses plain comparator context across repeated reordering" {
    const context_cmp = struct {
    ...
    list_sort.listSort(&mode, &head, context_cmp);
    ...
    try std.testing.expectEqualSlices(usize, &.{ 1, 3, 0, 5, 2, 4 }, ordinals[0..count]);
