...
pub fn build(b: *std.Build) void {
    ...
    const idr_slot = addIdrSlotStarterPacket(b, target, optimize);

    const shard_step = b.step(
        "phase3-bitmap-cpumask-idr-slot-test",
        "Run the Phase 3 bitmap/cpumask and IDR slot starter packets from one standalone tests-root shard",
    );
    shard_step.dependOn(&bitmap_cpumask.step);
    shard_step.dependOn(&idr_slot.step);

    const test_step = b.step("test", "Run the Phase 3 bitmap/cpumask plus IDR slot standalone shard");
    test_step.dependOn(shard_step);
}
