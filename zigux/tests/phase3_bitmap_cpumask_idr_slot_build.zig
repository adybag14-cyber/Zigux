...
fn addPhase3IdrSlotStarterPacket(
    ...
    idr_slot_view.addImport("err_ptr", err_ptr);
...
pub fn build(b: *std.Build) void {
    ...
    const bitmap_cpumask_tests = addPhase3BitmapCpumaskStarterPacket(b, target, optimize);
    const idr_slot_tests = addPhase3IdrSlotStarterPacket(b, target, optimize);

    const lane_step = b.step(
        "phase3-bitmap-cpumask-idr-slot-test",
        "Run the Phase 3 bitmap/cpumask and IDR-slot starter packets together",
    );
    lane_step.dependOn(&bitmap_cpumask_tests.step);
    lane_step.dependOn(&idr_slot_tests.step);

    const test_step = b.step("test", "Run the Phase 3 bitmap/cpumask and IDR-slot standalone shard");
    test_step.dependOn(lane_step);
}
