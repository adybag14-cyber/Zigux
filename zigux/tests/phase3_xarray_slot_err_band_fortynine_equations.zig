...
test "representative 49-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 999, 2039, 3045, 4045 };
...
test "every interior fortynine window keeps consecutive codes and alternating constructor families" {
...
test "fortynine windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
        const seam_payload_sum =
