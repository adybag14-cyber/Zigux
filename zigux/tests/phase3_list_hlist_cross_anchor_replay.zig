...
test "phase3 list/hlist cross anchor replay keeps the live list span visible while detached cross anchors stay off path" {
    ...
    shadow_left.next = @intFromPtr(&live_right);
    shadow_left.prev = @intFromPtr(&first);
    shadow_right.next = @intFromPtr(&tail);
    shadow_right.prev = @intFromPtr(&live_left);
...
test "phase3 list/hlist cross anchor replay reports the first visible list break when the live route adopts a detached cross anchor early" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_right = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    live_left.prev = @intFromPtr(&first);
    live_right.next = @intFromPtr(&tail);
    live_right.prev = @intFromPtr(&shadow_left);
    tail.next = @intFromPtr(&head);
    tail.prev = @intFromPtr(&live_right);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&shadow_left)), breakage.actual_prev);
    try std.testing.expect(!view.hasConsistentBacklinks());
}

test "phase3 list/hlist cross anchor replay keeps the live hlist span visible while detached cross anchors stay off path" {
    ...
    shadow_right.next = @intFromPtr(&tail);
    shadow_right.pprev = @intFromPtr(&live_left.next);
...
test "phase3 list/hlist cross anchor replay reports the first visible hlist break when the live route adopts a detached cross anchor early" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_right = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    live_left.pprev = @intFromPtr(&first.next);
    live_right.next = @intFromPtr(&tail);
    live_right.pprev = @intFromPtr(&shadow_left.next);
    tail.next = 0;
    tail.pprev = @intFromPtr(&live_right.next);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&shadow_left.next)), breakage.actual_pprev);
    try std.testing.expect(!view.hasConsistentPrevLinks());
}
