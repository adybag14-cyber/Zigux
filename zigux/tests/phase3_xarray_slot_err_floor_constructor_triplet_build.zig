...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_err_floor_constructor_triplet.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-xarray-slot-err-floor-constructor-triplet",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase3-xarray-slot-err-floor-constructor-triplet",
        "Run the focused Lane 29 err-floor constructor-triplet replay",
    );
