...
test "representative 38-raw err-band windows keep alias-err spacing exact" {
...
test "every interior thirtyeight window keeps consecutive codes and alternating constructor families" {
...
test "thirtyeight windows preserve dual-center seam equations and centered payload pairs" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
