const std = @import("std");
const Io = std.Io;
const abi = @import("abi_bindings");
const chrdev_notify_ack_window_plan = @import("chrdev_notify_ack_window_plan");

fn writeSummary(writer: anytype, summary: abi.ChrdevNotifyAckWindowSummary) !void {
    try writer.writeAll("{\"ack_budget_status\":");
    try writer.print("{d}", .{summary.ack_budget_status});
    try writer.writeAll(",\"ack_budget_after\":");
    try writer.print("{d}", .{summary.ack_budget_after});
    try writer.writeAll(",\"deferred_ack_budget_after\":");
    try writer.print("{d}", .{summary.deferred_ack_budget_after});
    try writer.writeAll(",\"window_flags\":");
    try writer.print("{d}", .{summary.window_flags});
    try writer.writeAll(",\"window_before\":");
    try writer.print("{d}", .{summary.window_before});
    try writer.writeAll(",\"window_after\":");
    try writer.print("{d}", .{summary.window_after});
    try writer.writeAll(",\"window_floor\":");
    try writer.print("{d}", .{summary.window_floor});
    try writer.writeAll(",\"window_status\":");
    try writer.print("{d}", .{summary.window_status});
    try writer.writeAll(",\"window_acked_count\":");
    try writer.print("{d}", .{summary.window_acked_count});
    try writer.writeAll(",\"window_deferred_count\":");
    try writer.print("{d}", .{summary.window_deferred_count});
    try writer.writeAll(",\"window_dropped_count\":");
    try writer.print("{d}", .{summary.window_dropped_count});
    try writer.writeAll(",\"window_suppressed_count\":");
    try writer.print("{d}", .{summary.window_suppressed_count});
    try writer.writeAll(",\"window_skipped_count\":");
    try writer.print("{d}", .{summary.window_skipped_count});
    try writer.writeAll("}");
}

pub fn main(init: std.process.Init) !void {
    const io = init.io;
    var stdout_buffer: [8192]u8 = undefined;
    var stdout_writer = Io.File.stdout().writer(io, &stdout_buffer);
    const writer = &stdout_writer.interface;

    const words = [_]usize{(@as(usize, 1) << 0) | (@as(usize, 1) << 3) | (@as(usize, 1) << 7)};
    const exhausted_words = [_]usize{(@as(usize, 1) << 0) | (@as(usize, 1) << 2) | (@as(usize, 1) << 4)};

    const acked_view = chrdev_notify_ack_window_plan.viewFromBits(words[0..], 240, 32, 8, 8, 2, abi.IDA_POLICY_LAST_FIT, 37, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_WRITE, abi.CHRDEV_IO_OP_WRITE, 20, 8, 1024, 4, 1, 3, 2, 1, 5, 1, 4, 2, 0x1111, 1, abi.CHRDEV_NOTIFY_MASK_SUCCESS, 1, 0xAAAA, 0, 1, 0, abi.CHRDEV_NOTIFY_ACK_MASK_ISSUED, 2, 0xA1A1, 1, 0, 1, 0, 0);
    const floor_held_view = chrdev_notify_ack_window_plan.viewFromBits(words[0..], 240, 32, 8, 8, 2, abi.IDA_POLICY_LAST_FIT, 37, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_WRITE, abi.CHRDEV_IO_OP_WRITE, 20, 8, 1024, 4, 1, 3, 2, 1, 5, 1, 4, 2, 0x1111, 1, abi.CHRDEV_NOTIFY_MASK_SUCCESS, 1, 0xCCCC, 0, 1, 0, abi.CHRDEV_NOTIFY_ACK_MASK_ISSUED, 2, 0xC3C3, 1, abi.CHRDEV_NOTIFY_ACK_POLICY_FORCE_DEFERRED, 1, 1, 2);
    const dropped_view = chrdev_notify_ack_window_plan.viewFromBits(words[0..], 240, 32, 8, 8, 2, abi.IDA_POLICY_LAST_FIT, 37, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_WRITE, abi.CHRDEV_IO_OP_WRITE, 20, 8, 1024, 4, 1, 3, 2, 1, 5, 1, 4, 2, 0x1111, 1, abi.CHRDEV_NOTIFY_MASK_SUCCESS, 1, 0xDDDD, 0, 1, 0, abi.CHRDEV_NOTIFY_ACK_MASK_ISSUED, 0, 0xD4D4, 1, 0, 1, 0, 0);
    const suppressed_view = chrdev_notify_ack_window_plan.viewFromBits(words[0..], 240, 32, 8, 8, 2, abi.IDA_POLICY_LAST_FIT, 37, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_WRITE, abi.CHRDEV_IO_OP_WRITE, 20, 8, 1024, 4, 1, 3, 2, 1, 5, 1, 4, 2, 0x1111, 1, abi.CHRDEV_NOTIFY_MASK_SUCCESS, 1, 0xE5E5, 0, 1, 0, abi.CHRDEV_NOTIFY_ACK_MASK_ISSUED, 0, 0xE5E5, 0, abi.CHRDEV_NOTIFY_ACK_POLICY_SUPPRESS_EXPIRED, 1, 1, 0);
    const skipped_view = chrdev_notify_ack_window_plan.viewFromBits(exhausted_words[0..], 240, 16, 5, 5, 2, abi.IDA_POLICY_FIRST_FIT, 20, abi.CHRDEV_MODE_READ, abi.CHRDEV_MODE_READ, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_READ, abi.CHRDEV_IO_OP_READ, 12, 32, 0, 0, 2, 2, 2, 1, 5, 1, 4, 2, 0x7777, 0, abi.CHRDEV_NOTIFY_MASK_FAILURE, 1, 0xF6F6, abi.CHRDEV_NOTIFY_POLICY_SUPPRESS_FAILURE, 3, 4, abi.CHRDEV_NOTIFY_ACK_MASK_ISSUED, 2, 0xF6F6, 0, 0, 1, 1, 0);
    const empty_view = abi.ChrdevNotifyAckWindowView{
        .bits_addr = 0,
        .major = 240,
        .first_minor = 0,
        .minor_count = 0,
        .max_scan = 0,
        .request_count = 2,
        .policy = abi.IDA_POLICY_FIRST_FIT,
        .target_minor = 0,
        .requested_mode = abi.CHRDEV_MODE_READ,
        .supported_mode = abi.CHRDEV_MODE_READ,
        .available_ops = abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_READ,
        .io_op = abi.CHRDEV_IO_OP_READ,
        .requested_bytes = 8,
        .max_chunk_bytes = 8,
        .file_offset = 0,
        .bytes_completed = 0,
        .max_segments = 1,
        .resume_passes = 2,
        .retry_budget = 1,
        .stall_budget = 1,
        .backoff_quanta = 5,
        .queue_depth = 0,
        .queue_capacity = 2,
        .requeue_budget = 1,
        .completion_cookie = 0x9999,
        .completion_budget = 0,
        .notify_mask = abi.CHRDEV_NOTIFY_MASK_SUCCESS,
        .notify_cookie = 0xFFFF,
        .notify_budget = 0,
        .reserved = 0,
        .policy_flags = 0,
        .policy_reserved = 0,
        .delivery_budget = 0,
        .deferred_budget = 0,
        .ack_mask = abi.CHRDEV_NOTIFY_ACK_MASK_ISSUED,
        .ack_window = 0,
        .ack_cookie = 0xABCD,
        .ack_observed = 0,
        .ack_reserved = 0,
        .ack_policy_flags = 0,
        .ack_policy_reserved = 0,
        .ack_budget = 0,
        .deferred_ack_budget = 0,
        .ack_budget_reserved = 0,
        .window_floor = 0,
        .window_reserved = 0,
    };

    try writer.writeAll("{\"constants\":{\"chrdev_notify_ack_window_flag_window_applied\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_FLAG_WINDOW_APPLIED});
    try writer.writeAll(",\"chrdev_notify_ack_window_flag_window_used\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_FLAG_WINDOW_USED});
    try writer.writeAll(",\"chrdev_notify_ack_window_flag_window_exhausted\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_FLAG_WINDOW_EXHAUSTED});
    try writer.writeAll(",\"chrdev_notify_ack_window_flag_floor_held\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_FLAG_FLOOR_HELD});
    try writer.writeAll(",\"chrdev_notify_ack_window_flag_floor_blocked\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_FLAG_FLOOR_BLOCKED});
    try writer.writeAll(",\"chrdev_notify_ack_window_status_none\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_NONE});
    try writer.writeAll(",\"chrdev_notify_ack_window_status_acked\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_ACKED});
    try writer.writeAll(",\"chrdev_notify_ack_window_status_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_ack_window_status_dropped\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_DROPPED});
    try writer.writeAll(",\"chrdev_notify_ack_window_status_suppressed\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_SUPPRESSED});
    try writer.writeAll(",\"chrdev_notify_ack_window_status_skipped\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_STATUS_SKIPPED});
    try writer.writeAll("}");

    try writer.writeAll(",\"acked\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_plan.summarize(acked_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"floor_held\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_plan.summarize(floor_held_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"dropped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_plan.summarize(dropped_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"suppressed\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_plan.summarize(suppressed_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"skipped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_plan.summarize(skipped_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"empty\":{\"is_valid\":");
    try writer.writeAll(if (chrdev_notify_ack_window_plan.isValid(empty_view)) "true" else "false");
    try writer.writeAll(",\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_plan.summarize(empty_view));
    try writer.writeAll("}}\n");
    try stdout_writer.flush();
}
