...
test "representative 133-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2960, 3960 };
...
test "every interior onehundredthirtythree window keeps consecutive codes and alternating constructor families" {
...
test "onehundredthirtythree windows preserve centered equations around the middle err slot and middle rejected payload" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        while (payload_idx < center_index) : (payload_idx += 2) {
        ...
        try testing.expectEqual(
            ...
            expectedRejectedPayload(center_residue),
        );
        try testing.expectEqual(
