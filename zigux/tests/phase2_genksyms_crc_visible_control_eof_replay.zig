...
test "lane19 visible EOF replay preserves leading low control bytes in a split continuation" {
    ...
    try gen.runGenksymsCrc(split_then_visible_controls.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_controls.items[0..c_line_payload_len])});
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_controls.items)});
