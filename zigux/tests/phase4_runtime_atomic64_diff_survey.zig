...
// phase4_review_checklist_blob_sha=30905777466eaf829473f08036d0bcc205ee4cbc
...
const phase4_gate_evidence_self_test_cases_line =
    ...
    "missing_validator_file,missing_phase4_build_file,missing_artifact_diff_helper_file,missing_workflow_route_checker_file,missing_atomic64_manifest_file," ++
...
test "phase 4 atomic64 survey keeps the gate-evidence runtime survey blob pins exact" {
    const gate_evidence_source = try readRepoFile(
        std.testing.allocator,
        "Documentation/zigux/phase4-gate-evidence.md",
    );
    ...
    const validator_blob_marker = try std.fmt.allocPrint(
        std.testing.allocator,
        "PHASE4_VALIDATOR_BLOB_SHA={s}",
        .{validator_blob_sha},
    );
    defer std.testing.allocator.free(validator_blob_marker);
    try expectMarker(gate_evidence_source, validator_blob_marker);
    ...
    defer std.testing.allocator.free(phase4_validation_matrix_source);
    const matrix_blob_sha = try gitBlobShaHex(phase4_validation_matrix_source);
    const matrix_blob_marker = try std.fmt.allocPrint(
        std.testing.allocator,
        "PHASE4_VALIDATION_MATRIX_BLOB_SHA={s}",
        .{matrix_blob_sha},
    );
    defer std.testing.allocator.free(matrix_blob_marker);
    try expectMarker(gate_evidence_source, matrix_blob_marker);
