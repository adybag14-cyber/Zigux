...
test "representative 91-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 957, 1999, 3001, 4003 };
...
test "every interior ninetyone window keeps consecutive codes and alternating constructor families" {
...
test "ninetyone windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
