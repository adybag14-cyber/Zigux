...
const errno: isize = -331;
const next_errno: isize = -332;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 331 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 331 raw equation stays between its errno neighbors" {
...
test "errno 331 adjacent raw lanes keep value and pointer decoders precise" {
