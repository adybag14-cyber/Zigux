...
test "representative 46-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1004, 2038, 3048, 4048 };
...
test "every interior fortysix window keeps consecutive codes and alternating constructor families" {
...
test "fortysix windows preserve dual-center seam equations and the centered rejected-payload law" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue) + center_left_index,
