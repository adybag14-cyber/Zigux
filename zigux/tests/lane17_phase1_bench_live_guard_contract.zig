...
test "lane17 guard self-test protects missing duplicate reordered and marker failures" {
    const guard_source = try readGuardSource();
    defer std.testing.allocator.free(guard_source);
    ...
    try expectContains(guard_source, "PHASE1_BENCH_LIVE_CHECK_WORKFLOW_SELF_TEST_CASE_COUNT=5");
