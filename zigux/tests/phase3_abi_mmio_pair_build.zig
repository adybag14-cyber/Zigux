...
fn addPhase3AbiMmioPair(
    ...
    const mmio_tests = b.addTest(.{
        .name = "phase3_abi_mmio_pair_mmio",
        .root_module = mmio,
    });
    ...
    const pair = b.step(
        ...
        "Run the Phase 3 ABI binding contract beside the MMIO helper packet",
    );
    pair.dependOn(&abi_run.step);
    pair.dependOn(&mmio_run.step);
    return pair;
}
...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = addPhase3AbiMmioPair(b, target, optimize);
    const test_step = b.step("test", "Run the Phase 3 ABI plus MMIO pair-build shard");
