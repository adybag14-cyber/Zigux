...
const errno: isize = -465;
const next_errno: isize = -466;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 465 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 465 raw equation stays between its errno neighbors" {
...
test "errno 465 adjacent raw lanes keep value and pointer decoders precise" {
