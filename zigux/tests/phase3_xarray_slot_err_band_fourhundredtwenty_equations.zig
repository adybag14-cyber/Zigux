...
const errno: isize = -420;
const next_errno: isize = -421;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 420 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 420 raw equation stays between its errno neighbors" {
...
test "errno 420 adjacent raw lanes keep value and pointer decoders precise" {
