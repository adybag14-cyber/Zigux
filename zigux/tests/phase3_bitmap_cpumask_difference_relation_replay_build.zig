...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_difference_relation_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step("phase3-bitmap-cpumask-difference-relation-replay", "Run the Lane 27 bitmap/cpumask difference relation replay");
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run the Lane 27 bitmap/cpumask difference relation replay");
