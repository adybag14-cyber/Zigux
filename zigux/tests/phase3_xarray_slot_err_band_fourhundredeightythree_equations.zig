...
const errno: isize = -483;
const next_errno: isize = -484;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 483 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 483 raw equation stays between its errno neighbors" {
...
test "errno 483 adjacent raw lanes keep value and pointer decoders precise" {
