...
test "septemvigintuple trailing empty banks keep early windows aligned and masked tail bits bounded" {
    const capacity = (29 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        bit(2) | bit(7) | bit(11) | bit(15),
        bit(1) | bit(5) | bit(9) | bit(14),
    } ++ [_]usize{0} ** 27 ++ [_]usize{
    ...
    try testing.expect(bitmap.isSet((29 * bitmap_view.word_bits) + 0));
    try testing.expect(bitmap.isSet((29 * bitmap_view.word_bits) + 10));
    try testing.expect(!bitmap.isSet((29 * bitmap_view.word_bits) + 9));
    ...
    try testing.expect(!cpumask.hasCpu((28 * bitmap_view.word_bits) + 12));
    try testing.expect(cpumask.hasCpu((29 * bitmap_view.word_bits) + 6));
}

test "septemvigintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (29 * bitmap_view.word_bits) + 11;
    ...
    const tail_only_words = [_]usize{0} ** 29 ++ [_]usize{
