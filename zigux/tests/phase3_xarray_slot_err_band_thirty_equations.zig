...
test "representative 30-raw err-band windows keep alias-err spacing exact" {
...
test "every interior thirty window keeps consecutive codes and alternating constructor families" {
...
test "thirty windows preserve seam equations and the centered rejected-payload law" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
