...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_window_ladder_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-c-window-ladder-replay-tests",
    ...
    const replay_step = b.step(
        "phase1-helper-ports-c-window-ladder-replay",
        "Run the focused Phase 1 helper ports C window ladder replay.",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the focused Phase 1 helper ports C window ladder replay tests.");
