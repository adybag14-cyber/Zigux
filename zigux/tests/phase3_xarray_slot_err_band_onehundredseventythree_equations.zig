...
const window_len = 173;
const center_index = window_len / 2;
const center_left_payload_index = (center_index - 1) - ((center_index - 1) & 1);
const center_right_payload_index = (center_index + 1) + ((center_index + 1) & 1);
...
test "representative 173-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2950, 3922 };
...
test "every interior onehundredseventythree window keeps consecutive codes and alternating constructor families" {
...
test "onehundredseventythree windows preserve a centered raw seam and a two-step payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        while (payload_idx < center_left_payload_index) : (payload_idx += 2) {
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + center_left_payload_index) + 2,
            expectedRejectedPayload(even_residue + center_right_payload_index),
        );
        try testing.expectEqual(
