...
test "list bridge-pair tail claim stays off-route until the visible tail adopts it" {
    ...
    claim_right.next = @intFromPtr(&head);
    ...
    try testing.expectEqual(@as(usize, @intFromPtr(&bridge_right)), breakage.expected_prev);
...
test "hlist bridge-pair tail claim stays off-route until the visible tail adopts it" {
    ...
    try testing.expectEqual(@as(usize, 3), breakage.current_index);
    try testing.expectEqual(@as(usize, @intFromPtr(&bridge_right.next)), breakage.expected_pprev);
