...
test "lane27 triple-bank full-valid bitmap replay keeps the trailing full window explicit" {
    const capacity = (bitmap_view.word_bits * 3) + 7;
    const words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        (@as(usize, 1) << 7) - 1 | invalidTailNoise(capacity),
    };
    ...
    try testing.expect(view.isSet((bitmap_view.word_bits * 3) - 1));
    try testing.expect(view.isSet(bitmap_view.word_bits * 3));
    try testing.expect(view.isSet(capacity - 1));
}

test "lane27 triple-bank full-valid cpumask replay ignores tail-only noise in subset checks" {
    const capacity = (bitmap_view.word_bits * 3) + 7;
...
test "lane27 triple-bank full-valid cpumask replay treats pure tail noise as disjoint" {
    const capacity = (bitmap_view.word_bits * 3) + 7;
    const base_words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
