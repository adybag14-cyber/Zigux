...
test "phase 9 runtime atomic64 survey note keeps exact selftest and loader snapshot checks explicit" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, note, "The survey artifacts now advance to `P9-L04` because the bounded sample-side loader scaffold, the shared runtime-loader facade plus allocator/init-flow contract replay, and the shared request-surface proof are landed and reviewable on `master`.") != null);
...
test "phase 9 runtime atomic64 module slice keeps the loader-backed survey packet explicit" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, note, "`PHASE9_LANE_KEY=P9-L04`") != null);
...
test "phase 9 runtime atomic64 survey source-checks the direct sample evidence packet" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, makefile, "phase9-runtime-atomic64-test:") != null);
    try std.testing.expect(std.mem.indexOf(u8, makefile, "$(ZIG) build phase9-runtime-atomic64-tests --build-file zigux/tests/phase9_build.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, makefile, "phase9-test:") != null);
    try std.testing.expect(std.mem.indexOf(u8, makefile, "phase9: phase9-test") != null);
}
