...
const errno: isize = -313;
const next_errno: isize = -314;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 313 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 313 raw equation stays between its errno neighbors" {
...
test "errno 313 adjacent raw lanes keep value and pointer decoders precise" {
