...
fn writeSummary(writer: anytype, summary: abi.ChrdevRequeueSummary) !void {
...
pub fn main(init: std.process.Init) !void {
    ...
    const requeue_view = chrdev_requeue_plan.viewFromBits(words[0..], 240, 32, 8, 8, 2, abi.IDA_POLICY_LAST_FIT, 37, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_WRITE, abi.CHRDEV_IO_OP_WRITE, 36, 8, 1024, 4, 1, 1, 1, 1, 0, 1, 4, 2);
    ...
    try writer.writeAll("{\"constants\":{\"chrdev_requeue_flag_truncated\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_TRUNCATED});
    try writer.writeAll(",\"chrdev_requeue_flag_found\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_FOUND});
    try writer.writeAll(",\"chrdev_requeue_flag_exhausted\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_EXHAUSTED});
    try writer.writeAll(",\"chrdev_requeue_flag_hit\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_HIT});
    try writer.writeAll(",\"chrdev_requeue_flag_permitted\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_PERMITTED});
    try writer.writeAll(",\"chrdev_requeue_flag_denied\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_DENIED});
    try writer.writeAll(",\"chrdev_requeue_flag_routable\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_ROUTABLE});
    try writer.writeAll(",\"chrdev_requeue_flag_blocked\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_BLOCKED});
    try writer.writeAll(",\"chrdev_requeue_flag_dispatchable\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_DISPATCHABLE});
    try writer.writeAll(",\"chrdev_requeue_flag_resumed\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_RESUMED});
    try writer.writeAll(",\"chrdev_requeue_flag_continuable\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_CONTINUABLE});
    try writer.writeAll(",\"chrdev_requeue_flag_completes\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_COMPLETES});
    try writer.writeAll(",\"chrdev_requeue_flag_progressed\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_PROGRESSED});
    try writer.writeAll(",\"chrdev_requeue_flag_stalled\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_STALLED});
    try writer.writeAll(",\"chrdev_requeue_flag_complete_ok\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_COMPLETE_OK});
    try writer.writeAll(",\"chrdev_requeue_flag_retryable\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_RETRYABLE});
    try writer.writeAll(",\"chrdev_requeue_flag_retry_planned\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_RETRY_PLANNED});
    try writer.writeAll(",\"chrdev_requeue_flag_retry_exhausted\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_RETRY_EXHAUSTED});
    try writer.writeAll(",\"chrdev_requeue_flag_backoff_applied\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_BACKOFF_APPLIED});
    try writer.writeAll(",\"chrdev_requeue_flag_fails\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_FAILS});
    try writer.writeAll(",\"chrdev_requeue_flag_requeueable\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_REQUEUEABLE});
    try writer.writeAll(",\"chrdev_requeue_flag_requeue_planned\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_REQUEUE_PLANNED});
    try writer.writeAll(",\"chrdev_requeue_flag_delayed\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_DELAYED});
    try writer.writeAll(",\"chrdev_requeue_flag_saturated\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_SATURATED});
    try writer.writeAll(",\"chrdev_requeue_flag_dropped\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_DROPPED});
    try writer.writeAll(",\"chrdev_requeue_flag_complete\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_FLAG_COMPLETE});
    try writer.writeAll(",\"chrdev_requeue_index_none\":");
    try writer.print("{d}", .{abi.CHRDEV_REQUEUE_INDEX_NONE});
    try writer.writeAll("},\"complete\":{\"summary\":");
    try writeSummary(writer, chrdev_requeue_plan.summarize(complete_view));
    try writer.writeAll("},\"requeue_planned\":{\"summary\":");
    try writeSummary(writer, chrdev_requeue_plan.summarize(requeue_view));
    try writer.writeAll("},\"delayed\":{\"summary\":");
    try writeSummary(writer, chrdev_requeue_plan.summarize(delayed_view));
    try writer.writeAll("},\"saturated\":{\"summary\":");
    try writeSummary(writer, chrdev_requeue_plan.summarize(saturated_view));
    try writer.writeAll("},\"denied\":{\"summary\":");
    try writeSummary(writer, chrdev_requeue_plan.summarize(denied_view));
    try writer.writeAll("},\"exhausted\":{\"summary\":");
    try writeSummary(writer, chrdev_requeue_plan.summarize(exhausted_view));
    try writer.writeAll("},\"empty\":{\"is_valid\":");
    try writer.writeAll(if (chrdev_requeue_plan.isValid(empty_view)) "true" else "false");
    try writer.writeAll(",\"summary\":");
    try writeSummary(writer, chrdev_requeue_plan.summarize(empty_view));
    try writer.writeAll("}}\n");
