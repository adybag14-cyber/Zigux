...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = b.step(
        "phase3-abi-narrow-pair-test",
        "Run the Phase 3 ABI replay beside the unsafe narrow helper tests",
    );
    pair_step.dependOn(&run_abi_replay_tests.step);
    pair_step.dependOn(&run_narrow_tests.step);

    const test_step = b.step("test", "Run the Phase 3 ABI narrow pair tests");
