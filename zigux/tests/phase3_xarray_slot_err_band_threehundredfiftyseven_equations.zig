...
const errno: isize = -357;
const next_errno: isize = -358;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 357 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 357 raw equation stays between its errno neighbors" {
...
test "errno 357 adjacent raw lanes keep value and pointer decoders precise" {
