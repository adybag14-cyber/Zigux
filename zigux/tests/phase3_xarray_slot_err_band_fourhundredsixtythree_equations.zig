...
const errno: isize = -463;
const next_errno: isize = -464;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 463 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 463 raw equation stays between its errno neighbors" {
...
test "errno 463 adjacent raw lanes keep value and pointer decoders precise" {
