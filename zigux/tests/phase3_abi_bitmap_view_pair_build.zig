...
pub fn build(b: *std.Build) void {
    ...
    const abi_tests = b.addTest(.{
        .name = "phase3_abi_bitmap_view_pair_abi_tests",
        .root_module = abi_root_module,
    });
    const bitmap_view_tests = b.addTest(.{
        .name = "phase3_abi_bitmap_view_pair_bitmap_view_tests",
        .root_module = bitmap_view_module,
    });

    const run_abi_tests = b.addRunArtifact(abi_tests);
    const run_bitmap_view_tests = b.addRunArtifact(bitmap_view_tests);

    const pair_step = b.step(
        "phase3-abi-bitmap-view-pair-test",
        "Run the Phase 3 ABI and bitmap view pair tests",
    );
    pair_step.dependOn(&run_abi_tests.step);
    pair_step.dependOn(&run_bitmap_view_tests.step);

    const test_step = b.step("test", "Run the Phase 3 ABI and bitmap view pair tests");
