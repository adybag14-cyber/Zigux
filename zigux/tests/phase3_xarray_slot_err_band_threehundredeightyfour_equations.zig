...
const errno: isize = -384;
const next_errno: isize = -385;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 384 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 384 raw equation stays between its errno neighbors" {
...
test "errno 384 adjacent raw lanes keep value and pointer decoders precise" {
