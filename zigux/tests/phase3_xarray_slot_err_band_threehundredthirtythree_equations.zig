...
const errno: isize = -333;
const next_errno: isize = -334;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 333 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 333 raw equation stays between its errno neighbors" {
...
test "errno 333 adjacent raw lanes keep value and pointer decoders precise" {
