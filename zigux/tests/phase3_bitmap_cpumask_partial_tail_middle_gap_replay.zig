...
test "partial tail middle-gap keeps interior clear-bit discovery aligned" {
    ...
    const expected_missing = bitmap_view.word_bits + 2;
...
test "partial tail middle-gap keeps subset and overlap blind to tail-only noise" {
    const capacity = bitmap_view.word_bits + 5;
