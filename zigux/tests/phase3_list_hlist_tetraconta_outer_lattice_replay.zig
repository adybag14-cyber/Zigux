...
const stage_break = visible_count / 2;

const route = [_]usize{
    39, 0,  37, 2,  35, 4,  33, 6,  31, 8,
    29, 10, 27, 12, 25, 14, 23, 16, 21, 18,
    20, 19, 22, 17, 24, 15, 26, 13, 28, 11,
    30, 9,  32, 7,  34, 5,  36, 3,  38, 1,
};
...
test "tetraconta outer lattice preserves forward list and hlist visibility before repair" {
    var nodes = initNodes();
...
test "tetraconta outer lattice repairs list backlinks and hlist pprev in staged order" {
    var nodes = initNodes();
    var list_head = ListHead{ .next = 0, .prev = 0 };
    var hlist_head = HListHead{ .first = 0 };
    installForwardOnly(&list_head, &hlist_head, &nodes);
