...
const route = [_]usize{ 25, 0, 23, 2, 21, 4, 19, 6, 17, 8, 15, 10, 13, 12, 14, 11, 16, 9, 18, 7, 20, 5, 22, 3, 24, 1 };
const detached_index = 26;

fn resetEntries(entries: *[27]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[27]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[27]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[27]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[27]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[27]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[27]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[27]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "hexacosa center clasp keeps forward list and hlist visibility before repair" {
    var entries: [27]Entry = undefined;
...
test "hexacosa center clasp staged repair crosses the middle hinge" {
    var entries: [27]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 20);
    repairHListPrefix(&hlist_head, &entries, 20);
    ...
    try std.testing.expectEqual(@as(usize, 20), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[19]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 20), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[19]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
