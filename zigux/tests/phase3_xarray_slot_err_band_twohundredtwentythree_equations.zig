...
test "twohundredtwentythree raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 54, 986, 2018, 2998, 3872 };
...
test "twohundredtwentythree constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredtwentythree keeps one centered raw and even payload stride" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        try testing.expectEqual(
            rejectedPayload(even_residue + previous_payload_index) + 1,
