...
const route = [_]usize{
    49, 0,  26, 24, 48, 1, 27, 23, 47, 2,  28, 22, 46, 3,  29, 21,
    45, 4,  30, 20, 44, 5, 31, 19, 43, 6,  32, 18, 42, 7,  33, 17,
    41, 8,  34, 16, 40, 9, 35, 15, 39, 10, 36, 14, 38, 11, 37, 12,
    25, 13,
};
...
test "pentaconta weave-clasp exposes the same visible route through list and hlist" {
...
test "pentaconta weave-clasp repairs staged reverse links after the outer weave" {
    var entries = [_]Entry{.{}} ** (visible_count + 1);
    var head = ListHead{ .next = 0, .prev = 0 };
    var hhead = HListHead{ .first = 0 };

    reset(&entries, &head, &hhead);
    const gate_position = 41;
