...
test "phase 15 architecture council review-process note stays aligned with checklist and handoff language" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, readme, "Phase 15 notes") != null);
    try std.testing.expect(std.mem.indexOf(u8, readme, "phase15-architecture-council-review-process.md") != null);
    try std.testing.expect(std.mem.indexOf(u8, readme, "phase15-indefinite-c-policy.md") != null);
    try std.testing.expect(std.mem.indexOf(u8, readme, "make -C zigux phase15") != null);
    try std.testing.expect(std.mem.indexOf(u8, readme, "maintenance mode") != null);
    try std.testing.expect(std.mem.indexOf(u8, readme, "named reopen triggers") != null);
}
