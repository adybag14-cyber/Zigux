...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_folded_tail_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-bitmap-cpumask-folded-tail-replay",
        .root_module = root_module,
    });
    const run = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase3-bitmap-cpumask-folded-tail-replay",
        "Run the Phase 3 bitmap/cpumask folded-tail replay",
    );
    replay_step.dependOn(&run.step);

    const test_step = b.step("test", "Run the Phase 3 bitmap/cpumask folded-tail replay");
