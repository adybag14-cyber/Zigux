...
const errno: isize = -401;
const next_errno: isize = -402;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 401 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 401 raw equation stays between its errno neighbors" {
...
test "errno 401 adjacent raw lanes keep value and pointer decoders precise" {
