...
test "representative 89-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 959, 2001, 3005, 4005 };
...
test "every interior eightynine window keeps consecutive codes and alternating constructor families" {
...
test "eightynine windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
