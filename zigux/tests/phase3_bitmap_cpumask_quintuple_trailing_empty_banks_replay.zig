...
test "quintuple trailing empty banks keep early windows aligned and late banks empty" {
    ...
    try testing.expect(!bitmap.isSet((6 * bitmap_view.word_bits) + 10));
    ...
    try testing.expect(!cpumask.hasCpu((6 * bitmap_view.word_bits) + 7));
}

test "quintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (6 * bitmap_view.word_bits) + 11;
