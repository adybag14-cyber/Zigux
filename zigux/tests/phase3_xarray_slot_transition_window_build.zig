...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_transition_window.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const test_step = b.step(
        "phase3-xarray-slot-transition-window",
        "Run the Phase 3 xarray slot transition-window replay",
    );
