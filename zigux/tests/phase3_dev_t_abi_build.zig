...
pub fn build(b: *std.Build) void {
    ...
    const phase3_dev_t = addPhase3DevTStarterPacket(b, target, optimize);
    const phase3_abi = addPhase3AbiCorePacket(b, target, optimize);

    const phase3_step = b.step(
        "phase3-dev-t-abi-test",
        "Run the shared Phase 3 dev_t starter and ABI core packets together from zigux/tests",
    );
    phase3_step.dependOn(&phase3_dev_t.step);
