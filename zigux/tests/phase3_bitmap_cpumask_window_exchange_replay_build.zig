...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_window_exchange_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-bitmap-cpumask-window-exchange-replay",
    ...
    const replay_step = b.step(
        "phase3-bitmap-cpumask-window-exchange-replay",
        "Run the focused Phase 3 bitmap/cpumask window-exchange replay",
    );
