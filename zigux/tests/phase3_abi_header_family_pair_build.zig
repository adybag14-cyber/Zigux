...
fn addPhase3AbiHeaderFamilyPair(
    b: *std.Build,
    ...
    const abi_tests = b.addTest(.{
        .name = "phase3_abi_header_family_pair_abi",
    ...
    const header_family_tests = b.addTest(.{
        .name = "phase3_abi_header_family_pair_header_family",
        .root_module = header_family_binding,
    });

    const abi_run = b.addRunArtifact(abi_tests);
    const header_family_run = b.addRunArtifact(header_family_tests);
    const pair = b.step(
        "phase3-abi-header-family-pair-test",
        "Run the Phase 3 ABI binding contract beside the header-family binding packet",
    );
    pair.dependOn(&abi_run.step);
    pair.dependOn(&header_family_run.step);
    return pair;
}
...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = addPhase3AbiHeaderFamilyPair(b, target, optimize);
    const test_step = b.step("test", "Run the Phase 3 ABI plus header-family pair-build shard");
