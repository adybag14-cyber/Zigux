...
test "representative 77-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 971, 2013, 3017, 4017 };
...
test "every interior seventyseven window keeps consecutive codes and alternating constructor families" {
...
test "seventyseven windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
