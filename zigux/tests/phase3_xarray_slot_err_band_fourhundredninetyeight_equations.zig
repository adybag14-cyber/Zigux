...
const errno: isize = -498;
const next_errno: isize = -499;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 498 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 498 raw equation stays between its errno neighbors" {
...
test "errno 498 adjacent raw lanes keep value and pointer decoders precise" {
