...
const route = [_]usize{ 33, 0, 16, 17, 32, 1, 15, 18, 31, 2, 14, 19, 30, 3, 13, 20, 29, 4, 12, 21, 28, 5, 11, 22, 27, 6, 10, 23, 26, 7, 9, 24, 25, 8 };
const detached_index = 34;
...
test "tetratriaconta outer spoke keeps forward list and hlist visibility before repair" {
...
test "tetratriaconta outer spoke staged repair reaches the spoke hinge" {
    var entries: [entry_count]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 29);
    repairHListPrefix(&hlist_head, &entries, 29);
    ...
    try std.testing.expectEqual(@as(usize, 29), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[28]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_spoke.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 29), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[28]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
