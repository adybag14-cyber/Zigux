...
const errno: isize = -472;
const next_errno: isize = -473;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 472 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 472 raw equation stays between its errno neighbors" {
...
test "errno 472 adjacent raw lanes keep value and pointer decoders precise" {
