...
test "list parallel tail braid stays detached until the tail route adopts it" {
    ...
    try expectListSequence(
        broken,
        &.{ &first, &live_left, &live_right, &braid_left, &braid_right, &tail },
    );
...
test "hlist parallel tail braid stays detached until the tail route adopts it" {
