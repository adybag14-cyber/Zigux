...
test "phase3 list/hlist head alias lag replay keeps the live list head route visible while the detached alias stays off path" {
...
test "phase3 list/hlist head alias lag replay reports the adopted list alias before the first backlink follows" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var alias = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var middle = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    try std.testing.expectEqual(@as(?*const list_view.ListHead, &alias), view.first());
    try std.testing.expectEqual(@as(?*const list_view.ListHead, &tail), view.last());
...
test "phase3 list/hlist head alias lag replay keeps the live hlist head route visible while the detached alias stays off path" {
...
test "phase3 list/hlist head alias lag replay reports the adopted hlist alias before the first prev-link follows" {
    var head = hlist_view.HListHead{ .first = 0 };
    var alias = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var middle = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    const view = hlist_view.HListView.init(&head);
    try std.testing.expectEqual(@as(usize, 4), view.len());
    try std.testing.expectEqual(@as(?*const hlist_view.HListNode, &alias), view.first());
