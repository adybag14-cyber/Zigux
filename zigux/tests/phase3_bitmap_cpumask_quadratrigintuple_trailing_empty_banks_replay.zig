...
test "bitmap replay keeps quadratrigintuple trailing empty banks aligned with a masked tail" {
...
test "cpumask replay mirrors the same quadratrigintuple tail behavior" {
    const base_words = makeBaseWords();
    var superset_words = makeBaseWords();
    var disjoint_words = [_]usize{0} ** active_word_count;
