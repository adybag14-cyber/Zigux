...
const staged_break_index = 46;
const route = [_]usize{
    65, 0,  40, 39, 64, 1,  41, 38, 63, 2,  42, 37, 62, 3,
    43, 36, 61, 4,  44, 35, 60, 5,  45, 34, 59, 6,  46, 33,
    58, 7,  47, 32, 57, 8,  48, 31, 56, 9,  49, 30, 55, 10,
    50, 29, 54, 11, 51, 28, 53, 12, 52, 27, 26, 13, 25, 24,
...
test "hexahexaconta weave-clasp list route exposes staged backlink repair" {
...
test "hexahexaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
