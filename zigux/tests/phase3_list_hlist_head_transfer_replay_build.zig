...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_head_transfer_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-list-hlist-head-transfer-replay-test",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const replay_step = b.step(
        "phase3-list-hlist-head-transfer-replay",
        "Run the focused Phase 3 list/hlist head-transfer replay",
    );
    replay_step.dependOn(&run_tests.step);

    const default_step = b.step("test", "Run the focused Phase 3 list/hlist head-transfer replay");
