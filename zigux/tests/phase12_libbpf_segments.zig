...
test "phase12 libbpf survey note records rollback and archived surveyed head" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "python3 scripts/zigux/check-phase12-release-readiness-packet.py --self-test") != null);
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "python3 scripts/zigux/check-phase12-release-readiness-packet.py") != null);
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "python3 scripts/zigux/check-phase12-shared-replay-contract.py --self-test") != null);
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "python3 scripts/zigux/check-phase12-shared-replay-contract.py") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "shared build-inventory, bounded libbpf snapshot, bounded libbpf packet-alignment, focused replay, release-readiness, and shared replay-contract checks all fail closed before the shared validator or bundled replay claim aligned evidence") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "zig build --build-file zigux/tests/phase12_libbpf_only_build.zig phase12-libbpf-focused-replay --summary all") != null);
