...
test "phase12 libbpf survey manifest records the heavy-helper segmentation gap" {
    ...
    for (manifest.gaps, 0..) |gap, i| {
        ...
        if (std.mem.eql(u8, gap.id, "phase12-libbpf-file-path-handle-helper-foundation")) {
            ...
            try std.testing.expectEqualStrings("starter_landed", gap.status);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "/proc/<pid>/fdinfo/<fd>") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "map-info text parsing") != null);
        ...
        if (std.mem.eql(u8, gap.id, "phase12-libbpf-object-and-elf-loader")) {
            ...
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "ELF collection") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "mirror-tree sprawl") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "helper-first") != null);
        }
        ...
        if (std.mem.eql(u8, gap.id, "phase12-libbpf-btf-relocation-and-program-load")) {
            ...
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "extern resolution") != null);
