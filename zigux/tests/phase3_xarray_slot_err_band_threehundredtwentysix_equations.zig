...
const errno: isize = -326;
const next_errno: isize = -327;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 326 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 326 raw equation stays between its errno neighbors" {
...
test "errno 326 adjacent raw lanes keep value and pointer decoders precise" {
