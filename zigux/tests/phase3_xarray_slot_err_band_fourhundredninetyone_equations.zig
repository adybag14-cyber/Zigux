...
const errno: isize = -491;
const next_errno: isize = -492;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 491 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 491 raw equation stays between its errno neighbors" {
...
test "errno 491 adjacent raw lanes keep value and pointer decoders precise" {
