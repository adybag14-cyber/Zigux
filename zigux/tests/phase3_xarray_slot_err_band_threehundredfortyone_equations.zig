...
const errno: isize = -341;
const next_errno: isize = -342;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 341 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 341 raw equation stays between its errno neighbors" {
...
test "errno 341 adjacent raw lanes keep value and pointer decoders precise" {
