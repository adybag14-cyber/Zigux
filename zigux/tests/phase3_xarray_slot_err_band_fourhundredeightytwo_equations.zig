...
const errno: isize = -482;
const next_errno: isize = -483;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 482 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 482 raw equation stays between its errno neighbors" {
...
test "errno 482 adjacent raw lanes keep value and pointer decoders precise" {
