...
pub fn build(b: *std.Build) void {
    ...
    const tests_mod = b.createModule(.{
        .root_source_file = b.path("phase3_errptr_xarray_witness_table_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-errptr-xarray-witness-table-replay",
        "Run the Phase 3 err_ptr/xarray witness-table replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 3 err_ptr/xarray witness-table replay");
