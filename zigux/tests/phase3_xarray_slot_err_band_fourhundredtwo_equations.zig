...
const errno: isize = -402;
const next_errno: isize = -403;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 402 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 402 raw equation stays between its errno neighbors" {
...
test "errno 402 adjacent raw lanes keep value and pointer decoders precise" {
