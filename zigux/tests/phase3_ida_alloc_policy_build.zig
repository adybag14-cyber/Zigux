...
pub fn build(b: *std.Build) void {
    ...
    const run_policy = b.addRunArtifact(policy_tests);

    const ida_alloc_policy_step = b.step(
        "phase3-ida-alloc-policy-test",
        "Run the shared Phase 3 IDA allocation starter packet beside the shared policy starter packet",
    );
    ida_alloc_policy_step.dependOn(&run_ida_alloc.step);
    ida_alloc_policy_step.dependOn(&run_policy.step);

    const test_step = b.step("test", "Run the Phase 3 IDA allocation plus policy standalone shard");
    test_step.dependOn(ida_alloc_policy_step);
}
