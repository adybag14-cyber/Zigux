...
const errno: isize = -355;
const next_errno: isize = -356;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 355 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 355 raw equation stays between its errno neighbors" {
...
test "errno 355 adjacent raw lanes keep value and pointer decoders precise" {
