...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_dual_window_parity_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-bitmap-cpumask-dual-window-parity-replay",
        .root_module = root_module,
    });
    const run = b.addRunArtifact(tests);

    const step = b.step(
        "phase3-bitmap-cpumask-dual-window-parity-replay",
        "Run the Phase 3 bitmap/cpumask dual-window parity replay",
    );
