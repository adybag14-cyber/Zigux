...
pub fn build(b: *std.Build) void {
    ...
    const tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("phase3_xarray_slot_floor_crossing_walk.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    const replay_step = b.step(
        "phase3-xarray-slot-floor-crossing-walk",
        "Run the Phase 3 xarray slot floor crossing walk replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 3 xarray slot floor crossing walk replay");
