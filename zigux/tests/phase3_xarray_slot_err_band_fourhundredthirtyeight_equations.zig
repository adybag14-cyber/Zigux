...
const errno: isize = -438;
const next_errno: isize = -439;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 438 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 438 raw equation stays between its errno neighbors" {
...
test "errno 438 adjacent raw lanes keep value and pointer decoders precise" {
