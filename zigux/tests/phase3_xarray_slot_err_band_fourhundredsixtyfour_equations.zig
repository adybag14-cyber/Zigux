...
const errno: isize = -464;
const next_errno: isize = -465;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 464 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 464 raw equation stays between its errno neighbors" {
...
test "errno 464 adjacent raw lanes keep value and pointer decoders precise" {
