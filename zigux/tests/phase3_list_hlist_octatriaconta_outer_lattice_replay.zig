...
const entry_count = 39;
const route = [_]usize{ 37, 0, 35, 2, 33, 4, 31, 6, 29, 8, 27, 10, 25, 12, 23, 14, 21, 16, 19, 18, 20, 17, 22, 15, 24, 13, 26, 11, 28, 9, 30, 7, 32, 5, 34, 3, 36, 1 };
const detached_index = 38;
...
test "octatriaconta outer lattice keeps forward list and hlist visibility before repair" {
...
test "octatriaconta outer lattice staged repair crosses the mirror hinge" {
    var entries: [entry_count]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    const list_after_lattice = list_view.ListView.init(&list_head);
    const hlist_after_lattice = hlist_view.HListView.init(&hlist_head);
    try expectListRoute(list_after_lattice, &entries);
    try expectHListRoute(hlist_after_lattice, &entries);

    const list_break = list_after_lattice.firstBrokenBacklink().?;
    ...
    const hlist_break = hlist_after_lattice.firstBrokenPrevLink().?;
