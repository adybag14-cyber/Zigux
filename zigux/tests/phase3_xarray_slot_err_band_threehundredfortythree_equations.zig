...
const errno: isize = -343;
const next_errno: isize = -344;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 343 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 343 raw equation stays between its errno neighbors" {
...
test "errno 343 adjacent raw lanes keep value and pointer decoders precise" {
