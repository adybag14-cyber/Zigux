...
const errno: isize = -471;
const next_errno: isize = -472;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 471 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 471 raw equation stays between its errno neighbors" {
...
test "errno 471 adjacent raw lanes keep value and pointer decoders precise" {
