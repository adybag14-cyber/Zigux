...
const errno: isize = -426;
const next_errno: isize = -427;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 426 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 426 raw equation stays between its errno neighbors" {
...
test "errno 426 adjacent raw lanes keep value and pointer decoders precise" {
