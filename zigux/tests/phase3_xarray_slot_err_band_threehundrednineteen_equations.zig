...
const errno: isize = -319;
const next_errno: isize = -320;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 319 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 319 raw equation stays between its errno neighbors" {
...
test "errno 319 adjacent raw lanes keep value and pointer decoders precise" {
