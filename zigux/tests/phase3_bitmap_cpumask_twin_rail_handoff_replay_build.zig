...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_twin_rail_handoff_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named = b.step("phase3-bitmap-cpumask-twin-rail-handoff-replay", "Run the Lane 27 bitmap/cpumask twin rail handoff replay");
    named.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 27 bitmap/cpumask twin rail handoff replay");
