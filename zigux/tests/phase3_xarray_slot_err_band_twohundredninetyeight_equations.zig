...
test "twohundredninetyeight raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 50, 984, 2016, 2996, 3797 };
...
test "twohundredninetyeight constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredninetyeight keeps centered raw code and payload equations exact" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
            rawFromResidue(even_residue) + center_left_index,
            rawFromResidue(even_residue + center_left_index),
        );
        try testing.expectEqual(
            expectedCode(even_residue) + @as(isize, @intCast(center_left_index)),
            expectedCode(even_residue + center_left_index),
        );
        try testing.expectEqual(
            rejectedPayload(even_residue) + (center_left_payload_index / 2),
            rejectedPayload(even_residue + center_left_payload_index),
        );
    }
}
