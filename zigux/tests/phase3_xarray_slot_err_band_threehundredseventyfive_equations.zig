...
const errno: isize = -375;
const next_errno: isize = -376;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 375 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 375 raw equation stays between its errno neighbors" {
...
test "errno 375 adjacent raw lanes keep value and pointer decoders precise" {
