...
const route = [_]usize{ 13, 0, 11, 2, 9, 4, 7, 6, 8, 5, 10, 3, 12, 1 };
const detached_index = 14;

fn resetEntries(entries: *[15]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[15]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[15]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[15]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[15]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[15]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[15]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[15]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "quattuordeca spiral gate preserves forward list and hlist visibility before repair" {
    var entries: [15]Entry = undefined;
...
test "quattuordeca spiral gate staged repair crosses the closed center gate" {
    var entries: [15]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 8);
    repairHListPrefix(&hlist_head, &entries, 8);

    const list_after_gate = list_view.ListView.init(&list_head);
    const hlist_after_gate = hlist_view.HListView.init(&hlist_head);
    try expectListRoute(list_after_gate, &entries);
    try expectHListRoute(hlist_after_gate, &entries);

    const list_break = list_after_gate.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 8), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[7]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_gate.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 8), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[7]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
