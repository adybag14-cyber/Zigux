...
const route = [_]usize{ 8, 3, 0, 7, 2, 6, 1, 5, 4 };

fn resetEntries(entries: *[10]Entry) void {
    for (entries) |*entry| {
...
fn linkListForward(head: *ListHead, entries: *[10]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[10]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[10]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[10]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[10]Entry) !void {
    ...
    try std.testing.expect(!view.contains(&entries[9].list));
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[10]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
    ...
    try std.testing.expect(!view.contains(&entries[9].hlist));
...
test "nona spoke weave preserves forward list and hlist visibility before repair" {
...
test "nona spoke weave staged repair moves first broken metadata witness" {
    var entries: [10]Entry = undefined;
    resetEntries(&entries);
    ...
    repairListPrefix(&list_head, &entries, 4);
    repairHListPrefix(&hlist_head, &entries, 4);
    ...
    try std.testing.expectEqual(@as(usize, 4), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[3]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_prefix.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 4), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[3]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
