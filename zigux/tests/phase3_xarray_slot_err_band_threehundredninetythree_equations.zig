...
const errno: isize = -393;
const next_errno: isize = -394;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 393 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 393 raw equation stays between its errno neighbors" {
...
test "errno 393 adjacent raw lanes keep value and pointer decoders precise" {
