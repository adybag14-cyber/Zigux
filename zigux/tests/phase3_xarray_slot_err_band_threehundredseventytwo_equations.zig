...
const errno: isize = -372;
const next_errno: isize = -373;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 372 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 372 raw equation stays between its errno neighbors" {
...
test "errno 372 adjacent raw lanes keep value and pointer decoders precise" {
