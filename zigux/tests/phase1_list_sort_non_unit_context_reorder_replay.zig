...
test "phase1 list_sort replay reuses non-unit comparator context across repeated reordering" {
    ...
    try std.testing.expectEqualSlices(i32, &.{ 5, 5, 4, 2, 1, 1 }, descending_keys[0..descending_count]);
    try std.testing.expectEqualSlices(usize, &.{ 1, 3, 4, 0, 2, 5 }, descending_ordinals[0..descending_count]);
    ...
    try std.testing.expectEqualSlices(i32, &.{ 1, 1, 2, 4, 5, 5 }, ascending_keys[0..ascending_count]);
    try std.testing.expectEqualSlices(usize, &.{ 2, 5, 0, 4, 1, 3 }, ascending_ordinals[0..ascending_count]);
