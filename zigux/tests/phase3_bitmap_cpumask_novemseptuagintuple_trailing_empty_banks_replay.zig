...
fn supersetWords() [word_count]Word {
    ...
    words[tail_word_index] |= localBit(tail_base + 1);
    return words;
}
...
test "bitmap view crosses novemseptuagintuple empty banks before masked tail" {
...
test "cpumask view mirrors bitmap traversal across novemseptuagintuple empty banks" {
    const words = fixtureWords();
    ...
    try std.testing.expect(!mask.hasCpu(tail_base + 1));
...
test "bitmap and cpumask masks ignore novemseptuagintuple tail noise in relations" {
