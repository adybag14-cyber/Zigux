...
const window_len = 129;
const center_index = window_len / 2;
const center_left_payload_index = center_index - 1;
const center_right_payload_index = center_index + 1;
...
test "representative 129-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 9, 935, 1969, 2963, 3963 };
...
test "every interior onehundredtwentynine window keeps consecutive codes and alternating constructor families" {
...
test "onehundredtwentynine windows preserve centered equations around the middle err slot and split payload seam" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
        ...
        for (0..(window_len / 2)) |pair_idx| {
            ...
            if (left_payload_residue >= center_residue) break;

            const right_payload_residue = odd_residue + ((window_len - 1) - ((pair_idx * 2) + 1));
            try testing.expectEqual(
