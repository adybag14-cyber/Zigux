...
test "representative 188-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 12, 942, 1980, 2962, 3906 };
    ...
    for (left_residues) |left_residue| {
        ...
        const raw_sum = @as(u128, raws[center_left_index]) + @as(u128, raws[center_right_index]);
...
test "every interior onehundredeightyeight window keeps consecutive codes and alternating constructor families" {
...
test "onehundredeightyeight windows preserve split raw and payload centers" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const center_code_sum =
            expectedCode(even_residue + center_left_index) +
            expectedCode(even_residue + center_right_index);
        const center_raw_sum =
