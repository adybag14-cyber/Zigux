...
test "representative 71-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 977, 2019, 3023, 4023 };
...
test "every interior seventyone window keeps consecutive codes and alternating constructor families" {
...
test "seventyone windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
