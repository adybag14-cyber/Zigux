...
const errno: isize = -315;
const next_errno: isize = -316;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 315 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 315 raw equation stays between its errno neighbors" {
...
test "errno 315 adjacent raw lanes keep value and pointer decoders precise" {
