...
const errno: isize = -457;
const next_errno: isize = -458;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 457 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 457 raw equation stays between its errno neighbors" {
...
test "errno 457 adjacent raw lanes keep value and pointer decoders precise" {
