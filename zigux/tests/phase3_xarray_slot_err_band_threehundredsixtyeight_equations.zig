...
const errno: isize = -368;
const next_errno: isize = -369;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 368 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 368 raw equation stays between its errno neighbors" {
...
test "errno 368 adjacent raw lanes keep value and pointer decoders precise" {
