...
test "representative 36-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1008, 2040, 3058, 4058 };
...
test "every interior thirtysix window keeps consecutive codes and alternating constructor families" {
...
test "thirtysix windows preserve dual-center seam equations and centered payload pairs" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
