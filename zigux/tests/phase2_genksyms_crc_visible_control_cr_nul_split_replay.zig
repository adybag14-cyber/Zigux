...
test "lane19 visible control split replay preserves leading low control bytes and leading carriage returns before an embedded NUL before the next line" {
    var split_then_controls_and_leading_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 10);
    defer split_then_controls_and_leading_cr_then_nul.deinit(std.testing.allocator);
    try split_then_controls_and_leading_cr_then_nul.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, '\x08');
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, '\x0c');
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, '\x01');
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, '\r');
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, '\r');
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, 'b');
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, 0);
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, 'c');
    try split_then_controls_and_leading_cr_then_nul.append(std.testing.allocator, '\n');
    try split_then_controls_and_leading_cr_then_nul.appendSlice(std.testing.allocator, "x\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try gen.runGenksymsCrc(split_then_controls_and_leading_cr_then_nul.items, &capture);

    const exact_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32(split_then_controls_and_leading_cr_then_nul.items[0..c_line_payload_len])},
    );
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_controls_and_leading_cr_then_nul.items)});
