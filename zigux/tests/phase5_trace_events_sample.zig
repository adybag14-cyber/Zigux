...
test "phase 5 trace-events sample keeps payload and callback boundaries explicit" {
    ...
    try std.testing.expectEqualSlices(sample.SampleFocus, &expected_focus, callback_boundary.checked_focus);
}
...
test "phase 5 trace-events sample makes ownership and teardown boundaries explicit" {
    ...
    try std.testing.expectEqualSlices(sample.SampleFocus, &expected_focus, replay.callback_boundary.checked_focus);
