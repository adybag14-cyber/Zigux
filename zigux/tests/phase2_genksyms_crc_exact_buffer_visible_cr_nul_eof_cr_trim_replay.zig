...
test "lane19 exact-buffer EOF replay preserves leading carriage returns before an embedded NUL while trimming trailing carriage returns" {
    var exact_then_visible_cr_then_trailing_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 7);
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_visible_cr_then_trailing_cr_then_nul.items)});
