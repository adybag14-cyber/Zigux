...
test "undecuple trailing empty banks keep early windows aligned and late banks empty" {
    const capacity = (12 * bitmap_view.word_bits) + 12;
    const words = [_]usize{
    ...
    try testing.expect(!bitmap.isSet((11 * bitmap_view.word_bits) + 8));
    try testing.expect(!bitmap.isSet((12 * bitmap_view.word_bits) + 11));
    ...
    try testing.expect(!cpumask.hasCpu((12 * bitmap_view.word_bits) + 7));
}

test "undecuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (12 * bitmap_view.word_bits) + 12;
