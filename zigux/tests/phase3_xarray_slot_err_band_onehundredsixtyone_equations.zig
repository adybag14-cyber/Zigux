...
test "representative 161-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2950, 3934 };
...
test "every interior onehundredsixtyone window keeps consecutive codes and alternating constructor families" {
...
test "onehundredsixtyone windows preserve a centered raw seam and a centered payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
