...
const errno: isize = -311;
const next_errno: isize = -312;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 311 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 311 raw equation stays between its errno neighbors" {
...
test "errno 311 adjacent raw lanes keep value and pointer decoders precise" {
