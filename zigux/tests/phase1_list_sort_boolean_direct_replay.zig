...
test "phase1 list_sort replay preserves stable order for a boolean-style comparator" {
    ...
    try std.testing.expectEqualSlices(usize, &.{ 1, 3, 0, 5, 2, 4 }, ordinals[0..idx]);
    try std.testing.expectEqualSlices(usize, &.{ 4, 2, 5, 0, 3, 1 }, reverse_ordinals[0..reverse_idx]);
