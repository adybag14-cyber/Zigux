...
fn writeSummary(writer: anytype, summary: abi.ChrdevNotifyPolicySummary) !void {
...
pub fn main(init: std.process.Init) !void {
    ...
    try writer.writeAll("{\"constants\":{\"chrdev_notify_policy_force_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_POLICY_FORCE_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_policy_suppress_failure\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_POLICY_SUPPRESS_FAILURE});
    try writer.writeAll(",\"chrdev_notify_policy_coalesce_cookie\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_POLICY_COALESCE_COOKIE});
    try writer.writeAll(",\"chrdev_notify_policy_status_none\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_POLICY_STATUS_NONE});
    try writer.writeAll(",\"chrdev_notify_policy_status_delivered\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_POLICY_STATUS_DELIVERED});
    try writer.writeAll(",\"chrdev_notify_policy_status_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_POLICY_STATUS_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_policy_status_suppressed\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_POLICY_STATUS_SUPPRESSED});
    try writer.writeAll(",\"chrdev_notify_policy_status_coalesced\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_POLICY_STATUS_COALESCED});
    try writer.writeAll("},\"delivered\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_policy_plan.summarize(delivered_view));
    try writer.writeAll("},\"forced_deferred\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_policy_plan.summarize(forced_deferred_view));
    try writer.writeAll("},\"suppressed_failure\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_policy_plan.summarize(suppressed_failure_view));
    try writer.writeAll("},\"coalesced\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_policy_plan.summarize(coalesced_view));
    try writer.writeAll("},\"dropped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_policy_plan.summarize(dropped_view));
    try writer.writeAll("},\"empty\":{\"is_valid\":");
    try writer.writeAll(if (chrdev_notify_policy_plan.isValid(empty_view)) "true" else "false");
    try writer.writeAll(",\"summary\":");
    try writeSummary(writer, chrdev_notify_policy_plan.summarize(empty_view));
    try writer.writeAll("}}\n");
