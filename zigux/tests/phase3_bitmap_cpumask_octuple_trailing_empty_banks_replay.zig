...
test "octuple trailing empty banks keep early windows aligned and late banks empty" {
    const capacity = (9 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
    ...
    try testing.expect(!bitmap.isSet((8 * bitmap_view.word_bits) + 8));
    try testing.expect(!bitmap.isSet((9 * bitmap_view.word_bits) + 10));
    ...
    try testing.expect(!cpumask.hasCpu((9 * bitmap_view.word_bits) + 7));
}

test "octuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (9 * bitmap_view.word_bits) + 11;
