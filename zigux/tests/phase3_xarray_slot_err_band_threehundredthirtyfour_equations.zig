...
const errno: isize = -334;
const next_errno: isize = -335;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 334 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 334 raw equation stays between its errno neighbors" {
...
test "errno 334 adjacent raw lanes keep value and pointer decoders precise" {
