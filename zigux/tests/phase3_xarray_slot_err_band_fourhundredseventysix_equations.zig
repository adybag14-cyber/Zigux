...
const errno: isize = -476;
const next_errno: isize = -477;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 476 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 476 raw equation stays between its errno neighbors" {
...
test "errno 476 adjacent raw lanes keep value and pointer decoders precise" {
