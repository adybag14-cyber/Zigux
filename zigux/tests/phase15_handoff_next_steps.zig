...
test "phase 15 handoff manifest records the parked governance contract" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.pending_next_steps[0], "shared Phase 15 replay remains green") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.pending_next_steps[0], "deep-core blocker posture remains unchanged") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.pending_next_steps[1], "named reopen trigger fires") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.pending_next_steps[1], "shared Phase 15 replay drifts again") != null);
...
test "phase 15 handoff note keeps the open gaps and parked next steps explicit" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, handoff_note, "shared replay path is green on current `master`") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, handoff_note, "named reopen trigger fires") != null);
