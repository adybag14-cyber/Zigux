...
test "phase 15 handoff manifest records the focused replay and validator as landed packet evidence" {
    ...
    try std.testing.expectEqual(@as(usize, 33), manifest.present_paths.len);
    try std.testing.expectEqual(@as(usize, 1), manifest.still_missing_paths.len);
    try std.testing.expectEqual(@as(usize, 9), manifest.required_markers.len);
...
test "phase 15 handoff note treats the focused replay and validator as present while broader shared-build companions stay blocked" {
