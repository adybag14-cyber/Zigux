...
test "phase 15 handoff manifest records the current parked packet" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.adjacent_lane_boundaries[0].why_out_of_scope, "current `master` still undercounts") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.adjacent_lane_boundaries[0].why_out_of_scope, "shared-summaries") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.pending_next_steps[1], "Current `master` still shows shared-summary truthfulness drift") != null);
...
test "phase 15 handoff note keeps the parked trigger catalog explicit" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, handoff_note, "compact docs-root Phase 15 reminder in `Documentation/zigux/README.md` still undercounts") != null);
    try std.testing.expect(std.mem.indexOf(u8, handoff_note, "Any docs-root reminder repair belongs to `shared-summaries` before this handoff lane reopens") != null);
    try std.testing.expect(std.mem.indexOf(u8, handoff_note, "older shared Phase 15 checklist undercount is not fully closed on current `master`") != null);
    try std.testing.expect(std.mem.indexOf(u8, handoff_note, "does not restate the parked handoff, readiness, or sequencing manifest pair") != null);
