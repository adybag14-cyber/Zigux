...
test "phase 15 handoff manifest records the parked governance contract" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.pending_next_steps[0], "broader shared Phase 15 replay stay green") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.pending_next_steps[1], "shared replay drifts") != null);
...
test "phase 15 handoff note keeps the open gaps and parked next steps explicit" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, handoff_note, "shared replay drifts") != null);
