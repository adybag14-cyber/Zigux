...
test "lane19 exact-buffer split replay preserves visible low control bytes in a continuation before the next line" {
    var exact_then_visible_controls = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 6);
    defer exact_then_visible_controls.deinit(std.testing.allocator);
    try exact_then_visible_controls.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try exact_then_visible_controls.append(std.testing.allocator, '\x08');
    try exact_then_visible_controls.append(std.testing.allocator, '\x0c');
    try exact_then_visible_controls.append(std.testing.allocator, '\x01');
    try exact_then_visible_controls.append(std.testing.allocator, 'b');
    try exact_then_visible_controls.append(std.testing.allocator, '\n');
    try exact_then_visible_controls.appendSlice(std.testing.allocator, "x\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try gen.runGenksymsCrc(exact_then_visible_controls.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_visible_controls.items[0..c_line_payload_len])});
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_visible_controls.items)});
