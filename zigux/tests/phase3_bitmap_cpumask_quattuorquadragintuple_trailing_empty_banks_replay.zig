...
test "quattuorquadragintuple trailing empty banks keep early windows aligned and masked tail bits bounded" {
    const capacity = (46 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        bit(1) | bit(6) | bit(10) | bit(13),
        bit(3) | bit(8) | bit(12),
    } ++ [_]usize{0} ** 44 ++ [_]usize{
        bit(0) | bit(4) | bit(7) | bit(10),
    ...
    try testing.expect(bitmap.isSet(6));
    try testing.expect(bitmap.isSet(bitmap_view.word_bits + 12));
    try testing.expect(!bitmap.isSet((2 * bitmap_view.word_bits) + 0));
    try testing.expect(bitmap.isSet((46 * bitmap_view.word_bits) + 0));
    try testing.expect(bitmap.isSet((46 * bitmap_view.word_bits) + 10));
    try testing.expect(!bitmap.isSet((46 * bitmap_view.word_bits) + 9));
    ...
    try testing.expect(!cpumask.hasCpu((45 * bitmap_view.word_bits) + 5));
    try testing.expect(cpumask.hasCpu((46 * bitmap_view.word_bits) + 7));
}

test "quattuorquadragintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (46 * bitmap_view.word_bits) + 11;
    ...
    const tail_only_words = [_]usize{0} ** 46 ++ [_]usize{
