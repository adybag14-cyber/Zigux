...
pub fn build(b: *std.Build) void {
    ...
    const tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("phase3_xarray_slot_low_raw_matrix.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    const replay_step = b.step(
        "phase3-xarray-slot-low-raw-matrix",
        "Run the Phase 3 xarray slot low raw matrix replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 3 xarray slot low raw matrix replay");
