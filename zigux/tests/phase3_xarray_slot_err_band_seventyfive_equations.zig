...
test "representative 75-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 973, 2015, 3019, 4019 };
...
test "every interior seventyfive window keeps consecutive codes and alternating constructor families" {
...
test "seventyfive windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
