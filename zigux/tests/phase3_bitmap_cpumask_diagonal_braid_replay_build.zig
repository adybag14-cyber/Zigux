...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_diagonal_braid_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-bitmap-cpumask-diagonal-braid-replay",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase3-bitmap-cpumask-diagonal-braid-replay",
        "Run the Lane 27 bitmap/cpumask diagonal braid replay",
    );
    test_step.dependOn(&run_tests.step);

    const alias_step = b.step("test", "Run the Lane 27 bitmap/cpumask diagonal braid replay");
