...
test "double-bank partial tail head-gap keeps discovery aligned after two saturated words" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    const words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        bit(1) | bit(2) | bit(3) | bit(4) | bit(8) | bit(15),
    };
    ...
    try testing.expectEqual(@as(?usize, bitmap_view.word_bits * 2), bitmap.firstClearBit());
...
test "double-bank partial tail head-gap keeps subset and overlap blind to tail-only noise" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    const base_words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        bit(1) | bit(3) | bit(4) | bit(7) | bit(19),
    };
