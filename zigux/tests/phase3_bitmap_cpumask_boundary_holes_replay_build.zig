...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_boundary_holes_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-bitmap-cpumask-boundary-holes-replay",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const step = b.step(
        "phase3-bitmap-cpumask-boundary-holes-replay",
        "Run the standalone Lane 27 boundary-holes bitmap/cpumask replay",
    );
