...
const errno: isize = -404;
const next_errno: isize = -405;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 404 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 404 raw equation stays between its errno neighbors" {
...
test "errno 404 adjacent raw lanes keep value and pointer decoders precise" {
