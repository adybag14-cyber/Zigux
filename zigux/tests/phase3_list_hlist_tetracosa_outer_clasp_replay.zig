...
const route = [_]usize{ 23, 0, 21, 2, 19, 4, 17, 6, 15, 8, 13, 10, 12, 11, 14, 9, 16, 7, 18, 5, 20, 3, 22, 1 };
const detached_index = 24;

fn resetEntries(entries: *[25]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[25]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[25]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[25]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[25]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[25]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[25]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[25]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "tetracosa outer clasp keeps forward list and hlist visibility before repair" {
    var entries: [25]Entry = undefined;
...
test "tetracosa outer clasp staged repair crosses the outer clasp" {
    var entries: [25]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 18);
    repairHListPrefix(&hlist_head, &entries, 18);
    ...
    try std.testing.expectEqual(@as(usize, 18), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[17]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 18), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[17]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
