...
const errno: isize = -329;
const next_errno: isize = -330;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 329 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 329 raw equation stays between its errno neighbors" {
...
test "errno 329 adjacent raw lanes keep value and pointer decoders precise" {
