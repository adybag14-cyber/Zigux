...
test "representative 98-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 954, 1996, 2998, 3996 };
    ...
    for (left_residues) |left_residue| {
        ...
        const center_payload = expectedRejectedPayload(left_residue + center_right_index - 1);
        for (0..(window_len / 2)) |pair_idx| {
            ...
            if (even_idx < center_right_index - 1) {
...
test "every interior ninetyeight window keeps consecutive codes and alternating constructor families" {
...
test "ninetyeight windows preserve a dual-center raw seam and a centered payload lane" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        const center_payload = expectedRejectedPayload(even_residue + center_right_index - 1);
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + center_right_index - 3) + 1,
            center_payload,
        );
        try testing.expectEqual(
            center_payload + 1,
            expectedRejectedPayload(even_residue + center_right_index + 1),
        );
        try testing.expectEqual(
