...
const errno: isize = -332;
const next_errno: isize = -333;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 332 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 332 raw equation stays between its errno neighbors" {
...
test "errno 332 adjacent raw lanes keep value and pointer decoders precise" {
