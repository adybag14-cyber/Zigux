...
test "representative 149-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2950, 3946 };
...
test "every interior onehundredfortynine window keeps consecutive codes and alternating constructor families" {
...
test "onehundredfortynine windows preserve centered equations around the middle err slot and the middle tagged err payload" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
