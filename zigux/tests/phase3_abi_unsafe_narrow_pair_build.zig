...
fn addPhase3AbiUnsafeNarrowPair(
    b: *std.Build,
    ...
    const abi_tests = b.addTest(.{
        .name = "phase3_abi_unsafe_narrow_pair_abi",
    ...
    const narrow_tests = b.addTest(.{
        .name = "phase3_abi_unsafe_narrow_pair_narrow",
        .root_module = narrow,
    });

    const abi_run = b.addRunArtifact(abi_tests);
    const narrow_run = b.addRunArtifact(narrow_tests);
    const pair = b.step(
        "phase3-abi-unsafe-narrow-pair-test",
        "Run the Phase 3 ABI binding contract beside the unsafe narrow packet",
    );
    pair.dependOn(&abi_run.step);
    pair.dependOn(&narrow_run.step);
    return pair;
}
...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = addPhase3AbiUnsafeNarrowPair(b, target, optimize);
    const test_step = b.step("test", "Run the Phase 3 ABI plus unsafe narrow pair-build shard");
