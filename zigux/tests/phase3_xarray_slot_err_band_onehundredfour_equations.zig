...
test "representative 104-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 946, 1988, 2990, 3990 };
...
test "every interior onehundredfour window keeps consecutive codes and alternating constructor families" {
...
test "onehundredfour windows preserve a dual-center raw seam and a paired payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        for (0..((center_left_payload_index / 2) + 1)) |pair_idx| {
