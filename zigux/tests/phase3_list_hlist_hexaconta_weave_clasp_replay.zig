...
const staged_break_index = 40;
const route = [_]usize{
    59, 0,  34, 33, 58, 1,  35, 32, 57, 2,  36, 31, 56, 3,
    37, 30, 55, 4,  38, 29, 54, 5,  39, 28, 53, 6,  40, 27,
    52, 7,  41, 26, 51, 8,  42, 25, 50, 9,  43, 24, 49, 10,
    44, 23, 48, 11, 45, 22, 47, 12, 46, 21, 20, 13, 19, 18,
    17, 16, 15, 14,
};
...
test "hexaconta weave-clasp list route exposes staged backlink repair" {
...
test "hexaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
