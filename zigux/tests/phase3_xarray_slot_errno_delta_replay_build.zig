...
pub fn build(b: *std.Build) void {
    ...
    const test_mod = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_errno_delta_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const test_step = b.step(
        "phase3-xarray-slot-errno-delta-replay",
        "Run the Lane 29 xarray slot errno-delta replay",
    );
    test_step.dependOn(&run_tests.step);

    const alias_step = b.step("test", "Run the Lane 29 xarray slot errno-delta replay");
