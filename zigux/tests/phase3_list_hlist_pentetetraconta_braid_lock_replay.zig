...
const visible_count = 45;
const detached_index = visible_count;
const route = [_]usize{
    44, 0,  22, 21, 43, 1,  42, 2,  20, 23, 3,  41, 19, 24, 4,
    40, 18, 25, 5,  39, 17, 26, 6,  38, 16, 27, 7,  37, 15, 28,
    8,  36, 14, 29, 9,  35, 13, 30, 10, 34, 12, 31, 11, 33, 32,
};
...
test "pentetetraconta braid-lock exposes the same visible route through list and hlist" {
...
test "pentetetraconta braid-lock repairs staged reverse links at the braid gate" {
    var entries = [_]Entry{.{}} ** (visible_count + 1);
    var head = ListHead{ .next = 0, .prev = 0 };
    var hhead = HListHead{ .first = 0 };

    reset(&entries, &head, &hhead);
    const lock_position = 29;
    const lock_index = route[lock_position];

    entries[lock_index].list.prev = @intFromPtr(&head);
    entries[lock_index].hlist.pprev = @intFromPtr(&hhead.first);

    const list_break = ListView.init(&head).firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, lock_position), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[lock_position - 1]].list)), list_break.expected_prev);
    ...
    try std.testing.expectEqual(@as(usize, lock_position), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[lock_position - 1]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hhead.first)), hlist_break.actual_pprev);

    entries[lock_index].list.prev = @intFromPtr(&entries[route[lock_position - 1]].list);
    entries[lock_index].hlist.pprev = @intFromPtr(&entries[route[lock_position - 1]].hlist.next);
