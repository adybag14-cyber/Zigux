...
test "closure note keeps genksyms CRC evidence materialized beside wrapper bridge evidence" {
    ...
    try expectContains(closure, "python3 scripts/zigux/check-phase2-genksyms-dual-implementation-survey.py --self-test");
    try expectContains(closure, "python3 scripts/zigux/check-phase2-genksyms-dual-implementation-survey.py");
...
test "dedicated survey checker guards the restored CRC-side materialization markers" {
    ...
    try expectContains(checker, "scripts/zigux/check-genksyms-crc-diff.py");
