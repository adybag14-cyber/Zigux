...
const center_payload_offset = center_left_index;
...
fn buildRawWindow(left_residue: usize, raws: *[window_len]usize, slots: *[window_len]xarray_slot_view.SlotView) void {
...
test "representative 22-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3060, 4072 };
    ...
    for (left_residues) |left_residue| {
        ...
        const center_payload = expectedRejectedPayload(left_residue + center_payload_offset);
        for (0..(center_payload_offset / 2) + 1) |pair_idx| {
            const even_offset = pair_idx * 2;
            const payload = expectedRejectedPayload(left_residue + even_offset);
            try testing.expectEqual(raws[even_offset], (payload << 1) | xa_value.value_tag_mask);

            if (pair_idx > 0) {
                const previous_payload = expectedRejectedPayload(left_residue + even_offset - 2);
                try testing.expectEqual(previous_payload + 1, payload);
            }

            if (even_offset < center_payload_offset) {
                const mirrored_payload = expectedRejectedPayload(left_residue + ((window_len - 2) - even_offset));
...
test "every interior twentytwo window keeps consecutive codes and alternating constructor families" {
    ...
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        for (0..(center_payload_offset / 2) + 1) |payload_idx| {
...
test "twentytwo windows preserve centered equations across the middle seam pair" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum = expectedCode(even_residue + center_left_index) + expectedCode(even_residue + center_right_index);
        const seam_raw_sum = @as(u128, rawFromResidue(even_residue + center_left_index)) + @as(u128, rawFromResidue(even_residue + center_right_index));
        const center_payload = expectedRejectedPayload(even_residue + center_payload_offset);
        ...
        for (0..center_payload_offset / 2) |pair_idx| {
            const left_even_residue = even_residue + (pair_idx * 2);
            const right_even_residue = even_residue + ((window_len - 2) - (pair_idx * 2));
            try testing.expectEqual(
                expectedRejectedPayload(left_even_residue) + expectedRejectedPayload(right_even_residue),
                center_payload * 2,
            );
        }

        try testing.expectEqual(
            expectedRejectedPayload(even_residue) + center_payload_offset,
            expectedRejectedPayload(even_residue + (window_len - 2)),
        );
    }
}
