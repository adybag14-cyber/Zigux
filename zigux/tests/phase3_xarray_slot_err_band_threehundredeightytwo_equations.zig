...
const errno: isize = -382;
const next_errno: isize = -383;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 382 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 382 raw equation stays between its errno neighbors" {
...
test "errno 382 adjacent raw lanes keep value and pointer decoders precise" {
