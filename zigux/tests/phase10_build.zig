...
pub fn build(b: *std.Build) void {
    ...
    const virtio_input_registration_blocker_module = b.createModule(.{
    ...
    const phase10_virtio_input_registration_blocker_module = b.createModule(.{
        .root_source_file = b.path("phase10_virtio_input_registration_blocker.zig"),
        .target = target,
        .optimize = optimize,
    });
    phase10_virtio_input_registration_blocker_module.addImport(
        "virtio_input_registration_blocker",
        virtio_input_registration_blocker_module,
    );
    ...
    const phase10_virtio_input_registration_blocker_tests = b.addTest(.{
        .name = "phase10-virtio-input-registration-blocker-tests",
        .root_module = phase10_virtio_input_registration_blocker_module,
    });
    const run_phase10_virtio_input_registration_blocker_tests = b.addRunArtifact(phase10_virtio_input_registration_blocker_tests);
    ...
    const test_step = b.step("test", "Run Phase 10 virtio core, virtio core survey, virtio ring, virtio ring reset reuse, virtio input, virtio input multitouch preflight, virtio input registration blocker, virtio mmio, virtio mmio queue isolation, virtio_input survey, virtio_ring survey, and virtio_mmio survey tests");
    ...
    test_step.dependOn(&run_phase10_virtio_input_registration_blocker_tests.step);
