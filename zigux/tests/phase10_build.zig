...
pub fn build(b: *std.Build) void {
    ...
    const phase10_virtio_net_module = b.createModule(.{
        .root_source_file = b.path("phase10_virtio_net.zig"),
        .target = target,
        .optimize = optimize,
    });
    phase10_virtio_net_module.addImport("virtio_net", virtio_net_module);
    ...
    const phase10_virtio_net_tests = b.addTest(.{
        .name = "phase10-virtio-net-tests",
        .root_module = phase10_virtio_net_module,
    });
    const run_phase10_virtio_net_tests = b.addRunArtifact(phase10_virtio_net_tests);
    ...
    const test_step = b.step("test", "Run Phase 10 virtio core, virtio ring, virtio input, virtio net, virtio_input survey, virtio_ring survey, and virtio_mmio survey tests");
    ...
    test_step.dependOn(&run_phase10_virtio_net_tests.step);
