...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_wrapped_low_mirror_pairs.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-xarray-slot-wrapped-low-mirror-pairs",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase3-xarray-slot-wrapped-low-mirror-pairs",
        "Run the focused Lane 29 wrapped-low mirror-pairs replay",
    );
