...
pub fn build(b: *std.Build) void {
    ...
    const phase3_errptr = addPhase3ErrptrXarrayStarterPacket(b, target, optimize);
    const phase3_abi = addPhase3AbiCorePacket(b, target, optimize);

    const phase3_step = b.step(
        "phase3-errptr-abi-test",
        "Run the shared Phase 3 err_ptr/xarray starter and ABI core packets together from zigux/tests",
    );
    phase3_step.dependOn(&phase3_errptr.step);
    phase3_step.dependOn(&phase3_abi.step);
}
