...
const errno: isize = -376;
const next_errno: isize = -377;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 376 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 376 raw equation stays between its errno neighbors" {
...
test "errno 376 adjacent raw lanes keep value and pointer decoders precise" {
