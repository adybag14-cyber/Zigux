...
test "representative 90-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 960, 2002, 3004, 4004 };
...
test "every interior ninety window keeps consecutive codes and alternating constructor families" {
...
test "ninety windows preserve a dual-center raw seam and a centered payload lane" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
