...
const errno: isize = -360;
const next_errno: isize = -361;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 360 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 360 raw equation stays between its errno neighbors" {
...
test "errno 360 adjacent raw lanes keep value and pointer decoders precise" {
