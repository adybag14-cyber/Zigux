...
test "phase 5 kobject survey packet stays repo-local and keeps the shared review surfaces explicit" {
    ...
    const checklist_markers = [_][]const u8{
        ...
        "if the change updates the landed Phase 5 `kobject_example` sample packet, do the note, shared checklist text, and paired manifest-backed replays keep the initialized-but-not-registered zero-active-attributes boundary, the already-registered duplicate-registration and replay-restart rejection packet plus the still-usable bounded foo roundtrip afterward, the registered teardown reset plus post-`exit()` show-or-store rejection packet, `ownershipSummary()` lifecycle packet, unnamed attribute-group shape, shared `baz`/`bar` dispatch, and the `abandoned_before_registration` versus `tore_down_registered_attributes` exit split explicit instead of implying sysfs, `kernel_kobj`, uevents, or module-registration parity?",
