...
test "phase 5 kobject survey packet stays repo-local and keeps the shared review surfaces explicit" {
    ...
    const required_markers = [_][]const u8{
        ...
        "approved Zigux idioms reviewable and repeatable",
    ...
    const docs_root_markers = [_][]const u8{
        ...
        "runPreRegistrationBoundaryReplay() access boundary",
        "`ownershipSummary()` plus sample-owned `runOwnershipReplay()` lifecycle cues",
    ...
    const tests_root_markers = [_][]const u8{
        ...
        "zigux/tests/phase5_kobject_example.zig",
        "zigux/tests/phase5_kobject_example_survey.zig",
        "`runPreRegistrationBoundaryReplay()` explicit for the initialized-but-not-registered zero-active-attributes plus show-or-store rejection boundary",
        "`runRegisteredBoundaryReplay()` explicit for the already-registered duplicate-registration and replay-restart rejection packet plus the still-usable bounded foo roundtrip afterward",
        "`runInputValidationReplay()` explicit for the shared `baz`/`bar` dispatch plus parse-failure visibility while the sample stays registered",
        "`ownershipSummary()` plus sample-owned `runOwnershipReplay()` explicit for the lifecycle packet",
        "`runTeardownReplay()` explicit for the registered teardown reset plus post-`exit()` show-or-store rejection packet",
        "`abandoned_before_registration` versus `tore_down_registered_attributes` exit split",
    };
