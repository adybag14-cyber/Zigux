...
test "phase 1 artifact-diff checker keeps its published contract catalog visible" {
    ...
    try expectContains(checker, "cli_help_output");
    try expectContains(checker, "text_missing_both");
    try expectContains(checker, "json_invalid_both");
    try expectContains(checker, "json_mismatch_repeat");
...
test "phase 1 artifact-diff helper keeps stable outward line markers" {
    const helper = try readRepoFile("scripts/zigux/artifact_diff.py", 64 * 1024);
    defer std.testing.allocator.free(helper);

    try expectContains(helper, "compare_artifacts");
    try expectContains(helper, "canonical_json");
    try expectContains(helper, "sha256_digest");
