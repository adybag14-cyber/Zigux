...
const errno: isize = -431;
const next_errno: isize = -432;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 431 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 431 raw equation stays between its errno neighbors" {
...
test "errno 431 adjacent raw lanes keep value and pointer decoders precise" {
