...
const errno: isize = -417;
const next_errno: isize = -418;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 417 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 417 raw equation stays between its errno neighbors" {
...
test "errno 417 adjacent raw lanes keep value and pointer decoders precise" {
