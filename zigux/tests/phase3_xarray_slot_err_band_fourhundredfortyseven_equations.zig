...
const errno: isize = -447;
const next_errno: isize = -448;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 447 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 447 raw equation stays between its errno neighbors" {
...
test "errno 447 adjacent raw lanes keep value and pointer decoders precise" {
