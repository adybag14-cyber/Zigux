...
test "rim bridge keeps bitmap and cpumask summaries aligned under tail noise" {
    const capacity = (3 * bitmap_view.word_bits) + 5;
...
test "rim bridge peers intersect at the bridges without collapsing into subsets" {
    const capacity = (3 * bitmap_view.word_bits) + 5;
    const primary_bits = [_]usize{
        0,
        bitmap_view.word_bits - 2,
...
test "narrow rim bridge stays a bounded subset of the broader shape" {
    const capacity = (3 * bitmap_view.word_bits) + 5;
    const primary_bits = [_]usize{
        0,
        bitmap_view.word_bits - 2,
