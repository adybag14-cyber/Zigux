...
test "thirty-second anchors decode to the expected err band milestones" {
...
test "thirty-second anchors keep the err band's symmetric 127 then 128 cadence" {
    ...
    for (0..anchor_count - 1) |index| {
        ...
        const expected_gap: usize = if (index == 0 or index == anchor_count - 2) 127 else 128;
    ...
    try testing.expectEqual(@as(usize, 255), raws[2] - raws[0]);
    try testing.expectEqual(@as(usize, 511), raws[4] - raws[0]);
    try testing.expectEqual(@as(usize, 1023), raws[8] - raws[0]);
    try testing.expectEqual(@as(usize, 2047), raws[16] - raws[0]);
    try testing.expectEqual(@as(usize, 2047), raws[32] - raws[16]);
}

test "interior thirty-second anchors stay even err raws bracketed by rejected aliases" {
