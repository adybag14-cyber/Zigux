...
test "representative 100-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 950, 1992, 2994, 3994 };
...
test "every interior onehundred window keeps consecutive codes and alternating constructor families" {
...
test "onehundred windows preserve a dual-center raw seam and a paired payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
