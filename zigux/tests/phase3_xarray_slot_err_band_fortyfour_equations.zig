...
test "representative 44-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1006, 2038, 3052, 4050 };
...
test "every interior fortyfour window keeps consecutive codes and alternating constructor families" {
...
test "fortyfour windows preserve dual-center seam equations and centered payload pairs" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
