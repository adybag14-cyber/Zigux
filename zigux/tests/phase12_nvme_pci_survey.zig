...
test "phase12 nvme pci survey manifest keeps the bounded starter packet truthful" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest_json, "\"status\": \"starter_verifier_direct_test_manifest_and_survey_gate_present_shared_build_absent\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest_json, "\"status\": \"recovery_budget_summary_dedicated_direct_replay_present_throughput_gate_missing\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest_json, "\"status\": \"driver_local_slice_note_manifest_survey_note_and_survey_gate_present_shared_build_absent\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest_json, "\"status\": \"shared_build_absent_direct_replay_and_survey_standalone\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest_json, "\"status\": \"survey_present_dedicated_verify_and_survey_retained_shared_build_absent\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest_json, "\"status\": \"survey_present_packet_local_route_retained\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest_json, "`zigux/tests/phase12_build.zig` still stays scoped to the shared virtio_net packet and does not yet wire the NVMe direct replay") != null);
}
...
test "phase12 nvme pci survey note keeps the shared direct replay and packet-local survey split explicit" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "PHASE12_STATUS=starter_verifier_direct_replay_manifest_and_survey_gate_present_shared_build_absent") != null);
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "the shared `zigux/tests/phase12_build.zig` route still stays scoped to the shared `virtio_net` packet and does not yet wire the bounded NVMe direct replay into `phase12-smoke`, `phase12-test`, or `phase12`") != null);
    try std.testing.expect(std.mem.indexOf(u8, survey_note, "the direct replay and verifier shard remain on the dedicated `phase12-nvme-pci-direct-test` route") != null);
...
test "phase12 nvme pci reopen governance note keeps the shared direct replay bounded" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, reopen_note, "stays outside the shared `phase12-smoke`, `phase12-test`, and aggregate `phase12` route while keeping its direct replay and survey gate on dedicated reruns") != null);
    try std.testing.expect(std.mem.indexOf(u8, reopen_note, "make -C zigux phase12-nvme-pci-direct-test") != null);
    try std.testing.expect(std.mem.indexOf(u8, reopen_note, "packet-local survey gate") != null);
...
test "phase12 nvme pci survey gate keeps the shared build and make wrapper surface explicit" {
    ...
    try std.testing.expect(std.mem.count(u8, shared_build, "b.createModule(.{") == 11);
    try std.testing.expect(std.mem.count(u8, shared_build, ".addImport(") == 5);
    try std.testing.expect(std.mem.count(u8, shared_build, "b.addTest(.{") == 6);
    try std.testing.expect(std.mem.count(u8, shared_build, "b.addRunArtifact(") == 6);
    try std.testing.expect(std.mem.count(u8, shared_build, "smoke_step.dependOn(") == 6);
    try std.testing.expect(std.mem.count(u8, shared_build, "test_step.dependOn(") == 6);
