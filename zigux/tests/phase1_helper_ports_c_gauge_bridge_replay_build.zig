...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_gauge_bridge_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-c-gauge-bridge-replay",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase1-helper-ports-c-gauge-bridge-replay",
        "Run Lane 10 Phase 1 helper ports C gauge bridge replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run Lane 10 Phase 1 helper ports C gauge bridge replay");
