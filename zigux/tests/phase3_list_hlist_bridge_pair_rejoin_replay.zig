...
test "list view keeps the live bridge visible ahead of a detached rejoin pair" {
    ...
    var rejoin_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var rejoin_right = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    rejoin_left.next = @intFromPtr(&rejoin_right);
    rejoin_left.prev = @intFromPtr(&bridge_right);
    rejoin_right.next = @intFromPtr(&tail);
    rejoin_right.prev = @intFromPtr(&rejoin_left);
...
test "list view reports the adopted rejoin pair once the reused tail keeps the stale backlink" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var entry = list_view.ListHead{ .next = 0, .prev = 0 };
    var bridge_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var bridge_right = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    bridge_left.next = @intFromPtr(&bridge_right);
    bridge_left.prev = @intFromPtr(&entry);
    bridge_right.next = @intFromPtr(&rejoin_left);
    bridge_right.prev = @intFromPtr(&bridge_left);
    tail.next = @intFromPtr(&head);
    tail.prev = @intFromPtr(&bridge_right);
    ...
    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &entry, &bridge_left, &bridge_right, &rejoin_left, &rejoin_right, &tail });

    const breakage = view.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 5), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&rejoin_right)), breakage.expected_prev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&bridge_right)), breakage.actual_prev);
    try std.testing.expect(!view.hasConsistentBacklinks());
}

test "hlist view keeps the live bridge visible ahead of a detached rejoin pair" {
    ...
    var rejoin_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var rejoin_right = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    rejoin_left.next = @intFromPtr(&rejoin_right);
    rejoin_left.pprev = @intFromPtr(&bridge_right.next);
    rejoin_right.next = @intFromPtr(&tail);
    rejoin_right.pprev = @intFromPtr(&rejoin_left.next);
...
test "hlist view reports the adopted rejoin pair once the reused tail keeps the stale prev-link" {
    var head = hlist_view.HListHead{ .first = 0 };
    var entry = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var bridge_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var bridge_right = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &entry, &bridge_left, &bridge_right, &rejoin_left, &rejoin_right, &tail });

    const breakage = view.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 5), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&rejoin_right.next)), breakage.expected_pprev);
