...
test "representative 34-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1020, 2042, 3058, 4060 };
...
test "thirtyfour windows preserve dual-center code seams and centered rejected-payload equations" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        for (0..(center_payload_index / 2) + 1) |pair_idx| {
        ...
        try testing.expectEqual(
            center_payload + (center_payload_index / 2),
