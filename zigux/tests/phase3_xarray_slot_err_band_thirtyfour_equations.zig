...
test "representative 34-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1020, 2044, 3058, 4060 };
    ...
    for (left_residues) |left_residue| {
        ...
        const center_payload = expectedRejectedPayload(left_residue + center_left_index);
...
test "every interior thirtyfour window keeps consecutive codes and alternating constructor families" {
...
test "thirtyfour windows preserve dual-center seam equations and paired rejected-payload sums" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        const center_payload = expectedRejectedPayload(even_residue + center_left_index);
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue) + (center_left_index / 2),
            center_payload,
        );
        try testing.expectEqual(
            center_payload + (center_left_index / 2),
