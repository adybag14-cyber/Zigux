...
test "representative 87-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 961, 2003, 3007, 4007 };
...
test "every interior eightyseven window keeps consecutive codes and alternating constructor families" {
...
test "eightyseven windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
