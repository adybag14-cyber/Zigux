...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_host_tools_smoke_list_sort_contract.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-host-tools-smoke-list-sort-contract",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const contract_step = b.step(
        "phase1-host-tools-smoke-list-sort-contract",
        "Guard the list_sort slice of the shared Phase 1 host-tools smoke harness",
    );
    contract_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the list_sort smoke contract");
