...
test "lane19 exact-buffer EOF replay preserves visible low control bytes before an embedded NUL while trimming trailing carriage returns" {
    var exact_then_control_nul_eof_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 9);
    defer exact_then_control_nul_eof_cr.deinit(std.testing.allocator);
    try exact_then_control_nul_eof_cr.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try exact_then_control_nul_eof_cr.append(std.testing.allocator, '\x08');
    try exact_then_control_nul_eof_cr.append(std.testing.allocator, '\x0c');
    try exact_then_control_nul_eof_cr.append(std.testing.allocator, '\x01');
    try exact_then_control_nul_eof_cr.append(std.testing.allocator, 'b');
    try exact_then_control_nul_eof_cr.append(std.testing.allocator, '\r');
    try exact_then_control_nul_eof_cr.append(std.testing.allocator, '\r');
    try exact_then_control_nul_eof_cr.append(std.testing.allocator, 0);
    try exact_then_control_nul_eof_cr.append(std.testing.allocator, 'c');

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try gen.runGenksymsCrc(exact_then_control_nul_eof_cr.items, &capture);

    const exact_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32(exact_then_control_nul_eof_cr.items[0..c_line_payload_len])},
    );
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_control_nul_eof_cr.items)});
