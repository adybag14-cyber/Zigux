...
pub fn build(b: *std.Build) void {
    ...
    const contract_module = b.createModule(.{
        .root_source_file = b.path("lane17_workflow_path_filter_contract.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const contract_tests = b.addTest(.{
        .name = "lane17-workflow-path-filter-contract",
        .root_module = contract_module,
    });

    const run_contract = b.addRunArtifact(contract_tests);
    const contract_step = b.step("lane17-workflow-path-filter-contract", "Run the Lane 17 workflow path-filter contract");
    contract_step.dependOn(&run_contract.step);

    const test_step = b.step("test", "Run the Lane 17 workflow path-filter contract");
