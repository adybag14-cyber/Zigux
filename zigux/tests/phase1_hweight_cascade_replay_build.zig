...
pub fn build(b: *std.Build) void {
    ...
    const replay_root = b.createModule(.{
        .root_source_file = b.path("phase1_hweight_cascade_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-hweight-cascade-replay",
        .root_module = replay_root,
    });

    const run_replay = b.addRunArtifact(replay_tests);
    const replay_step = b.step(
        "phase1-hweight-cascade-replay",
        "Run the Lane 08 hweight cascade replay",
    );
    replay_step.dependOn(&run_replay.step);

    const test_step = b.step("test", "Run the Lane 08 hweight cascade replay");
