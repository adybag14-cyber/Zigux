...
test "quadruple trailing empty banks keep early windows aligned and late banks empty" {
    ...
    try testing.expect(!cpumask.hasCpu((5 * bitmap_view.word_bits) + 7));
}

test "quadruple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (5 * bitmap_view.word_bits) + 11;
