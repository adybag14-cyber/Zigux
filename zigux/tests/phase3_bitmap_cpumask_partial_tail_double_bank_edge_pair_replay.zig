...
test "double-bank partial tail edge-pair keeps edge clear-bit discovery aligned" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    const words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        bit(1) | bit(2) | bit(3) | bit(8) | bit(13),
    };
...
test "double-bank partial tail edge-pair keeps subset and overlap blind to tail-only noise" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    const base_words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        bit(1) | bit(3) | bit(10) | bit(17),
    };
