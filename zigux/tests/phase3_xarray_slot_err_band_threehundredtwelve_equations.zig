...
const errno: isize = -312;
const next_errno: isize = -313;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 312 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 312 raw equation stays between its errno neighbors" {
...
test "errno 312 adjacent raw lanes keep value and pointer decoders precise" {
