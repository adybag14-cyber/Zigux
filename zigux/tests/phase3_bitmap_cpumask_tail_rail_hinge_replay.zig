...
test "tail rail hinge keeps shared-tail overlap aligned under a bounded union" {
    const capacity = bitmap_view.word_bits + 9;
    const tail_base = bitmap_view.word_bits;

    const union_words = [_]usize{
        std.math.maxInt(usize),
        bit(0) | bit(1) | bit(4) | bit(7) | bit(8),
    };
...
test "tail rail hinge keeps a hinge-only peer visible without leaking tail noise" {
    const capacity = bitmap_view.word_bits + 9;
    const tail_base = bitmap_view.word_bits;
    ...
    try testing.expect(!hinge_only.hasCpu(tail_base + 3));
