...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_shadow_ring_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-c-shadow-ring-replay-tests",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const shadow_ring_step = b.step(
        "phase1-helper-ports-c-shadow-ring-replay",
        "Run the Lane 10 helper ports C shadow-ring replay",
    );
    shadow_ring_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 10 helper ports C shadow-ring replay tests");
