...
const errno: isize = -388;
const next_errno: isize = -389;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 388 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 388 raw equation stays between its errno neighbors" {
...
test "errno 388 adjacent raw lanes keep value and pointer decoders precise" {
