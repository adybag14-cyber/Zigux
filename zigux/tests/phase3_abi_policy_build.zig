...
pub fn build(b: *std.Build) void {
    ...
    const phase3_abi_core = addPhase3AbiCorePacket(b, target, optimize);
    const phase3_policy = addPhase3PolicyStarterPacket(b, target, optimize);

    const phase3_step = b.step(
        "phase3-abi-policy-test",
        "Run the shared Phase 3 ABI core and policy packets together from zigux/tests",
    );
    phase3_step.dependOn(&phase3_abi_core.step);
    phase3_step.dependOn(&phase3_policy.step);

    const test_step = b.step("test", "Run the Phase 3 ABI core and policy packets together");
    test_step.dependOn(phase3_step);
    b.default_step.dependOn(phase3_step);
}
