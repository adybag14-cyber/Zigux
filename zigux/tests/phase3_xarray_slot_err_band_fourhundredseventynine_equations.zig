...
const errno: isize = -479;
const next_errno: isize = -480;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 479 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 479 raw equation stays between its errno neighbors" {
...
test "errno 479 adjacent raw lanes keep value and pointer decoders precise" {
