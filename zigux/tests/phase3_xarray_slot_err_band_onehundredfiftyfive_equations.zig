...
test "representative 155-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2950, 3940 };
...
test "every interior onehundredfiftyfive window keeps consecutive codes and alternating constructor families" {
...
test "onehundredfiftyfive windows preserve a centered raw seam and a split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
