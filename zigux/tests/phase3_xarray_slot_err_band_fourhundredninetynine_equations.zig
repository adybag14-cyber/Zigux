...
const errno: isize = -499;
const next_errno: isize = -500;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 499 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 499 raw equation stays between its errno neighbors" {
...
test "errno 499 adjacent raw lanes keep value and pointer decoders precise" {
