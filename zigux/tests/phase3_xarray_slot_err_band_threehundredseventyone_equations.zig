...
const errno: isize = -371;
const next_errno: isize = -372;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 371 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 371 raw equation stays between its errno neighbors" {
...
test "errno 371 adjacent raw lanes keep value and pointer decoders precise" {
