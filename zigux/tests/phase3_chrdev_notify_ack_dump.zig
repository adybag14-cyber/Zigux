...
fn writeSummary(writer: anytype, summary: abi.ChrdevNotifyAckSummary) !void {
...
pub fn main(init: std.process.Init) !void {
    ...
    try writer.writeAll("{\"constants\":{\"chrdev_notify_ack_status_none\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_STATUS_NONE});
    try writer.writeAll(",\"chrdev_notify_ack_status_acked\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_STATUS_ACKED});
    try writer.writeAll(",\"chrdev_notify_ack_status_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_STATUS_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_ack_status_expired\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_STATUS_EXPIRED});
    try writer.writeAll(",\"chrdev_notify_ack_status_skipped\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_STATUS_SKIPPED});
    try writer.writeAll(",\"chrdev_notify_ack_mask_issued\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_MASK_ISSUED});
    try writer.writeAll(",\"chrdev_notify_ack_mask_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_MASK_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_ack_mask_suppressed\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_MASK_SUPPRESSED});
    try writer.writeAll("}");
    try writer.writeAll(",\"acked\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_plan.summarize(acked_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"deferred\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_plan.summarize(deferred_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"expired\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_plan.summarize(expired_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"skipped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_plan.summarize(skipped_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"unmatched\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_plan.summarize(unmatched_view));
    try writer.writeAll("}");
    try writer.writeAll(",\"empty\":{\"is_valid\":");
    try writer.writeAll(if (chrdev_notify_ack_plan.isValid(empty_view)) "true" else "false");
    try writer.writeAll(",\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_plan.summarize(empty_view));
    try writer.writeAll("}}\n");
