...
test "representative 51-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 997, 2039, 3043, 4043 };
...
test "every interior fiftyone window keeps consecutive codes and alternating constructor families" {
...
test "fiftyone windows preserve centered equations around the middle rejected-alias slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
