...
const errno: isize = -425;
const next_errno: isize = -426;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 425 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 425 raw equation stays between its errno neighbors" {
...
test "errno 425 adjacent raw lanes keep value and pointer decoders precise" {
