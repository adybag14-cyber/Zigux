...
test "lane19 exact-buffer split replay preserves visible low control bytes while trimming trailing carriage returns before an embedded NUL and keeping the next line visible" {
    ...
    const exact_crc = try std.fmt.allocPrint(
        ...
        "0x{x:0>8}",
        .{gen.crc32(exact_then_control_then_cr_then_nul.items[0..c_line_payload_len])},
    );
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_control_then_cr_then_nul.items)});
