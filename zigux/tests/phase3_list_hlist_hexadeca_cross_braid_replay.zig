...
const route = [_]usize{ 15, 0, 12, 3, 10, 5, 8, 7, 9, 6, 11, 4, 13, 2, 14, 1 };
const detached_index = 16;

fn resetEntries(entries: *[17]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[17]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[17]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[17]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[17]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[17]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[17]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[17]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "hexadeca cross braid keeps forward list and hlist visibility before repair" {
    var entries: [17]Entry = undefined;
...
test "hexadeca cross braid staged repair passes the inner crossing" {
    var entries: [17]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 10);
    repairHListPrefix(&hlist_head, &entries, 10);

    const list_after_crossing = list_view.ListView.init(&list_head);
    const hlist_after_crossing = hlist_view.HListView.init(&hlist_head);
    try expectListRoute(list_after_crossing, &entries);
    try expectHListRoute(hlist_after_crossing, &entries);

    const list_break = list_after_crossing.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 10), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[9]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_crossing.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 10), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[9]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
