...
test "representative 79-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 969, 2011, 3015, 4015 };
...
test "every interior seventynine window keeps consecutive codes and alternating constructor families" {
...
test "seventynine windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
