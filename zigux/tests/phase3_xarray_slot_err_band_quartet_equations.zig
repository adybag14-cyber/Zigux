...
test "representative err-band quartets keep alias-err-alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1022, 2046, 3070, 4090 };
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [4]usize = undefined;
        var slots: [4]xarray_slot_view.SlotView = undefined;
        ...
        try testing.expectEqual(raws[0] + 3, raws[3]);

        try testing.expectEqual(
            @as(u128, raws[0]) + @as(u128, raws[3]),
            @as(u128, raws[1]) + @as(u128, raws[2]),
        );
...
test "every interior quartet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 3 <= err_band_span) : (even_residue += 2) {
...
test "quartet windows preserve pairwise code sums and rejected payload sums" {
    var even_residue: usize = 0;
    while (even_residue + 3 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[3], codes[1] + codes[2]);
        try testing.expectEqual(codes[0] + 3, codes[3]);
        try testing.expectEqual(first_payload + 1, third_payload);
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 3)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 2)),
        );
    }
}
