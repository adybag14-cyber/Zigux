...
pub fn build(b: *std.Build) void {
    ...
    const surface_dump_step = b.step(
        "phase3-abi-surface-dump-test",
        "Run the shared Phase 3 ABI core, export/UAPI layout, low-level wrapper, and dump packets through a standalone surface-plus-dump build shard",
    );
    surface_dump_step.dependOn(&phase3_abi_core_packet.step);
    surface_dump_step.dependOn(&phase3_export_uapi_layout.step);
    surface_dump_step.dependOn(&phase3_low_level_wrappers.step);
    surface_dump_step.dependOn(&phase3_abi_dump.step);
}
