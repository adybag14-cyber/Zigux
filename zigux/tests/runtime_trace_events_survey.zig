...
test "phase 9 runtime trace-events survey packet matches the current manifest and notes" {
    ...
    try std.testing.expectEqualStrings(
        "keep the trace-events survey note, module-slice note, manifest, direct module gate, the shipped `prepareSharedRequest`, `requestSharedRuntimeLoad`, and `releaseSharedWithoutSubstrate` bridge helpers, and the loader-substrate-drift replay aligned with the visible family-local packet while keeping the bounded pilot-module contract explicit and leaving the shared runtime-substrate blocker explicit",
