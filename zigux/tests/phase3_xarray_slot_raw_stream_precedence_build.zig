...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_raw_stream_precedence.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-xarray-slot-raw-stream-precedence",
        "Run the Phase 3 xarray-slot raw stream precedence replay",
    );
    replay_step.dependOn(&run_unit_tests.step);

    const test_step = b.step("test", "Run the default raw stream precedence replay");
