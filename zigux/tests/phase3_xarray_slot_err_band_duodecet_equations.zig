...
test "representative err-band duodecets keep alias-err spacing exact" {
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [12]usize = undefined;
        var slots: [12]xarray_slot_view.SlotView = undefined;
        ...
        const sum_011 = @as(u128, raws[0]) + @as(u128, raws[11]);
        const sum_110 = @as(u128, raws[1]) + @as(u128, raws[10]);
        const sum_29 = @as(u128, raws[2]) + @as(u128, raws[9]);
        const sum_38 = @as(u128, raws[3]) + @as(u128, raws[8]);
        const sum_47 = @as(u128, raws[4]) + @as(u128, raws[7]);
        const sum_56 = @as(u128, raws[5]) + @as(u128, raws[6]);
        try testing.expectEqual(sum_011, sum_110);
        try testing.expectEqual(sum_110, sum_29);
        try testing.expectEqual(sum_29, sum_38);
        try testing.expectEqual(sum_38, sum_47);
        try testing.expectEqual(sum_47, sum_56);
        ...
        try testing.expectEqual(first_payload + eleventh_payload, third_payload + ninth_payload);
        try testing.expectEqual(third_payload + ninth_payload, fifth_payload + seventh_payload);
...
test "every interior duodecet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 11 <= err_band_span) : (even_residue += 2) {
...
test "duodecet windows preserve centered equations across the middle seam" {
    var even_residue: usize = 0;
    while (even_residue + 11 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[11], codes[1] + codes[10]);
        try testing.expectEqual(codes[1] + codes[10], codes[2] + codes[9]);
        try testing.expectEqual(codes[2] + codes[9], codes[3] + codes[8]);
        try testing.expectEqual(codes[3] + codes[8], codes[4] + codes[7]);
        try testing.expectEqual(codes[4] + codes[7], codes[5] + codes[6]);
        try testing.expectEqual(first_payload + eleventh_payload, third_payload + ninth_payload);
        try testing.expectEqual(third_payload + ninth_payload, fifth_payload + seventh_payload);
        try testing.expectEqual(first_payload + 5, eleventh_payload);
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 11)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 10)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 10)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 9)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 9)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 8)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 8)),
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 7)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 7)),
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 6)),
        );
    }
}
