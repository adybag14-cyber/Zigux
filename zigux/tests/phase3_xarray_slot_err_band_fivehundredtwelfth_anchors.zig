...
test "five-hundred-twelfth anchors decode to the expected err band milestones" {
...
test "five-hundred-twelfth anchors keep the err band's symmetric 7 then 8 cadence" {
    ...
    for (0..anchor_count - 1) |index| {
        ...
        const expected_gap: usize = if (index == 0 or index == anchor_count - 2) 7 else 8;
    ...
    try testing.expectEqual(@as(usize, 15), raws[2] - raws[0]);
    try testing.expectEqual(@as(usize, 31), raws[4] - raws[0]);
    try testing.expectEqual(@as(usize, 63), raws[8] - raws[0]);
    try testing.expectEqual(@as(usize, 127), raws[16] - raws[0]);
    try testing.expectEqual(@as(usize, 255), raws[32] - raws[0]);
    try testing.expectEqual(@as(usize, 511), raws[64] - raws[0]);
    try testing.expectEqual(@as(usize, 1023), raws[128] - raws[0]);
    try testing.expectEqual(@as(usize, 2047), raws[256] - raws[0]);
    try testing.expectEqual(@as(usize, 2047), raws[512] - raws[256]);
}

test "interior five-hundred-twelfth anchors stay even err raws bracketed by rejected aliases" {
