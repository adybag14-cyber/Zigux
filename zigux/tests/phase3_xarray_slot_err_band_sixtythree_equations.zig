...
test "representative 63-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 985, 2027, 3031, 4031 };
...
test "every interior sixtythree window keeps consecutive codes and alternating constructor families" {
...
test "sixtythree windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
