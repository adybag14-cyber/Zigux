...
test "corridor gate keeps bitmap and cpumask summaries aligned under noisy tail storage" {
    const capacity = (6 * bitmap_view.word_bits) + 7;
    const corridor_gate_bits = [_]usize{
        0,
        2,
        bitmap_view.word_bits - 2,
        bitmap_view.word_bits + 1,
        bitmap_view.word_bits + 8,
        (2 * bitmap_view.word_bits) + 5,
        (3 * bitmap_view.word_bits) + 5,
        (4 * bitmap_view.word_bits) + 3,
        (4 * bitmap_view.word_bits) + 9,
        (5 * bitmap_view.word_bits) + 1,
        capacity - 1,
    };

    var words = [_]usize{ 0, 0, 0, 0, 0, 0, 0, 0 };
    fillPattern(words[0..], capacity, corridor_gate_bits[0..]);
    addNoise(words[0..], capacity);

    const bitmap = bitmap_view.BitmapView.init(words[0..], capacity);
    const cpumask = cpumask_view.CpuMaskView.init(words[0..], capacity);

    try testing.expectEqual(corridor_gate_bits.len, bitmap.countSetBits());
    ...
    try testing.expect(bitmap.isSet((3 * bitmap_view.word_bits) + 5));
...
test "peer corridor gate masks share hinges without collapsing into a subset" {
    const capacity = (6 * bitmap_view.word_bits) + 7;
    ...
    try testing.expect(base.hasCpu((5 * bitmap_view.word_bits) + 1));
    try testing.expect(!peer.hasCpu((5 * bitmap_view.word_bits) + 1));
    try testing.expect(!base.hasCpu((5 * bitmap_view.word_bits) + 4));
    try testing.expect(peer.hasCpu((5 * bitmap_view.word_bits) + 4));
}

test "inner corridor gate stays a bounded subset of the broader shape" {
    const capacity = (6 * bitmap_view.word_bits) + 7;
    const base_bits = [_]usize{
        0,
        2,
