...
const route = [_]usize{ 35, 0, 34, 1, 33, 2, 32, 3, 31, 4, 30, 5, 29, 6, 28, 7, 27, 8, 26, 9, 25, 10, 24, 11, 23, 12, 22, 13, 21, 14, 20, 15, 19, 16, 18, 17 };
const detached_index = 36;
...
test "hexatriaconta outer ladder keeps forward list and hlist visibility before repair" {
...
test "hexatriaconta outer ladder staged repair reaches the final rung" {
    var entries: [entry_count]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 34);
    repairHListPrefix(&hlist_head, &entries, 34);
    ...
    try std.testing.expectEqual(@as(usize, 34), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[33]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_ladder.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 34), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[33]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
