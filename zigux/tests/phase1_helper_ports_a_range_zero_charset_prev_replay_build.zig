...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_range_zero_charset_prev_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-range-zero-charset-prev-replay-tests",
        .root_module = replay_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const route = b.step(
        "phase1-helper-ports-a-range-zero-charset-prev-replay",
        "Run Lane 06 range/zero/charset/prev replay tests",
    );
    route.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run Lane 06 range/zero/charset/prev replay tests");
