...
const errno: isize = -353;
const next_errno: isize = -354;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 353 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 353 raw equation stays between its errno neighbors" {
...
test "errno 353 adjacent raw lanes keep value and pointer decoders precise" {
