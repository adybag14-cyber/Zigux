...
test "zipper channel keeps bitmap and cpumask summaries aligned under tail noise" {
    const capacity = (4 * bitmap_view.word_bits) + 7;
...
test "zipper channel peers intersect at shared rails without collapsing into subsets" {
    const capacity = (4 * bitmap_view.word_bits) + 7;
    const primary_bits = [_]usize{
        1,
        2,
    ...
    var primary_words = [_]usize{ 0, 0, 0, 0, 0, 0 };
...
test "narrow zipper channel stays a bounded subset of the broader shape" {
    const capacity = (4 * bitmap_view.word_bits) + 7;
    const primary_bits = [_]usize{
        1,
        2,
    ...
    var primary_words = [_]usize{ 0, 0, 0, 0, 0, 0 };
    var narrowed_words = [_]usize{ 0, 0, 0, 0, 0, 0 };
    fillPattern(primary_words[0..], capacity, primary_bits[0..]);
    ...
    try testing.expectEqual(@as(?usize, bitmap_view.word_bits - 1), narrowed_bitmap.firstSetBit());
    ...
    try testing.expect(!narrowed.hasCpu(1));
}
