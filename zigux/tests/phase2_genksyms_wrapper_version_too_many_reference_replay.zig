...
test "phase2 genksyms wrapper version fixture stays aligned before reference-limit failures" {
...
test "phase2 genksyms wrapper preserves mixed version counts before too many reference files" {
    try expectTooManyReferenceFailure(&.{ "--version", "--ver" }, 2);
    try expectTooManyReferenceFailure(&.{ "-V", "--version", "--ver" }, 3);
}

test "phase2 genksyms wrapper preserves abbreviated version counts before too many reference files" {
    try expectTooManyReferenceFailure(&.{"--ver"}, 1);
    try expectTooManyReferenceFailure(&.{ "--ver", "--ver" }, 2);
}
