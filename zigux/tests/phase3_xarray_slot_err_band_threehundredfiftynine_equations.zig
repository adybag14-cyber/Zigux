...
const errno: isize = -359;
const next_errno: isize = -360;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 359 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 359 raw equation stays between its errno neighbors" {
...
test "errno 359 adjacent raw lanes keep value and pointer decoders precise" {
