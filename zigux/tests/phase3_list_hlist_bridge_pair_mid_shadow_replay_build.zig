...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_bridge_pair_mid_shadow_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-list-hlist-bridge-pair-mid-shadow-replay",
        .root_module = root_module,
    });
    const run = b.addRunArtifact(tests);

    const step = b.step(
        "phase3-list-hlist-bridge-pair-mid-shadow-replay",
        "Run the Lane 28 list/hlist bridge-pair mid-shadow replay",
    );
