...
pub fn build(b: *std.Build) void {
    ...
    const errptr_xarray_starter = addPhase3ErrPtrXArrayStarterPacket(b, target, optimize);
    const low_level_wrappers = addPhase3LowLevelWrappers(b, target, optimize);

    const test_step = b.step(
        "phase3-errptr-xarray-low-level-test",
        "Run the focused Phase 3 err_ptr/xarray starter packet and low-level wrapper packet",
    );
    test_step.dependOn(&errptr_xarray_starter.step);
