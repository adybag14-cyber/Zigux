...
const errno: isize = -403;
const next_errno: isize = -404;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 403 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 403 raw equation stays between its errno neighbors" {
...
test "errno 403 adjacent raw lanes keep value and pointer decoders precise" {
