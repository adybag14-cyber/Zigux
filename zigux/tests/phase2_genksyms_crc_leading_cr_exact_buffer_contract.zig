...
fn collectNonEmptyRecords(allocator: std.mem.Allocator, input: []const u8) !std.ArrayList(Record) {
    ...
    while (nextCHarnessLineChunk(input, &cursor)) |line| {
        ...
        try records.append(allocator, .{
            ...
            .crc = crc32(line),
            .first = line[0],
            .second = if (line.len > 1) line[1] else 0,
...
test "phase2 genksyms crc keeps leading CR bytes in an exact visible record" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 1);
    defer input.deinit(std.testing.allocator);
    try input.appendSlice(std.testing.allocator, "\r\r");
    try input.appendNTimes(std.testing.allocator, 'v', c_line_payload_len - 2);
    ...
    try std.testing.expectEqual(@as(u8, '\r'), records.items[0].second);
    try std.testing.expectEqual(@as(u8, 'v'), records.items[0].last);
    try std.testing.expectEqual(crc32(input.items[0..c_line_payload_len]), records.items[0].crc);
    try std.testing.expect(!containsCrc(records.items, crc32(input.items[2..c_line_payload_len])));
}

test "phase2 genksyms crc trims only trailing CR bytes from a leading-CR exact record" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + "tail\n".len);
    defer input.deinit(std.testing.allocator);
    try input.appendSlice(std.testing.allocator, "\r\r");
    try input.appendNTimes(std.testing.allocator, 'x', c_line_payload_len - 4);
    try input.appendSlice(std.testing.allocator, "\r\r");
    ...
    const trimmed = input.items[0 .. c_line_payload_len - 2];
    try std.testing.expectEqual(@as(usize, 2), records.items.len);
    try std.testing.expectEqual(@as(usize, c_line_payload_len - 2), records.items[0].len);
    try std.testing.expectEqual(@as(u8, '\r'), records.items[0].first);
    try std.testing.expectEqual(@as(u8, '\r'), records.items[0].second);
    try std.testing.expectEqual(@as(u8, 'x'), records.items[0].last);
    try std.testing.expectEqual(crc32(trimmed), records.items[0].crc);
    ...
    try std.testing.expect(!containsCrc(records.items, crc32(input.items[2 .. c_line_payload_len - 2])));
}
