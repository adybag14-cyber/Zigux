...
test "phase 15 governance-lane sequencing manifest records the current direct packet" {
    ...
    try expectSliceContains(manifest.direct_packet_paths, "scripts\zigux/check_phase15_architecture_council_packet.zig");
    ...
    try expectSliceContains(manifest.direct_packet_paths, "scripts\zigux/check_phase15_handoff_note_alignment.zig");
...
test "phase 15 governance-lane sequencing note names the current packet and current gaps honestly" {
    ...
    try expectContains(sequencing_note, "the dedicated Architecture Council packet checker `scripts\zigux/check_phase15_architecture_council_packet.zig` is landed");
    ...
    try expectContains(sequencing_note, "the dedicated validator-first companion `scripts\zigux/validate_phase15.zig` is directly materialized");
    ...
    try std.testing.expect(std.mem.indexOf(u8, sequencing_note, "Current `master` still returns missing for `scripts\zigux/validate_phase15.zig`") == null);
}
...
test "phase 15 readiness manifest records the build companion and lane-owner replay as direct packet evidence" {
    ...
    try std.testing.expectEqualStrings("scripts\zigux/check_phase15_readiness_gate_packet.zig", readiness.readiness_packet_checker);
