...
test "representative 48-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1000, 2038, 3046, 4046 };
    ...
    for (left_residues) |left_residue| {
        ...
        for (0..window_len / 2) |pair_idx| {
            ...
            if (even_idx <= center_left_even_index) {
                ...
                try testing.expectEqual(payload + mirrored_payload, center_payload_sum);
            }
        }
    }
}

test "every interior fortyeight window keeps consecutive codes and alternating constructor families" {
...
test "fortyeight windows preserve dual-center seam equations and centered payload pairs" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        for (0..(center_left_even_index / 2) + 1) |pair_idx| {
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + center_left_even_index) + 1,
            expectedRejectedPayload(even_residue + center_right_even_index),
        );
        try testing.expectEqual(
