...
const errno: isize = -461;
const next_errno: isize = -462;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 461 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 461 raw equation stays between its errno neighbors" {
...
test "errno 461 adjacent raw lanes keep value and pointer decoders precise" {
