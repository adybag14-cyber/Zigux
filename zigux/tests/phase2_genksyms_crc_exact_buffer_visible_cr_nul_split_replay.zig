...
test "lane19 exact-buffer split replay preserves leading carriage returns before an embedded NUL while keeping the next line visible" {
    var exact_then_visible_cr_then_nul_then_next_line = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 8);
    ...
    try exact_then_visible_cr_then_nul_then_next_line.append(std.testing.allocator, 'c');
