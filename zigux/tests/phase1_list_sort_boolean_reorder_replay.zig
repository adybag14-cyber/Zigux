...
test "phase1 list_sort replay reuses boolean comparator across repeated reordering" {
    ...
    const boolean_cmp = struct {
        fn compare(priv: ?*anyopaque, a: *const list_sort.ListHead, b: *const list_sort.ListHead) i32 {
            ...
            return switch (mode.*) {
                .ascending => @intFromBool(lhs.key > rhs.key),
                .descending => @intFromBool(lhs.key < rhs.key),
            };
        }
    }.compare;
