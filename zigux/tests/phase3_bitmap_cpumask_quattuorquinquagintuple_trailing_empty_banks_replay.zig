...
fn replayWords() [tail_word_index + 1]usize {
    ...
    words[0] = (@as(usize, 1) << 2) | (@as(usize, 1) << 13);
...
test "lane27 quattuorquinquagintuple replay keeps bitmap discovery stable across 54 empty banks" {
    ...
    try testing.expectEqual(@as(?usize, 13), view.nextSetBit(3));
    try testing.expectEqual(@as(?usize, tail_base), view.nextSetBit(14));
    ...
    try testing.expectEqual(@as(?usize, 3), view.nextClearBit(3));
    try testing.expectEqual(@as(?usize, 14), view.nextClearBit(14));
    try testing.expectEqual(@as(?usize, tail_base + 1), view.nextClearBit(tail_base + 1));
}

test "lane27 quattuorquinquagintuple replay keeps cpumask traversal aligned with bitmap traversal" {
    const words = replayWords();
    ...
    try testing.expect(mask.hasCpu(2));
    ...
    try testing.expectEqual(bitmap.nextSetBit(14), mask.nextCpu(14));
...
test "lane27 quattuorquinquagintuple replay ignores tail-only noise in subset and overlap checks" {
    const base_words = replayWords();
    var superset_words = replayWords();
    var tail_noise_only_words = [_]usize{0} ** (tail_word_index + 1);
    superset_words[0] |= (@as(usize, 1) << 21);
