...
const errno: isize = -345;
const next_errno: isize = -346;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 345 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 345 raw equation stays between its errno neighbors" {
...
test "errno 345 adjacent raw lanes keep value and pointer decoders precise" {
