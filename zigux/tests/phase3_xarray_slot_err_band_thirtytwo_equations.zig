...
const first_right_even_index = center_right_index;
const last_left_even_index = center_left_index - 1;
...
test "representative 32-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1014, 2044, 3056, 4062 };
    ...
    for (left_residues) |left_residue| {
        ...
        const seam_payload_sum =
            expectedRejectedPayload(left_residue + last_left_even_index) +
            expectedRejectedPayload(left_residue + first_right_even_index);
        for (0..window_len / 2) |pair_idx| {
            ...
            if (even_idx < first_right_even_index) {
...
test "every interior thirtytwo window keeps consecutive codes and alternating constructor families" {
...
test "thirtytwo windows preserve twin-middle seam equations and rejected-payload pairing" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        const seam_payload_sum =
            expectedRejectedPayload(even_residue + last_left_even_index) +
            expectedRejectedPayload(even_residue + first_right_even_index);
