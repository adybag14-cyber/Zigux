...
test "representative 32-raw err-band windows keep alias-err spacing exact" {
    ...
    for (left_residues) |left_residue| {
        ...
        const seam_payload_sum =
            expectedRejectedPayload(left_residue + center_right_index - 2) +
        ...
        for (0..window_len / 2) |pair_idx| {
            ...
            if (even_idx < center_right_index) {
...
test "thirtytwo windows preserve dual-center seam equations and paired rejected-payload sums" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        const seam_payload_sum =
            expectedRejectedPayload(even_residue + center_right_index - 2) +
