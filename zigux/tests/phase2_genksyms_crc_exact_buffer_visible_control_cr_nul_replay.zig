...
test "lane19 exact-buffer split replay preserves visible low control bytes and leading carriage returns before an embedded NUL" {
    var exact_then_controls_and_leading_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 13);
