...
test "list view ignores detached return-path claims into the live route" {
    var head = ListHead{ .next = 0, .prev = 0 };
    var first = ListHead{ .next = 0, .prev = 0 };
    var middle = ListHead{ .next = 0, .prev = 0 };
    var tail = ListHead{ .next = 0, .prev = 0 };
    var detached_return = ListHead{ .next = 0, .prev = 0 };
    var detached_middle_shadow = ListHead{ .next = 0, .prev = 0 };
    ...
    // Detached nodes pretend the visible route returns from tail back into the
    // first and middle nodes, but the head-rooted walk must stay unchanged.
    detached_return.next = @intFromPtr(&first);
    detached_return.prev = @intFromPtr(&tail);
    detached_middle_shadow.next = @intFromPtr(&middle);
    detached_middle_shadow.prev = @intFromPtr(&detached_return);
...
test "hlist view ignores detached return-path claims into the live route" {
    var head = HListHead{ .first = 0 };
    var first = HListNode{ .next = 0, .pprev = 0 };
    var middle = HListNode{ .next = 0, .pprev = 0 };
    var tail = HListNode{ .next = 0, .pprev = 0 };
    var detached_return = HListNode{ .next = 0, .pprev = 0 };
    var detached_middle_shadow = HListNode{ .next = 0, .pprev = 0 };
    ...
    detached_return.next = @intFromPtr(&first);
    detached_return.pprev = @intFromPtr(&tail.next);
    detached_middle_shadow.next = @intFromPtr(&middle);
    detached_middle_shadow.pprev = @intFromPtr(&detached_return.next);
...
test "detached return-path claims preserve the live route's canonical links" {
    ...
    var list_detached_return = ListHead{ .next = 0, .prev = 0 };
    var list_detached_middle_shadow = ListHead{ .next = 0, .prev = 0 };
    ...
    list_detached_return.next = @intFromPtr(&list_first);
    list_detached_return.prev = @intFromPtr(&list_tail);
    list_detached_middle_shadow.next = @intFromPtr(&list_middle);
    list_detached_middle_shadow.prev = @intFromPtr(&list_detached_return);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&list_head)), list_first.prev);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&list_first)), list_detached_return.next);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&list_middle)), list_detached_middle_shadow.next);
    ...
    var hlist_detached_return = HListNode{ .next = 0, .pprev = 0 };
    var hlist_detached_middle_shadow = HListNode{ .next = 0, .pprev = 0 };
    ...
    hlist_detached_return.next = @intFromPtr(&hlist_first);
    hlist_detached_return.pprev = @intFromPtr(&hlist_tail.next);
    hlist_detached_middle_shadow.next = @intFromPtr(&hlist_middle);
    hlist_detached_middle_shadow.pprev = @intFromPtr(&hlist_detached_return.next);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hlist_head.first)), hlist_first.pprev);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hlist_first)), hlist_detached_return.next);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hlist_middle)), hlist_detached_middle_shadow.next);
}