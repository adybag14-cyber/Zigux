...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_zero_span_replace_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-zero-span-replace-replay",
        .root_module = root_module,
    });
    const run_replay_tests = b.addRunArtifact(replay_tests);

    const test_step = b.step(
        "phase1-helper-ports-a-zero-span-replace-replay",
        "Run focused Phase 1 helper ports A zero/span/replace replay",
    );
