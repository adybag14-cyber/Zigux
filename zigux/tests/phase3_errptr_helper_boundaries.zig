...
test "phase3 highest two xa_values stay contiguous below the pointer gap" {
    const next_value = xa_value.safe_inline_limit - 1;
    const highest_value = xa_value.safe_inline_limit;
    const next_raw = try xa_value.makeValue(next_value);
    const highest_raw = try xa_value.makeValue(highest_value);
    ...
    try std.testing.expectEqual(highest_raw + 1, gap_raw);
    try std.testing.expect(xa_value.isValue(next_raw));
    try std.testing.expect(xa_value.isValue(highest_raw));
    try std.testing.expectEqual(next_value, xa_value.toValue(next_raw));
    try std.testing.expectEqual(highest_value, xa_value.toValue(highest_raw));
    try std.testing.expect(err_ptr.isOkValue(gap_raw));
    try std.testing.expect(!xa_value.isValue(gap_raw));
}
