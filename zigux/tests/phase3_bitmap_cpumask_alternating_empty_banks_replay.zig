...
test "alternating empty banks keep scattered live windows and masked tail aligned" {
    const capacity = (4 * bitmap_view.word_bits) + 9;
    const words = [_]usize{
        bit(1) | bit(6) | bit(11),
        0,
        bit(0) | bit(5) | bit(9) | bit(13),
        0,
    ...
    try testing.expect(bitmap.isSet((2 * bitmap_view.word_bits) + 13));
    ...
    try testing.expect(cpumask.hasCpu((4 * bitmap_view.word_bits) + 2));
}

test "alternating empty banks keep subset and overlap checks bounded under masked tail noise" {
    const capacity = (4 * bitmap_view.word_bits) + 9;
