...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_prism_relay_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named_step = b.step(
        "phase1-helper-ports-c-prism-relay-replay",
        "Run Lane 10 Phase 1 helper ports C prism relay replay",
    );
    named_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run Lane 10 Phase 1 helper ports C prism relay replay");
