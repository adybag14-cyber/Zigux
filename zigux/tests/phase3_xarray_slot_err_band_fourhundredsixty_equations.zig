...
const errno: isize = -460;
const next_errno: isize = -461;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 460 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 460 raw equation stays between its errno neighbors" {
...
test "errno 460 adjacent raw lanes keep value and pointer decoders precise" {
