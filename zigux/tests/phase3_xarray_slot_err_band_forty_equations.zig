...
const payload_center_left_index = center_left_index - 1;
const payload_center_right_index = center_right_index;
...
test "representative 40-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1006, 2038, 3054, 4054 };
    ...
    for (left_residues) |left_residue| {
        ...
        const seam_payload_sum =
            expectedRejectedPayload(left_residue + payload_center_left_index) +
            expectedRejectedPayload(left_residue + payload_center_right_index);
        for (0..window_len / 2) |pair_idx| {
            ...
            if (even_idx < payload_center_right_index) {
...
test "every interior forty window keeps consecutive codes and alternating constructor families" {
...
test "forty windows preserve dual-center seam equations and dual-center payload pairs" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        const seam_payload_sum =
            expectedRejectedPayload(even_residue + payload_center_left_index) +
            expectedRejectedPayload(even_residue + payload_center_right_index);
        ...
        for (0..(payload_center_left_index / 2) + 1) |pair_idx| {
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue) + (payload_center_left_index / 2),
            expectedRejectedPayload(even_residue + payload_center_left_index),
        );
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + payload_center_left_index) + 1,
            expectedRejectedPayload(even_residue + payload_center_right_index),
        );
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + payload_center_right_index) + (payload_center_left_index / 2),
