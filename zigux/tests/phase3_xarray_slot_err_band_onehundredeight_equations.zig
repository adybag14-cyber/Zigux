...
const window_len = 108;
const center_left_index = (window_len / 2) - 1;
const center_right_index = window_len / 2;
const center_left_payload_index = center_right_index - 2;
const center_right_payload_index = center_right_index;
...
test "representative 108-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 4, 942, 1984, 2986, 3986 };
    ...
    for (left_residues) |left_residue| {
        ...
        for (0..(window_len / 2)) |pair_idx| {
            ...
            if (even_idx < center_left_payload_index) {
...
test "every interior onehundredeight window keeps consecutive codes and alternating constructor families" {
...
test "onehundredeight windows preserve a dual-center raw seam and a centered payload lane" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        const seam_payload_sum =
            expectedRejectedPayload(even_residue + center_left_payload_index) +
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + center_right_payload_index) + 1,
            expectedRejectedPayload(even_residue + center_right_payload_index + 2),
        );
        try testing.expectEqual(
