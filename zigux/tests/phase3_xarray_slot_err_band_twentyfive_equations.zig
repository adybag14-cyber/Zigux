...
test "representative 25-raw err-band windows keep alias-err spacing exact" {
...
test "every interior twentyfive window keeps consecutive codes and alternating constructor families" {
    ...
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        for (0..center_index + 1) |payload_idx| {
...
test "twentyfive windows preserve centered equations around the middle rejected-payload alias" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
