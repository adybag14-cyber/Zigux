...
test "representative 189-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 14, 944, 1982, 2964, 3906 };
...
test "every interior onehundredeightynine window keeps consecutive codes and alternating constructor families" {
...
test "onehundredeightynine windows preserve a centered raw seam and a centered payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
