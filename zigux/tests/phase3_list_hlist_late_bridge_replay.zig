...
test "phase3 list/hlist late bridge replay keeps the live list route visible while a detached late bridge stays off path" {
    ...
    live_left.next = @intFromPtr(&live_mid);
    live_left.prev = @intFromPtr(&first);
    live_mid.next = @intFromPtr(&live_right);
    live_mid.prev = @intFromPtr(&live_left);
    live_right.next = @intFromPtr(&tail);
    live_right.prev = @intFromPtr(&live_mid);
    ...
    try expectListSequence(view, &.{ &first, &live_left, &live_mid, &live_right, &tail });
    try std.testing.expect(view.hasConsistentBacklinks());
    try std.testing.expect(view.firstBrokenBacklink() == null);
}

test "phase3 list/hlist late bridge replay reports the first visible list break when the late bridge is adopted early" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_mid = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    live_mid.prev = @intFromPtr(&live_left);
    live_right.next = @intFromPtr(&tail);
    live_right.prev = @intFromPtr(&shadow_right);
    tail.next = @intFromPtr(&head);
    tail.prev = @intFromPtr(&live_right);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&live_mid)), breakage.expected_prev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&shadow_right)), breakage.actual_prev);
    try std.testing.expect(!view.hasConsistentBacklinks());
}

test "phase3 list/hlist late bridge replay keeps the live hlist route visible while a detached late bridge stays off path" {
    ...
    live_mid.next = @intFromPtr(&live_right);
    live_mid.pprev = @intFromPtr(&live_left.next);
    live_right.next = @intFromPtr(&tail);
    live_right.pprev = @intFromPtr(&live_mid.next);
    tail.next = 0;
    ...
    try expectHListSequence(view, &.{ &first, &live_left, &live_mid, &live_right, &tail });
    try std.testing.expect(view.firstPprevMatchesHead());
    try std.testing.expect(view.hasConsistentPrevLinks());
    try std.testing.expect(view.tailNextIsNull());
}

test "phase3 list/hlist late bridge replay reports the first visible hlist break when the late bridge is adopted early" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_mid = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    live_mid.pprev = @intFromPtr(&live_left.next);
    live_right.next = @intFromPtr(&tail);
    live_right.pprev = @intFromPtr(&shadow_right.next);
    tail.next = 0;
    tail.pprev = @intFromPtr(&live_right.next);
    ...
    try std.testing.expectEqual(@as(usize, 3), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&live_mid.next)), breakage.expected_pprev);
