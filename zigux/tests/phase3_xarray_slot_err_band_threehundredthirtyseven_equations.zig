...
const errno: isize = -337;
const next_errno: isize = -338;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 337 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 337 raw equation stays between its errno neighbors" {
...
test "errno 337 adjacent raw lanes keep value and pointer decoders precise" {
