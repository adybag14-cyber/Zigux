...
test "phase 7 string helpers survey keeps the roadmap and sample-root boundary explicit" {
    ...
    try expectContains(string_helpers_slice, "`kstrdup_quotable_cmdline()` through a bounded reusable cmdline-buffer core");
    ...
    try expectContains(string_helpers_tests, "phase 7 kstrdupQuotableCmdlineBuffer collapses argv-style NULs before escaping");
