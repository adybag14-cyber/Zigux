...
const errno: isize = -378;
const next_errno: isize = -379;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 378 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 378 raw equation stays between its errno neighbors" {
...
test "errno 378 adjacent raw lanes keep value and pointer decoders precise" {
