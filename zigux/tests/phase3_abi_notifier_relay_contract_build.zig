...
pub fn build(b: *std.Build) void {
    ...
    const contract_module = b.createModule(.{
        .root_source_file = b.path("phase3_abi_notifier_relay_contract.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const contract_tests = b.addTest(.{
        .name = "phase3_abi_notifier_relay_contract",
        .root_module = contract_module,
    });
    const run_contract_tests = b.addRunArtifact(contract_tests);

    const contract_step = b.step(
        "phase3-abi-notifier-relay-contract",
        "Run the Phase 3 ABI notifier/list/hlist relay contract",
    );
