...
test "representative 131-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 932, 1968, 2962, 3962 };
...
test "onehundredthirtyone windows preserve centered equations around the middle err slot and split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
