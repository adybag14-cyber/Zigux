...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_list_sort_rotated_multipass_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-list-sort-rotated-multipass-replay-tests",
    ...
    const replay_step = b.step(
        "phase1-list-sort-rotated-multipass-replay",
        "Run the Phase 1 list_sort rotated multipass replay",
    );
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run the Phase 1 list_sort rotated multipass replay");
