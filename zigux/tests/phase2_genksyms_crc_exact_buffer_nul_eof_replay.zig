...
test "lane19 exact-buffer EOF replay skips a NUL-prefixed continuation chunk" {
    ...
    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_nul_tail.items[0..c_line_payload_len])});
