...
const errno: isize = -489;
const next_errno: isize = -490;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 489 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 489 raw equation stays between its errno neighbors" {
...
test "errno 489 adjacent raw lanes keep value and pointer decoders precise" {
