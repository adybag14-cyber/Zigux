...
const route = [_]usize{ 10, 0, 8, 2, 6, 4, 9, 1, 7, 3, 5 };
const detached_index = 11;

fn resetEntries(entries: *[12]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[12]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[12]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[12]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[12]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[12]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[12]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[12]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "hendeca counter march preserves forward list and hlist visibility before repair" {
    var entries: [12]Entry = undefined;
...
test "hendeca counter march staged repair crosses the center hinge" {
    var entries: [12]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 6);
    repairHListPrefix(&hlist_head, &entries, 6);
    ...
    try std.testing.expectEqual(@as(usize, 6), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[5]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_prefix.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 6), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[5]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
