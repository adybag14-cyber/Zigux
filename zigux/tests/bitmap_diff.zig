...
test "bitmap diff survey keeps the current rounded fill drifts explicit against lib/test_bitmap.c and records the resolved 115-bit fill parity" {
    ...
    // Older Phase 4 validator markers stay quoted here for reviewability while the
    // current gate proves the rounded fill behavior now matches the Linux anchor.
