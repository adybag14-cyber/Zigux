...
test "double-bank partial tail double-gap keeps first missing cpu discovery aligned" {
    ...
    const first_missing = (bitmap_view.word_bits * 2) + 1;
    const second_missing = (bitmap_view.word_bits * 2) + 4;
...
test "double-bank partial tail double-gap keeps subset and overlap blind to invalid tail noise" {
    const capacity = (bitmap_view.word_bits * 2) + 7;
