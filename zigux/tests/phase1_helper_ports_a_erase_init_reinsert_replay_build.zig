...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_erase_init_reinsert_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-erase-init-reinsert-replay",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const replay_step = b.step("phase1-helper-ports-a-erase-init-reinsert-replay", "Run Lane 06 erase-init reinsert replay");
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run Lane 06 erase-init reinsert replay");
