...
test "representative 115-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 4, 936, 1978, 2980, 3980 };
...
test "every interior onehundredfifteen window keeps consecutive codes and alternating constructor families" {
...
test "onehundredfifteen windows preserve centered equations around the middle err-code slot" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
