...
pub fn build(b: *std.Build) void {
    ...
    const replay_root = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_staged_owner_swap_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-c-staged-owner-swap-replay-tests",
    ...
    const replay_step = b.step(
        "phase1-helper-ports-c-staged-owner-swap-replay",
        "Run Lane 10 Phase 1 helper ports C staged owner-swap replay",
    );
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run Lane 10 Phase 1 helper ports C staged owner-swap replay");
