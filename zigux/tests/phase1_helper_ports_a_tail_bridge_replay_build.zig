...
pub fn build(b: *std.Build) void {
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-tail-bridge-replay-tests",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase1-helper-ports-a-tail-bridge-replay",
        "Run the Lane 06 Phase 1 helper ports A tail bridge replay.",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 Phase 1 helper ports A tail bridge replay tests.");
