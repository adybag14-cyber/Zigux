...
test "representative 55-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 993, 2035, 3039, 4039 };
...
test "every interior fiftyfive window keeps consecutive codes and alternating constructor families" {
...
test "fiftyfive windows preserve centered equations around the middle rejected-alias slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
        const center_residue = odd_residue + center_index;
        const center_payload = expectedRejectedPayload(center_residue);

        try testing.expectEqual(rawFromResidue(center_residue), (center_payload << 1) | xa_value.value_tag_mask);
