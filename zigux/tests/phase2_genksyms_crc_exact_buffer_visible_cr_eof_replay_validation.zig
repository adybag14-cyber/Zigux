...
test "lane19 exact-buffer EOF replay preserves leading carriage returns in a continuation" {
    var exact_then_visible_leading_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 3);
    defer exact_then_visible_leading_cr.deinit(std.testing.allocator);
    try exact_then_visible_leading_cr.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try exact_then_visible_leading_cr.append(std.testing.allocator, '\r');
    try exact_then_visible_leading_cr.append(std.testing.allocator, '\r');
    try exact_then_visible_leading_cr.append(std.testing.allocator, 'b');

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try gen.runGenksymsCrc(exact_then_visible_leading_cr.items, &capture);

    const exact_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32(exact_then_visible_leading_cr.items[0..c_line_payload_len])},
    );
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_visible_leading_cr.items)});
