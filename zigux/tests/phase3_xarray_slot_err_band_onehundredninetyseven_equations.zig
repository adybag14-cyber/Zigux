...
test "representative 197-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 30, 960, 1998, 2978, 3898 };
        ...
        for (0..((window_len + 1) / 2)) |pair_idx| {
            ...
            if (even_idx < center_left_payload_index or even_idx > center_right_payload_index) {
                ...
                if ((mirrored_idx & 1) == 0) {
                    ...
                    try testing.expectEqual(payload + mirrored_payload, payload_sum);
                }
            }
        }
    }
...
test "every interior onehundredninetyseven window keeps consecutive codes and alternating constructor families" {
...
test "onehundredninetyseven windows preserve a centered raw seam and three-payload center" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
