...
const errno: isize = -495;
const next_errno: isize = -496;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 495 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 495 raw equation stays between its errno neighbors" {
...
test "errno 495 adjacent raw lanes keep value and pointer decoders precise" {
