...
const errno: isize = -405;
const next_errno: isize = -406;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 405 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 405 raw equation stays between its errno neighbors" {
...
test "errno 405 adjacent raw lanes keep value and pointer decoders precise" {
