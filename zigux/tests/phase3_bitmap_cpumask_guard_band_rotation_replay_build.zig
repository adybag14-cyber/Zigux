...
pub fn build(b: *std.Build) void {
    ...
    const tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("phase3_bitmap_cpumask_guard_band_rotation_replay.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    const test_step = b.step("phase3-bitmap-cpumask-guard-band-rotation-replay", "Run the Phase 3 bitmap/cpumask guard-band rotation replay");
    test_step.dependOn(&run_tests.step);

    const test_alias = b.step("test", "Run tests");
