...
const errno: isize = -396;
const next_errno: isize = -397;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 396 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 396 raw equation stays between its errno neighbors" {
...
test "errno 396 adjacent raw lanes keep value and pointer decoders precise" {
