...
test "representative err-band nonets keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3060, 4086 };
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [9]usize = undefined;
        var slots: [9]xarray_slot_view.SlotView = undefined;
        ...
        const sum_08 = @as(u128, raws[0]) + @as(u128, raws[8]);
        const sum_17 = @as(u128, raws[1]) + @as(u128, raws[7]);
        const sum_26 = @as(u128, raws[2]) + @as(u128, raws[6]);
        const sum_35 = @as(u128, raws[3]) + @as(u128, raws[5]);
        const doubled_middle = @as(u128, raws[4]) * 2;
        try testing.expectEqual(sum_08, sum_17);
        try testing.expectEqual(sum_17, sum_26);
        try testing.expectEqual(sum_26, sum_35);
        try testing.expectEqual(sum_35, doubled_middle);
...
test "every interior nonet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 8 <= err_band_span) : (even_residue += 2) {
...
test "nonet windows preserve centered equations around the middle err raw" {
    var even_residue: usize = 0;
    while (even_residue + 8 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[8], codes[1] + codes[7]);
        try testing.expectEqual(codes[1] + codes[7], codes[2] + codes[6]);
        try testing.expectEqual(codes[2] + codes[6], codes[3] + codes[5]);
        try testing.expectEqual(codes[3] + codes[5], codes[4] * 2);
        try testing.expectEqual(first_payload + ninth_payload, third_payload + seventh_payload);
        try testing.expectEqual(third_payload + seventh_payload, fifth_payload * 2);
        try testing.expectEqual(first_payload + 4, ninth_payload);
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 8)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 7)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 7)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 6)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 6)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 5)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 5)),
            @as(u128, rawFromResidue(even_residue + 4)) * 2,
        );
    }
}
