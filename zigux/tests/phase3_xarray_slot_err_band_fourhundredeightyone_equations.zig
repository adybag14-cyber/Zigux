...
const errno: isize = -481;
const next_errno: isize = -482;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 481 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 481 raw equation stays between its errno neighbors" {
...
test "errno 481 adjacent raw lanes keep value and pointer decoders precise" {
