...
test "triple window parity keeps three-word counts and first-gap discovery aligned" {
    const capacity = (bitmap_view.word_bits * 2) + 9;
    const words = [_]usize{
        bit(0) | bit(4) | bit(7),
        bit(1) | bit(3) | bit(8),
        bit(0) | bit(2) | bit(5) | bit(8) | bit(11),
    };
    ...
    try testing.expect(!cpumask.hasCpu((bitmap_view.word_bits * 2) + 7));
}

test "triple window parity keeps subset and overlap checks bounded under third-word tail noise" {
    const capacity = (bitmap_view.word_bits * 2) + 9;
    const base_words = [_]usize{
        bit(0) | bit(4) | bit(7),
        bit(1) | bit(3) | bit(8),
        bit(0) | bit(2) | bit(5) | bit(8),
    };
