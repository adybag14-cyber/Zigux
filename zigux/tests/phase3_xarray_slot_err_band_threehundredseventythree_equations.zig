...
const errno: isize = -373;
const next_errno: isize = -374;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 373 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 373 raw equation stays between its errno neighbors" {
...
test "errno 373 adjacent raw lanes keep value and pointer decoders precise" {
