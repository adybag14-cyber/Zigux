...
fn addContract(
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_host_tools_smoke_bitmap_range_render_contract.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-host-tools-smoke-bitmap-range-render-contract",
...
pub fn build(b: *std.Build) void {
    ...
    const contract_step = b.step(
        "phase1-host-tools-smoke-bitmap-range-render-contract",
        "Run the Phase 1 host-tools smoke bitmap range/render source contract",
    );
    contract_step.dependOn(&contract.step);

    const test_step = b.step("test", "Run the Phase 1 host-tools smoke bitmap range/render source contract");
