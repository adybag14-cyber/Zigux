...
test "representative 81-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 967, 2009, 3013, 4013 };
...
test "every interior eightyone window keeps consecutive codes and alternating constructor families" {
...
test "eightyone windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
