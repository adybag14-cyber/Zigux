...
test "starter packet keeps bitmap and cpumask summaries aligned for the same bounded words" {
    var backing = [_]usize{
        (@as(usize, 1) << 1) | (@as(usize, 1) << 4) | (@as(usize, 1) << 63),
        (@as(usize, 1) << 0) | (@as(usize, 1) << 7),
    };
    const nbits = bitmap_view.bits_per_word + 12;
    ...
    try testing.expectEqual(bitmap_view.testBit(bitmap, 1), cpumask_view.cpuIsSet(cpumask, 1));
    try testing.expectEqual(bitmap_view.testBit(bitmap, bitmap_view.bits_per_word + 7), cpumask_view.cpuIsSet(cpumask, bitmap_view.bits_per_word + 7));
...
test "starter packet keeps bitmap and cpumask all-clear bounded windows aligned" {
    var backing = [_]usize{0};
    const nbits: u32 = 16;
    const bitmap = bitmap_view.viewFromWords(backing[0..], nbits);
    const cpumask = cpumask_view.viewFromWords(backing[0..], nbits);
    ...
    try testing.expectEqual(bitmap_view.testBit(bitmap, nbits - 1), cpumask_view.cpuIsSet(cpumask, nbits - 1));
...
test "starter packet keeps bitmap and cpumask exact-word windows aligned" {
    ...
    const nbits = bitmap_view.bits_per_word * 2;
    const bitmap = bitmap_view.viewFromWords(backing[0..], nbits);
    const cpumask = cpumask_view.viewFromWords(backing[0..], nbits);
    const bitmap_summary = bitmap_view.summarize(bitmap);
    const cpumask_summary = cpumask_view.summarize(cpumask);
    ...
    try testing.expect(bitmap_view.isValid(bitmap));
    try testing.expect(cpumask_view.isValid(cpumask));
    try testing.expectEqual(~@as(usize, 0), bitmap_view.lastWordMask(nbits));
    try testing.expectEqual(bitmap_summary.first_set, cpumask_summary.first_set);
    try testing.expectEqual(bitmap_summary.first_zero, cpumask_summary.first_zero);
    try testing.expectEqual(bitmap_summary.weight, cpumask_summary.weight);
    try testing.expectEqual(bitmap_view.firstSet(bitmap), cpumask_view.firstCpu(cpumask));
    try testing.expectEqual(bitmap_view.firstZero(bitmap), cpumask_view.firstAbsentCpu(cpumask));
    try testing.expectEqual(bitmap_view.weight(bitmap), cpumask_view.weight(cpumask));
    try testing.expectEqual(bitmap_view.testBit(bitmap, bitmap_view.bits_per_word + 5), cpumask_view.cpuIsSet(cpumask, bitmap_view.bits_per_word + 5));
    try testing.expectEqual(bitmap_view.testBit(bitmap, bitmap_view.bits_per_word + 6), cpumask_view.cpuIsSet(cpumask, bitmap_view.bits_per_word + 6));
    try testing.expect(bitmap_view.testBit(bitmap, bitmap_view.bits_per_word + 5));
