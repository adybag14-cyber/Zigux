...
const errno: isize = -441;
const next_errno: isize = -442;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 441 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 441 raw equation stays between its errno neighbors" {
...
test "errno 441 adjacent raw lanes keep value and pointer decoders precise" {
