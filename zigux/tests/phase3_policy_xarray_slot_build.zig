...
pub fn build(b: *std.Build) void {
    ...
    const xarray_slot_tests = addPhase3XarraySlotStarterPacket(b, target, optimize);

    const lane_step = b.step(
        "phase3-policy-xarray-slot-test",
        "Run the Phase 3 policy and xarray-slot starter packets together",
    );
    lane_step.dependOn(&policy_tests.step);
    lane_step.dependOn(&xarray_slot_tests.step);

    const test_step = b.step("test", "Run the Phase 3 policy/xarray-slot standalone shard");
