...
const errno: isize = -448;
const next_errno: isize = -449;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 448 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 448 raw equation stays between its errno neighbors" {
...
test "errno 448 adjacent raw lanes keep value and pointer decoders precise" {
