...
const visible_count = 43;
const detached_index = visible_count;
const route = [_]usize{
    42, 0,  20, 22, 1,  41, 2,  40, 3,  39, 4,  38, 5,  37, 6,
    36, 7,  35, 8,  34, 9,  33, 10, 32, 11, 31, 12, 30, 13, 29,
    14, 28, 15, 27, 16, 26, 17, 25, 18, 24, 19, 23, 21,
};
...
test "tritetraconta cross-lace exposes the same visible route through list and hlist" {
...
test "tritetraconta cross-lace repairs staged reverse links after the crossing hinge" {
    var entries = [_]Entry{.{}} ** (visible_count + 1);
    var head = ListHead{ .next = 0, .prev = 0 };
    var hhead = HListHead{ .first = 0 };

    reset(&entries, &head, &hhead);
    const hinge_position = 24;
    const hinge_index = route[hinge_position];

    entries[hinge_index].list.prev = @intFromPtr(&head);
    entries[hinge_index].hlist.pprev = @intFromPtr(&hhead.first);

    const list_break = ListView.init(&head).firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, hinge_position), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[hinge_position - 1]].list)), list_break.expected_prev);
    ...
    try std.testing.expectEqual(@as(usize, hinge_position), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[hinge_position - 1]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hhead.first)), hlist_break.actual_pprev);

    entries[hinge_index].list.prev = @intFromPtr(&entries[route[hinge_position - 1]].list);
    entries[hinge_index].hlist.pprev = @intFromPtr(&entries[route[hinge_position - 1]].hlist.next);
