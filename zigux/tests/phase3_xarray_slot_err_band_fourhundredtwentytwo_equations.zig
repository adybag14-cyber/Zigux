...
const errno: isize = -422;
const next_errno: isize = -423;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 422 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 422 raw equation stays between its errno neighbors" {
...
test "errno 422 adjacent raw lanes keep value and pointer decoders precise" {
