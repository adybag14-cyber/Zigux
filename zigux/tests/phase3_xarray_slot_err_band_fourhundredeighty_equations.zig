...
const errno: isize = -480;
const next_errno: isize = -481;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 480 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 480 raw equation stays between its errno neighbors" {
...
test "errno 480 adjacent raw lanes keep value and pointer decoders precise" {
