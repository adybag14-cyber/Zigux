...
test "list view witnesses split-rotate adoption before backlink repair" {
    ...
    try expectListRoute(view, &.{ &four, &six, &two, &one, &five, &three });
...
test "hlist view witnesses split-rotate adoption before prev-link repair" {
    ...
    try expectHListRoute(view, &.{ &four, &six, &two, &one, &five, &three });
