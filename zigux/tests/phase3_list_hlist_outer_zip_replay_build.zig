...
fn addReplayTest(
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_outer_zip_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-list-hlist-outer-zip-replay",
...
pub fn build(b: *std.Build) void {
    ...
    const replay_step = b.step(
        "phase3-list-hlist-outer-zip-replay",
        "Run the Lane 28 Phase 3 list/hlist outer-zip replay.",
    );
    replay_step.dependOn(&replay.step);

    const test_step = b.step(
        "test",
        "Run the Lane 28 Phase 3 list/hlist outer-zip replay tests.",
    );
