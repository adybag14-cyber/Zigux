...
test "phase 8 exec-cmd focused replay falls back to cwd when logical PWD identity differs" {
    const config = exec_cmd.Config{
        .exec_name = "perf",
        .prefix = "/usr/libexec/perf-core",
        .exec_path = "libexec/perf-core",
    ...
    const pwd_identity = exec_cmd.FileIdentity{ .device = 9, .inode = 77 };
...
test "phase 8 exec-cmd deferred boundary note still matches the live C helper anchors" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, helper, "pub fn planDeferredExecvCallWithPwd(") != null);
