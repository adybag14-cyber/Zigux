...
fn expectContains(haystack: []const u8, needle: []const u8) !void { try std.testing.expect(std.mem.indexOf(u8, haystack, needle) != null); }
...
test "phase 8 exec-cmd module imports cleanly" { _ = exec_cmd; }
test "phase 8 exec-cmd focused replay keeps deferred path preparation and argv handoff aligned with exec-cmd.c" {
    ...
    var logical_env = exec_cmd.EnvMap.init(std.testing.allocator); defer logical_env.deinit();
    var logical_state = exec_cmd.ExecCmdState{}; defer logical_state.deinit(std.testing.allocator);
    ...
    var deferred_env = exec_cmd.EnvMap.init(std.testing.allocator); defer deferred_env.deinit();
    var deferred_state = exec_cmd.ExecCmdState{}; defer deferred_state.deinit(std.testing.allocator);
    ...
    var execl_env = exec_cmd.EnvMap.init(std.testing.allocator); defer execl_env.deinit();
    var execl_state = exec_cmd.ExecCmdState{}; defer execl_state.deinit(std.testing.allocator);
...
test "phase 8 exec-cmd focused replay keeps the empty-tail execl handoff explicit" {
    const config = exec_cmd.Config{ .exec_name = "perf", .prefix = "/usr/libexec/perf-core", .exec_path = "libexec/perf-core", .exec_path_env = "PERF_EXEC_PATH" };
    var env = exec_cmd.EnvMap.init(std.testing.allocator); defer env.deinit();
    var state = exec_cmd.ExecCmdState{}; defer state.deinit(std.testing.allocator);
...
test "phase 8 exec-cmd docs keep the validator alias and Phase 14 wording reviewable" {
    const slice_note = try readWorkspaceFile(std.testing.allocator, "Documentation/zigux/phase8-exec-cmd-slice.md", 32 * 1024); defer std.testing.allocator.free(slice_note);
    try expectContains(slice_note, "legacy validator alias: `PHASE8_SLICE=exec-cmd-tooling-starter`");
    const review_checklist = try readWorkspaceFile(std.testing.allocator, "Documentation/zigux/review-checklist.md", 64 * 1024); defer std.testing.allocator.free(review_checklist);
...
test "phase 8 exec-cmd build wiring keeps focused and shared gates explicit" {
    const focused_build = try readWorkspaceFile(std.testing.allocator, "zigux/tests/phase8_exec_cmd_only_build.zig", 16 * 1024); defer std.testing.allocator.free(focused_build);
    ...
    const shared_build = try readWorkspaceFile(std.testing.allocator, "zigux/tests/phase8_build.zig", 32 * 1024); defer std.testing.allocator.free(shared_build);
    ...
    const makefile = try readWorkspaceFile(std.testing.allocator, "zigux/Makefile", 32 * 1024); defer std.testing.allocator.free(makefile);
...
test "phase 8 exec-cmd evidence still matches the live C helper anchors" {
    const exec_cmd_c = try readWorkspaceFile(std.testing.allocator, "tools/lib/subcmd/exec-cmd.c", 32 * 1024); defer std.testing.allocator.free(exec_cmd_c);
