...
test "representative err-band undecets keep alias-err spacing exact" {
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [11]usize = undefined;
        var slots: [11]xarray_slot_view.SlotView = undefined;
        ...
        const sum_010 = @as(u128, raws[0]) + @as(u128, raws[10]);
        const sum_19 = @as(u128, raws[1]) + @as(u128, raws[9]);
        const sum_28 = @as(u128, raws[2]) + @as(u128, raws[8]);
        const sum_37 = @as(u128, raws[3]) + @as(u128, raws[7]);
        const sum_46 = @as(u128, raws[4]) + @as(u128, raws[6]);
        try testing.expectEqual(sum_010, sum_19);
        try testing.expectEqual(sum_19, sum_28);
        try testing.expectEqual(sum_28, sum_37);
        try testing.expectEqual(sum_37, sum_46);
        try testing.expectEqual(sum_46, @as(u128, raws[5]) * 2);
...
test "every interior undecet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 10 <= err_band_span) : (even_residue += 2) {
...
test "undecet windows preserve centered equations around the middle err slot" {
    var even_residue: usize = 0;
    while (even_residue + 10 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[10], codes[1] + codes[9]);
        try testing.expectEqual(codes[1] + codes[9], codes[2] + codes[8]);
        try testing.expectEqual(codes[2] + codes[8], codes[3] + codes[7]);
        try testing.expectEqual(codes[3] + codes[7], codes[4] + codes[6]);
        try testing.expectEqual(codes[4] + codes[6], codes[5] * 2);
        try testing.expectEqual(first_payload + eleventh_payload, third_payload + ninth_payload);
        try testing.expectEqual(third_payload + ninth_payload, fifth_payload + seventh_payload);
        try testing.expectEqual(first_payload + 5, eleventh_payload);
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 10)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 9)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 9)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 8)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 8)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 7)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 7)),
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 6)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 6)),
            @as(u128, rawFromResidue(even_residue + 5)) * 2,
        );
    }
}
