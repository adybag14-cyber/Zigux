...
test "representative err-band octodecets keep alias-err spacing exact" {
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [18]usize = undefined;
        var slots: [18]xarray_slot_view.SlotView = undefined;
        ...
        const sum_017 = @as(u128, raws[0]) + @as(u128, raws[17]);
        const sum_116 = @as(u128, raws[1]) + @as(u128, raws[16]);
        const sum_215 = @as(u128, raws[2]) + @as(u128, raws[15]);
        const sum_314 = @as(u128, raws[3]) + @as(u128, raws[14]);
        const sum_413 = @as(u128, raws[4]) + @as(u128, raws[13]);
        const sum_512 = @as(u128, raws[5]) + @as(u128, raws[12]);
        const sum_611 = @as(u128, raws[6]) + @as(u128, raws[11]);
        const sum_710 = @as(u128, raws[7]) + @as(u128, raws[10]);
        const sum_89 = @as(u128, raws[8]) + @as(u128, raws[9]);
        try testing.expectEqual(sum_017, sum_116);
        try testing.expectEqual(sum_116, sum_215);
        try testing.expectEqual(sum_215, sum_314);
        try testing.expectEqual(sum_314, sum_413);
        try testing.expectEqual(sum_413, sum_512);
        try testing.expectEqual(sum_512, sum_611);
        try testing.expectEqual(sum_611, sum_710);
        try testing.expectEqual(sum_710, sum_89);
...
test "every interior octodecet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 17 <= err_band_span) : (even_residue += 2) {
...
test "octodecet windows preserve centered equations across the middle seam" {
    var even_residue: usize = 0;
    while (even_residue + 17 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[17], codes[1] + codes[16]);
        try testing.expectEqual(codes[1] + codes[16], codes[2] + codes[15]);
        try testing.expectEqual(codes[2] + codes[15], codes[3] + codes[14]);
        try testing.expectEqual(codes[3] + codes[14], codes[4] + codes[13]);
        try testing.expectEqual(codes[4] + codes[13], codes[5] + codes[12]);
        try testing.expectEqual(codes[5] + codes[12], codes[6] + codes[11]);
        try testing.expectEqual(codes[6] + codes[11], codes[7] + codes[10]);
        try testing.expectEqual(codes[7] + codes[10], codes[8] + codes[9]);
        try testing.expectEqual(first_payload + seventeenth_payload, third_payload + fifteenth_payload);
        try testing.expectEqual(third_payload + fifteenth_payload, fifth_payload + thirteenth_payload);
        try testing.expectEqual(fifth_payload + thirteenth_payload, seventh_payload + eleventh_payload);
        try testing.expectEqual(seventh_payload + eleventh_payload, @as(usize, 2) * ninth_payload);
        ...
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 17)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 16)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 16)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 15)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 15)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 14)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 14)),
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 13)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 13)),
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 12)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 12)),
            @as(u128, rawFromResidue(even_residue + 6)) + @as(u128, rawFromResidue(even_residue + 11)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 6)) + @as(u128, rawFromResidue(even_residue + 11)),
            @as(u128, rawFromResidue(even_residue + 7)) + @as(u128, rawFromResidue(even_residue + 10)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 7)) + @as(u128, rawFromResidue(even_residue + 10)),
            @as(u128, rawFromResidue(even_residue + 8)) + @as(u128, rawFromResidue(even_residue + 9)),
        );
    }
}
