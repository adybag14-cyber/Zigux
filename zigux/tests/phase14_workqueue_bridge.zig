...
test "phase14 workqueue bridge descriptor stays at boundary-map posture" {
    ...
    try std.testing.expectEqual(@as(usize, 5), workqueue_bridge.WorkqueueBridgeLab.stayInCDecisionCount());
    ...
    try std.testing.expect(std.mem.indexOf(u8, workqueue_bridge.WorkqueueBridgeLab.nextAuditFocus(), "flush-drain active-color governance note") != null);
    try std.testing.expect(std.mem.indexOf(u8, workqueue_bridge.WorkqueueBridgeLab.nextAuditFocus(), "runtime max_active retuning boundary") != null);
    ...
    try std.testing.expectEqualStrings("dwork->cpu", audit.checkpoints[7].observed_fields[2]);
    ...
    try std.testing.expectEqualStrings("dwork->wq", audit.checkpoints[8].observed_fields[2]);
