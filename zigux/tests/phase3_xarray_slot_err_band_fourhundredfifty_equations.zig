...
const errno: isize = -450;
const next_errno: isize = -451;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 450 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 450 raw equation stays between its errno neighbors" {
...
test "errno 450 adjacent raw lanes keep value and pointer decoders precise" {
