...
const center_even_index = center_left_index;
...
test "representative 26-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3058, 4068 };
    ...
    for (left_residues) |left_residue| {
        ...
        const center_payload = expectedRejectedPayload(left_residue + center_even_index);
        try testing.expectEqual(raws[center_even_index], (center_payload << 1) | xa_value.value_tag_mask);

        for (0..(center_even_index / 2) + 1) |pair_idx| {
...
test "every interior twentysix window keeps consecutive codes and alternating constructor families" {
    ...
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        for (0..(center_even_index / 2) + 1) |payload_idx| {
...
test "twentysix windows preserve seam equations and the centered rejected-payload law" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        const center_payload = expectedRejectedPayload(even_residue + center_even_index);
        ...
        for (0..center_even_index / 2) |pair_idx| {
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue) + center_even_index,
