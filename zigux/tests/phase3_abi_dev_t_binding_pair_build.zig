...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = b.step(
        "phase3-abi-dev-t-binding-pair-test",
        "Run the Phase 3 ABI replay beside the dev_t binding tests",
    );
    pair_step.dependOn(&run_abi_replay_tests.step);
    pair_step.dependOn(&run_dev_t_binding_tests.step);

    const test_step = b.step("test", "Run the Phase 3 ABI dev_t binding pair tests");
