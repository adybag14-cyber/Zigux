...
const errno: isize = -364;
const next_errno: isize = -365;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 364 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 364 raw equation stays between its errno neighbors" {
...
test "errno 364 adjacent raw lanes keep value and pointer decoders precise" {
