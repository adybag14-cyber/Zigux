...
const staged_break_index = 38;
const route = [_]usize{
    57, 0,  32, 31, 56, 1,  33, 30, 55, 2,  34, 29, 54, 3,
    35, 28, 53, 4,  36, 27, 52, 5,  37, 26, 51, 6,  38, 25,
    50, 7,  39, 24, 49, 8,  40, 23, 48, 9,  41, 22, 47, 10,
    42, 21, 46, 11, 43, 20, 45, 12, 44, 19, 18, 13, 17, 16,
    15, 14,
};
...
test "octakaipentaconta weave-clasp list route exposes staged backlink repair" {
...
test "octakaipentaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
