...
const errno: isize = -434;
const next_errno: isize = -435;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 434 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 434 raw equation stays between its errno neighbors" {
...
test "errno 434 adjacent raw lanes keep value and pointer decoders precise" {
