...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_b_token_class_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-b-token-class-replay-tests",
    ...
    const replay_step = b.step(
        "phase1-helper-ports-b-token-class-replay",
        "Run the Phase 1 helper ports B token-class replay.",
    );
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run the focused Phase 1 helper ports B token-class replay.");
