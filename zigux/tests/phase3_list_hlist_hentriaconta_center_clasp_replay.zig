...
const route = [_]usize{ 30, 0, 28, 2, 26, 4, 24, 6, 22, 8, 20, 10, 18, 12, 16, 14, 15, 17, 13, 19, 11, 21, 9, 23, 7, 25, 5, 27, 3, 29, 1 };
const detached_index = 31;
...
test "hentriaconta center clasp keeps forward list and hlist visibility before repair" {
...
test "hentriaconta center clasp staged repair crosses the far hinge" {
    var entries: [entry_count]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 25);
    repairHListPrefix(&hlist_head, &entries, 25);
    ...
    try std.testing.expectEqual(@as(usize, 25), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[24]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 25), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[24]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
