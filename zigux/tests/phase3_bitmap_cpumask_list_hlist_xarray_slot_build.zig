...
pub fn build(b: *std.Build) void {
    ...
    const phase3_xarray_slot = addPhase3XarraySlotStarterPacket(b, target, optimize);

    const combined_step = b.step(
        "phase3-bitmap-cpumask-list-hlist-xarray-slot-test",
        "Run the shared Phase 3 bitmap/cpumask, list/hlist, and xarray-slot starter packets from zigux/tests",
    );
    combined_step.dependOn(&phase3_bitmap_cpumask.step);
    combined_step.dependOn(&phase3_list_hlist.step);
    combined_step.dependOn(&phase3_xarray_slot.step);

    const test_step = b.step(
        "test",
        "Run the Lane 04 Phase 3 bitmap/cpumask, list/hlist, and xarray-slot harness shard",
    );
