...
test "lane19 visible split replay preserves leading carriage returns while trimming trailing carriage returns before the next line" {
    ...
    try gen.runGenksymsCrc(split_then_visible_leading_and_trailing_crlf.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_leading_and_trailing_crlf.items[0..c_line_payload_len])});
