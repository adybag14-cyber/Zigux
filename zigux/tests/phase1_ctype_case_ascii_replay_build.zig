...
pub fn build(b: *std.Build) void {
    ...
    const replay_root_module = b.createModule(.{
        .root_source_file = b.path("phase1_ctype_case_ascii_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-ctype-case-ascii-replay-tests",
        .root_module = replay_root_module,
    });
    const run_replay_tests = b.addRunArtifact(replay_tests);
    const replay_step = b.step(
        "phase1-ctype-case-ascii-replay",
        "Run the Phase 1 ctype ASCII and case-helper replay",
    );
