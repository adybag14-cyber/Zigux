...
test "triple-bank partial tail last-present keeps only the final valid tail cpu visible" {
    ...
    try testing.expectEqual((bitmap_view.word_bits * 3) + 1, bitmap.countSetBits());
...
test "triple-bank partial tail last-present peers stay equivalent despite invalid extras" {
    const capacity = (bitmap_view.word_bits * 3) + 5;
    const last_valid = capacity - 1;
