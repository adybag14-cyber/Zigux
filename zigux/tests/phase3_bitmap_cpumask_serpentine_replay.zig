...
test "serpentine keeps bitmap and cpumask summaries aligned under noisy tail storage" {
    const capacity = (5 * bitmap_view.word_bits) + 11;
    const serpentine_bits = [_]usize{
        3,
        7,
    ...
    fillPattern(words[0..], capacity, serpentine_bits[0..]);
    addNoise(words[0..], capacity);

    const bitmap = bitmap_view.BitmapView.init(words[0..], capacity);
    const cpumask = cpumask_view.CpuMaskView.init(words[0..], capacity);

    try testing.expectEqual(serpentine_bits.len, bitmap.countSetBits());
...
test "peer serpentine overlaps on shared bends without collapsing into a subset" {
    const capacity = (5 * bitmap_view.word_bits) + 11;
...
test "inner serpentine stays a bounded subset of the broader path" {
    const capacity = (5 * bitmap_view.word_bits) + 11;
    const base_bits = [_]usize{
        3,
        7,
    ...
    try testing.expectEqual(@as(?usize, bitmap_view.word_bits - 5), inner_bitmap.firstSetBit());
    ...
    try testing.expect(inner.hasCpu(capacity - 6));
    try testing.expect(!inner.hasCpu(3));
}
