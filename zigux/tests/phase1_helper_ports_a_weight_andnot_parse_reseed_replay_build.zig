...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_weight_andnot_parse_reseed_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-weight-andnot-parse-reseed-replay",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(replay_tests);
    const replay_step = b.step(
        "phase1-helper-ports-a-weight-andnot-parse-reseed-replay",
        "Run the Lane 06 weighted/andnot/parse/reseed replay",
    );
