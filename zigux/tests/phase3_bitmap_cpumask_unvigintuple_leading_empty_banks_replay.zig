...
test "unvigintuple leading empty banks keep late windows aligned and masked tail bits bounded" {
    const capacity = (24 * word_bits) + 9;
    const words = [_]usize{0} ** 21 ++ [_]usize{
    ...
    try testing.expectEqual(@as(?usize, 21 * word_bits + 1), bitmap.firstSetBit());
    try testing.expectEqual(bitmap.firstSetBit(), cpumask.firstCpu());
    try testing.expectEqual(@as(?usize, 0), bitmap.firstClearBit());
    try testing.expectEqual(bitmap.firstClearBit(), cpumask.firstMissingCpu());

    try testing.expect(!bitmap.isSet(21 * word_bits + 0));
    try testing.expect(bitmap.isSet(21 * word_bits + 1));
    try testing.expect(bitmap.isSet(22 * word_bits + 15));
    try testing.expect(bitmap.isSet(23 * word_bits + 11));
    try testing.expect(!bitmap.isSet(24 * word_bits + 7));
    try testing.expect(bitmap.isSet(24 * word_bits + 8));
    try testing.expect(cpumask.hasCpu(21 * word_bits + 13));
    try testing.expect(!cpumask.hasCpu(22 * word_bits + 11));
    try testing.expect(cpumask.hasCpu(24 * word_bits + 4));
}

test "unvigintuple leading empty banks keep subset and overlap checks blind to invalid tail noise" {
    const capacity = (24 * word_bits) + 9;
    const base_words = [_]usize{0} ** 21 ++ [_]usize{
        bit(1) | bit(4) | bit(9) | bit(13),
        bit(0) | bit(6) | bit(10) | bit(15),
        bit(2) | bit(5) | bit(11),
        bit(0) | bit(4) | bit(8) | invalidTailNoise(9),
    };
    const subset_words = [_]usize{0} ** 21 ++ [_]usize{
        bit(4) | bit(13),
        bit(6) | bit(15),
        bit(5) | bit(11),
        bit(4) | bit(8) | invalidTailNoise(9),
    };
    const overlap_words = [_]usize{0} ** 21 ++ [_]usize{
        bit(0),
        bit(10),
        bit(5),
        bit(8) | invalidTailNoise(9),
    };
    const disjoint_words = [_]usize{0} ** 21 ++ [_]usize{
        bit(0) | bit(2),
        bit(1) | bit(5),
        bit(0) | bit(7),
        invalidTailNoise(9),
    };
    const tail_only_words = [_]usize{0} ** 24 ++ [_]usize{
        invalidTailNoise(9),
    };
