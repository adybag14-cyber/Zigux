...
const center_left_even_index = center_index - 1;
const center_right_even_index = center_index + 1;
...
test "representative 31-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3058, 4064 };
    ...
    for (left_residues) |left_residue| {
        ...
        const seam_payload_sum =
            expectedRejectedPayload(left_residue + center_left_even_index) +
            expectedRejectedPayload(left_residue + center_right_even_index);
        for (0..(center_right_even_index / 2) + 1) |pair_idx| {
            ...
            if (even_idx <= center_left_even_index) {
                const mirrored_payload =
                    expectedRejectedPayload(left_residue + ((window_len - 1) - even_idx));
...
test "every interior thirtyone window keeps consecutive codes and alternating constructor families" {
    ...
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        ...
        for (0..(center_right_even_index / 2) + 1) |payload_idx| {
...
test "thirtyone windows preserve centered err equations and the interior rejected-payload seam pair" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const center_raw = rawFromResidue(even_residue + center_index);
        const center_code = expectedCode(even_residue + center_index);
        const seam_payload_sum =
            expectedRejectedPayload(even_residue + center_left_even_index) +
            expectedRejectedPayload(even_residue + center_right_even_index);
        ...
        for (0..center_right_even_index / 2) |pair_idx| {
            const left_even_residue = even_residue + (pair_idx * 2);
            const right_even_residue = even_residue + ((window_len - 1) - (pair_idx * 2));
            try testing.expectEqual(
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue) + center_index,
            expectedRejectedPayload(even_residue + (window_len - 1)),
        );
    }
}
