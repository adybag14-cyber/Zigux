...
test "representative 31-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3056, 4064 };
    ...
    for (left_residues) |left_residue| {
        ...
        try testing.expectEqual(@as(usize, 0), raws[center_index] & xa_value.value_tag_mask);
        ...
        for (0..(center_index / 2) + 1) |pair_idx| {
            ...
            try testing.expectEqual(
                ...
                expectedRejectedPayload(left_residue + center_index - 1) +
                    expectedRejectedPayload(left_residue + center_index + 1),
            );
        }
    }
...
test "thirtyone windows preserve centered odd-code equations and paired rejected-payload seams" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const center_residue = even_residue + center_index;
        const center_code = expectedCode(center_residue);
        const center_raw = rawFromResidue(center_residue);
        const seam_payload_sum =
            expectedRejectedPayload(center_residue - 1) +
            expectedRejectedPayload(center_residue + 1);

        try testing.expectEqual(@as(usize, 0), center_raw & xa_value.value_tag_mask);
        ...
        try testing.expectEqual(
            expectedRejectedPayload(center_residue - 1) + 1,
            expectedRejectedPayload(center_residue + 1),
        );
    }
}
