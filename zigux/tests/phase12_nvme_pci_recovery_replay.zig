...
test "phase12 nvme pci recovery replay keeps default descriptor fields inert" {
...
test "phase12 nvme pci recovery replay reports explicit descriptor rebuild demand" {
    var lab = try nvme_pci.NvmePciQueueLab.init(4096, 8);
...
test "phase12 nvme pci recovery replay clears explicit descriptor rebuild after refresh" {
    var lab = try nvme_pci.NvmePciQueueLab.init(4096, 8);
    ...
    try std.testing.expect(!refreshed.descriptor_rebuild_required);
    try std.testing.expectEqual(@as(u32, 0), refreshed.descriptor_rebuild_dma_bytes);
