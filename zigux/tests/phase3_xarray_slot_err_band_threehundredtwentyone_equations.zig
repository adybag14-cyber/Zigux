...
const errno: isize = -321;
const next_errno: isize = -322;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 321 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 321 raw equation stays between its errno neighbors" {
...
test "errno 321 adjacent raw lanes keep value and pointer decoders precise" {
