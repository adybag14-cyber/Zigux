...
test "representative 192-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 20, 950, 1988, 2968, 3902 };
...
test "every interior onehundredninetytwo window keeps consecutive codes and alternating constructor families" {
...
test "onehundredninetytwo windows preserve split raw and payload centers" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
