...
fn replayWords() [tail_word_index + 1]usize {
    ...
    words[0] = (@as(usize, 1) << 2) | (@as(usize, 1) << 43);
...
test "lane27 quattuorseptuagintuple replay keeps bitmap discovery stable across 74 empty banks" {
    ...
    try testing.expectEqual(@as(?usize, 43), view.nextSetBit(3));
    try testing.expectEqual(@as(?usize, tail_base), view.nextSetBit(44));
    ...
    try testing.expectEqual(@as(?usize, 44), view.nextClearBit(44));
    try testing.expectEqual(@as(?usize, tail_base + 1), view.nextClearBit(tail_base + 1));
}

test "lane27 quattuorseptuagintuple replay keeps cpumask traversal aligned with bitmap traversal" {
    const words = replayWords();
    ...
    try testing.expectEqual(bitmap.nextSetBit(44), mask.nextCpu(44));
...
test "lane27 quattuorseptuagintuple replay ignores tail-only noise in subset and overlap checks" {
    const base_words = replayWords();
    var superset_words = replayWords();
    var tail_noise_only_words = [_]usize{0} ** (tail_word_index + 1);
    superset_words[0] |= (@as(usize, 1) << 58);
