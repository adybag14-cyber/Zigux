...
test "duovigintuple trailing empty banks keep early windows aligned and masked tail bits bounded" {
    const capacity = (24 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        bit(1) | bit(4) | bit(9) | bit(13),
        bit(0) | bit(6) | bit(10) | bit(15),
    } ++ [_]usize{0} ** 22 ++ [_]usize{
        bit(0) | bit(4) | bit(10) | bit(12) | bit(15),
    ...
    try testing.expect(bitmap.isSet(bitmap_view.word_bits + 15));
    try testing.expect(!bitmap.isSet((2 * bitmap_view.word_bits) + 3));
    try testing.expect(bitmap.isSet((24 * bitmap_view.word_bits) + 4));
    try testing.expect(bitmap.isSet((24 * bitmap_view.word_bits) + 10));
    ...
    try testing.expect(cpumask.hasCpu((24 * bitmap_view.word_bits) + 0));
}

test "duovigintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (24 * bitmap_view.word_bits) + 11;
