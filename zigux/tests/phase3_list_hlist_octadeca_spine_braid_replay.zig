...
const route = [_]usize{ 17, 0, 15, 2, 13, 4, 11, 6, 9, 8, 10, 7, 12, 5, 14, 3, 16, 1 };
const detached_index = 18;

fn resetEntries(entries: *[19]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[19]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[19]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[19]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[19]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[19]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[19]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[19]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "octadeca spine braid keeps forward list and hlist visibility before repair" {
    var entries: [19]Entry = undefined;
...
test "octadeca spine braid staged repair crosses the center spine" {
    var entries: [19]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 12);
    repairHListPrefix(&hlist_head, &entries, 12);

    const list_after_spine = list_view.ListView.init(&list_head);
    const hlist_after_spine = hlist_view.HListView.init(&hlist_head);
    try expectListRoute(list_after_spine, &entries);
    try expectHListRoute(hlist_after_spine, &entries);

    const list_break = list_after_spine.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 12), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[11]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_spine.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 12), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[11]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
