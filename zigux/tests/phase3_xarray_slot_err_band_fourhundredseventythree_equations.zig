...
const errno: isize = -473;
const next_errno: isize = -474;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 473 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 473 raw equation stays between its errno neighbors" {
...
test "errno 473 adjacent raw lanes keep value and pointer decoders precise" {
