...
test "representative err-band septets keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1018, 2044, 3062, 4088 };
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [7]usize = undefined;
        var slots: [7]xarray_slot_view.SlotView = undefined;
        ...
        const sum_06 = @as(u128, raws[0]) + @as(u128, raws[6]);
        const sum_15 = @as(u128, raws[1]) + @as(u128, raws[5]);
        const sum_24 = @as(u128, raws[2]) + @as(u128, raws[4]);
        const doubled_middle = @as(u128, raws[3]) * 2;
        try testing.expectEqual(sum_06, sum_15);
        try testing.expectEqual(sum_15, sum_24);
        try testing.expectEqual(sum_24, doubled_middle);
...
test "every interior septet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 6 <= err_band_span) : (even_residue += 2) {
...
test "septet windows preserve centered equations around the middle err raw" {
    var even_residue: usize = 0;
    while (even_residue + 6 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[6], codes[1] + codes[5]);
        try testing.expectEqual(codes[1] + codes[5], codes[2] + codes[4]);
        try testing.expectEqual(codes[2] + codes[4], codes[3] * 2);
        ...
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 6)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 5)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 5)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 4)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 4)),
            @as(u128, rawFromResidue(even_residue + 3)) * 2,
        );
    }
}
