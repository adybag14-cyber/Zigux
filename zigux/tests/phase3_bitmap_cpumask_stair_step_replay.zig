...
test "stair step keeps bitmap and cpumask summaries aligned under tail noise" {
    const capacity = (5 * bitmap_view.word_bits) + 13;
    const primary_bits = [_]usize{
        ...
        (2 * bitmap_view.word_bits) + 7,
        (3 * bitmap_view.word_bits) + 3,
        (3 * bitmap_view.word_bits) + 9,
        (4 * bitmap_view.word_bits) + 4,
        (4 * bitmap_view.word_bits) + 11,
        5 * bitmap_view.word_bits,
        capacity - 2,
    };
    ...
    try testing.expect(bitmap.isSet((3 * bitmap_view.word_bits) + 9));
    try testing.expect(bitmap.isSet(capacity - 2));
    try testing.expect(cpumask.hasCpu(bitmap_view.word_bits + 5));
    try testing.expect(cpumask.hasCpu((4 * bitmap_view.word_bits) + 11));
    try testing.expect(!cpumask.hasCpu(1));
    try testing.expect(!cpumask.hasCpu((2 * bitmap_view.word_bits) + 6));
}

test "stair step peers intersect at shared risers without collapsing into subsets" {
    const capacity = (5 * bitmap_view.word_bits) + 13;
    const primary_bits = [_]usize{
        0,
        2,
    ...
    const peer_bits = [_]usize{
        ...
        (2 * bitmap_view.word_bits) + 2,
        (2 * bitmap_view.word_bits) + 8,
        (3 * bitmap_view.word_bits) + 3,
        (3 * bitmap_view.word_bits) + 10,
        (4 * bitmap_view.word_bits) + 4,
        (4 * bitmap_view.word_bits) + 12,
        capacity - 2,
    };
...
test "narrow stair step stays a bounded subset of the broader shape" {
    const capacity = (5 * bitmap_view.word_bits) + 13;
    const primary_bits = [_]usize{
        0,
        2,
    ...
    try testing.expectEqual(@as(?usize, bitmap_view.word_bits + 5), narrowed_bitmap.firstSetBit());
