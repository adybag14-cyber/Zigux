...
const errno: isize = -361;
const next_errno: isize = -362;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 361 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 361 raw equation stays between its errno neighbors" {
...
test "errno 361 adjacent raw lanes keep value and pointer decoders precise" {
