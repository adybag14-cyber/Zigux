...
pub fn build(b: *std.Build) void {
    ...
    const test_mod = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_errno_sign_extension_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-xarray-slot-errno-sign-extension-replay",
        "Run the Phase 3 xarray slot errno sign-extension replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 3 xarray slot errno sign-extension replay");
