...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_abi_export_shim_success_status_contract.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-abi-export-shim-success-status-contract",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const contract_step = b.step(
        "phase3-abi-export-shim-success-status-contract",
        "Run the Lane 26 export-shim success status contract.",
    );
    contract_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 26 export-shim success status contract tests.");
