...
fn writeSummary(writer: anytype, summary: abi.ChrdevNotifySummary) !void {
...
pub fn main(init: std.process.Init) !void {
    const io = init.io;
    var stdout_buffer: [28672]u8 = undefined;
    ...
    const dropped_view = chrdev_notify_plan.viewFromBits(words[0..], 240, 32, 8, 8, 2, abi.IDA_POLICY_LAST_FIT, 37, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_WRITE, abi.CHRDEV_IO_OP_WRITE, 20, 8, 1024, 4, 1, 3, 2, 1, 5, 1, 4, 2, 0x1111, 1, abi.CHRDEV_NOTIFY_MASK_SUCCESS, 0, 0xBBBB);
    const deferred_view = chrdev_notify_plan.viewFromBits(words[0..], 240, 32, 8, 8, 2, abi.IDA_POLICY_LAST_FIT, 37, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_WRITE, abi.CHRDEV_IO_OP_WRITE, 36, 8, 1024, 4, 1, 1, 1, 1, 0, 1, 4, 2, 0x3333, 1, abi.CHRDEV_NOTIFY_MASK_DEFERRED, 0, 0xCCCC);
    const failed_view = chrdev_notify_plan.viewFromBits(exhausted_words[0..], 240, 16, 5, 5, 2, abi.IDA_POLICY_FIRST_FIT, 20, abi.CHRDEV_MODE_READ, abi.CHRDEV_MODE_READ, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_READ, abi.CHRDEV_IO_OP_READ, 12, 32, 0, 0, 2, 2, 2, 1, 5, 1, 4, 2, 0x7777, 0, abi.CHRDEV_NOTIFY_MASK_FAILURE, 1, 0xDDDD);
    ...
    const empty_view = abi.ChrdevNotifyView{
        .bits_addr = 0,
        .major = 240,
        .first_minor = 0,
        .minor_count = 0,
    ...
    try writer.writeAll("{\"constants\":{\"chrdev_notify_mask_success\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_MASK_SUCCESS});
    try writer.writeAll(",\"chrdev_notify_mask_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_MASK_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_mask_failure\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_MASK_FAILURE});
    try writer.writeAll(",\"chrdev_notify_flag_matched_notify\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_FLAG_MATCHED_NOTIFY});
    try writer.writeAll(",\"chrdev_notify_flag_notify_planned\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_FLAG_NOTIFY_PLANNED});
    try writer.writeAll(",\"chrdev_notify_index_none\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_INDEX_NONE});
    try writer.writeAll(",\"chrdev_notify_status_none\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_STATUS_NONE});
    try writer.writeAll(",\"chrdev_notify_status_delivered\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_STATUS_DELIVERED});
    try writer.writeAll(",\"chrdev_notify_status_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_STATUS_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_status_dropped\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_STATUS_DROPPED});
    try writer.writeAll("},\"delivered\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_plan.summarize(delivered_view));
    try writer.writeAll("},\"dropped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_plan.summarize(dropped_view));
    try writer.writeAll("},\"deferred\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_plan.summarize(deferred_view));
    try writer.writeAll("},\"failed\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_plan.summarize(failed_view));
    try writer.writeAll("},\"unmatched\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_plan.summarize(unmatched_view));
    try writer.writeAll("},\"empty\":{\"is_valid\":");
    try writer.writeAll(if (chrdev_notify_plan.isValid(empty_view)) "true" else "false");
    try writer.writeAll(",\"summary\":");
    try writeSummary(writer, chrdev_notify_plan.summarize(empty_view));
    try writer.writeAll("}}\n");
