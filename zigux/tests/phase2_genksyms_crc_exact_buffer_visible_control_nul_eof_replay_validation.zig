...
test "lane19 exact-buffer EOF replay preserves visible low control bytes before an embedded NUL" {
    var exact_then_control_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 5);
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_control_then_nul.items)});
