...
test "onehundredninetyfive raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 26, 956, 1994, 2974, 3900 };
...
test "onehundredninetyfive constructor windows keep consecutive codes and reject aliased values" {
...
test "onehundredninetyfive keeps one centered raw and split payload center" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
