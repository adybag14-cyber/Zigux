...
const errno: isize = -462;
const next_errno: isize = -463;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 462 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 462 raw equation stays between its errno neighbors" {
...
test "errno 462 adjacent raw lanes keep value and pointer decoders precise" {
