...
test "representative 113-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 5, 937, 1979, 2981, 3981 };
...
test "every interior onehundredthirteen window keeps consecutive codes and alternating constructor families" {
...
test "onehundredthirteen windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
