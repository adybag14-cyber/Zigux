...
test "triple-bank partial tail head gap keeps first missing cpu discovery aligned" {
    ...
    const words = [_]usize{
        ...
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        bit(1) | bit(2) | bit(3) | bit(4) | bit(5) | bit(6) | bit(9) | bit(13),
    };
...
test "triple-bank partial tail head gap keeps subset and overlap blind to invalid tail noise" {
    const capacity = (bitmap_view.word_bits * 3) + 7;
    const base_words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        bit(1) | bit(2) | bit(4) | bit(5) | bit(6) | bit(10) | bit(15),
    };
    const subset_words = [_]usize{
        bit(7) | bit(21),
        bit(0) | bit(35),
        bit(14) | bit(52),
        bit(1) | bit(4) | bit(6) | bit(11),
    };
    ...
    const head_gap_only_words = [_]usize{
        0,
        0,
        0,
        bit(0) | bit(9),
    ...
    const head_gap_only = cpumask_view.CpuMaskView.init(head_gap_only_words[0..], capacity);
    ...
    try testing.expect(!base.intersects(head_gap_only));
    try testing.expect(!head_gap_only.intersects(base));
    try testing.expect(!head_gap_only.isSubsetOf(base));
