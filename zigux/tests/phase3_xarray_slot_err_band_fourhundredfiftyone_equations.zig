...
const errno: isize = -451;
const next_errno: isize = -452;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 451 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 451 raw equation stays between its errno neighbors" {
...
test "errno 451 adjacent raw lanes keep value and pointer decoders precise" {
