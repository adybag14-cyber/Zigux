...
test "representative err-band octets keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1018, 2044, 3060, 4086 };
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [8]usize = undefined;
        var slots: [8]xarray_slot_view.SlotView = undefined;
        ...
        const sum_07 = @as(u128, raws[0]) + @as(u128, raws[7]);
        const sum_16 = @as(u128, raws[1]) + @as(u128, raws[6]);
        const sum_25 = @as(u128, raws[2]) + @as(u128, raws[5]);
        const sum_34 = @as(u128, raws[3]) + @as(u128, raws[4]);
        try testing.expectEqual(sum_07, sum_16);
        try testing.expectEqual(sum_16, sum_25);
        try testing.expectEqual(sum_25, sum_34);
        ...
        try testing.expectEqual(first_payload + seventh_payload, third_payload + fifth_payload);
...
test "every interior octet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 7 <= err_band_span) : (even_residue += 2) {
...
test "octet windows preserve pairwise centered equations across the interior seam" {
    var even_residue: usize = 0;
    while (even_residue + 7 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[7], codes[1] + codes[6]);
        try testing.expectEqual(codes[1] + codes[6], codes[2] + codes[5]);
        try testing.expectEqual(codes[2] + codes[5], codes[3] + codes[4]);
        try testing.expectEqual(first_payload + seventh_payload, third_payload + fifth_payload);
        try testing.expectEqual(first_payload + 3, seventh_payload);
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 7)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 6)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 6)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 5)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 5)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 4)),
        );
    }
}
