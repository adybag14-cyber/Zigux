...
const visible_count = 46;
const detached_index = visible_count;
const route = [_]usize{
    45, 0,  23, 22, 44, 1,  43, 2,  21, 24, 3,  42, 20, 25, 4, 41,
    19, 26, 5,  40, 18, 27, 6,  39, 17, 28, 7,  38, 16, 29, 8, 37,
    15, 30, 9,  36, 14, 31, 10, 35, 13, 32, 11, 34, 12, 33,
};
...
test "hexatetraconta bridge-braid exposes the same visible route through list and hlist" {
...
test "hexatetraconta bridge-braid repairs staged reverse links at the bridge gate" {
    var entries = [_]Entry{.{}} ** (visible_count + 1);
    var head = ListHead{ .next = 0, .prev = 0 };
    var hhead = HListHead{ .first = 0 };

    reset(&entries, &head, &hhead);
    const bridge_position = 30;
    const bridge_index = route[bridge_position];

    entries[bridge_index].list.prev = @intFromPtr(&head);
    entries[bridge_index].hlist.pprev = @intFromPtr(&hhead.first);

    const list_break = ListView.init(&head).firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, bridge_position), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[bridge_position - 1]].list)), list_break.expected_prev);
    ...
    try std.testing.expectEqual(@as(usize, bridge_position), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[bridge_position - 1]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hhead.first)), hlist_break.actual_pprev);

    entries[bridge_index].list.prev = @intFromPtr(&entries[route[bridge_position - 1]].list);
    entries[bridge_index].hlist.pprev = @intFromPtr(&entries[route[bridge_position - 1]].hlist.next);
