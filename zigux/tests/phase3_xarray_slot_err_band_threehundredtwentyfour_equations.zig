...
const errno: isize = -324;
const next_errno: isize = -325;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 324 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 324 raw equation stays between its errno neighbors" {
...
test "errno 324 adjacent raw lanes keep value and pointer decoders precise" {
