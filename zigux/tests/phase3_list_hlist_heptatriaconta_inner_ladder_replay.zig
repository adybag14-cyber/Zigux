...
const entry_count = 38;
const route = [_]usize{ 36, 1, 34, 3, 32, 5, 30, 7, 28, 9, 26, 11, 24, 13, 22, 15, 20, 17, 18, 19, 16, 21, 14, 23, 12, 25, 10, 27, 8, 29, 6, 31, 4, 33, 2, 35, 0 };
const detached_index = 37;
...
test "heptatriaconta inner ladder keeps forward list and hlist visibility before repair" {
...
test "heptatriaconta inner ladder staged repair crosses the center rung" {
    var entries: [entry_count]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    const list_after_ladder = list_view.ListView.init(&list_head);
    const hlist_after_ladder = hlist_view.HListView.init(&hlist_head);
    try expectListRoute(list_after_ladder, &entries);
    try expectHListRoute(hlist_after_ladder, &entries);

    const list_break = list_after_ladder.firstBrokenBacklink().?;
    ...
    const hlist_break = hlist_after_ladder.firstBrokenPrevLink().?;
