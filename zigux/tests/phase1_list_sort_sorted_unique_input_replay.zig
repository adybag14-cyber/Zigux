...
fn cmp(_: ?*anyopaque, a: *const ListHead, b: *const ListHead) i32 {
...
test "phase1 list_sort sorted unique input replay" {
    var head: ListHead = .{};
    head.init();
    ...
    try std.testing.expectEqualSlices(i32, &.{ 1, 2, 3, 4, 5 }, keys[0..idx]);
    try std.testing.expectEqualSlices(usize, &.{ 0, 1, 2, 3, 4 }, ordinals[0..idx]);
    try std.testing.expect(head.next == &entries[0].node);
    try std.testing.expect(head.prev == &entries[entries.len - 1].node);
    try std.testing.expect(entries[0].node.prev == &head);
    try std.testing.expect(entries[entries.len - 1].node.next == &head);
}
