...
test "list parallel merge braid stays detached until the live merge route adopts it" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_merge = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    live_left.next = @intFromPtr(&live_merge);
    live_left.prev = @intFromPtr(&first);
    live_merge.next = @intFromPtr(&live_right);
    live_merge.prev = @intFromPtr(&live_left);
    live_right.next = @intFromPtr(&tail);
    live_right.prev = @intFromPtr(&live_merge);
    tail.next = @intFromPtr(&head);
    tail.prev = @intFromPtr(&live_right);

    braid_left.next = @intFromPtr(&braid_right);
    braid_left.prev = @intFromPtr(&live_left);
    braid_right.next = @intFromPtr(&live_merge);
    ...
    try expectListSequence(stable, &.{ &first, &live_left, &live_merge, &live_right, &tail });
    ...
    try expectListSequence(
        broken,
        &.{ &first, &live_left, &braid_left, &braid_right, &live_merge, &live_right, &tail },
    );
...
test "hlist parallel merge braid stays detached until the live merge route adopts it" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_merge = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    live_merge.next = @intFromPtr(&live_right);
    live_merge.pprev = @intFromPtr(&live_left.next);
    live_right.next = @intFromPtr(&tail);
    live_right.pprev = @intFromPtr(&live_merge.next);
    tail.next = 0;
    ...
    try expectHListSequence(stable, &.{ &first, &live_left, &live_merge, &live_right, &tail });
