...
const errno: isize = -412;
const next_errno: isize = -413;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 412 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 412 raw equation stays between its errno neighbors" {
...
test "errno 412 adjacent raw lanes keep value and pointer decoders precise" {
