...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_tail_prev_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-tail-prev-replay-tests",
        .root_module = root_module,
    });
    const run_replay_tests = b.addRunArtifact(replay_tests);

    const replay_step = b.step("phase1-helper-ports-a-tail-prev-replay", "Run Lane 06 helper ports A tail and predecessor replay tests");
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run Lane 06 helper ports A tail and predecessor replay tests");
