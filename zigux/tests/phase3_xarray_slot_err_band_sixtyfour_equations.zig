...
test "representative 64-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 986, 2028, 3030, 4030 };
...
test "every interior sixtyfour window keeps consecutive codes and alternating constructor families" {
...
test "sixtyfour windows preserve dual-center seam equations and a paired payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
