...
const route = [_]usize{ 24, 0, 22, 2, 20, 4, 18, 6, 16, 8, 14, 10, 12, 11, 13, 9, 15, 7, 17, 5, 19, 3, 21, 1, 23 };
const detached_index = 25;

fn resetEntries(entries: *[26]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[26]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[26]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[26]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[26]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[26]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[26]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[26]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "pentacosa center clasp keeps forward list and hlist visibility before repair" {
    var entries: [26]Entry = undefined;
...
test "pentacosa center clasp staged repair crosses the middle hinge" {
    var entries: [26]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 19);
    repairHListPrefix(&hlist_head, &entries, 19);
    ...
    try std.testing.expectEqual(@as(usize, 19), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[18]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 19), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[18]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
