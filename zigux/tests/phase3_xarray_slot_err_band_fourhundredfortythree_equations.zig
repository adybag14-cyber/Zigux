...
const errno: isize = -443;
const next_errno: isize = -444;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 443 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 443 raw equation stays between its errno neighbors" {
...
test "errno 443 adjacent raw lanes keep value and pointer decoders precise" {
