...
test "twohundredtwentyfive raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 54, 986, 2018, 2998, 3870 };
...
test "twohundredtwentyfive constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredtwentyfive keeps one centered raw and even payload stride" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
