...
pub fn build(b: *std.Build) void {
    ...
    const test_mod = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_null_zero_gap_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const route = b.step(
        "phase3-xarray-slot-null-zero-gap-replay",
        "Run the Phase 3 xarray null/zero/gap replay",
    );
    route.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 3 xarray null/zero/gap replay");
