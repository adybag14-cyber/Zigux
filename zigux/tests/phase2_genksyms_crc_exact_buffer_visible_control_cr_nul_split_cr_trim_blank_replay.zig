...
test "lane19 exact-buffer split replay skips an extra blank continuation after a normalized visible-control-plus-leading-CR chunk" {
    var exact_then_controls_and_leading_cr_then_trailing_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 14);
