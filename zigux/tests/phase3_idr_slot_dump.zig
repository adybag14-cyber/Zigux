...
fn kindName(kind: idr_slot_view.SlotKind) []const u8 {
    return switch (kind) {
        .null => "null",
...
fn writeCase(writer: anytype, name: []const u8, raw: usize, trailing_comma: bool) !void {
    ...
    try writer.print(
        ...
        .{
            ...
            if (slot.isInvalid()) "true" else "false",
            if (idr_slot_view.isDirectPointerEncoding(raw)) "true" else "false",
            if (xa_value.isValue(raw)) "true" else "false",
            if (err_ptr.isErrValue(raw)) "true" else "false",
        },
    );
    try writeOptionalSigned(writer, if (err_ptr.isErrValue(raw)) err_ptr.toErrorCode(raw) else null);
    try writer.writeAll(",\n      \"decoded_value\": ");
    try writeOptionalUnsigned(writer, if (xa_value.isValue(raw)) xa_value.toValue(raw) else null);
    try writer.writeAll(",\n      \"pointer_raw\": ");
...
pub fn main(init: std.process.Init) !void {
    ...
    try writeCase(writer, "low_bit_tagged", 0x1001, false);
