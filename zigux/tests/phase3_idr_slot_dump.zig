...
fn writeCase(writer: anytype, name: []const u8, raw: usize, trailing_comma: bool) !void {
    ...
    try writer.print(
        ...
        .{
            ...
            raw_hex,
            raw_decimal,
            if (idr_slot_view.isTaggedInternalEntry(raw)) "true" else "false",
        },
    );
    try writeOptionalUnsigned(writer, slot.internalValue());
    try writer.writeAll(",\n      \"decoded_error\": ");
...
pub fn main(init: std.process.Init) !void {
    ...
    try writeCase(writer, "internal_zero", inline_zero_raw, true);
    try writeCase(writer, "internal_small", try xa_value.makeValue(29), true);
