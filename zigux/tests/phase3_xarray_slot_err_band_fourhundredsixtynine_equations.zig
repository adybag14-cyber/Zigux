...
const errno: isize = -469;
const next_errno: isize = -470;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 469 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 469 raw equation stays between its errno neighbors" {
...
test "errno 469 adjacent raw lanes keep value and pointer decoders precise" {
