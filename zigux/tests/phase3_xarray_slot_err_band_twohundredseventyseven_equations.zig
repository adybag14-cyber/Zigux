...
test "twohundredseventyseven raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 52, 986, 2018, 2998, 3818 };
...
test "twohundredseventyseven constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredseventyseven keeps centered raw code and payload equations exact" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
