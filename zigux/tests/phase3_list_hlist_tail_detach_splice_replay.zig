...
test "list tail detach can splice at head before backlink repair" {
    ...
    tail.next = @intFromPtr(&first);
    head.next = @intFromPtr(&tail);
    head.prev = @intFromPtr(&middle);

    var view = list_view.ListView.init(&head);
    try expectListRoute(&head, &tail, &first, &middle);
    ...
    try expectCleanListRoute(&head, &tail, &first, &middle);
}

test "hlist tail detach can splice at head before prev-link repair" {
    ...
    middle.next = 0;
    tail.next = @intFromPtr(&first);
    head.first = @intFromPtr(&tail);

    var view = hlist_view.HListView.init(&head);
    try expectHListRoute(&head, &tail, &first, &middle);
    ...
    tail.pprev = @intFromPtr(&head.first);
    ...
    try expectCleanHListRoute(&head, &tail, &first, &middle);
}
