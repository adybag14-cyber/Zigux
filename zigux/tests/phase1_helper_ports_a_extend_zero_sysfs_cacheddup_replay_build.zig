...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_extend_zero_sysfs_cacheddup_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-extend-zero-sysfs-cacheddup-replay-tests",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase1-helper-ports-a-extend-zero-sysfs-cacheddup-replay",
        "Run the Lane 06 extend/zero/sysfs/cached-dup replay",
    );
