...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_release_ring_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named_step = b.step("phase1-helper-ports-c-release-ring-replay", "Run the Lane 10 helper ports C release-ring replay");
    named_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 10 helper ports C release-ring replay");
