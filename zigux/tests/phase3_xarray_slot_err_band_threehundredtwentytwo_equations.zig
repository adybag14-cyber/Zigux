...
const errno: isize = -322;
const next_errno: isize = -323;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 322 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 322 raw equation stays between its errno neighbors" {
...
test "errno 322 adjacent raw lanes keep value and pointer decoders precise" {
