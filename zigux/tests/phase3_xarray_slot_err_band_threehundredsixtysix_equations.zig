...
const errno: isize = -366;
const next_errno: isize = -367;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 366 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 366 raw equation stays between its errno neighbors" {
...
test "errno 366 adjacent raw lanes keep value and pointer decoders precise" {
