...
test "phase10 virtio input survey manifest records the live starter and remaining gap" {
    ...
    for (manifest.gaps) |gap| {
        ...
        if (std.mem.eql(u8, gap.id, "phase10-virtio-core-lab-starter")) {
            saw_core_starter = true;
            try std.testing.expectEqualStrings("starter_landed", gap.status);
            try std.testing.expectEqualStrings("drivers/virtio/virtio.zig", gap.zigux_destination);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "status and queue bookkeeping") != null);
        }
        ...
        if (std.mem.eql(u8, gap.id, "phase10-virtio-input-queue-callback-preflight-replay")) {
            saw_queue_callback_preflight_replay = true;
            try std.testing.expectEqualStrings("starter_landed", gap.status);
        ...
        if (std.mem.eql(u8, gap.id, "phase10-virtio-input-slice-note")) {
            saw_slice_note = true;
            try std.testing.expectEqualStrings("starter_landed", gap.status);
            try std.testing.expectEqualStrings("Documentation/zigux/phase10-virtio-input-slice.md", gap.zigux_destination);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "remaining roadmap gap") != null);
        }
        ...
        if (std.mem.eql(u8, gap.id, "phase10-virtio-input-probe-preflight-helper")) {
            ...
            try std.testing.expectEqualStrings("starter_landed", gap.status);
            try std.testing.expectEqualStrings("drivers/virtio/virtio_input.zig", gap.zigux_destination);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "probe-preflight summary") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "identity staging") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "queue-fill readiness") != null);
