...
test "representative 78-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 972, 2014, 3016, 4016 };
...
test "every interior seventyeight window keeps consecutive codes and alternating constructor families" {
...
test "seventyeight windows preserve a dual-center raw seam and a centered payload lane" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + center_left_index - 2) + 1,
            center_payload,
        );
        try testing.expectEqual(
            center_payload + 1,
