...
test "representative 99-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 951, 1993, 2995, 3995 };
...
test "every interior ninetynine window keeps consecutive codes and alternating constructor families" {
...
test "ninetynine windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
