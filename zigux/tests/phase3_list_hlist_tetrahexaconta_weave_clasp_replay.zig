...
const staged_break_index = 44;
const route = [_]usize{
    63, 0,  38, 37, 62, 1,  39, 36, 61, 2,  40, 35, 60, 3,
    41, 34, 59, 4,  42, 33, 58, 5,  43, 32, 57, 6,  44, 31,
    56, 7,  45, 30, 55, 8,  46, 29, 54, 9,  47, 28, 53, 10,
    48, 27, 52, 11, 49, 26, 51, 12, 50, 25, 24, 13, 23, 22,
...
test "tetrahexaconta weave-clasp list route exposes staged backlink repair" {
...
test "tetrahexaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
