...
test "representative 43-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1006, 2038, 3054, 4052 };
...
test "every interior fortythree window keeps consecutive codes and alternating constructor families" {
...
test "fortythree windows preserve centered equations around the middle err-code slot" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_payload_sum =
