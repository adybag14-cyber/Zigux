...
const staged_break_index = 43;
const route = [_]usize{
    62, 0,  37, 36, 61, 1,  38, 35, 60, 2,  39, 34, 59, 3,
    40, 33, 58, 4,  41, 32, 57, 5,  42, 31, 56, 6,  43, 30,
    55, 7,  44, 29, 54, 8,  45, 28, 53, 9,  46, 27, 52, 10,
    47, 26, 51, 11, 48, 25, 50, 12, 49, 24, 23, 13, 22, 21,
...
test "trihexaconta weave-clasp list route exposes staged backlink repair" {
...
test "trihexaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
