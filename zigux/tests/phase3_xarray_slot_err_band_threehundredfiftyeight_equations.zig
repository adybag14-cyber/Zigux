...
const errno: isize = -358;
const next_errno: isize = -359;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 358 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 358 raw equation stays between its errno neighbors" {
...
test "errno 358 adjacent raw lanes keep value and pointer decoders precise" {
