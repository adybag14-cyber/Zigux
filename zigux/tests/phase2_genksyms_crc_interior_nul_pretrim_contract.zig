...
test "phase2 genksyms crc trims visible CR bytes before an interior NUL" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + "tail\n".len);
    defer input.deinit(std.testing.allocator);
    try input.appendSlice(std.testing.allocator, "symbol\r\r");
    try input.append(std.testing.allocator, 0);
    try input.appendSlice(std.testing.allocator, "hidden\r\r");
    try input.appendNTimes(std.testing.allocator, 'x', c_line_payload_len - input.items.len);
    ...
    try std.testing.expectEqual(@as(usize, "symbol".len), records.items[0].len);
    try std.testing.expectEqual(@as(u8, 's'), records.items[0].first);
    try std.testing.expectEqual(@as(u8, 'l'), records.items[0].last);
    try std.testing.expectEqual(crc32("symbol"), records.items[0].crc);
    try std.testing.expectEqual(crc32("tail"), records.items[1].crc);
    try std.testing.expect(!containsCrc(records.items, crc32("symbol\r\r")));
...
test "phase2 genksyms crc skips an interior-NUL record after visible CR pretrim empties it" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + "next\n".len);
    defer input.deinit(std.testing.allocator);
    try input.appendSlice(std.testing.allocator, "\r\r\r");
    try input.append(std.testing.allocator, 0);
    try input.appendSlice(std.testing.allocator, "hidden-symbol");
    ...
    try std.testing.expectEqual(@as(usize, "next".len), records.items[0].len);
    try std.testing.expectEqual(@as(u8, 'n'), records.items[0].first);
    try std.testing.expectEqual(@as(u8, 't'), records.items[0].last);
    try std.testing.expectEqual(crc32("next"), records.items[0].crc);
    try std.testing.expect(!containsCrc(records.items, crc32("\r\r\r")));
    try std.testing.expect(!containsCrc(records.items, crc32("hidden-symbol")));
    try std.testing.expect(!containsCrc(records.items, crc32(input.items[0..c_line_payload_len])));
}
