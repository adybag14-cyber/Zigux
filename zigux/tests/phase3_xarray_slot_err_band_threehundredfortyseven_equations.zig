...
const errno: isize = -347;
const next_errno: isize = -348;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 347 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 347 raw equation stays between its errno neighbors" {
...
test "errno 347 adjacent raw lanes keep value and pointer decoders precise" {
