...
test "representative err-band quattuordecets keep alias-err spacing exact" {
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [14]usize = undefined;
        var slots: [14]xarray_slot_view.SlotView = undefined;
        ...
        const sum_013 = @as(u128, raws[0]) + @as(u128, raws[13]);
        const sum_112 = @as(u128, raws[1]) + @as(u128, raws[12]);
        const sum_211 = @as(u128, raws[2]) + @as(u128, raws[11]);
        const sum_310 = @as(u128, raws[3]) + @as(u128, raws[10]);
        const sum_49 = @as(u128, raws[4]) + @as(u128, raws[9]);
        const sum_58 = @as(u128, raws[5]) + @as(u128, raws[8]);
        const sum_67 = @as(u128, raws[6]) + @as(u128, raws[7]);
        try testing.expectEqual(sum_013, sum_112);
        try testing.expectEqual(sum_112, sum_211);
        try testing.expectEqual(sum_211, sum_310);
        try testing.expectEqual(sum_310, sum_49);
        try testing.expectEqual(sum_49, sum_58);
        try testing.expectEqual(sum_58, sum_67);
        ...
        try testing.expectEqual(first_payload + thirteenth_payload, third_payload + eleventh_payload);
        try testing.expectEqual(third_payload + eleventh_payload, fifth_payload + ninth_payload);
        try testing.expectEqual(fifth_payload + ninth_payload, @as(usize, seventh_payload) * 2);
...
test "every interior quattuordecet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 13 <= err_band_span) : (even_residue += 2) {
...
test "quattuordecet windows preserve centered equations across the middle seam" {
    var even_residue: usize = 0;
    while (even_residue + 13 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[13], codes[1] + codes[12]);
        try testing.expectEqual(codes[1] + codes[12], codes[2] + codes[11]);
        try testing.expectEqual(codes[2] + codes[11], codes[3] + codes[10]);
        try testing.expectEqual(codes[3] + codes[10], codes[4] + codes[9]);
        try testing.expectEqual(codes[4] + codes[9], codes[5] + codes[8]);
        try testing.expectEqual(codes[5] + codes[8], codes[6] + codes[7]);
        try testing.expectEqual(first_payload + thirteenth_payload, third_payload + eleventh_payload);
        try testing.expectEqual(third_payload + eleventh_payload, fifth_payload + ninth_payload);
        try testing.expectEqual(fifth_payload + ninth_payload, @as(usize, seventh_payload) * 2);
        try testing.expectEqual(first_payload + 6, thirteenth_payload);
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 13)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 12)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 12)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 11)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 11)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 10)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 10)),
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 9)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 9)),
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 8)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 8)),
            @as(u128, rawFromResidue(even_residue + 6)) + @as(u128, rawFromResidue(even_residue + 7)),
        );
    }
}
