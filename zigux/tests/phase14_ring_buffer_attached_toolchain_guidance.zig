...
test "phase14 ring-buffer attached toolchain guidance stays packet-local and environment-only without a checkout tree" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, note, "`agent_files/zig-x86_64-linux-0.17.0-dev.758+748e7c5e3 (1).tar.xz`") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "mkdir -p /workspace/.toolchains/p14-l08 && tar -xf \"/workspace/agent_files/zig-x86_64-linux-0.17.0-dev.758+748e7c5e3 (1).tar.xz\" -C /workspace/.toolchains/p14-l08") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "`/workspace/.toolchains/p14-l08/zig-x86_64-linux-0.17.0-dev.758+748e7c5e3`") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "`0.17.0-dev.758+748e7c5e3`") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "`/workspace/.toolchains/p14-l08/zig-x86_64-linux-0.17.0-dev.758+748e7c5e3/zig version`") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "`/workspace/.toolchains/p14-l08/zig-x86_64-linux-0.17.0-dev.758+748e7c5e3/zig env`") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "zig env | rg -n \"zig_exe|lib_dir|std_dir|global_cache_dir|version|target\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, note, "capture that `zig env` reported an `x86_64-linux` target environment through the `.target` field") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "`.zig_exe`, `.lib_dir`, `.std_dir`, and `.version` fields") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "capture the `.global_cache_dir` field as runtime cache context") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "do not require or invent a top-level `.local_cache_dir` field") != null);
    try std.testing.expect(std.mem.indexOf(u8, note, "this attached dev build's `zig env` output does not print one") != null);
