...
pub fn build(b: *std.Build) void {
    ...
    const dump_current_module = b.createModule(.{
        .root_source_file = b.path("phase3_abi_dump_current.zig"),
        .target = target,
        .optimize = optimize,
    });
    dump_current_module.addImport("abi_bindings", abi_bindings_module);
    dump_current_module.addImport("notifier_abi", notifier_abi_module);

    const abi_replay_tests = b.addTest(.{
        .name = "phase3-abi-replay-test",
        .root_module = abi_replay_module,
    });
    const dump_current = b.addExecutable(.{
        .name = "phase3-abi-dump-current",
        .root_module = dump_current_module,
    });

    const run_abi_replay_tests = b.addRunArtifact(abi_replay_tests);
    const run_dump_current = b.addRunArtifact(dump_current);
    const pair_step = b.step(
        "phase3-abi-dump-current-pair-test",
        "Run the Phase 3 ABI replay beside the current ABI dump executable",
    );
    pair_step.dependOn(&run_abi_replay_tests.step);
    pair_step.dependOn(&run_dump_current.step);

    const test_step = b.step("test", "Run the Phase 3 ABI dump-current pair tests");
