...
const empty_bank_count = 63;
const tail_word_index = empty_bank_count + 1;
const tail_offset = 13;
...
fn replayWords() [tail_word_index + 1]usize {
    ...
    words[0] = (@as(usize, 1) << 5) | (@as(usize, 1) << 23);
...
test "lane27 tresexagintuple replay keeps bitmap discovery stable across 63 empty banks" {
    ...
    try testing.expectEqual(@as(?usize, 23), view.nextSetBit(6));
    try testing.expectEqual(@as(?usize, tail_base), view.nextSetBit(24));
    ...
    try testing.expectEqual(@as(?usize, 6), view.nextClearBit(6));
    try testing.expectEqual(@as(?usize, 24), view.nextClearBit(24));
    try testing.expectEqual(@as(?usize, tail_base + 1), view.nextClearBit(tail_base + 1));
}

test "lane27 tresexagintuple replay keeps cpumask traversal aligned with bitmap traversal" {
    const words = replayWords();
    ...
    try testing.expect(mask.hasCpu(5));
    try testing.expect(mask.hasCpu(tail_base));
    try testing.expect(mask.hasCpu(tail_base + tail_offset));
    try testing.expect(!mask.hasCpu(6));
    try testing.expectEqual(bitmap.nextSetBit(24), mask.nextCpu(24));
...
test "lane27 tresexagintuple replay ignores tail-only noise in subset and overlap checks" {
    const base_words = replayWords();
    var superset_words = replayWords();
    var tail_noise_only_words = [_]usize{0} ** (tail_word_index + 1);
    superset_words[0] |= (@as(usize, 1) << 31);
