...
const errno: isize = -399;
const next_errno: isize = -400;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 399 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 399 raw equation stays between its errno neighbors" {
...
test "errno 399 adjacent raw lanes keep value and pointer decoders precise" {
