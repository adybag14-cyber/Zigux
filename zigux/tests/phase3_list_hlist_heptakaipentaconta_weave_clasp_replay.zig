...
const staged_break_index = 37;
const route = [_]usize{
    56, 0,  31, 30, 55, 1,  32, 29, 54, 2,  33, 28, 53, 3,
    34, 27, 52, 4,  35, 26, 51, 5,  36, 25, 50, 6,  37, 24,
    49, 7,  38, 23, 48, 8,  39, 22, 47, 9,  40, 21, 46, 10,
    41, 20, 45, 11, 42, 19, 44, 12, 43, 18, 17, 13, 16, 15,
    14,
};
...
test "heptakaipentaconta weave-clasp list route exposes staged backlink repair" {
...
test "heptakaipentaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
