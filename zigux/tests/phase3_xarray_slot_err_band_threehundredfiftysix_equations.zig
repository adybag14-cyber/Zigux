...
const errno: isize = -356;
const next_errno: isize = -357;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 356 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 356 raw equation stays between its errno neighbors" {
...
test "errno 356 adjacent raw lanes keep value and pointer decoders precise" {
