...
Phase 10 flow
  ...
  * keep the shared Phase 10 tests-root packet aligned around the direct `drivers/virtio/virtio.zig` and `drivers/virtio/virtio_driver_id.zig` core surfaces, the ring packet, the input probe-preflight, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays, and the MMIO review surfaces while the broader helper names and risky-transport claims stay documented through the closure-manifest-backed reminder notes rather than direct parity claims
