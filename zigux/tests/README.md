...
Key entrypoints
  ...
  * current `master` still ships no standalone `samples/zigux/*string*` Phase 5 reference sample, so keep string-helper reviewability under the existing Phase 7 packet already recorded in `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and `samples/zigux/README.md` instead of counting it as a fifth approved Phase 5 sample idiom
