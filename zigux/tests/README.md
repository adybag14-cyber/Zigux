...
Guidance
  ...
  * keep `zigux/tests/fixtures/phase6_checksum_vectors.zig` and `zigux/tests/fixtures/phase6_hexdump_vectors.zig` explicit in the tests root too so the committed fixture-backed checksum and hexdump evidence stays reviewable from the same shared catalog instead of living only inside helper-local imports and slice prose
