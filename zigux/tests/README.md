...
Phase 3 review packet
  ...
  * keep the current shared Phase 3 reminder anchored to the bounded `dev_t` starter packet, the helper-local `err_ptr` / `xarray` slice, the focused policy slice, and the bounded low-level-wrapper reminder packet already described in `Documentation/zigux/phase3-abi-slice.md` and `Documentation/zigux/phase3-low-level-wrapper-boundary-survey.md`, instead of presenting the shared ABI checker, catalog, export/UAPI survey-validator, or broader IDR and IDA follow-through as shipped tests-root evidence
  * keep the landed low-level-wrapper packet explicit here too: `Documentation/zigux/phase3-low-level-wrapper-boundary-survey.md`, `zigux/helpers/atomic.zig`, `zigux/helpers/barrier.zig`, `zigux/helpers/mmio.zig`, `zigux/helpers/unsafe_policy.zig`, `zigux/unsafe/narrow.zig`, `scripts/zigux/validate-phase3-low-level-wrapper-survey.py`, `zigux/tests/phase3_low_level_wrappers.zig`, `zigux/tests/phase3_low_level_wrappers_build.zig`, and `zig build phase3-low-level-wrappers-test --build-file zigux/tests/phase3_low_level_wrappers_build.zig` now keep the bounded atomic, barrier, and MMIO wrapper reminder packet directly readable from the tests root without widening it into full Phase 3 completion
  * treat broader routes such as `Documentation/zigux/phase3-abi-header-family-survey.md`, `Documentation/zigux/phase3-abi-h-boundary-next-step.md`, `scripts/zigux/check-phase3-abi.py`, `scripts/zigux/phase3_catalog.py`, and `scripts/zigux/validate-phase3-export-uapi-survey.py` as repo-reality gaps until fresh current-tree proof lands, while keeping the separately readable shared validator entrypoint, notifier binding, export shim, and focused export/UAPI layout replay out of the gap list
  * keep `Documentation/zigux/phase3-shared-reminder-gap.md` limited to tracking any future docs-root, tests-root, or scripts-root reminder drift, because `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and the bounded low-level-wrapper survey already carry the current shared Phase 3 packet split and this tests-root packet should stay aligned with them
