...
Key entrypoints
  ...
  * current direct-readback Phase 1 reminder and bench-hardening packet: `scripts/zigux/check-phase1-string-review-packet.py`, `scripts/zigux/check-phase1-direct-owner-markers.py`, `scripts/zigux/check-phase1-bench.py`, and the bootstrap self-test route in `.github/workflows/zigux-bootstrap.yml`
  * repo-reality warning for the broader Phase 1 closure-and-replay packet: repeated authenticated contents reads on current `master` now return missing for `Documentation/zigux/phase1-closure.md`, `scripts/zigux/validate-phase1.py`, `scripts/zigux/validate-phase1-closure.py`, `scripts/zigux/check-phase1-parity.py`, `zigux/tests/phase1_helpers.zig`, `zigux/tests/phase1_bench.zig`, `zigux/tests/fixtures/phase1_bench_expectations.json`, and `zigux/tests/fixtures/phase1_helpers_c_harness.c`
  * keep current Phase 1 follow-through tied to the live owner-map plus string-review reminder packet, keep the directly readable bench checker and bootstrap self-test explicit, and keep the broader bench replay companions parked instead of reconstructing the validator-first tranche from those older missing closure-side files and routes alone
