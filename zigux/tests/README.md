...
- keep the Phase 4 atomic64 roadmap rename explicit: the roadmap still names `zigux/tests/atomic64_diff.zig`, but the live shared `zigux/tests/phase4_build.zig` entrypoint runs `zigux/tests/runtime_atomic64_diff.zig`, and `Documentation/zigux/phase4-validation-matrix.md` should remain the reviewer-facing note that explains that shipped rename instead of growing a duplicate gate file
