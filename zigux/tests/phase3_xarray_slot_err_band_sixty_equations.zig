...
test "representative 60-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 990, 2032, 3034, 4034 };
...
test "every interior sixty window keeps consecutive codes and alternating constructor families" {
...
test "sixty windows preserve dual-center seam equations and a paired payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
