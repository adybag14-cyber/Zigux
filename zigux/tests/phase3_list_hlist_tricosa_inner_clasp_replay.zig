...
const route = [_]usize{ 22, 0, 20, 2, 18, 4, 16, 6, 14, 8, 12, 10, 11, 13, 9, 15, 7, 17, 5, 19, 3, 21, 1 };
const detached_index = 23;

fn resetEntries(entries: *[24]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[24]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[24]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[24]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[24]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[24]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[24]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[24]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "tricosa inner clasp keeps forward list and hlist visibility before repair" {
...
test "tricosa inner clasp staged repair crosses the inner clasp" {
    var entries: [24]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 17);
    repairHListPrefix(&hlist_head, &entries, 17);
    ...
    try std.testing.expectEqual(@as(usize, 17), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[16]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 17), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[16]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
