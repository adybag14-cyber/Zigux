...
const errno: isize = -389;
const next_errno: isize = -390;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 389 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 389 raw equation stays between its errno neighbors" {
...
test "errno 389 adjacent raw lanes keep value and pointer decoders precise" {
    const even_before = raw - 1;
    const odd_after = raw + 1;
    const even_before_slot = xarray_slot_view.fromRaw(even_before);
    const odd_after_slot = xarray_slot_view.fromRaw(odd_after);

    try std.testing.expectEqual(xarray_slot_view.SlotKind.err, even_before_slot.kind());
    try std.testing.expectEqual(xarray_slot_view.SlotKind.err, odd_after_slot.kind());
    try std.testing.expect(!xa_value.isValue(raw));
    try std.testing.expect(!xa_value.isValue(even_before));
    try std.testing.expect(!xa_value.isValue(odd_after));
    try std.testing.expectEqual(@as(?usize, null), even_before_slot.pointerValue());
    try std.testing.expectEqual(@as(?usize, null), odd_after_slot.pointerValue());
}
