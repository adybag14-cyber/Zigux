...
const errno: isize = -335;
const next_errno: isize = -336;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 335 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 335 raw equation stays between its errno neighbors" {
...
test "errno 335 adjacent raw lanes keep value and pointer decoders precise" {
