...
fn outerZipListForwardOnly(head: *ListHead, nodes: *[6]ListHead) void {
    const order = [_]usize{ 0, 5, 4, 1, 2, 3 };
...
fn outerZipHListForwardOnly(head: *HListHead, nodes: *[6]HListNode) void {
    const order = [_]usize{ 0, 5, 4, 1, 2, 3 };
...
test "list view outer-zip route exposes staged backlink repair" {
    var head = ListHead{ .next = 0, .prev = 0 };
    var nodes = [_]ListHead{.{ .next = 0, .prev = 0 }} ** 6;
    seedCleanList(&head, &nodes);
    outerZipListForwardOnly(&head, &nodes);

    const view = list_view.ListView.init(&head);
    const order = [_]*const ListHead{ &nodes[0], &nodes[5], &nodes[4], &nodes[1], &nodes[2], &nodes[3] };
    ...
    try expectListBreak(view, 2, ptr(&nodes[5]), ptr(&nodes[3]));
...
test "hlist view outer-zip route exposes staged prev-link repair" {
    var head = HListHead{ .first = 0 };
    var nodes = [_]HListNode{.{ .next = 0, .pprev = 0 }} ** 6;
    seedCleanHList(&head, &nodes);
    outerZipHListForwardOnly(&head, &nodes);

    const view = hlist_view.HListView.init(&head);
    const order = [_]*const HListNode{ &nodes[0], &nodes[5], &nodes[4], &nodes[1], &nodes[2], &nodes[3] };
    ...
    try expectHListBreak(view, 2, ptr(&nodes[5].next), ptr(&nodes[3].next));
