...
test "list center braid cascade exposes forward route before backlink repair" {
    ...
    try std.testing.expectEqualSlices(usize, &.{ 2, 0, 3, 5, 1, 4 }, &listRoute(view, &nodes));
...
test "hlist center braid cascade exposes forward route before pprev repair" {
    ...
    try std.testing.expectEqualSlices(usize, &.{ 2, 0, 3, 5, 1, 4 }, &hlistRoute(view, &nodes));
    try std.testing.expectEqual(@as(usize, 6), view.len());
    ...
    four.pprev = @intFromPtr(&one.next);
