...
fn addErrPtrXarrayStarterTests(
    b: *std.Build,
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
) *std.Build.Step.Run {
...
pub fn build(b: *std.Build) void {
    ...
    const xarray_slot_view_tests = addXarraySlotViewTests(b, target, optimize);
    const errptr_xarray_starter = addErrPtrXarrayStarterTests(b, target, optimize);
    const xarray_slot_starter = addXarraySlotStarterTests(b, target, optimize);
    const dump_route = addDumpRoute(b, target, optimize);

    const step = b.step(
        "phase3-errptr-xarray-interop-slice-test",
        "Run the Lane 29 helper roots, starter packets, and dump route together",
    );
    step.dependOn(&err_ptr_tests.step);
    step.dependOn(&xa_value_tests.step);
    step.dependOn(&xarray_slot_view_tests.step);
    step.dependOn(&errptr_xarray_starter.step);
    step.dependOn(&xarray_slot_starter.step);
