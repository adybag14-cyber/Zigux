...
const errno: isize = -427;
const next_errno: isize = -428;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 427 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 427 raw equation stays between its errno neighbors" {
...
test "errno 427 adjacent raw lanes keep value and pointer decoders precise" {
