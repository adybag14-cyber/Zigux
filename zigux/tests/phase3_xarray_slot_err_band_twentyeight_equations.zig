...
test "representative 28-raw err-band windows keep alias-err spacing exact" {
    ...
    for (left_residues) |left_residue| {
        ...
        const center_payload_sum =
            expectedRejectedPayload(left_residue + center_left_index - 1) +
            expectedRejectedPayload(left_residue + center_right_index);
...
test "twentyeight windows preserve seam equations and the centered rejected-payload pair law" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        const center_payload_sum =
            expectedRejectedPayload(even_residue + center_left_index - 1) +
            expectedRejectedPayload(even_residue + center_right_index);
