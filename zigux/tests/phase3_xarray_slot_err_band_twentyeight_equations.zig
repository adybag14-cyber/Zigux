...
const center_left_even_index = center_left_index - 1;
const center_right_even_index = center_right_index;
...
test "representative 28-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3058, 4066 };
...
test "every interior twentyeight window keeps consecutive codes and alternating constructor families" {
...
test "twentyeight windows preserve centered equations across the middle seam pair" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue) + ((window_len / 2) - 1),
