...
test "switchback keeps bitmap and cpumask summaries aligned under noisy tail storage" {
    const capacity = (4 * bitmap_view.word_bits) + 13;
    const switchback_bits = [_]usize{
        2,
        6,
    ...
    fillPattern(words[0..], capacity, switchback_bits[0..]);
    addNoise(words[0..], capacity);

    const bitmap = bitmap_view.BitmapView.init(words[0..], capacity);
    const cpumask = cpumask_view.CpuMaskView.init(words[0..], capacity);

    try testing.expectEqual(switchback_bits.len, bitmap.countSetBits());
...
test "peer switchback overlaps on shared turns without collapsing into a subset" {
    const capacity = (4 * bitmap_view.word_bits) + 13;
    ...
    try testing.expect(base.hasCpu(bitmap_view.word_bits + 1));
    try testing.expect(!peer.hasCpu(bitmap_view.word_bits + 1));
    try testing.expect(!base.hasCpu(capacity - 3));
    try testing.expect(peer.hasCpu(capacity - 3));
}

test "inner switchback stays a bounded subset of the broader path" {
    const capacity = (4 * bitmap_view.word_bits) + 13;
    const base_bits = [_]usize{
        2,
        6,
    ...
    try testing.expect(inner.hasCpu(capacity - 5));
    try testing.expect(!inner.hasCpu(2));
}
