...
test "list view keeps the live pivot route over a detached detour" {
    ...
    var detour_exit = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    detour_entry.next = @intFromPtr(&detour_exit);
    detour_entry.prev = @intFromPtr(&left);
    detour_exit.next = @intFromPtr(&right);
    detour_exit.prev = @intFromPtr(&detour_entry);
...
test "list view reports the adopted detour when the pivot bypass leaves a stale rejoin backlink" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var left = list_view.ListHead{ .next = 0, .prev = 0 };
    var pivot = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    first.next = @intFromPtr(&left);
    first.prev = @intFromPtr(&head);
    left.next = @intFromPtr(&detour_entry);
    left.prev = @intFromPtr(&first);
    pivot.next = @intFromPtr(&right);
    pivot.prev = @intFromPtr(&left);
    right.next = @intFromPtr(&tail);
    ...
    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &first, &left, &detour_entry, &detour_exit, &right, &tail });

    const breakage = view.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 4), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&detour_exit)), breakage.expected_prev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&pivot)), breakage.actual_prev);
    try std.testing.expect(!view.hasConsistentBacklinks());
}

test "hlist view keeps the live pivot route over a detached detour" {
    ...
    var detour_exit = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    detour_entry.next = @intFromPtr(&detour_exit);
    detour_entry.pprev = @intFromPtr(&left.next);
    detour_exit.next = @intFromPtr(&right);
    detour_exit.pprev = @intFromPtr(&detour_entry.next);
...
test "hlist view reports the adopted detour when the pivot bypass leaves a stale rejoin prev-link" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var pivot = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &first, &left, &detour_entry, &detour_exit, &right, &tail });

    const breakage = view.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 4), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&detour_exit.next)), breakage.expected_pprev);
