...
test "lane19 exact-buffer split replay preserves low control bytes plus leading carriage returns before the next line" {
    var exact_then_controls_and_leading_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 10);
    ...
    try exact_then_controls_and_leading_cr.append(std.testing.allocator, '\n');
    try exact_then_controls_and_leading_cr.appendSlice(std.testing.allocator, "x\n");
