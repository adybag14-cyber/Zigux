...
fn writeSummary(writer: anytype, summary: abi.ChrdevNotifyAckWindowPolicySummary) !void {
    ...
    try writer.writeAll(",\"policy_window_acked_count\":");
    try writer.print("{d}", .{summary.policy_window_acked_count});
    try writer.writeAll(",\"policy_window_deferred_count\":");
    try writer.print("{d}", .{summary.policy_window_deferred_count});
    try writer.writeAll(",\"policy_window_suppressed_count\":");
    try writer.print("{d}", .{summary.policy_window_suppressed_count});
    try writer.writeAll(",\"policy_window_coalesced_count\":");
    try writer.print("{d}", .{summary.policy_window_coalesced_count});
    try writer.writeAll(",\"policy_window_dropped_count\":");
    try writer.print("{d}", .{summary.policy_window_dropped_count});
    try writer.writeAll(",\"policy_window_skipped_count\":");
    try writer.print("{d}", .{summary.policy_window_skipped_count});
    try writer.writeAll("}");
}
...
pub fn main(init: std.process.Init) !void {
    ...
    const empty_view = abi.ChrdevNotifyAckWindowPolicyView{
        .bits_addr = 0,
        .major = 240,
        .first_minor = 0,
        .minor_count = 0,
    ...
    try writer.writeAll(",\"chrdev_notify_ack_window_policy_status_none\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_NONE});
    try writer.writeAll(",\"chrdev_notify_ack_window_policy_status_acked\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_ACKED});
    try writer.writeAll(",\"chrdev_notify_ack_window_policy_status_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_ack_window_policy_status_suppressed\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_SUPPRESSED});
    try writer.writeAll(",\"chrdev_notify_ack_window_policy_status_coalesced\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_COALESCED});
    try writer.writeAll(",\"chrdev_notify_ack_window_policy_status_dropped\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_DROPPED});
    try writer.writeAll(",\"chrdev_notify_ack_window_policy_status_skipped\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_WINDOW_POLICY_STATUS_SKIPPED});
    try writer.writeAll("},\"acked\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_policy_plan.summarize(acked_view));
    try writer.writeAll("},\"forced_deferred\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_policy_plan.summarize(forced_deferred_view));
    try writer.writeAll("},\"dropped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_policy_plan.summarize(dropped_view));
    try writer.writeAll("},\"suppressed_dropped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_policy_plan.summarize(suppressed_dropped_view));
    try writer.writeAll("},\"coalesced\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_policy_plan.summarize(coalesced_view));
    try writer.writeAll("},\"skipped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_policy_plan.summarize(skipped_view));
    try writer.writeAll("},\"empty\":{\"is_valid\":");
    try writer.writeAll(if (chrdev_notify_ack_window_policy_plan.isValid(empty_view)) "true" else "false");
    try writer.writeAll(",\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_window_policy_plan.summarize(empty_view));
    try writer.writeAll("}}\n");
