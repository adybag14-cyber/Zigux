...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_errptr_xa_value_alias_windows.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-errptr-xa-value-alias-windows-test",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase3-errptr-xa-value-alias-windows",
        "Run the focused Phase 3 err_ptr xa_value alias-window replay",
    );
