...
pub fn build(b: *std.Build) void {
    ...
    const phase3_low_level = addPhase3LowLevelWrappers(b, target, optimize);

    const test_step = b.step(
        "phase3-abi-low-level-test",
        "Run the focused Phase 3 ABI core plus low-level wrapper shard",
    );
    test_step.dependOn(&phase3_abi_core.step);
    test_step.dependOn(&phase3_low_level.step);
}
