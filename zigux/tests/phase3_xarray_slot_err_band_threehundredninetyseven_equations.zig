...
const errno: isize = -397;
const next_errno: isize = -398;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 397 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 397 raw equation stays between its errno neighbors" {
...
test "errno 397 adjacent raw lanes keep value and pointer decoders precise" {
