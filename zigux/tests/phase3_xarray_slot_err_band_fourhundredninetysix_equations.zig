...
const errno: isize = -496;
const next_errno: isize = -497;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 496 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 496 raw equation stays between its errno neighbors" {
...
test "errno 496 adjacent raw lanes keep value and pointer decoders precise" {
