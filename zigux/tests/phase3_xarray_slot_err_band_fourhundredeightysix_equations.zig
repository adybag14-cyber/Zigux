...
const errno: isize = -486;
const next_errno: isize = -487;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 486 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 486 raw equation stays between its errno neighbors" {
...
test "errno 486 adjacent raw lanes keep value and pointer decoders precise" {
