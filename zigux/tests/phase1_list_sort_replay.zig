...
test "phase1 list_sort replay accepts boolean-style comparator explicitly" {
    const bool_cmp = struct {
        fn compare(_: ?*anyopaque, a: *const list_sort.ListHead, b: *const list_sort.ListHead) i32 {
            const lhs: *const Entry = @fieldParentPtr("node", a);
            const rhs: *const Entry = @fieldParentPtr("node", b);
    ...
    list_sort.listSort(null, &head, bool_cmp);
...
test "phase1 list_sort replay keeps plain comparator order when a later pass ties everything" {
    const SortMode = enum { ascending, descending };
...
test "phase1 list_sort replay can reorder the same circular list twice with plain comparator context" {
    const SortMode = enum { ascending, descending };
...
test "phase1 list_sort replay reuses non-unit context across repeated reordering" {
    const SortMode = enum { ascending, descending };
    ...
    const count = try collectSorted(&head, &keys, &ordinals);
    try std.testing.expectEqual(@as(usize, entries.len), count);
    try std.testing.expectEqualSlices(i32, &.{ 1, 1, 2, 4, 5, 5 }, keys[0..count]);
    try std.testing.expectEqualSlices(usize, &.{ 2, 5, 0, 4, 1, 3 }, ordinals[0..count]);
    try std.testing.expect(head.next == &entries[2].node);
    try std.testing.expect(head.prev == &entries[3].node);
    try std.testing.expect(entries[2].node.prev == &head);
    try std.testing.expect(entries[3].node.next == &head);
...
test "phase1 list_sort replay accepts non-unit comparator magnitudes" {
    const non_unit_cmp = struct {
        fn compare(_: ?*anyopaque, a: *const list_sort.ListHead, b: *const list_sort.ListHead) i32 {
            const lhs: *const Entry = @fieldParentPtr("node", a);
            const rhs: *const Entry = @fieldParentPtr("node", b);
    ...
    list_sort.listSort(null, &head, non_unit_cmp);
...
test "phase1 list_sort replay preserves stable parity bucket order" {
    const cmp = struct {
        fn compare(_: ?*anyopaque, a: *const list_sort.ListHead, b: *const list_sort.ListHead) i32 {
            const lhs: *const Entry = @fieldParentPtr("node", a);
            const rhs: *const Entry = @fieldParentPtr("node", b);
    ...
    const count = try collectSorted(&head, &keys, &ordinals);
    try std.testing.expectEqual(@as(usize, entries.len), count);
    try std.testing.expectEqualSlices(i32, &.{ 1, 3, 5, 7, 4, 6, 2, 8 }, keys[0..count]);
    try std.testing.expectEqualSlices(usize, &.{ 1, 3, 5, 7, 0, 2, 4, 6 }, ordinals[0..count]);
    try std.testing.expect(head.next == &entries[1].node);
    try std.testing.expect(head.prev == &entries[6].node);
    try std.testing.expect(entries[1].node.prev == &head);
    try std.testing.expect(entries[6].node.next == &head);
...
test "phase1 list_sort replay keeps reverse links aligned after reordering" {
    ...
    try std.testing.expectEqual(@as(usize, entries.len), forward_idx);
    try std.testing.expectEqual(@as(usize, entries.len), backward_idx);
...
test "phase1 list_sort replay handles empty and singleton lists" {
    const SingletonEntry = struct {
        key: i32,
        ordinal: usize,
        node: list_sort.ListHead = .{},
    };
    ...
    const cmp = struct {
        fn compare(_: ?*anyopaque, a: *const list_sort.ListHead, b: *const list_sort.ListHead) i32 {
            const lhs: *const SingletonEntry = @fieldParentPtr("node", a);
            const rhs: *const SingletonEntry = @fieldParentPtr("node", b);
    ...
    single_head.init();
    var entry = SingletonEntry{ .key = 7, .ordinal = 0 };
