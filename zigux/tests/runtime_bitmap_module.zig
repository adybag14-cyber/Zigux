...
test "runtime bitmap module gate keeps repeat-init rejection state-preserving across lifecycle stages" {
    var module = runtime_bitmap_sample.RuntimeBitmapSample{};
    const second_word_base = runtime_bitmap_sample.RuntimeBitmapSample.bitmap_nbits / 2;

    try module.initWithSetBits(&.{ 1, 5, second_word_base + 2 });
    ...
    try std.testing.expectError(
        ...
        module.initWithSetBits(&.{ 0, runtime_bitmap_sample.RuntimeBitmapSample.bitmap_nbits - 1 }),
    );
    ...
    try std.testing.expect(module.isSet(second_word_base + 2));
