...
pub fn build(b: *std.Build) void {
    ...
    const phase3_idr_slot = addPhase3IdrSlotStarterPacket(b, target, optimize);

    const combined_step = b.step(
        "phase3-bitmap-cpumask-policy-idr-slot-test",
        "Run the shared Phase 3 bitmap/cpumask, policy, and IDR-slot starter packets from zigux/tests",
    );
    combined_step.dependOn(&phase3_bitmap_cpumask.step);
    combined_step.dependOn(&phase3_policy.step);
    combined_step.dependOn(&phase3_idr_slot.step);

    const test_step = b.step(
        "test",
        "Run the Lane 04 Phase 3 bitmap/cpumask, policy, and IDR-slot harness shard",
    );
