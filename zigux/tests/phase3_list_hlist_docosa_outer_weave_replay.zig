...
const route = [_]usize{ 21, 0, 19, 2, 17, 4, 15, 6, 13, 8, 11, 10, 12, 9, 14, 7, 16, 5, 18, 3, 20, 1 };
const detached_index = 22;

fn resetEntries(entries: *[23]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[23]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[23]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[23]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[23]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[23]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[23]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[23]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "docosa outer weave keeps forward list and hlist visibility before repair" {
    var entries: [23]Entry = undefined;
...
test "docosa outer weave staged repair crosses the middle clasp" {
    var entries: [23]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 16);
    repairHListPrefix(&hlist_head, &entries, 16);

    const list_after_weave = list_view.ListView.init(&list_head);
    const hlist_after_weave = hlist_view.HListView.init(&hlist_head);
    try expectListRoute(list_after_weave, &entries);
    try expectHListRoute(hlist_after_weave, &entries);

    const list_break = list_after_weave.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 16), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[15]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_weave.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 16), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[15]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
