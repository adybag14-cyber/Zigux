...
test "twohundredeleven raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 56, 986, 2024, 3004, 3884 };
...
test "twohundredeleven constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredeleven keeps one centered raw and even payload stride" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
