...
test "leading empty bank keeps delayed first cpu and first gap aligned" {
    const capacity = bitmap_view.word_bits + 13;
    const words = [_]usize{
        0,
        bit(1) | bit(4) | bit(9) | bit(12) | bit(13) | bit(15),
    };
    ...
    try testing.expect(cpumask.hasCpu(bitmap_view.word_bits + 12));
    try testing.expect(!cpumask.hasCpu(bitmap_view.word_bits + 11));
    try testing.expect(!cpumask.hasCpu(bitmap_view.word_bits + 8));
}

test "leading empty bank keeps subset and overlap bounded under tail-only noise" {
    const capacity = bitmap_view.word_bits + 13;
    const base_words = [_]usize{
        0,
        bit(1) | bit(4) | bit(9) | bit(12),
    };
    const subset_words = [_]usize{
        0,
        bit(4) | bit(12) | bit(14) | bit(15),
    };
