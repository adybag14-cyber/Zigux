...
const errno: isize = -328;
const next_errno: isize = -329;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 328 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 328 raw equation stays between its errno neighbors" {
...
test "errno 328 adjacent raw lanes keep value and pointer decoders precise" {
