...
test "phase1 host-tools smoke reuses list_sort comparator context across repeated reordering" {
    ...
    head.init();
    var entries = [_]ListSortSmokeEntry{
        .{ .key = 2, .ordinal = 0 },
        .{ .key = 5, .ordinal = 1 },
        .{ .key = 1, .ordinal = 2 },
        .{ .key = 5, .ordinal = 3 },
    ...
    while (current != &head) : (current = current.?.next) {
        const entry: *const ListSortSmokeEntry = @fieldParentPtr("node", current.?);
        keys[idx] = entry.key;
