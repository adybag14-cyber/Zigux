...
test "representative 69-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 979, 2021, 3025, 4025 };
...
test "every interior sixtynine window keeps consecutive codes and alternating constructor families" {
...
test "sixtynine windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
