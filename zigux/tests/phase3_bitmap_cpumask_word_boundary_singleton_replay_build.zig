...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_word_boundary_singleton_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named_step = b.step(
        "phase3-bitmap-cpumask-word-boundary-singleton-replay",
        "Run the Phase 3 bitmap/cpumask word-boundary singleton replay",
    );
    named_step.dependOn(&run_unit_tests.step);

    const test_step = b.step("test", "Run the Phase 3 bitmap/cpumask word-boundary singleton replay");
