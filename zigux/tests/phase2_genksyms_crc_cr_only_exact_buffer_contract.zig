...
test "phase2 genksyms crc skips an exact CR-only payload before visible spillover" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + "visible\n".len);
    ...
    try std.testing.expect(!containsCrc(records.items, crc32("\rvisible")));
}

test "phase2 genksyms crc skips repeated CR-only hidden chunks before tail" {
    var input = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + "\r\r\ntail\n".len);
    defer input.deinit(std.testing.allocator);
    try input.appendNTimes(std.testing.allocator, '\r', c_line_payload_len);
    ...
    try std.testing.expectEqual(@as(usize, 1), records.items.len);
    try std.testing.expectEqual(@as(usize, "tail".len), records.items[0].len);
    try std.testing.expectEqual(@as(u8, 't'), records.items[0].first);
    try std.testing.expectEqual(@as(u8, 'l'), records.items[0].last);
    try std.testing.expectEqual(crc32("tail"), records.items[0].crc);
    try std.testing.expect(!containsCrc(records.items, crc32("\r\r")));
    try std.testing.expect(!containsCrc(records.items, crc32("\r\rtail")));
}
