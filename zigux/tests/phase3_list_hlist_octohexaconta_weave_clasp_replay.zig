...
const staged_break_index = 48;
const route = [_]usize{
    67, 0,  42, 41, 66, 1,  43, 40, 65, 2,  44, 39, 64, 3,
    45, 38, 63, 4,  46, 37, 62, 5,  47, 36, 61, 6,  48, 35,
    60, 7,  49, 34, 59, 8,  50, 33, 58, 9,  51, 32, 57, 10,
    52, 31, 56, 11, 53, 30, 55, 12, 54, 29, 28, 13, 27, 26,
...
test "octohexaconta weave-clasp list route exposes staged backlink repair" {
...
test "octohexaconta weave-clasp hlist route exposes staged pprev repair" {
    var seen = [_]bool{false} ** node_count;
    for (route) |node_index| {
        try std.testing.expect(node_index < node_count);
        try std.testing.expect(!seen[node_index]);
