...
test "twohundredsixtysix raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 52, 986, 2018, 2998, 3829 };
...
test "twohundredsixtysix constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredsixtysix keeps split raw and payload centers adjacent" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
