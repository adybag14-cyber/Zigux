...
pub fn build(b: *std.Build) void {
    ...
    const phase3_atomic = addAtomicTest(b, target, optimize);

    const step = b.step(
        "phase3-export-atomic-test",
        "Run the shared Phase 3 export/UAPI layout and atomic helper packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_atomic.step);
}
