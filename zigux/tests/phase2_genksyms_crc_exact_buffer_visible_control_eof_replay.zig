...
test "lane19 exact-buffer EOF replay preserves visible low control bytes in a continuation" {
    var exact_then_visible_controls = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 4);
