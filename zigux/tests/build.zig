...
fn addPhase3XarraySlotPublicReplay(
    b: *std.Build,
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
) *std.Build.Step.Run {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_public_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    root_module.addImport("err_ptr", err_ptr);
    ...
    const tests = b.addTest(.{
        .name = "phase3-xarray-slot-public-replay",
        .root_module = root_module,
    });
    return b.addRunArtifact(tests);
}

fn addPhase3XarraySlotConstructorWindows(
    b: *std.Build,
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
) *std.Build.Step.Run {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_constructor_windows.zig"),
        .target = target,
        .optimize = optimize,
    });
    root_module.addImport("err_ptr", err_ptr);
    ...
    const tests = b.addTest(.{
        .name = "phase3-xarray-slot-constructor-windows",
        .root_module = root_module,
    });
    return b.addRunArtifact(tests);
}
...
pub fn build(b: *std.Build) void {
    ...
    const phase3_xarray_slot_public_replay = addPhase3XarraySlotPublicReplay(
        b,
        target,
        optimize,
    );
    const phase3_xarray_slot_constructor_windows = addPhase3XarraySlotConstructorWindows(
        b,
        target,
        optimize,
    );
    ...
    const phase3_xarray_slot_public_replay_step = b.step(
        "phase3-xarray-slot-public-replay",
        "Run the shared Phase 3 xarray-slot public replay from zigux/tests",
    );
    phase3_xarray_slot_public_replay_step.dependOn(&phase3_xarray_slot_public_replay.step);

    const phase3_xarray_slot_constructor_windows_step = b.step(
        "phase3-xarray-slot-constructor-windows",
        "Run the shared Phase 3 xarray-slot constructor-window replay from zigux/tests",
    );
    phase3_xarray_slot_constructor_windows_step.dependOn(
        &phase3_xarray_slot_constructor_windows.step,
    );
