...
pub fn build(b: *std.Build) void {
    ...
    const phase3_idr_slot_step = b.step(
        ...
        "Run the shared Phase 3 idr-slot starter packet from zigux/tests",
    );
    phase3_idr_slot_step.dependOn(&phase3_idr_slot_starter_packet.step);

    const phase3_idr_slot_dump_step = b.step(
        "phase3-idr-slot-dump",
        "Run the shared Phase 3 idr-slot dump from zigux/tests",
    );
    phase3_idr_slot_dump_step.dependOn(&phase3_idr_slot_dump.step);
