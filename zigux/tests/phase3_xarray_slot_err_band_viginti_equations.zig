...
test "representative err-band vigintis keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3060, 4074 };
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [20]usize = undefined;
        var slots: [20]xarray_slot_view.SlotView = undefined;
        ...
        const sum_019 = @as(u128, raws[0]) + @as(u128, raws[19]);
        const sum_118 = @as(u128, raws[1]) + @as(u128, raws[18]);
        const sum_217 = @as(u128, raws[2]) + @as(u128, raws[17]);
        const sum_316 = @as(u128, raws[3]) + @as(u128, raws[16]);
        const sum_415 = @as(u128, raws[4]) + @as(u128, raws[15]);
        const sum_514 = @as(u128, raws[5]) + @as(u128, raws[14]);
        const sum_613 = @as(u128, raws[6]) + @as(u128, raws[13]);
        const sum_712 = @as(u128, raws[7]) + @as(u128, raws[12]);
        const sum_811 = @as(u128, raws[8]) + @as(u128, raws[11]);
        const sum_910 = @as(u128, raws[9]) + @as(u128, raws[10]);
        try testing.expectEqual(sum_019, sum_118);
        try testing.expectEqual(sum_118, sum_217);
        try testing.expectEqual(sum_217, sum_316);
        try testing.expectEqual(sum_316, sum_415);
        try testing.expectEqual(sum_415, sum_514);
        try testing.expectEqual(sum_514, sum_613);
        try testing.expectEqual(sum_613, sum_712);
        try testing.expectEqual(sum_712, sum_811);
        try testing.expectEqual(sum_811, sum_910);
        ...
        const nineteenth_payload = expectedRejectedPayload(left_residue + 18);
        ...
        try testing.expectEqual(seventeenth_payload + 1, nineteenth_payload);
        try testing.expectEqual(first_payload + nineteenth_payload, third_payload + seventeenth_payload);
        try testing.expectEqual(third_payload + seventeenth_payload, fifth_payload + fifteenth_payload);
        try testing.expectEqual(fifth_payload + fifteenth_payload, seventh_payload + thirteenth_payload);
        try testing.expectEqual(seventh_payload + thirteenth_payload, ninth_payload + eleventh_payload);
        ...
        try testing.expectEqual(raws[18], (nineteenth_payload << 1) | xa_value.value_tag_mask);
    }
}

test "every interior viginti keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 19 <= err_band_span) : (even_residue += 2) {
        ...
        const nineteenth_payload = expectedRejectedPayload(even_residue + 18);
        ...
        const eighteenth_slot = xarray_slot_view.fromErrorCode(expectedCode(even_residue + 17));
        const nineteenth_slot = xarray_slot_view.fromRaw(rawFromResidue(even_residue + 18));
        const twentieth_slot = xarray_slot_view.fromErrorCode(expectedCode(even_residue + 19));
        ...
        try testing.expectEqual(rawFromResidue(even_residue + 17), eighteenth_slot.rawValue());
        try testing.expectEqual(rawFromResidue(even_residue + 18), nineteenth_slot.rawValue());
        try testing.expectEqual(rawFromResidue(even_residue + 19), twentieth_slot.rawValue());
        ...
        try testing.expectEqual(seventeenth_slot.errorCode().? + 1, eighteenth_slot.errorCode().?);
        try testing.expectEqual(eighteenth_slot.errorCode().? + 1, nineteenth_slot.errorCode().?);
        try testing.expectEqual(nineteenth_slot.errorCode().? + 1, twentieth_slot.errorCode().?);
        ...
        try testing.expect(!xa_value.canRepresent(nineteenth_payload));
        ...
        try testing.expectError(error.ValueWouldOverlapErrPtr, xarray_slot_view.fromValue(nineteenth_payload));
    }
}

test "viginti windows preserve centered equations across the middle seam" {
    var even_residue: usize = 0;
    while (even_residue + 19 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
            ...
            expectedCode(even_residue + 17),
            expectedCode(even_residue + 18),
            expectedCode(even_residue + 19),
        };
        ...
        try testing.expectEqual(codes[0] + codes[19], codes[1] + codes[18]);
        try testing.expectEqual(codes[1] + codes[18], codes[2] + codes[17]);
        try testing.expectEqual(codes[2] + codes[17], codes[3] + codes[16]);
        try testing.expectEqual(codes[3] + codes[16], codes[4] + codes[15]);
        try testing.expectEqual(codes[4] + codes[15], codes[5] + codes[14]);
        try testing.expectEqual(codes[5] + codes[14], codes[6] + codes[13]);
        try testing.expectEqual(codes[6] + codes[13], codes[7] + codes[12]);
        try testing.expectEqual(codes[7] + codes[12], codes[8] + codes[11]);
        try testing.expectEqual(codes[8] + codes[11], codes[9] + codes[10]);
        try testing.expectEqual(first_payload + nineteenth_payload, third_payload + seventeenth_payload);
        try testing.expectEqual(third_payload + seventeenth_payload, fifth_payload + fifteenth_payload);
        try testing.expectEqual(fifth_payload + fifteenth_payload, seventh_payload + thirteenth_payload);
        try testing.expectEqual(seventh_payload + thirteenth_payload, ninth_payload + eleventh_payload);
        try testing.expectEqual(first_payload + 9, nineteenth_payload);
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 19)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 18)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 18)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 17)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 17)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 16)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 16)),
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 15)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 15)),
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 14)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 14)),
            @as(u128, rawFromResidue(even_residue + 6)) + @as(u128, rawFromResidue(even_residue + 13)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 6)) + @as(u128, rawFromResidue(even_residue + 13)),
            @as(u128, rawFromResidue(even_residue + 7)) + @as(u128, rawFromResidue(even_residue + 12)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 7)) + @as(u128, rawFromResidue(even_residue + 12)),
            @as(u128, rawFromResidue(even_residue + 8)) + @as(u128, rawFromResidue(even_residue + 11)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 8)) + @as(u128, rawFromResidue(even_residue + 11)),
            @as(u128, rawFromResidue(even_residue + 9)) + @as(u128, rawFromResidue(even_residue + 10)),
        );
    }
}
