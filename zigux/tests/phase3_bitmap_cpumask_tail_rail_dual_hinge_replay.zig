...
test "tail rail dual hinge keeps two shared tail rails aligned under a bounded union" {
    const capacity = bitmap_view.word_bits + 11;
    const tail_base = bitmap_view.word_bits;

    const union_words = [_]usize{
        std.math.maxInt(usize),
        bit(0) | bit(1) | bit(2) | bit(4) | bit(5) | bit(7) | bit(8) | bit(10),
    };
    ...
    const right_words = [_]usize{
        0,
        bit(2) | bit(4) | bit(5) | bit(7) | bit(8) | bit(10),
    };
    const noise_words = [_]usize{
        0,
        bit(11) | bit(13) | bit(15),
    };
    ...
    try testing.expectEqual(@as(?usize, tail_base + 3), union_bitmap.firstClearBit());
...
test "tail rail dual hinge keeps a hinge-only peer visible without promoting side rails" {
    const capacity = bitmap_view.word_bits + 11;
    const tail_base = bitmap_view.word_bits;
    ...
    try testing.expectEqual(@as(usize, 2), hinge_bitmap.countSetBits());
    try testing.expectEqual(@as(?usize, tail_base + 4), hinge_bitmap.firstSetBit());
    try testing.expectEqual(@as(?usize, 0), hinge_bitmap.firstClearBit());
    try testing.expectEqual(@as(usize, 2), hinge_only.countPresentCpus());
    try testing.expectEqual(@as(?usize, tail_base + 4), hinge_only.firstCpu());
    try testing.expectEqual(@as(?usize, 0), hinge_only.firstMissingCpu());
    try testing.expect(hinge_only.hasCpu(tail_base + 4));
    try testing.expect(hinge_only.hasCpu(tail_base + 7));
    try testing.expect(!hinge_only.hasCpu(tail_base + 5));
    try testing.expect(!hinge_only.hasCpu(capacity - 1));
