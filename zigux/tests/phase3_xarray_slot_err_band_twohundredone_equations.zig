...
test "twohundredone raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 38, 968, 2006, 2986, 3894 };
...
test "twohundredone constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredone keeps one centered raw and a three-payload center" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
