...
const errno: isize = -494;
const next_errno: isize = -495;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 494 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 494 raw equation stays between its errno neighbors" {
...
test "errno 494 adjacent raw lanes keep value and pointer decoders precise" {
