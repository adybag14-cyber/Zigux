...
pub fn build(b: *std.Build) void {
    ...
    const test_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_slot_err_code_sign_table_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-xarray-slot-err-code-sign-table-replay",
        "Run the Phase 3 xarray slot err-code sign-table replay.",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Phase 3 xarray slot err-code sign-table replay.");
