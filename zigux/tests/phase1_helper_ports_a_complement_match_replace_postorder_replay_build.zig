...
pub fn build(b: *std.Build) void {
    ...
    const replay_root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_complement_match_replace_postorder_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-complement-match-replace-postorder-replay-tests",
        .root_module = replay_root_module,
    });
    const run_replay_tests = b.addRunArtifact(replay_tests);
    run_replay_tests.setCwd(b.path("../.."));

    const replay_step = b.step("phase1-helper-ports-a-complement-match-replace-postorder-replay", "Run Lane 06 helper ports A complement/match/replace/postorder replay");
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run Lane 06 helper ports A complement/match/replace/postorder replay");
