...
test "representative 95-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 953, 1995, 2997, 3999 };
...
test "every interior ninetyfive window keeps consecutive codes and alternating constructor families" {
...
test "ninetyfive windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
