...
test "representative 50-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1000, 2038, 3044, 4044 };
...
test "every interior fifty window keeps consecutive codes and alternating constructor families" {
...
test "fifty windows preserve dual-center seam equations and a centered payload slot" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
