...
test "representative 143-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2952, 3950 };
...
test "every interior onehundredfortythree window keeps consecutive codes and alternating constructor families" {
...
test "onehundredfortythree windows preserve centered equations around the middle err slot and split rejected-payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
