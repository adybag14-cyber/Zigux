...
test "representative 151-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2950, 3944 };
...
test "every interior onehundredfiftyone window keeps consecutive codes and alternating constructor families" {
...
test "onehundredfiftyone windows preserve a centered raw seam and a split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
