...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_c_ledger_fold_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-c-ledger-fold-replay-tests",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);
    run_tests.setCwd(b.path("../.."));

    const test_step = b.step("test", "Run the Lane 10 ledger-fold helper ports C replay");
    test_step.dependOn(&run_tests.step);

    const replay_step = b.step(
        "phase1-helper-ports-c-ledger-fold-replay",
        "Run the Lane 10 ledger-fold helper ports C replay",
    );
