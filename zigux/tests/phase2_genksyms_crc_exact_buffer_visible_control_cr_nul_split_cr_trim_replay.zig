...
test "lane19 exact-buffer split replay preserves visible low control bytes and leading carriage returns before an embedded NUL while trimming trailing carriage returns and keeping the next line visible" {
    var exact_then_controls_and_leading_cr_then_trailing_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 12);
    defer exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.deinit(std.testing.allocator);
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, '\x08');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, '\x0c');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, '\x01');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, '\r');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, '\r');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, 'b');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, '\r');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, '\r');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, 0);
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, 'c');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.append(std.testing.allocator, '\n');
    try exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.appendSlice(std.testing.allocator, "x\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try gen.runGenksymsCrc(exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.items, &capture);

    const exact_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32(exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.items[0..c_line_payload_len])},
    );
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_controls_and_leading_cr_then_trailing_cr_then_nul.items)});
