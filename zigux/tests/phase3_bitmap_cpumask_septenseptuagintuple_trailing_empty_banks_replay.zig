...
const tail_b = tail_base + 11;
const tail_c = tail_base + 16;
...
fn fixtureWords() [word_count]Word {
    ...
    words[tail_word_index] =
        ...
        localBit(tail_base + tail_valid_bits + 9);
    return words;
}
...
fn supersetWords() [word_count]Word {
    ...
    words[0] |= localBit(41);
    words[tail_word_index] |= localBit(tail_base + 7);
    return words;
}
...
fn disjointWords() [word_count]Word {
    ...
    words[tail_word_index] =
        ...
        localBit(tail_base + 12) |
        localBit(tail_base + tail_valid_bits) |
        localBit(tail_base + tail_valid_bits + 5);
    return words;
}

test "bitmap view crosses septenseptuagintuple empty banks before masked tail" {
    ...
    try std.testing.expectEqual(@as(usize, 5), view.countSetBits());
    try std.testing.expectEqual(@as(?usize, first_bit), view.firstSetBit());
    try std.testing.expectEqual(@as(?usize, second_bit), view.nextSetBit(first_bit + 1));
    try std.testing.expectEqual(@as(?usize, tail_a), view.nextSetBit(second_bit + 1));
...
test "cpumask view mirrors bitmap traversal across septenseptuagintuple empty banks" {
    const words = fixtureWords();
    ...
    try std.testing.expect(mask.hasCpu(second_bit));
    try std.testing.expect(mask.hasCpu(tail_c));
    try std.testing.expect(!mask.hasCpu(tail_base + 7));
    ...
    try std.testing.expectEqual(bitmap.nextSetBit(second_bit + 1), mask.nextCpu(second_bit + 1));
    try std.testing.expectEqual(bitmap.nextSetBit(tail_b + 1), mask.nextCpu(tail_b + 1));
    try std.testing.expectEqual(@as(?usize, null), mask.nextCpu(tail_c + 1));
    try std.testing.expectEqual(bitmap.firstClearBit(), mask.firstMissingCpu());
    try std.testing.expectEqual(bitmap.nextClearBit(tail_a + 1), mask.nextMissingCpu(tail_a + 1));
}

test "bitmap and cpumask masks ignore septenseptuagintuple tail noise in relations" {
    ...
    try std.testing.expect(!disjoint.intersects(base));
