...
const route = [_]usize{
    47, 0, 25, 22, 46, 1, 26, 21, 45, 2,  27, 20, 44, 3,  28, 19,
    43, 4, 29, 18, 42, 5, 30, 17, 41, 6,  31, 16, 40, 7,  32, 15,
    39, 8, 33, 14, 38, 9, 34, 13, 37, 10, 35, 12, 36, 11, 24, 23,
};
...
test "octotetraconta mesh-ring exposes the same visible route through list and hlist" {
...
test "octotetraconta mesh-ring repairs staged reverse links after the mesh ring" {
    var entries = [_]Entry{.{}} ** (visible_count + 1);
    var head = ListHead{ .next = 0, .prev = 0 };
    var hhead = HListHead{ .first = 0 };

    reset(&entries, &head, &hhead);
    const gate_position = 34;
