...
const errno: isize = -493;
const next_errno: isize = -494;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 493 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 493 raw equation stays between its errno neighbors" {
...
test "errno 493 adjacent raw lanes keep value and pointer decoders precise" {
