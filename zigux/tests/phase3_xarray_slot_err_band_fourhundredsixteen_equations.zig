...
const errno: isize = -416;
const next_errno: isize = -417;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 416 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 416 raw equation stays between its errno neighbors" {
...
test "errno 416 adjacent raw lanes keep value and pointer decoders precise" {
