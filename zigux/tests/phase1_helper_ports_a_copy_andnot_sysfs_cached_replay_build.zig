...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_copy_andnot_sysfs_cached_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-copy-andnot-sysfs-cached-replay",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const replay_step = b.step("phase1-helper-ports-a-copy-andnot-sysfs-cached-replay", "Run Lane 06 copy/andnot sysfs cached replay");
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run Lane 06 copy/andnot sysfs cached replay");
