...
const errno: isize = -338;
const next_errno: isize = -339;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 338 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 338 raw equation stays between its errno neighbors" {
...
test "errno 338 adjacent raw lanes keep value and pointer decoders precise" {
