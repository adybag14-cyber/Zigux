...
const route = [_]usize{ 28, 0, 26, 2, 24, 4, 22, 6, 20, 8, 18, 10, 16, 12, 14, 13, 15, 11, 17, 9, 19, 7, 21, 5, 23, 3, 25, 1, 27 };
const detached_index = 29;

fn resetEntries(entries: *[30]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[30]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[30]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[30]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[30]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[30]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[30]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[30]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "enneacosa center clasp keeps forward list and hlist visibility before repair" {
    var entries: [30]Entry = undefined;
...
test "enneacosa center clasp staged repair crosses the middle hinge" {
    var entries: [30]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 23);
    repairHListPrefix(&hlist_head, &entries, 23);

    const list_after_clasp = list_view.ListView.init(&list_head);
    const hlist_after_clasp = hlist_view.HListView.init(&hlist_head);
    try expectListRoute(list_after_clasp, &entries);
    try expectHListRoute(hlist_after_clasp, &entries);

    const list_break = list_after_clasp.firstBrokenBacklink().?;
    try std.testing.expectEqual(@as(usize, 23), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[22]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_clasp.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 23), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[22]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
