...
test "representative 59-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 989, 2031, 3035, 4035 };
    ...
    for (left_residues) |left_residue| {
        ...
        const center_payload = expectedRejectedPayload(left_residue + center_index);
        for (0..window_len / 2) |pair_idx| {
            ...
            if (odd_idx < center_index) {
                const mirrored_payload = expectedRejectedPayload(left_residue + ((window_len - 1) - odd_idx));
...
test "every interior fiftynine window keeps consecutive codes and alternating constructor families" {
...
test "fiftynine windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
        const center_raw = rawFromResidue(odd_residue + center_index);
        const center_payload = expectedRejectedPayload(odd_residue + center_index);
        ...
        for (0..window_len / 4) |pair_idx| {
            const left_payload_residue = odd_residue + ((pair_idx * 2) + 1);
            const right_payload_residue = odd_residue + ((window_len - 2) - (pair_idx * 2));
            try testing.expectEqual(
                expectedRejectedPayload(left_payload_residue) + expectedRejectedPayload(right_payload_residue),
                center_payload * 2,
            );
        }

        try testing.expectEqual(
            expectedRejectedPayload(odd_residue + 1) + (center_index - 1),
            expectedRejectedPayload(odd_residue + (window_len - 2)),
        );
        try testing.expectEqual(
            expectedRejectedPayload(odd_residue + center_index - 2) + 1,
            center_payload,
        );
        try testing.expectEqual(
            center_payload + 1,
            expectedRejectedPayload(odd_residue + center_index + 2),
        );
        try testing.expectEqual(
            expectedCode(odd_residue + center_index - 1) + 2,
            expectedCode(odd_residue + center_index + 1),
        );
    }
}
