...
pub fn build(b: *std.Build) void {
    ...
    const test_step = b.step(
        "phase3-ida-range-bitmap-cpumask-test",
        "Run the Phase 3 IDA range and bitmap/cpumask starter packets together",
    );
    test_step.dependOn(&run_ida_range_tests.step);
    test_step.dependOn(&run_bitmap_cpumask_tests.step);

    const default_test_step = b.step("test", "Run the Phase 3 IDA range and bitmap/cpumask starter packets");
