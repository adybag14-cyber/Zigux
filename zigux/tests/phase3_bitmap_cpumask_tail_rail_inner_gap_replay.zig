...
test "tail rail inner gap keeps a shared hinge and a single valid gap aligned" {
    const capacity = bitmap_view.word_bits + 10;
    const tail_base = bitmap_view.word_bits;

    const union_words = [_]usize{
        std.math.maxInt(usize),
        bit(0) | bit(1) | bit(2) | bit(4) | bit(5) | bit(6) | bit(7) | bit(8) | bit(9),
    };
...
test "tail rail inner gap keeps the missing probe outside the bounded union" {
    const capacity = bitmap_view.word_bits + 10;
    const tail_base = bitmap_view.word_bits;
    ...
    try testing.expectEqual(@as(?usize, tail_base + 3), gap_probe_bitmap.firstSetBit());
    ...
    try testing.expectEqual(@as(?usize, tail_base + 3), gap_probe.firstCpu());
    try testing.expectEqual(@as(?usize, 0), gap_probe.firstMissingCpu());
    try testing.expect(gap_probe.hasCpu(tail_base + 3));
    try testing.expect(!gap_probe.hasCpu(tail_base + 4));
