...
pub fn build(b: *std.Build) void {
    ...
    const tests = b.addTest(.{
        .name = "phase3-ida-range-test",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step(
        "phase3-ida-range-test",
        "Run the Phase 3 IDA range starter packet through its standalone build root",
    );
    test_step.dependOn(&run_tests.step);

    const default_step = b.step("test", "Run the Phase 3 IDA range standalone test");
