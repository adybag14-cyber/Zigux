...
test "exact version survives before inline reference argument" {
    const args = [_][]const u8{
        "--version",
        "--reference=Module.symvers",
    };
    const expected_references = [_][]const u8{"Module.symvers"};

    try expectInlineLongRequest(&args, 1, &expected_references, null);
}

test "abbreviated version survives before abbreviated inline dump-types argument" {
    const args = [_][]const u8{
        "--ver",
        "--dump-t=types.out",
    };

    try expectInlineLongRequest(&args, 1, &.{}, "types.out");
}

test "mixed versions survive before inline long request arguments" {
    const args = [_][]const u8{
        "-V",
        "--version",
    ...
    try expectInlineLongRequest(&args, 2, &expected_references, "types.symtypes");
}
