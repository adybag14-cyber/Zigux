...
const errno: isize = -317;
const next_errno: isize = -318;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 317 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 317 raw equation stays between its errno neighbors" {
...
test "errno 317 adjacent raw lanes keep value and pointer decoders precise" {
