...
const errno: isize = -365;
const next_errno: isize = -366;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 365 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 365 raw equation stays between its errno neighbors" {
...
test "errno 365 adjacent raw lanes keep value and pointer decoders precise" {
