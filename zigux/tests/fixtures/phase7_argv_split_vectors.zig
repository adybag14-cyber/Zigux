...
test "phase 7 argv split fixture vectors stay reviewable" {
    try std.testing.expectEqual(@as(usize, 4), phase7_argv_split_vectors.len);
    ...
    try std.testing.expectEqualStrings("first_nul_truncation_keeps_tail_outside_packet", phase7_argv_split_vectors[2].name);
    try std.testing.expectEqualStrings("quoted_tokens_stay_whitespace_split", phase7_argv_split_vectors[3].name);
    try std.testing.expect(phase7_argv_split_vectors[1].expect_empty_storage_view);
    try std.testing.expect(phase7_argv_split_vectors[2].expect_first_nul_truncation);
    try std.testing.expectEqualStrings("gamma", phase7_argv_split_vectors[0].expected_tokens[2]);
    try std.testing.expectEqualStrings("beta", phase7_argv_split_vectors[2].expected_tokens[1]);
