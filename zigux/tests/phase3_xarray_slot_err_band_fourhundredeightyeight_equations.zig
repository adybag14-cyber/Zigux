...
const errno: isize = -488;
const next_errno: isize = -489;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 488 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 488 raw equation stays between its errno neighbors" {
...
test "errno 488 adjacent raw lanes keep value and pointer decoders precise" {
