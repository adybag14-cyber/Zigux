...
test "double-bank partial tail last-present keeps only the final valid tail cpu visible" {
    ...
    try testing.expectEqual((bitmap_view.word_bits * 2) + 1, bitmap.countSetBits());
...
test "double-bank partial tail last-present peers stay equivalent despite invalid extras" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    const last_valid = capacity - 1;
    ...
    const last_present_only_words = [_]usize{
        0,
        0,
        bit(4),
    };
    ...
    const last_present_only = cpumask_view.CpuMaskView.init(last_present_only_words[0..], capacity);
    ...
    try testing.expect(base.intersects(last_present_only));
    try testing.expect(last_present_only.intersects(base));
    ...
    try testing.expect(last_present_only.hasCpu(last_valid));
}
