...
pub fn build(b: *std.Build) void {
    ...
    const test_step = b.step(
        "phase3-abi-policy-pair-test",
        "Run the shared Phase 3 ABI core packet and paired policy helper replays through a standalone build shard",
    );
    test_step.dependOn(&phase3_abi_core_packet.step);
    test_step.dependOn(&allocator_policy.step);
    test_step.dependOn(&panic_policy.step);
}
