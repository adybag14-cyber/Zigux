...
const errno: isize = -363;
const next_errno: isize = -364;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 363 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 363 raw equation stays between its errno neighbors" {
...
test "errno 363 adjacent raw lanes keep value and pointer decoders precise" {
