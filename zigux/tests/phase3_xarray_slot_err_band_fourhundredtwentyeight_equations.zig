...
const errno: isize = -428;
const next_errno: isize = -429;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 428 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 428 raw equation stays between its errno neighbors" {
...
test "errno 428 adjacent raw lanes keep value and pointer decoders precise" {
