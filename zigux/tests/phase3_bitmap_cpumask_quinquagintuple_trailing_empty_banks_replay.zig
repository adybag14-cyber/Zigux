...
fn replayWords() [tail_word_index + 1]usize {
    ...
    words[0] = (@as(usize, 1) << 2) | (@as(usize, 1) << 11);
...
test "lane27 quinquagintuple replay keeps bitmap discovery stable across 50 empty banks" {
    ...
    try testing.expectEqual(@as(?usize, 11), view.nextSetBit(3));
    try testing.expectEqual(@as(?usize, tail_base), view.nextSetBit(12));
    ...
    try testing.expectEqual(@as(?usize, 12), view.nextClearBit(12));
    try testing.expectEqual(@as(?usize, tail_base + 1), view.nextClearBit(tail_base + 1));
}

test "lane27 quinquagintuple replay keeps cpumask traversal aligned with bitmap traversal" {
    const words = replayWords();
    ...
    try testing.expectEqual(bitmap.nextSetBit(12), mask.nextCpu(12));
...
test "lane27 quinquagintuple replay ignores tail-only noise in subset and overlap checks" {
    const base_words = replayWords();
    var superset_words = replayWords();
    var tail_noise_only_words = [_]usize{0} ** (tail_word_index + 1);
    superset_words[0] |= (@as(usize, 1) << 19);
