...
test "representative 137-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2958, 3958 };
...
test "every interior onehundredthirtyseven window keeps consecutive codes and alternating constructor families" {
...
test "onehundredthirtyseven windows preserve centered equations around the middle err slot and middle rejected payload" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
