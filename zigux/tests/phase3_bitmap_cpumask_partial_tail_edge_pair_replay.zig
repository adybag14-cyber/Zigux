...
test "partial tail edge-pair keeps edge clear-bit discovery aligned" {
    const capacity = bitmap_view.word_bits + 5;
    const words = [_]usize{
        std.math.maxInt(usize),
        bit(1) | bit(2) | bit(3) | bit(6) | bit(12),
    };
    ...
    try testing.expectEqual(@as(?usize, bitmap_view.word_bits), bitmap.firstClearBit());
...
test "partial tail edge-pair keeps subset and overlap blind to tail-only noise" {
    const capacity = bitmap_view.word_bits + 5;
