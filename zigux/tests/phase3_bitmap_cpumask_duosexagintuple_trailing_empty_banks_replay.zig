...
const empty_bank_count = 62;
const tail_word_index = empty_bank_count + 1;
const tail_offset = 11;
...
fn replayWords() [tail_word_index + 1]usize {
    ...
    words[0] = (@as(usize, 1) << 2) | (@as(usize, 1) << 19);
...
test "lane27 duosexagintuple replay keeps bitmap discovery stable across 62 empty banks" {
    ...
    try testing.expectEqual(@as(?usize, 19), view.nextSetBit(3));
    try testing.expectEqual(@as(?usize, tail_base), view.nextSetBit(20));
    ...
    try testing.expectEqual(@as(?usize, 20), view.nextClearBit(20));
    try testing.expectEqual(@as(?usize, tail_base + 1), view.nextClearBit(tail_base + 1));
}

test "lane27 duosexagintuple replay keeps cpumask traversal aligned with bitmap traversal" {
    const words = replayWords();
    ...
    try testing.expectEqual(bitmap.nextSetBit(20), mask.nextCpu(20));
...
test "lane27 duosexagintuple replay ignores tail-only noise in subset and overlap checks" {
