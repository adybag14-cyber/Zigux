...
const errno: isize = -325;
const next_errno: isize = -326;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 325 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 325 raw equation stays between its errno neighbors" {
...
test "errno 325 adjacent raw lanes keep value and pointer decoders precise" {
