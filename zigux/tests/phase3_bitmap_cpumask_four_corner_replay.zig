...
test "four-corner replay keeps bitmap and cpumask summaries aligned across a three-word rim" {
...
test "four-corner replay keeps subset and overlap checks bounded at the declared rim" {
    const capacity = (2 * bitmap_view.word_bits) + 5;
    ...
    const superset_words = [_]usize{
        bit(0) | bit(7) | bit(bitmap_view.word_bits - 1),
        bit(0) | bit(11) | bit(bitmap_view.word_bits - 1),
    ...
    try testing.expectEqual(@as(usize, 11), superset_bitmap.countSetBits());
