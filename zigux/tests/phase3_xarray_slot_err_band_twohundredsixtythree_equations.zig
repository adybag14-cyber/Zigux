...
test "twohundredsixtythree raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 49, 981, 2013, 2993, 3832 };
    ...
    for (left_residues) |left_residue| {
        ...
        const center_raw = @as(u128, raws[center_index]);
...
test "twohundredsixtythree constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredsixtythree keeps raw code and payload centers aligned" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
            rawFromResidue(even_residue + center_index),
            err_ptr.fromErrorCode(expectedCode(even_residue + center_index)),
        );
        try testing.expectEqual(
            rawFromResidue(even_residue + center_payload_index),
            (rejectedPayload(even_residue + center_payload_index) << 1) | xa_value.value_tag_mask,
        );
    }
}
