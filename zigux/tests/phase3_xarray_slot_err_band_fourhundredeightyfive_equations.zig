...
const errno: isize = -485;
const next_errno: isize = -486;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 485 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 485 raw equation stays between its errno neighbors" {
...
test "errno 485 adjacent raw lanes keep value and pointer decoders precise" {
