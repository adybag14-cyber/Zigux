...
pub fn build(b: *std.Build) void {
    ...
    const errptr = addPhase3ErrPtrXarrayStarterPacket(b, target, optimize);
    const export_uapi = addPhase3ExportUapiLayout(b, target, optimize);

    const test_step = b.step(
        "phase3-errptr-export-test",
        "Run the focused Phase 3 err_ptr/xarray and export/UAPI starter replays",
    );
    test_step.dependOn(&errptr.step);
    test_step.dependOn(&export_uapi.step);
}
