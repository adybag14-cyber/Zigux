...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_suffix_lift_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const unit_tests = b.addTest(.{
        .name = "phase3-list-hlist-suffix-lift-replay",
        .root_module = root_module,
    });
    const run_unit_tests = b.addRunArtifact(unit_tests);

    const replay_step = b.step(
        "phase3-list-hlist-suffix-lift-replay",
        "Run the Phase 3 list/hlist suffix lift replay",
    );
    replay_step.dependOn(&run_unit_tests.step);

    const test_step = b.step("test", "Run the Phase 3 list/hlist suffix lift replay");
