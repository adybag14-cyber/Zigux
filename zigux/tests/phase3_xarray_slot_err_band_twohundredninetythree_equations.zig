...
test "twohundredninetythree raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 50, 984, 2016, 2996, 3802 };
...
test "twohundredninetythree constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredninetythree keeps split raw and payload centers adjacent" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
            rawFromResidue(even_residue + center_index) + 1,
            rawFromResidue(even_residue + center_index + 1),
        );
        try testing.expectEqual(
