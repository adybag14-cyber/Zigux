...
const errno: isize = -394;
const next_errno: isize = -395;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 394 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 394 raw equation stays between its errno neighbors" {
...
test "errno 394 adjacent raw lanes keep value and pointer decoders precise" {
