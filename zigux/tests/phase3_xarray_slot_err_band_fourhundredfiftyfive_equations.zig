...
const errno: isize = -455;
const next_errno: isize = -456;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 455 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 455 raw equation stays between its errno neighbors" {
...
test "errno 455 adjacent raw lanes keep value and pointer decoders precise" {
