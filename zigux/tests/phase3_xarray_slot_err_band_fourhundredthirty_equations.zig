...
const errno: isize = -430;
const next_errno: isize = -431;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 430 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 430 raw equation stays between its errno neighbors" {
...
test "errno 430 adjacent raw lanes keep value and pointer decoders precise" {
