...
const errno: isize = -367;
const next_errno: isize = -368;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 367 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 367 raw equation stays between its errno neighbors" {
...
test "errno 367 adjacent raw lanes keep value and pointer decoders precise" {
