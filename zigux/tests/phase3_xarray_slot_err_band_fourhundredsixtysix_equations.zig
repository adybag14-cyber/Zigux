...
const errno: isize = -466;
const next_errno: isize = -467;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 466 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 466 raw equation stays between its errno neighbors" {
...
test "errno 466 adjacent raw lanes keep value and pointer decoders precise" {
