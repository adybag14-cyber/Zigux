...
fn addPhase3AbiVersionPair(
    ...
    const abi_tests = b.addTest(.{
        .name = "phase3_abi_version_pair_abi",
    ...
    const version_tests = b.addTest(.{
        .name = "phase3_abi_version_pair_version",
    ...
    const pair = b.step(
        ...
        "Run the Phase 3 ABI binding contract beside the uapi version packet",
    );
    pair.dependOn(&abi_run.step);
    pair.dependOn(&version_run.step);
    return pair;
}
...
pub fn build(b: *std.Build) void {
    ...
    const pair_step = addPhase3AbiVersionPair(b, target, optimize);
    const test_step = b.step("test", "Run the Phase 3 ABI plus uapi version pair-build shard");
