...
fn writeSummary(writer: anytype, summary: abi.ChrdevNotifyAckPolicySummary) !void {
...
pub fn main(init: std.process.Init) !void {
    ...
    try writer.writeAll("{\"constants\":{\"chrdev_notify_ack_policy_force_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_FORCE_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_ack_policy_suppress_expired\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_SUPPRESS_EXPIRED});
    try writer.writeAll(",\"chrdev_notify_ack_policy_coalesce_cookie\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_COALESCE_COOKIE});
    try writer.writeAll(",\"chrdev_notify_ack_policy_status_none\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_STATUS_NONE});
    try writer.writeAll(",\"chrdev_notify_ack_policy_status_acked\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_STATUS_ACKED});
    try writer.writeAll(",\"chrdev_notify_ack_policy_status_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_STATUS_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_ack_policy_status_suppressed\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_STATUS_SUPPRESSED});
    try writer.writeAll(",\"chrdev_notify_ack_policy_status_coalesced\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_STATUS_COALESCED});
    try writer.writeAll(",\"chrdev_notify_ack_policy_status_expired\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_STATUS_EXPIRED});
    try writer.writeAll(",\"chrdev_notify_ack_policy_status_skipped\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_ACK_POLICY_STATUS_SKIPPED});
    try writer.writeAll("},\"acked\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_policy_plan.summarize(acked_view));
    try writer.writeAll("},\"forced_deferred\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_policy_plan.summarize(forced_deferred_view));
    try writer.writeAll("},\"expired\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_policy_plan.summarize(expired_view));
    try writer.writeAll("},\"suppressed_expired\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_policy_plan.summarize(suppressed_expired_view));
    try writer.writeAll("},\"coalesced\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_policy_plan.summarize(coalesced_view));
    try writer.writeAll("},\"skipped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_policy_plan.summarize(skipped_view));
    try writer.writeAll("},\"empty\":{\"is_valid\":");
    try writer.writeAll(if (chrdev_notify_ack_policy_plan.isValid(empty_view)) "true" else "false");
    try writer.writeAll(",\"summary\":");
    try writeSummary(writer, chrdev_notify_ack_policy_plan.summarize(empty_view));
    try writer.writeAll("}}\n");
