...
pub fn build(b: *std.Build) void {
    ...
    const replay_mod = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_shear_gate_handoff_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-bitmap-cpumask-shear-gate-handoff-replay",
        "Run the Lane 27 shear-gate handoff bitmap/cpumask replay",
    );
    replay_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 27 shear-gate handoff replay tests");
