...
const route = [_]usize{ 11, 2, 9, 0, 7, 4, 10, 1, 8, 3, 6, 5 };
const detached_index = 12;

fn resetEntries(entries: *[13]Entry) void {
    for (entries) |*entry| {
...
fn poisonDetached(entries: *[13]Entry) void {
...
fn linkListForward(head: *ListHead, entries: *[13]Entry) void {
...
fn linkHListForward(head: *HListHead, entries: *[13]Entry) void {
...
fn repairListPrefix(head: *ListHead, entries: *[13]Entry, prefix_len: usize) void {
...
fn repairHListPrefix(head: *HListHead, entries: *[13]Entry, prefix_len: usize) void {
...
fn expectListRoute(view: list_view.ListView, entries: *[13]Entry) !void {
...
fn expectHListRoute(view: hlist_view.HListView, entries: *[13]Entry) !void {
    try std.testing.expect(!view.isEmpty());
    try std.testing.expect(!view.isSingular());
    try std.testing.expectEqual(@as(usize, route.len), view.len());
...
test "dozen sidewinder braid preserves forward list and hlist visibility before repair" {
    var entries: [13]Entry = undefined;
...
test "dozen sidewinder braid staged repair crosses both inner turns" {
    var entries: [13]Entry = undefined;
    resetEntries(&entries);
    poisonDetached(&entries);
    ...
    repairListPrefix(&list_head, &entries, 7);
    repairHListPrefix(&hlist_head, &entries, 7);
    ...
    try std.testing.expectEqual(@as(usize, 7), list_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[6]].list)), list_break.expected_prev);
    try std.testing.expectEqual(@as(usize, 0), list_break.actual_prev);

    const hlist_break = hlist_after_prefix.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 7), hlist_break.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&entries[route[6]].hlist.next)), hlist_break.expected_pprev);
    try std.testing.expectEqual(@as(usize, 0), hlist_break.actual_pprev);
