...
pub fn build(b: *std.Build) void {
    ...
    const phase3_abi_bindings = addAbiBindingsTest(b, target, optimize);

    const step = b.step(
        "phase3-export-abi-bindings-test",
        "Run the shared Phase 3 export/UAPI layout and ABI bindings packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_abi_bindings.step);
}
