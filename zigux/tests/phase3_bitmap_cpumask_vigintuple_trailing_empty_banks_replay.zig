...
test "vigintuple trailing empty banks keep early windows aligned and masked tail bits bounded" {
    const capacity = (22 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        bit(1) | bit(4) | bit(9) | bit(13),
        bit(0) | bit(6) | bit(10) | bit(15),
    } ++ [_]usize{0} ** 20 ++ [_]usize{
    ...
    try testing.expect(bitmap.isSet((22 * bitmap_view.word_bits) + 4));
    try testing.expect(bitmap.isSet((22 * bitmap_view.word_bits) + 10));
    ...
    try testing.expect(cpumask.hasCpu((22 * bitmap_view.word_bits) + 0));
}

test "vigintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (22 * bitmap_view.word_bits) + 11;
    ...
    const tail_only_words = [_]usize{0} ** 22 ++ [_]usize{
