...
test "phase3 list/hlist mid braid replay keeps the live list interior route visible while a detached braid stays off the witness path" {
    ...
    stale_left.prev = @intFromPtr(&live_left);
    stale_right.next = @intFromPtr(&live_right);
...
test "phase3 list/hlist mid braid replay reports the first visible list interior break when the live node adopts a detached braid early" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var first = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_left = list_view.ListHead{ .next = 0, .prev = 0 };
    var live_right = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    live_left.prev = @intFromPtr(&first);
    live_right.next = @intFromPtr(&tail);
    live_right.prev = @intFromPtr(&stale_right);
    tail.next = @intFromPtr(&head);
    tail.prev = @intFromPtr(&live_right);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&live_left)), breakage.expected_prev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&stale_right)), breakage.actual_prev);
    try std.testing.expect(!view.hasConsistentBacklinks());
}

test "phase3 list/hlist mid braid replay keeps the live hlist interior route visible while a detached braid stays off the witness path" {
    ...
    stale_left.next = @intFromPtr(&stale_right);
    stale_left.pprev = @intFromPtr(&live_left.next);
    stale_right.next = @intFromPtr(&live_right);
...
test "phase3 list/hlist mid braid replay reports the first visible hlist interior break when the live node adopts a detached braid early" {
    var head = hlist_view.HListHead{ .first = 0 };
    var first = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_left = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var live_right = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    live_left.pprev = @intFromPtr(&first.next);
    live_right.next = @intFromPtr(&tail);
    live_right.pprev = @intFromPtr(&stale_right.next);
    tail.next = 0;
    tail.pprev = @intFromPtr(&live_right.next);
    ...
    try std.testing.expectEqual(@as(usize, 2), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&live_left.next)), breakage.expected_pprev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&stale_right.next)), breakage.actual_pprev);
    try std.testing.expect(!view.hasConsistentPrevLinks());
}
