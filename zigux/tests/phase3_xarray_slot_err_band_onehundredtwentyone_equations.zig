...
test "representative 121-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 5, 935, 1977, 2979, 3973 };
...
test "every interior onehundredtwentyone window keeps consecutive codes and alternating constructor families" {
...
test "onehundredtwentyone windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
