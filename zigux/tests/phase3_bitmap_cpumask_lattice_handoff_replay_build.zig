...
pub fn build(b: *std.Build) void {
    ...
    const test_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_lattice_handoff_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named_step = b.step("phase3-bitmap-cpumask-lattice-handoff-replay", "Run the Lane 27 bitmap/cpumask lattice handoff replay");
    named_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 27 bitmap/cpumask lattice handoff replay");
