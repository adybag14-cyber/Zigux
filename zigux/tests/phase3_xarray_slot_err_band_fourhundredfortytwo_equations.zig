...
const errno: isize = -442;
const next_errno: isize = -443;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 442 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 442 raw equation stays between its errno neighbors" {
...
test "errno 442 adjacent raw lanes keep value and pointer decoders precise" {
