...
test "phase 9 runtime bitmap survey source-checks the direct sample evidence packet" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, loader_source, "test \"runtime bitmap loader keeps initialized shared-request snapshots stable across later exit activity\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, loader_source, "try std.testing.expectEqual(runtime_bitmap_sample.ModuleStage.exited, exited_snapshot.stage);") != null);
    try std.testing.expect(std.mem.indexOf(u8, loader_source, "try std.testing.expectEqual(@as(usize, 1), exited_snapshot.exit_runs);") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, loader_source, "test \"runtime bitmap loader surfaces prepared shared selftest-hook drift before any live bitmap claim\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, loader_source, "test \"runtime bitmap loader keeps selftest-complete shared-request snapshots stable across later exit activity\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, loader_source, "try std.testing.expectEqual(@as(usize, 1), exited_snapshot.selftest_runs);") != null);
    try std.testing.expect(std.mem.indexOf(u8, loader_source, "try std.testing.expectEqual(runtime_loader.HandoffStage.selftest_complete, pending_plan.init_flow.handoff_stage);") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, phase9_build, "phase9-runtime-bitmap-survey-tests") != null);
