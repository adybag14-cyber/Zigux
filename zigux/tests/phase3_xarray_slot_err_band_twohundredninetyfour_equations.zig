...
test "twohundredninetyfour raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 50, 984, 2016, 2996, 3801 };
...
test "twohundredninetyfour constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredninetyfour keeps centered raw code and payload equations exact" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
