...
const errno: isize = -350;
const next_errno: isize = -351;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 350 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 350 raw equation stays between its errno neighbors" {
...
test "errno 350 adjacent raw lanes keep value and pointer decoders precise" {
