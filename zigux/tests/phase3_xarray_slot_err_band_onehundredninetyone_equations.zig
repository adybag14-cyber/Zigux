...
test "representative 191-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 18, 948, 1986, 2968, 3904 };
...
test "every interior onehundredninetyone window keeps consecutive codes and alternating constructor families" {
...
test "onehundredninetyone windows preserve a centered raw seam and a centered payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
