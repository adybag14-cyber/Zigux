...
const errno: isize = -421;
const next_errno: isize = -422;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 421 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 421 raw equation stays between its errno neighbors" {
...
test "errno 421 adjacent raw lanes keep value and pointer decoders precise" {
