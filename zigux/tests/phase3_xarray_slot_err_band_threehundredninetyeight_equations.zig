...
const errno: isize = -398;
const next_errno: isize = -399;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 398 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 398 raw equation stays between its errno neighbors" {
...
test "errno 398 adjacent raw lanes keep value and pointer decoders precise" {
