...
const errno: isize = -386;
const next_errno: isize = -387;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 386 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 386 raw equation stays between its errno neighbors" {
...
test "errno 386 adjacent raw lanes keep value and pointer decoders precise" {
