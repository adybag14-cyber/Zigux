...
const visible_count = 47;
const detached_index = visible_count;
const route = [_]usize{
    46, 0, 24, 22, 45, 1, 25, 21, 44, 2,  26, 20, 43, 3,  27, 19,
    42, 4, 28, 18, 41, 5, 29, 17, 40, 6,  30, 16, 39, 7,  31, 15,
    38, 8, 32, 14, 37, 9, 33, 13, 36, 10, 34, 12, 35, 11, 23,
};
...
test "heptatetraconta mesh-gate exposes the same visible route through list and hlist" {
...
test "heptatetraconta mesh-gate repairs staged reverse links after the mesh gate" {
    var entries = [_]Entry{.{}} ** (visible_count + 1);
    var head = ListHead{ .next = 0, .prev = 0 };
    var hhead = HListHead{ .first = 0 };

    reset(&entries, &head, &hhead);
    const gate_position = 30;
