...
pub fn build(b: *std.Build) void {
    ...
    const contract_module = b.createModule(.{
        .root_source_file = b.path("phase3_abi_runtime_loader_request_state_contract.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const contract_tests = b.addTest(.{
        .name = "phase3-abi-runtime-loader-request-state-contract-tests",
    ...
    const contract_step = b.step(
        "phase3-abi-runtime-loader-request-state-contract",
        "Run the Lane 26 runtime loader request-state ABI contract",
    );
    contract_step.dependOn(&run_contract.step);

    const test_step = b.step("test", "Run the Lane 26 runtime loader request-state ABI contract");
