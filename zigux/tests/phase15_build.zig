...
pub fn build(b: *std.Build) void {
    ...
    const phase15_docs_root_reviewability_module = b.createModule(.{
        .root_source_file = b.path("phase15_docs_root_reviewability.zig"),
        .target = target,
        .optimize = optimize,
    });

    const phase15_evidence_archive_templates_module = b.createModule(.{
        .root_source_file = b.path("phase15_evidence_archive_templates.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const phase15_docs_root_reviewability_tests = b.addTest(.{
        .name = "phase15-docs-root-reviewability-tests",
        .root_module = phase15_docs_root_reviewability_module,
    });
    const run_phase15_docs_root_reviewability_tests = b.addRunArtifact(phase15_docs_root_reviewability_tests);

    const phase15_evidence_archive_templates_tests = b.addTest(.{
        .name = "phase15-evidence-archive-templates-tests",
        .root_module = phase15_evidence_archive_templates_module,
    });
    const run_phase15_evidence_archive_templates_tests = b.addRunArtifact(phase15_evidence_archive_templates_tests);
    ...
    test_step.dependOn(&run_phase15_docs_root_reviewability_tests.step);
    test_step.dependOn(&run_phase15_evidence_archive_templates_tests.step);
}
