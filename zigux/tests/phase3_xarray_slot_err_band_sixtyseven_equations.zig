...
test "representative 67-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 3, 981, 2023, 3027, 4027 };
...
test "every interior sixtyseven window keeps consecutive codes and alternating constructor families" {
...
test "sixtyseven windows preserve centered equations around the middle rejected-payload slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
