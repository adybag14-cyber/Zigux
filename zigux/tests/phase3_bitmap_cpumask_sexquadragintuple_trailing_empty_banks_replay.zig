...
test "sexquadragintuple trailing empty banks keep early windows aligned and masked tail bits bounded" {
    const capacity = (48 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
    ...
    try testing.expect(bitmap.isSet((48 * bitmap_view.word_bits) + 0));
    try testing.expect(bitmap.isSet((48 * bitmap_view.word_bits) + 10));
    try testing.expect(!bitmap.isSet((48 * bitmap_view.word_bits) + 9));
    ...
    try testing.expect(!cpumask.hasCpu((47 * bitmap_view.word_bits) + 5));
    try testing.expect(cpumask.hasCpu((48 * bitmap_view.word_bits) + 7));
}

test "sexquadragintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (48 * bitmap_view.word_bits) + 11;
    ...
    const tail_only_words = [_]usize{0} ** 48 ++ [_]usize{
