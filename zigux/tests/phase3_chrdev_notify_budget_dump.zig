...
fn writeSummary(writer: anytype, summary: abi.ChrdevNotifyBudgetSummary) !void {
...
pub fn main(init: std.process.Init) !void {
    ...
    const policy_deferred_view = chrdev_notify_budget_plan.viewFromBits(words[0..], 240, 32, 8, 8, 2, abi.IDA_POLICY_LAST_FIT, 37, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_MODE_READ | abi.CHRDEV_MODE_WRITE, abi.CHRDEV_FOP_OPEN | abi.CHRDEV_FOP_RELEASE | abi.CHRDEV_FOP_WRITE, abi.CHRDEV_IO_OP_WRITE, 20, 8, 1024, 4, 1, 3, 2, 1, 5, 1, 4, 2, 0x1111, 1, abi.CHRDEV_NOTIFY_MASK_SUCCESS, 1, 0xCCCC, abi.CHRDEV_NOTIFY_POLICY_FORCE_DEFERRED, 0, 1);
    ...
    try writer.writeAll("{\"constants\":{\"chrdev_notify_budget_status_none\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_BUDGET_STATUS_NONE});
    try writer.writeAll(",\"chrdev_notify_budget_status_issued\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_BUDGET_STATUS_ISSUED});
    try writer.writeAll(",\"chrdev_notify_budget_status_deferred\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_BUDGET_STATUS_DEFERRED});
    try writer.writeAll(",\"chrdev_notify_budget_status_dropped\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_BUDGET_STATUS_DROPPED});
    try writer.writeAll(",\"chrdev_notify_budget_status_suppressed\":");
    try writer.print("{d}", .{abi.CHRDEV_NOTIFY_BUDGET_STATUS_SUPPRESSED});
    try writer.writeAll(",\"chrdev_notify_policy_force_deferred\":");
    ...
    try writer.writeAll("},\"issued\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_budget_plan.summarize(issued_view));
    try writer.writeAll("},\"fallback_deferred\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_budget_plan.summarize(fallback_deferred_view));
    try writer.writeAll("},\"policy_deferred\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_budget_plan.summarize(policy_deferred_view));
    try writer.writeAll("},\"dropped\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_budget_plan.summarize(dropped_view));
    try writer.writeAll("},\"suppressed\":{\"summary\":");
    try writeSummary(writer, chrdev_notify_budget_plan.summarize(suppressed_view));
    try writer.writeAll("},\"empty\":{\"is_valid\":");
    try writer.writeAll(if (chrdev_notify_budget_plan.isValid(empty_view)) "true" else "false");
    try writer.writeAll(",\"summary\":");
    try writeSummary(writer, chrdev_notify_budget_plan.summarize(empty_view));
    try writer.writeAll("}}\n");
