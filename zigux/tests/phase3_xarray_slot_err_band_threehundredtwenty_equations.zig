...
const errno: isize = -320;
const next_errno: isize = -321;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 320 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 320 raw equation stays between its errno neighbors" {
...
test "errno 320 adjacent raw lanes keep value and pointer decoders precise" {
