...
pub fn build(b: *std.Build) void {
    ...
    const phase3_abi_core_packet = addPhase3AbiCorePacket(
        b,
        target,
        optimize,
    );

    const phase3_xarray_slot_abi_test = b.step(
        "phase3-xarray-slot-abi-test",
        "Run the Phase 3 xarray-slot and ABI core packets through one focused Lane 04 harness",
    );
    phase3_xarray_slot_abi_test.dependOn(&phase3_xarray_slot_starter_packet.step);
    phase3_xarray_slot_abi_test.dependOn(&phase3_abi_core_packet.step);
}
