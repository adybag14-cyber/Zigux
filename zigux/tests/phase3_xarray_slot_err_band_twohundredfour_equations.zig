...
test "twohundredfour raw windows keep err spacing and low-bit aliases exact" {
...
test "twohundredfour constructor windows keep consecutive codes and reject aliased values" {
...
test "twohundredfour keeps split raw and payload centers adjacent" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
