...
pub fn build(b: *std.Build) void {
    ...
    const replay_root_module = b.createModule(.{
        .root_source_file = b.path("phase1_argv_split_allocation_failure_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-argv-split-allocation-failure-replay-tests",
        .root_module = replay_root_module,
    });
    const run_replay_tests = b.addRunArtifact(replay_tests);
    const replay_step = b.step(
        "phase1-argv-split-allocation-failure-replay",
        "Run the Phase 1 argv_split allocation-failure replay",
    );
