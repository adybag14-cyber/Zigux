...
const errno: isize = -492;
const next_errno: isize = -493;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 492 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 492 raw equation stays between its errno neighbors" {
...
test "errno 492 adjacent raw lanes keep value and pointer decoders precise" {
