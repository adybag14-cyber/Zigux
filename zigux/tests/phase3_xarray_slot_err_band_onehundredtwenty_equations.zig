...
const center_payload_left_index = center_left_index - 1;
const center_payload_right_index = center_right_index;
...
test "onehundredtwenty windows preserve a dual-center raw seam and a split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
        ...
        for (0..(center_right_index / 2)) |pair_idx| {
