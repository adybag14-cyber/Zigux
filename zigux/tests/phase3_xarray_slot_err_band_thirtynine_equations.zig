...
test "representative 39-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1006, 2038, 3054, 4056 };
...
test "every interior thirtynine window keeps consecutive codes and alternating constructor families" {
...
test "thirtynine windows preserve centered equations around the middle err-code slot" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_payload_sum =
        ...
        try testing.expectEqual(
            expectedRejectedPayload(even_residue + center_index - 1) + 1,
            expectedRejectedPayload(even_residue + center_index + 1),
        );
    }
}
