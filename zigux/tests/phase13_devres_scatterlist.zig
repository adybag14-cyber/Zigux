...
test "phase13 devres scatterlist table teardown stays inert until the table is initialized" {
    const plan = devres_scatterlist.DevresScatterlistHelper.planManagedScatterlistTableTeardown(.{
        .original_entries = 6,
        .mapped_entries = 0,
...
test "phase13 devres scatterlist planner manifest records the dedicated helper-first packet" {
    ...
    try requireContains(manifest, "scripts\zigux/check_phase13_devres_scatterlist_planner.zig");
    ...
    try requireContains(manifest, "\"validation_guard\": \"scripts\zigux/check_phase13_devres_scatterlist_planner.zig\"");
...
test "phase13 devres scatterlist planner note keeps the helper-first scatterlist slice bounded" {
    ...
    try requireContains(note, "records whether uninitialized tables stay neither free-ready nor unmap-requiring until table initialization is explicit");
    ...
    try requireContains(note, "`scripts\zigux/check_phase13_devres_scatterlist_planner.zig` is the packet-local validation guard");
...
test "phase13 devres scatterlist slice and build shard stay packet-local" {
    ...
    try requireContains(slice, "uninitialized-table hold");
...
test "phase13 devres scatterlist planner checker stays packet-local" {
    const checker = try readRepoFile(std.testing.allocator, "scripts\zigux/check_phase13_devres_scatterlist_planner.zig");
    ...
    try requireContains(checker, "records whether uninitialized tables stay neither free-ready nor unmap-requiring until table initialization is explicit");
    try requireContains(checker, "phase13 devres scatterlist table teardown stays inert until the table is initialized");
