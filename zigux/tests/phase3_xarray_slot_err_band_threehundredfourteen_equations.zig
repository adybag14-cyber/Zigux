...
const errno: isize = -314;
const next_errno: isize = -315;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 314 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 314 raw equation stays between its errno neighbors" {
...
test "errno 314 adjacent raw lanes keep value and pointer decoders precise" {
