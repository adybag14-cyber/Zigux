...
test "buttress bridge keeps bitmap and cpumask summaries aligned under noisy tail storage" {
    const capacity = (5 * bitmap_view.word_bits) + 11;
    const buttress_bridge_bits = [_]usize{
        1,
        bitmap_view.word_bits - 1,
        bitmap_view.word_bits + 2,
        bitmap_view.word_bits + 10,
        (2 * bitmap_view.word_bits) + 0,
        (2 * bitmap_view.word_bits) + 6,
        (3 * bitmap_view.word_bits) + 4,
        (4 * bitmap_view.word_bits) + 1,
        (4 * bitmap_view.word_bits) + 8,
        (5 * bitmap_view.word_bits) + 2,
        capacity - 2,
    };

    var words = [_]usize{ 0, 0, 0, 0, 0, 0, 0, 0 };
    fillPattern(words[0..], capacity, buttress_bridge_bits[0..]);
    addNoise(words[0..], capacity);

    const bitmap = bitmap_view.BitmapView.init(words[0..], capacity);
    const cpumask = cpumask_view.CpuMaskView.init(words[0..], capacity);

    try testing.expectEqual(buttress_bridge_bits.len, bitmap.countSetBits());
    ...
    try testing.expect(bitmap.isSet((3 * bitmap_view.word_bits) + 4));
    try testing.expect(cpumask.hasCpu(capacity - 2));
    try testing.expect(!cpumask.hasCpu(0));
    try testing.expect(!cpumask.hasCpu((3 * bitmap_view.word_bits) + 1));
}

test "peer buttress bridge masks share pylons without collapsing into a subset" {
    const capacity = (5 * bitmap_view.word_bits) + 11;
    ...
    try testing.expect(base.hasCpu((5 * bitmap_view.word_bits) + 2));
    try testing.expect(!peer.hasCpu((5 * bitmap_view.word_bits) + 2));
    try testing.expect(!base.hasCpu((5 * bitmap_view.word_bits) + 5));
    try testing.expect(peer.hasCpu((5 * bitmap_view.word_bits) + 5));
}

test "inner buttress bridge stays a bounded subset of the broader shape" {
    const capacity = (5 * bitmap_view.word_bits) + 11;
    const base_bits = [_]usize{
        1,
        bitmap_view.word_bits - 1,
    ...
    try testing.expectEqual(@as(?usize, bitmap_view.word_bits + 2), inner_bitmap.firstSetBit());
