...
test "tail rail center bridge keeps a shared two-bit span aligned under a bounded union" {
    const capacity = bitmap_view.word_bits + 10;
    const tail_base = bitmap_view.word_bits;

    const union_words = [_]usize{
        std.math.maxInt(usize),
        bit(0) | bit(1) | bit(4) | bit(5) | bit(8) | bit(9),
    };
    ...
    try testing.expect(union_cpumask.hasCpu(tail_base + 5));
    try testing.expect(!union_cpumask.hasCpu(tail_base + 6));
...
test "tail rail center bridge keeps a bridge-only peer bounded to valid tail bits" {
    const capacity = bitmap_view.word_bits + 10;
    const tail_base = bitmap_view.word_bits;
    ...
    const bridge_only_words = [_]usize{
        0,
        bit(4) | bit(5) | bit(10) | bit(14),
    };

    const anchor = cpumask_view.CpuMaskView.init(anchor_words[0..], capacity);
    const bridge_only = cpumask_view.CpuMaskView.init(bridge_only_words[0..], capacity);
    const bridge_bitmap = bitmap_view.BitmapView.init(bridge_only_words[0..], capacity);

    try testing.expectEqual(@as(usize, 2), bridge_bitmap.countSetBits());
    try testing.expectEqual(@as(?usize, tail_base + 4), bridge_bitmap.firstSetBit());
    try testing.expectEqual(@as(?usize, 0), bridge_bitmap.firstClearBit());
    try testing.expectEqual(@as(usize, 2), bridge_only.countPresentCpus());
    try testing.expectEqual(@as(?usize, tail_base + 4), bridge_only.firstCpu());
    try testing.expectEqual(@as(?usize, 0), bridge_only.firstMissingCpu());
    try testing.expect(!bridge_only.hasCpu(tail_base + 3));
    try testing.expect(bridge_only.hasCpu(tail_base + 4));
    try testing.expect(bridge_only.hasCpu(tail_base + 5));
    try testing.expect(!bridge_only.hasCpu(tail_base + 6));
    try testing.expect(!bridge_only.hasCpu(capacity - 1));
    try testing.expect(bridge_only.isSubsetOf(anchor));
    try testing.expect(anchor.intersects(bridge_only));
    try testing.expect(bridge_only.intersects(anchor));
}
