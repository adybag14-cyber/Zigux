...
test "representative 132-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 10, 930, 1968, 2962, 3962 };
...
test "every interior onehundredthirtytwo window keeps consecutive codes and alternating constructor families" {
...
test "onehundredthirtytwo windows preserve a split raw seam and a split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
