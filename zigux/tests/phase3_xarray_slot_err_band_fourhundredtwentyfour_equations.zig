...
const errno: isize = -424;
const next_errno: isize = -425;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 424 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 424 raw equation stays between its errno neighbors" {
...
test "errno 424 adjacent raw lanes keep value and pointer decoders precise" {
