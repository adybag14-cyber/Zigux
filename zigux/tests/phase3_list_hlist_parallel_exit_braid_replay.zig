...
test "list parallel exit braid stays detached until the exit route adopts it" {
    ...
    try expectListSequence(stable, &.{ &first, &live_left, &live_right, &exit, &tail });
    ...
    try expectListSequence(
        broken,
        &.{ &first, &live_left, &live_right, &braid_left, &braid_right, &exit, &tail },
    );
...
test "hlist parallel exit braid stays detached until the exit route adopts it" {
    ...
    exit.next = @intFromPtr(&tail);
    exit.pprev = @intFromPtr(&live_right.next);
    tail.next = 0;
    ...
    try expectHListSequence(stable, &.{ &first, &live_left, &live_right, &exit, &tail });
