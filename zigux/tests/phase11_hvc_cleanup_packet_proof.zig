...
test "phase11 hvc cleanup packet proof keeps starter teardown helpers tied to matrix evidence" {
    ...
    try expectContains(matrix_doc, "`hvc_alloc()` slot selection");
    try expectContains(matrix_doc, "early console setup");
    try expectContains(matrix_doc, "and device selection");
    try expectContains(matrix_doc, "`__hvc_resize()` handoff");
    ...
    try expectContains(driver, "pub fn summarizeInstallOwnership(request: InstallOwnershipRequest) InstallOwnershipSummary {");
    try expectContains(driver, "pub fn summarizeAllocSlotHandoff(request: AllocSlotRequest) AllocSlotSummary {");
    try expectContains(driver, "pub fn summarizeConsoleSetup(request: ConsoleSetupRequest) ConsoleSetupSummary {");
    try expectContains(driver, "pub fn summarizeConsoleDeviceSelection(request: ConsoleDeviceRequest) ConsoleDeviceSummary {");
    try expectContains(driver, "pub fn summarizeResizeHandoff(request: ResizeHandoffRequest) ResizeHandoffSummary {");
