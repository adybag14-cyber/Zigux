...
const errno: isize = -433;
const next_errno: isize = -434;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 433 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 433 raw equation stays between its errno neighbors" {
...
test "errno 433 adjacent raw lanes keep value and pointer decoders precise" {
