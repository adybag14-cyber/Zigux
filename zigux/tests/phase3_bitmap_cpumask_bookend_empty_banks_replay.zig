...
test "bookend empty banks keep middle windows aligned and tail noise masked" {
    const capacity = (3 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        0,
        bit(1) | bit(5) | bit(9) | bit(14),
        bit(0) | bit(4) | bit(8),
        bit(12) | bit(15) | bit(19),
    };
    ...
    try testing.expect(!cpumask.hasCpu((3 * bitmap_view.word_bits) + 7));
}

test "bookend empty banks keep subset and overlap checks bounded at both edges" {
    const capacity = (3 * bitmap_view.word_bits) + 11;
