...
test "onehundredninetyeight raw windows keep err spacing and low-bit aliases exact" {
    const left_residues = [_]usize{ 0, 32, 962, 2000, 2980, 3896 };
...
test "onehundredninetyeight constructor windows keep consecutive codes and reject aliased values" {
...
test "onehundredninetyeight keeps split raw and payload centers adjacent" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        try testing.expectEqual(
        ...
        try testing.expectEqual(
            rejectedPayload(even_residue + center_left_index) + 1,
            rejectedPayload(even_residue + center_right_index + 1),
        );
    }
}
