...
test "representative 37-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1008, 2038, 3054, 4058 };
...
test "every interior thirtyseven window keeps consecutive codes and alternating constructor families" {
...
test "thirtyseven windows preserve centered equations and the centered rejected-payload law" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
