...
test "list view witnesses center-out weave before backlink repair" {
    ...
    try expectListRoute(view, &.{ &three, &four, &two, &five, &one, &six });
...
test "hlist view witnesses center-out weave before prev-link repair" {
    ...
    try expectHListRoute(view, &.{ &three, &four, &two, &five, &one, &six });
