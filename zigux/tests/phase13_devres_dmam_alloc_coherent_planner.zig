...
test "phase13 devres turns successful coherent-allocation planning into explicit detach cleanup transition planning" {
    ...
    try requireContains(source, ".provides_dmam_detach_cleanup_transition_planning = true");
    try requireContains(source, "pub const ManagedReleaseCallPlan = struct");
    try requireContains(source, "pub const ManagedDmamDetachCleanupPlan = struct");
    ...
    try requireContains(source, "pub fn planManagedDmamDetachCleanup(");
    try requireContains(source, "const cleanup = planManagedDmamFreeCoherent(allocation_plan.requested_size, release_record_matches);");
...
test "phase13 devres keeps detach cleanup transition warnable when the release record is missing" {
    ...
    const cleanup = devres.DevresHelperLab.planManagedDmamDetachCleanup(plan, false);
...
test "phase13 devres dmam_alloc_coherent planner manifest records the landed helper-first dma scope" {
    ...
    try requireContains(manifest, "provides_dmam_detach_cleanup_transition_planning");
    ...
    try requireContains(manifest, "ManagedDmamDetachCleanupPlan");
    try requireContains(manifest, "planManagedDmamDetachCleanup");
...
test "phase13 devres dmam_alloc_coherent planner note keeps the helper-first dma slice bounded" {
    ...
    try requireContains(note, "descriptor records the shared release-record lifetime, release-call, and detach-cleanup-transition planning markers");
    ...
    try requireContains(note, "turns that successful allocation plan into explicit detach cleanup transition planning through `planManagedDmamDetachCleanup(...)`");
    try requireContains(note, "routes `planManagedDmamDetachCleanup(...)` through `planManagedDmamFreeCoherent(...)`");
...
test "phase13 devres dmam_alloc_coherent checker stays packet-local" {
    ...
    try requireContains(checker, "NOTE_PATH = Path(\"Documentation/zigux/phase13-devres-dmam-alloc-coherent-planner.md\")");
    try requireContains(checker, "MANIFEST_PATH = Path(\"zigux/tests/phase13_devres_dmam_alloc_coherent_planner_manifest.json\")");
    try requireContains(checker, "REPLAY_PATH = Path(\"zigux/tests/phase13_devres_dmam_alloc_coherent_planner.zig\")");
    try requireContains(checker, "PHASE13_DEVRES_DMAM_ALLOC_COHERENT_PLANNER_SELF_TEST=pass");
    try requireContains(checker, "PHASE13_DEVRES_DMAM_ALLOC_COHERENT_PLANNER=pass");
}
