...
test "tail rail outer gap keeps tail counting and final-gap discovery aligned" {
    const capacity = (bitmap_view.word_bits * 2) + 9;
    const words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        bit(0) | bit(1) | bit(2) | bit(4) | bit(5) | bit(6) | bit(7) | bit(12),
    };

    const bitmap = bitmap_view.BitmapView.init(words[0..], capacity);
    const cpumask = cpumask_view.CpuMaskView.init(words[0..], capacity);

    try testing.expect(bitmap.isSet((bitmap_view.word_bits * 2) + 7));
    try testing.expect(!bitmap.isSet((bitmap_view.word_bits * 2) + 3));
    ...
    try testing.expectEqual(@as(?usize, (bitmap_view.word_bits * 2) + 3), bitmap.firstClearBit());
...
test "tail rail outer gap keeps subset and overlap checks bounded under tail-only noise" {
    const capacity = (bitmap_view.word_bits * 2) + 9;
