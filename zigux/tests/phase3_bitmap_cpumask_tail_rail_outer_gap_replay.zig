...
test "tail rail outer gap keeps a late valid gap aligned under a bounded union" {
    const capacity = bitmap_view.word_bits + 8;
    ...
    try testing.expectEqual(@as(usize, capacity - 1), union_bitmap.countSetBits());
    ...
    try testing.expectEqual(@as(?usize, tail_base + 6), union_bitmap.firstClearBit());
...
test "tail rail outer gap keeps the late-gap probe outside the bounded union" {
    const capacity = bitmap_view.word_bits + 8;
    const tail_base = bitmap_view.word_bits;
    ...
    try testing.expectEqual(@as(usize, 1), gap_probe_bitmap.countSetBits());
    try testing.expectEqual(@as(?usize, tail_base + 6), gap_probe_bitmap.firstSetBit());
    try testing.expectEqual(@as(?usize, 0), gap_probe_bitmap.firstClearBit());
    try testing.expectEqual(@as(usize, 1), gap_probe.countPresentCpus());
    try testing.expectEqual(@as(?usize, tail_base + 6), gap_probe.firstCpu());
    try testing.expectEqual(@as(?usize, 0), gap_probe.firstMissingCpu());
    try testing.expect(gap_probe.hasCpu(tail_base + 6));
    try testing.expect(!gap_probe.hasCpu(tail_base + 5));
