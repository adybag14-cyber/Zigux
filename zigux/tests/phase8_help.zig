...
test "phase 8 help focused replay keeps empty PATH fallback and section suppression stable" {
    const path_entries = [_]help.DirectoryEntry{
        .{ .name = "perf-report.exe", .is_executable = true },
        .{ .name = "perf-stat", .is_executable = true },
...
test "phase 8 help evidence still matches the live C helper anchors" {
    ...
    try expectContains(help_c, "available %s in '%s'\\n");
    try expectContains(help_c, "%s available from elsewhere on your $PATH\\n");
}
