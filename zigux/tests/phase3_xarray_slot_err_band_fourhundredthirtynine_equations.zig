...
const errno: isize = -439;
const next_errno: isize = -440;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 439 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 439 raw equation stays between its errno neighbors" {
...
test "errno 439 adjacent raw lanes keep value and pointer decoders precise" {
