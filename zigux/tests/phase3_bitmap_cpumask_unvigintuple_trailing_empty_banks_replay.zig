...
test "unvigintuple trailing empty banks keep early windows aligned and masked tail bits bounded" {
    const capacity = (23 * bitmap_view.word_bits) + 11;
    const words = [_]usize{
        bit(1) | bit(4) | bit(9) | bit(13),
        bit(0) | bit(6) | bit(10) | bit(15),
    } ++ [_]usize{0} ** 21 ++ [_]usize{
    ...
    try testing.expect(bitmap.isSet((23 * bitmap_view.word_bits) + 4));
    try testing.expect(bitmap.isSet((23 * bitmap_view.word_bits) + 10));
    ...
    try testing.expect(cpumask.hasCpu((23 * bitmap_view.word_bits) + 0));
}

test "unvigintuple trailing empty banks keep subset and overlap checks bounded under masked late noise" {
    const capacity = (23 * bitmap_view.word_bits) + 11;
