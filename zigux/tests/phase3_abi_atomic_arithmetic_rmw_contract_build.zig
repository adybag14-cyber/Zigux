...
pub fn build(b: *std.Build) void {
    ...
    const contract = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("phase3_abi_atomic_arithmetic_rmw_contract.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    const named_step = b.step(
        "phase3-abi-atomic-arithmetic-rmw-contract",
        "Run the Phase 3 ABI atomic arithmetic RMW contract",
    );
    named_step.dependOn(&run_contract.step);

    const test_step = b.step("test", "Run the Phase 3 ABI atomic arithmetic RMW contract");
