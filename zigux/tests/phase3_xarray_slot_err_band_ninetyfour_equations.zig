...
test "representative 94-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 956, 1998, 3000, 4000 };
...
test "every interior ninetyfour window keeps consecutive codes and alternating constructor families" {
...
test "ninetyfour windows preserve a dual-center raw seam and a centered payload lane" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
