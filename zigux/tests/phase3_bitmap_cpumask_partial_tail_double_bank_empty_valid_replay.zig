...
test "lane27 double-bank empty-valid bitmap replay keeps the trailing clear window explicit" {
    ...
    try testing.expectEqual(bitmap_view.word_bits * 2, view.countSetBits());
    try testing.expectEqual(@as(?usize, 0), view.firstSetBit());
    try testing.expectEqual(@as(?usize, bitmap_view.word_bits * 2), view.firstClearBit());
...
test "lane27 double-bank empty-valid cpumask replay ignores tail-only noise in subset checks" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    ...
    const peer_words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
        invalidTailNoise(capacity) | (@as(usize, 1) << 1),
    };
    ...
    try testing.expectEqual(bitmap_view.word_bits * 2, base.countPresentCpus());
    try testing.expectEqual(@as(?usize, 0), base.firstCpu());
    try testing.expectEqual(@as(?usize, bitmap_view.word_bits * 2), base.firstMissingCpu());
    try testing.expect(base.isSubsetOf(peer));
    try testing.expect(!peer.isSubsetOf(base));
    try testing.expect(base.intersects(peer));
}

test "lane27 double-bank empty-valid cpumask replay treats pure tail noise as disjoint" {
    const capacity = (bitmap_view.word_bits * 2) + 5;
    const base_words = [_]usize{
        std.math.maxInt(usize),
        std.math.maxInt(usize),
