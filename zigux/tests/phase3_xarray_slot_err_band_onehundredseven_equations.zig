...
test "representative 107-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 5, 943, 1985, 2987, 3987 };
...
test "every interior onehundredseven window keeps consecutive codes and alternating constructor families" {
...
test "onehundredseven windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
        const center_residue = odd_residue + center_index;
        const center_payload = expectedRejectedPayload(center_residue);
        ...
        try testing.expectEqual(
            expectedRejectedPayload(center_residue - 2) + 1,
            center_payload,
        );
        try testing.expectEqual(
            center_payload + 1,
            expectedRejectedPayload(center_residue + 2),
        );
        try testing.expectEqual(
