...
pub fn build(b: *std.Build) void {
    ...
    const phase3_layout_assert = addLayoutAssertTest(b, target, optimize);

    const step = b.step(
        "phase3-export-layout-assert-test",
        "Run the shared Phase 3 export/UAPI layout and layout_assert helper packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_layout_assert.step);
}
