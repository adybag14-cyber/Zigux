...
const errno: isize = -429;
const next_errno: isize = -430;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 429 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 429 raw equation stays between its errno neighbors" {
...
test "errno 429 adjacent raw lanes keep value and pointer decoders precise" {
