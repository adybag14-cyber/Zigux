...
test "triple-bank partial tail keeps discovery aligned after three saturated words" {
    ...
    try testing.expectEqual(@as(?usize, tail_base + 1), bitmap.firstClearBit());
...
test "triple-bank partial tail keeps subset and overlap blind to tail-only noise" {
    const capacity = (bitmap_view.word_bits * 3) + 5;
