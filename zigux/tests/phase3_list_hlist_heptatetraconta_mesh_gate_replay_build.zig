...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase3_list_hlist_heptatetraconta_mesh_gate_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step(
        "phase3-list-hlist-heptatetraconta-mesh-gate-replay",
        "Run the Lane 28 Phase 3 list/hlist heptatetraconta mesh-gate replay.",
    );
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step(
        "test",
        "Run the Lane 28 Phase 3 list/hlist heptatetraconta mesh-gate replay tests.",
    );
