...
const errno: isize = -414;
const next_errno: isize = -415;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 414 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 414 raw equation stays between its errno neighbors" {
...
test "errno 414 adjacent raw lanes keep value and pointer decoders precise" {
