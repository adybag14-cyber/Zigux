...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_xarray_helper_export_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase3-xarray-helper-export-replay",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(tests);

    const replay_step = b.step(
        "phase3-xarray-helper-export-replay",
        "Run the Phase 3 xarray helper cluster and export/UAPI layout replay from a focused standalone build root",
    );
