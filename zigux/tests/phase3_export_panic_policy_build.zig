...
pub fn build(b: *std.Build) void {
    ...
    const phase3_panic_policy = addPanicPolicyTest(b, target, optimize);

    const step = b.step(
        "phase3-export-panic-policy-test",
        "Run the shared Phase 3 export/UAPI layout and panic-policy helper packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_panic_policy.step);
}
