...
test "representative err-band sextets keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1020, 2044, 3066, 4088 };
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [6]usize = undefined;
        var slots: [6]xarray_slot_view.SlotView = undefined;
        ...
        const sum_05 = @as(u128, raws[0]) + @as(u128, raws[5]);
        const sum_14 = @as(u128, raws[1]) + @as(u128, raws[4]);
        const sum_23 = @as(u128, raws[2]) + @as(u128, raws[3]);
        try testing.expectEqual(sum_05, sum_14);
        try testing.expectEqual(sum_14, sum_23);
...
test "every interior sextet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 5 <= err_band_span) : (even_residue += 2) {
...
test "sextet windows preserve pairwise centered equations" {
    var even_residue: usize = 0;
    while (even_residue + 5 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[5], codes[1] + codes[4]);
        try testing.expectEqual(codes[1] + codes[4], codes[2] + codes[3]);
        ...
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 5)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 4)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 4)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 3)),
        );
    }
}
