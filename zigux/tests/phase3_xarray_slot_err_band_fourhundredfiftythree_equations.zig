...
const errno: isize = -453;
const next_errno: isize = -454;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 453 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 453 raw equation stays between its errno neighbors" {
...
test "errno 453 adjacent raw lanes keep value and pointer decoders precise" {
