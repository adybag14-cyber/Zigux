...
test "list view witnesses outside-in braid before backlink repair" {
    ...
    try expectListRoute(view, &.{ &one, &six, &two, &five, &three, &four });
...
test "hlist view witnesses outside-in braid before prev-link repair" {
    ...
    try expectHListRoute(view, &.{ &one, &six, &two, &five, &three, &four });
