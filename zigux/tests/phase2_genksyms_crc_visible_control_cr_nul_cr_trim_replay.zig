...
test "lane19 visible control split replay preserves visible low control bytes and leading carriage returns while trimming trailing carriage returns before an embedded NUL" {
    var split_then_controls_and_leading_cr_then_trailing_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 15);
