...
test "list view ignores a detached suffix echo that mirrors the live tail segment" {
    var head = ListHead{ .next = 0, .prev = 0 };
    var live_first = ListHead{ .next = 0, .prev = 0 };
    var live_middle = ListHead{ .next = 0, .prev = 0 };
    var live_tail = ListHead{ .next = 0, .prev = 0 };
    var detached_suffix_head = ListHead{ .next = 0, .prev = 0 };
    var detached_suffix_tail = ListHead{ .next = 0, .prev = 0 };
    ...
    // The detached pair mirrors the visible suffix shape, but the live chain
    // must still come only from the head-rooted links.
    detached_suffix_head.next = @intFromPtr(&detached_suffix_tail);
    detached_suffix_head.prev = @intFromPtr(&live_first);
    detached_suffix_tail.next = @intFromPtr(&head);
    detached_suffix_tail.prev = @intFromPtr(&detached_suffix_head);
...
test "hlist view ignores a detached suffix echo that mirrors the live tail segment" {
    ...
    var detached_suffix_head = HListNode{ .next = 0, .pprev = 0 };
    var detached_suffix_tail = HListNode{ .next = 0, .pprev = 0 };
    ...
    detached_suffix_head.next = @intFromPtr(&detached_suffix_tail);
    detached_suffix_head.pprev = @intFromPtr(&live_first.next);
    detached_suffix_tail.next = 0;
    detached_suffix_tail.pprev = @intFromPtr(&detached_suffix_head.next);
...
test "suffix echo replay keeps the live tail segment authoritative across both helpers" {
    ...
    var list_echo_head = ListHead{ .next = 0, .prev = 0 };
    var list_echo_tail = ListHead{ .next = 0, .prev = 0 };
    ...
    list_echo_head.next = @intFromPtr(&list_echo_tail);
    list_echo_head.prev = @intFromPtr(&list_first);
    list_echo_tail.next = @intFromPtr(&list_head);
    list_echo_tail.prev = @intFromPtr(&list_echo_head);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&list_echo_tail)), list_echo_head.next);
    ...
    var hlist_echo_head = HListNode{ .next = 0, .pprev = 0 };
    var hlist_echo_tail = HListNode{ .next = 0, .pprev = 0 };
    ...
    hlist_echo_head.next = @intFromPtr(&hlist_echo_tail);
    hlist_echo_head.pprev = @intFromPtr(&hlist_first.next);
    hlist_echo_tail.next = 0;
    hlist_echo_tail.pprev = @intFromPtr(&hlist_echo_head.next);
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&hlist_echo_tail)), hlist_echo_head.next);
}
