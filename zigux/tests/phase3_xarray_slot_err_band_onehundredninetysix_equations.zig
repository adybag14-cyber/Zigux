...
test "representative 196-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 28, 958, 1996, 2976, 3898 };
...
test "every interior onehundredninetysix window keeps consecutive codes and alternating constructor families" {
...
test "onehundredninetysix windows preserve split raw and payload centers" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
