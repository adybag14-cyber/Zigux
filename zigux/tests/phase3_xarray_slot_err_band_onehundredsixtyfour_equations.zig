...
test "representative 164-raw err-band windows keep alias-err spacing exact" {
...
test "every interior onehundredsixtyfour window keeps consecutive codes and alternating constructor families" {
...
test "onehundredsixtyfour windows preserve a split raw seam and a split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
