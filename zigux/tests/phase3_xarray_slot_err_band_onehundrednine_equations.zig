...
test "representative 109-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 1, 5, 941, 1983, 2985, 3985 };
...
test "every interior onehundrednine window keeps consecutive codes and alternating constructor families" {
...
test "onehundrednine windows preserve centered equations around the middle err-code slot" {
    var odd_residue: usize = 1;
    while (odd_residue + (window_len - 1) <= err_band_span) : (odd_residue += 2) {
