...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase3_bitmap_cpumask_bank_rotation_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const lane_step = b.step(
        "phase3-bitmap-cpumask-bank-rotation-replay",
        "Run the Lane 27 bitmap/cpumask bank rotation replay",
    );
    lane_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run Lane 27 bitmap/cpumask bank rotation tests");
