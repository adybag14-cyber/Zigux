...
const ReviewProcess = struct {
    ...
    ownership_evidence_field_count: usize,
...
const Metrics = struct {
    ...
    shared_governance_replay_route_count: usize,
    review_packet_field_count: usize,
    ownership_evidence_field_count: usize,
};
...
test "phase 15 parity scorecard manifest records all freeze-map anchors and decision-record templates" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "Documentation/zigux/phase15-freeze-map-governance.md") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, ".github/workflows/zigux-bootstrap.yml") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "scripts/zigux/check-phase15-scripts-readme-alignment.py") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "scripts/zigux/check-phase15-review-process-handoff.py") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_architecture_council_review_process_manifest.json") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_architecture_council_review_process.zig") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_readiness_gate_manifest.json") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_readiness_gate.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_freeze_map_governance.zig") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zig build test --build-file zigux/tests/phase15_build.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_indefinite_c_policy.json") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_indefinite_c_policy.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_indefinite_c_blocker_evidence.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_indefinite_c_lane_owner_alignment.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "zigux/tests/phase15_governance_lane_sequencing.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "make -C zigux phase15") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "review-process packet") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "workflow-backed replay anchor") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "dedicated review-process manifest-backed pair") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "paired handoff guard") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "dedicated readiness manifest-backed pair") != null);
    try std.testing.expect(std.mem.indexOf(u8, manifest.handoff_evidence.current_repo_handoff, "direct shared-build replay") != null);
    ...
    try std.testing.expectEqual(@as(usize, 4), manifest.metrics.shared_governance_replay_route_count);
...
test "phase 15 parity scorecard gaps stay bounded and blocker-focused" {
    ...
    var saw_shared_governance_replay_route_metric = false;
    ...
    for (gaps, 0..) |gap, i| {
        ...
        if (std.mem.eql(u8, gap.id, "phase15-shared-governance-replay-route-metric")) {
            saw_shared_governance_replay_route_metric = true;
            try std.testing.expectEqualStrings("starter_landed", gap.status);
            try std.testing.expectEqualStrings("zigux/tests/phase15_parity_scorecard.json", gap.zigux_destination);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "validator-first route") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "dedicated make-backed test route") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "direct shared-build route") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "shared make convenience route") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "four-route replay inventory") != null);
        }
        if (std.mem.eql(u8, gap.id, "phase15-scorecard-handoff-evidence-readback-sync")) {
            ...
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "dedicated review-process note") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "handoff note") != null);
            ...
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "workflow-backed replay anchor") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "dedicated review-process manifest-backed pair") != null);
            ...
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "dedicated readiness manifest-backed pair") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "direct shared-build replay") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "blocker-evidence replay") != null);
            try std.testing.expect(std.mem.indexOf(u8, gap.why_now, "make-backed governance routes") != null);
        }
    ...
    try std.testing.expect(saw_shared_governance_replay_route_metric);
...
test "phase 15 council review gate stays aligned between the scorecard and checklist" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "phase15-shared-governance-replay-route-metric") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "phase15-scorecard-handoff-evidence-readback-sync") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "shared Phase 15 governance replay routes visible from the scorecard: `4`") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "Documentation/zigux/phase15-freeze-map-governance.md") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "Documentation/zigux/phase15-architecture-council-review-process.md") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "Documentation/zigux/phase15-indefinite-c-policy.md") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "scripts/zigux/README.md") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/README.md") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, ".github/workflows/zigux-bootstrap.yml") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "scripts/zigux/validate-phase15.py") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_architecture_council_review_process_manifest.json") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_architecture_council_review_process.zig") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_readiness_gate_manifest.json") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_readiness_gate.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_freeze_map_governance.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_build.zig") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "scripts/zigux/check-phase15-scripts-readme-alignment.py") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "scripts/zigux/check-phase15-review-process-handoff.py") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_indefinite_c_policy.json") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_indefinite_c_policy.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_indefinite_c_blocker_evidence.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_indefinite_c_lane_owner_alignment.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zigux/tests/phase15_governance_lane_sequencing.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "phase15-test") != null);
    try std.testing.expect(std.mem.indexOf(u8, scorecard_doc, "zig build test --build-file zigux/tests/phase15_build.zig") != null);
