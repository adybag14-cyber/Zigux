...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_cleartail_zero_memchr_replaceleft_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-cleartail-zero-memchr-replaceleft-replay",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step("phase1-helper-ports-a-cleartail-zero-memchr-replaceleft-replay", "Run Lane 06 cleartail/zero/memchr/replaceleft replay");
