...
pub fn build(b: *std.Build) void {
    ...
    const phase3_unsafe_policy = addUnsafePolicyTest(b, target, optimize);

    const step = b.step(
        "phase3-export-unsafe-policy-test",
        "Run the shared Phase 3 export/UAPI layout and unsafe-policy helper packets through a standalone paired build shard",
    );
    step.dependOn(&phase3_export_uapi_layout.step);
    step.dependOn(&phase3_unsafe_policy.step);
}
