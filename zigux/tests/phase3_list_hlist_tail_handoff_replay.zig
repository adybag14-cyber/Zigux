...
test "list view keeps the live tail visible over a detached tail handoff" {
...
test "list view reports the adopted tail handoff once the new tail keeps the stale backlink" {
    var head = list_view.ListHead{ .next = 0, .prev = 0 };
    var entry = list_view.ListHead{ .next = 0, .prev = 0 };
    var bridge = list_view.ListHead{ .next = 0, .prev = 0 };
    var tail = list_view.ListHead{ .next = 0, .prev = 0 };
    ...
    const view = list_view.ListView.init(&head);
    try expectListSequence(view, &.{ &entry, &bridge, &tail, &handoff });
    ...
    try std.testing.expectEqual(@as(usize, @intFromPtr(&handoff)), breakage.actual_prev);
    try std.testing.expect(!view.hasConsistentBacklinks());
}

test "hlist view keeps the live tail visible over a detached tail handoff" {
...
test "hlist view reports the adopted tail handoff once the new tail keeps the stale prev-link" {
    var head = hlist_view.HListHead{ .first = 0 };
    var entry = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var bridge = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    var tail = hlist_view.HListNode{ .next = 0, .pprev = 0 };
    ...
    const view = hlist_view.HListView.init(&head);
    try expectHListSequence(view, &.{ &entry, &bridge, &tail, &handoff });

    const breakage = view.firstBrokenPrevLink().?;
    try std.testing.expectEqual(@as(usize, 3), breakage.current_index);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&tail.next)), breakage.expected_pprev);
    try std.testing.expectEqual(@as(usize, @intFromPtr(&handoff.next)), breakage.actual_pprev);
    try std.testing.expect(!view.hasConsistentPrevLinks());
}
