...
test "representative 190-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 16, 946, 1984, 2966, 3904 };
...
test "every interior onehundredninety window keeps consecutive codes and alternating constructor families" {
...
test "onehundredninety windows preserve split raw and payload centers" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
