...
test "representative err-band tredecets keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 1016, 2044, 3060, 4082 };
    ...
    for (left_residues) |left_residue| {
        ...
        var raws: [13]usize = undefined;
        var slots: [13]xarray_slot_view.SlotView = undefined;
        ...
        const sum_012 = @as(u128, raws[0]) + @as(u128, raws[12]);
        const sum_111 = @as(u128, raws[1]) + @as(u128, raws[11]);
        const sum_210 = @as(u128, raws[2]) + @as(u128, raws[10]);
        const sum_39 = @as(u128, raws[3]) + @as(u128, raws[9]);
        const sum_48 = @as(u128, raws[4]) + @as(u128, raws[8]);
        const sum_57 = @as(u128, raws[5]) + @as(u128, raws[7]);
        try testing.expectEqual(sum_012, sum_111);
        try testing.expectEqual(sum_111, sum_210);
        try testing.expectEqual(sum_210, sum_39);
        try testing.expectEqual(sum_39, sum_48);
        try testing.expectEqual(sum_48, sum_57);
        try testing.expectEqual(sum_57, @as(u128, raws[6]) * 2);
...
test "every interior tredecet keeps consecutive codes and alternating constructor families" {
    var even_residue: usize = 0;
    while (even_residue + 12 <= err_band_span) : (even_residue += 2) {
...
test "tredecet windows preserve centered equations around the middle rejected-alias slot" {
    var even_residue: usize = 0;
    while (even_residue + 12 <= err_band_span) : (even_residue += 2) {
        const codes = [_]isize{
        ...
        try testing.expectEqual(codes[0] + codes[12], codes[1] + codes[11]);
        try testing.expectEqual(codes[1] + codes[11], codes[2] + codes[10]);
        try testing.expectEqual(codes[2] + codes[10], codes[3] + codes[9]);
        try testing.expectEqual(codes[3] + codes[9], codes[4] + codes[8]);
        try testing.expectEqual(codes[4] + codes[8], codes[5] + codes[7]);
        try testing.expectEqual(codes[5] + codes[7], codes[6] * 2);
        try testing.expectEqual(first_payload + thirteenth_payload, third_payload + eleventh_payload);
        try testing.expectEqual(third_payload + eleventh_payload, fifth_payload + ninth_payload);
        try testing.expectEqual(fifth_payload + ninth_payload, @as(usize, seventh_payload) * 2);
        try testing.expectEqual(first_payload + 6, thirteenth_payload);
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue)) + @as(u128, rawFromResidue(even_residue + 12)),
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 11)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 1)) + @as(u128, rawFromResidue(even_residue + 11)),
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 10)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 2)) + @as(u128, rawFromResidue(even_residue + 10)),
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 9)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 3)) + @as(u128, rawFromResidue(even_residue + 9)),
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 8)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 4)) + @as(u128, rawFromResidue(even_residue + 8)),
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 7)),
        );
        try testing.expectEqual(
            @as(u128, rawFromResidue(even_residue + 5)) + @as(u128, rawFromResidue(even_residue + 7)),
            @as(u128, rawFromResidue(even_residue + 6)) * 2,
        );
    }
}
