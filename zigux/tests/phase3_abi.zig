...
test "phase3 abi keeps notifier and shared constants reviewable" {
    const tail = abi.NotifierBlock{ .notifier_call = 0, .next = 0, .priority = 3 };
    const middle = abi.NotifierBlock{ .notifier_call = 0, .next = @intFromPtr(&tail), .priority = 5 };
    const head = abi.NotifierBlock{ .notifier_call = 0, .next = @intFromPtr(&middle), .priority = 5 };
