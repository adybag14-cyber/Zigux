...
const errno: isize = -309;
const next_errno: isize = -310;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 309 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 309 raw equation stays between its errno neighbors" {
...
test "errno 309 adjacent raw lanes keep value and pointer decoders precise" {
