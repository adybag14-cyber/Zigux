...
test "representative 135-raw err-band windows keep alias-err spacing exact" {
...
test "every interior onehundredthirtyfive window keeps consecutive codes and alternating constructor families" {
...
test "onehundredthirtyfive windows preserve a centered raw equation and a split payload seam" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
