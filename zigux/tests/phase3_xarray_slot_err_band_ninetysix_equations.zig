...
test "representative 96-raw err-band windows keep alias-err spacing exact" {
    const left_residues = [_]usize{ 0, 2, 956, 1998, 3000, 3998 };
...
test "every interior ninetysix window keeps consecutive codes and alternating constructor families" {
...
test "ninetysix windows preserve a dual-center raw seam and a centered payload lane" {
    var even_residue: usize = 0;
    while (even_residue + (window_len - 1) <= err_band_span) : (even_residue += 2) {
        const seam_code_sum =
