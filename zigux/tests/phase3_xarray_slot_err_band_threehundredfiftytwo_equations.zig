...
const errno: isize = -352;
const next_errno: isize = -353;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 352 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 352 raw equation stays between its errno neighbors" {
...
test "errno 352 adjacent raw lanes keep value and pointer decoders precise" {
