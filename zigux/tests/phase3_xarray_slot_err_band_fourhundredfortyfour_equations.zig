...
const errno: isize = -444;
const next_errno: isize = -445;
const raw = err_ptr.fromErrorCode(errno);
const next_raw = err_ptr.fromErrorCode(next_errno);

test "errno 444 keeps xarray slot error decoding closed over adjacent lanes" {
...
test "errno 444 raw equation stays between its errno neighbors" {
...
test "errno 444 adjacent raw lanes keep value and pointer decoders precise" {
