...
test "phase3 uapi version keeps its published layout" {
    const snapshot = current();

    try std.testing.expectEqual(@as(u32, 0), snapshot.abi_major);
    try std.testing.expectEqual(@as(u32, 1), snapshot.abi_minor);
    try std.testing.expectEqual(@as(u32, 1), snapshot.header_family_revision);
    try std.testing.expect(eql(snapshot, current()));
}
