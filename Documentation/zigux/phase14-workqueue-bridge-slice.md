...
- adds a five-checkpoint concurrency audit outline that names manager-role serialization, `worker_pool` forward-progress fields, the `__queue_work()` `max_active` or ordered inactive-list gate, scheduler hook state transitions, and the rescuer mayday handoff while keeping all live ownership in C
...
The next honest bounded step in this same lane is to tighten the remaining lock-handoff audit around `__queue_work()`, `process_one_work()`, and `worker_thread()` so the bridge records where `last_pool->lock`, `pool->lock`, and sleep or wake transitions are released and reacquired before any wrapper leaves the current boundary-map-only posture.
