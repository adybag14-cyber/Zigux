...
The next honest bounded step in this same lane is to audit `queue_delayed_work_on()`, `mod_delayed_work_on()`, and `__queue_delayed_work()` so the bridge records the delayed timer handoff, CPU-online preconditions, and `clear_pending_if_disabled()` gate before any wrapper claims timer-base or requeue parity.
