...
This packet is now parked in blocked maintenance unless the shared Phase 14 smoke packet or this workqueue survey drifts. Any reopen should stay review-only and, if a narrower follow-up is still justified, audit `mod_delayed_work_on()` zero-delay rearm, its `try_to_grab_pending()` ownership checks, and the choice between `__queue_work()` versus `__queue_delayed_work()` while keeping timer-base, CPU-affinity, and requeue ownership in C.
