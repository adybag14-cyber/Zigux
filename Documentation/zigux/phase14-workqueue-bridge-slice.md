...
The current lane now also records the delayed-submission alias fan-in across `queue_delayed_work_on()`, `mod_delayed_work_on()`, and `__queue_delayed_work()` without claiming live timer expiry or delayed-work execution ownership.

The next honest bounded step in this same lane is to audit `delayed_work_timer_fn()` and its handoff into `__queue_work()` so the bridge records timer-expiry submission ownership before any wrapper leaves the current boundary-map-only posture.
