...
The next honest bounded step in this same lane is to audit `queue_delayed_work_on()`, `mod_delayed_work_on()`, and `__queue_delayed_work()` so the bridge records the delayed submission aliases around pending ownership, disabled-work bypass, and timer handoff before any wrapper claims CPU-affinity, timer, or requeue parity.
