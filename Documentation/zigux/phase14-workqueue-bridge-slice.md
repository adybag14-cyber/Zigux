...
Keep the following facts aligned across the bridge packet, manifest, reviewability test, the shared review checklist, and the directly coupled slice and survey notes:
  ...
  * the next same-lane step is still a packet-local reread until the bridge-local blocked-maintenance handoff is aligned again, leaving broader `phase14_build` rerun vocabulary to the shared Phase 14 smoke packet rather than treating it as a live execution port
