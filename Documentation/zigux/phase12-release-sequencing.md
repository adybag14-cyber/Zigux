...
- the two commit-pinned fallback notes now publish a four-step release order with direct `zig build smoke --build-file zigux/tests/phase12_build.zig --summary all` first, while `scripts/zigux/check-build-only-phase12-surface.py` still hard-codes the older three-step fallback markers that begin at `make -C zigux phase12-smoke`, so the remaining PMO risk is checker-side drift rather than broad naming churn
...
The docs-root, review-checklist, scripts-root, tests-root wording repairs are already landed on `master`, and the PMO closure companion is now available for manual tranche-close review, so the next bounded same-lane follow-through is still checker-side drift control rather than another naming pass:
- materialize `scripts/zigux/check-build-only-phase12-surface.py` from live `master`, update its fallback-note marker set and self-test corpus so both commit-pinned fallback notes fail closed on the actual four-step order (`zig build smoke --build-file zigux/tests/phase12_build.zig --summary all`, `make -C zigux phase12-smoke`, `zig build test --build-file zigux/tests/phase12_build.zig --summary all`, `make -C zigux phase12`), and keep the checker scoped to the shipped build-only contract instead of inventing a broader validator-first packet
- rerun `python3 scripts/zigux/check-build-only-phase12-surface.py --self-test` and the live checker after that bounded checker edit so the fallback-note smoke-order guard actually matches the release order already published in the two commit-pinned fallback notes
...
- if the lane reopens for another degraded-workflow drift, start by diffing those shipped packet surfaces and comparing them against the checker's fallback-note marker block before widening into any driver-local or helper-local Phase 12 work
