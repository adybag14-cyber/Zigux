...
## Packet Reading
  ...
  * Current `zigux/tests/phase12_build.zig` wires the same two bounded `virtio_net` follow-ups through both `smoke` and `test`, so the shared release packet should keep those bounded queue-resume and transmit-disposition replays explicit without rounding them up into live interrupt-backed transmit completion parity or queue-restart parity.
