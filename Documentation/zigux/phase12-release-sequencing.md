...
- Current `master` now exposes the bounded driver-local `phase12_nvme_pci` direct replay, slice, survey, survey gate, and manifest packet outside the wired shared release route, while the parked libbpf replay files still do not ship as shared build outputs, so this sequencing note must keep NVMe explicit as a driver-local published foothold without describing either family as part of the wired release packet.
