...
- the two commit-pinned fallback notes and `scripts/zigux/check-build-only-phase12-surface.py` now agree on the shipped smoke-first reminder, so the remaining PMO risk is keeping that checker-backed release wording aligned across the docs root, checklist, scripts root, tests root, workflow, Makefile, checklist, sequencing, and fallback-note surfaces instead of reopening the already-landed guard as if it were still missing
