...
- the two commit-pinned fallback notes now carry the shipped smoke-first release order, but `scripts/zigux/check-build-only-phase12-surface.py` still does not fail closed when either note drops or duplicates that three-step reminder
...
- extend `scripts/zigux/check-build-only-phase12-surface.py` so it exact-counts the smoke-first shared replay reminder inside `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md` and `Documentation/zigux/phase12-virtio-scsi-raw-github-fallback-catalog.md`, then rerun `python3 scripts/zigux/check-build-only-phase12-surface.py --self-test` and the live checker after any shared Phase 12 docs-root, review-checklist, scripts-root, tests-root, workflow, Makefile, or fallback-note edit so the shipped build-only contract stays fail-closed
