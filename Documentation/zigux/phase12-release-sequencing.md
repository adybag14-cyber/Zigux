...
The next honest same-lane follow-through is therefore to refresh only whichever shared reminder drifts next: the reviewer-facing wording in `Documentation/zigux/review-checklist.md` and `zigux/tests/README.md` already mirrors the repo-local `.zig-toolchain` then attached-Zig rerun order now carried by `scripts/zigux/README.md`, this sequencing note, and the shipped `make -C zigux phase12-validate` support bundle, so there is no separate PMO reminder target to reopen today. Keep the still-unwired-but-present NVMe foothold explicit and leave the already-landed scripts-root, tests-root, and checklist reminder syncs parked rather than widening into new driver-local behavior, fallback-catalog changes, or adjacent complex-driver wording.
