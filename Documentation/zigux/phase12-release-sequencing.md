...
- the two commit-pinned fallback notes and `scripts/zigux/check-build-only-phase12-surface.py` now agree on the shipped smoke-first reminder, so the remaining PMO risk is keeping that checker-backed release wording aligned across the docs root, checklist, scripts root, tests root, workflow, Makefile, and fallback-note surfaces instead of reopening the already-landed guard as if it were still missing
...
- rerun `python3 scripts/zigux/check-build-only-phase12-surface.py --self-test` and the live checker after any shared Phase 12 docs-root, review-checklist, scripts-root, tests-root, workflow, Makefile, or fallback-note edit so the already-shipped smoke-order guard stays fail-closed across the sequencing note and the two commit-pinned fallback notes instead of being rediscovered as a stale blocker later
