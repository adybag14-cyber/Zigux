...
- `scripts/zigux/README.md` still does not explicitly pin `Documentation/zigux/phase12-release-closure-checklist.md` inside its shared Phase 12 review-surface inventory, so the PMO closure companion is visible across docs-root, sequencing, tests-root, and the two commit-pinned fallback notes but not yet across every shipped shared index on `master`
- `scripts/zigux/check-build-only-phase12-surface.py` still does not explicitly pin `Documentation/zigux/phase12-release-closure-checklist.md` inside its fail-closed marker set, so even after the scripts-root companion reminder lands the PMO closure companion will still not be enforced by the shipped checker
...
The docs-root, review-checklist, and tests-root wording repairs are already landed on `master`, but `scripts/zigux/README.md` still omits `Documentation/zigux/phase12-release-closure-checklist.md` from the shared Phase 12 surface it inventories, so the next smallest same-lane follow-through is a scripts-root truthfulness sync before returning to checker-local drift control:
- refresh `scripts/zigux/README.md` so its Phase 12 flow explicitly keeps `Documentation/zigux/phase12-release-closure-checklist.md` beside `Documentation/zigux/phase12-release-sequencing.md`, `scripts/zigux/check-build-only-phase12-surface.py`, `.github/workflows/zigux-bootstrap.yml`, `zigux/tests/phase12_build.zig`, and `zigux/Makefile`
- after that scripts-root reminder lands, refresh `scripts/zigux/check-build-only-phase12-surface.py` so its fail-closed scripts-root marker set explicitly keeps `Documentation/zigux/phase12-release-closure-checklist.md` beside `Documentation/zigux/phase12-release-sequencing.md`, the shipped smoke-first replay order, and the current build-only contract surfaces
...
- after that scripts-root and checker-local closure-companion work lands, rerun `python3 scripts/zigux/check-build-only-phase12-surface.py --self-test`, `python3 scripts/zigux/check-build-only-phase12-surface.py`, `zig build smoke --build-file zigux/tests/phase12_build.zig --summary all`, `make -C zigux phase12-smoke`, `zig build test --build-file zigux/tests/phase12_build.zig --summary all`, and `make -C zigux phase12` so the closure companion stays tied to shipped replay routes instead of reopening validator-first or runtime-recovery claims
...
- if the lane reopens for another degraded-workflow drift, start by diffing those shipped packet surfaces, rereading `scripts/zigux/README.md`, and rereading `Documentation/zigux/phase12-release-closure-checklist.md` before widening into checker work, new wording, or any driver-local or helper-local Phase 12 task
