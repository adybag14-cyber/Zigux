...
- scope: keep the shipped Phase 12 release packet reviewable through the same current validator-first then smoke-first contract while keeping the roadmap's DMA-safe, queueing, throughput, segmented-rollout, rollback, and recovery boundaries explicit beside the still-unwired broader Phase 12 notes and the newer driver-local `virtio_scsi` rollback-lab survey packet
