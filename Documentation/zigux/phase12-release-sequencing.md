...
## Fallback Split
  ...
  * `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md` is still the named current-master gap-inventory companion for the shipped NVMe starter-plus-verifier-plus-direct-replay-plus-slice-plus-survey foothold, but direct contents and raw-URL reads currently return it missing in this runtime, so keep it treated as an absent degraded-read target rather than as a second commit-pinned or directly readable fallback artifact.
