...
The next honest same-lane follow-through is therefore to refresh the shared tests-root reminder packet first: `zigux/tests/README.md` still needs to mirror the repo-local `.zig-toolchain` then attached-Zig rerun wording now already carried by `scripts/zigux/README.md`, this sequencing note, and the shipped `make -C zigux phase12-validate` support bundle, while keeping the still-unwired-but-present NVMe foothold explicit and leaving the already-landed scripts-root reminder parked. After that one-file PMO sync lands, return only to whichever of `Documentation/zigux/review-checklist.md` or `scripts/zigux/check-build-only-phase12-surface.py` drifts next rather than widening into new driver-local behavior, fallback-catalog changes, or adjacent complex-driver wording.
