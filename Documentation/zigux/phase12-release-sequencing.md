...
The next honest same-lane follow-through is therefore to refresh only whichever of `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, or `scripts/zigux/check-build-only-phase12-surface.py` drifts next against that same support-checker-plus-validate-route reminder and the still-unwired-but-present NVMe foothold, while leaving the shared scripts-root closure-note retell to `P12-Y07`, before parking `P12-L07` again rather than widening into new driver-local behavior, fallback-catalog changes, or adjacent complex-driver wording.
