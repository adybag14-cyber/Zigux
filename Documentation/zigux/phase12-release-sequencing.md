...
The broader PMO reminder packet still has one shared-surface undercount, though: `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and `zigux/tests/README.md` do not yet mirror the shipped `phase12-validate` support-bundle wording, the repo-local `.zig-toolchain` then attached-Zig degraded rerun order, or the dedicated `scripts/zigux/check-phase12-release-readiness-packet.py` guard that `scripts/zigux/README.md`, this sequencing note, the release-readiness survey, the closure checklist, the coordination matrix, and the raw-coverage survey already keep explicit.
The next honest same-lane follow-through is therefore to refresh that shared docs-root, checklist, and tests-root reminder packet one surface at a time, starting with the docs root, before widening into new driver-local behavior, fallback-catalog changes, or adjacent complex-driver wording.