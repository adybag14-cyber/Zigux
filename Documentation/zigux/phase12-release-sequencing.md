...
The next honest same-lane follow-through is therefore to refresh only whichever shared reminder drifts next: `Documentation/zigux/review-checklist.md` is the first likely reread target because it still owns the reviewer-facing Phase 12 wording beside `scripts/zigux/check-build-only-phase12-surface.py`, while `zigux/tests/README.md` already mirrors the repo-local `.zig-toolchain` then attached-Zig rerun wording now carried by `scripts/zigux/README.md`, this sequencing note, and the shipped `make -C zigux phase12-validate` support bundle. Keep the still-unwired-but-present NVMe foothold explicit and leave the already-landed scripts-root and tests-root reminder syncs parked rather than widening into new driver-local behavior, fallback-catalog changes, or adjacent complex-driver wording.
