...
- `scripts/zigux/check-build-only-phase12-surface.py` still does not explicitly pin `Documentation/zigux/phase12-release-closure-checklist.md` inside its fail-closed marker set, so the PMO closure companion is visible across docs-root, sequencing, tests-root, and the two commit-pinned fallback notes but is not yet enforced by the shipped checker
