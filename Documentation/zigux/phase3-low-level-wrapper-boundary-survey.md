...
The live Phase 3 tree is no longer missing any roadmap-approved atomic, barrier, or MMIO wrapper leaf inside the current low-level wrapper packet. `zigux/helpers/atomic.zig` now directly exposes order-checked `load()`, `store()`, `exchange()`, `compareExchangeStrong()`, `compareExchangeWeak()`, and `fetchAdd()`/`fetchSub()`/`fetchNand()`/`fetchOr()`/`fetchAnd()`/`fetchXor()`/`fetchMin()`/`fetchMax()` helpers; `zigux/helpers/barrier.zig` keeps `compiler()`, `acquire()`, `release()`, `full()`, `acquireRelease()`, `fullFence()`, and `storeLoad()` explicit; and `zigux/helpers/mmio.zig` keeps typed, scoped, byte-policy, whole-record interop-policy, exchange, and masked volatile accessors directly readable on current `master`.

That live barrier helper surface is now slightly ahead of the shared survey-proof packet: `zigux/helpers/barrier.zig` already keeps the seq_cst `storeLoad()` alias explicit beside `fullFence()`, but the dedicated survey validator in `scripts/zigux/validate-phase3-low-level-wrapper-survey.py` still exact-requires only `fullFence()` and the focused replay in `zigux/tests/phase3_low_level_wrappers.zig` still routes its seq_cst reviewability through `fullFence()` alone. The next same-packet follow-through should therefore stay on validator-or-replay truthfulness for `storeLoad()` rather than widening into another helper-body wrapper step.
