...
- `zigux/tests/phase3_low_level_wrappers.zig` now directly proves the shipped helper surface, including `fetchNand`, signed atomic arithmetic and min/max edges, monotonic strong `compareExchange()`, `acq_rel` strong `compareExchange()` mismatch handling, weak compare-exchange coverage, explicit barrier-locality replay, the explicit MMIO interop-policy gate replay including the byte-scoped range plus byte-scoped read/write relays, the raw-pointer-bridge policy gate replay, non-`seq_cst` ordering, plus byte-addressed 16-bit, 32-bit, and 64-bit MMIO range descriptors and odd-offset MMIO behavior.
