...
- A second bounded governance split also remains on current `master`: the bridge source and helper-local reminder packet both say explicit `allconfig` overrides are supported for `allmodconfig`, `allnoconfig`, `allyesconfig`, and `randconfig`, but the fixture-backed `conf_cases` packet and `allconfig_override_packet` still only govern that explicit override path through `allmodconfig` and `randconfig`. `allnoconfig` and `allyesconfig` explicit overrides remain helper-local coverage only today.
...
- If the next same-family move reopens fixture governance instead, reread `scripts/zigux/kconfig/conf_bridge.zig`, `scripts/zigux/check-kconfig-bridge.py`, `zigux/tests/fixtures/kconfig_bridge/conf_manifest.json`, and `zigux/tests/fixtures/kconfig_bridge/cases.json` together before choosing between materializing explicit `allconfig` expected-output fixtures for `allnoconfig` and `allyesconfig` or narrowing the helper-local override reminder so it no longer overclaims fixture-backed governance.
