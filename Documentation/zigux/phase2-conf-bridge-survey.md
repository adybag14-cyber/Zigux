...
- The bootstrap ledger's bounded kconfig bridge scaffolding packet centers on `scripts/zigux/kconfig/conf_bridge.zig`, `scripts\zigux/check_kconfig_bridge.zig`, and `zigux/tests/fixtures/kconfig_bridge/`.
...
- `scripts\zigux/check_kconfig_bridge.zig` is present on `master` and still treats the conf-side packet as a bounded bridge-plus-fixture surface, with the current required mode inventory, manifest packet checks, and helper-anchor inventory review.
