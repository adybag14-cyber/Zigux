...
The adjacent PMO release note, `Documentation/zigux/README.md`, `zigux/tests/README.md`, and `scripts/zigux/check-build-only-phase12-surface.py` now all pin `zigux/tests/phase12_libbpf_snapshot_determinism.zig` as the canonical deterministic replay path and keep only `zigux/tests/fixtures/phase12_libbpf_snapshot.json` plus `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.json` under the fixtures tree.
That docs-root, tests-root, and checker-local determinism follow-through is closed on `master`, so this lane no longer needs to steer future runs back through the same replay-path or fixture-under-count repairs.
