...
- scope: shared release-planning, review-surface truthfulness, smoke-first replay reminders, fallback wording, and anti-overlap guidance for the bounded `nvme_pci`, `virtio_net`, and `virtio_scsi` families while current Phase 12 evidence stays split between published shared reminders, a shipped `virtio_scsi` lab packet, a starter-present `virtio_net` packet, and a published NVMe starter-plus-verifier reminder packet whose survey, slice, and replay surfaces still remain outside the wired shared release route
