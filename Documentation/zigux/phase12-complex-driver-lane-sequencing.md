...
The older shared tests-root release-readiness reminder is not fully closed on `master`: `zigux/tests/README.md` still names the stale `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.zig` path inside its broad Phase 12 packet even though the shipped replay path, the build-only checker, the docs-root packet, the scripts-root packet, and this lane note all treat `zigux/tests/phase12_libbpf_snapshot_determinism.zig` as the canonical deterministic replay entrypoint.
That means the narrowest honest same-lane reopen is now one tests-root wording repair for that stale fixtures-path claim rather than another broader PMO or checklist reread.
...
If this lane reopens, start by diffing `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `Documentation/zigux/freeze-map.md`, `Documentation/zigux/phase12-release-closure-checklist.md`, `Documentation/zigux/phase12-release-readiness-survey.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, `Documentation/zigux/phase12-release-coordination-matrix.md`, `Documentation/zigux/phase12-libbpf-heavy-consumer-lane-sequencing.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, `scripts/zigux/check-build-only-phase12-surface.py`, and `.github/workflows/zigux-bootstrap.yml` against this note before widening into any new driver-local wording claim.

The adjacent PMO release note, `Documentation/zigux/README.md`, `scripts/zigux/README.md`, and `scripts/zigux/check-build-only-phase12-surface.py` now all pin `zigux/tests/phase12_libbpf_snapshot_determinism.zig` as the canonical deterministic replay path and keep only `zigux/tests/fixtures/phase12_libbpf_snapshot.json` plus `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.json` under the fixtures tree. `zigux/tests/README.md` is the remaining shared-surface exception because it still carries the stale fixtures-tree path.
That shared determinism follow-through is therefore not fully closed on `master`, and the next honest same-lane step remains the one-file tests-root path repair above rather than another broader PMO reread.
