...
The adjacent PMO release note now pins `zigux/tests/phase12_libbpf_snapshot_determinism.zig` as the canonical deterministic replay path and leaves only `zigux/tests/fixtures/phase12_libbpf_snapshot.json` plus `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.json` under the fixtures tree, but `Documentation/zigux/README.md` still rounds that replay back into `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.zig` today. If this lane needs the next smallest same-lane follow-through, fix that one docs-root path drift before reopening any broader Phase 12 surface work.
