...
- Keep the current partial direct-read bridge explicit too: `Documentation/zigux/phase12-raw-github-coverage-survey.md` now records that `scripts/zigux/check-build-only-phase12-surface.py`, `.github/workflows/zigux-bootstrap.yml`, and `scripts/zigux/README.md` are directly readable on current `master`, while `zigux/tests/phase12_build.zig`, `zigux/Makefile`, and `scripts/zigux/check-phase12-release-readiness-packet.py` still fail through the same bridge, so the readable checker-plus-workflow-plus-scripts trio stays reminder evidence only rather than proof for the larger shared packet.
