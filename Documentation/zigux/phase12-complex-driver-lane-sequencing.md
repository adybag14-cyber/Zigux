...
- scope: shared release-planning truthfulness, build-only contract reminders, and anti-overlap guidance for the starter-present `virtio_net` packet, the last-known `virtio_scsi` rollback-lab packet vocabulary that current GitHub contents readback in this runtime does not rematerialize directly, and the published-but-still-unwired NVMe foothold
...
- treat the older `virtio_scsi` rollback-lab file family as last-known packet vocabulary until current `master` readback rematerializes it directly: keep `Documentation/zigux/phase12-virtio-scsi-slice.md`, `Documentation/zigux/phase12-virtio-scsi-survey.md`, `zigux/tests/phase12_virtio_scsi_manifest.json`, `zigux/tests/phase12_virtio_scsi_survey.zig`, `drivers/scsi/virtio_scsi.zig`, `zigux/tests/phase12_virtio_scsi.zig`, `zigux/tests/phase12_virtio_scsi_syntax_lab.zig`, `zigux/tests/phase12_virtio_scsi_repeated_replan_gate.zig`, `zigux/tests/phase12_virtio_scsi_repeated_rollback_gate.zig`, and `zigux/tests/phase12_virtio_scsi_packet.zig` framed as last-known packet members rather than direct current-slot proof while those paths still return missing through the live GitHub contents reads used in this lane
- shared PMO companions such as `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-readiness-survey.md`, and `Documentation/zigux/phase12-release-coordination-matrix.md` may still keep the `virtio_scsi` survey companions explicit as shared packet vocabulary, but this anti-overlap note must treat that wording as reminder context only until direct contents readback for those same files returns in this lane
- the driver-local `virtio_scsi` rollback-lab companions stay bounded review vocabulary only until that direct readback returns; do not reopen storage-driver, queueing, rollback execution, or completion-path claims from this note alone
...
- This note must keep the shared release wording bounded to the starter-present `virtio_net` packet, the last-known `virtio_scsi` rollback-lab packet vocabulary, the published-but-still-unwired NVMe foothold, and the shipped validator-first support bundle.
- This note must not recast the parked libbpf packet as direct shared replay or as a second active complex-driver lane.

## Next Bounded Step

If the shared Phase 12 packet moves again, reread this note beside `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-readiness-survey.md`, `Documentation/zigux/phase12-release-coordination-matrix.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, `Documentation/zigux/phase12-libbpf-heavy-consumer-lane-sequencing.md`, `scripts/zigux/check-build-only-phase12-surface.py`, and `zigux/tests/README.md` before widening any driver-local claims. If the `virtio_scsi` file family still returns missing on current `master`, keep future same-lane follow-through parked inside one shared reminder or checker truthfulness repair at a time instead of re-promoting those paths as direct evidence from this anti-overlap note, and keep any broader PMO wording framed as shared packet vocabulary rather than current-slot proof until that direct readback returns.
