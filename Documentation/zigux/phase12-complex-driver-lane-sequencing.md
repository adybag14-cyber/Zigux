...
That docs-root, scripts-root, tests-root, and checker-local determinism follow-through is closed on `master`, so this lane no longer needs to steer future runs back through the same replay-path or fixture-under-count repairs.
