...
- Keep the published NVMe reminder packet explicit through `Documentation/zigux/phase12-nvme-pci-slice.md`, `Documentation/zigux/phase12-nvme-pci-survey.md`, and `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md`, while treating direct `zigux/tests/phase12_nvme_pci*.zig`, `drivers/nvme/host/pci_verify.zig`, and `zigux/tests/phase12_nvme_pci_manifest.json` paths as lane-local unpublished-or-absent evidence until fresh repo reality on current `master` proves otherwise. Do not borrow the published NVMe docs as proof that the shared complex-driver packet has already widened into a shipped direct NVMe replay.
