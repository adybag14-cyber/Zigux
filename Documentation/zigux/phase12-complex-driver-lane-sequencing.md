...
If this lane reopens, start by diffing `Documentation/zigux/review-checklist.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, `Documentation/zigux/phase12-release-coordination-matrix.md`, `Documentation/zigux/phase12-libbpf-heavy-consumer-lane-sequencing.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` against this note plus `Documentation/zigux/README.md` before widening into any new driver-local wording claim.

The next likely same-lane undercount is `zigux/tests/README.md`: the shared tests-root Phase 12 reminder already keeps `zig build smoke --build-file zigux/tests/phase12_build.zig --summary all`, `make -C zigux phase12-smoke`, and `make -C zigux phase12` explicit, but it can still leave the direct `zig build test --build-file zigux/tests/phase12_build.zig --summary all` replay less visible than the now-aligned docs-root, fallback-overview, and compact release-coordination surfaces.

If live `master` still shows that tests-root undercount, land that one-file repair before widening; otherwise keep the next step inside the smallest review-checklist, fallback-overview, compact-matrix, shared libbpf anti-overlap, scripts-root, or other tests-root wording repair that restores the three driver-local ownership boundaries and the no-dedicated-`validate-phase12.py` posture instead of replaying already-landed docs-root or checker-local fixes.
