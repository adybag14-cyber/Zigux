...
If this lane reopens, start by diffing `Documentation/zigux/review-checklist.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, `Documentation/zigux/phase12-release-coordination-matrix.md`, `Documentation/zigux/phase12-libbpf-heavy-consumer-lane-sequencing.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` against this note before widening into any new driver-local wording claim.

The next likely same-lane undercount is now one of the non-owner shared surfaces rather than the broad docs root: either `Documentation/zigux/review-checklist.md` or `Documentation/zigux/phase12-raw-github-coverage-survey.md` if either stops keeping the two commit-pinned fallback artifacts, the compact release-coordination matrix, and the two anti-overlap companions equally visible beside the shared smoke-first replay packet.

If neither shared surface drifts, keep the next step inside the smallest compact-matrix, shared libbpf anti-overlap, scripts-root, or tests-root wording repair that restores the three driver-local ownership boundaries and the no-dedicated-`validate-phase12.py` posture instead of replaying already-landed tests-root, docs-root smoke-summary, docs-root fallback-catalog, or checker-local fixes.
