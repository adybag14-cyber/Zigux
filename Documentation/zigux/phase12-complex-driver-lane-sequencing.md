...
- This note must keep the shared release wording bounded to the split-helper `virtio_net` packet, its shared-route queue-resume, receive-refill replay, transmit-recycle, post-reset replay, and throughput-parity shards, the rollback-evidence-only `virtio_scsi` survey packet, the published-but-still-unwired NVMe foothold, and the directly readable validator-side support bundle while `make -C zigux phase12-validate` remains reminder vocabulary and the returned `phase12-smoke`, `phase12-test`, and `phase12` wrappers stay current `master` evidence.
