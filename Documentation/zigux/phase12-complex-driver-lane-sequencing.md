...
After that reread, keep the next step inside the smallest docs-root, review-checklist, scripts-root, tests-root, fallback-overview, compact-matrix, or shared libbpf anti-overlap wording repair that restores those three driver-local ownership boundaries. Right now the likely first check is whether `Documentation/zigux/README.md` still leaves `drivers/nvme/host/pci_verify.zig`, `make -C zigux phase12-smoke`, the direct `zig build test --build-file zigux/tests/phase12_build.zig --summary all` replay, or the explicit no-`validate-phase12.py` wording implicit compared with the checker-backed shared packet.
