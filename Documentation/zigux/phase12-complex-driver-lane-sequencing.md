...
The next likely same-lane undercount is now `Documentation/zigux/review-checklist.md` if it stops keeping the PMO closure companion, the two commit-pinned fallback artifacts, the compact release-coordination matrix, the two anti-overlap companions, and the freeze-boundary reminder equally visible beside the shared smoke-first replay packet.

If the shared checklist stays aligned too, keep the next step inside the smallest compact-matrix, shared libbpf anti-overlap, scripts-root, or tests-root wording repair that restores the three driver-local ownership boundaries and the no-dedicated-`validate-phase12.py` posture instead of replaying already-landed tests-root, docs-root smoke-summary, docs-root fallback-catalog, raw-fallback overview, or checker-local fixes.
