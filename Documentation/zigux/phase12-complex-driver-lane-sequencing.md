...
That means this lane should not treat the whole tests-root packet as settled yet. `scripts/zigux/check-build-only-phase12-surface.py` already forbids that stale `fixtures/` replay path on current `master`, so the next honest same-lane follow-through is only to repair the tests-root README wording rather than reopening checker logic.
...
That docs-root, scripts-root, and checker-local determinism follow-through is closed on `master`; the remaining same-lane repair is only the stale tests-root key-entrypoint path called out above.
