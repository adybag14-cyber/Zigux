...
The older shared tests-root release-readiness reminder is now also closed on `master`: `zigux/tests/README.md` now names `zigux/tests/phase12_libbpf_snapshot_determinism.zig` inside its Phase 12 key-entrypoint list, matching the shipped replay path and the build-only checker's forbidden `fixtures/` guard.
That tests-root, docs-root, scripts-root, and checker-local determinism follow-through is now closed on `master`, so this lane no longer needs to steer future runs back through that README wording repair.
...
The adjacent PMO release note, `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, and `scripts/zigux/check-build-only-phase12-surface.py` now all pin `zigux/tests/phase12_libbpf_snapshot_determinism.zig` as the canonical deterministic replay path and keep only `zigux/tests/fixtures/phase12_libbpf_snapshot.json` plus `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.json` under the fixtures tree.
That shared determinism follow-through is closed on `master`, so the next honest same-lane step, if one is needed later, is to reread the shared Phase 12 PMO, anti-overlap, and freeze-boundary surfaces together before reopening any smaller wording drift.
