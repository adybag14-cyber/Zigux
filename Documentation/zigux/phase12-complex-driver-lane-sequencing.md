...
The adjacent PMO release note and `Documentation/zigux/README.md` now both pin `zigux/tests/phase12_libbpf_snapshot_determinism.zig` as the canonical deterministic replay path and leave only `zigux/tests/fixtures/phase12_libbpf_snapshot.json` plus `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.json` under the fixtures tree.
If this lane needs the next smallest same-lane follow-through, keep that docs-root replay path under `scripts/zigux/check-build-only-phase12-surface.py` so future docs drift fails closed before any broader Phase 12 surface work reopens.
