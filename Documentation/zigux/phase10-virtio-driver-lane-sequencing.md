...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. The earlier tests-root follow-through is already complete on live `master`, so the current smallest shared-surface follow-through now sits in `Documentation/zigux/README.md`: keep `drivers/virtio/virtio_verify.zig`, `drivers/virtio/virtio_ring_verify.zig`, `drivers/virtio/virtio_input_verify.zig`, `drivers/virtio/virtio_mmio_verify.zig`, and `zigux/tests/phase10_closure_manifest.json` explicit beside the existing checker-backed build-and-make packet there instead of leaving the shipped verifier packet implicit at docs root.
