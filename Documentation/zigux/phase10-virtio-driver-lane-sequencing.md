...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. Fresh shared-surface readback now shows the docs-root Phase 10 summary already keeps the landed direct `drivers/virtio/virtio.zig`, `drivers/virtio/virtio_driver_id.zig`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_mmio.zig`, and `zigux/tests/phase10_virtio_input_probe_preflight.zig` replay explicit beside the focused verify, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain reminders.

The next same-lane follow-through is now one shared-summary truthfulness repair in `scripts/zigux/README.md`: keep the scripts-root Phase 10 flow explicit about the landed `zigux/tests/phase10_virtio_input_probe_preflight.zig` replay beside the already-named direct driver files, verify replays, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain reminders.

After that one scripts-root repair lands, the next remaining shared-summary reread should move to `zigux/tests/README.md` so the same focused probe-preflight replay stays visible there too, rather than reopening settled driver-local helper, manifest, or survey work.
