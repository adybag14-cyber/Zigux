...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. Fresh shared-surface readback shows the dedicated input probe-preflight, queue-callback-preflight, registration-preflight, and teardown-observation replays now stay explicit in the shared owner map beside the existing input verify and status-drain reminders, and the shared closure note now stays explicit beside the shared closure manifest and tests-root review companion.

The next same-lane follow-through is now one shared-summary truthfulness repair in `Documentation/zigux/README.md`: keep the docs-root Phase 10 summary explicit about the landed direct `drivers/virtio/virtio.zig`, `drivers/virtio/virtio_driver_id.zig`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_input.zig`, and `drivers/virtio/virtio_mmio.zig` review surface, and keep the focused `zigux/tests/phase10_virtio_input_probe_preflight.zig` replay visible beside the already-named verify, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain reminders.

After that one docs-root repair lands, leave this shared owner-map note parked again until another one-file checker, manifest, survey, or shared-summary truthfulness gap appears, rather than reopening settled driver-local helper, manifest, or survey work.
