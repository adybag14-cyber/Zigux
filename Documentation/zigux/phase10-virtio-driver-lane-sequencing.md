...
- the focused `zigux/tests/phase10_virtio_input_queue_callback_preflight.zig` and `zigux/tests/phase10_virtio_input_status_drain.zig` replays
...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. Fresh shared-surface readback shows the dedicated input queue-callback-preflight replay now stays explicit in the shared owner map beside the existing input verify and status-drain reminders, so the next same-lane follow-through should leave this shared note parked until a fresh one-file checker, manifest, survey, or shared-summary truthfulness gap appears, rather than reopening settled driver-local helper, manifest, or survey work.