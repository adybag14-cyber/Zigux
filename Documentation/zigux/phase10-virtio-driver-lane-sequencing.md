...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. Fresh shared-surface readback shows `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, and `zigux/tests/README.md` already keep the full verifier packet plus `zigux/tests/phase10_closure_manifest.json` explicit on live `master`, but `Documentation/zigux/review-checklist.md` still undercounts that same shared Phase 10 packet by omitting the explicit `drivers/virtio/virtio_verify.zig` and `drivers/virtio/virtio_mmio_verify.zig` paths from its shared reviewer prompt beside the already-landed ring and input verifier cues. The next same-lane follow-through should therefore stay inside that one checklist-local truthfulness repair only, rather than reopening settled driver-local helper, manifest, or survey work.
