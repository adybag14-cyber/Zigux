...
- the focused `zigux/tests/phase10_virtio_input_queue_callback_preflight.zig`, `zigux/tests/phase10_virtio_input_registration_preflight.zig`, `zigux/tests/phase10_virtio_input_teardown_observation.zig`, and `zigux/tests/phase10_virtio_input_status_drain.zig` replays

This lane consumes shared core, ring, and MMIO prerequisites, but it does not own wrapper-layer growth in those packets. Its next bounded work stays inside input registration truthfulness, ownership notes, wrapper-facing verify replays, focused queue-callback-preflight, registration-preflight, teardown-observation, or status-drain replays, or similarly narrow survey-backed repairs.
...
Keep this sequencing note parked as the shared driver-lane owner map while the remaining same-lane follow-through stays on the shared review surfaces rather than inside the driver-local packets. Fresh shared-surface readback shows the dedicated input queue-callback-preflight, registration-preflight, and teardown-observation replays now stay explicit in the shared owner map beside the existing input verify and status-drain reminders, and the shared closure note now stays explicit beside the shared closure manifest and tests-root review companion. The next same-lane follow-through has therefore narrowed to `Documentation/zigux/README.md`, which still compresses the direct `zigux/tests/phase10_virtio_core.zig`, `zigux/tests/phase10_virtio_driver_id.zig`, `zigux/tests/phase10_virtio_ring.zig`, and `zigux/tests/phase10_virtio_input.zig` anchors that the shared closure note, tests-root companion, and scripts-root summary already keep explicit. Once that docs-root shared-summary gap is closed, leave this sequencing note parked until a fresh one-file checker, manifest, survey, or shared-summary truthfulness gap appears, rather than reopening settled driver-local helper, manifest, or survey work.