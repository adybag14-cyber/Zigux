...
- ring lane `P10-L07` owns the manifest-backed ring survey packet in `zigux/tests/phase10_virtio_ring_manifest.json`, `Documentation/zigux/phase10-virtio-ring-survey.md`, `drivers/virtio/virtio_ring.zig`, `zigux/tests/phase10_virtio_ring.zig`, `zigux/tests/phase10_virtio_ring_reset_reuse.zig`, `zigux/tests/phase10_virtio_ring_survey.zig`, `drivers/virtio/virtio_ring_verify.zig`, and `scripts/zigux/check-phase10-ring-packet.py`, and keeps the remaining same-lane follow-through bounded to queue-local reviewability work such as the manifest's still-`ready_next` `phase10-notification-data-summary-helper` instead of drifting back into missing-file wording or widening into transport-backed lifecycle claims
...
- ring packet review owns the ring manifest, survey, direct helper, drained-reset reuse replay, verifier, and checker surfaces in `zigux/tests/phase10_virtio_ring_manifest.json`, `Documentation/zigux/phase10-virtio-ring-survey.md`, `drivers/virtio/virtio_ring.zig`, `zigux/tests/phase10_virtio_ring.zig`, `zigux/tests/phase10_virtio_ring_reset_reuse.zig`, `zigux/tests/phase10_virtio_ring_survey.zig`, `drivers/virtio/virtio_ring_verify.zig`, and `scripts/zigux/check-phase10-ring-packet.py`, and it keeps the remaining same-lane truthfulness anchored to the still-`ready_next` `phase10-notification-data-summary-helper` instead of reintroducing repo-reality gap wording for already-landed ring paths
