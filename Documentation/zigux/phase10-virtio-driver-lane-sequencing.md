...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. Fresh shared-surface readback now shows the docs-root, scripts-root, closure-note, lane-sequencing, companion, tests-root, and reviewer-facing checklist summaries already keep the landed direct `drivers/virtio/virtio.zig`, `drivers/virtio/virtio_driver_id.zig`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_mmio.zig`, the focused `zigux/tests/phase10_virtio_input_probe_preflight.zig` replay, and the adjacent queue-callback-preflight, registration-preflight, teardown-observation, and status-drain reminders explicit.

Leave the Phase 10 lane parked unless fresh inspection finds another equally small same-lane manifest, survey, checker, or shared-summary truthfulness gap inside the landed virtio core, ring, input, and MMIO packet. If it reopens soon, prefer the next one-file shared review-surface or validation-surface sync before widening helper, MMIO transport, IRQ, reset, queue-discovery, or lifecycle behavior.
