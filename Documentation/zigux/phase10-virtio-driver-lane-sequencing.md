...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. Fresh shared-surface readback shows `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, and `zigux/tests/README.md` already keep the full verifier packet plus `zigux/tests/phase10_closure_manifest.json` explicit on live `master`, including the explicit `drivers/virtio/virtio_verify.zig` and `drivers/virtio/virtio_mmio_verify.zig` anchors in the checklist-facing prompt. The next same-lane follow-through should therefore leave this shared note parked until a fresh one-file checker, manifest, survey, or shared-summary truthfulness gap appears, rather than reopening settled driver-local helper, manifest, or survey work.