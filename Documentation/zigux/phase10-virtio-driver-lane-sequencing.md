...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. Fresh shared-surface readback shows `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, and `zigux/tests/README.md` already keep the four verifier replays plus `zigux/tests/phase10_closure_manifest.json` explicit on live `master`, but `scripts/zigux/README.md` still undercounts that same shared Phase 10 packet by omitting the explicit `drivers/virtio/virtio_ring_verify.zig`, `zigux/tests/phase10_virtio_ring.zig`, `drivers/virtio/virtio_input_verify.zig`, and `zigux/tests/phase10_virtio_input.zig` paths beside its existing closure-manifest-backed checker and build route. The next same-lane follow-through should therefore stay inside that one scripts-root truthfulness repair only, rather than reopening settled driver-local helper, manifest, or survey work.
