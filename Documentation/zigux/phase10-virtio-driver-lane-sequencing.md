...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. Fresh shared-surface readback now shows both the docs-root and scripts-root Phase 10 summaries already keep the landed direct `drivers/virtio/virtio.zig`, `drivers/virtio/virtio_driver_id.zig`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_mmio.zig`, and `zigux/tests/phase10_virtio_input_probe_preflight.zig` replay explicit beside the focused verify, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain reminders.

The next same-lane follow-through is now one fail-closed validator repair in `scripts/zigux/check-phase10-input-packet.py`: keep that dedicated Phase 10 input packet guard explicit about the already-landed `zigux/tests/phase10_virtio_input_probe_preflight.zig` replay across the shared docs-root, scripts-root, closure-note, lane-sequencing, and companion surfaces instead of only the narrower queue-callback-preflight, registration-preflight, teardown-observation, and status-drain reminders.

After that checker repair lands, reread `zigux/tests/README.md` for the next shared-summary truthfulness pass so the same focused probe-preflight replay stays visible there too, rather than reopening settled driver-local helper, manifest, or survey work.
