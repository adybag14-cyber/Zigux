...
Keep this sequencing note parked unless future repo drift blurs the ownership boundary between the Phase 10 core, ring, input, and MMIO driver packets again. The earlier docs-root follow-through is already complete on live `master`, and the broader Phase 10 tests-root companion plus checklist-facing reminders already keep the four verifier replays explicit. The current smallest shared-surface follow-through now sits in the shorter Phase 10 flow summary inside `scripts/zigux/README.md`: keep `drivers/virtio/virtio_ring_verify.zig` and `drivers/virtio/virtio_input_verify.zig` explicit beside `drivers/virtio/virtio_verify.zig`, `drivers/virtio/virtio_mmio_verify.zig`, `zigux/tests/phase10_closure_manifest.json`, and the shared checker-backed build-and-make packet there instead of leaving the shipped ring and input verifier replays implicit in that scripts-root reminder.
