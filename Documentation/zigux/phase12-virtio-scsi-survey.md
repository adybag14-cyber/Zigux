...
- the live repo also now ships a bounded `drivers/scsi/virtio_scsi.zig` starter, dedicated `zigux/tests/phase12_virtio_scsi.zig` coverage, and `Documentation/zigux/phase12-virtio-scsi-slice.md`.
That starter is intentionally narrow: it models control, event, request, and `request_poll` queue-family planning in memory, preserves poll-queue clamping, keeps stable global virtqueue indexes, records a bounded `virtscsi_probe()` config snapshot through `num_queues`, `seg_max`, `cmd_per_lun`, `max_target`, `max_lun`, `max_sectors`, and the derived control or event versus request virtqueue layout, records a bounded queue-depth clamp summary plus its recovery-side restore summary, records a bounded blk-mq I/O queue map summary plus a recovery-side restore summary for default versus poll queue offsets, records a lab-only freeze or restore summary that blocks planning while transport is frozen and clears the old queue snapshot after restore, now also proves that a repeated transport-reset cycle reuses only the newly replanned queue topology while incrementing the bounded recovery generation, and now also records a bounded restore queue rebind summary, a bounded recovery event-rearm summary, a restore-time event-buffer ownership summary, and a rollback summary so the recovery packet keeps queue-family ranges, queue-depth clamp state, blk-mq map offsets, device-ready ordering, event rearm, event-buffer reservation, and restore-before-replan discipline explicit without claiming live DMA-backed submission.
...
- reversible-delivery evidence: the shipped starter keeps the lab-only freeze or restore boundary, repeated transport-reset recovery generation, queue-depth clamp summary, recovery queue-depth summary, blk-mq I/O queue map summary, recovery I/O queue map summary, restore queue rebind summary, recovery event-rearm summary, restore-time event-buffer ownership summary, and rollback summary explicit without claiming `scsi_scan_host()` replay, live completion handling, or runtime DMA parity.
...
- the landed `phase12-virtio-scsi-recovery-queue-depth-summary-starter`
- the landed `phase12-virtio-scsi-restore-queue-rebind-summary-starter`
- the landed `phase12-virtio-scsi-recovery-io-queue-map-summary-starter`
...
This keeps the lane explicit without overstating progress: Zigux now has a bounded virtio_scsi queue-layout, probe-config snapshot, queue-depth clamp summary, blk-mq I/O queue map summary, repeated transport-reset recovery-generation gate, restore queue rebind summary, recovery queue-depth and I/O-map restore summaries, recovery event-rearm summary, restore-time event-buffer ownership summary, and rollback summary inside the broader recovery packet, but it still does not claim command submission, event completion, TMF flow, SCSI-host registration, PM callback wiring, or DMA-backed virtqueue ownership.
...
- live blk-mq queue mapping or polling support beyond bounded queue-count and offset summaries
...
Keep this lane parked unless fresh repo inspection finds directly coupled drift in the landed queue-layout, probe-config snapshot, or recovery packet. Until a driver-local follow-up is approved, keep this survey aligned with the shared smoke-plus-build replay packet instead of letting the focused preflight shard drift back into build-file-only, Makefile-only, or fallback-catalog-only knowledge.
