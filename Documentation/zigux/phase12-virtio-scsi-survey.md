...
- the same bounded starter now also records one restore-sequencing summary that keeps `virtscsi_restore()` calling back through `virtscsi_init()`, `virtio_device_ready()`, and `virtscsi_kick_event_all()` reviewable without pretending to re-run `scsi_scan_host()` or port live `Scsi_Host` registration.
