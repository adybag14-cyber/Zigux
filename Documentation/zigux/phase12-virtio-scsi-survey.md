...
- reversible-delivery evidence: the shipped starter keeps the lab-only freeze or restore boundary, repeated transport-reset recovery generation, queue-window ownership summary, restore queue rebind summary, request-queue restart summary, recovery event-rearm summary, restore-time event-buffer ownership summary, and rollback summary explicit without claiming `scsi_scan_host()` replay, live completion handling, or runtime DMA parity.
...
Keep this lane parked unless fresh repo inspection finds directly coupled drift in the landed queue-layout, queue-window summary, probe-config snapshot, or recovery packet.
