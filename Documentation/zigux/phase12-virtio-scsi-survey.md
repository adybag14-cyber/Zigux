...
* the shared `zigux/tests/phase12_build.zig` route still covers only the `virtio_net` queue-resume, transmit-recycle, post-reset replay, and throughput-parity tests as support-bundle evidence rather than replaying a `virtio_scsi` lane-local packet
