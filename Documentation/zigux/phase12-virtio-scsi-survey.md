...
This keeps the lane explicit without overstating progress: Zigux now has a bounded virtio_scsi queue-layout, probe-config snapshot, restore queue rebind summary, and rollback summary inside the broader recovery packet, but it still does not claim command submission, event completion, TMF flow, SCSI-host registration, PM callback wiring, or DMA-backed virtqueue ownership.
