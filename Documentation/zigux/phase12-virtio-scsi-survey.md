...
- reversible-delivery evidence: the shipped starter keeps the lab-only freeze or restore boundary, repeated transport-reset recovery generation, queue-window ownership summary, restore queue rebind summary, recovery event-rearm summary, restore-time event-buffer ownership summary, and rollback summary explicit without claiming `scsi_scan_host()` replay, live completion handling, or runtime DMA parity.
...
This keeps the lane explicit without overstating progress: Zigux now has a bounded virtio_scsi queue-layout, queue-window summary, probe-config snapshot, repeated transport-reset recovery-generation gate, restore queue rebind summary, recovery event-rearm summary, restore-time event-buffer ownership summary, and rollback summary inside the broader recovery packet, but it still does not claim command submission, event completion, TMF flow, SCSI-host registration, PM callback wiring, or DMA-backed virtqueue ownership.
