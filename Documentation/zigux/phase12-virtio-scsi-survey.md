...
This keeps the lane explicit without overstating progress: Zigux now has a bounded virtio_scsi queue-layout, probe-config snapshot, repeated transport-reset recovery-generation gate, restore queue rebind summary, restore-time event-buffer ownership summary, and rollback summary inside the broader recovery packet, but it still does not claim command submission, event completion, TMF flow, SCSI-host registration, PM callback wiring, or DMA-backed virtqueue ownership.
...
Keep this lane parked unless fresh repo inspection finds directly coupled drift in the landed queue-layout, probe-config snapshot, or recovery packet.
