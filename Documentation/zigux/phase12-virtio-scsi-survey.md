...
- the live repo also now ships a bounded `drivers/scsi/virtio_scsi.zig` starter, dedicated `zigux/tests/phase12_virtio_scsi.zig` coverage, and `Documentation/zigux/phase12-virtio-scsi-slice.md`. That starter is intentionally narrow: it models control, event, request, and request_poll queue-family planning in memory, preserves poll-queue clamping, keeps stable global virtqueue indexes, records a lab-only freeze or restore summary that blocks planning while transport is frozen and clears the old queue snapshot after restore, captures one probe snapshot of `virtscsi_probe()` config fields such as `num_queues`, `seg_max`, `cmd_per_lun`, `max_target`, `max_lun`, and `max_sectors` alongside the derived control, event, default-request, and poll-request virtqueue layout, and now also lands one tiny host-limit summary helper that clamps `cmd_per_lun` against a synthetic `can_queue` while recording `max_target`, `max_lun`, `max_sectors`, and `nr_hw_queues` before any `scsi_host_alloc()`, `scsi_add_host()`, or `scsi_scan_host()` work is attempted.
- the lane now also records a commit-pinned raw GitHub fallback catalog in `Documentation/zigux/phase12-virtio-scsi-raw-github-fallback-catalog.md`, verified against inspected `master` head `658cf1efef9df701a00ee53d28a6ca3f49cbeb7d`, so future scheduled runs can recover the exact anchor, starter, survey, build, validator, and note files even if connector-backed reads are flaky.
- the current degraded-mode replay on that same inspected head stays validator-first: `python3 scripts/zigux/validate-phase12.py` passes with `PHASE12_MANIFEST_COUNT=4`, `PHASE12_SHARED_BUILD_TEST_COUNT=8`, `PHASE12_SHARED_BUILD_DEPEND_STEP_COUNT=8`, `PHASE12_EXPECTED_SUMMARY_LINE=Build Summary: 17/17 steps succeeded; 34/34 tests passed`, `PHASE12_STARTER_STATUS_COUNT=44`, `PHASE12_BLOCKED_DMA_STATUS_COUNT=3`, and `PHASE12_BLOCKED_OBJECT_STATUS_COUNT=2`, and the focused `zig test zigux/tests/phase12_virtio_scsi_survey.zig` replay still passes `1/1` tests.
...
This keeps the lane explicit without overstating progress: Zigux now has a bounded virtio_scsi queue-layout, recovery, probe snapshot, and host-limit summary starters plus an exact raw fallback evidence packet, but it still does not claim command submission, event completion, TMF flow, SCSI-host registration, PM callback wiring, or DMA-backed virtqueue ownership.
...
Keep this lane on survey or validation work until the roadmap-approved queue ownership, SCSI-host lifecycle, and DMA-backed transport substrate exists for a truthful follow-up beyond the current queue-layout, recovery, probe snapshot, and host-limit summary starters.
