...
- the pinned fallback packet also preserves one historical degraded-workflow replay from inspected head `7d653d8c5e57207763c07c1b1d020b514738c7f3`: the shared `python3 scripts/zigux/validate-phase12.py` replay was red on unrelated Phase 12 packet drift while the focused `zig test zigux/tests/phase12_virtio_scsi_survey.zig` replay still passed `1/1` tests. Treat that archived validator note as historical fallback evidence for the pinned packet only, not as the live validator state on newer Phase 12 heads.
