...
- next future target: stay in maintenance mode unless a named reopen trigger or the deep-core blocker posture changes; if one same-lane reviewer-facing truthfulness repair is still needed before then, start with `Documentation/zigux/review-checklist.md`, because the compact shared Phase 15 maintenance packet there still undercounts `Documentation/zigux/phase15-governance-lane-sequencing.md` together with `zigux/tests/phase15_handoff_next_steps_manifest.json` and `zigux/tests/phase15_readiness_gate_manifest.json`; route any broader docs-root, scripts-root, or tests-root summary repairs through `Documentation/zigux/phase15-governance-lane-sequencing.md` and `Documentation/zigux/phase15-handoff-next-steps-survey.md`, and reopen this review-process packet only if that drift changes the truthfulness of the required review fields, decision buckets, reopen-trigger catalog, or no-approval posture recorded here
...
- open `phase15-review-checklist-maintenance-packet-undercount`

That remaining reviewer-facing checklist undercount is now the smaller same-lane maintenance repair: `Documentation/zigux/review-checklist.md` already names the review-process, readiness, handoff, validator, and replay routes on current `master`, but it still omits `Documentation/zigux/phase15-governance-lane-sequencing.md` together with `zigux/tests/phase15_handoff_next_steps_manifest.json` and `zigux/tests/phase15_readiness_gate_manifest.json` even though the current parked governance packet and its handoff notes already treat those companion surfaces as shipped reviewer-facing maintenance evidence. The older compact docs-root omission remains a broader shared-summaries follow-up only; this review-process packet should reopen only if that broader drift changes the process fields or approval posture it owns.

This keeps the slice narrow. Zigux gains a reviewable Architecture Council process description that now points at the landed parity scorecard, aligns the required packet with the scorecard's decision-record fields, the freeze map's required approver-set and rollback-threshold expectations, and the indefinite-C policy linkage, names the retained stay-in-C closeout state, standardizes the reopen-trigger catalog, states the current no-approval posture plainly, records the dated-readback provenance refresh that avoids a quickly stale exact-head count, keeps the validator-first and dedicated test replay routes explicit in the note itself, and now points the one remaining same-lane reviewer-facing maintenance undercount at `Documentation/zigux/review-checklist.md` while leaving the broader docs-root drift parked in the shared-summary companion lane so this review-process packet only reopens if that wider reminder drift changes its own required fields, decision buckets, reopen-trigger catalog, or no-approval posture.
...
Unless a named reopen trigger or the deep-core blocker posture changes first, treat the smaller reviewer-facing maintenance undercount in `Documentation/zigux/review-checklist.md` as the first same-lane follow-up, and route any remaining broader docs-root, scripts-root, or tests-root reminder drift through `Documentation/zigux/phase15-governance-lane-sequencing.md` and `Documentation/zigux/phase15-handoff-next-steps-survey.md`; reopen this review-process packet only if that drift changes the truthfulness of the required review fields, decision buckets, reopen-trigger catalog, or no-approval posture recorded here.
