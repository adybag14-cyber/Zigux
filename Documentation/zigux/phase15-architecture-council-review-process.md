...
- `scripts/zigux/check-phase15-tests-readme-alignment.py` is the focused guard for the tests-root Phase 15 governance reminder packet that still needs to land in `zigux/tests/README.md`; until that reminder text exists, keep the tests-root surface tracked through `Documentation/zigux/phase15-shared-summary-gap.md` instead of treating `zigux/tests/README.md` as fully aligned current evidence, and do not imply that the broader validator-first or build-route companions are already landed
...
- land the missing shared tests-root Phase 15 governance reminder in `zigux/tests/README.md`, then keep it aligned with `scripts/zigux/check-phase15-tests-readme-alignment.py`, this note, and `Documentation/zigux/phase15-shared-summary-gap.md` before widening into broader Phase 15 prose
...
Keep this lane parked unless fresh repo inspection shows a new same-packet field drift in the Architecture Council request inventory, the stay-in-C closeout rule, the reopen-evidence rule, the focused review-process replay, the focused build-file replay, the missing tests-root Phase 15 governance reminder, or the current shared-summary gap tracking for the broader validator-first surfaces.