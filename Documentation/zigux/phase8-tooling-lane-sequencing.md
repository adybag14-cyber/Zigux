...
The first wording-only reopen target here should now be the docs-root and scripts-root summary pair first: current `master` already keeps the tests-root and survey-local parked shard reminders explicit, but `Documentation/zigux/README.md` and `scripts/zigux/README.md` still describe the parked libbpf packet without naming this anti-overlap sequencing note, and `scripts/zigux/validate-phase8.py` still does not require those shared owner-map reminders to stay present.
...
The next smallest wording-only reopen is now the docs-root and scripts-root summaries plus the paired shared-validator reminder: current `Documentation/zigux/README.md` already keeps the parked help-plus-kallsyms shard routes, the file-path bridge route, the libbpf segment-survey route, and the perf-buffer-poll route explicit, and current `scripts/zigux/README.md` already keeps the shared validator-first route plus the focused Phase 8 shard inventory explicit, but neither summary names `Documentation/zigux/phase8-tooling-lane-sequencing.md` even though this anti-overlap note is the shared owner map for keeping those parked surfaces split cleanly, and `scripts/zigux/validate-phase8.py` still does not require those shared owner-map reminders to remain visible beside the parked libbpf packet.

If another wording-only drift appears first after that docs-root-and-scripts-root-plus-validator reminder packet is repaired, start by rereading `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, and `Documentation/zigux/phase8-libbpf-segment-survey.md` together so the parked libbpf shard markers, the survey-local `zigux/tests/phase8_cpu_mask_only_build.zig` shard, the focused help-plus-kallsyms shard reminders, the pin-path and type-name helper inventory, and the focused make routes stay aligned before reopening any helper-local Phase 8 follow-up.
