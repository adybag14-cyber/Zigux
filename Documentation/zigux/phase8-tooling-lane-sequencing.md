...
The reviewer-facing packet is also aligned on current `master`: `Documentation/zigux/review-checklist.md` already keeps `scripts/zigux/check-phase8-perf-buffer-poll-gate.py` explicit beside `scripts/zigux/validate-phase8.py` and the focused `make -C zigux phase8-perf-buffer-poll-test` route, so this sequencing note should not steer the lane toward that already-landed checklist reminder either.
