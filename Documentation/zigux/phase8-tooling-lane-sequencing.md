...
- current 2026-05-15 authenticated contents readback reopens the symbol-lane owner-map question: `zigux/tests/phase8_kallsyms.zig`, `zigux/tests/phase8_kallsyms_only_build.zig`, and `zigux/tests/phase8_help_kallsyms_only_build.zig` still read cleanly, but `tools/lib/symbol/kallsyms.zig` still returns `404`, so the mixed help-plus-kallsyms build shard must stay a shared validation route instead of turning help-local and symbol-local follow-through into one owner
...
- current 2026-05-16 reread now splits the remaining scripts-root undercount by owner: `scripts/zigux/README.md` still omits `Documentation/zigux/phase8-kallsyms-slice.md`, `zigux/tests/phase8_kallsyms.zig`, and `zigux/tests/phase8_kallsyms_only_build.zig` on the symbol side, and it separately still omits `Documentation/zigux/phase8-perf-buffer-poll-slice.md`, `zigux/tests/phase8_perf_buffer_poll.zig`, and `zigux/tests/phase8_perf_buffer_poll_only_build.zig` on the libbpf side even though current `master` keeps both dedicated note-and-replay packets readable beside the shared make routes and dedicated Phase 8 gate checkers; that split is sequencing-only, because `scripts/zigux/README.md` remains a shared-wording surface rather than a symbol-lane or libbpf-lane owner
- current 2026-05-12 exact readback also shows `scripts/zigux/README.md` no longer has an active filename-case correction to reopen first, so future shared wording work should move past the already-landed scripts-root summary instead of replaying either the older casing fix or the older missing-summary cue
- `Documentation/zigux/review-checklist.md` is no longer the first shared wording reopen cue: current `master` already refreshes that shared Phase 8 libbpf review question with `scripts/zigux/check-phase8-perf-buffer-poll-gate.py`, `scripts/zigux/check-phase8-libbpf-segment-gate.py`, and `scripts/zigux/check-phase8-libbpf-shard-routes.py`, so the next shared-wording reread should start with `zigux/tests/README.md`, `scripts/zigux/README.md`, or `zigux/Makefile` instead of reopening the checklist
...
- shared wording owner map: keep the scripts-root perf-buffer-poll reminder repair under `P8-Y07` only, and leave the remaining kallsyms slice-and-replay omission under the separate symbol lane `P8-Y03` until the helper path is readable again
- the dedicated libbpf survey note and the broader shared reminder packet no longer point back to the same first reread surface because the checklist repair has already landed on current `master`; keep any future shared-wording follow-through limited to the next remaining scripts-root, tests-root, or make-surface drift instead of reopening the checklist
...
7. When the mixed `phase8-help-kallsyms` smoke route reopens, treat it as shared validation only: help-local output or command-source drift stays in the help lane, while parser, truncation, or callback-wrapper drift stays in the symbol lane until `tools/lib/symbol/kallsyms.zig` is readable again.

## Next bounded step
Exact 2026-05-15 reread keeps the earlier docs-root reopen cue closed instead of reopening it: current readable Phase 8 libbpf evidence still includes `tools/lib/bpf/zigux_segments/type_names.zig` and `Documentation/zigux/phase8-file-path-handle-bridge-slice.md`, and `Documentation/zigux/README.md` now names the live file-path bridge note in the broad Phase 8 docs summary. The earlier symbol-lane visible cue is reopened too: `zigux/tests/phase8_kallsyms.zig`, `zigux/tests/phase8_kallsyms_only_build.zig`, and `zigux/tests/phase8_help_kallsyms_only_build.zig` still read cleanly, but current authenticated contents readback for `tools/lib/symbol/kallsyms.zig` still returns `404`, so shared sequencing should treat the mixed help-and-kallsyms build shard as validation overlap only and keep parser follow-through in the dedicated `kallsyms` lane until the helper path is readable again. The older combined command-lane assumption is closed too: `Documentation/zigux/phase8-help-slice.md` and `Documentation/zigux/phase8-exec-cmd-slice.md` already describe separate parked packets, and current authenticated contents reads now carry the direct exec-cmd shard cleanly through `Documentation/zigux/phase8-exec-cmd-slice.md`, `tools/lib/subcmd/exec-cmd.zig`, `zigux/tests/phase8_exec_cmd.zig`, and `zigux/tests/phase8_exec_cmd_only_build.zig`, so future sequencing follow-through should keep the exec-cmd lane focused on direct command-packet truthfulness and leave the remaining mixed-source caution to the broader shared `zigux/tests/phase8_build.zig` route plus the neighboring file-path bridge surfaces. The older tests-root reopen cue is closed too: `zigux/tests/README.md` now names `scripts/zigux/check-phase8-perf-buffer-poll-gate.py`, `scripts/zigux/check-phase8-libbpf-segment-gate.py`, and `scripts/zigux/check-phase8-libbpf-shard-routes.py` together with the live libbpf shard routes, so the shared wording lane no longer needs a tests-root repair for that checker pair. The older scripts-root missing-summary cue is closed too: `scripts/zigux/README.md` now carries a dedicated `Phase 8 flow` section that keeps the validator-first packet, the exec-cmd checker, the help-and-kallsyms plus libbpf shard checkers, the bridge survey notes, the focused exec-cmd and file-path bridge replays, and the shared `phase8-validate` route visible in one scripts-root packet. Current 2026-05-16 reread narrows the remaining shared-wording reopen cue to that scripts-root packet itself, but now with explicit owner separation: `scripts/zigux/README.md` still omits `Documentation/zigux/phase8-kallsyms-slice.md`, `zigux/tests/phase8_kallsyms.zig`, and `zigux/tests/phase8_kallsyms_only_build.zig` on the symbol side, while the same scripts-root packet separately still omits `Documentation/zigux/phase8-perf-buffer-poll-slice.md`, `zigux/tests/phase8_perf_buffer_poll.zig`, and `zigux/tests/phase8_perf_buffer_poll_only_build.zig` on the libbpf side even though those dedicated note-and-replay surfaces are already landed on current `master` beside `make -C zigux phase8-kallsyms-test`, `make -C zigux phase8-perf-buffer-poll-test`, and the dedicated Phase 8 gate checkers. Keep the shared wording lane reopened for one reminder surface at a time: take the scripts-root perf-buffer-poll reminder repair through shared wording lane `P8-Y07` while using the parked libbpf packet only as readback context, and leave the remaining kallsyms slice-and-replay omission to the separate symbol lane `P8-Y03`. If the shared wording lane reopens after that fix lands, start with a fresh reread of `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml` together before widening to any validator, sequencing-note, or helper-packet follow-through.
