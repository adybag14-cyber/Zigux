...
- current authenticated 2026-05-14 contents readback remains inconsistent around the direct exec-cmd shard: `Documentation/zigux/phase8-help-slice.md`, `Documentation/zigux/phase8-tooling-lane-sequencing.md`, `tools/lib/subcmd/exec-cmd.zig`, and `zigux/tests/phase8_help.zig` still read cleanly, while the same contents route still returned `404` for `zigux/tests/phase8_exec_cmd.zig` and `zigux/tests/phase8_exec_cmd_only_build.zig` during this readback even though the public tree still lists them on `master`
...
- authenticated contents readback for the direct exec-cmd shard remains intermittent from this environment: `tools/lib/subcmd/exec-cmd.zig` now reads cleanly again, but the focused replay files still returned `404` during the current readback, so treat those test-file misses as route instability until both the public tree and readable blob evidence drop the same files
...
- current 2026-05-15 reread now narrows the remaining shared-wording reopen cue to one scripts-root undercount: `scripts/zigux/README.md` still omits `Documentation/zigux/phase8-kallsyms-slice.md`, `Documentation/zigux/phase8-perf-buffer-poll-slice.md`, `zigux/tests/phase8_kallsyms.zig`, `zigux/tests/phase8_kallsyms_only_build.zig`, `zigux/tests/phase8_perf_buffer_poll.zig`, and `zigux/tests/phase8_perf_buffer_poll_only_build.zig` even though current `master` keeps those dedicated note-and-replay surfaces readable beside the shared make routes and dedicated Phase 8 gate checkers
