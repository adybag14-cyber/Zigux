...
That older immediate next step is now complete only in part: `zigux/tests/README.md` already keeps `zigux/tests/phase8_bpf_type_names.zig`, `zigux/tests/phase8_file_path_handle_bridge.zig`, `zigux/tests/phase8_file_path_handle_bridge_only_build.zig`, and the focused `make -C zigux phase8-file-path-handle-bridge-test`, `make -C zigux phase8-libbpf-segments-test`, and `make -C zigux phase8-perf-buffer-poll-test` shard routes explicit, so this sequencing note should keep the remaining follow-up narrowed to the missing cpu-mask reminder instead of widening back into broader libbpf wording work.
