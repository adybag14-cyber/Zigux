...
- `Documentation/zigux/review-checklist.md` is again the first shared wording reopen cue: the shared Phase 8 libbpf review question still names `scripts/zigux/check-phase8-perf-buffer-poll-gate.py`, but current `master` already ships the broader checker packet through `scripts/zigux/check-phase8-libbpf-segment-gate.py` and `scripts/zigux/check-phase8-libbpf-shard-routes.py` too
...
- the dedicated libbpf survey note and the broader shared reminder packet no longer fully agree on the parked shared-wording posture because the checklist still undercounts the live libbpf checker packet; keep follow-up limited to that one checklist repair before widening anything else
