...
- current 2026-05-13 public raw readback resolves the symbol-lane reminder question enough for truthful shared sequencing: `tools/lib/symbol/kallsyms.zig`, `zigux/tests/phase8_kallsyms.zig`, and `zigux/tests/phase8_kallsyms_only_build.zig` are directly readable again through raw default-branch paths even while the authenticated contents route remains intermittent for the same files, so shared reminder surfaces should no longer describe the direct `kallsyms` helper-and-test packet as currently unreadable current-`master` evidence
...
- exact 2026-05-13 public raw readback now serves `tools/lib/symbol/kallsyms.zig`, `zigux/tests/phase8_kallsyms.zig`, and `zigux/tests/phase8_kallsyms_only_build.zig` again, while the authenticated contents route for the same files still remains intermittent from this environment
- `Documentation/zigux/phase8-kallsyms-slice.md` is directly readable again too, so shared reminder surfaces should treat the direct symbol packet as visible current-`master` evidence and leave any remaining helper-vs-wrapper CRLF truthfulness follow-through inside the dedicated `kallsyms` lane

Keep shared wording out of this lane unless a concrete symbol-lane packet drift appears on current `master`. If the symbol lane reopens next, compare the readable helper, focused test, and slice note from one consistent source before retelling CRLF behavior.
