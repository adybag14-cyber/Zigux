...
The remaining smallest wording-only reopen is therefore just the shared-validator reminder packet: current `scripts/zigux/validate-phase8.py` still does not require `Documentation/zigux/README.md` or `scripts/zigux/README.md` to keep `Documentation/zigux/phase8-tooling-lane-sequencing.md` visible beside the parked libbpf packet, even though the live summary pair now treats that note as the shared owner map.

If another wording-only drift appears first after that validator-only reminder packet is repaired, start by rereading `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, and `Documentation/zigux/phase8-libbpf-segment-survey.md` together so the parked libbpf shard markers, the survey-local `zigux/tests/phase8_cpu_mask_only_build.zig` shard, the focused help-plus-kallsyms shard reminders, the pin-path and type-name helper inventory, and the focused make routes stay aligned before reopening any helper-local Phase 8 follow-up.
