...
That older immediate next step is now complete: `zigux/tests/README.md` already keeps `zigux/tests/phase8_bpf_type_names.zig`, `zigux/tests/phase8_cpu_mask_only_build.zig`, `zigux/tests/phase8_file_path_handle_bridge.zig`, `zigux/tests/phase8_file_path_handle_bridge_only_build.zig`, and the focused `make -C zigux phase8-cpu-mask-test`, `make -C zigux phase8-file-path-handle-bridge-test`, `make -C zigux phase8-libbpf-segments-test`, and `make -C zigux phase8-perf-buffer-poll-test` shard routes explicit, so this sequencing note should stop steering the shared wording lane back toward that already-closed tests-root cpu-mask reminder and instead wait for another smaller shared-packet drift before reopening.

The smaller shared tests-root cpu-mask reminder is now closed on current `master`: `zigux/tests/README.md` directly names both `zigux/tests/phase8_cpu_mask_only_build.zig` and `make -C zigux phase8-cpu-mask-test` beside the parked libbpf shard routes, so the next wording-only reopen should stay with another shared reminder, checker, or sequencing-note drift instead of revisiting that already-landed cpu-mask reminder.
