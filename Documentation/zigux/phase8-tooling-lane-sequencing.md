...
The smaller shared tests-root cpu-mask reminder is not yet closed on current `master`: `zigux/tests/README.md` still names `zigux/tests/phase8_cpu_mask.zig` inside the shared packet, but it does not yet directly call out `zigux/tests/phase8_cpu_mask_only_build.zig` or `make -C zigux phase8-cpu-mask-test` beside the parked libbpf shard routes. If the wording lane reopens again, prefer that one-file tests-root reminder sync before widening helper-local follow-up.
