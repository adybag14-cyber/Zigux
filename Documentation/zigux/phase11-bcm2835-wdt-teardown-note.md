...
The current directly readable bcm2835 driver starter plus the new dedicated replay keep one small teardown boundary explicit:
...
- Treat this note as a bounded ownership and teardown reminder for the current directly readable driver starter plus one dedicated replay, not as proof of live platform remove or shared callback execution.
- Keep this note aligned with `drivers/watchdog/bcm2835_wdt.zig`, `zigux/tests/phase11_bcm2835_wdt.zig`, `Documentation/zigux/phase11-bcm2835-wdt-survey.md`, and `Documentation/zigux/phase11-bcm2835-wdt-validation-matrix.md` when the directly readable stop, restart, or poweroff helpers change.
- Do not describe `drivers/watchdog/bcm2835_wdt_verify.zig`, `zigux/tests/phase11_bcm2835_wdt_survey.zig`, manifest-backed packet closure, or remove-time cleanup helpers as live current-master evidence until those paths are directly readable again in this lane.
- If a later Phase 11 lane widens the bcm2835 packet into a manifest, survey gate, verify helper, or broader platform-registration work, refresh this note together with the bcm2835 survey and matrix so the teardown story stays truthful.

## Next Blocked Step

The next honest bcm2835 follow-through remains one manifest-backed, survey-gated, or verify-backed extension that matches the current driver starter plus dedicated replay. Until then, keep this teardown note bounded to the driver-local stop, restart, and poweroff model already visible on `master`.