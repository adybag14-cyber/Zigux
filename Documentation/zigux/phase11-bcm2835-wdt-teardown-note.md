...
- when `registrationOutcomeSummary()`, `poweroffSummary()`, `removeSummary()`, or `removeAfterRegistrationSummary()` change, update this note together with `Documentation/zigux/phase11-bcm2835-wdt-slice.md` and `Documentation/zigux/phase11-bcm2835-wdt-validation-matrix.md` so the lane keeps one honest teardown story
