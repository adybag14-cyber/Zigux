...
The directly readable Phase 4 packet therefore stays reviewable in a narrower aligned state:
  ...
  * `scripts/zigux/check-phase4-artifact-diff-determinism.py` now exact-requires the broader `Documentation/zigux/artifact-diff.md` note to keep the currently published `ARTIFACT_DIFF_SELF_TEST_CASE_COUNT=19` helper packet, the `23`-base / `28`-total contract packet, and the companion review anchors whenever that file is present in the checked tree, so the older broader-note packet now fails closed until a later same-family note refresh lands.
...
The remaining broader reminder gap stays narrower and external to that aligned packet: `Documentation/zigux/artifact-diff.md` is still only public-raw-readable in this runtime and still describes the older `ARTIFACT_DIFF_SELF_TEST_CASE_COUNT=19` `sha256_*` exact-check packet. The determinism guard now fails closed if that broader note disappears or drifts away from the currently published `19`-case helper and `23`-base / `28`-total contract packet while the file is present in the checked tree.

That keeps this lane truthfulness-focused rather than speculative: the directly readable packet is aligned, while the broader owner-and-rollback note still needs a later reread and refresh when an exact write path is appropriate.
## Next Safe Step
