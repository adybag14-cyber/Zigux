...
The remaining broader reminder gap stays narrower and external to that aligned packet: `Documentation/zigux/artifact-diff.md` is still only public-raw-readable in this runtime and still describes the older `ARTIFACT_DIFF_SELF_TEST_CASE_COUNT=19` `sha256_*` exact-check packet. The new determinism guard now at least fails closed if that broader note disappears or loses its core helper-and-contract anchors when the file is present in the checked tree.
