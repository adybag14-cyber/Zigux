...
- `zigux/Makefile` and `.github/workflows/zigux-bootstrap.yml` already expose the same validator-first replay surface through `make -C zigux phase4-validate`, and the workflow also carries named direct artifact-diff helper, contract, and determinism evidence steps through `python3 scripts/zigux/artifact_diff.py --self-test`, `python3 scripts/zigux/check-artifact-diff-contract.py --self-test`, `python3 scripts/zigux/check-artifact-diff-contract.py`, `python3 scripts/zigux/check-phase4-artifact-diff-determinism.py --self-test`, and `python3 scripts/zigux/check-phase4-artifact-diff-determinism.py`.
...
The live Phase 4 artifact-diff tooling gap is not a missing deterministic checker anymore. The current same-lane follow-through is a fail-closed reminder surface: this survey now records the live helper, contract, and determinism packet counts, the exact base-case and full-case contract catalogs, the direct workflow artifact-diff check set, and the bootstrap workflow helper, contract, determinism, and `phase4-validate` entrypoints so the shared `artifact_diff.py` packet stays visible as reviewable CI evidence rather than only as prose outside the checked catalog packet.
