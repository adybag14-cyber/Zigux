...
- scope: keep the current `gpio_wdt` starter honest about what is already validated, including the landed reboot-glue checkpoint, name the remaining registration, teardown, failure-mode, and hardware-backed validation gap, and avoid overclaiming platform registration or live GPIO behavior before those surfaces exist in Zigux
...
| reboot-glue checkpoint | `rebootGlueCheckpointSummary()` records the bounded `watchdog_stop_on_reboot()` ordering around `nowayout`, pre-registration startup, and the first `devm_watchdog_register_device()` request surface without claiming live reboot notifier registration | `zig test --dep gpio_wdt -Mroot=zigux/tests/phase11_gpio_wdt.zig -Mgpio_wdt=drivers/watchdog/gpio_wdt.zig` replays the reboot-glue checkpoint assertions in `zigux/tests/phase11_gpio_wdt.zig` through the shared watchdog packet | keep this bounded reboot-ordering evidence traceable while a later same-family lane decides whether broader teardown or hardware-backed validation work can grow without claiming live reboot integration | live `watchdog_stop_on_reboot()` execution, reboot notifier registration, platform-driver shutdown ordering, and hardware-backed reboot behavior |
...
- when a later teardown-parity, failure-mode, or hardware-validation handoff lands, update this matrix, the module-slice note, the survey note, the teardown note, and the survey manifest together so the lane keeps one truthful next step
