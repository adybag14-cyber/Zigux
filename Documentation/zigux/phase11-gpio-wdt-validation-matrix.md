...
- which areas must remain out of scope until a later driver-local handoff lands
...
| registration-facing handoff summary | `registrationHandoffSummary()` keeps startup state, line mode, timeout-init intent, stop policy, and reboot bookkeeping reviewable as metadata that would reach `devm_watchdog_register_device()` without claiming the registration call itself | `zig build test --build-file zigux/tests/phase11_build.zig --summary all` via the registration handoff assertions in `zigux/tests/phase11_gpio_wdt.zig` | add one tiny descriptor-facing registration handoff so GPIO acquisition versus watchdog registration ownership stays explicit before any live platform glue lands | real platform registration, `devm_gpiod_get()`, `devm_watchdog_register_device()`, and hardware-backed execution |
...
- if a later lane chooses a descriptor-facing registration handoff, update this matrix, the slice note, the survey note, and the survey manifest together so the lane keeps one truthful next step