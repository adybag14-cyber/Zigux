...
- scope: keep the current `gpio_wdt` starter honest about what is already validated, name the explicit teardown evidence, the explicit remaining-blocker evidence, plus the first bounded register-device call evidence, and avoid overclaiming live GPIO, platform registration, or reboot integration before those behaviors exist in Zigux
...
The bounded starter now covers `hw_algo` parsing, heartbeat-margin validation, an explicit watchdog metadata summary, starter-local `nowayout` policy bookkeeping, start or ping or stop transitions, a small probe-time summary, a nowayout-aware stop helper, an explicit `summarizeTeardown()` helper, a metadata-only registration plan, the first bounded register-device call summary, and the explicit remaining-blocker summary that keeps the parked request surface honest. The live repo still needed one reviewable note that explains:
...
- which remaining descriptor, platform-registration, and reboot blockers are already explicit as a parked failure summary, including the primary-failure ordering and blocker-count evidence, rather than implied by the request surface alone
...
| explicit remaining blocker surface | `registerDeviceFailureSummary()` keeps descriptor preflight, platform registration, and reboot glue blockers machine-checkable while preserving the startup state that would reach the parked request boundary; the current lane-local evidence also keeps the primary-failure ordering explicit by treating `descriptor_preflight_pending` as the first blocker, then `platform_registration_pending`, then `reboot_glue_pending`, and it preserves the blocker count through `failure_mode_count` instead of leaving those review details implicit | `zig build test --build-file zigux/tests/phase11_build.zig --summary all` via the register-device failure assertions in `zigux/tests/phase11_gpio_wdt.zig` | leave this failure summary parked unless a later lane can isolate one comparably small descriptor-backed or reboot-adjacent preflight step beside it | live descriptor acquisition, real register-device execution, reboot hooks, and hardware-backed validation |
...
- keep the explicit remaining blocker surface tied to the parked request boundary until a real descriptor-backed or reboot-glue widening exists, and keep the primary-failure ordering plus blocker-count wording explicit whenever that note is refreshed
