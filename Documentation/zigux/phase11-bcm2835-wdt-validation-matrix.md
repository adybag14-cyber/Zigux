...
It keeps the current starter packet honest while now including dedicated registration-scaffold coverage and a real bcm2835-local slice note, without overclaiming live platform-backed closure.
...
The current driver file plus the dedicated replay, registration-scaffold replay, verify helper, survey gate, and manifest-backed reminder packet now expose these bounded review surfaces:
...
- dedicated registration-scaffold replay in `zigux/tests/phase11_bcm2835_wdt_registration_scaffold.zig`
...
- survey-gate coverage plus manifest-backed reminder coverage in `zigux/tests/phase11_bcm2835_wdt_manifest.json` and `Documentation/zigux/phase11-bcm2835-wdt-slice.md`
...
| probe and PM-base handoff summary | `summarizeProbe()`, `summarizePlatformHandoff()`, the dedicated replay, the dedicated registration-scaffold replay, the compile-local verify helper, and the dedicated survey gate | nowayout flag readback, bootloader-running import signal, parent attachment, PM-base readiness, and claimed-versus-conflicting or controller-free ownership summary | live platform registration, callback installation, and PM-base execution wiring |
...
| dedicated review packet alignment | `zigux/tests/phase11_bcm2835_wdt_survey.zig`, `zigux/tests/phase11_bcm2835_wdt_manifest.json`, `Documentation/zigux/phase11-bcm2835-wdt-slice.md`, `Documentation/zigux/phase11-bcm2835-wdt-survey.md`, and this matrix | bcm2835-only truthfulness for the directly readable driver, replay, registration scaffold, verify helper, teardown note, and reminder packet | live platform registration, shared callback installation, and broader shared Phase 11 wording |

## Review Guardrails

- Treat this matrix as a truthfulness note for the current directly readable starter packet, dedicated replay, dedicated registration-scaffold replay, compile-local verify helper, dedicated survey gate, and manifest-backed reminder packet, not as proof of live platform-backed closure.
- Keep this matrix aligned with `drivers/watchdog/bcm2835_wdt.zig`, `drivers/watchdog/bcm2835_wdt_verify.zig`, `zigux/tests/phase11_bcm2835_wdt.zig`, `zigux/tests/phase11_bcm2835_wdt_registration_scaffold.zig`, `zigux/tests/phase11_bcm2835_wdt_survey.zig`, `zigux/tests/phase11_bcm2835_wdt_manifest.json`, `Documentation/zigux/phase11-bcm2835-wdt-slice.md`, `Documentation/zigux/phase11-bcm2835-wdt-survey.md`, and `Documentation/zigux/phase11-bcm2835-wdt-teardown-note.md` whenever the directly readable helpers or reminder packet changes.
- Do not claim live platform registration, PM-base plumbing, watchdog-core registration, or hardware-backed poweroff execution until the bcm2835 lane records a directly reviewable execution bridge and the wider Zig surface exists.

## Next Blocked Step

The next honest bcm2835-only follow-through is one execution bridge for the already-modeled PM-base and registration sequencing path. That follow-through should reuse the explicit validation plan captured by this slice-backed packet instead of reopening note-only drift. Until that lands, keep this matrix bounded to the directly readable bcm2835 surfaces already visible on `master`.
