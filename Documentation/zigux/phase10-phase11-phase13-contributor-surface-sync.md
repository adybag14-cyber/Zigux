...
- keep the inventory-backed shared contract explicit through `scripts/zigux/check-phase11-build-inventory.py`, `zigux/tests/fixtures/phase11_build_inventory.json`, `zigux/tests/phase11_build.zig`, the exact shared `zig build test --build-file zigux/tests/phase11_build.zig --summary all` replay, and `make -C zigux phase11-contract`, so broader contributor wording treats the landed shared build packet as shipped evidence rather than as a missing validator-first packet
...
- keep the currently direct replay anchors explicit too: `zigux/tests/phase11_gpio_wdt.zig`, `zigux/tests/phase11_bcm2835_wdt.zig`, `drivers/watchdog/bcm2835_wdt.zig`, `zigux/tests/phase11_dw_wdt.zig`, `zigux/tests/phase11_dw_wdt_registration_scaffold.zig`, `drivers/watchdog/bcm2835_wdt_verify.zig`, `drivers/watchdog/dw_wdt_verify.zig`, `zigux/tests/phase11_hvc_console.zig`, `zigux/tests/phase11_hvc_cleanup.zig`, `drivers/tty/hvc/hvc_console_verify.zig`, and `zigux/tests/phase11_uapi_header_parity_survey.zig` are all directly readable bounded replay evidence on current `master`; do not let compact contributor wording collapse them back to reminder-note-only inventory or repo-reality-gap language
