...
- keep the inventory-backed shared contract explicit through `scripts/zigux/check-phase11-build-inventory.py`, `zigux/tests/fixtures/phase11_build_inventory.json`, `make -C zigux phase11-contract`, and the exact `zig build test --build-file zigux/tests/phase11_build.zig --summary all` replay; direct contents reads can still 404 on `zigux/tests/phase11_build.zig`, so broader contributor wording should treat that inventory-backed contract as shipped shared evidence rather than as a missing validator-first packet
