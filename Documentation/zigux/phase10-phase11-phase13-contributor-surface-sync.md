...
- keep the shipped shared contract rooted in `Documentation/zigux/phase11-shared-replay-contract.md`, `scripts/zigux/check-phase11-shared-replay-contract.py`, `scripts/zigux/check-phase11-build-inventory.py`, `zigux/tests/fixtures/phase11_build_inventory.json`, `zigux/tests/phase11_build.zig`, `.github/workflows/zigux-bootstrap.yml`, `make -C zigux phase11-contract`, and the exact `zig build test --build-file zigux/tests/phase11_build.zig --summary all` replay so the shared replay-contract note, checker pair, workflow replay, and build-backed packet stay explicit together on current `master`
