...
- keep the inventory-backed shared contract explicit through `scripts/zigux/check-phase11-build-inventory.py`, `zigux/tests/fixtures/phase11_build_inventory.json`, `zigux/tests/phase11_build.zig`, `make -C zigux phase11-contract`, and the exact `zig build test --build-file zigux/tests/phase11_build.zig --summary all` replay; broader contributor wording should treat that inventory-backed contract as shipped shared evidence rather than as a missing validator-first packet or a fallback-only route
...
- keep the removed DesignWare reminder-note family `Documentation/zigux/phase11-dw-wdt-validation-matrix.md`, `Documentation/zigux/phase11-dw-wdt-survey.md`, and `Documentation/zigux/phase11-dw-wdt-teardown-note.md`, plus the absent direct companions `zigux/tests/phase11_dw_wdt_manifest.json` and `zigux/tests/phase11_dw_wdt_survey.zig`, framed as no longer shipped contributor-facing evidence on current `master`
