...
Contributor reminder:
- keep the validator-first Phase 13 release handle explicit through `scripts/zigux/check-phase13-landlock-ruleset-packet.py`, `scripts/zigux/check-phase13-devres-packet-alignment.py`, `scripts/zigux/check-phase13-notifier-priority-signal.py`, `scripts/zigux/validate-phase13-release.py`, stable `make -C zigux phase13-validate`, and blocked convenience route `make -C zigux phase13` while treating the missing `scripts/zigux/check-phase13-shared-summary-surfaces.py` path and the missing direct helper-local tests-root and notifier companions as repo-reality gaps until current-`master` readback proves them
- keep the shipped broader Phase 13 tests-root guide in `zigux/tests/README.md` explicit as shared packet evidence and keep it aligned with the contributor workflow guide, shared-helper sequencing note, and release-coordination matrix
