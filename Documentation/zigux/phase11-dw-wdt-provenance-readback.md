...
- the current DesignWare packet is still meaningful Phase 11 product work because the bounded owner note, survey note, validation matrix, manifest, focused survey gate, registration scaffold, restart helper, PM helper pair, and checker surfaces remain reviewable through authenticated contents reads.
- the honest owner-lane move is therefore to keep the readback split explicit instead of claiming that the broader direct-driver, verify-helper, and direct replay stack is directly readable through the same bridge as the smaller owner packet.
- the surviving same-lane gap is no longer stale survey wording about `zigux/tests/phase11_build.zig` or the landed restart and PM-helper inventory; it is the broader direct-driver, verify-helper, driver-test, slice, and teardown-note stack together with the later platform-backed acquisition and MMIO follow-through.
...
- if a future run uses public-tree fallback to inspect the broader DesignWare driver, verify-helper, slice, teardown-note, or direct replay stack, record that as fallback evidence instead of promoting it to authenticated current-head readback without a matching contents reread.
- keep the broader direct-driver, verify-helper, and replay stack framed as larger same-lane vocabulary until a fresh authenticated reread restores those paths through the same bridge.
