...
- `cmdline` now survives only through a helper-only foothold in this environment. Fresh authenticated contents reads in this slot did return `lib/cmdline.zig`, but the corresponding slice, dedicated test, survey, manifest, and fixture packet still returned missing on current `master`. That means `P7-L05` should keep same-lane follow-through limited to the returned helper foothold or to reminder truthfulness that keeps the missing companion packet explicit instead of presenting the broader cmdline packet as fully returned.
...
- `P7-L05` owns only cmdline helper-local parity, survey, manifest, fixture, or same-slice reminder drift; because the current slot could directly reread `lib/cmdline.zig` but not the corresponding slice, dedicated test, survey, manifest, or fixture packet, keep same-lane work limited to the returned helper foothold or to reminder truthfulness that keeps those missing companions explicit until a fresh reread proves more of the packet returned.
...
- If the drift is `cmdline`, first confirm whether the helper-only foothold in `lib/cmdline.zig` has changed; otherwise keep same-lane claims limited to that returned helper and the still-missing companion packet until a fresh reread proves the slice, dedicated test, survey, manifest, or fixture surfaces returned.
