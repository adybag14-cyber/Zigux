...
- rbtree direct-readback fallback packet, lane `P7-L13`:
  - `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`
  - `zigux/tests/phase7_rbtree_survey.zig`
  - `zigux/tests/phase7_rbtree_manifest.json`
  - parked same-lane reminder paths until they rematerialize on current `master`: `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, `zigux/tests/fixtures/phase7_rbtree.json`, `zigux/tests/fixtures/phase7_rbtree_c_harness.c`, and `scripts/zigux/check-phase7-rbtree-parity.py`
  - scheduled alias note: recurring scheduled lane `kernel-leaf-libraries` may advance into the broader rbtree helper-local packet only after same-lane rereads prove one of those parked helper-local surfaces has returned on current `master`
...
- `rbtree` currently survives only through `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`, `zigux/tests/phase7_rbtree_survey.zig`, and `zigux/tests/phase7_rbtree_manifest.json`. Fresh same-lane reread for this slot returned 404 for `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, `zigux/tests/fixtures/phase7_rbtree.json`, `zigux/tests/fixtures/phase7_rbtree_c_harness.c`, and `scripts/zigux/check-phase7-rbtree-parity.py`, while the shared `scripts/zigux/validate-phase7.py` and `zigux/tests/phase7_build.zig` paths remain parked reminder vocabulary in the shared-control packet rather than direct returned evidence. `P7-L13` should therefore keep its same-lane truthfulness packet limited to the direct-anchor note, survey, and manifest until one concrete helper-local rbtree surface rematerializes on current `master`.
...
- `P7-L13` owns only rbtree direct-anchor-note, survey, manifest, or missing-surface reminder drift; because the current slot could directly reread `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`, `zigux/tests/phase7_rbtree_survey.zig`, and `zigux/tests/phase7_rbtree_manifest.json` while the broader rbtree helper, dedicated test, fixture pair, and parity checker still return 404 on current `master`, keep same-lane work inside that direct-readback fallback packet and keep the missing helper-local surfaces plus the shared `phase7-*` build routes framed as parked reminder vocabulary.
...
- If the drift is the current `rbtree` fallback packet, keep the change inside `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`, `zigux/tests/phase7_rbtree_survey.zig`, or `zigux/tests/phase7_rbtree_manifest.json` until a fresh same-lane reread proves one concrete helper-local surface has rematerialized on current `master`.
