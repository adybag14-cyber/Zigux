...
- `cmdline` now survives through `lib/cmdline.zig`, `zigux/tests/phase7_cmdline_survey.zig`, and `zigux/tests/phase7_cmdline_manifest.json`. Fresh authenticated contents reads in this slot did return that helper-plus-survey-manifest foothold, but the corresponding slice, dedicated test, and fixture packet still returned missing on current `master`. That means `P7-L05` should keep same-lane follow-through limited to the returned helper-plus-survey-manifest foothold or to reminder truthfulness that keeps the missing companion packet explicit instead of presenting the broader cmdline packet as fully returned.
...
- `P7-L05` owns only cmdline helper-local parity, survey, manifest, fixture, or same-slice reminder drift; because the current slot could directly reread `lib/cmdline.zig`, `zigux/tests/phase7_cmdline_survey.zig`, and `zigux/tests/phase7_cmdline_manifest.json` but not the corresponding slice, dedicated test, or fixture packet, keep same-lane work limited to that returned helper-plus-survey-manifest foothold or to reminder truthfulness that keeps those missing companions explicit until a fresh reread proves more of the packet returned.
...
- If the drift is `cmdline`, first confirm whether the helper-plus-survey-manifest foothold in `lib/cmdline.zig`, `zigux/tests/phase7_cmdline_survey.zig`, and `zigux/tests/phase7_cmdline_manifest.json` has changed; otherwise keep same-lane claims limited to those returned surfaces and the still-missing companion packet until a fresh reread proves the slice, dedicated test, or fixture surfaces returned.
