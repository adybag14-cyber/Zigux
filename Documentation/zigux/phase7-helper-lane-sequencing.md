...
- rbtree direct-readback fallback packet, lane `P7-L13`:
  ...
  - parked same-lane reminder paths until they rematerialize on current `master`: `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/fixtures/phase7_rbtree.json`, `zigux/tests/fixtures/phase7_rbtree_c_harness.c`, and `scripts/zigux/check-phase7-rbtree-parity.py`
  - scheduled alias note: recurring scheduled lane `kernel-leaf-libraries` may advance into the broader rbtree helper-local packet only after same-lane rereads prove one more parked helper-local surface has returned on current `master`
...
- `rbtree` now survives through `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`, `tools/lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, `zigux/tests/phase7_rbtree_survey.zig`, and `zigux/tests/phase7_rbtree_manifest.json`. Fresh same-lane reread for this slot confirmed the dedicated replay at `zigux/tests/phase7_rbtree.zig` now directly materializes on current `master`, while `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/fixtures/phase7_rbtree.json`, `zigux/tests/fixtures/phase7_rbtree_c_harness.c`, and `scripts/zigux/check-phase7-rbtree-parity.py` still return 404. The shared `scripts/zigux/validate-phase7.py` and `zigux/tests/phase7_build.zig` paths remain parked reminder vocabulary in the shared-control packet rather than direct returned evidence. `P7-L13` should therefore keep its same-lane truthfulness packet limited to the direct-anchor note, tool-root helper, dedicated replay, survey, and manifest until one more concrete helper-local rbtree surface rematerializes on current `master`.
