...
- `rbtree` now survives through `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`, `tools/lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, and `zigux/tests/phase7_rbtree_manifest.json`. Fresh same-lane reread for this slot confirmed the dedicated replay at `zigux/tests/phase7_rbtree.zig` now directly materializes on current `master`, while `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/fixtures/phase7_rbtree.json`, `zigux/tests/fixtures/phase7_rbtree_c_harness.c`, and `scripts/zigux/check-phase7-rbtree-parity.py` still return 404. The shared `zigux/tests/phase7_build.zig` path remains parked reminder vocabulary in the shared-control packet rather than direct returned evidence, while `scripts/zigux/validate-phase7.py` has already returned only as the narrow shared validation foothold. `P7-L13` should therefore keep its same-lane truthfulness packet limited to the direct-anchor note, tool-root helper, dedicated replay, survey, and manifest until one more concrete helper-local rbtree surface rematerializes on current `master`.
