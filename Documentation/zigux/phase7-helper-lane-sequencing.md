...
Fresh repo-first inspection in Slot 043 shows a narrower direct-readback surface than several older Phase 7 reminder notes still describe.
...
- `argv_split` currently survives through `lib/argv_split.zig`, `zigux/tests/phase7_argv_split_survey.zig`, `zigux/tests/phase7_argv_split_manifest.json`, and `scripts/zigux/check-phase7-argv-split-packet.py`. Fresh authenticated contents reads in this slot still returned missing for `Documentation/zigux/phase7-argv-split-slice.md`, `zigux/tests/phase7_argv_split.zig`, and `zigux/tests/fixtures/phase7_argv_split_vectors.zig`. The shared readback surfaces `zigux/tests/phase7_build.zig` and `scripts/zigux/validate-phase7.py` are directly readable again, but they remain shared-control evidence rather than proof that the narrower argv_split packet is complete. That means `P7-L09` should treat that returned helper-plus-survey-manifest-checker anchor as the current same-lane packet and keep the missing slice, dedicated test, and fixture explicit instead of presenting the broader argv_split packet as fully returned.
- `rbtree` now survives through the public-fallback-visible helper packet `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, `zigux/tests/phase7_rbtree_survey.zig`, `zigux/tests/phase7_rbtree_manifest.json`, `zigux/tests/fixtures/phase7_rbtree.json`, `zigux/tests/fixtures/phase7_rbtree_c_harness.c`, and `scripts/zigux/check-phase7-rbtree-parity.py`. Fresh public fallback rereads in this slot also returned the adjacent shared readback surfaces `zigux/tests/phase7_build.zig` and `scripts/zigux/validate-phase7.py`, while `zigux/Makefile` still lacks dedicated `phase7-*` wrapper routes and `.github/workflows/zigux-bootstrap.yml` still lacks dedicated Phase 7 runtime-helper steps. That means `P7-L13` should keep same-lane work anchored to the restored rbtree packet plus that returned shared build-and-validator evidence while leaving the still-missing wrapper routes explicit instead of repeating the older missing-fixture or missing-validator story.
- the shared control packet is still only partly recoverable in this slot. `samples/zigux/README.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, `scripts/zigux/validate-phase7.py`, and `zigux/tests/phase7_build.zig` are directly readable again, but `zigux/Makefile` still lacks dedicated `phase7-*` wrapper routes and fresh repo evidence still does not show dedicated Phase 7 runtime-helper steps in `.github/workflows/zigux-bootstrap.yml`, so `P7-Y05` should treat the shared wrapper stack as a reminder packet whose live build-and-validator evidence has returned without overclaiming make-wrapper or workflow recovery.

That means the honest lane state is not "all four helper families are landed and fully direct-readback reviewable." The current live split is one fully readable `string_helpers` packet, one helper-plus-slice-survey-manifest-sample-boundary `cmdline` foothold, one helper-plus-survey-manifest-checker `argv_split` anchor whose broader slice, dedicated test, and fixture still remain missing, and one public-fallback-visible `rbtree` packet that now includes its fixture pair plus returned shared build-and-validator evidence while still leaving dedicated Makefile and workflow wrapper routes absent. Cross-helper truthfulness should keep the landed string-helpers packet explicit instead of repeating the older blocked-by-missing-string-helpers story inside the rbtree lane.
...
- Treat scheduled lane `P7-Y07` as the argv-split alias for `P7-L09`; if a scheduled run starts under `P7-Y07`, keep the work inside the currently returned `argv_split` helper, survey, manifest, and checker surfaces and do not claim the missing slice, dedicated test, or fixture as direct current-`master` evidence before those paths reread. If shared build-or-validation reminder surfaces are readable again, keep them explicit as non-owner evidence only.
- Treat scheduled lane `P7-Y04` as the rbtree alias for `P7-L13`; if a scheduled run starts under `P7-Y04`, keep the work inside the restored rbtree slice, helper, dedicated test, survey, manifest, fixture pair, and checker packet, and keep the returned `zigux/tests/phase7_build.zig` and `scripts/zigux/validate-phase7.py` evidence explicit without claiming the missing Makefile or workflow wrapper routes have returned.
...
- `P7-L09` owns only argv-split helper-local parity, survey, manifest, fixture, checker, or reminder drift; because the current slot could directly reread `lib/argv_split.zig`, `zigux/tests/phase7_argv_split_survey.zig`, `zigux/tests/phase7_argv_split_manifest.json`, and `scripts/zigux/check-phase7-argv-split-packet.py`, keep same-lane work inside those returned surfaces until a fresh reread proves the slice note, dedicated external test, or fixture returned. If shared build-and-validator routes are readable again, keep them explicit as non-owner evidence rather than using them to overstate the narrower argv-split packet.
- `P7-L13` owns only rbtree helper-local parity, traversal, manifest, fixture, checker, or reminder drift; because the current slot could publicly reread the slice note, helper, dedicated test, survey, manifest, fixture pair, and parity checker, and could also reread the adjacent shared `zigux/tests/phase7_build.zig` and `scripts/zigux/validate-phase7.py` evidence while `zigux/Makefile` and `.github/workflows/zigux-bootstrap.yml` still lack dedicated Phase 7 wrapper markers, keep same-lane work anchored to the restored rbtree packet and make the returned build-and-validator evidence explicit without claiming the missing wrapper routes have returned.
...
- If the drift is a partially returned `argv_split` surface, keep the change inside `lib/argv_split.zig`, `zigux/tests/phase7_argv_split_survey.zig`, `zigux/tests/phase7_argv_split_manifest.json`, or `scripts/zigux/check-phase7-argv-split-packet.py` until a fresh reread proves the slice note, dedicated external test, or fixture returned. If the visible issue is only shared build-or-validation wording, keep that evidence explicit as shared-control vocabulary instead of overclaiming argv_split packet recovery.
- If the drift is a publicly visible `rbtree` surface, keep the change inside the restored slice-helper-test-survey-manifest-fixture-checker packet or its attached direct-anchor reminder evidence, and keep the returned `zigux/tests/phase7_build.zig` and `scripts/zigux/validate-phase7.py` readback explicit without implying that the missing Makefile or workflow wrapper routes recovered before a fresh same-lane reread proves it.
