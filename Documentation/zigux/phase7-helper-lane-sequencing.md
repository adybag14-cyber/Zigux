...
- string_helpers packet, helper-local lane family:
  ...
  - scheduled historical-lane note: older helper-local lane `P7-L04` is still same-family `string_helpers` follow-through, not a shared-control workspace lane; keep it narrowed to the string_helpers slice, helper, dedicated replay, survey, manifest, sample-boundary, or checker surfaces while shared validator, Makefile, workflow, docs-root, tests-root, sample-root, and shared-build reminders stay with `P7-Y05`
...
- Treat older helper-local lane `P7-L04` as same-family string_helpers follow-through too; keep it narrowed to `Documentation/zigux/phase7-string-helpers-slice.md`, `lib/string_helpers.zig`, `zigux/tests/phase7_string_helpers.zig`, `zigux/tests/phase7_string_helpers_survey.zig`, `zigux/tests/phase7_string_helpers_manifest.json`, `zigux/tests/phase7_string_helpers_sample_boundary.zig`, or `scripts/zigux/check-phase7-string-helpers-packet.py` instead of reassigning that lane to shared-control ownership.
...
- If the drift is a string_helpers helper-local surface, keep the change inside `Documentation/zigux/phase7-string-helpers-slice.md`, `lib/string_helpers.zig`, `zigux/tests/phase7_string_helpers.zig`, `zigux/tests/phase7_string_helpers_survey.zig`, `zigux/tests/phase7_string_helpers_manifest.json`, `zigux/tests/phase7_string_helpers_sample_boundary.zig`, or `scripts/zigux/check-phase7-string-helpers-packet.py`; treat recurring helper-local lane `P7-Y01` and older helper-local lane `P7-L04` as same-family follow-through inside that one packet rather than as separate helper families, and route shared validator, Makefile, workflow, docs-root, tests-root, sample-root, and shared-build reminders back to `P7-Y05`.
