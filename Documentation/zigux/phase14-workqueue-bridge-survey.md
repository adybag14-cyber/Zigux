...
- `PHASE14_SURVEYED_COMMIT=7a4454d0474106972cad7e164b79293bd54a40c6`
- `PHASE14_SLICE=workqueue-delayed-submission-alias-audit`
- scope: the landed `kernel/workqueue_bridge.zig` boundary map plus its expanded concurrency audit outline and new delayed-submission alias checkpoint, its dedicated Phase 14 test gate and manifest, the shared Phase 14 build wiring, and the lane notes that compare the new foothold against the roadmap
- product boundary:
...
- the bridge now carries an expanded concurrency audit outline around `manage_workers()`, `worker_pool` forward-progress fields, the `__queue_work()` `max_active` gate, the `try_to_grab_pending()` plus `queue_work_on()` pending-bit claim handoff, the delayed-submission alias routing through `queue_delayed_work_on()`, `mod_delayed_work_on()`, and `__queue_delayed_work()`, the unbound `__queue_work()` `pwq->refcnt` retry path, the `__queue_work()` `last_pool->lock` handoff, the `process_one_work()` unlock or relock execution window, the `worker_thread()` idle sleep transition, `rescuer_thread()`, and `wq_worker_running()` or `wq_worker_sleeping()`, still without claiming live execution ownership.
- the next honest workqueue-facing step is a delayed-work timer handoff audit around `delayed_work_timer_fn()` so the lane records how timer expiry falls back into `__queue_work()` before any wrapper claims live delayed-work execution control.
...
- landed `phase14-workqueue-delayed-submission-alias-audit`
...
- delayed-work timer expiry ownership or CPU hotplug behavior
- rescuer execution
...
Stay in the Phase 14 workqueue lane and add one tiny `kernel/workqueue_bridge.zig` delayed-work timer handoff audit next, limited to `delayed_work_timer_fn()` so the bridge records the timer-expiry hop back into `__queue_work()` before any wrapper leaves the current boundary-map-only posture.
