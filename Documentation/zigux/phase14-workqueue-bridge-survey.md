...
- the next honest workqueue-facing step is a delayed-submission follow-up around `queue_delayed_work_on()`, `mod_delayed_work_on()`, and `__queue_delayed_work()` so the lane names the delayed submission alias layer around pending ownership, disabled-work bypass, and timer handoff before any wrapper claims CPU-affinity, timer, or requeue parity.
...
Stay in the Phase 14 workqueue lane and add one tiny `kernel/workqueue_bridge.zig` delayed-submission audit next, limited to `queue_delayed_work_on()`, `mod_delayed_work_on()`, and `__queue_delayed_work()` so the bridge records the alias layer around pending ownership, disabled-work bypass, and timer handoff before any wrapper claims CPU-affinity, timer, or requeue parity.
