...
Current `master` now materializes `security/landlock/syscalls.zig` as a helper-local starter for the roadmap-owned `security/landlock/syscalls.c` anchor. Keep the helper-owned wording tightly scoped to descriptor-backed create-ruleset planning, ABI-version query planning, delegated ruleset-creation planning, handled-access plus attr-size plus flag validation, and the top-level create-ruleset initialization plus attr-presence wrapper checks before the anonymous-fd install boundary, and do not present that helper packet as a shipped ruleset-fd install planner, live FD installation, credential replacement, or full Landlock enforcement.
...
- helper-owned wording must match the current helper boundary: create-ruleset planning, ABI-version query planning, delegated ruleset-creation planning, handled-access plus attr-size plus flag validation, top-level create-ruleset initialization plus attr-presence wrapper checks, and no shipped ruleset-fd install planner, live FD installation, or credential replacement
