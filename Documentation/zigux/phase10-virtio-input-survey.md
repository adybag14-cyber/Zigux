...
- scope: keep the current `drivers/virtio/virtio_input.zig` helper packet, its dedicated queue-callback-preflight helper and replay, its queue-handling replays, its teardown-preflight helper, its wrapper-facing teardown-reset verify parity, and the shared Phase 10 build gate reviewable as lab-only driver validation without claiming transport-backed queue execution, input registration lifecycle parity, freeze or restore behavior, or probe/remove closure
- product boundary:
...
- the live manifest keeps the current input lane concrete: the shared build gate, the direct helper foothold, the direct gate, the slice note, the module-facing slice note, the dedicated survey gate, the probe-preflight helper and replay, the dedicated queue-callback-preflight helper and replay, the registration-preflight helper and replay, the status-drain helper and replay, the teardown-preflight helper, the teardown-observation helper and replay, and the wrapper-facing verify replay now keep teardown-reset parity across reset explicit beside queue-callback ordering and registration prerequisites while freeze, restore, and remove lifecycle claims stay parked.
