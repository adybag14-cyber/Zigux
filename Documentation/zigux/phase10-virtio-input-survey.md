...
- scope: keep the current `drivers/virtio/virtio_input.zig` helper packet, its direct queue-callback-preflight summary, its queue-handling replays, its wrapper-facing teardown-reset verify parity, and the shared Phase 10 build gate reviewable as lab-only driver validation without claiming transport-backed queue execution, input registration lifecycle parity, freeze or restore behavior, or probe/remove closure
- product boundary:
...
- the live manifest keeps the current input lane concrete: the shared build gate, the direct helper foothold, the direct gate, the slice note, the module-facing slice note, the dedicated survey gate, the probe-preflight helper and replay, the direct queue-callback-preflight summary in `drivers/virtio/virtio_input.zig` plus its replay, the registration-preflight helper and replay, the status-drain helper and replay, the teardown-observation helper and replay, and the wrapper-facing verify replay now keep teardown-reset parity across reset explicit beside queue-callback ordering and registration prerequisites while freeze, restore, and remove lifecycle claims stay parked.
- current `master` still does not materialize a separate `drivers/virtio/virtio_input_queue_callback_preflight.zig` shard, so the queue-callback-preflight packet stays grounded in direct `VirtioInputLab.queueCallbackPreflightSummary()` behavior inside `drivers/virtio/virtio_input.zig` plus the replay in `zigux/tests/phase10_virtio_input_queue_callback_preflight.zig` rather than in a dedicated helper file.
- the landed queue-handling ladder remains bounded and in-memory: probe staging stays identity-first, queue-callback readiness still sits behind queue configuration and buffer fill, registration staging still stops at blocker reporting below `input_register_device()`, wrapper-facing teardown-reset verify parity stays explicit across reset, queued status completions are reclaimed only in memory, and teardown review remains reset-local.
- `Documentation/zigux/phase10-virtio-input-slice.md` keeps the direct packet-local ownership story explicit, while `Documentation/zigux/phase10-virtio-input-module-slice.md` keeps the module-facing queue-callback, registration-preflight, status-drain, teardown-local, and teardown-reset verify boundaries explicit in one packet-local companion.
- `zigux/tests/phase10_build.zig` remains part of the shared Phase 10 review packet and currently wires the direct input gate, the probe-preflight replay, the queue-callback-preflight replay, the registration-preflight replay, the status-drain replay, the teardown-observation replay, and the wrapper-facing verify replay into one bounded shared gate for the live input packet.
...
## Next bounded step
Keep the broader Phase 10 virtio lane parked unless fresh repo inspection finds one directly coupled same-lane follow-through. Inside this input lane, the next honest bounded step is to keep `zigux/tests/phase10_virtio_input_manifest.json`, `Documentation/zigux/phase10-virtio-input-slice.md`, `Documentation/zigux/phase10-virtio-input-module-slice.md`, this survey note, and the shared Phase 10 reminder surfaces aligned around the landed queue-handling, registration-preflight, and teardown-reset verify packet while treating `zigux/tests/phase10_build.zig` as already-landed shared validation evidence rather than routing those compile entries back into stale adjacent-lane `P10-L15` follow-through.
