...
This keeps the lane concrete and reviewable without overstating progress: the starter helper is real, the dedicated wrapper-facing verify replay is now recorded as its own landed validation surface, the dedicated queue-callback-preflight replay is now recorded in the same packet, the slot-planning foothold and registration-preflight boundary are now real, the queue-callback-preflight helper boundary is now real, the bounded status-drain replay and teardown-observation helper are now recorded in the same packet, the wrapper-ownership note is now real, and the risky registration and transport surface remains intentionally out of scope.
