...
This keeps the lane concrete and reviewable without overstating progress: the starter helper is real, the dedicated wrapper-facing verify replay is now recorded as its own landed validation surface, the bounded event-buffer refill path is now explicit inside the survey evidence, the dedicated queue-callback-preflight replay is now recorded in the same packet, the dedicated registration-preflight and teardown-observation replays are now recorded in the same packet, the slot-planning foothold, probe-preflight boundary, and registration-preflight boundary are now real, the queue-callback-preflight helper boundary is now real, the bounded status-drain replay and teardown-observation helper are now recorded in the same packet, the wrapper-ownership note is now real, and the risky registration and transport surface remains intentionally out of scope.
