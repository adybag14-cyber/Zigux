...
Current `master` also materializes the adjacent direct-evidence shards `zigux/bindings/notifier_abi.zig` and `include/zigux/abi.h`.
...
Current `master` also materializes the adjacent direct-evidence shards `zigux/bindings/notifier_abi.zig` and `include/zigux/abi.h`, but it still does not materialize these remaining direct notifier or list companions recorded in `Documentation/zigux/phase13-notifier-list-survey.md`, so shared contributor wording should keep them framed as adjacent repo-reality gaps rather than as independently shipped reviewer evidence:
