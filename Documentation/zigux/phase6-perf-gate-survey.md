...
- shared backlog gap: `Documentation/zigux/README.md` and `Documentation/zigux/review-checklist.md` still summarize the dedicated Phase 6 perf routes as checksum-plus-hexdump only, so the next safe shared-surface correction is to make those reviewer-facing summaries name the already-landed `make -C zigux phase6-base64-perf` route and `zigux/tests/phase6_base64_perf.zig` packet too
