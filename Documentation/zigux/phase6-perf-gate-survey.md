...
- `Documentation/zigux/README.md` already names that same checksum-plus-hexdump perf-marker command pair, and the dedicated checker now exact-counts the docs root, helper catalog, perf survey, scripts root, tests root, Makefile, and manifest surfaces so that shared reviewer-facing route cannot drift silently ahead of the larger `validate-phase6.py` follow-through
...
- the dedicated perf-marker checker, `Documentation/zigux/README.md`, `zigux/tests/phase6_helper_parity_manifest.json`, and `zigux/Makefile` already name `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py --self-test` plus `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py` explicitly, so any future shared-validator refresh should keep that same exact command pair aligned across the docs-root, shared marker surfaces, required-file inventory, and manifest-backed exact-check route instead of leaving the perf-marker packet implied behind helper-local notes alone
