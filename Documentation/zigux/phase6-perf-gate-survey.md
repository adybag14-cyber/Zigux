...
- `Documentation/zigux/README.md` already names that same checksum-plus-hexdump perf-marker command pair, but the dedicated checker still exact-counts the helper catalog, perf survey, scripts root, tests root, Makefile, and manifest surfaces only, so the next same-lane checker refresh should add docs-root coverage before the larger `validate-phase6.py` follow-through lands
...
- the dedicated perf-marker checker, `zigux/tests/phase6_helper_parity_manifest.json`, and `zigux/Makefile` already name `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py --self-test` plus `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py` explicitly, so any future shared-validator refresh should keep that same exact command pair aligned across the shared marker surfaces, required-file inventory, and manifest-backed exact-check route instead of leaving the perf-marker packet implied behind helper-local notes alone
- refresh `scripts/zigux/validate-phase6.py` in the same change when the shared scripts-root or tests-root checker-command route for `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py --self-test` or `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py` changes so the broader Phase 6 validator keeps the same checksum-plus-hexdump perf-marker packet exact-counted across its shared marker surfaces, required-file inventory, and manifest-backed exact-check contract
