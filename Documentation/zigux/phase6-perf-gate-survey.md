...
- `Documentation/zigux/README.md` already names that same checksum-plus-hexdump perf-marker command pair, and the dedicated checker plus `scripts/zigux/validate-phase6.py` now exact-count the docs root, helper catalog, perf survey, scripts root, tests root, Makefile, required-file inventory, and manifest-backed exact-check surfaces so that shared reviewer-facing route cannot drift silently
- refresh `scripts/zigux/README.md` and `zigux/tests/README.md` in the same change when `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py --self-test`, `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py`, `make -C zigux phase6-validate`, or `make -C zigux phase6-perf` changes so the shared scripts and tests inventories keep naming the same perf-marker packet as the docs root, helper catalog, manifest, Makefile, and validator-backed review path
- the dedicated perf-marker checker, `Documentation/zigux/README.md`, `zigux/tests/phase6_helper_parity_manifest.json`, `zigux/Makefile`, and `scripts/zigux/validate-phase6.py` already name `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py --self-test` plus `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py` explicitly, so the shared validator-backed route now keeps that same exact command pair aligned across the docs-root, shared marker surfaces, required-file inventory, and manifest-backed exact-check route instead of leaving the perf-marker packet implied behind helper-local notes alone
- refresh `scripts/zigux/validate-phase6.py` in the same change when the shared scripts-root or tests-root checker-command route for `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py --self-test` or `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py` changes so the broader Phase 6 validator keeps the same checksum-plus-hexdump perf-marker packet exact-counted across its docs-root, shared marker surfaces, required-file inventory, and manifest-backed exact-check contract
