...
- shared backlog gap: `Documentation/zigux/README.md` still summarizes the dedicated Phase 6 perf routes as checksum-plus-hexdump only, so the next safe shared-surface correction is to make that reviewer-facing summary name the already-landed `make -C zigux phase6-base64-perf` route and `zigux/tests/phase6_base64_perf.zig` packet too
