...
- refresh `scripts/zigux/README.md` and `zigux/tests/README.md` in the same change when `python3 scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py` changes so the shared scripts and tests inventories keep naming the same perf-marker packet as the docs root, manifest, Makefile, and validator-backed review path
