...
On current `master`, the older docs-root and tests-root notifier-helper follow-through are already closed: `Documentation/zigux/README.md` and `zigux/tests/README.md` now keep `scripts/zigux/check-phase13-notifier-packet.py`, `include/zigux/abi.h`, `zigux/helpers/list_view.zig`, and `zigux/helpers/hlist_view.zig` explicit in both the broad Phase 13 contributor-workflow inventory and the later adjacent release-surface clause. The next smallest same-lane follow-through is scripts-root only: `scripts/zigux/README.md` still leaves `Documentation/zigux/phase13-landlock-ruleset-ownership.md` and `Documentation/zigux/phase13-landlock-syscalls-governance.md` implicit in its broad Phase 13 contributor-workflow reminder even though the contributor guide, compact tests-root companion, docs-root summary, and this owner-map note already treat those helper-owned boundary notes as part of the live packet. After that one-file scripts-root repair lands, reread the remaining shared Phase 13 guidance surfaces for the next one-surface wording drift without reopening helper-local ownership or notifier packet scope.
