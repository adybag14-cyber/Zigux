...
- `landlock/syscalls` owns the syscall governance, slice, survey, helper starter, direct replay, reviewability replay, and manifest packet through `Documentation/zigux/phase13-landlock-syscalls-governance.md`, `Documentation/zigux/phase13-landlock-syscalls-slice.md`, `Documentation/zigux/phase13-landlock-syscalls-survey.md`, `security/landlock/syscalls.zig`, `zigux/tests/phase13_landlock_syscalls.zig`, `zigux/tests/phase13_landlock_syscalls_reviewability.zig`, and `zigux/tests/phase13_landlock_syscalls_manifest.json`, while only the shared `zigux/tests/phase13_build.zig` route plus the live credential, file-descriptor-installation, and ruleset-state surfaces stay recorded as repo-reality gaps on current `master`
...
6. If a shared reminder surface still treats the shipped Landlock syscall survey, direct replay, reviewability replay, or manifest as missing, correct that in the shared sequencing lane instead of reopening the syscall helper lanes.
