...
The shared reminder packet is aligned on current `master` for the shipped `libfs` slice, the returned `Documentation/zigux/phase13-libfs-survey.md`, and the still-missing addressability and shared-build-route companions. Keep `python3 scripts/zigux/validate-phase13-release.py` explicit as shipped release-discipline support for that shared reminder set instead of leaving it implied behind the tests-root companion.
...
6. If a shared reminder surface starts treating the still-missing Landlock syscall replay, reviewability replay, or manifest as shipped current-`master` evidence, correct that in this shared sequencing lane instead of reopening the syscall helper lanes.
