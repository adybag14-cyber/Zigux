...
On current `master`, the older docs-root and tests-root Phase 13 follow-through steps named in earlier lane notes are already closed: `Documentation/zigux/README.md` and `zigux/tests/README.md` now keep the helper-owned Landlock boundary notes, the adjacent notifier helper footholds, and the mixed interop anchor `drivers/tty/hvc/hvc_console.h` explicit in their shared contributor-workflow wording.

The scripts-root follow-through is smaller but still open: `scripts/zigux/README.md` already keeps the helper-owned Landlock boundary notes plus the adjacent notifier ABI and helper footholds explicit in its broad Phase 13 reminder, but it still leaves the mixed interop anchor `drivers/tty/hvc/hvc_console.h` implicit. Refresh that one scripts-root reminder next, then reread `Documentation/zigux/phase10-phase11-phase13-contributor-surface-sync.md`, `Documentation/zigux/phase13-contributor-workflow-guide.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, `Documentation/zigux/README.md`, `zigux/tests/README.md`, and `scripts/zigux/README.md` together to confirm the shared packet still uses the same nouns for the same adjacent evidence.
