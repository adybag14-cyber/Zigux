...
- atomic reality today: `zigux/helpers/atomic.zig` currently limits the approved wrapper set to `load`, `store`, `exchange`, `fetchAdd`, `fetchSub`, `fetchAnd`, `fetchOr`, `fetchXor`, and `compareExchange`, all parameterized by Zig atomic order rather than exposing a broader kernel-style helper family, and the focused replay now keeps both the success and mismatch compare-exchange paths reviewable instead of only the happy path
...
- focused replay gate: `zigux/tests/phase3_low_level_wrappers.zig` now keeps the atomic, barrier, MMIO, and scoped narrow-unsafe helper contract on its own compile-and-test path, including denied-scope checks, allowed scoped-MMIO reads and writes, width-specific 16-bit and 32-bit scoped MMIO coverage, misalignment and overflow failures, and the compare-exchange mismatch replay; `python3 scripts/zigux/validate-phase3.py` now also treats that gate as a real anti-regression surface instead of a presence-only file list
- dedicated survey path: `Documentation/zigux/phase3-low-level-wrapper-boundary-survey.md` and `scripts/zigux/validate-phase3-low-level-wrapper-survey.py` keep the currently approved wrapper packet, packet-local blob markers, and reviewable deferred-boundary wording in sync
- 64-bit MMIO or broader kernel-style atomic or barrier families still stay deferred until a roadmap-backed boundary slice needs one more explicit helper
