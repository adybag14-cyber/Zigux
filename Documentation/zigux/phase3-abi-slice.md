...
- atomic reality today: `zigux/helpers/atomic.zig` currently limits the approved wrapper set to `load`, `store`, `exchange`, `fetchAdd`, `fetchSub`, `fetchAnd`, `fetchOr`, `fetchXor`, `compareExchange`, and `compareExchangeWeak`, all parameterized by Zig atomic order rather than exposing a broader kernel-style helper family, and the focused replay now keeps both the strong and weak compare-exchange success plus mismatch paths reviewable instead of only the happy path
...
- `Documentation/zigux/phase3-low-level-wrapper-boundary-survey.md` and `scripts/zigux/validate-phase3-low-level-wrapper-survey.py` now keep that atomic, barrier, and MMIO packet explicit as its own bounded review surface beside the broader ABI slice note, so the already-landed 64-bit MMIO and weak compare-exchange wrappers stay reviewable there while broader kernel-style atomic or barrier families still stay deferred until a roadmap-backed boundary slice really needs them
- focused replay gate: `zigux/tests/phase3_low_level_wrappers.zig` now keeps the atomic, barrier, MMIO, and scoped narrow-unsafe helper contract on its own compile-and-test path, including strong and weak compare-exchange coverage, denied-scope checks, allowed scoped-MMIO reads and writes, width-specific 16-bit, 32-bit, and 64-bit scoped MMIO coverage, misalignment and overflow failures, and the compare-exchange mismatch replay; `python3 scripts/zigux/validate-phase3.py` now also treats that gate as a real anti-regression surface instead of a presence-only file list
