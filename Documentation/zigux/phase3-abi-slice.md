...
- `zigux/tests/phase3_low_level_wrappers.zig` now directly replays the shipped helper surface, including non-`seq_cst` atomic ordering coverage plus byte, 16-bit, and 32-bit MMIO access, while the shared ABI packet still carries the wider compile, layout, and dump proof.
