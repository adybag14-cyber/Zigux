...
- current shared ABI replay:
  ...
  - the current shared ABI packet also keeps the direct export/UAPI behavior replay explicit across `zigux/tests/phase3_export_uapi.zig` and `zigux/tests/phase3_export_uapi_build.zig` so canonical-header parity, future-compatible acceptance, and explicit compatibility-status relays stay reviewable beside the focused layout proof instead of being implied only by the shared dump or validator packet
