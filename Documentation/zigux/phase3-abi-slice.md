...
- current UAPI starter:
  ...
  - current `dev_t.zig` starter surface now stays limited to `minor_bits`, `minor_mask`, `major_max`, `EncodeError`, `majorValid`, `minorValid`, `encode`, `major`, `minor`, `rangeFits`, and `lastInRange`
