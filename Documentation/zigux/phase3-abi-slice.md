...
- current export shim starter:
  ...
  - current export-shim relay surface now keeps the named boundary-header relay, requested-header evaluation, and explicit status packet surface through `Header`, `abi_version`, `header_size`, `HeaderCompatibility`, `HeaderAcceptance`, `HeaderEvaluation`, `CompatibilityDecision`, `versionedHeader`, `canonicalHeader`, `boundaryHeader`, `compatibleHeader`, `header`, `acceptHeader`, `headerCompatibility`, `canonicalizeHeader`, `evaluateHeader`, `compatibilityStatus`, `isCurrentAbiVersion`, `isCompatibleSize`, `isCanonicalSize`, `isCompatibleHeader`, `isCanonicalHeader`, `ok`, `errno`, `normalize`, and `isOk`
- current UAPI starter:
  ...
  - current UAPI starter surface now stays limited to `Header`, `Compatibility`, `AcceptedHeader`, `HeaderEvaluation`, `abi_version`, `header_size`, `versionedHeader`, `canonicalHeader`, `boundaryHeader`, `compatibleHeader`, `isCurrentAbiVersion`, `isCompatibleSize`, `isCanonicalSize`, `compatibility`, `isCompatible`, `isCanonical`, `acceptHeader`, `canonicalizeHeader`, and `evaluateHeader`
...
- current shared ABI replay:
  ...
  - `PHASE3_ABI_MANIFEST_FILE_COUNT=53`
  - `PHASE3_ABI_HEADER_NEXT_STEP_NOTE=Documentation/zigux/phase3-abi-h-boundary-next-step.md`
  ...
  - the current shared ABI packet also keeps the dedicated `include/zigux/abi.h` next-step note explicit through `Documentation/zigux/phase3-abi-h-boundary-next-step.md` so the already-landed chrdev-heavy header growth stays reviewable as a bounded header-family follow-up instead of blurring back into export/UAPI, policy-and-unsafe, or low-level-wrapper closure.
