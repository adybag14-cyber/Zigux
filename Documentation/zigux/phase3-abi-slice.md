...
- focused replay gate: `zigux/tests/phase3_low_level_wrappers.zig` now keeps the atomic, barrier, direct MMIO, scoped MMIO, and narrow-unsafe helper contract on its own compile-and-test path, including denied-scope checks, width-specific direct and scoped 8-bit, 16-bit, 32-bit, and 64-bit MMIO coverage, misalignment and overflow failures, strong compare-exchange success and mismatch coverage, and weak compare-exchange retry and mismatch coverage; the decoded-policy MMIO bridge stays reviewable through `zigux/tests/phase3_policy_unsafe.zig`, and `python3 scripts/zigux/validate-phase3.py` now treats both focused gates as real anti-regression surfaces instead of presence-only file lists
