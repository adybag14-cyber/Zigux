...
- `PHASE3_CURRENT_INTEROP_FAMILIES_DETAIL=bitmap-cpumask-list-hlist-errptr-xarray-idr-ida-minor-alloc-dev-region-cdev-chrdev-notify-ack-window-delivery-guard`
- `PHASE3_CURRENT_INTEROP_GAP=repo-now-carries-curated-phase3-parity-slices-beyond-the-original-roadmap-anchor-set`
- `PHASE3_CURRENT_INTEROP_GAP_DETAIL=live-build-graph-now-carries-deep-chrdev-tail-packets-while-curated-rbtree-c-binding-surface-is-still-missing`
...
Current repo reality therefore includes minor-allocation, dev-region, cdev, and chrdev planning and notification chains.
...
- focused replay gate: `zigux/tests/phase3_low_level_wrappers.zig` now keeps the atomic, barrier, direct MMIO, scoped MMIO, and narrow-unsafe helper contract on its own compile-and-test path, including denied-scope checks, width-specific direct and scoped 8-bit, 16-bit, 32-bit, and 64-bit MMIO coverage, misalignment and overflow failures, `fetchMin()` and `fetchMax()` coverage, strong compare-exchange success and mismatch coverage, and weak compare-exchange retry and mismatch coverage; the decoded-policy MMIO bridge stays reviewable through `zigux/tests/phase3_policy_unsafe.zig`, and `python3 scripts/zigux/validate-phase3.py` now treats both focused gates as real anti-regression surfaces instead of presence-only file lists
