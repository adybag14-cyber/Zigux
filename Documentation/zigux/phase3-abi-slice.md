...
- `Documentation/zigux/phase3-low-level-wrapper-boundary-survey.md` and `scripts/zigux/validate-phase3-low-level-wrapper-survey.py` now keep that atomic, barrier, and MMIO packet explicit as its own bounded review surface beside the broader ABI slice note, so 64-bit MMIO or broader kernel-style atomic or barrier families still stay deferred until a roadmap-backed boundary slice really needs them
- focused replay gate: `zigux/tests/phase3_low_level_wrappers.zig` now keeps the atomic, barrier, MMIO, and scoped narrow-unsafe helper contract on its own compile-and-test path, including denied-scope checks, allowed scoped-MMIO reads and writes, width-specific 16-bit and 32-bit scoped MMIO coverage, misalignment and overflow failures, strong compare-exchange success and mismatch coverage, and weak compare-exchange retry and mismatch coverage; `python3 scripts/zigux/validate-phase3.py` now also treats that gate as a real anti-regression surface instead of a presence-only file list
