...
- current export shim starter:
  ...
  - current export-shim relay surface now keeps the named boundary-header relay and status packet explicit through `Header`, `abi_version`, `header_size`, `HeaderCompatibility`, `versionedHeader`, `canonicalHeader`, `boundaryHeader`, `compatibleHeader`, `header`, `headerCompatibility`, `canonicalizeHeader`, `compatibilityStatus`, `isCurrentAbiVersion`, `isCompatibleSize`, `isCanonicalSize`, `isCompatibleHeader`, `isCanonicalHeader`, `ok`, `errno`, `normalize`, and `isOk`
