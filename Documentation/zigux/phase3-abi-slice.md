...
- current export shim starter:
  ...
  - current export-shim relay surface now keeps the named boundary-header relay and status packet explicit through `Header`, `abi_version`, `header_size`, `HeaderCompatibility`, `HeaderAcceptance`, `versionedHeader`, `canonicalHeader`, `boundaryHeader`, `compatibleHeader`, `header`, `acceptHeader`, `headerCompatibility`, `canonicalizeHeader`, `compatibilityStatus`, `isCurrentAbiVersion`, `isCompatibleSize`, `isCanonicalSize`, `isCompatibleHeader`, `isCanonicalHeader`, `ok`, `errno`, `normalize`, and `isOk`
- current UAPI starter:
  ...
  - current UAPI starter surface now stays limited to `Header`, `Compatibility`, `AcceptedHeader`, `abi_version`, `header_size`, `versionedHeader`, `canonicalHeader`, `boundaryHeader`, `compatibleHeader`, `isCurrentAbiVersion`, `isCompatibleSize`, `isCanonicalSize`, `compatibility`, `isCompatible`, `isCanonical`, `acceptHeader`, and `canonicalizeHeader`
...
- current shared ABI replay:
  ...
  - `PHASE3_ABI_MANIFEST_FILE_COUNT=43`
  ...
  - the current shared ABI packet also keeps the focused export/UAPI boundary replay explicit across `zigux/tests/phase3_export_uapi_layout.zig`, `scripts/zigux/validate-phase3-export-uapi-survey.py`, and `Documentation/zigux/phase3-export-uapi-boundary-survey.md` so the manifest-backed replay surface matches the shipped packet-local starter proof, including accepted-header canonicalization and compatibility-status relays
  - the current shared ABI packet also keeps the focused validator-support packet explicit across `scripts/zigux/validate_phase3_selftest.py`, `scripts/zigux/check-phase3-selftest-surface.py`, `scripts/zigux/check-phase3-readme-tooling-inventory.py`, `scripts/zigux/check-phase3-abi-dump-gate.py`, `scripts/zigux/check-phase3-catalog-selftest.py`, `scripts/zigux/generate-phase3-check-wrappers.py`, and `scripts/zigux/phase3_catalog.py` so the shared helper self-checks, dump-gate proof, tooling inventory audit, catalog audit, and wrapper-drift guard remain reviewable beside the main ABI replay
...
The roadmap's Phase 3 target is no longer blocked on a missing starter boundary. Live `master` already carries the explicit export shim, curated bindings roots, layout-backed replay, policy helpers, wrapper packet, and narrow unsafe layer that make the first permanent C/Zigux boundary reviewable.

- the remaining gap against the roadmap is that the interop surface is still curated starter infrastructure rather than a broader generated-or-curated bindings expansion: the live bindings packet is still concentrated in `zigux/bindings/abi.zig`, `zigux/bindings/dev_t.zig`, and `zigux/bindings/notifier_abi.zig` even though `include/linux/zigux.h` now aggregates many already-approved helper views and summaries.
- `zigux/uapi/` still stops at `version.zig`, so the shipped UAPI surface remains a boundary-header starter contract rather than the wider exported family the long-term substrate will eventually need.
