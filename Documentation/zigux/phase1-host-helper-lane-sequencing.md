...
- the remaining shared bitmap validation follow-through is now closure-side summary sync rather than helper-anchor sync: `scripts/zigux/validate-phase1.py` already matches the live bitmap `review_packet_summary`, but `scripts/zigux/validate-phase1-closure.py` still trails that current bitmap manifest summary, so treat that exact closure-validator summary sync as the smallest remaining shared bitmap gap before widening bitmap-local work again
...
- Fresh repo-first readback also shows the remaining shared bitmap validation work is closure-side rather than helper-local: `scripts/zigux/validate-phase1-closure.py` still trails the live bitmap `review_packet_summary` in `zigux/tests/fixtures/phase1_helper_manifest.json`, so repair that exact closure-validator summary sync before reopening bitmap-local anchors.
...
- `PHASE1_BITMAP_NEXT_SAFE_STEP=bitmap first rechecks whether scripts/zigux/validate-phase1-closure.py still trails the live bitmap review_packet_summary in zigux/tests/fixtures/phase1_helper_manifest.json; if that exact closure-validator summary sync is still open, clear it before reopening bitmap-local anchors, otherwise keep bitmap parked unless a fresh reread finds new direct-anchor drift or committed shared replay drift; do not reopen the already-closed validator-summary packet by default`
