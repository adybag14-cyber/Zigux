...
- the bitmap owner map now also includes the already-landed shared closure-validator review markers recorded in `Documentation/zigux/phase1-closure.md` and exact-required by `scripts/zigux/validate-phase1-closure.py`, but that validator's built-in self-test still does not remove and recheck `PHASE1_BITMAP_FINAL_PARTIAL_WORD_REVIEW` and `PHASE1_BITMAP_LINUX_ALIAS_REVIEW`, so the remaining same-lane shared-validation step is narrower than a generic Phase 1 reopen cue
...
- `tools/lib/bitmap.zig` owns its helper-local bitmap anchors, the committed bitmap replay keys in `zigux/tests/fixtures/phase1_helpers.json`, and the already-landed shared closure-validator bitmap review markers in `Documentation/zigux/phase1-closure.md` plus `scripts/zigux/validate-phase1-closure.py`. The shared helper-split and manifest exactness surface in `scripts/zigux/validate-phase1.py` is already closed, and the closure validator already exact-requires those bitmap review markers, but the closure validator self-test still needs to remove and recheck the final-partial-word and Linux-alias bitmap markers before this validator packet is fully parked. Until then, a nearby bitmap reopen should start from that one self-test hardening step or from fresh direct-anchor or committed shared replay drift, not from replaying an older generic validator cue.
...
- For `tools/lib/bitmap.zig`, do not replay the older closed exact-marker validator cue; the remaining shared-validation step is to harden `scripts/zigux/validate-phase1-closure.py` so its built-in self-test removes and rechecks `PHASE1_BITMAP_FINAL_PARTIAL_WORD_REVIEW` and `PHASE1_BITMAP_LINUX_ALIAS_REVIEW`, then leave the bitmap validator packet parked unless a fresh reread shows direct-anchor drift or committed shared replay drift.
- After that closure-validator self-test gap is closed, the next smallest same-lane shared-validation step is to teach `scripts/zigux/validate-phase1.py` to exact-check the four `PHASE1_BITMAP_DIRECT_OWNER`, `PHASE1_FIND_BIT_DIRECT_OWNER`, `PHASE1_RBTREE_DIRECT_OWNER`, and `PHASE1_STRING_DIRECT_OWNER` lines in this note before widening into any helper-local replay change.
