...
- the remaining shared bitmap validation follow-through is now closure-side anchor sync rather than validator-summary wording: `scripts/zigux/validate-phase1.py` already matches the live bitmap `review_packet_summary`, but `scripts/zigux/validate-phase1-closure.py` still trails the current bitmap helper-anchor list recorded in `zigux/tests/fixtures/phase1_helper_manifest.json`, so treat that exact closure-validator anchor sync as the smallest remaining shared bitmap gap before widening bitmap-local work again
...
- the bootstrap ledger already treats commits 15 through 18 as the bounded Phase 1 closure, closure-gate, workflow-viability, and installer-backed follow-through packet, so `Documentation/zigux/phase1-host-helper-lane-sequencing.md`, `scripts/zigux/install-zig.py`, `scripts/zigux/check-phase1-installer-review-surfaces.py`, and `scripts/zigux/check-phase1-installer-companion-checks.py` should be treated as parked closure-maintenance surfaces rather than as evidence that the helper tranche needs another helper-batch reopen
...
- `tools/lib/bitmap.zig` owns its helper-local bitmap anchors, the committed bitmap replay keys in `zigux/tests/fixtures/phase1_helpers.json`, and the already-landed shared closure-validator bitmap review markers in `Documentation/zigux/phase1-closure.md` plus `scripts/zigux/validate-phase1-closure.py`. The older final-partial-word, Linux-alias, fill-tail-clamp, and cross-word `bitmap.scnprintf()` closure follow-through is closed, and the earlier bitmap validator-summary wording drift is closed too. The only remaining shared bitmap follow-through is the closure-validator helper-anchor sync in `scripts/zigux/validate-phase1-closure.py`, so a nearby bitmap reopen should clear that exact shared validator gap before widening into new direct-anchor drift or committed shared replay drift.
...
- `tools/lib/string.zig` keeps `strscpy()` and `strscpyPad()` copy-and-pad semantics, memparse safety, matched-prefix-length and suffix boundary, sysfs newline-aware equality and lookup order through `sysfsStreq()`, `sysfs_streq()`, `sysfsMatchString()`, and `sysfs_match_string()`, C-string list lookup through `matchString()` and `match_string()`, counted-search `strnchr()`, embedded-NUL trim preservation, and moving-earliest-dirty-byte `memchrInv()` coverage helper-local, while the committed shared replay still owns embedded-NUL `replaceChar` parity bytes and the current string fixture keys. Current `master` already exact-checks the manifest's memparse, matched-prefix and suffix, C-string lookup, and counted-search anchors through `scripts/zigux/validate-phase1-closure.py`, so the next smallest same-family validator follow-up is the still-unchecked helper-local sysfs review-anchor quartet before any broader string-local reopen. Reopen only for that sysfs-anchor validator gap, other drift inside those direct anchors, or committed shared-field drift; do not reopen a generic closure-validator tightening pass.
...
- For `tools/lib/bitmap.zig`, do not replay the older closed exact-marker cue or the parked validator-summary cue; current `master` already exact-requires and self-tests `PHASE1_BITMAP_FINAL_PARTIAL_WORD_REVIEW` and `PHASE1_BITMAP_LINUX_ALIAS_REVIEW`, and `scripts/zigux/validate-phase1.py` already matches the live bitmap `review_packet_summary`.
...
- Fresh repo-first readback also shows the remaining shared bitmap validation work is closure-side rather than helper-local: `scripts/zigux/validate-phase1-closure.py` still trails the live bitmap helper-anchor list in `zigux/tests/fixtures/phase1_helper_manifest.json`, so repair that exact closure-validator anchor sync before reopening bitmap-local anchors.
...
- Treat the helper-specific next-safe-step markers below as the default tie-breakers whenever multiple older saved helper cues still exist in Memory; for `tools/lib/bitmap.zig`, let the explicit `PHASE1_BITMAP_NEXT_SAFE_STEP` line below override the older parked manifest note until the closure-validator anchor sync is no longer the live shared bitmap gap.
...
- `PHASE1_BITMAP_NEXT_SAFE_STEP=bitmap first rechecks whether scripts/zigux/validate-phase1-closure.py still trails the live bitmap helper-anchor list in zigux/tests/fixtures/phase1_helper_manifest.json; if that exact closure-validator anchor sync is still open, clear it before reopening bitmap-local anchors, otherwise keep bitmap parked unless a fresh reread finds new direct-anchor drift or committed shared replay drift; do not reopen the already-closed validator-summary packet by default`
