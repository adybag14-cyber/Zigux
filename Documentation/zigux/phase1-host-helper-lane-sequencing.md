...
- For `tools/lib/bitmap.zig`, do not replay older validator-first or make-route cue names by default; current authenticated reads now recover `Documentation/zigux/phase1-closure.md` and `scripts/zigux/validate-phase1-closure.py`, but `scripts/zigux/validate-phase1.py` and `zigux/Makefile` still do not, so bitmap follow-through should stay on the live manifest-backed direct anchors plus the restored closure packet unless those broader historical packet members become directly readable again.
