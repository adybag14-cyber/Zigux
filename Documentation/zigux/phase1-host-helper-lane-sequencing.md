...
- broader shared reminder surfaces now keep the missing installer-companion warning aligned on current `master`, but they still overstate one bench-side gap: `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, and `scripts/zigux/README.md` all still treat `scripts/zigux/check-phase1-bench.py` as historical even though current `master` ships that checker and `.github/workflows/zigux-bootstrap.yml` now self-tests it, so the shared reminder packet still needs one bench-wording follow-up before it can be treated as fully parked
- `PHASE1_DIRECT_OWNER_SHARED_REMINDER_GAPS=all four shared reminder surfaces now keep the missing installer companion warning aligned, but they still treat scripts/zigux/check-phase1-bench.py as historical even though current master ships it and bootstrap self-tests it, so the shared reminder packet still needs one bench-wording sync before it can stay fully parked`
- `PHASE1_DIRECT_OWNER_SHARED_REMINDER_ACTIVE_PACKET=Documentation/zigux/README.md,Documentation/zigux/review-checklist.md,zigux/tests/README.md,scripts/zigux/README.md,scripts/zigux/check-phase1-string-review-packet.py,scripts/zigux/check-phase1-direct-owner-markers.py`
- `PHASE1_DIRECT_OWNER_SHARED_REMINDER_ROUTE_SPLIT=Documentation/zigux/README.md, Documentation/zigux/review-checklist.md, zigux/tests/README.md, and scripts/zigux/README.md all keep the repo-reality warning explicit for the missing installer companion, but those same four surfaces still need one bench-wording sync for the now-shipped scripts/zigux/check-phase1-bench.py route, while scripts/zigux/check-phase1-string-review-packet.py and scripts/zigux/check-phase1-direct-owner-markers.py carry the live self-test-versus-guard split for the shipped Phase 1 reminder packet`
...
- `tools/lib/bitmap.zig` owns its helper-local bitmap anchors and the committed bitmap replay keys in `zigux/tests/fixtures/phase1_helpers.json`. Older closure-side bitmap note names remain historical packet members until current `master` exposes them to authenticated readback again, so nearby bitmap rereads should stay on the live manifest-backed anchors rather than reopening missing closure-validator surfaces by default.
...
- `PHASE1_BITMAP_DIRECT_OWNER=bitmap helper-local anchors plus the committed bitmap replay keys it already owns; older closure-side bitmap note names stay historical until current master exposes them again`
...
- For `tools/lib/bitmap.zig`, do not replay older closure-side or validator-route cue names by default; current authenticated reads for `Documentation/zigux/phase1-closure.md`, `scripts/zigux/validate-phase1.py`, `scripts/zigux/validate-phase1-closure.py`, and `zigux/Makefile` return missing content in this environment, so bitmap follow-through should stay on live manifest-backed direct anchors unless those historical packet members become directly readable again.
...
- `PHASE1_DIRECT_OWNER_SHARED_REMINDER_NEXT_STEP=finish the one remaining shared reminder sync for scripts/zigux/check-phase1-bench.py across Documentation/zigux/README.md, Documentation/zigux/review-checklist.md, zigux/tests/README.md, and scripts/zigux/README.md before reopening helper-local follow-through, unless one of the helper-specific next-safe-step markers below exposes a smaller same-family drift first`
...
- the already-landed OR-path proof in `test "find or bit returns the next set bit from either bitmap"` belongs to that same `find_bit` direct-anchor packet, so if it drifts, refresh the existing helper-family notes instead of widening shared replay ownership
