...
- `tools/lib/rbtree.zig` now keeps cached-root coverage helper-local while the committed shared replay owns duplicate-search parity through `matchIterator()` as well as `find()`, `findFirst()`, and `nextMatch()`. The dedicated `low_level_alias_anchor` in `zigux/tests/fixtures/phase1_helper_manifest.json` also keeps the low-level Linux-style alias proof named explicitly inside that same helper-local packet instead of leaving it implied only by the broader helper test list. The next widening, if any, is the dedicated cached-root leftmost-return fixture key only; do not batch a second widening into the same reopen step.
- `tools/lib/string.zig` keeps `strscpy()` and `strscpyPad()` copy-and-pad semantics, memparse safety, matched-prefix-length and suffix boundary, sysfs newline-aware equality and lookup order through `sysfsStreq()`, `sysfs_streq()`, `sysfsMatchString()`, and `sysfs_match_string()`, C-string list lookup through `matchString()` and `match_string()`, counted-search `strnchr()`, embedded-NUL trim preservation, and moving-earliest-dirty-byte `memchrInv()` coverage helper-local, while the committed shared replay still owns embedded-NUL `replaceChar` parity bytes and the current string fixture keys. Current `master` already exact-checks the manifest's memparse, matched-prefix and suffix, sysfs, C-string lookup, and counted-search anchor groups through `scripts/zigux/validate-phase1-closure.py`, but the broader closure validator still leaves the manifest's string review-summary scalars and helper-specific `next_safe_step_note` enforced only through `scripts/zigux/check-phase1-string-review-packet.py`. Until that one-file closure-validator sync lands, treat it as the immediate same-family shared-review follow-up; after it lands, reopen only for drift inside those direct anchors, for helper-local sysfs review-anchor alignment drift between the string review packet and the closure note, or for committed shared-field drift. The counted-search owner term here also covers the current `strnchrNul()` and `strnchrnul()` match-or-NUL boundary anchor already cataloged in `zigux/tests/fixtures/phase1_helper_manifest.json`, so future string-only rereads should keep that helper-local boundary proof inside the same counted-search packet instead of treating it as an unowned follow-up beside `strnchr()`.
- `Documentation/zigux/phase1-closure.md` should keep that still-open split explicit too: `scripts/zigux/check-phase1-string-review-packet.py`, `python3 scripts/zigux/check-phase1-string-review-packet.py --self-test`, and `python3 scripts/zigux/check-phase1-string-review-packet.py` remain the dedicated exact-count guard for the manifest's string review-summary scalars and helper-specific `next_safe_step_note` until `scripts/zigux/validate-phase1-closure.py` promotes those markers into the broader closure validator.
...
- Until that one-file string summary sync lands, do not treat the `PHASE1_STRING_NEXT_SAFE_STEP` marker below as proof the shared-validation gap is closed; it resumes once `scripts/zigux/validate-phase1-closure.py` exact-checks the string review-summary scalars and helper-specific `next_safe_step_note` that current `master` already carries in `zigux/tests/fixtures/phase1_helper_manifest.json`.
- Current `master` already exact-checks the string manifest's memparse, prefix and suffix, sysfs, lookup, and `strnchr()` anchor groups through `scripts/zigux/validate-phase1-closure.py`, but it still stops short of the manifest's string review-summary scalars and helper-specific `next_safe_step_note`. The smallest same-lane string follow-up therefore remains the one-file closure-validator hardening that promotes those markers into `scripts/zigux/validate-phase1-closure.py`; after that lands, leave string parked unless those direct anchors drift, the string review packet and closure note fall out of sync, or committed shared string replay fields drift.
