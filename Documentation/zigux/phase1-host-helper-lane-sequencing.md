...
- the bitmap owner map now also includes the already-landed shared closure-validator bitmap review markers recorded in `Documentation/zigux/phase1-closure.md`, including the fill-tail-clamp marker, but current `master` still keeps the cross-word `bitmap.scnprintf()` proof outside that closure note, so treat that closure packet as note-maintenance-only rather than fully parked until one bounded bitmap-local closure-evidence repair lands
...
- `PHASE1_BITMAP_NEXT_SAFE_STEP=bitmap stays parked on helper semantics and validator-summary work, but a fresh reread may still take one bounded closure-evidence or note repair if Documentation/zigux/phase1-closure.md continues to under-describe the helper-local cross-word bitmap.scnprintf proof`
