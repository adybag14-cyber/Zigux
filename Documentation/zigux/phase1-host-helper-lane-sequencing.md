...
- the bitmap owner map now also includes the already-landed shared closure-validator bitmap review markers recorded in `Documentation/zigux/phase1-closure.md`, but current `master` still keeps newer bitmap-local anchors such as the fill-tail-clamp and cross-word `bitmap.scnprintf()` proofs outside that closure note, so treat that closure packet as note-maintenance-only rather than fully parked until one bounded bitmap-local closure-evidence repair lands
...
- `tools/lib/bitmap.zig` owns its helper-local bitmap anchors, the committed bitmap replay keys in `zigux/tests/fixtures/phase1_helpers.json`, and the already-landed shared closure-validator bitmap review markers in `Documentation/zigux/phase1-closure.md` plus `scripts/zigux/validate-phase1-closure.py`. The older final-partial-word and Linux-alias closure follow-through is closed, and the earlier bitmap validator-summary wording drift is closed too, but the bitmap closure-note packet still under-describes newer direct anchors such as the fill-tail-clamp and cross-word `bitmap.scnprintf()` proofs. A nearby bitmap reopen should therefore prefer one bounded closure-evidence or note repair before treating the whole bitmap closure packet as fully parked.
...
- `PHASE1_STRING_DIRECT_OWNER=string keeps memparse safety, matched-prefix-length and suffix boundary, C-string list lookup through matchString() and match_string(), counted-search strnchr, embedded-NUL trim preservation, and moving-earliest-dirty-byte memchrInv coverage helper-local while the committed shared replay owns embedded-NUL replaceChar parity bytes and the current string fixture keys`
...
- For `tools/lib/bitmap.zig`, do not replay the older closed final-partial-word or Linux-alias closure cue; those markers are already landed. If a fresh bitmap reread still finds the closure packet behind the live helper-local surface, keep the follow-through to one bounded closure-evidence or note repair naming the remaining direct anchors before reopening any validator-script work.
...
- `PHASE1_BITMAP_NEXT_SAFE_STEP=bitmap stays parked on helper semantics and validator-summary work, but a fresh reread may still take one bounded closure-evidence or note repair if Documentation/zigux/phase1-closure.md continues to under-describe live direct anchors such as the fill-tail-clamp or cross-word bitmap.scnprintf proofs`
...
- `PHASE1_STRING_NEXT_SAFE_STEP=string reopens only for direct-anchor drift inside memparse, matched-prefix-length or suffix boundary, matchString()/match_string() C-string list lookup, counted-search strnchr, embedded-NUL trim, or moving-earliest-dirty-byte memchrInv coverage, or for committed replaceChar or current string fixture drift; do not reopen a generic closure-validator pass`
