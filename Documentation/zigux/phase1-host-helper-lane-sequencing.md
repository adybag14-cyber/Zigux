...
- the closure-side validator packet still needs its own wording reread: `scripts/zigux/validate-phase1-closure.py` and `scripts/zigux/check-phase1-direct-owner-markers.py` still carry the older bitmap `review_packet_summary` ordering, so treat that as the smallest remaining shared-validation truthfulness gap before widening bitmap-local work again
...
- `tools/lib/bitmap.zig` owns its helper-local bitmap anchors, the committed bitmap replay keys in `zigux/tests/fixtures/phase1_helpers.json`, and the already-landed shared closure-validator bitmap review markers in `Documentation/zigux/phase1-closure.md` plus `scripts/zigux/validate-phase1-closure.py`. The older final-partial-word, Linux-alias, fill-tail-clamp, and cross-word `bitmap.scnprintf()` closure follow-through is closed, and the earlier bitmap validator-summary wording drift is closed too. A nearby bitmap reopen should therefore start from new direct-anchor drift or committed shared replay drift instead of another closure-note repair.
...
- Fresh repo-first readback also shows the remaining shared-validation work is closure-side rather than helper-local: `scripts/zigux/validate-phase1-closure.py` and `scripts/zigux/check-phase1-direct-owner-markers.py` still carry the older bitmap `review_packet_summary` ordering, so repair that validator-side wording drift before reopening bitmap-local anchors.
