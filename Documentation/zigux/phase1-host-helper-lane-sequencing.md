...
- `tools/lib/string.zig` keeps `strscpy()` and `strscpyPad()` copy-and-pad semantics, memparse safety, matched-prefix-length and suffix boundary, sysfs newline-aware equality and lookup order through `sysfsStreq()`, `sysfs_streq()`, `sysfsMatchString()`, and `sysfs_match_string()`, C-string list lookup through `matchString()` and `match_string()`, counted-search `strnchr()`, embedded-NUL trim preservation, and moving-earliest-dirty-byte `memchrInv()` coverage helper-local, while the committed shared replay still owns embedded-NUL `replaceChar` parity bytes and the current string fixture keys. Current `master` already exact-checks the manifest's memparse, matched-prefix and suffix, sysfs, C-string lookup, counted-search anchor groups, string review-summary scalars, and helper-specific `next_safe_step_note` through `scripts/zigux/validate-phase1-closure.py`, so leave string parked unless a fresh reread finds drift inside those direct anchors, helper-local sysfs review-anchor alignment drift between the string review packet and the closure note, or committed shared-field drift. The counted-search owner term here also covers the current `strnchrNul()` and `strnchrnul()` match-or-NUL boundary anchor already cataloged in `zigux/tests/fixtures/phase1_helper_manifest.json`, so future string-only rereads should keep that helper-local boundary proof inside the same counted-search packet instead of treating it as an unowned follow-up beside `strnchr()`.
- `Documentation/zigux/phase1-closure.md` should keep the dedicated string-review checker explicit too: `scripts/zigux/check-phase1-string-review-packet.py`, `python3 scripts/zigux/check-phase1-string-review-packet.py --self-test`, and `python3 scripts/zigux/check-phase1-string-review-packet.py` remain the exact-count guard for the broader string packet across the direct helper anchors, the manifest summaries, the shared replay markers, the shared fixture keys, and the helper-specific `next_safe_step_note` even though `scripts/zigux/validate-phase1-closure.py` now exact-checks the manifest's string review-summary scalars and that `next_safe_step_note` inside the closure validator too.
...
- `PHASE1_STRING_NEXT_SAFE_STEP` below is now backed by the landed closure-validator promotion: `scripts/zigux/validate-phase1-closure.py` exact-checks the string review-summary scalars and helper-specific `next_safe_step_note` that current `master` carries in `zigux/tests/fixtures/phase1_helper_manifest.json`, so use that marker as a parked tie-breaker unless a fresh reread finds new string-only drift.
- Current `master` already exact-checks the string manifest's memparse, prefix and suffix, sysfs, lookup, counted-search, review-summary scalar, and helper-specific `next_safe_step_note` markers through `scripts/zigux/validate-phase1-closure.py`, so the smallest same-lane string follow-up is no longer another closure-validator pass. Leave string parked unless those direct anchors drift, the string review packet and closure note fall out of sync, or committed shared string replay fields drift.
