...
- `zigux/tests/fixtures/phase1_helper_manifest.json` currently records helper-local `next_safe_step_note` entries for `tools/lib/rbtree.zig` and `tools/lib/string.zig`, but not yet for `tools/lib/bitmap.zig` or `tools/lib/find_bit.zig`; until current `master` lands those two matching manifest notes, treat `PHASE1_BITMAP_NEXT_SAFE_STEP` and `PHASE1_FIND_BIT_NEXT_SAFE_STEP` below as the authoritative tie-breakers instead of reopening those helper families from older saved cues or shared-validator paths.
