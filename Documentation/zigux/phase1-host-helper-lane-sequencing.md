...
- the bitmap owner map now also includes the already-landed shared closure-validator bitmap review markers recorded in `Documentation/zigux/phase1-closure.md`, and current `master` now exact-requires and self-tests those markers in `scripts/zigux/validate-phase1-closure.py`, so the next same-lane shared-validation step is narrower still: leave that closure-validator packet parked and move only to the remaining direct-owner marker exactness gap in `scripts/zigux/validate-phase1.py`
- current repo evidence narrows the bitmap follow-through even further: `zigux/tests/fixtures/phase1_helper_manifest.json` now records the empty-bitmap caller-buffer preservation wording in the bitmap review packet summary, but `scripts/zigux/validate-phase1.py` still carries the older bitmap summary text, so the remaining bitmap-local closure drift is that one validator-summary refresh rather than any new helper or closure-marker expansion
- the helper-specific direct-owner marker lines in this note still need direct reread until `scripts/zigux/validate-phase1.py` exact-checks them too
...
- `tools/lib/bitmap.zig` owns its helper-local bitmap anchors, the committed bitmap replay keys in `zigux/tests/fixtures/phase1_helpers.json`, and the already-landed shared closure-validator bitmap review markers in `Documentation/zigux/phase1-closure.md` plus `scripts/zigux/validate-phase1-closure.py`. The closure-validator packet is parked again, but the shared helper-split and manifest exactness surface in `scripts/zigux/validate-phase1.py` still has one bitmap-local truthfulness gap: its baked-in bitmap review-packet summary predates the now-landed empty-bitmap caller-buffer preservation wording in `zigux/tests/fixtures/phase1_helper_manifest.json`. A nearby bitmap reopen should therefore start with that one summary refresh before widening back to direct-anchor or committed shared replay drift.
