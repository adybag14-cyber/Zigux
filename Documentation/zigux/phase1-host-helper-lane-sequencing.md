...
- broader shared reminder surfaces still lag that checker on current `master`, so treat `scripts/zigux/check-phase1-direct-owner-markers.py`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml` as the authoritative owner-map packet until a dedicated docs-root or tests-root sync lands
