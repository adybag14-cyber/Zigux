...
- scope: owned command-list handling, bounded filesystem-backed command discovery and executable filtering, injected command-source filtering, injected raw-`PATH` splitting, injected terminal-dimensions resolution, and pure pretty-print emission planning
- product boundary:
...
The Phase 8 roadmap explicitly names `tools/lib/subcmd/help.c` as a userspace-adjacent tooling anchor, recommends `tools/lib/subcmd/*.zig` as the first Zigux destination for this tranche, and uses this packet to prove Zigux inside serious repo-hosted tooling instead of only tiny helpers.

This lane keeps the shipped `help.zig` starter slice aligned with the stable command-list manipulation, bounded filesystem-backed directory iteration, injected raw-`PATH` handling, terminal-dimensions resolution, and output-stable pretty-print emission already carved out of `help.c`, because those helper-first surfaces still give Zigux output-stable tooling behavior that is easier to validate honestly than direct directory walking, environment inspection, terminal probing, or a full `cmd_help()`-adjacent CLI surface.
...
- bounded filesystem-backed command discovery through `addExecutableEntriesFromDir()` and `loadCommandListFromDir()`, including prefixed directory-entry filtering plus stat-backed executable checks
- injected command-source loading that models `load_command_list()` exec-path priority, PATH de-duplication, and cross-list exclusion without widening into the full `cmd_help()` CLI surface
...
- filesystem-backed temp-directory replays cover bounded directory iteration, stat-backed executable filtering, and exec-path-priority behavior without widening into the whole `cmd_help()` CLI surface
...
- layout planning preserves the same column math used before printing, including the single-column fallback for narrow terminals, environment-driven column selection through the injected terminal helper, the empty-list row contract, and stable blank-cell rendering when a discovered command name is empty
...
- full `opendir()` or `readdir()` parity for command discovery across the whole `cmd_help()`-adjacent CLI surface
