...
- the shared Phase 9 runtime-loader study packet must keep `Documentation/zigux/phase9-runtime-loader-gap-survey.md`, `Documentation/zigux/phase9-runtime-pilot-lane-sequencing.md`, `zigux/tests/phase9_build.zig`, `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, `zigux/Makefile`, `scripts/zigux/check-phase9-loader-substrate-plan.py`, and the sample-only `samples/zigux/runtime_trace_events_loader.zig` scaffold explicit together so returned loader-substrate evidence is not restated as absent backlog while `kernel/workqueue.c` and `kernel/trace/ring_buffer.c` remain study-only anchors rather than delivery targets
