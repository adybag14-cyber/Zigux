...
- allocator-backed `parse_int_array()` coverage that preserves Linux's count-prefixed output layout, reuses the existing `get_options()` base and sign semantics, keeps the current explicit leading-`+` rejection visible through the shared Phase 7 gate, stops at the first C-string terminator, truncates wide values to `i32`, and returns a no-entry error when the input contains no parseable integers
