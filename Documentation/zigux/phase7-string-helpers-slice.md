...
- the bounded `samples/zigux/string_helpers_sample.zig` replay for descriptor ownership, lifecycle transitions, newline-tolerant matching, binary size rendering, compact no-space-no-bytes formatting, one in-place replacement preview, one fixed-width padding preview, one bounded ASCII case-conversion preview, one exact-fit unescape destination proof, one bounded no-terminator escape window proof, deterministic only-selected newline escaping, and append-selected newline hex escaping through the shared Phase 7 build
- the manifest-backed `zigux/tests/phase7_string_helpers_sample_survey.zig` gate so the helper, shared fixtures, sample replay, and slice note stay aligned in one reviewable packet after the added compact-format, bounded ASCII case-conversion, exact-fit unescape boundary, bounded no-terminator escape window, buffer-mutation preview, only-selected newline escaping, and append-selected escape proofs
