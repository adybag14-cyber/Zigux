...
- `Documentation/zigux/review-checklist.md` should continue to name the same shared Phase 6 leaf-helper packet through `Documentation/zigux/README.md`, `Documentation/zigux/phase6-helper-parity-catalog.md`, `Documentation/zigux/phase6-perf-gate-survey.md`, `scripts/zigux/validate-phase6.py`, `scripts/zigux/check-phase6-docs-root-external-parity.py`, `scripts/zigux/check-phase6-base64-catalog-evidence.py`, `scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py`, `zigux/tests/phase6_helper_parity_manifest.json`, `zigux/tests/phase6_build.zig`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml`, so the parked four-helper scope, shared `phase6-perf` replay, and the docs-root, base64-catalog, plus checksum-hexdump perf-marker fail-closed helpers stay visible in reviewer guidance instead of living only in packet-local notes.
