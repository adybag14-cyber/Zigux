...
- `zigux/tests/phase6_base64_c_harness.c`, `zigux/tests/phase6_base64_c_casegen.zig`, and `zigux/tests/phase6_base64_c_parity.zig` replay that same representative base64 surface through `python3 scripts/zigux/check-phase6-base64-c-parity.py`; `python3 scripts/zigux/check-phase6-base64-c-parity.py --self-test` currently reports `PHASE6_BASE64_C_PARITY_SELF_TEST_CASE_COUNT=7`, keeping the parity script's missing-path guards, generated build template, sorted-output normalization, and representative-output fail-closed drift checks reviewable without a live toolchain replay, and the committed case generator still rebuilds the C include payload from `zigux/tests/fixtures/phase6_base64_vectors.zig` before the current `PHASE6_BASE64_C_PARITY_CASES=96` spot check runs.
