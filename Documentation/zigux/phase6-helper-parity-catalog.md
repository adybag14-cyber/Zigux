...
- verified head: `24f7deaec43d83cdd09d54af0affc974459781ba`
...
- `zigux/tests/phase6_checksum_c_parity.zig` and `zigux/tests/fixtures/phase6_checksum_c_harness.c` now replay 27 direct C-vs-Zig checksum parity cases through `python3 scripts/zigux/check-phase6-checksum-c-parity.py`, and `python3 scripts/zigux/check-phase6-checksum-c-parity.py --self-test` now reports `PHASE6_CHECKSUM_C_PARITY_SELF_TEST_CASE_COUNT=12` while proving the committed exact 27-line expected surface, missing-path guards, and C-vs-Zig mismatch handling for the live compute, pseudo-header, carry-discipline, direct `add16` and `sub16`, and incremental replacement packet.
...
- `python3 scripts/zigux/check-phase6-checksum-c-parity.py --self-test` is the current tool-free reviewability check for the bounded checksum external parity script before the live `zig` plus `cc` replay runs, and it now reports `PHASE6_CHECKSUM_C_PARITY_SELF_TEST_CASE_COUNT=12` while `python3 scripts/zigux/check-phase6-checksum-c-parity.py` currently passes with `PHASE6_CHECKSUM_C_PARITY_CASES=27`.
...
- `Documentation/zigux/review-checklist.md` should continue to name the same shared Phase 6 leaf-helper packet through `Documentation/zigux/README.md`, `Documentation/zigux/phase6-helper-parity-catalog.md`, `Documentation/zigux/phase6-perf-gate-survey.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, `scripts/zigux/validate-phase6.py`, `scripts/zigux/check-phase6-docs-root-external-parity.py`, `scripts/zigux/check-phase6-base64-c-parity.py`, `scripts/zigux/check-phase6-base64-catalog-evidence.py`, `scripts/zigux/check-phase6-bsearch-c-parity.py`, `scripts/zigux/check-phase6-checksum-c-parity.py`, `scripts/zigux/check-phase6-checksum-hexdump-perf-markers.py`, `scripts/zigux/check-phase6-hexdump-c-parity.py`, `zigux/tests/phase6_helper_parity_manifest.json`, `zigux/tests/phase6_build.zig`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml`, so the parked four-helper scope, shared `phase6-perf` replay, the scripts-root and tests-root inventories, the four external portability hooks, and the docs-root, base64-catalog, plus checksum-hexdump perf-marker fail-closed helpers stay visible in reviewer guidance instead of living only in packet-local notes.
