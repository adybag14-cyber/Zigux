...
- the current numeric thresholds are `max_slowdown_pct = 175` for `16B-plain` and `max_slowdown_pct = 425` for `32B-ascii-g2`, checked against the committed `fixtures.prepareExpectedLine(...)` reference path.
...
- the current hexdump perf packet measures helper output against the committed `fixtures.prepareExpectedLine(...)` reference path and rejects regressions above `max_slowdown_pct = 175` for `16B-plain` and `max_slowdown_pct = 425` for `32B-ascii-g2`.
