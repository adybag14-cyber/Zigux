...
- `python3 scripts/zigux/check-phase6-checksum-c-parity.py --self-test` is the current tool-free reviewability check for the bounded checksum external parity script before the live `zig` plus `cc` replay runs, and `python3 scripts/zigux/check-phase6-checksum-c-parity.py` currently passes with `PHASE6_CHECKSUM_C_PARITY_CASES=21`.
