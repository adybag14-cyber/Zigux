...
- verified head: `15af8bad92abdcbeb8f5d9e991d945be877f0617`
...
- `zigux/tests/fixtures/phase6_base64_vectors.zig` is the current static base64 corpus with 22 standard encode vectors, 24 variant encode vectors, 22 standard decode vectors, 16 variant decode vectors, 28 invalid decode vectors, 2 committed perf payload cases, and 10 committed perf replay cases, including a repeated-alternate-alphabet multi-quartet sample family for the URL-safe and IMAP branches plus padded malformed tail-bit rejects across the standard, URL-safe, and IMAP variants.
- `zigux/tests/fixtures/phase6_base64_c_harness.c`, `zigux/tests/phase6_base64_c_casegen.zig`, and `zigux/tests/phase6_base64_c_parity.zig` replay that same representative base64 surface through `python3 scripts/zigux/check-phase6-base64-c-parity.py`; the built-in `--self-test` path now covers the parity script's missing-path guards, generated build template, sorted-output normalization, and representative-output fail-closed drift checks without requiring a live toolchain replay, and the committed case generator rebuilds the transient `zigux/tests/fixtures/phase6_base64_c_generated_cases.inc` include payload from `zigux/tests/fixtures/phase6_base64_vectors.zig` before the current `PHASE6_BASE64_C_PARITY_CASES=112` spot check runs.
...
- `zigux/tests/phase6_checksum_c_parity.zig` and `zigux/tests/fixtures/phase6_checksum_c_harness.c` now replay 22 direct C-vs-Zig checksum parity cases through `python3 scripts/zigux/check-phase6-checksum-c-parity.py`, and `python3 scripts/zigux/check-phase6-checksum-c-parity.py --self-test` currently reports `PHASE6_CHECKSUM_C_PARITY_SELF_TEST_CASE_COUNT=6`.
...
- `python3 scripts/zigux/validate-phase6.py --self-test` currently reports `PHASE6_VALIDATOR_SELF_TEST_CASE_COUNT=23` and now fail-closes on catalog-head drift, script-README wording, catalog marker drift, perf-survey marker drift, hexdump-slice perf wording drift, shared-gates drift, helper-entry drift, required-test inventory drift, and determinism-evidence drift across the shipped base64, bsearch, checksum, and hexdump packet before the broader Phase 6 review surface claims alignment.
