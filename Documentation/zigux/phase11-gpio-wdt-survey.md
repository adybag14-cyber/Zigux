...
- `Documentation/zigux/phase11-gpio-wdt-validation-matrix.md` now records the shared replay surface, the current teardown-facing stop evidence, the explicit teardown-summary evidence, the starter-local `nowayout` policy bookkeeping, and the first bounded register-device call evidence so the lane's validation posture does not hide inside the starter, the registration-plan follow-up, or the tests alone
