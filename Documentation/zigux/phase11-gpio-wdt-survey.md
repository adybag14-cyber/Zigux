...
The next honest bounded step inside the same lane is to pick one tiny drvdata-order checkpoint that stays immediately adjacent to the new descriptor preflight and timeout-property boundaries, while still avoiding live GPIO and platform glue until the handoff bookkeeping is no longer the blocker.
