...
- `Documentation/zigux/phase11-gpio-wdt-validation-matrix.md` now records the shared replay surface and the current metadata-only registration evidence so the lane's validation posture does not hide inside the starter, the registration-plan follow-up, or the tests alone
