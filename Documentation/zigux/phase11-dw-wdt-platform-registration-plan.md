...
`drivers/watchdog/dw_wdt_verify.zig`, or
...
leaving the wider driver, verify-helper, driver-test, and older reminder stack
unpromoted.
...
3. keep the current manifest, owner notes, survey gate, validation matrix, survey note, scaffold, restart helper, PM helper pair, and DesignWare checker pair aligned before reopening broader helper-backed claims
...
- claims that the broader direct driver, verify-helper, driver-test, slice, or teardown-note stack has returned on this read path without a fresh reread
