...
The current helper stays intentionally narrow:
  * preserves the already-landed managed `__devm_ioremap()` lifetime bookkeeping so the lane can distinguish retained release records from free-on-failure cleanup, keep the `devm_iounmap()` pointer match exact, and keep the explicit `devm_ioremap_uc()`, `devm_ioremap_wc()`, and `devm_ioremap_np()` wrapper planners reviewable without claiming live MMIO side effects
