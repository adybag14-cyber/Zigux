...
The next honest bounded step in this same lane is to keep the work helper-first and add one small in-tree reviewability or packet-level proof that this already-landed `devm_arch_phys_wc_add()` token planner remains documented as shipped without widening into live mappings, generic devres groups, or cross-subsystem device-resource state.
