...
- records the managed request-region, remap, WC token, and WC memtype reservation failure branches so the lane stays explicit about when the helper would surface `-EINVAL`, `-EBUSY`, or `-ENOMEM`, and when a failed remap, token add, or memtype reservation would avoid keeping a detach-time release record
...
The next honest bounded step in this same lane is to keep the work helper-first and add one small adjacent write-combined resource wrapper around `devm_ioremap_resource_wc()` without widening into live mappings, generic devres groups, or cross-subsystem device-resource state.
