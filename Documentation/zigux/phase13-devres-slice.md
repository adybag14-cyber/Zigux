...
This slice does not claim live `devres_alloc_node()` ownership, actual MMIO mappings, resource-region side effects, ioport helpers, devres groups, `devm_of_iomap()`, or the broader managed resource-family teardown behavior from `lib/devres.c`.

The next honest bounded step in this same lane is to keep the work helper-first and add one small adjacent planner around `devm_of_iomap()` or another `ioremap` family wrapper without widening into live mappings, generic devres groups, or cross-subsystem device-resource state.
