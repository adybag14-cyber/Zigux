...
Fresh follow-up reread on 2026-05-26 narrowed the remaining same-lane drift more precisely. Authenticated connector rereads still returned `404` for `samples/zigux/kobject_example.zig`, `zigux/tests/phase5_kobject_example_manifest.json`, and `zigux/tests/phase5_kobject_example_survey.zig`, so the mixed packet above still stands. The smallest open shared-surface gap is therefore no longer inside this survey note itself; it sits in the remaining shared reminder surfaces and checker text that still overstate the owner path as direct authenticated proof instead of keeping it shared-reminder-backed beside the public-tree-backed companion pair.
...
If the lane reopens soon, start with `Documentation/zigux/phase5-kobject-sample-survey.md`, then compare `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, and `scripts/zigux/check-phase5-review-guide-surface.py` against `Documentation/zigux/phase5-sample-review-guide.md`, `Documentation/zigux/phase5-sample-lane-sequencing.md`, `samples/zigux/README.md`, and `zigux/tests/README.md`. Repair only the remaining shared-surfaces that still describe `samples/zigux/kobject_example.zig` as direct authenticated proof instead of a shared-reminder-backed owner path alongside the current public-tree-backed manifest-plus-survey companion pair, and leave sample behavior, runtime families, and freeze-map boundaries untouched.