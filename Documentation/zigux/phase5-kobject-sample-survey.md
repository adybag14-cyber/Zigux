...
Current `master` also still ships no `samples/zigux/*rbtree*` Phase 5 reference sample. This note keeps the stale `rbtree-style` shorthand corrected by pointing rbtree evidence back to the separate Phase 7 helper bundle in `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, and `zigux/tests/phase7_build.zig` instead of treating a missing Phase 5 sample as an open kobject-side port gap.
...
- does the survey note still keep the no-`samples/zigux/*rbtree*` Phase 5 boundary explicit by pointing reviewers back to `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, and `zigux/tests/phase7_build.zig` instead of reviving the stale `rbtree-style` shorthand for this lane?
...
- current `master` still ships no `samples/zigux/*rbtree*` Phase 5 reference sample, so this survey corrects the older `rbtree-style` shorthand by keeping rbtree evidence under the separate Phase 7 helper bundle instead of treating it as an open sample port inside the kobject family
