...
- `PHASE5_SURVEYED_COMMIT=4c43b2a09196e51947cb5221b94d41ed8cd97b79`
...
- it uses a tiny `init()` -> `registerAttributes()` -> `showValue()` or `storeValue()` -> `exit()` lifecycle so the initialized-but-not-registered ownership boundary, the initialized-only abandonment path, the registered teardown path, and the explicit ownership-state transitions remain reviewable without runtime substrate
...
- `runAnchorReplay()` requires `init()` first, claims attribute ownership through exactly one `register_runs` increment, moves the ownership state from `initialized_without_registered_attributes` to `registered_attributes_owned`, registers exactly three attributes, and leaves the sample in the `registered` stage with attributes accessible
...
- the initialized-but-not-registered stage keeps the active attribute count at `0`, reports the `initialized_without_registered_attributes` ownership state, and rejects show or store calls until `registerAttributes()` claims ownership
- the initialized-only `exit()` path returns an `abandoned_before_registration` teardown summary, keeps `register_runs` at `0`, moves to the `released_after_exit` ownership state, and still leaves the sample exited with no active attributes
...
- registered `exit()` returns a teardown summary, moves the ownership state from `registered_attributes_owned` to `released_after_exit`, clears the tracked values, removes the active attribute count, and the post-`exit()` `init()`, `registerAttributes()`, `showValue()`, and `storeValue()` calls all remain rejected

## Latest verification snapshot

Current sample behavior was re-verified against `master` commit `4c43b2a09196e51947cb5221b94d41ed8cd97b79` on 2026-05-01 with the attached Zig toolchain.
...
Those live runs confirmed that the shipped kobject sample still matches the exact bounded checks above: the in-memory replay keeps the Linux `foo`/`baz`/`bar` attribute order and shared `0664` mode pattern explicit, makes the single `register_runs` ownership claim visible by moving from `initialized_without_registered_attributes` to `registered_attributes_owned`, keeps the initialized-but-not-registered stage at an active attribute count of `0` while rejecting show or store access, reports `abandoned_before_registration` for the initialized-only `exit()` path before moving to `released_after_exit`, keeps the shared sample-root catalog and review-checklist prompts aligned with the manifest-backed packet, and clears registered teardown state while rejecting later `init()`, `registerAttributes()`, `showValue()`, and `storeValue()` calls.
...
- do the ownership checks still keep the initialized-but-not-registered stage explicit by requiring zero active attributes, the `initialized_without_registered_attributes` ownership state, and no show or store access until `registerAttributes()` claims ownership?
...
- this approved ownership-and-lifetime idiom is now pinned to `PHASE5_SURVEYED_COMMIT=4c43b2a09196e51947cb5221b94d41ed8cd97b79` so the survey note, manifest-backed checks, shared sample-root catalog, shared review checklist, and contributor review path all point at the same inspected `master` head
...
- contributor guidance still needs to keep the in-memory directory, unnamed-group shape, attribute-array order, initialized-only abandonment path, registered teardown path, explicit ownership-state transitions, and pre-registration ownership boundary visibly separate from real sysfs or module substrate claims and from the later runtime pilot families
