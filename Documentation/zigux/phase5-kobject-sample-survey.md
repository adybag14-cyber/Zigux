...
If the lane reopens soon, start with `Documentation/zigux/phase5-kobject-sample-survey.md`, `Documentation/zigux/README.md`, and `Documentation/zigux/review-checklist.md` and verify only whether the current sample-root owner, direct sample-owned replay, bounded attr-group companion, focused attr-group replay, dedicated attr-group survey guard, shared checklist, shared build-route companion, and public-tree-backed manifest-plus-survey companions still keep the same split before widening anything else.
