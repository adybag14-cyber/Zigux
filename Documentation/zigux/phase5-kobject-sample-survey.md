...
Current `master` also still ships no `samples/zigux/*string*`, `samples/zigux/*cmdline*`, or `samples/zigux/*rbtree*` Phase 5 reference sample. This note keeps those helper boundaries explicit by pointing string, cmdline, and rbtree evidence back to the separate Phase 7 helper bundles in `Documentation/zigux/phase7-string-helpers-slice.md`, `lib/string_helpers.zig`, `zigux/tests/phase7_string_helpers.zig`, `Documentation/zigux/phase7-cmdline-slice.md`, `lib/cmdline.zig`, `zigux/tests/phase7_cmdline.zig`, `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, and `zigux/tests/phase7_build.zig` instead of treating missing Phase 5 samples as open kobject-side port gaps.
