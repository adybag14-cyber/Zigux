...
Fresh shared-surface reread on 2026-05-27 narrowed the remaining drift again. `Documentation/zigux/phase5-sample-lane-sequencing.md`, `samples/zigux/README.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` now keep that mixed packet truthful, but `Documentation/zigux/review-checklist.md` still uses the older direct-proof wording for `samples/zigux/kobject_example.zig`, and `scripts/zigux/check-phase5-review-guide-surface.py` still encodes that older checklist marker. The smallest open shared-surface gap is therefore no longer broad reminder drift; it is the still-unrepaired checklist-plus-checker pair that overstates the owner path as direct authenticated proof instead of keeping it shared-reminder-backed beside the public-tree-backed companion pair.
...
If the lane reopens soon, start with `Documentation/zigux/phase5-kobject-sample-survey.md`, then compare `Documentation/zigux/review-checklist.md` and `scripts/zigux/check-phase5-review-guide-surface.py` against `Documentation/zigux/phase5-sample-lane-sequencing.md`, `Documentation/zigux/phase5-sample-review-guide.md`, `samples/zigux/README.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md`. Repair only the remaining checklist-plus-checker pair that still describes `samples/zigux/kobject_example.zig` as direct authenticated proof instead of a shared-reminder-backed owner path alongside the current public-tree-backed manifest-plus-survey companion pair, and leave sample behavior, runtime families, and freeze-map boundaries untouched.