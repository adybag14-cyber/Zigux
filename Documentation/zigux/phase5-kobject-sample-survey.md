...
The landed sample remains an approved Phase 5 ownership-and-lifetime idiom. `ownershipSummary()` now keeps replay readiness reviewable alongside the cold, initialized, registered, and exited stages, so reviewers do not have to infer when `runAnchorReplay()` is valid from method guards alone.
