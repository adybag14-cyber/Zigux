...
- maintenance-mode parked: the Phase 15 packet remains bounded, the freeze-in-C blocker posture is unchanged, and the next visible same-lane maintenance follow-through is still the smaller tests-root plus scripts-root manifest-pair undercount rather than any deeper Architecture Council status change or freeze-map policy reopen
...
Keep the Phase 15 governance lane in maintenance mode unless the shared Phase 15 replay drifts again, the shared review checklist drops the explicit workflow-backed or direct shared-build replay markers, the dedicated `phase15-test` route disappears from the top-level readiness packet or the tests-root reminder, one of the two dedicated `phase15-validate` checker routes disappears, or one of the four recorded deep-core blocker dispositions changes. If another same-lane maintenance follow-through is needed before any blocker-posture change, start by making `zigux/tests/README.md` and `scripts/zigux/check-phase15-scripts-readme-alignment.py` keep `zigux/tests/phase15_handoff_next_steps_manifest.json` and `zigux/tests/phase15_readiness_gate_manifest.json` explicit beside the already-named dedicated `make -C zigux phase15-test` route so this parked readiness note, the tests-root summary, and the shared scripts-root guard return to exact agreement.
