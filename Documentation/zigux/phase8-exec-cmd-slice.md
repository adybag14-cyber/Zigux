...
- the injected environment wrapper keeps `PREFIX`, the configured exec-path environment key, and the resulting `PATH` value aligned, including the inherited-empty-`PATH` trailing-`:` edge, the skipped empty explicit exec-path segment, and the inherited-empty exec-path environment fallback to the configured path
