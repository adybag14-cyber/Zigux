...
- `phase11-hvc-hv-ops-layout-proof`: `zigux/tests/phase11_hvc_hv_ops_layout_proof.zig`, `zigux/tests/phase11_hvc_hv_ops_layout_build.zig`, and `drivers/tty/hvc/hvc_console.h` keep the `struct hv_ops` callback table directly readable as the surviving proof shard in this header-boundary family.
...
- It does not claim returned `watchdog_info` or `winsize` layout proofs, a restored shared build route, restored checker coverage, tty registration parity, notifier execution, watchdog-core integration, or whole-Phase-11 closure.
