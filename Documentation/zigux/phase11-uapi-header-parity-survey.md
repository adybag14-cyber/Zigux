...
This survey note records the shared Phase 11 header boundary after re-reading `master` `a0bfa2fc2d924b16bcb342294bd4456dac3632f5`.
...
- the shared Phase 11 header survey gate now also replays that `hvc_console.zig` snapshot directly, pinning the mirrored console-limit constants, exported instantiate or alloc or remove or poll or resize or kick surface, the default `notifier_*_irq()` helper exports, and the bounded `hv_ops` callback booleans so the shared note is backed by code instead of prose alone
