...
- `phase11-header-boundary-checker`: `scripts\zigux/check_phase11_header_boundary_packet.zig` now fail-closes on the survey note and validation matrix through `zig run check_phase11_header_boundary_packet.zig --self-test` and `zig run check_phase11_header_boundary_packet.zig`, but that returned checker should still be treated as note-side evidence only until a future reread proves the shared manifest, survey source, or build route returned beside it.
