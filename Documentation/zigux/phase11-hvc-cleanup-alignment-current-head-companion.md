...
It only records that current `master` keeps the returned direct HVC starter, the standalone targetless-unregister witness pair, and the smaller proof-backed continuity packet reviewable while the broader starter-depth packet and dedicated survey checker are not both back in direct readback in this run.
