...
- current authority: the committed helper manifest, this closure note, the live owner-map reminders, and the shared tests-root smoke route remain the trustworthy current-master sources for the closed helper tranche
...
- `PHASE1_CURRENT_GAP_PACKET=scripts/zigux/validate-phase1.py,scripts/zigux/validate-phase1-closure.py,scripts/zigux/check-phase1-parity.py,scripts/zigux/check-phase1-bench.py,zigux/tests/phase1_helpers.zig,zigux/tests/phase1_bench.zig,zigux/tests/fixtures/phase1_bench_expectations.json,zigux/tests/fixtures/phase1_helpers_c_harness.c,zigux/Makefile`
...
That route keeps a minimal shared import-and-wire smoke check alive for the current helper packet while the dedicated closure validator is still absent on current `master`.
...
The next bounded same-lane follow-through is to add the dedicated current-master-safe closure validator and then sync one shared reminder surface against this restored closure note.
