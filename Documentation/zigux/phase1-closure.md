...
- current authority: the committed helper manifest, this closure note, the narrow closure validator, the shipped bench checker, the shipped shared reminder checker, the live owner-map reminders, and the shared tests-root smoke route remain the trustworthy current-master sources for the closed helper tranche
...
The current smallest helper-family tie-breaker inside that packet is the bitmap direct-anchor route: keep `tools/lib/bitmap.zig` parked unless a fresh reread finds drift in the manifest-backed fill-tail clamp, copy-alias, cross-word or truncation `scnprintf`, empty-buffer, allocator-reset, zero-bit, or Linux-style alias anchors, drift in live caller-window or multiword-tail `xorBits()` clamp proofs, or drift in the already-committed bitmap replay fields, and do not reopen the older validator-first or Phase 1 make-route names by default. Current `master` still keeps the helper-local zero-bit logical and Linux-style alias witnesses directly in `tools/lib/bitmap.zig`, so fresh bitmap rereads should treat those tests as part of the same live direct-anchor packet even when the manifest-backed reminder surface is focusing attention on a narrower named anchor list.
