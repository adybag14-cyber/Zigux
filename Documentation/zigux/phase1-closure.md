...
Current `master` also ships `scripts/zigux/check-phase1-rbtree-review-packet.py` as a current-master-safe packet guard: it exact-checks the live rbtree review packet across `tools/lib/rbtree.zig`, `Documentation/zigux/phase1-host-helper-lane-sequencing.md`, this closure note, `zigux/tests/fixtures/phase1_helper_manifest.json`, `zigux/tests/fixtures/phase1_helpers.json`, and `zigux/tests/phase1_host_tools_smoke.zig`, so the committed duplicate-search and exact cached-leftmost-return packet stays aligned with the still-helper-local cached-root anchors.
...
Current `master` also now keeps helper-local byte-copy and pad coverage explicit through `memcpyAndPad()`, `memcpy_and_pad()`, `strtomem()`, and `strtomem_pad()`, with direct tests for requested-prefix copying, first-NUL truncation, and destination-tail padding, so treat those anchors as part of the same string-local review packet until dedicated shared fixture keys land.
