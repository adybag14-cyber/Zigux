...
The next bounded same-lane follow-through is to sync `Documentation/zigux/README.md` with this restored closure note so the docs root explicitly names `zigux/tests/build.zig`, `zigux/tests/phase1_host_tools_smoke.zig`, and `.github/workflows/zigux-bootstrap.yml` as live Phase 1 reminder companions already carried by the other reminder surfaces, rather than widening back into the older validator-first or replay-side closure stack.

The current smallest helper-family tie-breaker inside that packet is the bitmap direct-anchor route: keep `tools/lib/bitmap.zig` parked unless a fresh reread finds drift in the manifest-backed fill-tail clamp, copy-alias, cross-word or truncation `scnprintf`, empty-buffer, or allocator-reset anchors, drift in live caller-window or multiword-tail `xorBits()` clamp proofs, or drift in the already-committed bitmap replay fields, and do not reopen the older validator-first or Phase 1 make-route names by default. Current `master` still keeps the helper-local zero-bit logical and Linux-style alias tests directly in `tools/lib/bitmap.zig`, but the manifest-backed reminder surface no longer treats them as named reopen cues, so fresh bitmap rereads should leave them as background coverage unless the narrower direct-anchor packet itself drifts.

- `PHASE1_NEXT_SAFE_STEP=sync Documentation/zigux/README.md with the restored closure note so the docs root explicitly names zigux/tests/build.zig, zigux/tests/phase1_host_tools_smoke.zig, and .github/workflows/zigux-bootstrap.yml as live Phase 1 reminder companions already carried by the other reminder surfaces before reopening any helper-family tie-breaker or widening back into the older validator-first or replay-side closure stack.`
