...
Current `master` also still carries extra helper-local `memtostr()`, `memtostrPad()`, and `memtostr_pad()` coverage in `tools/lib/string.zig`, but the shipped manifest-backed string review packet currently spells only the `memcpyAndPad()`, `memcpy_and_pad()`, `strtomem()`, and `strtomem_pad()` copy-fill anchors explicitly. Treat the `memtostr*` tests as helper-local reread evidence instead of as committed shared-fixture or validator-owned requirements until dedicated packet entries land.
