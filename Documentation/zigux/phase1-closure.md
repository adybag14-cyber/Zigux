...
The direct helper-local duplicate-range anchor `test "rbtree matchIterator walks the duplicate range in order"` must also stay review-visible whenever `matchIterator()` changes. The shared Phase 1 replay does not consume committed iterator-specific fixture fields yet, so this direct helper-local test remains the owning proof that iterator-based duplicate walks stay aligned with the already-shared `findFirst()` and `nextMatch()` replay contract instead of drifting into a separate traversal path.

The direct helper-local cached-root follow-up anchors `test "rbtree replaceNodeCached keeps non-leftmost leftmost unchanged"`, `test "rbtree eraseInitCached detaches nodes while keeping cached leftmost aligned"`, and `test "rbtree eraseInitCached clears singleton cached roots before reseed"` are also owning proofs for now. The shared Phase 1 replay does not consume committed cached-root replacement, detach, or reseed fixture fields directly yet, so reviewers must keep those three helper-local anchors explicit whenever `replaceNodeCached()` or `eraseInitCached()` changes.
