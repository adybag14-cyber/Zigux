...
- `tools/lib/string.zig` direct Zig unit coverage also exercises `strim` and `strreplace` wrapper aliases alongside `trimSpaces`, `skipSpaces`, `removeSpaces`, and `replaceChar` so the alias surface cannot silently drift from the underlying helpers.
