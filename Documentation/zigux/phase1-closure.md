...
- `tools/lib/find_bit.zig` direct Zig unit coverage also keeps same-word shared-bit start masking aligned so inclusive starts can return the current shared bit, later starts skip earlier same-word shared matches, and tail scans still clamp to `nbits`.
