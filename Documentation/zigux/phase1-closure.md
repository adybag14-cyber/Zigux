...
Current `master` also ships `scripts/zigux/check-phase1-rbtree-direct-anchors.py` as a helper-local current-head guard: it exact-checks the ordered Linux-style alias packet, the low-level alias packet, and the cached-root insert-miss, leftmost-sync, alias, replacement, singleton-erase, detach, reseed, and direct entry-point anchors directly in `tools/lib/rbtree.zig` while the broader cached-root replay packet remains bounded.
...
- `PHASE1_RBTREE_DIRECT_ANCHOR_GUARD=python3 scripts/zigux/check-phase1-rbtree-direct-anchors.py exact-checks ordered Linux-style alias, low-level alias, and cached-root insert-miss, leftmost-sync, alias, replacement, singleton-erase, detach, reseed, and direct entry-point anchors directly in tools/lib/rbtree.zig`
