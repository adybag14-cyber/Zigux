...
- `tools/lib/rbtree.zig` direct Zig unit coverage also keeps `find()` returning a matching node or null while `findFirst()` stays pinned to the leftmost duplicate match for equal keys.
