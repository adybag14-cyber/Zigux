...
- `tools/lib/string.zig` direct Zig unit coverage also keeps the string-facing `memparse()` route aligned by delegating through `cmdline.memparse()` while preserving decimal, hexadecimal, suffix-bearing, invalid, and binary-unit-tail inputs including optional trailing B forms without changing the parsed value or rest pointer contract.
