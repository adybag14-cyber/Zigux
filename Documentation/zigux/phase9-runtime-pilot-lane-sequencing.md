...
- surviving shared workflow rerun guard: `.github/workflows/zigux-bootstrap.yml` reruns `zig test samples/zigux/runtime_trace_events.zig`, `zig test samples/zigux/runtime_trace_events_unregistered_gate.zig`, `zig test samples/zigux/runtime_trace_events_exit_rollback_guard.zig`, and `zig test samples/zigux/runtime_trace_events_registration_reentry_gate.zig` while staying a shared repo-level workflow surface rather than a dedicated `phase9-*` build route
...
- no current shared reminder surface should describe `zigux/tests/phase9_build.zig`, the broader shared `zigux/tests/runtime_*` replay family beyond the returned trace-events survey witness, `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, or the older `samples/zigux/runtime_*_loader.zig` scaffolds as shipped current-`master` evidence unless a fresh repo reread proves they have returned; `.github/workflows/zigux-bootstrap.yml` should stay framed as the returned shared repo-level workflow rerun guard for the narrow trace-events sample family rather than as dedicated `phase9-*` build-route proof, and `zigux/Makefile` should stay framed only as a readable non-owner surface whose live body still lacks dedicated `phase9-*` runtime-pilot routes
...
- keep `.github/workflows/zigux-bootstrap.yml` explicit as the returned shared repo-level workflow rerun guard for the surviving trace-events packet without treating it as dedicated `phase9-*` build-route proof
...
2. Keep the shipped `scripts/zigux/check-phase9-trace-events-runtime-packet.py` guard and the returned `.github/workflows/zigux-bootstrap.yml` rerun guard visible whenever this note names the surviving direct runtime sample packet.
