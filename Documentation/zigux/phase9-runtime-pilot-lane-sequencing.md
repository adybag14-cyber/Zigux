...
- `make -C zigux phase9-runtime-loader-shared-tests` remains the focused shared-loader replay for the runtime-loader facade, runtime-loader contract, allocator/init-flow proof bundle, selftest-complete exit-after-prepare proof, and loader-gap survey
...
- direct readback now also shows `zigux/tests/runtime_loader_selftest_complete_exit_parity.zig` keeps the shared selftest-complete exit-after-prepare proof explicit across atomic64, bitmap, trace-events, and kretprobe, so the shared owner-map note should keep that replay visible beside the allocator/init-flow and loader-gap proofs instead of undercounting the current shared loader packet
