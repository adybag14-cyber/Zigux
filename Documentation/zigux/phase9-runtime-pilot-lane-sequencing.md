...
- `P9-L11`: `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, `zigux/tests/runtime_loader_allocator_init_flow.zig`, `scripts/zigux/check-phase9-build-only-surface.py`, `zigux/tests/phase9_build.zig`, and the focused `phase9-runtime-loader-shared-tests` step own the shared Phase 9 loader-handoff packet. Shared loader-facade, contract, allocator/init-flow, checker-local reviewability, and bundled-route truthfulness stay here instead of being borrowed by any one pilot-family lane. The current unresolved same-packet proof gap is still the missing `runtime_loader.keepsRequestStateAndPlanExplicit(request, .prepared, request.plan)` reassertions after rejected `requestRuntimeLoad()` prepared-plan drift branches in `zigux/tests/runtime_loader_allocator_init_flow.zig`: the live test still omits those explicitness checks after the loader-not-required, module-name-drift, anchor-drift, entry-symbol-drift, exit-symbol-drift, selftest-hook-drift, and init-flow-drift branches, while the allocator-handoff drift branch already keeps dedicated `prepared_plan` versus live `plan` equality checks.
