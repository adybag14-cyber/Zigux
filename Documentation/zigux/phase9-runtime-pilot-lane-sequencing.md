...
- `zigux/tests/runtime_loader_allocator_init_flow.zig` now keeps prepared requests pinned across loader-not-required, module-name drift, anchor drift, entry-symbol drift, exit-symbol drift, selftest-hook drift, and init-flow drift; allocator-handoff drift still remains indirect there through the neighboring facade and contract tests, so the next same-lane follow-through should start with that one remaining shared replay proof instead of reopening broader summaries
...
2. shared loader lane: start with the remaining allocator-handoff prepared-plan-drift proof inside `zigux/tests/runtime_loader_allocator_init_flow.zig`; once that lands, prefer the next smallest lifecycle-state or invalid-transition proof inside `zigux/kernel/runtime_loader.zig` or `zigux/kernel/runtime_loader_contract.zig`, and if a new bounded validation surface lands, wire it through `scripts/zigux/check-phase9-build-only-surface.py` before reopening broader request-contract, allocator/init-flow, or build-only reviewability work
