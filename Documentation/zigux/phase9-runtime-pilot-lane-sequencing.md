...
- `zigux/tests/runtime_loader_allocator_init_flow.zig` still leaves one bounded shared-loader proof unfinished on current `master`: `requestRuntimeLoad()` proves prepared requests stay explicit across loader-not-required, module-name drift, selftest-hook drift, and init-flow drift, but the same replay has not yet pinned that prepared-request behavior across anchor drift plus entry-symbol and exit-symbol drift
- with the broad docs-root, scripts-root, and tests-root reminders all parked on current `master`, the next same-lane follow-through should publish the saved `runtime_loader_allocator_init_flow.zig` request-drift proof before widening into another shared-loader lifecycle gap or reopening any broader summary surface
...
2. shared loader lane: first publish the saved `zigux/tests/runtime_loader_allocator_init_flow.zig` request-drift proof so the shared replay also fail-closes on anchor drift plus entry-symbol and exit-symbol drift; after that lands, prefer the next smallest lifecycle-state or invalid-transition proof inside `zigux/kernel/runtime_loader_contract.zig` or `zigux/tests/runtime_loader_allocator_init_flow.zig`; if a new bounded validation surface lands, wire it through `scripts/zigux/check-phase9-build-only-surface.py` before reopening broader request-contract, allocator/init-flow, or build-only reviewability work
