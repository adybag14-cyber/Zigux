...
- the returned bitmap module gate and diff gate now stay inside that same bounded packet instead of the older repo-reality-gap bucket, while the returned cold-stage guard remains sample-root proof only until the shared build shard or a dedicated shared reminder surface names it directly
