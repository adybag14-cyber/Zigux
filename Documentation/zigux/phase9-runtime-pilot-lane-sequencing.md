...
- treat that review-checklist prompt and `scripts/zigux/check-phase9-build-only-surface.py` as one coupled release-discipline packet: a checklist-side Phase 9 reminder refresh is incomplete unless the shared checker still requires this sequencing note together with the shipped self-test-hook ownership marker instead of only trusting the surrounding reminder surfaces
- `scripts/zigux/check-phase9-build-only-surface.py` already exact-counts the older freeze-map prompt follow-through on current `master`, but the next honest shared-loader checker step is still to keep the checklist-side owner map and the shipped self-test-hook ownership marker fail-closed together rather than letting the review packet drift back to prose-only alignment
