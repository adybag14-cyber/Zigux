...
- `zigux/tests/runtime_loader_allocator_init_flow.zig` now also keeps the shared request-drift packet parked on current `master`: `requestRuntimeLoad()` proves prepared requests stay explicit not only across loader-not-required and module-name drift, but also across anchor drift, entry-symbol drift, exit-symbol drift, selftest-hook drift, and init-flow drift, so the shared loader lane no longer has a parked allocator/init-flow follow-through inside that replay
- with the broad docs-root, scripts-root, and tests-root reminders all parked on current `master`, the next same-lane follow-through should prefer the smallest shared-loader lifecycle-proof gap inside `zigux/kernel/runtime_loader_contract.zig` or `zigux/tests/runtime_loader_allocator_init_flow.zig` before reopening any broader summary surface
