...
- `make -C zigux phase9-runtime-loader-shared-tests` remains the focused shared-loader replay for the runtime-loader facade, runtime-loader contract, allocator/init-flow proof bundle, selftest-complete exit-after-prepare proof, lifecycle-boundary guard, trace-events loader-substrate-drift proof, and loader-gap survey
...
- direct readback now shows `Documentation/zigux/review-checklist.md` and `scripts/zigux/README.md` already defer the exact shared owner map back to this sequencing note, keep the current shared loader packet literal, and keep later-phase inventory outside the bounded shared runtime-pilot reminder packet
- direct readback now also shows `Documentation/zigux/README.md` and `zigux/tests/README.md` keep the exact owner map and blocked publication boundary deferred back to this sequencing note while also naming the shipped selftest-complete exit parity, lifecycle-boundary guard, and trace-events loader-substrate-drift replays beside allocator/init-flow plus loader-gap coverage, so docs-root and tests-root reminder follow-through is now parked on current `master` unless one of those shared loader markers regresses again
...
- direct readback now shows `zigux/tests/runtime_loader_allocator_init_flow.zig` already keeps the prepared-plan drift replay explicit across rejected `requestRuntimeLoad()` calls for `requires_runtime_substrate`, `module_name`, `anchor`, `entry_symbol`, `exit_symbol`, selftest-hook, and init-flow drift, while allocator-handoff drift stays covered by dedicated prepared-plan-versus-live-plan equality checks, so the older shared-lane publication handoff for the seven direct explicitness assertions is now stale and future shared follow-through should move to reminder or checker alignment instead of reopening the same replay body
- direct readback now also shows the allocator-handoff prepared-plan drift branch in `zigux/tests/runtime_loader_allocator_init_flow.zig` now carries `runtime_loader.keepsRequestStateAndPlanExplicit(request, .prepared, request.plan)` beside those older prepared-plan-versus-live-plan equality checks, so allocator-handoff drift should now be treated as part of the landed explicitness packet rather than as a surviving replay gap
...
- direct readback now also shows `zigux/tests/runtime_loader_selftest_complete_exit_parity.zig` keeps the shared selftest-complete exit-after-prepare proof explicit across atomic64, bitmap, trace-events, and kretprobe, so the shared owner-map note should keep that replay visible beside the allocator/init-flow, lifecycle-boundary, trace-events loader-substrate-drift, and loader-gap proofs instead of undercounting the current shared loader packet
- direct readback now also shows `zigux/tests/runtime_loader_lifecycle_boundary_guard.zig` keeps the shared request-state and registration-boundary guard explicit, so the shared owner-map note should keep that replay visible beside the allocator/init-flow, selftest-complete exit-after-prepare, trace-events loader-substrate-drift, and loader-gap proofs instead of undercounting the current shared loader packet
- direct readback now also shows `zigux/tests/runtime_trace_events_loader_substrate_drift.zig` now rides the same `phase9-runtime-loader-shared-tests` bundle while staying trace-events-local for pilot-family ownership, so the shared owner-map note should name that shared replay as part of the live loader-facing route without collapsing the trace-events survey packet into shared ownership
...
- direct readback now also shows `Documentation/zigux/review-checklist.md`, `Documentation/zigux/README.md`, and `zigux/tests/README.md` all keep the blocked `.modinfo`, `MODULE_ALIAS()`, `modules.alias`, `modules.order`, `modules.builtin`, module install-root publication, and `depmod` script or manifest state boundary explicit, so the broader shared reminder packet is now parked unless one of those shared summaries regresses around that blocked boundary or starts reclaiming family-local scope
...
- the current trace-events follow-through stays trace-events-local in `P9-L10`: the manifest, survey note, module-slice note, dedicated `zigux/tests/runtime_trace_events_loader_substrate_drift.zig` replay, survey gate, and the module-level selftest-ready failed-exit plus rejected re-selftest rollback proof stay family-local while the shared loader packet remains adjacent only
- the current kretprobe follow-through is the manifest-backed loader-plan, survey-gate lifecycle, and tracing proof sync tracked in `P9-L16`, and the older family-local `P9-L13` and `P9-L10` labels should not be reused unless repo evidence explicitly moves ownership back
...
Current repo-first readback keeps the reviewer-facing shared reminder packet aligned on `master`: `Documentation/zigux/review-checklist.md`, `Documentation/zigux/README.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` all defer the exact owner map and blocked publication boundary back here without reclaiming family-local survey, manifest, module-slice, or survey-gate work. Keep this lane parked on future one-file reminder rereads or sequencing-note refreshes when the shared loader-facing packet or owner map moves again.
...
The shared freeze-map and study-boundary enforcement survey governance packet now stays on `P9-L13` beside the shared reminder surfaces. Keep roadmap-versus-freeze-map wording, study-only boundary enforcement, and repo-level anti-overlap guidance there instead of routing pilot-family manifest or survey upkeep through that lane.
...
- `P9-L10`: owns the current runtime trace-events manifest, survey note, module-slice note, and survey-gate packet. Keep the dedicated loader-substrate-drift replay plus the module-level selftest-ready failed-exit and rejected re-selftest rollback proof inside that same trace-events-local packet while the shared loader-facing reminder packet remains adjacent only.
- `P9-L16`: owns the current runtime kretprobe manifest-backed loader-plan, survey-gate lifecycle, and tracing proof follow-through. Keep kretprobe-local proof there while the older family-local `P9-L13` and `P9-L10` labels remain historical only unless repo evidence explicitly moves ownership back.
...
2. If a shared reminder surface stops deferring the exact owner map, convenience-target names, or blocked publication boundary back to this note, refresh the smallest one-file reminder surface that drifted.
3. If the shared reminder packet already defers correctly to this note, refresh the smallest shipped shared summary that still undercounts the live shared loader packet or drifts around the blocked module-metadata and depmod-publication boundary and the stale repo-root loader inventory, starting with `Documentation/zigux/README.md`, then `zigux/tests/README.md`, while keeping `scripts/zigux/README.md` parked unless a later reread shows it reclaiming family-local owner-map detail again.
4. If `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, or `zigux/tests/README.md` reclaims pilot-family survey, manifest, module-slice, or survey-gate work, repair that one file at a time instead of widening into pilot behavior or checker growth.
5. If a different shared reminder surface starts overstating blocked publication surfaces or inventing shared validation routes, repair that one file at a time instead of widening into pilot behavior or checker growth.
6. If the shared loader-facing family changes or a direct shared-loader proof lands, refresh this sequencing note before later shared reminder passes inherit stale owner labels.
7. Only after the shared reminder packet is truthful again should any family-local survey note be refreshed to describe shared loader adjacency.
...
If a scheduled run is assigned Phase 9 shared-governance work such as `P9-L13`, keep the run inside shared reminder truthfulness, repo-reality recording, and next-step narrowing. Do not consume pilot-family backlog just because those local files are easy to read from the same packet.
