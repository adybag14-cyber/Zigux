...
- direct readback now also shows `Documentation/zigux/review-checklist.md`, `Documentation/zigux/README.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` still spell that same shared loader packet in repo-root `zigux/kernel/runtime_loader*.zig` terms, so the next shared reminder pass should stay on that exact stale inventory instead of reopening loader replay bodies or any family-local packet
- direct readback now also shows `Documentation/zigux/review-checklist.md` still omits the blocked `.modinfo`, `MODULE_ALIAS()`, `modules.alias`, `modules.order`, `modules.builtin`, module install-root publication, and `depmod` script or manifest state boundary from its shared Phase 9 prompt, so the smallest current same-lane follow-through is the reviewer-facing checklist sync before any broader docs-root or scripts-root reminder pass
...
2. If the shared reminder packet already defers correctly to this note, refresh the smallest shipped shared summary that still drifts around the blocked module-metadata and depmod-publication boundary and the stale repo-root loader inventory, starting with `Documentation/zigux/review-checklist.md` so the blocked `.modinfo`, `MODULE_ALIAS()`, `modules.alias`, `modules.order`, `modules.builtin`, module install-root publication, and `depmod` script or manifest state boundary becomes explicit there before any broader docs-root, scripts-root, or tests-root reminder pass.
