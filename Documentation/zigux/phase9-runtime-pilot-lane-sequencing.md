...
- no current shared reminder surface should describe `zigux/tests/phase9_build.zig`, the broader shared `zigux/tests/runtime_*` replay family beyond the returned trace-events survey witness, `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, or the older `samples/zigux/runtime_*_loader.zig` scaffolds as shipped current-`master` evidence unless a fresh repo reread proves they have returned; `.github/workflows/zigux-bootstrap.yml` should stay framed as the returned shared repo-level workflow rerun guard for the narrow trace-events sample family plus its family-local survey witness rather than as dedicated `phase9-*` build-route proof, and `zigux/Makefile` should stay framed only as a readable non-owner surface whose live body still lacks dedicated `phase9-*` runtime-pilot routes
...
- the next honest shared Phase 9 move is no longer another speculative reminder rewrite; it is a fresh repo-first reread if one of the surviving shared reminder surfaces, the returned family-local survey witness, or the surviving direct trace-events sample family changes again
...
1. Keep the roadmap-versus-repo relationship explicit: Phase 9 still targets runtime modules, selftest hooks, and lifecycle parity, but current `master` now shows only a narrow surviving trace-events packet plus one family-local `zigux/tests/runtime_*` witness rather than the older shared runtime-loader family.
...
6. Keep `Documentation/zigux/phase9-runtime-trace-events-survey.md`, `Documentation/zigux/phase9-runtime-trace-events-module-slice.md`, `zigux/tests/runtime_trace_events_manifest.json`, and `zigux/tests/runtime_trace_events_survey.zig` visible as the returned family-local `zigux/tests/runtime_*` witness whenever this note summarizes the currently shipped runtime sample family, and keep that narrow witness framed as sample-local pilot-module reviewability rather than proof that the broader shared runtime-loader family returned.
7. Do not describe `zigux/tests/phase9_build.zig`, the broader shared `zigux/tests/runtime_*` replays beyond the returned trace-events survey witness, shared runtime-loader kernel files, or the older `samples/zigux/runtime_*_loader.zig` scaffolds as shipped evidence unless a fresh repo reread proves they have returned; keep `.github/workflows/zigux-bootstrap.yml` explicit only as the returned shared repo-level rerun guard for the narrow trace-events family, and keep `zigux/Makefile` explicit only as a readable non-owner surface whose live body still lacks dedicated `phase9-*` routes.
8. Treat the older runtime bitmap packet as backlog-only support material until a fresh repo reread proves the exact file family returned; do not let those historical bitmap names become present runtime proof or extra Phase 5 evidence by implication.
9. Treat the removed `phase9-runtime-loader-gap-survey` note plus the missing `runtime_loader_gap_*` survey companions as retired wider-family metadata and depmod vocabulary, not as current shared-owner proof for the surviving narrow packet; only restore those references after a fresh repo reread proves the exact paths returned.
10. Treat stale reminder overclaim as the active blocker before reopening checker-local or runtime-behavior work.
11. Keep `kernel/workqueue.c` and `kernel/trace/ring_buffer.c` framed as freeze-map study-only anchors, and route that shared anchor inventory back through `Documentation/zigux/phase15-study-only-anchor-accounting.md`, instead of letting the surviving runtime-pilot packet imply deeper runtime-substrate or bridge readiness across those boundaries.
12. Refresh one shared reminder surface at a time when the tree changes again.
13. If the broader shared runtime-loader packet returns later, reread the exact file family before widening this note back out.

## Recommended next-step order

1. Re-read `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, `samples/zigux/README.md`, `zigux/tests/README.md`, `scripts/zigux/check-phase9-review-checklist-phase-boundaries.py`, `scripts/zigux/check-phase9-trace-events-runtime-packet.py`, `.github/workflows/zigux-bootstrap.yml`, `Documentation/zigux/phase9-runtime-trace-events-survey.md`, `Documentation/zigux/phase9-runtime-trace-events-module-slice.md`, `zigux/tests/runtime_trace_events_manifest.json`, `zigux/tests/runtime_trace_events_survey.zig`, `samples/zigux/runtime_trace_events.zig`, `samples/zigux/runtime_trace_events_unregistered_gate.zig`, `samples/zigux/runtime_trace_events_exit_rollback_guard.zig`, `samples/zigux/runtime_trace_events_registration_reentry_gate.zig`, and `zigux/Makefile` before touching the shared reminder packet again.
2. If one of those surviving surfaces drifts, trim only the smallest one-file reminder surface that overclaims removed loader evidence, drops the narrow trace-events checker-backed packet, or forgets the returned family-local survey witness.
