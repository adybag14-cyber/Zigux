...
- `zigux/tests/runtime_loader_allocator_init_flow.zig` now keeps prepared requests pinned across loader-not-required, module-name drift, anchor drift, entry-symbol drift, exit-symbol drift, selftest-hook drift, and init-flow drift; allocator-handoff drift still remains indirect in the shared loader packet through the neighboring facade and contract tests, so the next same-lane follow-through should start in `zigux/kernel/runtime_loader.zig` or `zigux/kernel/runtime_loader_contract.zig` instead of reopening broader summaries or replaying this allocator/init-flow file
...
- with the broad docs-root, scripts-root, and tests-root reminders all parked on current `master`, the next same-lane follow-through should stay inside `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, or `zigux/tests/runtime_loader_allocator_init_flow.zig` and begin with the remaining allocator-handoff prepared-plan-drift proof before reopening a broader summary surface
...
2. shared loader lane: start with the remaining allocator-handoff prepared-plan-drift proof inside `zigux/kernel/runtime_loader.zig` or `zigux/kernel/runtime_loader_contract.zig`; once that lands, prefer the next smallest lifecycle-state or invalid-transition proof in the other shared loader file, and if a new bounded validation surface lands, wire it through `scripts/zigux/check-phase9-build-only-surface.py` before reopening broader request-contract, allocator/init-flow, or build-only reviewability work
