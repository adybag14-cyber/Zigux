...
- direct readback now also shows the dedicated `Phase 9 flow` heading in `zigux/tests/README.md` keeps running into `zigux/tests/phase10_build.zig` and later inventories after the shared runtime-pilot packet, so tests-root wording is no longer undercounting the loader-gap packet but still overstates same-lane scope until a bounded trim stops that section at the Phase 9 surfaces
...
2. If the shared reminder packet already defers correctly to this note, refresh the smallest shipped shared summary that still drifts around the blocked module-metadata and depmod-publication boundary and the stale repo-root loader inventory, starting with `Documentation/zigux/README.md` and then trimming `zigux/tests/README.md` if the dedicated `Phase 9 flow` section still carries `zigux/tests/phase10_build.zig` or later phase inventory after the shared runtime-pilot packet.
3. If `zigux/tests/README.md` still carries `zigux/tests/phase10_build.zig` or later phase inventory inside the `Phase 9 flow` section after that docs-root pass, treat the next shared tests-root follow-through as a one-file trim that stops the section at the shared loader packet plus the four Phase 9 survey gates.
4. If a different shared reminder surface starts reclaiming family-local survey, manifest, module-slice, or survey-gate work, repair that one file at a time instead of widening into pilot behavior or checker growth.
