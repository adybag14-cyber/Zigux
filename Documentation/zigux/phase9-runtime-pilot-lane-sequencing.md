...
- `Documentation/zigux/phase9-runtime-bitmap-survey.md`, `Documentation/zigux/phase9-runtime-bitmap-module-slice.md`, and `zigux/tests/runtime_bitmap_module.zig` now also own the stage-specific bitmap exit-parity proof: initialized-stage and selftest-complete snapshots preserve `first_set`, `first_zero`, `weight`, and `nbits` across `exit()`, while post-exit mutation, re-init, and double-exit attempts fail closed; that lifecycle proof belongs to the bitmap pilot family and should not be flattened into shared loader ownership
...
- `scripts/zigux/check-phase9-build-only-surface.py` and `zigux/kernel/runtime_loader_contract.zig` also keep the blocked module-metadata and depmod boundary fail-closed on current `master`: the shared checker still requires the contract test that keeps `.modinfo`, `module_alias`, `module_aliases`, `modules.alias`, `modules.order`, `modules.builtin`, module install-root, and `depmod` script, manifest, and alias state outside the shared request contract, so later reminder or checklist refreshes should not soften that boundary into a shipped module-metadata or depmod bridge claim
...
3. bitmap lane: only if the goal is to refine the already-landed top-bit companion, the stage-specific lifecycle-exit parity packet, or the survey-local `phase9_build.zig` marker alignment inside the runtime bitmap family
