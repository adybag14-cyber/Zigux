...
- direct readback now also shows `zigux/tests/runtime_loader_allocator_init_flow.zig` already keeps the prepared-plan drift replay explicit across rejected `requestRuntimeLoad()` calls for `requires_runtime_substrate`, `module_name`, `anchor`, `entry_symbol`, `exit_symbol`, allocator-handoff, selftest-hook, and init-flow drift, so the older shared-lane publication handoff for that exact proof is now stale and future shared follow-through should move to reminder or checker alignment instead of reopening the same replay body
...
2. If the shared reminder packet already defers correctly to this note, refresh this note first when the active packet-local owner labels change, and then decide whether the shipped `scripts/zigux/check-phase9-build-only-surface.py` guard should fail closed on the exact `P9-L04`/`P9-L08`/`P9-L10`/`P9-L13` split before another broader shared reminder pass.
