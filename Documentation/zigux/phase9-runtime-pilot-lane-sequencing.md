...
6. Treat the removed `phase9-runtime-loader-gap-survey` note plus the missing `runtime_loader_gap_*` survey companions as retired wider-family metadata and depmod vocabulary, not as current shared-owner proof for the surviving narrow packet; only restore those references after a fresh repo reread proves the exact paths returned.
