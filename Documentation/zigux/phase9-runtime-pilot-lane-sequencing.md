...
- the current atomic64 follow-through is the manifest-backed survey-versus-module-slice packet tracked in `P9-L16`, with the adjacent freeze-boundary evidence refresh staying coupled through `P9-L17`
- the current bitmap follow-through stays bitmap-local in the live runtime bitmap packet; because the recent scheduled work around that family is still phase-mismatched, use `Documentation/zigux/phase9-runtime-bitmap-survey.md`, `Documentation/zigux/phase9-runtime-bitmap-module-slice.md`, `zigux/tests/runtime_bitmap_manifest.json`, and `zigux/tests/runtime_bitmap_survey.zig` as the authority instead of reusing an older stale `P9-*` owner label from shared-governance notes
...
- `P9-L16`: owns the current runtime atomic64 manifest-backed survey-versus-module-slice packet. Family-local manifest, survey, and directly coupled module-slice wording should stay there, while the adjacent `P9-L17` freeze-boundary evidence stays coupled without replacing that packet-local owner.
- the current runtime bitmap packet should be treated as its own authority for owner-map recovery until the phase-mismatched scheduler labels around that family are cleaned up again; use `Documentation/zigux/phase9-runtime-bitmap-survey.md`, `Documentation/zigux/phase9-runtime-bitmap-module-slice.md`, `zigux/tests/runtime_bitmap_manifest.json`, and `zigux/tests/runtime_bitmap_survey.zig` directly instead of reusing the stale `P9-L08` label from older shared-governance notes.
