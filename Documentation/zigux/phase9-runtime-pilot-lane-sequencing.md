...
- direct readback now also shows the sharpest surviving shared reminder drift has narrowed to inventory wording: `scripts/zigux/README.md` and `zigux/tests/README.md` still omit `zigux/tests/runtime_loader_gap_survey.zig` from their shared Phase 9 packet lists even though `zigux/tests/phase9_build.zig` routes that survey through the same `phase9-runtime-loader-shared-tests` bundle, so the next safe shared follow-through is a one-reminder-at-a-time packet sync instead of reopening the older allocator/init-flow replay proof or inventing a new validator lane
...
2. If the shared reminder packet already defers correctly to this note, refresh the smallest shipped shared summary that still undercounts `zigux/tests/runtime_loader_gap_survey.zig` before widening into checker changes or another broader shared reminder pass, starting with `scripts/zigux/README.md` and then `zigux/tests/README.md` if the tests-root packet still trails behind.
