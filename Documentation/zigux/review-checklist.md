...
- if the change touches the parked Phase 8 `exec-cmd` helper packet, do `Documentation/zigux/phase8-exec-cmd-slice.md`, `Documentation/zigux/review-checklist.md`, and `zigux/tests/phase8_exec_cmd.zig` still keep deferred execution helper-only, name `kernel/workqueue.c` as the later Phase 14 boundary-study target, and stop before `execv_cmd()`, `execvp()`, queue ownership, or scheduler-facing transport claims?
...
- if the change is a Phase 13 shared helper slice, do `Documentation/zigux/phase13-devres-slice.md`, `zigux/tests/phase13_devres.zig`, and `zigux/tests/phase13_build.zig` still agree on the same helper-only MMIO or arch-memtype scope, exact replay surface, and explicit no-live-side-effects posture?
- if the change touches the shared Phase 13 release-discipline packet, do `Documentation/zigux/phase13-release-notes-survey.md`, `Documentation/zigux/phase13-roadmap-traceability.md`, `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, and `zigux/tests/phase13_build.zig` still agree that the tranche is active rather than closed, that `make -C zigux phase13` routes through the validator before the shared replay, and that `lib/devres.c` remains the only roadmap anchor without a manifest-backed survey packet?
