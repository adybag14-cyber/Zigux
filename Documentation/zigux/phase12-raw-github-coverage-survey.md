...
## Exact Coverage Evidence
  ...
  * exact coverage evidence checked on `2026-05-16`: direct contents reads now succeed for `scripts/zigux/check-build-only-phase12-surface.py` at blob `5603edf89a320933da023716cae7d4eddb507433` and `.github/workflows/zigux-bootstrap.yml` at blob `704ce77624eb9919745ff41f6b5e8bdd01954618`, while direct contents reads for `scripts/zigux/README.md`, `zigux/tests/phase12_build.zig`, `zigux/Makefile`, and `scripts/zigux/check-phase12-release-readiness-packet.py` still return 404 through the same current `master` bridge
  * exact runtime-reality evidence checked on `2026-05-16`: treat the directly readable checker plus workflow pair as bounded reminder evidence only, not authoritative degraded-read coverage proof, until the current direct-read support packet (`scripts/zigux/README.md`, `zigux/tests/phase12_build.zig`, `zigux/Makefile`, and `scripts/zigux/check-phase12-release-readiness-packet.py`) is restored through the live bridge or this compact fallback note is retargeted to the smaller directly readable packet
## Review Use
