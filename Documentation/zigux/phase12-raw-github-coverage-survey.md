...
- exact public-fallback divergence evidence checked on `2026-05-23`: browser-side raw GitHub readback for `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md` still returns the note, but its body lags the authenticated contents view and keeps the older reminder-only `make -C zigux phase12-validate` wording plus the older support-checker bundle, so treat public raw success for that current-master gap-note companion as coverage proof only rather than same-body proof
