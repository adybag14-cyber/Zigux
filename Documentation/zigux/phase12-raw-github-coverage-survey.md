...
## Exact Coverage Evidence
  ...
  * exact coverage evidence checked on `2026-05-17`: direct contents reads now succeed for `scripts/zigux/check-build-only-phase12-surface.py` at blob `a28c959c5bc45937dba9d049f069a007c06170f8`, `scripts/zigux/check-phase12-release-readiness-packet.py` at blob `6922f26451a0866e90c7de13e19660b86d01e3f9`, `.github/workflows/zigux-bootstrap.yml` at blob `a6f3429b53380a522d6bbc72a47c4927d50cc297`, and `scripts/zigux/README.md` at blob `7a69b488fdf04e77bfdf384044998a432578231b`, while direct contents reads for `zigux/tests/phase12_build.zig` and `zigux/Makefile` still return 404 through the same current `master` bridge
  * exact runtime-reality evidence checked on `2026-05-17`: treat the directly readable build-only checker, release-readiness checker, workflow, and scripts-root README quartet as bounded reminder evidence only, not authoritative degraded-read coverage proof, until the current direct-read support pair (`zigux/tests/phase12_build.zig` and `zigux/Makefile`) is restored through the live bridge or this compact fallback note is retargeted to the smaller directly readable packet
## Review Use
