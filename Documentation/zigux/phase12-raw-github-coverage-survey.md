...
- exact coverage evidence checked on `2026-05-11`: the shared raw-read anchors remain `scripts/zigux/check-build-only-phase12-surface.py` at blob `ee320c1c311b4d634806dc02794d2e309fcfd515` and `zigux/tests/phase12_build.zig` at blob `b1a29719cf21562eb162c0dba77569b4c01955bc`; the build file still exposes the shipped `smoke` and `test` steps through `smoke_step.dependOn(&run_syntax_tests.step);`, `test_step.dependOn(&run_contract_tests.step);`, and `test_step.dependOn(&run_syntax_tests.step);` even when a degraded contents read for that path no longer gives the normal file body
