...
- exact coverage evidence checked on `2026-05-21`: authenticated contents-bridge reads still return the commit-pinned direct replay catalog on current `master` as `Documentation/zigux/phase12-virtio-scsi-raw-github-fallback-catalog.md`, while direct container-side raw-URL fetches to `raw.githubusercontent.com` remain tunnel-blocked with HTTP 403 in this runtime
- exact coverage evidence checked on `2026-05-21`: authenticated contents-bridge reads still return `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md` on current `master`, while direct container-side raw-URL fetches for that same note remain tunnel-blocked with HTTP 403 in this runtime
...
- exact runtime-reality evidence checked on `2026-05-21`: the commit-pinned `virtio_scsi` fallback catalog and the current-master NVMe gap-note companion still exist as the two driver-local fallback-note surfaces on current `master`, but in this runtime both direct raw-URL fetches are tunnel-blocked and only the GitHub contents bridge returned them; the build-only anchor pair and the broader shared support bundle remain separate contents-bridge states, so future fallback wording should reread all four surfaces before widening shared coverage claims
