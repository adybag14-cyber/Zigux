...
## Exact Coverage Evidence
  ...
  * exact coverage evidence checked on `2026-05-17`: direct contents reads now succeed for `scripts/zigux/check-build-only-phase12-surface.py` at blob `5603edf89a320933da023716cae7d4eddb507433`, `.github/workflows/zigux-bootstrap.yml` at blob `efe2224e160b4af16989d46dfab783f07528e315`, and `scripts/zigux/README.md` at blob `3b98030db4e6ab18574217dabb675952742be3f1`, while direct contents reads for `zigux/tests/phase12_build.zig`, `zigux/Makefile`, and `scripts/zigux/check-phase12-release-readiness-packet.py` still return 404 through the same current `master` bridge
  * exact runtime-reality evidence checked on `2026-05-17`: treat the directly readable checker, workflow, and scripts-root README trio as bounded reminder evidence only, not authoritative degraded-read coverage proof, until the current direct-read support packet (`zigux/tests/phase12_build.zig`, `zigux/Makefile`, and `scripts/zigux/check-phase12-release-readiness-packet.py`) is restored through the live bridge or this compact fallback note is retargeted to the smaller directly readable packet
## Review Use
  ...
  * reread `scripts/zigux/check-build-only-phase12-surface.py`, `.github/workflows/zigux-bootstrap.yml`, and `scripts/zigux/README.md` beside direct contents reads of `zigux/tests/phase12_build.zig`, `zigux/Makefile`, and `scripts/zigux/check-phase12-release-readiness-packet.py` before widening fallback claims; if those direct reads still fail, keep this note parked as a degraded-reality warning instead of as coverage proof for the larger shared packet
