...
- exact coverage evidence checked on `2026-05-18`: the current GitHub contents bridge directly reads `scripts/zigux/check-build-only-phase12-surface.py` at blob `c3fef7ac427b843578cd6087aab57652c04847cc`, `scripts/zigux/check-phase12-release-readiness-packet.py` at blob `e3e467197e77b8a1b04699b9e025472d87950e56`, `.github/workflows/zigux-bootstrap.yml` at blob `2404e03e33938fb0829c6f4e315b7f8e548e86ef`, `scripts/zigux/README.md` at blob `c53b5393a808936a799b4c5b0cc09a0eb3324bb1`, and `zigux/Makefile` at blob `fa967e9708358943437ee7c5dafb25ccba17362c`, while a direct contents read for `zigux/tests/phase12_build.zig` still returns `404` through the same current `master` bridge
...
## Review Use
  ...
  * keep the current validator-first then smoke-first order explicit through the reminder-only `make -C zigux phase12-validate` vocabulary, `zig build smoke --build-file zigux/tests/phase12_build.zig --summary all`, the reminder-only `make -C zigux phase12-smoke` vocabulary, `zig build test --build-file zigux/tests/phase12_build.zig --summary all`, and the reminder-only `make -C zigux phase12` vocabulary
