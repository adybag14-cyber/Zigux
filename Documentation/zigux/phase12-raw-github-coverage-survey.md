...
- exact public-read split checked on `2026-05-29`: browser-side public GitHub readback for `Documentation/zigux/phase12-raw-github-coverage-survey.md` now reaches the current `master` survey body through both the blob page and the `raw/refs/heads/master` redirect, while direct same-runtime `curl -L --max-time 20 https://raw.githubusercontent.com/adybag14-cyber/Zigux/master/Documentation/zigux/phase12-raw-github-coverage-survey.md` still fails with `CONNECT tunnel failed, response 403`; keep same-runtime validation anchored to the GitHub contents bridge until raw URL access is directly reachable here.
