...
- exact coverage evidence checked on `2026-05-17`: the current GitHub contents bridge directly reads `scripts/zigux/check-build-only-phase12-surface.py` at blob `bd07243aa47ed7367141e3d859d40e2f6dc52d5d`, `scripts/zigux/check-phase12-release-readiness-packet.py` at blob `860daa5a90a14843d8d0894bace032c4eca78400`, `.github/workflows/zigux-bootstrap.yml` at blob `0130fad8afd0a3c241e65e2b91fc07d042d3b79f`, and `scripts/zigux/README.md` at blob `be85c607e6ec63ee813b42fdb0c21519a970858c`, while direct contents reads for `zigux/tests/phase12_build.zig` and `zigux/Makefile` still return `404` through the same current `master` bridge
- exact runtime-reality evidence checked on `2026-05-17`: keep the directly readable build-only checker, release-readiness checker, workflow, and scripts-root README quartet as bounded reminder evidence only, not authoritative degraded-read coverage proof, until the current direct-read support pair (`zigux/tests/phase12_build.zig` and `zigux/Makefile`) is restored through the live bridge or this compact fallback note is retargeted to the smaller directly readable packet
## Review Use
