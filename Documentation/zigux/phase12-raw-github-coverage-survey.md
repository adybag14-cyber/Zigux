...
- driver-local current-master gap inventory companion:
  - `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md` as the truthful current-master gap map for the shipped NVMe starter-plus-verifier shards while the direct `nvme_pci` survey, slice, and replay packet remains absent
...
- exact coverage evidence checked on `2026-05-13`: the current-master NVMe gap-inventory companion is currently present on `master` as `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md` at blob `43d5f7699143534b7c39056bd46ba9bf85fbf788`, while the bounded shipped foothold it describes is now `drivers/nvme/host/pci.zig` at blob `2844c0ae8162c65ba73d8a0b84b6f1bb105f0e19` plus `drivers/nvme/host/pci_verify.zig` at blob `4e66712d57db672fe6ca355a8ec3d20398a43f37`
...
- exact runtime-reality evidence checked on `2026-05-13`: current `master` ships `scripts/zigux/validate-phase12.py` at blob `8b3df821a3b001b068a8b7722680624b608bc03a`, but there is still no shipped `make -C zigux phase12-validate` route and the shared fallback packet remains the existing build-only smoke-first packet rather than a validator-first replay surface
...
- This note must keep the NVMe map framed as a current-master gap inventory companion for the shipped starter-plus-verifier foothold rather than as a second commit-pinned direct replay artifact.
