...
- exact coverage evidence checked on `2026-05-23`: authenticated contents-bridge reads and browser-side raw GitHub readback now converge again on the same archival `master` body for the commit-pinned direct replay catalog at `Documentation/zigux/phase12-virtio-scsi-raw-github-fallback-catalog.md`, while direct container-side raw-URL fetches in this runtime still fail through the proxy tunnel with HTTP `403`
...
- exact runtime-reality evidence checked on `2026-05-21`: keep the directly readable build-only checker, complex-driver lane checker, libbpf snapshot checker, libbpf heavy-consumer checker, release-readiness checker, workflow, scripts-root README, current Makefile, and current `zigux/tests/phase12_build.zig` as bounded reminder evidence only, and keep the driver-local NVMe gap-note companion bounded to its own packet rather than as authoritative degraded-read coverage proof for the larger shared packet
- exact runtime-reality evidence checked on `2026-05-23`: the commit-pinned `virtio_scsi` fallback catalog, the current-master NVMe gap-note companion, and the two shared-tree anchors still exist on current `master`; authenticated contents-bridge reads and browser-side raw GitHub readback now converge again on the same archival `virtio_scsi` catalog body, while direct container-side `curl`, `wget`, and `urllib` raw-URL fetches still tunnel-fail with HTTP `403`, so shell-side fallback verification remains split between browser-visible raw-read proof and GitHub contents-bridge reads
