...
## Exact Coverage Evidence
  ...
  * exact coverage evidence checked on `2026-05-16`: `scripts/zigux/check-build-only-phase12-surface.py` is still present on `master`, but direct contents reads for `scripts/zigux/README.md`, `zigux/tests/phase12_build.zig`, `.github/workflows/zigux-bootstrap.yml`, `zigux/Makefile`, and `scripts/zigux/check-phase12-release-readiness-packet.py` now return 404 through the same current `master` bridge, so the older shared raw-read anchor pair is no longer a truthful current-master fallback summary by itself
  * exact runtime-reality evidence checked on `2026-05-16`: treat `scripts/zigux/check-build-only-phase12-surface.py` as ahead-of-anchor reminder evidence only, not authoritative degraded-read coverage proof, until the currently unreadable direct-read support packet (`scripts/zigux/README.md`, `zigux/tests/phase12_build.zig`, `.github/workflows/zigux-bootstrap.yml`, `zigux/Makefile`, and `scripts/zigux/check-phase12-release-readiness-packet.py`) is restored through the live bridge or the compact fallback note is retargeted to the smaller live packet
## Review Use
