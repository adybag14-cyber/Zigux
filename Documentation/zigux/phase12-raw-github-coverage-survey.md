...
- exact coverage evidence checked on `2026-05-18`: the current GitHub contents bridge directly reads `scripts/zigux/check-build-only-phase12-surface.py` at blob `bd07243aa47ed7367141e3d859d40e2f6dc52d5d`, `scripts/zigux/check-phase12-release-readiness-packet.py` at blob `860daa5a90a14843d8d0894bace032c4eca78400`, `.github/workflows/zigux-bootstrap.yml` at blob `0130fad8afd0a3c241e65e2b91fc07d042d3b79f`, and `scripts/zigux/README.md` at blob `5a4a6fe8fc1bbbbf868cdd5ae44b1df9154e0782`, while direct contents reads for `zigux/tests/phase12_build.zig` and `zigux/Makefile` still return `404` through the same current `master` bridge
- exact runtime-reality evidence checked on `2026-05-18`: keep the directly readable build-only checker, release-readiness checker, workflow, and scripts-root README quartet as bounded reminder evidence only, not authoritative degraded-read coverage proof, until the current direct-read support pair (`zigux/tests/phase12_build.zig` and `zigux/Makefile`) is restored through the live bridge or this compact fallback note is retargeted to the smaller directly readable packet
## Review Use
  ...
  * reread `scripts/zigux/check-build-only-phase12-surface.py`, `scripts/zigux/check-phase12-release-readiness-packet.py`, `.github/workflows/zigux-bootstrap.yml`, and `scripts/zigux/README.md` beside direct contents reads of `zigux/tests/phase12_build.zig` and `zigux/Makefile` before widening fallback claims; if either of those two direct reads still fails, keep this note parked as a degraded-reality warning instead of as coverage proof for the larger shared packet
