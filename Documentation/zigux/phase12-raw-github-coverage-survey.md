...
- exact shared-summary drift checked on `2026-05-14`: the remaining same-family lag is no longer checker-local only. `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, and `scripts/zigux/check-build-only-phase12-surface.py` still carry older absent-direct-replay wording for `phase12_nvme_pci`, even though the fallback map now records the shipped NVMe starter-plus-verifier-plus-direct-test-plus-slice-note-plus-survey-note-plus-survey-gate-plus-manifest packet on current `master`; broader docs-root or scripts-root wording remains adjacent shared-summary work rather than part of this note-local fallback split.
...
- keep the narrower shared-summary drift explicit too: the fallback map now records the shipped NVMe starter-plus-verifier-plus-direct-test-plus-slice-note-plus-survey-note-plus-survey-gate-plus-manifest packet, but `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, and `scripts/zigux/check-build-only-phase12-surface.py` still carry older absent-direct-replay wording; reread this note beside those shared reminder surfaces before assuming the Phase 12 packet has fully caught up, and leave broader docs-root or scripts-root wording to the adjacent shared-summary lane.
...
## Next Bounded Step
If the fallback split changes later, update this note together with `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, the release-order, closure, readiness, coordination, driver anti-overlap, libbpf survey, libbpf anti-overlap, verify-shard, scripts-root, and tests-root companions so the shared Phase 12 packet keeps one truthful public-read story. The next same-lane repair should stay shared-summary-local: refresh `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, or `scripts/zigux/check-build-only-phase12-surface.py` so they stop describing the direct `phase12_nvme_pci` replay packet as absent while keeping the parked `phase12_libbpf_*` boundaries explicit, and leave broader docs-root or scripts-root wording to the adjacent shared-summary lane.
