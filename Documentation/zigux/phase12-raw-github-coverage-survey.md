...
The shared release-coordination reading for this same split now lives in `Documentation/zigux/phase12-release-readiness-survey.md`, and `python3 scripts/zigux/check-phase12-release-readiness-packet.py` keeps the release-facing PMO packet honest against that same mixed fallback split, so PMO review and degraded public-read review name the same two commit-pinned artifacts, the same two shared-tree-only anchors, and the same validator-first rollback path. That same release-facing prompt also remains visible in `Documentation/zigux/review-checklist.md`, so checklist-driven PMO review does not have to infer the mixed fallback split from the survey note alone.
