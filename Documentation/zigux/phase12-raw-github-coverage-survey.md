...
- exact shared-summary drift checked on `2026-05-14`: `zigux/tests/README.md` now keeps the bounded NVMe starter-plus-verifier-plus-direct-test-plus-slice-note-plus-survey-note-plus-survey-gate-plus-manifest packet explicit, so the remaining same-family lag in the shared reminder packet is checker-local: `scripts/zigux/check-build-only-phase12-surface.py` still hard-codes older scripts-root and tests-root markers that describe the direct `phase12_nvme_pci` replay family as absent, while broader docs-root and review-checklist reminder drift remains adjacent shared-summary work rather than part of this note-local fallback split.
...
- keep the narrower checker-local drift explicit too: `zigux/tests/README.md` now matches the landed NVMe starter-plus-verifier-plus-direct-test-plus-slice-note-plus-survey-note-plus-survey-gate-plus-manifest packet, but `scripts/zigux/check-build-only-phase12-surface.py` still carries older absent-direct-replay markers; reread this note beside that checker before assuming the shared reminder packet has fully caught up, and leave broader docs-root or review-checklist wording to the adjacent shared-summary lane.
...
## Next Bounded Step
If the fallback split changes later, update this note together with `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, the release-order, closure, readiness, coordination, driver anti-overlap, libbpf survey, libbpf anti-overlap, verify-shard, scripts-root, and tests-root companions so the shared Phase 12 packet keeps one truthful public-read story. The next same-lane repair should stay checker-local: refresh `scripts/zigux/check-build-only-phase12-surface.py` so it stops describing the direct `phase12_nvme_pci` replay packet as absent while keeping the parked `phase12_libbpf_*` boundaries explicit, and leave broader docs-root or review-checklist wording to the adjacent shared-summary lane.