...
## Exact Coverage Evidence
  ...
  * exact coverage evidence checked on `2026-05-16`: the shared raw-read anchors remain `python3 scripts/zigux/check-build-only-phase12-surface.py` plus `zigux/tests/phase12_build.zig`, but the current public raw readback of `zigux/tests/phase12_build.zig` still exposes the older shared smoke/test packet centered on `phase12_virtio_net.zig`, `phase12_virtio_net_syntax_lab.zig`, `phase12_virtio_scsi.zig`, `phase12_nvme_pci.zig`, `phase12_nvme_pci_survey.zig`, and the `phase12_libbpf_*` survey and reviewability pair rather than the newer transmit-recycle, queue-resume, repeated-replan, repeated-rollback, or packet-test wiring now described by the checker`
  * exact runtime-reality evidence checked on `2026-05-16`: until the checker and raw build packet land the same shared contract again, treat `zigux/tests/phase12_build.zig` as the authoritative shared replay anchor during degraded contents reads and treat `scripts/zigux/check-build-only-phase12-surface.py` as ahead-of-anchor evidence that still needs a same-lane fail-closed repair before it can be used as truthful coverage proof for those newer routes
## Review Use
