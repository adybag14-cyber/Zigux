...
If this lane reopens, reread `Documentation/zigux/phase12-libbpf-segment-survey.md`, `Documentation/zigux/phase12-libbpf-heavy-consumer-lane-sequencing.md`, `Documentation/zigux/phase12-release-readiness-survey.md`, and `zigux/tests/README.md` against the same live verify shard before choosing another shared reminder repair. Current `master` already keeps `zigux/tests/README.md` explicit about both this verify-shard companion and `tools/lib/bpf/zigux_segments/verify.zig`, so the next honest same-lane follow-through is whichever remaining one-file shared reminder drifts next while keeping the shared smoke-first packet, the checker pair, and the bounded verify-shard ownership split aligned before widening into helper behavior or blocked object-model work.
