...
- current repo reality on `master`: the bounded verify shard and its paired Phase 12 libbpf reviewability files are not present in this checkout, but the shared `scripts/zigux/check-build-only-phase12-surface.py` checker now stays intentionally narrower than that parked libbpf shard instead of treating those missing paths as shipped required files
...
- today, the honest repo-reality signal is narrower: current `master` still returns direct contents-read 404s for `tools/lib/bpf/zigux_segments/verify.zig`, `zigux/tests/phase12_libbpf_reviewability.zig`, `zigux/tests/phase12_libbpf_segments.zig`, `zigux/tests/phase12_libbpf_manifest.json`, and `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.json`, while the shared Phase 12 checker only enforces the shipped docs, workflow, Makefile, and `virtio_scsi` smoke-first build packet
...
2. treat the current contents-read 404 set for the parked verify-shard files as the truthful bounded signal until those files actually land again on `master`
