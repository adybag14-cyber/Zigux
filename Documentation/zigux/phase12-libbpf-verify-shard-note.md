...
- current repo reality on `master`: the parked libbpf verify packet is mixed rather than fully absent or fully shipped. `tools/lib/bpf/zigux_segments/manifest.json` and `zigux/tests/fixtures/phase12_libbpf_snapshot.json` still read back on current `master`, but the bounded verify shard and its paired Phase 12 libbpf reviewability source files do not, so the shared `scripts/zigux/check-build-only-phase12-surface.py` checker stays intentionally narrower than that parked shard instead of treating those missing paths as shipped required files
...
This note now keeps that split explicit while also recording the current mixed-state repo reality.

## Current Ownership

The intended `verify.zig` shard is a reviewability-only packet, but current `master` only exposes the manifest-plus-snapshot anchors, not the verify and reviewability source files themselves.
...
- today, the honest repo-reality signal is mixed rather than binary: current `master` still returns direct contents-read 404s for `tools/lib/bpf/zigux_segments/verify.zig`, `zigux/tests/phase12_libbpf_reviewability.zig`, `zigux/tests/phase12_libbpf_segments.zig`, `zigux/tests/phase12_libbpf_manifest.json`, `zigux/tests/fixtures/phase12_libbpf_snapshot_determinism.json`, and `zigux/tests/phase12_libbpf_snapshot_determinism.zig`, while `tools/lib/bpf/zigux_segments/manifest.json` and `zigux/tests/fixtures/phase12_libbpf_snapshot.json` still read back successfully and the shared Phase 12 checker only enforces the shipped docs, workflow, Makefile, and `virtio_scsi` smoke-first build packet
...
- treat the mixed present-versus-404 state as the truthful bounded signal here: the manifest-plus-snapshot anchors are still reviewable, but they do not by themselves re-open the parked compile-together verify shard
...
2. treat the current mixed-state readback set for the parked verify-shard files as the truthful bounded signal until those files actually land again on `master`
