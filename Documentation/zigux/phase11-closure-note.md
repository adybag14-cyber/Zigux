...
- first, reread `scripts/zigux/README.md`, `zigux/tests/README.md`, `Documentation/zigux/review-checklist.md`, and `scripts/zigux/check-phase11-shared-replay-contract.py` together, starting with `scripts/zigux/README.md` still summarizing the shared Phase 11 support packet without the dedicated gpio teardown companion and still compressing the DesignWare companion down to a teardown-only note even though `Documentation/zigux/phase11-shared-replay-contract.md` already keeps `Documentation/zigux/phase11-gpio-wdt-teardown-note.md` plus the dedicated `scripts/zigux/check-phase11-dw-wdt-packet.py` archival route explicit, so the shared Phase 11 support packet keeps the parked gpio, bcm2835, DesignWare, HVC, and header-boundary companions truthful across scripts-root, tests-root, checklist, and checker-local wording
