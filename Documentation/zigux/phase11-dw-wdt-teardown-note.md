...
- do not treat this note as evidence of platform-driver registration, clock or reset acquisition, IRQ registration, live MMIO execution, suspend or resume behavior, or hardware-backed shutdown
- when `stop()`, `teardownSummary()`, or `removeSummary()` change, update this note together with `Documentation/zigux/phase11-dw-wdt-slice.md` and `Documentation/zigux/phase11-dw-wdt-validation-matrix.md` so the lane keeps one honest teardown story
