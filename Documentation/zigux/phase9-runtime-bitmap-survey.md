...
- lifecycle exit parity: the sample and module replays still prove that both initialized-stage and selftest-complete snapshots preserve `first_set`, `first_zero`, `weight`, and `nbits` across `exit()` while later mutation, re-init, and double-exit attempts fail with `InvalidLifecycleTransition`
