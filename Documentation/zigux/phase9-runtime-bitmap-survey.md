...
- the shared `zigux/kernel/runtime_loader.zig` facade remains a review-only Phase 9 packet under the freeze map's study-only `kernel/workqueue.c` and `kernel/trace/ring_buffer.c` boundary, so this lane keeps the bitmap handoff reviewable without claiming scheduler-facing substrate closure or a freeze-map status change.
...
- a landed shared runtime-loader facade plus allocator/init-flow contract replay under `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, and `zigux/tests/runtime_loader_allocator_init_flow.zig`, including prepared shared-request stability across later selftest and exit activity before any live runtime binding exists
...
- failed-init recovery: an out-of-range `initWithSetBits()` still fails with `BitRangeOutOfBounds`, leaves the sample cold with zero init runs and an empty summary, and still allows a later valid init to succeed cleanly without leaking partial bits
...
- no-op and copy-boundary replay: the sample-owned `runNoOpAndCopyBoundaryReplay()` still preserves the initialized `2,7` starter summary across zero-length `setRange(5, 0)` and `clearRange(128, 0)`, accepts initialized and selftest-complete copy sources, rejects cold and exited copy sources, and restores the original bitmap after the replay finishes
- cross-word boundary replay: the sample-owned `runCrossWordBoundaryReplay()` still spans the `bits_per_long - 1` boundary with a three-bit set, then clears the first two boundary bits so only `boundary_start + 2` remains set while the replay stays in the initialized stage
...
- shared-loader drift guards: the loader also keeps prepared shared-request snapshots stable across later selftest activity, preserves both initialized-stage and selftest-complete shared-request snapshots across later sample exit activity, rejects prepared approved-family drift in the `lib/test_bitmap.c` anchor plus the `zigux_runtime_bitmap_init` and `zigux_runtime_bitmap_exit` symbols, rejects shared-plan or prepared-request drift in `provides_selftest_hook`, and preserves loader-versus-shared-request state synchronization when `releaseSharedWithoutSubstrate()` is attempted before the shared request reaches `waiting_on_runtime_substrate`
...
- tail-overflow mutation replay: the sample-owned `runTailOverflowMutationReplay()` still keeps the selftested `1,127` starter summary stable while rejecting `setRange(127, 2)` and `clearRange(127, 2)` as overflow mutations past the highest valid bit
