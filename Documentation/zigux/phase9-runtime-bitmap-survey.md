...
- shared-loader drift guards: the loader also keeps prepared shared-request snapshots stable across later selftest activity, rejects prepared or pending plan drift in `provides_selftest_hook`, and preserves loader-versus-shared-request state synchronization when `releaseSharedWithoutSubstrate()` is attempted before the shared request reaches `waiting_on_runtime_substrate`
