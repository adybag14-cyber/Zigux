...
Keep this shared sequencing lane parked except for one remaining shared-owner reminder repair. Fresh repo-first rereads on current `master` still show `zigux/tests/phase6_base64.zig`, `zigux/tests/fixtures/phase6_base64_vectors.zig`, `zigux/tests/phase6_base64_perf.zig`, and `zigux/tests/phase6_bsearch.zig` directly readable, while `lib/checksum.zig`, `zigux/tests/phase6_checksum.zig`, `zigux/tests/phase6_checksum_perf.zig`, and `zigux/tests/fixtures/phase6_checksum_vectors.zig` remain absent. `zigux/tests/README.md` already keeps that split explicit in both the longer guidance block and the top-level Phase 6 key-entrypoint list, including the shared manifest, the shared catalog and perf survey, the shared checker, the restored `zigux/tests/phase6_base64_perf.zig` replay, and the remaining checksum public-tree gaps. The older `P6-Y11` tests-root key-entrypoint drift is therefore closed on current `master`; reopen it only if a future reread finds a fresh one-file tests-root reminder mismatch. The remaining shared reminder drift has narrowed back to `P6-Y10`: fresh rereads now show `scripts/zigux/README.md` already reflects the current shared wrapper posture and the restored base64-perf-versus-missing-checksum split, while `Documentation/zigux/README.md` still overstates the shared `phase6` and `phase6-validate` route posture and still treats `zigux/tests/phase6_base64_perf.zig` like a public-tree gap instead of current evidence. The current backlog-backed next safe step is one shared-owner reminder repair inside `Documentation/zigux/README.md`, then park `P6-Y10` again; route any actual base64 helper changes back to `P6-L01` or `P6-Y01`, and route any actual checksum helper restoration back to `P6-Y06`, `P6-L13`, or `P6-L16` instead of reopening the shared lane.