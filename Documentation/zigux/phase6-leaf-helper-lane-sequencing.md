...
Leave this lane parked unless a later Phase 6 run changes the shared `phase6` packet routing, the aggregate `phase6-perf` posture, the shared surface checker, or the owner split between the shared packet and the four helper packets. Fresh repo-first rereads on current `master` still show `zigux/tests/phase6_base64.zig`, `zigux/tests/fixtures/phase6_base64_vectors.zig`, `zigux/tests/phase6_base64_perf.zig`, and `zigux/tests/phase6_bsearch.zig` directly readable, while `lib/checksum.zig`, `zigux/tests/phase6_checksum.zig`, `zigux/tests/phase6_checksum_perf.zig`, and `zigux/tests/fixtures/phase6_checksum_vectors.zig` remain absent. `Documentation/zigux/README.md` and the longer Phase 6 guidance block in `zigux/tests/README.md` already keep that split explicit, and the top-level Phase 6 key-entrypoint list in `zigux/tests/README.md` now also carries the shared manifest, the shared catalog and perf survey, the shared checker, the restored `zigux/tests/phase6_base64_perf.zig` replay, and the remaining checksum public-tree gaps. The older `P6-Y11` tests-root key-entrypoint drift is therefore closed on current `master`; reopen it only if a future reread finds a fresh one-file tests-root reminder mismatch. The current backlog-backed next safe step is to leave the shared sequencing lane parked unless a new shared route, checker, or packet-owner split drifts again, then route any actual base64 helper changes back to `P6-L01` or `P6-Y01`, and route any actual checksum helper restoration back to `P6-Y06`, `P6-L13`, or `P6-L16` instead of reopening the shared lane.