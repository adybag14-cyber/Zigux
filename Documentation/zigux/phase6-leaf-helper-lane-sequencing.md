...
Keep both shared-owner reminder lanes parked on a fresh reread of current `master`. The older `P6-Y11` tests-root key-entrypoint drift is still closed: `zigux/tests/README.md` keeps the shared manifest, the shared catalog and perf survey, the shared checker, the restored `zigux/tests/phase6_base64_perf.zig` replay, and the remaining checksum public-tree gaps explicit in both the top-level Phase 6 key-entrypoint list and the longer tests-root guidance block. The older `P6-Y10` docs-root reminder drift is also now closed on the same reread: `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `Documentation/zigux/phase6-helper-parity-catalog.md`, `Documentation/zigux/phase6-perf-gate-survey.md`, and `zigux/tests/phase6_helper_parity_manifest.json` all agree that the current packet is only partially blocked by the checksum-owned helper gaps, that `zigux/tests/phase6_base64_perf.zig` is live current evidence, and that the broader `phase6`, `phase6-validate`, and `phase6-perf` wrappers should be read as shared route inventory rather than as committed target-body proof. The next honest same-phase follow-up is therefore no longer another shared reminder repair. Park `P6-Y10` and `P6-Y11` again and route the next bounded move back to the owning helper packet: `P6-Y08` for the hexdump empty-ASCII length-packet closure still called out by `Documentation/zigux/phase6-hexdump-slice.md`, `P6-L01` or `P6-Y01` for any future base64 helper or perf-boundary change, `P6-L08`, `P6-L11`, or `P6-Y04` for any bsearch corpus or review-gate drift, and `P6-Y06`, `P6-L13`, or `P6-L16` for any actual checksum helper restoration instead of reopening the shared lane without fresh packet-wide drift.