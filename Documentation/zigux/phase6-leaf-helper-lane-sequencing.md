...
The remaining shared drift has narrowed again: `scripts/zigux/README.md`, `zigux/tests/README.md`, `Documentation/zigux/phase6-helper-parity-catalog.md`, `Documentation/zigux/phase6-perf-gate-survey.md`, and `zigux/tests/phase6_helper_parity_manifest.json` now all describe the restored checksum packet and the helper-local `phase6-base64-perf` plus `phase6-checksum-perf` wrappers consistently, reserving the inventory-only caveat for the aggregate `phase6-validate`, `phase6-perf`, and `phase6` routes.

Keep `P6-Y10` limited to one shared owner-map, scripts-root, checker, or packet-level manifest, catalog, or perf-summary correction only when a fresh shared-route drift appears. Until then, leave helper-local base64, bsearch, checksum, and hexdump follow-through in their owning lanes and keep this shared lane parked.
