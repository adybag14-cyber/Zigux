...
The remaining shared drift is narrower: `Documentation/zigux/phase6-perf-gate-survey.md`, `zigux/tests/README.md`, and any shared-surface checker expectations that still describe the checksum packet as absent or still undercount the restored checksum packet now need one shared retelling pass, while helper-local checksum follow-through stays under `P6-L13`, `P6-L16`, `P6-L17`, or `P6-L18`.

Keep `P6-Y10` limited to one shared owner-map, tests-root, checker, or packet-level manifest, catalog, or perf-summary correction that makes the restored checksum packet and the remaining helper-owner split obvious without reopening helper-local checksum work or reusing stale lane keys.
