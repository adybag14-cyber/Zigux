...
Leave this lane parked unless a later Phase 6 run changes the shared `phase6` packet routing, the aggregate `phase6-perf` posture, the shared surface checker, or the owner split between the shared packet and the four helper packets. Fresh repo-first rereads on current `master` still show `zigux/tests/phase6_base64.zig` and `zigux/tests/fixtures/phase6_base64_vectors.zig` back in the tree while `zigux/tests/phase6_base64_perf.zig`, `lib/checksum.zig`, `zigux/tests/phase6_checksum.zig`, `zigux/tests/phase6_checksum_perf.zig`, and `zigux/tests/fixtures/phase6_checksum_vectors.zig` remain absent. `Documentation/zigux/README.md` already keeps that split explicit, but `zigux/tests/README.md` still drifts in two places: the top-level Phase 6 key-entrypoint list undercounts the shared checker-backed packet and does not restate the remaining base64-perf plus checksum public-tree gaps beside the present entrypoints, and the longer Phase 6 guidance block still mentions `zigux/tests/fixtures/phase6_checksum_vectors.zig` as if it were committed fixture evidence even though current `master` still treats that path as a public-tree gap. The current backlog-backed next safe step for `P6-Y11` is therefore one tests-root reminder sync before any broader checker rewrite, because direct current-`master` file reads still show `zigux/tests/phase6_base64.zig`, `zigux/tests/fixtures/phase6_base64_vectors.zig`, and `zigux/tests/phase6_bsearch.zig`, while `zigux/tests/phase6_base64_perf.zig`, `lib/checksum.zig`, `zigux/tests/phase6_checksum.zig`, `zigux/tests/phase6_checksum_perf.zig`, and `zigux/tests/fixtures/phase6_checksum_vectors.zig` are not directly readable from that same head. Keep `P6-Y11` limited to one tests-root, closure-note, ledger, or similarly small shared-summary sync that makes that shared-checker-plus-gap split obvious at the top of the review packet, then route any actual base64 helper restoration back to `P6-L01` or `P6-Y01`, and route any actual checksum helper restoration back to `P6-Y06`, `P6-L13`, or `P6-L16` instead of reopening the shared lane.
