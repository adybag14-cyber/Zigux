...
- Keep the shared-control packet focused on reminder, checker, workflow, and narrow validation-foothold truthfulness only. The helper-local `argv_split`, `cmdline`, `rbtree`, and `string_helpers` packets remain separate owners for direct helper behavior and survey drift.

## Parked Reminders

- Keep `scripts/zigux/check-phase7-make-wrapper.py` and `zigux/tests/phase7_build.zig` framed as parked reminder vocabulary until a fresh current-`master` reread proves those paths returned.
