...
| shared reminder posture | `Documentation/zigux/phase11-driver-lane-sequencing.md`, `Documentation/zigux/phase11-validation-matrix-gap-survey.md`, and the returned `Documentation/zigux/phase11-shared-replay-contract.md` reminder note keep this matrix adjacent shared evidence rather than a driver-local HVC or watchdog lane member | direct reread of this matrix together with the sequencing, reminder-contract, and matrix-gap notes keeps the lane split honest | refresh `Documentation/zigux/phase11-uapi-header-parity-survey.md` and the shared reminder contract only when the smaller surviving proof-shard packet changes again instead of the older shared replay family returning | folding this packet into `P11-L16`, the watchdog-local lanes, or broad contributor-note ownership |
| older shared replay family | `Documentation/zigux/phase11-shared-replay-contract.md` now rematerializes as a reminder note, but current direct contents reads still do not rematerialize `zigux/tests/phase11_uapi_header_parity_manifest.json`, `zigux/tests/phase11_uapi_header_parity_survey.zig`, `zigux/tests/phase11_build.zig`, or `scripts/zigux/check-phase11-header-boundary-packet.py` | keep the returned contract note framed as reminder-only continuity evidence and keep the still-missing shared replay family explicit as repo-reality gaps until a future reread proves they returned | if any one of those shared packet anchors rematerializes, refresh this matrix in the same pass that restores the corresponding survey wording | claiming shared replay, manifest, checker, or build-route coverage as current-head evidence from historical wording alone |
...
- Treat `Documentation/zigux/phase11-shared-replay-contract.md` as returned reminder-only continuity evidence unless the missing shared replay files rematerialize beside it.
