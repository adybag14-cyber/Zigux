...
The live repo already has a bounded `drivers/virtio/virtio.zig` core starter with queue callback bookkeeping. This survey started by making the missing ring-helper gap explicit, and it now records that the first `drivers/virtio/virtio_ring.zig` lab slice has landed without pretending queue lifecycle parity is complete.
...
- this means the roadmap's "virtqueue wrappers first, MMIO wrappers later" rule still points to another tiny in-memory queue helper next, not MMIO or DMA work.
...
This keeps the lane concrete and reviewable without overstating `virtio_ring` progress: the queue-shape foothold is real, but the risky transport-facing work is still intentionally blocked.
