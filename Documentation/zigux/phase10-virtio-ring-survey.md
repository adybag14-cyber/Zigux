...
- the live packet now records the direct repo reality honestly: the queue-local ring helper ladder, the wrapper-facing verify replay, the publish-readiness replay, the registration-summary wrapper, the used-buffer-poll wrapper, the broader replay `zigux/tests/phase10_virtio_ring.zig`, the focused notification-data, registration, prepare-kick, reset-reuse, reset-readiness, broken-queue, and delayed-callback replays, and the dedicated survey replay are directly readable on current `master`.
