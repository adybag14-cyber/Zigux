...
- the current Zigux VirtIO surface now includes a bounded `drivers/virtio/virtio_ring.zig` helper for queue registration, layout metadata, outstanding-chain accounting, used-buffer polling, callback re-enable bookkeeping, delayed-callback pacing, and notify-prepare bookkeeping.
...
- this means the roadmap's "virtqueue wrappers first, MMIO wrappers later" rule now points to one tiny in-memory MMIO register-window helper next, while interrupt and lifecycle paths stay blocked behind that smaller transport foothold.
...
This keeps the lane concrete and reviewable without overstating `virtio_ring` progress: the queue-shape foothold is real, used-buffer polling, callback re-enable, and delayed-callback pacing are now landed, and the risky transport-facing lifecycle work is still intentionally blocked.
...
Stay in the broader Phase 10 virtio lane and add one small in-memory MMIO register-window helper next before widening into interrupt acknowledgement, reset, queue discovery, or probe lifecycle work.
