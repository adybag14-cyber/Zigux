...
- scope: survey manifest, dedicated survey gate, the dedicated ring packet review guard, the direct ring helper replay, the focused drained-reset reuse replay, the wrapper-facing ring verifier replay, shared Phase 10 build wiring, the shared reset-queue and driver-id replays, the Linux-style replay route, and one lane-level note that records the current queue-local virtqueue foothold plus the remaining roadmap lab-driver gap
- product boundary:
...
This keeps the lane concrete without overstating progress.
