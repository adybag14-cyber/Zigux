...
Leave the checksum helper lane parked unless fresh repo inspection finds a concrete parity, perf, or directly coupled review-packet drift inside `lib/checksum.zig`, `zigux/tests/phase6_checksum.zig`, `zigux/tests/fixtures/phase6_checksum_vectors.zig`, `zigux/tests/phase6_checksum_c_parity.zig`, `zigux/tests/fixtures/phase6_checksum_c_harness.c`, `scripts/zigux/check-phase6-checksum-c-parity.py`, `zigux/tests/phase6_checksum_perf.zig`, or the shared Phase 6 packet. The live checksum slice, parity script, parity runner, and C harness still agree on the 3-case IPv6 pseudo-header corpus, the 27-case checksum parity replay, and the 11-case checksum parity script self-test, but the shared Phase 6 packet still carries a separate parked-versus-active status mismatch between the parked catalog plus helper slice notes and the active machine-readable manifest plus shared validator expectation. The next same-family follow-up should therefore stay narrow to that shared-packet status repair rather than reopening `lib/checksum.zig` or its directly coupled checksum parity runners.
