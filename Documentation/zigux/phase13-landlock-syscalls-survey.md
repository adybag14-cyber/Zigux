...
- `PHASE13_SLICE=landlock-syscalls-helper-restrict-self-credential-handoff`
...
- a focused attached-toolchain check still compiles and passes the dedicated `zigux/tests/phase13_landlock_syscalls.zig` gate at inspected head `9c17b0790799d8240ef9f964903f5ce2db64af89`, so the current wrapper and policy planners remain aligned with the bounded helper contract recorded in this lane packet.
...
- compared against the Phase 13 roadmap and the current repo reality, the previously open syscall-helper follow-up in this packet was the bounded `landlock_restrict_self()` credential handoff order after flag parsing, so this lane now closes that pure gap by making the privilege gate, prepared-credential allocation, optional ruleset merge, optional sibling-thread synchronization, and final commit order explicit without claiming live mutation.
...
- the helper still does not claim anonymous inode creation, live user-memory access, live FD ownership, path-backed or port-backed rule insertion, live `prepare_creds()` mutation, live sibling-thread synchronization, or live domain merges.
- after the restrict-self credential handoff slice, the next honest syscall-facing move is to stay parked unless another tiny validation-only or lifetime-discipline follow-up can remain pure without widening into anonymous inode internals, live user-memory access, live FD ownership, deeper credential mutation, or domain state.
...
This keeps the lane explicit without overstating progress: Zigux now has a real `syscalls.zig` helper foothold for ABI, bounded user-struct copy discipline, create-ruleset, restrict-self flag translation, restrict-self credential handoff ordering, add-rule, ruleset-FD lookup, path-FD lookup, path-beneath handoff, net-port handoff, and ruleset-FD creation handoff planning, but it still does not claim anonymous inode creation internals, live user-memory access, live path or port import, or task enforcement.
