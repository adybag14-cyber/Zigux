...
- `PHASE13_LANE_KEY=P13-L13`
- `PHASE13_SLICE=landlock-syscalls-helper-ruleset-fd-install`
- `PHASE13_SURVEYED_COMMIT=599ee1519c5464bb86a0ffdcab52dfe958c40571`
...
- the current `security/landlock/syscalls.zig` slice now stays intentionally narrow around `build_check_abi()` sizing, `landlock_create_ruleset()` query and mask validation, `landlock_restrict_self()` logging-flag translation including the special `ruleset_fd == -1` mute-subdomains-only case, the first `landlock_add_rule()` planner for rule-type dispatch and bounded rule-shape validation, the in-memory `get_ruleset_from_fd()` planner for bad-FD rejection, ruleset-FD type checks, `FMODE_CAN_WRITE` or `FMODE_CAN_READ` access checks, and the single-layer guard, the in-memory `get_path_from_fd()` planner for ruleset-FD rejection, internal-mount filtering, non-user-visible inode rejection, and owned path reference handoff, the in-memory `add_rule_path_beneath()` planner that combines copied path-beneath attrs with the bounded path-FD handoff and the later `put_path()` release responsibility, the in-memory ruleset-FD installation planner that keeps `anon_inode_getfd("[landlock-ruleset]", &ruleset_fops, ruleset, O_RDWR | O_CLOEXEC)` and the failure-side `landlock_put_ruleset()` release explicit, plus the new in-memory `ruleset_fops` planner that keeps `fop_ruleset_release()` together with the dummy read-or-write `FMODE_CAN_READ` or `FMODE_CAN_WRITE` enablement and shared `-EINVAL` return contract explicit.
- the focused `zigux/tests/phase13_landlock_syscalls_reviewability.zig` shard is part of the current helper-local product boundary as direct evidence for that bounded syscall packet, even though it stays outside the shipped eight-test shared release replay.
...
- landed `phase13-landlock-ruleset-fd-install-followup`
- landed `phase13-landlock-ruleset-fops-followup`

This keeps the lane explicit without overstating progress: Zigux now has a real `syscalls.zig` helper foothold for ABI, create-ruleset, restrict-self, add-rule, ruleset-FD, path-FD, path-beneath handoff, the bounded ruleset-FD installation contract, and the combined `ruleset_fops` release-or-dummy-handler contract, but it still does not claim anonymous inode creation, live file-operations wiring, path import, or task enforcement.
...
Keep this packet parked unless a future Phase 13 syscalls run can add another equally bounded planner without widening into live file-operations wiring, credential updates, or domain state. Do not treat this landed ruleset-FD installation planner or `ruleset_fops` planner as permission to imply anonymous inode creation, FD ownership, or live enforcement.
