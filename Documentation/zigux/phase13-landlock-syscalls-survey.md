...
- compared against the Phase 13 roadmap and the current repo reality, the next pure syscall-helper follow-up in this packet was the dedicated ruleset file-operations contract around the already-landed ruleset-FD creation handoff, so this lane now closes that gap by making the `fop_ruleset_release()` ownership drop and the dummy read or write handlers behind `FMODE_CAN_READ` and `FMODE_CAN_WRITE` explicit without claiming live wiring.
...
- after the ruleset file-operations contract slice, the next honest syscall-facing move is to stay parked unless another tiny validation-only or lifetime-discipline follow-up can remain pure without widening into anonymous inode internals, live user-memory access, live FD ownership, deeper credential mutation, or domain state.
...
This keeps the lane explicit without overstating progress: Zigux now has a real `syscalls.zig` helper foothold for ABI, bounded user-struct copy discipline, create-ruleset, restrict-self flag translation, restrict-self credential handoff ordering, add-rule, ruleset-FD lookup, path-FD lookup, path-beneath handoff, net-port handoff, ruleset-FD creation handoff, and the dedicated ruleset file-operations contract, but it still does not claim anonymous inode creation internals, live user-memory access, live path or port import, or task enforcement.
