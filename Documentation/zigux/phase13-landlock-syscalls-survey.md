...
This keeps the lane explicit without overstating progress: Zigux now has a real `syscalls.zig` helper foothold for ABI, the shared initialization gate, bounded user-struct copy discipline, create-ruleset, restrict-self flag translation, restrict-self credential handoff ordering, add-rule, ruleset-FD lookup, path-FD lookup, path-beneath handoff, net-port handoff, ruleset-FD creation handoff, and the dedicated ruleset file-operations contract, but it still does not claim anonymous inode creation internals, live user-memory access, live path or port import, or task enforcement.
