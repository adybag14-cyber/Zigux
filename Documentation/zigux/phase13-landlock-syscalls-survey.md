...
This keeps the lane explicit without overstating progress: Zigux now has a real `syscalls.zig` helper foothold for ABI, create-ruleset, restrict-self, add-rule, ruleset-FD, path-FD, path-beneath handoff, and the combined `ruleset_fops` release-or-dummy-handler contract, but it still does not claim anonymous inode creation, live file-operations wiring, path import, or task enforcement.
...
Keep this packet parked unless a future Phase 13 syscalls run can add another equally bounded planner without widening into live file-operations wiring, credential updates, or domain state. Do not treat this landed `ruleset_fops` planner as permission to imply anonymous inode creation, FD ownership, or live enforcement.
