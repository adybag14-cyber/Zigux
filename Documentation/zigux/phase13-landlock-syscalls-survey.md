...
The Phase 13 roadmap still keeps `security/landlock/syscalls.c` inside bounded security-helper pilots. Current `master` now materializes a helper-local and reviewability packet around that anchor.
...
This is still honest Phase 13 product progress because it surveys one bounded shared helper, keeps the mixed-language boundary explicit, and adds a direct reviewability witness without widening into live syscall ownership claims.
...
- current `master` materializes the direct replay or manifest companion and the packet can add a validation-only follow-through without widening into new helper behavior
