...
- `PHASE13_SURVEYED_COMMIT=3c2928980cc1847d86f8a152f2a71350de0da0d1`
...
- the next honest syscall-facing step is one tiny planner around `add_rule_net_port()` handoff rules, still in-memory and still outside real rule insertion, socket state, or credential handling.
...
This keeps the lane explicit without overstating progress: Zigux now has a real `syscalls.zig` helper foothold for ABI, create-ruleset, restrict-self, add-rule, ruleset-FD, path-FD, and path-beneath handoff planning, but it still does not claim anonymous inode creation, path import, or task enforcement.
...
Stay in the Phase 13 Landlock syscalls lane and add one tiny `security/landlock/syscalls.zig` planner around `add_rule_net_port()` handoff rules next, limited to combining copied net-port attrs with the bounded validation path and the later `landlock_append_net_rule()` handoff before any live rule insertion, socket state, credential updates, or live domain state are attempted.
