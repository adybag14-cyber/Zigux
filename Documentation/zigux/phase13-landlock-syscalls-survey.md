...
- scope: the shipped `security/landlock/syscalls.zig` helper starter, the paired `Documentation/zigux/phase13-landlock-syscalls-governance.md` note, the direct helper-local replay and manifest that now keep the packet machine-checkable on current `master`, and the still-missing direct helper-local companions that would be needed before this packet could claim a fuller review bundle
- product boundary:
...
Current `master` already ships a small `security/landlock/syscalls.zig` helper starter and now also ships a direct helper-local replay plus paired manifest, but it still does not ship the direct helper-local slice note or reviewability gate that would turn this into a fuller review packet. The highest-value bounded work in this lane is therefore to keep the shipped helper surface and its direct replay packet easy to reread instead of widening into ruleset ownership or shared reminder churn.
...
- the shipped `planAddRule()` and `planLandlockAddRule()` planners keep the `landlock_add_rule()` packet explicit by separating `AddRuleAction.path_beneath` from `AddRuleAction.net_port`, requiring ruleset write access for both, keeping parent-fd lookup local to `path_beneath`, keeping port handoff local to `net_port`, and preserving the top-level syscall-wrapper validation step as pure planning data.
...
- current `master` now ships `zigux/tests/phase13_landlock_syscalls.zig` and `zigux/tests/phase13_landlock_syscalls_manifest.json`, which keep the helper packet machine-checkable on current `master` even while the wider shared build route, the slice note, and the dedicated reviewability shard remain absent.
- the immediate repo-reality gap versus the roadmap is therefore no longer an absent direct replay or manifest. The remaining packet-local gaps are the dedicated slice note and reviewability shard around the helper that current `master` already ships.
...
- current `master` still exports `pub fn planRestrictSelf(`, `pub fn planAddRule(`, `pub fn planLandlockAddRule(`, `pub fn planFopRulesetRelease(`, and `pub fn planRulesetFops(`, so the helper starter already covers the narrow policy-planning, syscall-wrapper-discipline, and release-side ownership surface that the roadmap permits for this bounded security pilot.
- `zigux/tests/phase13_landlock_syscalls.zig` and `zigux/tests/phase13_landlock_syscalls_manifest.json` are now present on current `master`, while `Documentation/zigux/phase13-landlock-syscalls-slice.md` and `zigux/tests/phase13_landlock_syscalls_reviewability.zig` still return absent-path readback on current `master`.
...
- landed `phase13-landlock-syscalls-direct-replay`
...
This keeps the packet honest: Zigux now has a reviewable helper starter plus paired governance and survey notes, a direct replay, and a paired manifest for the syscall-facing Landlock packet, but it still does not claim a dedicated reviewability shard, a slice note, live credential mutation, live ruleset ownership, or live syscall enforcement.
