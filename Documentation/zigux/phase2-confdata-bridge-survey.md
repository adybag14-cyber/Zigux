...
- The real gap this run reopened was note-local truthfulness only: the older survey wording had drifted behind current repo reality by naming a no-longer-shipped dedicated helper-anchor checker route, by restating the older 20-anchor and 13-case packet, and by undercounting the live duplicate-malformed expected output. This note now reflects the live `23`-anchor and `14`-case confdata packet plus the current shared checker surfaces.
