...
- `Documentation/zigux/phase2-closure.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` already describe the same shared kconfig packet and keep the bridge reviewable through the dedicated `scripts/zigux/check-phase2-confdata-helper-anchor-alignment.py` guard, the shared `check-kconfig-bridge.py` gate, plus the direct `zig test scripts/zigux/kconfig/confdata_bridge.zig` replay instead of implying any current-`master` surface is missing.
