...
- `Documentation/zigux/phase2-closure.md`, `Documentation/zigux/review-checklist.md`, and `scripts/zigux/README.md` already keep the dedicated `scripts/zigux/check-phase2-confdata-helper-anchor-alignment.py` guard explicit, while `zigux/tests/README.md` continues to carry the shared kconfig packet together with the shared `check-kconfig-bridge.py` gate and the direct `zig test scripts/zigux/kconfig/confdata_bridge.zig` replay. Together these reminder surfaces keep the bridge reviewable without implying any current-`master` surface is missing.
