...
- `Documentation/zigux/phase2-toolchain-bootstrap-notes.md`, `zigux/tests/README.md`, `scripts/zigux/README.md`, and `scripts/zigux/check-phase2-toolchain-pin-scope.py` now keep the repo-local `.zig-toolchain` fallback wording explicit, while `Documentation/zigux/README.md` and `Documentation/zigux/review-checklist.md` still keep that pinning note tied to the same shared validator and closure packet instead of leaving it as stand-alone reference text.
