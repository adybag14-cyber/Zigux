...
- the fixdep manifest packet in `zigux/tests/fixtures/fixdep/manifest.json` still keeps the case list, stdout/stderr packet, and helper-local anchors compact, while the direct `zig test scripts/zigux/fixdep.zig` surface now spans the broader fifteen-test replay listed above
