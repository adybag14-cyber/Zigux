...
- `scripts\zigux/check_genksyms_bridge.zig`, `scripts\zigux/check_phase2_genksyms_selftest_alignment.zig`, and `scripts/zigux/genksyms.zig` remain the live checker, closure-alignment guard, and Zig bridge helper on current `master`.
- `scripts\zigux/check_phase2_genksyms_dual_implementation_survey.zig` remains the dedicated survey guard that keeps the wrapper-first bridge evidence, restored CRC-side tool-plus-checker evidence, and current closure packet explicit.
...
- `scripts\zigux/check_phase2_tool_manifest.zig`, `scripts\zigux/check_phase2_bootstrap_workflow_routes.zig`, and `scripts\zigux/check_phase2_artifact_tools_manifest.zig` keep the shared manifest, workflow-route, and artifact-support packet explicit from current `master`.
