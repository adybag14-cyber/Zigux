...
This note keeps the current Phase 2 closure-side packet aligned to the directly readable toolchain, installer, cross-route, kconfig-bridge, genksyms bridge, make-wrapper, manifest, and validator surfaces on current `master`.
...
The next bounded same-lane follow-through is to leave this closure packet parked unless a future Phase 2 reread finds fresh drift between this closure note, `scripts/zigux/validate-phase2-closure.py`, `zigux/tests/fixtures/phase2_tool_manifest.json`, `zigux/tests/fixtures/phase2_cross_targets.json`, `zigux/Makefile`, the current kconfig bridge fixture roster, or the bounded genksyms bridge fixture roster.

- `PHASE2_NEXT_SAFE_STEP=leave the current Phase 2 closure packet parked unless the returned installer, cross-route, manifest, shared reminder, make-wrapper, current kconfig bridge packet, or bounded genksyms bridge packet drifts on current master`
