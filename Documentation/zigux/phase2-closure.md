...
The bounded fixdep closure packet currently keeps seven committed artifact cases, the current fixdep manifest packet, and the broader direct same-file replay reviewable:
...
- `PHASE2_FIXDEP_HELPER_LOCAL_ANCHOR_COUNT=9`
- `PHASE2_FIXDEP_DIRECT_TEST_COUNT=16`
- `PHASE2_FIXDEP_DIRECT_TESTS=config parsing trims _MODULE and deduplicates symbols,config parsing stops at the first embedded NUL,dep parsing returns NoTargets for comment-only depfiles,dep parsing keeps escaped spaces inside tokens,dep parsing unescapes escaped colons inside tokens,dep parsing continues dependency tokens across escaped newlines,dep parsing skips bytes after the first embedded NUL,ignored and no-parse file classification matches fixdep rules,file read errors map to C-style messages,read failure wording matches C perror prefix,stat failure wording matches C perror prefix,output write failure uses C-style wording,flush helper preserves the primary error,dependency file reads beyond the legacy one mebibyte ceiling,escaped hash dependency survives concatenated target comment path,escaped space dependency survives concatenated target comment path`
...
- the fixdep manifest packet in `zigux/tests/fixtures/fixdep/manifest.json` still keeps the case list, stdout/stderr packet, and helper-local anchors compact, while the direct `zig test scripts/zigux/fixdep.zig` surface now spans the broader sixteen-test replay listed above
- helper-local anchors in `zig test scripts/zigux/fixdep.zig` now include `dep parsing returns NoTargets for comment-only depfiles`, `dep parsing keeps escaped spaces inside tokens`, `dep parsing unescapes escaped colons inside tokens`, `dep parsing continues dependency tokens across escaped newlines`, `dep parsing skips bytes after the first embedded NUL`, `dependency file reads beyond the legacy one mebibyte ceiling`, `output write failure uses C-style wording`, `escaped hash dependency survives concatenated target comment path`, and `escaped space dependency survives concatenated target comment path`
...
- `PHASE2_GENKSYMS_BRIDGE_CASE_COUNT=25`
- `PHASE2_GENKSYMS_BRIDGE_CASES=minimal,debug_reference_types,long_options,abbreviated_long_options,ambiguous_long_option,quiet_overrides_warning,explicit_option_terminator,positional_passthrough,lone_dash_passthrough,help,version_then_short_help,version_then_long_help,version_then_invalid_option,abbreviated_help,unexpected_help_argument,version_then_unexpected_help_argument,version,abbreviated_version,invalid_option,missing_reference_argument,missing_dump_types_argument,unsupported_long_option,missing_long_reference_argument,missing_long_dump_types_argument,too_many_reference_files`
...
- `PHASE2_GENKSYMS_BRIDGE_PROCESS_PACKET=ambiguous_long_option_expected.json,help_expected.json,version_then_help_expected.json,version_then_invalid_option_expected.json,unexpected_help_argument_expected.json,version_then_unexpected_help_argument_expected.json,version_expected.json,abbreviated_version_expected.json,invalid_option_expected.json,missing_reference_argument_expected.json,missing_dump_types_argument_expected.json,unsupported_long_option_expected.json,missing_long_reference_argument_expected.json,missing_long_dump_types_argument_expected.json,too_many_reference_files_expected.json`
- `PHASE2_GENKSYMS_BRIDGE_NORMALIZED_STDERR_PACKET=ambiguous_long_option_expected.json,version_then_invalid_option_expected.json,unexpected_help_argument_expected.json,version_then_unexpected_help_argument_expected.json,invalid_option_expected.json,missing_reference_argument_expected.json,missing_dump_types_argument_expected.json,unsupported_long_option_expected.json,missing_long_reference_argument_expected.json,missing_long_dump_types_argument_expected.json,too_many_reference_files_expected.json`
...
- bridge-process coverage stays anchored by the ambiguous-long-option fixture, the help, version-then-help, version-then-invalid-option, and version-then-unexpected-help-argument fixtures, the abbreviated-help fixture, the unexpected-help-argument failure fixture, the version side-effect fixtures including `abbreviated_version_expected.json`, and the normalized invalid-option, missing-reference-argument, missing-dump-types-argument, and too-many-reference error fixtures in the same bounded packet
...
- `PHASE2_KCONFIG_BRIDGE_CONF_PACKET=zigux/tests/fixtures/kconfig_bridge/conf_manifest.json`
- the shared Phase 2 tool manifest points at that same tool-local packet through `kconfig_conf_bridge_packet`, keeping the committed conf bridge case list, stdout packet, mode-arg cases, allconfig sentinel packet, allconfig override packet, and helper-local anchors reviewable without widening the broader Phase 2 manifest surface
...
- `PHASE2_KCONFIG_BRIDGE_CONFDATA_CASES=sample,escaped_strings,escaped_control_sequences,trailing_escaped_backslash,sample_crlf,explicit_n_tristate,final_trailing_carriage_return,final_unterminated_unset_comment,uppercase_tristate,non_config_lines,empty_config_symbol_names,malformed_quoted_duplicate`
- `PHASE2_KCONFIG_BRIDGE_CONFDATA_EXPECTED_PACKET=sample_expected.json,escaped_strings_expected.json,escaped_control_sequences_expected.json,trailing_escaped_backslash_expected.json,sample_crlf_expected.json,explicit_n_tristate_expected.json,final_trailing_carriage_return_expected.json,final_unterminated_unset_comment_expected.json,uppercase_tristate_expected.json,non_config_lines_expected.json,empty_config_symbol_names_expected.json,malformed_quoted_duplicate_expected.json`
...
- `Documentation/zigux/phase2-toolchain-bootstrap-notes.md`, `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, `scripts/zigux/README.md`, `scripts/zigux/check-phase2-tests-readme-alignment.py`, and `scripts/zigux/check-phase2-toolchain-pin-scope.py` now keep the repo-local `.zig-toolchain` fallback wording explicit, so the archive-pin note stays tied to the same shared validator and closure packet instead of being stranded as stand-alone reference text.
