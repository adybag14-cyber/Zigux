...
- the checker self-test must stay in the Linux-style `phase2-cross` path before live compile replay so duplicate tool entries, duplicate manifest targets, manifest-count drift, and explicit-target failure drift cannot hide behind local Zig availability
...
- `PHASE2_CROSS_MANIFEST_POLICY=check-phase2-cross.py rejects duplicate tool entries, duplicate manifest targets, and manifest-count drift before live compile replay`
