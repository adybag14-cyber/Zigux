...
- the current `kconfig` closure packet now stays explicit as the `11-case` conf bridge plus `6-case` confdata fixture replay under `zigux/tests/fixtures/kconfig_bridge/cases.json`, and `scripts/zigux/check-kconfig-bridge.py` now fail-closes when the committed `conf_cases` order drifts from the live `scripts/zigux/kconfig/conf_bridge.zig` `Mode` enum before the artifact replay runs
