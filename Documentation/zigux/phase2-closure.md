...
- the live confdata closure packet now also keeps the later helper-local bridge coverage explicit on current `master`: uppercase tristate recognition, ignored non-`CONFIG_` lines, empty symbol-name rejection, and the shared kconfig selftest-alignment gate all agree on the same bounded `11-case` external fixture replay instead of collapsing those later bridge tightenings back into generic scaffold wording
