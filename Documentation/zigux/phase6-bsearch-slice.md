...
Current `master` also still carries `zigux/tests/fixtures/phase6_bsearch_vectors.zig` as a compact shared seed companion for the representative ascending, descending, hit-or-miss, symbol, and packed-record cases. Reviewers should treat that fixture as supporting evidence for the inline comparison-budget packet rather than as a separate replay surface or a standalone timing-style perf route.
