...
The current packet intentionally keeps its representative sorted inputs inline in `zigux/tests/phase6_bsearch.zig`, with the lower- and upper-bound and direct C ABI companion replays carrying the remaining bounded insertion-point and equality corpus under the same reviewable comparison-budget posture. The same packet still keeps its bounded comparison-budget evidence instead of a dedicated `phase6_bsearch_perf` route, and the dedicated bsearch-only rerun routes keep that packet reviewable without dragging the rest of the shared Phase 6 helper bundle into every follow-up.
