...
If this helper reopens, keep the next move inside `lib/bsearch.zig`, `zigux/tests/phase6_bsearch.zig`, `zigux/tests/phase6_bsearch_lower_bound_c_abi.zig`, `zigux/tests/phase6_bsearch_c_abi_budget.zig`, `zigux/tests/fixtures/phase6_bsearch_vectors.zig`, or this slice note only. The first safe follow-up after this restore would be another helper-local parity proof or a dedicated corpus checker only if fresh repo inspection shows drift in one of the already-materialized typed or raw C ABI companions, not broadening helper semantics.