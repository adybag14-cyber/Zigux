...
- The shared validator and the dedicated gate-evidence checker now fail closed on the current shipped blob-pin set, while the scripts-root and tests-root README summaries still stop short of naming the runtime atomic64 survey gate as a distinct build surface.
