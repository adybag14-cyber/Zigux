...
- `PHASE4_GATE_EVIDENCE_CHECKER_BLOB_SHA=449430bcd7e0e37ac9f6c0aca7b39b0c43820d3b`
...
- the dedicated `scripts/zigux/check-phase4-gate-evidence.py` self-test now also mutates its own blob pin plus the shared `zigux/tests/phase4_build.zig` and `zigux/Makefile` blob pins, so those published ledger targets fail closed in the same synthetic replay instead of relying only on live-note inspection.
