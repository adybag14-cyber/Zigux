...
- The dedicated parked `test_fsmount` gap packet at `Documentation/zigux/phase4-test-fsmount-gap-survey.md`, `zigux/tests/phase4_test_fsmount_manifest.json`, and `zigux/tests/phase4_test_fsmount_survey.zig` is now present on `master`; its Linux replay remains `make M=samples/vfs`, it keeps `Validation and Perf Team` as both survey owner and rollback owner, and it stays adjacent evidence only while the shared exact-readback packet still stops short of claiming a shipped Zig starter or shared gate-evidence promotion for `samples/zigux/test_fsmount.zig`.
...
- `samples/zigux/test_fsmount.zig` remains absent, and the adjacent survey packet now makes that missing-starter boundary reviewable without pretending that the shared exact-readback gate already owns the `test_fsmount` note, manifest, or survey route.
...
- The current exact-readback note is aligned again to the live validator, README, workflow, Makefile, and Phase 4 gate surfaces on `master`, and `zigux/tests/README.md` now explicitly carries the shipped local-only perf-baseline pair `zigux/tests/phase4_perf_baseline_manifest.json` plus `zigux/tests/phase4_perf_baseline_survey.zig`, while the adjacent `test_fsmount` gap packet is now visible as parked evidence with its own note, manifest, and survey route. The next same-lane follow-through is one focused promotion that teaches the shared Phase 4 validator and gate-evidence packet about that same `test_fsmount` note, manifest, and replay command before this lane widens into threshold-approval work.
