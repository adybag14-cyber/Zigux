...
- `PHASE4_EXACT_READBACK_HEAD=307d711664b6bd9b6dafcaf3c8709fdda939319b`
...
- the last recorded inspected head for this note is now `307d711664b6bd9b6dafcaf3c8709fdda939319b`, which tightened `scripts/zigux/check-phase4-gate-evidence.py` so the dedicated Phase 4 gate-evidence checker now also pins the dedicated workflow-route checker blob while keeping the broader shared Phase 4 packet unchanged; the current connector rerun refreshed the checker pin above, confirmed that the shared Phase 4 validator still sits at `PHASE4_VALIDATOR_BLOB_SHA=3d9d1bd148c457c01ad2ec5d5db6c32e70e9e7d8`, confirmed that the dedicated gate-evidence checker now lives at `PHASE4_GATE_EVIDENCE_CHECKER_BLOB_SHA=26df52e986040edb456a8430ca4d6fa787ec767d`, and confirmed that the checker still fails closed on explicit contract-checker status bullets, the dedicated workflow-route checker blob pin, and the dedicated scripts-root bitmap replay sentence inside this evidence note; the shared survey provenance still carried by `zigux/tests/phase4_runtime_atomic64_diff_survey.zig`, `zigux/tests/phase4_kprobe_example_survey.zig`, `zigux/tests/phase4_test_fsmount_survey.zig`, and `zigux/tests/phase4_perf_baseline_survey.zig`, and their sibling manifests remains `3ba64cd4e41a4de1c8fd8dbaecb23702ad9701a3`.
