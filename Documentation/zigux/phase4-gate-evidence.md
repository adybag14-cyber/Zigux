...
- `PHASE4_EXACT_READBACK_HEAD=5db89ade6f9199f3402048b38c59512e9051a8c5`
...
- `PHASE4_VALIDATOR_BLOB_SHA=3d9d1bd148c457c01ad2ec5d5db6c32e70e9e7d8`
- `PHASE4_GATE_EVIDENCE_CHECKER_BLOB_SHA=9370c11fd8b267276dadd5fc86160d78f8ebaf05`
...
- the last recorded inspected head for this note is now `5db89ade6f9199f3402048b38c59512e9051a8c5`, which landed the dedicated workflow-route checker tightening for the scripts-root bitmap replay route and paired shared-build marker while keeping the broader shared Phase 4 packet unchanged; the current connector rerun refreshed the checker pin above, confirmed that the shared Phase 4 validator still sits at `PHASE4_VALIDATOR_BLOB_SHA=3d9d1bd148c457c01ad2ec5d5db6c32e70e9e7d8`, confirmed that the dedicated gate-evidence checker now lives at `PHASE4_GATE_EVIDENCE_CHECKER_BLOB_SHA=9370c11fd8b267276dadd5fc86160d78f8ebaf05`, and confirmed that the checker still fails closed on explicit contract-checker status bullets while now also exact-counting the dedicated scripts-root bitmap replay sentence inside this evidence note; the shared survey provenance still carried by `zigux/tests/phase4_runtime_atomic64_diff_survey.zig`, `zigux/tests/phase4_kprobe_example_survey.zig`, `zigux/tests/phase4_test_fsmount_survey.zig`, and `zigux/tests/phase4_perf_baseline_survey.zig`, and their sibling manifests remains `3ba64cd4e41a4de1c8fd8dbaecb23702ad9701a3`.
...
The separate `PHASE4_WORKFLOW_ROUTE_CHECKER_BLOB_SHA` pin above therefore remains part of the shared validator's exact-readback contract for the dedicated workflow-route checker file itself, and the tightened checker now exact-counts the scripts-root `make -C zigux phase4-bitmap-diff` route plus the paired `phase4-bitmap-diff-tests` shared-build marker in addition to the earlier workflow and Makefile markers, while `PHASE4_GATE_EVIDENCE_TARGET_COUNT=17` continues to describe only the narrower gate-evidence-checker-enforced blob target set.
