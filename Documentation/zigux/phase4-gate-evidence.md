...
- The matrix, validator, shared build entrypoint, and `Documentation/zigux/artifact-diff.md` are aligned on the current live gate definitions, while the three root README surfaces still under-describe the four-surface build packet by not naming the runtime atomic64 survey gate explicitly.
