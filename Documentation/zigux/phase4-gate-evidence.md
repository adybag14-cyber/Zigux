...
- `PHASE4_EXACT_READBACK_HEAD=895e3c7d551e0a2dea26fbd962b4d324f90a2c39`
...
- `PHASE4_VALIDATOR_BLOB_SHA=831e0aec6c82c83dcb97d091f4896d34c5650541`
- `PHASE4_GATE_EVIDENCE_CHECKER_BLOB_SHA=49a8d5a4041f06775ab3901cbd2909f5a6dead8d`
...
- `PHASE4_SCRIPT_README_BLOB_SHA=3dacf8e4846b254d9855654797b7156df26fd51d`
...
- the last recorded inspected head for this note is now `895e3c7d551e0a2dea26fbd962b4d324f90a2c39`, which landed the dedicated gate-evidence checker self-test coverage for validator-blob drift while keeping the rest of the shared Phase 4 packet unchanged; the current connector rerun confirmed that the shared Phase 4 validator still sits at `PHASE4_VALIDATOR_BLOB_SHA=831e0aec6c82c83dcb97d091f4896d34c5650541`, that the dedicated gate-evidence checker now lives at `PHASE4_GATE_EVIDENCE_CHECKER_BLOB_SHA=49a8d5a4041f06775ab3901cbd2909f5a6dead8d`, that the checker still fails closed on explicit contract-checker status bullets instead of leaving those published results implicit in prose alone, and that it now also proves validator-blob drift inside its own synthetic coverage path; the shared survey provenance still carried by `zigux/tests/phase4_runtime_atomic64_diff_survey.zig`, `zigux/tests/phase4_kprobe_example_survey.zig`, `zigux/tests/phase4_test_fsmount_survey.zig`, and `zigux/tests/phase4_perf_baseline_survey.zig`, and their sibling manifests remains `3ba64cd4e41a4de1c8fd8dbaecb23702ad9701a3`.
...
- `Documentation/zigux/README.md` still keeps the current Phase 4 rollback-readiness packet visible from the docs root, including the dedicated `make -C zigux phase4-runtime-atomic64-diff`, `phase4-kprobe-example-survey`, `phase4-test-fsmount-survey`, `phase4-perf-baseline-survey`, and `phase4-bitmap-diff` local replay routes, the shared `phase4-runtime-atomic64-diff-tests`, `phase4-runtime-atomic64-diff-survey-tests`, `phase4-kprobe-example-survey-tests`, `phase4-test-fsmount-survey-tests`, `phase4-perf-baseline-survey-tests`, and `phase4-bitmap-diff-tests` build entries, the dedicated `check-phase4-gate-evidence.py` checker, the dedicated `check-phase4-workflow-route-counts.py` checker, and the current `perf_thresholds_unapproved_until_bounded_phase4_benchmarks_land` posture, so the docs-root summary now matches the isolated runtime atomic64 and bitmap route wording carried elsewhere in this packet.
...
- `scripts/zigux/README.md` now exposes the full current Phase 4 scripts-root packet too: it names the shared validator, the dedicated gate-evidence checker, the dedicated `check-phase4-kprobe-example-packet.py` checker, the dedicated `check-phase4-workflow-route-counts.py` checker, the `phase4-runtime-atomic64-diff`, `phase4-kprobe-example-survey`, `phase4-test-fsmount-survey`, `phase4-perf-baseline-survey`, and `phase4-bitmap-diff` local replays, and the `phase4-runtime-atomic64-diff-tests`, `phase4-runtime-atomic64-diff-survey-tests`, `phase4-kprobe-example-survey-tests`, `phase4-test-fsmount-survey-tests`, `phase4-perf-baseline-survey-tests`, and `phase4-bitmap-diff-tests` shared-build entries together.
...
The current Phase 4 rollback-ownership and lab-matrix packet remains reviewable on live `master` for the gate-definition files enumerated above: the matrix, validator, dedicated gate-evidence checker, dedicated workflow-route checker, Makefile, workflow, the two shipped rollback gates, the four exact blob-ledger survey packets, and the three index surfaces named above still match the exact blob ledger recorded in this note, and the docs-root summary now matches the isolated runtime atomic64 and bitmap replay wording carried elsewhere in the same packet.
