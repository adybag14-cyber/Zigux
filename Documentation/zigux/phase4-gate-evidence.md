...
- `Documentation/zigux/artifact-diff.md`, `Documentation/zigux/README.md`, and `scripts/zigux/README.md` now point at the full currently shipped Phase 4 rollback-readiness packet on `master`, while `zigux/tests/README.md` still keeps the shared rollback packet, the dedicated local-only perf-baseline pair, and the parked `test_fsmount` packet explicit but does not yet list the adjacent parked kprobe gap survey in its Phase 4 reminder.
...
- The current exact-readback note is aligned again to the live validator, workflow, Makefile, and the full docs-root and scripts-root Phase 4 packet on `master`; `zigux/tests/README.md` now explicitly carries the shipped local-only perf-baseline pair `zigux/tests/phase4_perf_baseline_manifest.json` plus `zigux/tests/phase4_perf_baseline_survey.zig`, and the parked `test_fsmount` gap packet is visible there as adjacent evidence, but the tests-root Phase 4 reminder still omits the parked kprobe gap survey even though this exact-readback note and the dedicated local survey wrapper already keep it explicit.
