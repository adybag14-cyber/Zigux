...
- the shared Phase 4 reminder surfaces now keep that split explicit too, so the next same-lane follow-through is the policy decision recorded in `Documentation/zigux/phase4-validation-matrix.md` and `Documentation/zigux/review-checklist.md`: keep those approved limits local-only or intentionally promote a broader shared-CI perf-coverage claim.
