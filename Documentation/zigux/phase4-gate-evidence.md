...
- The current exact-readback note is aligned again to the live validator, README, workflow, Makefile, and Phase 4 gate surfaces on `master`, and `zigux/tests/README.md` now explicitly carries the shipped local-only perf-baseline pair `zigux/tests/phase4_perf_baseline_manifest.json` plus `zigux/tests/phase4_perf_baseline_survey.zig`, so the parked kprobe gap packet remains adjacent evidence only and the next same-lane follow-through is to land one manifest-backed Phase 4 `test_fsmount` gap survey packet before this lane widens into threshold-approval work.
