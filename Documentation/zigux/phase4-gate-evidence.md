...
- The matrix, validator, shared build entrypoint, `Documentation/zigux/artifact-diff.md`, and `Documentation/zigux/README.md` are aligned on the current live gate definitions, while the scripts-root and tests-root README summaries still stop short of naming the runtime atomic64 survey gate as a distinct build surface.
