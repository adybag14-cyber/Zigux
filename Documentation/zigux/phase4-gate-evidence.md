...
- `PHASE4_BUILD_BLOB_SHA=50ceb87e3bb8424b2d8ae859a54d21e6e41bfc82`
...
- `PHASE4_DOC_README_BLOB_SHA=83396d825e8ff9c16e9b8ad732726d7e6adb056f`
...
- The shared build packet on current `master` now wires five named test steps: `phase4-runtime-atomic64-diff-tests`, `phase4-runtime-atomic64-diff-survey-tests`, `phase4-bitmap-diff-tests`, `phase4-bitmap-live-helper-replay-tests`, and `phase4-perf-baseline-survey-tests`.
- Direct GitHub readback still returns `404` for `zigux/tests/phase4_perf_baseline_survey.zig`, `zigux/tests/phase4_perf_baseline_manifest.json`, and `Documentation/zigux/phase4-perf-baseline-survey.md`, so the shared build entrypoint is currently ahead of the readable shipped Phase 4 survey packet rather than proving a live committed perf-baseline lane.
- `Documentation/zigux/artifact-diff.md` still truthfully names the four shipped rollback-readiness replays it owns, while the three root README summaries keep the validator route, the roadmap-facing `zigux/tests/atomic64_diff.zig` wrapper, the shared `zigux/tests/runtime_atomic64_diff.zig` replay body, `zigux/tests/bitmap_diff.zig`, and the shared `zigux/tests/phase4_build.zig` route explicit without yet enumerating `zigux/tests/phase4_runtime_atomic64_diff_survey.zig` as a distinct shipped build gate.
...
- Current `master` also does not ship shared-gate blob targets for `phase4_kprobe_example` or `phase4_test_fsmount`; the live matrix still treats those as remaining roadmap gaps rather than part of the shipped validator-backed gate packet that should be blob-pinned here.
...
- The matrix, validator inventory, and `Documentation/zigux/artifact-diff.md` are still aligned on the current bounded rollback packet, but the shared build entrypoint has moved ahead by referencing a missing perf-baseline survey packet, and the three root README surfaces still under-describe the runtime atomic64 survey gate.
- The remaining roadmap-backed gaps are otherwise unchanged and still truthful in the matrix: `samples/zigux/kprobe_example.zig` remains absent, `samples/zigux/test_fsmount.zig` remains absent, and hard perf thresholds for the shipped atomic64 and bitmap rollback gates remain intentionally unapproved.