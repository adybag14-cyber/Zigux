...
- `PHASE4_VALIDATOR_BLOB_SHA=b7bf9a78846e5e6dd2114d1b0346d04aea0d9120`
...
- `PHASE4_MAKEFILE_BLOB_SHA=5198cf012bf6508725d0b3a3ef7f98eb3f6fef3d`
...
- `Documentation/zigux/artifact-diff.md` truthfully names that four-surface build packet, while the three root README summaries still keep the validator route, the roadmap-facing `zigux/tests/atomic64_diff.zig` wrapper, the shared `zigux/tests/runtime_atomic64_diff.zig` replay body, `zigux/tests/bitmap_diff.zig`, and the shared `zigux/tests/phase4_build.zig` route explicit without yet enumerating `zigux/tests/phase4_runtime_atomic64_diff_survey.zig` as a distinct shipped build gate.
...
- The matrix, validator, shared build entrypoint, and `Documentation/zigux/artifact-diff.md` are aligned on the current live gate definitions, while the three root README surfaces still under-describe the four-surface build packet by not naming the runtime atomic64 survey gate explicitly and the standalone gate-evidence checker still describes a broader, not-yet-shipped packet.
