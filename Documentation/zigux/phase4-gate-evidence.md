...
- `PHASE4_VALIDATION_MATRIX_BLOB_SHA=10b2c86768ecf61360ed7eb7ba38cbe4c9354e29`
...
- `PHASE4_MAKEFILE_BLOB_SHA=0d6720521b86f711bac2b612f8d5f6ada29f7f5f`
- `PHASE4_WORKFLOW_BLOB_SHA=4bfe161e7ab127b67337b85f10ba38d3d9ec20cf`
- `PHASE4_DOC_README_BLOB_SHA=a3269a7acb1e6516594229f2678d90db754fa025`
- `PHASE4_SCRIPT_README_BLOB_SHA=4eff17fd6421dcc08cc178d73af36096d8e05940`
- `PHASE4_TESTS_README_BLOB_SHA=a969fc8a7a25714b3e689f33a932b8fb54158c43`
...
- `PHASE4_RUNTIME_ATOMIC64_MANIFEST_BLOB_SHA=a660498b29cd883f2146384b5369645bc9f6ab4b`
- `PHASE4_RUNTIME_ATOMIC64_SURVEY_BLOB_SHA=bab1c9817666c91de202ae63f2df7da6430dd9e9`
- `PHASE4_RUNTIME_ATOMIC64_REVIEW_CHECKLIST_BLOB_SHA=a64c4400f26216b266c5733fdf0b8bbd205a4b5b`
...
- `Documentation/zigux/artifact-diff.md`, `Documentation/zigux/README.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` now point at the full currently shipped Phase 4 rollback-readiness packet on `master`, including the adjacent parked kprobe gap survey beside the dedicated local-only perf-baseline pair and the parked `test_fsmount` packet.
...
- That published eighteen-case self-test catalog now also exercises the runtime atomic64 packet's `validate-phase4.py`, `phase4-validation-matrix.md`, and `Documentation/zigux/review-checklist.md` manifest and survey blob drift paths inside the existing manifest-backed drift coverage, and it now checks the adjacent kprobe gap packet's note or manifest presence drift plus the shipped perf-baseline packet's manifest-presence drift path and the adjacent `test_fsmount` gap packet's manifest-presence drift path too, so those validator, matrix, reviewer-checklist, kprobe packet, perf-baseline packet, and parked `test_fsmount` packet expectations are no longer an unstated self-test gap.
...
- The shipped local perf-baseline survey packet is intentionally separate from that shared exact-readback set: it now keeps the approved local-only atomic64 benchmark-command and acceptable-limit evidence plus the still-pending bitmap acceptable-limit posture machine-checked without turning the Phase 4 validator or CI path into a shared perf-approval claim.
...
## Current Conclusion
- shared Phase 4 perf coverage remains intentionally unapproved: the local-only atomic64 acceptable limit is now approved inside the dedicated survey packet, while the bitmap acceptable limit remains pending.
- the dedicated local perf-baseline survey packet is still the truthful way to keep that split posture measurable: it exact-pins the approved local-only atomic64 command-and-limit evidence while keeping the bitmap acceptable-limit gap explicit until one bounded threshold packet lands.
