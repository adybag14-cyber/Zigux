...
This keeps the lane honest: current `master` has real bounded DMA, scatterlist, iounmap, iomap planning, helper-side iomap cleanup-handoff evidence, dedicated build-shard entrypoints for the zero-sized coherent replay and helper-first scatterlist replay, and one current-packet consistency checker, but it still does not claim live DMA allocation side effects, generic DMA mapping ownership, live scatterlist ownership, `sg_table` lifecycle control, helper-first non-posted or arch-memtype planners, live MMIO mappings, live device-tree walks, arch memtype mutation, or the older broader direct helper packet.
