...
- reviewed against live `master` `032c57dbde9a95ad6e28ad891cca54bd2e3bfcf1`
- scope: the landed `lib/devres.zig` helper lab, its dedicated Phase 13 test, the focused reviewability gate, the dedicated coherent-DMA boundary replay, the dedicated boundary-evidence gate, the shared Phase 13 build and make wiring, and the lane notes that compare the current helper-only DMA/scatterlist boundary against the roadmap
- product boundary:
  - `lib/devres.zig`
...
- `lib/devres.zig` already models the starter `__devm_ioremap()` lifetime split between retained release records and free-on-failure cleanup, keeps `devm_iounmap()` and `devm_ioport_unmap()` pointer matching exact, and now exposes direct `devm_ioremap_uc()`, `devm_ioremap_wc()`, and `devm_ioremap_np()`-style wrappers as pure uncached, write-combined, and non-posted lifetime-planning steps.
...
- the dedicated `zigux/tests/phase13_devres_boundary_evidence.zig` gate now cross-checks exact boundary evidence across the manifest, slice note, and survey note so the blocked `dmam_alloc_*`, `dma_map_sgtable()`, `struct scatterlist`, and `sg_table lifecycle` families remain explicit and aligned.
- the shared Phase 13 build and make target already replay the devres packet, so the remaining lane-local gap is not new helper behavior first. It is keeping the helper-only DMA/scatterlist boundary, the exact `devm_iounmap()` and `devm_ioport_unmap()` release matching, and the already-landed uncached, WC, and non-posted wrapper surface explicit and machine-checkable wherever the survey packet records current Phase 13 evidence.
- exact boundary evidence on current `master`: `lib/devres.zig` still exposes no `dmam_alloc_*`, `dma_map_*`, `dma_unmap_*`, `dma_sync_*`, `dma_map_sgtable()`, the plain `scatterlist` token, `struct scatterlist`, `sg_table`, or `sg_*` ownership surface; the shipped planner set still stops at helper-first ioremap, ioport release matching, translated-resource, and WC memtype bookkeeping.
