...
- scope: the docs-side devres slice note, the dedicated `dmam_alloc_coherent()` helper and replay, the planning notes and manifests, the direct DMA-boundary replay, the helper-first scatterlist helper and replay, the roadmap-backed `lib/devres.c` anchor, and the missing MMIO or iomap helper-first packet that this lane still has to keep visible
- product boundary:
