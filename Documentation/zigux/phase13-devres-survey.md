...
- exact helper-source readback on current `master` still shows `lib/devres.zig` touching no live device lists, no live MMIO side effects, and no live arch memtype mutation; the shipped planner set still stops at helper-first ioremap, translated-resource, region-bookkeeping, and WC token bookkeeping.
