...
- scope: the docs-side devres slice note, the dedicated `dmam_alloc_coherent()` helper and replay, the planning notes and manifests, the direct DMA-boundary replay, the helper-first scatterlist helper and replay, the dedicated scatterlist helper slice and build shard, the dedicated DMA-boundary checker, the dedicated MMIO packet checker, the roadmap-backed `lib/devres.c` anchor, and the missing MMIO or iomap helper-first packet that this lane still has to keep visible
- product boundary:
...
- the live `check-phase13-devres-dma-boundary.py` checker still fail-closes on the helper-source blocked DMA and scatterlist markers plus the coupled survey wording that the coherent-DMA replay depends on.
...
- the live `check-phase13-devres-mmio-packet.py` packet guard requires the survey note, slice note, planner note, scatterlist slice, planner manifest, focused replays, helper-local MMIO absences, scatterlist helper markers, the dedicated DMA-boundary checker, and the scatterlist build shard to agree before the bounded MMIO packet can pass.
