...
- `lib/devres.zig` already models the starter `__devm_ioremap()` lifetime split between retained release records and free-on-failure cleanup, keeps `devm_iounmap()` pointer matching exact, and now exposes direct `devm_ioremap_uc()`, `devm_ioremap_wc()`, and `devm_ioremap_np()`-style wrappers as pure uncached, write-combined, and non-posted lifetime-planning steps.
...
- the shared Phase 13 build and make target already replay the devres packet, so the remaining lane-local gap is not new helper behavior first. It is keeping the helper-only DMA/scatterlist boundary and the already-landed uncached, WC, and non-posted wrapper surface explicit and machine-checkable wherever the survey packet records current Phase 13 evidence.
