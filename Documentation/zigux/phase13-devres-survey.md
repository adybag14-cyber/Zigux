...
Current `master` still carries a real helper-first `lib/devres.zig` foothold for managed ioremap lifetime, pure `devm_of_iomap()` translation handoff, adjacent pure ioport detach bookkeeping, and WC token bookkeeping. The highest-value bounded work in this lane is therefore to keep that MMIO-facing packet explicit about what has landed and what live MMIO, live ioport, live device-tree, live region, live DMA-backed mapping, live scatterlist, and arch memtype state is still blocked, while DMA or scatterlist ownership stays outside this helper-local packet.

## Survey findings

- `lib/devres.zig` still models the starter `__devm_ioremap()` lifetime split between retained release records and free-on-failure cleanup, keeps `devm_iounmap()` pointer matching exact through the dedicated `planManagedIounmap()` planner, and preserves the pure `devm_ioremap()`, `devm_ioremap_uc()`, `devm_ioremap_wc()`, and `devm_ioremap_np()` wrapper planners.
- the same helper lab also keeps adjacent pure ioport detach bookkeeping explicit: `planManagedIoportMap()` models `devm_ioport_map()` release-record retention versus free-on-failure cleanup, while `planManagedIoportUnmap()` keeps `devm_ioport_unmap()` pointer matching exact without claiming live ioport allocation, mapping, or unmapping side effects.
...
- that same `__devm_ioremap_resource()` helper packet still stops at planning-time region bookkeeping: it records whether a mapping would request a region and whether remap failure would unwind that reservation, but it still does not claim live `request_mem_region()` or `release_mem_region()` side effects.
...
- exact helper-source readback on current `master` still shows `lib/devres.zig` touching no live device lists, no live MMIO side effects, no live ioport side effects, and no live arch memtype mutation; the shipped planner set still stops at helper-first ioremap, adjacent pure ioport detach bookkeeping, translated-resource, region-bookkeeping, and WC token bookkeeping.
...
- helper-source readback also now makes the next packet-truthfulness follow-through clearer: the shipped ioport planners are real current-`master` evidence in `lib/devres.zig`, but the manifest-backed devres packet still centers the MMIO-facing gap map, so the next same-lane validation step should decide whether those ioport planners now need dedicated manifest and checker accounting rather than leaving them implicit inside helper-source readback alone.
...
- the same helper readback also still shows `.provides_ioport_lifetime_planning = true`, `.provides_ioport_unmap_call_planning = true`, `pub const ManagedIoportMapResult`, `pub const ManagedIoportUnmapPlan`, `pub fn planManagedIoportMap(`, and `pub fn planManagedIoportUnmap(` in `lib/devres.zig`, so the adjacent helper-local ioport detach-bookkeeping packet is shipped current-`master` evidence instead of survey-only prose.
- the same helper readback still shows `.provides_ioremap_plain_wrapper_planning = true`, `.provides_ioremap_uc_wrapper_planning = true`, `.provides_ioremap_wc_wrapper_planning = true`, `.provides_ioremap_np_wrapper_planning = true`, `pub fn planManagedIoremapAcquirePlain(`, `pub fn planManagedIoremapAcquireUc(`, `pub fn planManagedIoremapAcquireWc(`, and `pub fn planManagedIoremapAcquireNp(` in `lib/devres.zig`, so the full pure wrapper packet remains present on current `master` instead of stopping at the older UC or WC-only wording.
- the same helper readback still shows `.provides_arch_io_wc_memtype_planning = true`, `pub const ManagedMemtypeReserveInput`, `pub const ManagedMemtypeReservePlan`, and `pub fn planArchIoReserveMemtypeWc(` in `lib/devres.zig`, so the `devm_arch_io_reserve_memtype_wc()` detach-cleanup planner remains part of the current helper-first packet.
- the same helper readback also still shows `pub fn planManagedIoremapResource(`, `.requests_region = true`, and `.releases_region_on_remap_failure = true` in `lib/devres.zig`, so the `__devm_ioremap_resource()` packet still records request-region intent and remap-unwind planning without claiming live region side effects.
...
- `Documentation/zigux/phase13-devres-slice.md` still names `devm_iounmap()`, `devm_ioremap_uc()`, `devm_ioremap_wc()`, `devm_of_iomap()`, `devm_arch_io_reserve_memtype_wc()`, and `devm_arch_phys_wc_add()` as shipped helper-first evidence, and now also keeps the adjacent `devm_ioport_map()` plus `devm_ioport_unmap()` planners explicit as helper-local detach bookkeeping rather than leaving them implicit inside the source file alone.
...
- blocked `phase13-devres-live-region-reservation`
...
This keeps the lane explicit without overstating progress: Zigux has a real helper-first MMIO safety foothold for managed ioremap lifetime planning, exact `devm_iounmap()` matching, adjacent pure `devm_ioport_map()` plus `devm_ioport_unmap()` detach bookkeeping, pure translated-resource `devm_of_iomap()` handoff, detach-time WC memtype bookkeeping, detach-time WC token bookkeeping, the direct devres replay, the direct boundary-evidence replay, and reviewability guards, but it still does not claim live MMIO mappings, live ioport side effects, live region reservation, live release-region mutation, live device-tree walking, live arch memtype state transitions, live DMA-backed mappings, live scatterlist ownership, or `sg_table` lifecycle control, and it still keeps DMA-backed helper ownership outside this helper-local packet.
...
- live ioport allocation, mapping, or unmapping side effects
