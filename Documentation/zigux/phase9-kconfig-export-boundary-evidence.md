...
- `confdata_bridge.zig` currently sets `config_prefix = "CONFIG_"`, truncates lines at the first NUL byte, trims a leading UTF-8 BOM and trailing carriage return, parses `# CONFIG_FOO is not set`, and classifies values as tristate, string, or plain value.
