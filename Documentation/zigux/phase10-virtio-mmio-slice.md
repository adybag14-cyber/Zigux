...
- `PHASE10_SLICE=virtio-mmio-queue-register-helper`
- scope: bounded MMIO register offsets, device-feature page selection, driver-feature page writes, queue-select and queue-size planning, queue-ready bookkeeping, status and reset bookkeeping, config-generation tracking, interrupt-status acknowledge bookkeeping, dedicated Phase 10 MMIO tests, and a slice note only
- product boundary:
...
The live repo now has the virtio core, ring, and input lab footholds plus the earlier MMIO survey lane. This slice lands the next honest follow-on after the register-window starter: an in-memory queue-register helper that models only the smallest reviewable queue-select, queue-size, and queue-ready surface from `virtio_mmio.c` without pretending to own queue notify side effects, probe and remove lifecycle, or DMA-facing transport work.

## Landed starter surface

- module descriptor metadata anchored to `drivers/virtio/virtio_mmio.c`
- bounded register-offset constants for the device-features, driver-features, queue-select, queue-size, queue-ready, interrupt, status, and config-generation window
...
- dedicated reset bookkeeping that clears in-memory queue size and ready state without claiming queue teardown parity
...
- queue notify side effects or queue setup and teardown parity
...
Stay in the Phase 10 MMIO lane and add one small queue-notify snapshot helper next without widening into probe, remove, DMA, or full interrupt-handler paths.
