...
- lane key: `P13-L18`
- surveyed commit: `master-readback-2026-05-23`
- surveyed state: `current master` readback refreshed on `2026-05-23`
...
As of `2026-05-23`, current `master` keeps this adjacent notifier-facing packet explicit through:
...
- `include/zigux/abi.h` blob `bf54fd9503bb5106fde8ba30705f36d7f4527acb` mirrors the same bounded C-side packet through `struct zigux_notifier_block`, `struct zigux_list_head`, `struct zigux_hlist_head`, `zigux_notifier_chain_has_nonincreasing_priority()`, `zigux_notifier_first_chain_priority_increase()`, `zigux_list_first_broken_backlink()`, `zigux_list_has_consistent_backlinks()`, `zigux_hlist_first_broken_prev_link()`, and `zigux_hlist_has_consistent_prev_links()`.
...
- `zigux/tests/phase13_notifier_list_manifest.json` blob `6b05e09ad6beccb74c0fc8d7bc408c02103c278a`, `zigux/tests/phase13_notifier_list_reviewability.zig` blob `a21c7f7deb7a5240e775c193edc224f409d6d64e`, `scripts/zigux/check-phase13-notifier-packet.py` blob `e1f20086652c55b4deb366f372bb81ddda7a4bae`, and `scripts/zigux/validate-phase13-release.py` blob `3e3afacdc4aef07dbd139b9347f4400f2ca0ede3` still gate the adjacent packet and still frame the missing notifier-chain helper, dedicated notifier ABI header, shared Phase 13 build shard, and Makefile route names as repo-reality gaps rather than shipped evidence.
The shipped adjacent `zigux/bindings/notifier_abi.zig` foothold keeps notifier priority ordering plus `list_head` and `hlist` layout witnesses explicit through `NotifierBlock`, `ListHead`, `HListHead`, `HListNode`, `chainHasNonincreasingPriority()`, `firstChainPriorityIncrease()`, `firstBrokenBacklink()`, `listHasConsistentBacklinks()`, `firstBrokenPrevLink()`, and `hlistHasConsistentPrevLinks()` without claiming callback execution, registration, SRCU, or blocking-notifier semantics.
