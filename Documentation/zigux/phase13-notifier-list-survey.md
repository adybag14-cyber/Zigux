...
- lane key: `P13-L18`
- surveyed commit: `master-readback-2026-05-24`
- surveyed state: `current master` readback refreshed on `2026-05-24`
...
As of `2026-05-24`, current `master` keeps this adjacent notifier-facing packet explicit through:
...
- `zigux/bindings/notifier_abi.zig` blob `80dd7a4cadefdb5f135b2a6c26b7ee1a3ca0586e` keeps the bounded read-only ABI foothold explicit through `NotifierResult`, `resultFromInt()`, `resultIsKnown()`, `resultStopsChain()`, `NotifierBlock`, `NotifierChainPriorityIncrease`, `ListHead`, `HListHead`, `HListNode`, `chainHasNonincreasingPriority()`, `firstChainPriorityIncrease()`, `firstBrokenBacklink()`, `listHasConsistentBacklinks()`, `firstPprevMatchesHead()`, `firstBrokenPrevLink()`, and `hlistHasConsistentPrevLinks()`, plus layout and witness tests for result-alignment, empty, single-node, descending, and broken-link cases.
...
- `zigux/tests/phase13_notifier_list_manifest.json` blob `7072758250b75e4450e31ed654b044d2f15a5553`, `zigux/tests/phase13_notifier_list_reviewability.zig` blob `af563c39c0010700f1655d093b143ce279f6a00c`, `scripts/zigux/check-phase13-notifier-packet.py` blob `cf73cf083abb227a5abbfbccc4cbe2ec9a90788b`, and `scripts/zigux/validate-phase13-release.py` blob `ea3088e5bf457f1ff522a5fd513f14e046827d3e` still gate the adjacent packet and still frame the missing notifier-chain helper, dedicated notifier ABI header, shared Phase 13 build shard, and Makefile route names as repo-reality gaps rather than shipped evidence.
The shipped adjacent `zigux/bindings/notifier_abi.zig` foothold keeps notifier priority ordering plus `list_head` and `hlist` layout witnesses explicit through `NotifierResult`, `NotifierBlock`, `ListHead`, `HListHead`, `HListNode`, `resultFromInt()`, `resultIsKnown()`, `resultStopsChain()`, `chainHasNonincreasingPriority()`, `firstChainPriorityIncrease()`, `firstBrokenBacklink()`, `listHasConsistentBacklinks()`, `firstPprevMatchesHead()`, `firstBrokenPrevLink()`, and `hlistHasConsistentPrevLinks()` without claiming callback execution, registration, SRCU, or blocking-notifier semantics.
The shipped adjacent `zigux/helpers/list_view.zig` and `zigux/helpers/hlist_view.zig` helpers stay read-only: they walk `list_head` and `hlist` links, report backlink or prev-link consistency witnesses, keep the first-node `pprev` versus head witness explicit through `firstPprevMatchesHead()`, and stop short of mutation or notifier callback behavior.
...
The focused checker `scripts/zigux/check-phase13-notifier-packet.py` now keeps that adjacent packet fail-closed around the survey note, manifest, reviewability gate, ABI foothold, the exported first-node `pprev` witness, read-only list helpers, and the Linux-side HVC notifier declarations. The focused Zig gate `zigux/tests/phase13_notifier_list_reviewability.zig` keeps the manifest and checker visible without widening the shared Phase 13 build packet.
...
- it keeps the shipped `zigux/bindings/notifier_abi.zig` plus `include/zigux/abi.h` ABI footholds explicit as adjacent notifier and list/hlist interop evidence, including the exported first-node `pprev` witness `zigux_hlist_first_pprev_matches_head()`
