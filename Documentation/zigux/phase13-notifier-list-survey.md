...
- `zigux/bindings/notifier_abi.zig` blob `257e14f84906628e67fbd9505105aaf797b01fc3` keeps the bounded read-only ABI foothold explicit through `NotifierResult`, `resultFromInt()`, `resultIsKnown()`, `resultStopsChain()`, `NotifierBlock`, `NotifierChainPriorityIncrease`, `ListHead`, `HListHead`, `HListNode`, `chainHasNonincreasingPriority()`, `firstChainPriorityIncrease()`, `firstBrokenBacklink()`, `listHasConsistentBacklinks()`, `firstBrokenPrevLink()`, and `hlistHasConsistentPrevLinks()`, plus layout and witness tests for result-alignment, empty, single-node, descending, and broken-link cases.
...
- `zigux/tests/phase13_notifier_list_manifest.json` blob `2817aba0c6a07212757a895b416dd82aed256bf2`, `zigux/tests/phase13_notifier_list_reviewability.zig` blob `c228b2caddb062bd04187ccbd983e7fbba63ceec`, `scripts/zigux/check-phase13-notifier-packet.py` blob `240704910abd1751b4971d0d0ccea6fca7f6b2c9`, and `scripts/zigux/validate-phase13-release.py` blob `ea3088e5bf457f1ff522a5fd513f14e046827d3e` still gate the adjacent packet and still frame the missing notifier-chain helper, dedicated notifier ABI header, shared Phase 13 build shard, and Makefile route names as repo-reality gaps rather than shipped evidence.
The shipped adjacent `zigux/bindings/notifier_abi.zig` foothold keeps notifier priority ordering plus `list_head` and `hlist` layout witnesses explicit through `NotifierResult`, `NotifierBlock`, `ListHead`, `HListHead`, `HListNode`, `resultFromInt()`, `resultIsKnown()`, `resultStopsChain()`, `chainHasNonincreasingPriority()`, `firstChainPriorityIncrease()`, `firstBrokenBacklink()`, `listHasConsistentBacklinks()`, `firstBrokenPrevLink()`, and `hlistHasConsistentPrevLinks()` without claiming callback execution, registration, SRCU, or blocking-notifier semantics.
