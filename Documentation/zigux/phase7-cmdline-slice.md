...
Keep the helper slice parked for behavior, fixtures, and cmdline-owned reminder wording: current `master` already carries `zigux/tests/phase7_cmdline_manifest.json` across `scripts/zigux/README.md`, `zigux/tests/README.md`, `Documentation/zigux/review-checklist.md`, and `Documentation/zigux/README.md`, and `scripts/zigux/validate-phase7.py` already fail-closes on those cmdline manifest reminders as well as the manifest file itself.

The next honest cmdline-owned follow-up is narrower still: reopen only if fresh repo inspection finds a new cmdline-local parity, survey, manifest, fixture, or shared reminder drift inside the parked `next_arg()`, `get_option()`, `get_options()`, `memparse()`, and `parse_option_str()` packet. The broader Phase 7 lane still carries one separate validator-only follow-through outside this slice: live `scripts/zigux/validate-phase7.py` does not yet exact-count the newly explicit `Documentation/zigux/review-checklist.md` markers in `Documentation/zigux/phase7-string-helpers-slice.md` and `Documentation/zigux/phase7-rbtree-slice.md`, so that one-file hardening should land on its own rather than being treated as a cmdline slice reopening.
