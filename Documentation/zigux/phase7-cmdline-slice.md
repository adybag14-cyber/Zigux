...
* lane state: the helper, dedicated test, dedicated survey, dedicated packet checker, committed manifest packet, and committed `nextArg()` fixture remain visible on current `master`; keep the cmdline packet parked unless a fresh parity, survey, manifest, fixture, checker, or same-slice reminder drift appears inside that landed helper-local packet
* product boundary:
...
- keep range parsing, suffix-aware size parsing, exact bare-option matching for comma-delimited flags, and `nextArg()` token splitting reviewable through deterministic Zig replays plus one dedicated packet checker
...
3. keep the dedicated packet checker explicit
...
6. keep the shared Phase 7 helper gate explicit as a shared cross-packet route
...
That means the dedicated cmdline helper replay, dedicated cmdline survey, and dedicated packet checker remain reviewable inside this slice, while the broader shared `phase7_build.zig` route is again a present shared replay surface on `master` rather than a missing-sibling blocker.
...
Keep this cmdline slice parked unless fresh repo inspection finds one concrete cmdline parity, survey, manifest, fixture, checker, or same-slice reminder drift inside the current helper packet.
