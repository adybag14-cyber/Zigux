...
Keep the helper slice parked unless fresh repo inspection shows one real same-lane reviewability gap inside the existing shared packet. The current shared follow-up is still open on `master`: `zigux/tests/phase7_cmdline_manifest.json` is already part of the parked cmdline packet, `scripts/zigux/validate-phase7.py` already requires that manifest file to exist, and `Documentation/zigux/review-checklist.md` already treats it as shipped review surface, but the broader shared owner-map reminders still omit that manifest path. The next safe step is to repair `zigux/tests/README.md` so it keeps `zigux/tests/phase7_cmdline_manifest.json` explicit beside the already-named cmdline survey and `next_arg()` fixture, then mirror that same manifest in `scripts/zigux/README.md` and tighten any remaining shared Phase 7 marker checks around the same reminder without widening into helper behavior, fixture expansion, or unrelated Phase 7 lanes.
