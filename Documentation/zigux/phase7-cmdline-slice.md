...
Keep the helper slice parked unless fresh repo inspection shows one real same-lane reviewability gap inside the existing shared packet. The current smaller shared follow-up is docs-only on `master`: `zigux/tests/phase7_cmdline_manifest.json` is already part of the parked cmdline packet, `scripts/zigux/validate-phase7.py` already fail-closes on that manifest path, and `Documentation/zigux/review-checklist.md` already treats it as shipped review surface, but `scripts/zigux/README.md` and `zigux/tests/README.md` still omit the cmdline manifest from their broad Phase 7 reminders. The next safe step is to align those shared owner maps around the committed cmdline manifest without widening into helper behavior, fixture expansion, or unrelated Phase 7 lanes.
