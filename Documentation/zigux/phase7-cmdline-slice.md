...
Keep the helper slice parked unless fresh repo inspection shows one real same-lane reviewability gap inside the existing shared packet. The current shared follow-up is narrower on current `master`: `zigux/tests/phase7_cmdline_manifest.json` is already part of the parked cmdline packet, `scripts/zigux/validate-phase7.py` already requires that manifest file to exist, `Documentation/zigux/review-checklist.md` and `Documentation/zigux/README.md` already treat it as shipped review surface, and the remaining shared owner-map gaps now sit in `scripts/zigux/README.md` and `zigux/tests/README.md`, which still omit `zigux/tests/phase7_cmdline_manifest.json` from their broader Phase 7 reminders. The next safe step is to repair one of those shared README reminders and then tighten any remaining shared Phase 7 marker checks around the same scripts-root or tests-root surface without widening into helper behavior, fixture expansion, or unrelated Phase 7 lanes.
