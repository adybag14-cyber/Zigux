...
Keep the helper slice parked unless fresh repo inspection shows one real same-lane reviewability gap inside the existing shared packet. The current smaller shared follow-up is docs-only on `master`: `zigux/tests/phase7_cmdline_manifest.json` is already part of the parked cmdline packet, `scripts/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and `scripts/zigux/validate-phase7.py` already treat it as shipped review surface, but `zigux/tests/README.md` still omits that manifest from its Phase 7 key-entrypoint list and its broad shared-packet reminder. The next safe step is to align the tests-root owner map around the committed cmdline manifest without widening into helper behavior, fixture expansion, or unrelated Phase 7 lanes.
