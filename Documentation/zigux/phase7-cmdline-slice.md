...
Current `master` still keeps the broader Phase 5 no-cmdline-sample reminder under-described from `Documentation/zigux/README.md`: the docs-root note names the helper, survey, manifest, fixture, and shared build entrypoint, but it still does not spell out `Documentation/zigux/phase7-make-wrapper-selftest-alignment.md`, `samples/zigux/README.md`, `scripts/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, `scripts/zigux/validate-phase7.py`, `scripts/zigux/check-phase7-make-wrapper.py`, `scripts/zigux/check-phase7-make-wrapper-selftest-alignment.py`, `scripts/zigux/check-phase7-build-wiring.py`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml` as shared parked-packet surfaces beside the landed cmdline helper, survey, manifest, fixture, and shared build entrypoint.
The next bounded same-lane step is to tighten that docs-root Phase 5 no-cmdline-sample reminder so it matches the fuller shared Phase 7 packet already recorded here and in `Documentation/zigux/review-checklist.md`.
