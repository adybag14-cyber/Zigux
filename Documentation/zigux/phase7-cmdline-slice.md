...
Keep the helper slice parked for behavior, fixtures, and cmdline-owned reminder wording: current `master` already carries `zigux/tests/phase7_cmdline_manifest.json` across `scripts/zigux/README.md`, `zigux/tests/README.md`, `Documentation/zigux/review-checklist.md`, and `Documentation/zigux/README.md`, and live `scripts/zigux/validate-phase7.py` now exact-counts those cmdline manifest reminders as well as the explicit `Documentation/zigux/review-checklist.md` markers in the sibling string-helpers and rbtree slice notes.

The next honest cmdline-owned follow-up is narrower still: reopen only if fresh repo inspection finds a new cmdline-local parity, survey, manifest, fixture, or shared reminder drift inside the parked `next_arg()`, `get_option()`, `get_options()`, `memparse()`, and `parse_option_str()` packet.
