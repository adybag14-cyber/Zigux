...
Keep the helper slice parked unless fresh repo inspection shows one real same-lane reviewability gap inside the existing shared packet. The current shared follow-up is narrower on current `master`: `zigux/tests/phase7_cmdline_manifest.json` is already part of the parked cmdline packet, `scripts/zigux/validate-phase7.py` already requires that manifest file to exist, `Documentation/zigux/review-checklist.md` already treats it as shipped review surface, and `scripts/zigux/README.md` already keeps that manifest explicit beside the cmdline survey and `next_arg()` fixture. The remaining same-lane owner-map gap sits in `zigux/tests/README.md`, which still omits `zigux/tests/phase7_cmdline_manifest.json` from the broader Phase 7 reminder. The next safe step is to repair that tests-root reminder and then tighten any remaining shared Phase 7 marker checks around the same tests-root surface without widening into helper behavior, fixture expansion, or unrelated Phase 7 lanes.
