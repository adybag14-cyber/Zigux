...
- among those shared summaries, the scripts-root helper inventory is the smallest direct truthfulness gap on current `master` because it is the validator-first helper catalog reviewers hit first and it still undercounts the landed header-family survey packet.
- compared against the roadmap-backed ABI substrate packet and the bootstrap ledger's Phase 3 skeleton scope, the smallest same-lane truthfulness gap is therefore no longer another adjacent `abi.h` family foothold. The smaller live gap is the scripts-root accounting drift around the already-landed `include/zigux/abi.h` review packet, with the broader shared-summary follow-through parked behind it.
...
- refresh `scripts/zigux/README.md` first so the current bootstrap helper inventory and the Phase 3 flow explicitly carry `scripts/zigux/validate-phase3-abi-header-family-survey.py` beside `Documentation/zigux/phase3-abi-header-family-survey.md`, `Documentation/zigux/phase3-abi-h-boundary-next-step.md`, and the starter `zigux/uapi/dev_t.zig` surface they help keep honest
- after that smaller scripts-root repair lands, refresh the broader shared review surfaces that summarize the same shipped ABI validator-support packet: `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and `zigux/tests/README.md`
