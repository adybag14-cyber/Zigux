...
- `scripts/zigux/validate-phase3.py` already requires that same landed packet, and the broad shared summaries are no longer uniformly stale: `Documentation/zigux/README.md` and `Documentation/zigux/review-checklist.md` now keep `Documentation/zigux/phase3-abi-h-boundary-next-step.md`, `Documentation/zigux/phase3-abi-header-family-survey.md`, and `scripts/zigux/validate-phase3-abi-header-family-survey.py` explicit inside the shared Phase 3 ABI packet.
- the remaining broad-summary drift is narrower on current `master`: `zigux/tests/README.md` still undercounts the focused validator-support packet by omitting `scripts/zigux/validate-phase3-abi-header-family-survey.py`, and `scripts/zigux/README.md` still keeps that checker explicit in the Phase 3 flow note but not in the top-level `Current bootstrap helpers` inventory.
- among those shared reminders, the scripts-root helper inventory is the smallest direct truthfulness gap on current `master` because it is a one-entry omission inside an already-shipped file family that otherwise points reviewers at the correct `make -C zigux phase3-validate` route and the now-landed header-family survey packet.
- compared against the roadmap-backed ABI substrate packet and the bootstrap ledger's Phase 3 skeleton scope, the smallest same-lane truthfulness gap is therefore no longer another adjacent `abi.h` family foothold or a docs-root accounting repair. The smaller live gap is the scripts-root helper-inventory undercount around the already-landed header-family survey checker, with the broader tests-root validator-support reminder follow-through parked behind it.
...
- refresh `scripts/zigux/README.md` first so the top-level `Current bootstrap helpers` inventory explicitly carries `validate-phase3-abi-header-family-survey.py` beside the other Phase 3 validators already wired into `make -C zigux phase3-validate`
- after that smaller scripts-root repair lands, refresh the broader tests-root validator-support reminder in `zigux/tests/README.md` so it also names `scripts/zigux/validate-phase3-abi-header-family-survey.py` beside `scripts/zigux/validate-phase3-abi-bindings-syntax.py` and `scripts/zigux/survey-phase3-abi-constant-parity.py`
- keep those shared reminders explicitly carrying `Documentation/zigux/phase3-abi-header-family-survey.md`, `scripts/zigux/validate-phase3-abi-header-family-survey.py`, and `Documentation/zigux/phase3-abi-h-boundary-next-step.md` as part of the current Phase 3 ABI packet instead of leaving them implicit behind the baseline constant-parity survey alone
...
- leave `include/zigux/abi.h`, `zigux/bindings/abi.zig`, `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, the landed header-family survey checker, and the shared Phase 3 validator logic untouched until the scripts-root helper inventory and tests-root reminder stop understating the already-landed packet
