...
- `scripts/zigux/validate-phase3.py` already requires that same landed packet, and `scripts/zigux/README.md` already carries the header-family survey inside both the helper inventory and the validator-support roster, so the current gap is no longer missing packet membership, another fresh `abi.h` foothold inside the live chrdev ladder, or a scripts-root inventory repair.
- the remaining drift is in the broad shared review summaries only: `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and `zigux/tests/README.md` still summarize the shared Phase 3 ABI packet without keeping the landed `Documentation/zigux/phase3-abi-h-boundary-next-step.md`, `Documentation/zigux/phase3-abi-header-family-survey.md`, and `scripts/zigux/validate-phase3-abi-header-family-survey.py` equally explicit.
- among those shared summaries, the docs-root Phase 3 summary is the smallest direct truthfulness gap on current `master` because it is the first broad product-facing reminder reviewers hit after the now-correct scripts-root inventory, and it still undercounts the landed header-family survey packet.
- compared against the roadmap-backed ABI substrate packet and the bootstrap ledger's Phase 3 skeleton scope, the smallest same-lane truthfulness gap is therefore no longer another adjacent `abi.h` family foothold or a scripts-root accounting repair. The smaller live gap is the docs-root accounting drift around the already-landed `include/zigux/abi.h` review packet, with the broader checklist and tests-root follow-through parked behind it.
...
- refresh `Documentation/zigux/README.md` first so the current broad Phase 3 summary explicitly carries `Documentation/zigux/phase3-abi-header-family-survey.md`, `Documentation/zigux/phase3-abi-h-boundary-next-step.md`, and `scripts/zigux/validate-phase3-abi-header-family-survey.py` beside the shared ABI slice and the starter `zigux/uapi/dev_t.zig` surface they help keep honest
- after that smaller docs-root repair lands, refresh the broader shared review surfaces that summarize the same shipped ABI validator-support packet: `Documentation/zigux/review-checklist.md` and `zigux/tests/README.md`
- make those shared summaries explicitly carry `Documentation/zigux/phase3-abi-header-family-survey.md`, `scripts/zigux/validate-phase3-abi-header-family-survey.py`, and `Documentation/zigux/phase3-abi-h-boundary-next-step.md` as part of the current Phase 3 ABI packet instead of leaving them implicit behind the baseline constant-parity survey alone
...
- leave `scripts/zigux/README.md`, `include/zigux/abi.h`, `zigux/bindings/abi.zig`, the landed header-family survey checker, and the shared Phase 3 validator logic untouched until those broader shared summaries stop understating the already-landed packet
