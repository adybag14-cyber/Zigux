...
Future Phase 11 follow-through should stay inside the next smallest hardware-validation matrix, focused replay, teardown-parity note, failure-mode note, registration-facing handoff note, or shared reviewability sync now that the shipped watchdog, header-boundary, and HVC packet is explicit, and it should avoid broader validator assets until those files actually land.
