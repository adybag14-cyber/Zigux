...
On current `master`, that planning note is still the only live DesignWare companion inside the shared Phase 11 packet, so this contract must not imply a landed `phase11-dw-wdt` survey, validation-matrix, teardown-note, manifest, registration-scaffold, or verify-helper packet.
