...
`Documentation/zigux/README.md`, `Documentation/zigux/phase11-shared-replay-contract.md`, `Documentation/zigux/phase11-closure-note.md`, and `Documentation/zigux/phase11-driver-lane-sequencing.md` keep the landed gpio teardown note explicit across the shared Phase 11 packet today, while `scripts/zigux/README.md`, `zigux/tests/README.md`, and `Documentation/zigux/review-checklist.md` still need a narrower reminder-surface follow-through before that gpio teardown note is named everywhere the shared packet is summarized. `Documentation/zigux/phase11-gpio-wdt-module-slice.md` keeps the platform-drvdata, reboot-glue, and register-device request ordering packet explicit for the gpio starter, `Documentation/zigux/phase11-gpio-wdt-teardown-note.md` keeps the bounded GPIO stop-policy, stop-transition, and teardown-handoff split explicit for the starter packet, `drivers/watchdog/dw_wdt_verify.zig` keeps reset-controlled versus continued-heartbeat remove, idle remove, idle stop, and IRQ-mode teardown-outcome replays explicit beside that note, `zigux/tests/phase11_gpio_wdt_platform_drvdata.zig` keeps the focused early `platform_set_drvdata()` ordering checkpoint explicit beside the shared route without implying shared-build coverage, `zigux/tests/phase11_hvc_cleanup.zig` keeps the bounded `hvc_cleanup()` tty-port release handoff, missing-reference failure mode, and teardown-gating replay reviewable without implying live tty teardown or host-backed cleanup, `drivers/tty/hvc/hvc_console_verify.zig` keeps compile-local final-close, hung-up or detached teardown, cleanup-prerequisite, notifierless-open, targetless-sysrq, no-dispatch sysrq notifier-deferral, never-registered notifier, targetless notifier, notifier-prerequisite, and khvcd worker-entry sleep, kick, poll-mask, minimum-timeout flooring, maximum-timeout clamping, timeout-backoff, and invalid-open-count replays beside the shared packet. The same verifier also keeps the impossible hangup buffered-write failure-mode replay explicit beside that shared packet. `Documentation/zigux/phase11-hvc-console-slice.md` keeps the active starter scope named beside the survey note, teardown note, and validation matrix so the dedicated HVC packet does not undercount its own current boundary surfaces, `Documentation/zigux/phase11-hvc-console-teardown-note.md` keeps the close, cleanup, remove, write-to-hangup, and hangup-disconnect ownership split explicit in one driver-local note, `zigux/tests/phase11_hvc_console_manifest.json` keeps the archival HVC landing checkpoint named alongside the slice note, survey note, and validation matrix, and `Documentation/zigux/phase11-closure-note.md` now keeps the parked shared simple-driver packet summarized in one bounded closure note so later runs do not have to reconstruct that shared closure state from the replay contract alone.
...
On current `master`, that older docs-root reread is already closed: `Documentation/zigux/README.md` still carries `Documentation/zigux/phase11-gpio-wdt-survey.md`, `Documentation/zigux/phase11-gpio-wdt-teardown-note.md`, and `Documentation/zigux/phase11-dw-wdt-survey.md` beside the watchdog matrices, HVC packet, and shared header-boundary note. The next shared-surface reread should therefore start with the smallest scripts-root drift in `scripts/zigux/README.md`: the broad Phase 11 summary sentence still skips the dedicated gpio teardown companion even though the Phase 11 note list already names `Documentation/zigux/phase11-gpio-wdt-teardown-note.md`. After that scripts-root fix lands, reopen `zigux/tests/README.md` before `Documentation/zigux/review-checklist.md` or any driver-local note; the broad Phase 11 tests-root summary still skips the dedicated DesignWare packet-checker route even though the shared contract, the DesignWare survey packet, and `scripts/zigux/README.md` all keep `scripts/zigux/check-phase11-dw-wdt-packet.py` explicit on `master`.
