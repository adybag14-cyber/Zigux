...
On current `master`, the older docs-root reread is already closed and the scripts-root summary now keeps the dedicated gpio teardown companion explicit beside the parked driver-lane owner map, the dedicated DesignWare teardown companion, and the dedicated HVC survey packet. Reopen `zigux/tests/README.md` before `Documentation/zigux/review-checklist.md` or any driver-local note; the broad Phase 11 tests-root summary still skips the dedicated DesignWare packet-checker route even though the shared contract, the DesignWare survey packet, and `scripts/zigux/README.md` all keep `scripts/zigux/check-phase11-dw-wdt-packet.py` explicit on `master`.
