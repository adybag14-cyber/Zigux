...
- `lib/argv_split.zig` plus `zigux/tests/phase7_argv_split_survey.zig`, `zigux/tests/phase7_argv_split_manifest.json`, and `scripts/zigux/check-phase7-argv-split-packet.py` are directly readable, but the older helper-local slice, dedicated test, fixture, and shared build-or-validation companions are not directly readable in the same reread, so treat it as a helper-plus-survey-manifest-checker anchor until those companion reminders return; if a shared tests-root or scripts-root reminder still omits or overstates that directly readable anchor, fix the reminder one file at a time instead of reconstructing the missing companion packet
- `lib/cmdline.zig` plus `zigux/tests/phase7_cmdline_survey.zig` and `zigux/tests/phase7_cmdline_manifest.json` are directly readable again on current `master`, but the older helper-local slice, dedicated test, fixture, and shared build-or-validation companions are not directly readable in the same reread, so treat it as a helper-plus-survey-manifest foothold until those companion reminders return; if a shared reminder still frames the helper as missing or broader than the returned foothold, fix the reminder one file at a time instead of reconstructing the missing companion packet
- the surviving `rbtree` packet now directly reads back as `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`, `tools/lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, `zigux/tests/phase7_rbtree_survey.zig`, and `zigux/tests/phase7_rbtree_manifest.json`, while the dedicated slice note, roadmap-path port, fixture pair, parity checker, shared build file, and shared validator still do not directly materialize on current `master` during the same reread
...
- surviving `rbtree` direct-helper packet work: validate by rereading `Documentation/zigux/phase7-leaf-helper-lane-sequencing.md`, `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`, `tools/lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, `zigux/tests/phase7_rbtree_survey.zig`, and `zigux/tests/phase7_rbtree_manifest.json` together so the returned packet stays explicit without implying that the missing slice note, roadmap-path port, fixtures, parity checker, shared build file, or shared validator have returned
...
- `lib/argv_split.zig`: helper-local drift in empty-view reuse, copied-storage tokenization, or null-terminated argv export, or one shared reminder-surface truthfulness repair that explicitly names the returned helper-plus-survey-manifest-checker anchor in `lib/argv_split.zig`, `zigux/tests/phase7_argv_split_survey.zig`, `zigux/tests/phase7_argv_split_manifest.json`, and `scripts/zigux/check-phase7-argv-split-packet.py` plus its `zig test lib/argv_split.zig` replay without recreating the missing slice, dedicated test, fixture, or shared build-or-validation packet
- `lib/cmdline.zig`: helper-local drift in borrowed-slice parsing, `nextArg()` quoting, or `memparse()` ownership behavior, or one shared reminder-surface truthfulness repair that explicitly names the returned helper-plus-survey-manifest foothold in `lib/cmdline.zig`, `zigux/tests/phase7_cmdline_survey.zig`, and `zigux/tests/phase7_cmdline_manifest.json` plus its `zig test lib/cmdline.zig` replay without recreating the missing slice, dedicated test, fixture, or shared build-or-validation packet
- surviving `rbtree` direct-helper packet: keep same-lane follow-through inside `Documentation/zigux/phase7-rbtree-direct-anchor-note.md`, `tools/lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, `zigux/tests/phase7_rbtree_survey.zig`, or `zigux/tests/phase7_rbtree_manifest.json` until a fresh reread proves `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, the fixture pair, or `scripts/zigux/check-phase7-rbtree-parity.py` has returned
