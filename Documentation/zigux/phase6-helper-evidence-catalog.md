...
Fresh direct GitHub contents reads on current `master` still return missing for the remaining generator-side base64 direct C parity companions:

- `zigux/tests/fixtures/phase6_base64_c_parity_vectors.zig`
- `zigux/tests/phase6_base64_c_casegen.zig`

Authenticated current-master rereads now directly recover `zigux/tests/phase6_base64_c_parity.zig`, `zigux/tests/fixtures/phase6_base64_c_harness.c`, `scripts/zigux/check-phase6-base64-c-parity.py`, and `Documentation/zigux/phase6-perf-gate-survey.md`, so the only current direct-readback gaps left inside the broader base64 parity family are the two generator-side paths above.
...
The docs-root README now keeps a dedicated Phase 6 helper-evidence stanza aligned with surveyed head `current-master-readback-2026-05-24`, so keep `Documentation/zigux/README.md` inside the current direct-readback packet rather than treating it as a remaining shared-note follow-through gap.

A fresh authenticated current-master reread on 2026-05-24 reconfirmed the broader parity-side companion through `Documentation/zigux/phase6-helper-parity-catalog.md`, `zigux/tests/phase6_helper_parity_manifest.json`, `scripts/zigux/check-phase6-shared-surface.py`, and `scripts/zigux/validate-phase6.py`, while the generator-side base64 pair above still remained the only direct-readback gaps. Keep this helper-evidence note aligned with that narrower current parity packet instead of leaving it parked on the older 2026-05-22 survey head.
