...
Current public raw readback still helps recover `Documentation/zigux/phase6-perf-gate-survey.md`, but that broader perf note now lags the directly readable helper-local measurement packet: it still says `bsearch` has no dedicated wall-clock replay and it still pins older checksum slowdown counts. Keep that broader perf note as a stale public-tree-backed companion that needs a survey refresh before it is treated as current shared evidence again rather than as direct authenticated shared-packet proof in this runtime.
...
The broader shared perf survey is publicly readable again but still sits outside the direct authenticated shared packet in this runtime, and it is now stale on two concrete measurement points: the directly readable packet already includes `zigux/tests/phase6_bsearch_perf.zig` as a dedicated helper-local perf replay and `zigux/tests/phase6_checksum_perf.zig` plus `zigux/tests/fixtures/phase6_checksum_vectors.zig` now keep the larger payload slowdown counts (`64B` at `iterations = 200_000`, `1501B` at `iterations = 12_000`) together with the `checksum.ipFastCsum` IPv4 fast-path matrix. Treat the remaining shared perf-note risk as survey-truthfulness work rather than missing helper-local measurement coverage, and keep the directly readable helper-evidence packet above as the current source of truth until `Documentation/zigux/phase6-perf-gate-survey.md` is refreshed.
...
- `checksum` keeps a dedicated helper-vs-reference slowdown gate in `zigux/tests/phase6_checksum_perf.zig`, with the committed payload threshold matrix (`64B`, `1501B`) and the `checksum.ipFastCsum` IPv4 fast-path matrix (`IPV4_20B`, `IPV4_24B`, `IPV4_60B`) still owned by `zigux/tests/fixtures/phase6_checksum_vectors.zig`; the shared replay packet exposes that packet through `zig build phase6-checksum-perf-matrix-test --build-file zigux/tests/phase6_build.zig`, `make -C zigux phase6-checksum-perf-matrix-test`, `zig build phase6-checksum-perf --build-file zigux/tests/phase6_build.zig`, `make -C zigux phase6-checksum-perf`, and `make -C zigux phase6-perf`.
...
- the remaining roadmap-aligned measurement gap now sits in shared survey truthfulness rather than helper-local replay coverage: `Documentation/zigux/phase6-perf-gate-survey.md` is still publicly readable for fallback inspection, but it now understates the shipped measurement packet by claiming `bsearch` lacks a dedicated perf harness and by pinning older checksum slowdown counts instead of the current helper-local payload plus IPv4 fast-path matrices. Exact authenticated blob-pin refresh for that broader perf note is therefore still pending together with a note-local measurement-story repair.
