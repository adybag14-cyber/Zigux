...
- records one tiny PRP metadata summary with command-inline pointer count, PRP-list-covered pages, extra descriptor DMA footprint, and reset-time descriptor rebuild need without claiming live descriptor allocation or DMA mapping
...
The next honest bounded step inside the same Phase 12 lane is now one equally narrow recovery-facing helper or review note that keeps PRP metadata and reset-rebuild behavior explicit without widening into live DMA mapping, PCI queue bring-up, IRQ routing, or blk-mq flows before the roadmap-approved DMA-safe transport substrate exists.
