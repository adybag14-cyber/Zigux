...
- scope: roadmap-vs-repo sample delivery, approved payload, callback, and ownership-lifetime idiom guidance, plus the bounded formatting cue already shipped in the current non-runtime trace-events packet
- product boundary:
...
- the in-memory replay keeps the bounded array payload, selected string, formatted message, event-family counts, callback-registration balance, and post-exit rejection cues reviewable together in one small packet
...
Fresh current-`master` reread on 2026-05-12 compared the live Phase 5 trace-events note packet against the roadmap, the bootstrap ledger, the current sample-backed manifest, and the shared docs surfaces.

- connector-backed readback confirmed the current survey note had drifted beyond the manifest-backed contract, so this note was narrowed back to the exact checks still declared in `zigux/tests/phase5_trace_events_sample_manifest.json`
...
- no local `zig` replay was run in this lane because this scheduled environment did not provide a normal writable Zigux checkout or direct raw-file fetch path for truthful end-to-end reruns; validation for this note update stayed on connector-backed repo inspection and roadmap-to-manifest alignment
...
- do `zigux/tests/phase5_trace_events_sample_manifest.json` and `zigux/tests/phase5_trace_events_sample_survey.zig` still describe the same vararg-payload, message, relative-location, callback-path, and teardown contract run through `zigux/tests/phase5_build.zig`?
...
- does function-callback replay still stay a balanced register-then-unregister idiom rather than implying `kthread_run()`, scheduling, or tracepoint enablement parity?
...
Keep this trace-events note packet parked unless fresh repo inspection shows the note, manifest, survey gate, or shared sample guidance drifting apart again on current `master`. If that happens, reopen only for the next smallest same-family note or survey sync and keep the formatting boundary tied to the landed selected-string plus `iter=%d` replay rather than widening into a standalone formatting sample claim.
