...
- `PHASE5_SURVEYED_COMMIT=076ff0673ea07d423d3bf195a002339426f90f18`
...
It is to keep the current sample-backed trace-events packet honest on current `master`: keep the bounded formatting companion, the bounded callback-focus companion, and the bounded payload-preview companion explicit where direct reread still proves them, keep the broader non-runtime sample-local companions visible as public-tree-backed companion evidence while the contents route still misses them, keep the shared `zigux/tests/phase5_build.zig` route explicit as a returned shared rerun handle now that direct reread serves it again, and avoid borrowing exact replay wording from sample-local files that this run could not re-read directly.

## Current repo reality on `master`
Fresh mixed reread on 2026-05-27 still directly reconfirmed the roadmap anchor plus the bounded formatting companion at `samples/zigux/trace_events_string_formatting_sample.zig`, the bounded callback-focus companion at `samples/zigux/trace_events_callback_focus_contract.zig`, and the bounded payload-preview companion at `samples/zigux/trace_events_payload_preview_contract.zig` through authenticated sample-root readback.

A fresh 2026-05-27 authenticated reread in this lane also returned this survey note directly again, so it no longer belongs in the still-missing authenticated sample-local companion set.

That same 2026-05-27 reread still kept the broader non-runtime trace-events sample-local companions out of the directly readable packet in this runtime:
- `samples/zigux/trace_events_sample.zig`
- `zigux/tests/phase5_trace_events_sample.zig`
- `zigux/tests/phase5_trace_events_sample_manifest.json`
- `zigux/tests/phase5_trace_events_sample_survey.zig`

Authenticated contents readback still returned `404` for the four sample-local companion paths above on 2026-05-27.
That same 2026-05-27 authenticated reread also returned `zigux/tests/phase5_build.zig` directly again, including the focused `phase5-trace-events-sample-tests` and `phase5-trace-events-sample-survey-tests` routes beside the broader Phase 5 sample bundle.
...
The bounded formatting companion, the bounded callback-focus companion, and the bounded payload-preview companion are therefore the strongest direct sample-root trace-events evidence this run could reconfirm.
...
The directly coupled trace-events packet is currently strongest in the bounded formatting companion, the bounded callback-focus companion, the bounded payload-preview companion, and the shared reminder surfaces that keep their limits explicit.

Fully aligned reminder surfaces for the direct formatting, callback-focus, and payload-preview packet in this run:
...
A fresh 2026-05-27 sample-root reread in this run confirms the shared sample-root reminder is still aligned on the narrow direct packet:
- `samples/zigux/README.md` keeps the bounded formatting companion and bounded callback-focus companion as the direct authenticated proof, and the same current packet now also directly returns `samples/zigux/trace_events_payload_preview_contract.zig` as the bounded payload-shape companion while the broader non-runtime trace-events companions stay framed as shared-reminder, historical-support, or current public-tree-backed companion surfaces until a fresh reread proves they returned directly on current `master`

The scripts-root reminder is aligned in this run too:
- `scripts/zigux/README.md` keeps the approved-idiom, guide-surface, bytestream, kretprobe, kobject-split, and no-extra-sample boundaries explicit, and its Phase 5 companion roster keeps the same narrow trace-events posture grounded in the shared reminder packet rather than in a broader trace-events replay claim
...
- it does not restate helper-level trace-events cues such as `runPayloadBoundaryReplay()`, `runCallbackBoundaryRecoveryReplay()`, `runStringFormattingCycleReplay()`, `runLifecycleBoundaryReplay()`, `lifecycleSummary()`, the selected-string plus `iter=%d` formatting cue, or `OutstandingRegistration`; those cues remain explicit in the bounded formatting companion, the bounded callback-focus companion, the bounded payload-preview companion, and the shared reminder packet, not in any broader sample-local file that this run could directly re-read
...
## Landed cue posture
Current direct evidence in this run is limited to the bounded formatting companion, the bounded callback-focus companion, the bounded payload-preview companion, and the shared reminder packet that names them.
...
- the bounded callback-focus companion remains `samples/zigux/trace_events_callback_focus_contract.zig`
- the bounded payload-preview companion remains `samples/zigux/trace_events_payload_preview_contract.zig`
...
- callback-side reviewability remains part of the direct packet too: `anchorFocusOrder()` and `callbackBoundaryContract()` keep `payload_shape`, `string_selection`, `formatted_message`, `conditional_event_families`, `function_callback_registration`, and `ownership_and_lifetime` explicit together with callback-registration recovery cues without turning that companion into a fifth sample
- payload-side reviewability remains part of the direct packet too: `referencePattern()` keeps `payload_shape`, `string_selection`, `formatted_message`, `conditional_event_families`, `function_callback_registration`, and `ownership_and_lifetime` tied to the five-case preview ladder, the bounded `payload_preview` growth, the preserved initialized-stage posture, and the largest direct preview case `"One ring to rule them all"` plus `"iter=4"` without turning that companion into a fifth sample
...
- selected-string exact-fit behavior also remains part of the approved idiom reminder: `formatSelectedIterationMessageInto(3, [11]u8)` stays a no-space boundary, while `formatSelectedIterationMessageInto(3, [12]u8)` stays the success-sized `Frodo iter=3` case without turning this packet into a standalone string-helper sample
...
The more accurate same-lane state on 2026-05-27 is:
- the roadmap-backed trace-events anchor still has directly readable bounded formatting, callback-focus, and payload-preview companions plus aligned shared reminder surfaces
...
- the approved-idiom owner note must keep the direct payload-preview companion explicit alongside the formatting and callback-focus companions so the current direct packet does not get undercounted in shared reminder work

So the honest follow-through is to keep this survey note anchored to the narrow direct packet that current reread actually proved, while leaving the broader sample-local replay family parked in current public-tree-backed companion status until a future reread returns those files directly again, while keeping the approved-idiom-owner wording aligned with the shipped survey gate, and while naming the remaining shared-reminder posture precisely instead of inventing a broader trace-events gap that current `master` already narrowed.
