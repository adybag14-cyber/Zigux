...
- the replay records the `0xdeadbeef` bitmask word and marks the relative-location payload path as checked
...
- the callback replay requires registration first and restores the registration balance to zero before the sample completes
...
- do `zigux/tests/phase5_trace_events_sample_manifest.json` and `zigux/tests/phase5_trace_events_sample_survey.zig` still describe the exact payload, message, callback-registration, and teardown contract run through `zigux/tests/phase5_build.zig`?
...
- the repo now has a reviewable Phase 5 `trace_events_sample` reference sample plus manifest-backed checks for payload shape, string selection, formatted messages, bounded family counts, callback-registration balance, and teardown
