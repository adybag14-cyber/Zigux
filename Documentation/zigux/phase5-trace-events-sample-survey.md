...
- do the sample-backed survey note and the paired sample-root guidance still say plainly that current `master` ships no standalone Phase 5 formatting-helper sample and that the selected-string plus `iter=%d` replay is the approved formatting cue while standalone formatting evidence stays under the Phase 1 `vsprintf` packet and the bounded Phase 7 `string_get_size()` helper packet?
