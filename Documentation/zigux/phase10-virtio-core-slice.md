...
- scope: status sequencing, bounded feature negotiation, queue callback bookkeeping, registration identity bookkeeping, config-change, driver-binding, remove-side bookkeeping, and config-generation bookkeeping, dedicated Phase 10 test wiring, and a lab-only review note only
- product boundary:
...
- covered now:
  ...
  - core-side status sequencing, feature negotiation, and one bounded remove-side handoff
...
This keeps the Phase 10 core lane honest: the live Zigux slice now describes one queue's shape, one bounded registration identity path, one bounded config-change path, one bounded config-generation observation path, one bounded remove-side handoff, and the last config-change delivery disposition in memory, which narrows the gap to future virtqueue work without pretending any transport or ring code has already landed.
...
Leave the Phase 10 virtio-core helper parked unless fresh review drift appears inside the landed config-generation, config-change disposition, or remove-side bookkeeping packet. Any future reopen in this lane should stay explicit about the remaining blocked probe, full remove, and transport-backed reset lifecycle gap instead of widening into `virtio_mmio` or `virtio_ring` work indirectly.
