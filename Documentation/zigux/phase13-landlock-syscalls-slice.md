...
- adds one tiny in-memory ruleset-FD installation planner that keeps `anon_inode_getfd("[landlock-ruleset]", &ruleset_fops, ruleset, O_RDWR | O_CLOEXEC)` and the failure-side `landlock_put_ruleset()` release explicit without touching anonymous inode creation or the live FD table
