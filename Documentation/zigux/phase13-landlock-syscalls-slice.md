...
- adds one in-memory `get_ruleset_from_fd()` planner for ruleset-FD presence, ruleset-file type, required read or write mode, and the existing single-layer guard without touching the live FD table

This slice does not claim anonymous-fd creation, owned FD lookup, path imports, credential preparation, thread synchronization, domain merges, or live syscall enforcement.

The next honest bounded step in this same lane is to add one small in-memory planner around `get_path_from_fd()` parent-FD rejection checks for ruleset files, `MNT_INTERNAL`, and `SB_NOUSER` before widening into path-backed rule import, credential updates, or live domain state.
