...
The current helper stays intentionally narrow:
  ...
  * keeps the helper descriptor explicit about what is in scope today: create-ruleset planning, ABI-version query planning, ruleset-fd install planning, handled-access validation, attr-size validation, and flag validation
  * keeps delegated ruleset creation planning explicit through `security/landlock/ruleset.zig` and keeps one ruleset-fd install planner explicit about the fixed `anon_inode_getfd()` label, `O_RDWR | O_CLOEXEC` flags, and release-on-fd-failure discipline without presenting live anonymous-fd installation or returned-file-descriptor ownership as shipped Zigux behavior
