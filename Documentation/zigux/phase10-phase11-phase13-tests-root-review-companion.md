...
- shared reminder surfaces: `Documentation/zigux/phase10-closure-evidence.md`, `Documentation/zigux/phase10-virtio-driver-lane-sequencing.md`, `scripts/zigux/check-phase10-core-packet.py`, `scripts/zigux/check-phase10-ring-packet.py`, `scripts/zigux/check-phase10-input-packet.py`, `scripts/zigux/check-phase10-mmio-packet.py`, `scripts/zigux/check-phase10-mmio-freeze-boundary.py`, `scripts/zigux/check-phase10-harness-coverage.py`, `scripts/zigux/check-phase10-tests-readme-core-surfaces.py`, `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, and `.github/workflows/zigux-bootstrap.yml`
...
Keep the current-head simple-driver truthfulness packet explicit through these currently materialized shared reminder surfaces, the narrower DesignWare continuity pair, the current-head HVC companion packet, the shared build-inventory anchor, and the surviving HVC proof shards:
...
Treat those shared reminder notes, the directly readable build-inventory anchor, and the HVC proof packet above as the current Phase 11 evidence on `master`. Current direct contents reads in this run did not rematerialize `Documentation/zigux/phase11-shared-replay-contract.md`, `Documentation/zigux/phase11-closure-note.md`, `scripts/zigux/check-phase11-shared-replay-contract.py`, `scripts/zigux/check-phase11-shared-summary-surfaces.py`, `zigux/tests/phase11_build.zig`, or `zigux/Makefile`, and the shared `make -C zigux phase11-contract` route plus the exact shared `zig build test --build-file zigux/tests/phase11_build.zig --summary all` replay therefore also remain non-current-head route vocabulary here, so broader tests-root wording should not present that older shared contract-and-route packet as live current-head evidence until a future reread proves those surfaces returned. Current direct contents reads in this run also did not rematerialize `zigux/tests/phase11_gpio_wdt.zig`, `zigux/tests/phase11_bcm2835_wdt.zig`, `drivers/watchdog/dw_wdt.zig`, or `drivers/tty/hvc/hvc_console_verify.zig`, so keep those archived direct watchdog and HVC replay paths framed as inventory-backed vocabulary rather than current-head evidence until a future reread proves they returned.

Keep the validation-matrix gap survey authoritative for bcm2835, gpio, DesignWare, and HVC matrix drift until fresh direct rereads recover the missing driver-local validation matrices. Keep the current DesignWare follow-through framed through `Documentation/zigux/phase11-dw-wdt-clock-acquisition-plan.md` and `Documentation/zigux/phase11-dw-wdt-verify-alignment-gap.md` rather than restating the older direct `dw_wdt` replay packet as current-head evidence without a fresh reread.

Keep the HVC packet framed as inventory-backed archival continuity through `Documentation/zigux/phase11-hvc-console-survey.md`, `Documentation/zigux/phase11-hvc-cleanup-alignment-current-head-companion.md`, `Documentation/zigux/phase11-hvc-verify-helper-boundary.md`, `scripts/zigux/check-phase11-hvc-cleanup-current-head.py`, `zigux/tests/fixtures/phase11_build_inventory.json`, `zigux/tests/phase11_hvc_export_surface_layout_proof.zig`, `zigux/tests/phase11_hvc_cleanup_packet_proof.zig`, and `zigux/tests/phase11_hvc_cleanup_packet_build.zig` instead of calling the archived direct driver, teardown, split-replay, sysrq, shared-build, or broader Makefile-route surfaces current-head readback evidence.

Tests-root reviewer prompt:
- Do the docs-root notes, the tests-root packet, `Documentation/zigux/phase11-driver-lane-sequencing.md`, `Documentation/zigux/phase11-validation-matrix-gap-survey.md`, `Documentation/zigux/phase11-hvc-console-survey.md`, `Documentation/zigux/phase11-hvc-cleanup-alignment-current-head-companion.md`, `Documentation/zigux/phase11-hvc-verify-helper-boundary.md`, `Documentation/zigux/phase11-uapi-header-parity-validation-matrix.md`, the DesignWare continuity pair `Documentation/zigux/phase11-dw-wdt-clock-acquisition-plan.md` and `Documentation/zigux/phase11-dw-wdt-verify-alignment-gap.md`, `scripts/zigux/check-phase11-build-inventory.py`, `scripts/zigux/check-phase11-matrix-gap-survey.py`, `scripts/zigux/check-phase11-hvc-cleanup-current-head.py`, the shared `zigux/tests/fixtures/phase11_build_inventory.json` anchor, and the HVC proof packet `zigux/tests/phase11_hvc_export_surface_layout_proof.zig`, `zigux/tests/phase11_hvc_cleanup_packet_proof.zig`, and `zigux/tests/phase11_hvc_cleanup_packet_build.zig` still describe the same smaller current-head shared-versus-dedicated simple-driver packet while keeping bcm2835, gpio, DesignWare matrix drift, and the HVC archival packet honest, without implying that the missing shared replay-contract note, closure note, checker pair, shared build file, `phase11-contract` route, exact shared `zig build test --build-file zigux/tests/phase11_build.zig --summary all` replay, `zigux/tests/phase11_gpio_wdt.zig`, `zigux/tests/phase11_bcm2835_wdt.zig`, `drivers/watchdog/dw_wdt.zig`, or `drivers/tty/hvc/hvc_console_verify.zig` are back on current `master` before a fresh reread rematerializes them?
