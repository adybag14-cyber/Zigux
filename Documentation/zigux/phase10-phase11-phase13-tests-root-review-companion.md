...
Keep the current bounded simple-driver packet explicit through the shared reminder surfaces, the shared validation-matrix packet, the returned DesignWare owner packet, the narrower HVC current-head continuity packet, and the adjacent header-boundary proof shard.
...
Tests-root reviewer prompt:
- Does the shared Phase 11 reminder keep the shared owner-map note, the matrix-gap survey, the contributor-sync companion, the returned bcm2835, gpio, HVC, DesignWare, and header-boundary validation packet, the DesignWare owner packet, the narrower HVC current-head continuity packet, the returned HVC proof-backed adjunct inventory, and the still-missing shared Phase 11 build, replay-contract, deeper HVC replay surfaces, plus the still-missing `make -C zigux phase11` and `make -C zigux phase11-contract` routes aligned without promoting platform-backed execution or a broader replay roster into current tests-root proof?
