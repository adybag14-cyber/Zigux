...
Keep the validation-matrix gap survey authoritative for the shared matrix packet and keep the four directly readable driver-local validation matrices explicit for bcm2835, gpio, HVC, and DesignWare without using that restored matrix packet to overclaim broader shared-route or execution parity. Keep the current DesignWare follow-through framed through `Documentation/zigux/phase11-dw-wdt-clock-acquisition-plan.md` and `Documentation/zigux/phase11-dw-wdt-verify-alignment-gap.md` rather than restating the older direct `dw_wdt` replay packet as current-head evidence without a fresh packet-local reread.
