...
Keep the current contributor-facing Phase 13 packet explicit through these shared surfaces:
...
Treat the Linux-style make routes above as the stable contributor-facing replay handles. Keep the shared validator-first eight-test release packet anchored to current repo reality instead of assuming every direct Phase 13 companion is still materialized on current `master`. If direct companions such as `zigux/tests/phase13_build.zig`, `zigux/tests/phase13_libfs*.zig`, `zigux/tests/phase13_devres*.zig`, `zigux/tests/phase13_landlock_*.zig`, the direct Phase 13 manifest files under `zigux/tests/`, `scripts/zigux/check-phase13-devres-packet.py`, or `scripts/zigux/check-phase13-notifier-packet.py` cannot be materialized on current `master`, record them as repo-reality gaps instead of presenting them here as independently shipped review evidence.

Tests-root reviewer prompt:
- Do the contributor workflow guide, shared-helper sequencing note, release-notes and roadmap-traceability notes, helper-family slice and survey notes, the helper-owned Landlock ownership and syscall-governance notes, the landlock ruleset packet checker, the adjacent notifier survey and priority-signal guard, the shared validator-first eight-test release packet, the shared contributor-surface sync note, the shared review checklist, the broader tests-root guide, helper footholds, and Linux-style make routes still describe the same active Phase 13 contributor packet while keeping missing direct companions such as `zigux/tests/phase13_build.zig`, `zigux/tests/phase13_libfs*.zig`, `zigux/tests/phase13_devres*.zig`, `zigux/tests/phase13_landlock_*.zig`, the direct Phase 13 manifest files under `zigux/tests/`, `scripts/zigux/check-phase13-devres-packet.py`, and `scripts/zigux/check-phase13-notifier-packet.py` framed as repo-reality gaps rather than shipped current-master evidence, and does the sequencing note keep `libfs`, `devres` helper parity, `devres` packet truthfulness, `landlock`, and adjacent notifier evidence from being treated as one shared owner while keeping the landed nonincreasing-priority signal explicit?
...
- Phase 13: do the contributor workflow guide, shared-helper sequencing note, release-notes and roadmap-traceability notes, the helper-owned Landlock ownership and syscall-governance notes, the landlock ruleset packet checker, the adjacent notifier survey and priority-signal guard, the shared validator-first eight-test release packet, the shared contributor-surface sync note, the shared review checklist, the broader tests-root guide, helper footholds, and Linux-style make routes still keep the active contributor packet aligned while treating missing direct companions such as `zigux/tests/phase13_build.zig`, `zigux/tests/phase13_libfs*.zig`, `zigux/tests/phase13_devres*.zig`, `zigux/tests/phase13_landlock_*.zig`, the direct Phase 13 manifest files under `zigux/tests/`, `scripts/zigux/check-phase13-devres-packet.py`, and `scripts/zigux/check-phase13-notifier-packet.py` as repo-reality gaps rather than shipped evidence?
