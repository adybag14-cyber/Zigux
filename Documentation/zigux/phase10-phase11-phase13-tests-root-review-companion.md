...
Keep the current shared-versus-dedicated simple-driver packet explicit through these shared surfaces and the bounded packet-local proof surfaces that still shape review on current `master`:
...
Treat the shared reminder notes, shipped checkers, build-backed replay routes, the DesignWare plan-note-plus-checker pair together with `Documentation/zigux/phase11-dw-wdt-validation-matrix.md`, `Documentation/zigux/phase11-dw-wdt-survey.md`, `Documentation/zigux/phase11-dw-wdt-teardown-note.md`, `zigux/tests/phase11_dw_wdt.zig`, `zigux/tests/phase11_dw_wdt_manifest.json`, `zigux/tests/phase11_dw_wdt_registration_scaffold.zig`, `zigux/tests/phase11_dw_wdt_survey.zig`, and `drivers/watchdog/dw_wdt_verify.zig`, plus the dedicated HVC archival packet through `Documentation/zigux/phase11-hvc-console-teardown-note.md`, `Documentation/zigux/phase11-hvc-console-validation-matrix.md`, `zigux/tests/phase11_hvc_console.zig`, `zigux/tests/phase11_hvc_cleanup.zig`, `drivers/tty/hvc/hvc_console_verify.zig`, `zigux/tests/phase11_hvc_console_manifest.json`, `zigux/tests/phase11_hvc_console_survey.zig`, `zigux/tests/phase11_hvc_console_modem_control_split.zig`, `zigux/tests/phase11_hvc_console_poll_retry_split.zig`, `drivers/tty/hvc/hvc_console_sysrq.zig`, and `make -C zigux phase11-hvc-survey` as the current directly re-readable Phase 11 evidence on `master`.

Tests-root reviewer prompt:
- Do the docs-root notes, checker split, shared summary-surface checker, tests-root packet, and Linux-style make routes still describe the same shared-versus-dedicated simple-driver packet, the parked shared closure checkpoint, the parked driver-lane owner map, the dedicated bcm2835 archival packet, the landed DesignWare validation, teardown, verify, survey, and platform-registration scaffold packet, the dedicated archival `hvc_console` teardown note, the validation matrix, direct `zigux/tests/phase11_hvc_console.zig` plus `zigux/tests/phase11_hvc_cleanup.zig` replays, the direct `drivers/tty/hvc/hvc_console_verify.zig` replay boundary, the manifest-backed survey gate, the dedicated `scripts/zigux/check-phase11-hvc-survey-packet.py` plus `make -C zigux phase11-hvc-survey` checker-backed replay route, the modem-control split, poll-retry split, and sysrq-helper boundary as shipped current-`master` evidence, and without implying a removed `validate-phase11.py`, a missing build-inventory fixture, or a broader checker stack than current `master` ships?
...
- Phase 11: do the docs-root notes, checker split, shared summary-surface checker, tests-root packet, and Linux-style make routes still keep the shared-versus-dedicated simple-driver packet, the dedicated bcm2835 archival packet, the landed DesignWare validation, teardown, verify, survey, and platform-registration scaffold packet, the dedicated archival `hvc_console` teardown note, the validation matrix, direct `zigux/tests/phase11_hvc_console.zig` plus `zigux/tests/phase11_hvc_cleanup.zig` replays, the direct `drivers/tty/hvc/hvc_console_verify.zig` replay boundary, the manifest-backed survey gate, the dedicated `scripts/zigux/check-phase11-hvc-survey-packet.py` plus `make -C zigux phase11-hvc-survey` checker-backed replay route, the modem-control split, poll-retry split, and sysrq-helper boundary aligned as shipped current-`master` evidence, and without implying removed validators or missing inventory fixtures?
