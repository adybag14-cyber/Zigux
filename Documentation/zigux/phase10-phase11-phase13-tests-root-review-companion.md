...
- current direct lane readback still does not materialize `Documentation/zigux/phase10-virtio-core-slice.md`, `zigux/tests/phase10_virtio_core_reset_queue.zig`, `zigux/tests/phase10_virtio_core_interrupt_compound_ack.zig`, `drivers/virtio/virtio_driver_id.zig`, `drivers/virtio/virtio_verify.zig`, `zigux/tests/phase10_virtio_core_manifest.json`, `zigux/tests/phase10_virtio_core_survey.zig`, and `zigux/tests/phase10_virtio_ring.zig` through the direct readback available in this lane, but public current-`master` readback now rematerializes `Documentation/zigux/phase10-virtio-core-slice.md`, `drivers/virtio/virtio_verify.zig`, `zigux/tests/phase10_virtio_core_manifest.json`, `zigux/tests/phase10_virtio_core_survey.zig`, `zigux/tests/phase10_virtio_core_reset_queue.zig`, `zigux/tests/phase10_virtio_core_interrupt_compound_ack.zig`, and `zigux/tests/phase10_virtio_ring.zig`; keep those public-read companions explicit beside the returned bounded core replay `zigux/tests/phase10_virtio_core.zig`, while only `drivers/virtio/virtio_driver_id.zig` and `zigux/tests/phase10_virtio_driver_id.zig` stay framed as the remaining repo-reality gaps in this lane.
...
Tests-root reviewer prompt:
- Does the shared Phase 11 reminder keep the shared owner-map note, the matrix-gap survey, the contributor-sync companion, the returned bcm2835, gpio, HVC, DesignWare, and header-boundary validation packet, the DesignWare owner packet, the narrower HVC current-head continuity packet, the returned HVC proof-backed adjunct inventory, the returned shared validator `scripts/zigux/validate-phase11.py`, the returned `zigux/Makefile` surface plus `make -C zigux phase11-validate` as the shared validation-and-build gate, and the still-missing shared Phase 11 build, replay-contract, deeper HVC replay surfaces, plus the still-missing `make -C zigux phase11` and `make -C zigux phase11-contract` routes aligned without promoting platform-backed execution or a broader replay roster into current tests-root proof?
