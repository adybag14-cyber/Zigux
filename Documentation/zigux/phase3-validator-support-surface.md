...
`scripts/zigux/README.md` now has a separate packet-local export/UAPI reminder drift: its Phase 3 scripts-root summary still lists `scripts/zigux/validate-phase3-export-uapi-survey.py`, `Documentation/zigux/phase3-export-uapi-boundary-survey.md`, `zigux/tests/phase3_export_uapi_layout.zig`, `zigux/tests/phase3_export_uapi_layout_build.zig`, and `zigux/kernel/export_shim.zig` as current ABI-boundary companions even though this validator-support note still treats that export-boundary packet as absent on current `master`.

`Documentation/zigux/phase3-shared-reminder-gap.md` remains the direct-readback tracker for the docs-root, tests-root, and review-checklist three-slice cleanup. Keep the scripts-root follow-up narrower: bring `scripts/zigux/README.md` back into alignment with the same repo-reality posture before reopening broader shared reminder work, so the repo stops presenting missing export/UAPI routes as shipped Phase 3 evidence.

Keep the remaining follow-up focused on those reminder surfaces so they stop parking `include/zigux/abi.h` and `zigux/bindings/abi.zig` as broader missing routes, stop presenting the absent export/UAPI packet as live scripts-root evidence, and instead record any residual reminder drift directly until additional current-tree-backed validator or export-boundary proof lands.

## Scope

This note is limited to the current validator-support posture for Phase 3. It keeps the directly readable starter packet, the machine-readable manifests, the direct compile replay, and the two focused helper slices explicit, marks representative broader validator and export-boundary routes as current gaps, and records the remaining shared-reminder plus scripts-root export/UAPI follow-up without claiming a wider shipped Phase 3 packet.