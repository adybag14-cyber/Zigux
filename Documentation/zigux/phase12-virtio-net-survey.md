...
If live master still matches this note on the next same-lane pass, keep the false slice-note path closed first: `Documentation/zigux/README.md` and `zigux/tests/README.md` already name `zigux/tests/phase12_virtio_net_syntax_lab.zig` beside `zigux/tests/phase12_virtio_net.zig` and `zigux/tests/phase12_virtio_net_survey.zig`, but the narrower tests-root metadata packet still undercounts the manifest-backed surface because the Phase 12 key-entrypoint list omits `zigux/tests/phase12_virtio_net_manifest.json`. Treat that as the next same-family metadata repair before widening into helper or runtime-data-path discussion.
