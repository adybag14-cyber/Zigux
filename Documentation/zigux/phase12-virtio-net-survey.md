...
- current `master` now carries `zigux/tests/phase12_build.zig`, and that shared build route now carries the direct `virtio_net` syntax-lab smoke shard plus the dedicated `virtio_net_transmit_recycle` replay alongside the shipped `virtio_scsi` packet
- `zigux/Makefile` still carries `phase12-smoke`, `phase12-test`, and `phase12`, and those shared routes now pick up the bounded `virtio_net` syntax-lab smoke shard plus the direct transmit-recycle replay through the shared Phase 12 build route

Those checks mean the current lane has moved forward from a control-queue payload-shaping packet into a bounded transmit-recycle follow-up, but it is still intentionally below any live runtime or DMA-backed data-path claim.
...
1. keep the current starter focused on probe fallback, queue-topology summary, packet-buffer choice, refill-order reviewability, control-queue recovery sequencing, control-queue payload shaping, queue-reset recovery planning, and transmit-recycle disposition reviewability instead of widening into live DMA or lifecycle code
2. revisit the direct test, transmit-recycle replay, syntax lab, manifest, survey gate, or this note only if another exact reviewability refresh becomes necessary inside the same packet
...
Until then, treat the current starter as a real but deliberately small Phase 12 transmit-recycle follow-up step, not as a live runtime proof.