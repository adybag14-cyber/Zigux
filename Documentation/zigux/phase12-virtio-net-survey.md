...
If live master still matches this note on the next same-lane pass, the next honest review-surface follow-through is smaller than a new driver-local slice: `Documentation/zigux/README.md` still undercounts the shipped direct smoke shard because it names `zigux/tests/phase12_virtio_net.zig` and `zigux/tests/phase12_virtio_net_survey.zig` but not `zigux/tests/phase12_virtio_net_syntax_lab.zig`, so that docs-root sync should land before anyone reopens the false slice-note path.
