...
This document records the bounded Phase 12 survey lane around `drivers/net/virtio_net.c`, the landed Zigux probe starter tied to it, and the already-landed queue-recovery, `hdr_len`, and header-scatter follow-ups that keep this lane reviewable without widening into runtime data-path work.

## Status

- `PHASE12_STATUS=active`
- `PHASE12_SLICE=virtio-net-survey`
- scope: survey manifest, dedicated survey gate, shared Phase 12 build wiring, the landed `drivers/net/virtio_net.zig` probe snapshot plus `hdr_len` and header-scatter helpers, and a lane note that compares the live repo state against the roadmap for the broader driver
- product boundary:
...
The highest-value honest step in this lane is therefore a very small probe snapshot helper plus matching `hdr_len` and header-scatter summaries with bounded build wiring and risk notes, not a premature runtime data-path or net-device scaffold.
...
- this checkpoint has now been re-verified against surveyed `master` snapshot `6afa667980bb289b2d811660aa92ea8106fbc043`, with the same probe snapshot, queue-recovery, `hdr_len`, and header-scatter helpers still defining the live bounded footing.
...
- the lane now also lands one tiny header-scatter follow-up: the probe snapshot records whether `virtnet_probe()` can split the header into a separate scatter-gather segment through `VIRTIO_F_ANY_LAYOUT` or `VIRTIO_F_VERSION_1`, and the new summary mirrors whether `needed_headroom` should stay at zero or track the negotiated header length without claiming any live DMA-backed receive path.
...
- the landed `phase12-virtio-net-header-scatter-followup`
- the still-blocked `phase12-virtio-net-runtime-data-path`

This keeps the lane explicit without overstating progress: Zigux now has a reviewable Phase 12 probe snapshot starter plus the bounded queue-recovery summary, header-shape, and header-scatter follow-ups, but it still does not claim DMA-backed queue setup, NAPI, control-virtqueue commands, or a usable net-driver lifecycle.
...
Keep this lane on another probe-only capability handoff, most likely a tiny receive-buffer constraint summary, until the roadmap-approved DMA and queueing substrate exists for a truthful follow-up beyond the current probe snapshot, queue-recovery summary, `hdr_len`, and header-scatter helpers.