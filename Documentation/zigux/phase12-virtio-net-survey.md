...
If live master still matches this note on the next same-lane pass, do not steer future work back toward the older shared tests-root release-readiness reminder. `zigux/tests/README.md` already keeps `Documentation/zigux/phase12-release-readiness-survey.md` explicit beside the shared smoke-first packet, the PMO closure companion, the compact release-coordination matrix, the two anti-overlap companions, the shared fallback-overview note, the freeze-boundary reminder, and the shipped smoke-first plus shared-build replay order.

After that shared reminder closure, the next honest same-family move still stays on the blocked runtime-data-path boundary: any future follow-up still needs roadmap-approved DMA-safe abstractions, queueing correctness substrate work, and throughput plus recovery parity evidence before Zigux can claim live page-pool DMA, refill loops, XDP execution, NAPI, control-virtqueue commands, or net-device lifecycle behavior.
