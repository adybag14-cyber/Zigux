...
- this note still pins the last packet-local verification head `8b69e4dfd04553afeb08c0ecbf3060f800e7ecd1`, with the same probe snapshot, queue-recovery, queue-resume, `hdr_len`, receive-path, and mergeable-refill helpers still defining the published bounded footing.
...
- the lane now also lands one bounded mergeable-refill follow-up: once a probe snapshot exists, the lab can turn queue-entry count plus negotiated `hdr_len`, MTU, and headroom state into packet budget bytes, a minimum buffer length summary, and a recycled-room-versus-fresh-room handoff without claiming live page-pool, DMA refill, or NAPI execution.
