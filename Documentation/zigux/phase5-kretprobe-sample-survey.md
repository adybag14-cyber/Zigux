...
- the earlier full-packet replay captured on 2026-05-06 still stands as the last complete shared-packet readback: `zig fmt --check` covered `samples/zigux/kretprobe_example.zig`, `zigux/tests/phase5_kretprobe_example.zig`, `zigux/tests/phase5_kretprobe_example_survey.zig`, and the focused scratch `zigux/tests/phase5_kretprobe_example_build.zig`, while `zig test samples/zigux/kretprobe_example.zig` and the full `zigux/tests/phase5_build.zig` packet both passed the then-current sample and shared replay sets via `zig build test --build-file zigux/tests/phase5_build.zig --summary all`
