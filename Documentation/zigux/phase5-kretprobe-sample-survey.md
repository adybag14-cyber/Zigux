...
When a contributor updates `samples/zigux/kretprobe_example.zig` or one of its directly coupled reminder surfaces, keep these packet-local prompts explicit here instead of relying on the broader shared guides alone:
  ...
  * does `samples/zigux/kretprobe_example_instance_budget_contract.zig` still keep the Linux `func` parameter name, shared `0o644` mode, default `kernel_clone`, one-word private-data shape, and the `nmissed` / `maxactive` cue explicit, and do `zig test samples/zigux/kretprobe_example_instance_budget_contract.zig` plus `zig test --dep kretprobe_example_instance_budget_contract -Mroot=zigux/tests/phase5_kretprobe_example_instance_budget_contract.zig -Mkretprobe_example_instance_budget_contract=samples/zigux/kretprobe_example_instance_budget_contract.zig` stay visible as the companion-only self-check and focused replay routes?
  * does `samples/zigux/kretprobe_example_probe_spec.zig` still keep the anchor path, default `kernel_clone`, `private_data_size_bytes = 8`, default `maxactive = 20`, replay `return_value = 42`, replay `duration_ns = 75`, replay `nmissed = 1`, and the pre-init-only symbol and maxactive tuning cues explicit, and does `zig test samples/zigux/kretprobe_example_probe_spec.zig` stay visible as the companion-only self-check route for that bounded probe-spec packet?
