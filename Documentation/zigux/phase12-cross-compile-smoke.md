...
- shared PMO reading: the active-not-closed posture, the approved `x86_64-linux-musl`, `aarch64-linux-musl`, and `riscv64-linux-musl` smoke set, the focused libbpf-only replay shard, and the mixed two commit-pinned versus two shared-tree-only raw fallback split should stay aligned across that release packet, the shared replay-contract note, and the adjacent checklist and tests-root surfaces instead of being inferred from this narrower smoke note alone
