...
- rollback drill: run `make -C zigux phase12-validate` after `python3 scripts/zigux/check-phase12-build-inventory.py --self-test`, `python3 scripts/zigux/check-phase12-build-inventory.py`, `python3 scripts/zigux/check-phase12-libbpf-snapshot.py --self-test`, and `python3 scripts/zigux/check-phase12-libbpf-snapshot.py`; if the libbpf survey packet is the only failing slice, repair `scripts/zigux/check-phase12-libbpf-snapshot.py` plus `zigux/tests/fixtures/phase12_libbpf_snapshot.json` first if the reproducibility packet drifted, otherwise remove the `phase12-libbpf-segment-survey-tests` and `phase12-libbpf-reviewability-tests` entries from `zigux/tests/phase12_build.zig`, keep the C anchor and landed helper files unchanged, then rerun `make -C zigux phase12-validate` followed by `zig build test --build-file zigux/tests/phase12_build.zig --summary all` so the shared Phase 12 packet stays truthful while the libbpf survey note or gate is repaired.
