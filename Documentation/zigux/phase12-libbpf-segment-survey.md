...
- the older segment catalog still leaves two bounded shared-bridge helpers explicitly nearer than the object-model wall: current `master` now materializes the helper-only fdinfo map-info and map-reuse compatibility packet inside the shared `tools/lib/bpf/zigux_segments/file_path_handle_bridge.zig` destination, while the deterministic Phase 12 tracked-helper snapshot still stays narrower than that shared bridge file and the heavier shared file-path-and-handle bridge remains its own deferred bucket. The current same-lane truthfulness gap is that the review packet still labels those two bridge-local helper slices as `ready-next`, so the next bounded follow-through is to reconcile that manifest-backed label set with the already-landed shared bridge helper file before widening into the deferred bridge or object-model wall.
