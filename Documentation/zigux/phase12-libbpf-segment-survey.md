...
- the older segment catalog still leaves three bounded shared-bridge helpers explicitly nearer than the object-model wall: current `master` now materializes the helper-only fdinfo map-info packet, the map-reuse compatibility packet, and token-path readiness planning inside the shared `tools/lib/bpf/zigux_segments/file_path_handle_bridge.zig` destination, while the deterministic Phase 12 tracked-helper snapshot still stays narrower than that shared bridge file and the heavier shared file-path-and-handle bridge remains its own deferred bucket because direct procfs reads, token opens, bpffs reopens, and fd ownership semantics are still not landed.
