...
That matters because current `master` still exposes a bounded directly readable `zigux_segments` footing through `logging.zig`, `pin_path.zig`, `cpu_mask.zig`, `type_names.zig`, `perf_buffer_poll.zig`, `verify.zig`, and the legacy `manifest.json` catalog, while the direct `phase12_libbpf_*` replay files plus `tools/lib/bpf/zigux_segments/file_path_handle_bridge.zig` and `tools/lib/bpf/zigux_segments/online_cpu_routing.zig` remain parked note-owned boundaries. That mixed state preserves real segmented progress while also keeping the broader reviewability packet smaller than the shipped replay order.
...
- current `master` still exposes a bounded directly readable libbpf segment footing under `tools/lib/bpf/zigux_segments/` through `logging.zig`, `pin_path.zig`, `cpu_mask.zig`, `type_names.zig`, `perf_buffer_poll.zig`, `verify.zig`, and the legacy `manifest.json` catalog.
