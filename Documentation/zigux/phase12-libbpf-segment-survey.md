...
Keep the Phase 12 libbpf survey and reviewability lane aligned with the live helper set, and when a lossless whole-file publication path is available, harden `scripts/zigux/validate-phase12.py` so the shared validator exact-counts the same focused libbpf-only replay review hook that the focused replay checker and packet checker already fail closed on today. Only reopen `tools/lib/bpf/zigux_segments/` for another bounded utility slice if fresh repo reality shows something materially smaller than the still-blocked skeleton, loader, and relocation work.
