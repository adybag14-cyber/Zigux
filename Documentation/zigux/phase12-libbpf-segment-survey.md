...
- reversible delivery evidence: this Phase 12 packet only adds `zigux/tests/phase12_libbpf_segments.zig`, `zigux/tests/phase12_libbpf_reviewability.zig`, `zigux/tests/phase12_libbpf_only_build.zig`, `scripts/zigux/check-phase12-libbpf-focused-replay.py`, and this survey note around preexisting helper foundations, so the survey can be removed without inventing a second libbpf implementation or mutating `tools/lib/bpf/libbpf.c`.
