...
- the live repo already ships the earlier `tools/lib/bpf/zigux_segments/manifest.json` survey plus five landed helper slices:
  ...
  - `file_path_handle_bridge.zig` for explicit `/proc/<pid>/fdinfo/<fd>` path construction, bounded `map_type`, `key_size`, `value_size`, `max_entries`, and `map_flags` text parsing, and token-preparation recovery classification without direct procfs reads, token creation, or reopen-flow parity
...
- the bounded file-path-and-handle helper now also mirrors the libbpf token-preparation recovery split more faithfully: optional probes still degrade gracefully, mandatory probes still fail hard, and the optional `-ENOENT` delegation-miss path stays distinct from the generic open or token-create failure path without claiming any direct `open()` or `bpf_token_create()` parity.
