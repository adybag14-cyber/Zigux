...
- the shared Phase 12 validation path now also carries a committed reproducibility packet for this bounded libbpf lane: `scripts/zigux/check-phase12-libbpf-snapshot.py` rebuilds the manifest, survey gate, reviewability gate, survey note, and legacy segment catalog snapshot twice, proves the JSON is stable across repeat runs, and compares it against `zigux/tests/fixtures/phase12_libbpf_snapshot.json` before the shared Phase 12 validator passes. The same bounded packet now also treats `python3 scripts/zigux/check-phase12-libbpf-snapshot.py --self-test` as the first checker replay before the live fixture comparison runs through `make -C zigux phase12-validate`.
