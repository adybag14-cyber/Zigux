...
- the dedicated libbpf verify-shard packet stays parked behind `Documentation/zigux/phase12-libbpf-verify-shard-note.md`. This survey must not present `zigux/tests/phase12_libbpf_segments.zig`, `zigux/tests/phase12_libbpf_reviewability.zig`, `zigux/tests/phase12_libbpf_manifest.json`, the tracked-helper snapshot fixtures, or `tools/lib/bpf/zigux_segments/verify.zig` as part of the shipped replay order unless a future repo change makes that packet live again.
...
- the parked verify-shard and reviewability packet described in `Documentation/zigux/phase12-libbpf-verify-shard-note.md`, which remains outside the shipped replay order until the repo exposes that packet again
