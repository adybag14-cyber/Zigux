...
- the live repo already ships the earlier `tools/lib/bpf/zigux_segments/manifest.json` survey plus five landed helper slices:
  ...
  - `file_path_handle_bridge.zig` for explicit `/proc/<pid>/fdinfo/<fd>` path construction plus bounded `map_type`, `key_size`, `value_size`, `max_entries`, and `map_flags` text parsing without direct procfs reads, token creation, or reopen-flow parity
