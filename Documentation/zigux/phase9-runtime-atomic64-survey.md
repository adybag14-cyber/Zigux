...
- `PHASE9_SURVEYED_COMMIT=2026-05-20-runtime-atomic64-route-truthfulness`
...
4. `zigux/tests/phase9_build.zig` keeps the direct `phase9-runtime-atomic64-diff` rerun explicit and the shared `phase9-first-loadable-runtime-module-parity-survey-tests` cross-family reminder route visible; it does not currently materialize a broader family-local atomic64 sample, module, loader, survey, or aggregate route packet on current `master`.
5. No dedicated `make -C zigux phase9-runtime-atomic64-test` route is currently materialized on current `master`, so that family-local convenience handle stays backlog vocabulary instead of shipped make-route evidence.
6. `Documentation/zigux/phase9-first-loadable-runtime-module-parity.md`, `Documentation/zigux/phase9-runtime-loader-gap-survey.md`, `scripts/zigux/check-phase9-build-only-surface.py`, `.github/workflows/zigux-bootstrap.yml`, `zigux/tests/phase9_build.zig`, `zigux/tests/runtime_loader_allocator_init_flow.zig`, `zigux/tests/runtime_loader_selftest_complete_exit_parity.zig`, `zigux/tests/runtime_loader_gap_manifest.json`, `zigux/tests/runtime_loader_gap_survey.zig`, `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, and `zigux/Makefile` remain visible shared loader-facing and first-loadable parity review surfaces, while the broader runtime substrate stays blocked.
