# Phase 12 Virtio SCSI Slice

This bounded Phase 12 slice adds the first Zigux `virtio scsi` starter anchored to `drivers/scsi/virtio_scsi.c`.

The starter stays intentionally narrow:

- plans the control, event, and request virtqueue family layout from the requested request-queue count and poll-queue hint
- preserves the Linux driver rule that poll queues are clamped so at least one default request queue remains
- classifies each planned request queue as either `request` or `request_poll` with stable global virtqueue indexes starting after the control and event queues
- derives one bounded queue-window summary that keeps the control and event queue gap plus the default-versus-poll request-queue ranges explicit without claiming blk-mq queue mapping or live request ownership
- records one bounded `virtscsi_probe()` config snapshot through `num_queues`, `seg_max`, `cmd_per_lun`, `max_target`, `max_lun`, `max_sectors`, and the derived control or event versus request virtqueue layout before any blk-mq submission or DMA-backed queue work
- records one bounded pre-registration host-shape summary from that probe-facing state so `sg_tablesize`, `cmd_per_lun`, `max_sectors`, `max_id`, `max_lun`, `max_cmd_len`, `nr_hw_queues`, `nr_maps`, and `dma_boundary` stay reviewable before any `Scsi_Host` registration, live blk-mq queue mapping, or DMA-backed request ownership
- records the fixed event-buffer fanout used by the driver and derives one bounded restore-time event-buffer ownership summary so the event queue remains reserved across freeze and restore without claiming event work handling, request submission, or live transport reset completion
- freezes queue planning and event recycling intent across a lab-only transport freeze or restore boundary, then clears the old queue snapshot so the next step must replan instead of pretending virtqueues stayed live; the focused gate now also proves that a second freeze or restore cycle uses the newly replanned topology and increments the bounded recovery generation instead of reusing stale queue state
- records one bounded freeze-time request-queue quiesce summary from the frozen queue layout so the starter keeps control and event queue reservation, default-versus-poll request-queue quiesce order, blocked request-queue use during freeze, and restore-before-replan discipline explicit without claiming live `del_vqs()` side effects or DMA-backed recovery
- derives one bounded restore-sequencing summary from the frozen queue layout so the starter keeps `virtscsi_restore()` calling `find_vqs`, `virtio_device_ready()`, and event rearm reviewable without pretending to re-run `scsi_scan_host()`
- records one bounded request-queue restart summary from the frozen queue layout so the starter keeps the post-reset request-queue restart boundary tied to `find_vqs()`, `virtio_device_ready()`, event rearm, and fresh queue replanning without claiming live command submission, completion handling, or host scan replay
- records one bounded request-queue ownership summary from the frozen queue layout so the starter keeps the default-versus-poll partition, blocked request-queue access, restore-before-replan discipline, and event-rearm-before-reuse governance explicit across transport reset without pretending to re-run `scsi_scan_host()` or resume live command submission
- records one bounded recovery event-rearm summary from the frozen queue layout so the starter keeps the reused event queue index, the device-ready-before-rearm rule, and event rearm before event recycling or request queue reuse explicit without claiming live completion handling
- records one bounded recovery rollback summary from the frozen queue layout so the starter keeps restore-before-replan discipline, blocked request-queue reuse, and stale-layout clearing explicit without pretending transport reset leaves live request paths behind

This slice does not claim DMA mapping, scatter-gather command assembly, `Scsi_Host` registration, blk-mq submission, event-work recycling, TMF handling, hotplug, or live transport reset recovery.

The shared Phase 12 packet now keeps the direct smoke preflight explicit here too: `zig build smoke --build-file zigux/tests/phase12_build.zig --summary all` and `make -C zigux phase12-smoke` rerun this bounded `virtio scsi` starter before the broader survey-backed replay, so the slice should stay aligned with that smoke-plus-build order instead of leaving it implicit in `zigux/tests/phase12_build.zig`, `zigux/Makefile`, or `Documentation/zigux/phase12-virtio-scsi-raw-github-fallback-catalog.md` alone.

The next honest bounded step inside the same Phase 12 lane is no longer another queue-family-only helper. Keep this slice parked until the roadmap approves queue ownership, SCSI host registration, or DMA-backed queue work beyond the now-landed `virtscsi_probe()` config snapshot, pre-registration host-shape summary, freeze-time request-queue quiesce summary, request-queue restart summary, request-queue ownership summary, and recovery rollback summary.
