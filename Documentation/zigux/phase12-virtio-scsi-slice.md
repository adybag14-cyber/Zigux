...
- records one bounded queue-depth clamp summary plus a recovery-side queue-depth restore summary so the starter keeps `cmd_per_lun`, synthetic `can_queue`, and restore-time `change_queue_depth` expectations explicit without claiming live SCSI-host queue registration
- records one bounded blk-mq I/O queue map summary plus a recovery-side I/O queue map restore summary so the starter keeps default versus poll queue offsets, virtio-affinity expectations, and restore-time poll-map reuse explicit without claiming live blk-mq tag-set wiring
...
This slice does not claim DMA mapping, scatter-gather command assembly, `Scsi_Host` registration, live blk-mq tag-set wiring, event-work recycling, TMF handling, hotplug, or live transport reset recovery.
