...
- records one bounded restore queue rebind summary from the frozen queue layout so the starter keeps the default-versus-poll request queue ranges explicit across restore without claiming blk-mq queue mapping or live request resubmission
...
- records one bounded rollback summary from the frozen queue layout so the starter keeps restore-before-replan, frozen-layout preservation, and queue-access blocking explicit without claiming live DMA-backed recovery parity
...
The next honest bounded step inside the same Phase 12 lane is no longer another queue-family-only helper. Keep this slice parked until the roadmap approves queue ownership, SCSI host registration, or DMA-backed queue work beyond the now-landed probe-config snapshot and bounded recovery packet.
