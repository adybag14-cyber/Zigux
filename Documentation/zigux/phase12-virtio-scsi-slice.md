...
- derives one bounded restore-sequencing summary from the frozen queue layout so the starter now records `virtscsi_restore()` re-entering `virtscsi_init()`, calling `virtio_device_ready()`, and rearming event buffers with `virtscsi_kick_event_all()` while still making it explicit that the restore helper does not pretend to re-run `scsi_scan_host()` or port live `Scsi_Host` registration
