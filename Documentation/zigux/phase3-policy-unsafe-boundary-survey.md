...
- `zigux/helpers/mmio.zig` routes scoped MMIO helpers back through that same narrow unsafe layer, `zigux/helpers/mmio.zig` is now the shipped second boundary helper that consumes `DecodedInteropPolicy` directly outside the focused `phase3_policy_unsafe` test packet`, and the focused replay now reaches that typed-policy MMIO surface through `readScopedWithPolicy`, `mmio.write32Policy()` and `mmio.read32Policy()` so width-specific decoded-policy MMIO access stays attached to the same narrow boundary packet instead of widening into a generic raw-pointer helper family
