...
The current tree still does not ship a dedicated `phase3_policy_unsafe` replay pair, but the live proof is no longer shared-ABI-slice-only. It is split between the helper-local policy starter packet, the focused policy dump route, the directly coupled low-level-wrapper packet, and the dedicated blob-marker survey guard. This note should stay tied to those current packet-local surfaces instead of using `Documentation/zigux/phase3-abi-slice.md`, `zigux/tests/fixtures/phase3_abi_manifest.json`, or `scripts/zigux/validate-phase3.py` as its parking trigger.
## Ledger Alignment
