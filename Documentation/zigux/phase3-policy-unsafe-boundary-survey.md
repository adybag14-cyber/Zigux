...
- `zigux/tests/phase3_policy_unsafe_build.zig` and `zigux/tests/phase3_policy_unsafe.zig` now keep `layout_assert`, panic, allocator, typed `InteropPolicy` decoding, unsafe-byte decoding, declared-scope enforcement, decoded-policy MMIO bridging, and decoded-policy raw-pointer bridge reads on their own focused replay path rather than leaving that packet visible only through the broader `phase3_abi.zig` bundle, and the focused replay now also keeps `phase3 policy layout stays explicit at the ABI boundary` attached to the shared compile-time `InteropPolicy` contract instead of leaving the enum-byte ABI proof only in the broader ABI bundle
