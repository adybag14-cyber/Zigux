...
- scope: shared release-planning truthfulness, fallback wording, smoke-first replay reminders, and anti-overlap guidance for the bounded libbpf survey packet plus the parked verify-shard boundary, the checked-in reviewability gate, and the helper-local determinism companion already documented on current `master`
