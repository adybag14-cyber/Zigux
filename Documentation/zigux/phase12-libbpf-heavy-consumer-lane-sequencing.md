...
- Keep the snapshot-only-versus-parked-file boundary and reviewability wording explicit so the release-facing libbpf packet does not collapse back into manifest-only prose, does not hide the still-shipped snapshot anchor, and does not wrongly treat absent reviewability files as shipped current-`master` surfaces.
