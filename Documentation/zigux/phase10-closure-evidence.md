...
That means the current same-lane truthfulness work is now the next shared docs-root-plus-harness-checker repair, not the already-caught-up tests-root guide, compact companion, docs-root helper marker, or ring-only reviewer note.
...
2. refresh `Documentation/zigux/README.md` together with the coupled docs-root marker block in `scripts/zigux/check-phase10-harness-coverage.py` so the docs-root Phase 10 summary and its fail-closed checker stop treating the restored ring-plus-input slice notes as missing repo-reality gaps
3. once that paired docs-root and harness-checker sync lands, re-reread `Documentation/zigux/review-checklist.md` and `scripts/zigux/README.md` together while keeping the remaining missing core and MMIO slice-note companions explicit, the publicly visible direct ring and input driver-local surfaces plus the direct MMIO helper explicit, and the still-unreadable `drivers/virtio/virtio_mmio_verify.zig` boundary plus the current MMIO helper ladder through `phase10-mmio-selected-queue-readiness-helper` fail-closed and explicit until the next direct current-`master` reread says otherwise
