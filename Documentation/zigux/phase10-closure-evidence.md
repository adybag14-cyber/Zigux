...
Fresh direct rereads now show that `Documentation/zigux/phase10-virtio-input-slice.md` and `Documentation/zigux/phase10-virtio-input-module-slice.md` are also re-readable on current `master`, and both the compact tests-root companion and `zigux/tests/README.md` now match that narrower slice-gap posture by treating only the core and MMIO slice companions as remaining repo-reality gaps. `Documentation/zigux/README.md` now matches that same narrower posture, and fresh public raw rereads show that `scripts/zigux/README.md` does too. The broader shared reminder drift therefore now sits only in `Documentation/zigux/review-checklist.md`, which still frames the ring, input, and input-module slice companions as repo-reality gaps even though the current closure packet, the compact tests-root companion, the tests-root guide, the docs root, and the scripts root already treat those three notes as directly re-readable evidence.

No narrower shared-surface undercount was directly verified in this run beyond that remaining review-checklist drift. The older checker-local, compact-companion, tests-root, docs-root, and scripts-root follow-throughs recorded here are already landed on current `master`, and live repo-first comparison now shows the smaller next same-lane repair is the paired `Documentation/zigux/review-checklist.md` plus `scripts/zigux/check-phase10-harness-coverage.py` sync: the checklist still treats all five slice-note companions as repo-reality gaps, and the dedicated harness checker still fail-closes on that older checklist marker set even though the current docs-root, scripts-root, tests-root companion, tests-root guide, lane note, and closure manifest only leave `Documentation/zigux/phase10-virtio-core-slice.md` and `Documentation/zigux/phase10-virtio-mmio-slice.md` absent.
...
That means the current same-lane truthfulness work is now the paired review-checklist plus harness-coverage repair, not the already-caught-up scripts-root, tests-root guide, compact companion, docs-root helper marker, or ring-only reviewer note.
...
2. refresh `Documentation/zigux/review-checklist.md` together with the coupled review-checklist marker block in `scripts/zigux/check-phase10-harness-coverage.py` so the shared checklist and its fail-closed checker stop treating the restored ring-plus-input slice notes as missing repo-reality gaps before any transport-facing work
3. after that paired shared-summary repair lands, reread `Documentation/zigux/phase10-closure-evidence.md` beside the remaining missing core and MMIO slice-note companions, the publicly visible direct ring and input driver-local surfaces, the direct MMIO helper surface, and the still-unreadable `drivers/virtio/virtio_mmio_verify.zig` boundary before widening any follow-through further