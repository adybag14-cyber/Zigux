...
They also confirm that the remaining packet-local Phase 10 slice companions still missing on current `master` are narrower than the older shared summaries claim: only `Documentation/zigux/phase10-virtio-mmio-slice.md` remains a repo-reality gap, while `Documentation/zigux/phase10-virtio-core-slice.md`, `Documentation/zigux/phase10-virtio-ring-slice.md`, `Documentation/zigux/phase10-virtio-input-slice.md`, and `Documentation/zigux/phase10-virtio-input-module-slice.md` are directly re-readable again.
...
- `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, and `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md` now keep most of the current input packet explicit beside the queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays, and the compact tests-root companion now also keeps the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper explicit beside `drivers/virtio/virtio_input.zig` and `drivers/virtio/virtio_input_verify.zig` instead of carrying the older helper-undercount. `Documentation/zigux/README.md` also keeps that direct `drivers/virtio/virtio_input_probe_preflight.zig` helper explicit beside `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_input_verify.zig`, and the tests-root input preflight, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays, and fresh public raw rereads now show that `scripts/zigux/README.md` matches that narrower slice-gap posture too.

## Current Truthfulness Posture

Fresh rereads show that the earlier broad shared reminder-surface drift is now closed in the docs-root Phase 10 summary: current `master` keeps the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper explicit there, and the live `scripts/zigux/check-phase10-harness-coverage.py` marker set plus self-test already fail closed on that same docs-root helper marker.
...
Fresh direct rereads now show that `Documentation/zigux/phase10-virtio-core-slice.md`, `Documentation/zigux/phase10-virtio-input-slice.md`, and `Documentation/zigux/phase10-virtio-input-module-slice.md` are also re-readable on current `master`, and both the compact tests-root companion and `zigux/tests/README.md` now match that narrower slice-gap posture by treating only the MMIO slice companion as a remaining repo-reality gap. `Documentation/zigux/README.md` now matches that same narrower posture, and fresh public raw rereads show that `scripts/zigux/README.md` does too. Current live rereads also no longer support an older checklist-local gap claim: `Documentation/zigux/review-checklist.md` is directly readable on current `master`, but it does not currently carry a separate shared Phase 10 reminder bullet that still frames the restored ring-plus-input slice notes as missing repo-reality gaps.

No narrower shared-surface undercount was directly verified in this run beyond the remaining dedicated harness-checker drift. The older checklist-local, compact-companion, tests-root, docs-root, and scripts-root follow-throughs recorded here are already landed on current `master`, and live repo-first comparison now shows the smaller next same-lane repair is the dedicated `scripts/zigux/check-phase10-harness-coverage.py` marker block plus self-test fixture: that checker still fail-closes on an older review-checklist marker set even though the current docs-root, scripts-root, tests-root companion, tests-root guide, lane note, closure-manifest, and live-checklist packet no longer publish that older five-slice-gap sentence.
...
That means the current same-lane truthfulness work is now the dedicated harness-coverage checker repair, not the already-caught-up review checklist, scripts-root, tests-root guide, compact companion, docs-root helper marker, or ring-only reviewer note.
...
2. refresh `scripts/zigux/check-phase10-harness-coverage.py` together with its self-test fixture so the dedicated checker stops requiring a nonexistent Phase 10 review-checklist marker block and instead matches the current docs-root, scripts-root, tests-root companion, tests-root guide, lane note, closure-manifest, and live-checklist packet before any transport-facing work
3. after that checker-local repair lands, reread `Documentation/zigux/phase10-closure-evidence.md` beside the remaining missing MMIO slice-note companion, the publicly visible direct core, ring, and input driver-local surfaces, the direct MMIO helper surface, and the still-unreadable `drivers/virtio/virtio_mmio_verify.zig` boundary before widening any follow-through further
