...
- `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, and `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md` now keep most of the current input packet explicit beside the queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays, and the compact tests-root companion now also keeps the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper explicit beside `drivers/virtio/virtio_input.zig` and `drivers/virtio/virtio_input_verify.zig` instead of carrying the older helper-undercount. `Documentation/zigux/README.md` still undercounts that direct input-helper packet on current `master`: it keeps the direct `drivers/virtio/virtio_input.zig` and `drivers/virtio/virtio_input_verify.zig` surfaces plus the tests-root preflight, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays explicit, but it still omits the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper that the compact companion and lane-sequencing note already name.

## Current Truthfulness Posture

Fresh rereads show that the earlier broad shared reminder-surface drift is mostly closed inside this lane, and the previously noted compact tests-root companion undercount is now closed on current `master`. The remaining same-lane drift is narrower: the docs-root Phase 10 summary and the shared harness checker still stop short of the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper that the compact companion, lane note, and input survey already keep explicit.
...
`Documentation/zigux/README.md` still stops at `drivers/virtio/virtio_input.zig` and `drivers/virtio/virtio_input_verify.zig` plus the tests-root input preflight, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays, so the broad docs-root Phase 10 summary still omits the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper that the compact companion, lane note, and input survey already keep explicit.
...
One narrower same-lane checker gap therefore remains open on current `master`: the live `scripts/zigux/check-phase10-harness-coverage.py` marker set still fail-closes on the docs-root Phase 10 packet without requiring the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper marker in `Documentation/zigux/README.md`, even though the compact companion, lane-sequencing note, and input survey already keep that direct helper explicit. This means the broad docs-root undercount can still slip past the shared harness checker until that marker is added and the checker self-test proves it.
...
That means the current same-lane truthfulness work is no longer a broad tests-root repair or a closed checker-support story. The sharpest remaining shared-surface drift is the docs-root and harness-checker undercount around `drivers/virtio/virtio_input_probe_preflight.zig`, while the five slice-note companions remain the broader repo-reality gaps behind it.
...
2. refresh `Documentation/zigux/README.md` so the broad Phase 10 docs-root summary keeps the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper explicit beside `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_input_verify.zig`, and the queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays
3. then harden `scripts/zigux/check-phase10-harness-coverage.py` so the shared docs-root coverage check fails closed on that direct probe-preflight helper marker and its self-test proves the new docs-root requirement
4. if that narrower docs-root-plus-checker repair lands, continue to keep the missing slice-note companions explicit, keep the now-publicly-visible direct ring, input, and MMIO driver-local surfaces explicit, and keep the current MMIO helper ladder through `phase10-mmio-selected-queue-readiness-helper` fail-closed and explicit until the next direct current-`master` reread says otherwise
