...
Fresh rereads show that the earlier broad shared reminder-surface drift is now closed inside this lane: current `master` now keeps the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper explicit in the docs-root Phase 10 summary beside the compact companion, lane note, and input survey. The remaining same-lane drift is narrower: the shared harness checker still stops short of that docs-root helper marker, so the broad summary can drift again without the shared checker failing closed.
...
One narrower same-lane checker gap therefore remains open on current `master`: the live `scripts/zigux/check-phase10-harness-coverage.py` marker set still fail-closes on the docs-root Phase 10 packet without requiring the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper marker in `Documentation/zigux/README.md`. Now that the docs-root summary has been refreshed, the remaining same-lane follow-through is to teach the shared harness checker and its self-test to fail closed on that direct helper marker too.
...
That means the current same-lane truthfulness work is no longer a broad tests-root repair or a docs-root undercount. The sharpest remaining shared-surface drift is now checker-local: the harness checker still undercounts the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper even though the shared reminder surfaces now keep it explicit.
...
2. harden `scripts/zigux/check-phase10-harness-coverage.py` so the shared docs-root coverage check fails closed on the direct `drivers/virtio/virtio_input_probe_preflight.zig` helper marker and its self-test proves the new docs-root requirement
3. if that narrower checker repair lands, continue to keep the missing slice-note companions explicit, keep the now-publicly-visible direct ring, input, and MMIO driver-local surfaces explicit, and keep the current MMIO helper ladder through `phase10-mmio-selected-queue-readiness-helper` fail-closed and explicit until the next direct current-`master` reread says otherwise
