...
3. once those broader summary surfaces catch up, keep the remaining missing core and MMIO slice-note companions explicit, keep the publicly visible direct ring and input driver-local surfaces plus the direct MMIO helper explicit, and keep the still-unreadable `drivers/virtio/virtio_mmio_verify.zig` boundary plus the current MMIO helper ladder through `phase10-mmio-selected-queue-readiness-helper` fail-closed and explicit until the next direct current-`master` reread says otherwise
