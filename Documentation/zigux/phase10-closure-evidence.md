...
Fresh direct rereads now show that `Documentation/zigux/phase10-virtio-input-slice.md` and `Documentation/zigux/phase10-virtio-input-module-slice.md` are also re-readable on current `master`, so the compact tests-root companion is now the sharpest stale shared summary: it still groups those two input-facing slice notes with the still-missing core and MMIO slice companions instead of limiting the repo-reality gap wording to the actual missing pair.

The broader shared reminder drift still extends into `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, and `Documentation/zigux/README.md`, which still carry older slice-gap wording derived from the all-five-missing or four-missing posture even though current `master` now re-materializes the ring, input, and input-module slice companions. But the smallest next repair is the compact tests-root companion because it is narrower than the checklist-first and broad summary follow-throughs while still feeding the reviewer-facing packet directly.

No narrower shared-surface undercount was directly verified in this run beyond that compact tests-root companion drift. The older checker-local and tests-root ring-surface follow-throughs recorded here are already landed on current `master`, so the honest same-lane repair in this pass is to stop pointing future shared-surface rereads at the already-closed ring-only mismatch and to keep the next reopen on the compact companion's stale slice-gap wording first.
...
That means the current same-lane truthfulness work is no longer a broad tests-root repair, a docs-root undercount, or a ring-only reviewer note. The directly verified stale item in this pass is the compact tests-root companion wording around which Phase 10 slice companions are still missing, with the broader review-checklist, scripts-root, and docs-root summaries parked behind that narrower follow-through.
...
2. after this closure-note refresh, reread `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md` against `Documentation/zigux/phase10-virtio-ring-slice.md`, `Documentation/zigux/phase10-virtio-input-slice.md`, `Documentation/zigux/phase10-virtio-input-module-slice.md`, and `zigux/tests/phase10_closure_manifest.json` so the compact reviewer-facing Phase 10 companion stops treating the restored ring-plus-input slice notes as missing repo-reality gaps without reopening the already-caught-up lane note or the broader docs-root summaries
3. once that compact companion repair lands, reread `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, and `Documentation/zigux/README.md` for the same stale slice-gap wording, keeping the remaining missing core and MMIO slice-note companions explicit, keeping the re-readable ring, input, and input-module slices explicit, keeping the publicly visible direct ring, input, and MMIO driver-local surfaces explicit, and keeping the current MMIO helper ladder through `phase10-mmio-selected-queue-readiness-helper` fail-closed and explicit until the next direct current-`master` reread says otherwise
