...
Public-tree fallback rereads on current `master` do materialize `scripts/zigux/validate-phase10.py`, `scripts/zigux/validate-phase10-closure.py`, `zigux/Makefile`, `zigux/tests/phase10_closure_manifest.json`, `Documentation/zigux/phase10-virtio-core-survey.md`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_ring_verify.zig`, and `zigux/tests/phase10_virtio_mmio_manifest.json`, so treat the missing-read line above as a connector-readback limitation rather than as a claim that those public-tree paths are absent from `master`.
