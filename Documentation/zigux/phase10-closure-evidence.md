...
This keeps the closure packet aligned with the roadmap's real Phase 10 requirements: queue-facing lab wrappers are landed, the bounded MMIO register-window, queue-register, and queue-notify helpers are now landed, the lab validation gate is real, and risky transport expansion is still intentionally blocked.
