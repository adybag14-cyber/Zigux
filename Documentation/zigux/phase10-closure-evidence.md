...
The older narrowed readback shortcut that treated both `Documentation/zigux/README.md` and `scripts/zigux/README.md` as already aligned is no longer fully accurate. Fresh rereads still support treating `scripts/zigux/README.md` as aligned shared reminder context for this packet, but the broader docs-root summary in `Documentation/zigux/README.md` and the compact tests-root companion still need a fresh repo-first correction before this lane should move on to checker-only follow-through.
