...
- the public `drivers/virtio` tree now shows `drivers/virtio/virtio_ring.zig` and `drivers/virtio/virtio_ring_verify.zig` on current `master`, so the live ring packet is no longer only a manifest-and-survey reminder. Shared Phase 10 reminder surfaces therefore need to keep those direct ring helper and verify paths explicit beside `zigux/tests/phase10_virtio_ring_manifest.json` and `Documentation/zigux/phase10-virtio-ring-survey.md`, while still failing closed on the missing packet-local slice companion `Documentation/zigux/phase10-virtio-ring-slice.md` and any focused tests-root ring replay paths that this run could not re-read directly through the authenticated contents bridge.
- `Documentation/zigux/phase10-virtio-input-survey.md` remains directly readable on current `master`, and the public `drivers/virtio` tree now also keeps `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_input_probe_preflight.zig`, and `drivers/virtio/virtio_input_verify.zig` visible again. The shared reminder surfaces therefore need to keep that direct input helper-facing packet explicit beside `zigux/tests/phase10_virtio_input_manifest.json`, while still treating `Documentation/zigux/phase10-virtio-input-slice.md` and `Documentation/zigux/phase10-virtio-input-module-slice.md` as repo-reality gaps and keeping real event delivery, `input_register_device()` lifecycle coverage, freeze or restore parity, and transport-backed queue callbacks intentionally blocked.
...
The live closure manifest, the directly readable tests-root companion, the shared scripts-root summary, and the public `drivers/virtio` tree now agree on a wider directly visible Phase 10 packet than the docs root currently describes: current `master` already keeps the direct ring, input, and MMIO helpers, verifier surfaces, focused replays, shared closure note, lane-sequencing note, shared checkers, and validator-backed make routes explicit across `zigux/tests/README.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, and `zigux/tests/phase10_closure_manifest.json`. The remaining shared drift is therefore no longer in the tests root or scripts root. It is the docs-root Phase 10 summary in `Documentation/zigux/README.md`, which still presents the five missing packet-local slice companions as shipped docs-root evidence and still under-describes the checker-backed closure packet that current `master` already carries.

The remaining honest repo-reality gaps in this lane are still the packet-local slice companions `Documentation/zigux/phase10-virtio-core-slice.md`, `Documentation/zigux/phase10-virtio-ring-slice.md`, `Documentation/zigux/phase10-virtio-input-slice.md`, `Documentation/zigux/phase10-virtio-input-module-slice.md`, and `Documentation/zigux/phase10-virtio-mmio-slice.md`. Shared reminder surfaces should keep those five note paths framed as repo-reality gaps rather than shipped current-`master` evidence while keeping the already-landed direct ring, input, and MMIO helper-facing packet explicit.

That means the remaining honest Phase 10 gap in this lane is no longer an absent direct ring packet or a stale scripts-root summary. It is one docs-root truthfulness task: keep `Documentation/zigux/README.md` aligned with the closure manifest, the tests-root review companion, the review checklist, the shared scripts-root packet, the public-tree-visible direct ring, input, and MMIO driver-local surfaces, and the newer MMIO helper ladder, without overclaiming the still-missing slice-note companions or widening into risky transport work.
...
2. refresh `Documentation/zigux/README.md` so its Phase 10 docs-root summary stops listing `Documentation/zigux/phase10-virtio-core-slice.md`, `Documentation/zigux/phase10-virtio-ring-slice.md`, `Documentation/zigux/phase10-virtio-input-slice.md`, `Documentation/zigux/phase10-virtio-input-module-slice.md`, and `Documentation/zigux/phase10-virtio-mmio-slice.md` as shipped docs-root evidence, and instead matches the live tests-root and scripts-root packet by keeping `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, `Documentation/zigux/phase10-virtio-driver-lane-sequencing.md`, `Documentation/zigux/phase10-closure-evidence.md`, `scripts/zigux/check-phase10-harness-coverage.py`, `scripts/zigux/check-phase10-tests-readme-core-surfaces.py`, `scripts/zigux/validate-phase10.py`, `scripts/zigux/validate-phase10-closure.py`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_ring_verify.zig`, `zigux/tests/phase10_virtio_ring_reset_reuse.zig`, `zigux/tests/phase10_virtio_ring_manifest.json`, the direct input helper plus verifier and focused preflight or teardown replays, the direct MMIO helper plus verifier packet, and the `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10` routes explicit while framing those five slice-note companions as repo-reality gaps on current `master`
