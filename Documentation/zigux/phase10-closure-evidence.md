...
That previously narrower same-lane checker gap is now closed on current `master`: the live `scripts/zigux/check-phase10-harness-coverage.py` marker set already includes the compact tests-root companion's direct `drivers/virtio/virtio_input_probe_preflight.zig` helper wording beside the ring drained-reset reuse reminder, and its built-in self-test now carries focused missing-marker cases for that companion surface.
...
That means the current same-lane truthfulness work is no longer a broad docs-root repair, a compact companion refresh, or a still-open harness-checker support gap. Most of the shipped shared reminder surfaces are aligned again, so the next bounded same-lane follow-through should stay parked unless a fresh repo-first reread finds another one-file reminder, checker, manifest, survey, or closure-note drift inside the existing virtio packet.
...
2. keep the now-aligned `scripts/zigux/check-phase10-harness-coverage.py` support surface in sync with the compact tests-root companion, the input survey, and the lane-sequencing note; do not reopen that earlier checker gap unless a fresh reread finds new drift across those exact reminder surfaces
