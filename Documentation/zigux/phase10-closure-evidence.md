...
Fresh shared-surface readback now shows `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and `zigux/tests/README.md` already keep the landed direct `drivers/virtio/virtio.zig`, `drivers/virtio/virtio_driver_id.zig`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_mmio.zig`, and the focused `zigux/tests/phase10_virtio_input_probe_preflight.zig` replay explicit beside the focused verify, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain reminders.

The next same-lane truthfulness gap is now the missing shared tests-root companion note `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, which several live Phase 10 surfaces still reference even though it is no longer present on `master`. If this lane reopens soon, prefer deciding whether to restore that companion note in a truthful reduced form or replace those lingering references with a slimmer existing shared surface before widening into helper, MMIO transport, IRQ, reset, queue-discovery, or lifecycle behavior.
