...
- scope: one shared closure note for the shipped virtio core, ring, input, and MMIO lab slices, the bounded core-side `virtio_driver_id` helper, the current manifest-backed checker packet, the 14-file tests-root build packet, the focused core, ring, input, and MMIO verifier replays, the dedicated input queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays, the direct MMIO replay, and Linux-style replay routes
