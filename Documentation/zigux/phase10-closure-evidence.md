...
Fresh shared-surface readback now shows `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, and `Documentation/zigux/review-checklist.md` already keep the landed direct `drivers/virtio/virtio.zig`, `drivers/virtio/virtio_driver_id.zig`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_mmio.zig`, and the focused `zigux/tests/phase10_virtio_input_probe_preflight.zig` replay explicit beside the focused verify, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain reminders.

Leave the shared closure note parked until fresh inspection finds another equally small same-lane manifest, survey, checker, or shared-summary truthfulness gap inside the landed virtio core, ring, input, and MMIO packet. If the lane reopens soon, prefer the next one-file shared review-surface or validation-surface sync before widening helper, MMIO transport, IRQ, reset, queue-discovery, or lifecycle behavior.
