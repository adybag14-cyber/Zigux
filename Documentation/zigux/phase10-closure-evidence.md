...
- shared packet direct-readback inventory is mixed on current `master`: the shared docs and manifests remain readable, the surviving direct driver anchors are `drivers/virtio/virtio_input.zig` and `drivers/virtio/virtio_mmio.zig`, and the surviving direct lab-validation replays stay limited to `zigux/tests/phase10_build.zig` plus the input and MMIO test packet
...
- drivers: `drivers/virtio/virtio_input.zig` and `drivers/virtio/virtio_mmio.zig`
