...
That means the older shared reminder-surface drift called out in this lane is no longer the compact tests-root companion. The remaining truthful closure-note gap is narrower: keep the shared packet anchored to the aligned closure manifest, the refreshed tests-root and docs-root reminders, the restored core, ring, input, and input-module slice companions, and the still-missing MMIO slice companion without re-expanding the lane into neighboring summary ownership.
