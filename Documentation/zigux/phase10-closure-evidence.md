...
- the shared reminder surfaces in `Documentation/zigux/README.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, `scripts/zigux/README.md`, and this closure note, which keep the landed shared validation route, the shared harness-coverage checker, and the direct-core tests-root checker explicit on current `master` while the remaining checklist and tests-root follow-through gaps stay called out below instead of being overstated as already closed
...
Second, the broad shared reminder surfaces still carry two smaller follow-through gaps. `Documentation/zigux/review-checklist.md` keeps the larger Phase 10 packet explicit, but it still omits `scripts/zigux/check-phase10-tests-readme-core-surfaces.py` even though the docs root, scripts root, lane-sequencing note, tests-root companion, and this closure note already treat that guard as shipped evidence. `zigux/tests/README.md` still needs the direct `drivers/virtio/virtio.zig` plus `drivers/virtio/virtio_driver_id.zig` reminder carried explicitly in the same broad Phase 10 packet that the focused checker already expects.
...
1. if live `master` still omits it, publish only the one-file `Documentation/zigux/review-checklist.md` sync that adds `scripts/zigux/check-phase10-tests-readme-core-surfaces.py` to the shared Phase 10 checklist packet beside the already-explicit harness-coverage checker and validation route
2. after that smaller checklist repair lands, publish only the one-file `zigux/tests/README.md` direct-core reminder sync that `scripts/zigux/check-phase10-tests-readme-core-surfaces.py` already expects, so the broad Phase 10 tests-root packet explicitly carries `drivers/virtio/virtio.zig` and `drivers/virtio/virtio_driver_id.zig` beside the existing closure-manifest-backed core packet
