...
- scope: one shared closure note for the shipped virtio core, ring, input, and MMIO lab slices, the bounded core-side `virtio_driver_id` helper, the current per-slice-manifest-backed checker packet, the direct core, ring, input, and MMIO replays, the focused core, ring, input, and MMIO verifier replays, the four dedicated survey gates, the dedicated input probe-preflight, queue-callback-preflight, registration-preflight, teardown-observation, and status-drain replays, and Linux-style replay routes
