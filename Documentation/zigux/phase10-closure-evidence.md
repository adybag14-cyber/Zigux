...
Fresh direct rereads now show that `Documentation/zigux/phase10-virtio-input-slice.md` and `Documentation/zigux/phase10-virtio-input-module-slice.md` are also re-readable on current `master`, and both the compact tests-root companion and `zigux/tests/README.md` now match that narrower slice-gap posture by treating only the core and MMIO slice companions as remaining repo-reality gaps. `Documentation/zigux/README.md` now matches that same narrower posture too. The broader shared reminder drift therefore now sits one layer higher only in `Documentation/zigux/review-checklist.md` and `scripts/zigux/README.md`, which still frame the ring, input, and input-module slice companions as repo-reality gaps even though the current closure packet and the compact tests-root companion already treat those three notes as directly re-readable evidence.

No narrower shared-surface undercount was directly verified in this run beyond that remaining review-checklist and scripts-root drift. The older checker-local, compact-companion, tests-root, and docs-root follow-throughs recorded here are already landed on current `master`, and live repo-first comparison now shows the smaller next same-lane repair is the one-file `scripts/zigux/README.md` summary sync: its Phase 10 section still treats all five slice-note companions as repo-reality gaps even though current docs-root, tests-root companion, lane note, closure manifest, and this closure note only leave `Documentation/zigux/phase10-virtio-core-slice.md` and `Documentation/zigux/phase10-virtio-mmio-slice.md` absent. After that one-file scripts-root repair lands, the larger paired `Documentation/zigux/review-checklist.md` plus `scripts/zigux/check-phase10-harness-coverage.py` sync remains the next same-lane shared-surface follow-through before any transport-facing work.
...
That means the current same-lane truthfulness work is now the one-file `scripts/zigux/README.md` repair first, not the already-caught-up tests-root guide, compact companion, docs-root helper marker, or ring-only reviewer note.
