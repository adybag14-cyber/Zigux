...
- keep the current C anchor path, anchor blob, replay command, dedicated local survey wrapper, direct validation entrypoint, owner, rollback owner, reversible-delivery evidence, and missing-Zig-starter posture reviewable
- keep this packet adjacent to the shared Phase 4 validator-first packet while the shared gate-evidence note now names that same survey note, manifest, replay command, local survey wrapper, direct validation entrypoint, and reversible-delivery evidence without claiming a shipped Zig starter
...
- the dedicated parked gap packet already spans this note, `zigux/tests/phase4_kprobe_example_manifest.json`, and `zigux/tests/phase4_kprobe_example_survey.zig`, and the shared gate-evidence note now names that same survey note, manifest, replay command, local survey wrapper, direct validation entrypoint, and reversible-delivery evidence as adjacent evidence without claiming that a shipped Zig starter exists
