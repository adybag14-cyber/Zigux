...
- carries a small wrapper-facing verification replay in `drivers/virtio/virtio_input_verify.zig` that checks queue-callback and registration-preflight ordering without widening into transport-backed behavior
...
The current live review packet is broader than this module-local summary alone: `Documentation/zigux/phase10-virtio-input-slice.md`, `Documentation/zigux/phase10-virtio-input-survey.md`, `scripts/zigux/check-phase10-input-packet.py`, `drivers/virtio/virtio_input_verify.zig`, `zigux/tests/phase10_virtio_input.zig`, `zigux/tests/phase10_virtio_input_manifest.json`, `zigux/tests/phase10_virtio_input_survey.zig`, `zigux/tests/phase10_virtio_input_probe_preflight.zig`, `zigux/tests/phase10_virtio_input_queue_callback_preflight.zig`, `zigux/tests/phase10_virtio_input_registration_preflight.zig`, `zigux/tests/phase10_virtio_input_teardown_observation.zig`, `zigux/tests/phase10_virtio_input_status_drain.zig`, `zigux/tests/phase10_build.zig`, `zigux/Makefile`, `make -C zigux phase10-test`, and `make -C zigux phase10` keep the bounded starter, the direct helper-facing replay, the dedicated packet guard, the manifest-backed survey replay, the wrapper-facing verify replay, the dedicated probe-preflight replay, the dedicated queue-callback-preflight replay, the focused registration-preflight replay, the focused teardown-observation replay, the focused status-drain replay, and the shared Phase 10 build-and-make routes aligned on `master` instead of leaving this input lane reviewable only through the module slice.

Wrapper ownership stays outside this module-local slice even after the verify replay landed: `drivers/virtio/virtio.zig` still owns shared device-status bookkeeping, `drivers/virtio/virtio_ring.zig` still owns virtqueue wrapper shape and notification planning, and `drivers/virtio/virtio_mmio.zig` still owns MMIO wrapper planning. The input lane only consumes those already-landed packets as prerequisites for the bounded lab starter, the wrapper-facing verify replay, the dedicated probe-preflight replay, the dedicated queue-callback-preflight replay, the focused registration-preflight replay, the focused teardown-observation replay, and the focused status-drain replay.
