...
This slice does not claim MMIO transport work, DMA-facing queue plumbing, multitouch slot planning, input core capability registration, transport-backed config reads, or probe and remove lifecycle parity yet.

The next honest bounded step inside the same lane is to add one tiny multitouch slot-planning helper keyed off `ABS_MT_SLOT`, while still avoiding MMIO and broader transport glue.
