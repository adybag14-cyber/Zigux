...
- `scripts/zigux/validate-phase2.py` and `Documentation/zigux/phase2-closure.md` keep `zig test scripts/zigux/genksyms.zig` explicit as the bounded direct replay, while `.github/workflows/zigux-bootstrap.yml` keeps that replay in the shipped Phase 2 route indirectly by rerunning `python3 scripts/zigux/validate-phase2.py` instead of carrying a second standalone `zig test scripts/zigux/genksyms.zig` step. `scripts/zigux/README.md` and `zigux/tests/README.md` intentionally keep the broader reminder packet centered on the committed bridge fixtures and checker surfaces instead of duplicating that direct replay command in every shared note.
