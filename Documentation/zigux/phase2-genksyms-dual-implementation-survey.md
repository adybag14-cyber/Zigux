...
- The external `version_expected.json` and `version_then_missing_long_reference_argument_expected.json` packets now prove the same version-side-effect request and parse-failure behavior as the helper-local anchors already covered in `scripts/zigux/genksyms.zig`, so the helper-local and external bridge packets are aligned on both bounded version paths.
