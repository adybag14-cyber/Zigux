...
- `zigux/tests/fixtures/genksyms_bridge/manifest.json` is present, marks the tool `closed`, records `mode: "wrapper-first bridge"`, and names the current 24-case external bridge packet.
- `scripts/zigux/check-genksyms-bridge.py` now matches that 24-case packet, including the committed `missing_dump_types_argument` process case, the external `version` bridge case, and the new `version_then_missing_long_reference_argument` process case that prove the bounded `-V` side effect both before later request parsing and before a later missing-argument failure.
- `scripts/zigux/validate-phase2.py`, `Documentation/zigux/phase2-closure.md`, and `.github/workflows/zigux-bootstrap.yml` all keep `zig test scripts/zigux/genksyms.zig` explicit as the bounded direct replay on current `master`: the shared validator still owns the direct replay inside the broader Phase 2 packet, and the workflow now also carries a standalone `run: zig test scripts/zigux/genksyms.zig` step beside the dedicated `python3 scripts/zigux/check-genksyms-bridge.py --self-test` and `python3 scripts/zigux/check-genksyms-bridge.py` bridge checks. `scripts/zigux/README.md` and `zigux/tests/README.md` intentionally keep the broader reminder packet centered on the committed bridge fixtures and checker surfaces instead of duplicating that direct replay command in every shared note.
- `Documentation/zigux/phase2-closure.md` keeps the same live `24-case` genksyms bridge packet explicit, so the earlier closure-note undercount that motivated the first follow-through is no longer present on current `master`.
- The external `version_expected.json` and `version_then_missing_long_reference_argument_expected.json` packets now prove the same version-side-effect request and parse-failure behavior as the helper-local anchors already covered in `scripts/zigux/genksyms.zig`, including the direct `genksyms bridge preserves version side effects before later parse failures` unit anchor, so the helper-local and external bridge packets are aligned on both bounded version paths.

## Survey result

- Current `master` does not have a remaining roadmap gap at the level of genksyms wrapper scaffolding, version-side-effect proof, closure-note count, or bounded direct-replay ownership. The bridge entrypoint, committed 24-case fixture packet, manifest, checker, helper-local anchors, direct `zig test scripts/zigux/genksyms.zig` route, and checked reminder surfaces are all present.
