...
Current `master` materializes the dedicated devres slice note, the dedicated survey note, the helper-local `lib/devres.zig` starter, the manifest-backed packet, the direct devres replay, the direct reviewability replay, the adjacent coherent-DMA boundary replay, the dedicated packet-alignment checker, and the stable validator-first make routes for the `lib/devres.c` roadmap anchor. If older direct companions such as `zigux/tests/phase13_build.zig` or `zigux/tests/phase13_devres_boundary_evidence.zig` cannot be materialized on current `master`, record them as repo-reality gaps rather than presenting them here as shipped repo evidence.
