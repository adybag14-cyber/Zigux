...
Current `master` materializes the dedicated devres slice note, the dedicated survey note, the helper-local `lib/devres.zig` starter, the manifest-backed packet, the direct devres replay, the direct reviewability replay, the adjacent coherent-DMA boundary replay, the dedicated packet-alignment checker, and the stable validator-first make routes for the `lib/devres.c` roadmap anchor. If older direct companions such as `zigux/tests/phase13_build.zig`, `zigux/tests/phase13_devres_boundary_evidence.zig`, or `scripts/zigux/check-phase13-devres-packet.py` cannot be materialized on current `master`, record them as repo-reality gaps rather than presenting them here as shipped repo evidence.

That bounded `devres` packet still covers managed ioremap lifetime planning, exact `devm_iounmap()` pointer matching, pure `devm_ioremap_uc()` and `devm_ioremap_wc()` wrapper planning, translated-resource `devm_of_iomap()` handoff, and detach-time arch WC token bookkeeping without claiming live MMIO mappings, live device-tree walking, live arch memtype mutation, live scatterlist ownership, or `sg_table` lifecycle control. The dedicated devres notes, helper-local starter, manifest-backed packet, direct replays, and checker should stay visible here as roadmap-to-repo evidence, while the still-missing shared-build and boundary-evidence companions stay recorded as repo reality until current `master` materializes them again; the adjacent coherent-DMA shard still supports the shared Phase 13 packet rather than creating an extra replay step or a closure claim.
