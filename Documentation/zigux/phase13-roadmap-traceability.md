...
- `landlock/syscalls` maps to the bounded shared-helper tranche and should keep its governance boundary explicit through the shipped `Documentation/zigux/phase13-landlock-syscalls-governance.md`, `Documentation/zigux/phase13-landlock-syscalls-survey.md`, and `security/landlock/syscalls.zig` helper starter.
...
Current `master` materializes the dedicated governance note, the dedicated survey note, the helper-local `security/landlock/syscalls.zig` starter, and the stable validator-first make routes for the `landlock/syscalls` lane. If direct companions such as `Documentation/zigux/phase13-landlock-syscalls-slice.md`, `zigux/tests/phase13_landlock_syscalls_manifest.json`, `zigux/tests/phase13_landlock_syscalls.zig`, `zigux/tests/phase13_landlock_syscalls_reviewability.zig`, or `zigux/tests/phase13_build.zig` cannot be materialized on current `master`, record them as repo-reality gaps rather than presenting them here as shipped repo evidence.
...
The dedicated governance note, the shipped survey note, and the shipped helper-local syscalls starter should stay visible here as roadmap-to-repo evidence, and any still-missing slice, manifest, direct-test, reviewability, or direct-build companions should stay recorded as repo reality until current `master` materializes them again, but they still support the shared Phase 13 packet rather than creating an extra replay step or a closure claim.
...
- `security/landlock/syscalls.zig` is the shipped helper-local `landlock/syscalls` starter on current `master`; together with `Documentation/zigux/phase13-landlock-syscalls-governance.md`, the shipped `Documentation/zigux/phase13-landlock-syscalls-survey.md` note, and the stable make routes, it keeps the syscall-facing roadmap anchor reviewable as one bounded helper-first packet while `Documentation/zigux/phase13-landlock-syscalls-slice.md`, `zigux/tests/phase13_landlock_syscalls_manifest.json`, `zigux/tests/phase13_landlock_syscalls.zig`, `zigux/tests/phase13_landlock_syscalls_reviewability.zig`, and `zigux/tests/phase13_build.zig` stay framed as repo-reality gaps rather than closure evidence.
