...
- `libfs` stays mapped through `Documentation/zigux/phase13-libfs-survey.md`, `Documentation/zigux/phase13-libfs-slice.md`, the shipped `fs/libfs.zig` starter, the direct `zigux/tests/phase13_libfs.zig` replay, the direct `zigux/tests/phase13_libfs_reviewability.zig` companion, and `zigux/tests/phase13_libfs_manifest.json`. Keep the helper-local `phase13-libfs-addressability-helper` recorded through the survey, starter, direct replay, and manifest markers rather than inventing a separate `zigux/tests/phase13_libfs_addressability.zig` owner lane, and keep the broader shared `zigux/tests/phase13_build.zig` route as the repo-reality gap.
