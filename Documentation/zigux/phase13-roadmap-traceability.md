...
- stable tests-root alignment guard: `zig run scripts/zigux/check_phase13_tests_readme_alignment.zig`
...
Keep `zigux/Makefile` distinct from the still-missing shared wrapper names `make -C zigux phase13-validate` and `make -C zigux phase13`, and keep `zig run scripts/zigux/check_phase13_roadmap_traceability.zig` explicit as the shipped note-level guard for this four-anchor owner map rather than leaving the roadmap traceability proof note-only.
