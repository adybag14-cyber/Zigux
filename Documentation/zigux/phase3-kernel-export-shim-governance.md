...
- `currentVersion()`, `versionMatchesCurrent()`, and `validateVersion()` keep the starter version relay and compatibility gate explicit through the shared `zigux/bindings/version.zig` surface, with `validateVersion()` now delegating to the UAPI-backed status-tagged validator instead of rebuilding that status locally.
