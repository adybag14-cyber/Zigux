...
- live `zigux/kernel/export_shim.zig` also exposes status-tagged `validateDeviceNumber()`, `validateDeviceRange()`, `encodeDeviceNumber()`, and `lastDeviceNumberInRange()` relays plus the pure `decodeDeviceNumber()` view over the shipped `zigux/uapi/dev_t.zig` starter companion without re-homing `dev_t` ownership into `zigux/kernel/`
