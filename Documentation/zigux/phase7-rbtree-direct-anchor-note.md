...
Keep the current Phase 7 rbtree reminder surface tied to the returned tool-root helper, the dedicated slice note, the dedicated replay companion, the returned survey and manifest, and the parity checker instead of claiming the roadmap-path port, dedicated fixture pair, shared build file, or shared validator as returned.
...
- Do not widen this note into make-wrapper or workflow-recovery claims until a fresh same-lane reread proves one more concrete rbtree companion surface such as `lib/rbtree.zig`, `zigux/tests/fixtures/phase7_rbtree.json`, `zigux/tests/fixtures/phase7_rbtree_c_harness.c`, `zigux/tests/phase7_build.zig`, or `scripts/zigux/validate-phase7.py` has rematerialized on current `master`.