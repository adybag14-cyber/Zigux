...
Current `master` still carries Phase 15 reminder language in `Documentation/zigux/README.md`, `zigux/tests/README.md`, and the shipped `scripts\zigux/check_phase15_scripts_readme_alignment.zig` checker. But the scripts root itself has not caught up: `scripts/zigux/README.md` still stops at earlier phases, while the broader Phase 15 validator-and-build packet the checker expects is not fully present on current `master`.
...
- `Documentation/zigux/README.md` still names `scripts\zigux/check_phase15_scripts_readme_alignment.zig` and the `make -C zigux phase15-*` routes in its Phase 15 summary
- `zigux/tests/README.md` still names the same scripts-root checker and Phase 15 routes in the tests-root review packet
- `scripts\zigux/check_phase15_scripts_readme_alignment.zig` still hard-codes `Phase 15 flow`, `make -C zigux phase15-validate`, `make -C zigux phase15-test`, `make -C zigux phase15`, `scripts\zigux/validate_phase15.zig`, `zigux/tests/phase15_build.zig`, and `zigux/tests/phase15_architecture_council_review_process_manifest.json` as required current-repo markers
...
- do not assume `scripts\zigux/check_phase15_scripts_readme_alignment.zig` represents a fully materialized validator-first route
...
- a rebuilt `scripts\zigux/validate_phase15.zig`
...
- or narrow `scripts\zigux/check_phase15_scripts_readme_alignment.zig` so it only enforces the Phase 15 surfaces that actually exist on current `master`
