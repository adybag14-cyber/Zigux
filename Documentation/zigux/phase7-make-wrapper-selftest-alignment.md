...
Fresh repo inspection now shows the older docs-root and tests-root follow-through is already complete: `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, and the dedicated alignment checker already keep this note explicit inside the parked Phase 7 packet. If another same-lane truthfulness repair is needed, prefer the smaller scripts-root follow-through: inspect whether `scripts/zigux/README.md` should also keep the full `make -C zigux phase7` route explicit beside `make -C zigux phase7-validate`, rather than reopening helper code, fixtures, or any wider Phase 7 surface.
