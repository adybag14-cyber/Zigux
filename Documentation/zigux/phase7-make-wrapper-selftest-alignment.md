...
- current `master` keeps `scripts\zigux/check_phase7_make_wrapper_selftest_alignment.zig` as the dedicated checker for the parked Phase 7 make-wrapper posture
- current `master` keeps `scripts\zigux/check_phase7_shared_control_gap.zig` as the broader shared-control checker that already guards the parked-path contract across the sequencing note, string-helpers slice, workflow, readable non-owner files, and direct shared-control packet
...
- current `master` still does not materialize `scripts\zigux/check_phase7_make_wrapper.zig`, `scripts\zigux/validate_phase7.zig`, or `zigux/tests/phase7_build.zig`, so treat those older wrapper, validator, and shared-build paths as parked reminder vocabulary rather than direct current-tree proof
...
If a future reread shows new Phase 7 shared-control drift, keep the next step inside `scripts\zigux/check_phase7_shared_control_gap.zig`, `scripts\zigux/check_phase7_make_wrapper_selftest_alignment.zig`, `.github/workflows/zigux-bootstrap.yml`, `Documentation/zigux/phase7-helper-lane-sequencing.md`, or this note before widening into helper-local packets or trying to resurrect the older `phase7-*` wrapper stack.
