...
- the published `ARTIFACT_DIFF_CONTRACT_BASE_CASE_COUNT=23`, `ARTIFACT_DIFF_CONTRACT_REPEAT_CASE_COUNT=4`, and `ARTIFACT_DIFF_CONTRACT_CASE_COUNT=27` outputs keep stale expected-output and catalog drift small, auditable, and easy to refresh, with the help surface and parser failure shapes now called out explicitly alongside the malformed JSON failure markers named through `EXPECTED_JSON_ERROR=` and `ACTUAL_JSON_ERROR=` and the binary both-missing `EXPECTED_EXISTS=False` plus `ACTUAL_EXISTS=False` fail shape.
