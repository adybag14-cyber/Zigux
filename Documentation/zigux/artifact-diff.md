...
- `scripts/zigux/check-phase4-remaining-gap-matrix.py` and `scripts/zigux/check-phase4-workflow-route-counts.py` keep the parked `kprobe_example`, `test_fsmount`, and local-only perf-promotion reminder rows plus the shipped Linux-style wrapper inventory aligned beside the broader validator-backed rollback surface without turning this note into the full ownership record.
...
- `zigux/tests/phase4_perf_baseline_manifest.json` and `zigux/tests/phase4_perf_baseline_survey.zig` keep the approved local benchmark commands and approved local-only acceptable limits for both shipped rollback gates reviewable through `zig build phase4-perf-baseline-survey --build-file zigux/tests/phase4_build.zig` and `make -C zigux phase4-perf-baseline-survey` without promoting shared CI perf thresholds.
- `scripts/zigux/validate-phase4.py` keeps the shared Phase 4 validator packet, workflow wiring, the artifact-diff contract checker, the artifact-diff determinism checker, the dedicated remaining-gap matrix checker, the dedicated workflow-route-count checker, the dedicated local perf-baseline survey packet, and the Phase 4 documentation markers aligned before the Zig tests run.
