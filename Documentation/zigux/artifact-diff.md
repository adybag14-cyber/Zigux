...
- `zigux/tests/fixtures/phase3_chrdev_notify_ack_window_policy_budget_windowDelivery_window_budget_window/expected.json` anchors the bounded Phase 3 chrdev notify ack window policy budget windowDelivery_window_budget_window parity claim.
- `python3 scripts/zigux/run-phase3-checks.py --slug chrdev-notify-ack-window-policy-budget-window-delivery_window_budget_window` compares that committed JSON fixture against both the bounded C harness and the Zig chrdev notify ack window policy budget window delivery_window_budget_window dump.
