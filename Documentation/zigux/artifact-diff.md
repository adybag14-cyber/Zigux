...
- `python3 scripts/zigux/check-artifact-diff-contract.py` keeps one stable text pass case, both missing-file failure shapes, the direct JSON mismatch replay, malformed expected and actual JSON failure shapes, and SHA-256 pass and drift cases as part of the reviewable CLI contract, so the outward `ARTIFACT_DIFF=...`, `MODE=...`, `EXPECTED_EXISTS=...`, `ACTUAL_EXISTS=...`, `EXPECTED_JSON_ERROR=...`, `ACTUAL_JSON_ERROR=...`, `SHA256=...`, `EXPECTED_SHA256=...`, and `ACTUAL_SHA256=...` fields stay small, auditable, and easy to refresh. This keeps stale expected-output and catalog drift small, auditable, and easy to refresh, and it keeps the explicit `EXPECTED_JSON_ERROR=` and `ACTUAL_JSON_ERROR=` contract markers reviewable beside the published SHA-256 fields.
