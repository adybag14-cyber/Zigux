...
- `python3 scripts/zigux/check-artifact-diff-contract.py` replays one stable pass case plus one missing-file failure shape for the outward `ARTIFACT_DIFF=...` CLI lines, so the shared command-line contract does not live only inside `artifact_diff.py --self-test`.
...
- `scripts/zigux/validate-phase4.py` keeps that shared self-test, the external CLI-contract replay, the Phase 4 build entrypoint, workflow wiring, and Phase 4 documentation markers aligned before the Zig tests run.
- `Documentation/zigux/phase4-validation-matrix.md` records the current rollback owners, threshold posture, host-tool replay matrix, and the reversible-delivery evidence that ties each shipped gate back to its current C anchor if the shared Phase 4 entrypoint has to drop that Zig gate.
