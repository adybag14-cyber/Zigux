...
Current Phase 3 use
- `zigux/tests/fixtures/phase3_*/expected.json` fixtures capture bounded parity claims for the discovered Phase 3 interop slices.
- `python3 scripts/zigux/run-phase3-checks.py --slug <slice>` compares each committed Phase 3 fixture against the bounded C harness and the matching Zig dump.
- `python3 scripts/zigux/phase3_catalog.py --rewrite-artifact-diff-phase3-section` can regenerate the detailed Phase 3 policy block from the discovered slice catalog when the tranche grows.
