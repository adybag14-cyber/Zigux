...
- the published `ARTIFACT_DIFF_CONTRACT_BASE_CASE_COUNT=19`, `ARTIFACT_DIFF_CONTRACT_REPEAT_CASE_COUNT=4`, and `ARTIFACT_DIFF_CONTRACT_CASE_COUNT=23` outputs keep stale expected-output and catalog drift small, auditable, and easy to refresh, with the malformed JSON failure shapes still named directly through `EXPECTED_JSON_ERROR=` and `ACTUAL_JSON_ERROR=` and the binary both-missing `EXPECTED_EXISTS=False` plus `ACTUAL_EXISTS=False` fail shape now called out explicitly instead of being left implicit inside the broader contract summary.
