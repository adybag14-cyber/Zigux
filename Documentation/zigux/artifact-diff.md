...
- `zigux/tests/fixtures/phase3_chrdev_notify_ack_delivery_budget_guard_window_policy_budget_window_DELIVERY_WINDOW/expected.json` anchors the bounded Phase 3 chrdev notify ack delivery budget guard window policy budget window delivery window budget window delivery window parity claim.
- `python3 scripts/zigux/run-phase3-checks.py --slug chrdev-notify-ack-delivery-budget-guard-window-policy-budget-window-delivery-window-budget-window-delivery-window-budget-window-delivery` compares that committed JSON fixture against both the bounded C harness and the Zig chrdev notify ack delivery budget guard window policy budget window delivery window budget window delivery window budget window delivery dump.
- `zigux/tests/fixtures/phase3_chrdev_notify_ack_delivery_window_budget_window_delivery_window_budget/expected.json` anchors the bounded Phase 3 chrdev notify ack delivery budget window delivery window budget parity claim.
- `python3 scripts/zigux/run-phase3-checks.py --slug chrdev-notify-ack-delivery-window-budget-window-delivery-window-budget` compares that committed JSON fixture against both the bounded C harness and the Zig chrdev notify ack delivery window budget window delivery window budget dump.
...
- `python3 scripts/zigux/check-artifact-diff-contract.py` keeps one stable text pass case, both missing-file failure shapes, the direct JSON mismatch replay, malformed expected and actual JSON failure shapes, and SHA-256 pass and drift cases as part of the reviewable CLI contract, so the outward `ARTIFACT_DIFF=...`, `MODE=...`, `EXPECTED_EXISTS=...`, `ACTUAL_EXISTS=...`, `EXPECTED_JSON_ERROR=...`, `ACTUAL_JSON_ERROR=...`, `SHA256=...`, `EXPECTED_SHA256=...`, and `ACTUAL_SHA256=...` fields stay small, auditable, and easy to refresh.
- that exact contract evidence keeps stale expected-output and catalog drift small, auditable, and easy to refresh, with the malformed JSON failure shapes still named directly through `EXPECTED_JSON_ERROR=` and `ACTUAL_JSON_ERROR=` instead of only being implied by the broader contract summary.
