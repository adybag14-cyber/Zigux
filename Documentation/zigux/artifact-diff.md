...
- `python3 scripts/zigux/check-artifact-diff-contract.py` keeps one stable text pass case, both missing-file text and JSON failure shapes, the explicit binary `sha256_missing_both` failure path, the direct JSON mismatch replay, malformed expected and actual JSON failure shapes, and SHA-256 pass and drift cases as part of the reviewable CLI contract, so the outward `ARTIFACT_DIFF=...`, `MODE=...`, `EXPECTED_EXISTS=...`, `ACTUAL_EXISTS=...`, `EXPECTED_JSON_ERROR=...`, `ACTUAL_JSON_ERROR=...`, `SHA256=...`, `EXPECTED_SHA256=...`, and `ACTUAL_SHA256=...` fields stay small, auditable, and easy to refresh.
- that same contract replay now reruns the stable text pass, direct JSON mismatch, and SHA-256 drift cases back to back, so repeat-run CLI output stays reviewable instead of being inferred from one-shot cases alone.
- that exact contract evidence keeps stale expected-output and catalog drift small, auditable, and easy to refresh, with the malformed JSON failure shapes still named directly through `EXPECTED_JSON_ERROR=` and `ACTUAL_JSON_ERROR=` and the binary both-missing `EXPECTED_EXISTS=False` plus `ACTUAL_EXISTS=False` fail shape now called out explicitly instead of being left implicit inside the broader contract summary.
