...
- `python3 scripts/zigux/run-phase3-checks.py --slug chrdev-notify-ack-delivery-budget-guard-window-policy-budget-window-delivery-window-budget`that committed JSON fixture against both the bounded C harness and the Zig chrdev notify ack delivery budget guard window policy budget window delivery window budget dump.
...
- `zigux/tests/atomic64_diff.zig` keeps the roadmap-facing Phase 4 atomic64 entrypoint published while `zigux/tests/runtime_atomic64_diff.zig` remains the single bounded replay body for the current `lib/atomic64_test.c` anchor inside `zigux/tests/phase4_build.zig`.
