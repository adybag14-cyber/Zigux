...
Current Phase 4 use
- `python3 scripts/zigux/artifact_diff.py --self-test` keeps the shared comparison layer that already backs the bounded host-side tools under `scripts/zigux/` deterministic before the rollback-readiness lanes run.
- `python3 scripts/zigux/check-artifact-diff-contract.py` replays one stable pass case plus one missing-file case so the published `ARTIFACT_DIFF=...`, `MODE=...`, and exit-code surface stay reviewable outside the helper's built-in self-test.
- `Documentation/zigux/artifact-diff.md`, `Documentation/zigux/phase4-validation-matrix.md`, and `scripts/zigux/validate-phase4.py` keep the current Phase 4 use, current C anchor, reversible-delivery evidence, and shared Phase 4 entrypoint aligned in one bounded documentation packet.
- `zigux/tests/runtime_atomic64_diff.zig` keeps `lib/atomic64_test.c` as the current C anchor while the bounded replay gate stays wired through the shared Phase 4 entrypoint in `zigux/tests/phase4_build.zig`.
- `zigux/tests/bitmap_diff.zig` keeps `lib/test_bitmap.c` as the current C anchor while the bounded replay gate stays wired through the shared Phase 4 entrypoint in `zigux/tests/phase4_build.zig`.
- `python3 scripts/zigux/check-artifact-diff-contract.py` now makes `EXPECTED_JSON_ERROR=` and `ACTUAL_JSON_ERROR=` part of the reviewable CLI contract, which keeps stale expected-output and catalog drift small, auditable, and easy to refresh.
- The reversible-delivery evidence for the two shipped Phase 4 gates stays in `Documentation/zigux/phase4-validation-matrix.md`, which names the rollback owner, the current C anchor, and the shared Phase 4 entrypoint before the shared `phase4-test` bundle can claim the packet is still aligned.
