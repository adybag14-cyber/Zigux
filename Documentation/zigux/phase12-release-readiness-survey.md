...
The earlier checker-side freeze-boundary follow-through is now closed too: `scripts/zigux/check-build-only-phase12-surface.py` already requires the docs-root Phase 12 summary to keep the freeze-boundary reminder explicit, so this lane no longer needs to steer future runs back through that same checker-local guard.
