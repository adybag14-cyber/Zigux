...
Start that reread by confirming that the docs-root shared Phase 12 summary in `Documentation/zigux/README.md` still keeps `Documentation/zigux/phase12-virtio-scsi-raw-github-fallback-catalog.md` explicit and still keeps the canonical deterministic replay path at `zigux/tests/phase12_libbpf_snapshot_determinism.zig` instead of drifting that replay into `zigux/tests/fixtures/`.
