...
If this `pmo-release` lane reopens, the next honest follow-up is to promote the already-landed PMO packet surfaces into the shared validator so `python3 scripts/zigux/validate-phase12.py` fails closed on the same release-facing bundle that the docs now publish. That promotion should require `scripts/zigux/check-phase12-release-readiness-packet.py`, `Documentation/zigux/phase12-release-readiness-survey.md`, the Phase 12 docs-root release summary in `Documentation/zigux/README.md`, and the dedicated Phase 12 PMO checklist question in `Documentation/zigux/review-checklist.md` exactly once, while keeping the current active-not-closed posture, the approved three-target musl smoke set, the mixed two commit-pinned versus two shared-tree-only fallback split, and the existing tests-root PMO handoff markers intact. Keep that work bounded to the shared validator surface and adjacent release-facing marker counts without reopening DMA transport, queue ownership, throughput, recovery, object-model, loader, relocation, helper behavior, or direct driver behavior.