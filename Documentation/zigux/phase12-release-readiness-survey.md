...
If this `pmo-release` lane reopens, the next honest follow-up is to align `zigux/tests/README.md` with the same Phase 12 release-readiness and raw-fallback checker stack already named by `Documentation/zigux/phase12-release-readiness-survey.md`, `Documentation/zigux/phase12-shared-replay-contract.md`, `scripts/zigux/README.md`, and `scripts/zigux/check-phase12-release-readiness-packet.py`. That keeps the tests-root contributor guidance in step with the live focused replay, raw-coverage, and release-packet contract without reopening DMA transport, queue ownership, throughput, recovery, object-model, loader, relocation, helper behavior, or direct driver behavior.
