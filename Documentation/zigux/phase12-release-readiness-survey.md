...
If this PMO lane reopens, the next honest follow-up is to tighten `scripts/zigux/check-phase12-release-readiness-packet.py` so it also fail-closes on the raw-fallback coverage packet the survey already treats as release evidence: `scripts/zigux/check-phase12-raw-github-coverage.py`, `zigux/tests/phase12_raw_github_coverage_manifest.json`, and `zigux/tests/phase12_raw_github_coverage_survey.zig`. That keeps the mixed public-read fallback split reviewable inside the same dedicated release packet without reopening DMA transport, queue ownership, throughput, recovery, object-model, loader, relocation, helper behavior, or direct driver behavior.
