...
If this `pmo-release` lane reopens, the next honest follow-up is to promote `scripts/zigux/check-phase12-release-readiness-packet.py` and `Documentation/zigux/phase12-release-readiness-survey.md` into the shared `scripts/zigux/validate-phase12.py` surface so the validator itself names the dedicated PMO packet guard, the release-facing survey note, and the adjacent scripts-root, docs-root, and checklist release stack together. Keep that work bounded to the saved validator-publication bundle for `scripts/zigux/validate-phase12.py` once a safe whole-file materialization path is available, without reopening DMA transport, queue ownership, throughput, recovery, object-model, loader, relocation, helper behavior, or direct driver behavior.
