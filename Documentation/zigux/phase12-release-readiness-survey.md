...
- the dedicated Phase 12 PMO checklist question is still the next bounded review-surface follow-up, so the checklist does not yet restate the active-not-closed release posture, the approved three-target musl smoke set, and the current two commit-pinned versus two shared-tree-only fallback split in one release-facing prompt
