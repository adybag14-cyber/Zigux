...
If this `pmo-release` lane reopens, the next honest follow-up is to tighten `scripts/zigux/check-phase12-release-readiness-packet.py` around the raw-fallback coverage packet so `Documentation/zigux/phase12-raw-github-coverage-survey.md`, `scripts/zigux/check-phase12-raw-github-coverage.py`, `zigux/tests/phase12_raw_github_coverage_manifest.json`, and `zigux/tests/phase12_raw_github_coverage_survey.zig` stay aligned with the current two commit-pinned versus two shared-tree-only split. Keep that work bounded to the dedicated release-packet checker and adjacent raw-fallback packet surfaces without reopening DMA transport, queue ownership, throughput, recovery, object-model, loader, relocation, helper behavior, or direct driver behavior.
