...
- direct `perf_event_open()` setup, perf-buffer ring `mmap()` setup, `ioctl(PERF_EVENT_IOC_ENABLE)` enablement, per-CPU perf-event-array updates, or epoll-backed perf FD registration
