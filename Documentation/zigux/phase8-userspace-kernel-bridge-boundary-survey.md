...
- `tools/lib/subcmd/exec-cmd.zig` currently exposes the bounded `ExtractArgv0Result.command_name` field together with `Config.exec_path_env`, `buildSearchPath()`, and the pure `execl_cmd()`-style argument collectors, so the landed command packet stays at path-resolution, injected-environment setup, cwd-choice modeling, and null-terminated argv preparation rather than direct launch behavior
- `tools/lib/subcmd/help.zig` currently exposes `resolveTerminalDimensions()` beside owned command-name handling, injected command-source filtering, raw `PATH` splitting, and pure section-render planning or emission, so the parked command packet stays output-stable without claiming direct terminal io
...
- `tools/lib/bpf/zigux_segments/file_path_handle_bridge.zig` currently exposes `buildCurrentProcessFdinfoPath()`, `chooseReusedMapName()`, `planTokenPreparation()`, and `classifyTokenPreparationFailure()` beside bounded fdinfo text parsing from `bpf_get_map_info_from_fdinfo()`, so the landed helper packet stops at current-process path construction, map-name reuse decisions, and token-preparation planning instead of live procfs or bpffs io
