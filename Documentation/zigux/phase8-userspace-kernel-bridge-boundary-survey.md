...
- scope: helper-first review of the current file-path-plus-fdinfo bridge packet plus the landed adjacent perf-buffer poll helper packet and the still-queued adjacent bridge steps only
- product boundary:
...
The live Phase 8 packet already carries a bounded file-path-and-fdinfo helper slice, but the adjacent bridge boundary was still only implicit across the file-path helper note and the broader libbpf segment survey. This survey keeps that boundary explicit so the validator-first Phase 8 packet can describe the shipped helpers and the queued follow-through without implying procfs, bpffs, object-model, or deferred interrupt-routing closure.

## Roadmap alignment

This survey stays attached to the Phase 8 roadmap anchor at `tools/lib/bpf/libbpf.c` and its recommended segmented Zigux destination under `tools/lib/bpf/zigux_segments/`.

It does not own the separate command-side tooling packet around `tools/lib/subcmd/exec-cmd.c` or `tools/lib/subcmd/help.c`; those remain parked under the dedicated `exec-cmd` and `help` Phase 8 slices.
...
- `tools/lib/bpf/zigux_segments/file_path_handle_bridge.zig` keeps exact `"/proc/%d/fdinfo/%d"` plus adjacent `"/proc/%d/fd/%d"` assembly, bounded fdinfo map-info parsing, explicit decimal or octal or hex numeric handling, compact completion summaries, the helper-only `mapReuseObservationFromFdinfo()` handoff from parsed fdinfo fields into reuse planning, planning-only reuse-pinned-map attempt gating, the planning-only `planTokenPreparation()` gate, and planning-only `planTokenBridgePath()` plus `resolveTokenBridgeAttempt()` summaries reviewable
- the landed reuse-planning helper requires a non-empty pinned path plus compatible fdinfo-derived map info before any real reopen attempt can even be considered, the landed token-preparation helper requires a non-empty token path plus a ready reused-map bridge plan before any future token open attempt can be considered, and the landed token bridge helper keeps an optional-or-mandatory token bridge path mode plus planning-only token bridge attempt outcomes explicit before any future bpffs open or token-create side effects can be considered
- `zigux/tests/phase8_file_path_handle_bridge.zig` keeps the helper packet wired to stable path, ignored-line, repeated-field, numeric-base, summary, planning-only reopen-attempt, planning-only token-preparation, and planning-only token bridge path and attempt expectations
...
- `planTokenPreparation()` now stays inside the shipped helper packet as a planning-only gate: it only checks for a non-empty token path plus a ready reused-map bridge plan before any future token open attempt is allowed to proceed
- `planTokenBridgePath()` now stays inside the shipped helper packet as a planning-only bridge-path gate: it only selects a prevented, optional, or mandatory token bridge mode before any future bpffs open attempt is allowed to proceed
- `resolveTokenBridgeAttempt()` now stays inside the shipped helper packet as a planning-only bridge-attempt summary: it only classifies bounded open and create results before any future token install path is allowed to proceed
...
- token materialization or capability handoff and map reopen or bpffs compatibility closure still remain queued follow-through beyond the current planning-only gates
...
- deferred `execv_cmd()` or `execl_cmd()` command-boundary work from the separate Phase 8 command packet
