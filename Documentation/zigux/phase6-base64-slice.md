...
Leave the base64 helper lane parked unless fresh repo inspection finds a concrete parity, perf, or directly coupled review-packet gap inside `lib/base64.zig`, `zigux/tests/phase6_base64.zig`, `zigux/tests/phase6_base64_perf.zig`, `zigux/tests/phase6_base64_c_parity.zig`, `zigux/tests/phase6_base64_c_casegen.zig`, `zigux/tests/fixtures/phase6_base64_vectors.zig`, `zigux/tests/fixtures/phase6_base64_c_harness.c`, `scripts/zigux/check-phase6-base64-c-parity.py`, or the shared Phase 6 packet. Current repo evidence shows the helper-local base64 packet itself is aligned at `PHASE6_BASE64_C_PARITY_CASES=122` across the parity script, shared catalog, machine-readable manifest, and the live `scripts/zigux/validate-phase6.py` markers, so the next same-family follow-up should stay narrow to future shared-packet drift rather than reopening `lib/base64.zig`, its directly coupled tests, or claiming the validator still expects the older `112` count.
