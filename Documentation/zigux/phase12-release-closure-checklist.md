...
- the smallest same-lane follow-through is still to add `Documentation/zigux/phase12-release-closure-checklist.md` to the Phase 12 shared-surface summary in `zigux/tests/README.md` so the closure companion stops being only partly visible from the tests-root packet before the checker gap is widened.
- after that tests-root sync lands, extend `scripts/zigux/check-build-only-phase12-surface.py` so the closure companion is fail-closed against scripts-root drift too.
...
- Any future PMO follow-through should therefore start with that tests-root sync, then rerun `scripts/zigux/check-build-only-phase12-surface.py` and reread `zigux/tests/README.md` before widening into checker work, new wording, or closure claims.
