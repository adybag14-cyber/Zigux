...
- The active shared build packet on current `master` is the four-file `virtio_net` follow-up quartet wired through `zigux/tests/phase12_build.zig`: `zigux/tests/phase12_virtio_net_queue_resume.zig`, `zigux/tests/phase12_virtio_net_transmit_recycle.zig`, `zigux/tests/phase12_virtio_net_post_reset_replay.zig`, and `zigux/tests/phase12_virtio_net_throughput_parity.zig`.
- The broader starter-present `virtio_net` direct and syntax-lab packet, the driver-local `virtio_scsi` smoke-first plus rollback-lab packet, and the bounded driver-local `nvme_pci` foothold stay explicit as adjacent review surfaces outside the shared `smoke` and `test` build route.
...
- `virtio_scsi` remains a smoke-first and rollback-lab packet outside the shared `phase12` smoke-and-test route, not a deeper runtime-delivery or transport-complete packet.
