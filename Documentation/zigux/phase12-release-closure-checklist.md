...
## Active release blocker
  ...
  * The broader scripts-root blocker is still open on current `master`: `scripts/zigux/README.md` still omits `Documentation/zigux/phase12-libbpf-verify-shard-note.md` from its shared Phase 12 packet while also naming absent `Documentation/zigux/phase12-nvme-pci-slice.md` and `Documentation/zigux/phase12-nvme-pci-survey.md`, so the next honest same-lane follow-through remains a one-file scripts-root reread before PMO wording widens again.
  * Live GitHub contents readback on 2026-05-12 confirms that this closure companion is still aligned with the same smoke-first `virtio_scsi` packet as `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-readiness-survey.md`, `Documentation/zigux/phase12-release-coordination-matrix.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md`, even though the scripts-root reminder itself still needs that narrower same-lane repair.
