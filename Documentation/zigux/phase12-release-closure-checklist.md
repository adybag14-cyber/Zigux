...
## Active release blocker
  ...
  * Current `master` also already ships the dedicated libbpf verify shard: `Documentation/zigux/phase12-release-readiness-survey.md` and `Documentation/zigux/phase12-libbpf-heavy-consumer-lane-sequencing.md` keep `tools/lib/bpf/zigux_segments/verify.zig` explicit inside the active reviewability bundle, so this closure companion should keep that shard visible too instead of reducing the release-facing libbpf packet to manifest-only wording.
