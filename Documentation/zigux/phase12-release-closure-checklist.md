...
## Active release blocker
  ...
  * The broader shared-summary blocker is still open on current `master`: `scripts/zigux/README.md` still omits `Documentation/zigux/phase12-libbpf-verify-shard-note.md` from its shared Phase 12 packet while also naming absent `Documentation/zigux/phase12-nvme-pci-slice.md` and `Documentation/zigux/phase12-nvme-pci-survey.md`, and both `Documentation/zigux/phase12-release-coordination-matrix.md` plus `Documentation/zigux/review-checklist.md` still name that same absent `nvme_pci` slice-and-survey pair inside their shared Phase 12 summary bullets, so the next honest same-lane follow-through remains one narrow shared-summary reread at a time.
  * Live GitHub contents readback on 2026-05-12 confirms that this closure companion is aligned with the same smoke-first `virtio_scsi` packet as `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-readiness-survey.md`, and `zigux/tests/README.md`, while the scripts-root summary, compact release-coordination matrix, and shared review checklist still need narrower same-lane repo-reality rereads around the absent `Documentation/zigux/phase12-nvme-pci-{slice,survey}.md` pair and the missing verify-shard companion mention.
