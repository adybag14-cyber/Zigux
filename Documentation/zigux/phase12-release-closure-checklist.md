...
## Active release blocker
  ...
  * The live PMO job is now narrowed to the tests-root truthfulness sync: current `master` still returns 404s for `zigux/tests/phase12_nvme_pci.zig`, `zigux/tests/phase12_nvme_pci_manifest.json`, `zigux/tests/phase12_virtio_net.zig`, `zigux/tests/phase12_virtio_net_syntax_lab.zig`, `zigux/tests/phase12_libbpf_segments.zig`, and `tools/lib/bpf/zigux_segments/verify.zig`, while `zigux/tests/README.md` still presents those absent direct replay families as shipped release-packet surfaces, so same-lane PMO follow-through should repair that tests-root reminder before reopening broader docs-root, scripts-root, or fallback wording.
  * Live GitHub contents readback on 2026-05-12 confirms that tests-root blocker is still open: current `master` still returns 404s for `zigux/tests/phase12_nvme_pci.zig`, `zigux/tests/phase12_nvme_pci_manifest.json`, `zigux/tests/phase12_virtio_net.zig`, `zigux/tests/phase12_virtio_net_syntax_lab.zig`, `zigux/tests/phase12_libbpf_segments.zig`, and `tools/lib/bpf/zigux_segments/verify.zig`, so shared PMO follow-through should continue to treat the tests-root Phase 12 overclaim as active until `zigux/tests/README.md` stops presenting those absent paths as shipped release-packet surfaces.
