...
## Active release blocker
  ...
  * The live PMO job is now narrowed to the scripts-root truthfulness sync: current `master` still returns 404s for `zigux/tests/phase12_nvme_pci.zig`, `zigux/tests/phase12_virtio_net.zig`, `zigux/tests/phase12_libbpf_segments.zig`, and `tools/lib/bpf/zigux_segments/verify.zig`, while `zigux/tests/README.md` now keeps only the shared smoke-first packet and parked Phase 12 companion notes explicit, so same-lane PMO follow-through should repair `scripts/zigux/README.md` before reopening broader docs-root, fallback, or driver-local wording.
  * Live GitHub contents readback on 2026-05-12 confirms that scripts-root blocker is still open: current `master` still returns 404s for `zigux/tests/phase12_nvme_pci.zig`, `zigux/tests/phase12_virtio_net.zig`, `zigux/tests/phase12_libbpf_segments.zig`, and `tools/lib/bpf/zigux_segments/verify.zig`, while `scripts/zigux/README.md` still presents those absent direct replay families as shipped release-packet surfaces, so shared PMO follow-through should continue to treat the scripts-root Phase 12 overclaim as active until that README stops presenting those absent paths as shipped current-`master` evidence.
