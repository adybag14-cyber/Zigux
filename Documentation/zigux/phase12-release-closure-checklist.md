...
## Active release blocker
  ...
  * Current `master` still leaves the tests-root summary thinner than that shipped four-anchor packet: `zigux/tests/README.md` keeps `zigux/tests/phase12_virtio_net_manifest.json` visible, but it still omits `zigux/tests/phase12_nvme_pci_manifest.json`, `zigux/tests/phase12_virtio_scsi_manifest.json`, and `zigux/tests/phase12_libbpf_manifest.json`, so that tests-root undercount is the smallest same-lane reminder drift to close before any broader Phase 12 wording work.
