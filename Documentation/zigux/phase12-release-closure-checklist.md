...
## Active release blocker
  ...
  * The older shared-summary drift is now closed on current `master`: `scripts/zigux/README.md` and `Documentation/zigux/review-checklist.md` already keep `Documentation/zigux/phase12-libbpf-verify-shard-note.md` explicit, keep the unpublished `Documentation/zigux/phase12-nvme-pci-slice.md` plus `Documentation/zigux/phase12-nvme-pci-survey.md` pair framed as absent, keep the shipped-but-unwired `scripts/zigux/validate-phase12.py` helper explicit, and keep the starter-present direct `virtio_net` packet visible without promoting any direct `phase12_nvme_pci` or `phase12_libbpf_*` replay files into the shipped packet. `P12-Y07` is therefore parked again until the shared Phase 12 packet changes instead of waiting on another review-checklist reread.
  * Live GitHub contents readback on 2026-05-13 confirms that this closure companion is aligned with the same starter-present `virtio_net` plus smoke-first `virtio_scsi` packet as `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-readiness-survey.md`, and `Documentation/zigux/phase12-release-coordination-matrix.md`, while `scripts/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and `zigux/tests/README.md` already keep the parked verify-shard note, the unpublished NVMe doc-pair boundary, the shipped-but-unwired `scripts/zigux/validate-phase12.py` helper, and the starter-present direct `virtio_net` packet explicit. No additional shared-summary reread remains active under `P12-Y07` unless the shipped packet moves again.
  ...
  * That means the remaining blocker is release status itself, not an already-landed reminder sync.
