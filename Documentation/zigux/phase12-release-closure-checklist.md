...
## Active release blocker
  ...
  * Live GitHub contents readback on 2026-05-11 shows the older tests-root closure claim is not actually closed yet: current `master` still returns 404s for `zigux/tests/phase12_nvme_pci.zig`, `zigux/tests/phase12_nvme_pci_manifest.json`, `zigux/tests/phase12_virtio_net.zig`, `zigux/tests/phase12_virtio_net_syntax_lab.zig`, `zigux/tests/phase12_libbpf_segments.zig`, and `tools/lib/bpf/zigux_segments/verify.zig`, so shared PMO follow-through should treat the tests-root Phase 12 overclaim as open again until `zigux/tests/README.md` stops presenting those absent paths as shipped release-packet surfaces.
