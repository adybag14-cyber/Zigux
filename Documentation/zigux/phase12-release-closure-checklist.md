...
## Active release blocker
  ...
  * the smallest same-lane follow-through is now one complex-driver owner-map truthfulness sync rather than another shared README reminder.
  * The live PMO job is to refresh `Documentation/zigux/phase12-complex-driver-lane-sequencing.md` so its future reread set explicitly includes `Documentation/zigux/phase12-release-sequencing.md` beside the already-shipped closure, readiness, coordination, fallback, verify-shard, scripts-root, and tests-root companions; until that lands, keep the broader shared-surface packet parked on drift control only.
