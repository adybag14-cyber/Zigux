...
- `PHASE9_SURVEYED_COMMIT=2026-05-22-first-loadable-parity-bitmap-module-direct-packet`
- scope: cross-family repo-reality survey for the bounded atomic64 direct packet, the still-partial bitmap reminder packet with restored module proof and blocked diff follow-through, and the shared loader contract surfaces only

## Current Repo Reality
Trusted current-tree reads on 2026-05-22 now show a split Phase 9 pilot picture: the atomic64 side returns a direct packet, the bitmap side remains partial but now includes direct module proof, and the shared loader contract surfaces are readable without proving shipped loader parity.
...
- the bitmap side still exposes only a partial trusted-path packet around the direct sample, loader, top-bit companion, manifest-backed ownership packet, survey gate, bounded build bundle, and restored module proof while the diff leg remains blocked
