...
- `PHASE9_SURVEYED_COMMIT=2026-05-23-first-loadable-parity-bitmap-diff-direct-packet`
- scope: cross-family repo-reality survey for the bounded atomic64 direct packet without a returned family-local loader scaffold, the still-partial bitmap reminder packet with restored module and diff proof, and the shared loader contract surfaces only

## Current Repo Reality
Trusted current-tree reads on 2026-05-23 now show a split Phase 9 pilot picture: the atomic64 side returns a direct packet without a returned family-local loader scaffold, the bitmap side remains partial but now includes direct module and diff proof, and the shared loader contract surfaces are readable without proving shipped loader parity.
...
- the bitmap side still exposes only a partial trusted-path packet around the direct sample, loader, top-bit companion, manifest-backed ownership packet, survey gate, bounded build bundle, and restored module and diff proof while broader shared loader completion remains blocked
