...
the current survey packet is pinned to `master` commit `355b71d89807a217a6b7c405c996cbd623c48ca0`.
...
- the landed shared-request trio still builds its shipped shared requests with `command_name = null`, and the adjacent trace-events scaffold also keeps `command_name` review-only; the non-null command-name evidence in this family is still limited to synthetic preservation checks for `perf-runtime-atomic64`, `perf-runtime-bitmap`, `perf-runtime-kretprobe`, and `perf-runtime-trace-events` in `samples/zigux/runtime_atomic64_loader.zig`, `samples/zigux/runtime_bitmap_loader.zig`, `samples/zigux/runtime_kretprobe_loader.zig`, and `samples/zigux/runtime_trace_events_loader.zig`, so this packet still records a field contract rather than a live runtime command surface
