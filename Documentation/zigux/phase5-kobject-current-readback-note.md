...
This note records the bounded 2026-05-23 readback state for the roadmap-backed `samples/kobject/kobject-example.c` anchor.
...
- the direct sample-root file, bounded attr-group companion, focused tests-root replay, dedicated survey note, and shared build route are readable through the authenticated contents route used here
- the manifest-backed contract and survey replay remain visible on public current `master` even though this run's authenticated contents route still returned `404` for those two companion paths
...
- `Documentation/zigux/phase5-kobject-sample-survey.md` remains the direct survey-note companion for the same packet, while `zigux/tests/phase5_kobject_example_manifest.json` and `zigux/tests/phase5_kobject_example_survey.zig` remain the manifest-backed and survey-replay companions even when this run had to prove those two through public current-`master` fallback instead of authenticated contents readback
...
Fresh same-lane rereads kept the wider shared reminder packet aligned with the mixed split above.

`Documentation/zigux/phase5-kobject-sample-survey.md`, `Documentation/zigux/README.md`, `Documentation/zigux/phase5-sample-review-guide.md`, `Documentation/zigux/phase5-sample-lane-sequencing.md`, `samples/zigux/README.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, `Documentation/zigux/review-checklist.md`, and `scripts/zigux/check-phase5-review-guide-surface.py` now keep the direct sample-root file, dedicated survey note, focused tests-root replay, direct shared build-route companion, public-tree-backed manifest and survey companions, and bounded attr-group companion aligned with this note.

No same-lane drift in the currently visible kobject reminder packet justified another broader shared-surface sweep in this run.

That means the next honest follow-through stays parked until a fresh repo-first reread proves a new one-surface mismatch against this split on current `master`.
...
1. reread this note beside the exact Phase 5 surface that looks stale and confirm whether the direct sample-root file, dedicated survey note, bounded attr-group companion, focused tests-root replay, shared build-route companion, and public-tree-backed manifest and survey companions are still described with the same split
...
If the lane reopens now, start with `Documentation/zigux/phase5-sample-review-guide.md`, `samples/zigux/README.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md`, and repair only one bounded surface if it stops matching the direct survey-note plus public-tree-backed manifest-and-survey-replay split above.
