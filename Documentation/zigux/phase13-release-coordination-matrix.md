...
4. If direct file reads are degraded, keep the release wording anchored to that same validator-first handle, keep the dedicated `scripts/zigux/check-phase13-shared-summary-surfaces.py` guard explicit as part of `make -C zigux phase13-validate`, and do not invent a shared `zigux/tests/phase13_build.zig` replay route, a notifier-only replay route, or a closure-only checklist.
