...
- shared-summary reread target: `scripts/zigux/README.md` still needs a same-tranche refresh because its current Phase 13 scripts-root summary treats the shipped `Documentation/zigux/phase13-landlock-syscalls-survey.md`, `zigux/tests/phase13_landlock_syscalls.zig`, and `zigux/helpers/notifier_chain_view.zig` surfaces as missing; until that scripts-root reminder is repaired, treat this matrix plus the docs-root and tests-root Phase 13 companions as the authoritative shared-summary packet.
