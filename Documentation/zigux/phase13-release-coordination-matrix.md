...
- shared-summary reread target: `scripts/zigux/README.md`, `zigux/tests/README.md`, this matrix, the docs-root Phase 13 companions, and the tests-root companion note are the shared surfaces to reread together; specifically reread `Documentation/zigux/phase13-contributor-workflow-guide.md`, `Documentation/zigux/phase13-shared-helper-lane-sequencing.md`, `Documentation/zigux/review-checklist.md`, `Documentation/zigux/phase10-phase11-phase13-contributor-surface-sync.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md`. Current `master` now keeps the docs-root, workflow, sequencing, tests-root, checklist, shared contributor-sync, tests-root companion, and compact PMO coordination packet aligned around the shipped `Documentation/zigux/phase13-libfs-survey.md` plus manifest-backed `libfs` packet, the shipped devres and Landlock packets, and adjacent notifier evidence, but `scripts/zigux/README.md` still undercounts the shipped Landlock direct replay evidence by keeping only the manifest-backed packet explicit in its current direct-evidence sentence instead of also naming `zigux/tests/phase13_landlock_ruleset.zig`, `zigux/tests/phase13_landlock_syscalls.zig`, and `zigux/tests/phase13_landlock_syscalls_reviewability.zig`. The next same-lane shared-summary follow-up should therefore start with that one-file scripts-root direct-evidence sync instead of staying parked or replaying the already-closed tests-root gap.
