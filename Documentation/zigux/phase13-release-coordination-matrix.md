...
- shared-summary reread target: `scripts/zigux/README.md`, `zigux/tests/README.md`, this matrix, the docs-root Phase 13 companions, and the tests-root companion note are the shared surfaces to reread together; current `master` now keeps the docs-root and scripts-root packet aligned around the shipped libfs foothold, the shipped devres and Landlock packets, the compact PMO coordination companion, and adjacent notifier evidence, but the broader `zigux/tests/README.md` guide still lacks the dedicated Phase 13 packet summary carried by `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, so the next same-lane shared-summary follow-up should start with that tests-root gap instead of replaying the older scripts-root omission.
