...
That shared packet matters because it keeps the workqueue, ring-buffer, skbuff, and RCU anchor notes tied to the same surveyed commits, parked-or-blocked posture, stay-in-C decisions, shared smoke manifest, full-bundle workqueue reviewability replay, and validator-backed smoke plus full-replay routes instead of drifting independently or disappearing from the shared evidence path. It also acts as the current owner-map surface for bounded-internal follow-through: workqueue routes through `P14-L04`, ring buffer routes through `P14-L08`, skbuff routes through `P14-L12`, and RCU currently routes through the manifest-backed `P14-L14` owner. Shared-lane runs should treat older packet-local owner labels as packet-local cleanup work only, not as permission to reopen a different bounded-internal lane.
