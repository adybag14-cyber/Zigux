...
Keep this cross-anchor traceability note aligned only when one of the four anchor packets or the shared smoke packet changes in a way that would otherwise hide a roadmap or stay-in-C boundary shift. Shared-lane follow-through should keep packet-local owner-label cleanup inside the affected anchor packet unless the cross-anchor owner map itself drifts. Same-phase bounded-internal follow-up should stay only in the existing workqueue and ring-buffer lanes, while skbuff and RCU remain Phase 15-governed freeze-in-C evidence rather than bounded-internal next-step lanes.
