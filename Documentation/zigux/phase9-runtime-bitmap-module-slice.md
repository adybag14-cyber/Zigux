...
- scope: sample starter, diff gate, loader scaffold, top-bit companion replay, module gate, survey gate, manifest-backed ownership packet, shared selftest-complete exit parity replay, and shared loader handoff plus shared build routes only
...
The shared `zigux/tests/runtime_loader_selftest_complete_exit_parity.zig` replay keeps the prepared selftest-complete bitmap request pinned even if the live sample later exits, so shared selftest evidence and release-without-substrate state stay reviewable beside the bitmap-local packet without pretending that the missing runtime substrate has landed.
...
7. `zig build phase9-runtime-loader-selftest-complete-exit-parity-tests --build-file zigux/tests/phase9_build.zig`
8. `zig build phase9-runtime-loader-shared-tests --build-file zigux/tests/phase9_build.zig`
9. `make -C zigux phase9`

## Next Bounded Step

Leave the runtime bitmap packet parked unless a smaller same-lane lifecycle, repeat-init, diff, top-bit boundary, prepared-plan handoff, shared selftest-complete exit parity, or selftest-hook reviewability gap appears before the blocked shared runtime substrate work moves again.
