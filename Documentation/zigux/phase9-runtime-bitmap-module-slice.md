...
The returned direct sample and top-bit proof make the starter packet more concrete again, but the shared `zigux/tests/phase9_build.zig` bundle still does not prove the broader shared runtime-loader packet returned.
...
- the current runtime bitmap packet is still `partial_packet_without_loadable_runtime_substrate`: the survey note, module-slice note, survey gate, bounded build bundle, direct sample, and top-bit companion proof are visible, but the loader companion, module gate, diff gate, manifest-backed ownership packet, and broader shared runtime-loader substrate are not
...
Treat the shared `zigux/tests/phase9_build.zig` bitmap route names as rerun handles only while the direct loader, module, diff, and manifest surfaces stay absent on the trusted path; they do not prove the broader shared runtime-loader packet returned.
