...
- verified against current `master` head `0255e633baf88547854b6120c01bb8ecfc2ee256`

## Why this gap note exists

The Phase 14 roadmap keeps `net/core/skbuff.c` under freeze-in-C governance.
The attached-toolchain and environment guidance also says to run the narrowest
honest Zig verification available and not to imply compile checks that the live
repo cannot actually replay.
...
- `Documentation/zigux/phase14-skbuff-bridge-survey.md` says current `master`
  ...
  `zigux/tests/phase14_build.zig`, or `net/core/skbuff_bridge.zig`.
- the same survey note says the earlier `full_bundle_only` compile path is
  archival only and must not be treated as live compile evidence.
...
The survey note is truthful today, but it does not yet have a dedicated
...
compile wording without restoring the packet first.
