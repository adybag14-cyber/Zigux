...
- do `zigux/tests/phase5_bytestream_fifo.zig`, `zigux/tests/phase5_bytestream_fifo_manifest.json`, and `zigux/tests/phase5_bytestream_fifo_survey.zig` still describe the exact queue-order replay, the explicit transfer-count contract, preview truncation boundary, the non-destructive snapshot contract, lifecycle boundary, bounded helper contract, and remaining-capacity evidence run through `zigux/tests/phase5_build.zig`?
...
- `zig test samples/zigux/bytestream_fifo.zig` passed `6/6` sample self-checks
