...
- do the shared Phase 5 packet and this sample note still say clearly that there is no standalone `samples/zigux/*bitmap*` Phase 5 reference sample, so direct bitmap helper reviewability stays under `tools/lib/bitmap.zig`, `Documentation/zigux/phase1-closure.md`, and `Documentation/zigux/phase4-validation-matrix.md` while the separate Phase 9 runtime bitmap family stays under `Documentation/zigux/phase9-runtime-bitmap-survey.md`, `samples/zigux/runtime_bitmap.zig`, `samples/zigux/runtime_bitmap_loader.zig`, `samples/zigux/runtime_bitmap_top_bit_contract.zig`, `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, and `zigux/tests/phase9_build.zig` instead of being misread as a fifth Phase 5 sample?
