...
- do the direct validation routes stay explicit too: `zig test samples/zigux/bytestream_fifo.zig`, `zig test zigux/tests/phase5_bytestream_fifo.zig`, and `zig test zigux/tests/phase5_bytestream_fifo_survey.zig` should stay visible as the sample-owned self-check route, the focused replay route, and the survey-packet guard, while the shared `zigux/tests/phase5_build.zig` line stays current public-tree-backed companion evidence rather than direct authenticated proof?
