...
- does `samples/zigux/README.md` still separate the four Phase 5 reference samples from the later `runtime_*` starters that share the same directory, keep the dedicated bytestream FIFO review-packet stanza focused on the exact replay contract, the exact `checked_focus` order, the helper-only review surface, and out-of-scope runtime claims, and still say plainly that current `master` ships no `samples/zigux/*string*` or `samples/zigux/*cmdline*` Phase 5 reference sample because those helper families stay under the separate Phase 7 bundles?
