...
- the shared Phase 5 guide in `Documentation/zigux/phase5-sample-review-guide.md`, the sample-root summary in `samples/zigux/README.md`, the scripts-root summary in `scripts/zigux/README.md`, and the tests-root summary in `zigux/tests/README.md` already keep this anchor routed through the survey note, the directly readable manifest companion, and the broader bytestream packet wording instead of pretending the sample stands alone.
- `zigux/tests/README.md` now names the bytestream survey note, the sample-root anchor, and the directly readable bytestream manifest companion from the tests root while still framing `zigux/tests/phase5_bytestream_fifo.zig`, `zigux/tests/phase5_bytestream_fifo_survey.zig`, and `zigux/tests/phase5_build.zig` as current public-tree-backed companion evidence rather than direct authenticated-contents proof.
...
- rollover and split-window behavior stay explicit through `runWrappedPreviewReplay()`, `visibleSpanSummary()`, `writableSpanSummary()`, and `usesWrappedStorageWindow()`
- remaining-capacity and occupancy boundaries stay explicit through `runRemainingCapacityReplay()`, `available()`, and `occupancySummary()`
...
- occupancy helper checks now stay explicit too: `occupancySummary()` reports `(used=0, available=32, empty=true, full=false, wrapped_window=false)` when cold, `(used=5, available=27)` after enqueuing `"hello"`, full-and-not-wrapped at capacity before a skip, wrapped-and-full after the skip-plus-refill case, and empty again after reset and exit
