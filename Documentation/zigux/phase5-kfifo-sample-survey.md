...
The shared scripts-root guide in `scripts/zigux/README.md` is part of that contributor-facing boundary now too, because it names the direct `zig test samples/zigux/bytestream_fifo.zig` replay, the direct helper-review replay `zig test zigux/tests/phase5_bytestream_fifo.zig`, the paired `zig test zigux/tests/phase5_bytestream_fifo_survey.zig` replay, and the shared `phase5_build.zig` entrypoint beside the same Phase 7 helper-boundary and later runtime-follow-on cues.
The shared sample-root catalog in `samples/zigux/README.md`, the shared tests-root guide in `zigux/tests/README.md`, the top-level docs root in `Documentation/zigux/README.md`, and the shared prompts in `Documentation/zigux/review-checklist.md` are part of that contributor-facing boundary now too, because together they keep the landed bytestream FIFO idiom visibly separate from the later runtime starters. The docs root and shared tests-root guide point reviewers back to the direct `zig test samples/zigux/bytestream_fifo.zig` replay, the direct helper-review replay `zig test zigux/tests/phase5_bytestream_fifo.zig`, the paired `zig test zigux/tests/phase5_bytestream_fifo_survey.zig` replay, and the exact shipped review packet, while the review checklist keeps the same family tied to the descriptor, manifest-backed survey, sample-backed survey note, the direct sample replay, the paired survey replay, and the shared `phase5_build.zig` entrypoint without over-claiming the helper-review route.
...
The same shared packet now also keeps the current no `samples/zigux/*rbtree*` Phase 5 boundary explicit, so the landed bytestream idiom does not drift into the separate Phase 7 rbtree helper bundle rooted in `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, `zigux/tests/phase7_rbtree.zig`, and `zigux/tests/phase7_build.zig`.
...
- the shared scripts-root guide in `scripts/zigux/README.md` is part of that same contributor packet now, because it names the direct `zig test samples/zigux/bytestream_fifo.zig` replay, the direct helper-review replay `zig test zigux/tests/phase5_bytestream_fifo.zig`, the paired `zig test zigux/tests/phase5_bytestream_fifo_survey.zig` replay, and the shared `phase5_build.zig` entrypoint beside the same helper-boundary and runtime-follow-on cues.
...
- the shared sample-root catalog, shared tests-root guide, and docs root now also keep the no `samples/zigux/*rbtree*` Phase 5 boundary explicit, so rbtree review stays in the separate Phase 7 helper bundle instead of reading like another missing sample port.
...
- `scripts/zigux/README.md` now stays in the same contributor packet so the direct sample replay, helper-review replay, paired survey replay, and shared `phase5_build.zig` entrypoint remain named together beside the same helper-boundary and runtime-follow-on cues
- this approved in-memory FIFO idiom is now pinned to `PHASE5_SURVEYED_COMMIT=a15760c3e46103fd41ae0da852b61f612e9116c6` so the survey note, manifest-backed checks, shared sample-root catalog, shared scripts-root guide, shared tests-root guide, shared review checklist, and contributor refresh path all point at the same inspected `master` head
...
- the same sample-root packet also needs to keep the no-`samples/zigux/*rbtree*` boundary visible so the bytestream survey lane does not accidentally count the separate Phase 7 rbtree helper bundle as a missing Phase 5 sample gap
