...
If a future Phase 9 lane reopens this metadata family, keep the next step narrow: first harden `scripts/zigux/validate-phase9.py --self-test` so the shared validator mutates this dedicated module-metadata packet and proves the already-landed survey note, manifest, focused replay, dedicated checker, and shared `phase9-module-metadata-survey` route markers fail closed inside the validator itself. Only after that lands should later work consider adding a real metadata-export surface or extending the shared handoff to the trace-events loader scaffold without overstating depmod parity.
