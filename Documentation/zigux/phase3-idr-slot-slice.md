...
- `zigux/tests/phase3_idr_slot_build.zig` now exposes one shared bundle route that runs the landed starter packet and dump together without pretending that the broader shared Phase 3 validator packet has widened.
- `zigux/Makefile` still exposes one focused starter-packet wrapper and one focused dump wrapper so the helper-local rerun surface stays reachable without widening into the aggregate shared Phase 3 lane.
...
That keeps the current follow-through narrow and reviewable while making the landed `idr_slot` packet easier to rerun as one bounded interop slice.
...
This note is limited to the helper-local `idr_slot_view` classifier layered on the already-landed tagged `xarray_slot_view`, `xa_value`, and `err_ptr` helpers, together with one starter packet, one fixture-backed dump parity packet, one shared bundle build route, one dedicated dump build file, one helper-local manifest, two focused Makefile wrappers, and two checkers. It does not claim broader IDR traversal semantics, export-shim wiring, xarray dump parity outside this packet, or wider IDA coverage.
