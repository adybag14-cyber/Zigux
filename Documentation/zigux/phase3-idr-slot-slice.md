...
This note is limited to the helper-local `idr_slot_view` classifier layered on the already-landed tagged `xarray_slot_view`, `xa_value`, and `err_ptr` helpers, together with one starter packet, one fixture-backed dump parity packet, one dedicated dump build file, one helper-local manifest, and two checkers. It does not claim broader IDR traversal semantics, export-shim wiring, xarray dump parity outside this packet, or wider IDA coverage.
