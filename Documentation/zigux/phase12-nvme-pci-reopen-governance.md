...
- Current `master` does not carry the older direct replay, survey gate, survey note, slice note, or fallback-note paths that earlier lane memory referenced, so this owner map and the manifest must stay explicit about the smaller live packet until those review surfaces are deliberately relanded.
- The reopen rule is therefore narrow: reopen this packet only when the bounded starter itself drifts, when the manifest or this owner map drifts against the smaller live packet, when the shared Phase 12 build route changes, or when a later roadmap-backed transport-facing shard is explicitly proposed.

## Owner Split

- `P12-L08` owns the substantive driver-local NVMe starter packet in `drivers/nvme/host/pci.zig`, `drivers/nvme/host/pci_verify.zig`, `zigux/tests/phase12_nvme_pci_manifest.json`, and this owner-map note when those surfaces drift together.
...
2. The driver-local review packet drifts.
   This includes changes to this owner map, the manifest in `zigux/tests/phase12_nvme_pci_manifest.json`, or any relanded direct replay, survey gate, survey note, slice note, or fallback note that describes the same bounded packet.
...
3. one lane-local truthfulness repair if the manifest or this owner map drifts against the smaller live packet
