...
Current `master` ships the bounded starter `drivers/nvme/host/pci.zig`, the bounded verifier shard `drivers/nvme/host/pci_verify.zig`, the direct replay `zigux/tests/phase12_nvme_pci.zig`, the slice note `Documentation/zigux/phase12-nvme-pci-slice.md`, the survey note `Documentation/zigux/phase12-nvme-pci-survey.md`, the survey gate `zigux/tests/phase12_nvme_pci_survey.zig`, and the manifest anchor `zigux/tests/phase12_nvme_pci_manifest.json`. Treat those files as queue-planning, PRP-shape, queue-restart, dropped-backlog retirement, rollback-gate, and frozen queue-restore budgeting evidence only, not as proof that the shared build wiring or a live runtime path is already complete.
...
## Ownership Split
- `P12-L08` owns the shipped direct packet: queue planning, queue-count reservation, PRP buffer-shape accounting, reset-summary bookkeeping, queue-restart review, dropped-backlog retirement review, rollback-gate review, frozen queue-restore budgeting, the verifier shard, the direct replay, the slice note, the survey note, the survey gate, the manifest anchor, and bounded DMA-note or recovery-governance preflights that stay within the current starter-plus-verifier-plus-review-surface packet
...
1. a bounded survey, slice-note, or owner-map truthfulness repair that keeps the starter, verifier, direct replay, slice note, survey note, survey gate, and manifest anchor explicit while the shared build route stays unwired on current `master`
...
## Review Rule
If a proposed NVMe PCI change touches files outside the driver-local reminder packet, owner-map note, starter shard, verifier shard, direct tests, slice note, survey note, survey gate, or manifest, it should be treated as out of lane unless the only extra edit is a shared pointer update in `Documentation/zigux/phase12-complex-driver-lane-sequencing.md`.

## Next Bounded Step
If the NVMe PCI lane family reopens again, keep it to one approved same-driver packet:
- leave `drivers/nvme/host/pci.zig`, `drivers/nvme/host/pci_verify.zig`, `zigux/tests/phase12_nvme_pci.zig`, `Documentation/zigux/phase12-nvme-pci-slice.md`, `Documentation/zigux/phase12-nvme-pci-survey.md`, `zigux/tests/phase12_nvme_pci_survey.zig`, and `zigux/tests/phase12_nvme_pci_manifest.json` framed as the bounded shipped packet until the shared build route actually lands
- pair those shipped surfaces with shared build wiring only when the smoke-first packet is ready
