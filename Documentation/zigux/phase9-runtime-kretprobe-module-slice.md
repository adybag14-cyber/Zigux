...
- a bounded `runtime_kretprobe_loader` scaffold that keeps the planned `register_kretprobe()` and `unregister_kretprobe()` labels, explicit init and exit symbol names as review-only handoff data, private-data size, idle registration snapshot, and failed-exit state retention explicit while the real runtime substrate is still unavailable.
...
- live `register_kretprobe()` or `unregister_kretprobe()` parity
- live `module_init()`, `module_exit()`, or kernel initcall-order parity
