...
- adds one bounded `dcache_dir_close()` release-bookkeeping planner that keeps the work in pure `dput(file->private_data)` intent and zero-error close results without claiming live refcount mutation, inode ownership, or directory teardown
...
The next honest bounded step in this same lane is to stay helper-first and survey whether the `scan_positives()` `need_resched()` handoff around `__hlist_del()` plus `hlist_add_behind()` can be modeled without claiming live sibling traversal, reschedule ownership, or `cond_resched()` execution.
