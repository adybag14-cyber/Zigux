...
- adds one bounded post-scan cursor-reposition planner for the `dcache_dir_lseek()` and `dcache_readdir()` tails so Zigux can validate when the private cursor is merely unhashed versus re-anchored before or behind a found positive dentry, without claiming live sibling-list mutation, lock ordering, or cursor dentry ownership
- adds one tiny `dcache_dir_close()` release planner that keeps the close path in pure `dput(file->private_data)` intent, optional private-cursor presence, and guaranteed zero-return bookkeeping without claiming live refcount mutation, inode ownership, or directory teardown
...
The next honest bounded step in this same lane is no longer another tiny close-path wrapper. After the landed preconditions, cursor reposition bookkeeping, and close release planner, the remaining `dcache` helpers are blocked on live sibling-list traversal, parent-lock ordering, reschedule-aware walking, and broader VFS state that this helper-first slice still does not claim.
