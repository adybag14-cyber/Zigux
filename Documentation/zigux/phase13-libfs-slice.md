...
The next honest bounded step in this same lane is no longer another transaction helper. It is the deeper `dcache_readdir()` cursor-resume packet around sibling traversal, reschedule-aware cursor movement, and lock-ordering preconditions, which should remain blocked until those boundaries are spelled out without pretending to own live VFS state.
