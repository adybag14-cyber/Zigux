...
- adds one tiny `dcache_dir_close()` release planner that keeps the work in pure `dput(file->private_data)` intent and zero-error close bookkeeping without claiming live refcount mutation, inode ownership, or directory teardown
