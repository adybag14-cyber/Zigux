...
- adds the landed transaction publish helper surface around `simple_transaction_set()` by keeping the work in response-size validation, the pre-existing buffer requirement, and the barrier-backed rule that the readable size stays hidden until publish bookkeeping is complete before any live readback or release behavior is claimed

This slice does not claim `d_add()` side effects, cursor-backed directory iteration, inode allocation, pseudo-fs mounting, simple-transaction read or release state, or any other live VFS plumbing from the wider `fs/libfs.c` body.

The next honest bounded step in this same lane is to stay helper-first and add one small transaction read helper surface that still avoids live dentries, inode-backed state, release lifecycle work, and pseudo-filesystem ownership.
