...
- adds the landed `dcache_dir_close()` cursor-release helper surface by keeping the work in the `file->private_data` release decision, the null-cursor no-op allowance, and the unconditional zero return path before any sibling traversal, lock ordering, or live cursor mutation is claimed
- adds the landed `dcache_dir_lseek()` cursor-reposition helper surface by keeping the work in hashed-cursor detach, explicit before-target versus behind-target reinsertion, the unlinked fallback when no positive target is found, and temporary-target release bookkeeping before any live sibling traversal, lock ordering, or reschedule-aware cursor movement is claimed
