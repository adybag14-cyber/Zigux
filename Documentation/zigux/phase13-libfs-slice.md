...
The next honest bounded step in this same lane is to stay helper-first and model the tiny `dcache_dir_close()` release bookkeeping around `dput(file->private_data)` and the zero-error close outcome without claiming live refcount mutation, inode ownership, or directory teardown.
