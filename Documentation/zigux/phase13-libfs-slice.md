...
The next honest bounded step in this same lane is to stay helper-first and model the post-scan cursor reposition bookkeeping around `hlist_del_init()` plus `hlist_add_before()` / `hlist_add_behind()` without claiming live sibling-list mutation, lock ordering, or cursor dentry ownership.
