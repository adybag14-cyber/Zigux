...
- `Documentation/zigux/phase11-bcm2835-wdt-validation-matrix.md` now names the shared Phase 11 test gate, the current timeout, probe, registration, registration-outcome, platform-handoff, poweroff-path, and remove-time evidence, and the still-blocked live platform-registration or PM-base decision
...
This lane is no longer survey-only, and the archival survey now carries `P11-L08` packet identity so the bcm2835 watchdog review record stays traceable alongside the live manifest, survey gate, and validator ownership for the current lane key. It does not claim watchdog-core registration, PM base wiring, live remove-time poweroff-handler release behavior, or hardware-backed execution.
