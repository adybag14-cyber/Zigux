...
The current driver file, dedicated replay, compile-local verify helper, survey gate, and archival reminder packet now keep this bounded bcm2835 packet reviewable through:
...
* manifest-backed reminder coverage in `zigux/tests/phase11_bcm2835_wdt_manifest.json` and `Documentation/zigux/phase11-bcm2835-wdt-slice.md` for the landed starter, teardown note, validation matrix, shared replay evidence, and the still-blocked hardware-validation plan boundary
...
It does record one narrower fact: current `master` now keeps a directly readable bcm2835 driver starter, one dedicated replay-backed test surface, one compile-local verify helper, one dedicated survey gate, one bounded remove-time cleanup summary, and one archival manifest-backed reminder packet. That packet is already large enough that the next same-lane move should be an explicit validation plan for any future platform-registration or PM-base follow-through, not more stale wording about whether the manifest or slice exists.

## Next Bounded Step

The next honest same-lane follow-through is one bcm2835-only explicit validation plan that matches the landed driver starter, verify helper, dedicated replay, dedicated survey gate, manifest, slice note, teardown note, and validation matrix.
Keep that follow-through inside the bcm2835 watchdog packet only. Do not widen into `gpio_wdt`, `dw_wdt`, shared Phase 11 wording, or live platform-registration work until that explicit validation plan is written.