...
This lane is no longer survey-only, but the archival survey now keeps its current `P11-L05` identity so the bcm2835 watchdog review packet does not drift away from the live scheduled lane that maintains it. It does not claim watchdog-core registration, PM base wiring, live remove-time poweroff-handler release behavior, or hardware-backed execution.
