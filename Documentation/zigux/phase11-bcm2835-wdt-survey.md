...
- `Documentation/zigux/phase11-bcm2835-wdt-validation-matrix.md` now names the shared Phase 11 test gate, the current timeout, register-image transition, probe, registration, registration-outcome, platform-handoff, poweroff-path, and remove-time evidence, the shipped `make -C zigux phase11` wrapper route, and the still-blocked live platform-registration or PM-base decision
