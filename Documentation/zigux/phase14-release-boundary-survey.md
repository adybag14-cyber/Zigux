...
- current reminder-surface alignment: `Documentation/zigux/phase14-end-to-end-smoke-survey.md`, `Documentation/zigux/phase14-shared-smoke-current-master-gap.md`, and `Documentation/zigux/phase14-attached-toolchain-guidance-gap.md` already keep the recovered study-only packet explicit, keep the directly readable validator surface and workqueue reviewability shard visible, and keep `make -C zigux phase14-validate` explicit as the current shared-smoke route. `Documentation/zigux/README.md` still lags that returned route split by describing the readable `zigux/Makefile` as if it also lacked `phase14-validate`; `scripts/zigux/README.md` still spells out the older attached-toolchain wrapper triplet directly as packet-local traceability vocabulary; `zigux/tests/README.md` already separates that historical wrapper vocabulary from the returned `phase14-validate` gate; and `Documentation/zigux/review-checklist.md` is readable too and now carries the dedicated Phase 14 shared-smoke checkpoint, so the next smaller truthful reminder follow-through is docs-root or scripts-root wording rather than another tests-root or checklist repair
...
If a future same-lane reread keeps `Documentation/zigux/README.md` or `scripts/zigux/README.md` readable while one of them still misstates the returned `phase14-validate` split or still spells out the older attached-toolchain wrapper triplet more strongly than the shared packet now does, tighten that smaller reminder surface next before reopening tests-root wording.
