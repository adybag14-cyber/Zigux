...
- directly recoverable shared smoke packet in this release-facing lane:
  ...
  - `scripts\zigux/check_phase14_shared_smoke_route.zig` now returns through the current contents path and keeps the returned `phase14-validate` Makefile route plus workflow gate explicit in the current shared-smoke packet
  - `scripts\zigux/validate_phase14.zig` through the current contents path
  - `scripts\zigux/check_phase14_rollback_threshold_sequencing.zig` now returns through the current contents path and keeps the shared rollback threshold, fallback path, automatic return-to-blocked triggers, and returned Makefile split explicit in the current shared-smoke packet
  - `scripts\zigux/check_phase14_rcu_rollback_guardrail.zig` now returns through the current contents path and keeps the dedicated `kernel/rcu/tree.c` freeze-in-C rollback guard explicit beside the shared smoke packet
  ...
  - `scripts\zigux/check_phase14_tests_readme_smoke_summary.zig` now returns through the current contents path and keeps the tests-root reminder packet aligned with the recovered study-only shared-smoke split without promoting the broader `phase14-smoke`, `phase14-test`, or `phase14` wrappers
...
The directly readable route checker matters now too: `scripts\zigux/check_phase14_shared_smoke_route.zig` again proves the returned `phase14-validate` route in both the readable Makefile body and the readable bootstrap workflow, so this release-facing note should keep that checker explicit instead of implying the route evidence lives only in neighboring reminder prose.

The directly readable tests-root checker matters too: `scripts\zigux/check_phase14_tests_readme_smoke_summary.zig` is now part of the returned release-facing packet, and it proves that the shared tests-root reminder stays aligned with the same recovered study-only packet and returned `phase14-validate` split. That means the older tests-root undercount no longer owns the next same-lane follow-through.

The directly readable rollback guards matter too: `scripts\zigux/check_phase14_rollback_threshold_sequencing.zig` now keeps the shared rollback threshold contract explicit, while `scripts\zigux/check_phase14_rcu_rollback_guardrail.zig` keeps the dedicated `kernel/rcu/tree.c` freeze-in-C rollback guard explicit beside the same returned packet. `Documentation/zigux/phase14-rcu-tree-survey.md` is directly readable again too, so the release-facing note should keep that returned freeze-in-C companion visible instead of leaving the RCU posture implicit in broader reminder prose.
...
- keep `scripts\zigux/check_phase14_shared_smoke_route.zig` explicit as the directly readable current route-truthfulness guard for that returned gate
- keep `scripts\zigux/check_phase14_tests_readme_smoke_summary.zig` explicit as the directly readable current tests-root reminder-truthfulness guard for that same returned split
- keep `scripts\zigux/check_phase14_rollback_threshold_sequencing.zig` explicit as the directly readable shared rollback-threshold truthfulness guard
- keep `scripts\zigux/check_phase14_rcu_rollback_guardrail.zig` explicit as the directly readable dedicated freeze-in-C rollback-truthfulness guard for `kernel/rcu/tree.c`
- keep `Documentation/zigux/phase14-rcu-tree-survey.md` explicit as the directly readable dedicated freeze-in-C RCU companion note
...
- keep the next same-lane follow-through focused on the next smallest shared reminder or raw-build truthfulness drift now that the manifest and the returned checker packet already carry the returned compile-count split
...
- the directly readable shared-smoke route guard through `scripts\zigux/check_phase14_shared_smoke_route.zig` on the current contents path
- the directly readable validator reminder through `scripts\zigux/validate_phase14.zig` on the current contents path
- the directly readable shared rollback-threshold guard through `scripts\zigux/check_phase14_rollback_threshold_sequencing.zig`
- the directly readable dedicated RCU rollback guard through `scripts\zigux/check_phase14_rcu_rollback_guardrail.zig`
- the directly readable release-boundary exact-count guard through `scripts\zigux/check_phase14_release_boundary_exact_counts.zig`
- the directly readable tests-root reminder guard through `scripts\zigux/check_phase14_tests_readme_smoke_summary.zig`
...
- the directly readable dedicated RCU survey companion through `Documentation/zigux/phase14-rcu-tree-survey.md`
