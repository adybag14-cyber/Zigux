...
If a future same-lane reread keeps `scripts/zigux/README.md` and `zigux/tests/README.md` readable while they still spell out the older attached-toolchain wrapper triplet directly, tighten those smaller reminder surfaces next so they match the returned `phase14-validate` route plus the broader `phase14-*` gap posture already carried by the shared notes.
