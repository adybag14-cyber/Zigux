...
- compact PMO coordination now also stays explicit through `Documentation/zigux/phase14-release-coordination-matrix.md`, so the shared Phase 14 packet has a dedicated owner-and-handoff summary between the release-boundary note and the parked Phase 15 governance packet instead of leaving that release-planning bridge implicit in survey prose alone
...
- attached-toolchain replay fallback: `ZIG=/absolute/path/to/attached-zig/zig make -C zigux phase14-smoke`, `ZIG=/absolute/path/to/attached-zig/zig make -C zigux phase14-test`, and `ZIG=/absolute/path/to/attached-zig/zig make -C zigux phase14` remain the release-facing fallback when neither the repo-local `.zig-toolchain` candidate nor the shell's default `zig` binary is available, so the same bounded smoke-shard, full-bundle, and combined replay routes stay usable without inventing a separate environment-only command packet
