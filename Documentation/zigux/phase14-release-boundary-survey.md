...
The docs-root, scripts-root, and tests-root reminders are now aligned on that split, so this release-facing note should keep pace with the current shared reminder packet instead of reopening the earlier tests-root undercount story.
