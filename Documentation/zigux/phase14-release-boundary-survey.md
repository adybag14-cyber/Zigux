...
- current reminder-surface alignment: `Documentation/zigux/README.md`, `Documentation/zigux/phase14-end-to-end-smoke-survey.md`, and `scripts/zigux/README.md` already keep the recovered study-only packet explicit, keep the directly readable workqueue reviewability shard visible, and frame the older `phase14-*` route names as packet-local or repo-reality-gap vocabulary rather than current Makefile-backed proof; `zigux/tests/README.md` still carries one narrower reviewer-prompt undercount of the returned Makefile route families, so this release-facing note should not overstate tests-root alignment yet
...
The docs-root and scripts-root reminders are already aligned on that split, while `zigux/tests/README.md` still carries one narrower reviewer-prompt undercount of the returned Makefile route families. This release-facing note should therefore keep pace with the current shared reminder packet instead of overstating full tests-root alignment.
