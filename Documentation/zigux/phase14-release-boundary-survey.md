...
If a future same-lane reread finds another broader shared reminder surface still undercounting the returned exact-count checker or overstating the current `phase14-*` Makefile routes, tighten that smallest shared note before revisiting any validator-local handoff.
