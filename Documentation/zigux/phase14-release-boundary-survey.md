...
- directly recoverable shared smoke packet in this release-facing lane:
  ...
  - `scripts/zigux/check-phase14-rollback-threshold-sequencing.py` now returns through the current contents path and keeps the shared rollback threshold, fallback path, automatic return-to-blocked triggers, and returned Makefile split explicit in the current shared-smoke packet
  - `scripts/zigux/check-phase14-rcu-rollback-guardrail.py` now returns through the current contents path and keeps the dedicated `kernel/rcu/tree.c` freeze-in-C rollback guard explicit beside the shared smoke packet
...
The directly readable rollback guards matter too: `scripts/zigux/check-phase14-rollback-threshold-sequencing.py` now keeps the shared rollback threshold contract explicit, while `scripts/zigux/check-phase14-rcu-rollback-guardrail.py` keeps the dedicated `kernel/rcu/tree.c` freeze-in-C rollback guard explicit beside the same returned packet. `Documentation/zigux/phase14-rcu-tree-survey.md` is directly readable again too, so the release-facing note should keep that returned freeze-in-C companion visible instead of leaving the RCU posture implicit in broader reminder prose.
...
- keep `scripts/zigux/check-phase14-rollback-threshold-sequencing.py` explicit as the directly readable shared rollback-threshold truthfulness guard
- keep `scripts/zigux/check-phase14-rcu-rollback-guardrail.py` explicit as the directly readable dedicated freeze-in-C rollback-truthfulness guard for `kernel/rcu/tree.c`
...
- the directly readable shared rollback-threshold guard through `scripts/zigux/check-phase14-rollback-threshold-sequencing.py`
- the directly readable dedicated RCU rollback guard through `scripts/zigux/check-phase14-rcu-rollback-guardrail.py`
