...
- adds one bounded `merge_tree()` replay planner so a source single-layer rule can be validated, translated to `dst->num_layers`, and handed off to `insert_rule()` as pure data without claiming locked tree traversal, destination mask upgrades, or hierarchy ownership
...
The next honest bounded step in this same lane is blocked until there is a justified way to study the live locking, destination access-mask upgrade, hierarchy lifetime, workqueue-backed teardown, and other real ruleset state around `merge_tree()`, `merge_ruleset()`, or `inherit_ruleset()` without pretending this helper lab already owns Landlock storage or policy enforcement.
