...
Fresh authenticated file checks in this run also keep `scripts/zigux/check-phase13-landlock-ruleset-packet.py`, `Documentation/zigux/phase13-landlock-ruleset-survey.md`, and `zigux/tests/phase13_landlock_ruleset_manifest.json` materialized on current `master`, so the broader shared reminder packet should treat that checker-backed Landlock ruleset companion as shipped same-lane evidence rather than as a repo-reality gap.
