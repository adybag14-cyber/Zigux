...
What remains open inside this shared-subsystems lane has therefore moved out of the stable contributor-facing handle and into `Documentation/zigux/phase13-release-notes-survey.md`: that broader docs-root reminder still says `zigux/tests/README.md` carries one stale repo-reality-gap sentence that lists returned `scripts/zigux/validate-phase13-release.py` as missing even though the tests-root packet now keeps that validator explicit beside the shared-summary guard and returned `zigux/Makefile` posture. Keep `Documentation/zigux/phase13-libfs-survey.md` and `zigux/tests/phase13_libfs_addressability.zig` recorded as repo-reality gaps while narrowing the next same-lane follow-through to that release-notes reminder drift.
