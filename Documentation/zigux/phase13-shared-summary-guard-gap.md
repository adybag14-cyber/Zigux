...
What remains open inside this shared-subsystems lane has narrowed again: `Documentation/zigux/phase13-release-notes-survey.md` no longer carries the older tests-root validator-gap claim, so the stable contributor-facing handle and the broader release-facing reminder now agree that `scripts/zigux/validate-phase13-release.py` is shipped current-`master` release-discipline support. Keep `zigux/tests/phase13_libfs_addressability.zig` recorded as a repo-reality gap while treating the next same-lane follow-through as a fresh reread for any remaining broader reminder drift or checker-local exactness miss.
