...
- the live repo now also ships `python3 scripts/zigux/check-phase10-survey-provenance.py` as a supporting provenance readback that compares the shared closure manifest's core, ring, input, and MMIO lane keys plus surveyed commits back against the four survey manifests, so the core lane's mirrored `P10-L01` and `surveyed_commit` evidence no longer rely on reviewer eyeballing alone even though that checker remains narrower than the published shared Phase 10 exact-check path.
