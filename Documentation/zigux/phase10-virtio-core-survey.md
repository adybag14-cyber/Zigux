...
- public current-`master` readback of `zigux/tests/phase10_build.zig` still exposes `phase10-virtio-core-tests`, but this rerun did not find `phase10_virtio_core_survey_module`, `phase10-virtio-core-survey-tests`, or `run_phase10_virtio_core_survey_tests` in the shared build route
...
- shared core survey build omission: `zigux/tests/phase10_build.zig` does not currently expose `phase10_virtio_core_survey_module`, `phase10-virtio-core-survey-tests`, or `run_phase10_virtio_core_survey_tests` beside the already-landed `phase10-virtio-core-tests` route
...
- public current-`master` blob readback of `Documentation/zigux/phase10-virtio-core-slice.md`, `scripts/zigux/check-phase10-core-packet.py`, `zigux/tests/phase10_virtio_core_manifest.json`, `zigux/tests/phase10_virtio_core_survey.zig`, `drivers/virtio/virtio.zig`, `drivers/virtio/virtio_verify.zig`, `zigux/tests/phase10_virtio_core.zig`, `zigux/tests/phase10_virtio_core_reset_queue.zig`, and `zigux/tests/phase10_virtio_core_interrupt_compound_ack.zig`
...
No attached-Zig replay was available in this run because there is still no writable live Zigux checkout in this workspace and the direct core-survey shared-build route is the remaining same-packet omission.

## Next bounded step

Keep the follow-through inside the same core packet only:

- wire `zigux/tests/phase10_virtio_core_survey.zig` into `zigux/tests/phase10_build.zig` so the shared Phase 10 build route exposes `phase10_virtio_core_survey_module`, `phase10-virtio-core-survey-tests`, and `run_phase10_virtio_core_survey_tests` beside `phase10-virtio-core-tests`, then rerun the narrowest honest Phase 10 core survey replay available
- if that shared-build follow-through stays blocked, keep future rereads explicit about the mixed-source readback requirement instead of regressing to a false "core packet missing" story
