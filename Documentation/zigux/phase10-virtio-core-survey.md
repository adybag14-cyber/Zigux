...
- the live manifest still records lane `P10-L01`, surveyed commit `c11221dc7a68d7511ae1c69d64b3f08528287ed8`, roadmap destinations `drivers/virtio/*.zig`, `zigux/kernel/`, and `zigux/helpers/`, `preexisting_phase10_test_files: 11`, and a mostly landed starter packet, but current public and exact-path rereads no longer support its older claim that `phase10-driver-id-helper`, `phase10-driver-id-coverage-disposition-helper`, and `phase10-driver-id-gate` are already materialized on current `master`
...
- stale follow-through drift: this survey's earlier next-step guidance kept pointing at `Documentation/zigux/phase10-virtio-core-slice.md` and `zigux/tests/phase10_virtio_core_manifest.json` even though current public current-`master` rereads now show both packet members already leave `drivers/virtio/virtio_driver_id.zig` and `zigux/tests/phase10_virtio_driver_id.zig` outside shipped claims, so the remaining same-lane truthfulness work is keeping the survey itself aligned with that already-landed packet state rather than reopening slice-or-manifest wording
...
- keep `Documentation/zigux/phase10-virtio-core-survey.md` aligned with the already-correct packet-local truth on current `master`: the `virtio_driver_id` helper-and-replay pair is not shipped evidence until those two paths actually materialize again, and the slice note plus manifest no longer need the previously planned correction
- keep future rereads explicit about the mixed-source readback requirement while `zigux/tests/phase10_virtio_core_survey.zig` still `404`s through direct contents reads from this environment, and treat the next same-lane truthfulness follow-through as survey-local guidance and current-head reread hygiene instead of widening into ring, MMIO, or transport work
