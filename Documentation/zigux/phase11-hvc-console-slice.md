...
`Documentation/zigux/phase11-hvc-console-validation-matrix.md` now records the first kernel-integration validation matrix for close-wait teardown parity, the landed remove-path handoff, the landed tty-registration handoff, and the still-pending notifier-facing and sysrq-facing follow-up without widening into host-backed I/O.
