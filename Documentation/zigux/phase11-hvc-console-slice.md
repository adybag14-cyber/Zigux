...
`Documentation/zigux/phase11-hvc-console-validation-matrix.md` now records the first kernel-integration validation matrix for close-wait teardown parity, the final-close teardown handoff, tty registration, the notifier-add open handoff, the landed khvcd polling-contract evidence, the khvcd worker-entry boundary, the khvcd sleep-and-reschedule handoff, the `__hvc_poll()` drain-order handoff, the `hvc_hangup()` disconnect handoff, the `hvc_remove()` teardown handoff, and the `hvc_cleanup()` tty-port release handoff without widening into host-backed I/O.
