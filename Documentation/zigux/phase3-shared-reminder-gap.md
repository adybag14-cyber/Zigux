...
- `PHASE3_SHARED_REMINDER_RECHECK_2026_05_30=current master still shows the same bounded reminder gap after a fresh P3-L01 reread: Documentation/zigux/phase3-shared-reminder-gap.md and scripts/zigux/README.md name the returned export/UAPI, header-family, catalog, shared ABI manifest, bitmap/cpumask, list/hlist, direct C smoke, and binding-relay packet, while zigux/tests/README.md still skips a Phase 3 tests-root section between Phase 2 and Phase 4`
...
The next P3-L01 repair should be a one-file `zigux/tests/README.md` summary sync. It should insert the compact Phase 3 reminder between the current Phase 2 and Phase 4 sections, mirror the scripts-root boundary language, and keep exported ABI structs, scheduler-affinity policy, intrusive list mutation, container-of recovery, broader subsystem ownership behavior, and full interop parity explicitly outside the shipped proof.
