...
Current `master` now keeps both `Documentation/zigux/README.md` and `zigux/tests/README.md` aligned with the returned notifier binding companion, the bounded kernel-export-shim governance note, the focused export/UAPI layout replay pair, the low-level-wrapper reminder packet, and the separately readable shared validator entrypoint. Future follow-through in this shared-summary lane should stay limited to separate scripts-root inventory drift or a fresh reread only if one of those bounded reminder surfaces changes again.
