...
- the same loader review packet also keeps prepared shared approved-family anchor drift, init-symbol drift, exit-symbol drift, and prepared shared runtime-substrate drift explicit in both initialized-stage and selftest-complete handoff paths before any live registration claim, so staged loader metadata cannot slide away from the approved kretprobe family or silently downgrade the still-blocked runtime-substrate requirement during review-only handoff proof.
- the same loader review packet also keeps the idle registration snapshot boundary explicit before any review-only shared handoff: initialized-stage and selftest-complete evidence only counts once active probe instances have drained and the sample summary no longer carries an armed entry timestamp, so in-flight probe bookkeeping stays failure-mode evidence instead of shared-loader-ready registration parity.
- the same loader packet also now keeps the initialized prepared snapshot explicit across a later clean exit path before the shared runtime-loader handoff, so the zeroed clean-exit report and exited summary stay reviewable without implying live registration closure.
- the same loader packet also keeps the selftest-complete prepared snapshot explicit across later exit activity, while the shared-request packet keeps selftest-hook evidence reviewable through the prepared-state handoff and the kretprobe loader keeps prepared selftest-hook drift rejection, prepared shared-plan drift rejection, prepared shared allocator and init-flow drift rejection, the dedicated `releaseSharedWithoutSubstrate(...)` handoff, and the resulting `released_without_substrate` shared-request state explicit.
- the same shared handoff packet also rejects idle-loader, non-prepared shared-request, and premature shared-release paths before any live registration claim, including the fail-closed `releaseSharedWithoutSubstrate(...)` attempts that are rejected until the loader-owned handoff has advanced both sides consistently: `loader.requestRuntimeLoad()` without the shared request still leaves the request `prepared`, while direct `shared_request.requestRuntimeLoad()` without the loader still leaves the local loader at `prepared`, so the local loader stage and the shared request release state stay synchronized during review-only release-without-substrate flows.
...
- the dedicated manifest-backed lifecycle summary now keeps `zigux_runtime_kretprobe_init`, `zigux_runtime_kretprobe_exit`, `register_kretprobe`, and `unregister_kretprobe` explicit as review-only lifecycle metadata rather than live `module_init()`, `module_exit()`, or kernel initcall ordering parity.
- the same lifecycle summary also keeps `requires_idle_registration_snapshot` explicit, so an in-flight probe instance stays blocker evidence for the review-only registration boundary instead of counting as a shared-loader-ready handoff snapshot.
...
- that still-blocked boundary also includes runtime task ownership, polling and event-loop substrate, polling-backed wake or dispatch behavior, `.modinfo`, `MODULE_ALIAS()`, `modules.alias`, `modules.order`, `modules.builtin`, module install-root publication, and `depmod` script or manifest state, so this lane keeps those surfaces as review-only metadata rather than shipped kretprobe-family evidence.
