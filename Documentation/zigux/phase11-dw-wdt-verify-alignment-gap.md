...
This note therefore no longer treats the returned validation matrix as absent from the current contents bridge. Instead it records that the current bridge now keeps the DesignWare matrix readable together with the manifest, the owner note, the PM helper, and the note-only continuity packet, while the broader verify-helper, direct-driver, survey, slice, and replay stack still remains outside that narrower direct-read packet.
...
- `scripts/zigux/check-phase11-dw-wdt-verify-alignment.py` should keep this narrower current-head packet fail-closed around the returned validation matrix rather than asserting direct readability for the broader verify-helper stack

## Next Bounded Same-Lane Step

- leave this note, `Documentation/zigux/phase11-dw-wdt-validation-matrix.md`, `Documentation/zigux/phase11-dw-wdt-platform-registration-plan.md`, `zigux/tests/phase11_dw_wdt_manifest.json`, `drivers/watchdog/dw_wdt_pm.zig`, and `scripts/zigux/check-phase11-dw-wdt-verify-alignment.py` parked unless the narrower authenticated-contents packet drifts again
