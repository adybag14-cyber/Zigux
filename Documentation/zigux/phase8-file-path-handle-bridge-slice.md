...
`bpf_get_map_info_from_fdinfo()` stayed the right starter because it keeps the landed work inside reviewable path formatting, fdinfo text parsing, and planning-only reuse gating without widening into direct procfs reads, pinned-map reopen flow, or object-loader state.
