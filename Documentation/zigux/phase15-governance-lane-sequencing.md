...
- current `master` already closed the earlier shared tests-root plus checker manifest-pair follow-through: `zigux/tests/README.md` now keeps `zigux/tests/phase15_handoff_next_steps_manifest.json` and `zigux/tests/phase15_readiness_gate_manifest.json` explicit beside the dedicated `make -C zigux phase15-test` route, and `scripts/zigux/check-phase15-scripts-readme-alignment.py` now fail-closes on both manifest markers, so future `P15-L06` runs should stay parked unless another shared-summary or shared build-wiring drift appears
...
1. keep `P15-L06` parked unless another shared-summary or shared build-wiring drift appears; current `master` already closed the earlier tests-root plus checker manifest-pair follow-through, so there is no standing shared-summary catch-up left in this packet today
