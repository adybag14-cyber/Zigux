...
- current repo reality: the core Phase 15 governance notes are landed, the dedicated review-process manifest is landed, the dedicated governance-lane sequencing manifest plus focused replay are landed, the focused parity-scorecard machine-readable companion plus focused replay are landed, the dedicated handoff manifest plus focused handoff-specific replay plus focused handoff-note checker are landed, the focused indefinite-C lane-owner companion is landed, the focused review-checklist study-only alignment checker is landed, the dedicated validator-first companion `scripts/zigux/validate-phase15.py` is directly materialized, the dedicated shared build companion `zigux/tests/phase15_build.zig` is now directly materialized, the dedicated deep-core blocker survey is now landed, and the remaining broader repo-reality gaps stay narrowed to missing Phase 15 make-wrapper and shared-CI route bodies rather than a missing build companion
