...
- the earlier docs-root parity-scorecard-survey omission is no longer the active shared-summary problem; the live 2026-05-14 reread shows `Documentation/zigux/README.md` already keeps that survey explicit, while the current docs-root drift is still one-file and narrower than a packet rewrite: the Phase 15 summary still under-specifies `Documentation/zigux/phase15-readiness-gate-survey.md`, `Documentation/zigux/phase15-handoff-next-steps-survey.md`, `Documentation/zigux/phase15-governance-lane-sequencing.md`, and the shipped `make -C zigux phase15-validate` plus `make -C zigux phase15-test` routes that `scripts/zigux/validate-phase15.py` already expects, while `zigux/tests/README.md` stays governed by the sequencing-marker plus replay-route packet instead of duplicating packet-local docs reminders
