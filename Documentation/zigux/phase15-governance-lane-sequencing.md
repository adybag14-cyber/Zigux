...
- the earlier docs-root parity-scorecard-survey omission is no longer the active shared-summary problem; the fresh 2026-05-15 reread still shows `Documentation/zigux/README.md` keeping the parity survey, the readiness and handoff notes, the governance-lane sequencing note, and the shipped `make -C zigux phase15-validate` plus `make -C zigux phase15-test` routes explicit, but the scripts-root summary reopened instead: `scripts/zigux/README.md` still describes `make -C zigux phase15-validate` as rerunning only `validate-phase15.py`, `check-phase15-scripts-readme-alignment.py`, and `check-phase15-review-process-handoff.py` even though current `zigux/Makefile` also reruns `check-phase15-docs-readme-alignment.py` plus `check-phase15-shared-summary-gap.py`, while `scripts/zigux/check-phase15-scripts-readme-alignment.py` already fails closed on that expanded validator-first route wording; shared-summary follow-through should therefore reopen only for that bounded scripts-root README repair, and the reread should still start with `python3 scripts/zigux/check-phase15-docs-readme-alignment.py`, `python3 scripts/zigux/check-phase15-scripts-readme-alignment.py`, `python3 scripts/zigux/check-phase15-shared-summary-gap.py`, and `python3 scripts/zigux/validate-phase15.py` before widening anywhere else, while `zigux/tests/README.md` stays governed by the sequencing-marker plus replay-route packet instead of duplicating packet-local docs reminders
...
- if a shared-summary truthfulness drift appears first instead, reread `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` against `Documentation/zigux/phase15-freeze-map-governance.md`, `Documentation/zigux/phase15-architecture-council-review-process.md`, `Documentation/zigux/phase15-parity-scorecard-survey.md`, `Documentation/zigux/phase15-parity-scorecard.md`, `Documentation/zigux/phase15-indefinite-c-policy.md`, `Documentation/zigux/phase15-readiness-gate-survey.md`, `Documentation/zigux/phase15-handoff-next-steps-survey.md`, `Documentation/zigux/phase15-governance-lane-sequencing.md`, `scripts/zigux/validate-phase15.py`, `scripts/zigux/check-phase15-docs-readme-alignment.py`, `scripts/zigux/check-phase15-scripts-readme-alignment.py`, `scripts/zigux/check-phase15-shared-summary-gap.py`, `zigux/tests/phase15_handoff_next_steps_manifest.json`, and `zigux/tests/phase15_readiness_gate_manifest.json`, then run `python3 scripts/zigux/check-phase15-docs-readme-alignment.py`, `python3 scripts/zigux/check-phase15-scripts-readme-alignment.py`, `python3 scripts/zigux/check-phase15-shared-summary-gap.py`, and `python3 scripts/zigux/validate-phase15.py` first, and keep the repair scoped to the first summary surface that actually drifted on current `master`; with the fresh 2026-05-15 packet, `Documentation/zigux/README.md` still keeps `Documentation/zigux/phase15-readiness-gate-survey.md`, `Documentation/zigux/phase15-handoff-next-steps-survey.md`, `Documentation/zigux/phase15-governance-lane-sequencing.md`, and the shipped `make -C zigux phase15-validate` plus `make -C zigux phase15-test` routes explicit, but `scripts/zigux/README.md` remains the first reopened shared-summary repair because it still undercounts the shipped `check-phase15-docs-readme-alignment.py` and `check-phase15-shared-summary-gap.py` runs inside `phase15-validate`, while `scripts/zigux/check-phase15-scripts-readme-alignment.py` already fails closed on that exact validator-route sentence; keep that follow-through scoped to the scripts-root README first, keep `Documentation/zigux/review-checklist.md` aligned in the same bounded shared-summary packet only if that same reread shows checklist drift too, and treat `zigux/tests/README.md` follow-through as a separate sequencing-marker or replay-route repair instead of widening it into duplicate docs-root packet-local reminders
