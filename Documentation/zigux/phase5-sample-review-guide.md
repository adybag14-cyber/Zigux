...
Fresh repo-first inspection on 2026-05-13 confirmed that current `master` still carries the bounded four-anchor Phase 5 packet together with its shared contributor surfaces, and the direct public-tree kobject evidence is restored again.
...
That same inspection also confirmed that the kobject anchor is back to a directly readable public-tree packet on current `master`: keep shared contributor wording aligned with `Documentation/zigux/phase5-kobject-sample-survey.md`, `samples/zigux/kobject_example.zig`, `zigux/tests/phase5_build.zig`, `zigux/tests/phase5_kobject_example.zig`, `zigux/tests/phase5_kobject_example_manifest.json`, and `zigux/tests/phase5_kobject_example_survey.zig`, and avoid repeating the older missing-path caveat.
...
* the approved Phase 5 packet on current `master` is still the same four-anchor packet, and all four sample-root anchors are now directly readable from `samples/zigux/`, with the kobject anchor also kept explicit across its paired note, focused test, manifest, dedicated survey replay, and shared build route
* shared docs that describe those anchors, their paired test packets, and the shared `phase5_build.zig` route should stay aligned with that exact landed packet instead of reviving older pre-landing wording or falling back to older missing-path caveats about the current kobject readback surface
...
* sample-local contract work belongs in exactly one landed packet at a time: the sample file under `samples/zigux/`, the paired survey note under `Documentation/zigux/`, the focused replay under `zigux/tests/phase5_*.zig`, the paired manifest, and the paired survey replay; for the current kobject anchor, keep `Documentation/zigux/phase5-kobject-sample-survey.md`, `samples/zigux/kobject_example.zig`, `zigux/tests/phase5_build.zig`, `zigux/tests/phase5_kobject_example.zig`, `zigux/tests/phase5_kobject_example_manifest.json`, and `zigux/tests/phase5_kobject_example_survey.zig` explicit together as the current readable kobject packet
...
* if a shared doc claims a sample-local replay route, the corresponding directly readable sample file, paired tests, paired manifest, and build entrypoint can all be read directly from the repo instead of being inferred from stale wording alone; for the current kobject anchor, keep `Documentation/zigux/phase5-kobject-sample-survey.md`, `samples/zigux/kobject_example.zig`, `zigux/tests/phase5_build.zig`, `zigux/tests/phase5_kobject_example.zig`, `zigux/tests/phase5_kobject_example_manifest.json`, and `zigux/tests/phase5_kobject_example_survey.zig` explicit together as the directly readable packet
...
Review the current readable kobject packet through `Documentation/zigux/phase5-kobject-sample-survey.md`, `samples/zigux/kobject_example.zig`, `zigux/tests/phase5_build.zig`, `zigux/tests/phase5_kobject_example.zig`, `zigux/tests/phase5_kobject_example_manifest.json`, and `zigux/tests/phase5_kobject_example_survey.zig`.

Keep the sample root, the paired survey note, the focused replay, the manifest, the dedicated survey replay, and the shared `zigux/tests/phase5_build.zig` route explicit together as the current readable kobject packet on `master`.
...
Keep `scripts/zigux/README.md` honest here too: the scripts-root Phase 5 summary may stay generic at the four-sample level, but it still needs to keep `Documentation/zigux/phase5-kobject-sample-survey.md`, `samples/zigux/kobject_example.zig`, `zigux/tests/phase5_build.zig`, `zigux/tests/phase5_kobject_example.zig`, `zigux/tests/phase5_kobject_example_manifest.json`, and `zigux/tests/phase5_kobject_example_survey.zig` directly readable together as the exact current kobject packet beside the shared replay route instead of flattening this sample back to an unnamed summary or reverting to older missing-path caveats.
