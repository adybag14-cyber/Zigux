...
Fresh GitHub-app readback on 2026-05-13 in this run confirmed that current `master` still carries the bounded four-anchor Phase 5 packet together with its shared contributor surfaces, but the directly readable sample-local evidence is mixed rather than fully restored across every anchor.
...
The remaining shared-surface drift still waiting its own one-file follow-through is:

* `Documentation/zigux/README.md`, which still frames `Documentation/zigux/phase5-kretprobe-sample-survey.md` as if it recorded a landed `samples/zigux/kretprobe_example.zig` replay packet on current `master`
* `scripts/zigux/README.md`, which still lists the missing `samples/zigux/kretprobe_example.zig`, `zigux/tests/phase5_kretprobe_example*.zig`, and `zigux/tests/phase5_build.zig` packet as current shared evidence while understating the directly readable kobject packet that did return
* `zigux/tests/README.md`, which still lists the missing `phase5_build.zig` and `phase5_kretprobe_example*` tests-root packet as current direct evidence
* `zigux/Makefile` and `.github/workflows/zigux-bootstrap.yml`, which still route shared Phase 5 execution through the missing `zigux/tests/phase5_build.zig` packet
...
* within that shared set, `Documentation/zigux/phase5-sample-review-guide.md`, `Documentation/zigux/phase5-kretprobe-sample-survey.md`, `Documentation/zigux/review-checklist.md`, and `samples/zigux/README.md` already reflect the survey-note-only kretprobe posture, while `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml` still need their own bounded follow-through before they should be reused as source text for kretprobe or shared-build claims
...
* do not borrow kretprobe or shared-build wording from `Documentation/zigux/README.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, `zigux/Makefile`, or `.github/workflows/zigux-bootstrap.yml` until those drifted surfaces are repaired or a fresh reread proves the missing `samples/zigux/kretprobe_example.zig`, `zigux/tests/phase5_kretprobe_example*`, and `zigux/tests/phase5_build.zig` packet returned
* keep the bytestream packet aligned with `Documentation/zigux/phase5-kfifo-sample-survey.md` plus `samples/zigux/bytestream_fifo.zig` until the older tests-root and shared-build companion paths are directly readable again
...
Keep `StorageBacking.embedded_fixed_buffer`, `previewInto()`, `snapshotInto()`, `runPreviewBoundaryReplay()`, `runWrappedPreviewReplay()`, the replay-visible preview markers, the short-drain `"hel"` plus queued `"lo"` helper boundary, the explicit queue-shape helpers around `available()`, `visibleSpanSummary()`, and `usesWrappedStorageWindow()`, and the `init()` -> `runAnchorReplay()` -> `exit()` ownership path explicit together as one bounded non-runtime packet.
