...
For the bytestream anchor, keep the split explicit: `Documentation/zigux/phase5-kfifo-sample-survey.md` now preserves current public-tree blob readback for those remaining bytestream companion files even though authenticated contents readback still fails. Keep shared contributor wording aligned with that mixed packet instead of flattening every anchor into the same restored or gap-only posture.
