...
Fresh repo-first inspection on 2026-05-13 confirmed that current `master` still carries the bounded four-anchor Phase 5 packet together with its shared contributor surfaces, while the kobject anchor remains a narrower docs-and-tests packet in direct readback.
...
* the approved Phase 5 packet on current `master` is still the same four-anchor packet, but only `samples/zigux/bytestream_fifo.zig`, `samples/zigux/kretprobe_example.zig`, and `samples/zigux/trace_events_sample.zig` are directly readable from `samples/zigux/`; keep the kobject anchor explicit through `Documentation/zigux/phase5-kobject-sample-survey.md`, `zigux/tests/phase5_kobject_example.zig`, and `zigux/tests/phase5_kobject_example_manifest.json` together with the shared `zigux/tests/phase5_build.zig` route
* shared docs that describe those anchors, their paired test packets, and the shared `phase5_build.zig` route should stay aligned with that same split packet instead of reviving broader restored-public-tree wording or relying on stale kobject sample-root claims
