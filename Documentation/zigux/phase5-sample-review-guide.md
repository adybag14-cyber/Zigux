...
* `scripts/zigux/README.md`, which still lists the missing `samples/zigux/kretprobe_example.zig`, `zigux/tests/phase5_kretprobe_example*.zig`, and `zigux/tests/phase5_build.zig` packet as current shared evidence while also treating the missing bytestream-focused tests-root packet as shipped and understating the narrower directly readable kobject packet that did return; current `zigux/Makefile` no longer exposes `phase5-test` or `phase5` wrappers and `.github/workflows/zigux-bootstrap.yml` no longer carries a dedicated Phase 5 replay step, so those two surfaces are no longer the active shared-route drift in this lane
...
* within that shared set, `Documentation/zigux/phase5-sample-review-guide.md`, `Documentation/zigux/phase5-kretprobe-sample-survey.md`, `Documentation/zigux/review-checklist.md`, `Documentation/zigux/README.md`, `samples/zigux/README.md`, and `zigux/tests/README.md` already reflect the survey-note-only kretprobe posture, while `scripts/zigux/README.md` still needs its own bounded follow-through before it should be reused as source text for kretprobe or shared-build claims
...
* do not borrow kretprobe or shared-build wording from `scripts/zigux/README.md` until that drifted surface is repaired or a fresh reread proves the missing `samples/zigux/kretprobe_example.zig`, `zigux/tests/phase5_kretprobe_example*`, and `zigux/tests/phase5_build.zig` packet returned
