...
`Documentation/zigux/phase5-kobject-sample-survey.md` is directly readable again on current `master` and should stay grouped with the shared reminder packet while the remaining tests-root companion pair still depends on fresh public current-`master` fallback in this runtime:
...
`Documentation/zigux/phase5-kobject-sample-survey.md` is the current directly readable shared survey note for this packet.
