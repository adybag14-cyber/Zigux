...
- scope: verify the bounded `nvme_pci` Zig starter around queue-pair planning, PRP buffer-shape accounting, reset summaries, verifier-backed starter checks, and frozen queue-restore budgeting without widening into live DMA mapping, PRP or SGL construction, blk-mq submission, interrupt routing, or transport-backed reset recovery
...
- current `master` now carries `zigux/tests/phase12_nvme_pci.zig` as the direct bounded replay for the queue planner and PRP-shape starter
- current `master` now carries `zigux/tests/phase12_nvme_pci_manifest.json` as the lane-local roadmap-gap manifest
- current `master` still carries `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md` and `Documentation/zigux/phase12-nvme-pci-reopen-governance.md`, so the bounded starter-plus-verifier packet remains explicit even while the broader slice note and survey gate remain absent
...
Those checks mean the current lane now has a truthful survey note for the existing NVMe starter packet, but it is still intentionally below any live DMA-backed queue execution, timeout recovery, or transport-backed reset claim.
...
- current `master` now carries `drivers/nvme/host/pci.zig`, and the current starter keeps queue-pair sizing, host-DMA budgeting, PRP buffer-shape pressure, reset freeze state, and frozen queue-restore budgeting reviewable
- current `master` now carries the bounded verifier shard, the direct replay, the manifest anchor, the fallback map, the reopen-governance note, and this survey note, so the starter is directly reviewable through driver-local surfaces
...
The next honest same-lane move is a survey-packet follow-through, not a transport-heavy implementation jump.

The next bounded step is:

1. keep the current starter focused on queue-pair planning, PRP buffer-shape accounting, reset summaries, verifier-backed checks, and frozen queue-restore budgeting instead of widening into live DMA or runtime PCI work
2. pair this survey note with the still-missing `Documentation/zigux/phase12-nvme-pci-slice.md` and `zigux/tests/phase12_nvme_pci_survey.zig` surfaces before claiming a fuller direct NVMe review packet
