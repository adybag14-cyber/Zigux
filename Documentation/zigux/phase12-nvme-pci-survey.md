...
- that pinned verification head did not widen the NVMe PCI starter beyond the earlier queue-planner, PRP buffer-shape, and pointer-selection packet, which means the survey's roadmap gap remains transport- and recovery-heavy rather than merely stale.
...
- the shared Phase 12 validator path on that same pinned head was already broken before runtime with `SyntaxError: '{' was never closed` at `scripts/zigux/validate-phase12.py:239`.
- the shared Phase 12 Zig replay on the same pinned head stopped at `phase12-virtio-net-survey-tests` with `zigux/tests/phase12_virtio_net_survey.zig:237:83: error: expected ')', found 'EOF'`, and the bundle result there was `Build Summary: 14/17 steps succeeded (1 failed); 52/52 tests passed`.
- even with that cross-lane stop, the shared bundle still reached `phase12-nvme-pci-tests 9 pass (9 total)` and `phase12-nvme-pci-survey-tests 1 pass (1 total)` before the unrelated `virtio_net` survey compile failure.
