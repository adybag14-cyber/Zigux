...
- `PHASE14_VALIDATION_SENTINEL=scripts\zigux/check_phase14_ring_buffer_shared_route_gap.zig`

## Current Evidence

The Phase 14 manifest now records that `scripts\zigux/validate_phase14.zig` runs
`scripts\zigux/check_phase14_ring_buffer_compile_route.zig`, and that the shared
...
The remaining gap is narrower: `scripts\zigux/check_phase14_shared_smoke_route.zig`
...
`scripts\zigux/check_phase14_shared_smoke_route.zig` to require:
...
`scripts\zigux/check_phase14_ring_buffer_shared_route_gap.zig` should be retired or
