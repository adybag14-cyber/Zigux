...
- scope: roadmap traceability, shared helper replay entrypoints, the four manifest-backed survey packets already present on `master`, the adjacent notifier-list reviewability packet plus its landed read-only generic notifier foothold and dedicated exported C header, and the explicit helper-only `devres` DMA/scatterlist boundary plus its adjacent coherent DMA and `devm_iounmap()` reviewability replays
- product boundary:
...
- the shared release packet now also needs to say plainly that the dedicated `phase13-devres-iounmap-reviewability-tests` step is part of the same shared replay packet instead of floating only in `zigux/tests/phase13_build.zig`
