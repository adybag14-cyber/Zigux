...
At the same time, fresh direct readback narrows but does not fully close the older reminder drift. `Documentation/zigux/phase13-devres-slice.md`, `Documentation/zigux/phase13-devres-survey.md`, `Documentation/zigux/phase13-roadmap-traceability.md`, `lib/devres.zig`, and `zigux/tests/phase13_devres_dma_coherent.zig` confirm that the helper-local packet now includes the direct DMA-boundary replay beside the planner, the pure `dmam_alloc_coherent()` planning helper, and the scatterlist packet. Current `master` now keeps `Documentation/zigux/phase13-contributor-workflow-guide.md`, `Documentation/zigux/phase13-shared-helper-lane-sequencing.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, and `zigux/tests/README.md` aligned with that direct `devres` packet, while `Documentation/zigux/phase10-phase11-phase13-contributor-surface-sync.md` and `Documentation/zigux/README.md` still frame `Documentation/zigux/phase13-devres-survey.md` and `lib/devres.zig` as repo-reality gaps. The still-missing `Documentation/zigux/phase13-landlock-syscalls-survey.md`, `zigux/tests/phase13_landlock_syscalls.zig`, `zigux/tests/phase13_landlock_syscalls_reviewability.zig`, and `zigux/tests/phase13_landlock_syscalls_manifest.json` remain separate repo-reality gaps rather than shipped evidence.

Keep this survey truthful about that narrower docs-root and contributor-sync `devres` wording drift instead of claiming that the broader shared-summary packet is already aligned.
...
Keep broad release wording tied to that reminder packet while the missing validator-first helpers and missing shared build route surfaces remain explicit repo-reality gaps, and keep the remaining docs-root and contributor-sync `devres` wording drift explicit until those surfaces stop describing `Documentation/zigux/phase13-devres-survey.md` and `lib/devres.zig` as missing.
...
Current `master` still shows one release-surface drift inside the broader reminder packet: `Documentation/zigux/README.md` and `Documentation/zigux/phase10-phase11-phase13-contributor-surface-sync.md` still frame `Documentation/zigux/phase13-devres-survey.md` and `lib/devres.zig` as repo-reality gaps even though the current `devres` slice, survey, roadmap traceability note, helper file, and DMA-boundary replay now materialize them. `Documentation/zigux/phase13-contributor-workflow-guide.md`, `Documentation/zigux/phase13-shared-helper-lane-sequencing.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, `zigux/tests/README.md`, and `scripts/zigux/README.md` stay aligned with the narrower current packet, so the still-open follow-through is now a smaller docs-root and contributor-sync repair rather than the older whole-packet drift.

Keep only that docs-root and contributor-sync `devres` wording drift framed as open survey drift until a same-lane reread resolves it or the missing direct `devres` and Landlock syscall packet members actually rematerialize.
...
- the release-note packet should keep any remaining docs-root and contributor-sync follow-through explicit while the missing direct Landlock syscall companions stay recorded as repo-reality gaps
