...
- `landlock/ruleset` stays helper-local through its ownership, slice, survey, source, manifest-backed replay surfaces, and dedicated packet checker, while `landlock/syscalls` stays helper-local through its governance note, slice, survey, packet checker, and source starter without collapsing into docs-only governance metadata; the direct syscall replay, reviewability, and manifest companions still stay recorded as repo-reality gaps
