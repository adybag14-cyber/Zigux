...
Broad summaries should use that compact coordination companion to keep the active-not-closed packet, the shared validator-first replay handle, and the current repo-reality gaps explicit without inventing a broader shared build route. Current `master` now keeps the shared contributor-surface sync note, the shared tests-root review companion, the broader tests-root guide, the scripts-root reminder, and the compact PMO coordination packet aligned around the shipped `Documentation/zigux/phase13-libfs-survey.md`, `zigux/tests/phase13_libfs.zig`, `zigux/tests/phase13_libfs_reviewability.zig`, `Documentation/zigux/phase13-devres-slice.md`, `zigux/tests/phase13_devres.zig`, `zigux/tests/phase13_devres_reviewability.zig`, `zigux/tests/phase13_devres_boundary_evidence.zig`, the shipped Landlock helper packet, and adjacent notifier evidence, but `Documentation/zigux/README.md` still lists `make -C zigux phase13` inside the shipped Phase 13 packet summary without restating that it is blocked convenience wiring while `zigux/tests/phase13_build.zig` remains absent. The next same-lane broad-summary follow-through should therefore stay narrowed to that docs-root command-posture repair rather than reopening the now-closed scripts-root undercount or replaying the already-aligned tests-root reminder.
