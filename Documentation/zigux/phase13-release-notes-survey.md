...
Current `master` still shows one release-surface drift inside the broader reminder packet: `Documentation/zigux/README.md` still frames `Documentation/zigux/phase13-devres-survey.md` and `lib/devres.zig` as repo-reality gaps even though the current `devres` slice, survey, roadmap traceability note, helper file, and DMA-boundary replay now materialize them. `Documentation/zigux/phase10-phase11-phase13-contributor-surface-sync.md`, `Documentation/zigux/phase13-contributor-workflow-guide.md`, `Documentation/zigux/phase13-shared-helper-lane-sequencing.md`, `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md`, `zigux/tests/README.md`, and `scripts/zigux/README.md` stay aligned with the narrower current packet, so the still-open follow-through is now a smaller docs-root repair rather than the older whole-packet drift.

Keep only that docs-root `devres` wording drift framed as open survey drift until a same-lane reread resolves it or the missing direct `devres` and Landlock syscall packet members actually rematerialize.
