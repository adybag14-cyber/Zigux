...
At the same time, fresh direct readback now keeps the broader docs-root reminder aligned with that narrower packet. `Documentation/zigux/README.md`, `Documentation/zigux/phase10-phase11-phase13-contributor-surface-sync.md`, `Documentation/zigux/phase13-contributor-workflow-guide.md`, `Documentation/zigux/phase13-shared-helper-lane-sequencing.md`, and `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md` now keep `Documentation/zigux/phase13-devres-survey.md` and `lib/devres.zig` explicit as shipped current-`master` `devres` evidence, while the still-missing `Documentation/zigux/phase13-landlock-syscalls-survey.md`, `zigux/tests/phase13_landlock_syscalls.zig`, `zigux/tests/phase13_landlock_syscalls_reviewability.zig`, and `zigux/tests/phase13_landlock_syscalls_manifest.json` remain separate repo-reality gaps rather than shipped evidence. `zigux/tests/README.md` stays inside the same shared handle and does keep `Documentation/zigux/phase13-libfs-survey.md` recorded as a repo-reality gap, but fresh direct reread in this run shows that the tests-root Phase 13 reminder plus `scripts/zigux/check-phase13-tests-readme-alignment.py` still undercount the returned `scripts/zigux/validate-phase13-release.py`: both still describe that release-discipline validator as missing even though current `master` now materializes it. `scripts/zigux/README.md` stays inside the same shared handle too, but fresh direct reread in this run separately leaves one parked scripts-root `libfs` reminder drift: that Phase 13 section names the shipped `Documentation/zigux/phase13-libfs-slice.md` packet without also keeping missing `Documentation/zigux/phase13-libfs-survey.md` and `zigux/tests/phase13_libfs_addressability.zig` explicit as repo-reality gaps. Keep the tests-root validator-reminder sync and the parked scripts-root `libfs` sentence recorded as distinct follow-through items instead of carrying older broader docs-root drift forward.
...
Current `master` now keeps `Documentation/zigux/README.md`, `Documentation/zigux/phase10-phase11-phase13-contributor-surface-sync.md`, `Documentation/zigux/phase13-contributor-workflow-guide.md`, `Documentation/zigux/phase13-shared-helper-lane-sequencing.md`, and `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md` aligned around the narrower current `devres` packet while still keeping the shipped helper-first `libfs` slice plus tests-root packet explicit and `Documentation/zigux/phase13-libfs-survey.md` recorded as a repo-reality gap. `zigux/tests/README.md` keeps that `libfs` gap explicit too, but fresh direct reread in this run also shows that `zigux/tests/README.md` plus `scripts/zigux/check-phase13-tests-readme-alignment.py` still undercount the returned `scripts/zigux/validate-phase13-release.py` by leaving it in repo-reality-gap wording. `scripts/zigux/README.md` separately remains a same-family reminder drift because its Phase 13 section names the shipped `Documentation/zigux/phase13-libfs-slice.md` packet without keeping `Documentation/zigux/phase13-libfs-survey.md` and `zigux/tests/phase13_libfs_addressability.zig` explicit as repo-reality gaps. `Documentation/zigux/phase13-notifier-summary-gap.md` keeps the missing notifier-chain helper and notifier header recorded beside the shipped adjacent notifier evidence, so the remaining follow-through now stays limited to those two narrower reminder repairs rather than the old docs-root repair.
