...
At the same time, fresh direct readback now shows the broader reminder packet is only partly planner-expanded. `Documentation/zigux/phase13-contributor-workflow-guide.md`, `Documentation/zigux/phase10-phase11-phase13-contributor-surface-sync.md`, and `Documentation/zigux/phase13-shared-helper-lane-sequencing.md` already keep `Documentation/zigux/phase13-devres-scatterlist-planner.md` and `zigux/tests/phase13_devres_scatterlist_planner_manifest.json` explicit beside `Documentation/zigux/phase13-devres-survey.md` and `lib/devres.zig`, while `Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md` still describes the narrower current `devres` packet without consistently mirroring both planner surfaces yet. The still-missing `Documentation/zigux/phase13-landlock-syscalls-survey.md`, `zigux/tests/phase13_landlock_syscalls.zig`, `zigux/tests/phase13_landlock_syscalls_reviewability.zig`, and `zigux/tests/phase13_landlock_syscalls_manifest.json` remain separate repo-reality gaps rather than shipped evidence. `scripts/zigux/README.md` stays inside the same shared handle too, and fresh direct reread in this run keeps the shipped `Documentation/zigux/phase13-libfs-slice.md` packet explicit while also keeping missing `Documentation/zigux/phase13-libfs-survey.md` and `zigux/tests/phase13_libfs_addressability.zig` recorded as repo-reality gaps. `zigux/tests/README.md` keeps the shared-summary guard, the returned `zigux/Makefile` posture, and shipped `scripts/zigux/validate-phase13-release.py` explicit, so keep the broader tests-root reminder aligned with that current packet instead of carrying forward the older validator-gap story.
