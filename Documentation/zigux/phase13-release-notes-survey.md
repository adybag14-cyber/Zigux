...
- `landlock/ruleset` stays helper-local through its survey, source, manifest-backed replay surfaces, and dedicated packet checker, while the direct `Documentation/zigux/phase13-landlock-ruleset-ownership.md` and `Documentation/zigux/phase13-landlock-ruleset-slice.md` note pair still remains repo-reality-gap evidence; `landlock/syscalls` stays helper-local through its governance note, slice, survey, packet checker, and source starter without collapsing into docs-only governance metadata, and the direct syscall replay, reviewability, and manifest companions still stay recorded as repo-reality gaps
