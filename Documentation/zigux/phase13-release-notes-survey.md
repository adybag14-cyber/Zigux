...
- the shared release packet also keeps the dedicated `scripts/zigux/check-phase13-libfs-packet.py`, `scripts/zigux/check-phase13-devres-packet.py`, and `scripts/zigux/check-phase13-notifier-packet.py` guards visible as part of that published review path, so the libfs helper packet, the stricter helper-first `devres` boundary contract, and the adjacent notifier reviewability packet are not left implicit in packet-local notes or the shared build alone
...
- the shared replay inventory now also keeps the dedicated `scripts/zigux/check-phase13-libfs-packet.py`, `scripts/zigux/check-phase13-devres-packet.py`, and `scripts/zigux/check-phase13-notifier-packet.py` guards visible, so the validator-first route still names the libfs helper packet, the stricter helper-first `devres` boundary contract, and the adjacent notifier reviewability packet before the fifteen-step replay bundle runs
...
- the shared release packet now also keeps the dedicated `scripts/zigux/check-phase13-libfs-packet.py`, `scripts/zigux/check-phase13-devres-packet.py`, and `scripts/zigux/check-phase13-notifier-packet.py` guards visible, so the libfs helper packet, the stricter helper-first `devres` boundary contract, and the adjacent notifier reviewability packet stay reviewable instead of hiding only in packet-local notes or the shared build
