...
- the helper packet now makes two lane-local fail-closed policy edges explicit as well: empty-access level-zero extensions are rejected the same way new-rule insertion is, and tree-link planning refuses retained root-search state or attachment plans with no recorded descent before any root or parent link mode is treated as reviewable.
