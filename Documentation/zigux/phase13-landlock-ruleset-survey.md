...
- scope: the shipped `security/landlock/ruleset.zig` helper starter, the shipped `Documentation/zigux/phase13-landlock-ruleset-slice.md` boundary note, the direct `zigux/tests/phase13_landlock_ruleset.zig` replay, the manifest-backed ruleset packet, the paired ownership note, the dedicated packet checker, and the still-missing shared-build companion that would be required before this packet could claim a broader replay bundle
- product boundary:
...
Current `master` already ships a small ruleset helper starter, a helper-local slice note, a paired ownership note, a direct replay, a manifest-backed packet, and a dedicated packet checker. The highest-value bounded work in this lane is therefore to keep that shipped helper packet rereadable as its own helper-local surface instead of leaving the ruleset boundary implied through the ownership note and broader release-facing reminders alone.
...
- the shipped `Documentation/zigux/phase13-landlock-ruleset-slice.md` note and the paired ownership note now keep that helper-only packet explicit beside the survey, including the current helper planners, the direct replay, the packet checker, and the blocked live-state boundaries.
- the direct `zigux/tests/phase13_landlock_ruleset.zig` replay now proves the descriptor contract including `provides_insert_rule_branch_planning`, the no-match tree-link planner, the matched-rule replacement planner, and the manifest-backed packet markers without widening into live rb-tree ownership.
- `zigux/tests/phase13_landlock_ruleset_manifest.json` now records the same bounded packet truthfully: it keeps the ruleset helper starter, the helper-local slice note, the direct replay, and the dedicated packet checker marked as landed while the older shared `zigux/tests/phase13_build.zig` surface plus live tree and hierarchy state stay blocked.
...
- the shipped slice note and ownership note keep `security/landlock/ruleset.zig`, `zigux/tests/phase13_landlock_ruleset.zig`, `zigux/tests/phase13_landlock_ruleset_manifest.json`, and `scripts/zigux/check-phase13-landlock-ruleset-packet.py` grouped as one helper-local packet while still calling out the blocked shared-build and live-state follow-through.
- the shipped direct replay now checks the descriptor contract including `provides_insert_rule_branch_planning`, the no-match tree-link path, the matched-rule replacement path, and the manifest packet markers while keeping the `rb_replace_node()` signal reviewable as a planner output rather than a live mutation.
- the shipped manifest still records `"current_phase13_build_present": false`, `"current_ruleset_zig_present": true`, `"current_phase13_landlock_ruleset_slice_present": true`, `"current_phase13_landlock_ruleset_test_present": true`, `"current_landlock_ruleset_packet_checker_present": true`, `"status": "blocked_on_live_tree_state"`, and `"status": "blocked_on_hierarchy_lifetime"`, so the packet remains explicit about the still-blocked live-tree and hierarchy boundaries.
