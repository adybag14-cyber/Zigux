...
- the new `security/landlock/ruleset.zig` starter stays intentionally narrow around `landlock_create_ruleset()` planning, handled-access unioning, per-layer mask initialization, and `landlock_unmask_layers()` bit clearing.
- the starter does not claim object references, locking, rule insertion, hierarchy allocation, workqueue-backed deferred frees, or interaction with `security/landlock/syscalls.c`.
- the next honest ruleset-facing step is one small planner around `insert_rule()` access extension versus merged-layer intersection, still in-memory and still outside rb-tree storage and hierarchy lifetime.
...
- ready-next `phase13-landlock-rule-layer-merge-followup`
- blocked `phase13-landlock-live-tree-and-hierarchy-state`

This keeps the lane explicit without overstating progress: Zigux now has a real `ruleset.zig` helper foothold for access-mask accounting, but it still does not claim live rule storage, hierarchy ownership, or full Landlock policy enforcement.
...
Stay in the Phase 13 landlock ruleset lane and add one tiny `security/landlock/ruleset.zig` rule-layer merge planner next, limited to `insert_rule()` access extension versus merged-layer intersection before any rb-tree storage, hierarchy lifetime, or live LSM state is attempted.
