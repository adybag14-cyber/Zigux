...
This keeps the lane explicit without overstating progress: Zigux now has a real `ruleset.zig` helper foothold for access-mask accounting, build-check-style capacity reviewability, matching-rule insertion planning, tree-search outcome planning, no-match tree-link planning, helper-only `create_rule()` materialization planning, and helper-only `free_rule()` release planning, but it still does not claim live rule storage, hierarchy ownership, or full Landlock policy enforcement.
