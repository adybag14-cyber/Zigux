...
Current `master` already ships a small ruleset helper starter, a helper-local slice note, a direct replay, a manifest-backed packet, and a dedicated packet checker. The highest-value bounded work in this lane is therefore to keep that shipped helper packet rereadable as its own helper-local surface instead of leaving the ruleset boundary implied through the ownership note and broader release-facing reminders alone.
...
- the shipped `Documentation/zigux/phase13-landlock-ruleset-slice.md` note now keeps that helper-only packet explicit beside the ownership note and survey, including the current helper planners, the direct replay, the packet checker, and the blocked live-state boundaries.
...
- the immediate repo-reality gap versus the roadmap is therefore not an absent ruleset helper packet. It is the still-missing shared-build companion around the helper, slice, replay, manifest, checker, and survey surfaces that current `master` already ships.
...
- the shipped slice note now keeps `security/landlock/ruleset.zig`, `zigux/tests/phase13_landlock_ruleset.zig`, `zigux/tests/phase13_landlock_ruleset_manifest.json`, and `scripts/zigux/check-phase13-landlock-ruleset-packet.py` grouped as one helper-local packet while still calling out the blocked shared-build and live-state follow-through.
