...
- scope: the shipped helper-first libfs packet plus the landed dcache cursor precondition and cursor-release side packets, keeping helper-local planner evidence explicit while the older shared Phase 13 build route and live filesystem mutation remain out of scope
- product boundary:
  - `fs/libfs.zig`
...
- landed `phase13-libfs-dcache-cursor-release-planner`
...
- blocked `phase13-libfs-live-cursor-unlink`
- blocked `phase13-libfs-live-cursor-release-lock-ordering`

This keeps the lane explicit without understating shipped progress: current `master` exposes the direct helper, focused replay, reviewability gate, packet checker, the cursor-precondition side packet, and the new cursor-release side packet, while the shared Phase 13 build route plus live filesystem mutation remain out of scope.
...
- live cursor sibling traversal, private cursor relocation, or private cursor unlinking
- close-path lock ordering or shared release-surface ownership for unrelated Phase 13 helpers

## Next bounded step

If the libfs family reopens, prefer one packet-local follow-through: reread the libfs survey, slice, and manifest together and close the smallest wording drift before widening into deeper teardown semantics. Keep verification-only replay work on `P13-L03`.
