...
- scope: the landed `fs/libfs.zig` helper starter, its dedicated Phase 13 tests, the shared Phase 13 build wiring, the focused `generic_check_addressable()` helper proof, and the lane notes that compare the current helper-surface footing against the roadmap
- product boundary:
  - `fs/libfs.zig`
...
- the current starter stays intentionally narrow around `simple_statfs()` defaults, the `always_delete_dentry()` policy, the branch decisions inside `simple_lookup()`, the pure buffer-copy helper trio, the early `dcache_dir_lseek()` and `offset_dir_llseek()` seek-policy surface, one tiny `dcache_readdir()`-adjacent emit planner, the pure `dcache_dir_open()` cursor-setup planner, the pure `dcache_dir_close()` cursor-release planner, the pure `simple_transaction_get()` acquire planner, the pure `simple_transaction_set()` publish planner, the pure `simple_transaction_release()` lifetime planner, the pure `generic_check_addressable()` planner around shift overflow, zero-block passthrough, minimum block size, and explicit sector or page-index caps, and the pure `simple_open()` planner around the `inode->i_private` to `file->private_data` handoff.
...
- directory cursor helpers such as the now-landed `dcache_dir_open()` and `dcache_dir_close()` pair still stay below the deeper cursor-backed `dcache_readdir()` traversal, which remains riskier because it depends on cursor dentries, sibling lists, lock ordering, and reschedule-aware traversal.
...
This keeps the lane explicit without overstating progress: Zigux now has a real `fs/libfs.zig` helper foothold plus a reviewability checkpoint, one pure capacity-checking helper, the bounded `dcache_dir_open()` and `dcache_dir_close()` cursor pair, and the bounded `simple_open()` private-data handoff, but it still does not claim live dcache parity, pseudo-filesystem mounting, inode lifecycle work, rename-state behavior, or cursor-backed directory traversal.
