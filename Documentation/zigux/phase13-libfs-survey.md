...
- the current starter stays intentionally narrow around `simple_statfs()` defaults, the `always_delete_dentry()` policy, the branch decisions inside `simple_lookup()`, the pure buffer-copy helper trio, the early `dcache_dir_lseek()` and `offset_dir_llseek()` seek-policy surface, one tiny `dcache_readdir()`-adjacent emit planner, and the pure `simple_transaction_get()` acquire planner.
...
Stay in the Phase 13 libfs lane and add one tiny `fs/libfs.zig` transaction publish helper next, limited to reviewable `simple_transaction_set()` response-size validation and publish bookkeeping before any live readback, release, cursor dentry, inode, or pseudo-filesystem work.
