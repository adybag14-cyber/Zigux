...
This keeps the lane concrete and reviewable without overstating MMIO progress: the queue-facing footholds are real, the first MMIO register-window step is now the next honest move, and the broader transport-facing lifecycle work is still intentionally blocked.
...
Stay in the broader Phase 10 MMIO lane and add one small `virtio_mmio` register-window helper next without claiming queue setup, IRQ, probe, or remove parity.
