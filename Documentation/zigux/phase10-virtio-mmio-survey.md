...
This document tracks the bounded Phase 10 survey lane around `drivers/virtio/virtio_mmio.c` and the first landed follow-on helper.

## Status

- `PHASE10_STATUS=active`
- `PHASE10_SLICE=virtio-mmio-survey`
- scope: survey manifest, dedicated survey gate, shared Phase 10 build wiring, a lane-level note that records what is present in the live repo plus the remaining MMIO transport gap against the roadmap, and the first tiny MMIO register-window helper that now makes the next queue-register gap concrete
- product boundary:
...
- the landed helper stays intentionally narrow: it models MMIO register offsets, bounded feature-page selection, status and reset bookkeeping, config-generation tracking, and interrupt-ack bookkeeping only.
- this means the roadmap's "virtqueue wrappers first, MMIO wrappers later" rule is now satisfied for one tiny MMIO foothold, and the next honest follow-up is a queue-register planning helper rather than probe, remove, DMA, or broader lifecycle glue.
...
This keeps the lane concrete and reviewable without overstating MMIO progress: the queue-facing footholds are real, one tiny MMIO register-window step is now landed, and the broader transport-facing lifecycle work is still intentionally blocked.
...
Stay in the broader Phase 10 MMIO lane and add one small queue-register helper next without claiming queue setup, IRQ delivery, probe, or remove parity.
