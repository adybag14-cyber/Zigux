...
- the live helper also still hard-codes its magic, version, and vendor assumptions inside the bounded probe-preflight path rather than exposing one explicit transport-identity snapshot, so the next reviewable lab-driver gap against the roadmap is a transport-safe identity helper rather than broader lifecycle work.
- this means the roadmap's \"virtqueue wrappers first, MMIO wrappers later\" rule now has a real bounded config-window foothold, one small lab-only config-write planning foothold, one small lab-only config-write disposition foothold, and one early probe-preflight observation foothold, while the next honest follow-on remains a transport-identity snapshot and the riskier transport and lifecycle paths remain intentionally blocked.
...
- the ready-next `phase10-mmio-transport-identity-helper`
- the still-blocked `phase10-mmio-lifecycle-and-irq-paths`

This keeps the lane concrete and reviewable without overstating MMIO progress: the queue-facing footholds are real, the shared Phase 10 packet now acknowledges the bounded register-window, queue-size, feature-word, config-window, config-write planning, config-write disposition, and probe-preflight surfaces honestly, including the shorter-restage stale-data regression proof, the next transport-safe roadmap gap is now named as a transport-identity snapshot helper, and the broader transport-facing lifecycle work is still intentionally blocked.
...
Keep the Phase 10 MMIO lane narrow and land one transport-identity snapshot helper that makes the current magic, version, and vendor assumptions reviewable through a shared observation surface before widening into interrupt acknowledgement, queue discovery, reset paths, or probe lifecycle work.