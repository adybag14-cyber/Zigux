...
- this means the roadmap's `virtqueue wrappers first, MMIO wrappers later` rule now has a real bounded config-window foothold, one small lab-only config-write planning foothold, one small lab-only config-write disposition foothold, one explicit transport-identity foothold, one early probe-preflight observation foothold, and one compact selected-queue readiness foothold, while the riskier transport and lifecycle paths remain intentionally blocked.
...
This keeps the lane concrete and reviewable without overstating MMIO progress: the queue-facing footholds are real, the shared Phase 10 packet now honestly records the bounded register-window, queue-size, selected-queue readiness, feature-word, config-window, config-write planning, transport-identity, config-write disposition, and probe-preflight surfaces together with the shorter-restage stale-data regression proof, and the broader transport-facing lifecycle work is still intentionally blocked.
