...
- scope: survey manifest, dedicated survey gate, shared Phase 10 build wiring, a lane-level note that records what is present in the live repo plus the remaining MMIO transport gap against the roadmap, the first MMIO register-window helper, and the bounded queue-register helper that now makes the next queue-notify gap concrete
- product boundary:
...
- the landed MMIO helper stays intentionally narrow: it now models MMIO register offsets, bounded feature-page selection, queue-select and queue-size planning, queue-ready bookkeeping, status and reset bookkeeping, config-generation tracking, and interrupt-ack bookkeeping only.
- this means the roadmap's "virtqueue wrappers first, MMIO wrappers later" rule now holds for a small but real MMIO foothold, and the next honest follow-up is a queue-notify snapshot helper rather than probe, remove, DMA, or broader lifecycle glue.
...
- the ready-next `phase10-mmio-queue-notify-helper`
- the still-blocked `phase10-mmio-lifecycle-and-irq-paths`

This keeps the lane concrete and reviewable without overstating MMIO progress: the queue-facing footholds are real, the bounded register-window and queue-register steps are now landed, and the broader transport-facing lifecycle work is still intentionally blocked.
...
- queue notify side effects or queue setup and teardown parity from `vm_setup_vq()` and `vm_del_vqs()`
...
Stay in the broader Phase 10 MMIO lane and add one small queue-notify helper next without claiming queue setup, IRQ delivery, probe, or remove parity.
