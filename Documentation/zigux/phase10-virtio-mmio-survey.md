...
This keeps the lane concrete and reviewable without overstating MMIO progress: the queue-facing footholds are real, the shared Phase 10 packet now acknowledges the bounded register-window, queue-size, feature-word, config-window, config-write planning, config-write disposition, and probe-preflight surfaces honestly, and the broader transport-facing lifecycle work is still intentionally blocked.
