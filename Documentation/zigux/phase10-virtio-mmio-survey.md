...
- the dedicated MMIO tests now replay two staged device-feature words plus two staged config words and prove both windows stay inside the helper instead of implying interrupt acknowledgement, queue discovery, reset, or probe lifecycle behavior.
...
This keeps the lane concrete and reviewable without overstating MMIO progress: the helper-backed queue, status, feature-word, and config-window footholds are real, the shared Phase 10 packet now acknowledges them honestly, and the riskier lifecycle paths remain intentionally blocked.
...
Keep the Phase 10 MMIO lane narrow and prefer another one-file or tightly coupled survey, manifest, slice-note, or helper-test truthfulness repair before widening into interrupt acknowledgement, queue discovery, reset paths, or probe lifecycle work.
