...
Current `master` still keeps the shared Phase 10 closure packet, this MMIO survey note, and the broader freeze-map reminder surfaces explicit. The live packet-local manifest `zigux/tests/phase10_virtio_mmio_manifest.json` still carries the parked MMIO helper ladder and blocked transport follow-through recorded by the shared closure packet, and current raw GitHub fallback now directly re-reads that manifest beside `zigux/tests/phase10_build.zig`, `zigux/tests/phase10_virtio_mmio.zig`, `zigux/tests/phase10_virtio_mmio_survey.zig`, and the dedicated `drivers/virtio/virtio_mmio_verify.zig` replay. The authenticated contents bridge still remains incomplete for several focused Phase 10 files in this environment, so this note records the raw-fallback-backed MMIO verifier packet explicitly instead of leaving the shared build route pointed at a missing verifier surface.
...
- raw GitHub fallback now directly re-reads `zigux/tests/phase10_virtio_mmio_manifest.json`, `zigux/tests/phase10_build.zig`, `zigux/tests/phase10_virtio_mmio.zig`, `zigux/tests/phase10_virtio_mmio_survey.zig`, and `drivers/virtio/virtio_mmio_verify.zig`, but this run still did not rerun the shared build plus focused MMIO replay routes, so fresh verifier-backed compile and replay validation remain pending here
- the still-blocked `phase10-mmio-lifecycle-and-irq-paths`

That keeps the MMIO lane aligned with current repo evidence: the helper ladder and blocked transport follow-through are still recorded in the shared closure packet, the public tree still shows the MMIO wrapper family under `drivers/virtio/`, raw fallback now recovers the focused tests-root MMIO packet, the shared build gate, and the dedicated driver-local verifier replay, while the actual remaining gap is fresh build-backed replay validation rather than missing verifier readback.
...
2. run the focused MMIO replay trio through the shared build packet now that current raw GitHub fallback re-reads `zigux/tests/phase10_build.zig`, `zigux/tests/phase10_virtio_mmio.zig`, `zigux/tests/phase10_virtio_mmio_survey.zig`, and `drivers/virtio/virtio_mmio_verify.zig`; this run still did not rerun that shared build-backed replay route
- `zig build test --build-file zigux/tests/phase10_build.zig`
- `make -C zigux phase10-test`
- `make -C zigux phase10`

3. keep the dedicated MMIO packet checker and dedicated freeze-boundary checker aligned with the next successful build-backed replay of the restored driver-local MMIO verifier instead of treating raw readback alone as proof that the focused MMIO packet was freshly replayed
