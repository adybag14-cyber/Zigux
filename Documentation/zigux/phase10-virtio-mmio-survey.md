...
- `drivers/virtio/virtio_mmio_verify.zig` keeps the wrapper-facing transport-identity, queue-readiness, and config-write-disposition proof explicit beside the helper-local packet.
...
- the helper-local tests inside `drivers/virtio/virtio_mmio.zig` cover zero-valued known feature words, shared-versus-mismatched feature bits, stale-plan invalidation after a config-generation bump, stale-plan invalidation after config-byte restaging, legacy guest-page-size probe gating, and non-mutating config-write disposition reporting.
...
- `zigux/tests/phase10_build.zig` remains part of the shared Phase 10 review packet and now runs the helper-local MMIO tests through `drivers/virtio/virtio_mmio.zig`, the wrapper-facing MMIO verify replay, and the dedicated MMIO survey replay through `zigux/tests/phase10_virtio_mmio_survey.zig`. The shared gate should still be read as helper-local MMIO coverage plus one survey replay rather than a broader transport-backed replay.
...
2. the dedicated survey replay: `zig test zigux/tests/phase10_virtio_mmio_survey.zig`
3. `zigux/tests/phase10_build.zig`, `make -C zigux phase10-test`, and `make -C zigux phase10` remain shared Phase 10 reminder routes, and the current build file now runs the helper-local MMIO tests, the wrapper-facing MMIO verify replay, and the dedicated MMIO survey replay together

These gates should be read as helper-local review evidence and shared packet reminders only, not as proof of a transport-backed MMIO driver or a dedicated MMIO lifecycle replay.

## Next bounded step
Keep the broader Phase 10 MMIO lane parked unless fresh repo inspection finds one directly coupled follow-through. The next honest same-lane step is to align one additional shared reminder surface around the restored MMIO survey replay while keeping lifecycle-and-IRQ transport work blocked.
