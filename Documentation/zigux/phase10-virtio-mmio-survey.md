...
- scope: survey manifest, dedicated survey gate, shared Phase 10 build wiring, a lane-level note that records what is present in the live repo plus the remaining MMIO transport gap against the roadmap, the first MMIO register-window helper, the bounded queue-register helper, the landed queue-notify and queue-address helpers, and the new config-window helper that now makes the next config-write gap concrete
- product boundary:
...
- the landed MMIO helper stays intentionally narrow: it now models MMIO register offsets, bounded feature-page selection, queue-select and queue-size planning, queue-ready bookkeeping, queue-notify snapshots, version-scoped queue-address planning, read-only config-window snapshots, status and reset bookkeeping, config-generation tracking, and interrupt-ack bookkeeping only.
- this means the roadmap's "virtqueue wrappers first, MMIO wrappers later" rule now holds for a small but real MMIO foothold, and the next honest follow-up is a tiny config-window write-planning helper rather than queue setup, probe, remove, DMA, or broader lifecycle glue.
...
- config-space writes or full config-field parity through `VIRTIO_MMIO_CONFIG`
...
Stay in the broader Phase 10 MMIO lane and add one small config-window write-planning helper next without claiming queue setup, IRQ delivery, probe, or remove parity.
