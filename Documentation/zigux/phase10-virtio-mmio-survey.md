...
- scope: survey manifest, dedicated survey gate, shared Phase 10 build wiring, a lane-level note that records what is present in the live repo plus the remaining MMIO transport gap against the roadmap, the first MMIO register-window helper, the bounded queue-register helper, the queue-notify helper, and the queue-address helper that now keeps the next config-window gap concrete
- product boundary:
...
- the landed MMIO helper stays intentionally narrow: it now models MMIO register offsets, bounded feature-page selection, queue-select and queue-size planning, queue-ready bookkeeping, queue-notify snapshots, version-scoped queue-address planning, status and reset bookkeeping, config-generation tracking, and interrupt-ack bookkeeping only.
- this means the roadmap's "virtqueue wrappers first, MMIO wrappers later" rule now holds for a small but real MMIO foothold, and the next honest follow-up is a config-window snapshot helper rather than probe, remove, DMA, or broader lifecycle glue.
...
This keeps the lane concrete and reviewable without overstating MMIO progress: the queue-facing footholds are real, the bounded register-window, queue-register, queue-notify, and queue-address steps are now landed, and the broader transport-facing lifecycle work is still intentionally blocked.
...
Stay in the broader Phase 10 MMIO lane and add one small config-window snapshot helper next without claiming queue setup, IRQ delivery, probe, or remove parity.
