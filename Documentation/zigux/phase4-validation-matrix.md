...
The `samples/zigux/kprobe_example.zig` roadmap row is no longer matrix-only: the manifest-backed survey gate now lives in `zigux/tests/phase4_kprobe_example_manifest.json`, runs through `phase4-kprobe-example-survey-tests` in `zigux/tests/phase4_build.zig`, and keeps the direct `zig build phase4-kprobe-example-survey --build-file zigux/tests/phase4_build.zig` local replay path plus the current lab posture C-anchor-only through `make M=samples/kprobes CONFIG_SAMPLE_KPROBES=m` until the bounded Zig sample itself lands.
