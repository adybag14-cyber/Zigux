...
| perf baselines and thresholds for the two shipped rollback gates | `zigux/tests/atomic64_diff.zig` and `zigux/tests/bitmap_diff.zig` are still correctness-only gates today | benchmark command is still unapproved for both landed gates, and acceptable limit is still unapproved for both landed gates | land one bounded benchmark command and one acceptable limit per gate before Phase 4 claims perf coverage |
