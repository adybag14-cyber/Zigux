...
## Lab And CI Matrix
  ...
  * `scripts/zigux/check-phase4-reversible-delivery-pins.py` dedicated shared exact-readback handoff pin replay for `Documentation/zigux/phase4-reversible-delivery-evidence.md`, the shared exact-readback packet, the anti-overlap sequencing note, and the adjacent local-only perf packet `Tooling and Validation Team` `Tooling and Validation Team` `python3 scripts/zigux/validate-phase4.py` in `.github/workflows/zigux-bootstrap.yml`, which reruns the reversible-delivery pin checker before the Zig gates while keeping the handoff packet narrower than the broader shared rollback-test and starter-gap surfaces `python3 scripts/zigux/check-phase4-reversible-delivery-pins.py --self-test` then `python3 scripts/zigux/check-phase4-reversible-delivery-pins.py` then `python3 scripts/zigux/validate-phase4.py` `reviewability_only_no_perf_threshold`
  ...
  * dedicated local checker: `python3 scripts/zigux/check-phase4-perf-baseline-packet.py --self-test` then `python3 scripts/zigux/check-phase4-perf-baseline-packet.py`; this checker is rerun through the shared `make -C zigux phase4-validate` validator path, but it keeps the dedicated perf-baseline survey local-only and outside the shared `phase4-test` route while shared CI perf promotion stays pending
...
The dedicated reversible-delivery pin checker now reruns through the shared validator-first packet today, but it still stays narrower than the broader shared Zig rollback-test and starter-gap surfaces: it keeps the handoff note, sequencing note, and adjacent local-only perf packet exact until a later shared exact-readback lane intentionally decides whether any broader handoff replay belongs in the shared rollback-test surface.
