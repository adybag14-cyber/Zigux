...
- exact bounded checks: `bitmap_fill(..., 35)` proves the 35-bit prefix start plus the 34, 35, and `BITS_PER_LONG` boundary bits, the paired `bitmap_zero(..., 35)` replay keeps the same 35-bit checkpoint explicit from the filled side, `bitmap_zero(..., 115)` still proves the two-word rounded clear, the cross-boundary `bitmap_set(..., 79, 19)` and `bitmap_clear(..., 79, 19)` cases keep the 64..78 and 98..1023 boundaries explicit, full-width `bitmap_fill(..., 1024)` and `bitmap_zero(..., 1024)` keep the endpoints honest, `bitmap_scnprintf()` preserves both the full `1-3,7,10-11` summary and the truncated `1-3` rendering, `bitmap_copy()` replays the 23-bit single-word window, the full-width cleared-destination and filled-destination copies, the 109-bit partial-tail, and the 97-bit aligned-copy cases while `bitmap.copyClearTail()` keeps the 109-bit cleared-tail contract, and `find_nth_bit()` records both the full-width nth-7 and nth-8 outcomes plus the reduced-width `64 * 3 - 1` cutoff that still returns bit 123 for nth 7 and the cutoff width for nth 8
