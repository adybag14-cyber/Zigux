...
## Status
  ...
  * direct-readback split: this broader matrix packet is still live on current `master`, but the smaller current direct-readback packet stays intentionally narrower until authenticated contents reads recover for the broader companions again; today that direct-readback packet is limited to `Documentation/zigux/phase4-reversible-delivery-evidence.md`, `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, `scripts/zigux/check-phase4-repo-reality-warning.py`, `scripts/zigux/check-phase4-reversible-delivery-pins.py`, `scripts/zigux/check-phase4-perf-baseline-packet.py`, `zigux/tests/phase4_perf_baseline_manifest.json`, and `zigux/tests/phase4_perf_baseline_survey.zig`
...
### `scripts/zigux/check-phase4-artifact-diff-validator-replays.py`
  * anchor: `scripts/zigux/validate-phase4.py`
  * phase bucket: `Phase 4 host-side differential-validation tooling validator replay alignment`
  * owner: `Tooling and Validation Team`
  * rollback owner: `Tooling and Validation Team`
  * implementation note: `scripts/zigux/check-phase4-artifact-diff-validator-replays.py` keeps the current helper self-test, contract replay, determinism replay, and validator-replay commands aligned across `scripts/zigux/validate-phase4.py`, `.github/workflows/zigux-bootstrap.yml`, and the shared reversible-delivery handoff note so the validator-first artifact-diff packet fails closed before the broader Phase 4 validator and Zig rollback gates continue
  * fallback path: keep `scripts/zigux/validate-phase4.py`, `.github/workflows/zigux-bootstrap.yml`, `Documentation/zigux/artifact-diff.md`, and `Documentation/zigux/phase4-reversible-delivery-evidence.md` truthful about the current validator-first artifact-diff packet if the dedicated checker regresses until that narrower gate is repaired
  * perf threshold status: reviewability-only gate today; there is no timing claim on the validator-replay alignment packet
...
### `scripts/zigux/check-phase4-reversible-delivery-pins.py`
  * anchor: `Documentation/zigux/phase4-reversible-delivery-evidence.md`
  * phase bucket: `Phase 4 shared exact-readback reversible-delivery handoff pin replay`
  * owner: `Tooling and Validation Team`
  * rollback owner: `Tooling and Validation Team`
  * implementation note: `scripts/zigux/check-phase4-reversible-delivery-pins.py` reruns the shipped reversible-delivery handoff note against the current shared exact-readback packet, exact-pinning `Documentation/zigux/phase4-gate-evidence.md`, `Documentation/zigux/phase4-validation-matrix.md`, `scripts/zigux/check-phase4-remaining-gap-matrix.py`, `scripts/zigux/check-phase4-workflow-route-counts.py`, `scripts/zigux/validate-phase4.py`, `zigux/tests/phase4_build.zig`, `zigux/Makefile`, `.github/workflows/zigux-bootstrap.yml`, `Documentation/zigux/review-checklist.md`, `Documentation/zigux/phase4-validation-lane-sequencing.md`, and the dedicated local-only perf packet so shared handoff drift fails closed without widening that exact-readback packet into starter implementation or shared CI perf approval
  * fallback path: keep `Documentation/zigux/phase4-reversible-delivery-evidence.md` truthful about the current shared exact-readback packet, anti-overlap sequencing note, and dedicated local-only perf packet if the dedicated pin checker regresses until that narrower handoff gate is repaired
  * perf threshold status: reviewability-only gate today; it exact-pins the shared handoff packet and adjacent local-only perf packet without claiming direct timing coverage
...
### `zigux/tests/phase4_bitmap_diff_survey.zig`
  ...
  * fallback path: keep `zigux/tests/bitmap_diff.zig`, `zigux/tests/phase4_bitmap_live_helper_replay.zig`, and this matrix as the truthful rollback surface and remove that survey from the shared Phase 4 build entrypoint if the manifest drifts
...
## Lab And CI Matrix
  ...
  * `scripts/zigux/check-phase4-artifact-diff-validator-replays.py` bounded validator-first artifact-diff replay inventory for the helper self-test, contract replay, determinism replay, and validator-replay checker commands across the shared validator, workflow, and reversible-delivery handoff note `Tooling and Validation Team` `Tooling and Validation Team` `python3 scripts/zigux/validate-phase4.py` in `.github/workflows/zigux-bootstrap.yml`, which reruns the validator-replay checker before the gate-evidence checker and Zig gates `python3 scripts/zigux/check-phase4-artifact-diff-validator-replays.py --self-test` then `python3 scripts/zigux/check-phase4-artifact-diff-validator-replays.py` then `python3 scripts/zigux/validate-phase4.py` `reviewability_only_no_perf_threshold`
  ...
  * `scripts/zigux/check-phase4-reversible-delivery-pins.py` dedicated shared exact-readback handoff pin replay for `Documentation/zigux/phase4-reversible-delivery-evidence.md`, the shared exact-readback packet, the anti-overlap sequencing note, and the adjacent local-only perf packet `Tooling and Validation Team` `Tooling and Validation Team` not on the shared workflow or validator packet yet; keep this handoff checker local until a later bounded shared exact-readback lane intentionally promotes it `python3 scripts/zigux/check-phase4-reversible-delivery-pins.py --self-test` then `python3 scripts/zigux/check-phase4-reversible-delivery-pins.py` `reviewability_only_no_perf_threshold`
  ...
  * dedicated local checker: `python3 scripts/zigux/check-phase4-perf-baseline-packet.py --self-test` then `python3 scripts/zigux/check-phase4-perf-baseline-packet.py`; this checker keeps the perf-baseline survey packet local-only and fail-closed without promoting the survey itself into the shared `phase4-test` replay surface or a shared CI perf claim, while the shared bootstrap workflow only reruns the checker through `make -C zigux phase4-validate` and shared CI perf promotion stays pending
...
The dedicated reversible-delivery pin checker likewise stays outside the shared validator-first packet today: it keeps the handoff note, sequencing note, and adjacent local-only perf packet exact as a narrower local gate until a later shared exact-readback lane intentionally decides whether that handoff replay belongs in the broader validator-backed route.
...
## Review Rules
  ...
  * if the reversible-delivery handoff pin checker regresses, repair `scripts/zigux/check-phase4-reversible-delivery-pins.py` or keep `Documentation/zigux/phase4-reversible-delivery-evidence.md`, `Documentation/zigux/phase4-validation-lane-sequencing.md`, and the adjacent local-only perf packet truthful until that narrower handoff gate is honest again
