...
| --- | --- | --- | --- |
| `samples/zigux/kprobe_example.zig` | not present on `master`; the current anchor remains `samples/kprobes/kprobe_example.c` through `samples/kprobes/Makefile` and `CONFIG_SAMPLE_KPROBES`, the validator-backed absence check keeps that true today, and the manifest-backed survey gate now lives in `zigux/tests/phase4_kprobe_example_manifest.json` plus `zigux/tests/phase4_kprobe_example_survey.zig` under the shared `phase4-kprobe-example-survey-tests` replay | reserve `Validation and Perf Team` as both survey owner and rollback owner while the current replay stays on the C anchor via `make M=samples/kprobes CONFIG_SAMPLE_KPROBES=m`; the dedicated survey packet keeps the Zig lab matrix C-anchor-only and no hard timing threshold is approved before a bounded Zig sample lands | land one bounded starter under `samples/zigux/kprobe_example.zig` that keeps the same owner, rollback owner, and `make M=samples/kprobes CONFIG_SAMPLE_KPROBES=m` replay contract before claiming this anchor as active Phase 4 work |
...
The `samples/zigux/kprobe_example.zig` roadmap row is no longer matrix-only: the manifest-backed survey gate now lives in `zigux/tests/phase4_kprobe_example_manifest.json`, runs through `phase4-kprobe-example-survey-tests` in `zigux/tests/phase4_build.zig`, and keeps the dedicated `make -C zigux phase4-kprobe-example-survey` wrapper plus the direct `zig build phase4-kprobe-example-survey --build-file zigux/tests/phase4_build.zig` local replay paths alongside the current lab posture C-anchor-only through `make M=samples/kprobes CONFIG_SAMPLE_KPROBES=m` until the bounded Zig sample itself lands.
