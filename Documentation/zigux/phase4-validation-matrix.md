...
## Lab And CI Matrix
  ...
  * dedicated local checker: `python3 scripts/zigux/check-phase4-perf-baseline-packet.py --self-test` then `python3 scripts/zigux/check-phase4-perf-baseline-packet.py`; this checker keeps the perf-baseline packet local-only and fail-closed without promoting it into the shared workflow or the shared `phase4-test` replay surface while shared CI perf promotion stays pending
