...
- the external artifact-diff contract replay that keeps the published `ARTIFACT_DIFF=...`, malformed-JSON markers, SHA-256 digest fields, and exit-code surface reviewable outside the helper's built-in self-test
...
- pending threshold record: `zigux/tests/phase4_runtime_atomic64_diff_manifest.json` now keeps the current threshold owner, rollback owner, posture, approval state, and placeholder benchmark-command plus acceptable-limit fields machine-checkable until this gate widens enough to claim perf coverage
