...
| --- | --- | --- |
| `samples/zigux/kprobe_example.zig` | still absent; the current anchor remains `samples/kprobes/kprobe_example.c` through `samples/kprobes/Makefile` and `CONFIG_SAMPLE_KPROBES`, and the shared Phase 4 validator now checks that this matrix still records the current `make M=samples/kprobes CONFIG_SAMPLE_KPROBES=m` replay plus the absence of any dedicated `phase4_kprobe_example_*` survey packet | no promoted `phase4_build.zig` replay route or approved Zig sample starter has landed yet |
| `samples/zigux/test_fsmount.zig` | still absent; the current anchor remains `samples/vfs/test-fsmount.c` through `samples/vfs/Makefile` and `userprogs-always-y += test-fsmount`, and the shared Phase 4 validator now checks that this matrix still records the current `make M=samples/vfs` replay plus the absence of any dedicated `phase4_test_fsmount_*` survey packet | no promoted `phase4_build.zig` replay route or approved Zig sample starter has landed yet |
| perf thresholds for `zigux/tests/atomic64_diff.zig` and `zigux/tests/bitmap_diff.zig` | still pending by design; the two shipped rollback gates above remain correctness-first only until a later lane approves benchmark commands plus acceptable limits, and the shared Phase 4 validator now checks that benchmark command and acceptable limit remain unapproved for both landed gates | no threshold command-and-limit pair is approved yet, so the matrix keeps only the pending posture markers in the shipped gate rows above |
