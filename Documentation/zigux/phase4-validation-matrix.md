...
- the shared validator boundary note that `scripts/zigux/validate-phase4.py` still owns the broader gate-definition, matrix, manifest, gate-evidence, and roadmap-gap packet while `scripts/zigux/check-phase4-workflow-route-counts.py` now owns exact single-run-line counts for `make -C zigux phase4-validate` and `make -C zigux phase4-test` across `.github/workflows/zigux-bootstrap.yml`, `zigux/Makefile`, and the docs-root, scripts-root, and tests-root route summaries
...
- perf threshold status: correctness-only gate today; no hard timing threshold is approved until the lane widens beyond the current bounded add, sub, bitwise, exchange, cmpxchg, add_unless, inc_not_zero, and selftest-family plus post-selftest replay set
...
- current parity note: no longer true that direct `bitmap_fill(..., 115)` still stops at bit 114 in the shipped Zig helper, so the Phase 4 packet keeps that mismatch survey-only instead of claiming parity with the `lib/test_bitmap.c` rounded two-word anchor; the live replay now rounds to the same two-word `0-127` anchor as `lib/test_bitmap.c` while the broader gate still remains correctness-only until a bounded benchmark lands
...
| --- | --- | --- | --- |
| `samples/zigux/kprobe_example.zig` | not present on `master`; the current anchor remains `samples/kprobes/kprobe_example.c` through `samples/kprobes/Makefile` and `CONFIG_SAMPLE_KPROBES`, the validator-backed absence check keeps that true today, and the manifest-backed survey gate now lives in `zigux/tests/phase4_kprobe_example_manifest.json` plus `zigux/tests/phase4_kprobe_example_survey.zig` under the shared `phase4-kprobe-example-survey-tests` replay | reserve `Validation and Perf Team` as both survey owner and rollback owner while the current replay stays on the C anchor via `make M=samples/kprobes CONFIG_SAMPLE_KPROBES=m`; the dedicated survey packet keeps the Zig lab matrix C-anchor-only, and the broader shared `scripts/zigux/validate-phase4.py` contract still does not fail closed on this manifest-backed survey packet before sample-starter work begins | promote the already-landed kprobe survey packet into `scripts/zigux/validate-phase4.py`, then land one bounded starter under `samples/zigux/kprobe_example.zig` that keeps the same owner, rollback owner, and `make M=samples/kprobes CONFIG_SAMPLE_KPROBES=m` replay contract before claiming this anchor as active Phase 4 work |
...
| `perf baselines and thresholds for the two shipped rollback gates` | `zigux/tests/atomic64_diff.zig` and `zigux/tests/bitmap_diff.zig` are still correctness-only gates today, and `zigux/tests/phase4_perf_baseline_manifest.json` now records one pending threshold-plan placeholder per gate | benchmark command and acceptable limit are still unapproved for both landed gates | land one bounded benchmark command and one acceptable limit per gate before Phase 4 claims perf coverage |
...
The `samples/zigux/kprobe_example.zig` roadmap row is no longer matrix-only: the manifest-backed survey gate now lives in `zigux/tests/phase4_kprobe_example_manifest.json`, runs through `phase4-kprobe-example-survey-tests` in `zigux/tests/phase4_build.zig`, and keeps the dedicated `make -C zigux phase4-kprobe-example-survey` wrapper plus the direct `zig build phase4-kprobe-example-survey --build-file zigux/tests/phase4_build.zig` local replay paths alongside the current lab posture C-anchor-only through `make M=samples/kprobes CONFIG_SAMPLE_KPROBES=m` until the bounded Zig sample itself lands. The next bounded same-lane step is still the shared-validator promotion recorded in the manifest and gate-evidence packet: `scripts/zigux/validate-phase4.py` still undercounts the kprobe survey packet today, while the dedicated `scripts/zigux/check-phase4-kprobe-example-packet.py` path keeps that survey surface fail-closed in the meantime.
...
The Phase 4 perf-baseline gap is no longer prose-only either: the manifest-backed survey packet now lives in `zigux/tests/phase4_perf_baseline_manifest.json`, runs through `phase4-perf-baseline-survey-tests` in `zigux/tests/phase4_build.zig`, and keeps the dedicated `make -C zigux phase4-perf-baseline-survey` local replay path plus the current `threshold_pending_until_runtime_atomic64_scope_widens` and `threshold_pending_until_bitmap_gate_grows_beyond_bounded_correctness_checks` posture explicit. The same manifest now also carries one pending threshold-plan placeholder per shipped rollback gate, pinning the current isolated correctness replays `make -C zigux phase4-runtime-atomic64-diff` and `make -C zigux phase4-bitmap-diff` beside the still-unapproved benchmark command and acceptable limit so those future threshold steps stay tied to the current owners and rollback owners until one bounded benchmark command and one acceptable limit land for each gate.
...
The current validation boundary is explicit too: `scripts/zigux/validate-phase4.py` still owns the broader workflow-route, matrix, manifest, gate-evidence, and roadmap-gap packet, while `scripts/zigux/check-phase4-workflow-route-counts.py` now owns exact single-run-line counts for `make -C zigux phase4-validate` and `make -C zigux phase4-test` across `.github/workflows/zigux-bootstrap.yml`, `zigux/Makefile`, and the docs-root, scripts-root, plus tests-root route summaries.
