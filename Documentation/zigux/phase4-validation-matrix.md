...
- exact bounded checks: `bitmap_fill(..., 35)` currently proves the 35-bit prefix start plus the 34, 35, and `BITS_PER_LONG` boundary bits, `bitmap_zero(..., 115)` still proves the two-word rounded clear, the cross-boundary `bitmap_set(..., 79, 19)` and `bitmap_clear(..., 79, 19)` cases keep the 64..78 and 98..1023 boundaries explicit, full-width `bitmap_fill(..., 1024)` and `bitmap_zero(..., 1024)` keep the endpoints honest, `bitmap_scnprintf()` preserves both the full `1-3,7,10-11` summary and the truncated `1-3` rendering, full-width `bitmap_copy(..., 1024)` from cleared and filled destinations drops stale tail bits outside the source window, full-width `bitmap_copy(..., 109)` and `bitmap_copy(..., 97)` keep the stale tail visible through bit 1023 while `bitmap.copyClearTail()` preserves the same 109-bit contract, and `find_nth_bit()` records both the full-width nth-7 and nth-8 outcomes plus the reduced-width `64 * 3 - 1` cutoff that still returns bit 123 for nth 6 and the cutoff width for nth 7
