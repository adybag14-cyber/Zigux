...
- one reviewer-facing boundary note that the shared validator still proves the Phase 4 workflow step names and the presence of `make -C zigux phase4-validate` plus `make -C zigux phase4-test`, while the exact single-run-line counts for those two workflow commands still live in `Documentation/zigux/phase4-gate-evidence.md` until a later validator hardening lands
...
The current validator boundary is explicit too: exact single-run-line counts for `make -C zigux phase4-validate` and `make -C zigux phase4-test` still live in `Documentation/zigux/phase4-gate-evidence.md` rather than in `scripts/zigux/validate-phase4.py`, so workflow-count drift still needs the paired exact-readback note refreshed until a later validator hardening lands.
...
- if the shared `artifact_diff.py --self-test`, `check-artifact-diff-contract.py`, `check-phase4-gate-evidence.py`, or standalone `validate-phase4.py --self-test` surface changes, update this matrix in the same change so the roadmap's deterministic host-tool preflight and validator-contract packet stay explicit
- until `scripts/zigux/validate-phase4.py` hardens exact workflow run-line counts, refresh `Documentation/zigux/phase4-gate-evidence.md` in the same change whenever `.github/workflows/zigux-bootstrap.yml` adds, removes, or duplicates either Phase 4 `make -C zigux phase4-validate` or `make -C zigux phase4-test` run line
