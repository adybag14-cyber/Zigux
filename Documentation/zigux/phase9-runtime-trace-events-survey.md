...
- the same shared-request bridge now also rejects non-prepared shared requests before `requestSharedRuntimeLoad()`, rejects prepared shared-request contract drift before any local runtime handoff, and keeps shared release failures from desynchronizing the local loader stage, so request-state failures stay reviewable before the runtime substrate exists.
