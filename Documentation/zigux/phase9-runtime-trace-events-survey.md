...
The next same-family follow-through should stay small and literal: add the missing prepared shared runtime-substrate drift rejection replay to `samples/zigux/runtime_trace_events_loader.zig`, then mirror that cue into `zigux/tests/runtime_trace_events_survey.zig` so the dedicated survey gate fails closed if the loader packet drops that handoff proof later. Keep the trace-events survey note, module-slice note, and manifest aligned with that visible family-local packet while leaving the shared runtime-substrate blocker plus the blocked runtime task ownership, polling and event-loop substrate, polling-backed wake or dispatch behavior, and blocked alias or depmod publication boundary spelled out until the separate shared runtime-substrate lanes actually move.
