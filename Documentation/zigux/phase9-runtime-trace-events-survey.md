...
The next same-family follow-through should stay small and literal: keep the trace-events survey note, module-slice note, and manifest aligned with the visible family-local packet, keep the bounded pilot-module contract explicit, and leave the shared runtime-substrate blocker plus the blocked runtime task ownership, polling and event-loop substrate, polling-backed wake or dispatch behavior, and blocked alias or depmod publication boundary spelled out until the separate shared runtime-substrate lanes actually move.
