...
The next same-family follow-through should stay small and literal: mirror the same selftest-ready failed-exit rollback cue into `zigux/tests/runtime_trace_events_survey.zig` so the dedicated survey gate fails closed if the survey note, module-slice note, or manifest drops that family-local lifecycle reminder later, then keep the trace-events survey note, module-slice note, and manifest aligned with the visible family-local packet while leaving the shared runtime-substrate blocker plus the blocked runtime task ownership, polling and event-loop substrate, polling-backed wake or dispatch behavior, and blocked alias or depmod publication boundary spelled out until the separate shared runtime-substrate lanes actually move.
