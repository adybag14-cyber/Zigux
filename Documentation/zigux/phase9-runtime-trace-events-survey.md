...
- the same shared-request bridge also rejects prepared shared-request contract drift before any local runtime handoff and keeps shared release failures from desynchronizing the local loader stage, so request-state failures stay reviewable before the runtime substrate exists.
- unlike the sibling atomic64, bitmap, and kretprobe loader packets, the trace-events loader still does not carry a direct pilot-local replay for the invalid transition where a shared request is no longer in the prepared state before `requestSharedRuntimeLoad()`, so that guard is still only implied by the loader-state check rather than pinned by its own focused trace-events replay.
- the trace-events loader also refuses to prepare while function-callback registration depth is non-zero, keeping the metadata-only registration snapshot idle before the shared runtime-loader request begins.
- the live repo also carries `zigux/kernel/runtime_loader_contract.zig`, `zigux/tests/runtime_loader_allocator_init_flow.zig`, and the focused `phase9-runtime-loader-shared-tests` build step, so allocator handoff, init-flow counts, release-without-substrate behavior, and shared-request drift stay reviewable beside the trace-events starter packet instead of hiding in the shared build alone.
...
- the landed trace-events packet satisfies the bounded selftest-hook and lifecycle-evidence part of that roadmap, but it does not yet satisfy the loadable-module part.
- the honest current state is `starter_landed_without_loadable_runtime_substrate`.
- the missing capability is a shared runtime substrate that can turn the bounded `tracepoint_probe_register` and `tracepoint_probe_unregister` handoff plan into a real loadable module path.
...
Stay in the Phase 9 runtime trace-events lane and add the missing pilot-local replay that rejects a non-prepared shared request before `requestSharedRuntimeLoad()` before widening back out toward the shared runtime-loader substrate.