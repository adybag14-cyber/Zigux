...
- the same shared-request bridge now also rejects non-prepared shared requests before `requestSharedRuntimeLoad()`, rejects prepared shared-request contract drift before any local runtime handoff, and the shared allocator/init-flow replay now keeps that trace-events request-shape drift concrete by exact-checking anchor drift plus entry-symbol and exit-symbol drift before `requestRuntimeLoad()`, while shared release failures still cannot desynchronize the local loader stage, so request-state failures and approved-family request-shape drift stay reviewable before the runtime substrate exists.
