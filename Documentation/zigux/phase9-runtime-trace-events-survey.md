...
- the same shared-request bridge now also rejects non-prepared shared requests before `requestSharedRuntimeLoad()`, rejects prepared shared-request contract drift before any local runtime handoff, and the shared allocator/init-flow replay now keeps that trace-events request-shape drift concrete by exact-checking `requires_runtime_substrate`, module-name, anchor, entry-symbol, exit-symbol, allocator-handoff, selftest-hook, and init-flow drift before `requestRuntimeLoad()`, while the trace-events loader itself also fail-closes direct shared-load-plan snapshot drift and registration snapshot drift before any live registration claim, and shared release failures still cannot desynchronize the local loader stage, so request-state failures, snapshot drift, and prepared-plan drift stay reviewable before the runtime substrate exists.
