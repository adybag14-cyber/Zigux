...
* direct contents reads in this run now rematerialize `drivers/tty/hvc/hvc_console.zig`, `drivers/tty/hvc/hvc_console_verify.zig`, `drivers/tty/hvc/hvc_console_sysrq.zig`, `zigux/tests/phase11_hvc_console.zig`, `zigux/tests/phase11_hvc_cleanup.zig`, `zigux/tests/phase11_hvc_console_survey.zig`, `zigux/tests/phase11_hvc_console_manifest.json`, `zigux/tests/phase11_hvc_console_modem_control_split.zig`, `zigux/tests/phase11_hvc_console_poll_retry_split.zig`, `Documentation/zigux/phase11-hvc-console-slice.md`, `Documentation/zigux/phase11-hvc-console-teardown-note.md`, `Documentation/zigux/phase11-hvc-console-validation-matrix.md`, and `scripts/zigux/check-phase11-hvc-survey-packet.py`, so keep those as direct-readback archival packet members beside the shared inventory-backed continuity anchors
...
The current bounded HVC archival packet should therefore be treated as a direct-readback packet on `master` rather than as an inventory-backed packet:
...
Current direct readback in this run confirmed this survey note, the direct driver, test, split-replay, dedicated-checker, and coupled-doc companions above, plus the shared inventory-backed continuity anchors `zigux/tests/fixtures/phase11_build_inventory.json` and `scripts/zigux/check-phase11-build-inventory.py`. Keep that direct-readback archival packet explicit while the lane stays below live tty registration, notifier callback execution, khvcd execution, live sysrq dispatch, and host-backed teardown parity.
