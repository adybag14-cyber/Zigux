...
Fresh rereads on 2026-05-19 show that the attached-toolchain reminder split is now narrower than this note previously recorded:
...
- `Documentation/zigux/README.md` still lags the returned Phase 14 route split: its Phase 14 docs-root reminder block still says the readable `zigux/Makefile` exposes no `phase14-validate`, `phase14-smoke`, `phase14-test`, or `phase14` targets even though current `master` now exposes `phase14-validate` while still omitting only the broader three routes
- `Documentation/zigux/review-checklist.md` now also lags the returned Phase 14 route split: its shared-smoke checkpoint still treats `phase14-validate` as packet-local or repo-reality-gap vocabulary alongside the still-missing `phase14-smoke`, `phase14-test`, and `phase14` wrappers even though current `master` now exposes `phase14-validate`
...
- `zigux/tests/README.md` is already aligned with the returned route split: it keeps the attached-toolchain fallback explicit only as packet-local rerun vocabulary and separately records that the readable `zigux/Makefile` now exposes `phase14-validate` while still omitting `phase14-smoke`, `phase14-test`, and `phase14`
- `scripts/zigux/check-phase14-shared-smoke-route.py` is directly readable again through the current contents path and now keeps the returned `phase14-validate` Makefile route plus workflow gate explicit rather than leaving that shared-smoke route proof implicit in neighboring reminder text
...
That means the older stale-summary framing recorded by this note is no longer the active same-lane gap on current `master`.

The remaining readback split is narrower:
- the shared smoke note and release-boundary note now keep the attached-toolchain boundary explicit while treating the older `phase14-*` wrapper names as historical packet-local vocabulary rather than current fallback guidance
- `Documentation/zigux/README.md` still lags the returned route split by describing the readable `zigux/Makefile` as if it also lacked `phase14-validate`
- `Documentation/zigux/review-checklist.md` lags the same route split by leaving `phase14-validate` in the same packet-local or repo-reality-gap bucket as the broader missing `phase14-*` wrappers
- `zigux/tests/README.md` already reflects the returned `phase14-validate` route split, so same-lane follow-through should not reopen that tests-root reminder unless a later shared-smoke edit makes it drift again
- `scripts/zigux/README.md` still spells out the older `ZIG=/absolute/path/to/attached-zig/zig make -C zigux phase14-*` examples directly, but it already frames them as packet-local traceability vocabulary rather than active Makefile-backed fallback guidance
- `scripts/zigux/check-phase14-shared-smoke-route.py` now keeps the shared `phase14-validate` gate explicit in both the readable Makefile body and the readable bootstrap workflow, so later same-lane follow-through should treat that checker as current route evidence instead of leaving it implied by adjacent reminder prose
...
- the current reminder packet should now record the narrower split truthfully, so later same-lane work does not reopen a closed tests-root alignment point or re-promote the older wrapper names as active Makefile-backed proof by mistake
...
1. keep `Documentation/zigux/phase14-end-to-end-smoke-survey.md`, `Documentation/zigux/phase14-release-boundary-survey.md`, `Documentation/zigux/phase14-productization-gap-survey.md`, `Documentation/zigux/phase14-shared-smoke-current-master-gap.md`, `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, and `zigux/tests/README.md` aligned on the fact that the attached-toolchain boundary is still explicit, the readable `zigux/Makefile` now exposes `phase14-validate`, and the broader `phase14-smoke`, `phase14-test`, and `phase14` names remain historical packet-local or repo-reality-gap vocabulary; today the surviving mismatch is isolated to the docs-root README plus the review checklist because the tests-root reminder is already aligned
2. keep `scripts/zigux/README.md` explicit about the older attached-toolchain wrapper examples only as packet-local traceability cues rather than active Makefile-backed fallback guidance
3. keep `scripts/zigux/check-phase14-shared-smoke-route.py`, `scripts/zigux/validate-phase14.py`, `scripts/zigux/check-phase14-release-boundary-exact-counts.py`, the workqueue reviewability shard, `zigux/tests/phase14_build.zig`, and the other executable packet members framed according to the exact readback mode that is actually available in this lane
