...
- `scripts/zigux/README.md` still spells out the older attached-toolchain wrapper triplet directly, but it already frames those names only as packet-local traceability vocabulary rather than active Makefile-backed fallback guidance; the remaining scripts-root drift is narrower and more concrete now, because that same Phase 14 summary still undercounts the returned dedicated `scripts/zigux/check-phase14-rcu-rollback-guardrail.py` guard and the returned `Documentation/zigux/phase14-rcu-tree-survey.md` companion beside the already-listed workqueue, ring-buffer, route-checker, validator, and release-boundary surfaces
...
- the smallest remaining reminder-side drift is now specific: `scripts/zigux/README.md` should be the first same-lane shared reminder surface reread against the returned `phase14-validate` split, the readable Makefile toolchain-selection chain, the returned dedicated RCU rollback guard, and the returned dedicated RCU survey note before reopening any broader docs-root or checklist follow-through
...
3. treat `scripts/zigux/README.md` as the next smallest same-lane reminder-side follow-through, because it is now the narrowest surviving summary surface that can drift against the readable Makefile toolchain-selection chain, the returned `phase14-validate` gate, the returned dedicated RCU rollback guard, and the returned dedicated RCU survey note without needing to reopen already-aligned docs-root or checklist wording first
