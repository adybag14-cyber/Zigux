...
- `zigux/tests/README.md` now keeps the attached-toolchain fallback explicit as packet-local rerun vocabulary and aligns its broader Phase 14 Makefile summary with the readable `zigux/Makefile` split by naming `phase14-validate` as present while keeping `phase14-smoke`, `phase14-test`, and `phase14` absent
...
- `zigux/tests/README.md` no longer belongs in that stale-summary bucket; its current Phase 14 packet already mirrors the returned `phase14-validate` route while keeping the broader three wrapper names absent
