...
- `summarizeWriteTeardownHandoff()` owns the bounded write-to-hangup handoff where retry-after-`-EAGAIN`, fatal-drop with no invented buffered bytes, partial-write carryover, stale-hangup buffered-byte preservation, notifier-hangup ownership, and kept console binding stay reviewable without claiming live transport or callback timing.
...
| write teardown handoff | `summarizeWriteTeardownHandoff()` | retry-after-`-EAGAIN`, fatal-drop without fabricated buffered bytes, partial-write carryover into hangup review, stale-hangup buffered-byte preservation, notifier-hangup ownership, and kept console binding | live putchars transport timing, notifier callback execution, and host-backed flush or disconnect races |
