...
Keep same-lane follow-through inside this slice-backed direct-helper packet by rereading `zigux/tests/phase7_rbtree_survey.zig` and `zigux/tests/phase7_rbtree_manifest.json` against this note so the returned `lib/rbtree.zig` helper path stays explicit, the returned shared `phase7-validate` route and already-landed public-fallback provenance for shared non-owner build evidence stay explicit, and `tools/lib/rbtree.zig` remains framed as readable legacy companion evidence rather than current helper-local ownership. The next smallest same-lane follow-through after this reminder repair is to decide whether `zigux/tests/phase7_rbtree_survey.zig` should also fail-close on this slice note's shared `phase7-validate` wording instead of checking that route only through the direct-anchor note. Do not widen into dedicated make-wrapper, workflow-recovery, or fixture-recovery lanes unless one more concrete still-missing companion surface such as `zigux/tests/fixtures/phase7_rbtree.json` or `zigux/tests/fixtures/phase7_rbtree_c_harness.c` rematerializes on current `master`.
