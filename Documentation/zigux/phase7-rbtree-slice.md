...
- ordered erase, erase-and-detach, and direct node replacement helpers
...
This lane is parked after the bounded helper surface compiled cleanly, the focused module tests passed, the shared Phase 7 helper gate continued to import and exercise the live `rbtree` slice, the dedicated survey gate and Linux-style `make -C zigux phase7-rbtree-survey` route stayed aligned around the same parked packet, the shared validator-first, dedicated make-wrapper, dedicated make-wrapper selftest-alignment, dedicated build-wiring, and Linux-style `make -C zigux phase7-validate` plus `make -C zigux phase7` routes stayed aligned around that same packet, and the committed parity fixture now locks ordered insert, duplicate-range lookup, erase-and-detach reset, replace, reverse traversal, and postorder behavior against the C helper surface while the focused Zig module tests also keep the landed `replaceNodeCached()` helper reviewable.
