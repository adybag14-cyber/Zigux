...
Keep same-lane follow-through inside this slice-backed direct-helper packet by rereading `zigux/tests/phase7_rbtree_survey.zig` and `zigux/tests/phase7_rbtree_manifest.json` against this note for shared non-owner build-evidence and public-fallback provenance, especially whether `zigux/tests/phase7_build.zig` still stays explicit as the only returned path that needed public blob/raw fallback after the authenticated contents bridge returned `404`, and do not widen into dedicated make-wrapper, workflow-recovery, roadmap-path, or fixture-recovery lanes unless one more concrete rbtree companion surface such as `lib/rbtree.zig`, `zigux/tests/fixtures/phase7_rbtree.json`, or `zigux/tests/fixtures/phase7_rbtree_c_harness.c` rematerializes on current `master`.