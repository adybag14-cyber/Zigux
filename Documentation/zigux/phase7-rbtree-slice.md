...
- `PHASE7_STATUS=helper_local_slice_note_build_test_survey_manifest_checker_fixture_harness_anchor`
...
- `zigux/tests/phase7_rbtree_build.zig`
...
- dedicated replay, helper-local build shard, survey, manifest, direct-anchor note, slice note, parity checker, JSON fixture, and C harness reviewability
...
- the dedicated helper-local build shard at `zigux/tests/phase7_rbtree_build.zig` keeps the rbtree replay and survey rerunnable without widening into Makefile wrappers or the broader shared Phase 7 graph
- same-lane truthfulness stays rooted at the returned runtime-root helper, the returned notes, the returned helper-local build shard, the returned survey, the returned manifest, the returned parity checker, the returned JSON fixture, and the returned C harness
...
- same-lane truthfulness keeps the returned slice note, direct-anchor note, parity checker, replay, helper-local build shard, survey, manifest, JSON fixture, and C harness explicit
- helper-local build-surface truthfulness keeps the dedicated build shard at `zigux/tests/phase7_rbtree_build.zig` explicit while shared non-owner evidence such as `tools/lib/rbtree.zig`, `scripts/zigux/check-phase7-build-wiring.py`, `scripts/zigux/validate-phase7.py`, `zigux/tests/phase7_build.zig`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml` stays outside this packet
- public-fallback provenance stays explicit through the now-empty `public_fallback_non_owner_paths` field in `zigux/tests/phase7_rbtree_manifest.json`, because the dedicated helper-local build shard, `zigux/tests/phase7_build.zig`, and the other listed legacy or shared non-owner surfaces all rematerialized through authenticated rereads in this slot
...
Keep same-lane follow-through inside this slice-backed direct-helper packet by replaying `zigux/tests/phase7_rbtree_build.zig` against a live checkout or deciding whether the sibling `argv_split` packet needs a matching helper-local build shard. Do not widen into Makefile wrappers, workflow-recovery, or broader shared-control lanes unless those surfaces actually return.
