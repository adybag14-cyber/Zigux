...
- `verify.zig` now directly imports `logging.zig`, `perf_buffer_poll.zig`, `pin_path.zig`, and `type_names.zig`, keeps their bounded entrypoints explicit, and now replays stable logging, type-name, pin-path, and perf-buffer helper outputs inside one narrow verifier shard.
...
Current `master` now exposes a slightly larger helper-first Phase 8 libbpf packet because `logging.zig` and `perf_buffer_poll.zig` are exact-readable current-head evidence again, but the directly readable subset is still smaller than the broader bridge-and-build packet named by docs-root reminder surfaces.
...
- `verify.zig`, `logging.zig`, `perf_buffer_poll.zig`, `type_names.zig`, and `pin_path.zig` are current exact-readable evidence
...
Preferred order:
1. reread `verify.zig`, `logging.zig`, `perf_buffer_poll.zig`, `type_names.zig`, `pin_path.zig`, `Documentation/zigux/README.md`, and this survey note before treating any libbpf shard as removed
