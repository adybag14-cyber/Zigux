...
- bounded `/proc//fdinfo/` path assembly, compact fdinfo map-info parsing, helper-only reused-map compatibility checks, planning-only reopen outcomes, planning-only token-preparation outcomes, and planning-only token bridge path and attempt outcomes stay explicit without widening into direct procfs reads or pinned-object reopen flow
