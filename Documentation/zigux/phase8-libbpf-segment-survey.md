...
Current `master` now exposes a slightly larger helper-first Phase 8 libbpf packet because `perf_buffer_poll.zig` is exact-readable current-head evidence again, but the directly readable subset is still smaller than the broader bridge-and-build packet named by docs-root reminder surfaces.
...
- `verify.zig`, `perf_buffer_poll.zig`, `type_names.zig`, and `pin_path.zig` are current exact-readable evidence
...
Preferred order:
1. reread `verify.zig`, `perf_buffer_poll.zig`, `type_names.zig`, `pin_path.zig`, `Documentation/zigux/README.md`, and this survey note before treating any libbpf shard as removed
