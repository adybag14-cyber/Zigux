...
That timing-adjacent note is also now the roadmap-safe place to keep timeout classification explicit: current `master` already lands bounded `perf_buffer__poll(timeout_ms)` wait-result shaping and ready-buffer bookkeeping, but it still stops short of standalone timer helper behavior, standalone clockevent helper behavior, or broader timeout-sensitive routing behavior.
...
That keeps the next honest same-lane step smaller than new helper growth: preserve the helper-plus-build readback here, keep the checker-backed helper-local routing evidence explicit, and repair this survey whenever its truthfulness falls behind the landed checker or bridge-plus-build packet.
...
2. keep the manifest-backed `fdinfo-map-info-helpers`, `map-reuse-compatibility`, planning-only `planTokenPreparation()` gate, and deferred `file-path-and-handle-bridge` segmentation explicit without widening into procfs reads, token materialization, bpffs reopen flow, or fd ownership claims
3. keep the helper-local `online_cpu_routing.zig` evidence explicit, keep `Documentation/zigux/phase8-perf-buffer-poll-slice.md` explicit as the bounded timing-adjacent note for `perf_buffer__poll(timeout_ms)` classification, and stay smaller than deferred `perf-buffer-online-cpu-routing`, `perf_event_open()` setup, epoll registration, mmap-backed ring ownership, standalone timer or clockevent helper behavior, or broader timeout-sensitive routing behavior
