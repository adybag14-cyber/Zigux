...
`tools/lib/bpf/zigux_segments/manifest.json` has since advanced both `fdinfo-map-info-helpers` and `map-reuse-compatibility` as landed helper-first slices with the newer shared bridge rationale, so the smallest same-family reminder drift is now that the focused bridge test still quotes the older pre-landing `why_now` strings for those two slices rather than the current landed wording.
...
3. if sibling reminder surfaces drift again against the current helper-plus-build evidence, correct the smallest reminder sentence first; the current same-family follow-through to watch is the focused bridge test's older pre-landing `why_now` wording for `fdinfo-map-info-helpers` and `map-reuse-compatibility`
