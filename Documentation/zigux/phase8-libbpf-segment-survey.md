...
- bounded fdinfo map metadata parsing accepts reordered lines and explicit `map_flags` or `map_extra` bases, keeps omitted fields zero-initialized, resolves duplicate keys last-wins, and still rejects malformed values
- parsed `map_extra` values now stay available to the same helper's reuse-compatibility check, so mmap-oriented map metadata no longer falls back to a silent zero during the bounded fdinfo path
