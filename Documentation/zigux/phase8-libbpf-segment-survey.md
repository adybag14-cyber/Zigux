...
The readable helper packet now keeps stable cpu-mask parsing, logging/version/errno formatting, pin-path composition and sanitization, type-name formatter outputs, perf-buffer summary bookkeeping, ready-buffer window mapped-size and lookup-return outputs, helper-local online-CPU routing outputs, and cpu-mask-backed next-route CPU-index and buffer-FD wrappers explicit below the still-deferred setup-side routing boundary.
