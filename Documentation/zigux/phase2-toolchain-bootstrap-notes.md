...
- `zigux/tests/README.md`, `scripts/zigux/kconfig/conf_bridge.zig`, `scripts/zigux/kconfig/confdata_bridge.zig`, `zigux/tests/fixtures/phase2_tool_manifest.json`, `zigux/tests/fixtures/phase2_cross_targets.json`, and the `zigux/tests/fixtures/kconfig_bridge/` manifest roster keep the bounded tests-facing, toolchain, cross-target, and bridge packet reviewable without widening into missing closure routes.
...
- Treat the absent validator-first, direct toolchain-resolution, cross-route, installer, and Linux-style make replay names as historical packet members until same-lane work rematerializes them on `master`.
