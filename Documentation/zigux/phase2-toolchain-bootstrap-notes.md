...
- `scripts/zigux/check-zig-toolchain.py` keeps the current pinned-Zig resolution, minimum-version, and policy-only replay surface explicit.
...
- `zigux/tests/README.md`, `scripts/zigux/kconfig/conf_bridge.zig`, `scripts/zigux/kconfig/confdata_bridge.zig`, `zigux/tests/fixtures/phase2_tool_manifest.json`, `zigux/tests/fixtures/phase2_artifact_tools_manifest.json`, `zigux/tests/fixtures/phase2_cross_targets.json`, and the `zigux/tests/fixtures/kconfig_bridge/` manifest roster keep the bounded tests-facing, toolchain, artifact, cross-target, and bridge packet reviewable without widening into missing closure routes.
