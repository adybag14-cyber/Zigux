...
- `python3 scripts/zigux/check-phase2-kconfig-selftest-alignment.py --self-test` and `python3 scripts/zigux/check-phase2-kconfig-selftest-alignment.py` keep this bootstrap note aligned with the already-shipped `check-kconfig-bridge.py` route, the workflow-backed self-test plus live guard, the Phase 2 closure record, the `phase2-kconfig` Makefile lane, and the bounded `zig test scripts/zigux/kconfig/conf_bridge.zig` plus `zig test scripts/zigux/kconfig/confdata_bridge.zig` replays so the bounded kconfig bridge packet stays visible beside the archive pin instead of being implied only by closure-only surfaces
