...
- shared confdata helper-anchor self-test: `python3 scripts/zigux/check-phase2-confdata-helper-anchor-alignment.py --self-test`
- shared confdata helper-anchor gate: `python3 scripts/zigux/check-phase2-confdata-helper-anchor-alignment.py`
...
- `scripts/zigux/check-zig-toolchain.py` also emits `ZIG_TOOLCHAIN_ARCHIVE_TARGET`, `ZIG_TOOLCHAIN_ARCHIVE_EXPECTED_SHA256`, and `ZIG_TOOLCHAIN_ARCHIVE_SHA256_STATUS=verified` when that repo-local cache is present, so the pinned archive policy stays reviewable through live checker output instead of living only in `scripts/zigux/zig-toolchain-policy.json`
...
- `python3 scripts/zigux/check-phase2-kconfig-selftest-alignment.py --self-test` and `python3 scripts/zigux/check-phase2-kconfig-selftest-alignment.py` keep this dedicated bootstrap note aligned with `Documentation/zigux/phase2-closure.md`, `Documentation/zigux/phase2-confdata-bridge-survey.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/validate-phase2.py`, and `scripts/zigux/validate-phase2-closure.py` so the shipped `16-case` conf bridge plus `13-case` confdata fixture replay stays reviewable from the same Phase 2 note instead of being implied only by sibling reminder surfaces
- `python3 scripts/zigux/check-phase2-confdata-helper-anchor-alignment.py --self-test` and `python3 scripts/zigux/check-phase2-confdata-helper-anchor-alignment.py` keep this dedicated bootstrap note aligned with `scripts/zigux/kconfig/confdata_bridge.zig`, `zigux/tests/fixtures/kconfig_bridge/confdata_manifest.json`, `Documentation/zigux/phase2-confdata-bridge-survey.md`, `scripts/zigux/validate-phase2.py`, and `scripts/zigux/validate-phase2-closure.py` so the shipped confdata helper-local anchor packet stays reviewable from the same Phase 2 note instead of being left implicit in sibling reminder surfaces
