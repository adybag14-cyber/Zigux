...
- `python3 scripts/zigux/check-fixdep-diff.py --self-test` and `python3 scripts/zigux/check-fixdep-diff.py` keep this bootstrap note aligned with the already-shipped workflow-backed fixdep artifact replay, the `phase2-tools` Makefile lane, and the bounded direct `zig test scripts/zigux/fixdep.zig` replay so the dedicated `fixdep.zig` helper surface stays visible beside the archive pin instead of living only in the closure record
