...
- `python3 scripts/zigux/check-phase2-fixdep-gate.py --self-test`, `python3 scripts/zigux/check-phase2-fixdep-gate.py`, `python3 scripts/zigux/check-fixdep-diff.py --self-test`, and `python3 scripts/zigux/check-fixdep-diff.py` keep this bootstrap note aligned with the already-shipped workflow-backed fixdep gate ordering, the `phase2-validate` plus `phase2-tools` Makefile lanes, and the bounded direct `zig test scripts/zigux/fixdep.zig` replay so the dedicated `fixdep.zig` helper surface and its CI packet stay visible beside the archive pin instead of living only in the closure record
...
- `python3 scripts/zigux/check-mk-elfconfig-diff.py` keeps this bootstrap note aligned with the already-shipped workflow-backed parity replay, the `phase2-tools` Makefile lane, and the bounded direct `zig test scripts/zigux/mk_elfconfig.zig` replay so the dedicated `mk_elfconfig.zig` helper surface stays visible beside the archive pin instead of living only in the closure record
