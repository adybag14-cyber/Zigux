...
- attached-toolchain fallback commands:
  - `make -C zigux phase14-smoke ZIG=`
  - `make -C zigux phase14-test ZIG=`
  - `make -C zigux phase14 ZIG=`
...
- `Documentation/zigux/README.md`, `Documentation/zigux/phase14-release-boundary-survey.md`, `Documentation/zigux/review-checklist.md`, `zigux/tests/README.md`, `zigux/tests/phase14_end_to_end_smoke_manifest.json`, and `zigux/Makefile` already converge on a study-only smoke packet with `phase14-smoke`, `phase14-test`, and `phase14` replay routes.
...
- there is no shared `validate-phase14.py`, `check-phase14-docs-root-smoke-summary.py`, `check-phase14-release-boundary-exact-counts.py`, or `make -C zigux phase14-validate` surface on current `master`; the honest shared packet is smoke-and-test replay plus the aligned docs and manifest evidence above.

## Productization evidence

- named owner: `Core-Adjacent Pod`
- status bucket: `study_only`
- validation gate: `make -C zigux phase14-smoke && make -C zigux phase14-test && make -C zigux phase14`
...
- fallback path: keep `kernel/workqueue.c`, `net/core/skbuff.c`, `kernel/trace/ring_buffer.c`, and `kernel/rcu/tree.c` as the source of truth and keep the shared smoke packet limited to survey-backed reviewability evidence.
...
1. Run the focused smoke shard:
   - `make -C zigux phase14-smoke`
   - `zig build phase14-smoke --build-file zigux/tests/phase14_build.zig --summary all`
2. Run the shared full-bundle replay:
   - `make -C zigux phase14-test`
   - `zig build test --build-file zigux/tests/phase14_build.zig --summary all`
3. Run the convenience wrapper:
   - `make -C zigux phase14`
4. Use the attached-toolchain fallback only when `zig` is not already on `PATH`:
   - `make -C zigux phase14-smoke ZIG=`
   - `make -C zigux phase14-test ZIG=`
   - `make -C zigux phase14 ZIG=`
