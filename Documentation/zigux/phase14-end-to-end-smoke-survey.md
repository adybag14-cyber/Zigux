...
Keep the anchor-local packets parked. The next same-lane truthfulness repair is scripts-root only: current `master` still jumps from the shared Phase 13 contributor packet to the Phase 15 governance packet in `scripts/zigux/README.md`, so the scripts-root reminder omits the live Phase 14 shared smoke packet entirely, including the cross-anchor note `Documentation/zigux/phase14-core-boundary-traceability.md`. Any follow-up should therefore stay limited to restoring that scripts-root reminder surface rather than reopening anchor-local study surfaces. All four anchor packets remain parked on blocked or governance-only posture, so no anchor-local reopen is justified from this lane unless a new shared-packet mismatch appears.
