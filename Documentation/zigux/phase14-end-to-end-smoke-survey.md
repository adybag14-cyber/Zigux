...
## Gates
  ...
  * `ZIG=/absolute/path/to/attached-zig/zig make -C zigux phase14`
## Next bounded step
Keep this core-adjacent lane parked on the current validator-local exactness follow-through until `scripts/zigux/validate-phase14.py` also exact-counts the already-landed tests-readme smoke checker self-test and live rerun from `zigux/Makefile`.
Current `master` already carries the restored scripts-root reminder, the tests-readme smoke checker, and the Makefile reruns, but the validator exactness follow-through is still open. The next same-lane step should publish that one-file `scripts/zigux/validate-phase14.py` repair before reopening broader shared-packet rereads.
All four anchor packets remain parked on blocked or governance-only posture, so no anchor-local reopen is justified from this lane until that validator-local shared-packet mismatch is closed.
## Footer