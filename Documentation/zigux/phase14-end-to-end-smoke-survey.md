...
- `Documentation/zigux/README.md`, `Documentation/zigux/phase14-release-boundary-survey.md`, `Documentation/zigux/review-checklist.md`, and `zigux/tests/README.md` still converge on the same broader study-only smoke packet, while `Documentation/zigux/phase14-end-to-end-smoke-survey.md`, `scripts/zigux/README.md`, `scripts/zigux/validate-phase14.py`, `zigux/tests/phase14_end_to_end_smoke_manifest.json`, `.github/workflows/zigux-bootstrap.yml`, and `zigux/Makefile` now converge on the dedicated `phase14-validate` route.
...
- `scripts/zigux/validate-phase14.py` now keeps the shared validate entrypoint fail-closed by requiring the manifest command inventory, the shared smoke survey, the scripts-root flow, the workflow step, and the `phase14: phase14-validate phase14-smoke phase14-test` make wrapper to stay aligned.
...
1. Run the shared validation route:
   - `make -C zigux phase14-validate`
