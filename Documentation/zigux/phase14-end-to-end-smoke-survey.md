...
- `zigux/Makefile` still exposes `make -C zigux phase14-test` as the wrapper-backed full-bundle replay, still exposes `make -C zigux phase14-smoke` as the focused shared smoke shard, and still honors the standard `ZIG` environment override so the attached archive can be injected with the literal `ZIG=/absolute/path/to/attached-zig/zig` examples above when the default toolchain path is unavailable in the local shell.
...
Keep this shared smoke lane parked unless one of the four anchor-local manifests, survey notes, the compile shard matrix, or the shared replay wiring drifts. Current `master` already narrows the smallest same-lane reopen target to one review-facing correction: `Documentation/zigux/review-checklist.md` still undercounts two live shared-smoke surfaces that the manifest, this survey note, `scripts/zigux/validate-phase14.py`, and `scripts/zigux/README.md` already carry, namely `Documentation/zigux/phase14-core-boundary-traceability.md` and `scripts/zigux/check-phase14-tests-readme-smoke-summary.py`. The next safe correction is to align that one shared review prompt with the existing smoke packet before touching any deeper Phase 14 study surfaces. All four anchor packets remain parked on blocked or governance-only posture with no smaller ready-next gap recorded in the shared packet, so no anchor-local reopen is justified from this lane until that shared reminder drift is closed.
