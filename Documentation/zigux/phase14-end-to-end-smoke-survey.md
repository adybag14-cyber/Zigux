...
## Shared smoke findings
  ...
  * the current scripts-root and tests-root reminders already mirror the same Phase 14 route split and keep the missing `phase14-*` wrappers framed as packet-local or repo-reality-gap vocabulary, so this shared note should stay aligned with that current posture instead of sending the lane back toward an already-closed tests-root rewrite
