...
Keep the anchor-local packets parked. Current `master` already carries the dedicated Phase 14 scripts-root summary plus the dedicated tests-readme smoke checker and its validator-first alignment, so the older same-lane follow-up that treated the whole scripts-root packet as missing is now stale. The remaining scripts-root drift is narrower: `scripts/zigux/README.md` still omits `Documentation/zigux/phase14-core-boundary-traceability.md` from the shared smoke packet even though this survey note, the docs root, the release-boundary note, the validator, and the tests-root packet already keep that cross-anchor traceability companion explicit. Any follow-up should therefore stay limited to restoring that scripts-root marker and then tightening `scripts/zigux/validate-phase14.py` so the scripts-root reminder fails closed on the current full Phase 14 docs packet rather than reopening anchor-local study surfaces. All four anchor packets remain parked on blocked or governance-only posture, so no anchor-local reopen is justified from this lane unless a new shared-packet mismatch appears.
