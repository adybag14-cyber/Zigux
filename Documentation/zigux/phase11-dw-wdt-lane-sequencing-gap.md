...
- the surviving repo-reality gap inside the authenticated packet is still the broader direct-driver, verify-helper, replay, validation-matrix, survey, slice, and teardown-note stack together with the older packet-checker handle, not the returned restart helper, PM helper pair, manifest-backed scaffold, or paired truthfulness checkers
...
- keep the manifest, the registration scaffold, the restart helper, the PM helper pair, the reminder packet, and the checker pair explicit
