...
- the surviving repo-reality gap inside the authenticated packet is still the broader direct-driver, verify-helper, driver-test, slice, and teardown-note stack together with the older packet-checker handle, not the returned survey note, validation matrix, survey gate, restart helper, PM helper pair, manifest-backed scaffold, or paired truthfulness checkers
...
- keep the manifest, the survey gate, the survey note, the validation matrix, the registration scaffold, the restart helper, the PM helper pair, the reminder packet, and the checker pair explicit
