...
- `scripts/zigux/fixdep.zig` now carries twenty-four helper-local tests covering CONFIG token trimming, prefixed-token rejection, punctuation and embedded-NUL handling, comment-only depfiles, escaped-space and escaped-newline parsing, CRLF and bare-carriage-return behavior, read-error wording, larger-than-legacy depfiles, flush-primary-error preservation, and the escaped hash or colon comment-path survivors.
