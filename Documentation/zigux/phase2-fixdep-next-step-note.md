...
- The live bootstrap workflow and `zigux/Makefile` still expose direct fixdep replay routes through `zig run check_phase2_fixdep_gate.zig`, `zig run check_fixdep_diff.zig`, `make -C zigux phase2-fixdep`, and `zig test scripts/zigux/fixdep.zig`.
