...
Against the roadmap, the remaining gap here is still broader unfinished Phase 3 interop-substrate coverage outside this starter packet, not a missing export/UAPI companion, missing shared-route wiring, missing starter boundary-header validation relay inside the packet itself, or a missing kernel-facing governance note for `zigux/kernel/export_shim.zig`.

This survey should keep the manifest-backed ABI inventory, the returned linux-header governance note, the dedicated kernel export-shim governance note, the returned catalog-selftest guard, the live curated binding companions, the already-shipped starter boundary-header validation relay, the already-shipped starter `dev_t` validation relays, the shared tests-root replay wiring, and the direct C smoke replay explicit as shipped same-family evidence so the roadmap's permanent boundary does not get understated as shim-plus-UAPI only.
