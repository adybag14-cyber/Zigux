...
- `zigux/tests/fixtures/phase3_abi_manifest.json` keeps the shared Phase 3 inventory explicit for `zigux/kernel/export_shim.zig`, `zigux/uapi/version.zig`, `zigux/uapi/dev_t.zig`, `include/zigux/dev_t.h`, this survey, and the shared dump and build route anchors, so reviewers can see the starter boundary remains manifest-backed without reintroducing retired export/UAPI-only replay files.
...
The Phase 3 roadmap still requires a narrow and explicit export shim plus starter UAPI boundary. On the current inspected `master`, the remaining same-lane reminder gap was packet-local proof for the shared manifest-backed inventory that already carries this starter surface. This survey now pins that shared manifest beside `zigux/kernel/export_shim.zig`, `zigux/uapi/version.zig`, `zigux/uapi/dev_t.zig`, and `include/zigux/dev_t.h`, so no larger export/UAPI starter-boundary gap remains visible on the current inspected head.

- current `master` already ships `Documentation/zigux/phase3-kernel-export-shim-governance.md`, `zigux/kernel/export_shim.zig`, `zigux/uapi/version.zig`, `zigux/uapi/dev_t.zig`, `include/zigux/dev_t.h`, and `zigux/tests/fixtures/phase3_abi_manifest.json`, and the dedicated survey, kernel-facing governance note, header-family survey, and manifest-backed inventory now keep that starter packet aligned, including the explicit accepted-header extension-byte accounting exposed through the starter version helper and export shim relay.
- the remaining lane rule is to keep the shared interop, build, dump, and manifest wording accurate and to avoid claiming dedicated `phase3_export_uapi*` replay files unless those files actually land alongside the shared ABI packet and its reminder surfaces.
