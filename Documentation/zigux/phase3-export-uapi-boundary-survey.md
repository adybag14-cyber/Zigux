...
Current `master` does not currently carry `zigux/tests/phase3_export_uapi_layout_build.zig`, so the older dedicated `phase3-export-uapi-layout-test` route remains unshipped compile wiring rather than live same-family evidence.
...
The packet now also keeps a direct C-facing smoke proof for the Linux-header relays, and it keeps a dedicated kernel-facing governance note for `zigux/kernel/export_shim.zig`, so the remaining gap here is no longer missing shared-route wiring, missing compile evidence for the named C boundary helpers, missing proof that the exported boundary-header validation relay is callable from C, or missing a packet-local owner map for the shim itself.
...
Against the roadmap, the remaining gap here is still broader unfinished Phase 3 interop-substrate coverage outside this starter packet, plus the missing dedicated standalone layout-build handoff for the already-shipped `phase3_export_uapi_layout.zig` replay, not a missing export/UAPI companion, missing shared-route wiring, missing starter boundary-header validation relay inside the packet itself, or a missing kernel-facing governance note for `zigux/kernel/export_shim.zig`.

This survey should keep the shared tests-root export/UAPI layout replay, the direct C smoke replay, the kernel-facing governance note, and the starter validation relays explicit as shipped same-family evidence while treating the absent dedicated `phase3_export_uapi_layout_build.zig` handoff as a current repo-reality gap.
...
It does not claim a larger UAPI family or a dedicated standalone layout-build handoff than the current starter packet actually ships.