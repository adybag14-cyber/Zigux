...
- current `master` already ships `Documentation/zigux/phase3-kernel-export-shim-governance.md`, `Documentation/zigux/phase3-linux-zigux-header-governance.md`, `zigux/kernel/export_shim.zig`, `zigux/uapi/version.zig`, `zigux/uapi/dev_t.zig`, `include/linux/zigux.h`, `include/zigux/dev_t.h`, and `zigux/tests/fixtures/phase3_abi_manifest.json`, and the dedicated survey, kernel-facing governance note, header-governance note, header-family survey, and manifest-backed inventory now keep that starter packet aligned, including the explicit accepted-header extension-byte accounting exposed through the starter version helper, the export shim relay, the kernel-facing relay over the shipped starter `dev_t` encode and range surface, the C-facing compatible-header constructor, and the dedicated `include/linux/zigux.h` blob marker that keeps the Linux-facing relay evidence explicit beside the starter UAPI file markers.
