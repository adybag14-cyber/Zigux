...
- `PHASE3_LINUX_ZIGUX_H_BLOB_SHA=e503304fbe2e9265b205d74bcc8b93e48bd358f0`
...
The Phase 3 roadmap still requires a narrow and explicit export shim plus starter UAPI boundary. On the current inspected `master`, the packet-local survey drift is closed: this note now pins the shared manifest-backed starter packet around `zigux/kernel/export_shim.zig`, `zigux/uapi/version.zig`, `zigux/uapi/dev_t.zig`, `include/linux/zigux.h`, and `include/zigux/dev_t.h`, while the Linux-facing relay now exposes named canonical and forward-compatible constructors plus thin ABI-version and canonical-versus-compatible size predicates beside the already-shipped starter helpers. The remaining repo-reality gap visible from this lane is broader reminder truthfulness, not missing starter boundary scaffold: `Documentation/zigux/README.md` still summarizes Phase 3 mainly through the shared ABI slice, header-family reminder, policy-and-unsafe survey, low-level-wrapper survey, and validator-support packet, so the top-level docs root still understates this dedicated export/UAPI survey, its packet-local validator, and the shipped starter boundary files even though this note and `scripts/zigux/README.md` now keep that starter packet explicit on current `master`.

- current `master` already ships `Documentation/zigux/phase3-kernel-export-shim-governance.md`, `Documentation/zigux/phase3-linux-zigux-header-governance.md`, `zigux/kernel/export_shim.zig`, `zigux/uapi/version.zig`, `zigux/uapi/dev_t.zig`, `include/linux/zigux.h`, `include/zigux/dev_t.h`, and `zigux/tests/fixtures/phase3_abi_manifest.json`, and the dedicated survey, kernel-facing governance note, header-governance note, header-family survey, and manifest-backed inventory now keep that starter packet aligned, including the explicit accepted-header extension-byte accounting exposed through the starter version helper, the kernel-facing evaluation and status relays in `zigux/kernel/export_shim.zig`, the starter decode view over `dev_t`, the C-facing canonical and compatible-header constructors, the thin ABI-version and size predicates, and the dedicated `include/linux/zigux.h` blob marker that keeps the Linux-facing relay evidence explicit beside the starter UAPI file markers.
- the remaining same-lane rule is to keep the shared interop, build, dump, and manifest wording accurate, including the shared `make -C zigux phase3-abi`, `make -C zigux phase3-interop`, and `make -C zigux phase3` reminders, and to avoid claiming dedicated `phase3_export_uapi*` replay files unless those files actually land alongside the shared ABI packet and its reminder surfaces.
- the next adjacent shared-reminder repair is to make `Documentation/zigux/README.md` name this dedicated export/UAPI survey, its packet-local validator, and the starter export/UAPI boundary files directly instead of leaving that surface discoverable only through this survey and the scripts-root reminder.
