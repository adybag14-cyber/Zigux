...
- `zigux/kernel/export_shim.zig` now keeps wrapper ownership equally explicit by forwarding `canonicalHeader`, `compatibleHeader`, `versionedHeader`, `headerCompatibility`, `canonicalizeHeader`, `isCurrentAbiVersion`, `isCompatibleSize`, `isCanonicalSize`, `isCompatibleHeader`, and `isCanonicalHeader` straight to `zigux/uapi/version.zig`, so the kernel-facing shim does not silently fork boundary-header policy away from the curated UAPI starter
...
- `zigux/tests/phase3_export_uapi_layout.zig` now keeps the canonical boundary-header and export-status size and field-offset contract on its own focused layout replay, and `zigux/tests/phase3_export_uapi_layout_build.zig` keeps that check callable as `phase3-export-uapi-layout-test` beside the broader export/UAPI smoke gate while also proving that future-compatible headers canonicalize back to the current shape and mismatched-version headers still fail canonicalization under the same narrow packet
...
- `zigux/tests/fixtures/phase3_abi/phase3_abi_manifest.json` and `zigux/Makefile` already keep that same focused layout replay named beside the broader export/UAPI smoke gate, so the canonical boundary layout contract does not disappear into the larger Phase 3 bundle
...
- `zigux/tests/fixtures/phase3_abi/phase3_abi_manifest.json` now carries those focused replay paths inside the same ABI substrate packet rather than presenting them as a broader UAPI tranche
...
- keep `zigux/kernel/export_shim.zig` delegating boundary-header construction, compatibility classification, canonicalization, and current-version or size predicates to `zigux/uapi/version.zig` so future kernel-facing growth does not fork boundary policy into a second local owner
