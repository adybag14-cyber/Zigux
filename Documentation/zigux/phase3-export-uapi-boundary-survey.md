...
- the export shim now operates as a relay, explicit compatibility-evaluation wrapper, and status-normalization layer; it does not yet establish another separately owned top-level boundary family, new export-facing header, or wider generated-or-curated entry-point root beyond the shipped starter helpers.
- the next safe packet-local step is one new top-level boundary family that either extends `zigux/uapi/` beyond `version.zig` or lands as another explicitly owned export-facing boundary root, with direct behavior replay, focused layout or dump proof, paired build-wrapper refresh when those replay surfaces change, manifest inclusion, a `scripts/zigux/validate-phase3-export-uapi-survey.py` refresh when the packet-local evidence list changes, a shared review-surface refresh, and the corresponding `Documentation/zigux/phase3-linux-zigux-header-governance.md` refresh.
