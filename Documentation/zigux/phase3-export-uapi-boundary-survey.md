...
- `PHASE3_EXPORT_UAPI_LAYOUT_BLOB_SHA=b839c55a0f49fbc26ae5d7b799926bbf002b0a45`
...
- this starter packet still rolls up under ledger entry `26`, `feat(zigux): start bounded Phase 3 abi substrate skeleton`, even now that the shipped UAPI starter spans both `zigux/uapi/version.zig` and `zigux/uapi/dev_t.zig`; widening this note without a new top-level family should still be treated as bounded ABI-substrate follow-through rather than a separate tranche.
...
- the export shim still operates as a relay, explicit compatibility-tag and compatibility-evaluation wrapper, and status-normalization layer; it does not yet establish another separately owned top-level boundary family, new export-facing header, or wider generated-or-curated entry-point root beyond the shipped starter helpers.
