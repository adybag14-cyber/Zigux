...
Leave the hexdump helper lane parked unless fresh repo inspection finds a concrete parity, perf, or directly coupled review-packet drift inside `lib/hexdump.zig`, `zigux/tests/phase6_hexdump.zig`, `zigux/tests/phase6_hexdump_perf.zig`, `zigux/tests/phase6_hexdump_c_parity.zig`, `zigux/tests/fixtures/phase6_hexdump_vectors.zig`, `zigux/tests/fixtures/phase6_hexdump_c_harness.c`, `scripts/zigux/check-phase6-hexdump-c-parity.py`, or the shared Phase 6 packet. The widened external grouped-2 parity packet still covers 29 direct C-vs-Zig lines and the shared catalog, machine-readable manifest, and shared validator still agree on that hexdump-local helper surface, but the broader Phase 6 packet separately still carries a parked-versus-active status mismatch between the parked catalog plus helper slice notes and the active manifest plus shared validator expectation. The next honest same-family follow-up should therefore stay narrow to that shared-packet status repair rather than reopening `lib/hexdump.zig` or its directly coupled hexdump parity and perf harnesses.
