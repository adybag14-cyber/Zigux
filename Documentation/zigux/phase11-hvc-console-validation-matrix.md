...
| direct companion gap | the survey gate, survey note, slice note, teardown note, checker, and this matrix all now make the missing direct companion trio explicit instead of presenting them as landed replay evidence | the dedicated survey route and packet checker keep those repo-reality gaps visible and fail closed if the wording drifts back into overclaiming | if one of those direct companions lands later, update only the same-packet HVC governance surfaces and checker to match the newly landed scope | broader runtime-parity closure, shared Phase 11 closure claims, and non-HVC driver work |

## Failure-Mode Evidence

- final-close teardown wording, explicit close-wait ownership, and cleanup-time tty-port ownership stay explicit through the driver starter, the survey gate, the survey note, the teardown note, and this matrix without claiming direct replay or cleanup companions that current `master` does not ship.
