...
The bounded starter now covers slot validation, CRLF framing, flush-progress intent, teardown, the final-close wait boundary, and a tiny tty-registration handoff summary. The live repo still needed one reviewable note that explains:

- which parts of the lane are already exercised by the shared Phase 11 gate
- which khvcd and notifier-facing behaviors are still only documented as the next follow-up
...
| close-wait teardown boundary | `summarizeCloseBoundary()` records final-close, hung-up close, `HVC_CLOSE_WAIT`-shaped wait intent, and `port_initialized` clearing without claiming live tty teardown | `zig build test --build-file zigux/tests/phase11_build.zig --summary all` via the final-close and hung-up-close checks in `zigux/tests/phase11_hvc_console.zig` | carry the same final-close evidence forward while the lane deepens khvcd and notifier boundary summaries | real tty core teardown, khvcd worker wakeups, and backend drain timing |
...
- exact shared outcome:
  - `Build Summary: 17/17 steps succeeded; 41/41 tests passed`
  - included hvc artifact: `run test phase11-hvc-console-tests 6 pass (6 total)`
...
- treat this lane as a bounded driver-starter plus validation-note lane until a khvcd polling-contract helper actually lands
...
- when the khvcd polling-contract helper lands, update this matrix, the slice note, the survey note, and the survey manifest together so the lane keeps one truthful next step