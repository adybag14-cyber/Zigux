...
- `PHASE11_HVC_CONSOLE_STATUS=hvc_remove_handoff_landed`
...
The bounded starter now covers slot validation, CRLF framing, flush-progress intent, teardown, the final-close wait boundary, and a tiny remove-path handoff summary. The live repo still needed one reviewable note that explains:
