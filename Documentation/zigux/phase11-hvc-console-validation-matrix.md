...
| route and checker boundary | `scripts/zigux/check-phase11-build-inventory.py` and `scripts/zigux/check-phase11-hvc-cleanup-current-head.py` remain directly readable, while `scripts/zigux/check-phase11-hvc-survey-packet.py` and a dedicated `make -C zigux phase11-hvc-survey` route do not; the standalone targetless-unregister witness build shard is directly readable, and its focused route is still exercised by both `.github/workflows/zigux-bootstrap.yml` and `zigux/Makefile` `phase11-validate`, while intentionally remaining outside the shared build-inventory route set for now | keep the build-inventory checker and the current-head cleanup checker explicit, keep the bootstrap workflow self-testing and running the shared build-inventory guard before the shared proof replay set, and keep the separate targetless-unregister witness exercised as a standalone route rather than promoting it into a shared route claim | if the dedicated survey checker or route returns, refresh the matrix and survey together; if the shared build-inventory packet expands again, refresh the checker and inventory in one bounded pass | reviving route or checker claims from historical wording alone |
