...
| helper-local failure-mode edges | keep helper-local failure-mode edges reviewable through `Documentation/zigux/phase11-hvc-verify-helper-boundary.md` rather than treating `drivers/tty/hvc/hvc_console_verify.zig` as a returned direct-readback anchor | preserve helper-local cleanup, notifier-unregister, targetless-dispatch, and remove-handoff wording through the boundary note instead of through an absent helper file | if `drivers/tty/hvc/hvc_console_verify.zig` returns on a future reread, refresh this matrix and the coupled note together | treating helper-local review history as proof of live notifier or sysrq execution |
