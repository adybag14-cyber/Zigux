...
- those same direct contents rereads do not currently rematerialize `Documentation/zigux/phase11-dw-wdt-slice.md`, `Documentation/zigux/phase11-dw-wdt-teardown-note.md`, `drivers/watchdog/dw_wdt.zig`, or `zigux/tests/phase11_dw_wdt.zig`, so keep that broader direct-driver, direct replay, and older reminder stack framed as larger same-lane vocabulary until a future direct reread proves it returned through the same bridge
...
- no claim that the broader direct driver, driver-test, slice, or teardown-note stack has returned on this direct contents bridge without a fresh reread
