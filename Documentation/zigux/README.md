...
Phase 4 notes
- `Documentation/zigux/phase4-validation-matrix.md` is the shared Phase 4 notes record for rollback ownership, lab and CI matrix, perf threshold status, and reversible-delivery evidence across the current rollback-readiness packet.
- `Documentation/zigux/artifact-diff.md`, `python3 scripts/zigux/artifact_diff.py --self-test`, `check-artifact-diff-contract.py`, and `validate-phase4.py` keep the host-side artifact-diff preflight explicit before the bounded rollback gates claim the packet is aligned.
- `make -C zigux phase4-validate` and the bootstrap workflow steps `Validate Phase 4 diff gates` and `Run Phase 4 diff tests` now keep the shared validator-first packet explicit instead of relying on generic replay wording.
- `zigux/tests/phase4_build.zig` now keeps `phase4-test-fsmount-survey`, `phase4-test-fsmount-survey-tests`, `phase4-runtime-atomic64-diff-survey-tests`, `phase4_perf_baseline_manifest.json`, and `phase4-perf-baseline-survey-tests` visible at the docs index level, including the dedicated `make -C zigux phase4-perf-baseline-survey` replay path.
- the same Phase 4 notes also keep `perf_thresholds_unapproved_until_bounded_phase4_benchmarks_land` explicit, so threshold claims stay parked until one bounded benchmark command and one acceptable limit land for each shipped rollback gate.
- the same docs index now points reviewers back to the Phase 4 reversible-delivery evidence instead of letting the rollback-ready packet drift out of the top-level documentation view.
