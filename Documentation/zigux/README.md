...
Phase 4 notes
- `Documentation/zigux/phase4-validation-matrix.md` and `scripts/zigux/validate-phase4.py` keep the current Phase 4 notes explicit across the shared `make -C zigux phase4-validate` entrypoint, the host-side `python3 scripts/zigux/artifact_diff.py --self-test` and `check-artifact-diff-contract.py` preflights, the bootstrap workflow steps `Validate Phase 4 diff gates` and `Run Phase 4 diff tests`, the shared `phase4-test-fsmount-survey` and `phase4-runtime-atomic64-diff-survey-tests` replay anchors, the manifest-backed `phase4_perf_baseline_manifest.json` and `phase4-perf-baseline-survey-tests` packet, the dedicated `make -C zigux phase4-perf-baseline-survey` local replay, the published `perf_thresholds_unapproved_until_bounded_phase4_benchmarks_land` posture, and the reversible-delivery evidence that ties the shipped rollback gates back to their current C anchors.
