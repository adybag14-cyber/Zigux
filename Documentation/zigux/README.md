...
now keep the current docs-root Phase 14 reminder packet explicit from the documentation root so the shared smoke lane stays reviewable as a study-only and freeze-aware boundary packet instead of disappearing behind the blocked validator-local follow-through.
  * keep the shared Phase 14 smoke packet aligned with `Documentation/zigux/phase14-end-to-end-smoke-survey.md`, `Documentation/zigux/phase14-release-boundary-survey.md`, `Documentation/zigux/phase14-core-boundary-traceability.md`, `Documentation/zigux/review-checklist.md`, `Documentation/zigux/freeze-map.md`, `scripts/zigux/README.md`, `scripts/zigux/validate-phase14.py`, `scripts/zigux/check-phase14-docs-root-smoke-summary.py`, `scripts/zigux/check-phase14-tests-readme-smoke-summary.py`, `scripts/zigux/check-phase14-rollback-threshold-sequencing.py`, `scripts/zigux/check-phase14-release-boundary-exact-counts.py`, `zigux/tests/README.md`, `zigux/tests/phase14_end_to_end_smoke_manifest.json`, `zigux/tests/phase14_end_to_end_smoke_survey.zig`, `zigux/tests/phase14_workqueue_reviewability.zig`, `zigux/tests/phase14_build.zig`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml` so the docs root keeps the same shared packet visible as the survey and checklist.
  ...
  * keep the shared replay routes explicit and bounded: `make -C zigux phase14-validate`, `make -C zigux phase14-smoke`, `make -C zigux phase14-test`, `make -C zigux phase14`, and `zig build phase14-smoke --build-file zigux/tests/phase14_build.zig --summary all` are the current shared smoke handles, while the attached-toolchain fallback examples belong to the shared survey rather than to a new anchor-local route family.
  * keep the shared packet parked on blocker-accounting discipline rather than anchor-local expansion: the workqueue reviewability shard, the smoke manifest and survey, the rollback-threshold and release-boundary guards, and the workflow-backed smoke replay are current evidence, while anchor-local reopen or new direct bridge claims stay blocked until the same packet and the freeze map move together.
