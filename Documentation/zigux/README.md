...
- `python3 scripts/zigux/check-phase6-base64-c-parity.py` now replays the representative external C-vs-Zig base64 spot check, compiling `zigux/tests/fixtures/phase6_base64_c_harness.c` and comparing it against `zigux/tests/phase6_base64_c_parity.zig` so the current helper packet carries one portable cross-language parity replay beyond the shared Zig fixture module.
...
- the current bounded base64 decision is no longer whether the helper needs any external parity replay at all; it is whether the representative external C-vs-Zig spot check should stay as-is, widen into a generated fixture flow, or eventually give way to a better shared parity substrate.
