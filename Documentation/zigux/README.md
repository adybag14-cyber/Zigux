...
- `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, `scripts/zigux/check-build-only-phase12-surface.py`, `.github/workflows/zigux-bootstrap.yml`, `zigux/tests/README.md`, `zigux/tests/phase12_build.zig`, `zigux/Makefile`, the bounded Phase 12 nvme, virtio_net, virtio_scsi, and libbpf test modules wired through that build, the committed Phase 12 manifests under `zigux/tests/`, and `tools/lib/bpf/zigux_segments/manifest.json` now keep the current nvme pci, virtio_net, virtio_scsi, and libbpf survey-backed complex-driver bundle reviewable through the shipped build-backed release packet instead of implying removed validator, raw-coverage, or PMO checker surfaces.
- the current shared Phase 12 review surface on `master` is the docs-root, scripts-root, tests-root, Makefile, workflow, release-sequencing, and shared-replay-contract packet above together with the bounded Phase 12 nvme, virtio_net, virtio_scsi, and libbpf test modules wired through `zigux/tests/phase12_build.zig`, the committed Phase 12 manifests under `zigux/tests/`, and `tools/lib/bpf/zigux_segments/manifest.json`.
- `scripts/zigux/check-build-only-phase12-surface.py --self-test`, `scripts/zigux/check-build-only-phase12-surface.py`, `zig build test --build-file zigux/tests/phase12_build.zig --summary all`, and `make -C zigux phase12` now keep that shared Phase 12 build-backed review surface explicit on `master`, while the public fallback split remains limited to two commit-pinned artifacts (`Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md`, `Documentation/zigux/phase12-virtio-scsi-raw-github-fallback-catalog.md`) and two shared-tree-only anchors (`virtio_net`, `libbpf`), so future follow-through should not describe removed `validate-phase12.py`, `check-phase12-*.py`, `phase12-validate`, `phase12_raw_github_coverage_manifest.json`, `phase12_raw_github_coverage_survey.zig`, or `phase12_libbpf_only_build.zig` as shipped surfaces.
Phase 13 notes
