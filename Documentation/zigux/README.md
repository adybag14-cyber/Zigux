...
- `scripts/zigux/validate-phase3-low-level-wrapper-survey.py` and `make -C zigux phase3-validate` now keep that dedicated low-level wrapper survey packet explicit alongside the broader roadmap-gap, export/UAPI, and policy/unsafe Phase 3 notes, including the packet-local blob markers that make direct connector-era readback reviewable without a trustworthy branch-tip SHA.
- `Documentation/zigux/phase3-policy-unsafe-boundary-survey.md` now records the dedicated policy-and-unsafe boundary packet for `zigux/helpers/layout_assert.zig`, `zigux/helpers/panic_policy.zig`, `zigux/helpers/allocator_policy.zig`, `zigux/helpers/interop_policy.zig`, `zigux/unsafe/narrow.zig`, and the directly coupled scoped MMIO path in `zigux/helpers/mmio.zig`, so the typed interop-policy boundary and the still-deferred second boundary-facing consumer are visible without reopening runtime policy scope.
- `scripts/zigux/validate-phase3-policy-unsafe-survey.py`, `make -C zigux phase3-validate`, and the bootstrap workflow now keep that dedicated policy-and-unsafe survey packet explicit alongside the broader ABI slice note, including the packet-local blob markers, the shared docs-index hook, and the shared scripts-index hook that make the current narrow boundary reviewable even on shallow history.
...
- the same trace-events survey note now doubles as the sample-backed contributor guide for the landed non-runtime `trace-events` slice by naming the descriptor, shared sample-root catalog, shared review checklist, manifest, and shared `phase5_build.zig` entrypoint prompts that reviewers should keep in sync while staying distinct from the separate Phase 9 runtime pilot tranche.
...
- `python3 scripts/zigux/validate-phase7.py --self-test`, `python3 scripts/zigux/validate-phase7.py`, `make -C zigux phase7-validate`, `make -C zigux phase7-test`, `make -C zigux phase7`, `zig build test --build-file zigux/tests/phase7_build.zig --summary all`, `python3 scripts/zigux/check-phase7-cmdline-parity.py`, and `python3 scripts/zigux/check-phase7-rbtree-parity.py` are the published shared replay and fail-closed review hooks for that packet.
...
- `python3 scripts/zigux/validate-phase8.py`, `make -C zigux phase8-validate`, `make -C zigux phase8`, `zigux/tests/phase8_exec_cmd_only_build.zig`, `zigux/tests/phase8_help_only_build.zig`, `zigux/tests/phase8_kallsyms_only_build.zig`, and `zigux/tests/phase8_libbpf_segments_only_build.zig` now keep the current Phase 8 flow reviewable across the focused subcmd, symbol, and segmented libbpf shards plus the shared tooling bundle.
- the same shared Phase 8 packet now also keeps `zigux/tests/phase8_bridge_boundary_survey.zig`, `zigux/tests/phase8_file_path_handle_bridge.zig`, and `zigux/tests/phase8_bpf_type_names.zig` explicit as bounded bridge and libbpf-helper review surfaces inside that shared tooling bundle, so the docs root matches the live stable-output and bridge-boundary evidence already shipped on `master`.
...
- `Documentation/zigux/phase15-readiness-gate-survey.md` records the current roadmap-versus-ledger readiness check for that same governance bundle and keeps the shared `zig build test --build-file zigux/tests/phase15_build.zig` plus `make -C zigux phase15` replay path explicit.
- `Documentation/zigux/phase15-handoff-next-steps-survey.md` is the parked-next-step synthesis for the current Phase 15 maintenance mode handoff, including the focused handoff guard, the named reopen triggers, the remaining broader replay drift on current `master`, and the unchanged deep-core blocker posture.
