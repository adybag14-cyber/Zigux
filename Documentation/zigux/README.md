...
- `scripts/zigux/validate-phase3-policy-unsafe-survey.py`, `make -C zigux phase3-validate`, and the bootstrap workflow now keep that dedicated policy-and-unsafe survey packet explicit alongside the broader ABI slice note, including the packet-local blob markers plus the shared docs-index, scripts-index, and review-checklist hooks that make the current narrow boundary reviewable even on shallow history.
