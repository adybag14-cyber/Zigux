...
- `zigux/tests/phase12_build.zig`, `scripts/zigux/validate-phase12.py`, `make -C zigux phase12-validate`, and `make -C zigux phase12` now keep the current nvme pci, virtio_net, virtio_scsi, and libbpf survey-backed complex-driver bundle reviewable through one validator-first Phase 12 lane instead of widening into ad hoc per-slice checks.
...
- `tools/lib/bpf/zigux_segments/manifest.json` and `scripts/zigux/check-phase12-libbpf-packet.py` now keep the heavier libbpf packet-alignment and legacy-segment cross-check visible beside that same shared survey route instead of leaving the blocked object-model split implied.
