...
- the current shared rollback-lab replay and committed fast-inventory snapshot now agree on `Build Summary: 17/17 steps succeeded; 51/51 tests passed`, so this docs index keeps the active storage-driver and rollback-lab evidence packet aligned with the shared Phase 12 fixture instead of carrying the older resolved mismatch forward.
