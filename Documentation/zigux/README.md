...
- `Documentation/zigux/phase5-kretprobe-sample-survey.md` now records the landed `samples/zigux/kretprobe_example.zig` reference sample, its exact skip, return-value, duration, and `nmissed` replay checks, and the remaining non-goals around probe registration, `pt_regs`, and module wiring.
...
- the Phase 5 notes now also keep the remaining sample gap explicit: the non-runtime reference reading for the `trace-events` anchor still remains open even though a later Phase 9 runtime starter already exists for that family.
