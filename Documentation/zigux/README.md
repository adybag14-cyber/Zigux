...
now keep the current docs-root Phase 12 reminder packet explicit from the documentation root so the current starter-present `virtio_net` plus smoke-first `virtio_scsi` release packet reviewable from the docs root through the shipped build-only contract while the broader libbpf and NVMe packet boundaries stay separate from shared replay claims.
  * keep `Documentation/zigux/phase12-raw-github-coverage-survey.md` and `Documentation/zigux/phase12-libbpf-verify-shard-note.md` explicit beside `scripts/zigux/validate-phase12.py` and `make -C zigux phase12-validate`, and keep the build-only checker, release-readiness checker, and narrower PMO notes framed as support material inside that shipped `phase12-validate` route rather than standalone proof of broader driver delivery.
  * keep the current starter-present `virtio_net` plus smoke-first `virtio_scsi` release packet reviewable from the docs root through the shipped build-only contract, and keep `Documentation/zigux/phase12-virtio-net-survey.md`, `zigux/tests/phase12_virtio_net_manifest.json`, `zigux/tests/phase12_virtio_net_survey.zig`, `Documentation/zigux/phase12-virtio-scsi-slice.md`, and `Documentation/zigux/phase12-virtio-scsi-survey.md` explicit beside the direct replay packet instead of flattening that reminder packet into one undifferentiated build claim.
