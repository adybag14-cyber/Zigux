...
- `python3 scripts/zigux/check-artifact-diff-contract.py` keeps one stable text pass case, both missing-file failure shapes, the direct JSON mismatch replay, malformed expected and actual JSON failure shapes, and SHA-256 pass and drift cases explicit, so the docs-root Phase 4 packet now names the outward `ARTIFACT_DIFF=...`, `MODE=...`, `EXPECTED_EXISTS=...`, `ACTUAL_EXISTS=...`, `EXPECTED_JSON_ERROR=...`, `ACTUAL_JSON_ERROR=...`, `SHA256=...`, `EXPECTED_SHA256=...`, and `ACTUAL_SHA256=...` markers instead of implying them only through the broader preflight summary.
