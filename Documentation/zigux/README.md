...
- `Documentation/zigux/phase9-runtime-loader-substrate-plan.md` now keeps the adjacent `Documentation/zigux/freeze-map.md` boundary explicit for the shared runtime-loader handoff note, so `kernel/workqueue.c` stays `Study / Boundary Only` and the current shared request surface does not get mistaken for a scheduler-facing status change or live runtime-loader claim.
...
- `python3 scripts/zigux/validate-phase9.py` and `make -C zigux phase9-validate` now fail fast if the shared Phase 9 checklist, loader-gap survey, loader-substrate freeze-map boundary note, trace-events freeze-map boundary packet, README notes, workflow wiring, and `zigux/tests/phase9_build.zig` entrypoint drift apart.
