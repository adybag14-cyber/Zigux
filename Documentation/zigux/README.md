...
- `Documentation/zigux/phase9-runtime-loader-gap-survey.md`, `Documentation/zigux/phase9-runtime-loader-substrate-plan.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, `Documentation/zigux/review-checklist.md`, `zigux/tests/runtime_loader_gap_manifest.json`, `zigux/tests/runtime_loader_gap_survey.zig`, `zigux/kernel/runtime_loader.zig`, `zigux/kernel/runtime_loader_contract.zig`, `zigux/tests/runtime_loader_allocator_init_flow.zig`, `zigux/tests/phase9_build.zig`, `zigux/Makefile`, and the four `samples/zigux/runtime_*_loader.zig` scaffolds now keep the current runtime atomic64, bitmap, trace-events, and kretprobe pilot bundle reviewable through one shared runtime-loader lane together with the shared build, the Linux-style `make -C zigux phase9` replay route, and the loader-gap evidence instead of widening into ad hoc per-slice checks or understating the shipped runtime-loader facade, contract, and allocator/init-flow replay on `master`.
- the current bounded Phase 9 decision is no longer whether the runtime-pilot lane still needs starter samples, loader-handoff scaffolds, or a shared request surface; those surfaces plus the shared `phase9_build.zig` replay, `make -C zigux phase9` route, and loader-gap evidence are now in place on `master`, so the next follow-up should stay inside the smallest shared runtime-loader substrate or validation step that keeps those four handoffs, the shared `zigux/kernel/runtime_loader.zig` facade, and the `zigux/kernel/runtime_loader_contract.zig` plus allocator/init-flow replay reviewable without widening into a larger runtime-module implementation.
