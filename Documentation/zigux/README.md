...
- `python3 scripts/zigux/validate-phase10.py` now keeps the narrower virtio_input packet aligned, so the Phase 10 docs, helper, tests, and survey manifest all keep the same landed registration-preflight summary, ready-next queue-callback preflight helper, and registration-lifecycle blocker posture.
