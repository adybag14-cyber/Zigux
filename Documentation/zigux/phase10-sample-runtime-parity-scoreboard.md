...
- `scripts\zigux/check_phase10_sample_runtime_scoreboard.zig`
...
`scripts\zigux/check_phase10_sample_runtime_scoreboard.zig` keeps this notes-only scoreboard tied to the validator-backed evidence it summarizes. The guard fails closed if this note drops the shared Phase 10 validator route, the ring manifest destination checker, the closure evidence surface, or the explicit blocked risky-transport boundary.

The guard also checks `scripts\zigux/validate_phase10.zig` for the `phase10-ring-manifest-destinations` live check so the scoreboard cannot keep advertising validator-backed evidence after the shared validator route has moved.
