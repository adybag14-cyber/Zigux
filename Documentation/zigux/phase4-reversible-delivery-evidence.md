...
Use this note only as a truthful current-head handoff for the directly readable reminder surfaces. The next honest same-family follow-through is to update `zigux/tests/README.md` so its Phase 4 reminder keeps the directly readable dedicated local-only perf packet explicit while continuing to distinguish the broader authenticated-readback gaps from the public-fallback-only companions; restore the roadmap-backed `zigux/tests/atomic64_diff.zig` pair only after those exact current-`master` paths reread cleanly; or republish one missing broader companion, so this handoff can eventually replace the current `PHASE4_REVERSIBLE_DELIVERY_LAST_KNOWN_*` provenance with fresh current-head evidence.