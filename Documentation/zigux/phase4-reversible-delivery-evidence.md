...
The direct checker pair now publishes `PHASE4_REPO_REALITY_WARNING_SELF_TEST_CASES=21` and `PHASE4_REVERSIBLE_DELIVERY_PIN_SELF_TEST_CASE_COUNT=19` here, so future exact-readback passes can fail closed on stale checker-coverage claims as well as stale packet-member claims.
