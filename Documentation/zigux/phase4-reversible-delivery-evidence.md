...
## Status
  ...
  * `PHASE4_REVERSIBLE_DELIVERY_TESTS_README_BLOB_SHA=7a0d7be6e47a79cfad0456fc06dfc2e91d99f464`
...
The tests-root guide should mirror this same current-head posture, including the difference between authenticated direct-readback gaps and public current-`master` fallback visibility. The Phase 4 repo-reality warning in `zigux/tests/README.md` should stay open until that broader validator, lab-matrix, and local-only perf packet is directly readable again, and the roadmap-backed `atomic64_diff` pair should stay framed as missing current-head destinations until a same-family lane republishes them, so the next same-family follow-through is the smallest truthful reminder-surface or checker repair inside this live warning packet rather than a fresh exact-pin pass against missing companions.
