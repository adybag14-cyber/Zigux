...
- `scripts\zigux/check_phase14_rcu_compile_route.zig` through the current contents path
...
- `scripts\zigux/check_phase14_shared_smoke_route.zig` now returns through the current contents path and keeps the returned `phase14-validate` Makefile route plus workflow gate explicit instead of leaving that route proof only in neighboring reminder prose
...
- `scripts\zigux/check_phase14_rollback_threshold_sequencing.zig` now returns through the current contents path and keeps the shared rollback threshold, fallback path, automatic return-to-blocked triggers, and narrowed study-only route split explicit instead of leaving that rollback contract only in neighboring reminder prose
- `scripts\zigux/check_phase14_skbuff_stay_in_c_guardrail.zig` now returns through the current contents path and keeps the dedicated `net/core/skbuff.c` freeze-in-C stay-in-C guard explicit beside the shared packet, so the current productization note should count that returned guard surface instead of leaving the skbuff ownership boundary implicit inside broader reminder prose
- `scripts\zigux/check_phase14_skbuff_compile_route.zig` now returns through the current contents path and keeps the dedicated `net/core/skbuff.c` compile-shard route explicit beside the shared packet, the shared smoke manifest, and the current validator route, so the current productization note should count that returned guard surface instead of leaving skbuff compile evidence implicit inside broader reminder prose
- `scripts\zigux/check_phase14_ring_buffer_compile_route.zig` now returns through the current contents path and keeps the dedicated `kernel/trace/ring_buffer.c` compile-shard row explicit beside the shared packet, the shared smoke manifest, and the current validator route, so the current productization note should count that returned guard surface instead of leaving ring-buffer compile evidence implicit inside broader reminder prose
- `scripts\zigux/check_phase14_rcu_compile_route.zig` now returns through the current contents path and keeps the dedicated `kernel/rcu/tree.c` compile-shard row explicit beside the shared packet, the shared smoke manifest, and the current validator route, so the current productization note should count that returned guard surface instead of leaving the RCU compile-route posture implicit inside broader reminder prose
- `scripts\zigux/check_phase14_rcu_rollback_guardrail.zig` now returns through the current contents path and keeps the dedicated `kernel/rcu/tree.c` freeze-in-C rollback guard explicit beside the shared packet, so the current productization note should count that returned guard surface instead of leaving the RCU rollback posture implicit inside the broader validator
- `scripts\zigux/validate_phase14.zig` now returns through the current contents path and carries a real shared-smoke validator surface rather than the older placeholder-only body
- `scripts\zigux/check_phase14_release_boundary_exact_counts.zig` now returns through the current contents path and keeps the release-facing exact-count posture aligned with the recovered shared reminder packet
