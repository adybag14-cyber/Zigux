...
Current `master` does expose the shared smoke note, the cross-anchor traceability note, the release-boundary note, the freeze map, the shared gap notes, the returned shared smoke manifest, the returned rollback-threshold, dedicated skbuff stay-in-C, dedicated skbuff compile-route, dedicated ring-buffer compile-route, and dedicated RCU rollback guards, and the returned ring-buffer survey companion through the exact or mixed readback modes available in this lane.
