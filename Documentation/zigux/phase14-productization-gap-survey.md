...
Current `master` does expose the shared smoke note, the cross-anchor traceability note, the release-boundary note, the freeze map, the shared gap notes, the returned shared smoke manifest, and the returned ring-buffer survey companion through the exact or mixed readback modes available in this lane.
