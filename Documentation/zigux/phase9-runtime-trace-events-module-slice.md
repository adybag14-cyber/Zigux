...
- scope: lifecycle starter, bounded event-emission and registration behavior, a machine-checkable diagnostics summary with explicit main-thread and function-thread event totals, direct sample-local selftest and exit proof, a tiny payload-oriented diff gate, dedicated Phase 9 test wiring, and lane-local survey-manifest closure only
- product boundary:
...
- dedicated Phase 9 module, sample, diff, and manifest coverage wired into the shared `zigux/tests/phase9_build.zig` gate
