...
Fresh 2026-05-18 repo-first rereads confirmed that the surviving direct trace-events sample family has widened beyond the older two-file packet, so this note now keeps the exit-rollback and registration-reentry companions explicit alongside the direct sample and the fail-closed companion without treating the older manifest-backed, survey-backed, or loader-backed companions as shipped current-`master` evidence.
...
The same direct sample also keeps rejected re-selftest cues explicit inside the shipped file, so the surviving packet still carries one real runtime selftest plus bounded lifecycle-rollback proof on current `master`.
...
The exit-rollback companion keeps failed-exit rollback plus later post-exit invalid-lifecycle rejections explicit after reusable selftest replay, so the surviving packet now also carries one dedicated exit-boundary proof on current `master` without widening into shared loader parity.
...
These boundaries are reviewable pilot-module cues for the roadmap's selftest-hook and lifecycle goals only; they are not live loader-backed module-registration parity.
  ...
  * `samples/zigux/runtime_trace_events_exit_rollback_guard.zig` now proves that a failed `exit()` with outstanding registration preserves the selftest-complete summary, that reusable function-thread replay can continue until the outstanding registration is released, and that the exited summary then stays stable across later invalid-lifecycle rejections.
...
2. Keep the surviving direct sample plus its rejected re-selftest cues explicit as real current-packet reviewable selftest-hook and sample-local lifecycle proof.
3. Keep the surviving fail-closed companion explicit as the same packet's function-thread boundary proof.
4. Keep the surviving exit-rollback companion explicit as the same packet's failed-exit rollback and post-exit invalid-lifecycle proof.
5. Keep the surviving registration-reentry companion explicit as the same packet's balanced function-thread reuse proof across the initialized and selftest_complete stages.
6. Keep the exact init and registration evidence above tied to shipped sample-local cues instead of inferring broader loader behavior, live `module_init()` or `module_exit()` parity, or depmod-visible registration proof that current `master` does not expose.
7. Keep the real blocker explicit: the broader shared runtime-loader, build, kernel, multi-file runtime replay packet, and older paired survey note are absent on current `master`, so this note must not borrow evidence from older missing paths.
8. Do not invent `validate-phase9.py`, a trace-events-only validator, or a returned loader family.
9. Keep earlier-phase references in their own lanes: `scripts/zigux/kconfig/conf_bridge.zig` and `scripts/zigux/kconfig/confdata_bridge.zig` remain Phase 2 references, while `rust/exports.c` and `zigux/kernel/export_shim.zig` remain Phase 3 export-boundary references.

## Review posture

- the current direct runtime proof is the narrow four-file sample family made of `samples/zigux/runtime_trace_events.zig`, `samples/zigux/runtime_trace_events_unregistered_gate.zig`, `samples/zigux/runtime_trace_events_exit_rollback_guard.zig`, and `samples/zigux/runtime_trace_events_registration_reentry_gate.zig`
...
- the direct sample also keeps rejected re-selftest cues explicit inside the shipped file
...
- the exit-rollback companion keeps failed-exit rollback explicit after reusable selftest replay and preserves the exited summary across later invalid-lifecycle rejections
...
Keep this module-slice note aligned with the surviving four-file direct sample packet, including the exact init and registration evidence plus the rejected re-selftest, fail-closed companion, exit-rollback companion, and balanced registration-reentry cues already shipped in `samples/zigux/runtime_trace_events.zig`, `samples/zigux/runtime_trace_events_unregistered_gate.zig`, `samples/zigux/runtime_trace_events_exit_rollback_guard.zig`, and `samples/zigux/runtime_trace_events_registration_reentry_gate.zig`, while continuing to say plainly that those cues remain sample-local review evidence rather than returned loader parity. If the broader shared runtime-loader family or the older paired survey note returns later, reread the exact file family before widening this note back out.
