...
- the re-init rollback companion keeps later `init()` retries fail-closed across initialized, selftest-complete, and exited summaries
...
The paired family-local survey packet through `Documentation/zigux/phase9-runtime-trace-events-survey.md`, `zigux/tests/runtime_trace_events_manifest.json`, `zigux/tests/runtime_trace_events_survey.zig`, and `zigux/tests/runtime_trace_events_module.zig` now keeps that pilot-module story directly reviewable under `zigux/tests/runtime_*` again without pretending the wider loader-backed family returned.
That paired survey packet stays adjacent to the shared repo-level rerun guard in `.github/workflows/zigux-bootstrap.yml`, which reruns `zig test samples/zigux/runtime_trace_events.zig`, `zig test samples/zigux/runtime_trace_events_unregistered_gate.zig`, `zig test samples/zigux/runtime_trace_events_exit_rollback_guard.zig`, `zig test samples/zigux/runtime_trace_events_registration_reentry_gate.zig`, `zig test samples/zigux/runtime_trace_events_reinit_rollback_guard.zig`, and `zig test samples/zigux/runtime_trace_events_reinit_reexit_guard.zig`, and `zig test zigux/tests/runtime_trace_events_survey.zig` without turning the workflow into dedicated family-local loader parity proof.
...
- a family-local loader parity witness beyond the dedicated `phase9-runtime-trace-events-tests` rerun route, the direct module-boundary witness, and the adjacent shared allocator/init-flow and command/environment boundary packet
...
Current `master` still keeps the separate Phase 9 runtime bitmap reminder packet visible through `Documentation/zigux/phase9-runtime-bitmap-survey.md`, `Documentation/zigux/phase9-runtime-bitmap-module-slice.md`, `zigux/tests/runtime_bitmap_manifest.json`, `zigux/tests/runtime_bitmap_survey.zig`, `zigux/tests/runtime_bitmap_module.zig`, `zigux/tests/runtime_bitmap_diff.zig`, `samples/zigux/runtime_bitmap_loader.zig`, and the bounded `zigux/tests/phase9_build.zig` bundle, but that neighboring bitmap packet still must not be promoted into proof that every broader runtime boundary returned. So the family-local boundary above is about avoiding trace-events overclaim rather than claiming the bitmap-side packet closes blocked publication, install-root, or wider loader work.
The broader runtime substrate is still blocked as well: runtime task ownership, polling and event-loop substrate, and polling-backed wake or dispatch behavior remain outside this family-local proof even while the shared loader-facing reminder packet stays reviewable.
...
2. Keep the surviving six-file sample family explicit as current sample-local pilot-module proof.
3. Keep `Documentation/zigux/phase9-runtime-trace-events-survey.md`, `zigux/tests/runtime_trace_events_manifest.json`, `zigux/tests/runtime_trace_events_survey.zig`, and `zigux/tests/runtime_trace_events_module.zig` paired with this slice as family-local review witnesses.
...
7. Do not treat blocked runtime task ownership, polling and event-loop substrate, or polling-backed wake or dispatch behavior as solved just because the trace-events packet and loader-facing reminder surfaces remain readable.
8. Do not invent `validate-phase9.py`, a trace-events-only validator, or a loader-backed runtime-substrate claim that current `master` does not expose.

## Next bounded step

Keep the survey note, direct module witness, and this module-slice note aligned with the surviving sample family while naming the adjacent shared loader shard truthfully. If the broader shared runtime-loader family widens again, reread the exact file family before widening this note back out, and keep the separate blocked runtime task or polling and event-loop substrate boundary explicit until a real substrate step lands.