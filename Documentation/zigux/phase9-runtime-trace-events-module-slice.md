...
That means the honest current review surface is a landed family-local packet plus the adjacent shared loader-facing reminder surfaces. The family-local trace-events sample, loader scaffold, dedicated loader-substrate-drift replay, direct module and diff gates, dedicated survey gate, and paired report surfaces are all visible on current `master`. The direct module gate also keeps the bounded pilot-module contract explicit through `RuntimeTraceEventsSample.descriptor()` plus `runSelftest()`, so the roadmap-facing selftest-hook and lifecycle-parity proof stays family-local instead of being borrowed from the shared loader lane. The loader scaffold now also keeps initialized prepared requests explicit across later direct registration replay, keeps prepared snapshots stable across later selftest or exit activity, rejects shared-request drift around allocator/init-flow and selftest-hook evidence before local handoff, keeps registration-snapshot and outstanding-registration-drain proofs explicit, rejects non-idle registration state at the metadata-only handoff boundary, and keeps shared release failures from desynchronizing loader state; the dedicated loader-substrate-drift replay keeps the prepared shared runtime-substrate rejection explicit for both selftest-complete and initialized-stage handoffs, while the family-local module gate owns the selftest-ready failed-exit rollback path that preserves lifecycle state until registration drain finishes, while still leaving the broader runtime-substrate handoff as a separate blocked step. At the same time, the live runtime substrate is still missing. The family-local module gate also keeps rejected re-selftest rollback explicit, so invalid repeat selftest attempts leave both the selftest-complete and exited summaries stable while the broader runtime-substrate handoff stays blocked. The same loader scaffold and shared-request bridge keep `zigux_runtime_trace_events_init`, `zigux_runtime_trace_events_exit`, `tracepoint_probe_register`, and `tracepoint_probe_unregister` readable as review-only lifecycle metadata, so the family-local packet still does not claim live `module_init()`, `module_exit()`, `tracepoint_probe_register()`, or `tracepoint_probe_unregister()` execution or kernel initcall ordering parity. That blocked handoff still stops before `.modinfo`, `MODULE_ALIAS()`, `modules.alias`, `modules.order`, `modules.builtin`, module install-root publication, and any `depmod` script, manifest, or alias publication state; those alias and depmod surfaces remain review-only metadata boundaries rather than shipped trace-events-family evidence.
