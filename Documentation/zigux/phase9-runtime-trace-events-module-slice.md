...
4. Keep the exact init and registration evidence above tied to shipped sample-local cues instead of inferring broader loader behavior, live `module_init()` or `module_exit()` parity, or depmod-visible registration proof that current `master` does not expose.
