...
- a bounded `runtime_trace_events_loader` scaffold that names the planned entry and exit hooks, the `tracepoint_probe_register` and `tracepoint_probe_unregister` metadata-only labels, the current event-family summary, the prepared and initialized-stage snapshots that stay explicit even if the sample mutates again before runtime handoff, the selftest-complete prepared snapshot that stays explicit even if the live sample exits before shared runtime handoff, the shared-request bridge through the existing runtime-loader contract, the prepared shared-request plus direct initialized and selftest-complete shared selftest-hook drift checks before any live handoff claim, the no-substrate release path, and the idle-registration-snapshot requirement while the shared runtime-loader surface remains unavailable
