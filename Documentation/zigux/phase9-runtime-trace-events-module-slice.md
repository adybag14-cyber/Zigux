...
That means the honest current review surface is a landed family-local packet plus the adjacent shared loader-facing reminder surfaces. The family-local trace-events sample, loader scaffold, dedicated loader-substrate-drift replay, direct module and diff gates, dedicated survey gate, and paired report surfaces are all visible on current `master`.
The direct module gate also keeps the bounded pilot-module contract explicit through `RuntimeTraceEventsSample.descriptor()` plus `runSelftest()`, so the roadmap-facing selftest-hook and lifecycle-parity proof stays family-local instead of being borrowed from the shared loader lane.
The loader scaffold now also keeps prepared snapshots stable across later selftest or exit activity, rejects shared-request drift around allocator/init-flow and selftest-hook evidence before local handoff, keeps registration-snapshot and outstanding-registration-drain proofs explicit, rejects non-idle registration state at the metadata-only handoff boundary, and keeps shared release failures from desynchronizing loader state; the dedicated loader-substrate-drift replay keeps the prepared shared runtime-substrate rejection explicit for both selftest-complete and initialized-stage handoffs, while the family-local module gate owns the selftest-ready failed-exit rollback path that preserves lifecycle state until registration drain finishes, while still leaving the broader runtime-substrate handoff as a separate blocked step.
At the same time, the live runtime substrate is still missing. The family-local module gate also keeps rejected re-selftest rollback explicit, so invalid repeat selftest attempts leave both the selftest-complete and exited summaries stable while the broader runtime-substrate handoff stays blocked.
The same loader scaffold and shared-request bridge keep `zigux_runtime_trace_events_init`, `zigux_runtime_trace_events_exit`, `tracepoint_probe_register`, and `tracepoint_probe_unregister` readable as review-only lifecycle metadata, so the family-local packet still does not claim live `module_init()`, `module_exit()`, `tracepoint_probe_register()`, or `tracepoint_probe_unregister()` execution or kernel initcall ordering parity.
That blocked handoff still stops before `.modinfo`, `MODULE_ALIAS()`, `modules.alias`, `modules.order`, `modules.builtin`, module install-root publication, and any `depmod` script, manifest, or alias publication state; those alias and depmod surfaces remain review-only metadata boundaries rather than shipped trace-events-family evidence.
...
The same blocked boundary also still includes `.modinfo`, `MODULE_ALIAS()`, `modules.alias`, `modules.order`, `modules.builtin`, module install-root publication, and `depmod` script, manifest, or alias publication state, which remain outside both the family-local trace-events packet and the adjacent shared loader handoff.
...
The honest current review posture is:
  ...
  * the family-local module gate already proves the bounded pilot-module contract, the selftest-ready failed exit that preserves the current replay summary until registration drain completes, and the rejected re-selftest rollback path that leaves the selftest-complete and exited summaries unchanged
...
The next honest follow-through in the same `runtime-pilot` lane is to keep the packet-local survey note, module-slice note, and manifest aligned with the visible family-local trace-events packet, keep the dedicated `zigux/tests/runtime_trace_events_loader_substrate_drift.zig` replay plus the focused `phase9-runtime-trace-events-tests` route explicit, keep the bounded pilot-module contract explicit, and then leave broader follow-up to the separate shared runtime-substrate lanes until a real substrate step lands.
