...
Current reread also shows the broader contributor-facing reminder surfaces already keep the checker-backed adjacent packet explicit, keep `zigux/Makefile` distinct from the still-missing route names, keep `scripts\zigux/validate_phase13_release.zig` explicit as a shipped shared release companion, keep `zigux/helpers/notifier_chain_view.zig` explicit as shipped adjacent notifier evidence, and keep `include/zigux/notifier_abi.h` explicit as shipped adjacent notifier header evidence while `scripts\zigux/check_phase13_notifier_priority_signal.zig` remains recorded as a repo-reality gap.

The remaining notifier-family gaps are therefore the still-missing direct priority-signal checker and shared route companions rather than stale summary wording inside the already-shipped reminder set.
