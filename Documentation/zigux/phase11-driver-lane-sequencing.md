...
* DesignWare lane `P11-L10` owns `Documentation/zigux/phase11-dw-wdt-platform-registration-plan.md`, `scripts/zigux/check-phase11-dw-wdt-packet.py`, `drivers/watchdog/dw_wdt.zig`, `drivers/watchdog/dw_wdt_verify.zig`, and `zigux/tests/phase11_dw_wdt_registration_scaffold.zig` as the surviving bounded DesignWare packet; keep the surviving scaffold packet, the compile-local teardown or restart proofs, and the missing timer-clock registration block explicit in shared summaries without treating the older raw-fallback `zigux/tests/phase11_dw_wdt.zig` replay as the active bounded packet or widening any of those surfaces into broader platform-registration closure claims
...
* HVC archival packet lane `P11-L16` owns HVC reminder-note and checker follow-through; keep the landed archival packet explicit through `Documentation/zigux/phase11-hvc-console-validation-matrix.md`, `Documentation/zigux/phase11-hvc-console-survey.md`, `Documentation/zigux/phase11-hvc-console-teardown-note.md`, `drivers/tty/hvc/hvc_console.zig`, `zigux/tests/phase11_hvc_console_manifest.json`, `zigux/tests/phase11_hvc_console.zig`, `zigux/tests/phase11_hvc_console_survey.zig`, `zigux/tests/phase11_hvc_console_modem_control_split.zig`, `zigux/tests/phase11_hvc_console_poll_retry_split.zig`, `zigux/tests/phase11_hvc_cleanup.zig`, `drivers/tty/hvc/hvc_console_verify.zig`, `drivers/tty/hvc/hvc_console_sysrq.zig`, and `make -C zigux phase11-hvc-survey` without widening that bounded archival packet into notifier, khvcd, or host-backed execution closure claims
...
8. Keep the DesignWare lane honest: on current `master` the surviving DesignWare lane evidence is `Documentation/zigux/phase11-dw-wdt-platform-registration-plan.md`, `scripts/zigux/check-phase11-dw-wdt-packet.py`, `drivers/watchdog/dw_wdt.zig`, `drivers/watchdog/dw_wdt_verify.zig`, and `zigux/tests/phase11_dw_wdt_registration_scaffold.zig`, pinned to `P11-L10`, while the next bounded step still remains platform-backed registration scaffolding rather than reviving removed manifest-backed reminder surfaces, widening the compile-local teardown or restart proofs into hardware-backed closure, counting the older raw-fallback `zigux/tests/phase11_dw_wdt.zig` replay as the current scaffold anchor before it is refreshed to the surviving driver API, or treating a missing timer clock as registration-ready.
