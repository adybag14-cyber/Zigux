...
8. Keep the HVC delivery-gate lane honest: on current `master` the landed HVC archival packet is the manifest-backed survey gate, cleanup handoff, modem-control split, poll-retry split, sysrq helper, verify replay, teardown note, validation matrix, and dedicated `phase11-hvc-survey` route rather than a missing or purely reminder-only packet.
