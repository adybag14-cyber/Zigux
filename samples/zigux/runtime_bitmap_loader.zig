...
test "runtime bitmap loader keeps initialized shared-request snapshots stable across later exit activity" {
    var module = runtime_bitmap_sample.RuntimeBitmapSample{};
    try module.initWithSetBits(&.{ 0, 5, 64, 70 });
...
test "runtime bitmap loader rejects prepared shared runtime-substrate drift before any live bitmap claim" {
    var module = runtime_bitmap_sample.RuntimeBitmapSample{};
    try module.initWithSetBits(&.{ 0, 5, 64, 70 });
    _ = try module.runSelftest();
...
test "runtime bitmap loader rejects prepared shared allocator drift before any local runtime handoff" {
    var module = runtime_bitmap_sample.RuntimeBitmapSample{};
    try module.initWithSetBits(&.{ 0, 5, 64, 70 });
    _ = try module.runSelftest();
    ...
    shared_request.plan.allocator_handoff = .caller_provided;
...
test "runtime bitmap loader rejects prepared shared init-flow drift before any local runtime handoff" {
    var module = runtime_bitmap_sample.RuntimeBitmapSample{};
    try module.initWithSetBits(&.{ 0, 5, 64, 70 });
    _ = try module.runSelftest();
...
test "runtime bitmap loader rejects shared selftest-hook drift before any live bitmap claim" {
    var module = runtime_bitmap_sample.RuntimeBitmapSample{};
    try module.initWithSetBits(&.{ 0, 5, 64, 70 });
...
test "runtime bitmap loader keeps selftest-complete shared-request snapshots stable across later exit activity" {
    ...
    try std.testing.expect(runtime_loader.keepsSelftestHookEvidenceConsistent(prepared_plan));
    try std.testing.expectEqual(runtime_bitmap_sample.ModuleStage.selftest_complete, selftested_snapshot.stage);
