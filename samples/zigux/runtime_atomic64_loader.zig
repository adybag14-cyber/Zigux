...
test "runtime atomic64 loader keeps initialized shared-request snapshots stable across later clean exit activity" {
    var module = runtime_atomic64_sample.RuntimeAtomic64Sample{};
    try module.init(9);
    ...
    try std.testing.expect(runtime_loader.keepsSelftestHookEvidenceConsistent(prepared_plan));
    try std.testing.expectEqual(runtime_atomic64_sample.ModuleStage.initialized, initialized_snapshot.stage);
    ...
    try std.testing.expectEqual(@as(i64, 9), initialized_summary.counter_snapshot);
    ...
    try std.testing.expectEqual(initialized_summary.counter_snapshot, exited_summary.counter_snapshot);
