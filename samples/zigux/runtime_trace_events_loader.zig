...
pub const RuntimeTraceEventsLoader = struct {
    ...
    allocator_handoff: runtime_loader.AllocatorHandoff = .caller_provided,
...
test "runtime trace-events loader keeps selftest-complete shared contract plans explicit" {
    ...
    try std.testing.expectEqualStrings("runtime_trace_events", shared_plan.module_name);
    ...
    try std.testing.expectEqualStrings(
        ...
        shared_plan.entry_symbol,
    );
    try std.testing.expectEqualStrings(
        "zigux_runtime_trace_events_exit",
        shared_plan.exit_symbol,
    );
...
test "runtime trace-events loader keeps selftest-complete shared requests stable across later direct activity" {
    var module = runtime_trace_events_sample.RuntimeTraceEventsSample{};
    try module.init();
    try module.runSelftest();
    ...
    const prepared_summary = module.summary();

    _ = try module.emitMainIteration(5);
    try module.registerFunctionThread();
    _ = try module.emitFunctionIteration(11);
    try module.unregisterFunctionThread();

    const live_summary = module.summary();
    try std.testing.expectEqual(prepared_summary.init_runs, live_summary.init_runs);
    try std.testing.expectEqual(prepared_summary.selftest_runs, live_summary.selftest_runs);
    try std.testing.expectEqual(prepared_summary.exit_runs, live_summary.exit_runs);

    const live_plan = try RuntimeTraceEventsLoader.planFor(&module, .caller_provided);
...
test "runtime trace-events loader keeps initialized-stage shared requests blocked by the current loader family contract" {
    ...
    const initialized_plan = try RuntimeTraceEventsLoader.planFor(&module, .caller_provided);
    try std.testing.expectEqual(
        runtime_loader.HandoffStage.initialized,
        initialized_plan.init_flow.handoff_stage,
    );
    try std.testing.expectEqual(@as(usize, 1), initialized_plan.init_flow.init_runs);
    try std.testing.expectEqual(@as(usize, 0), initialized_plan.init_flow.selftest_runs);
    try std.testing.expect(runtime_loader.keepsSelftestHookEvidenceConsistent(initialized_plan));
...
test "runtime trace-events loader rejects cold and exited sample stages before preparing a shared request" {
    var cold_module = runtime_trace_events_sample.RuntimeTraceEventsSample{};
    try std.testing.expectError(
        error.InvalidLifecycleTransition,
        RuntimeTraceEventsLoader.planFor(&cold_module, .caller_provided),
    );

    var exited_module = runtime_trace_events_sample.RuntimeTraceEventsSample{};
    try exited_module.init();
    try exited_module.runSelftest();
    try exited_module.exit();
    try std.testing.expectError(
        error.InvalidLifecycleTransition,
        RuntimeTraceEventsLoader.planFor(&exited_module, .caller_provided),
    );
}
