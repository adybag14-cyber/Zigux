...
test "phase9 trace-events sample keeps unregistered function-thread failures fail-closed" {
    ...
    const replayed_main_after_failed_exit = try module.emitMainIteration(5);
    try std.testing.expectEqual(@as(usize, 4), replayed_main_after_failed_exit);
    const replayed_fn_after_failed_exit = try module.emitFunctionIteration(7);
    try std.testing.expectEqual(@as(usize, 2), replayed_fn_after_failed_exit);

    const initialized_before_unregister = module.summary();
    try std.testing.expectEqual(ModuleStage.initialized, initialized_before_unregister.stage);
    try std.testing.expectEqual(@as(usize, 1), initialized_before_unregister.registration_depth);
    try std.testing.expectEqual(@as(usize, 1), initialized_before_unregister.main_iterations);
    try std.testing.expectEqual(@as(usize, 1), initialized_before_unregister.fn_iterations);
    try std.testing.expectEqual(@as(usize, 4), initialized_before_unregister.main_thread_events);
    try std.testing.expectEqual(@as(usize, 2), initialized_before_unregister.fn_thread_events);
    try std.testing.expectEqual(@as(usize, 6), initialized_before_unregister.total_events);
    try std.testing.expectEqual(@as(?usize, 4), initialized_before_unregister.last_main_emitted_events);
    try std.testing.expectEqual(@as(?usize, 2), initialized_before_unregister.last_fn_emitted_events);
    try std.testing.expectEqual(@as(?usize, 0), initialized_before_unregister.last_main_conditional_event_count);
    try std.testing.expectEqual(@as(usize, 1), initialized_before_unregister.init_runs);
    try std.testing.expectEqual(@as(usize, 0), initialized_before_unregister.selftest_runs);
    try std.testing.expectEqual(@as(usize, 0), initialized_before_unregister.exit_runs);
    try std.testing.expectEqual(@as(i32, 5), initialized_before_unregister.last_main_count);
    try std.testing.expectEqual(@as(i32, 7), initialized_before_unregister.last_fn_count);
    try std.testing.expect(initialized_before_unregister.saw_vararg_payload);
    try std.testing.expect(initialized_before_unregister.saw_rel_loc_payload);
    try std.testing.expect(!initialized_before_unregister.saw_conditional_path);
    try std.testing.expectEqualStrings("event-sample", initialized_before_unregister.main_thread_label orelse return error.ExpectedMainThreadLabel);
    try std.testing.expectEqualStrings("event-sample-fn", initialized_before_unregister.function_thread_label orelse return error.ExpectedFunctionThreadLabel);
    try std.testing.expectEqualStrings("foo_bar_reg", initialized_before_unregister.last_register_label orelse return error.ExpectedRegisterLabel);
    try std.testing.expectEqual(@as(?[]const u8, null), initialized_before_unregister.last_unregister_label);
    try std.testing.expectEqualStrings("hello", initialized_before_unregister.last_main_foo_bar_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqualStrings("Mother Goose", initialized_before_unregister.last_main_random_choice_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqual(@as(usize, 0), initialized_before_unregister.last_main_vararg_array_length orelse return error.ExpectedMainPayload);
    try std.testing.expect(initialized_before_unregister.last_main_vararg_array_terminator_zero orelse return error.ExpectedMainPayload);
    try std.testing.expectEqualStrings("HELLO", initialized_before_unregister.last_main_template_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqual(@as(?[]const u8, null), initialized_before_unregister.last_main_conditional_message);
    try std.testing.expectEqual(@as(?[]const u8, null), initialized_before_unregister.last_main_template_cond_message);
    try std.testing.expectEqualStrings("I have to be different", initialized_before_unregister.last_main_template_print_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqualStrings("Hello __rel_loc", initialized_before_unregister.last_main_relative_location_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqualStrings("Look at me", initialized_before_unregister.last_function_foo_bar_message orelse return error.ExpectedFunctionPayload);
    try std.testing.expectEqualStrings("Look at me too", initialized_before_unregister.last_function_template_message orelse return error.ExpectedFunctionPayload);
    try std.testing.expectEqualStrings("iter=%d", initialized_before_unregister.last_format_template orelse return error.ExpectedMainPayload);
    ...
    try std.testing.expectEqual(@as(usize, 1), initialized_fail_closed_before.main_iterations);
    try std.testing.expectEqual(@as(usize, 1), initialized_fail_closed_before.fn_iterations);
    try std.testing.expectEqual(@as(usize, 4), initialized_fail_closed_before.main_thread_events);
    try std.testing.expectEqual(@as(usize, 2), initialized_fail_closed_before.fn_thread_events);
    try std.testing.expectEqual(@as(usize, 6), initialized_fail_closed_before.total_events);
    try std.testing.expectEqual(@as(?usize, 4), initialized_fail_closed_before.last_main_emitted_events);
    try std.testing.expectEqual(@as(?usize, 2), initialized_fail_closed_before.last_fn_emitted_events);
    try std.testing.expectEqual(@as(?usize, 0), initialized_fail_closed_before.last_main_conditional_event_count);
    ...
    try std.testing.expectEqual(@as(i32, 5), initialized_fail_closed_before.last_main_count);
    try std.testing.expectEqual(@as(i32, 7), initialized_fail_closed_before.last_fn_count);
    ...
    try std.testing.expectEqualStrings("hello", initialized_fail_closed_before.last_main_foo_bar_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqualStrings("Mother Goose", initialized_fail_closed_before.last_main_random_choice_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqual(@as(usize, 0), initialized_fail_closed_before.last_main_vararg_array_length orelse return error.ExpectedMainPayload);
    try std.testing.expect(initialized_fail_closed_before.last_main_vararg_array_terminator_zero orelse return error.ExpectedMainPayload);
    try std.testing.expectEqualStrings("HELLO", initialized_fail_closed_before.last_main_template_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqual(@as(?[]const u8, null), initialized_fail_closed_before.last_main_conditional_message);
    try std.testing.expectEqual(@as(?[]const u8, null), initialized_fail_closed_before.last_main_template_cond_message);
    try std.testing.expectEqualStrings("I have to be different", initialized_fail_closed_before.last_main_template_print_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqualStrings("Hello __rel_loc", initialized_fail_closed_before.last_main_relative_location_message orelse return error.ExpectedMainPayload);
    try std.testing.expectEqualStrings("Look at me", initialized_fail_closed_before.last_function_foo_bar_message orelse return error.ExpectedFunctionPayload);
    try std.testing.expectEqualStrings("Look at me too", initialized_fail_closed_before.last_function_template_message orelse return error.ExpectedFunctionPayload);
    try std.testing.expectEqualStrings("iter=%d", initialized_fail_closed_before.last_format_template orelse return error.ExpectedMainPayload);
    ...
    try std.testing.expectEqual(@as(usize, 3), selftest_complete_before.main_iterations);
    try std.testing.expectEqual(@as(usize, 2), selftest_complete_before.fn_iterations);
    try std.testing.expectEqual(@as(usize, 14), selftest_complete_before.main_thread_events);
    try std.testing.expectEqual(@as(usize, 4), selftest_complete_before.fn_thread_events);
    try std.testing.expectEqual(@as(usize, 18), selftest_complete_before.total_events);
    ...
    try std.testing.expectEqual(@as(usize, 3), exited_before.main_iterations);
    try std.testing.expectEqual(@as(usize, 2), exited_before.fn_iterations);
    try std.testing.expectEqual(@as(usize, 14), exited_before.main_thread_events);
    try std.testing.expectEqual(@as(usize, 4), exited_before.fn_thread_events);
    try std.testing.expectEqual(@as(usize, 18), exited_before.total_events);
