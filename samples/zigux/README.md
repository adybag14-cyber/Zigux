...
But fresh direct readback in this run still did not recover `samples/zigux/kretprobe_example.zig`, `zigux/tests/phase5_kretprobe_example.zig`, `zigux/tests/phase5_kretprobe_example_manifest.json`, `zigux/tests/phase5_kretprobe_example_survey.zig`, or the shared `zigux/tests/phase5_build.zig` route. Keep contributor wording aligned with that survey note instead of treating the missing sample-root, focused tests-root, manifest-backed, survey-replay, or shared-build paths as directly readable shipped evidence on current `master`.
