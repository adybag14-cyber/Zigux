...
Current `master` also ships `samples/zigux/bytestream_fifo_window_contract.zig` as a bounded bytestream companion. Keep that file framed as reviewability help for the current two-window visible-span and writable-span packet rather than as a fifth Phase 5 sample family.
Keep `zig test samples/zigux/bytestream_fifo_window_contract.zig` explicit as the companion-only validation route for that bounded packet while `zigux/tests/phase5_build.zig` stays the current directly readable shared build-route companion for the broader bytestream packet.
...
* `zig test samples/zigux/bytestream_fifo.zig`, `zig test --dep bytestream_fifo_sample -Mroot=zigux/tests/phase5_bytestream_fifo.zig -Mbytestream_fifo_sample=samples/zigux/bytestream_fifo.zig`, and `zig test zigux/tests/phase5_bytestream_fifo_survey.zig` stay explicit as the sample-owned self-check, focused replay, and survey guard routes, while `zig test samples/zigux/bytestream_fifo_window_contract.zig` stays explicit as the companion-only validation route for the two-window packet and `zigux/tests/phase5_build.zig` remains current directly readable shared build-route companion evidence only
...
Current `master` also keeps the bounded non-runtime trace-events packet visible through the broader direct sample-root companion `samples/zigux/trace_events_sample.zig`, the direct formatting companion `samples/zigux/trace_events_string_formatting_sample.zig`, and the shared Phase 5 reminder packet.
