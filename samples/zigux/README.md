...
Keep the older runtime bitmap family framed as backlog-only Phase 9 support material until a fresh repo reread proves the exact file family returned on current `master`: `samples/zigux/runtime_bitmap.zig`, `samples/zigux/runtime_bitmap_loader.zig`, `samples/zigux/runtime_bitmap_top_bit_contract.zig`, `zigux/tests/runtime_bitmap_module.zig`, `zigux/tests/runtime_bitmap_diff.zig`, `zigux/tests/runtime_bitmap_survey.zig`, `zigux/tests/runtime_bitmap_manifest.json`, and `Documentation/zigux/phase9-runtime-bitmap-survey.md`. Those historical bitmap names remain lane-owned backlog vocabulary rather than directly readable neighboring runtime proof, proof that the broader shared runtime-loader packet returned, or evidence that a fifth approved Phase 5 sample family landed here.
