...
- keep the ownership-and-lifetime review surface explicit outside the main replay path: the pre-registration zero-active-attributes boundary, initialized-only abandonment path, registered teardown path, explicit ownership-state transitions, and post-exit rejection boundaries remain part of the shipped contract
