...
* `samples/zigux/bytestream_fifo_window_contract.zig` keeps the bounded two-window reference pattern explicit by naming the preview `{ 10, 0 }`, wrapped-full `{ 28, 4 }`, and post-drain `{ 23, 1 }` visible-window splits together with the matching writable-span boundaries `{ 15, 7 }`, `{ 0, 0 }`, and `{ 8, 0 }`
* `zig test samples/zigux/bytestream_fifo.zig`, `zig test samples/zigux/bytestream_fifo_window_contract.zig`, `zig test --dep bytestream_fifo_sample -Mroot=zigux/tests/phase5_bytestream_fifo.zig -Mbytestream_fifo_sample=samples/zigux/bytestream_fifo.zig`, and `zig test zigux/tests/phase5_bytestream_fifo_survey.zig` stay explicit as the sample-owned self-check, reference-pattern companion, focused replay, and survey guard routes, while `zigux/tests/phase5_build.zig` remains current directly readable shared build-route companion evidence only and now reruns the window-contract companion too
