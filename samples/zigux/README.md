...
- current `master` still ships no `samples/zigux/*rbtree*` Phase 5 reference sample; keep `rbtree` reviewability under `Documentation/zigux/phase7-rbtree-slice.md`, `lib/rbtree.zig`, and `zigux/tests/phase7_build.zig` instead of counting it as a fifth Phase 5 sample
- the Phase 5 bytestream FIFO packet keeps `samples/zigux/bytestream_fifo.zig` as the approved Phase 5 in-memory FIFO idiom for the roadmap's `samples/kfifo/bytestream-example.c` anchor, preserving the approved in-memory queue-order and ownership-and-lifetime idiom cues for review while also keeping the landed preview-truncation and short-drain helper-boundary checks explicit in the shared sample-root summary
