...
- keep the landed replay contract explicit: exact queue-order replay, transfer counts, skip-and-peek markers, non-destructive snapshot, bounded 32-byte capacity, and the exact `checked_focus` order `bounded_fifo_order`, `wraparound_requeue`, `peek_and_skip`, `non_destructive_snapshot`, `reset_and_replay`, and `ownership_and_lifetime`
- keep the helper-only review surface explicit outside the main replay path: empty-queue null handling, overflow rejection at capacity, skip-at-capacity, and queue-only reset remain part of the shipped contract
