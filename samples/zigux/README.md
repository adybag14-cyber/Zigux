...
The restored kobject anchor should stay explicit here too. Current shared Phase 5 guidance still treats `samples/kobject/kobject-example.c` as one of the four approved anchors, and current `master` now exposes the full directly readable kobject packet again:
...
Keep shared sample-root wording aligned with that restored kobject packet instead of repeating the older missing-path caveat.
...
Those anchors remain the approved Phase 5 target set. On current `master`, shared contributor guidance should keep all four directly readable `samples/zigux/*.zig` files explicit from this directory and keep the restored kobject packet above aligned with the same shared replay and survey surfaces instead of restating a missing sample-root path.
...
Use those shared surfaces to keep roadmap scope, contributor wording, the current directly readable four-sample Phase 5 packet, and the Phase 5-versus-runtime boundary honest.