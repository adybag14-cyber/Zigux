...
test "runtime kretprobe loader keeps selftest-complete shared-request snapshots stable across later exit activity" {
    ...
    try std.testing.expect(runtime_loader.keepsSelftestHookEvidenceConsistent(prepared_plan));
    try std.testing.expectEqual(runtime_kretprobe_sample.ModuleStage.selftest_complete, module.stage());
    ...
    try std.testing.expectEqualStrings("register_kretprobe", prepared_registration_snapshot.register_api);
    try std.testing.expectEqualStrings("unregister_kretprobe", prepared_registration_snapshot.unregister_api);
    try std.testing.expectEqualStrings("do_sys_openat2", prepared_registration_snapshot.symbol_name);
    try std.testing.expectEqual(@as(usize, 20), prepared_registration_snapshot.maxactive);
    try std.testing.expectEqual(@sizeOf(runtime_kretprobe_sample.InstancePrivateData), prepared_registration_snapshot.private_data_bytes);
    try std.testing.expectEqual(@as(usize, 1), prepared_registration_snapshot.skipped_kernel_threads);
    try std.testing.expectEqual(@as(usize, 1), prepared_registration_snapshot.nmissed);
    try std.testing.expectEqual(@as(usize, 42), prepared_registration_snapshot.last_retval);
    try std.testing.expectEqual(@as(i64, 75), prepared_registration_snapshot.last_duration_ns);
    try std.testing.expectEqual(@as(usize, 1), prepared_registration_snapshot.selftest_runs);
    try std.testing.expect(!prepared_registration_snapshot.entry_timestamp_armed);
    ...
    try std.testing.expect(runtime_loader.keepsSelftestHookEvidenceConsistent(pending_plan));
    try std.testing.expectEqual(runtime_kretprobe_sample.ModuleStage.selftest_complete, cached_pending_plan.handoff_stage);
    try std.testing.expect(keepsRegistrationSnapshotExplicit(cached_pending_plan, prepared_registration_snapshot));
    try std.testing.expectEqual(@as(usize, 1), cached_pending_plan.summary.skipped_kernel_threads);
    try std.testing.expectEqual(@as(usize, 1), cached_pending_plan.summary.nmissed);
    try std.testing.expectEqual(@as(usize, 42), cached_pending_plan.summary.last_retval);
    try std.testing.expectEqual(@as(i64, 75), cached_pending_plan.summary.last_duration_ns);
    try std.testing.expectEqual(@as(usize, 1), cached_pending_plan.summary.selftest_runs);
    try std.testing.expect(!cached_pending_plan.summary.entry_timestamp_armed);
...
test "runtime kretprobe loader rejects prepared shared runtime-substrate drift before any local runtime handoff" {
    var module = runtime_kretprobe_sample.RuntimeKretprobeSample{};
    try module.retargetSymbol("do_sys_openat2");
    try module.init();
    _ = try module.runSelftest();
...
test "runtime kretprobe loader rejects non-prepared shared requests before any local runtime handoff" {
    var module = runtime_kretprobe_sample.RuntimeKretprobeSample{};
    try module.retargetSymbol("do_sys_openat2");
    try module.init();
    _ = try module.runSelftest();
...
test "runtime kretprobe loader recovers from initialized-stage failed exit into the same bounded handoff plan" {
    var module = runtime_kretprobe_sample.RuntimeKretprobeSample{};
    try module.retargetSymbol("do_sys_openat2");
    try module.init();
    try std.testing.expect(try module.entryHandler(true, 500));
    ...
    try std.testing.expectEqual(runtime_kretprobe_sample.ModuleStage.initialized, module.stage());
    try std.testing.expectEqual(@as(usize, 1), blocked_summary.active_instances);
    try std.testing.expectEqual(@as(usize, 0), blocked_summary.skipped_kernel_threads);
    try std.testing.expectEqual(@as(usize, 0), blocked_summary.nmissed);
    try std.testing.expectEqual(@as(usize, 0), blocked_summary.last_retval);
    try std.testing.expectEqual(@as(i64, 0), blocked_summary.last_duration_ns);
    try std.testing.expectEqual(@as(usize, 0), blocked_summary.selftest_runs);
    try std.testing.expect(blocked_summary.entry_timestamp_armed);

    try std.testing.expectError(error.OutstandingProbeInstance, module.exit());
    try std.testing.expectError(error.OutstandingProbeStateForLoader, RuntimeKretprobeLoader.planFor(&module));

    var blocked_loader = RuntimeKretprobeLoader{};
    try std.testing.expectError(error.OutstandingProbeStateForLoader, blocked_loader.prepareSharedRequest(&module));
    ...
    const recovered = try RuntimeKretprobeLoader.planFor(&module);
    try std.testing.expectEqual(runtime_kretprobe_sample.ModuleStage.initialized, recovered.handoff_stage);
    try std.testing.expectEqualStrings("do_sys_openat2", recovered.symbol_name);
    try std.testing.expectEqual(@as(usize, 20), recovered.maxactive);
    try std.testing.expectEqual(@sizeOf(runtime_kretprobe_sample.InstancePrivateData), recovered.private_data_bytes);
    try std.testing.expectEqual(@as(usize, 0), recovered.summary.active_instances);
    try std.testing.expectEqual(@as(usize, 0), recovered.summary.skipped_kernel_threads);
    try std.testing.expectEqual(@as(usize, 0), recovered.summary.nmissed);
    try std.testing.expectEqual(@as(usize, 17), recovered.summary.last_retval);
    try std.testing.expectEqual(@as(i64, 60), recovered.summary.last_duration_ns);
    try std.testing.expectEqual(@as(usize, 0), recovered.summary.selftest_runs);
    try std.testing.expect(!recovered.summary.entry_timestamp_armed);
