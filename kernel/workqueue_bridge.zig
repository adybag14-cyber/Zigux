...
pub const AuditGuard = enum {
    ...
    pending_bit_with_irqs_disabled,
    pwq_refcnt_retry_loop,
    last_pool_lock_handoff,
    flush_color_cascade_under_wq_mutex,
    ...
    delayed_disable_wrapper_aliases,
    delayed_submission_alias_handoff,
    delayed_timer_expiry_requeue_handoff,
...
const audit_checkpoints = [_]AuditCheckpoint{
    ...
    .{
        ...
        .summary = "Keep PENDING-bit ownership and the irq-disabled queueing window under the existing atomic and local-IRQ rules.",
        .guard = .pending_bit_with_irqs_disabled,
        .observed_fields = &[_][]const u8{ "WORK_STRUCT_PENDING_BIT", "work->data", "irq_flags", "pool->lock" },
        .blocked_by = "try_to_grab_pending() and queue_work_on() both rely on owning WORK_STRUCT_PENDING_BIT while local IRQs stay disabled so the item never sits off-queue and interruptible between claiming PENDING and either queuing or clearing it, which is exactly the kind of submission control Zigux should audit before claiming a live wrapper.",
        .ownership = .stay_in_c,
    },
    .{
        .id = "unbound-pwq-refcnt-retry",
        .anchor_symbol = "__queue_work",
        .summary = "Record the unbound pwq refcount retry loop before any wrapper claims stable submission ownership across CPU or pool reselection.",
        .guard = .pwq_refcnt_retry_loop,
        .observed_fields = &[_][]const u8{ "pwq->refcnt", "wq->cpu_pwq", "pool->lock", "req_cpu" },
        .blocked_by = "__queue_work() may race an unbound pwq release, observe pwq->refcnt == 0 after locking pool->lock, drop the lock, cpu_relax(), and retry pwq selection until a replacement pwq is visible, so Zigux should record the retry seam instead of pretending unbound submission ownership is stable in Zig.",
        .ownership = .stay_in_c,
    },
    .{
    ...
    .{
        .id = "flush-workqueue-color-cascade",
        .anchor_symbol = "__flush_workqueue",
        .summary = "Keep flush-color rotation, first-flusher ownership, and overflow cascading under the existing workqueue mutex and color-discipline rules.",
        .guard = .flush_color_cascade_under_wq_mutex,
        .observed_fields = &[_][]const u8{ "wq->work_color", "wq->flush_color", "wq->first_flusher", "wq->flusher_overflow" },
        .blocked_by = "__flush_workqueue() advances work_color with work_next_color(), installs or clears first_flusher, and cascades overflowed flushers back into flusher_queue under wq->mutex, so Zigux should audit the color handoff instead of claiming live flush completion ownership.",
        .ownership = .stay_in_c,
    },
    .{
        .id = "flush-work-barrier-insertion",
        .anchor_symbol = "start_flush_work",
        .summary = "Keep barrier insertion and the executing-worker handoff under the existing pool-lock and RCU lookup rules.",
        .guard = .flush_barrier_insert_under_pool_lock,
        .observed_fields = &[_][]const u8{ "pool->lock", "worker->current_pwq", "work->data", "wq->saved_max_active" },
        .blocked_by = "start_flush_work() resolves the live pwq or executing worker under RCU, validates flush dependency state, and inserts a barrier while pool->lock is still held, so Phase 14 should name the barrier handoff instead of claiming live flush or cancel ownership in Zig.",
        .ownership = .stay_in_c,
    },
    .{
        .id = "pwq-in-flight-color-release",
        .anchor_symbol = "pwq_dec_nr_in_flight",
        .summary = "Keep flush-color completion and in-flight release under the existing color accounting, atomic countdown, and pwq lifetime rules.",
        .guard = .in_flight_color_release_completion,
        .observed_fields = &[_][]const u8{ "pwq->nr_in_flight", "pwq->flush_color", "wq->nr_pwqs_to_flush", "wq->first_flusher" },
        .blocked_by = "pwq_dec_nr_in_flight() decrements the color bucket, may clear pwq->flush_color only at the flushing tip, may drop the unbound pool lock later in the path, and completes wq->first_flusher only when nr_pwqs_to_flush reaches zero, so Zigux should audit the release and completion seam rather than claim live drain progression.",
        .ownership = .stay_in_c,
    },
    .{
        .id = "drain-reflush-and-cancel-sync",
        .anchor_symbol = "drain_workqueue/__flush_work/__cancel_work_sync",
        .summary = "Keep reflush looping, single-work barrier waiting, and cancellation completion under the existing drain flag, completion, and context rules.",
        .guard = .drain_reflush_and_cancel_sync,
        .observed_fields = &[_][]const u8{ "wq->nr_drainers", "wq->flags", "barr.done", "work->data" },
        .blocked_by = "drain_workqueue() raises __WQ_DRAINING while nr_drainers is non-zero, loops back through __flush_workqueue() until pwq_is_empty() under pool->lock, __flush_work() waits on barr.done after start_flush_work() inserts the barrier, and __cancel_work_sync() only re-enables the work item after that wait path completes, so Zigux should record the reflush and cancellation boundary instead of claiming live draining or cancel completion parity.",
        .ownership = .stay_in_c,
    },
    .{
    ...
    .{
        .id = "delayed-disable-wrapper-aliases",
        .anchor_symbol = "disable_delayed_work/disable_delayed_work_sync/enable_delayed_work",
        .summary = "Record that the delayed disable and re-enable helpers are thin aliases over the existing cancel-sync and enable_work paths, not independent wrapper ownership.",
        .guard = .delayed_disable_wrapper_aliases,
        .observed_fields = &[_][]const u8{ "dwork->work", "WORK_CANCEL_DELAYED", "WORK_CANCEL_DISABLE", "offqd.disable" },
        .blocked_by = "disable_delayed_work() is only __cancel_work(&dwork->work, WORK_CANCEL_DELAYED | WORK_CANCEL_DISABLE), disable_delayed_work_sync() is the matching __cancel_work_sync() alias, and enable_delayed_work() simply returns enable_work(&dwork->work), so Zigux should record the delayed alias layer before claiming timer, execution, or requeue ownership.",
        .ownership = .stay_in_c,
    },
    .{
        .id = "delayed-submission-alias-handoff",
        .anchor_symbol = "queue_delayed_work_on/mod_delayed_work_on/__queue_delayed_work",
        .summary = "Record that delayed submission helpers are alias layers over timer-backed queueing and pending ownership rather than independent execution ownership.",
        .guard = .delayed_submission_alias_handoff,
        .observed_fields = &[_][]const u8{ "dwork->timer", "work->data", "delay", "cpu" },
        .blocked_by = "queue_delayed_work_on() and mod_delayed_work_on() are thin delayed-entry wrappers over __queue_delayed_work(), which must preserve pending ownership on work->data, refuse disabled work instead of bypassing off-queue disable state, and hand the item to dwork->timer until timer expiry re-enters __queue_work(), so Zigux should record the alias and timer handoff instead of claiming CPU-affinity, timer-base, or requeue parity.",
        .ownership = .stay_in_c,
    },
    .{
        .id = "delayed-timer-expiry-handoff",
        .anchor_symbol = "delayed_work_timer_fn",
        .summary = "Record the delayed timer expiry callback and its handoff back into __queue_work() as a review-only ownership boundary.",
        .guard = .delayed_timer_expiry_requeue_handoff,
        .observed_fields = &[_][]const u8{ "dwork->timer", "work->data", "WORK_STRUCT_PENDING_BIT", "pool->lock" },
        .blocked_by = "delayed_work_timer_fn() fires after delayed submission parks the item on dwork->timer, keeps WORK_STRUCT_PENDING_BIT ownership on work->data until timer expiry hands the item back into __queue_work(), and then rejoins the same pool routing under pool->lock, so Zigux should record the timer-expiry handoff instead of claiming timer-base, CPU-affinity, or requeue parity.",
        .ownership = .stay_in_c,
    },
    .{
...
pub const WorkqueueBridgeLab = struct {
    ...
    pub fn nextAuditFocus() []const u8 {
        return "Leave this lane in blocked maintenance unless the shared Phase 14 smoke packet or this workqueue survey drifts; any reopen should stay review-only and keep timer-base, CPU-affinity, and requeue ownership in C.";
    }
};
...
test "workqueue bridge boundary map records stay-in-c decisions" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, WorkqueueBridgeLab.nextAuditFocus(), "CPU-affinity") != null);
...
test "workqueue bridge concurrency audit stays review-only" {
    ...
    try std.testing.expectEqualStrings("WORK_STRUCT_PENDING_BIT", audit.checkpoints[3].observed_fields[0]);
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[3].blocked_by, "queue_work_on()") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[4].blocked_by, "cpu_relax()") != null);
    ...
    try std.testing.expectEqualStrings("wq->first_flusher", audit.checkpoints[6].observed_fields[2]);
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[6].blocked_by, "flusher_queue") != null);
    ...
    try std.testing.expectEqualStrings("worker->current_pwq", audit.checkpoints[7].observed_fields[1]);
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[7].blocked_by, "inserted a barrier") == null);
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[7].blocked_by, "inserts a barrier") != null);
    ...
    try std.testing.expectEqualStrings("wq->nr_pwqs_to_flush", audit.checkpoints[8].observed_fields[2]);
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[8].blocked_by, "nr_pwqs_to_flush") != null);
    ...
    try std.testing.expectEqualStrings("dwork->work", audit.checkpoints[11].observed_fields[0]);
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[11].blocked_by, "__cancel_work_sync()") != null);
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[11].blocked_by, "enable_work(&dwork->work)") != null);
    ...
    try std.testing.expectEqualStrings("pwq->nr_in_flight", audit.checkpoints[14].observed_fields[3]);
    ...
    try std.testing.expectEqualStrings("pool->lock", audit.checkpoints[15].observed_fields[2]);

    try std.testing.expectEqualStrings("scheduler-running-hooks", audit.checkpoints[16].id);
    try std.testing.expect(audit.checkpoints[16].guard == .scheduler_callback_under_pool_lock);
    try std.testing.expectEqualStrings("pool->nr_running", audit.checkpoints[16].observed_fields[1]);

    try std.testing.expectEqualStrings("rescuer-mayday-handoff", audit.checkpoints[17].id);
    try std.testing.expect(audit.checkpoints[17].guard == .mayday_lock_then_pool_lock);
    try std.testing.expectEqualStrings("pwq->mayday_cursor", audit.checkpoints[17].observed_fields[2]);
    try std.testing.expect(std.mem.indexOf(u8, audit.checkpoints[17].blocked_by, "kicks regular workers") != null);
}
