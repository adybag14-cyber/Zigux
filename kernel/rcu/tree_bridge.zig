...
test "tree bridge boundary map stays review-only" {
    ...
    try std.testing.expectEqualStrings("grace_period_sequence_publication", blocked_boundaries[0].id);
    try std.testing.expectEqualStrings("rcu_start_this_gp", blocked_boundaries[0].anchor_symbols[0]);
    try std.testing.expectEqualStrings("rcu_gp_init", blocked_boundaries[0].anchor_symbols[1]);
    try std.testing.expectEqualStrings("__note_gp_changes", blocked_boundaries[0].anchor_symbols[2]);
    try std.testing.expect(contains(blocked_boundaries[0].rationale, "gp_seq"));
    try std.testing.expectEqualStrings("memory_ordering_lock_network", blocked_boundaries[1].id);
    try std.testing.expectEqualStrings("raw_spin_lock_rcu_node", blocked_boundaries[1].anchor_symbols[0]);
    try std.testing.expectEqualStrings("smp_mb__after_unlock_lock", blocked_boundaries[1].anchor_symbols[1]);
    try std.testing.expectEqualStrings("smp_store_release", blocked_boundaries[1].anchor_symbols[2]);
    try std.testing.expect(contains(blocked_boundaries[1].summary, "lock-ordering"));
    try std.testing.expect(contains(blocked_boundaries[1].rationale, "ordering contract"));
    try std.testing.expectEqualStrings("expedited_funnel_and_stall_path", blocked_boundaries[2].id);
    try std.testing.expectEqualStrings("sync_rcu_exp_select_cpus", blocked_boundaries[2].anchor_symbols[0]);
    try std.testing.expectEqualStrings("synchronize_rcu_expedited_wait_once", blocked_boundaries[2].anchor_symbols[1]);
    try std.testing.expectEqualStrings("rcu_exp_gp_seq_end", blocked_boundaries[2].anchor_symbols[2]);
    try std.testing.expect(contains(blocked_boundaries[2].summary, "wait serialization"));
    try std.testing.expect(contains(blocked_boundaries[2].rationale, "stall-sensitive"));
    try std.testing.expectEqualStrings("nocb_offload_wakeup_handoff", blocked_boundaries[3].id);
    try std.testing.expectEqualStrings("rcu_nocb_bypass_lock", blocked_boundaries[3].anchor_symbols[0]);
    try std.testing.expectEqualStrings("wake_nocb_gp_defer", blocked_boundaries[3].anchor_symbols[1]);
    try std.testing.expectEqualStrings("do_nocb_deferred_wakeup", blocked_boundaries[3].anchor_symbols[2]);
    try std.testing.expect(contains(blocked_boundaries[3].summary, "callback-offload"));
    try std.testing.expect(contains(blocked_boundaries[3].rationale, "bypass-list pressure"));
    ...
    try std.testing.expectEqualStrings("rcu_force_quiescent_state", blocked_boundaries[7].anchor_symbols[0]);
    try std.testing.expectEqualStrings("rcu_gp_kthread_wake", blocked_boundaries[7].anchor_symbols[1]);
    try std.testing.expectEqualStrings("rcu_gp_fqs_loop", blocked_boundaries[7].anchor_symbols[2]);
    ...
    try std.testing.expect(contains(blocked_boundaries[7].rationale, "gp_flags"));
    try std.testing.expectEqualStrings("poll_cookie_and_sync_waithead_rollover", blocked_boundaries[8].id);
    try std.testing.expectEqualStrings("rcu_poll_gp_seq_start_unlocked", blocked_boundaries[8].anchor_symbols[0]);
    try std.testing.expectEqualStrings("rcu_poll_gp_seq_end_unlocked", blocked_boundaries[8].anchor_symbols[1]);
    try std.testing.expectEqualStrings("rcu_sr_normal_gp_init", blocked_boundaries[8].anchor_symbols[2]);
    ...
    try std.testing.expect(contains(blocked_boundaries[8].rationale, "gp_seq_polled"));
