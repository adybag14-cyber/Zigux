...
const REQUIRED_EXACT_LINES_ENTRIES = [_]struct { file: []const u8, marker: []const u8 }{
    ...
    .{ .file = "Documentation/zigux/README.md", .marker = "  * `zig run validate_phase1_closure.zig`, `zig run check_phase1_string_review_packet.zig --self-test`, `zig run check_phase1_direct_owner_markers.zig --self-test`, and `zig run check_phase1_bench.zig --self-test` replay the bounded current reminder checks, while the live checker routes guard the shipped Phase 1 packet without widening it back into the older closure-side or installer-companion stack." },
    ...
    .{ .file = "scripts/zigux/README.md", .marker = "- `zig run validate_phase1_closure.zig`, `zig run check_phase1_string_review_packet.zig --self-test`, `zig run check_phase1_direct_owner_markers.zig --self-test`, and `zig run check_phase1_bench.zig --self-test` replay the shipped bounded Phase 1 reminder checks, and `zig build phase1-host-tools-smoke --build-file zigux/tests/build.zig` replays the shipped shared tests-root smoke route" },
