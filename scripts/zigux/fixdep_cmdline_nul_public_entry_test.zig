...
test "runFixdep preserves NUL command bytes in savedcmd prelude" {
    ...
    const depfile_name = "zigux_fixdep_nul_cmdline_test.d";
    const source_name = "zigux_fixdep_nul_cmdline_source.c";

    try Io.Dir.cwd().writeFile(std.testing.io, .{
        .sub_path = source_name,
        .data = "/* CONFIG_ZIGUX_CMDLINE_NUL */\n",
    });
    ...
    const cmdline = "cc\x00-DZIGUX_CMDLINE_NUL=1 -c " ++ source_name;
    ...
    try std.testing.expectEqualStrings(
        "savedcmd_sample.o := " ++ cmdline ++ "\n\n" ++
            ...
            "deps_sample.o := \\\n" ++
            "    $(wildcard include/config/ZIGUX_CMDLINE_NUL) \\\n" ++
            "\n" ++
