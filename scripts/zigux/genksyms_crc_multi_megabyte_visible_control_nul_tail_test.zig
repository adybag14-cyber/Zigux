...
test "lane19 multi-megabyte exact-buffer split keeps a visible control tail before NUL and still reaches the next line beyond the old CLI cap" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + c_line_payload_len + 13,
    );
