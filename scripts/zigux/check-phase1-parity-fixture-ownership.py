...
SELF_TEST_CASES = [
    ...
    "duplicate_owned_key_across_groups",
    "sentinel_drift",
]
    ...
    for helper, section in HELPER_TO_SECTION.items():
        ...
        for group_name, keys in groups.items():
            ...
            for key in keys:
                ...
                if group_name != "parity_fixture_keys":
                    owners_by_key.setdefault(key, []).append(group_name)

        for key, group_names in owners_by_key.items():
            ensure(
                len(group_names) == 1,
                issues,
                f"manifest:overlapping_secondary_owned_key:{helper}:{key}",
            )
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_fixture_ownership_") as tmp_dir:
        ...
        duplicate_owned_key_across_groups = root / "duplicate_owned_key_across_groups"
        write_sample_root(duplicate_owned_key_across_groups)
        manifest = load_json(duplicate_owned_key_across_groups / MANIFEST_REL)
        assert isinstance(manifest, dict)
        review_anchors = manifest["review_anchors"]
        assert isinstance(review_anchors, dict)
        bitmap_anchor = review_anchors["tools/lib/bitmap.zig"]
        assert isinstance(bitmap_anchor, dict)
        shared_range = bitmap_anchor["shared_range_fixture_keys"]
        assert isinstance(shared_range, list)
        shared_range[0] = "and_values"
        write_json(duplicate_owned_key_across_groups / MANIFEST_REL, manifest)
        expect_failure(
            "duplicate_owned_key_across_groups",
            lambda: assert_validation_pass(duplicate_owned_key_across_groups),
        )
        covered.append("duplicate_owned_key_across_groups")
