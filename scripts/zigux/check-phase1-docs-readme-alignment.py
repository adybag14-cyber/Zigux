...
def collect_failures(root: Path) -> list[str]:
    ...
    for marker in FORBIDDEN_MARKERS:
        count = source.count(marker)
        if count != 0:
            failures.append(f"docs_readme:forbidden:{marker}:actual_count={count}")
    return failures
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_docs_readme_") as tmp_dir:
        ...
        for marker in cases:
            ...
            expected = [f"docs_readme:missing:{marker.rstrip()}"]
            if failures != expected:
                raise AssertionError(
                    f"unexpected failures for {marker.rstrip()!r}: {failures}"
                )
            case_count += 1

        _write(
            root / DOCS_README_PATH,
            _sample_docs_readme() + FORBIDDEN_MARKERS[0] + "\n",
        )
        failures = collect_failures(root)
        expected = [f"docs_readme:forbidden:{FORBIDDEN_MARKERS[0]}:actual_count=1"]
        if failures != expected:
            raise AssertionError(f"unexpected forbidden-marker failures: {failures}")
        case_count += 1
