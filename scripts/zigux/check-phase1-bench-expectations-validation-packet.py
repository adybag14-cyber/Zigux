...
SELF_TEST_MARKERS = {
    ...
    "duplicate_exact_checksum_text": 'kind, payload = validate_expectations(load_expectations_text(duplicate_exact_text))',
...
def run_self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="lane16-bench-expectations-validation-") as tmp:
        ...
        workflow_path.write_text(
            original_workflow.replace(WORKFLOW_MARKERS["bench_self_test_command"], "", 1),
            encoding="utf-8",
        )
        kind, payload = validate_root(root)
