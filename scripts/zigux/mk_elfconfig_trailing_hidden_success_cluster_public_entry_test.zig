...
test "first non-ELF header stays authoritative with hidden valid ELF cluster" {
    const input = not_elf_header ++ elf32_header ++ elf64_header;
    try expectRun(&input, 1, "", not_elf_text);
}

test "first invalid class stays authoritative with hidden valid ELF cluster" {
    const input = invalid_class_header ++ elf64_header ++ elf32_header;
    try expectRun(&input, 1, "", "");
}

test "first 32-bit ELF header stays authoritative with hidden valid ELF cluster" {
    const input = elf32_header ++ elf64_header ++ elf32_header;
    try expectRun(&input, 0, elf32_define, "");
}

test "first 64-bit ELF header stays authoritative with hidden valid ELF cluster" {
    const input = elf64_header ++ elf32_header ++ elf64_header;
