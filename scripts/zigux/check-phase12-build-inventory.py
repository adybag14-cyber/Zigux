...
def render_inventory(build_text: str) -> dict[str, object]:
    ...
    return {
        ...
        "smoke_step_depend_steps": SMOKE_DEPEND_RE.findall(build_text),
        "test_step_depend_steps": TEST_DEPEND_RE.findall(build_text),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check that the Phase 12 shared-build inventory fixture matches the "
            "live build packet structure."
        )
    )
    ...
    inventory = CURRENT_FIXTURE
    print("PHASE12_BUILD_INVENTORY=pass")
    print(f"PHASE12_BUILD_INVENTORY_TEST_COUNT={len(inventory['build_test_names'])}")
    print(
        "PHASE12_BUILD_INVENTORY_MODULE_COUNT="
        f"{len(inventory['module_root_source_files'])}"
    )
    return 0
