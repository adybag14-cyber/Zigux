    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_export_uapi_") as tmp_dir:
        ...
        makefile_path.write_text(
            makefile_path.read_text(encoding="utf-8").replace(
                "cd $(ZIGUX_ROOT) && $(PYTHON) scripts/zigux/validate-phase3-export-uapi-survey.py --self-test",
                "cd $(ZIGUX_ROOT) && $(PYTHON) scripts/zigux/validate-phase3-export-uapi-survey-missing.py --self-test",
                1,
            ),
            encoding="utf-8",
            newline="\n",
        ...
        issues = validate(root)
        expected = [
            "missing_makefile_marker:cd $(ZIGUX_ROOT) && $(PYTHON) scripts/zigux/validate-phase3-export-uapi-survey.py --self-test"
        ]
        if issues != expected:
            raise SystemExit(f"phase3-export-uapi-self-test:makefile_guard_failed:{issues}")
        _write(
            root / MAKEFILE_REL,
            "\n".join(
                (
