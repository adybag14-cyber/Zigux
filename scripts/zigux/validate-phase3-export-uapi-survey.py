...
def validate(root: Path) -> list[str]:
    issues: list[str] = []
    for rel in (SURVEY, DOCS_README, SCRIPTS_README, WORKFLOW, EXPORT_SHIM, UAPI_VERSION, BUILD_FILE, MAKEFILE, VALIDATOR):
    ...
    for key, rel in (("PHASE3_EXPORT_SHIM_BLOB_SHA", EXPORT_SHIM), ("PHASE3_UAPI_VERSION_BLOB_SHA", UAPI_VERSION)):
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_export_uapi_") as tmp:
        ...
        write(root / EXPORT_SHIM, (root / EXPORT_SHIM).read_text(encoding="utf-8") + "// drift\n")
