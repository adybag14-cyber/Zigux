...
EXPORT_UAPI_BEHAVIOR_REL = "zigux/tests/phase3_export_uapi.zig"
...
SURVEY_EXACT_MARKERS = (
    ...
    f"`PHASE3_EXPORT_UAPI_BEHAVIOR_PATH={EXPORT_UAPI_BEHAVIOR_REL}`",
...
SURVEY_BLOB_MARKERS = (
    ...
    ("PHASE3_EXPORT_UAPI_BEHAVIOR_BLOB_SHA", EXPORT_UAPI_BEHAVIOR_REL),
...
def minimal_export_uapi_behavior_text() -> str:
    return "\n".join(
        (
            'const export_shim = @import("export_shim");',
            'const uapi_version = @import("uapi_version");',
...
def baseline_survey(root: Path) -> str:
    return "\n".join(
        (
            ...
            f"- `PHASE3_EXPORT_UAPI_BEHAVIOR_BLOB_SHA={blob_sha(root / EXPORT_UAPI_BEHAVIOR_REL)}`",
            f"- {SURVEY_EXACT_MARKERS[9]}",
            f"- `PHASE3_EXPORT_UAPI_BUILD_BLOB_SHA={blob_sha(root / EXPORT_UAPI_BUILD_REL)}`",
            f"- {SURVEY_EXACT_MARKERS[10]}",
            f"- `PHASE3_EXPORT_UAPI_LAYOUT_BLOB_SHA={blob_sha(root / EXPORT_UAPI_LAYOUT_REL)}`",
            f"- {SURVEY_EXACT_MARKERS[11]}",
            f"- `PHASE3_EXPORT_UAPI_LAYOUT_BUILD_BLOB_SHA={blob_sha(root / EXPORT_UAPI_LAYOUT_BUILD_REL)}`",
            f"- {SURVEY_EXACT_MARKERS[12]}",
...
def build_valid_workspace(root: Path) -> None:
    ...
    _write(root / EXPORT_UAPI_BEHAVIOR_REL, minimal_export_uapi_behavior_text())
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_export_uapi_") as tmp_dir:
        ...
        issues = validate(root)
        assert len(issues) == 2, issues
        assert issues[0].startswith("stale_survey_blob:PHASE3_EXPORT_UAPI_BUILD_BLOB_SHA:"), issues
        assert issues[1] == 'missing_marker:zigux/tests/phase3_export_uapi_build.zig:export_shim_module.addImport("uapi_version", uapi_version_module);', issues
        build_valid_workspace(root)
        ...
        issues = validate(root)
        assert len(issues) == 2, issues
        assert issues[0].startswith("stale_survey_blob:PHASE3_EXPORT_UAPI_LAYOUT_BUILD_BLOB_SHA:"), issues
        assert issues[1] == 'missing_marker:zigux/tests/phase3_export_uapi_layout_build.zig:.root_source_file = b.path("phase3_export_uapi_layout.zig"),', issues
        build_valid_workspace(root)
        ...
        survey_path.write_text(
            survey_path.read_text(encoding="utf-8").replace(
                f"`PHASE3_EXPORT_UAPI_BEHAVIOR_PATH={EXPORT_UAPI_BEHAVIOR_REL}`",
                "`PHASE3_EXPORT_UAPI_BEHAVIOR_PATH=broken`",
                1,
            ),
            encoding="utf-8",
            newline="\n",
        ...
        assert validate(root) == [
            f"missing_survey_marker:`PHASE3_EXPORT_UAPI_BEHAVIOR_PATH={EXPORT_UAPI_BEHAVIOR_REL}`"
        ]
        build_valid_workspace(root)
        ...
        survey_path.write_text(
            survey_path.read_text(encoding="utf-8").replace(
                f"`PHASE3_EXPORT_UAPI_BUILD_PATH={EXPORT_UAPI_BUILD_REL}`",
                "`PHASE3_EXPORT_UAPI_BUILD_PATH=broken`",
                1,
            ),
            encoding="utf-8",
            newline="\n",
        ...
        assert validate(root) == [
            f"missing_survey_marker:`PHASE3_EXPORT_UAPI_BUILD_PATH={EXPORT_UAPI_BUILD_REL}`"
        ]
        build_valid_workspace(root)
        ...
        survey_path.write_text(
            survey_path.read_text(encoding="utf-8").replace(
                f"`PHASE3_EXPORT_UAPI_LAYOUT_BUILD_PATH={EXPORT_UAPI_LAYOUT_BUILD_REL}`",
                "`PHASE3_EXPORT_UAPI_LAYOUT_BUILD_PATH=broken`",
                1,
            ),
            encoding="utf-8",
            newline="\n",
        ...
        assert validate(root) == [
            f"missing_survey_marker:`PHASE3_EXPORT_UAPI_LAYOUT_BUILD_PATH={EXPORT_UAPI_LAYOUT_BUILD_REL}`"
        ]
        build_valid_workspace(root)

        _write(root / EXPORT_UAPI_BEHAVIOR_REL, minimal_export_uapi_behavior_text().replace('    const canonical_status = export_shim.compatibilityStatus(canonical, -22, .kernel);\n', '', 1))
        issues = validate(root)
        assert len(issues) == 2, issues
        assert issues[0].startswith("stale_survey_blob:PHASE3_EXPORT_UAPI_BEHAVIOR_BLOB_SHA:"), issues
        assert issues[1] == 'missing_marker:zigux/tests/phase3_export_uapi.zig:const canonical_status = export_shim.compatibilityStatus(canonical, -22, .kernel);', issues
