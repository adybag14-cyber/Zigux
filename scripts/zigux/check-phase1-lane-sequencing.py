#!/usr/bin/env python3
"""Guard the Phase 1 lane-sequencing packet against split and tie-breaker drift."""
...
MANIFEST_EXPECTATIONS = {
    ...
    ("review_anchors", "tools/lib/bitmap.zig", "next_safe_step_note"): EXPECTED_BITMAP_NEXT_SAFE_STEP,
    ("review_anchors", "tools/lib/find_bit.zig", "andnot_scan_entrypoints"): EXPECTED_FIND_BIT_ANDNOT_SCAN_ENTRYPOINTS,
    ("review_anchors", "tools/lib/find_bit.zig", "next_safe_step_note"): EXPECTED_FIND_BIT_NEXT_SAFE_STEP,
    ("review_anchors", "tools/lib/rbtree.zig", "cached_leftmost_fixture_keys"): ["cached_leftmost_return_serials"],
    ("review_anchors", "tools/lib/rbtree.zig", "next_safe_step_note"): EXPECTED_RBTREE_NEXT_SAFE_STEP,
    ("review_anchors", "tools/lib/string.zig", "counted_search_review_anchors"): EXPECTED_STRING_COUNTED_SEARCH_REVIEW_ANCHORS,
    ("review_anchors", "tools/lib/string.zig", "next_safe_step_note"): EXPECTED_STRING_NEXT_SAFE_STEP,
}
...
def run_self_test() -> int:
    ...
    for relative_path, lines in REQUIRED_EXACT_LINES.items():
        for idx, line in enumerate(lines):
            cases.append((f"missing_line:{relative_path.as_posix()}:{idx}", "remove_line", (relative_path, line)))
            cases.append((f"duplicate_line:{relative_path.as_posix()}:{idx}", "duplicate_line", (relative_path, line)))
...
def main() -> int:
    ...
    print(f"PHASE1_LANE_SEQUENCING_REQUIRED_LINE_COUNT={sum(len(lines) for lines in REQUIRED_EXACT_LINES.values())}")
