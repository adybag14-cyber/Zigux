...
def run_self_test() -> int:
    ...
    mutation_cases = (
        ...
        (
            "missing_lane_rule_summary",
            lambda root: write_file(
                root,
                LANE_NOTE_REL,
                sample_text(LANE_NOTE_REL).replace(
                    REQUIRED_EXACT_LINES[LANE_NOTE_REL][2] + "\n",
                    "",
                    1,
                ),
            ),
        ...
        (
            "missing_closure_next_step",
            lambda root: write_file(
                root,
                PHASE1_CLOSURE_REL,
                sample_text(PHASE1_CLOSURE_REL).replace(
                    "helper-family tie-breaker",
                    "helper-family follow-up",
                    1,
                ),
            ),
        ),
        (
            "missing_tests_followthrough_note",
            lambda root: write_file(
                root,
                TESTS_README_REL,
                sample_text(TESTS_README_REL).replace(
                    REQUIRED_EXACT_LINES[TESTS_README_REL][1] + "\n",
                    "",
                    1,
                ),
            ),
                ...
                json.dumps(
                    {
                        **json.loads(sample_manifest()),
                        "review_anchors": {
                            **json.loads(sample_manifest())["review_anchors"],
                            "tools/lib/string.zig": {"next_safe_step_note": "drift"},
                        },
                    },
                    indent=2,
                )
    ...
    for _name, mutate in mutation_cases:
        with tempfile.TemporaryDirectory(prefix="phase1-lane-sequencing-") as tmpdir:
            root = Path(tmpdir)
            build_sample_root(root)
            mutate(root)
            if not collect_failures(root):
                print("PHASE1_LANE_SEQUENCING_SELF_TEST=fail")
                print(f"mutation_case_passed_unexpectedly:{_name}")
                return 1
            cases_run += 1
