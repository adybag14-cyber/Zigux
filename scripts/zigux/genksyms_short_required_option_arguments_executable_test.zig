...
test "genksyms executable keeps short required option arguments as data" {
    ...
    const exe_path = try std.fmt.bufPrint(
        &exe_path_buffer,
        ".zig-cache/tmp/{s}/genksyms-short-required-args",
        .{tmp.sub_path},
    );
    ...
    try testing.expectEqualStrings(
        "{\"tool\":\"scripts/genksyms/genksyms\",\"stdin\":\"cpp-stream\",\"stdout\":\"symversions\",\"argv\":[\"scripts/genksyms/genksyms\",\"-r\",\"-d\",\"-T\",\"-p\",\"-w\",\"unit.c\"],\"options\":{\"debug_level\":0,\"warnings\":true,\"dump_defs\":false,\"preserve\":false,\"reference_files\":[\"-d\"],\"dump_types_file\":\"-p\"}}\n",
        run_result.stdout,
    );
}
