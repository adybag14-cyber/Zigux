...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_validate_bootstrap_") as tmp_dir:
        ...
        write_text(
            root,
            "scripts/zigux/README.md",
            read_text(root, "scripts/zigux/README.md").replace("- `scripts/zigux/stage-pinned-zig-archive.py`\n", "", 1),
        )
        assert ("MISSING_SCRIPTS_README_MARKER", "scripts/zigux/stage-pinned-zig-archive.py") in collect_issues(root)
        checks += 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate that the current Zigux bootstrap packet still exposes its charter, docs, pinned-toolchain, staged-archive, and workflow surfaces."
    )
