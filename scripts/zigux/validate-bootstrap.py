    ...
    write_text(
        ...
        "\n".join(
            (
                ...
                "",
                "- Lane 05 bootstrap CI keeps the repo-local path explicit.",
                "- `third_party/zig-x86_64-linux-0.17.0-dev.87+9b177a7d2.tar.xz`",
                "- `scripts/zigux/check-lane05-local-first-archive-workflow.py` and `scripts/zigux/check-lane05-local-archive-readme.py` are the shipped reminder guards for that local-first archive path.",
            )
        )
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_validate_bootstrap_") as tmp_dir:
        ...
        write_text(
            ...
            replace_exact_line(
                ...
                "run: python3 scripts/zigux/missing-lane05-selftest.py",
            ),
        )
        assert ("MISSING_WORKFLOW_LINE", REQUIRED_WORKFLOW_LINES[3]) in collect_issues(root)
        checks += 1
