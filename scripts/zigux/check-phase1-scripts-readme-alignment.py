...
REQUIRED_MISSING_FILES = (
    ...
    Path("zigux/tests/phase1_helpers_c_harness.c"),
)
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_scripts_readme_alignment_") as tmp_dir:
        ...
        for marker in README_MARKERS:
            broken_root = root / ("missing-marker-" + str(checks_run))
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-sample-root",
        type=Path,
        help="Write a minimal current-like sample root for no-checkout validation and exit",
    )
    ...
    if args.write_sample_root is not None:
        ...
        print(f"PHASE1_SCRIPTS_README_ALIGNMENT_SAMPLE_ROOT={args.write_sample_root.resolve()}")
        return 0
