...
"""
    ...
    missing_core_interrupt_compound_ack = good.replace(
        "  * `zigux/tests/phase10_virtio_core_interrupt_compound_ack.zig`\n",
        "",
        1,
    )
    try:
        check_text(missing_core_interrupt_compound_ack)
    except SystemExit as exc:
        assert "`zigux/tests/phase10_virtio_core_interrupt_compound_ack.zig`" in str(exc)
    else:
        raise AssertionError("expected missing core interrupt compound ack marker failure")
