#!/usr/bin/env python3
"""Fail-close the bounded Phase 3 list/hlist parity packet."""
...
SLICE_PATH = Path("Documentation/zigux/phase3-list-hlist-slice.md")
...
REQUIRED_MARKERS = {
    ...
    BINDINGS_PATH: (
        ...
        "pub const list_head_size: usize",
        "pub const hlist_head_size: usize",
        "pub const hlist_node_size: usize",
    ),
    LIST_VIEW_PATH: (
        "pub const ListHead = extern struct",
            ...
            if isinstance(packet_files, list):
                for required_path in (
                    ...
                    if required_path not in packet_files:
                        issues.append(
                            "phase3_list_hlist_manifest.json missing packet_files entry: "
                            f"{required_path}"
                        )
            if isinstance(replay_routes, list):
                for route in (
                    ...
                    if route not in replay_routes:
                        issues.append(
                            "phase3_list_hlist_manifest.json missing replay route: "
                            f"{route}"
                        )
...
def run_self_test() -> int:
    ...
    print(f"PHASE3_LIST_HLIST_SELF_TEST_CASE_COUNT={len(SELF_TEST_CASES)}")
    return 0

def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the current Phase 3 list/hlist parity packet."
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
