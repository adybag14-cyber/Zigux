...
test "runGenksymsCrc escapes visible vertical tab while hiding NUL tail" {
    const visible = "pre\x0bvisible";
    const hidden = "hidden\x0btail";
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"pre\\u000bvisible\"") != null);
    ...
    try std.testing.expect(std.mem.indexOfScalar(u8, capture.list.items, 0x0b) == null);
