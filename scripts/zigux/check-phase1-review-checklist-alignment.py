...
REQUIRED_MARKERS = (
    ...
    "`zigux/Makefile`",
    "current repo evidence for the returned non-Phase-1 route families",
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_review_checklist_") as tmpdir:
        ...
        _write(
            root / REVIEW_CHECKLIST_PATH,
            _sample_review_checklist().replace("`scripts/zigux/check-phase1-shared-reminder-packet.py`", "", 1),
        )
        failures = collect_failures(root)
        ...
        _write(
            root / REVIEW_CHECKLIST_PATH,
            _sample_review_checklist().replace("`zigux/Makefile`", "", 1),
        )
        failures = collect_failures(root)
        expected = ["phase1_marker:missing:`zigux/Makefile`"]
        if failures != expected:
            raise AssertionError(f"unexpected Makefile marker failure: {failures}")
        case_count += 1
