...
REQUIRED_MARKERS = (
    "Phase 14 notes - `Documentation/zigux/phase14-end-to-end-smoke-survey.md` - `Documentation/zigux/phase14-core-boundary-traceability.md` - `Documentation/zigux/phase14-release-boundary-survey.md` - `Documentation/zigux/phase14-productization-gap-survey.md` - `Documentation/zigux/phase14-shared-smoke-current-master-gap.md` - `Documentation/zigux/phase14-attached-toolchain-guidance-gap.md` - `Documentation/zigux/review-checklist.md` - `scripts/zigux/README.md` - `zigux/tests/README.md` - `zigux/tests/phase14_end_to_end_smoke_manifest.json` keep the bounded Phase 14 docs-root packet explicit through the recovered shared-smoke documentation layer, the directly readable route, tests-root, rollback-threshold, validator, and release-boundary guards, the machine-readable shared-smoke manifest, and the returned `phase14-validate` split without promoting the missing `phase14-smoke`, `phase14-test`, or `phase14` wrappers into current proof.",
    "* the current docs-root Phase 14 reminder packet should stay parked on `Documentation/zigux/phase14-end-to-end-smoke-survey.md`, `Documentation/zigux/phase14-core-boundary-traceability.md`, `Documentation/zigux/phase14-release-boundary-survey.md`, `Documentation/zigux/phase14-productization-gap-survey.md`, `Documentation/zigux/phase14-shared-smoke-current-master-gap.md`, `Documentation/zigux/phase14-attached-toolchain-guidance-gap.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/README.md`, `zigux/tests/README.md`, and `zigux/tests/phase14_end_to_end_smoke_manifest.json` so the docs root matches the same recovered study-only packet already carried by the shared smoke note, the release-boundary note, the scripts-root reminder, the tests-root reminder, and the manifest-backed machine-readable split.",
    "* `zigux/Makefile`, `scripts/zigux/check-phase14-shared-smoke-route.py`, `scripts/zigux/check-phase14-tests-readme-smoke-summary.py`, `scripts/zigux/check-phase14-rollback-threshold-sequencing.py`, `scripts/zigux/validate-phase14.py`, and `scripts/zigux/check-phase14-release-boundary-exact-counts.py` keep the current returned `phase14-validate` route, the aligned tests-root reminder, the rollback-threshold contract, the shared validator surface, and the release-facing exact-count posture explicit from the docs root while the broader `phase14-smoke`, `phase14-test`, and `phase14` wrappers remain absent on current `master`.",
...
SECTION_ORDER = ("Phase 12 notes", "Phase 14 notes", "Phase 15 notes")
...
def check_section_order(root: Path) -> str | None:
    ...
    if positions != sorted(positions):
        return "section_order:Phase12->Phase14->Phase15"
    return None
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_lane01_phase14_") as tmp_dir:
        ...
        order_error = check_section_order(root)
        if order_error != "section_order:Phase12->Phase14->Phase15":
...
def main() -> int:
    ...
    if missing or order_error is not None:
        for item in missing:
            print(f"ERROR: {item}")
        if order_error is not None:
    ...
    print("LANE01_DOCS_ROOT_PHASE14_NOTES_SECTION_ORDER=Phase12->Phase14->Phase15")
    return 0
