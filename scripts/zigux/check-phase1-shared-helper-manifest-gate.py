...
EXPECTED_ARGV_SPLIT_REVIEW = {
    "phase1_helper_replay_anchor": 'test "phase1 host-tools smoke exercises live helper behavior"',
    "shared_replay_summary": (
        "the committed Phase 1 fixture and current host-tools smoke route still carry the baseline "
        "argv_split token-shape replay, while current master keeps repeated-whitespace collapse and "
        "blank-input behavior explicit at the helper surface for this reusable leaf helper"
    ),
    "whitespace_anchor": 'test "argvSplit collapses repeated whitespace and blank inputs to zero arguments"',
    "next_safe_step_note": (
        "If this helper lane reopens, keep argv_split parked unless a fresh reread finds drift in "
        "the committed argv_split replay fields, the shared host-tools smoke token-shape witness, "
        "or the helper-local whitespace-collapse and blank-input anchors; do not widen into "
        "neighboring shared-replay parked helpers by default."
    ),
}
...
EXPECTED_LIST_SORT_REVIEW = {
    ...
    "shared_replay_summary": (
        "the committed Phase 1 fixture still owns the tri-state and boolean-style comparator parity "
        "keys for list_sort, while current master keeps comparator-context handling, repeat-sort "
        "circular-list integrity, reverse-link alignment, sorted-input idempotence, parity-bucket "
        "stability, longer modulo-bucket stability, all-ties stability, and empty-or-singleton "
        "handling explicit at the helper surface until the broader shared replay packet returns"
    ),
    ...
    "review_packet_summary": (
        "keep list_sort parked in the shared-replay helper family for fixture ownership, but reread "
        "the helper-local proof packet before reopening the lane: current master already names direct "
        "witnesses for comparator-context ordering, repeat-sort circular integrity, reverse-link "
        "alignment, sorted-input idempotence, parity-bucket stability, longer modulo-bucket "
        "stability, all-ties stability, and empty-or-singleton handling beside the committed parity keys"
    ),
    "next_safe_step_note": (
        "If this helper lane reopens, keep list_sort parked unless a fresh reread finds drift in "
        "the committed `tri_sorted_*` or `bool_sorted_*` fixture keys, or in the current helper-local "
        "anchors for comparator-context ordering, repeat-sort circular integrity, reverse-link "
        "alignment, sorted-input idempotence, parity-bucket stability, longer modulo-bucket "
        "stability, all-ties stability, or empty-or-singleton handling; do not widen into the "
        "missing shared replay stack by default."
    ),
}
...
def _collect_review_issues(
    ...
    issues: list[str] = []
    review_entry = review_anchors.get(helper_path)
    if not isinstance(review_entry, dict):
        return [f"manifest:{Path(helper_path).stem}_review_anchor"]
    for field, expected_value in expected_review.items():
        if review_entry.get(field) != expected_value:
            issues.append(f"manifest:{Path(helper_path).stem}_review={field}")
    return issues
...
def good_manifest() -> dict[str, object]:
    return {
        ...
        "review_anchors": {
            "tools/lib/argv_split.zig": copy.deepcopy(EXPECTED_ARGV_SPLIT_REVIEW),
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_shared_helper_manifest_gate_") as tmp_dir:
        ...
        assert_issue_case(
            ...
            lambda: (
                lambda manifest: (
                    manifest["review_anchors"]["tools/lib/argv_split.zig"].pop("shared_replay_summary"),
                    write_manifest(root, manifest),
                )
            )(load_current()),
            "manifest:argv_split_review=shared_replay_summary",
        )
        case_count += 1
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-sample-root",
        help="Write a current-like sample repo root for focused checker validation.",
    )
    ...
    print("PHASE1_SHARED_HELPER_MANIFEST_GATE_REVIEW_PACKETS=argv_split,list_sort")
    return 0
