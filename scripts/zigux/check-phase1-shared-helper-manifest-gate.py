...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_shared_helper_manifest_gate_") as tmp_dir:
        ...
        assert_issue_case(
            ...
            lambda: (
                lambda manifest: (
                    manifest["review_anchors"]["tools/lib/list_sort.zig"].pop("helper_test_anchors"),
                    write_manifest(root, manifest),
                )
            )(load_current()),
            "manifest:list_sort_review=helper_test_anchors",
        )
        case_count += 1
...
def main() -> int:
    ...
    print(f"PHASE1_SHARED_HELPER_MANIFEST_GATE_DIRECT_HELPER_COUNT={len(EXPECTED_DIRECT_HELPERS)}")
