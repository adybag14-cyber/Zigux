...
def write_sample_root(root: Path) -> None:
    write_text(
        ...
        "\n".join(("# sample", *DOCS_README_MARKERS)) + "\n",
    )
    write_text(
        root / "Documentation" / "zigux" / "phase2-toolchain-bootstrap-notes.md",
        "\n".join(("# sample", *PHASE2_NOTES_MARKERS)) + "\n",
    )
    write_text(
        root / "Documentation" / "zigux" / "review-checklist.md",
        "\n".join(("# sample", *REVIEW_CHECKLIST_MARKERS)) + "\n",
    )
    write_text(
        root / "zigux" / "tests" / "README.md",
        "\n".join(("# sample", *TESTS_README_MARKERS)) + "\n",
    )
    write_text(
        root / "third_party" / "README.md",
        "\n".join(("# sample", *THIRD_PARTY_README_MARKERS)) + "\n",
    )
