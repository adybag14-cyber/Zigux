#!/usr/bin/env python3
"""Fail-close the current Phase 3 xarray-slot parity packet."""
...
REQUIRED_MARKERS = {
    SLICE_PATH: (
        ...
        "one tiny fixture-backed parity dump",
    ),
    VALIDATOR_NOTE_PATH: (
        "zigux/tests/phase3_xarray_slot_dump.zig",
        "zigux/tests/phase3_xarray_slot_dump_build.zig",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the current Phase 3 xarray-slot parity packet."
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
