...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    for snippet in VALIDATE_REQUIRED_SNIPPETS:
        if snippet not in validate_phase2_text:
            issues.append(("VALIDATE_PHASE2_KCONFIG_ROUTE_DRIFT", snippet))
...
def run_self_test() -> int:
    case_count = 0
    with tempfile.TemporaryDirectory(prefix="phase2_kconfig_route_contract_") as tmp_dir:
        root = Path(tmp_dir)

        build_good_root(root)
        if collect_issues(root):
            raise SystemExit("phase2-kconfig-route-contract:self-test:good_root")
        case_count += 1
        ...
        if ("VALIDATE_PHASE2_KCONFIG_ROUTE_DRIFT", VALIDATE_REQUIRED_SNIPPETS[0]) not in issues:
            raise SystemExit("phase2-kconfig-route-contract:self-test:missing_validate_snippet")
        case_count += 1
        ...
        if ("MISSING_MAKEFILE_LINE", REQUIRED_MAKEFILE_LINES[1]) not in issues:
            raise SystemExit("phase2-kconfig-route-contract:self-test:missing_makefile_line")
        case_count += 1
        ...
        if ("DUPLICATE_MAKEFILE_LINE", f"{REQUIRED_MAKEFILE_LINES[1]}:count=2") not in issues:
            raise SystemExit("phase2-kconfig-route-contract:self-test:duplicate_makefile_line")
        case_count += 1
        ...
        if ("MISSING_WORKFLOW_LINE", REQUIRED_WORKFLOW_LINES[0]) not in issues:
            raise SystemExit("phase2-kconfig-route-contract:self-test:missing_workflow_line")
        case_count += 1
        ...
        issues = collect_issues(root)
        if ("DUPLICATE_WORKFLOW_LINE", f"{REQUIRED_WORKFLOW_LINES[0]}:count=2") not in issues:
            raise SystemExit("phase2-kconfig-route-contract:self-test:duplicate_workflow_line")
        case_count += 1
        ...
        if ("MISSING_REQUIRED_PATH", REQUIRED_PATHS[0].as_posix()) not in issues:
            raise SystemExit("phase2-kconfig-route-contract:self-test:missing_required_path")
        case_count += 1

    print("PHASE2_KCONFIG_ROUTE_CONTRACT_SELF_TEST=pass")
    print(f"PHASE2_KCONFIG_ROUTE_CONTRACT_SELF_TEST_CASE_COUNT={case_count}")
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check that the live Phase 2 kconfig packet stays aligned across "
    ...
    print(f"PHASE2_KCONFIG_ROUTE_CONTRACT_REQUIRED_PATH_COUNT={len(REQUIRED_PATHS)}")
    print(f"PHASE2_KCONFIG_ROUTE_CONTRACT_VALIDATE_SNIPPET_COUNT={len(VALIDATE_REQUIRED_SNIPPETS)}")
    print(f"PHASE2_KCONFIG_ROUTE_CONTRACT_MAKEFILE_LINE_COUNT={len(REQUIRED_MAKEFILE_LINES)}")
    print(f"PHASE2_KCONFIG_ROUTE_CONTRACT_WORKFLOW_LINE_COUNT={len(REQUIRED_WORKFLOW_LINES)}")
    return 0
