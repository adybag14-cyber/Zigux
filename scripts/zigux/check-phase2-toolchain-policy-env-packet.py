...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_toolchain_policy_env_packet_") as tmp_dir:
        ...
        write_text(
            root,
            WORKFLOW,
            replace_exact_line(read_text(root, WORKFLOW), ORDERED_MARKERS[4], "raise SystemExit('wrong target count')"),
        )
        ...
        write_text(
            root,
            WORKFLOW,
            replace_exact_line(read_text(root, WORKFLOW), ORDERED_MARKERS[12], 'print(f"ZIGUX_ZIG_URL={url}")'),
        )
        ...
        write_text(
            root,
            WORKFLOW,
            replace_exact_line(read_text(root, WORKFLOW), ORDERED_MARKERS[16], 'repo_archive_path="third_party/archive.tar.xz"'),
        )
