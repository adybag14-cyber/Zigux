...
required_closure_markers = [
    ...
    'PHASE2_GENKSYMS_BRIDGE_EVIDENCE=artifact fixtures plus abbreviated-long, inline-short, lone-dash, explicit-terminator, empty-long-name, abbreviated-dump-types, and reference-limit coverage are required for closure',
    ...
    'confdata bridge keeps escaped control letters literal in quoted strings',
    'confdata bridge preserves literal low control bytes distinct from escaped letters',
    ...
    'confdata bridge ignores non-CONFIG lines',
    ...
    'PHASE2_KCONFIG_BRIDGE_EVIDENCE=artifact fixtures plus conf bridge mode coverage, allconfig env, mode-arg, manifest-determinism, and confdata escaped-control decode, escaped-control literal handling, low-control JSON emission, CRLF, explicit-n tristate, empty-string, malformed-quoted-string, ignore-non-CONFIG, numeric-kind, duplicate-assignment, and plus-signed numeric anchors are required for closure',
    'empty_string_expected.json',
    'escaped_strings_expected.json',
    'explicit_n_tristate_expected.json',
    'ignore_non_config_lines_expected.json',
    'malformed_quoted_string_expected.json',
    'numeric_kinds_expected.json',
    'sample_expected.json',
    'sample_crlf_expected.json',
    'signed_numeric_kinds_expected.json',
