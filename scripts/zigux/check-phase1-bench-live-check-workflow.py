...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        miswired_check = root / "miswired-check"
        write_sample_root(miswired_check)
        workflow = (miswired_check / WORKFLOW_REL).read_text(encoding="utf-8")
        workflow = workflow.replace(
            f"        run: {BENCH_LIVE_CHECK_RUN}\n\n",
            f"        run: {BENCH_SELF_TEST_RUN}\n\n",
            1,
        )
        (miswired_check / WORKFLOW_REL).write_text(workflow, encoding="utf-8")
        expect_failure("miswired live bench check", miswired_check, BENCH_LIVE_CHECK_RUN)
    ...
    print("PHASE1_BENCH_LIVE_CHECK_WORKFLOW_SELF_TEST_CASE_COUNT=6")
