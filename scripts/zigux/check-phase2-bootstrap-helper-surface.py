...
def write_sample_root(root: Path) -> None:
    write_json(root / MANIFEST, build_sample_manifest())
    write_text(root / INSTALL_ZIG, "\n".join(INSTALL_ZIG_MARKERS) + "\n")
    write_text(root / STAGE_HELPER, "\n".join(STAGE_HELPER_MARKERS) + "\n")
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_bootstrap_helper_surface_") as temp_dir:
        root = Path(temp_dir)
