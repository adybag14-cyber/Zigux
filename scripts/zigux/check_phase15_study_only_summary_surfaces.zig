// Ported from check-phase15-study-only-summary-surfaces.py by port_phase1_guards.py
const std = @import("std");
const Io = std.Io;
const guard = @import("zigux_guard.zig");

pub const pass_marker = "PHASE15_STUDY_ONLY_SUMMARY_SURFACES_SELF_TEST=pass";
...
fn collectFailures(
...

    {
        ...
        for (FREEZE_MAP_REQUIRED_MARKERS) |marker| {
...

    {
        ...
        defer allocator.free(text);
        for (STUDY_ONLY_ACCOUNTING_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...

    {
        ...
        defer allocator.free(text);
        for (DOCS_README_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...

    {
        ...
        defer allocator.free(text);
        for (REVIEW_CHECKLIST_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...
pub fn runSelfTest(io: Io, allocator: std.mem.Allocator) !u8 {
    ...
    try guard.printLine(io, "PHASE15_STUDY_ONLY_SUMMARY_SURFACES_SELF_TEST_CASE_COUNT={d}", .{@as(usize, 1)});
    return 0;
}
...
pub fn main(init: std.process.Init) !void {
    ...
    if (failures.items.len > 0) {
        try guard.printLine(io, "PHASE15_STUDY_ONLY_SUMMARY_SURFACES_SELF_TEST=fail", .{});
        for (failures.items) |failure| try guard.printLine(io, "{s}", .{failure});
        std.process.exit(1);
    }

    try guard.printLine(io, "{s}", .{pass_marker});
    try guard.printLine(io, "PHASE15_FREEZE_MAP_MARKER_COUNT={d}", .{@as(usize, FREEZE_MAP_REQUIRED_MARKERS.len)});
    try guard.printLine(io, "PHASE15_DOCS_README_MARKER_COUNT={d}", .{@as(usize, DOCS_README_REQUIRED_MARKERS.len)});
    std.process.exit(0);
}
