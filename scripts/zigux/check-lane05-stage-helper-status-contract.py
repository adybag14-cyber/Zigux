...
def require_last_order(text: str, earlier: str, later: str, label: str) -> None:
    earlier_index = text.rfind(earlier)
...
def validate_helper(root: Path) -> tuple[int, int]:
    ...
    for earlier, later in zip(ORDERED_MARKERS, ORDERED_MARKERS[1:]):
        require_last_order(helper_text, earlier, later, "success output order")
