...
def _run_python_script(root: Path, relative_path: str, *args: str) -> tuple[int, list[str]]:
    result = subprocess.run([sys.executable, str(root / relative_path), *args], cwd=root, capture_output=True, text=True, check=False)
...
def run_artifact_diff_contract_self_test_check(root: Path) -> list[str]:
    returncode, lines = _run_python_script(root, "scripts/zigux/check-artifact-diff-contract.py", "--self-test")
    if returncode != 0:
...
def run_phase4_gate_evidence_self_test_check(root: Path) -> list[str]:
    returncode, lines = _run_python_script(root, "scripts/zigux/check-phase4-gate-evidence.py", "--self-test")
    if returncode != 0:
...
def run_phase4_runtime_atomic64_packet_check(root: Path) -> list[str]:
    ...
    for field, relative_path in PHASE4_RUNTIME_ATOMIC64_PIN_TARGETS.items():
        ...
        if actual is None:
            ...
            continue
        if actual != expected:
            missing.append(f"phase4_runtime_atomic64_packet:unexpected_manifest_sha:{field}:{actual}:{expected}")
        count = survey_text.count(expected)
        if count != 1:
            missing.append(f"phase4_runtime_atomic64_packet:survey_sha_exact_count:{field}:{expected}:{count}")
    return missing
...
def _write_contract_checker_fixture(path: Path, *, base_case_count: str | None, base_cases: list[str] | None, repeat_case_count: str | None, repeat_cases: list[str] | None, case_count: str | None, cases: list[str] | None, include_pass: bool = True, self_test_case_count: str | None, self_test_cases: list[str] | None, include_self_test_pass: bool = True) -> None:
    ...
    if self_test_cases is not None:
        lines.append("    print('ARTIFACT_DIFF_CONTRACT_SELF_TEST_CASES=" + ",".join(self_test_cases) + "')")
...
def _write_phase4_gate_evidence_checker_fixture(path: Path, *, include_check_pass: bool = True, target_count: str | None = str(EXPECTED_PHASE4_GATE_EVIDENCE_TARGET_COUNT), include_self_test_pass: bool = True, self_test_case_count: str | None = None, self_test_cases: list[str] | None = None) -> None:
    lines = ["#!/usr/bin/env python3", "import sys", "if '--self-test' in sys.argv:"]
    if include_self_test_pass:
    ...
    if self_test_cases is not None:
        lines.append("    print('PHASE4_GATE_EVIDENCE_SELF_TEST_CASES=" + ",".join(self_test_cases) + "')")
    lines.append("else:")
...
def _write_phase4_runtime_atomic64_packet_fixture(root: Path) -> None:
    ...
    manifest = {"phase4_build_blob_sha": phase4_build_sha, "phase4_validator_blob_sha": validator_sha, "phase4_validation_matrix_blob_sha": matrix_sha, "phase9_build_blob_sha": phase9_build_sha}
    _write(root / "zigux/tests/phase4_runtime_atomic64_diff_manifest.json", json.dumps(manifest, indent=2) + "\n")
    _write(root / "zigux/tests/phase4_runtime_atomic64_diff_survey.zig", "\n".join(['const std = @import("std");', "", 'test "fixture keeps current phase4 build, validator, matrix, and phase9 build pins" {', f"    // phase4 build pin {phase4_build_sha}", f"    // validator pin {validator_sha}", f"    // matrix pin {matrix_sha}", f"    // phase9 build pin {phase9_build_sha}", "}", ""]))

def _write_phase4_fixture_docs(root: Path) -> None:
    _write(root / "Documentation/zigux/artifact-diff.md", "\n".join(["# Artifact Diff Policy", "", "Current Phase 4 use", "- `scripts/zigux/artifact_diff.py`", "- `scripts/zigux/check-phase4-artifact-diff-determinism.py`", "- `scripts/zigux/check-phase4-gate-evidence.py` and `Documentation/zigux/phase4-gate-evidence.md` keep the dedicated exact-readback companion packet explicit beside the validator-backed rollback surfaces.", "- `zigux/tests/atomic64_diff.zig`", "- `zigux/tests/runtime_atomic64_diff.zig`", "- `zigux/tests/phase4_runtime_atomic64_diff_survey.zig`", "- `zigux/tests/bitmap_diff.zig`", "- `zigux/tests/phase4_bitmap_live_helper_replay.zig`", "- `zigux/tests/phase4_build.zig`", "- `scripts/zigux/validate-phase4.py`", "- `Documentation/zigux/phase4-validation-matrix.md`", ""]))
    _write(root / "Documentation/zigux/phase4-gate-evidence.md", "\n".join(["# Phase 4 Gate Evidence", "", "## Status", "", "- `PHASE4_EVIDENCE_DATE=2026-05-02`", "- `PHASE4_EVIDENCE_MODE=github_connector_readback`", "- `PHASE4_EVIDENCE_SCOPE=rollback_ownership_and_lab_matrix_current_gate_definitions`", "- `PHASE4_VALIDATOR_BLOB_SHA=placeholder`", "- `PHASE4_GATE_EVIDENCE_SELF_TEST_CASE_COUNT=14`", "- `PHASE4_GATE_EVIDENCE_SELF_TEST_CASES=" + ",".join(EXPECTED_PHASE4_GATE_EVIDENCE_SELF_TEST_CASES) + "`", "- `PHASE4_SEPARATE_GATE_EVIDENCE_CHECKER_PRESENT=true`", "- `PHASE4_SHARED_VALIDATOR_RERUNS_GATE_EVIDENCE_CHECK=true`", "- `PHASE4_SHARED_VALIDATOR_RERUNS_GATE_EVIDENCE_SELF_TEST=true`", "- `PHASE4_SHARED_VALIDATOR_EXPECTED_GATE_EVIDENCE_TARGET_COUNT=16`", "- `PHASE4_SHARED_VALIDATOR_EXPECTED_GATE_EVIDENCE_SELF_TEST_CASE_COUNT=14`", "- `PHASE4_SHARED_TEST_FSMOUNT_SURVEY_PACKET_PRESENT=false`", "- `PHASE4_SHARED_PERF_BASELINE_SURVEY_PACKET_PRESENT=false`", "", "## Exact Readback Evidence", "- `zigux/tests/phase4_runtime_atomic64_diff_manifest.json` stays in the packet.", "- `zigux/tests/phase4_runtime_atomic64_diff_survey.zig` stays in the packet.", "", "## Current Conclusion", "- hard perf thresholds for the shipped atomic64 and bitmap rollback gates remain intentionally unapproved.", ""]))
    _write(root / "Documentation/zigux/phase4-validation-matrix.md", "\n".join(["# Phase 4 Validation Matrix", "", "scripts/zigux/check-phase4-artifact-diff-determinism.py", "scripts/zigux/check-phase4-workflow-route-counts.py", "atomic64_diff.zig", "runtime_atomic64_diff.zig", "phase4_runtime_atomic64_diff_manifest.json", "phase4_runtime_atomic64_diff_survey.zig", "bitmap_diff.zig", "phase4_bitmap_live_helper_replay.zig", "rollback owner", "Lab And CI Matrix", "threshold posture", "zig build test --build-file zigux/tests/phase4_build.zig", "Remaining Roadmap Gaps", "samples/zigux/kprobe_example.zig", "samples/kprobes/kprobe_example.c", "samples/zigux/test_fsmount.zig", "samples/vfs/test-fsmount.c", "hard perf thresholds and acceptable limits for the atomic64 and bitmap gates remain intentionally unapproved", ""]))
    _write(root / "Documentation/zigux/README.md", "\n".join(["Phase 4 notes", "validate-phase4.py", "check-phase4-artifact-diff-determinism.py", "check-phase4-workflow-route-counts.py", "phase4-gate-evidence.md", "phase4-validation-matrix.md", "atomic64_diff.zig", "runtime_atomic64_diff.zig", "phase4_runtime_atomic64_diff_survey.zig", "intentionally unapproved perf-threshold posture", ""]))
    _write(root / "Documentation/zigux/review-checklist.md", "\n".join(["if the change touches the shared Phase 4 validation packet", "Documentation/zigux/phase4-validation-matrix.md", "Documentation/zigux/phase4-gate-evidence.md", "scripts/zigux/check-phase4-gate-evidence.py", "scripts/zigux/check-phase4-artifact-diff-determinism.py", "scripts/zigux/check-phase4-workflow-route-counts.py", "scripts/zigux/validate-phase4.py", "zigux/tests/phase4_runtime_atomic64_diff_manifest.json", "zigux/tests/phase4_runtime_atomic64_diff_survey.zig", "zigux/tests/phase4_bitmap_live_helper_replay.zig", "zig build test --build-file zigux/tests/phase4_build.zig", "intentionally unapproved perf-threshold posture", ""]))
    _write(root / "scripts/zigux/README.md", "\n".join(["validate-phase4.py", "check-phase4-artifact-diff-determinism.py", "check-phase4-workflow-route-counts.py", "atomic64_diff.zig", "runtime_atomic64_diff.zig", "phase4_runtime_atomic64_diff_survey.zig", "Phase 4 flow", "phase4_build.zig", "phase4-gate-evidence.md", "phase4-validation-matrix.md", "make -C zigux phase4-bitmap-live-helper-replay", "intentionally unapproved perf-threshold posture", ""]))
    _write(root / "zigux/tests/README.md", "\n".join(["zigux/tests/atomic64_diff.zig", "zigux/tests/runtime_atomic64_diff.zig", "zigux/tests/phase4_runtime_atomic64_diff_survey.zig", "zigux/tests/bitmap_diff.zig", "zigux/tests/phase4_build.zig", "scripts/zigux/validate-phase4.py", "scripts/zigux/check-phase4-artifact-diff-determinism.py", "scripts/zigux/check-phase4-workflow-route-counts.py", ""]))

def _write_phase4_fixture_sources(root: Path) -> None:
    _write(root / "scripts/zigux/artifact_diff.py", "# placeholder\n")
    _write(root / "scripts/zigux/check-phase4-artifact-diff-determinism.py", "# placeholder\n")
    _write(root / "scripts/zigux/check-phase4-workflow-route-counts.py", "# placeholder\n")
    _write(root / "scripts/zigux/validate-phase4.py", "# placeholder\n")
    _write(root / "zigux/Makefile", "PHONY += phase4-validate phase4-test\nphase4-validate:\n\tscripts/zigux/validate-phase4.py\nphase4-test:\n\tzigux/tests/phase4_build.zig\n")
    _write(root / ".github/workflows/zigux-bootstrap.yml", "python3 scripts/zigux/validate-phase4.py\npython3 scripts/zigux/validate-phase4.py --self-test\nzig build test --build-file zigux/tests/phase4_build.zig\n")
    ...
    _write(root / "zigux/tests/phase4_build.zig", "\n".join(["scripts/zigux/check-phase4-artifact-diff-determinism.py", "scripts/zigux/check-phase4-workflow-route-counts.py", "atomic64_diff.zig", "phase4_runtime_atomic64_diff_survey.zig", "bitmap_diff.zig", "phase4_bitmap_live_helper_replay.zig", "phase4-runtime-atomic64-diff-tests", "phase4-runtime-atomic64-diff-survey-tests", "phase4-bitmap-diff-tests", "phase4-bitmap-live-helper-replay-tests", ""]))
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase4_validator_selftest_") as tmp_dir_str:
        ...
        _write_contract_checker_fixture(root / "scripts/zigux/check-artifact-diff-contract.py", base_case_count=str(len(EXPECTED_ARTIFACT_DIFF_CONTRACT_BASE_CASES)), base_cases=EXPECTED_ARTIFACT_DIFF_CONTRACT_BASE_CASES, repeat_case_count=str(len(EXPECTED_ARTIFACT_DIFF_CONTRACT_REPEAT_CASES)), repeat_cases=EXPECTED_ARTIFACT_DIFF_CONTRACT_REPEAT_CASES, case_count=str(len(EXPECTED_ARTIFACT_DIFF_CONTRACT_CASES)), cases=EXPECTED_ARTIFACT_DIFF_CONTRACT_CASES, self_test_case_count=str(len(EXPECTED_ARTIFACT_DIFF_CONTRACT_SELF_TEST_CASES)), self_test_cases=EXPECTED_ARTIFACT_DIFF_CONTRACT_SELF_TEST_CASES)
        _write_phase4_gate_evidence_checker_fixture(root / "scripts/zigux/check-phase4-gate-evidence.py", self_test_case_count=str(len(EXPECTED_PHASE4_GATE_EVIDENCE_SELF_TEST_CASES)), self_test_cases=EXPECTED_PHASE4_GATE_EVIDENCE_SELF_TEST_CASES)
...
def main() -> int:
    ...
    parser.add_argument("--self-test", action="store_true", help="Run isolated Phase 4 validator coverage in a temporary workspace.")
    ...
    if missing_markers:
        ...
        return 1
    for label, failures in [("ARTIFACT_DIFF_CONTRACT_CHECK", run_artifact_diff_contract_check(ROOT)), ("ARTIFACT_DIFF_CONTRACT_SELF_TEST_CHECK", run_artifact_diff_contract_self_test_check(ROOT)), ("PHASE4_GATE_EVIDENCE_CHECK", run_phase4_gate_evidence_check(ROOT)), ("PHASE4_GATE_EVIDENCE_SELF_TEST_CHECK", run_phase4_gate_evidence_self_test_check(ROOT)), ("PHASE4_RUNTIME_ATOMIC64_PACKET_CHECK", run_phase4_runtime_atomic64_packet_check(ROOT))]:
        if failures:
            print("PHASE4_VALIDATION=fail")
