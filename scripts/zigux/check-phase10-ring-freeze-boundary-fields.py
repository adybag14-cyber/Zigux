...
def run_self_test() -> int:
    ...
    if baseline_problems:
        raise SystemExit(
            "phase10-ring-freeze-boundary-fields:self-test-baseline-failed:"
    ...
    for field_name, drift_value in DRIFT_CASES.items():
        ...
        if not any(problem.startswith(expected_problem) for problem in problems):
            raise SystemExit(
                "phase10-ring-freeze-boundary-fields:drift-not-detected:"
                f"{field_name}"
            )

    with tempfile.TemporaryDirectory(prefix="zigux_phase10_ring_freeze_boundary_") as tmp_dir:
        root = Path(tmp_dir)
        write_manifest(root, fixture_manifest())
        problems = validate_manifest(read_manifest(root))
        if problems:
            raise SystemExit(
                "phase10-ring-freeze-boundary-fields:self-test-file-roundtrip-failed:"
                + ",".join(problems)
            )

    print("PHASE10_RING_FREEZE_BOUNDARY_FIELDS_SELF_TEST=pass")
    print(f"PHASE10_RING_FREEZE_BOUNDARY_FIELDS_SELF_TEST_CASE_COUNT={len(DRIFT_CASES) + 2}")
    return 0


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Validate Phase 10 virtio-ring freeze-boundary manifest fields."
    )
    ...
    if problems:
        print("PHASE10_RING_FREEZE_BOUNDARY_FIELDS=fail")
        for problem in problems:
            print(f"problem:{problem}")
        return 1

    print("PHASE10_RING_FREEZE_BOUNDARY_FIELDS=pass")
    print(f"PHASE10_RING_FREEZE_BOUNDARY_FIELD_COUNT={len(EXPECTED_FREEZE_FIELDS)}")
    return 0
