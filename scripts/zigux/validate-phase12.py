...
REQUIRED_MARKERS = {
    ...
    RAW_GITHUB_COVERAGE_PATH: [
        "- exact coverage evidence checked on `2026-05-24`: the current GitHub contents bridge directly reads `scripts/zigux/check-build-only-phase12-surface.py` `1793b998777d7d402b79108690ecd0ba070a5492`, `scripts/zigux/check-phase12-complex-driver-lane-packet.py` `24946f6a72b3b67faa6be4d54ed09d59518fa210`, `scripts/zigux/check-phase12-libbpf-snapshot.py` `92759632d6db2a6419de41d561aa8c5ffba6dd05`, `scripts/zigux/check-phase12-libbpf-heavy-consumer-packet.py` `7f2c4668a5b3c587e5487bd0d04c59c2cd150e13`, `scripts/zigux/validate-phase12.py` `53dca74f65eda0574d9b4803fbc273fd1c48a27e`, `scripts/zigux/check-phase12-release-readiness-packet.py` `6e7a04e31e8ff25a8a5ad7abd20a085c9e87bd02`, `.github/workflows/zigux-bootstrap.yml` `6b64dc0a417737cf69344f51d6b3dd4cbe40cd22`, `scripts/zigux/README.md` `07fd8f897c346bac967dc19929f9b22ee4549b90`, `zigux/Makefile` `d4b17492ab78ed3ff882f8bec48f170fa6d8d7e8`, and `zigux/tests/phase12_build.zig` `c338d24f4d12317c6a58d25708bbc14a5006852c` on current `master`; browser-side raw GitHub readback remains the matching public-read fallback for the shipped Phase 12 support bundle while direct container-side `curl`, `wget`, and `urllib` raw-URL fetches in this runtime still fail through the proxy tunnel with HTTP `403`",
        "- exact runtime-reality evidence checked on `2026-05-24`: the directly readable `zigux/Makefile` blob `d4b17492ab78ed3ff882f8bec48f170fa6d8d7e8` still prefers the repo-local `.zig-toolchain` executable through `ZIG_PINNED_EXECUTABLE`, `ZIG_LOCAL_TOOLCHAIN`, `ZIG_PINNED_TOOLCHAIN`, and `ZIG ?= $(if $(ZIG_PINNED_TOOLCHAIN),$(ZIG_PINNED_TOOLCHAIN),zig)`, and the directly readable workflow blob `6b64dc0a417737cf69344f51d6b3dd4cbe40cd22` still rebuilds that repo-local fallback by trying the pinned `third_party` archive first, then the Zig community-mirror list, and finally `ziglang.org` before rerunning `make -C zigux phase12-validate`, `make -C zigux phase12-smoke`, `make -C zigux phase12-test`, and `make -C zigux phase12`",
        *RAW_GITHUB_BRIDGE_MARKERS,
