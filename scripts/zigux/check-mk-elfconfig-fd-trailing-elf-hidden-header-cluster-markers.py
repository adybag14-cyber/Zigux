...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-fd-trailing-elf-hidden-header-cluster-") as tmp_dir:
        ...
        missing_path = missing_root / "scripts/zigux/mk_elfconfig_fd_trailing_elf_hides_not_elf_test.zig"
        ...
        try:
            ...
            if "missing_marker:fd_trailing_elf64_hides_not_elf_no_skip" not in str(exc):
                raise
        else:
        ...
        duplicate_path = duplicate_root / "scripts/zigux/mk_elfconfig_fd_trailing_elf_hides_invalid_class_test.zig"
        ...
        try:
            ...
            if "duplicate_marker:fd_trailing_elf32_hides_invalid_class" not in str(exc):
                raise
        else:
        ...
        reordered_path = reordered_root / "scripts/zigux/mk_elfconfig_fd_trailing_elf_hides_not_elf_test.zig"
    ...
    print("MK_ELFCONFIG_FD_TRAILING_ELF_HIDDEN_HEADER_CLUSTER_MARKERS_SELF_TEST=pass")
    print("MK_ELFCONFIG_FD_TRAILING_ELF_HIDDEN_HEADER_CLUSTER_MARKERS_SELF_TEST_CASE_COUNT=4")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the mk_elfconfig trailing-ELF hidden-header fd replay pair stays present."
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    print("MK_ELFCONFIG_FD_TRAILING_ELF_HIDDEN_HEADER_CLUSTER_MARKERS=pass")
    print(f"MK_ELFCONFIG_FD_TRAILING_ELF_HIDDEN_HEADER_CLUSTER_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
