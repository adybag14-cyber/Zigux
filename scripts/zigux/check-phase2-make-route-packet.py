    ...
    with tempfile.TemporaryDirectory(prefix="phase2_make_route_packet_") as tmp_dir:
        ...
        write_text(
            ...
            replace_first(
                ...
                WORKFLOW_REQUIRED_LINES[0],
                "run: make -C zigux something-else",
            ),
        )
        issues = collect_issues(root)
        ...
        write_text(
            ...
            replace_first(
                ...
                VALIDATOR_REQUIRED_SNIPPETS[0],
                '"scripts/zigux/other-checker.py",',
            ),
        )
        issues = collect_issues(root)
        ...
        issues = collect_issues(root)
        assert (
            f"missing_bootstrap_notes_marker:{BOOTSTRAP_NOTES_REQUIRED_MARKERS[0]}"
            in issues
        )
        case_count += 1
