...
test "runFixdep preserves ACK command bytes in savedcmd prelude" {
    ...
    const depfile_name = "zigux_fixdep_ack_cmdline_test.d";
    const source_name = "zigux_fixdep_ack_cmdline_source.c";

    try Io.Dir.cwd().writeFile(std.testing.io, .{
        .sub_path = source_name,
        .data = "/* CONFIG_ZIGUX_CMDLINE_ACK */\n",
    });
    ...
    const cmdline = "cc\x06-DZIGUX_CMDLINE_ACK=1 -c " ++ source_name;
    ...
    try std.testing.expectEqualStrings(
        "savedcmd_sample.o := " ++ cmdline ++ "\n\n" ++
            ...
            "deps_sample.o := \\\n" ++
            "    $(wildcard include/config/ZIGUX_CMDLINE_ACK) \\\n" ++
            "\n" ++
