...
def collect_manifest_issues(root: Path) -> list[tuple[str, str]]:
    ...
    for category, expected_values in EXPECTED_MANIFEST_SURFACES.items():
        actual = present_surfaces.get(category)
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues.extend(
        collect_forbidden_markers(notes_text, PHASE2_NOTES_FORBIDDEN_MARKERS, "FORBIDDEN_PHASE2_NOTES_MARKERS")
    )
    issues.extend(
        collect_missing_markers(checklist_text, REVIEW_CHECKLIST_MARKERS, "MISSING_REVIEW_CHECKLIST_MARKERS")
    )
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_current_packet_") as tmp_dir:
        ...
        for marker in WORKFLOW_LINES:
            ...
            write_text(path, replace_exact_line(read_text(path), marker))
        ...
        build_sample_root(root)
        manifest = json.loads(read_text(manifest_path))
        manifest["present_surfaces"]["bridge_helpers"] = manifest["present_surfaces"]["bridge_helpers"][:-1]
        write_text(manifest_path, json.dumps(manifest, indent=2) + "\n")
        assert (
            "MANIFEST_SURFACE_VALUE_MISSING",
            "bridge_helpers:scripts/zigux/genksyms_version_before_invalid_long_option_test.zig",
        ) in collect_issues(root)
        checks_run += 1
