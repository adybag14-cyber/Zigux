...
def read_text(root: Path, rel: Path) -> str:
    ...
    try:
        ...
        raise SystemExit(
            f"phase2 tests-readme local-archive checker missing file: {path}"
        ) from exc
    except OSError as exc:
        raise SystemExit(
            f"phase2 tests-readme local-archive checker unreadable file {path}: {exc}"
        ) from exc
...
def require_paths(root: Path) -> None:
    for rel in REQUIRED_PATHS:
        ...
        if not path.is_file():
            raise SystemExit(
                f"phase2 tests-readme local-archive checker missing required path: {path}"
            )
...
def run_self_test() -> int:
    case_count = 0
    with tempfile.TemporaryDirectory(prefix="lane25_tests_readme_local_archive_") as tmp:
    ...
    print(
        "PHASE2_TESTS_README_LOCAL_ARCHIVE_PACKET_SELF_TEST_CASE_COUNT="
        f"{case_count}"
    )
    return 0
...
def main() -> int:
    ...
    print(f"PHASE2_TESTS_README_LOCAL_ARCHIVE_PACKET_ROOT={args.root.resolve()}")
    print(
        "PHASE2_TESTS_README_LOCAL_ARCHIVE_PACKET_REQUIRED_PATH_COUNT="
        f"{len(REQUIRED_PATHS)}"
    )
    print(
        "PHASE2_TESTS_README_LOCAL_ARCHIVE_PACKET_REVIEW_MARKER_COUNT="
        f"{len(REVIEW_MARKERS)}"
    )
    print(
        "PHASE2_TESTS_README_LOCAL_ARCHIVE_PACKET_TESTS_MARKER_COUNT="
        f"{len(TESTS_MARKERS)}"
    )
    print(
