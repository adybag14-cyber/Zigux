...
test "runFixdep preserves percent target names in public output" {
    const allocator = std.testing.allocator;
    const target = "zigux%target.o";
    const depfile_path = "zigux_fixdep_target_percent_public_entry.d";
    const source_path = "zigux_fixdep_target_percent_source.c";
    const header_path = "zigux_fixdep_target_percent_header.h";
    const cmdline = "zig cc -DTARGET_PERCENT=1 -c zigux_fixdep_target_percent_source.c";

    try std.Io.Dir.cwd().writeFile(std.testing.io, .{
        .sub_path = source_path,
        .data = "/* CONFIG_ZIGUX_TARGET_PERCENT_SOURCE */\n",
    });
    defer std.Io.Dir.cwd().deleteFile(std.testing.io, source_path) catch {};

    try std.Io.Dir.cwd().writeFile(std.testing.io, .{
        .sub_path = header_path,
        .data = "#define CONFIG_ZIGUX_TARGET_PERCENT_HEADER 1\n",
    });
    ...
    try std.testing.expectEqualStrings(
        "savedcmd_zigux%target.o := " ++ cmdline ++ "\n\n" ++
            ...
            "deps_zigux%target.o := \\\n" ++
            "    $(wildcard include/config/ZIGUX_TARGET_PERCENT_SOURCE) \\\n" ++
            "  " ++ header_path ++ " \\\n" ++
            "    $(wildcard include/config/ZIGUX_TARGET_PERCENT_HEADER) \\\n" ++
            "\n" ++
