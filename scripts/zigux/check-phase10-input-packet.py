...
REQUIRED_GAPS = {
    ...
    "phase10-virtio-input-registration-preflight-helper": ("starter_landed", "drivers/virtio/virtio_input_registration_preflight.zig"),
...
EXPECTED_GAP_WHY_NOW = {
    ...
    "phase10-virtio-input-registration-preflight-helper": "The dedicated registration-preflight helper keeps event-queue, status-queue, event-buffer, capability-setup, and multitouch-slot blockers explicit below input_register_device() without widening into lifecycle parity.",
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase10_input_packet_") as tmp_dir:
        ...
        module_path.write_text(
            original_module.replace("zigux/tests/phase10_virtio_input_status_drain.zig", "zigux/tests/phase10_virtio_input_status_drain_missing.zig", 1),
            encoding="utf-8",
        )
        expect_missing_marker(root, "module:zigux/tests/phase10_virtio_input_status_drain.zig", "phase10-input-self-test:module_status_drain_path")
        module_path.write_text(original_module, encoding="utf-8")
        case_count += 1

        module_path.write_text(
            original_module.replace("the bounded status-drain helper plus replay", "the bounded status-drain helper drift", 1),
            encoding="utf-8",
        )
        expect_missing_marker(root, "module:the bounded status-drain helper plus replay", "phase10-input-self-test:module_status_drain_phrase")
        module_path.write_text(original_module, encoding="utf-8")
        case_count += 1
