...
const fixture_cases = [_]FixtureCase{
    .{
        ...
        .expected_stdout_path = "zigux/tests/fixtures/fixdep/sample_comment_only_expected.txt",
    ...
    .{
        ...
        .expected_stdout_path = "zigux/tests/fixtures/fixdep/sample_missing_dep_expected.txt",
    ...
    .{
        ...
        .expected_stdout_path = "zigux/tests/fixtures/fixdep/sample_output_write_expected.txt",
...
fn runCapturedCase(allocator: std.mem.Allocator, fixture_case: FixtureCase) !void {
...
fn runDevFullCase(allocator: std.mem.Allocator, fixture_case: FixtureCase) !void {
    if (fixture_case.expected_stdout_path) |expected_stdout_path| {
        const expected_stdout = try readFixture(allocator, expected_stdout_path, 4 * 1024);
        defer allocator.free(expected_stdout);
...
test "fixdep replays the shipped failure fixture matrix" {
    const allocator = std.testing.allocator;

    inline for (fixture_cases) |fixture_case| {
        switch (fixture_case.stdout_mode) {
            .capture => try runCapturedCase(allocator, fixture_case),
            .dev_full => try runDevFullCase(allocator, fixture_case),
        }
    }
}
