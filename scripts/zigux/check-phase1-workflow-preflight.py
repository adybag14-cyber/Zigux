...
def require_absent_steps(workflow_text: str, step_names: tuple[str, ...]) -> list[str]:
    failures: list[str] = []
    for step_name in step_names:
        count = count_exact_line(workflow_text, f"      - name: {step_name}")
        if count != 0:
            failures.append(f"workflow_step:{step_name}:expected=0:actual={count}")
    return failures
...
def collect_failures(root: Path) -> list[str]:
    ...
    failures.extend(require_order(workflow_text, "workflow_preflight_order", ORDER_CHAIN))
    failures.extend(require_order(workflow_text, "workflow_phase1_tail_order", PHASE1_CHAIN))
    failures.extend(require_absent_steps(workflow_text, FORBIDDEN_WORKFLOW_STEPS))
...
def run_self_test() -> int:
    cases: list[tuple[str, object | None]] = [
        ("baseline", None),
        (
            "duplicate_forbidden_preflight_step",
...
def main() -> int:
    ...
    print(
        "PHASE1_WORKFLOW_PREFLIGHT_INSERTION_POINT=Setup Python,Setup pinned Zig toolchain"
    )
    return 0
