...
def check_workflow(text: str) -> None:
    for marker, label in (
        ...
        (DOWNLOAD_HELPER, "download helper marker"),
        (DOWNLOAD_FETCH, "download fetch marker"),
    ...
    for broken_workflow, expected in (
        ...
        (
            good_workflow.replace(
                ...
                f"            {DOWNLOAD_FAILURE_EXTRACT_CLEANUP}\n",
                '            local url="$1"\n'
                f"            {DOWNLOAD_FAILURE_FILE_CLEANUP}\n"
                f"            {DOWNLOAD_FAILURE_EXTRACT_CLEANUP}\n"
                f"            {DOWNLOAD_FETCH}\n"
