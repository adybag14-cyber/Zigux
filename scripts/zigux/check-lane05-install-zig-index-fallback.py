...
def check_root(root: Path) -> tuple[int, str]:
    ...
    return len(REQUIRED_MARKERS), "0.17.0-dev.87+9b177a7d2"
...
def run_self_test() -> int:
    ...
    def expect_pass() -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="lane05_index_fallback_pass_") as tmp_dir:
            ...
            assert check_root(root) == (len(REQUIRED_MARKERS), "0.17.0-dev.87+9b177a7d2")
            case_count += 1
    ...
    expect_failure(
        lambda root: (root / INSTALLER_PATH).write_text(
            load_text(root) + "assert load_index('0.17.0-dev.87+9b177a7d2') == {}\n",
            encoding="utf-8",
        ),
