...
test "genksyms executable keeps version stderr before rendered request stdout" {
    ...
    const exe_path = try std.fmt.bufPrint(
        &exe_path_buffer,
        ".zig-cache/tmp/{s}/genksyms-mixed-version-request",
        .{tmp.sub_path},
    );
    ...
    try testing.expectEqualStrings(version_text, run_result.stderr);
    try testing.expectEqualStrings(
        "{\"tool\":\"scripts/genksyms/genksyms\",\"stdin\":\"cpp-stream\",\"stdout\":\"symversions\",\"argv\":[\"scripts/genksyms/genksyms\",\"-V\",\"--debug\",\"--warnings\",\"--quiet\",\"--reference\",\"baseline.symref\",\"--dump-types=types.out\",\"--preserve\",\"leftover.c\"],\"options\":{\"debug_level\":1,\"warnings\":false,\"dump_defs\":false,\"preserve\":true,\"reference_files\":[\"baseline.symref\"],\"dump_types_file\":\"types.out\"}}\n",
        run_result.stdout,
    );
}
