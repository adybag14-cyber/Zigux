...
test "lane19 multi-megabyte exact-buffer split keeps a visible CR-led tail beyond the old CLI cap" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + c_line_payload_len + 11,
    );
    ...
    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(input.items[skipped_record_count * skipped_record_len .. skipped_record_count * skipped_record_len + c_line_payload_len])});
