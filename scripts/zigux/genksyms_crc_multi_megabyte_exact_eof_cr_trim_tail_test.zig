...
test "lane19 multi-megabyte exact-buffer EOF continuation trims trailing carriage returns beyond the old CLI cap" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + gen.c_line_payload_len + 3,
    );
    ...
    const untrimmed_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("b\r\r")});
