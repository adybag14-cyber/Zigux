// Ported from check-phase15-docs-readme-alignment.py by port_phase1_guards.py
const std = @import("std");
const Io = std.Io;
const guard = @import("zigux_guard.zig");

pub const pass_marker = "PHASE15_DOCS_README_ALIGNMENT_SELF_TEST=pass";
...
const LANDED_DOCS_REQUIRED_MARKERS = [_][]const u8{
    "Phase 15 notes",
...
const LANDED_HANDOFF_REQUIRED_MARKERS = [_][]const u8{
...
const LANDED_SHARED_GAP_REQUIRED_MARKERS = [_][]const u8{
...
const LANE_SEQ_NOTE_PATH = "Documentation/zigux/phase15-governance-lane-sequencing.md";
...
const PARKED_DOCS_FORBIDDEN_MARKERS = [_][]const u8{
    "Phase 15 notes",
    "`Documentation/zigux/phase15-readiness-gate-survey.md`",
    "`Documentation/zigux/phase15-handoff-next-steps-survey.md`",
    "`Documentation/zigux/phase15-governance-lane-sequencing.md`",
...
fn collectFailures(
...

    {
        ...
        for (PARKED_DOCS_REQUIRED_MARKERS) |marker| {
...

    {
        ...
        defer allocator.free(text);
        for (PARKED_DOCS_FORBIDDEN_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...

    {
        ...
        defer allocator.free(text);
        for (LANDED_DOCS_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...

    {
        ...
        defer allocator.free(text);
        for (PARKED_HANDOFF_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...

    {
        ...
        defer allocator.free(text);
        for (LANDED_HANDOFF_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...

    {
        ...
        defer allocator.free(text);
        for (PARKED_SHARED_GAP_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...

    {
        ...
        defer allocator.free(text);
        for (LANDED_SHARED_GAP_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...

    {
        ...
        defer allocator.free(text);
        for (LANE_SEQ_REQUIRED_MARKERS) |marker| {
            if (std.mem.indexOf(u8, text, marker) == null) {
                const issue = try std.fmt.allocPrint(allocator, "missing_marker:{s}", .{marker});
                try failures.append(allocator, issue);
            }
...
pub fn runSelfTest(io: Io, allocator: std.mem.Allocator) !u8 {
    ...
    try guard.printLine(io, "PHASE15_DOCS_README_ALIGNMENT_SELF_TEST_CASES={d}", .{@as(usize, 1)});
    return 0;
}
...
pub fn main(init: std.process.Init) !void {
    ...
    if (failures.items.len > 0) {
        try guard.printLine(io, "PHASE15_DOCS_README_ALIGNMENT_SELF_TEST=fail", .{});
