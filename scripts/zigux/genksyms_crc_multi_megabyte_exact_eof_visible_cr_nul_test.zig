...
test "lane19 multi-megabyte exact-buffer EOF continuation keeps a visible CR-led tail before NUL beyond the old CLI cap" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + c_line_payload_len + 5,
    );
