...
test "lane19 multi-megabyte exact-buffer split preserves visible low control bytes, truncates at NUL, trims carriage returns, and keeps the following line" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + gen.c_line_payload_len + 10,
    );
