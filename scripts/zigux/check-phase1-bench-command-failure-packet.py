...
def run_self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="lane16-bench-command-failure-") as tmp:
        ...
        checker_path.write_text(
            read_text(checker_path).replace(BENCH_COMMAND_MARKERS[0] + "\n", "", 1),
            encoding="utf-8",
        )
        kind, payload = run_check(root)
        expect(kind, "bench_command_markers", payload, [BENCH_COMMAND_MARKERS[0]])
        case_count += 1
        write_sample_root(root)

        checker_path.write_text(
            read_text(checker_path).replace(BENCH_FAILURE_MARKERS[1] + "\n", "", 1),
            encoding="utf-8",
        )
        kind, payload = run_check(root)
        expect(kind, "bench_failure_markers", payload, [BENCH_FAILURE_MARKERS[1]])
        case_count += 1
        write_sample_root(root)
