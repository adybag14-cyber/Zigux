...
def collect_failures(root: Path) -> list[str]:
    ...
    for rel, marker in zip(DIRECT_REMINDER_ROUTE_PATHS, DIRECT_REMINDER_ROUTE_MARKERS):
    ...
    for marker in BROADER_GAP_PATH_MARKERS:
        if f"`{marker}`" not in note:
            failures.append(f"scorecard_note:missing_gap_path_marker:`{marker}`")
...
def _seed_repo(root: Path) -> None:
    ...
    for rel in DIRECT_PACKET_PATHS + DIRECT_REMINDER_ROUTE_PATHS:
        path = root / rel
        path.parent.mkdir(parents=True, exist_ok=True)
        if not path.exists():
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase15_parity_scorecard_packet_") as tmpdir:
        ...
        if collect_failures(root):
            raise AssertionError(f"baseline fixture should pass: {collect_failures(root)}")
        case_count += 1
        ...
        expected = [
            "scorecard_note:missing_route_marker:`python3 scripts/zigux/check-phase15-tests-readme-alignment.py`"
        ]
        if failures != expected:
        ...
        expected = [
            "scorecard_note:missing_marker:surveyed against dated current-master readback marker `current-master-readback-2026-05-21`",
            "scorecard_note:stale_marker:current-master-readback-2026-05-20",
        ]
        if failures != expected:
