...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase15_parity_scorecard_packet_") as tmpdir:
        ...
        _write(
            ...
            _sample_note().replace("`python3 scripts/zigux/check-phase15-tests-readme-alignment.py`, ", "", 1),
        )
        ...
        expected = [
            "scorecard_note:missing_marker:surveyed against dated current-master readback marker `current-master-readback-2026-05-19`",
            "scorecard_note:stale_marker:current-master-readback-2026-05-18",
        ]
        if failures != expected:
