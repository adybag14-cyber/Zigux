...
const elf_fragment = [_]u8{ 0x7f, 'E', 'L', 'F', 2, 1, 1 };
const invalid_fragment = [_]u8{ 0x7f, 'E', 'L', 'F', 3, 1, 1, 0, 0 };
const not_elf_fragment = [_]u8{ 0x00, 'E', 'L', 'F', 1, 1 };
...
test "first 32-bit ELF header stays authoritative with interleaved trailing fragments" {
    const input = elf32_header ++ invalid_fragment ++ elf64_header ++ not_elf_fragment ++ invalid_class_header;
    try expectRun(&input, 0, elf32_define, "");
}

test "first 64-bit ELF header stays authoritative with interleaved trailing fragments" {
    const input = elf64_header ++ not_elf_fragment ++ invalid_class_header ++ elf_fragment ++ not_elf_header;
    try expectRun(&input, 0, elf64_define, "");
}

test "first invalid-class header stays authoritative with interleaved trailing fragments" {
    const input = invalid_class_header ++ elf_fragment ++ not_elf_header ++ invalid_fragment ++ elf32_header;
    try expectRun(&input, 1, "", "");
}

test "first non-ELF header stays authoritative with interleaved trailing fragments" {
    const input = not_elf_header ++ invalid_fragment ++ elf32_header ++ not_elf_fragment ++ elf64_header;
