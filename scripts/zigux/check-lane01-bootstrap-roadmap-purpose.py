...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_lane01_purpose_") as tmp_dir:
        ...
        if errors:
            raise AssertionError(f"baseline purpose fixture should pass: {errors}")
        case_count += 1
        ...
        _write(
            root / ROADMAP_PATH,
            _sample_roadmap().replace("## Bootstrap Status Note\n\n", "", 1),
        )
        errors = check_purpose_packet(root)
        if not errors or errors[0] != "purpose packet mismatch":
            raise AssertionError(f"expected missing status-note mismatch, got: {errors}")
        _write(root / ROADMAP_PATH, _sample_roadmap())
        case_count += 1
        ...
        errors = check_purpose_packet(root)
        if not errors or errors[0] != "purpose packet mismatch":
            raise AssertionError(f"expected missing baseline-line mismatch, got: {errors}")
        _write(root / ROADMAP_PATH, _sample_roadmap())
        case_count += 1
        ...
        errors = check_purpose_packet(root)
        if not errors or errors[0] != "purpose packet mismatch":
            raise AssertionError(f"expected ordering mismatch, got: {errors}")
        _write(root / ROADMAP_PATH, _sample_roadmap())
        case_count += 1
...
def main() -> int:
    ...
    print("LANE01_BOOTSTRAP_ROADMAP_PURPOSE_SECTION_ORDER=Purpose->BootstrapStatusNote->InputsReviewed")
    return 0
