...
test "runGenksymsCrc keeps right-bracket-prefixed visible bytes before NUL" {
    ...
    const next_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{genksyms_crc.crc32("]next")});
    defer std.testing.allocator.free(next_crc);
    const hidden_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{genksyms_crc.crc32("hidden]tail")});
    defer std.testing.allocator.free(hidden_crc);

    const expected = try std.fmt.allocPrint(
        std.testing.allocator,
        "{{\"cases\":[{{\"input\":\"]]visible\",\"crc_hex\":\"{s}\"}},{{\"input\":\"]next\",\"crc_hex\":\"{s}\"}}]}}\n",
