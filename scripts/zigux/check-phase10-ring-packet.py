...
MARKERS = {
    ...
    "Documentation/zigux/phase10-virtio-ring-survey.md": [
        ...
        "Only `zigux/tests/phase10_build.zig`, `Documentation/zigux/phase10-virtio-ring-survey.md`, `Documentation/zigux/phase10-virtio-ring-slice.md`, `Documentation/zigux/phase10-virtio-ring-freeze-boundary-survey.md`, and `zigux/tests/phase10_virtio_ring_manifest.json` remain directly re-readable inside the ring packet today.",
        "the ring freeze-boundary companion keeps the `P10-L10` queue-local wrapper packet and the adjacent `P10-L11` risky-transport owner split explicit",
...
def write_fixture(root: Path) -> None:
    fixture = {
        ...
        "Documentation/zigux/phase10-virtio-driver-lane-sequencing.md": "\n".join(MARKERS["Documentation/zigux/phase10-virtio-driver-lane-sequencing.md"]) + "\n",
...
def expect_missing_marker(root: Path, rel_path: str, old: str, new: str, expected: str) -> None:
    ...
    if expected not in missing_markers:
        ...
        raise SystemExit(f"phase10-ring-packet-self-test:expected={expected}:actual={actual}")
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase10_ring_packet_") as tmp_dir:
        ...
        if missing_files or missing_markers:
            raise SystemExit(
                "phase10-ring-packet-self-test:baseline_failed:"
        ...
        cases = [
            (
                ...
                "phase10-closure-evidence.md:`virtqueue_wrappers=repo_reality_gap`",
            ),
            (
                "Documentation/zigux/phase10-virtio-ring-survey.md",
                "Documentation/zigux/phase10-virtio-ring-freeze-boundary-survey.md",
                "Documentation/zigux/phase10-virtio-ring-gap-note.md",
                "phase10-virtio-ring-survey.md:Documentation/zigux/phase10-virtio-ring-freeze-boundary-survey.md",
            ),
            (
                "Documentation/zigux/phase10-virtio-ring-survey.md",
                "the ring freeze-boundary companion keeps the `P10-L10` queue-local wrapper packet and the adjacent `P10-L11` risky-transport owner split explicit",
                "the ring note no longer tracks owner split",
                "phase10-virtio-ring-survey.md:the ring freeze-boundary companion keeps the `P10-L10` queue-local wrapper packet and the adjacent `P10-L11` risky-transport owner split explicit",
            ),
            (
                "Documentation/zigux/phase10-virtio-driver-lane-sequencing.md",
                "queue-local wrapper reviewability does not drift into MMIO-owned blocked transport claims",
                "queue-local wrapper reviewability is optional",
                "phase10-virtio-driver-lane-sequencing.md:queue-local wrapper reviewability does not drift into MMIO-owned blocked transport claims",
            ),
            (
                "Documentation/zigux/phase10-virtio-ring-slice.md",
                "phase10-notification-data-summary-helper",
                "phase10-notification-summary-helper",
                "phase10-virtio-ring-slice.md:phase10-notification-data-summary-helper",
            ),
            (
                "Documentation/zigux/phase10-virtio-ring-freeze-boundary-survey.md",
                "adjacent freeze-boundary owner: `P10-L11`",
                "adjacent freeze-boundary owner: `P10-L12`",
                "phase10-virtio-ring-freeze-boundary-survey.md:adjacent freeze-boundary owner: `P10-L11`",
            ),
            (
                "Documentation/zigux/phase10-phase11-phase13-tests-root-review-companion.md",
                "closure-manifest-backed ring packet vocabulary",
                "archived ring packet vocabulary",
        ...
        run_manifest_case(
            root,
            "freeze_in_c_anchors",
            EXPECTED_FREEZE_IN_C_ANCHORS[:-1],
    ...
    print("PHASE10_RING_PACKET_SELF_TEST_CASES=13")
    return 0
...
def main(argv: list[str]) -> int:
    ...
    if missing_files:
        ...
        print("MISSING_PHASE10_RING_PACKET_FILES_START")
        for item in missing_files:
            print(item)
        print("MISSING_PHASE10_RING_PACKET_FILES_END")
        return 1
    if missing_markers:
        print("PHASE10_RING_PACKET=fail")
        print("MISSING_PHASE10_RING_PACKET_MARKERS_START")
        for item in missing_markers:
            print(item)
        print("MISSING_PHASE10_RING_PACKET_MARKERS_END")
        return 1

    print("PHASE10_RING_PACKET=pass")
    print(f"PHASE10_RING_PACKET_DIRECT_FILE_COUNT={len(DIRECT_PACKET_FILES)}")
    print(
        "PHASE10_RING_PACKET_REQUIRED_MARKER_COUNT="
        f"{sum(len(markers) for markers in MARKERS.values()) + len(MANIFEST_SCALARS) + 5 + len(EXPECTED_GAPS) * 4}"
    )
    return 0
