...
const ABSENT_WORKFLOW_MARKERS = [_][]const u8{
...
const ABSENT_MAKEFILE_MARKERS = [_][]const u8{
    "phase7-test:",
    "phase7:",
...
const REQUIRED_SEQUENCING_SNIPPETS = [_][]const u8{
...
const FORBIDDEN_SEQUENCING_SNIPPETS = [_][]const u8{
...
const REQUIRED_REVIEW_SNIPPETS = [_][]const u8{
...
const REQUIRED_WORKSPACE_BOOTSTRAP_SURVEY_SNIPPETS = [_][]const u8{
...
const REQUIRED_STRING_HELPERS_SNIPPETS = [_][]const u8{
...
const REQUIRED_CMDLINE_SLICE_SNIPPETS = [_][]const u8{
...
fn checkRepo(io: Io, allocator: std.mem.Allocator, root: []const u8) !void {
    const text_direct_packet_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    ...
    const text_absent_workflow_markers_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    defer allocator.free(text_absent_workflow_markers_path);
    const text_absent_workflow_markers = try guard.readUtf8File(io, allocator, text_absent_workflow_markers_path);
    defer allocator.free(text_absent_workflow_markers);
    for (ABSENT_WORKFLOW_MARKERS) |marker| try guard.requireMarker(text_absent_workflow_markers, marker);
    const text_absent_makefile_markers_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    defer allocator.free(text_absent_makefile_markers_path);
    const text_absent_makefile_markers = try guard.readUtf8File(io, allocator, text_absent_makefile_markers_path);
    defer allocator.free(text_absent_makefile_markers);
    for (ABSENT_MAKEFILE_MARKERS) |marker| try guard.requireMarker(text_absent_makefile_markers, marker);
    const text_required_workflow_lines_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    ...
    const text_required_sequencing_snippets_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    defer allocator.free(text_required_sequencing_snippets_path);
    const text_required_sequencing_snippets = try guard.readUtf8File(io, allocator, text_required_sequencing_snippets_path);
    defer allocator.free(text_required_sequencing_snippets);
    for (REQUIRED_SEQUENCING_SNIPPETS) |marker| try guard.requireMarker(text_required_sequencing_snippets, marker);
    const text_forbidden_sequencing_snippets_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    defer allocator.free(text_forbidden_sequencing_snippets_path);
    const text_forbidden_sequencing_snippets = try guard.readUtf8File(io, allocator, text_forbidden_sequencing_snippets_path);
    defer allocator.free(text_forbidden_sequencing_snippets);
    for (FORBIDDEN_SEQUENCING_SNIPPETS) |marker| {
        if (std.mem.indexOf(u8, text_forbidden_sequencing_snippets, marker) != null) return guard.GuardError.MissingMarker;
    }
    const text_required_review_snippets_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    defer allocator.free(text_required_review_snippets_path);
    const text_required_review_snippets = try guard.readUtf8File(io, allocator, text_required_review_snippets_path);
    defer allocator.free(text_required_review_snippets);
    for (REQUIRED_REVIEW_SNIPPETS) |marker| try guard.requireMarker(text_required_review_snippets, marker);
    const text_required_workspace_bootstrap_survey_snippets_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    defer allocator.free(text_required_workspace_bootstrap_survey_snippets_path);
    const text_required_workspace_bootstrap_survey_snippets = try guard.readUtf8File(io, allocator, text_required_workspace_bootstrap_survey_snippets_path);
    defer allocator.free(text_required_workspace_bootstrap_survey_snippets);
    for (REQUIRED_WORKSPACE_BOOTSTRAP_SURVEY_SNIPPETS) |marker| try guard.requireMarker(text_required_workspace_bootstrap_survey_snippets, marker);
    const text_required_string_helpers_snippets_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    defer allocator.free(text_required_string_helpers_snippets_path);
    const text_required_string_helpers_snippets = try guard.readUtf8File(io, allocator, text_required_string_helpers_snippets_path);
    defer allocator.free(text_required_string_helpers_snippets);
    for (REQUIRED_STRING_HELPERS_SNIPPETS) |marker| try guard.requireMarker(text_required_string_helpers_snippets, marker);
    const text_required_cmdline_slice_snippets_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
    defer allocator.free(text_required_cmdline_slice_snippets_path);
    const text_required_cmdline_slice_snippets = try guard.readUtf8File(io, allocator, text_required_cmdline_slice_snippets_path);
    defer allocator.free(text_required_cmdline_slice_snippets);
    for (REQUIRED_CMDLINE_SLICE_SNIPPETS) |marker| try guard.requireMarker(text_required_cmdline_slice_snippets, marker);
    const text_required_makefile_lines_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-helper-lane-sequencing.md");
