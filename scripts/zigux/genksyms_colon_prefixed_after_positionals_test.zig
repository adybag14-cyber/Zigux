...
test "genksyms bridge treats colon-prefixed values after positionals as request input" {
...
test "genksyms bridge accepts colon-prefixed required option arguments after positionals" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "genksyms bridge keeps colon-prefixed terminator tails out of parser state" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "genksyms bridge renders colon-prefixed terminator tail in bridge json" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
