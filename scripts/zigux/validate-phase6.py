...
required_phase6_catalog_markers = [
    ...
    'every replayed lookup must stay within `std.math.log2_int_ceil(len) + 1` comparator calls, and the run still requires `avg_compare_calls <= std.math.log2_int_ceil(len) + 1`',
...
required_manifest_pairs = {
    ...
    'surveyed_commit': phase6_catalog_verified_head,
}
...
if not isinstance(helpers, list) or len(helpers) != 4:
    ...
    for helper in helpers:
        ...
        if set(helper.get('generators', [])) != expected.get('generators', set()):
            missing_markers.append(f'phase6_manifest:{helper_id}:generators')
