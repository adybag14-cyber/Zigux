...
const elf32_fragment = elf32_header[0..8].*;
const elf64_fragment = elf64_header[0..12].*;
...
test "first 32-bit ELF header stays authoritative with trailing fragments-failure-header cluster" {
    const input = elf32_header ++ elf64_fragment ++ invalid_class_header ++ elf64_header;
    try expectRun(&input, 0, elf32_define, "");
}

test "first 64-bit ELF header stays authoritative with trailing fragments-failure-header cluster" {
    const input = elf64_header ++ elf32_fragment ++ not_elf_header ++ elf32_header;
    try expectRun(&input, 0, elf64_define, "");
}

test "first invalid-class header stays authoritative with trailing fragments-failure-header cluster" {
    const input = invalid_class_header ++ elf64_fragment ++ not_elf_header ++ elf32_header;
    try expectRun(&input, 1, "", "");
}

test "first non-ELF header stays authoritative with trailing fragments-failure-header cluster" {
    const input = not_elf_header ++ elf32_fragment ++ invalid_class_header ++ elf64_header;
