...
test "lane19 exact-buffer EOF skips blank visible-leading-carriage-returns before an embedded NUL" {
    ...
    try gen.runGenksymsCrc(exact_then_blank_visible_cr_then_nul.items, &capture);

    const exact_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32(exact_then_blank_visible_cr_then_nul.items[0..c_line_payload_len])},
    );
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(exact_then_blank_visible_cr_then_nul.items)});
