...
def require_exact_lines(text: str, code_missing: str, code_duplicate: str, lines: tuple[str, ...]) -> list[tuple[str, str]]:
    failures: list[tuple[str, str]] = []
    for line in lines:
...
def collect_failures(root: Path) -> list[tuple[str, str]]:
    ...
    for rel, text in reminder_texts.items():
        failures.extend(require_markers(text, f"MISSING_REMINDER_MARKER:{rel.as_posix()}", VALIDATOR_PATH_MARKERS))

    failures.extend(
        require_markers(
            reminder_texts[PHASE2_CLOSURE],
            f"MISSING_CLOSURE_COMMAND_MARKER:{PHASE2_CLOSURE.as_posix()}",
            VALIDATOR_COMMAND_MARKERS,
        )
    )

    failures.extend(require_exact_lines(workflow_text, "MISSING_WORKFLOW_LINE", "DUPLICATE_WORKFLOW_LINE", WORKFLOW_LINES))
    failures.extend(require_exact_lines(makefile_text, "MISSING_MAKEFILE_LINE", "DUPLICATE_MAKEFILE_LINE", MAKEFILE_LINES))

    failures.extend(
        require_markers(validate_phase2_text, "MISSING_VALIDATE_PHASE2_SNIPPET", VALIDATE_PHASE2_SNIPPETS)
    )
    failures.extend(
        require_markers(
...
- `scripts/zigux/validate-phase2.py` and `scripts/zigux/validate-phase2-closure.py` keep the current Phase 2 reminder packet explicit from the scripts root beside the returned toolchain, kconfig, genksyms, fixdep, and validator surfaces
"""
...
"""
    ...
    write_text(resolve(root, SCRIPTS_README), scripts_readme)
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_validator_pair_surface_") as tmpdir:
        ...
        for rel in REMINDER_SURFACES:
            for marker in VALIDATOR_PATH_MARKERS:
                ...
                assert any(code.startswith("MISSING_REMINDER_MARKER") and detail == marker for code, detail in collect_failures(root))
                checks += 1

        for marker in VALIDATOR_COMMAND_MARKERS:
            build_sample_root(root)
            path = resolve(root, PHASE2_CLOSURE)
            write_text(path, remove_once(read_text(path), marker))
            assert ("MISSING_CLOSURE_COMMAND_MARKER:Documentation/zigux/phase2-closure.md", marker) in collect_failures(root)
            checks += 1
        ...
        assert ("VALIDATOR_LIST_MISMATCH", "scripts/zigux/validate-phase2-closure.py") in collect_failures(root)
        checks += 1
