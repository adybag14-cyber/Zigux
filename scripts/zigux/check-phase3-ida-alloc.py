#!/usr/bin/env python3
"""Fail-close the current Phase 3 ida allocation starter-plus-dump packet."""
...
DOC_PATH = Path("Documentation/zigux/phase3-ida-alloc-slice.md")
BITMAP_HELPER_PATH = Path("zigux/helpers/ida_bitmap_view.zig")
HELPER_PATH = Path("zigux/helpers/ida_alloc_view.zig")
STARTER_TEST_PATH = Path("zigux/tests/phase3_ida_alloc_starter_packet.zig")
STARTER_BUILD_PATH = Path("zigux/tests/phase3_ida_alloc_starter_packet_build.zig")
STARTER_CHECK_PATH = Path("scripts/zigux/check-phase3-ida-alloc-starter-packet.py")
DUMP_PATH = Path("zigux/tests/phase3_ida_alloc_dump.zig")
DUMP_BUILD_PATH = Path("zigux/tests/phase3_ida_alloc_dump_build.zig")
EXPECTED_PATH = Path("zigux/tests/fixtures/phase3_ida_alloc/expected.json")
C_HARNESS_PATH = Path("zigux/tests/fixtures/phase3_ida_alloc/phase3_ida_alloc_c_harness.c")
...
def _run_c_harness(repo_root: Path, cc: str) -> object:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_ida_alloc_c_") as temp_dir:
        binary = Path(temp_dir) / "phase3_ida_alloc_c_harness"
        compile_result = _run(
...
SELF_TEST_CASES = (
    (DOC_PATH, "`python3 scripts/zigux/check-phase3-ida-alloc.py --repo-root . --zig zig --cc gcc`"),
    (HELPER_PATH, "pub const Selection = struct {"),
    (DUMP_PATH, '"unordered_window"'),
    (EXPECTED_PATH, '"name": "full_window"'),
)


def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_ida_alloc_") as temp_dir:
        root = Path(temp_dir)
        files_to_copy = (
            DOC_PATH,
            BITMAP_HELPER_PATH,
        ...
        if issues:
            print("PHASE3_IDA_ALLOC_SELF_TEST=fail")
            print("\n".join(issues))
            return 1
    ...
    print("PHASE3_IDA_ALLOC_SELF_TEST=pass")
    print(f"PHASE3_IDA_ALLOC_SELF_TEST_CASES={len(SELF_TEST_CASES)}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the current Phase 3 ida allocation starter-plus-dump packet."
    )
