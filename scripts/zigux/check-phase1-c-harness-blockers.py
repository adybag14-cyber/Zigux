...
def c_harness_path(root: Path) -> Path:
    return root / EXPECTED_C_HARNESS_PATH
...
def validate_blockers_payload(payload: object, root: Path) -> list[str]:
    ...
    if c_harness_path(root).exists():
        issues.append(f"unexpected_c_harness:{EXPECTED_C_HARNESS_PATH}")
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_c_harness_") as tmp_dir:
        ...
        unexpected_c_harness_root = root / "unexpected-c-harness-root"
        write_sample_root(unexpected_c_harness_root)
        write_text(c_harness_path(unexpected_c_harness_root), "/* unexpected */\n")
        assert_case(
            "unexpected_c_harness_present",
            validate_root(unexpected_c_harness_root)
            == [f"unexpected_c_harness:{EXPECTED_C_HARNESS_PATH}"],
        )
        covered.append("unexpected_c_harness_present")
...
def main() -> int:
    ...
    print("PHASE1_C_HARNESS_BLOCKERS_C_HARNESS_PRESENT=False")
