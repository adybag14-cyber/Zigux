// Ported from check-phase1-bitmap-helper-packet.py by port_phase1_guards.py
const std = @import("std");
const Io = std.Io;
const guard = @import("zigux_guard.zig");

pub const pass_marker = "PHASE1_BITMAP_HELPER_PACKET_SELF_TEST=pass";

const BITMAP_HELPER_REL = "tools/lib/bitmap.zig";

const EXPECTED_BITMAP_DIRECT_TEST_ANCHORS = [_][]const u8{
...
const EXPECTED_BITMAP_SOURCE_SYMBOLS = [_][]const u8{
...
const EXPECTED_SUMMARY = "Phase 1 bitmap stays on the direct-anchor family for the current helper-local bit-window orBits and xorBits coverage plus the contiguous-range scnprintf packet, so future rereads should refresh this one helper-local checker before widening into closure-note or reminder-surface sync work.";
...
pub fn runSelfTest(io: Io, allocator: std.mem.Allocator) !u8 {
    ...
    try guard.printLine(io, "PHASE1_BITMAP_HELPER_PACKET_SELF_TEST_CASE_COUNT={d}", .{@as(usize, 1)});
    return 0;
}
...
pub fn main(init: std.process.Init) !void {
    ...
    if (failures.items.len > 0) {
        try guard.printLine(io, "PHASE1_BITMAP_HELPER_PACKET_SELF_TEST=fail", .{});
