...
def run_self_test() -> int:
    ...
    missing_required = base_text.replace(REQUIRED_SNIPPETS[0], "", 1)
    issues = collect_issues(missing_required)
    assert ("REQUIRED_SNIPPET_COUNT_MISMATCH", f"{REQUIRED_SNIPPETS[0]}:actual=0:expected=1") in issues
    checks_run += 1
