...
test "escaped colon dependency survives concatenated target CRLF comment path" {
    ...
    try processor.parseDepFile(
        &capture,
        "sample.o: sample.rmeta dep-first.so \\\r\n# generated by rustc\\\\\r\n  still comment\r\nmodule/sample.o: temp.rmeta dir\\:crate.rmeta later.rmeta\r\n",
        "sample.o",
    );
...
test "runFixdep preserves escaped colon dependencies through the public entry path" {
    ...
    const dep_body =
        "CONFIG_ZIGUX_COLON CONFIG_ZIGUX_SHARED\n";
    try tmp.dir.writeFile(std.testing.io, .{
