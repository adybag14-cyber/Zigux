...
test "genksyms bridge keeps repeated version side effects before abbreviated long request options" {
...
test "genksyms bridge keeps mixed repeated version prefixes before abbreviated long request options" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
