...
def extract_next_steps_packet(root: Path) -> tuple[str, ...]:
    ...
    try:
        next_index = roadmap_lines.index(NEXT_HEADING, section_index + 1)
    except ValueError as exc:
        raise AssertionError(f"missing heading: {NEXT_HEADING}") from exc

    if not previous_index < section_index < next_index:
        raise AssertionError(
            f"unexpected heading order: {PREVIOUS_HEADING} -> {SECTION_HEADING} -> {NEXT_HEADING}"
        )

    return tuple(line for line in roadmap_lines[section_index + 1 : next_index] if line.strip())
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_lane01_next_steps_") as tmp_dir:
        ...
        if errors:
            raise AssertionError(f"baseline Lane 01 next-steps fixture should pass: {errors}")
        case_count += 1
        ...
        _write(
            root / ROADMAP_PATH,
            _sample_roadmap().replace("Immediate next steps after this document lands:\n\n", "", 1),
        )
        errors = check_next_steps_packet(root)
        if not errors or errors[0] != "next-steps packet mismatch":
            raise AssertionError(f"expected intro mismatch, got: {errors}")
        _write(root / ROADMAP_PATH, _sample_roadmap())
        case_count += 1
        ...
        errors = check_next_steps_packet(root)
        if not errors or errors[0] != "next-steps packet mismatch":
            raise AssertionError(f"expected missing-step mismatch, got: {errors}")
        _write(root / ROADMAP_PATH, _sample_roadmap())
        case_count += 1
        ...
        _write(
            ...
            _sample_roadmap().replace(
                "5. do not start runtime kernel ports before the Phase 2-4 gates are in place\n",
                "5. do not start runtime kernel ports before the Phase 2-4 gates are in place\n"
                "6. expand runtime kernel ports in parallel with the earliest helper tranche\n",
                1,
            ),
        )
        errors = check_next_steps_packet(root)
        if not errors or errors[0] != "next-steps packet mismatch":
            raise AssertionError(f"expected extra-step mismatch, got: {errors}")
        _write(root / ROADMAP_PATH, _sample_roadmap())
        case_count += 1
...
def main() -> int:
    ...
    print(f"LANE01_BOOTSTRAP_ROADMAP_NEXT_STEPS_REQUIRED_LINE_COUNT={len(EXPECTED_LINES)}")
    print(
