...
EXACT_WORKFLOW_RUN_COUNTS = {
    ...
    "python3 scripts/zigux/check-phase2-workflow-action-path.py --self-test": 1,
    "python3 scripts/zigux/check-phase2-workflow-action-path.py": 1,
}
...
def build_self_test_root(root: Path) -> None:
    ...
    write_text(resolve_path(root, WORKFLOW_ACTION_CHECKER), "#!/usr/bin/env python3\n")
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_workflow_action_path_") as tmp_dir:
        ...
        for command in (
            ...
            "python3 scripts/zigux/check-phase2-workflow-action-path.py",
        ):
            build_self_test_root(root)
        ...
        assert (
            ...
            "python3 scripts/zigux/check-phase2-workflow-action-path.py:count=2",
        ...
        for first, second in (
            ("Self-test current Zig installer helper", "Self-test current Zig toolchain checker"),
            ("Check current Phase 2 cross alignment packet", "Self-test current Phase 2 direct cross-route checker"),
            ("Check current Phase 2 direct cross-route packet", "Self-test current Phase 2 workflow action-path checker"),
        ):
            build_self_test_root(root)
            workflow_path = resolve_path(root, WORKFLOW)
            workflow_path.write_text(
...
def main() -> int:
    ...
    print("PHASE2_WORKFLOW_ACTION_PATH_REQUIRED_FILE_COUNT=5")
