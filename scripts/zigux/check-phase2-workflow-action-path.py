...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_workflow_action_path_") as tmp_dir:
        ...
        for first, second in (
            ("Self-test current Phase 2 direct cross-route checker", "Check current Phase 2 direct cross-route packet"),
            ("Check current Phase 2 direct cross-route packet", "Self-test current Phase 2 cross selftest alignment checker"),
            ("Check current Phase 2 workflow action-path packet", "Self-test current Phase 2 toolchain pinning checker"),
        ):
            build_self_test_root(root)
            workflow_path = resolve_path(root, WORKFLOW)
            workflow_path.write_text(
