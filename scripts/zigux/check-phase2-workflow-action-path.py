...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_workflow_action_path_") as tmp_dir:
        ...
        for command in (
            ...
            "python3 scripts/zigux/check-phase2-workflow-action-path.py --self-test",
        ...
        workflow_path.write_text(
            swap_step_markers(
                ...
                "Self-test current Phase 2 workflow action-path checker",
            ),
            encoding="utf-8",
        )
        assert (
            "STEP_ORDER",
            "- name: Check current Phase 2 cross alignment packet -> - name: Self-test current Phase 2 workflow action-path checker",
        ...
        workflow_path.write_text(
            swap_step_markers(
                workflow_path.read_text(encoding="utf-8"),
                "Check current Phase 2 workflow action-path packet",
        ...
        assert (
            "STEP_ORDER",
            "- name: Check current Phase 2 workflow action-path packet -> - name: Self-test current Phase 2 toolchain pinning checker",
        ) in collect_issues(root)
        checks_run += 1
