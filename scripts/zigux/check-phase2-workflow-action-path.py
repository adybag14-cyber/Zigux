...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_workflow_action_path_") as tmp_dir:
        ...
        workflow_path.write_text(
            swap_step_markers(
                workflow_path.read_text(encoding="utf-8"),
                "Self-test restored Zig installer helper",
        ...
        assert (
            "STEP_ORDER",
            "- name: Self-test restored Zig installer helper -> - name: Self-test current Zig toolchain checker",
        ) in collect_issues(root)
        checks_run += 1
        ...
        workflow_path.write_text(
            swap_step_markers(
                ...
                "Self-test current Phase 2 direct cross-route checker",
            ),
            encoding="utf-8",
        )
        assert (
            "STEP_ORDER",
            "- name: Check current Phase 2 cross alignment packet -> - name: Self-test current Phase 2 direct cross-route checker",
        ) in collect_issues(root)
        checks_run += 1
        ...
        assert (
            "STEP_ORDER",
            "- name: Check current Phase 2 direct cross-route packet -> - name: Self-test current Phase 2 toolchain pinning checker",
        ) in collect_issues(root)
        checks_run += 1
