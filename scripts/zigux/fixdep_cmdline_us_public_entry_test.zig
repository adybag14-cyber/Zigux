...
test "runFixdep preserves US byte in savedcmd prelude" {
    const depfile_name = "zigux_fixdep_us_cmdline_test.d";
    const source_name = "zigux_fixdep_us_cmdline_source.c";
    const target_name = "zigux_fixdep_us_cmdline.o";

    try Io.Dir.cwd().writeFile(std.testing.io, .{
        .sub_path = source_name,
        .data = "int zigux_fixdep_us_cmdline(void) { return CONFIG_ZIGUX_CMDLINE_US_SOURCE; }\n",
    });
    ...
    const cmdline = "cc" ++ [_]u8{0x1f} ++ "-DZIGUX_CMDLINE_US=1 -c " ++ source_name;
    ...
    const expected =
        ...
        "deps_" ++ target_name ++ " := \\\n" ++
        "    $(wildcard include/config/ZIGUX_CMDLINE_US_SOURCE) \\\n" ++
        "\n" ++
