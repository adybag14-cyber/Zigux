...
EVIDENCE_MARKERS = [
    "lane: `P14-L09`",
    "manifest `surveyed_commit`: `40f2a065b1c06d7ea621c1c0c388e6202b0b22b7`",
...
BUILD_MARKERS = [
    "const phase14_workqueue_bridge_tests",
    "const phase14_workqueue_reviewability_tests",
    "const phase14_skbuff_bridge_tests",
    "const phase14_ring_buffer_survey_tests",
    "const phase14_rcu_tree_survey_tests",
    "const phase14_end_to_end_smoke_tests",
    "const smoke_step = b.step(\"phase14-smoke\"",
...
def write_sample_root(root: Path) -> None:
    ...
    manifest = {
        "surveyed_commit": "40f2a065b1c06d7ea621c1c0c388e6202b0b22b7",
    ...
    build_text = """const phase14_workqueue_bridge_tests = 0;
const phase14_workqueue_reviewability_tests = 0;
const phase14_skbuff_bridge_tests = 0;
const phase14_ring_buffer_survey_tests = 0;
const phase14_rcu_tree_survey_tests = 0;
const phase14_end_to_end_smoke_tests = 0;
const smoke_step = b.step(\"phase14-smoke\", \"focused\");
const test_step = b.step(\"test\", \"full\");
"""
    (root / BUILD_PATH).write_text(build_text, encoding="utf-8")

    makefile_text = """phase14-validate:
...
def run_self_test() -> int:
    ...
    try:
        ...
        build_path.write_text(build_path.read_text(encoding="utf-8").replace(
            "const phase14_skbuff_bridge_tests = 0;\n", ""
        ), encoding="utf-8")
        negative = validate(temp_root)
        if not any(item.startswith(f"missing_marker:{BUILD_PATH.as_posix()}:const phase14_skbuff_bridge_tests") for item in negative):
            print("PHASE14_COMPILE_SHARD_MATRIX_EVIDENCE_SELF_TEST=fail")
