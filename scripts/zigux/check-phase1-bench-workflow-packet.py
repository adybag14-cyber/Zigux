#!/usr/bin/env python3
"""Guard the live Phase 1 bench-adjacent workflow packet on current master."""
...
FORBIDDEN_LINES = (
    ...
    "- name: Self-test Phase 1 find-bit bench anchor checker",
    "- name: Check Phase 1 find-bit bench anchor packet",
)
...
def collect_failures(root: Path) -> list[str]:
    ...
    if positions != sorted(positions):
        failures.append("phase1_bench_workflow_packet:order_drift")
...
def build_sample_root(root: Path) -> None:
    for relative_path in (ROUTE_SUMMARY_REL, BENCH_REL, FIND_BIT_BENCH_REL):
...
def run_self_test() -> int:
    cases: list[tuple[str, object | None]] = [
        ...
        (
            "missing_route_summary_selftest_name",
        ...
        (
            "missing_find_bit_bench_packet",
            lambda root: remove_line(root, "- name: Check current Phase 1 find-bit bench anchor packet"),
        ),
        (
            "duplicate_find_bit_bench_live",
            lambda root: duplicate_line(root, "run: python3 scripts/zigux/check-phase1-find-bit-bench-anchors.py"),
        ),
        (
            "bad_order_inside_cluster",
        ...
        (
            "cluster_before_find_bit_review_boundary",
            lambda root: swap_lines(
                root,
                "- name: Check current Phase 1 find-bit review packet",
                "- name: Self-test current Phase 1 route summary checker",
        ...
        (
            "cluster_after_shared_reminder_boundary",
            lambda root: swap_lines(
                root,
                "- name: Check current Phase 1 find-bit bench anchor packet",
                "- name: Self-test current Phase 1 shared reminder checker",
        ...
        (
            "forbidden_old_label",
            lambda root: append_line(root, "      - name: Self-test Phase 1 bench checker"),
        ),
        ("missing_bench_file", lambda root: (root / BENCH_REL).unlink()),
        ("missing_find_bit_bench_file", lambda root: (root / FIND_BIT_BENCH_REL).unlink()),
        ("missing_route_summary_file", lambda root: (root / ROUTE_SUMMARY_REL).unlink()),
    ]

    for name, mutate in cases:
        with tempfile.TemporaryDirectory(prefix="phase1-bench-workflow-") as tmpdir:
            root = Path(tmpdir)
            ...
            if name == "baseline":
                if failures:
                    print(f"phase1-bench-workflow:{name}:unexpected={failures}")
                    return 1
            elif not failures:
                print(f"phase1-bench-workflow:{name}:expected_failure")
                return 1
...
def main() -> int:
    ...
    if args.write_sample_root:
        ...
        print("PHASE1_BENCH_WORKFLOW_PACKET_SAMPLE_ROOT=written")
        return 0
    ...
    print(f"PHASE1_BENCH_WORKFLOW_PACKET_REQUIRED_LINE_COUNT={len(EXACT_ONCE_LINES)}")
    return 0
