...
pub fn build(b: *std.Build) void {
    ...
    const proof_module = b.createModule(.{
        .root_source_file = b.path("genksyms_exact_dump_long_executable_test.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const proof_step = b.step(
        "lane23-genksyms-exact-dump-long-executable",
        "Run the Lane 23 genksyms exact dump long executable proof",
    );
    proof_step.dependOn(&run_proof.step);

    const test_step = b.step("test", "Run the Lane 23 genksyms exact dump long executable proof");
