...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    print("PHASE2_GENKSYMS_ROUTE_CONTRACT=fail")
...
def run_self_test() -> int:
    case_count = 0
    with tempfile.TemporaryDirectory(prefix="phase2_genksyms_route_contract_") as tmp_dir:
        root = Path(tmp_dir)

        build_good_root(root)
        if collect_issues(root):
            raise SystemExit("phase2-genksyms-route-contract:self-test:good_root")
        case_count += 1
        ...
        if ("MISSING_VALIDATE_PHASE2_SNIPPET", VALIDATE_REQUIRED_SNIPPETS[0]) not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:missing_validate_snippet")
        case_count += 1

        build_good_root(root)
        validate_path = root / VALIDATE_PHASE2
        validate_path.write_text(
            duplicate_exact_line(read_text(validate_path), VALIDATE_REQUIRED_SNIPPETS[15], "    "),
            encoding="utf-8",
        )
        issues = collect_issues(root)
        if ("DUPLICATE_VALIDATE_PHASE2_SNIPPET", f"{VALIDATE_REQUIRED_SNIPPETS[15]}:count=2") not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:duplicate_validate_route")
        case_count += 1
        ...
        if ("MISSING_MAKEFILE_LINE", REQUIRED_MAKEFILE_LINES[1]) not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:missing_makefile_line")
        case_count += 1
        ...
        if ("DUPLICATE_MAKEFILE_LINE", f"{REQUIRED_MAKEFILE_LINES[2]}:count=2") not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:duplicate_makefile_line")
        case_count += 1
        ...
        if ("MISSING_WORKFLOW_LINE", REQUIRED_WORKFLOW_LINES[0]) not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:missing_workflow_line")
        case_count += 1

        build_good_root(root)
        workflow_path = root / WORKFLOW
        workflow_path.write_text(
            duplicate_exact_line(read_text(workflow_path), REQUIRED_WORKFLOW_LINES[2], "  "),
            encoding="utf-8",
        )
        issues = collect_issues(root)
        if ("DUPLICATE_WORKFLOW_LINE", f"{REQUIRED_WORKFLOW_LINES[2]}:count=2") not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:duplicate_workflow_line")
        case_count += 1
        ...
        if ("MISSING_REQUIRED_PATH", REQUIRED_PATHS[0].as_posix()) not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:missing_required_path")
        case_count += 1
        ...
        if ("MISSING_REQUIRED_FILE", VALIDATE_PHASE2.as_posix()) not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:missing_validate_file")
        case_count += 1
        ...
        if ("MISSING_REQUIRED_FILE", MAKEFILE.as_posix()) not in issues:
            raise SystemExit("phase2-genksyms-route-contract:self-test:missing_makefile_file")
        case_count += 1

    print("PHASE2_GENKSYMS_ROUTE_CONTRACT_SELF_TEST=pass")
    print(f"PHASE2_GENKSYMS_ROUTE_CONTRACT_SELF_TEST_CASE_COUNT={case_count}")
    if case_count != EXPECTED_SELF_TEST_CASE_COUNT:
        raise SystemExit("phase2-genksyms-route-contract:self-test:unexpected_case_count")
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check that the live Phase 2 genksyms packet stays aligned across "
    ...
    print("PHASE2_GENKSYMS_ROUTE_CONTRACT=pass")
    print(f"PHASE2_GENKSYMS_ROUTE_CONTRACT_REQUIRED_PATH_COUNT={len(REQUIRED_PATHS)}")
    print(f"PHASE2_GENKSYMS_ROUTE_CONTRACT_VALIDATE_SNIPPET_COUNT={len(VALIDATE_REQUIRED_SNIPPETS)}")
    print(f"PHASE2_GENKSYMS_ROUTE_CONTRACT_MAKEFILE_LINE_COUNT={len(REQUIRED_MAKEFILE_LINES)}")
    print(f"PHASE2_GENKSYMS_ROUTE_CONTRACT_WORKFLOW_LINE_COUNT={len(REQUIRED_WORKFLOW_LINES)}")
    return 0
