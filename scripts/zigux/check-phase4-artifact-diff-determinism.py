...
EXPECTED_SELF_TEST_CASES = [
    ...
    "survey_inventory_marker_round_trip",
    "survey_inventory_marker_drift",
...
REQUIRED_SURVEY_NOTE_MARKERS = [
    ...
    f"- `PHASE4_ARTIFACT_DIFF_CONTRACT_BASE_CASES={','.join(EXPECTED_BASE_CONTRACT_CASES)}`",
...
REQUIRED_SURVEY_INVENTORY_MARKERS = [
...
REQUIRED_REVIEW_NOTE_MARKERS = [
    ...
    "- deterministic survey self-test catalog: `PHASE4_ARTIFACT_DIFF_DETERMINISM_SELF_TEST_CASE_COUNT` and `PHASE4_ARTIFACT_DIFF_DETERMINISM_SELF_TEST_CASES` must stay aligned with the isolated phase4-use, survey-note, survey-inventory, survey-replay, review-note, docs-root, scripts-root, helper-summary, and contract-catalog drift coverage",
]
...
def run_self_test() -> int:
    ...
    survey_inventory_text = "\n".join(REQUIRED_SURVEY_INVENTORY_MARKERS)
    assert_markers(
        survey_inventory_text,
        REQUIRED_SURVEY_INVENTORY_MARKERS,
        "survey_inventory",
    )
    covered_cases.append("survey_inventory_marker_round_trip")

    expect_assertion(
        "survey_inventory_marker_drift",
        lambda: assert_markers(
            REQUIRED_SURVEY_INVENTORY_MARKERS[0],
            REQUIRED_SURVEY_INVENTORY_MARKERS,
            "survey_inventory",
        ),
    )
    covered_cases.append("survey_inventory_marker_drift")
...
def main() -> int:
    ...
    assert_markers(survey_note_text, REQUIRED_SURVEY_NOTE_MARKERS, "survey_note")
    assert_markers(survey_note_text, REQUIRED_SURVEY_INVENTORY_MARKERS, "survey_inventory")
