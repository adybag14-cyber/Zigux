#!/usr/bin/env python3
"""Check that the Phase 2 toolchain action path stays aligned."""
...
EXPECTED_MAKE_WRAPPERS = ("make -C zigux phase2-toolchain",)
...
REQUIRED_PHASE2_PHONY_TARGETS = ("phase2-toolchain",)
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    if len(workflow_indices) == len(REQUIRED_WORKFLOW_LINES) and workflow_indices != sorted(
        ...
        issues.append(("WORKFLOW_ORDER_MISMATCH", "phase2-toolchain-route-order"))
    ...
    if len(makefile_indices) == len(REQUIRED_MAKEFILE_LINES) and makefile_indices != sorted(
        ...
        issues.append(("MAKEFILE_ORDER_MISMATCH", "phase2-toolchain-packet-order"))
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_toolchain_action_path_") as tmp_dir:
        ...
        assert ("WORKFLOW_ORDER_MISMATCH", "phase2-toolchain-route-order") in collect_issues(root)
        checks += 1
        ...
        path.write_text("\n".join((phony, *body)) + "\n", encoding="utf-8")
        assert ("MAKEFILE_ORDER_MISMATCH", "phase2-toolchain-packet-order") in collect_issues(root)
        checks += 1
        ...
        for label, expected in (
            ("archive_support", EXPECTED_ARCHIVE_SUPPORT),
            ("bootstrap_helpers", EXPECTED_BOOTSTRAP_HELPERS),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the Phase 2 toolchain action path stays aligned."
    )
    ...
    print(f"PHASE2_TOOLCHAIN_ACTION_PATH_DOCS_MARKER_COUNT={len(REQUIRED_DOCS_README_MARKERS)}")
    print(f"PHASE2_TOOLCHAIN_ACTION_PATH_BOOTSTRAP_MARKER_COUNT={len(REQUIRED_BOOTSTRAP_MARKERS)}")
    print(f"PHASE2_TOOLCHAIN_ACTION_PATH_CLOSURE_MARKER_COUNT={len(REQUIRED_CLOSURE_MARKERS)}")
    print(f"PHASE2_TOOLCHAIN_ACTION_PATH_REVIEW_MARKER_COUNT={len(REQUIRED_REVIEW_MARKERS)}")
    print(f"PHASE2_TOOLCHAIN_ACTION_PATH_SCRIPTS_MARKER_COUNT={len(REQUIRED_SCRIPTS_README_MARKERS)}")
    print(f"PHASE2_TOOLCHAIN_ACTION_PATH_TESTS_MARKER_COUNT={len(REQUIRED_TESTS_README_MARKERS)}")
    return 0
