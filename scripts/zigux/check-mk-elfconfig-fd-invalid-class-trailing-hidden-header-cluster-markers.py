...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-fd-invalid-class-trailing-hidden-header-cluster-") as tmp_dir:
        ...
        missing_path = missing_root / "scripts/zigux/mk_elfconfig_fd_invalid_class_trailing_hides_not_elf_test.zig"
        missing_path.write_text(
            EXPECTED_MARKERS[Path("scripts/zigux/mk_elfconfig_fd_invalid_class_trailing_hides_not_elf_test.zig")][0][1]
            + "\n",
            encoding="utf-8",
        )
        try:
            ...
            if "missing_marker:fd_invalid_class_trailing_hides_not_elf_no_skip" not in str(exc):
                raise
        else:
    ...
    print("MK_ELFCONFIG_FD_INVALID_CLASS_TRAILING_HIDDEN_HEADER_CLUSTER_MARKERS_SELF_TEST=pass")
    print("MK_ELFCONFIG_FD_INVALID_CLASS_TRAILING_HIDDEN_HEADER_CLUSTER_MARKERS_SELF_TEST_CASE_COUNT=4")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the mk_elfconfig invalid-class trailing hidden-header fd replay cluster stays present."
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    print("MK_ELFCONFIG_FD_INVALID_CLASS_TRAILING_HIDDEN_HEADER_CLUSTER_MARKERS=pass")
    print(f"MK_ELFCONFIG_FD_INVALID_CLASS_TRAILING_HIDDEN_HEADER_CLUSTER_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
