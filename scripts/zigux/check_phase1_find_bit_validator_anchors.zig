...
const REQUIRED_CLOSURE_LINES = [_]struct { label: []const u8, marker: []const u8 }{
    ...
    .{ .label = "find_bit_bench_guard", .marker = "- `PHASE1_FIND_BIT_BENCH_ANCHOR_GUARD=zig run check_phase1_find_bit_bench_anchors.zig exact-checks inclusive-boundary, past-nbits no-read, clump8 past-end no-read, and findLastBit tail-clamp anchors directly in tools/lib/find_bit.zig`" },
};
...
const REQUIRED_SCRIPTS_README_LINES = [_]struct { label: []const u8, marker: []const u8 }{
    .{ .label = "phase1_replay_line", .marker = "- `zig run validate_phase1_closure.zig`, `zig run check_phase1_string_review_packet.zig --self-test`, `zig run check_phase1_direct_owner_markers.zig --self-test`, `zig run check_phase1_bench.zig --self-test`, and `zig run check_phase1_shared_reminder_packet.zig --self-test` replay the shipped bounded Phase 1 reminder checks, and `zig build phase1-host-tools-smoke --build-file zigux/tests/build.zig` replays the shipped shared tests-root smoke route" },
