...
test "runFixdep parses an unterminated final dependency line" {
    ...
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "visible.h",
        .data = "#define CONFIG_ZIGUX_FINAL_LINE_VISIBLE_MODULE 1\n",
    });
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "sentinel.so",
        .data = "CONFIG_ZIGUX_FINAL_LINE_SO_SHOULD_NOT_PARSE\n",
    });
    ...
    const expected = try std.fmt.allocPrint(
        ...
        "savedcmd_sample.o := cc -c source.c -o sample.o\n\n" ++
            ...
            "deps_sample.o := \\\n" ++
            "  {s} \\\n" ++
            "    $(wildcard include/config/ZIGUX_FINAL_LINE_VISIBLE) \\\n" ++
            "  {s} \\\n" ++
            "\n" ++
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "ZIGUX_FINAL_LINE_VISIBLE_MODULE") == null);
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "ZIGUX_FINAL_LINE_SO_SHOULD_NOT_PARSE") == null);
}
