    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_validator_") as temp_dir:
        ...
        cases = (
            (
                ABI_HEADER_PATH,
                "#define ZIGUX_ABI_VERSION 1U\n",
                "missing include/zigux/abi.h marker: #define ZIGUX_ABI_VERSION 1U",
            ),
            (
                ABI_BINDINGS_PATH,
                'const notifier_abi = @import("notifier_abi.zig");\n',
                'missing zigux/bindings/abi.zig marker: const notifier_abi = @import("notifier_abi.zig");',
            ),
            (
                NOTIFIER_BINDINGS_PATH,
                "pub const NotifierResult = enum(u32) {\n",
                "missing zigux/bindings/notifier_abi.zig marker: pub const NotifierResult = enum(u32) {",
            ),
            (
                RUNNER_PATH,
                'Path("scripts/zigux/check-phase3-low-level-wrappers.py")\n',
                'missing scripts/zigux/run-phase3-checks.py marker: Path("scripts/zigux/check-phase3-low-level-wrappers.py")',
            ),
            (
                VALIDATE_PHASE3_SELFTEST_PATH,
                'Path("scripts/zigux/check-phase3-list-hlist.py")\n',
                'missing scripts/zigux/validate_phase3_selftest.py marker: Path("scripts/zigux/check-phase3-list-hlist.py")',
            ),
            (
                TESTS_BUILD_PATH,
                "const phase3_abi_core_packet = addPhase3AbiCorePacket(b, target, optimize);\n",
                "missing zigux/tests/build.zig marker: const phase3_abi_core_packet = addPhase3AbiCorePacket(b, target, optimize);",
            ),
            (
                ABI_TEST_PATH,
                'test "phase3 abi keeps shared layout assertions wired into the replay" {\n',
                'missing zigux/tests/phase3_abi.zig marker: test "phase3 abi keeps shared layout assertions wired into the replay" {',
            ),
            (
                ABI_DUMP_PATH,
                'const abi = @import("abi_bindings");\n',
                'missing zigux/tests/phase3_abi_dump_current.zig marker: const abi = @import("abi_bindings");',
            ),
            (
                EXPORT_UAPI_LAYOUT_PATH,
                'const header_family = @import("header_family_binding");\n',
            ...
            (
                EXPORT_UAPI_LAYOUT_BUILD_PATH,
                '.root_source_file = b.path("../uapi/dev_t.zig"),\n',
                'missing zigux/tests/phase3_export_uapi_layout_build.zig marker: .root_source_file = b.path("../uapi/dev_t.zig"),',
            ),
            (
                EXPORT_SHIM_BUILD_PATH,
                '.root_source_file = b.path("../kernel/export_shim.zig"),\n',
                'missing zigux/tests/phase3_export_shim_build.zig marker: .root_source_file = b.path("../kernel/export_shim.zig"),',
            ),
            (
                EXPORT_SHIM_PATH,
                "pub const Header = version.Header;\n",
                "missing zigux/kernel/export_shim.zig marker: pub const Header = version.Header;",
            ),
            (
                UAPI_VERSION_PATH,
                "pub fn validateBoundaryHeader(header: Header) abi.ExportStatus {\n",
        ...
        packet_file_checks = (
            ...
            ("zigux/tests/fixtures/phase3_bitmap_cpumask/phase3_bitmap_cpumask_c_harness.c", "expected bitmap cpumask c-harness drift was not reported"),
            ("zigux/tests/fixtures/phase3_bitmap_cpumask/expected.json", "expected bitmap cpumask expected-json drift was not reported"),
            ...
            ("zigux/tests/phase3_list_hlist_dump.zig", "expected list hlist dump zig drift was not reported"),
            ("zigux/tests/phase3_list_hlist_dump_build.zig", "expected list hlist dump build drift was not reported"),
            ("zigux/tests/fixtures/phase3_list_hlist/phase3_list_hlist_c_harness.c", "expected list hlist c-harness drift was not reported"),
            ("zigux/tests/fixtures/phase3_list_hlist/expected.json", "expected list hlist expected-json drift was not reported"),
            ...
            ("scripts/zigux/check-phase3-list-hlist.py", "expected full list hlist checker drift was not reported"),
        ...
        replay_route_checks = (
            ...
            ("python3 scripts/zigux/check-phase3-low-level-wrappers.py --self-test", "expected low-level-wrapper checker self-test route drift was not reported"),
            ("python3 scripts/zigux/check-phase3-low-level-wrappers.py", "expected low-level-wrapper checker direct route drift was not reported"),
            ...
            ("python3 scripts/zigux/check-phase3-list-hlist.py --self-test", "expected full list hlist self-test route drift was not reported"),
            ("python3 scripts/zigux/check-phase3-list-hlist.py --repo-root . --zig zig --cc gcc", "expected full list hlist direct route drift was not reported"),
            ...
            ("zig build phase3-list-hlist-dump --build-file zigux/tests/phase3_list_hlist_dump_build.zig", "expected full list hlist dump route drift was not reported"),
