    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_validator_selftest_") as tmp_dir_str:
        ...
        wrapper_test_path.write_text(
            "\n".join(
                marker
                for marker in LOW_LEVEL_WRAPPER_REQUIRED_MARKERS
                if marker != "try mmio.write8InteropPolicy(base, 0, 0x33, mmio_policy);"
            )
            + "\n",
            encoding="utf-8",
            newline="\n",
        )
        missing_mmio_interop_issues: list[str] = []
        validate_low_level_wrapper_markers(root, "abi", missing_mmio_interop_issues)
        assert missing_mmio_interop_issues == [
            "abi:missing_low_level_wrapper_marker=try mmio.write8InteropPolicy(base, 0, 0x33, mmio_policy);"
        ]
