...
test "runGenksymsCrc preserves DEL as data in JSON output" {
    const record = "alpha\x7fbeta";
    var capture = try Capture(192).init(std.testing.allocator);
    defer capture.deinit();

    try genksyms_crc.runGenksymsCrc(record ++ "\n", &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{genksyms_crc.crc32(record)});
    defer std.testing.allocator.free(exact_crc);

    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, record) != null);
