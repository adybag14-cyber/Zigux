...
pub fn build(b: *std.Build) void {
    ...
    const consistency_mod = b.createModule(.{
        .root_source_file = b.path("mk_elfconfig_public_entry_classify_render_consistency_test.zig"),
        .target = target,
        .optimize = optimize,
    });
    const consistency_tests = b.addTest(.{
        .name = "mk-elfconfig-public-entry-consistency-tests",
        .root_module = consistency_mod,
    });
    const run_consistency_tests = b.addRunArtifact(consistency_tests);

    const consistency_step = b.step(
        "mk-elfconfig-public-entry-consistency",
        "Run mk_elfconfig public-entry consistency tests",
    );
    consistency_step.dependOn(&run_consistency_tests.step);
