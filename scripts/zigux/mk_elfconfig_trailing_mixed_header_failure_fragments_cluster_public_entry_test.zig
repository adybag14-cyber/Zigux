...
test "first 32-bit ELF header stays authoritative with mixed trailing header-plus-failure-fragments cluster" {
    const input = elf32_header ++ elf64_header ++ invalid_class_header ++ not_elf_header;
    try expectRun(&input, 0, elf32_define, "");
}

test "first 64-bit ELF header stays authoritative with mixed trailing header-plus-failure-fragments cluster" {
    const input = elf64_header ++ elf32_header ++ invalid_class_header ++ not_elf_header;
    try expectRun(&input, 0, elf64_define, "");
}

test "first invalid-class header stays authoritative with mixed trailing header-plus-failure-fragments cluster" {
    const input = invalid_class_header ++ elf32_header ++ elf64_header ++ not_elf_header;
    try expectRun(&input, 1, "", "");
}

test "first non-ELF header stays authoritative with mixed trailing header-plus-failure-fragments cluster" {
    const input = not_elf_header ++ elf64_header ++ elf32_header ++ invalid_class_header;
