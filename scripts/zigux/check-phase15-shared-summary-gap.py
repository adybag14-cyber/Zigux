...
def collect_gap_failures(root: Path) -> list[str]:
    ...
    for marker in DOCS_README_OVERCLAIM_MARKERS:
        if marker not in docs_readme:
            failures.append(f"docs-root Phase 15 overclaim marker disappeared and the gap note needs refresh: {marker}")
    ...
    for relative_path in MISSING_PATHS:
        if (root / relative_path).exists():
            failures.append(f"previously missing Phase 15 path now exists and the gap note must be refreshed: {relative_path}")
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _write(note_root / GAP_NOTE_PATH, _sample_gap_note().replace("`zigux/tests/phase15_build.zig`\n", ""))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "tests-root, and repo-reality drift."
        )
    )
    parser.add_argument(
        "--root",
