...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_lane01_tree_shape_") as tmp_dir:
        ...
        failures = collect_failures(root)
        expected = ["forbidden-dir:zigux-alpha/drivers"]
        if failures != expected:
        ...
        failures = collect_failures(root)
        expected = ["forbidden-dir:zigux-alpha/zigux"]
        if failures != expected:
        ...
        failures = collect_failures(root)
        expected = ["forbidden-dir:zigux-alpha/tools"]
        if failures != expected:
        ...
        failures = collect_failures(root)
        expected = ["forbidden-dir:zigux-alpha/samples"]
        if failures != expected:
