...
REQUIRED_NOTE_LINES = (
    ...
    "- `PHASE1_WORKFLOW_CURRENT_TAIL_STEPS=Self-test current Phase 4 repo-reality warning checker,Check current Phase 4 repo-reality warning packet,Self-test current Phase 4 reversible-delivery pin checker,Check current Phase 4 reversible-delivery pin packet,Self-test current Phase 4 tests README checker,Check current Phase 4 tests README packet,Self-test current Phase 7 shared-control gap checker,Check current Phase 7 shared-control gap packet,Self-test current Phase 9 review checklist boundary checker,Check current Phase 9 review checklist phase boundaries,Self-test current Phase 9 trace-events runtime packet checker,Check current Phase 9 trace-events runtime packet,Self-test current Phase 12 build-only checker,Check current docs-root sanity markers`",
...
WORKFLOW_LINE_RULES = (
    ...
    ("workflow:selftest_phase9_review_name", "      - name: Self-test current Phase 9 review checklist boundary checker"),
    ("workflow:selftest_phase9_review_run", "        run: python3 scripts/zigux/check-phase9-review-checklist-phase-boundaries.py --self-test"),
    ("workflow:check_phase9_review_name", "      - name: Check current Phase 9 review checklist phase boundaries"),
    ("workflow:check_phase9_review_run", "        run: python3 scripts/zigux/check-phase9-review-checklist-phase-boundaries.py"),
    ("workflow:selftest_phase9_trace_name", "      - name: Self-test current Phase 9 trace-events runtime packet checker"),
    ("workflow:selftest_phase9_trace_run", "        run: python3 scripts/zigux/check-phase9-trace-events-runtime-packet.py --self-test"),
    ("workflow:check_phase9_trace_name", "      - name: Check current Phase 9 trace-events runtime packet"),
    ("workflow:check_phase9_trace_run", "        run: python3 scripts/zigux/check-phase9-trace-events-runtime-packet.py"),
...
STEP_ORDER = (
    ...
    "Self-test current Phase 9 review checklist boundary checker",
    "Check current Phase 9 review checklist phase boundaries",
...
def build_sample_repo(root: Path) -> None:
    ...
    write_file(root, Path("Documentation/zigux/README.md"), "placeholder\n")
    write_file(root, Path("Documentation/zigux/phase12-release-readiness-survey.md"), DOCS_SANITY_MARKER + "\n`PHASE12_STATUS=active`\n`PHASE12_RELEASE_CLOSED=no`\nshared-summary lane owner: `pmo-release`\n")
    write_file(root, Path("zigux/tests/README.md"), "placeholder\n")
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase1-workflow-viability-") as tmpdir:
        ...
        workflow_path.write_text(
            rewrite_once(workflow_path.read_text(encoding="utf-8"), "      - name: Self-test current Zig toolchain checker\n"),
            encoding="utf-8",
        )
        ...
        build_sample_repo(root)
        workflow_path.write_text(
            rewrite_once(workflow_path.read_text(encoding="utf-8"), "      - name: Check current Phase 7 shared-control gap packet\n"),
            encoding="utf-8",
        )
        if "workflow:check_phase7_name:expected=1:actual=0" not in collect_failures(root):
            print("self-test:missing_phase7_step_case_failed")
            return 1
        case_count += 1

        build_sample_repo(root)
        workflow_path.write_text(
            rewrite_once(workflow_path.read_text(encoding="utf-8"), "      - name: Check current Phase 9 review checklist phase boundaries\n"),
            encoding="utf-8",
        )
        if "workflow:check_phase9_review_name:expected=1:actual=0" not in collect_failures(root):
            print("self-test:missing_phase9_review_step_case_failed")
            return 1
        case_count += 1

        build_sample_repo(root)
        workflow_path.write_text(
            rewrite_once(workflow_path.read_text(encoding="utf-8"), "      - name: Check current Phase 9 trace-events runtime packet\n"),
            encoding="utf-8",
        )
        if "workflow:check_phase9_trace_name:expected=1:actual=0" not in collect_failures(root):
            print("self-test:missing_phase9_trace_step_case_failed")
            return 1
        case_count += 1
        ...
        if "workflow_forbidden:scripts/zigux/check-phase9-build-only-surface.py:unexpected_present" not in collect_failures(root):
            print("self-test:stale_phase9_neighbor_case_failed")
            return 1
        case_count += 1
