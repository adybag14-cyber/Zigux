...
PHASE9_BUFFER_STEPS = (
    ...
    ("Run current Phase 9 trace-events survey witness", "zig test zigux/tests/runtime_trace_events_survey.zig"),
)
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase1-workflow-viability-") as tmpdir:
        ...
        write_file(root, WORKFLOW_REL, rewrite_once(workflow_text, old, new))
        failures = collect_failures(root)
        expected = f"workflow_adjacent_chain:missing:{'->'.join(LANE_ADJACENT_CHAIN)}"
        if expected not in failures:
        ...
        write_file(root, WORKFLOW_REL, workflow_text + "\n" + LIVE_BENCH_LINE + "\n")
        failures = collect_failures(root)
        if "workflow_forbidden:live_phase1_bench:expected=0:actual=1" not in failures:
