...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase1-workflow-viability-") as tmpdir:
        ...
        note_path.write_text(rewrite_once(note_path.read_text(encoding="utf-8"), "- `PHASE1_WORKFLOW_INSERTION_POINT=after current Phase 1 shared reminder packet and before current Phase 3 interop packet`\n"), encoding="utf-8")
        if not any(item.startswith("note:expected=1:actual=0") for item in collect_failures(root)):
            print("self-test:missing_note_line_case_failed")
            return 1
        case_count += 1
