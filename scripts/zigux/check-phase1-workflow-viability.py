...
PHASE2_STEPS = (
    ...
    ("Run current Phase 2 genksyms bridge unit tests", "zig test scripts/zigux/genksyms.zig"),
...
def build_note_text() -> str:
    ...
    lines.extend(
        (
            ...
            "- preserve the newer current-`master` Phase 2 neighbor packet ahead of the lane-local insertion, including the pinned Zig archive probe, toolchain pin-scope pair, required-make-routes pair, shared reminder pair, the `genksyms` bridge unit-test step, and the `validate-phase2.py` tool-packet step.",
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase1-workflow-viability-") as tmpdir:
        ...
        if "missing_file:scripts/zigux/genksyms.zig" not in collect_failures(root):
            print("self-test:missing_phase2_genksyms_file_case_failed")
            return 1
        case_count += 1
        ...
        if "workflow_step:Run current Phase 2 genksyms bridge unit tests:expected=1:actual=0" not in collect_failures(root):
            print("self-test:missing_phase2_genksyms_step_case_failed")
            return 1
        case_count += 1
        ...
        if "workflow_step:Validate current Phase 2 tool packet:expected=1:actual=0" not in collect_failures(root):
            print("self-test:missing_phase2_validate_step_case_failed")
            return 1
        case_count += 1
