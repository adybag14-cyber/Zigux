...
REQUIRED_FILE_RELS = (
    ...
    Path("scripts/zigux/check-phase9-build-only-surface.py"),
...
WORKFLOW_LINE_RULES = (
    ...
    ("workflow:selftest_phase9_name", "      - name: Self-test current Phase 9 build-only checker"),
    ("workflow:selftest_phase9_run", "        run: python3 scripts/zigux/check-phase9-build-only-surface.py --self-test"),
    ("workflow:check_phase9_name", "      - name: Check current Phase 9 build-only surface"),
    ("workflow:check_phase9_run", "        run: python3 scripts/zigux/check-phase9-build-only-surface.py"),
    ("workflow:run_phase9_name", "      - name: Run Phase 9 runtime helper tests"),
    ("workflow:run_phase9_run", "        run: make -C zigux phase9"),
...
STEP_ORDER = (
    ...
    "Self-test current Phase 9 build-only checker",
    "Check current Phase 9 build-only surface",
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase1-workflow-viability-") as tmpdir:
        ...
        build_sample_repo(root)
        (root / "scripts/zigux/check-phase9-build-only-surface.py").unlink()
        if "missing_file:scripts/zigux/check-phase9-build-only-surface.py" not in collect_failures(root):
            print("self-test:missing_phase9_file_case_failed")
            return 1
        case_count += 1
        ...
        if "workflow:check_phase9_name:expected=1:actual=0" not in collect_failures(root):
            print("self-test:missing_phase9_surface_case_failed")
            return 1
        case_count += 1
        ...
        if "workflow:run_phase9_name:expected=1:actual=0" not in collect_failures(root):
            print("self-test:missing_phase9_runtime_case_failed")
            return 1
        case_count += 1
