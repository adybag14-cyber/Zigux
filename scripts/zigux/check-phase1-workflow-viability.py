...
def collect_failures(root: Path) -> list[str]:
    ...
    for snippet in REQUIRED_TESTS_BUILD_SNIPPETS:
        failures.extend(require_once(tests_build_text, "tests_build", snippet))
...
def sample_tests_build_text() -> str:
    return "\n".join(
        [
...
def write_placeholder_tree(root: Path) -> None:
    ...
    write_text(root, TESTS_BUILD_REL, sample_tests_build_text())
    for relative_path in REQUIRED_FILE_RELS:
        if relative_path in (
            WORKFLOW_REL,
            DOCS_ROOT_REL,
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="lane17-workflow-viability-") as tmpdir:
        ...
        broken_root = root / "missing-tests-build"
        write_sample_root(broken_root)
        (broken_root / TESTS_BUILD_REL).unlink()
        failures = collect_failures(broken_root)
        if f"missing_file:{TESTS_BUILD_REL.as_posix()}" not in failures:
            print("self-test:missing_tests_build_not_detected")
            return 1
        case_count += 1
        ...
        broken_root = root / "missing-tests-build-snippet"
        write_sample_root(broken_root)
        tests_build_text = load_text(broken_root, TESTS_BUILD_REL)
        write_text(
            broken_root,
            TESTS_BUILD_REL,
            rewrite_once(tests_build_text, REQUIRED_TESTS_BUILD_SNIPPETS[1] + ",\n"),
        )
        failures = collect_failures(broken_root)
        if "tests_build:expected=1:actual=0" not in failures:
            print("self-test:missing_tests_build_snippet_not_detected")
            return 1
        case_count += 1
