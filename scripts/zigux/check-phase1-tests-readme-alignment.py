#!/usr/bin/env python3
"""Guard the current Phase 1 tests-root reminder packet against stale replay wording."""
...
def collect_failures(root: Path) -> list[str]:
    ...
    if not (root / TESTS_README_REL).is_file():
        return failures
    ...
    cases = (
        ...
        (
            ...
            lambda root: write_text(
                ...
                replace_once(read_text(root, TESTS_README_REL), REQUIRED_MARKERS[6] + "\n"),
            ),
        ),
        (
            "duplicate_marker",
        ...
        (
            ...
            lambda root: write_text(
                root / TESTS_README_REL,
                read_text(root, TESTS_README_REL) + FORBIDDEN_MARKERS[1] + "\n",
            ),
        ),
        (
            "missing_required_file",
            lambda root: (
                root / Path("scripts/zigux/check-phase1-direct-anchor-manifest-gate.py")
            ).unlink(),
        ),
    )
