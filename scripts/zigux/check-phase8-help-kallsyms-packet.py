...
FILE_MARKERS: dict[Path, tuple[str, ...]] = {
    HELP_SLICE: (
        ...
        "current public raw reread shows `zigux/tests/phase8_help.zig` now directly embeds this slice note and exercises the shipped helper surface through `CommandNames`, `trimCommandPrefix`, `computePrettyLayout`, `renderPrettyStringList`, and `renderCommandSections`, so the dedicated help replay is again honest same-lane evidence even while authenticated contents reads still flap here",
        "the current public raw `zigux/tests/phase8_help.zig` replay now matches that shipped helper surface through direct `@embedFile()` note binding plus helper-local command-set, pretty-layout, and section-rendering checks, so it is honest same-source proof for the parked help packet again even while authenticated contents reads still lag",
        "then land the smallest help-local reminder, helper, or focused-test follow-through that the same-source reread actually proves without widening into exec-cmd ownership, symbol-lane parser behavior, or bridge-heavy libbpf routing.",
    ),
    KALLSYMS_SLICE: (
    ...
    HELP_TEST: (
        ...
        "main_cmds.count()",
        'main_cmds.contains("")',
        'main_cmds.contains("trace")',
        "main_cmds.longest()",
        'help.trimCommandPrefix("perf-record.exe", help.default_command_prefix).?',
        'help.trimCommandPrefix("trace2html", help.default_command_prefix)',
        "help.computePrettyLayout(7, 8, 41);",
        "const narrow_layout = help.computePrettyLayout(3, 8, 9);",
        "const empty_layout = help.computePrettyLayout(0, 8, 41);",
        "help.renderPrettyStringList(std.testing.allocator, &cmds, 32);",
        "help.renderCommandSections(",
    ),
    HELP_SOURCE: (
