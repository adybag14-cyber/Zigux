...
FILE_MARKERS: dict[Path, tuple[str, ...]] = {
    HELP_SLICE: (
        ...
        "authenticated GitHub contents reads still lag on some broader Phase 8 files from this environment, but the dedicated help note, helper, focused replay, focused build shard, and `zigux/Makefile` now read directly on current `master`",
        ...
        "then land the smallest help-local reminder, helper, or focused-test follow-through that the same reread actually proves without widening into exec-cmd ownership, symbol-lane parser behavior, or bridge-heavy libbpf routing.",
    ),
    KALLSYMS_SLICE: (
