...
test "sample_missing_dep fixture replay preserves CLI stdout stderr and exit code" {
    const allocator = std.testing.allocator;

    const expected_stdout = try readFixture(allocator, "zigux/tests/fixtures/fixdep/sample_missing_dep_expected.txt");
    defer allocator.free(expected_stdout);
    const expected_stderr = try readFixture(allocator, "zigux/tests/fixtures/fixdep/sample_missing_dep_expected.stderr.txt");
    ...
    const binary_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/tmp/{s}/fixdep-sample-missing-dep-replay",
        .{tmp.sub_path[0..]},
    );
    ...
    try std.testing.expectEqual(std.process.Child.Term{ .exited = 2 }, replay_result.term);
    ...
    try std.testing.expect(std.mem.indexOf(u8, replay_result.stdout, "MISSING_SOURCE") != null);
    try std.testing.expect(std.mem.indexOf(u8, replay_result.stdout, "sample_missing_dep.h") != null);
}
