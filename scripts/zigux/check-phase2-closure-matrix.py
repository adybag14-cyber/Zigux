...
def manifest_expectations(root: Path, module) -> dict[str, tuple[str, ...]]:
    manifest = load_json(module.resolve(root, module.MANIFEST_REL))
    if not isinstance(manifest, dict):
        raise AssertionError("expected manifest root to be a dictionary")
    surfaces = manifest.get("present_surfaces")
    if not isinstance(surfaces, dict):
        raise AssertionError("expected present_surfaces to be a dictionary")
    expectations: dict[str, tuple[str, ...]] = {}
    for key, value in surfaces.items():
        if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
            raise AssertionError(f"expected manifest surface list for {key}")
        expectations[key] = tuple(value)
    return expectations


def collect_external_manifest_issues(module, root: Path, expected_groups: dict[str, tuple[str, ...]]) -> list[tuple[str, str]]:
    manifest = load_json(module.resolve(root, module.MANIFEST_REL))
    if not isinstance(manifest, dict):
    ...
    for key, expected_values in expected_groups.items():
        ...
        if not isinstance(actual, list) or not all(isinstance(item, str) for item in actual):
            ...
            continue
        for marker in expected_values:
            if marker not in actual:
                issues.append(("MISSING_MANIFEST_SURFACE", f"{key}:{marker}"))
    return issues


def combined_issues(module, root: Path, expected_groups: dict[str, tuple[str, ...]]) -> list[tuple[str, str]]:
    issues = list(module.collect_issues(root))
    for issue in collect_external_manifest_issues(module, root, expected_groups):
...
def run_matrix(module, seed_root) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_closure_matrix_") as tmp_dir:
        root = Path(tmp_dir)
        seed_root(root)
        expected_groups = manifest_expectations(root, module)
        if combined_issues(module, root, expected_groups) != []:
        ...
        write_json(manifest_path, payload)
        issues = combined_issues(module, root, expected_groups)
        if ("INVALID_MANIFEST_SHAPE", "present_surfaces") not in issues:
            raise AssertionError(f"missing expected issue ('INVALID_MANIFEST_SHAPE', 'present_surfaces'); saw {issues!r}")
        checks_run += 1
        ...
        issues = combined_issues(module, root, expected_groups)
        if ("INVALID_MANIFEST_SHAPE", "root") not in issues:
            raise AssertionError(f"missing expected issue ('INVALID_MANIFEST_SHAPE', 'root'); saw {issues!r}")
        checks_run += 1
        ...
        for key, expected in expected_groups.items():
            ...
            write_json(manifest_path, payload)
            issues = combined_issues(module, root, expected_groups)
            if ("INVALID_MANIFEST_SHAPE", key) not in issues:
                raise AssertionError(f"missing expected issue ('INVALID_MANIFEST_SHAPE', {key!r}); saw {issues!r}")
            checks_run += 1
            ...
            for marker in expected:
                ...
                write_json(manifest_path, payload)
                issues = combined_issues(module, root, expected_groups)
                expected_issue = ("MISSING_MANIFEST_SURFACE", f"{key}:{marker}")
                if expected_issue not in issues:
                    raise AssertionError(f"missing expected issue {expected_issue!r}; saw {issues!r}")
                checks_run += 1
...
def build_self_test_root(root: Path) -> None:
    ...
    resolve(root, MANIFEST_REL).write_text(json.dumps({
        ...
        "present_surfaces": {
            ...
            "bootstrap_helpers": ["install-a.py", "install-b.py"],
            "policy": ["policy-a.json", "policy-b.json"],
            "archive_support": ["archive-a", "archive-b"],
            "cross_route_support": ["cross-a", "cross-b"],
            "artifact_support": ["artifact-a", "artifact-b"],
            "fixdep_support": ["fixdep-a", "fixdep-b"],
            "make_wrappers": ["make-a", "make-b"],
