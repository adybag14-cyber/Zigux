...
MANIFEST_SURFACE_ATTRS = (
    ...
    ("bootstrap_helpers", "EXPECTED_MANIFEST_BOOTSTRAP_HELPERS"),
    ("policy", "EXPECTED_MANIFEST_POLICY"),
    ("archive_support", "EXPECTED_MANIFEST_ARCHIVE_SUPPORT"),
    ("cross_route_support", "EXPECTED_MANIFEST_CROSS_SUPPORT"),
...
def iter_manifest_surface_expectations(module) -> tuple[tuple[str, tuple[str, ...]], ...]:
    pairs: list[tuple[str, tuple[str, ...]]] = []
    for key, attr_name in MANIFEST_SURFACE_ATTRS:
        if not hasattr(module, attr_name):
            continue
        value = getattr(module, attr_name)
        if not isinstance(value, tuple) or not all(isinstance(item, str) for item in value):
            raise AssertionError(f"validator manifest surface expectation has unsupported shape: {attr_name}")
...
def run_matrix(module, seed_root) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_closure_matrix_") as tmp_dir:
        ...
        for key, expected in iter_manifest_surface_expectations(module):
            seed_root(root)
            manifest_path = module.resolve(root, module.MANIFEST_REL)
            payload = load_json(manifest_path)
