...
def load_policy_contract(root: Path) -> dict[str, object]:
    ...
    return {
        "required_make_routes": tuple(required_make_routes),
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    required_make_routes = policy_contract["required_make_routes"]
    if required_make_routes != EXPECTED_REQUIRED_MAKE_ROUTES:
        issues.append(("INVALID_REQUIRED_MAKE_ROUTES", json.dumps(required_make_routes)))

    archive_target_scope = policy_contract["archive_target_scope"]
    issues.extend(collect_fixture_issues(root, archive_target_scope))
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_cross_makefile_contract_") as tmp_dir:
        ...
        build_sample_root(root)
        path.write_text(
            replace_exact_line(path.read_text(encoding="utf-8"), MAKEFILE_VALIDATE_DEPENDENCY, "phase2-validate: phase2-toolchain phase2-tools"),
            encoding="utf-8",
        )
        ...
        workflow_path.write_text(
            replace_exact_line(workflow_path.read_text(encoding="utf-8"), WORKFLOW_LINES[-1], "run: make -C zigux phase2-toolchain"),
            encoding="utf-8",
        )
...
def main() -> int:
    ...
    print(
        "PHASE2_CROSS_MAKEFILE_CONTRACT_REQUIRED_ROUTE_COUNT="
        f"{len(policy_contract['required_make_routes'])}"
    )
    print(
        "PHASE2_CROSS_MAKEFILE_CONTRACT_TARGET_COUNT="
