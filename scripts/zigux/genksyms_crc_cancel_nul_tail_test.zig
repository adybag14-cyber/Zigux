...
test "runGenksymsCrc escapes visible cancel byte before hiding a NUL tail" {
    const visible_cancel = [_]u8{ 'a', 'l', 'p', 'h', 'a', 0x18 };
    const hidden_tail = [_]u8{ 'h', 'i', 'd', 'd', 'e', 'n', 0x18, 't', 'a', 'i', 'l' };
    const input = visible_cancel ++ [_]u8{0} ++ hidden_tail ++ [_]u8{ '\n', 'n', 'e', 'x', 't', '\n' };

    var capture = try Capture(256).init(std.testing.allocator);
    defer capture.deinit();
    try genksyms_crc.runGenksymsCrc(&input, &capture);

    const visible_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{genksyms_crc.crc32(&visible_cancel)});
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"alpha\\u0018\"") != null);
