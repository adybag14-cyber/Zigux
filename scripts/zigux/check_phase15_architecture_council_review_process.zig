// Ported from check-phase15-architecture-council-review-process.py by port_phase1_guards.py
const std = @import("std");
const Io = std.Io;
const guard = @import("zigux_guard.zig");

pub const pass_marker = "PHASE15_ARCHITECTURE_COUNCIL_REVIEW_PROCESS_SELF_TEST=pass";

const BUILD_GATE_PATH = "zigux/tests/phase15_architecture_council_review_process_build.zig";

const CURRENT_READBACK_MARKER = "current-master-readback-2026-05-19";
...
pub fn runSelfTest(io: Io, allocator: std.mem.Allocator) !u8 {
    ...
    try guard.printLine(io, "PHASE15_ARCHITECTURE_COUNCIL_REVIEW_PROCESS_SELF_TEST_CASE_COUNT={d}", .{@as(usize, 1)});
    return 0;
}
...
pub fn main(init: std.process.Init) !void {
    ...
    if (failures.items.len > 0) {
        try guard.printLine(io, "PHASE15_ARCHITECTURE_COUNCIL_REVIEW_PROCESS_SELF_TEST=fail", .{});
