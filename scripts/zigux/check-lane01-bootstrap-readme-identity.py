...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_lane01_identity_") as tmp_dir:
        ...
        _write(
            root / README_PATH,
            _sample_readme().replace("`zigux-alpha` is the Zigux bootstrap workspace.\n", "", 1),
        )
        errors = check_identity_packet(root)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify the Lane 01 bootstrap README identity packet.")
