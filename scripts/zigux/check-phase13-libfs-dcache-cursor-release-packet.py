...
DOC_PATH = "Documentation/zigux/phase13-libfs-dcache-cursor-release-planner.md"
HELPER_PATH = "fs/libfs_dcache_cursor_release.zig"
REPLAY_PATH = "zigux/tests/phase13_libfs_dcache_cursor_release.zig"
MANIFEST_PATH = "zigux/tests/phase13_libfs_dcache_cursor_release_manifest.json"

EXPECTED_GAPS = {
    "phase13-libfs-dcache-dir-close-release-planner": "starter_landed",
    "phase13-libfs-dcache-cursor-release-review-packet": "starter_landed",
    "phase13-build-gate": "missing_on_current_master",
    "phase13-libfs-live-cursor-unlink": "blocked_on_dcache_state",
    "phase13-libfs-live-cursor-release-lock-ordering": "blocked_on_locking",
}
...
def seed_fixture_tree(root: Path) -> None:
    ...
    manifest = {"gaps": [{"id": gap_id, "status": status} for gap_id, status in EXPECTED_GAPS.items()]}
...
def run_self_test() -> int:
    case_count = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase13_libfs_dcache_cursor_release_") as temp_dir:
        root = Path(temp_dir)
        ...
        assert_only(
            ...
            [
                ...
                "manifest:gap_status_mismatch:phase13-libfs-live-cursor-release-lock-ordering:None",
            ],
    ...
    print("PHASE13_LIBFS_DCACHE_CURSOR_RELEASE_PACKET_SELF_TEST=pass")
    print(f"PHASE13_LIBFS_DCACHE_CURSOR_RELEASE_PACKET_SELF_TEST_CASE_COUNT={case_count}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the Phase 13 libfs dcache cursor release packet stays aligned with its helper, replay, manifest, and note."
    )
    ...
    print("PHASE13_LIBFS_DCACHE_CURSOR_RELEASE_PACKET=pass")
    return 0
