...
ZIG_TOOL = ROOT / "scripts" / "zigux" / "mk_elfconfig_fd_elf64_second_call_test.zig"
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-elf64-second-call-") as tmp_dir:
        ...
        duplicate_path.write_text(
            "\n".join(
                [
                    EXPECTED_MARKERS["elf64_then_elf32"],
                    EXPECTED_MARKERS["elf64_then_elf32"],
                    EXPECTED_MARKERS["double_elf64"],
                    EXPECTED_MARKERS["elf64_then_truncated"],
                ]
            )
            + "\n",
            encoding="utf-8",
    ...
    print("MK_ELFCONFIG_ELF64_SECOND_CALL_MARKERS_SELF_TEST=pass")
    print("MK_ELFCONFIG_ELF64_SECOND_CALL_MARKERS_SELF_TEST_CASE_COUNT=3")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the mk_elfconfig 64-bit second-call replay markers stay present and ordered."
    )
    parser.add_argument(
        "--self-test",
        action="store_true",
    ...
    print("MK_ELFCONFIG_ELF64_SECOND_CALL_MARKERS=pass")
    print(f"MK_ELFCONFIG_ELF64_SECOND_CALL_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
