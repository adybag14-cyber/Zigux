...
test "genksyms bridge treats dot-prefixed values after positionals as request input" {
...
test "genksyms bridge accepts dot-prefixed required option arguments after positionals" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "genksyms bridge keeps dot-prefixed terminator tail out of option parsing" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "genksyms bridge renders dot-prefixed terminator tail in bridge json" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
