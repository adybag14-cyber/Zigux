...
PHASE2_CLOSURE_DOC_MARKERS = (
    ...
    "direct replay owners stay bounded on current `master`: `zig test scripts/zigux/fixdep.zig`, `zig test scripts/zigux/genksyms.zig`, and `zig test scripts/zigux/kconfig/confdata_bridge.zig` remain the shipped direct Phase 2 Zig replays, while the broader artifact-tools packet plus the `scripts/zigux/kconfig/conf_bridge.zig` request-plan evidence stay documented through `zigux/tests/fixtures/phase2_tool_manifest.json`, `zigux/tests/fixtures/phase2_artifact_tools_manifest.json`, `zigux/tests/fixtures/kconfig_bridge/conf_manifest.json`, `zigux/tests/fixtures/kconfig_bridge/confdata_manifest.json`, `zigux/tests/README.md`, and `zigux/Makefile` instead of implying extra standalone bridge or artifact replay entrypoints on current `master`",
)
...
PHASE2_BOOTSTRAP_NOTES_MARKERS = (
    ...
    "the closure note, tests root, and Makefile keep the committed `zigux/tests/fixtures/phase2_tool_manifest.json` plus `zigux/tests/fixtures/phase2_artifact_tools_manifest.json` packet, the bounded direct `zig test scripts/zigux/fixdep.zig`, `zig test scripts/zigux/genksyms.zig`, and `zig test scripts/zigux/kconfig/confdata_bridge.zig` replays, and the checker-backed `scripts/zigux/kconfig/conf_bridge.zig` request-plan packet reviewable without reopening the dedicated genksyms or kconfig lanes from this bootstrap note",
    "the active Phase 2 closure note and tests root keep the validator-routed direct `zig test scripts/zigux/fixdep.zig`, `zig test scripts/zigux/genksyms.zig`, and `zig test scripts/zigux/kconfig/confdata_bridge.zig` replays explicit beside the same bounded Phase 2 tools and kconfig routes",
