...
def run_self_test() -> None:
    ...
    marker_cases = [
        ...
        (
            "review_checklist_help_packet_marker",
            REVIEW_CHECKLIST_PATH,
            "if the change touches the parked Phase 8 `help` packet,",
            "if the change touches the shared Phase 8 `help` packet,",
            f"{REVIEW_CHECKLIST_PATH}: if the change touches the parked Phase 8 `help` packet,",
        ),
        (
            "review_checklist_help_test_route_marker",
            REVIEW_CHECKLIST_PATH,
            "`make -C zigux phase8-help-test`",
            "`make -C zigux phase8-help-review`",
            f"{REVIEW_CHECKLIST_PATH}: `make -C zigux phase8-help-test`",
        ),
        (
        ...
        (
            "review_checklist_libbpf_shard_routes_marker",
            REVIEW_CHECKLIST_PATH,
            "`scripts/zigux/check-phase8-libbpf-shard-routes.py`",
            "`scripts/zigux/check-phase8-libbpf-route-checks.py`",
            f"{REVIEW_CHECKLIST_PATH}: `scripts/zigux/check-phase8-libbpf-shard-routes.py`",
        ),
        (
            "review_checklist_cpu_mask_test_route_marker",
            REVIEW_CHECKLIST_PATH,
            "`make -C zigux phase8-cpu-mask-test`",
            "`make -C zigux phase8-cpu-mask-review`",
            f"{REVIEW_CHECKLIST_PATH}: `make -C zigux phase8-cpu-mask-test`",
        ),
        (
        ...
        (
            ...
            "`scripts/zigux/check-phase8-libbpf-segment-gate.py`",
            "`scripts/zigux/check-phase8-libbpf-gate.py`",
            f"{DOCS_ROOT_PATH}: `scripts/zigux/check-phase8-libbpf-segment-gate.py`",
        ),
        (
        ...
        (
            "tests_readme_help_test_route_marker",
            TESTS_README_PATH,
            "`make -C zigux phase8-help-test`",
            "`make -C zigux phase8-help-review`",
            f"{TESTS_README_PATH}: `make -C zigux phase8-help-test`",
        ),
        (
            "tests_readme_cpu_mask_test_route_marker",
            TESTS_README_PATH,
            "`make -C zigux phase8-cpu-mask-test`",
            "`make -C zigux phase8-cpu-mask-review`",
            f"{TESTS_README_PATH}: `make -C zigux phase8-cpu-mask-test`",
        ),
        (
        ...
        (
            "boundary_survey_config_helper_marker",
            BOUNDARY_SURVEY_PATH,
            "`mapReuseObservationFromFdinfo()`",
            "`observeMapReuseFromFdinfo()`",
            f"{BOUNDARY_SURVEY_PATH}: `mapReuseObservationFromFdinfo()`",
        ),
        (
            "boundary_survey_reuse_planning_marker",
            BOUNDARY_SURVEY_PATH,
            "`resolveReusePinnedMapAttempt()`",
            "`planReusePinnedMapAttempt()`",
            f"{BOUNDARY_SURVEY_PATH}: `resolveReusePinnedMapAttempt()`",
        ),
        (
            "boundary_survey_token_planning_marker",
            BOUNDARY_SURVEY_PATH,
            "`planTokenPreparation()`",
            "`planTokenBridgePreparation()`",
            f"{BOUNDARY_SURVEY_PATH}: `planTokenPreparation()`",
        ),
        (
            "boundary_survey_capability_boundary_marker",
            BOUNDARY_SURVEY_PATH,
            "token materialization or capability handoff",
            "token setup or capability relay",
            f"{BOUNDARY_SURVEY_PATH}: token materialization or capability handoff",
        ),
        (
            "boundary_survey_bpffs_boundary_marker",
            BOUNDARY_SURVEY_PATH,
            "map reopen or bpffs compatibility closure",
            "map reopen or bpffs parity closure",
            f"{BOUNDARY_SURVEY_PATH}: map reopen or bpffs compatibility closure",
        ),
        (
        ...
        (
            "makefile_cpu_mask_test_route_marker",
            MAKEFILE_PATH,
            "phase8-cpu-mask-test:",
            "phase8-cpu-mask-review:",
        ...
        (
            "libbpf_segment_survey_libbpf_segments_route_marker",
            LIBBPF_SEGMENT_SURVEY_PATH,
            "`make -C zigux phase8-libbpf-segments-test`",
            "`make -C zigux phase8-libbpf-segments-review`",
            f"{LIBBPF_SEGMENT_SURVEY_PATH}: `make -C zigux phase8-libbpf-segments-test`",
        ),
        (
            "libbpf_segment_survey_libbpf_segments_build_route_marker",
            LIBBPF_SEGMENT_SURVEY_PATH,
            "`zig build test --build-file zigux/tests/phase8_libbpf_segments_only_build.zig --summary all`",
            "`zig build test --build-file zigux/tests/phase8_libbpf_segments_build.zig --summary all`",
            f"{LIBBPF_SEGMENT_SURVEY_PATH}: `zig build test --build-file zigux/tests/phase8_libbpf_segments_only_build.zig --summary all`",
        ),
        (
            "libbpf_segment_survey_perf_buffer_poll_route_marker",
            LIBBPF_SEGMENT_SURVEY_PATH,
            "`make -C zigux phase8-perf-buffer-poll-test`",
            "`make -C zigux phase8-perf-buffer-poll-review`",
            f"{LIBBPF_SEGMENT_SURVEY_PATH}: `make -C zigux phase8-perf-buffer-poll-test`",
        ),
        (
            "libbpf_segment_survey_perf_buffer_poll_build_route_marker",
            LIBBPF_SEGMENT_SURVEY_PATH,
            "`zig build test --build-file zigux/tests/phase8_perf_buffer_poll_only_build.zig --summary all`",
            "`zig build test --build-file zigux/tests/phase8_perf_buffer_poll_build.zig --summary all`",
            f"{LIBBPF_SEGMENT_SURVEY_PATH}: `zig build test --build-file zigux/tests/phase8_perf_buffer_poll_only_build.zig --summary all`",
        ),
        (
            "libbpf_segment_survey_shared_phase8_route_marker",
            LIBBPF_SEGMENT_SURVEY_PATH,
            "`make -C zigux phase8-test`",
            "`make -C zigux phase8-review`",
            f"{LIBBPF_SEGMENT_SURVEY_PATH}: `make -C zigux phase8-test`",
        ),
        (
            "libbpf_segment_survey_shared_phase8_build_route_marker",
            LIBBPF_SEGMENT_SURVEY_PATH,
            "`zig build test --build-file zigux/tests/phase8_build.zig --summary all`",
            "`zig build test --build-file zigux/tests/phase8_review_build.zig --summary all`",
            f"{LIBBPF_SEGMENT_SURVEY_PATH}: `zig build test --build-file zigux/tests/phase8_build.zig --summary all`",
        ),
        (
