#!/usr/bin/env python3
"""Fail-close guard for the Lane 05 Zig toolchain contract."""
...
SCRIPT_MARKERS = (
    ...
    'expected_archive_metadata(archive_target)',
...
def require_exact_count(text: str, marker: str, expected: int, label: str) -> None:
    ...
    if actual != expected:
        raise ValueError(f"expected {expected} occurrences of {label} {marker}, found {actual}")
...
def validate_policy(root: Path) -> tuple[str, str]:
    ...
    if channel != EXPECTED_CHANNEL:
        raise ValueError(f"unexpected policy channel in {POLICY_REL}: {channel!r}")

    archives = payload.get("archive_sha256")
    if not isinstance(archives, dict) or list(archives.keys()) != [EXPECTED_TARGET]:
        raise ValueError(f"unexpected archive_sha256 target set in {POLICY_REL}")
    ...
    if archive_target_scope != [EXPECTED_TARGET]:
        raise ValueError(f"unexpected archive_target_scope in {POLICY_REL}: {archive_target_scope!r}")

    required_make_routes = upgrade_policy.get("required_make_routes")
    if required_make_routes != EXPECTED_REQUIRED_MAKE_ROUTES:
        raise ValueError(
            f"unexpected required_make_routes in {POLICY_REL}: {required_make_routes!r}"
        )

    return EXPECTED_TARGET, channel
...
def validate_script(root: Path) -> int:
    ...
    require_exact_count(
        ...
        'add_search_root(root / ".zig-toolchain")',
        1,
        "repo-local search-root marker",
    )
    require_exact_count(
        text,
        'add_search_root(root / "third_party")',
        1,
        "archive search-root marker",
    )
    require_exact_count(
        text,
        'add_search_root(root / "agent_files")',
...
def validate_workflow(root: Path) -> int:
    ...
    require_exact_count(
        ...
        'python3 scripts/zigux/check-zig-toolchain.py --archive-only --allow-missing',
        1,
        "workflow archive-only allow-missing call",
    )
    require_exact_count(
        text,
        'python3 scripts/zigux/check-zig-toolchain.py --archive-only --archive "$repo_archive_path" --archive-target "$ZIGUX_ZIG_TARGET"',
        ...
        json.dumps(
            {
                ...
                "archive_sha256": {EXPECTED_TARGET: "313b231e76f3cc9b718044602dbc3c42b531693507203a6baf2fa892c9533e77"},
                "upgrade_policy": {
...
def run_self_test() -> int:
    cases = [
        ("baseline", None),
        (
            "missing_script_search_root",
            lambda root: (root / SCRIPT_REL).write_text(
                load_text(root, SCRIPT_REL).replace(
                    'add_search_root(root / "agent_files")\n', "", 1
                ),
                encoding="utf-8",
            ),
    ...
    for name, mutate in cases:
        with tempfile.TemporaryDirectory(prefix="lane05-zig-toolchain-contract-") as tmp:
            root = Path(tmp)
            write_fixture(root)
    ...
    print(f"LANE05_ZIG_TOOLCHAIN_CONTRACT_SELF_TEST_CASE_COUNT={len(cases)}")
    return 0
