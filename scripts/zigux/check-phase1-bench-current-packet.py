...
def collect_issues(root: Path) -> list[str]:
    ...
    for relative_path, markers in MARKERS.items():
        ...
        block_lines = {
            line for block in EXPECTED_ASSERT_BLOCKS.get(relative_path, ()) for line in block
        }
        if relative_path == WORKFLOW_REL:
            lines = text.splitlines()
            for marker in markers:
                if marker in block_lines:
                    continue
...
def build_sample_file(relative_path: str) -> str:
    ...
    lines = [marker for marker in MARKERS[relative_path] if marker not in block_lines]
