...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase1-bench-current-packet-duplicate-first-line-") as tmpdir:
        root = Path(tmpdir)
        build_sample_repo(root)
        path = root / BENCH_CHECKER_REL
        text = path.read_text(encoding="utf-8")
        first_line = "status_mismatch_output = ok_output.replace("
        path.write_text(text.replace(first_line, f"{first_line}\n{first_line}", 1), encoding="utf-8")
        issues = collect_issues(root)
        expected = f"{BENCH_CHECKER_REL}:marker_count:{first_line}:expected=1:actual=2"
        if issues != [expected]:
            print("PHASE1_BENCH_CURRENT_PACKET_SELF_TEST=fail")
            print("case=duplicate_section_start_fails_closed")
            print(f"actual={issues!r}")
            return 1
