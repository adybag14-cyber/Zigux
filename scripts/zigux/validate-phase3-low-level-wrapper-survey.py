...
REQUIRED_MARKERS = {
    NOTE_PATH: (
        ...
        "Reviewers should treat the low-level wrapper family as materially landed as a bounded packet on current `master`: one atomic helper shard, one barrier helper companion, one MMIO helper companion, one directly readable unsafe-policy companion, the shared narrow-unsafe decoder, the dedicated survey validator, one focused low-level-wrapper replay shard, one dedicated shared build companion, and one direct replay command are directly readable, while the broader shared Phase 3 ABI checker and catalog routes remain separate gaps.",
        "Current `master` now separately ships the adjacent shared Phase 3 validator entrypoint through `scripts/zigux/validate-phase3.py` and the starter export/UAPI boundary packet through `Documentation/zigux/phase3-export-uapi-boundary-survey.md`, `scripts/zigux/validate-phase3-export-uapi-survey.py`, and `zigux/tests/phase3_export_uapi_layout.zig` plus `zigux/tests/phase3_export_uapi_layout_build.zig`, so this wrapper note should treat those surfaces as neighboring landed packets rather than as missing same-lane gaps.",
    ),
    ABI_SLICE_PATH: (
