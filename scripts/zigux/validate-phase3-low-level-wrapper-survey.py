...
REQUIRED_MARKERS = {
    ...
    ABI_SLICE_PATH: (
        ...
        "That reminder surface keeps one directly readable MMIO helper companion, the directly coupled helper-local `zigux/helpers/unsafe_policy.zig` companion, the manifest-backed shared ABI inventory companion, the shared validator entrypoint, the shared ABI checker, the Phase 3 catalog helper, the export/UAPI survey validator, the dedicated survey validator, the focused replay shard, and the dedicated shared build companion explicit without implying that the broader catalog-selftest guard or linux-zigux header-governance note already ship.",
        "`zig build phase3-low-level-wrappers-test --build-file zigux/tests/phase3_low_level_wrappers_build.zig`",
    ),
    CHECKER_PATH: (
