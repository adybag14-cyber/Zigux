#!/usr/bin/env python3
"""Fail closed when the validator manifest-surface coverage checker drifts."""
...
VALIDATOR_COVERAGE_CHECKER_REL = Path(
    "scripts/zigux/check-phase2-validator-manifest-surface-coverage.py"
)
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    checker_path = resolve(root, VALIDATOR_COVERAGE_CHECKER_REL)
    if not checker_path.exists():
        return [("MISSING_REQUIRED_FILE", VALIDATOR_COVERAGE_CHECKER_REL.as_posix())]
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS=fail")
...
"""
    write_text(resolve(root, VALIDATOR_COVERAGE_CHECKER_REL), content)


def run_self_test() -> int:
    checks_run = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_validator_manifest_surface_marker_counts_") as tmp_dir:
        root = Path(tmp_dir)
        ...
        assert (
            "DUPLICATE_SURFACE_KEY",
            "EXPECTED_CONSTANT_NAMES:artifact_support",
        ...
        assert any(
            code == "DUPLICATE_MARKER" and "scripts/zigux/check-phase2-cross.py" in value
            for code, value in collect_issues(root)
        )
        checks_run += 1

        build_sample_root(root)
        resolve(root, VALIDATOR_COVERAGE_CHECKER_REL).unlink()
        assert (
            "MISSING_REQUIRED_FILE",
            VALIDATOR_COVERAGE_CHECKER_REL.as_posix(),
        ) in collect_issues(root)
        checks_run += 1

    print("PHASE2_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SELF_TEST=pass")
    print(f"PHASE2_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SELF_TEST_CASE_COUNT={checks_run}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check the validator manifest-surface coverage checker for duplicate or missing surface markers."
    )
    ...
    if args.write_sample_root is not None:
        ...
        print(
            "PHASE2_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SAMPLE_ROOT="
    ...
    print("PHASE2_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS=pass")
    print(f"PHASE2_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SURFACE_COUNT={len(EXPECTED_SURFACE_ANCHORS)}")
    return 0
