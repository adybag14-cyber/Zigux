#!/usr/bin/env python3
"""Fail closed when the shared-validator manifest-surface coverage checker drifts."""
...
COVERAGE_CHECKER_REL = Path("scripts/zigux/check-phase2-validator-manifest-surface-coverage.py")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check the shared-validator manifest-surface coverage checker for duplicate or missing surface markers."
    )
    ...
    if args.write_sample_root is not None:
        ...
        print(f"PHASE2_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SAMPLE_ROOT={args.write_sample_root.resolve()}")
        return 0
