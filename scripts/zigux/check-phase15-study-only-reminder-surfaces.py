...
LANE_SEQ_MARKERS = (
    ...
    "they do not own freeze-map status decisions themselves",
...
Phase 9 notes
* the current docs-root Phase 9 reminder packet should stay parked on `Documentation/zigux/freeze-map.md`, `Documentation/zigux/phase15-study-only-anchor-accounting.md`, and `scripts/zigux/check-phase9-freeze-map-study-boundaries.py`
...
Keep the current bounded Phase 15 governance reminder explicit through `Documentation/zigux/phase15-study-only-anchor-accounting.md` and `scripts/zigux/check-phase15-review-checklist-study-only-alignment.py`.
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="phase15_study_only_surfaces_") as tmpdir:
        ...
        cases = (
            ...
            (SCRIPTS_README_REL, SCRIPTS_README_MARKERS[2]),
            (TESTS_README_REL, TESTS_README_MARKERS[3]),
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify that the Phase 15 study-only reminder surfaces stay aligned with the live freeze-map packet."
    )
    parser.add_argument("--root", type=Path, default=Path.cwd(), help="repository root")
    parser.add_argument("--self-test", action="store_true", help="run the synthetic checker self-test")
    ...
    if failures:
        print("PHASE15_STUDY_ONLY_REMINDER_SURFACES=fail")
        for failure in failures:
            print(f"ERROR: {failure}")
        return 1

    print("PHASE15_STUDY_ONLY_REMINDER_SURFACES=pass")
    print(f"PHASE15_STUDY_ONLY_REMINDER_SURFACES_REQUIRED_PATH_COUNT={len(REQUIRED_FILES)}")
    print("PHASE15_STUDY_ONLY_REMINDER_SURFACES_ANCHOR_COUNT=2")
    return 0
