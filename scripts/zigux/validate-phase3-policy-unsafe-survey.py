...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_policy_unsafe_validator_") as tmp_dir:
        ...
        build_valid_workspace(root)
        missing_next_step = (root / SURVEY_REL).read_text(encoding="utf-8").replace("- `PHASE3_NEXT_BOUNDED_STEP=keep-this-survey-aligned-with-the-live-helper-roles-and-shared-abi-markers-until-a-real-policy-or-unsafe-helper-expansion-lands`\n", "", 1)
        write_file(root / SURVEY_REL, missing_next_step)
        issues = validate(root)
        assert "missing_marker:PHASE3_NEXT_BOUNDED_STEP=keep-this-survey-aligned-with-the-live-helper-roles-and-shared-abi-markers-until-a-real-policy-or-unsafe-helper-expansion-lands" in issues
