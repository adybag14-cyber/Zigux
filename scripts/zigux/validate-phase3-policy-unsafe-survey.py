...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_policy_unsafe_validator_") as tmp_dir:
        ...
        build_valid_workspace(root)
        missing_validate_gate = (root / SURVEY_REL).read_text(encoding="utf-8").replace("- `PHASE3_VALIDATE_GATE=python3 scripts/zigux/validate-phase3.py --slug abi`\n", "", 1)
        write_file(root / SURVEY_REL, missing_validate_gate)
        issues = validate(root)
        assert "missing_marker:PHASE3_VALIDATE_GATE=python3 scripts/zigux/validate-phase3.py --slug abi" in issues
