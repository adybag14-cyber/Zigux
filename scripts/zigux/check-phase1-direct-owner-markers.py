...
EXPECTED_BITMAP_REVIEW_PACKET_SUMMARY = (
    ...
    "logical short-circuit coverage, exact-word-boundary equality fast-path masking, tail-masked "
    "predicate behavior, out-of-range tail-bit full or empty or weight masking, caller-window xor "
    "and or clamping, multiword-tail xor and or clamp witnesses, weighted tail-count clamping, "
    "terminator-only and zero-length caller-view formatting, empty-bitmap caller-buffer "
    "preservation, Linux-style alias mirror coverage, and allocator optional-reset coverage."
)
EXPECTED_BITMAP_NEXT_SAFE_STEP_NOTE = (
    ...
    "equality fast-path masking, caller-window xor and or clamp, weighted tail-count "
    "clamp, empty-buffer, allocator-reset, zero-bit logical short-circuit, and Linux-style "
    "alias mirror anchors here, and if the separate bitmap closure-validator anchor-sync "
...
EXPECTED_FIND_BIT_NEXT_SAFE_STEP_NOTE = (
    ...
    "entry points, or tail-word skip anchors, or committed tail-clamped or "
    "tail-inclusive-boundary replay drift; do not reopen older saved validator cues "
