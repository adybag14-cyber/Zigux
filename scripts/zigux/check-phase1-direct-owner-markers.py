...
REQUIRED_EXACT_LINES = {
    ...
    LANE_NOTE_REL: [
        ...
        "- `PHASE1_RBTREE_NEXT_SAFE_STEP=rbtree reopens only to keep the already-landed cached_leftmost_return_serials shared replay aligned across the manifest, direct-owner note, and any shared parity gates, or for drift inside the still-helper-local ordered Linux-style alias proof, dedicated low_level_alias_anchor, cached-root insert-miss, leftmost-sync, cached-root alias, singleton-erase, replacement, detach, and reseed anchors; do not widen cached-root follow-through beyond the existing direct packet until another committed shared replay field lands`",
        "- `PHASE1_STRING_NEXT_SAFE_STEP=string stays parked unless a fresh reread finds drift in the helper-local sysfs review packet, counted-search and search-length packet, memparse safety, matched-prefix and suffix-boundary packet, embedded-NUL trim preservation, moving-earliest-dirty-byte memchrInv coverage, or committed replaceChar parity bytes or current string fixture keys; do not reopen missing closure-side validator names by default`",
        "- current lane split reminder: the shared replay helper family is parked on argv_split, cmdline, ctype, hweight, list_sort, slab, str_error_r, vsprintf, and zalloc, while only bitmap, find_bit, rbtree, and string still carry bounded direct helper-local follow-up anchors on current `master`",
