...
EXPECTED_BITMAP_REVIEW_PACKET_SUMMARY = (
    ...
    "copy/copy-clear-tail/copy-and-extend replay, scnprintf output, truncation, tiny-buffer "
    "handling, logical operator outputs, range set/clear/fill/zero outcomes, and partial-window "
    "xor replay, while current master keeps the direct helper-local bitmap packet bounded to "
    "whole-word range edges, raw copy alias behavior, tail-clearing and extension semantics, "
    "zero and aligned copyAndExtend handling, zero-sized destination-view no-op coverage, "
    ...
    "caller-window xor and or clamping, multiword-tail xor and or clamp witnesses, weighted "
    "tail-count clamping, terminator-only and zero-length caller-view formatting, empty-bitmap "
    "caller-buffer preservation, Linux-style alias mirror coverage, and allocator optional-reset "
    "coverage."
)
EXPECTED_BITMAP_NEXT_SAFE_STEP_NOTE = (
    ...
    "copy, logical, range, allocation, formatting, or partial-window parity fields; current master "
    "still ships direct fill-tail clamp, raw copy alias, cross-word scnprintf, "
    "exact-word-boundary equality fast-path masking, caller-window xor and or clamp, weighted "
    "tail-count clamp, empty-buffer, allocator-reset, zero-bit logical short-circuit, and "
    "Linux-style alias mirror anchors here; do not reopen older closure-side or validator-route "
    "cue names by default."
)
EXPECTED_FIND_BIT_NEXT_SAFE_STEP_NOTE = (
    ...
    "`next_after_word`, `first_zero`, `next_zero`, `first_and`, `next_and`, or `last` fixture "
    "keys; do not reopen older saved validator cues or neighboring helper families."
)
