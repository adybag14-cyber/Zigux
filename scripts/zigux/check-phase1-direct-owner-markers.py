...
NEXT_STEP_MARKERS = [
    ...
    "- `PHASE1_FIND_BIT_NEXT_SAFE_STEP=find_bit reopens only for direct-anchor drift inside same-word start-mask, inclusive-boundary, zero-window, zero-sized short-circuit, past-nbits, underscore-alias, Linux-style alias, or tail-word skip anchors, or for committed tail-clamped replay drift`",
...
EXPECTED_MANIFEST_NEXT_SAFE_STEPS = {
    ...
    "tools/lib/rbtree.zig": "If this helper lane reopens, the smallest shared-replay expansion is now the dedicated cached-root leftmost-return fixture key; until then, cached-root leftmost-return and singleton-erase behavior stay owned by direct helper-local anchors.",
