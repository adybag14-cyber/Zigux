...
test "runFixdep preserves plus signs in target output" {
    ...
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "zigux_fixdep_target_plus_source.c",
        .data = "CONFIG_ZIGUX_TARGET_PLUS_SOURCE\n",
    });

    const source_path = try std.fmt.allocPrint(
        std.testing.allocator,
        ".zig-cache/tmp/{s}/zigux_fixdep_target_plus_source.c",
        .{tmp.sub_path[0..]},
    );
    ...
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "target_plus.d",
        .data = depfile_bytes,
    });

    const depfile_path = try std.fmt.allocPrint(
        std.testing.allocator,
        ".zig-cache/tmp/{s}/target_plus.d",
        .{tmp.sub_path[0..]},
    );
    ...
    const cmdline = try std.fmt.allocPrint(
        std.testing.allocator,
        "cc -DZIGUX_TARGET_PLUS=1 -c {s} -o {s}",
        .{ source_path, target },
    );
    ...
    const expected = try std.fmt.allocPrint(
        ...
        "savedcmd_{s} := {s}\n\n" ++
            "source_{s} := {s}\n\n" ++
            "deps_{s} := \\\n" ++
            "    $(wildcard include/config/ZIGUX_TARGET_PLUS_SOURCE) \\\n" ++
            "\n" ++
            "{s}: $(deps_{s})\n\n" ++
            "$(deps_{s}):\n",
