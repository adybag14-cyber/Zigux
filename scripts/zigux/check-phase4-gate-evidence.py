...
def collect_exact_workflow_run_count_markers(
    workflow: str, gate_evidence: str
) -> list[str]:
    missing: list[str] = []
    ...
    for command, expected_count in EXACT_WORKFLOW_RUN_COUNT_EXPECTATIONS.items():
        ...
        if actual_count != expected_count:
            missing.append(
                f"workflow_exact_count:{command}:{actual_count}:{expected_count}"
            )
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase4_gate_evidence_") as tmp_dir:
        ...
        missing = validate_root(root)
        assert "phase4_gate_evidence:PHASE4_GATE_EVIDENCE_CHECK=pass" in missing, missing
        ...
        missing = validate_root(root)
        assert "phase4_gate_evidence:phase4_test_fsmount_survey.zig" in missing, missing
        ...
        missing = validate_root(root)
        assert "phase4_gate_evidence:Self-test Phase 4 validator" in missing, missing
        ...
        gate_evidence.write_text(
            gate_evidence.read_text(encoding="utf-8").replace(
                "one `make -C zigux phase4-validate` run line",
                "two `make -C zigux phase4-validate` run lines",
                1,
            ),
            encoding="utf-8",
        )
        ...
        missing = validate_root(root)
        assert (
            "workflow_exact_count:make -C zigux phase4-test:2:1" in missing
        ), missing
        ...
        gate_evidence.write_text(
            gate_evidence.read_text(encoding="utf-8").replace(
                ...
                "PHASE4_VALIDATION_MATRIX_BLOB_SHA=broken",
                1,
            ),
            encoding="utf-8",
        )
        missing = validate_root(root)
        assert any(
            marker.startswith("phase4_gate_evidence:PHASE4_VALIDATION_MATRIX_BLOB_SHA:")
            for marker in missing
        ), missing
        ...
        gate_evidence.write_text(
            gate_evidence.read_text(encoding="utf-8").replace(
                ...
                "PHASE4_VALIDATOR_BLOB_SHA=broken",
                1,
            ),
            encoding="utf-8",
        )
        missing = validate_root(root)
        assert any(
            marker.startswith("phase4_gate_evidence:PHASE4_VALIDATOR_BLOB_SHA:")
            for marker in missing
        ), missing
        ...
        missing = validate_root(root)
        assert any(
            marker.startswith("phase4_gate_evidence:PHASE4_WORKFLOW_BLOB_SHA:")
            for marker in missing
        ), missing
        ...
        gate_evidence.write_text(
            gate_evidence.read_text(encoding="utf-8").replace(
                ...
                "PHASE4_SCRIPT_README_BLOB_SHA=broken",
                1,
            ),
            encoding="utf-8",
        )
        missing = validate_root(root)
        assert any(
            marker.startswith("phase4_gate_evidence:PHASE4_SCRIPT_README_BLOB_SHA:")
            for marker in missing
        ), missing
        ...
        gate_evidence.write_text(
            gate_evidence.read_text(encoding="utf-8").replace(
                ...
                "PHASE4_PERF_BASELINE_MANIFEST_BLOB_SHA=broken",
                1,
            ),
            encoding="utf-8",
        )
        ...
        gate_evidence.write_text(
            gate_evidence.read_text(encoding="utf-8").replace(
                ...
                "PHASE4_RUNTIME_ATOMIC64_SURVEY_BLOB_SHA=broken",
                1,
            ),
            encoding="utf-8",
        )
        ...
        assert any(
            marker.startswith(
                "phase4_gate_evidence:PHASE4_RUNTIME_ATOMIC64_SURVEY_BLOB_SHA:"
            )
            for marker in missing
        ), missing
