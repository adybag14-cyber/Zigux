    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_cross_policy_fixture_contract_") as tmpdir:
        ...
        cases = [
            ...
            (
                ...
                lambda: write_text(
                    resolve_path(root, POLICY),
                    '{"archive_sha256":{"x86_64-linux":"one","x86_64-linux":"two"},"upgrade_policy":{"archive_target_scope":["x86_64-linux"],"required_make_routes":["phase2-toolchain","phase2-tools","phase2-validate","phase2-cross"]}}\n',
                ),
                "duplicate json key",
            ),
            (
        ...
        case_count = 0
        for _name, mutate, expected_fragment in cases:
            build_passing_root(root)
            mutate()
            try:
                ...
                if expected_fragment not in str(exc):
                    print(
                        "PHASE2_CROSS_POLICY_FIXTURE_CONTRACT_SELF_TEST_FAIL="
                        f"{_name}:expected={expected_fragment}:actual={exc}"
                    )
                    return 1
            case_count += 1
