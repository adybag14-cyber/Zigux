...
test "standalone confdata first line bom is trimmed before nul truncation" {
    var capture = try TestCapture.init(std.testing.allocator);
    defer capture.deinit();

    try confdata_bridge.runConfdataBridge(
        std.testing.allocator,
        "\xef\xbb\xbfCONFIG_ALPHA=y\x00CONFIG_ALPHA=m\n" ++
            "CONFIG_AFTER=m\n",
        &capture,
    );
...
test "standalone confdata first line bom does not hide a nul-truncated malformed quote" {
    var capture = try TestCapture.init(std.testing.allocator);
    defer capture.deinit();

    try confdata_bridge.runConfdataBridge(
        std.testing.allocator,
        "\xef\xbb\xbfCONFIG_TITLE=\"zig\x00ux\"\n" ++
            "CONFIG_AFTER=y\n",
        &capture,
    );
