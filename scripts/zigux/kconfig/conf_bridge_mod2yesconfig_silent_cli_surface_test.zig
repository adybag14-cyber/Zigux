...
test "conf bridge mod2yesconfig accepts generic silent option through cli" {
    const allocator = std.testing.allocator;
    const cwd = std.Io.Dir.cwd();
    try cwd.createDirPath(std.testing.io, ".zig-cache/tmp");

    const exe_path = ".zig-cache/tmp/conf_bridge_mod2yesconfig_silent_test";
    const build_args = [_][]const u8{
        ...
        "-femit-bin=" ++ exe_path,
        "--cache-dir",
        ".zig-cache/conf-bridge-mod2yesconfig-silent-build",
        "--global-cache-dir",
        ".zig-cache/conf-bridge-mod2yesconfig-silent-build/global",
    };
    ...
    try std.testing.expect(contains(run_result.stdout, "\"mode\":\"mod2yesconfig\""));
    try std.testing.expect(contains(run_result.stdout, "\"argv\":[\"scripts/kconfig/conf\",\"--silent\",\"--mod2yesconfig\",\"Kconfig\"]"));
    try std.testing.expect(contains(run_result.stdout, "\"ARCH\":\"x86\""));
    try std.testing.expect(contains(run_result.stdout, "\"KCONFIG_CONFIG\":\"promote/.config\""));
