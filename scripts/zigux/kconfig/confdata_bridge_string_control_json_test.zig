...
test "phase2 confdata bridge keeps quoted-string control-byte json escaping when trailing suffix bytes follow the closing quote" {
    const Capture = struct {
        list: std.ArrayList(u8),
        allocator: std.mem.Allocator,
    ...
    try confdata_bridge.runConfdataBridge(
        std.testing.allocator,
        "CONFIG_BANNER=\"prefix\x01mid\x0bend\"suffix_noise\n",
        &capture,
    );
