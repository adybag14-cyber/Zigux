...
test "savedefconfig mode argument remains before later bridge options" {
    ...
    const exe_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/tmp/{s}/conf_bridge_savedefconfig_options",
        .{tmp.sub_path[0..]},
    );
    ...
    const result = try run(allocator, &.{
        ...
        "saved/.config",
        "x86_64",
        "out/generated/savedefconfig=debug",
        "silent",
    });
    ...
    try std.testing.expect(std.mem.indexOf(
        u8,
        result.stdout,
        "\"argv\":[\"scripts/kconfig/conf\",\"--silent\",\"--savedefconfig\",\"out/generated/savedefconfig=debug\",\"Kconfig\"]",
    ) != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"mode\":\"savedefconfig\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"ARCH\":\"x86_64\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"KCONFIG_CONFIG\":\"saved/.config\"") != null);
