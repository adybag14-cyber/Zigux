...
const conf_bridge_exe = ".zig-cache/tmp/conf_bridge_oldconfig_silent_cli_surface_test_bin";
...
test "oldconfig silent CLI emits stable bridge packet" {
    ...
    try expectExitedZero(result);
    try std.testing.expectEqual(@as(usize, 0), result.stderr.len);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"mode\":\"oldconfig\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"argv\":[\"scripts/kconfig/conf\",\"--silent\",\"--oldconfig\",\"Kconfig\"]") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"ARCH\":\"powerpc\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"KCONFIG_CONFIG\":\"out/old/.config\"") != null);
