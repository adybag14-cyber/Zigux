...
const conf_bridge_exe = ".zig-cache/tmp/conf_bridge_helpnewconfig_silent_cli_surface_test_bin";
...
test "helpnewconfig silent CLI emits stable bridge packet" {
    ...
    try expectExitedZero(result);
    try std.testing.expectEqual(@as(usize, 0), result.stderr.len);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"mode\":\"helpnewconfig\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"argv\":[\"scripts/kconfig/conf\",\"--silent\",\"--helpnewconfig\",\"Kconfig\"]") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"ARCH\":\"s390\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.stdout, "\"KCONFIG_CONFIG\":\"out/help/.config\"") != null);
