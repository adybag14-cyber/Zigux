...
test "confdata bridge preserves prior duplicate state after malformed quoted duplicate assignments" {
    ...
    try std.testing.expectEqualStrings("tristate", summary.entries[2].kind.text());
    try std.testing.expectEqualStrings("y", summary.entries[2].value);
}

test "confdata bridge emits preserved duplicate state after malformed quoted duplicate assignments" {
    const allocator = std.testing.allocator;
    const input =
        \\CONFIG_ALPHA="stable"
        \\CONFIG_ALPHA="unterminated
