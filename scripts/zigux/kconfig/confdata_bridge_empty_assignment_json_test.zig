...
test "confdata bridge keeps explicit empty assignments distinct from quoted empties in parsed entries" {
...
test "confdata bridge emits explicit empty assignments distinctly in json output after unset override" {
    const allocator = std.testing.allocator;
    const input =
        \\CONFIG_EMPTY=
        \\CONFIG_QUOTED=""
