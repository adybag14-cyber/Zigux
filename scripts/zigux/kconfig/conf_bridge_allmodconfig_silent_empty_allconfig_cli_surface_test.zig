...
test "allmodconfig silent empty allconfig CLI emits explicit override packet" {
    const allocator = std.testing.allocator;
    const exe_path = ".zig-cache/tmp/conf_bridge_allmodconfig_silent_empty_allconfig_cli_surface_test_bin";
    ...
    try std.testing.expect(std.mem.indexOf(u8, run_result.stdout, "\"mode\":\"allmodconfig\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, run_result.stdout, "\"argv\":[\"scripts/kconfig/conf\",\"--silent\",\"--allmodconfig\",\"Kconfig\"]") != null);
    try std.testing.expect(std.mem.indexOf(u8, run_result.stdout, "\"ARCH\":\"arm\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, run_result.stdout, "\"KCONFIG_CONFIG\":\"mod/.config\"") != null);
