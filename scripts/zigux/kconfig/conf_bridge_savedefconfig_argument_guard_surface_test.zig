...
const missing_savedefconfig_path = "Error: savedefconfig mode requires <path>\n";
...
test "savedefconfig rejects missing and option-shaped mode arguments before emitting json" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();

    const binary_path = try std.fmt.allocPrint(
        std.testing.allocator,
        ".zig-cache/tmp/{s}/conf_bridge_arg_guard",
        .{tmp.sub_path[0..]},
    );
    ...
    try expectExit(try std.process.run(std.testing.allocator, std.testing.io, .{
        .argv = &.{ binary_path, "savedefconfig", "Kconfig", ".config", "x86_64" },
        .stdout_limit = .limited(1024),
        .stderr_limit = .limited(1024),
    }), missing_savedefconfig_path);
    ...
    for (option_shaped_paths) |arg| {
        try expectExit(try std.process.run(std.testing.allocator, std.testing.io, .{
            .argv = &.{ binary_path, "savedefconfig", "Kconfig", ".config", "x86_64", arg },
            .stdout_limit = .limited(1024),
            .stderr_limit = .limited(1024),
        }), missing_savedefconfig_path);
    }
