...
test "confdata bridge emits only the last duplicate state across unset and set transitions" {
...
test "confdata bridge keeps repeated unset duplicates stable in emitted summary" {
    const allocator = std.testing.allocator;
    const input =
