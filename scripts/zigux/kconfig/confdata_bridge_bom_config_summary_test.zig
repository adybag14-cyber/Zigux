...
test "confdata bridge trims BOM on first config assignment before malformed duplicate" {
    const allocator = std.testing.allocator;
    const input =
        "\xef\xbb\xbfCONFIG_ALPHA=\"kept\"\n" ++
        "CONFIG_ALPHA=\"broken\n" ++
    ...
    try std.testing.expectEqualStrings("kept", summary.entries[0].value);
...
test "confdata bridge emits BOM trimmed json for first config assignment" {
    const allocator = std.testing.allocator;
    const input =
        "\xef\xbb\xbfCONFIG_ALPHA=\"kept\"\n" ++
        "CONFIG_ALPHA=\"broken\n" ++
    ...
    try std.testing.expectEqualStrings(
        "{\"counts\":{\"set\":2,\"unset\":1},\"entries\":[{\"name\":\"CONFIG_ALPHA\",\"kind\":\"string\",\"value\":\"kept\"},{\"name\":\"CONFIG_DEBUG\",\"kind\":\"unset\",\"value\":\"n\"},{\"name\":\"CONFIG_GAMMA\",\"kind\":\"value\",\"value\":\"7\"}]}\n",
        capture.list.items,
    );
