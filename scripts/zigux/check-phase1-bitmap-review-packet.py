...
EXPECTED_BITMAP_PACKET = {
    ...
    "review_packet_summary": (
        ...
        "exact-word-boundary equality fast-path masking, tail-masked predicate behavior, "
    ...
    "next_safe_step_note": (
        ...
        "copy-alias, truncation, cross-word scnprintf, exact-word-boundary equality fast-path "
        "masking, caller-window xor and or clamp, weighted tail-count clamp, empty-buffer, "
        "allocator-reset, zero-bit logical short-circuit, and Linux-style alias mirror anchors "
        "here; do not reopen older closure-side or validator-route cue names by default."
    ),
}
...
def run_self_test() -> int:
    ...
    for anchor in iter_source_anchor_strings():
        cases.append(("helper", anchor, False))
        cases.append(("helper", anchor, True))
    for _, marker in EXPECTED_BITMAP_LANE_MARKERS:
        cases.append(("lane", marker, False))
        cases.append(("lane", marker, True))
    for key in EXPECTED_BITMAP_PACKET:
        cases.append(("manifest", key, None))
    cases.append(("missing_file", BITMAP_HELPER_REL.as_posix(), None))
    cases.append(("missing_file", BITMAP_MANIFEST_REL.as_posix(), None))
    cases.append(("missing_file", BITMAP_LANE_NOTE_REL.as_posix(), None))

    for kind, value, duplicate in cases:
