...
def _run_header_binding_markers_self_test() -> int:
    script_path = Path(__file__).resolve().with_name(HEADER_BINDING_MARKERS_SCRIPT)
    ...
    assert "PHASE3_HEADER_BINDING_MARKER_SELF_TEST=pass" in stdout_lines
    return 0
...
def _run_build_roots_self_test() -> int:
    script_path = Path(__file__).resolve().with_name(BUILD_ROOTS_SCRIPT)
    result = subprocess.run(
        ["python3", str(script_path), "--self-test"],
        text=True,
        capture_output=True,
...
def _run_abi_duplicate_declarations_self_test() -> int:
    script_path = Path(__file__).resolve().with_name(ABI_DUPLICATE_DECLARATIONS_SCRIPT)
    result = subprocess.run(
        ["python3", str(script_path), "--self-test"],
        text=True,
        capture_output=True,
...
def _run_policy_unsafe_survey_self_test() -> int:
    script_path = Path(__file__).resolve().with_name(Path(ABI_POLICY_UNSAFE_SURVEY_CHECK_REL).name)
    result = subprocess.run(
        ["python3", str(script_path), "--self-test"],
        text=True,
        capture_output=True,
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_validator_selftest_") as tmp_dir_str:
        ...
        assert _run_header_binding_markers_self_test() == 0
        ...
        assert _run_policy_unsafe_survey_self_test() == 0
