...
EXPECTED_BITMAP_REVIEW_ANCHORS = {
    ...
    "next_safe_step_note": "If this helper lane reopens, keep bitmap parked unless a fresh reread finds new direct-anchor drift inside the current helper-local packet or committed shared replay drift in the bitmap parity fields; current master still ships direct fill-tail clamp, copy-alias, truncation, cross-word scnprintf, caller-window xor and or clamp, weighted tail-count clamp, empty-buffer, allocator-reset, zero-bit logical short-circuit, and Linux-style alias mirror anchors here, and if the separate bitmap closure-validator anchor-sync repair is still outstanding, treat that as the only other bitmap follow-through.",
}
                            ...
                            **json.loads(load_text(root, MANIFEST_REL))["review_anchors"],
                            "tools/lib/bitmap.zig": {
                                **json.loads(load_text(root, MANIFEST_REL))["review_anchors"]["tools/lib/bitmap.zig"],
                                "review_packet_summary": "older bitmap review summary",
                            },
                        },
                    },
                    indent=2,
