...
def run_self_test() -> int:
    cases: list[tuple[str, object | None]] = [
        ...
        ("missing_rbtree_direct_anchor_guard", lambda root: write_text(root / PHASE1_CLOSURE_REL, load_text(root, PHASE1_CLOSURE_REL).replace(EXPECTED_CLOSURE_MARKERS["rbtree_direct_anchor_guard"] + "\n", "", 1))),
        ("stale_rbtree_direct_anchor_guard", lambda root: write_text(root / PHASE1_CLOSURE_REL, load_text(root, PHASE1_CLOSURE_REL).replace(EXPECTED_CLOSURE_MARKERS["rbtree_direct_anchor_guard"], "`PHASE1_RBTREE_DIRECT_ANCHOR_GUARD=drifted rbtree direct anchor marker`", 1))),
