...
def collect_manifest_review_anchor_markers(manifest: object) -> list[str]:
    ...
    actual_helpers = set()
    for helper_path, fields in anchors.items():
        if not isinstance(helper_path, str):
            missing.append("manifest:review_helper_type=str")
            continue
        actual_helpers.add(helper_path)
        if helper_path not in EXPECTED_REVIEW_ANCHORS:
            missing.append(f"manifest:unexpected_review_helper={helper_path}")
            continue
        if not isinstance(fields, dict):
            missing.append(f"manifest:review_anchor_object={helper_path}")
            continue
        expected_fields = EXPECTED_REVIEW_ANCHORS[helper_path]
