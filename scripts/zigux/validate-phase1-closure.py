...
def run_self_test() -> int:
    cases: list[tuple[str, object | None]] = [
        ...
        ("missing_scripts_restored_closure_marker", lambda root: write_text(root / SCRIPTS_README_REL, load_text(root, SCRIPTS_README_REL).replace(EXPECTED_SCRIPTS_README_MARKERS[1] + "\n", "", 1))),
        ("stale_scripts_makefile_summary_marker", lambda root: write_text(root / SCRIPTS_README_REL, load_text(root, SCRIPTS_README_REL).replace(EXPECTED_SCRIPTS_README_MARKERS[2], FORBIDDEN_SCRIPTS_README_MARKERS[0], 1))),
        ("missing_scripts_direct_anchor_marker", lambda root: write_text(root / SCRIPTS_README_REL, load_text(root, SCRIPTS_README_REL).replace(EXPECTED_SCRIPTS_README_MARKERS[3] + "\n", "", 1))),
