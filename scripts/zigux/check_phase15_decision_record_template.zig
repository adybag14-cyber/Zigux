// Ported from check-phase15-decision-record-template.py by port_phase1_guards.py
const std = @import("std");
const Io = std.Io;
const guard = @import("zigux_guard.zig");

pub const pass_marker = "PHASE15_DECISION_RECORD_TEMPLATE_SELF_TEST=pass";
...
pub fn runSelfTest(io: Io, allocator: std.mem.Allocator) !u8 {
    ...
    try guard.printLine(io, "PHASE15_DECISION_RECORD_TEMPLATE_SELF_TEST_CASE_COUNT={d}", .{@as(usize, 1)});
    return 0;
}
...
pub fn main(init: std.process.Init) !void {
    ...
    if (failures.items.len > 0) {
        try guard.printLine(io, "PHASE15_DECISION_RECORD_TEMPLATE_SELF_TEST=fail", .{});
