...
pub const pass_marker = "PHASE11_HVC_VERIFY_REMOVE_HANDOFF_SELF_TEST=pass";

const VERIFY_DETACHED_BINDING_MARKERS = [_][]const u8{
...
const VERIFY_TTY_ABSENT_MARKERS = [_][]const u8{
...
const ALL_VERIFY_REMOVE_HANDOFF_MARKERS = [_][]const u8{
    "VERIFY_DETACHED_BINDING_MARKERS+VERIFY_TTY_ABSENT_MARKERS",
};

const SCRIPT_PATH = [_][]const u8{
    "scripts/zigux/check_phase11_hvc_verify_remove_handoff.zig",
};

const VERIFY_FILE = [_][]const u8{
    "drivers/tty/hvc/hvc_console_verify.zig",
};

pub fn checkText(text: []const u8) guard.GuardError!void {
    for (VERIFY_DETACHED_BINDING_MARKERS) |marker| try guard.requireMarker(text, marker);
    for (VERIFY_TTY_ABSENT_MARKERS) |marker| try guard.requireMarker(text, marker);
    for (ALL_VERIFY_REMOVE_HANDOFF_MARKERS) |marker| try guard.requireMarker(text, marker);
    for (SCRIPT_PATH) |marker| try guard.requireMarker(text, marker);
    for (VERIFY_FILE) |marker| try guard.requireMarker(text, marker);
}
