...
fn collectFailures(
    ...
    {
        const relative_path = "scripts\\zigux/check_phase1_direct_anchor_manifest_gate.zig";
        const full_path = try guard.joinPath(allocator, root, relative_path);
        defer allocator.free(full_path);
        if (!guard.pathExists(io, full_path)) {
            const issue = try std.fmt.allocPrint(allocator, "missing_file:{s}", .{relative_path});
...
pub fn runSelfTest(io: Io, allocator: std.mem.Allocator) !u8 {
    ...
    try guard.printLine(io, "PHASE1_TESTS_README_ALIGNMENT_SELF_TEST_CASE_COUNT={d}", .{@as(usize, 1)});
    return 0;
}
...
pub fn main(init: std.process.Init) !void {
    ...
    if (failures.items.len > 0) {
        try guard.printLine(io, "PHASE1_TESTS_README_ALIGNMENT_SELF_TEST=fail", .{});
        for (failures.items) |failure| try guard.printLine(io, "{s}", .{failure});
        std.process.exit(1);
    }

    try guard.printLine(io, "{s}", .{pass_marker});
    try guard.printLine(io, "PHASE1_TESTS_README_ALIGNMENT_REQUIRED_FILE_COUNT={d}", .{@as(usize, REQUIRED_FILES.len)});
    try guard.printLine(io, "PHASE1_TESTS_README_ALIGNMENT_REQUIRED_MARKER_COUNT={d}", .{@as(usize, REQUIRED_MARKERS.len)});
    try guard.printLine(io, "PHASE1_TESTS_README_ALIGNMENT_FORBIDDEN_MARKER_COUNT={d}", .{@as(usize, FORBIDDEN_MARKERS.len)});
    std.process.exit(0);
}
