#!/usr/bin/env python3
"""Check that the Phase 2 fixdep action path stays aligned."""
...
EXPECTED_MAKE_WRAPPER = "make -C zigux phase2-fixdep"
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    if len(makefile_indices) == len(REQUIRED_MAKEFILE_LINES) and makefile_indices != sorted(
        ...
        issues.append(("MAKEFILE_ORDER_MISMATCH", "phase2-fixdep-packet-order"))
    ...
    expect_subset(
        issues,
        "fixdep_support",
        require_manifest_list(issues, manifest, "fixdep_support"),
        EXPECTED_FIXDEP_SUPPORT,
    )
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_FIXDEP_ACTION_PATH=fail")
...
def run_self_test() -> int:
    ...
    checks = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_fixdep_action_path_") as tmp_dir:
        root = Path(tmp_dir)
        ...
        assert ("WORKFLOW_ORDER_MISMATCH", "phase2-fixdep-route-order") in collect_issues(root)
        checks += 1
        ...
        assert ("MAKEFILE_ORDER_MISMATCH", "phase2-fixdep-packet-order") in collect_issues(root)
        checks += 1
        ...
        path = resolve(root, TOOL_MANIFEST)
        payload = json.loads(path.read_text(encoding="utf-8"))
        payload["present_surfaces"]["fixdep_support"].remove(EXPECTED_FIXDEP_SUPPORT[-1])
        path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        assert (
            "MISSING_MANIFEST_SURFACE",
            f"fixdep_support:{EXPECTED_FIXDEP_SUPPORT[-1]}",
    ...
    print(f"PHASE2_FIXDEP_ACTION_PATH_SELF_TEST_CASE_COUNT={checks}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the Phase 2 fixdep action path stays aligned."
    )
    ...
    print(f"PHASE2_FIXDEP_ACTION_PATH_BOOTSTRAP_MARKER_COUNT={len(REQUIRED_BOOTSTRAP_MARKERS)}")
