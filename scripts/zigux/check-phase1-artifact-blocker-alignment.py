...
EXPECTED_FIXTURE = {
    ...
    "zalloc": {"zeroed": True, "freed_is_null": True, "value_zeroed": True, "value_freed_is_null": True},
}
...
def write_sample_root(root: Path) -> None:
    ...
    write_json(root / "zigux/tests/fixtures/phase1_helper_manifest.json", EXPECTED_MANIFEST)
    write_json(root / "zigux/tests/fixtures/phase1_replay_blockers.json", EXPECTED_REPLAY_BLOCKERS)
    write_json(root / "zigux/tests/fixtures/phase1_helpers.json", EXPECTED_FIXTURE)
...
def collect_failures(root: Path) -> list[str]:
    ...
    if tuple(sorted(review_anchors.keys())) != tuple(sorted(EXPECTED_REVIEW_ANCHOR_HELPERS)):
    ...
    if blocker_lane.get("manifest") != EXPECTED_REPLAY_BLOCKERS["lane_sequencing"]["manifest"]:
