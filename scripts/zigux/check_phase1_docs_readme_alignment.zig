// Ported from check-phase1-docs-readme-alignment.py by port_phase1_guards.py
const std = @import("std");
const Io = std.Io;
const guard = @import("zigux_guard.zig");

pub const pass_marker = "PHASE1_DOCS_README_ALIGNMENT_SELF_TEST=pass";
...
pub fn runSelfTest(io: Io, allocator: std.mem.Allocator) !u8 {
    ...
    try guard.printLine(io, "PHASE1_DOCS_README_ALIGNMENT_SELF_TEST_CASES={d}", .{@as(usize, 1)});
    return 0;
}
...
pub fn main(init: std.process.Init) !void {
    ...
    if (failures.items.len > 0) {
        try guard.printLine(io, "PHASE1_DOCS_README_ALIGNMENT_SELF_TEST=fail", .{});
