...
ZIG_TOOL = ROOT / "scripts" / "zigux" / "mk_elfconfig_fd_trailing_second_call_test.zig"

EXPECTED_MARKERS = {
    "fd_trailing_second_call_elf32": 'test "fd-backed trailing 32-bit ELF input leaves only trailing bytes for the next call" {',
    "fd_trailing_second_call_elf64": 'test "fd-backed trailing 64-bit ELF input leaves only trailing bytes for the next call" {',
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-fd-trailing-second-call-") as tmp_dir:
        ...
        try:
            ...
            if "missing_marker:fd_trailing_second_call_hidden_later_header" not in str(exc):
                raise
        else:
        ...
        try:
            ...
            if "duplicate_marker:fd_trailing_second_call_not_elf" not in str(exc):
                raise
        else:
        ...
        reordered_path.write_text(
            "\n".join(
                [
                    EXPECTED_MARKERS["fd_trailing_second_call_elf32"],
                    EXPECTED_MARKERS["fd_trailing_second_call_hidden_later_header"],
    ...
    print("MK_ELFCONFIG_FD_TRAILING_SECOND_CALL_MARKERS_SELF_TEST=pass")
    print("MK_ELFCONFIG_FD_TRAILING_SECOND_CALL_MARKERS_SELF_TEST_CASE_COUNT=4")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that mk_elfconfig keeps the fd trailing-second-call packet visible."
    )
    parser.add_argument(
        "--self-test",
        action="store_true",
    ...
    print("MK_ELFCONFIG_FD_TRAILING_SECOND_CALL_MARKERS=pass")
    print(f"MK_ELFCONFIG_FD_TRAILING_SECOND_CALL_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
