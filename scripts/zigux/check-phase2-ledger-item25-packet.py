...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    issues: list[tuple[str, str]] = []
    issues.extend(collect_marker_issues(read_text(root / PHASE2_CLOSURE), PHASE2_CLOSURE_MARKERS, "PHASE2_CLOSURE_MARKER_MISSING", "PHASE2_CLOSURE_MARKER_DUPLICATED"))
    issues.extend(collect_marker_issues(read_text(root / ARTIFACT_DIFF_NOTE), ARTIFACT_DIFF_MARKERS, "ARTIFACT_DIFF_MARKER_MISSING", "ARTIFACT_DIFF_MARKER_DUPLICATED"))
    issues.extend(collect_marker_issues(read_text(root / SCRIPTS_README), SCRIPTS_README_MARKERS, "SCRIPTS_README_MARKER_MISSING", "SCRIPTS_README_MARKER_DUPLICATED"))
    issues.extend(collect_marker_issues(read_text(root / BOOTSTRAP_LEDGER), BOOTSTRAP_LEDGER_MARKERS, "BOOTSTRAP_LEDGER_MARKER_MISSING", "BOOTSTRAP_LEDGER_MARKER_DUPLICATED"))
    return issues
...
def run_self_test() -> int:
    marker_total = len(PHASE2_CLOSURE_MARKERS) + len(ARTIFACT_DIFF_MARKERS) + len(SCRIPTS_README_MARKERS) + len(BOOTSTRAP_LEDGER_MARKERS)
    file_total = 4
    checks_run = 0
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_ledger_item25_") as tmp_dir:
        ...
        checks_run += 1
        for rel_path, markers, code in ((PHASE2_CLOSURE, PHASE2_CLOSURE_MARKERS, "PHASE2_CLOSURE_MARKER_MISSING"), (ARTIFACT_DIFF_NOTE, ARTIFACT_DIFF_MARKERS, "ARTIFACT_DIFF_MARKER_MISSING"), (SCRIPTS_README, SCRIPTS_README_MARKERS, "SCRIPTS_README_MARKER_MISSING"), (BOOTSTRAP_LEDGER, BOOTSTRAP_LEDGER_MARKERS, "BOOTSTRAP_LEDGER_MARKER_MISSING")):
            for marker in markers:
                build_sample_root(root)
                path = root / rel_path
...
def main() -> int:
    ...
    print("PHASE2_LEDGER_ITEM25_PACKET_MARKER_COUNT=" f"{len(PHASE2_CLOSURE_MARKERS) + len(ARTIFACT_DIFF_MARKERS) + len(SCRIPTS_README_MARKERS) + len(BOOTSTRAP_LEDGER_MARKERS)}")
    return 0
