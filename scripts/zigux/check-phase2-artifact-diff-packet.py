...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_artifact_diff_packet_") as tmp_dir:
        ...
        notes_path.write_text(
            notes_path.read_text(encoding="utf-8").replace(TEXT_MARKERS[BOOTSTRAP_NOTES][1], "", 1),
            encoding="utf-8",
        )
        assert any(
            code == "MISSING_TEXT_MARKER" and value.startswith(f"{BOOTSTRAP_NOTES.as_posix()}::")
        ...
        docs_path.write_text(
            docs_path.read_text(encoding="utf-8") + TEXT_MARKERS[DOCS_README][0] + "\n",
            encoding="utf-8",
        )
        assert any(
            code == "DUPLICATE_TEXT_MARKER" and value.startswith(f"{DOCS_README.as_posix()}::")
            for code, value in collect_issues(root)
        )
        checks += 1
        ...
        kconfig_path.write_text(
            kconfig_path.read_text(encoding="utf-8").replace(EXPECTED_CONSUMER_MARKERS[KCONFIG_CHECKER][1], "", 1),
            encoding="utf-8",
        )
        assert any(
            code == "MISSING_CONSUMER_MARKER" and value.startswith(f"{KCONFIG_CHECKER.as_posix()}::")
            for code, value in collect_issues(root)
        )
        checks += 1
        ...
        assert any(
            code == "DUPLICATE_CONSUMER_MARKER" and value.startswith(f"{FIXDEP_CHECKER.as_posix()}::")
            for code, value in collect_issues(root)
        )
        checks += 1
