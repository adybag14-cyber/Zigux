...
test "runFixdep preserves question-mark target names through the public entry path" {
    ...
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "question-target.h",
        .data = "CONFIG_ZIGUX_TARGET_QUESTION=y\n",
    });
    ...
    const header_path = try std.fmt.allocPrint(
        std.testing.allocator,
        ".zig-cache/tmp/{s}/question-target.h",
        .{tmp.sub_path[0..]},
    );
    ...
    try fixdep.runFixdep(
        ...
        depfile_path,
        target,
        "zig cc -DQUESTION_TARGET=1 -c module/source.c",
    );

    const expected = try std.fmt.allocPrint(
        std.testing.allocator,
        "savedcmd_{s} := zig cc -DQUESTION_TARGET=1 -c module/source.c\n\n" ++
            "source_{s} := {s}\n\n" ++
            "deps_{s} := \\\n" ++
            "  {s} \\\n" ++
            "    $(wildcard include/config/ZIGUX_TARGET_QUESTION) \\\n" ++
            "\n" ++
            "{s}: $(deps_{s})\n\n" ++
            "$(deps_{s}):\n",
