...
REQUIRED_LANDED_RING_HELPER_EVIDENCE = {
    "zigux/tests/phase10_virtio_ring_manifest.json": [
...
def require_mapping_list_members(
    drift: list[str],
    manifest: dict,
    field_name: str,
...
def collect_drift(manifest: dict) -> list[str]:
    ...
    require_list_members(
        ...
        virtqueue_wrappers.get("evidence"),
        REQUIRED_RING_SCOREBOARD_EVIDENCE,
    )
    ...
    require_list_members(
        ...
        lab_only_driver_validation.get("evidence"),
        REQUIRED_LAB_VALIDATION_EVIDENCE
        + REQUIRED_INPUT_LAB_VALIDATION_EVIDENCE
        + REQUIRED_MMIO_LAB_VALIDATION_EVIDENCE
        + REQUIRED_CORE_LAB_VALIDATION_EVIDENCE,
    ...
    require_list_members(
        ...
        dual_implementations.get("evidence"),
        REQUIRED_DUAL_IMPLEMENTATION_SCOREBOARD_EVIDENCE,
    )
    ...
    if input_followup != REQUIRED_INPUT_READY_TRANSPORT_GAP:
        drift.append(
            ...
            f"{REQUIRED_INPUT_READY_TRANSPORT_PATH}:{input_followup!r}!="
            f"{REQUIRED_INPUT_READY_TRANSPORT_GAP!r}"
        )
    ...
    if mmio_followup != REQUIRED_MMIO_READY_TRANSPORT_GAP:
        drift.append(
            "ready_transport_followups:"
            f"{REQUIRED_MMIO_READY_TRANSPORT_PATH}:{mmio_followup!r}!="
            f"{REQUIRED_MMIO_READY_TRANSPORT_GAP!r}"
        )
...
def fixture_manifest() -> dict:
    return {
        ...
        "tests": [f"test-{index}" for index in range(30)],
        ...
        "landed_ring_helper_evidence": REQUIRED_LANDED_RING_HELPER_EVIDENCE,
        "landed_input_helper_evidence": REQUIRED_LANDED_INPUT_HELPER_EVIDENCE,
        "landed_mmio_helper_evidence": REQUIRED_LANDED_MMIO_HELPER_EVIDENCE,
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase10_manifest_counts_") as tmp_dir:
        ...
        expect_contains(
            validate(root)[1],
            "landed_ring_helper_evidence:zigux/tests/phase10_virtio_ring_manifest.json:missing",
            "phase10-manifest-counts-self-test",
        )
        cases += 1
        write_fixture(root)
        ...
        broken["landed_ring_helper_evidence"]["zigux/tests/phase10_virtio_ring_manifest.json"] = [
            item
            for item in broken["landed_ring_helper_evidence"]["zigux/tests/phase10_virtio_ring_manifest.json"]
            if item != "phase10-queue-publish-readiness-helper"
        ]
        write_manifest(broken)
        expect_contains(
            validate(root)[1],
            "landed_ring_helper_evidence:zigux/tests/phase10_virtio_ring_manifest.json:'phase10-queue-publish-readiness-helper':missing",
            "phase10-manifest-counts-self-test",
        )
        cases += 1
        write_fixture(root)
        ...
        broken["landed_input_helper_evidence"]["zigux/tests/phase10_virtio_input_manifest.json"] = [
            ...
            if item != "phase10-virtio-input-registration-preflight-helper"
        ]
        write_manifest(broken)
        expect_contains(
            validate(root)[1],
            "landed_input_helper_evidence:zigux/tests/phase10_virtio_input_manifest.json:'phase10-virtio-input-registration-preflight-helper':missing",
            "phase10-manifest-counts-self-test",
        )
        cases += 1
        write_fixture(root)
        ...
        expect_contains(
            validate(root)[1],
            "landed_mmio_helper_evidence:zigux/tests/phase10_virtio_mmio_manifest.json:'phase10-mmio-config-write-apply-observation-helper':missing",
            "phase10-manifest-counts-self-test",
        )
        cases += 1
        write_fixture(root)
...
def main() -> int:
    ...
    print(
        "PHASE10_CLOSURE_MANIFEST_COUNTS_REQUIRED_LANDED_RING_HELPER_COUNT="
        f"{sum(len(labels) for labels in REQUIRED_LANDED_RING_HELPER_EVIDENCE.values())}"
    )
    print(
        "PHASE10_CLOSURE_MANIFEST_COUNTS_REQUIRED_LANDED_INPUT_HELPER_COUNT="
        f"{sum(len(labels) for labels in REQUIRED_LANDED_INPUT_HELPER_EVIDENCE.values())}"
    )
    print(
        "PHASE10_CLOSURE_MANIFEST_COUNTS_REQUIRED_LANDED_MMIO_HELPER_COUNT="
        f"{sum(len(labels) for labels in REQUIRED_LANDED_MMIO_HELPER_EVIDENCE.values())}"
    )
