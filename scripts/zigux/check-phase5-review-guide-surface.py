...
REVIEW_CHECKLIST_MARKERS = (
    ...
    "keep `samples/zigux/kobject_example.zig`, `zigux/tests/phase5_kobject_example.zig`, and `samples/zigux/kobject_example_attr_group_contract.zig` explicit as the current direct sample-root, focused-test, and bounded attr-group companion evidence in this runtime, keep `Documentation/zigux/phase5-kobject-sample-survey.md`, `zigux/tests/phase5_kobject_example_manifest.json`, `zigux/tests/phase5_kobject_example_survey.zig`, and `zigux/tests/phase5_build.zig` framed as current public-tree-backed companion evidence until a fresh reread proves broader direct authenticated proof again,",
)
