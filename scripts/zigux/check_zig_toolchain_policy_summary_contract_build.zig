...
pub fn build(b: *std.Build) void {
    ...
    const contract_step = b.step(
        ...
        "Run the Lane 03 check-zig-toolchain policy summary contract",
    );
    contract_step.dependOn(&run_contract_tests.step);

    const test_step = b.step("test", "Run the Lane 03 check-zig-toolchain policy summary contract tests");
