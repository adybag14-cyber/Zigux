...
test "fd-backed exact non-ELF header leaves a following 64-bit ELF header for the next call" {
    var temp_dir = std.testing.tmpDir(.{});
    defer temp_dir.cleanup();
    const io = std.testing.io;
    const file = try temp_dir.dir.createFile(io, "not_elf_then_elf64.bin", .{ .read = true });
    defer file.close(io);
