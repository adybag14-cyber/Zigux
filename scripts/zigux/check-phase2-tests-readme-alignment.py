...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_tests_readme_") as tmp_dir:
        ...
        tests_readme_path.write_text(
            original_tests_readme.replace(
                "make -C zigux phase2 so the tests root names the same three-target compile matrix",
                "make -C zigux phase2-removed so the tests root names the same three-target compile matrix",
                1,
            ),
            encoding="utf-8",
        )
        expect_missing(
            ...
            tmp_root,
            "tests_readme:make -C zigux phase2 so the tests root names the same three-target compile matrix",
        )
        tests_readme_path.write_text(original_tests_readme, encoding="utf-8")
