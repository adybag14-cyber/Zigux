...
test "ESC remains escaped before NUL truncates hidden tail" {
    const visible = [_]u8{ 'e', 's', 'c', '-', 0x1b, '-', 'v', 'i', 's', 'i', 'b', 'l', 'e' };
    const hidden = "hidden-esc-tail";
    ...
    try std.testing.expect(std.mem.indexOfScalar(u8, capture.list.items, 0x1b) == null);
