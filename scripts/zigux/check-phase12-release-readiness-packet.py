...
RELEASE_READINESS_MARKERS = [
    ...
    "The broader shared-summary packet is now almost closed on current `master`: `scripts/zigux/README.md`, `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-closure-checklist.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, and `scripts/zigux/check-build-only-phase12-surface.py` all keep the shipped `python3 scripts/zigux/check-phase12-cross.py --self-test` companion and the dedicated `scripts/zigux/check-phase12-release-readiness-packet.py` guard explicit inside the same `make -C zigux phase12-validate` support bundle, but `Documentation/zigux/review-checklist.md` still does not call out that bounded cross-selftest companion in its shared Phase 12 reviewer wording.",
    "`scripts/zigux/check-phase12-release-readiness-packet.py` is now the fail-closed truthfulness guard for that narrower shared packet: this note should keep the remaining reviewer-facing `Documentation/zigux/review-checklist.md` gap explicit until the bounded `scripts/zigux/check-phase12-cross.py --self-test` companion appears there too, instead of overclaiming that the full shared-support-bundle reminder drift is already closed.",
    ...
    "Current `master` keeps this readiness companion, the workflow, Makefile, docs-root, tests-root, `scripts/zigux/README.md`, `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-closure-checklist.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, and `scripts/zigux/check-build-only-phase12-surface.py` aligned around the starter-present `virtio_net` plus smoke-first `virtio_scsi` packet, including the parked `zigux/tests/fixtures/phase12_libbpf_snapshot.json` anchor and the shipped `phase12-validate` support bundle. `Documentation/zigux/review-checklist.md` is the remaining shared reminder surface that still needs the bounded `scripts/zigux/check-phase12-cross.py --self-test` companion called out beside the dedicated release-readiness checker and the shipped validator-first route. Leave this readiness companion parked until that smaller reviewer-facing note lands or a fresher repo-first reread finds a different one-file reminder drift before widening into any new driver claims.",
]
...
def run_self_test() -> int:
    ...
    try:
        ...
        expect_contains(
            check(tmp_root, source_text=MARKER),
            "The broader shared-summary packet is now almost closed on current `master`",
        ...
        expect_contains(
            check(tmp_root, source_text=MARKER),
            "remaining reviewer-facing `Documentation/zigux/review-checklist.md` gap explicit",
        ...
        expect_contains(
            check(tmp_root, source_text=MARKER),
            "Leave this readiness companion parked until that smaller reviewer-facing note lands",
