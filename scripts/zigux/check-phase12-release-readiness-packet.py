...
RELEASE_READINESS_MARKERS = [
    ...
    "The broader shared-summary packet is now almost aligned on current `master`: `Documentation/zigux/README.md`, `Documentation/zigux/review-checklist.md`, `.github/workflows/zigux-bootstrap.yml`, and `zigux/Makefile` already keep the dedicated `scripts/zigux/check-phase12-release-readiness-packet.py` guard plus the shipped `make -C zigux phase12-validate` route explicit, and `zigux/tests/README.md` does the same in its Phase 12 inventory, but `scripts/zigux/README.md` still names only `scripts/zigux/check-build-only-phase12-surface.py`, `scripts/zigux/check-phase12-release-readiness-packet.py`, `scripts/zigux/validate-phase12.py`, and the shipped validator-first then smoke-first routes even though current `zigux/Makefile` also self-tests the bounded `scripts/zigux/check-phase12-cross.py` companion inside that same shipped `phase12-validate` support bundle.",
    "`scripts/zigux/check-build-only-phase12-surface.py` and `scripts/zigux/check-phase12-release-readiness-packet.py` still keep the shipped support bundle and broader release packet fail-closed, so the next honest same-lane follow-through is the smallest scripts-root reminder repair that makes `scripts/zigux/check-phase12-cross.py --self-test` explicit without reopening driver-local, fallback-catalog, or verify-shard wording first.",
    ...
    "Current `master` keeps the release-order, readiness, coordination, closure, workflow, Makefile, docs-root, review-checklist, and tests-root companions aligned around the starter-present `virtio_net` plus smoke-first `virtio_scsi` packet, including the parked `zigux/tests/fixtures/phase12_libbpf_snapshot.json` anchor and the shipped `phase12-validate` support bundle, but `scripts/zigux/README.md` still undercounts the bounded `scripts/zigux/check-phase12-cross.py --self-test` companion inside that same shipped validator-first route. The next honest same-lane follow-through is therefore to repair only that scripts-root reminder drift before widening into any new driver claims.",
]
...
def run_self_test() -> int:
    ...
    try:
        ...
        expect_contains(
            check(tmp_root, source_text=MARKER),
            "The broader shared-summary packet is now almost aligned on current `master`",
        ...
        expect_contains(
            check(tmp_root, source_text=MARKER),
            "scripts-root reminder drift before widening into any new driver claims.",
