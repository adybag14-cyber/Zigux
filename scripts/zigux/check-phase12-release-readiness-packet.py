...
REQUIRED_MARKERS = {
    ...
    RELEASE_SEQUENCING_PATH: [
        ...
        "Current `zigux/tests/phase12_build.zig` wires that same six-file bounded `virtio_net` follow-up sextet through both `smoke` and `test`, so the shared release packet should keep those bounded queue-resume, receive-refill replay, transmit-disposition, post-reset replay, throughput-parity, and survey-gate replays explicit without rounding them up into live interrupt-backed transmit completion parity, queue-restart parity, or transport-backed throughput delivery.",
