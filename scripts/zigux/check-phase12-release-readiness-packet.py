...
REQUIRED_MARKERS = {
    ...
    RELEASE_SEQUENCING_PATH: [
        ...
        "keep the shipped `make -C zigux phase12-validate` wrapper explicit ahead of the attached-Zig reruns `make -C zigux phase12-smoke ZIG=<attached-zig-path>`, `make -C zigux phase12-test ZIG=<attached-zig-path>`, and `make -C zigux phase12 ZIG=<attached-zig-path>`",
    ...
    RAW_GITHUB_COVERAGE_SURVEY_PATH: [
        ...
        "exact coverage evidence checked on `2026-05-21`: the current GitHub contents bridge directly reads `scripts/zigux/check-build-only-phase12-surface.py` at blob `81a71a9bfe2c263120ea102d8b9cf361121c472a`, `scripts/zigux/validate-phase12.py` at blob `de1e248f6688adf89b9f9edb3abd824ece6ddae5`, `scripts/zigux/check-phase12-release-readiness-packet.py` at blob `ca25d80ac6190123aa7474a0afc84e7e8c896a01`, `.github/workflows/zigux-bootstrap.yml` at blob `d33dbde416395f8d7cd0e79da73d90b6e5dea3bb`, `scripts/zigux/README.md` at blob `929ebe22494d052da408cf3a5be1de903f18ff24`, `zigux/Makefile` at blob `001155e688ededb7cbf96c814741cd4dc2867f64`, and `zigux/tests/phase12_build.zig` at blob `c338d24f4d12317c6a58d25708bbc14a5006852c`",
        "keep the directly readable validator, build-only checker, release-readiness checker, workflow, scripts-root README, current Makefile, and current `zigux/tests/phase12_build.zig` as bounded reminder evidence only",
    ],
    PHASE12_COMPLEX_DRIVER_LANE_PATH: [
...
EXACT_COUNT_MARKERS = {
    ...
    RELEASE_READINESS_SURVEY_PATH: {
        "The route story on current `master` is now fully returned rather than split: the directly readable scripts-side support packet is still present through `scripts/zigux/validate-phase12.py`, `scripts/zigux/check-build-only-phase12-surface.py`, `scripts/zigux/check-phase12-release-readiness-packet.py`, and `.github/workflows/zigux-bootstrap.yml`, and current `zigux/Makefile` now provides shared `phase12-validate`, `phase12-smoke`, `phase12-test`, and `phase12` wrapper routes again.": 1,
        "The active shared build route on current `master` is the six-file `virtio_net` smoke-and-test packet in `zigux/tests/phase12_build.zig`: `zigux/tests/phase12_virtio_net_queue_resume.zig`, `zigux/tests/phase12_virtio_net_transmit_recycle.zig`, `zigux/tests/phase12_virtio_net_receive_refill_replay.zig`, `zigux/tests/phase12_virtio_net_post_reset_replay.zig`, `zigux/tests/phase12_virtio_net_throughput_parity.zig`, and `zigux/tests/phase12_virtio_net_survey.zig` are the directly wired shared reruns, and the `smoke` plus `test` steps both stay bounded to those six queue-resume, transmit-recycle, receive-refill replay, post-reset replay, throughput-parity, and survey-gate replays.": 1,
        "That means the PMO release notes can treat `make -C zigux phase12-validate`, `make -C zigux phase12-smoke`, `make -C zigux phase12-test`, and `make -C zigux phase12` as shipped current-`master` evidence again, while still keeping the validator-first support packet distinct from deeper driver-delivery claims.": 1,
    },
    RELEASE_SEQUENCING_PATH: {
        "Current repo-reality override: the route story on current `master` is now fully returned rather than split. `zigux/Makefile` now exposes shared `phase12-validate`, `phase12-smoke`, `phase12-test`, and `phase12` wrappers again, so this sequencing note should treat that validator-first then smoke-first wrapper set as shipped current-`master` evidence beside the directly readable rerun surfaces `python3 scripts/zigux/check-build-only-phase12-surface.py --self-test`, `python3 scripts/zigux/check-phase12-release-readiness-packet.py --self-test`, `scripts/zigux/validate-phase12.py`, `zig build smoke --build-file zigux/tests/phase12_build.zig --summary all`, and `zig build test --build-file zigux/tests/phase12_build.zig --summary all`.": 1,
        "The active smoke-first direct shard set on current `master` is `zigux/tests/phase12_virtio_net_queue_resume.zig`, `zigux/tests/phase12_virtio_net_receive_refill_replay.zig`, `zigux/tests/phase12_virtio_net_transmit_recycle.zig`, `zigux/tests/phase12_virtio_net_post_reset_replay.zig`, `zigux/tests/phase12_virtio_net_throughput_parity.zig`, and `zigux/tests/phase12_virtio_net_survey.zig`, because those are the six files the current `smoke` step actually runs.": 1,
        "Current `zigux/tests/phase12_build.zig` wires that same six-file bounded `virtio_net` follow-up packet through both `smoke` and `test`, so the shared release packet should keep those bounded queue-resume, receive-refill replay, transmit-disposition, post-reset replay, throughput-parity, and survey-gate replays explicit without rounding them up into live interrupt-backed transmit completion parity, queue-restart parity, or transport-backed throughput delivery.": 1,
    },
    RELEASE_COORDINATION_MATRIX_PATH: {
        "freeze-map companion: `Documentation/zigux/freeze-map.md`": 1,
        "`Documentation/zigux/freeze-map.md` remains the boundary owner for deeper queueing and transport anchors, so this matrix must keep that governance note explicit whenever it summarizes the shared Phase 12 release packet.": 1,
        "`Documentation/zigux/review-checklist.md`, `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-readiness-survey.md`, `Documentation/zigux/phase12-release-closure-checklist.md`, `Documentation/zigux/freeze-map.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, `Documentation/zigux/phase12-complex-driver-lane-sequencing.md`, `Documentation/zigux/phase12-libbpf-heavy-consumer-lane-sequencing.md`, `Documentation/zigux/phase12-libbpf-verify-shard-note.md`, `Documentation/zigux/phase12-nvme-pci-reopen-governance.md`, `Documentation/zigux/phase12-virtio-scsi-survey.md`, `zigux/tests/phase12_virtio_scsi_manifest.json`, `zigux/tests/phase12_virtio_scsi_survey.zig`, `zigux/tests/fixtures/phase12_libbpf_snapshot.json`, `scripts/zigux/README.md`, and `zigux/tests/README.md` whenever the shared Phase 12 packet changes.": 1,
