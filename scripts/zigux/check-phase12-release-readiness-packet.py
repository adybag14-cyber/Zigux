...
RELEASE_READINESS_MARKERS = [
    ...
    "The broader shared-summary packet is now partly closed on current `master`: `scripts/zigux/README.md` already keeps `python3 scripts/zigux/check-phase12-cross.py --self-test` explicit inside the shipped `make -C zigux phase12-validate` support bundle beside `scripts/zigux/check-build-only-phase12-surface.py`, `scripts/zigux/check-phase12-release-readiness-packet.py`, `scripts/zigux/validate-phase12.py`, and the shipped validator-first then smoke-first routes, but `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-closure-checklist.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, and `scripts/zigux/check-build-only-phase12-surface.py` still carry the older scripts-root reminder-drift story on current `master`.",
    "`scripts/zigux/check-phase12-release-readiness-packet.py` is therefore the narrowest live same-lane PMO repair: this note can now close the already-landed scripts-root reminder drift while leaving `scripts/zigux/check-build-only-phase12-surface.py` and the remaining shared PMO companions for the next bounded follow-through instead of reopening driver-local, fallback-catalog, verify-shard, or cross-build wording first.",
    ...
    "Current `master` keeps this readiness companion, the workflow, Makefile, docs-root, review-checklist, tests-root, and `scripts/zigux/README.md` aligned around the starter-present `virtio_net` plus smoke-first `virtio_scsi` packet, including the parked `zigux/tests/fixtures/phase12_libbpf_snapshot.json` anchor and the shipped `phase12-validate` support bundle, but `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-release-closure-checklist.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, and `scripts/zigux/check-build-only-phase12-surface.py` still carry the older scripts-root reminder-drift story. The next honest same-lane follow-through is therefore to refresh that remaining shared-summary packet before widening into any new driver claims.",
]
...
def run_self_test() -> int:
    ...
    try:
        ...
        expect_contains(
            check(tmp_root, source_text=MARKER),
            "The broader shared-summary packet is now partly closed on current `master`",
        ...
        expect_contains(
            check(tmp_root, source_text=MARKER),
            "narrowest live same-lane PMO repair",
        ...
        expect_contains(
            check(tmp_root, source_text=MARKER),
            "refresh that remaining shared-summary packet before widening into any new driver claims.",
