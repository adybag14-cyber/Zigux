...
REQUIRED_MARKERS = {
    ...
    RAW_GITHUB_COVERAGE_SURVEY_PATH: [
        ...
        "- exact same-day support-bundle refresh checked later on `2026-05-25`: the current GitHub contents bridge now directly reads `scripts/zigux/validate-phase12.py` `f5c2db6f5b154c8f8576046bf4e75cc1d843f54e`, `scripts/zigux/check-phase12-release-readiness-packet.py` `cd38624956acd428da30ca014649d21ed2ffd9c3`, and `.github/workflows/zigux-bootstrap.yml` `a103321967ee775fab0da20e4e99bd183d2bdddf`; keep this later same-day readback explicit so the support-bundle inventory stays truthful when the rest of the returned `2026-05-25` bridge set remains unchanged.",
...
EXACT_COUNT_MARKERS = {
    RAW_GITHUB_COVERAGE_SURVEY_PATH: {
        ...
        "- exact same-day support-bundle refresh checked later on `2026-05-25`: the current GitHub contents bridge now directly reads `scripts/zigux/validate-phase12.py` `f5c2db6f5b154c8f8576046bf4e75cc1d843f54e`, `scripts/zigux/check-phase12-release-readiness-packet.py` `cd38624956acd428da30ca014649d21ed2ffd9c3`, and `.github/workflows/zigux-bootstrap.yml` `a103321967ee775fab0da20e4e99bd183d2bdddf`; keep this later same-day readback explicit so the support-bundle inventory stays truthful when the rest of the returned `2026-05-25` bridge set remains unchanged.": 1,
        "- exact runtime-reality evidence checked on `2026-05-25`: the directly readable `zigux/Makefile` blob `adcb33fc7dd30009e24210b2c44e3412427854fb` still prefers the repo-local `.zig-toolchain` executable through `ZIG_PINNED_EXECUTABLE`, `ZIG_LOCAL_TOOLCHAIN`, `ZIG_PINNED_TOOLCHAIN`, and `ZIG ?= $(if $(ZIG_PINNED_TOOLCHAIN),$(ZIG_PINNED_TOOLCHAIN),zig)`, and the directly readable workflow blob `f72787f799d497897122b86ddc65fd1ada31d77e` still rebuilds that repo-local fallback by trying the pinned `third_party` archive first, then the Zig community-mirror list, and finally `ziglang.org` before rerunning `make -C zigux phase12-validate`, `make -C zigux phase12-smoke`, `make -C zigux phase12-test`, and `make -C zigux phase12`": 1,
    },
}
