#!/usr/bin/env python3
"""Guard the Phase 2 cross packet described by the docs-root bootstrap note."""
...
def expected_note_markers(packet: dict[str, object]) -> tuple[str, ...]:
    archive_target = str(packet["archive_target"])
    route_contract_target = str(packet["route_contract_target"])
    return (
        f"limits archive digests to `{archive_target}`",
        f"through the pinned `{archive_target}` `archive_required` lane plus the `{route_contract_target}` `route_contract_only` lane",
...
def collect_note_issues(root: Path, packet: dict[str, object]) -> list[tuple[str, str]]:
    note_text = read_text(resolve_path(root, BOOTSTRAP_NOTES))
    return [
        ("MISSING_BOOTSTRAP_NOTE_MARKER", marker)
        for marker in expected_note_markers(packet)
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues: list[tuple[str, str]] = []
    issues.extend(collect_note_issues(root, packet))
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_CROSS_BOOTSTRAP_NOTE_CONTRACT=fail")
...
def run_self_test() -> int:
    expected_case_count = (
        1
        + len(expected_note_markers(load_expected_packet_from_archive_target("x86_64-linux")))
        + len(WORKFLOW_LINES)
        + len(WORKFLOW_LINES)
        + len(MAKEFILE_LINES)
        + len(MAKEFILE_LINES)
    ...
    checks_run = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_cross_bootstrap_note_") as tmp_dir:
        root = Path(tmp_dir)
        ...
        for marker in expected_note_markers(load_expected_packet_from_archive_target("x86_64-linux")):
            build_sample_root(root)
            note_path = resolve_path(root, BOOTSTRAP_NOTES)
            note_path.write_text(replace_once(note_path.read_text(encoding="utf-8"), marker), encoding="utf-8")
        ...
        for path in (BOOTSTRAP_NOTES, TOOLCHAIN_POLICY, FIXTURE, WORKFLOW, MAKEFILE):
            build_sample_root(root)
    ...
    print("PHASE2_CROSS_BOOTSTRAP_NOTE_CONTRACT_SELF_TEST=pass")
    print(f"PHASE2_CROSS_BOOTSTRAP_NOTE_CONTRACT_SELF_TEST_CASE_COUNT={checks_run}")
    return 0


def load_expected_packet_from_archive_target(archive_target: str) -> dict[str, object]:
    route_contract_target = next(target for target in SUPPORTED_TARGETS if target != archive_target)
    return {
        "archive_target": archive_target,
        "route_contract_target": route_contract_target,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the Phase 2 bootstrap note keeps the live cross packet explicit."
    )
    ...
    if args.write_sample_root is not None:
        ...
        print(f"PHASE2_CROSS_BOOTSTRAP_NOTE_CONTRACT_SAMPLE_ROOT={args.write_sample_root.resolve()}")
        return 0
    ...
    packet = load_expected_packet(root)
    print("PHASE2_CROSS_BOOTSTRAP_NOTE_CONTRACT=pass")
    print(f"PHASE2_CROSS_BOOTSTRAP_NOTE_CONTRACT_MARKER_COUNT={len(expected_note_markers(packet))}")
    print(f"PHASE2_CROSS_BOOTSTRAP_NOTE_CONTRACT_ARCHIVE_TARGET={packet['archive_target']}")
    print(f"PHASE2_CROSS_BOOTSTRAP_NOTE_CONTRACT_ROUTE_TARGET={packet['route_contract_target']}")
    return 0
