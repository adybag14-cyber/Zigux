...
test "runGenksymsCrc escapes visible tabs before hidden NUL tails" {
    var capture = try Capture.init(std.testing.allocator);
    defer capture.deinit();

    const visible_tab_record = "alpha\tbeta";
    const input = visible_tab_record ++ "\x00\tsecret\nnext\tvisible\n";
    try gen.runGenksymsCrc(input, &capture);

    const visible_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(visible_tab_record)});
    defer std.testing.allocator.free(visible_crc);
    const hidden_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("\tsecret")});
    defer std.testing.allocator.free(hidden_crc);
    const next_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("next\tvisible")});
