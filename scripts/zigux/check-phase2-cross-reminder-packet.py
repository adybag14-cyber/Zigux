...
TESTS_MARKERS = (
    ...
    "`zigux/tests/fixtures/phase2_cross_targets.json`",
    "`make -C zigux phase2-cross`",
    "returned installer, direct cross-route, and cross-target fixture packet explicit here instead of leaving it in the historical-gap bucket",
)
...
CURRENT_LIKE_CLOSURE = """# Phase 2 Closure

The current closure-side packet now stays anchored to the materialized closure note, validator entrypoint, closure validator, toolchain policy, returned local-first archive pair, returned installer and cross-route companions, and current manifests.
...
CURRENT_LIKE_SCRIPTS_README = """# scripts/zigux
...
def collect_forbidden_issues(text: str) -> list[tuple[str, str]]:
    issues: list[tuple[str, str]] = []
    for marker in FORBIDDEN_MARKERS:
        if marker in text:
            issues.append(("FORBIDDEN_MARKERS", marker))
    return issues
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues: list[tuple[str, str]] = []
    issues.extend(collect_marker_issues(closure_text, CLOSURE_MARKERS, "MISSING_CLOSURE_MARKERS"))
    issues.extend(collect_marker_issues(scripts_text, SCRIPTS_MARKERS, "MISSING_SCRIPTS_MARKERS"))
    issues.extend(collect_marker_issues(tests_text, TESTS_MARKERS, "MISSING_TESTS_MARKERS"))
    issues.extend(collect_forbidden_issues(closure_text))
    issues.extend(collect_forbidden_issues(scripts_text))
    issues.extend(collect_forbidden_issues(tests_text))
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_CROSS_REMINDER_PACKET=fail")
...
def build_sample_root(root: Path) -> None:
    write_text(root / CLOSURE_PATH, CURRENT_LIKE_CLOSURE)
    write_text(root / SCRIPTS_README_PATH, CURRENT_LIKE_SCRIPTS_README)
...
def run_self_test() -> int:
    checks_run = 0
    expected_case_count = 1 + len(CLOSURE_MARKERS) + len(SCRIPTS_MARKERS) + len(TESTS_MARKERS) + len(FORBIDDEN_MARKERS) + 4
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_cross_reminder_packet_") as tmp_dir:
        root = Path(tmp_dir)
        ...
        for marker in SCRIPTS_MARKERS:
            ...
            path.write_text(replace_marker(path.read_text(encoding="utf-8"), marker), encoding="utf-8")
            issues = collect_issues(root)
            assert ("MISSING_SCRIPTS_MARKERS", marker) in issues
            checks_run += 1
    ...
    print("PHASE2_CROSS_REMINDER_PACKET_SELF_TEST=pass")
    print(f"PHASE2_CROSS_REMINDER_PACKET_SELF_TEST_CASE_COUNT={checks_run}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Keep the shared Phase 2 cross-route reminder packet aligned to current repo reality."
    )
    ...
    print("PHASE2_CROSS_REMINDER_PACKET=pass")
    print(f"PHASE2_CROSS_REMINDER_PACKET_FILE_COUNT=4")
    print(f"PHASE2_CROSS_REMINDER_PACKET_CLOSURE_MARKER_COUNT={len(CLOSURE_MARKERS)}")
    print(f"PHASE2_CROSS_REMINDER_PACKET_SCRIPTS_MARKER_COUNT={len(SCRIPTS_MARKERS)}")
    print(f"PHASE2_CROSS_REMINDER_PACKET_TESTS_MARKER_COUNT={len(TESTS_MARKERS)}")
    print(f"PHASE2_CROSS_REMINDER_PACKET_TARGET_COUNT={len(EXPECTED_FIXTURE['cross_targets'])}")
    return 0
