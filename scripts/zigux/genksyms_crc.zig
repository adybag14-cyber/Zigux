...
test "runGenksymsCrc skips a NUL-prefixed EOF continuation after an exact-buffer split" {
    ...
    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc(exact_then_nul_tail.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(exact_then_nul_tail.items[0..c_line_payload_len])});
    defer std.testing.allocator.free(exact_crc);
    const skipped_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("bc")});
    defer std.testing.allocator.free(skipped_crc);
...
test "runGenksymsCrc preserves leading low control bytes in a visible split continuation" {
    ...
    try split_then_visible_controls.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try split_then_visible_controls.append(std.testing.allocator, '\x08');
    try split_then_visible_controls.append(std.testing.allocator, '\x0c');
    try split_then_visible_controls.append(std.testing.allocator, '\x01');
    try split_then_visible_controls.append(std.testing.allocator, 'b');
    try split_then_visible_controls.append(std.testing.allocator, '\n');
    try split_then_visible_controls.appendSlice(std.testing.allocator, "x\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc(split_then_visible_controls.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_controls.items[0..c_line_payload_len])});
    defer std.testing.allocator.free(exact_crc);
    const control_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("\x08\x0c\x01b")});
...
test "runGenksymsCrc preserves leading low control bytes while trimming trailing carriage returns in a visible split continuation before the next line" {
    var split_then_visible_controls_and_trailing_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 10);
    defer split_then_visible_controls_and_trailing_cr.deinit(std.testing.allocator);
    try split_then_visible_controls_and_trailing_cr.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, '\x08');
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, '\x0c');
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, '\x01');
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, 'b');
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, '\r');
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, '\r');
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, '\n');
    try split_then_visible_controls_and_trailing_cr.appendSlice(std.testing.allocator, "x\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc(split_then_visible_controls_and_trailing_cr.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_controls_and_trailing_cr.items[0..c_line_payload_len])});
    defer std.testing.allocator.free(exact_crc);
...
test "runGenksymsCrc preserves leading low control bytes in a visible EOF continuation" {
    var split_then_visible_controls = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 4);
    defer split_then_visible_controls.deinit(std.testing.allocator);
    try split_then_visible_controls.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try split_then_visible_controls.append(std.testing.allocator, '\x08');
    try split_then_visible_controls.append(std.testing.allocator, '\x0c');
    ...
    const trimmed_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("b")});
    defer std.testing.allocator.free(trimmed_crc);
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_controls.items)});
    defer std.testing.allocator.free(unsplit_crc);
...
test "runGenksymsCrc preserves leading low control bytes while trimming trailing carriage returns in a visible EOF continuation" {
    var split_then_visible_controls_and_trailing_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 6);
    defer split_then_visible_controls_and_trailing_cr.deinit(std.testing.allocator);
    try split_then_visible_controls_and_trailing_cr.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, '\x08');
    try split_then_visible_controls_and_trailing_cr.append(std.testing.allocator, '\x0c');
    ...
    const trimmed_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("b")});
    defer std.testing.allocator.free(trimmed_crc);
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_controls_and_trailing_cr.items)});
    defer std.testing.allocator.free(unsplit_crc);
...
test "runGenksymsCrc preserves leading carriage returns in a visible EOF continuation" {
    var split_then_visible_leading_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 3);
    ...
    const trimmed_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("b")});
    defer std.testing.allocator.free(trimmed_crc);
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_leading_cr.items)});
    defer std.testing.allocator.free(unsplit_crc);
...
test "runGenksymsCrc preserves leading carriage returns while trimming trailing carriage returns in a visible EOF continuation" {
    var split_then_visible_leading_and_trailing_cr = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 5);
    defer split_then_visible_leading_and_trailing_cr.deinit(std.testing.allocator);
    try split_then_visible_leading_and_trailing_cr.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try split_then_visible_leading_and_trailing_cr.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_cr.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_cr.append(std.testing.allocator, 'b');
    try split_then_visible_leading_and_trailing_cr.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_cr.append(std.testing.allocator, '\r');

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc(split_then_visible_leading_and_trailing_cr.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_leading_and_trailing_cr.items[0..c_line_payload_len])});
    defer std.testing.allocator.free(exact_crc);
    const normalized_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("\r\rb")});
    defer std.testing.allocator.free(normalized_crc);
    const trailing_cr_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("\r\rb\r\r")});
    defer std.testing.allocator.free(trailing_cr_crc);
    const trimmed_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("b")});
    defer std.testing.allocator.free(trimmed_crc);
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_leading_and_trailing_cr.items)});
    defer std.testing.allocator.free(unsplit_crc);
...
test "runGenksymsCrc preserves leading carriage returns while trimming trailing carriage returns in a visible split continuation before the next line" {
    ...
    try split_then_visible_leading_and_trailing_crlf.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try split_then_visible_leading_and_trailing_crlf.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_crlf.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_crlf.append(std.testing.allocator, 'b');
    try split_then_visible_leading_and_trailing_crlf.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_crlf.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_crlf.append(std.testing.allocator, '\n');
    try split_then_visible_leading_and_trailing_crlf.appendSlice(std.testing.allocator, "x\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc(split_then_visible_leading_and_trailing_crlf.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_leading_and_trailing_crlf.items[0..c_line_payload_len])});
    defer std.testing.allocator.free(exact_crc);
    const normalized_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("\r\rb")});
    defer std.testing.allocator.free(normalized_crc);
    const trailing_cr_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("\r\rb\r\r")});
...
test "runGenksymsCrc preserves leading carriage returns while trimming trailing carriage returns before an embedded NUL in a visible EOF continuation" {
    var split_then_visible_leading_and_trailing_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 8);
    defer split_then_visible_leading_and_trailing_cr_then_nul.deinit(std.testing.allocator);
    try split_then_visible_leading_and_trailing_cr_then_nul.appendNTimes(std.testing.allocator, 'a', c_line_payload_len);
    try split_then_visible_leading_and_trailing_cr_then_nul.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_cr_then_nul.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_cr_then_nul.append(std.testing.allocator, 'b');
    try split_then_visible_leading_and_trailing_cr_then_nul.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_cr_then_nul.append(std.testing.allocator, '\r');
    try split_then_visible_leading_and_trailing_cr_then_nul.append(std.testing.allocator, 0);
    try split_then_visible_leading_and_trailing_cr_then_nul.append(std.testing.allocator, 'c');

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc(split_then_visible_leading_and_trailing_cr_then_nul.items, &capture);

    const exact_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_leading_and_trailing_cr_then_nul.items[0..c_line_payload_len])});
    defer std.testing.allocator.free(exact_crc);
    const normalized_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("\r\rb")});
    defer std.testing.allocator.free(normalized_crc);
    const trailing_cr_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("\r\rb\r\r")});
    ...
    const suffix_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32("c")});
    defer std.testing.allocator.free(suffix_crc);
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(split_then_visible_leading_and_trailing_cr_then_nul.items)});
    defer std.testing.allocator.free(unsplit_crc);
