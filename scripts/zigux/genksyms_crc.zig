...
test "runGenksymsCrc preserves leading carriage returns while trimming trailing carriage returns before an embedded NUL in a visible split continuation" {
    var split_then_visible_leading_and_trailing_cr_then_nul = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 10);
    ...
    try split_then_visible_leading_and_trailing_cr_then_nul.append(std.testing.allocator, '\n');
    try split_then_visible_leading_and_trailing_cr_then_nul.appendSlice(std.testing.allocator, "x\n");
