...
fn truncateAtFirstNul(text: []const u8) []const u8 {
    if (std.mem.indexOfScalar(u8, text, 0)) |nul_index| {
        return text[0..nul_index];
    }
    return text;
}
...
test "runGenksymsCrc truncates embedded NUL before trimming carriage return" {
    const Capture = struct {
        list: std.ArrayList(u8),
        allocator: std.mem.Allocator,
    ...
    var capture = try Capture.init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc("int\r\x00ignored\n", &capture);
    try std.testing.expectEqualStrings(
        "{\"cases\":[{\"input\":\"int\",\"crc_hex\":\"0x1451dab1\"}]}\n",
        capture.list.items,
    );
...
test "runGenksymsCrc trims carriage return at payload boundary before split newline" {
    const Capture = struct {
        list: std.ArrayList(u8),
        allocator: std.mem.Allocator,
    ...
    var line = try std.ArrayList(u8).initCapacity(std.testing.allocator, c_line_payload_len + 1);
    defer line.deinit(std.testing.allocator);
    try line.appendNTimes(std.testing.allocator, 'a', c_line_payload_len - 1);
    try line.append(std.testing.allocator, '\r');
    try line.append(std.testing.allocator, '\n');

    var capture = try Capture.init(std.testing.allocator);
    defer capture.deinit();
    try runGenksymsCrc(line.items, &capture);

    const payload = line.items[0 .. c_line_payload_len - 1];
    const expected_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{crc32(payload)});
    defer std.testing.allocator.free(expected_crc);
    const expected = try std.fmt.allocPrint(
        std.testing.allocator,
        "{{\"cases\":[{{\"input\":\"{s}\",\"crc_hex\":\"{s}\"}}]}}\n",
