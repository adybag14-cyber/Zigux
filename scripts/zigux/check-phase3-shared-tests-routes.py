...
SELF_TEST_CASES = (
    ...
    (BUILD_PATH, '"phase3-errptr-xarray-dump"'),
    (BUILD_PATH, "phase3_test_step.dependOn(&phase3_policy_starter_packet.step);"),
...
def validate_repo(repo_root: Path) -> list[str]:
    ...
    try:
        ...
        for marker in SELFTEST_DRIVER_MARKERS:
            if marker not in driver_text:
                issues.append(f"missing {SELFTEST_DRIVER_PATH.as_posix()} marker: {marker}")

    return issues


def _populate_repo(root: Path) -> None:
    _write(root / BUILD_PATH, SAMPLE_BUILD)
    _write(root / SELFTEST_DRIVER_PATH, SAMPLE_DRIVER)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("."),
        help="repository root that contains the shared Phase 3 tests-root build file",
    )
