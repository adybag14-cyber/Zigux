...
ARCHIVE_CHECK_CMD = "zig run scripts/zigux/check_zig_toolchain.zig -- --archive-only --allow-missing"
INSTALL_SELF_TEST_STEP = "- name: Self-test current Zig installer helper"
INSTALL_SELF_TEST_CMD = "zig run scripts/zigux/install_zig.zig -- --self-test"
STAGE_HELPER_SELF_TEST_STEP = "- name: Self-test current staged pinned Zig archive helper"
STAGE_HELPER_SELF_TEST_CMD = "zig run scripts/zigux/stage_pinned_zig_archive.zig -- --self-test"
...
def check_workflow(text: str) -> None:
    for marker, label in (
        ...
        (SELF_TEST_CMD, "self checker self-test command"),
        (CHECK_STEP, "self checker check step"),
        (CHECK_CMD, "self checker check command"),
...
"""
    ...
    try:
        ...
        assert STAGE_HELPER_SELF_TEST_STEP in str(exc), str(exc)
        case_count += 1
    else:
        raise AssertionError("expected duplicate stage helper step failure")
    ...
    try:
        ...
        raise AssertionError("expected reordered contract steps failure")

    print("LANE05_STAGE_HELPER_SELFTEST_SELF_TEST=pass")
    print(f"LANE05_STAGE_HELPER_SELFTEST_SELF_TEST_CASE_COUNT={case_count}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that Lane 05 bootstrap runs the staged-archive helper self-test packet."
    )
