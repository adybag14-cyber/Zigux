...
test "lane19 exact-buffer split preserves visible CR-led bytes, truncates at NUL, and trims trailing carriage returns" {
    ...
    const collapsed_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("b")});
    defer std.testing.allocator.free(collapsed_crc);
    ...
    const nul_suffix_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("c")});
    defer std.testing.allocator.free(nul_suffix_crc);
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, collapsed_crc) == null);
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, untruncated_crc) == null);
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, nul_suffix_crc) == null);
