...
const REQUIRED_EXACT_LINES_ENTRIES = [_]struct { file: []const u8, marker: []const u8 }{
    ...
    .{ .file = "Documentation/zigux/README.md", .marker = "  * `zig run check_phase1_string_review_packet.zig --self-test`, `zig run check_phase1_direct_owner_markers.zig --self-test`, and `zig run check_phase1_bench.zig --self-test` replay the bounded current reminder checks, while the live checker routes guard the shipped Phase 1 packet without widening it back into the older closure-side or installer-companion stack." },
