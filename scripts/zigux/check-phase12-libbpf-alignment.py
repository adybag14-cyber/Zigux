...
def check_alignment(root: Path) -> list[str]:
    ...
    tracked_paths = snapshot.get("tracked_paths")
    if not isinstance(tracked_paths, list):
        ...
        if len(tracked_paths) != len(EXPECTED_TRACKED_PATHS):
            missing.append("snapshot:path_count")
        for index, expected_path in enumerate(EXPECTED_TRACKED_PATHS):
            if index >= len(tracked_paths):
                break
            if tracked_paths[index] != expected_path:
                missing.append(f"snapshot:path:{index}")
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="zigux_phase12_libbpf_alignment_") as tmp_dir_str:
        ...
        replace_once(
            ...
            '"zigux/tests/phase12_libbpf_reviewability_old.zig"',
        )
        if "snapshot:path:2" not in check_alignment(tmp_root):
