...
test "genksyms bridge treats equals-prefixed values after positionals as request input" {
...
test "genksyms bridge accepts equals-prefixed required option arguments after positionals" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "genksyms bridge keeps equals-prefixed terminator tails out of parser state" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "genksyms bridge renders equals-prefixed terminator tail in bridge json" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
