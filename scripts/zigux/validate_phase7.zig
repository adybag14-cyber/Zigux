...
const BUILD_WIRING_FAILURE_MARKERS = [_][]const u8{
...
fn checkRepo(io: Io, allocator: std.mem.Allocator, root: []const u8) !void {
    ...
    const text_required_makefile_lines_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-leaf-library-evidence-catalog.md");
    ...
    const text_build_wiring_failure_markers_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase7-leaf-library-evidence-catalog.md");
    ...
    for (BUILD_WIRING_FAILURE_MARKERS) |marker| try guard.requireMarker(text_build_wiring_failure_markers, marker);
