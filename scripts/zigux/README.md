...
- `zigux/tests/phase7_build.zig` currently keeps the helper packet split in one deliberate way: `phase7_string_helpers.zig`, `phase7_cmdline.zig`, `phase7_argv_split.zig`, and `phase7_rbtree.zig` receive `lib/*.zig` code through `addImport(...)` aliases; `phase7_string_helpers_survey.zig` stays standalone so the helper-only slice keeps its roadmap and sample-root boundary explicit without pretending there is a Phase 5 `samples/zigux/*string*` reference sample; and `phase7_argv_split_survey.zig` plus `phase7_rbtree_survey.zig` stay self-contained and read `zigux/tests/phase7_argv_split_manifest.json` plus `zigux/tests/phase7_rbtree_manifest.json` from repo-root paths at runtime.
