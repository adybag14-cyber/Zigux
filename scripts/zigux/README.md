...
- current `master` still ships no standalone `validate-phase5.py`, no `check-phase5-*.py` checker packet, and no `phase5-validate` target; keep same-lane follow-through inside sample-backed contributor guidance, survey-note truthfulness, manifest prompts, or exact replay-contract repairs unless a new shipped validation surface lands first.
