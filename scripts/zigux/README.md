...
- the same validator now also snapshots the shared build inventory in `zigux/tests/fixtures/phase12_build_inventory.json` so the exact shared build inventory snapshot stays committed instead of leaving the replay shape implicit in handwritten marker checks alone.
