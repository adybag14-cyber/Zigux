...
- the same validator now also guards the coupled Phase 11 survey and validation notes that record live `master` provenance, the bcm2835 hardware-validation handoff posture, the dw_wdt ready-next resource-preflight note, the shared watchdog or hvc header-boundary survey, and the exact hvc shared-replay evidence so note drift fails fast instead of lingering behind the manifest packet.
