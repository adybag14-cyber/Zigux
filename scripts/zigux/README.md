...
- `zig build test --build-file zigux/tests/phase4_build.zig`, `make -C zigux phase4`, `make -C zigux phase4-bitmap-live-helper-replay`, and the intentionally unapproved perf-threshold posture around `zigux/tests/phase4_perf_baseline_manifest.json` plus `zigux/tests/phase4_perf_baseline_survey.zig` keep the rollback packet measurable without implying a shipped Phase 4 slowdown budget.
