...
Phase 8 flow
- `python3 scripts/zigux/validate-phase8.py`, `make -C zigux phase8-validate`, `zig test tools/lib/subcmd/exec-cmd.zig`, `make -C zigux phase8-exec-cmd-test`, `make -C zigux phase8`, `zig test zigux/tests/phase8_libbpf_segments.zig`, `zigux/tests/phase8_exec_cmd_only_build.zig`, `zigux/tests/phase8_help_only_build.zig`, `zigux/tests/phase8_kallsyms_only_build.zig`, `zigux/tests/phase8_libbpf_segments_only_build.zig`, and `zigux/tests/phase8_perf_buffer_poll.zig` now keep the current Phase 8 flow reviewable across the direct helper-local `exec-cmd` replay, the focused subcmd and symbol shards, the segmented libbpf shards, and the shared tooling bundle.
...
- `Documentation/zigux/phase12-shared-replay-contract.md`, `zigux/tests/phase12_build.zig`, `zigux/tests/phase12_libbpf_only_build.zig`, `scripts/zigux/check-phase12-libbpf-focused-replay.py`, `scripts/zigux/validate-phase12.py`, and `zigux/tests/phase12_libbpf_manifest.json` keep the scripts-root Phase 12 route summary aligned with the same shared-versus-focused libbpf replay boundary named in `zigux/tests/README.md` and the docs-root contract note instead of leaving the dedicated shard implied behind the broader shared build inventory.
