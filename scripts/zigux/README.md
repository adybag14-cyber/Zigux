...
- `check-phase13-devres-inventory-contract.py`
...
- `make -C zigux phase4-runtime-atomic64-diff`, `make -C zigux phase4-kprobe-example-survey`, `make -C zigux phase4-test-fsmount-survey`, and `make -C zigux phase4-perf-baseline-survey` keep the isolated runtime atomic64 gate and the three manifest-backed survey gates reviewable without widening them into landed Zig samples or approved benchmark thresholds.
- `phase4-runtime-atomic64-diff-tests`, `phase4-runtime-atomic64-diff-survey-tests`, `phase4-kprobe-example-survey-tests`, `phase4-test-fsmount-survey-tests`, and `phase4-perf-baseline-survey-tests` remain explicit in `phase4_build.zig`, so the shared replay surface stays measurable instead of dissolving into prose-only notes.
...
- `Documentation/zigux/phase2-toolchain-bootstrap-notes.md` and `Documentation/zigux/review-checklist.md` keep that same bounded toolchain packet reviewable from the docs root, so the dedicated note, the scripts-root pin-scope guard, the shared validators, the bootstrap workflow routes, and the Linux-style `make -C zigux phase2-validate` plus `make -C zigux phase2` entrypoints stay aligned as one kbuild-facing review path.
...
Phase 14 flow
- `make -C zigux phase14-validate`, `make -C zigux phase14-smoke`, and `zig build test --build-file zigux/tests/phase14_build.zig --summary all` are the current shared review path for the Phase 14 boundary-only smoke packet rooted in `Documentation/zigux/phase14-end-to-end-smoke-survey.md`, `Documentation/zigux/phase14-release-boundary-survey.md`, and `zigux/tests/phase14_end_to_end_smoke_manifest.json`, keeping the shared-surface accounting explicit from a study-only posture.
