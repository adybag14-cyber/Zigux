...
- this keeps the current input lane honest while the registration-preflight helper remains the single ready-next step, with the already-landed `ABS_MT_SLOT` planning helper staying as its bounded prerequisite before any wider `input_register_device()` claim.

Phase 11 flow
- `validate-phase11.py` keeps the shared Phase 11 simple-driver bundle aligned before replay by checking the workflow wiring, `zigux/Makefile`, `zigux/tests/phase11_build.zig`, the shared build inventory snapshot in `zigux/tests/fixtures/phase11_build_inventory.json`, the four driver-local manifests `zigux/tests/phase11_gpio_wdt_manifest.json`, `zigux/tests/phase11_bcm2835_wdt_manifest.json`, `zigux/tests/phase11_dw_wdt_manifest.json`, and `zigux/tests/phase11_hvc_console_manifest.json`, plus the shared `zigux/tests/phase11_uapi_header_parity_manifest.json`.
- `make -C zigux phase11-validate` is the fail-fast bundle check for that shared simple-driver packet before the shared Zig replay runs.
- `make -C zigux phase11` keeps the current watchdog and console tranche reviewable through one shared entrypoint instead of ad hoc per-lane commands.
- the current shared bundle intentionally splits replay ownership: `zigux/tests/phase11_build.zig` runs `phase11-gpio-wdt-tests`, `phase11-gpio-wdt-survey-tests`, `phase11-bcm2835-wdt-tests`, `phase11-bcm2835-wdt-survey-tests`, `phase11-dw-wdt-tests`, `phase11-dw-wdt-survey-tests`, `phase11-uapi-header-parity-survey-tests`, and `phase11-hvc-console-tests`, while `zigux/tests/phase11_hvc_console_survey.zig` remains a dedicated replay so the hvc console archival checkpoint does not silently become shared-gate coverage.
- the dedicated hvc_console survey note and validation matrix stay in the same packet so reviewers can see the exact shared-versus-dedicated replay commands and observed outcome lines before widening tty-registration, khvcd, notifier, or host-backed I/O claims.
- exact shared-versus-dedicated replay commands and observed outcome lines are currently:
- shared: `zig build test --build-file zigux/tests/phase11_build.zig --summary all` -> `Build Summary: 17/17 steps succeeded; 37/37 tests passed`; included hvc artifact `run test phase11-hvc-console-tests 6 pass (6 total)` and no `phase11-hvc-console-survey-tests` artifact
- dedicated: `zig test zigux/tests/phase11_hvc_console_survey.zig` -> `2/2 ... OK`
