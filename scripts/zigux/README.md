...
- `Documentation/zigux/phase4-validation-matrix.md` keeps the current rollback owners, threshold posture, exact workflow steps `Validate Phase 4 diff gates` and `Run Phase 4 diff tests`, and the shared `phase4-runtime-atomic64-diff-tests` plus `phase4-bitmap-diff-tests` replay anchors explicit for the shipped Phase 4 gates.