...
Phase 12 flow
- the current shared Phase 12 release-planning surface on `master` is `Documentation/zigux/phase12-release-readiness-survey.md`, `Documentation/zigux/phase12-release-coordination-matrix.md`, `Documentation/zigux/phase12-release-sequencing.md`, `Documentation/zigux/phase12-shared-replay-contract.md`, `Documentation/zigux/phase12-cross-compile-smoke.md`, `Documentation/zigux/phase12-raw-github-coverage-survey.md`, `Documentation/zigux/phase12-nvme-pci-raw-github-fallback-map.md`, `Documentation/zigux/phase12-virtio-scsi-raw-github-fallback-catalog.md`, `scripts/zigux/check-phase12-release-readiness-packet.py`, `scripts/zigux/check-phase12-raw-github-coverage.py`, `scripts/zigux/check-phase12-libbpf-focused-replay.py`, `scripts/zigux/validate-phase12.py`, `zigux/tests/phase12_raw_github_coverage_manifest.json`, `zigux/tests/phase12_raw_github_coverage_survey.zig`, `zigux/tests/phase12_libbpf_only_build.zig`, `zigux/tests/phase12_build.zig`, and `zigux/Makefile`.
- `check-phase12-release-readiness-packet.py` keeps the active-not-closed release posture, the compact owner split, the approved `x86_64-linux-musl`, `aarch64-linux-musl`, and `riscv64-linux-musl` smoke set, and the current two commit-pinned versus two shared-tree-only fallback split visible before the broader replay gates run.
- `validate-phase12.py` and `make -C zigux phase12-validate` keep that same bounded Phase 12 release packet aligned across the PMO survey, rollback matrix, sequencing note, raw-coverage checker, focused libbpf replay shard, and shared replay contract before `make -C zigux phase12` reruns the full tranche entrypoint.
