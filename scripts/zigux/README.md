...
- the active Phase 11 review packet should keep the exact shared-versus-dedicated replay commands, the named pre-replay checker stack, and the observed outcome lines explicit in review notes so the shared build inventory and the separate hvc_console survey boundary do not drift into prose-only claims.
