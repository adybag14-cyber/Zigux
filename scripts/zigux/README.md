...
- `python3 scripts/zigux/validate-phase14.py` and `make -C zigux phase14-validate` keep the shared smoke packet fail-closed before the Zig replay routes run by checking the manifest command inventory, the shared smoke survey, the scripts-root flow, the workflow step, and the combined make wrapper.
