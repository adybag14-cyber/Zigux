...
- `check-phase10-harness-coverage.py --self-test` and `check-phase10-harness-coverage.py` keep the focused harness-coverage packet explicit across `zigux/tests/phase10_virtio_input_multitouch_preflight.zig`, `zigux/tests/phase10_virtio_mmio_queue_isolation.zig`, `zigux/tests/phase10_build.zig`, `zigux/tests/phase10_closure_manifest.json`, `Documentation/zigux/phase10-closure-evidence.md`, `zigux/Makefile`, and `.github/workflows/zigux-bootstrap.yml` before the broader Phase 10 validator and closure gate claim that lab-only driver validation is still reviewable.
