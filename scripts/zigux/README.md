...
- the bootstrap workflow now replays the dedicated `phase11-hvc-survey` archival gate separately from the shared `phase11_build.zig` replay, so CI proves the bounded `hvc_console` survey packet without pretending that survey test already runs inside the shared build shard.
...
- `make -C zigux phase11-hvc-survey` remains the dedicated local archival replay, while `make -C zigux phase11` still keeps the broader shared simple-driver bundle reviewable through one validator-first path.
...
- the current shared Zig replay has moved ahead again: `zig build test --build-file zigux/tests/phase12_build.zig --summary all` now reports `Build Summary: 17/17 steps succeeded; 52/52 tests passed`, and the committed `zigux/tests/fixtures/phase12_build_inventory.json` fast-inventory packet now records that same summary so future note or validator work can treat the shared build inventory as current.
