...
- the current shared packet now carries four manifest-backed roadmap-anchor surveys, and the `devres` survey plus reviewability gate keep the helper-only MMIO or resource-planner surface explicit while still blocking live DMA-backed mappings and scatterlist ownership.
