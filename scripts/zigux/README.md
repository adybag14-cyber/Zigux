...
Phase 6 flow
- `validate-phase6.py` checks that the bounded Phase 6 leaf-helper bundle still keeps `zigux/tests/phase6_build.zig`, `make -C zigux phase6-validate`, the bootstrap workflow, `Documentation/zigux/phase6-helper-parity-catalog.md`, `python3 scripts/zigux/check-phase6-bsearch-c-parity.py`, and the published base64, bsearch, checksum, and hexdump slice notes aligned.
