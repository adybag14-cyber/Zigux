...
- `check-artifact-diff-contract.py` now stays in the same `make -C zigux phase4-validate` bundle so the outward `ARTIFACT_DIFF=pass`, `ARTIFACT_DIFF=fail`, `MODE=...`, and exit-code contract is replayed outside the helper's built-in self-test.
