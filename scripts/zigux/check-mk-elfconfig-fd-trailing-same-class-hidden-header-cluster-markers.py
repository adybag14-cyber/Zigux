...
EXPECTED_MARKERS = {
    Path("scripts/zigux/mk_elfconfig_fd_trailing_same_class_elf_test.zig"): [
        (
            "fd_trailing_elf32_hides_elf32",
            'test "fd-backed trailing 32-bit ELF input keeps a later 32-bit ELF header hidden behind the trailing bytes" {',
        ),
        (
            "fd_trailing_elf32_hides_elf32_no_skip",
            'test "fd-backed trailing 32-bit ELF input does not silently skip forward to the hidden 32-bit ELF header" {',
        ),
        (
            "fd_trailing_elf64_hides_elf64",
            'test "fd-backed trailing 64-bit ELF input keeps a later 64-bit ELF header hidden behind the trailing bytes" {',
        ),
        (
            "fd_trailing_elf64_hides_elf64_no_skip",
            'test "fd-backed trailing 64-bit ELF input does not silently skip forward to the hidden 64-bit ELF header" {',
        ),
    ],
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-fd-trailing-same-class-hidden-header-cluster-") as tmp_dir:
        ...
        duplicate_path = duplicate_root / "scripts/zigux/mk_elfconfig_fd_trailing_same_class_elf_test.zig"
        ...
        duplicate_path.write_text(
            "\n".join(
                [
                    duplicate_marker,
                    EXPECTED_MARKERS[Path("scripts/zigux/mk_elfconfig_fd_trailing_same_class_elf_test.zig")][1][1],
                    EXPECTED_MARKERS[Path("scripts/zigux/mk_elfconfig_fd_trailing_same_class_elf_test.zig")][2][1],
                    EXPECTED_MARKERS[Path("scripts/zigux/mk_elfconfig_fd_trailing_same_class_elf_test.zig")][3][1],
                    duplicate_marker,
                ]
            )
            + "\n",
        ...
        try:
            ...
            if "duplicate_marker:fd_trailing_elf32_hides_elf32" not in str(exc):
                raise
        else:
        ...
        reordered_path = reordered_root / "scripts/zigux/mk_elfconfig_fd_trailing_same_class_elf_test.zig"
        reordered_markers = EXPECTED_MARKERS[Path("scripts/zigux/mk_elfconfig_fd_trailing_same_class_elf_test.zig")]
    ...
    print("MK_ELFCONFIG_FD_TRAILING_SAME_CLASS_HIDDEN_HEADER_CLUSTER_MARKERS_SELF_TEST=pass")
    print("MK_ELFCONFIG_FD_TRAILING_SAME_CLASS_HIDDEN_HEADER_CLUSTER_MARKERS_SELF_TEST_CASE_COUNT=4")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the mk_elfconfig same-class trailing hidden-header fd replay pair stays present."
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    print("MK_ELFCONFIG_FD_TRAILING_SAME_CLASS_HIDDEN_HEADER_CLUSTER_MARKERS=pass")
    print(f"MK_ELFCONFIG_FD_TRAILING_SAME_CLASS_HIDDEN_HEADER_CLUSTER_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
