...
EXPECTED_LANE_NOTE_LINES = (
    ...
    "or for committed tail-clamped replay drift; do not reopen older saved validator cues "
...
EXPECTED_NEXT_SAFE_STEP_NOTES = {
    "tools/lib/bitmap.zig": (
        ...
        "truncation, cross-word scnprintf, caller-window xor and or clamp, weighted tail-count clamp, "
        "empty-buffer, allocator-reset, zero-bit logical short-circuit, and Linux-style alias mirror "
        "anchors here, and if the separate bitmap closure-validator anchor-sync repair is still "
        "outstanding, treat that as the only other bitmap follow-through."
    ),
    "tools/lib/find_bit.zig": (
        ...
        "anchors, or committed tail-clamped replay drift; do not reopen older saved validator cues "
        "or neighboring helper families."
    ),
    "tools/lib/rbtree.zig": (
