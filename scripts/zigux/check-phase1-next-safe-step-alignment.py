#!/usr/bin/env python3
"""Guard the current Phase 1 helper-specific next-safe-step packet."""
...
EXPECTED_CLOSURE_LINE = (
    ...
    "tie-breaker against the restored closure note, the closure validator, the shared "
    "tests-root smoke route, and the helper-specific next_safe_step_note entries in "
    "the committed manifest rather than widening back into the older validator-first "
    "or replay-side closure stack.`"
)
...
EXPECTED_LANE_NOTE_LINES = (
    ...
    "coverage including the shipped andnot scan entry points, or tail-word skip anchors, "
    "or for committed tail-clamped or tail-inclusive-boundary replay drift; do not reopen "
    ...
    "leftmost-sync, cached-root alias, singleton-erase, replacement, detach, and reseed "
    ...
    "C-string list lookup, counted-search and search-length anchors through strpbrk(), "
    "strspn(), strcspn(), strnchr(), strnchrNul() or strnchrnul(), strchr(), strrchr(), "
    "strlen(), and strnlen(), embedded-NUL trim, or moving-earliest-dirty-byte memchrInv "
    "coverage, or for committed replaceChar or current string fixture drift; keep the "
    "helper-local sysfs review anchors aligned across the string review packet and this lane "
...
EXPECTED_NEXT_SAFE_STEP_NOTES = {
    "tools/lib/bitmap.zig": (
        ...
        "direct-anchor drift inside the current helper-local packet or committed shared replay drift "
        "in the bitmap parity fields; current master still ships direct fill-tail clamp, copy-alias, "
        "truncation, cross-word scnprintf, exact-word-boundary equality fast-path masking, caller-window "
        "xor and or clamp, weighted tail-count clamp, empty-buffer, allocator-reset, zero-bit logical "
        "short-circuit, and Linux-style alias mirror anchors here; do not reopen older closure-side or "
    ...
    "tools/lib/find_bit.zig": (
        ...
        "Linux-style alias coverage including the shipped andnot scan entry points, or tail-word skip "
        "anchors, or committed tail-clamped or tail-inclusive-boundary replay drift; do not reopen "
        "older saved validator cues or neighboring helper families."
    ),
    "tools/lib/rbtree.zig": (
        ...
        "direct cached-root anchors; the ordered Linux-style alias proof, dedicated `low_level_alias_anchor`, "
        "and the remaining cached-root insert-miss, leftmost-sync, cached-root alias, singleton-erase, "
        "replacement, detach, and reseed behavior stay owned by direct helper-local anchors until another "
        "committed cached-root field lands."
    ),
    "tools/lib/string.zig": (
...
def collect_failures(root: Path) -> list[str]:
    ...
    failures.extend(
        require_exact_line(
            closure_text,
            f"{PHASE1_CLOSURE_REL.as_posix()}:PHASE1_NEXT_SAFE_STEP",
            EXPECTED_CLOSURE_LINE,
        )
    )
    ...
    if direct_anchor_helpers != EXPECTED_DIRECT_ANCHOR_HELPERS:
        failures.append(
            f"{MANIFEST_REL.as_posix()}:lane_sequencing.direct_anchor_followup_helpers:"
            f"expected={EXPECTED_DIRECT_ANCHOR_HELPERS!r}:actual={direct_anchor_helpers!r}"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-sample-root",
        help="write a current-like sample repository rooted at the given path",
    )
    ...
    if args.write_sample_root:
        ...
        print("PHASE1_NEXT_SAFE_STEP_ALIGNMENT_SAMPLE_ROOT=written")
        return 0
    ...
    print("PHASE1_NEXT_SAFE_STEP_ALIGNMENT_REQUIRED_MARKER_COUNT=11")
    return 0
