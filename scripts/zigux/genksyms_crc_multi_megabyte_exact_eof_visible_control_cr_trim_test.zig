...
test "lane19 multi-megabyte exact-buffer EOF continuation preserves visible control bytes while trimming trailing carriage returns beyond the old CLI cap" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + gen.c_line_payload_len + 7,
    );
