...
BOOTSTRAP_MARKERS = (
    ...
    "`scripts/zigux/check-phase2-toolchain-pinning.py`, `scripts/zigux/check-phase2-toolchain-pin-scope.py`",
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues.extend(collect_forbidden_markers(review_text, REVIEW_FORBIDDEN_MARKERS, "FORBIDDEN_REVIEW_MARKERS"))
    issues.extend(collect_exact_count_markers(review_text, REVIEW_EXACT_COUNT_MARKERS, "EXACT_COUNT_REVIEW_MARKERS"))
...
def run_self_test() -> int:
    ...
    expected_case_count = (
        1
        + len(REVIEW_MARKERS)
        + len(REVIEW_FORBIDDEN_MARKERS)
        + len(REVIEW_EXACT_COUNT_MARKERS)
        + len(TESTS_MARKERS)
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_review_toolchain_pin_scope_") as tmp_dir:
        root = Path(tmp_dir)
        ...
        for marker in REVIEW_FORBIDDEN_MARKERS:
            build_self_test_root(root)
            path = resolve_path(root, REVIEW_CHECKLIST)
            path.write_text(path.read_text(encoding="utf-8") + marker + "\n", encoding="utf-8")
            issues = collect_issues(root)
            assert ("FORBIDDEN_REVIEW_MARKERS", marker) in issues
            checks_run += 1

        for marker in REVIEW_EXACT_COUNT_MARKERS:
            build_self_test_root(root)
            path = resolve_path(root, REVIEW_CHECKLIST)
            path.write_text(path.read_text(encoding="utf-8") + marker + "\n", encoding="utf-8")
            issues = collect_issues(root)
            assert ("EXACT_COUNT_REVIEW_MARKERS", f"2::{marker}") in issues
            checks_run += 1
        ...
        for rel_path in (REVIEW_CHECKLIST, TESTS_README, BOOTSTRAP_NOTES):
            build_self_test_root(root)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Keep the shared Phase 2 toolchain pin-scope reminder packet aligned across the review checklist, tests README, and bootstrap notes."
    )
    ...
    print(f"PHASE2_REVIEW_CHECKLIST_TOOLCHAIN_PIN_SCOPE_PACKET_REVIEW_MARKER_COUNT={len(REVIEW_MARKERS)}")
    print(f"PHASE2_REVIEW_CHECKLIST_TOOLCHAIN_PIN_SCOPE_PACKET_TESTS_MARKER_COUNT={len(TESTS_MARKERS)}")
    print(f"PHASE2_REVIEW_CHECKLIST_TOOLCHAIN_PIN_SCOPE_PACKET_BOOTSTRAP_MARKER_COUNT={len(BOOTSTRAP_MARKERS)}")
    print(
        "PHASE2_REVIEW_CHECKLIST_TOOLCHAIN_PIN_SCOPE_PACKET_FORBIDDEN_MARKER_COUNT="
        f"{len(REVIEW_FORBIDDEN_MARKERS) + len(TESTS_FORBIDDEN_MARKERS) + len(BOOTSTRAP_FORBIDDEN_MARKERS)}"
    )
    return 0
