    ...
    *,
    ...
    if os.name != "nt":
        with open("/dev/full", "w", encoding="utf-8") as stdout_handle:
            return subprocess.run(cmd, check=False, text=True, cwd=cwd, stdout=stdout_handle, stderr=subprocess.PIPE)

    full_path = shutil.which("full")
    if full_path is None:
        raise FileNotFoundError("stdout_mode=dev_full requires the 'full' helper on PATH when running on Windows")
    with open(full_path, "w", encoding="utf-8") as stdout_handle:
        return subprocess.run(cmd, check=False, text=True, cwd=cwd, stdout=stdout_handle, stderr=subprocess.PIPE)
...
def expect_failure(name: str, func, expected_substring: str) -> None:
    try:
        func()
    except Exception as exc:  # noqa: BLE001 - self-test helper
        actual = str(exc)
        if expected_substring not in actual:
            raise AssertionError(
                f"fixdep:self-test:{name}:expected={expected_substring!r}:actual={actual!r}"
            ) from exc
    else:
        raise AssertionError(f"fixdep:self-test:{name}:expected failure")
...
def find_case(cases: list[dict[str, object]], name: str) -> dict[str, object]:
    for case in cases:
        if case["name"] == name:
            return case
    raise AssertionError(f"missing case {name} in self-test setup")


def run_self_test() -> int:
    valid_cases = json.loads(CASES_PATH.read_text(encoding="utf-8"))
    validated_cases = validate_cases(valid_cases)
    if len(validated_cases) != len(EXPECTED_CASES):
        raise AssertionError("validated cases length mismatch")

    duplicate_cases = copy_valid_cases(valid_cases)
    duplicate_cases.append(dict(duplicate_cases[0]))
    expect_failure(
        "duplicate_case",
        lambda: validate_cases(duplicate_cases),
        f"{CASES_PATH}:duplicate_name:sample",
    )

    wrong_order_cases = copy_valid_cases(valid_cases)
    wrong_order_cases[0], wrong_order_cases[1] = wrong_order_cases[1], wrong_order_cases[0]
    expect_failure(
        "case_order",
        lambda: validate_cases(wrong_order_cases),
        f"{CASES_PATH}:case_order={['sample_multi_target', 'sample', *EXPECTED_CASE_ORDER[2:]]!r},expected={EXPECTED_CASE_ORDER!r}",
    )

    missing_name_cases = copy_valid_cases(valid_cases)
    find_case(missing_name_cases, "sample")["name"] = ""
    expect_failure(
        "missing_name",
    ...
    find_case(unexpected_name_cases, "sample")["name"] = "unexpected"
    expect_failure(
        "unexpected_name",
        lambda: validate_cases(unexpected_name_cases),
        f"{CASES_PATH}:unexpected_name:unexpected",
    )
    ...
    expect_failure(
        ...
        f"{CASES_PATH}:sample:target=None,expected='sample.o'",
    )

    wrong_expected_exit_cases = copy_valid_cases(valid_cases)
    find_case(wrong_expected_exit_cases, "sample")["expected_exit_code"] = 1
    expect_failure(
        "wrong_expected_exit_code",
        lambda: validate_cases(wrong_expected_exit_cases),
        f"{CASES_PATH}:sample:expected_exit_code=1,expected=0",
    )
    ...
    expect_failure(
        ...
        f"{CASES_PATH}:sample_comment_only_stdout_full:stdout_mode=None,expected='dev_full'",
    )

    missing_expected_stderr_cases = copy_valid_cases(valid_cases)
    find_case(missing_expected_stderr_cases, "sample_comment_only").pop("expected_stderr", None)
    expect_failure(
        "missing_expected_stderr",
        lambda: validate_cases(missing_expected_stderr_cases),
...
def compile_run_c_wsl(
    ...
    wsl_tmp = windows_to_wsl(tmp_dir)
    wsl_exe = windows_to_wsl(exe)
    wsl_root = windows_to_wsl(ROOT)
    wsl_depfile = windows_to_wsl(depfile)

    compile_script = (
        f"set -euo pipefail; "
        f"cd {shlex.quote(wsl_root)}; "
        f"{shlex.quote(compiler)} -std=gnu11 -Wall -Wextra -I scripts/include -o {shlex.quote(wsl_exe)} scripts/basic/fixdep.c"
    )
    subprocess.run(["wsl", "bash", "-lc", compile_script], check=True, text=True)
    ...
    run_command = " ".join(
        shlex.quote(part)
        for part in (wsl_exe, wsl_depfile, target, cmdline)
    )

    if stdout_mode is None:
        completed = subprocess.run(
            ["wsl", "bash", "-lc", run_prefix + run_command],
            check=False,
            text=True,
            capture_output=True,
        )
        return subprocess.CompletedProcess(completed.args, completed.returncode, completed.stdout, completed.stderr)

    if stdout_mode != "dev_full":
        raise ValueError(f"unsupported stdout mode: {stdout_mode}")

    stderr_path = tmp_dir / "wsl.stderr.txt"
    wsl_stderr = windows_to_wsl(stderr_path)
    full_command = run_prefix + run_command + f" >/dev/full 2>{shlex.quote(wsl_stderr)}"
    completed = subprocess.run(["wsl", "bash", "-lc", full_command], check=False, text=True, capture_output=True)
    stderr_text = stderr_path.read_text(encoding="utf-8") if stderr_path.exists() else completed.stderr
    return subprocess.CompletedProcess(completed.args, completed.returncode, completed.stdout, stderr_text)
