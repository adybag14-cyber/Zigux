...
def validate_self_test_case_count(case_count: int) -> None:
    if case_count != EXPECTED_SELF_TEST_CASE_COUNT:
        raise SystemExit(
            f"fixdep:self-test:count={case_count},expected={EXPECTED_SELF_TEST_CASE_COUNT}"
        )
...
def run_self_test() -> int:
    ...
    validate_self_test_case_count(case_count)
    print("FIXDEP_SELF_TEST=pass")
    print(f"FIXDEP_SELF_TEST_CASE_COUNT={case_count}")
    return 0
