...
FIXTURE_DIR = ROOT / 'zigux' / 'tests' / 'fixtures' / 'phase3_chrdev_xfer'
EXPECTED = FIXTURE_DIR / 'expected.json'
HARNESS = FIXTURE_DIR / 'phase3_chrdev_xfer_c_harness.c'
...
def compile_and_run_c(tmp_dir: Path, compiler: str, actual: Path) -> None:
    exe = tmp_dir / ('phase3_chrdev_xfer_c_harness.exe' if os.name == 'nt' else 'phase3_chrdev_xfer_c_harness')
    flags = ['-I', str(ROOT / 'include')]

    if os.name == 'nt' and shutil.which('wsl'):
        script_path = tmp_dir / 'run_phase3_chrdev_xfer_c.sh'
...
def compile_and_run_zig(zig: str, actual: Path) -> None:
    env = os.environ.copy()
    env['ZIG'] = zig
    result = run([zig, 'build', 'phase3-chrdev-xfer-dump', '--build-file', str(ROOT / 'zigux' / 'tests' / 'build.zig')], cwd=str(ROOT), capture_output=True, env=env)
    actual.write_text(result.stdout, encoding='utf-8', newline='\n')


def main() -> int:
    parser = argparse.ArgumentParser(description='Check bounded Phase 3 chrdev xfer interop parity.')
    ...
    with tempfile.TemporaryDirectory(prefix='zigux_phase3_chrdev_xfer_') as tmp_dir_str:
        tmp_dir = Path(tmp_dir_str)
        c_actual = tmp_dir / 'phase3_chrdev_xfer_c.actual.json'
        zig_actual = tmp_dir / 'phase3_chrdev_xfer_zig.actual.json'
    ...
    print('PHASE3_CHRDEV_XFER_DIFF=pass')
