...
GENKSYMS_SURVEY_MARKERS = (
    ...
    "- The narrower repo-reality gap now sits entirely in one directly coupled reminder-surface pair: current `zigux/tests/README.md` still undercounts the live genksyms packet because it stops at the older bridge-fixture roster, while `scripts/zigux/check-phase2-tests-readme-alignment.py` still codifies that reduced roster and therefore does not yet fail closed on the returned survey note, selftest-alignment checker, standalone proof pair, dedicated manifest, dash-prefixed-long-option-arguments-as-data expected-output fixture, or the nine process-output fixtures that `Documentation/zigux/phase2-closure.md`, `Documentation/zigux/phase2-genksyms-dual-implementation-survey.md`, and `zigux/tests/fixtures/phase2_tool_manifest.json` already treat as current packet members.",
