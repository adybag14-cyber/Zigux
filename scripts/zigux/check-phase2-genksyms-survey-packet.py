...
FORBIDDEN_GENKSYMS_SURVEY_MARKERS = (
    "The remaining same-family gap is now narrower and checker-local: current `master` still lacks the dedicated `scripts/zigux/check-phase2-genksyms-survey-packet.py` guard",
    "2. Reopen the same family only to land `scripts/zigux/check-phase2-genksyms-survey-packet.py` so the dedicated survey note fails closed",
)
...
def require_absent_markers(text: str, markers: tuple[str, ...], label: str) -> None:
    for marker in markers:
        if marker in text:
            raise SystemExit(f"phase2 genksyms survey checker found stale {label} marker: {marker}")
...
def check_root(root: Path) -> None:
    ...
    require_markers(survey_text, GENKSYMS_SURVEY_MARKERS, "genksyms survey")
    require_absent_markers(survey_text, FORBIDDEN_GENKSYMS_SURVEY_MARKERS, "genksyms survey")
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="lane25_phase2_genksyms_survey_") as tmp:
        ...
        broken = sample_root / GENKSYMS_SURVEY
        broken.write_text(
            SAMPLE_GENKSYMS_SURVEY
            + "\n"
            + "2. Reopen the same family only to land `scripts/zigux/check-phase2-genksyms-survey-packet.py` so the dedicated survey note fails closed against `Documentation/zigux/phase2-closure.md`, `Documentation/zigux/phase2-genksyms-dual-implementation-survey.md`, `scripts/zigux/README.md`, and `zigux/tests/README.md` without reopening those already-aligned reminder bodies.\n",
            encoding="utf-8",
        )
        try:
            check_root(sample_root)
        except SystemExit as exc:
            assert "stale genksyms survey marker" in str(exc)
            case_count += 1
        else:
            raise AssertionError("expected stale genksyms survey marker failure")
        write_sample_root(sample_root)
...
def main() -> int:
    ...
    print(f"PHASE2_GENKSYMS_SURVEY_PACKET_FORBIDDEN_SURVEY_MARKER_COUNT={len(FORBIDDEN_GENKSYMS_SURVEY_MARKERS)}")
