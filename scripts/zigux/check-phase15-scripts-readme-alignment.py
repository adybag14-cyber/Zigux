...
SHARED_SUMMARY_GUARD_SNIPPET = (
    "- `check-phase15-shared-summary-gap.py` remains the dedicated shared-summary drift guard for the parked "
    "Phase 15 governance packet's docs-root, checklist, scripts-root, and tests-root reminder wording, "
    "including the no-approval and reopen-trigger maintenance posture that must stay explicit without widening "
    "into a freeze-map status change."
)

README_SNIPPETS = (
    "- keep the parked Phase 15 governance packet explicit in the tests root too: `Documentation/zigux/README.md`, `Documentation/zigux/freeze-map.md`, `Documentation/zigux/phase15-freeze-map-governance.md`, `Documentation/zigux/phase15-architecture-council-review-process.md`, `Documentation/zigux/phase15-parity-scorecard-survey.md`, `Documentation/zigux/phase15-parity-scorecard.md`, `Documentation/zigux/phase15-indefinite-c-policy.md`, `Documentation/zigux/phase15-handoff-next-steps-survey.md`, `Documentation/zigux/phase15-readiness-gate-survey.md`, `Documentation/zigux/phase15-governance-lane-sequencing.md`, `Documentation/zigux/review-checklist.md`, `scripts/zigux/check-phase15-scripts-readme-alignment.py`, `scripts/zigux/check-phase15-review-process-handoff.py`, `.github/workflows/zigux-bootstrap.yml`, `zigux/tests/phase15_architecture_council_review_process_manifest.json`, `zigux/tests/phase15_build.zig`, `zigux/tests/phase15_freeze_map_governance.zig`, `zigux/tests/phase15_parity_scorecard.zig`, `zigux/tests/phase15_architecture_council_review_process.zig`, `zigux/tests/phase15_indefinite_c_policy.json`, `zigux/tests/phase15_indefinite_c_policy.zig`, `zigux/tests/phase15_indefinite_c_blocker_evidence.zig`, `zigux/tests/phase15_indefinite_c_lane_owner_alignment.zig`, `zigux/tests/phase15_governance_lane_sequencing.zig`, `zigux/tests/phase15_readiness_gate.zig`, `zigux/Makefile`, and `make -C zigux phase15` should continue to keep the current freeze-map, review-process, parity-scorecard-survey, parity-scorecard, handoff-next-steps, blocker-evidence, indefinite-C policy, lane-owner alignment, governance-lane sequencing, and readiness-gate governance packet reviewable through the shipped scripts-root validator-first route, the workflow-backed replay, and the shared build-and-make path without implying any Architecture Council approval for a freeze-map status change.",
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase15_scripts_readme_alignment_") as tmp_dir:
        ...
        missing_readme_snippet = SHARED_SUMMARY_GUARD_SNIPPET
        _write(root / README_REL, baseline_readme.replace(missing_readme_snippet + "\n", "", 1))
        _assert_only(
            validate(root),
            [f"readme:missing:{missing_readme_snippet}"],
            "missing_shared_summary_readme_snippet_guard_failed",
        )
        _write(root / README_REL, baseline_readme)
        case_count += 1

        updated_readme = baseline_readme.replace(
            "Documentation/zigux/phase15-parity-scorecard-survey.md`, ",
            "",
            1,
        ).replace(
            "review-process, parity-scorecard-survey, parity-scorecard, handoff-next-steps, ",
            "review-process, parity-scorecard, handoff-next-steps, ",
            1,
        )
        _write(root / README_REL, updated_readme)
        _assert_only(
            validate(root),
            [f"readme:missing:{README_SNIPPETS[0]}"],
            "missing_parity_scorecard_survey_readme_guard_failed",
        )
        _write(root / README_REL, baseline_readme)
        case_count += 1
