...
def collect_issues(root: Path) -> list[str]:
    ...
    if isinstance(c_harness, dict):
        ...
        require(
            ...
            "c_harness helper roster no longer matches the phase1 fixture order",
            issues,
        )
        require(
...
def build_current_like_root(root: Path) -> None:
    ...
    for path in (fixture_path, blockers_path):
        if path.is_dir():
            shutil.rmtree(path)
        elif path.exists():
            path.unlink()
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_shared_replay_roster_") as tmp_dir:
        ...
        blockers_path.write_text(json.dumps(blockers, indent=2) + "\n", encoding="utf-8")
        assert "c_harness helper roster no longer matches the phase1 fixture order" in collect_issues(root)
        cases += 1
