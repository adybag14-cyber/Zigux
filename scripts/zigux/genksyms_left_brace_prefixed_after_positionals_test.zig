...
test "genksyms bridge treats left-brace-prefixed values after positionals as request input" {
...
test "genksyms bridge accepts left-brace-prefixed required option arguments after positionals" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "genksyms bridge keeps left-brace-prefixed terminator tail out of option parsing" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "genksyms bridge renders left-brace-prefixed terminator tail in bridge json" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
