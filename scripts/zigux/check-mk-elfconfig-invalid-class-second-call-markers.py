...
ZIG_TOOL = ROOT / "scripts" / "zigux" / "mk_elfconfig_fd_invalid_class_second_call_test.zig"
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-invalid-class-second-call-") as tmp_dir:
        ...
        duplicate_path.write_text(
            "\n".join(
                [
                    EXPECTED_MARKERS["invalid_class_then_elf32"],
                    EXPECTED_MARKERS["invalid_class_then_elf32"],
                    EXPECTED_MARKERS["invalid_class_then_not_elf"],
                    EXPECTED_MARKERS["invalid_class_then_truncated"],
                ]
            )
            + "\n",
            encoding="utf-8",
    ...
    print("MK_ELFCONFIG_INVALID_CLASS_SECOND_CALL_MARKERS_SELF_TEST=pass")
    print("MK_ELFCONFIG_INVALID_CLASS_SECOND_CALL_MARKERS_SELF_TEST_CASE_COUNT=3")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the mk_elfconfig invalid-class second-call replay markers stay present and ordered."
    )
    parser.add_argument(
        "--self-test",
        action="store_true",
    ...
    print("MK_ELFCONFIG_INVALID_CLASS_SECOND_CALL_MARKERS=pass")
    print(f"MK_ELFCONFIG_INVALID_CLASS_SECOND_CALL_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
