...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_policy_unsafe_mmio_consumer_") as tmp_dir_str:
        ...
        _write(root, POLICY_BUILD_REL, "\n".join(REQUIRED_POLICY_BUILD_SNIPPETS) + "\n")
        _write(root, POLICY_BUILD_REL, "\n".join(snippet for snippet in REQUIRED_POLICY_BUILD_SNIPPETS if snippet != 'root_module.addImport("interop_policy", interop_policy_module);') + "\n")
        issues = validate(root)
        assert 'missing_policy_build_snippet:root_module.addImport("interop_policy", interop_policy_module);' in issues
