...
fn buildWrapper(allocator: std.mem.Allocator, tmp: testing.TmpDir) ![]u8 {
    const exe_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/tmp/{s}/genksyms-cluster-tail-wrapper",
        .{tmp.sub_path},
    );
...
test "required short option cluster tails stay data at executable boundary" {
    ...
    try testing.expectEqualStrings(
        "{\"tool\":\"scripts/genksyms/genksyms\",\"stdin\":\"cpp-stream\",\"stdout\":\"symversions\",\"argv\":[\"scripts/genksyms/genksyms\",\"-rVdqDwp\",\"-TqDdwVr\",\"-d\"],\"options\":{\"debug_level\":1,\"warnings\":false,\"dump_defs\":false,\"preserve\":false,\"reference_files\":[\"VdqDwp\"],\"dump_types_file\":\"qDdwVr\"}}\n",
        run_result.stdout,
    );
}
