...
test "runGenksymsCrc escapes visible NAK before NUL-hidden tail" {
    const input = "visible\x15nak\x00hidden\x15tail\nnext\n";

    var capture = try Capture(256).init(std.testing.allocator);
    defer capture.deinit();
    try genksyms_crc.runGenksymsCrc(input, &capture);

    const visible_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{genksyms_crc.crc32("visible\x15nak")});
    defer std.testing.allocator.free(visible_crc);
    const hidden_tail_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{genksyms_crc.crc32("hidden\x15tail")});
    defer std.testing.allocator.free(hidden_tail_crc);

    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"visible\\u0015nak\"") != null);
