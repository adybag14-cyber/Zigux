#!/usr/bin/env python3
"""Check that the Phase 2 validate action path stays aligned."""
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_validate_action_path_") as tmp_dir:
        ...
        for marker in EXPECTED_VALIDATORS:
            build_sample_root(root)
            path = resolve(root, TOOL_MANIFEST)
            payload = json.loads(path.read_text(encoding="utf-8"))
            payload["present_surfaces"]["validators"].remove(marker)
            path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
            assert ("MISSING_MANIFEST_SURFACE", f"validators:{marker}") in collect_issues(root)
            checks += 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the Phase 2 validate action path stays aligned."
    )
    ...
    print(f"PHASE2_VALIDATE_ACTION_PATH_DOCS_MARKER_COUNT={len(REQUIRED_DOCS_README_MARKERS)}")
    ...
    print(f"PHASE2_VALIDATE_ACTION_PATH_REVIEW_MARKER_COUNT={len(REQUIRED_REVIEW_MARKERS)}")
    print(f"PHASE2_VALIDATE_ACTION_PATH_SCRIPTS_MARKER_COUNT={len(REQUIRED_SCRIPTS_README_MARKERS)}")
    print(f"PHASE2_VALIDATE_ACTION_PATH_TESTS_MARKER_COUNT={len(REQUIRED_TESTS_README_MARKERS)}")
    return 0
