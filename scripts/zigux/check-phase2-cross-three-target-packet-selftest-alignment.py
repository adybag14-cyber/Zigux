...
CHECKER = ROOT / "scripts" / "zigux" / "check-phase2-cross-three-target-packet.py"
...
REQUIRED_CASE_MARKERS = (
    'replace(DIRECT_CHECKER_MARKERS[0], "", 1)',
    'replace(ALIGNMENT_MARKERS[0], "", 1)',
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_CROSS_THREE_TARGET_PACKET_SELFTEST_ALIGNMENT=fail")
...
def run_self_test() -> int:
    ...
    print("PHASE2_CROSS_THREE_TARGET_PACKET_SELFTEST_ALIGNMENT_SELF_TEST=pass")
    print(f"PHASE2_CROSS_THREE_TARGET_PACKET_SELFTEST_ALIGNMENT_SELF_TEST_CASE_COUNT={checks}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Keep the Lane 21 three-target packet checker's self-test surface intact."
    )
    ...
    print("PHASE2_CROSS_THREE_TARGET_PACKET_SELFTEST_ALIGNMENT=pass")
    print(
        "PHASE2_CROSS_THREE_TARGET_PACKET_SELFTEST_ALIGNMENT_SOURCE_MARKER_COUNT="
        f"{len(REQUIRED_SOURCE_MARKERS)}"
    )
    print(
        "PHASE2_CROSS_THREE_TARGET_PACKET_SELFTEST_ALIGNMENT_CASE_MARKER_COUNT="
