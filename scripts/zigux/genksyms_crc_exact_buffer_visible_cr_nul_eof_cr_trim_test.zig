...
const c_line_payload_len = 4096 - 1;
...
test "lane19 exact-buffer visible-leading-carriage-return EOF trims trailing carriage returns before an embedded NUL" {
    ...
    try gen.runGenksymsCrc(split_then_visible_leading_and_trailing_cr_then_nul.items, &capture);

    const exact_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32(split_then_visible_leading_and_trailing_cr_then_nul.items[0..c_line_payload_len])},
    );
    ...
    const unsplit_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32(split_then_visible_leading_and_trailing_cr_then_nul.items)});
