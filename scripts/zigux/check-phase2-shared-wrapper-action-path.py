#!/usr/bin/env python3
"""Check that the shared Phase 2 make-wrapper action path stays aligned."""
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    if len(workflow_indices) == len(REQUIRED_WORKFLOW_LINES) and workflow_indices != sorted(
        ...
        issues.append(("WORKFLOW_ORDER_MISMATCH", "phase2-shared-wrapper-route-order"))
    ...
    if len(makefile_indices) == len(REQUIRED_MAKEFILE_LINES) and makefile_indices != sorted(
        ...
        issues.append(("MAKEFILE_ORDER_MISMATCH", "phase2-shared-wrapper-packet-order"))
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_SHARED_WRAPPER_ACTION_PATH=fail")
...
def build_sample_root(root: Path) -> None:
    ...
    write_text(
        ...
        json.dumps(
            {"present_surfaces": {"make_wrappers": list(EXPECTED_MAKE_WRAPPERS)}},
            indent=2,
        )
        + "\n",
    )
...
def run_self_test() -> int:
    ...
    checks = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_shared_wrapper_action_path_") as tmp_dir:
        root = Path(tmp_dir)
        ...
        for marker in REQUIRED_WORKFLOW_LINES:
            ...
            path.write_text(
                replace_exact_line(path.read_text(encoding="utf-8"), marker, "run: other"),
                encoding="utf-8",
            )
        ...
        assert ("WORKFLOW_ORDER_MISMATCH", "phase2-shared-wrapper-route-order") in collect_issues(root)
        checks += 1
        ...
        path.write_text("\n".join((phony, *body)) + "\n", encoding="utf-8")
        assert ("MAKEFILE_ORDER_MISMATCH", "phase2-shared-wrapper-packet-order") in collect_issues(root)
        checks += 1
    ...
    print("PHASE2_SHARED_WRAPPER_ACTION_PATH_SELF_TEST=pass")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_SELF_TEST_CASE_COUNT={checks}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the shared Phase 2 make-wrapper action path stays aligned."
    )
    ...
    print("PHASE2_SHARED_WRAPPER_ACTION_PATH=pass")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_WORKFLOW_LINE_COUNT={len(REQUIRED_WORKFLOW_LINES)}")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_MAKEFILE_LINE_COUNT={len(REQUIRED_MAKEFILE_LINES)}")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_DOCS_MARKER_COUNT={len(REQUIRED_DOCS_README_MARKERS)}")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_CLOSURE_MARKER_COUNT={len(REQUIRED_CLOSURE_MARKERS)}")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_REVIEW_MARKER_COUNT={len(REQUIRED_REVIEW_MARKERS)}")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_SCRIPTS_MARKER_COUNT={len(REQUIRED_SCRIPTS_README_MARKERS)}")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_TESTS_MARKER_COUNT={len(REQUIRED_TESTS_README_MARKERS)}")
    print(f"PHASE2_SHARED_WRAPPER_ACTION_PATH_MANIFEST_MAKE_WRAPPER_COUNT={len(EXPECTED_MAKE_WRAPPERS)}")
    return 0
