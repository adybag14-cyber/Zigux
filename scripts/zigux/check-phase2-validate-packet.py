#!/usr/bin/env python3
"""Guard the current Phase 2 validator pair packet."""
...
def build_sample_root(root: Path) -> None:
    ...
    write_text(resolve(root, PHASE2_CLOSURE), "# Phase 2 Closure\n\n" + "\n".join(f"- {marker}" for marker in REQUIRED_CLOSURE_MARKERS) + "\n")
        ...
        json.dumps(
            {
                "present_surfaces": {
                    ...
                    "closure_notes": list(EXPECTED_CLOSURE_NOTES),
                }
            },
            indent=2,
        )
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_validate_packet_") as tmp_dir:
        ...
        manifest_path = resolve(root, TOOL_MANIFEST)
        payload = json.loads(manifest_path.read_text(encoding="utf-8"))
        payload["present_surfaces"]["closure_notes"].remove(EXPECTED_CLOSURE_NOTES[0])
        manifest_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        assert (
            "MISSING_MANIFEST_SURFACE",
            f"closure_notes:{EXPECTED_CLOSURE_NOTES[0]}",
        ) in collect_issues(root)
        checks += 1
    ...
    print(f"PHASE2_VALIDATE_PACKET_SELF_TEST_CASE_COUNT={checks}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Check that the Phase 2 validator pair packet stays aligned.")
    ...
    if args.write_sample_root is not None:
        ...
        print("PHASE2_VALIDATE_PACKET_SAMPLE_ROOT=written")
        print(f"PHASE2_VALIDATE_PACKET_SAMPLE_ROOT_PATH={args.write_sample_root.resolve()}")
        return 0
    ...
    print(f"PHASE2_VALIDATE_PACKET_WORKFLOW_LINE_COUNT={len(REQUIRED_WORKFLOW_LINES)}")
    print(f"PHASE2_VALIDATE_PACKET_MAKEFILE_LINE_COUNT={len(REQUIRED_MAKEFILE_LINES)}")
    print(f"PHASE2_VALIDATE_PACKET_README_MARKER_COUNT={len(REQUIRED_SCRIPTS_README_MARKERS)}")
    print(f"PHASE2_VALIDATE_PACKET_CLOSURE_MARKER_COUNT={len(REQUIRED_CLOSURE_MARKERS)}")
    return 0
