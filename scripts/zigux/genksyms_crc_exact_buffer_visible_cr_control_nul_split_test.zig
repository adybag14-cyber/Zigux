...
test "lane19 exact-buffer visible-CR control NUL split preserves the visible prefix and keeps the next line" {
    var exact_then_visible_cr_controls_then_nul_then_next_line = try std.ArrayList(u8).initCapacity(std.testing.allocator, gen.c_line_payload_len + 10);
    defer exact_then_visible_cr_controls_then_nul_then_next_line.deinit(std.testing.allocator);
    try exact_then_visible_cr_controls_then_nul_then_next_line.appendNTimes(std.testing.allocator, 'a', gen.c_line_payload_len);
    try exact_then_visible_cr_controls_then_nul_then_next_line.append(std.testing.allocator, '\r');
    try exact_then_visible_cr_controls_then_nul_then_next_line.append(std.testing.allocator, '\x08');
    try exact_then_visible_cr_controls_then_nul_then_next_line.append(std.testing.allocator, '\x0c');
    try exact_then_visible_cr_controls_then_nul_then_next_line.append(std.testing.allocator, '\x01');
    try exact_then_visible_cr_controls_then_nul_then_next_line.append(std.testing.allocator, 'b');
    try exact_then_visible_cr_controls_then_nul_then_next_line.append(std.testing.allocator, 0);
    try exact_then_visible_cr_controls_then_nul_then_next_line.append(std.testing.allocator, 'c');
    try exact_then_visible_cr_controls_then_nul_then_next_line.append(std.testing.allocator, '\n');
    try exact_then_visible_cr_controls_then_nul_then_next_line.appendSlice(std.testing.allocator, "x\n");

    var capture = try Capture(16384).init(std.testing.allocator);
    defer capture.deinit();
    try gen.runGenksymsCrc(exact_then_visible_cr_controls_then_nul_then_next_line.items, &capture);

    const exact_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32(exact_then_visible_cr_controls_then_nul_then_next_line.items[0..gen.c_line_payload_len])},
    );
    ...
    const untruncated_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("\r\x08\x0c\x01b\x00c")});
