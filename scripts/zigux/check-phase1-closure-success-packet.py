...
def repo_root(explicit_root: str | None) -> Path:
    return Path(explicit_root).resolve() if explicit_root else DEFAULT_ROOT.resolve()
...
def validate_root(root: Path) -> tuple[str, object]:
    missing_files = [
        str(relative_path)
        for relative_path in (WORKFLOW_PATH, VALIDATOR_PATH, CLOSURE_NOTE_PATH)
    ...
    closure_note_text = read_text(root, CLOSURE_NOTE_PATH)
    ...
    missing_closure_note = [
        marker for marker in REQUIRED_CLOSURE_NOTE_MARKERS if marker not in closure_note_text
    ]
...
def repo_root(_: str | None):
    return None


def collect_failures(_: object):
    return []
...
def run_self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="lane16-phase1-closure-success-") as tmp:
        ...
        assert payload == [str(CLOSURE_NOTE_PATH)]
        case_count += 1
...
def main() -> int:
    ...
    kind, payload = validate_root(repo_root(args.root))
    if kind != "pass":
