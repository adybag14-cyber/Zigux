...
test "genksyms abbreviated version side effect carries into later request options" {
...
test "genksyms abbreviated version side effect carries into later request arguments" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
