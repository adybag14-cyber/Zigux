...
test "runFixdep truncates parsed source contents at the first embedded NUL" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();

    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "visible-source.c",
        .data = "CONFIG_ZIGUX_SOURCE_VISIBLE_MODULE\x00CONFIG_ZIGUX_SOURCE_AFTER_NUL CONFIG_ZIGUX_SOURCE_AFTER_NUL_MODULE\n",
    });
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "visible-header.h",
        .data = "CONFIG_ZIGUX_HEADER_VISIBLE CONFIG_ZIGUX_HEADER_VISIBLE\n",
    });
    try tmp.dir.writeFile(std.testing.io, .{
    ...
    const source_path = try std.fmt.allocPrint(
        ...
        ".zig-cache/tmp/{s}/visible-source.c",
        .{tmp.sub_path[0..]},
    );
    ...
    const header_path = try std.fmt.allocPrint(
        std.testing.allocator,
        ".zig-cache/tmp/{s}/visible-header.h",
        .{tmp.sub_path[0..]},
    );
    ...
    try fixdep.runFixdep(
        ...
        depfile_path,
        "sample.o",
        "cc -c visible-source.c -o sample.o",
    );

    const expected = try std.fmt.allocPrint(
        std.testing.allocator,
        "savedcmd_sample.o := cc -c visible-source.c -o sample.o\n\n" ++
            "source_sample.o := {s}\n\n" ++
            "deps_sample.o := \\\n" ++
            "    $(wildcard include/config/ZIGUX_SOURCE_VISIBLE) \\\n" ++
            "  {s} \\\n" ++
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "ZIGUX_SOURCE_AFTER_NUL") == null);
