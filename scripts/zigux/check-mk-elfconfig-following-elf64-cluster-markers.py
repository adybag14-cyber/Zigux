...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-following-elf64-cluster-") as tmp_dir:
        ...
        missing_path = missing_root / "scripts" / "zigux" / "mk_elfconfig_fd_invalid_class_following_elf64_test.zig"
        ...
        duplicate_path = duplicate_root / "scripts" / "zigux" / "mk_elfconfig_fd_not_elf_following_elf64_test.zig"
    ...
    print("MK_ELFCONFIG_FOLLOWING_ELF64_CLUSTER_MARKERS_SELF_TEST=pass")
    print("MK_ELFCONFIG_FOLLOWING_ELF64_CLUSTER_MARKERS_SELF_TEST_CASE_COUNT=3")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the mk_elfconfig following-elf64 standalone replay cluster stays present."
    )
    parser.add_argument(
        "--self-test",
        action="store_true",
    ...
    print("MK_ELFCONFIG_FOLLOWING_ELF64_CLUSTER_MARKERS=pass")
    print(f"MK_ELFCONFIG_FOLLOWING_ELF64_CLUSTER_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
