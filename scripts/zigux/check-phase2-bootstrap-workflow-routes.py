...
def collect_phony_issues(text: str) -> list[tuple[str, str]]:
    phony_lines = [line.strip() for line in text.splitlines() if line.strip().startswith(".PHONY:")]
    if not phony_lines:
        return [("MISSING_MAKEFILE_PHONY_LINE", ".PHONY:")]
    phony_tokens = tuple(phony_lines[0].split(":", 1)[1].split())
    missing = [token for token in PHONY_ROUTE_TOKENS if token not in phony_tokens]
    return [("MISSING_MAKEFILE_PHONY_TOKEN", token) for token in missing]
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues.extend(collect_phony_issues(makefile_text))
...
def remove_from_phony_line(text: str, token: str) -> str:
    lines = text.splitlines()
    for index, line in enumerate(lines):
        stripped = line.strip()
        if stripped.startswith(".PHONY:") and token in stripped:
            prefix, rest = line.split(":", 1)
            tokens = rest.split()
            try:
                tokens.remove(token)
            except ValueError as exc:
                raise AssertionError(f"phony token not found: {token}") from exc
            lines[index] = f"{prefix}: {' '.join(tokens)}"
            return "\n".join(lines) + "\n"
    raise AssertionError(f"phony token not found: {token}")
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_bootstrap_workflow_routes_") as tmp_dir:
        ...
        for token in PHONY_ROUTE_TOKENS:
            build_sample_root(root)
            path = resolve_path(root, MAKEFILE)
            write_text(path, remove_from_phony_line(read_text(path), token))
            assert ("MISSING_MAKEFILE_PHONY_TOKEN", token) in collect_issues(root)
            checks_run += 1
