...
test "lane19 multi-megabyte exact-buffer split keeps a visible control tail with trimmed carriage returns beyond the old CLI cap" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + c_line_payload_len + 12,
    );
