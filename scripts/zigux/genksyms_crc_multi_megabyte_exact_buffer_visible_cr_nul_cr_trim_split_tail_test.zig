...
test "lane19 multi-megabyte exact-buffer split preserves visible CR-led bytes, truncates at NUL, trims carriage returns, and still reaches the next tail line beyond the old CLI cap" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + gen.c_line_payload_len + 11,
    );
    ...
    const controls_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("b")});
    defer std.testing.allocator.free(controls_crc);
    const untruncated_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32("\r\rb\r\r\x00c")},
    );
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, controls_crc) == null);
