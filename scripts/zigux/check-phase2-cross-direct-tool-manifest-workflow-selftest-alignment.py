...
CHECKER = ROOT / "scripts" / "zigux" / "check-phase2-cross-direct-tool-manifest-workflow.py"
...
def run_check(root: Path) -> int:
    ...
    if issues:
        for code, detail in issues:
            print(f"PHASE2_CROSS_DIRECT_TOOL_MANIFEST_WORKFLOW_SELFTEST_ALIGNMENT_ISSUE={code}:{detail}")
        print(f"PHASE2_CROSS_DIRECT_TOOL_MANIFEST_WORKFLOW_SELFTEST_ALIGNMENT_ISSUE_COUNT={len(issues)}")
        return 1

    print("PHASE2_CROSS_DIRECT_TOOL_MANIFEST_WORKFLOW_SELFTEST_ALIGNMENT=pass")
    print(f"PHASE2_CROSS_DIRECT_TOOL_MANIFEST_WORKFLOW_SELFTEST_ALIGNMENT_SOURCE_MARKER_COUNT={len(REQUIRED_SOURCE_MARKERS)}")
    print(f"PHASE2_CROSS_DIRECT_TOOL_MANIFEST_WORKFLOW_SELFTEST_ALIGNMENT_CASE_MARKER_COUNT={len(REQUIRED_CASE_MARKERS)}")
    return 0
...
def run_self_test() -> int:
    checks = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_cross_direct_tool_manifest_workflow_alignment_") as tmp_dir:
        root = Path(tmp_dir)
        ...
        for marker in REQUIRED_SOURCE_MARKERS:
            ...
            checker_path.write_text(read_text(checker_path).replace(marker, "", 1), encoding="utf-8")
        ...
        build_self_test_root(root)
        checker_path = resolve_path(root, CHECKER)
        checker_path.write_text(read_text(checker_path) + REQUIRED_SOURCE_MARKERS[0] + "\n", encoding="utf-8")
        assert run_check(root) == 1
        checks += 1

    print("PHASE2_CROSS_DIRECT_TOOL_MANIFEST_WORKFLOW_SELFTEST_ALIGNMENT_SELF_TEST=pass")
    print(f"PHASE2_CROSS_DIRECT_TOOL_MANIFEST_WORKFLOW_SELFTEST_ALIGNMENT_SELF_TEST_CASE_COUNT={checks}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Keep the Lane 21 direct tool-manifest workflow checker self-test markers intact."
    )
