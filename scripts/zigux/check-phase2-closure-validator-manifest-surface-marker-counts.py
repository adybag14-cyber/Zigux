#!/usr/bin/env python3
"""Fail closed when the closure-validator manifest-surface coverage checker drifts."""
...
COVERAGE_CHECKER_REL = Path("scripts/zigux/check-phase2-closure-validator-manifest-surface-coverage.py")
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    for surface_key in EXPECTED_SURFACE_ANCHORS:
        require_exact_count(issues, checker_text, EXPECTED_CONSTANT_NAMES[surface_key], 3)
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS=fail")
...
def run_self_test() -> int:
    checks_run = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_closure_validator_manifest_surface_marker_counts_") as tmp_dir:
        root = Path(tmp_dir)
        ...
        checker_path.write_text(
            read_text(checker_path).replace(
                "constant_name = EXPECTED_CONSTANT_NAMES[surface_key]",
                "constant_name = DRIFTED_CONSTANT_NAMES[surface_key]",
                1,
            ),
            encoding="utf-8",
        )
        assert (
            "MISSING_MARKER",
            "constant_name = EXPECTED_CONSTANT_NAMES[surface_key]:expected=1:actual=0",
        ...
        assert (
            "DUPLICATE_MARKER",
            "ensure_manifest_surface_anchors(issues, manifest, surface_key):expected=1:actual=2",
        ) in collect_issues(root)
        checks_run += 1
    ...
    print("PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SELF_TEST=pass")
    print(f"PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SELF_TEST_CASE_COUNT={checks_run}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check the closure-validator manifest-surface coverage checker for duplicate or missing surface markers."
    )
    ...
    if args.write_sample_root is not None:
        ...
        print(f"PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SAMPLE_ROOT={args.write_sample_root.resolve()}")
        return 0
    ...
    print("PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS=pass")
    print(f"PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_MARKER_COUNTS_SURFACE_COUNT={len(EXPECTED_SURFACE_ANCHORS)}")
    return 0
