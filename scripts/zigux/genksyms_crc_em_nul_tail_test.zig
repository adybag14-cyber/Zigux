...
test "runGenksymsCrc escapes visible EM before NUL-hidden tail" {
    const input = "visible\x19em\x00hidden\x19tail\nnext\n";

    var capture = try Capture(256).init(std.testing.allocator);
    defer capture.deinit();
    try genksyms_crc.runGenksymsCrc(input, &capture);

    const visible_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{genksyms_crc.crc32("visible\x19em")});
    defer std.testing.allocator.free(visible_crc);
    const hidden_tail_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{genksyms_crc.crc32("hidden\x19tail")});
    defer std.testing.allocator.free(hidden_tail_crc);

    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"visible\\u0019em\"") != null);
