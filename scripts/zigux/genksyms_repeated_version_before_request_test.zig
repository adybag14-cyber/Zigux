...
test "genksyms repeated long version side effects accumulate before later request options" {
...
test "genksyms mixed repeated version side effects accumulate before later request arguments" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
