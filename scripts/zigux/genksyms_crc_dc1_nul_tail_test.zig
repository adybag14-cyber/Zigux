...
test "runGenksymsCrc escapes visible DC1 before hiding a NUL tail" {
    const visible = "prefix" ++ [_]u8{0x11} ++ "dc1";
    const input = visible ++ [_]u8{0} ++ "hidden" ++ [_]u8{0x11} ++ "tail\nnext\n";
    ...
    const hidden_crc = try std.fmt.allocPrint(std.testing.allocator, "0x{x:0>8}", .{gen.crc32("hidden" ++ [_]u8{0x11} ++ "tail")});
    ...
    try expectContains(capture.list.items, "\"input\":\"prefix\\u0011dc1\"");
