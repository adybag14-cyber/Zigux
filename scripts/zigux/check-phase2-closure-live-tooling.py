...
def collect_toolchain_policy_issues(policy: object) -> list[tuple[str, str]]:
    ...
    if upgrade_policy.get("required_make_routes") != ["phase2-toolchain", "phase2-validate", "phase2-cross"]:
...
def collect_genksyms_manifest_issues(payload: object) -> list[tuple[str, str]]:
    ...
    if not isinstance(cases, list) or any(case not in cases for case in expected_cases):
    ...
    if not isinstance(expected_packet, list) or any(name not in expected_packet for name in expected_files):
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_closure_live_tooling_") as tmp_dir:
        ...
        write_json(manifest_path, {"case_count": 7, "cases": [], "bridge_expected_packet": []})
        issues = collect_issues(root)
