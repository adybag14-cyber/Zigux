...
GENKSYMS_MANIFEST = ROOT / "zigux" / "tests" / "fixtures" / "genksyms_bridge" / "manifest.json"
...
def collect_toolchain_policy_issues(policy: object) -> list[tuple[str, str]]:
    ...
    if policy.get("minimum_version") != "0.17.0-dev.87+9b177a7d2":
        issues.append(("INVALID_TOOLCHAIN_POLICY", f"minimum_version::{policy.get('minimum_version')}"))
    ...
    if upgrade_policy.get("channel_minimum_lockstep") is not True:
        issues.append(("INVALID_TOOLCHAIN_POLICY", f"channel_minimum_lockstep::{upgrade_policy.get('channel_minimum_lockstep')}"))
    if upgrade_policy.get("archive_target_scope") != ["x86_64-linux"]:
        issues.append(("INVALID_TOOLCHAIN_POLICY", f"archive_target_scope::{upgrade_policy.get('archive_target_scope')}"))
    if upgrade_policy.get("required_make_routes") != REQUIRED_MAKE_ROUTES:
        issues.append(("INVALID_TOOLCHAIN_POLICY", f"required_make_routes::{upgrade_policy.get('required_make_routes')}"))
    return issues
...
def collect_cross_target_issues(payload: object) -> list[tuple[str, str]]:
    ...
    expected = {
        ("x86_64-linux", "archive_required", "make -C zigux phase2-cross"),
        ("aarch64-linux", "route_contract_only", "make -C zigux phase2-cross"),
    }
    found = {
        (entry.get("target"), entry.get("validation_mode"), entry.get("route"))
...
def collect_genksyms_manifest_issues(payload: object) -> list[tuple[str, str]]:
    ...
    if not isinstance(cases, list) or any(case not in cases for case in EXPECTED_GENKSYMS_CASES):
        issues.append(("INVALID_GENKSYMS_MANIFEST", f"cases::{cases}"))
    expected_packet = payload.get("bridge_expected_packet")
    if not isinstance(expected_packet, list) or any(name not in expected_packet for name in EXPECTED_GENKSYMS_PACKET):
        issues.append(("INVALID_GENKSYMS_MANIFEST", f"bridge_expected_packet::{expected_packet}"))
    standalone_proofs = payload.get("standalone_proof_packet")
    if not isinstance(standalone_proofs, list) or any(name not in standalone_proofs for name in EXPECTED_STANDALONE_PROOFS):
        issues.append(("INVALID_GENKSYMS_MANIFEST", f"standalone_proof_packet::{standalone_proofs}"))
    return issues
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues.extend(collect_exact_count_markers(closure_text, EXACT_COUNT_CLOSURE_MARKERS, "EXACT_COUNT_CLOSURE_MARKERS"))
...
def build_self_test_root(root: Path) -> None:
    ...
    write_json(
        ...
        {
            "case_count": 10,
            "cases": EXPECTED_GENKSYMS_CASES,
            "bridge_expected_packet": EXPECTED_GENKSYMS_PACKET,
            "standalone_proof_packet": EXPECTED_STANDALONE_PROOFS,
        },
    )
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_closure_live_tooling_") as tmp_dir:
        ...
        issues = collect_issues(root)
        assert any(code == "INVALID_TOOLCHAIN_POLICY" for code, _ in issues)
        checks_run += 1
        ...
        write_json(manifest_path, {"case_count": 7, "cases": [], "bridge_expected_packet": [], "standalone_proof_packet": []})
        issues = collect_issues(root)
