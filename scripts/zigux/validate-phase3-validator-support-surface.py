...
REQUIRED_SHARED_GAP_MARKERS = (
    "PHASE3_SHARED_REMINDER_GAP=current master now keeps the docs-root and tests-root Phase 3 summaries aligned with the landed notifier binding companion, bounded kernel-export-shim note, focused export/UAPI layout replay, low-level-wrapper reminder packet, and separately readable shared validator entrypoint, while scripts-root inventory follow-through remains a separate reminder surface",
    "PHASE3_SHARED_REMINDER_NEXT_STEP=keep future same-lane follow-through limited to separate scripts-root inventory drift or a fresh shared-summary reread only if one of the bounded Phase 3 reminder surfaces changes again",
    ...
    "`zigux/tests/README.md` is now aligned with the bounded current packet: it keeps the returned notifier-binding, kernel-export-shim, focused export/UAPI layout replay, and low-level-wrapper reminder packet explicit without widening them into the absent export/UAPI survey, shared validator, or catalog routes.",
    "The earlier shared-reminder drift is now closed on the shared docs-root and tests-root summaries.",
)
...
SELF_TEST_CASES = (
    ...
    ("gap", "PHASE3_SHARED_REMINDER_GAP=current master now keeps the docs-root and tests-root Phase 3 summaries aligned with the landed notifier binding companion, bounded kernel-export-shim note, focused export/UAPI layout replay, low-level-wrapper reminder packet, and separately readable shared validator entrypoint, while scripts-root inventory follow-through remains a separate reminder surface", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: PHASE3_SHARED_REMINDER_GAP=current master now keeps the docs-root and tests-root Phase 3 summaries aligned with the landed notifier binding companion, bounded kernel-export-shim note, focused export/UAPI layout replay, low-level-wrapper reminder packet, and separately readable shared validator entrypoint, while scripts-root inventory follow-through remains a separate reminder surface"),
    ("gap", "PHASE3_SHARED_REMINDER_NEXT_STEP=keep future same-lane follow-through limited to separate scripts-root inventory drift or a fresh shared-summary reread only if one of the bounded Phase 3 reminder surfaces changes again", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: PHASE3_SHARED_REMINDER_NEXT_STEP=keep future same-lane follow-through limited to separate scripts-root inventory drift or a fresh shared-summary reread only if one of the bounded Phase 3 reminder surfaces changes again"),
    ...
    ("gap", "`zigux/tests/README.md` is now aligned with the bounded current packet: it keeps the returned notifier-binding, kernel-export-shim, focused export/UAPI layout replay, and low-level-wrapper reminder packet explicit without widening them into the absent export/UAPI survey, shared validator, or catalog routes.", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: `zigux/tests/README.md` is now aligned with the bounded current packet: it keeps the returned notifier-binding, kernel-export-shim, focused export/UAPI layout replay, and low-level-wrapper reminder packet explicit without widening them into the absent export/UAPI survey, shared validator, or catalog routes."),
    ("gap", "The earlier shared-reminder drift is now closed on the shared docs-root and tests-root summaries.", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: The earlier shared-reminder drift is now closed on the shared docs-root and tests-root summaries."),
)
