...
REQUIRED_MARKERS = (
    ...
    "Current `master` now carries one bounded `dev_t` starter packet with paired `dev_t` and version bindings, one focused helper-local `err_ptr` / `xarray` interop slice, and one focused helper-local policy slice.",
    ...
    "Do not treat the current starter packet, its manifest-backed replay guard, its direct Zig compile replay, plus the two focused helper slices as evidence that the broader Phase 3 ABI substrate",
    ...
    "`Documentation/zigux/phase3-shared-reminder-gap.md` is the current direct-readback tracker for that cleanup",
    "Keep that remaining follow-up focused on those shared summaries",
