...
REQUIRED_MARKERS = (
    ...
    "Current `master` now carries one bounded `dev_t` starter packet, one focused helper-local `err_ptr` / `xarray` interop slice, and one focused helper-local policy slice.",
    ...
    "Do not treat the current starter packet, its manifest-backed replay guard, plus the two focused helper slices as evidence that the broader Phase 3 ABI substrate",
