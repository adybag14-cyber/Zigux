...
REQUIRED_SHARED_GAP_MARKERS = (
    "PHASE3_SHARED_REMINDER_GAP=current master now keeps the landed notifier binding companion plus one bounded kernel-export-shim note and focused export/UAPI layout replay explicit, but the shared docs-root and tests-root Phase 3 summaries drift in opposite directions: Documentation/zigux/README.md still overclaims absent export/UAPI survey and manifest companions while zigux/tests/README.md still frames the landed low-level-wrapper reminder packet together with the returned notifier-binding and layout replay packet as broader gaps",
    "PHASE3_SHARED_REMINDER_NEXT_STEP=refresh one shared summary surface at a time, starting with Documentation/zigux/README.md so it swaps the absent export/UAPI survey and manifest claims for the returned phase3-kernel-export-shim-governance.md note, zigux/bindings/notifier_abi.zig, and the focused phase3_export_uapi_layout replay pair, then follow with the tests-root Phase 3 summary so it stops omitting the landed low-level-wrapper reminder packet while it also retires the stale notifier-binding and focused layout-replay gap wording",
    ...
    "`zigux/tests/README.md` still needs a separate tests-root refresh so it stops omitting the landed low-level-wrapper reminder packet and stops treating `zigux/bindings/notifier_abi.zig`, `zigux/kernel/export_shim.zig`, and the focused export/UAPI layout replay as broader gaps.",
)
...
SELF_TEST_CASES = (
    ...
    ("gap", "PHASE3_SHARED_REMINDER_GAP=current master now keeps the landed notifier binding companion plus one bounded kernel-export-shim note and focused export/UAPI layout replay explicit, but the shared docs-root and tests-root Phase 3 summaries drift in opposite directions: Documentation/zigux/README.md still overclaims absent export/UAPI survey and manifest companions while zigux/tests/README.md still frames the landed low-level-wrapper reminder packet together with the returned notifier-binding and layout replay packet as broader gaps", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: PHASE3_SHARED_REMINDER_GAP=current master now keeps the landed notifier binding companion plus one bounded kernel-export-shim note and focused export/UAPI layout replay explicit, but the shared docs-root and tests-root Phase 3 summaries drift in opposite directions: Documentation/zigux/README.md still overclaims absent export/UAPI survey and manifest companions while zigux/tests/README.md still frames the landed low-level-wrapper reminder packet together with the returned notifier-binding and layout replay packet as broader gaps"),
    ("gap", "`Documentation/zigux/README.md` is now the highest-value shared reminder drift because it still treats `Documentation/zigux/phase3-export-uapi-boundary-survey.md`, `Documentation/zigux/phase3-linux-zigux-header-governance.md`, `scripts/zigux/validate-phase3-export-uapi-survey.py`, and `zigux/tests/fixtures/phase3_abi_manifest.json` as live docs-root evidence even though authenticated reads still return them missing.", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: `Documentation/zigux/README.md` is now the highest-value shared reminder drift because it still treats `Documentation/zigux/phase3-export-uapi-boundary-survey.md`, `Documentation/zigux/phase3-linux-zigux-header-governance.md`, `scripts/zigux/validate-phase3-export-uapi-survey.py`, and `zigux/tests/fixtures/phase3_abi_manifest.json` as live docs-root evidence even though authenticated reads still return them missing."),
    ("gap", "`zigux/tests/README.md` still needs a separate tests-root refresh so it stops omitting the landed low-level-wrapper reminder packet and stops treating `zigux/bindings/notifier_abi.zig`, `zigux/kernel/export_shim.zig`, and the focused export/UAPI layout replay as broader gaps.", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: `zigux/tests/README.md` still needs a separate tests-root refresh so it stops omitting the landed low-level-wrapper reminder packet and stops treating `zigux/bindings/notifier_abi.zig`, `zigux/kernel/export_shim.zig`, and the focused export/UAPI layout replay as broader gaps."),
)
