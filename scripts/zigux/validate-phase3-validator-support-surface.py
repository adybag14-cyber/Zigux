...
REQUIRED_NOTE_MARKERS = (
    ...
    "Keep any remaining follow-up focused on the shared docs-root and tests-root reminder refresh first, then separate scripts-root inventory drift only if that broader surface changes again.",
    ...
    "records the still-open docs-root and tests-root reminder drift",
)
...
SELF_TEST_CASES = (
    ...
    ("note", "`Documentation/zigux/review-checklist.md` stays aligned with that bounded three-slice posture, but `Documentation/zigux/README.md` and `zigux/tests/README.md` still need the shared-summary refresh recorded in `Documentation/zigux/phase3-shared-reminder-gap.md` before they can be treated as aligned current-master reminder surfaces.", f"missing {NOTE_PATH.as_posix()} marker: `Documentation/zigux/review-checklist.md` stays aligned with that bounded three-slice posture, but `Documentation/zigux/README.md` and `zigux/tests/README.md` still need the shared-summary refresh recorded in `Documentation/zigux/phase3-shared-reminder-gap.md` before they can be treated as aligned current-master reminder surfaces."),
    ("note", "`Documentation/zigux/phase3-shared-reminder-gap.md` still records that the shared docs-root and tests-root export/UAPI reminder drift is open on current `master`.", f"missing {NOTE_PATH.as_posix()} marker: `Documentation/zigux/phase3-shared-reminder-gap.md` still records that the shared docs-root and tests-root export/UAPI reminder drift is open on current `master`."),
    ("note", "Keep any remaining follow-up focused on the shared docs-root and tests-root reminder refresh first, then separate scripts-root inventory drift only if that broader surface changes again.", f"missing {NOTE_PATH.as_posix()} marker: Keep any remaining follow-up focused on the shared docs-root and tests-root reminder refresh first, then separate scripts-root inventory drift only if that broader surface changes again."),
    ("note", "records the still-open docs-root and tests-root reminder drift", f"missing {NOTE_PATH.as_posix()} marker: records the still-open docs-root and tests-root reminder drift"),
    ("gap", "PHASE3_SHARED_REMINDER_GAP=current master now keeps the landed notifier binding companion plus one focused export-or-UAPI layout replay explicit in the dedicated ABI note, but the shared docs-root and tests-root Phase 3 summaries still undercount that returned surface, so the earlier shared-reminder sentence drift is not fully closed yet", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: PHASE3_SHARED_REMINDER_GAP=current master now keeps the landed notifier binding companion plus one focused export-or-UAPI layout replay explicit in the dedicated ABI note, but the shared docs-root and tests-root Phase 3 summaries still undercount that returned surface, so the earlier shared-reminder sentence drift is not fully closed yet"),
    ("gap", "`Documentation/zigux/README.md` still needs a docs-root Phase 3 summary refresh so it stops treating `zigux/bindings/notifier_abi.zig` and the focused export/UAPI layout replay as broader gaps.", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: `Documentation/zigux/README.md` still needs a docs-root Phase 3 summary refresh so it stops treating `zigux/bindings/notifier_abi.zig` and the focused export/UAPI layout replay as broader gaps."),
    ("gap", "`zigux/tests/README.md` still needs the same tests-root Phase 3 summary refresh for that returned shared ABI surface.", f"missing {SHARED_REMINDER_GAP_PATH.as_posix()} marker: `zigux/tests/README.md` still needs the same tests-root Phase 3 summary refresh for that returned shared ABI surface."),
)
