...
test "runGenksymsCrc escapes visible bell before NUL and hides tail" {
    const visible = "bell\x07vis";
    const hidden = "hidden\x07tail";
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"bell\\u0007vis\"") != null);
