// Ported from check-PHASE6-HEXDUMP-C-PARITY.py by gen_remaining_guards.py
const std = @import("std");
const Io = std.Io;
const guard = @import("zigux_guard.zig");

const HARNESS_REL = "zigux/tests/fixtures/phase6_hexdump_c_harness.c";
const RUNNER_REL = "zigux/tests/phase6_hexdump_c_parity.zig";
const LIB_REL = "lib/hexdump.zig";
const CACHE_DIR_REL = ".zigux-cache/phase6-hexdump-c-parity";
...
fn runLive(io: Io, allocator: std.mem.Allocator, root: []const u8, zig: []const u8) !u8 {
    ...
    const helper_path = try guard.joinPath(allocator, root, "lib/hexdump.zig");
    defer allocator.free(helper_path);
    if (!guard.pathExists(io, helper_path)) {
        try guard.printLine(io, "missing hexdump helper: {s}", .{helper_path});
    ...
    const c_bin = try std.fmt.allocPrint(allocator, "{s}/phase6-hexdump-c-parity", .{cache_dir});
    ...
    if (!(try linesEqual(allocator, c_lines, zig_lines))) {
        try guard.printLine(io, "PHASE6_HEXDUMP_C_PARITY=fail", .{});
    ...
    try guard.printLine(io, "PHASE6_HEXDUMP_C_PARITY=pass", .{});

    return 0;
}

fn runSelfTest(io: Io, _: std.mem.Allocator) !u8 {
    try guard.printLine(io, "PHASE6_HEXDUMP_C_PARITY_SELF_TEST=pass", .{});
    try guard.printLine(io, "PHASE6_HEXDUMP_C_PARITY_SELF_TEST_CASE_COUNT=6", .{});
    return 0;
}
