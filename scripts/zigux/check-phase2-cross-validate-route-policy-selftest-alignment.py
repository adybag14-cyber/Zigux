...
CHECKER = ROOT / "scripts" / "zigux" / "check-phase2-cross-validate-route-policy.py"
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_CROSS_VALIDATE_ROUTE_POLICY_SELFTEST_ALIGNMENT=fail")
...
def run_self_test() -> int:
    ...
    checks = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_cross_validate_route_policy_alignment_") as tmp_dir:
        root = Path(tmp_dir)
    ...
    print("PHASE2_CROSS_VALIDATE_ROUTE_POLICY_SELFTEST_ALIGNMENT_SELF_TEST=pass")
    print(f"PHASE2_CROSS_VALIDATE_ROUTE_POLICY_SELFTEST_ALIGNMENT_SELF_TEST_CASE_COUNT={checks}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Keep the Lane 21 cross validate-route-policy checker's self-test surface intact."
    )
    ...
    print("PHASE2_CROSS_VALIDATE_ROUTE_POLICY_SELFTEST_ALIGNMENT=pass")
    print(f"PHASE2_CROSS_VALIDATE_ROUTE_POLICY_SELFTEST_ALIGNMENT_SOURCE_MARKER_COUNT={len(REQUIRED_SOURCE_MARKERS)}")
    print(f"PHASE2_CROSS_VALIDATE_ROUTE_POLICY_SELFTEST_ALIGNMENT_CASE_MARKER_COUNT={len(REQUIRED_CASE_MARKERS)}")
    return 0
