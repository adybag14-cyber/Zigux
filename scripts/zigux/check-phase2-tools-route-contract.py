...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase2_tools_route_contract_") as tmp_dir:
        ...
        validate_path = root / VALIDATE_PHASE2
        validate_path.write_text(
            duplicate_exact_line(read_text(validate_path), VALIDATE_REQUIRED_SNIPPETS[5], "    "),
            encoding="utf-8",
        )
        issues = collect_issues(root)
        if ("DUPLICATE_VALIDATE_PHASE2_SNIPPET", f"{VALIDATE_REQUIRED_SNIPPETS[5]}:count=2") not in issues:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check that the live Phase 2 tools packet stays aligned across "
