...
def run_self_test() -> int:
    cases = [
        ...
        (
            "manifest_tail_inclusive_boundary_fixture_keys_drift",
            "manifest:tail_inclusive_boundary_fixture_keys:expected_current_packet",
        ),
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_find_bit_review_") as tmp_dir:
        ...
        if cases[8][1] not in collect_failures(tmp_root):
            raise SystemExit("phase1-find-bit-review:self-test:manifest_tail_inclusive_boundary_fixture_keys_drift")
