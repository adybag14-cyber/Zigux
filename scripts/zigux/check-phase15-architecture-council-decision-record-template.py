...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="phase15_decision_record_template_") as tmp_dir:
        ...
        _write(
            missing_template_rule / DECISION_RECORD_TEMPLATE_PATH,
        ...
        expected = [
            "template:missing_usage_rule:If any required field above cannot be stated honestly, keep the request blocked and leave the C implementation as the product source of truth."
        ]
        ...
        _write(
            missing_checklist_boundary / REVIEW_CHECKLIST_PATH,
            _sample_review_checklist().replace(
                "owners of the exact Architecture Council field inventory, stay-in-C closeout record, and reopen-evidence details, and `Documentation/zigux/phase15-indefinite-c-policy.md` remains the dedicated stay-in-C policy companion for retained blocker posture, trigger-specific evidence refresh, and return-to-blocked wording?",
                "owners of the exact Architecture Council field inventory?",
                1,
            ),
        )
        ...
        expected = [
            "review_checklist:missing_prompt_boundary:stay-in-C closeout record",
            "review_checklist:missing_prompt_boundary:reopen-evidence details",
            "review_checklist:missing_stay_in_c_boundary",
        ]
        if failures != expected:
        ...
        _write(
            missing_policy_coupling / INDEFINITE_C_POLICY_PATH,
            "# Phase 15 Indefinite-C Policy\n\n- required approver set\n- automatic return-to-blocked trigger\n- trigger-specific evidence refresh\n- parity scorecard link or blocker record\n",
        )
        ...
        _write(
            missing_handoff_template / HANDOFF_NOTE_PATH,
            "# Phase 15 Handoff Next Steps Survey\n\n- `zigux/tests/phase15_architecture_council_review_process_build.zig`\n",
        )
        failures = collect_failures(missing_handoff_template)
        expected = [
            "handoff_note:missing_marker:`Documentation/zigux/phase15-architecture-council-decision-record-template.md`"
        ]
        if failures != expected:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify that the Phase 15 Architecture Council decision-record template stays aligned with the landed governance packet."
    )
