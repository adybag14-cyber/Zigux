...
def run_check(root: Path) -> None:
    ...
    expect_equal(payload, "workflow_phase11_steps", WORKFLOW_PHASE11_STEPS)
        ...
        json.dumps(
            {
                ...
                "workflow_phase11_steps": WORKFLOW_PHASE11_STEPS,
...
def run_self_test() -> int:
    ...
    try:
        ...
        cases = [
            ...
            (DRIVER_PATH, "pub fn summarizeInstallOwnership(request: InstallOwnershipRequest) InstallOwnershipSummary {"),
            (DRIVER_PATH, "pub fn summarizeAllocSlotHandoff(request: AllocSlotRequest) AllocSlotSummary {"),
            (DRIVER_PATH, "pub fn summarizeConsoleSetup(request: ConsoleSetupRequest) ConsoleSetupSummary {"),
            (DRIVER_PATH, "pub fn summarizeConsoleDeviceSelection(request: ConsoleDeviceRequest) ConsoleDeviceSummary {"),
            (DRIVER_PATH, "pub fn summarizeResizeHandoff(request: ResizeHandoffRequest) ResizeHandoffSummary {"),
            ...
            (DRIVER_PATH, "pub const TargetlessNotifierEdgeRequest = struct {"),
            ...
            (DRIVER_PATH, "test \"phase11 hvc console keeps successful hvc_install ownership reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps failed hvc_install cleanup ownership reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps missing hvc_install lookup from lighting later ownership surfaces\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps hvc_alloc slot-match handoff reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps hvc_alloc empty-slot fallback reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps hvc_alloc hotplug extension distinct from console slot claims\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps early console setup success reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps early console setup ENODEV gates reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps console-device selection success reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps adapter-missing device lookup null reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps driver-missing device lookup distinct from slot export\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps resize handoff summary reviewable\" {"),
            (DRIVER_PATH, "test \"phase11 hvc console keeps zeroed resize geometry explicit\" {"),
        ...
        bad_workflow = tmpdir / "bad_workflow"
        shutil.copytree(fixture, bad_workflow, dirs_exist_ok=True)
        payload = read_inventory(bad_workflow)
        payload["workflow_phase11_steps"] = [{"name": "other", "run": "other"}]
        write(bad_workflow / INVENTORY_PATH, json.dumps(payload, indent=2) + "\n")
        expect_failure(bad_workflow, "workflow_phase11_steps")
        ...
        print(f"PHASE11_HVC_CLEANUP_CURRENT_HEAD_SELF_TEST_CASE_COUNT={len(cases) + 3}")
        return 0
    finally:
