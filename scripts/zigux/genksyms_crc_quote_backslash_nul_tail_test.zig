...
test "visible quote and backslash bytes escape before an embedded NUL tail" {
    const visible = "quote \" and slash \\ before tail";
    const hidden = "hidden \" tail \\ must stay invisible";
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"quote ") != null);
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "quote \\\" and") != null);
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "slash \\\\ before tail\"") != null);
