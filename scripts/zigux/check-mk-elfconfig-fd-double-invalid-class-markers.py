...
ZIG_TOOL = ROOT / "scripts" / "zigux" / "mk_elfconfig_fd_double_invalid_class_test.zig"
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-fd-double-invalid-class-") as tmp_dir:
        ...
        duplicate_path.write_text(
            "\n".join(
                [
                    EXPECTED_MARKERS["fd_double_invalid_class"],
                    EXPECTED_MARKERS["fd_double_invalid_class"],
                ]
            )
            + "\n",
        ...
        try:
            ...
            if "duplicate_marker:fd_double_invalid_class" not in str(exc):
                raise
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that mk_elfconfig keeps the fd double-invalid-class replay visible."
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    validate_markers(args.root / "scripts" / "zigux" / "mk_elfconfig_fd_double_invalid_class_test.zig")
    print("MK_ELFCONFIG_FD_DOUBLE_INVALID_CLASS_MARKERS=pass")
    print(f"MK_ELFCONFIG_FD_DOUBLE_INVALID_CLASS_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
