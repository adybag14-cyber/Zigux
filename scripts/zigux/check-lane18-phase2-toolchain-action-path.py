...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues.extend(
        collect_exact_line_issues(
            workflow,
            WORKFLOW_RUN_LINES,
            "MISSING_WORKFLOW_ACTION_PATH_MARKERS",
            "DUPLICATE_WORKFLOW_ACTION_PATH_MARKERS",
        )
    )
    ...
    issues.extend(
        collect_exact_line_issues(
            read_text(resolve(root, MAKEFILE)),
            MAKEFILE_MARKERS,
            "MISSING_MAKEFILE_ACTION_PATH_MARKERS",
            "DUPLICATE_MAKEFILE_ACTION_PATH_MARKERS",
        )
    )
    return issues
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_lane18_toolchain_action_path_") as tmp:
        ...
        build_sample_root(root)
        path = resolve(root, WORKFLOW)
        path.write_text(duplicate_exact_line(path.read_text(encoding="utf-8"), WORKFLOW_RUN_LINES[1]), encoding="utf-8")
        ...
        second = workflow_lines.index(WORKFLOW_RUN_LINES[-1])
        ...
        path.write_text(remove_first_marker(path.read_text(encoding="utf-8"), BOOTSTRAP_MARKERS[-1]), encoding="utf-8")
        assert any(code == "MISSING_BOOTSTRAP_NOTE_MARKERS" for code, _ in collect_issues(root))
        checks += 1

        build_sample_root(root)
        path = resolve(root, SCRIPTS_README)
        path.write_text(remove_first_marker(path.read_text(encoding="utf-8"), SCRIPTS_MARKERS[0]), encoding="utf-8")
        assert any(code == "MISSING_SCRIPTS_README_MARKERS" for code, _ in collect_issues(root))
        checks += 1

        build_sample_root(root)
        path = resolve(root, TESTS_README)
        path.write_text(remove_first_marker(path.read_text(encoding="utf-8"), TESTS_MARKERS[-1]), encoding="utf-8")
        assert any(code == "MISSING_TESTS_README_MARKERS" for code, _ in collect_issues(root))
        checks += 1

        build_sample_root(root)
        path = resolve(root, REVIEW_CHECKLIST)
        path.write_text(remove_first_marker(path.read_text(encoding="utf-8"), REVIEW_MARKERS[-1]), encoding="utf-8")
        assert any(code == "MISSING_REVIEW_CHECKLIST_MARKERS" for code, _ in collect_issues(root))
        checks += 1

        build_sample_root(root)
        path = resolve(root, THIRD_PARTY_README)
        path.write_text(remove_first_marker(path.read_text(encoding="utf-8"), THIRD_PARTY_MARKERS[1]), encoding="utf-8")
        assert any(code == "MISSING_THIRD_PARTY_README_MARKERS" for code, _ in collect_issues(root))
        checks += 1

        build_sample_root(root)
        path = resolve(root, MAKEFILE)
        path.write_text(remove_first_marker(path.read_text(encoding="utf-8"), MAKEFILE_MARKERS[-1]), encoding="utf-8")
        assert any(code == "MISSING_MAKEFILE_ACTION_PATH_MARKERS" for code, _ in collect_issues(root))
        checks += 1

        build_sample_root(root)
        path = resolve(root, MAKEFILE)
        path.write_text(duplicate_exact_line(path.read_text(encoding="utf-8"), MAKEFILE_MARKERS[0]), encoding="utf-8")
        assert any(code == "DUPLICATE_MAKEFILE_ACTION_PATH_MARKERS" for code, _ in collect_issues(root))
        checks += 1

        build_sample_root(root)
        path = resolve(root, WORKFLOW)
        path.write_text(remove_first_marker(path.read_text(encoding="utf-8"), WORKFLOW_SETUP_MARKERS[0]), encoding="utf-8")
        assert any(code == "MISSING_WORKFLOW_SETUP_MARKERS" for code, _ in collect_issues(root))
        checks += 1

        for rel in (WORKFLOW, BOOTSTRAP_NOTES, SCRIPTS_README, TESTS_README, REVIEW_CHECKLIST, THIRD_PARTY_README, MAKEFILE):
            build_sample_root(root)
...
def main() -> int:
    ...
    print(f"LANE18_PHASE2_TOOLCHAIN_ACTION_PATH_MAKEFILE_MARKER_COUNT={len(MAKEFILE_MARKERS)}")
    return 0
