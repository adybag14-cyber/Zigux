...
FIXTURE_DIR = ROOT / 'zigux' / 'tests' / 'fixtures' / 'phase3_chrdev_notify_ack_window_policy_budget_window_delivery'
EXPECTED = FIXTURE_DIR / 'expected.json'
HARNESS = FIXTURE_DIR / 'phase3_chrdev_notify_ack_window_policy_budget_window_delivery_c_harness.c'
...
def compile_and_run_c(tmp_dir: Path, compiler: str, actual: Path) -> None:
    exe = tmp_dir / ('phase3_chrdev_notify_ack_window_policy_budget_window_delivery_c_harness.exe' if os.name == 'nt' else 'phase3_chrdev_notify_ack_window_policy_budget_window_delivery_c_harness')
    flags = ['-I', str(ROOT / 'include')]

    if os.name == 'nt' and shutil.which('wsl'):
        script_path = tmp_dir / 'run_phase3_chrdev_notify_ack_window_policy_budget_window_delivery_c.sh'
...
def compile_and_run_zig(zig: str, actual: Path) -> None:
    env = os.environ.copy()
    env['ZIG'] = zig
    result = run([zig, 'build', 'phase3-chrdev-notify-ack-window-policy-budget-window-delivery-dump', '--build-file', str(ROOT / 'zigux' / 'tests' / 'build.zig')], cwd=str(ROOT), capture_output=True, env=env)
    actual.write_text(result.stdout, encoding='utf-8', newline='\n')
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='zigux_phase3_chrdev_notify_ack_window_policy_budget_window_delivery_') as tmp_dir_str:
        tmp_dir = Path(tmp_dir_str)
        c_actual = tmp_dir / 'phase3_chrdev_notify_ack_window_policy_budget_window_delivery_c.actual.json'
        zig_actual = tmp_dir / 'phase3_chrdev_notify_ack_window_policy_budget_window_delivery_zig.actual.json'
    ...
    print('PHASE3_CHRDEV_NOTIFY_ACK_WINDOW_POLICY_BUDGET_WINDOW_DELIVERY_DIFF=pass')
