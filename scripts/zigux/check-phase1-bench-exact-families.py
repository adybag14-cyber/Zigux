...
def run_self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="phase1-bench-exact-families-") as tmp:
        ...
        write_sample_root(missing_rbtree_closure_root)
        closure_path = missing_rbtree_closure_root / CLOSURE
        closure_text = closure_path.read_text(encoding="utf-8").replace(
            "`PHASE1_RBTREE_BENCH_GUARD=\n",
            "",
        )
        closure_path.write_text(closure_text, encoding="utf-8")
        kind, payload = validate(missing_rbtree_closure_root)
        assert kind == "missing_reminder_markers", (kind, payload)
        assert payload == [f"{CLOSURE}:`PHASE1_RBTREE_BENCH_GUARD="]
        case_count += 1
