...
CHECKER = ROOT / "scripts" / "zigux" / "check-phase2-cross-validate-validator-order.py"
...
def run_check(root: Path) -> int:
    ...
    if issues:
        for code, detail in issues:
            print(
                "PHASE2_CROSS_VALIDATE_VALIDATOR_ORDER_SELFTEST_ALIGNMENT_ISSUE="
                f"{code}:{detail}"
            )
        print(
            "PHASE2_CROSS_VALIDATE_VALIDATOR_ORDER_SELFTEST_ALIGNMENT_ISSUE_COUNT="
            f"{len(issues)}"
        )
        return 1

    print("PHASE2_CROSS_VALIDATE_VALIDATOR_ORDER_SELFTEST_ALIGNMENT=pass")
    print(
        "PHASE2_CROSS_VALIDATE_VALIDATOR_ORDER_SELFTEST_ALIGNMENT_SOURCE_MARKER_COUNT="
        f"{len(REQUIRED_SOURCE_MARKERS)}"
    )
    print(
        "PHASE2_CROSS_VALIDATE_VALIDATOR_ORDER_SELFTEST_ALIGNMENT_CASE_MARKER_COUNT="
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(
        prefix="zigux_phase2_cross_validate_validator_order_alignment_"
    ) as tmp_dir:
        root = Path(tmp_dir)
    ...
    print("PHASE2_CROSS_VALIDATE_VALIDATOR_ORDER_SELFTEST_ALIGNMENT_SELF_TEST=pass")
    print(
        "PHASE2_CROSS_VALIDATE_VALIDATOR_ORDER_SELFTEST_ALIGNMENT_SELF_TEST_CASE_COUNT="
        f"{checks}"
    )
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Keep the Lane 21 validator-order checker self-test markers intact."
    )
