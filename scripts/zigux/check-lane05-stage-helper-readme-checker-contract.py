...
def check_readme_checker(root: Path, contract: dict[str, object]) -> int:
    checker_text = read_text(root / README_CHECKER_PATH)
    target = str(contract["target"])
    size = int(contract["size"])
    ...
    for marker in checker_markers:
        require_marker(checker_text, marker, "README checker marker")

    require_order(
        checker_text,
        f'"{target}": {size},',
        'expected_archive_filename(target, channel)',
        "README checker contract order",
    )
    require_order(
        checker_text,
        'payload_status = "missing_allowed"',
        'payload_status = "present"',
        "README checker payload status order",
    )
    require_order(
        checker_text,
        'LANE05_LOCAL_ARCHIVE_TARGET=',
        'LANE05_LOCAL_ARCHIVE_README_MARKER_COUNT=',
        "README checker output order",
    )
    require_order(
        checker_text,
        'LANE05_LOCAL_ARCHIVE_README_MARKER_COUNT=',
...
def write_fixture(root: Path) -> None:
    ...
    (root / README_CHECKER_PATH).write_text(
        "\n".join(
            [
                'README_PATH = Path("third_party/README.md")',
                'POLICY_PATH = Path("scripts/zigux/zig-toolchain-policy.json")',
                ...
                'LANE05_LOCAL_ARCHIVE_PAYLOAD_STATUS=',
                'LANE05_LOCAL_ARCHIVE_README_SELF_TEST=pass',
                'LANE05_LOCAL_ARCHIVE_README_SELF_TEST_CASE_COUNT=5',
            ]
        )
        + "\n",
        encoding="utf-8",
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="lane05_stage_helper_contract_") as tmp_dir:
        ...
        assert check_readme_checker(root, contract) == 15
    ...
    def expect_failure(mutator, expected_substring: str) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="lane05_stage_helper_contract_fail_") as tmp_dir:
            ...
            try:
                ...
                check_readme_checker(root, contract)
    ...
    expect_failure(
        lambda root: (root / README_CHECKER_PATH).write_text("missing\n", encoding="utf-8"),
        "missing README checker marker",
    )
    expect_failure(
    ...
    expect_failure(
        lambda root: (root / README_CHECKER_PATH).write_text(
            (root / README_CHECKER_PATH).read_text(encoding="utf-8").replace(
                '"x86_64-linux": 58159088,',
                '"x86_64-linux": 1,',
            ),
            encoding="utf-8",
