...
pub const self_test_pass_marker = "PHASE15_SHARED_SUMMARY_GAP_SELF_TEST=pass";
pub const live_pass_marker = "PHASE15_SHARED_SUMMARY_GAP=pass";
...
const contracts = [_]FileContract{
    .{ .rel = "Documentation/zigux/phase15-shared-summary-gap.md", .markers = &markers_0 },
...
fn emitCounts(io: Io) !void {
    try guard.printLine(io, "PHASE15_SHARED_SUMMARY_GAP_REQUIRED_FILE_COUNT={d}", .{contracts.len});
    try guard.printLine(io, "PHASE15_SHARED_SUMMARY_GAP_REQUIRED_MARKER_COUNT={d}", .{@as(usize, 7)});
}
...
pub fn main(init: std.process.Init) !void {
    ...
    if (self_test) try guard.printLine(io, "PHASE15_SHARED_SUMMARY_GAP_SELF_TEST_CASE_COUNT=1", .{});
    try emitCounts(io);
}
