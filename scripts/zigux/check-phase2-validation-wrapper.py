...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_validation_wrapper_") as tmp_dir:
        ...
        for marker in (REQUIRED_WORKFLOW_LINES[0], REQUIRED_WORKFLOW_LINES[-1]):
            build_self_test_root(root)
            path = root / WORKFLOW
        ...
        for marker in (REQUIRED_VALIDATOR_MARKERS[2], REQUIRED_VALIDATOR_MARKERS[-1]):
            build_self_test_root(root)
            path = root / VALIDATOR
        ...
        build_self_test_root(root)
        path = root / VALIDATOR
        path.write_text(read_text(path) + REQUIRED_VALIDATOR_MARKERS[0] + "\n", encoding="utf-8")
        assert ("DUPLICATE_VALIDATOR_MARKER", f"{REQUIRED_VALIDATOR_MARKERS[0]}:count=2") in collect_issues(root)
        checks += 1
        ...
        for rel, marker, code in (
            (TOOLCHAIN_NOTES, REQUIRED_TOOLCHAIN_NOTES_SNIPPETS[2], "MISSING_TOOLCHAIN_NOTES_MARKER"),
            (PHASE2_CLOSURE, REQUIRED_CLOSURE_SNIPPETS[2], "MISSING_CLOSURE_MARKER"),
            (TOOL_MANIFEST, REQUIRED_MANIFEST_SNIPPETS[2], "MISSING_MANIFEST_MARKER"),
        ):
            build_self_test_root(root)
            path = root / rel
            path.write_text(remove_once(read_text(path), marker), encoding="utf-8")
        ...
        for rel in (VALIDATOR, CLOSURE_VALIDATOR, TOOL_MANIFEST):
            build_self_test_root(root)
            (root / rel).unlink()
