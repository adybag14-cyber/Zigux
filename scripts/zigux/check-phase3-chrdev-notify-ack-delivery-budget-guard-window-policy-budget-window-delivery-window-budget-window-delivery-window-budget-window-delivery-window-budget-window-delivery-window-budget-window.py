...
def compile_and_run_c(tmp_dir: Path, compiler: str, actual: Path) -> None:
    exe = tmp_dir / ('phase3_harness.exe' if os.name == 'nt' else 'phase3_harness')
    flags = ['-I', str(ROOT / 'include')]
    if os.name == 'nt' and shutil.which('wsl'):
        script_path = tmp_dir / 'run_phase3_c.sh'
...
def compile_and_run_zig(zig: str, actual: Path) -> None:
    ...
    result = run([
        zig,
        'build',
        'phase3-chrdev-notify-ack-delivery-budget-guard-window-policy-budget-window-delivery-window-budget-window-delivery-window-budget-window-delivery-window-budget-window-delivery-window-budget-window-dump',
        '--build-file',
        str(ROOT / 'zigux' / 'tests' / 'build.zig'),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='zigux_phase3_tail_') as tmp_dir_str:
        tmp_dir = Path(tmp_dir_str)
        c_actual = tmp_dir / 'c.actual.json'
        zig_actual = tmp_dir / 'zig.actual.json'
        compile_and_run_c(tmp_dir, compiler, c_actual)
        compile_and_run_zig(zig, zig_actual)
        compare_json(EXPECTED, c_actual)
        compare_json(EXPECTED, zig_actual)
