...
test "runFixdep keeps double-backslash-before-hash dependency comments aligned with C fixdep" {
    ...
    const source_path = try std.fmt.allocPrint(
        std.testing.allocator,
        "{s}/sample_double_backslash_hash_source.rmeta",
        .{base_path},
    );
    ...
    const depfile_path = try std.fmt.allocPrint(
        std.testing.allocator,
        "{s}/sample_double_backslash_hash.d",
        .{base_path},
    );
    ...
    const cmdline = try std.fmt.allocPrint(
        std.testing.allocator,
        "clang -c {s} -o sample_double_backslash_hash.o",
        .{source_path},
    );
    ...
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "missing\\",
        .data = "#define CONFIG_ZIGUX_DOUBLE_BACKSLASH_HASH 1\n",
    });

    const depfile_text = try std.fmt.allocPrint(
        std.testing.allocator,
        "sample_double_backslash_hash.o: {s} {s}#ignored_comment_suffix.h\n",
        .{ source_path, double_backslash_dep_path },
    );
    defer std.testing.allocator.free(depfile_text);

    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "sample_double_backslash_hash.d",
        .data = depfile_text,
    });
    ...
    try fixdep.runFixdep(
        ...
        &capture,
        depfile_path,
        "sample_double_backslash_hash.o",
        cmdline,
    );

    const expected = try std.fmt.allocPrint(
        std.testing.allocator,
        "savedcmd_sample_double_backslash_hash.o := {s}\n\n" ++
            "source_sample_double_backslash_hash.o := {s}\n\n" ++
            "deps_sample_double_backslash_hash.o := \\\n" ++
            "  {s} \\\n" ++
            "    $(wildcard include/config/ZIGUX_DOUBLE_BACKSLASH_HASH) \\\n" ++
            "\n" ++
            "sample_double_backslash_hash.o: $(deps_sample_double_backslash_hash.o)\n\n" ++
            "$(deps_sample_double_backslash_hash.o):\n",
