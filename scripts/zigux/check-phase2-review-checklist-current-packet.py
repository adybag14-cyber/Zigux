...
def collect_structured_issues(
    tool_manifest: object,
    artifact_manifest: object,
    cross_targets: object,
    toolchain_policy: object,
) -> list[tuple[str, str]]:
    issues: list[tuple[str, str]] = []
    ...
    if not isinstance(toolchain_policy, dict):
        ...
        if not isinstance(upgrade_policy, dict):
            ...
            if upgrade_policy.get("archive_target_scope") != ["x86_64-linux"]:
                issues.append(
                    ("toolchain-policy-target-scope", repr(upgrade_policy.get("archive_target_scope")))
                )
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues.extend(
        collect_exact_count(third_party_readme, THIRD_PARTY_EXACT_COUNT_MARKERS, "third-party-readme-count")
    )
