...
test "fixdep skips leading blank lines on the public entry path" {
    ...
    const depfile_path = try std.fmt.allocPrint(
        std.testing.allocator,
        "{s}/sample_leading_blank_lines.d",
        .{absolute_base_path},
    );
    ...
    const cmdline = try std.fmt.allocPrint(
        std.testing.allocator,
        "clang -c {s} -o sample_leading_blank_lines.o",
        .{source_path},
    );
    ...
    const expected = try std.fmt.allocPrint(
        std.testing.allocator,
        "savedcmd_sample_leading_blank_lines.o := {s}\n\n" ++
            "source_sample_leading_blank_lines.o := {s}\n\n" ++
            "deps_sample_leading_blank_lines.o := \\\n" ++
            "  {s} \\\n" ++
            "\n" ++
            "sample_leading_blank_lines.o: $(deps_sample_leading_blank_lines.o)\n\n" ++
            "$(deps_sample_leading_blank_lines.o):\n",
