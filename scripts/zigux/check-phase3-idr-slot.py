#!/usr/bin/env python3
"""Fail-close the current Phase 3 idr-slot starter-plus-dump packet."""
...
STARTER_TEST_PATH = Path("zigux/tests/phase3_idr_slot_starter_packet.zig")
STARTER_BUILD_PATH = Path("zigux/tests/phase3_idr_slot_starter_packet_build.zig")
STARTER_CHECK_PATH = Path("scripts/zigux/check-phase3-idr-slot-starter-packet.py")
DUMP_PATH = Path("zigux/tests/phase3_idr_slot_dump.zig")
DUMP_BUILD_PATH = Path("zigux/tests/phase3_idr_slot_dump_build.zig")
...
def _run_zig_dump(repo_root: Path, zig: str) -> object:
    result = _run(
        [zig, "build", "phase3-idr-slot-dump", "--build-file", str(repo_root / DUMP_BUILD_PATH)],
        cwd=repo_root,
    )
    if result.returncode != 0:
        raise RuntimeError(
...
def _run_c_harness(repo_root: Path, cc: str) -> object:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_idr_slot_c_") as temp_dir:
        binary = Path(temp_dir) / "phase3_idr_slot_c_harness"
        compile_result = _run(
    ...
    if manifest_path.exists():
        try:
            ...
            if not isinstance(replay_routes, list):
                issues.append("phase3_idr_slot_manifest.json replay_routes is not a list")
            if isinstance(packet_files, list):
                for required_path in REQUIRED_PACKET_FILES:
                    if required_path not in packet_files:
                        issues.append(
                            "phase3_idr_slot_manifest.json missing packet_files entry: "
                            f"{required_path}"
                        )
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase3_idr_slot_") as temp_dir:
        root = Path(temp_dir)
        _populate_repo(root)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the current Phase 3 idr-slot starter-plus-dump packet."
    )
