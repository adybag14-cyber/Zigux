#!/usr/bin/env python3
"""Guard the live Phase 1 string workflow packet on current master."""
...
FORBIDDEN_LINES = (
    ...
    "- name: Self-test Phase 1 find-bit review checker",
    "- name: Check Phase 1 find-bit review packet",
)
...
def collect_failures(root: Path) -> list[str]:
    ...
    if positions != sorted(positions):
        failures.append("phase1_string_workflow_packet:order_drift")
...
def build_sample_root(root: Path) -> None:
    for relative_path in (DIRECT_ANCHOR_REL, STRING_REVIEW_REL, FIND_BIT_REVIEW_REL):
...
def run_self_test() -> int:
    cases: list[tuple[str, object | None]] = [
        ...
        (
            "missing_string_live",
            lambda root: remove_line(root, "run: python3 scripts/zigux/check-phase1-string-review-packet.py"),
        ),
        (
            "duplicate_find_bit_selftest",
            lambda root: duplicate_line(root, "run: python3 scripts/zigux/check-phase1-find-bit-review-packet.py --self-test"),
        ),
        (
            "missing_bitmap_boundary",
            lambda root: remove_line(root, "- name: Self-test current Phase 1 bitmap direct-anchor checker"),
        ),
        (
            "bad_order",
        ...
        (
            "forbidden_old_label",
            lambda root: append_line(root, "      - name: Self-test Phase 1 string review checker"),
        ),
        ("missing_direct_anchor_file", lambda root: (root / DIRECT_ANCHOR_REL).unlink()),
        ("missing_string_file", lambda root: (root / STRING_REVIEW_REL).unlink()),
        ("missing_find_bit_file", lambda root: (root / FIND_BIT_REVIEW_REL).unlink()),
    ]

    for name, mutate in cases:
        with tempfile.TemporaryDirectory(prefix="phase1-string-workflow-") as tmpdir:
            root = Path(tmpdir)
            ...
            if name == "baseline":
                if failures:
                    print(f"phase1-string-workflow:{name}:unexpected={failures}")
                    return 1
            elif not failures:
                print(f"phase1-string-workflow:{name}:expected_failure")
                return 1

    print("PHASE1_STRING_WORKFLOW_PACKET_SELF_TEST=pass")
    print(f"PHASE1_STRING_WORKFLOW_PACKET_SELF_TEST_CASE_COUNT={len(cases)}")
    return 0
...
def main() -> int:
    ...
    if args.write_sample_root:
        ...
        print("PHASE1_STRING_WORKFLOW_PACKET_SAMPLE_ROOT=written")
        return 0
    ...
    if failures:
        print("PHASE1_STRING_WORKFLOW_PACKET=fail")
        for failure in failures:
            print(failure)
        return 1

    print("PHASE1_STRING_WORKFLOW_PACKET=pass")
    print(f"PHASE1_STRING_WORKFLOW_PACKET_REQUIRED_FILE_COUNT={len(REQUIRED_FILES)}")
    print(f"PHASE1_STRING_WORKFLOW_PACKET_REQUIRED_LINE_COUNT={len(EXACT_ONCE_LINES)}")
    return 0
