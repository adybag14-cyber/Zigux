...
test "fd-backed consecutive exact invalid-class headers advance one header per call" {
    var temp_dir = std.testing.tmpDir(.{});
    defer temp_dir.cleanup();
    const io = std.testing.io;
    const file = try temp_dir.dir.createFile(io, "double_invalid_class.bin", .{ .read = true });
    defer file.close(io);
