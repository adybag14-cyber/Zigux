#!/usr/bin/env python3
"""Fail closed when the closure-side Phase 2 validator leaves manifest surfaces implicit."""
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_COVERAGE=fail")
...
def run_self_test() -> int:
    ...
    checks = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_closure_validator_manifest_surface_") as tmp_dir:
        root = Path(tmp_dir)
    ...
    print("PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_COVERAGE_SELF_TEST=pass")
    print(f"PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_COVERAGE_SELF_TEST_CASE_COUNT={checks}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that scripts/zigux/validate-phase2-closure.py explicitly covers the extra present_surfaces groups already shipped in the Phase 2 tool manifest."
    )
    ...
    if args.write_sample_root is not None:
        ...
        print(f"PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_COVERAGE_SAMPLE_ROOT={args.write_sample_root.resolve()}")
        return 0
    ...
    print("PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_COVERAGE=pass")
    print(f"PHASE2_CLOSURE_VALIDATOR_MANIFEST_SURFACE_COVERAGE_SURFACE_COUNT={len(EXPECTED_SURFACE_ANCHORS)}")
    return 0
