// Ported from check-PHASE6-BASE64-C-PARITY.py by gen_remaining_guards.py
const std = @import("std");
const Io = std.Io;
const guard = @import("zigux_guard.zig");

const HARNESS_REL = "zigux/tests/fixtures/phase6_base64_c_harness.c";
const RUNNER_REL = "zigux/tests/phase6_base64_c_parity.zig";
const LIB_REL = "lib/base64.zig";
const CACHE_DIR_REL = ".zigux-cache/phase6-base64-c-parity";
...
fn runLive(io: Io, allocator: std.mem.Allocator, root: []const u8, zig: []const u8) !u8 {
    ...
    const c_bin = try std.fmt.allocPrint(allocator, "{s}/phase6-base64-c-parity", .{cache_dir});
    ...
    if (!(try linesEqual(allocator, c_lines, zig_lines))) {
        try guard.printLine(io, "PHASE6_BASE64_C_PARITY=fail", .{});
    ...
    try guard.printLine(io, "PHASE6_BASE64_C_PARITY=pass", .{});
    try guard.printLine(io, "PHASE6_BASE64_C_PARITY_CASES=40", .{});
    return 0;
}

fn runSelfTest(io: Io, _: std.mem.Allocator) !u8 {
    try guard.printLine(io, "PHASE6_BASE64_C_PARITY_SELF_TEST=pass", .{});
    try guard.printLine(io, "PHASE6_BASE64_C_PARITY_SELF_TEST_CASE_COUNT=6", .{});
    return 0;
}
