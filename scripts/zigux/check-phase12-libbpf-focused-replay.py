...
def check_paths(build_path: Path, note_path: Path) -> list[str]:
    missing: list[str] = []
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix='zigux_phase12_libbpf_focused_replay_') as tmp_dir:
        ...
        missing = check_paths(build_path, root / 'Documentation/zigux/phase12-libbpf-segment-survey.md')
        ...
        missing = check_paths(root / 'zigux/tests/phase12_libbpf_only_build.zig', note_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    missing = check_paths(BUILD_PATH, NOTE_PATH)
    if missing:
