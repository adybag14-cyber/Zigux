...
def require_exact_marker(text: str, label: str, marker: str) -> list[str]:
    if label.split(":", 1)[0] in EXACT_LINE_FILES:
...
def collect_failures(root: Path) -> list[str]:
    ...
    for relative_path, markers in EXACT_MARKERS.items():
        ...
        for marker in markers:
            failures.extend(require_exact_marker(text, f"{relative_path}:{marker}", marker))
...
def build_sample_repo(root: Path) -> None:
    for relative_path in REQUIRED_FILES:
        ...
        if relative_path in FORBIDDEN_MARKERS:
            lines.extend(["# forbidden markers intentionally absent"])
...
def run_self_test() -> int:
    ...
    for relative_path, markers in EXACT_MARKERS.items():
        for marker in markers:
            cases.append((f"missing_marker:{relative_path}:{abs(hash(marker))}", ("remove", relative_path, marker)))
            cases.append((f"duplicate_marker:{relative_path}:{abs(hash(marker))}", ("duplicate", relative_path, marker)))
    for relative_path, markers in FORBIDDEN_MARKERS.items():
        for marker in markers:
