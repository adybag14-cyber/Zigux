...
EXPECTED_MARKERS = {
    Path(
        "scripts/zigux/mk_elfconfig_fd_trailing_same_class_invalid_class_hides_following_invalid_class_test.zig"
    ): [
        (
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(
        prefix="lane14-fd-trailing-same-class-invalid-class-hidden-header-"
    ) as tmp_dir:
        ...
        try:
            ...
            if "missing_marker:fd_trailing_invalid_class_hidden_invalid_class_no_skip" not in str(exc):
                raise
        else:
        ...
        try:
            ...
            if "duplicate_marker:fd_trailing_invalid_class_hides_following_invalid_class" not in str(exc):
                raise
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check that the mk_elfconfig same-class trailing invalid-class hidden-header replay stays present."
        )
    )
    parser.add_argument(
        "--root",
    ...
    print(
        "MK_ELFCONFIG_FD_TRAILING_SAME_CLASS_INVALID_CLASS_HIDES_FOLLOWING_INVALID_CLASS_MARKERS_COUNT=1"
    )
    return 0
