...
def collect_failures(root: Path) -> list[str]:
    ...
    if not missing_headings and positions != sorted(positions):
        failures.append("order:Purpose->BootstrapStatusNote->InputsReviewed->BundleNormalizationNotes")
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_lane01_inputs_reviewed_") as tmp_dir:
        ...
        _write(root / ROADMAP_PATH, _sample_roadmap().replace("## Inputs Reviewed\n\n", "", 1))
        failures = collect_failures(root)
        expected = [
...
def main() -> int:
    ...
    print(f"LANE01_BOOTSTRAP_ROADMAP_INPUTS_REVIEWED_REQUIRED_LINE_COUNT={len(REQUIRED_LINES)}")
    print(
