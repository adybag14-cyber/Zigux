...
DOCS_README_MARKERS = (
    ...
    "the bounded genksyms bridge helper packet",
    "the current genksyms bridge fixture roster keeping the same packet aligned across docs-root, scripts-root, and tests-root surfaces.",
    "`make -C zigux phase2-genksyms`",
    "bounded current Phase 2 closure-side, bounded genksyms bridge, and make-wrapper packet",
)
...
def emit_issues(issues: list[tuple[str, str]]) -> int:
    ...
    print("PHASE2_DOCS_README_GENKSYMS_PACKET=fail")
...
def run_self_test() -> int:
    checks_run = 0
    expected_case_count = 1 + len(DOCS_README_MARKERS) + len(DOCS_README_FORBIDDEN_MARKERS) + len(TESTS_README_MARKERS) + 2
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_docs_readme_genksyms_") as tmp_dir:
        root = Path(tmp_dir)
    ...
    print("PHASE2_DOCS_README_GENKSYMS_PACKET_SELF_TEST=pass")
    print(f"PHASE2_DOCS_README_GENKSYMS_PACKET_SELF_TEST_CASE_COUNT={checks_run}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Keep the docs-root Phase 2 genksyms packet aligned to current repo reality."
    )
    ...
    print("PHASE2_DOCS_README_GENKSYMS_PACKET=pass")
    print(f"PHASE2_DOCS_README_GENKSYMS_PACKET_MARKER_COUNT={len(DOCS_README_MARKERS)}")
    print(f"PHASE2_DOCS_README_GENKSYMS_PACKET_FORBIDDEN_MARKER_COUNT={len(DOCS_README_FORBIDDEN_MARKERS)}")
    print(f"PHASE2_DOCS_README_GENKSYMS_PACKET_TESTS_MARKER_COUNT={len(TESTS_README_MARKERS)}")
    return 0
