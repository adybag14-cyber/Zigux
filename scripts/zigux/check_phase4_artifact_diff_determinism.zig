...
const CURRENT_DIRECT_PACKET = [_][]const u8{
...
const SURVEY_EXACT_PACKET_MARKERS = [_][]const u8{
    "`PHASE4_ARTIFACT_DIFF_CURRENT_HELPER_SELF_TEST_CASE_COUNT=23`",
...
const ARTIFACT_DIFF_NOTE_MARKERS = [_][]const u8{
...
const HELPER_EXPECTED_MODE_CHOICES = [_][]const u8{
    "text",
    "json",
    "bytes",
};

const HELPER_EXPECTED_RESULT_LINES = [_][]const u8{
...
const HELPER_EXPECTED_BYTES_PASS_DETAIL = [_][]const u8{
    "f\"SHA256={expected_digest}\"",
};

const HELPER_EXPECTED_BYTES_FAIL_DETAILS = [_][]const u8{
    "f\"EXPECTED_SHA256={expected_digest}\"",
    "f\"ACTUAL_SHA256={actual_digest}\"",
};

const HELPER_EXPECTED_ERROR_DETAILS = [_][]const u8{
...
const HELPER_EXPECTED_SELF_TEST_CASES = [_][]const u8{
    "text_pass",
    "text_mismatch",
    "json_pass",
    "json_mismatch",
...
const CONTRACT_BASE_CASES = [_][]const u8{
    "helper_self_test",
    "cli_help_output",
...
const CONTRACT_REPEAT_CASES = [_][]const u8{
...
const CONTRACT_SELF_TEST_CASES = [_][]const u8{
    "catalog_shape",
...
const VALIDATOR_REPLAYS_EXPECTED_SELF_TEST_CASES = [_][]const u8{
    "catalog_shape",
...
fn checkRepo(io: Io, allocator: std.mem.Allocator, root: []const u8) !void {
    const text_current_direct_packet_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_current_direct_packet_path);
    const text_current_direct_packet = try guard.readUtf8File(io, allocator, text_current_direct_packet_path);
    defer allocator.free(text_current_direct_packet);
    for (CURRENT_DIRECT_PACKET) |marker| try guard.requireMarker(text_current_direct_packet, marker);
    const text_expected_self_test_cases_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    ...
    const text_survey_markers_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    ...
    const text_survey_exact_packet_markers_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_survey_exact_packet_markers_path);
    const text_survey_exact_packet_markers = try guard.readUtf8File(io, allocator, text_survey_exact_packet_markers_path);
    defer allocator.free(text_survey_exact_packet_markers);
    for (SURVEY_EXACT_PACKET_MARKERS) |marker| try guard.requireMarker(text_survey_exact_packet_markers, marker);
    const text_note_markers_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    ...
    const text_artifact_diff_note_markers_path = try guard.joinPath(allocator, root, "Documentation/zigux/artifact-diff.md");
    defer allocator.free(text_artifact_diff_note_markers_path);
    const text_artifact_diff_note_markers = try guard.readUtf8File(io, allocator, text_artifact_diff_note_markers_path);
    defer allocator.free(text_artifact_diff_note_markers);
    for (ARTIFACT_DIFF_NOTE_MARKERS) |marker| try guard.requireMarker(text_artifact_diff_note_markers, marker);
    ...
    const text_helper_expected_mode_choices_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_helper_expected_mode_choices_path);
    const text_helper_expected_mode_choices = try guard.readUtf8File(io, allocator, text_helper_expected_mode_choices_path);
    defer allocator.free(text_helper_expected_mode_choices);
    for (HELPER_EXPECTED_MODE_CHOICES) |marker| try guard.requireMarker(text_helper_expected_mode_choices, marker);
    const text_helper_expected_result_lines_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_helper_expected_result_lines_path);
    const text_helper_expected_result_lines = try guard.readUtf8File(io, allocator, text_helper_expected_result_lines_path);
    defer allocator.free(text_helper_expected_result_lines);
    for (HELPER_EXPECTED_RESULT_LINES) |marker| try guard.requireExactLineCount(text_helper_expected_result_lines, marker, 1);
    const text_helper_expected_bytes_pass_detail_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_helper_expected_bytes_pass_detail_path);
    const text_helper_expected_bytes_pass_detail = try guard.readUtf8File(io, allocator, text_helper_expected_bytes_pass_detail_path);
    defer allocator.free(text_helper_expected_bytes_pass_detail);
    for (HELPER_EXPECTED_BYTES_PASS_DETAIL) |marker| try guard.requireMarker(text_helper_expected_bytes_pass_detail, marker);
    const text_helper_expected_bytes_fail_details_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_helper_expected_bytes_fail_details_path);
    const text_helper_expected_bytes_fail_details = try guard.readUtf8File(io, allocator, text_helper_expected_bytes_fail_details_path);
    defer allocator.free(text_helper_expected_bytes_fail_details);
    for (HELPER_EXPECTED_BYTES_FAIL_DETAILS) |marker| try guard.requireMarker(text_helper_expected_bytes_fail_details, marker);
    const text_helper_expected_error_details_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_helper_expected_error_details_path);
    const text_helper_expected_error_details = try guard.readUtf8File(io, allocator, text_helper_expected_error_details_path);
    defer allocator.free(text_helper_expected_error_details);
    for (HELPER_EXPECTED_ERROR_DETAILS) |marker| try guard.requireMarker(text_helper_expected_error_details, marker);
    const text_helper_expected_self_test_cases_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_helper_expected_self_test_cases_path);
    const text_helper_expected_self_test_cases = try guard.readUtf8File(io, allocator, text_helper_expected_self_test_cases_path);
    defer allocator.free(text_helper_expected_self_test_cases);
    for (HELPER_EXPECTED_SELF_TEST_CASES) |marker| try guard.requireMarker(text_helper_expected_self_test_cases, marker);
    const text_contract_base_cases_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_contract_base_cases_path);
    const text_contract_base_cases = try guard.readUtf8File(io, allocator, text_contract_base_cases_path);
    defer allocator.free(text_contract_base_cases);
    for (CONTRACT_BASE_CASES) |marker| try guard.requireMarker(text_contract_base_cases, marker);
    const text_contract_repeat_cases_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_contract_repeat_cases_path);
    const text_contract_repeat_cases = try guard.readUtf8File(io, allocator, text_contract_repeat_cases_path);
    defer allocator.free(text_contract_repeat_cases);
    for (CONTRACT_REPEAT_CASES) |marker| try guard.requireMarker(text_contract_repeat_cases, marker);
    const text_contract_self_test_cases_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_contract_self_test_cases_path);
    const text_contract_self_test_cases = try guard.readUtf8File(io, allocator, text_contract_self_test_cases_path);
    defer allocator.free(text_contract_self_test_cases);
    for (CONTRACT_SELF_TEST_CASES) |marker| try guard.requireMarker(text_contract_self_test_cases, marker);
    const text_validator_replays_expected_self_test_cases_path = try guard.joinPath(allocator, root, "Documentation/zigux/phase4-artifact-diff-tooling-survey.md");
    defer allocator.free(text_validator_replays_expected_self_test_cases_path);
    const text_validator_replays_expected_self_test_cases = try guard.readUtf8File(io, allocator, text_validator_replays_expected_self_test_cases_path);
    defer allocator.free(text_validator_replays_expected_self_test_cases);
    for (VALIDATOR_REPLAYS_EXPECTED_SELF_TEST_CASES) |marker| try guard.requireMarker(text_validator_replays_expected_self_test_cases, marker);
