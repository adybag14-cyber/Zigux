...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_cross_target_replay_alignment_") as tmp_dir:
        ...
        assert ("DUPLICATE_CASE_MARKER", f"{REQUIRED_CASE_MARKERS[0]}:count=2") in collect_issues(root)
        checks += 1
...
def main() -> int:
    ...
    print(f"PHASE2_CROSS_TARGET_REPLAY_SELFTEST_ALIGNMENT_SOURCE_MARKER_COUNT={len(REQUIRED_SOURCE_MARKERS)}")
    print(f"PHASE2_CROSS_TARGET_REPLAY_SELFTEST_ALIGNMENT_CASE_MARKER_COUNT={len(REQUIRED_CASE_MARKERS)}")
    return 0
