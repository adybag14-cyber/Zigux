...
SELF_CHECKER_REL = Path("scripts/zigux/check-phase1-bench-command-failure-expectations-packet.py")
...
def run_self_test() -> None:
    case_count = 0
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_bench_command_failure_expectations_") as tmp_dir:
        root = Path(tmp_dir)
    ...
    print("PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_SELF_TEST=pass")
    print(f"PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_SELF_TEST_CASE_COUNT={case_count}")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the bounded Lane 16 bench command-failure expectations-path packet."
    )
    ...
    if args.write_sample_root is not None:
        ...
        print(
            "PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_SAMPLE_ROOT="
    ...
    if missing_files:
        print("PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET=fail")
        print("MISSING_PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_FILES_START")
        for item in missing_files:
            print(item)
        print("MISSING_PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_FILES_END")
        return 1

    missing_markers = collect_missing_markers(root)
    if missing_markers:
        print("PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET=fail")
        print("MISSING_PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_MARKERS_START")
        for item in missing_markers:
            print(item)
        print("MISSING_PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_MARKERS_END")
        return 1

    order_kind, order_payload = validate_marker_order(root)
    if order_kind != "pass":
        print("PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET=fail")
        print(f"PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_REASON={order_kind}")
    ...
    print("PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET=pass")
    print(f"PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_REQUIRED_FILE_COUNT={len(REQUIRED_FILES)}")
    print(
        "PHASE1_BENCH_COMMAND_FAILURE_EXPECTATIONS_PACKET_REQUIRED_MARKER_COUNT="
