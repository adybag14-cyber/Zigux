...
test "lane19 multi-megabyte exact-buffer EOF continuation keeps a visible CR-led control tail until NUL while trimming trailing carriage returns beyond the old CLI cap" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + gen.c_line_payload_len + 14,
    );
    ...
    const hidden_suffix_crc = try std.fmt.allocPrint(
        std.testing.allocator,
        "0x{x:0>8}",
        .{gen.crc32("hidden")},
    );
    defer std.testing.allocator.free(hidden_suffix_crc);
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, hidden_suffix_crc) == null);
