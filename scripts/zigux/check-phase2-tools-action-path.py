#!/usr/bin/env python3
"""Guard the current Phase 2 tools action path."""
...
EXPECTED_MAKE_WRAPPER = "make -C zigux phase2-tools"
...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    if len(workflow_indices) == len(REQUIRED_WORKFLOW_LINES) and workflow_indices != sorted(
        ...
        issues.append(("WORKFLOW_ORDER_MISMATCH", "phase2-tools-route-order"))
    ...
    if len(makefile_indices) == len(REQUIRED_MAKEFILE_LINES) and makefile_indices != sorted(
        ...
        issues.append(("MAKEFILE_ORDER_MISMATCH", "phase2-tools-packet-order"))

    collect_marker_issues(issues, docs_readme_text, "MISSING_DOCS_README_MARKER", REQUIRED_DOCS_README_MARKERS)
    collect_marker_issues(issues, bootstrap_notes_text, "MISSING_BOOTSTRAP_MARKER", REQUIRED_BOOTSTRAP_MARKERS)
    ...
    expect_subset(
        issues,
        "artifact_support",
        require_manifest_list(issues, manifest, "artifact_support"),
        EXPECTED_ARTIFACT_SUPPORT,
    )
        ...
        json.dumps(
            {
                "present_surfaces": {
                    ...
                    "artifact_support": list(EXPECTED_ARTIFACT_SUPPORT),
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_tools_action_path_") as tmp_dir:
        ...
        assert ("WORKFLOW_ORDER_MISMATCH", "phase2-tools-route-order") in collect_issues(root)
        checks += 1
        ...
        assert ("MAKEFILE_ORDER_MISMATCH", "phase2-tools-packet-order") in collect_issues(root)
        checks += 1
        ...
        payload["present_surfaces"]["checkers"].remove(EXPECTED_CHECKERS[0])
        path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        assert ("MISSING_MANIFEST_SURFACE", f"checkers:{EXPECTED_CHECKERS[0]}") in collect_issues(root)
        checks += 1

        build_sample_root(root)
        path = resolve(root, TOOL_MANIFEST)
        payload = json.loads(path.read_text(encoding="utf-8"))
        payload["present_surfaces"]["artifact_support"].remove(EXPECTED_ARTIFACT_SUPPORT[1])
        path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        assert (
            "MISSING_MANIFEST_SURFACE",
            f"artifact_support:{EXPECTED_ARTIFACT_SUPPORT[1]}",
        ...
        path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        assert ("MISSING_MANIFEST_SURFACE", f"make_wrappers:{EXPECTED_MAKE_WRAPPER}") in collect_issues(root)
        checks += 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the Phase 2 tools action path stays aligned."
    )
    ...
    print(f"PHASE2_TOOLS_ACTION_PATH_DOCS_MARKER_COUNT={len(REQUIRED_DOCS_README_MARKERS)}")
    print(f"PHASE2_TOOLS_ACTION_PATH_BOOTSTRAP_MARKER_COUNT={len(REQUIRED_BOOTSTRAP_MARKERS)}")
    print(f"PHASE2_TOOLS_ACTION_PATH_REVIEW_MARKER_COUNT={len(REQUIRED_REVIEW_MARKERS)}")
    print(f"PHASE2_TOOLS_ACTION_PATH_SCRIPTS_MARKER_COUNT={len(REQUIRED_SCRIPTS_README_MARKERS)}")
    print(f"PHASE2_TOOLS_ACTION_PATH_TESTS_MARKER_COUNT={len(REQUIRED_TESTS_README_MARKERS)}")
    return 0
