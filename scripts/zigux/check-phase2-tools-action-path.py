#!/usr/bin/env python3
"""Check that the Phase 2 tools action path stays aligned."""
...
EXPECTED_MAKE_WRAPPERS = ("make -C zigux phase2-tools",)
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_tools_action_path_") as tmp_dir:
        ...
        for marker in EXPECTED_ARTIFACT_SUPPORT:
            build_sample_root(root)
            path = resolve(root, TOOL_MANIFEST)
            payload = json.loads(path.read_text(encoding="utf-8"))
            payload["present_surfaces"]["artifact_support"].remove(marker)
            path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
            assert ("MISSING_MANIFEST_SURFACE", f"artifact_support:{marker}") in collect_issues(root)
            checks += 1
...
def main() -> int:
    ...
    print(f"PHASE2_TOOLS_ACTION_PATH_CLOSURE_MARKER_COUNT={len(REQUIRED_CLOSURE_MARKERS)}")
