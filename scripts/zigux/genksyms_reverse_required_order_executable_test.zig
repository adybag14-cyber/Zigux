...
fn buildWrapper(allocator: std.mem.Allocator, tmp: testing.TmpDir) ![]u8 {
    const exe_path = try std.fmt.allocPrint(
        allocator,
        ".zig-cache/tmp/{s}/genksyms-reverse-required-order-wrapper",
        .{tmp.sub_path},
    );
...
test "reverse required option order keeps side effects at executable boundary" {
    ...
    try testing.expectEqualStrings(
        "{\"tool\":\"scripts/genksyms/genksyms\",\"stdin\":\"cpp-stream\",\"stdout\":\"symversions\",\"argv\":[\"scripts/genksyms/genksyms\",\"-Ttypes.symtypes\",\"-rref.symref\",\"-VdwDp\",\"-q\",\"unit.c\"],\"options\":{\"debug_level\":1,\"warnings\":false,\"dump_defs\":true,\"preserve\":true,\"reference_files\":[\"ref.symref\"],\"dump_types_file\":\"types.symtypes\"}}\n",
        run_result.stdout,
    );
}
