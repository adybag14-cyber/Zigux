...
EXPECTED_MAKE_WRAPPER = "make -C zigux phase2-kconfig"
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_kconfig_action_path_") as tmp_dir:
        ...
        path = resolve(root, TOOL_MANIFEST)
        payload = json.loads(path.read_text(encoding="utf-8"))
        payload["present_surfaces"]["bridge_helpers"].remove(EXPECTED_BRIDGE_HELPERS[-1])
        path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        assert (
            "MISSING_MANIFEST_SURFACE",
            f"bridge_helpers:{EXPECTED_BRIDGE_HELPERS[-1]}",
        ...
        path = resolve(root, TOOL_MANIFEST)
        payload = json.loads(path.read_text(encoding="utf-8"))
        payload["present_surfaces"]["fixture_roster"].remove(EXPECTED_FIXTURE_ROSTER[-1])
        path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        assert (
            "MISSING_MANIFEST_SURFACE",
            f"fixture_roster:{EXPECTED_FIXTURE_ROSTER[-1]}",
        ) in collect_issues(root)
        checks += 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the Phase 2 kconfig action path stays aligned."
    )
    ...
    print(f"PHASE2_KCONFIG_ACTION_PATH_BOOTSTRAP_MARKER_COUNT={len(REQUIRED_BOOTSTRAP_MARKERS)}")
    ...
    print(f"PHASE2_KCONFIG_ACTION_PATH_TESTS_MARKER_COUNT={len(REQUIRED_TESTS_README_MARKERS)}")
    return 0
