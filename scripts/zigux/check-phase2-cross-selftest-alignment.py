...
def normalize_workflow_scope_line(line: str) -> str:
...
def validate_workflow_scope_fragments(text: str) -> list[str]:
    normalized_lines = {
        normalize_workflow_scope_line(line)
        for line in text.splitlines()
        if normalize_workflow_scope_line(line)
    }
    issues: list[str] = []
    for marker in WORKFLOW_SCOPE_REQUIRED_FRAGMENTS:
        if marker not in normalized_lines:
            issues.append(f"workflow_scope:missing_marker:{marker}")
    return issues
...
def run_self_test() -> int:
    ...
    bad_phase = dict(valid_targets)
    bad_phase["phase"] = "Phase X"
    issues = validate_targets_manifest(bad_phase)
    if "targets:phase='Phase X':expected='Phase 2'" not in issues:
        raise SystemExit("phase2-cross-alignment:self-test:phase_mismatch")
    checks_run += 1

    bad_status = dict(valid_targets)
    bad_status["status"] = "open"
    issues = validate_targets_manifest(bad_status)
    if "targets:status='open':expected='closed'" not in issues:
    ...
    non_list_targets = dict(valid_targets)
    non_list_targets["targets"] = EXPECTED_TARGETS[0]
    issues = validate_targets_manifest(non_list_targets)
    if issues != ["targets:targets:expected_list"]:
        raise SystemExit("phase2-cross-alignment:self-test:target_list_type_mismatch")
    checks_run += 1
    ...
    scope_text = "\n".join(
        f"      - '{marker}'" for marker in WORKFLOW_SCOPE_REQUIRED_FRAGMENTS
    )
    ...
    for marker in WORKFLOW_SCOPE_REQUIRED_FRAGMENTS:
        scope_missing = validate_workflow_scope_fragments(
            "\n".join(
                f"      - '{item}'"
                for item in WORKFLOW_SCOPE_REQUIRED_FRAGMENTS
                if item != marker
            )
        )
        expected_scope_issue = f"workflow_scope:missing_marker:{marker}"
        if scope_missing != [expected_scope_issue]:
    ...
    for marker in WORKFLOW_SCOPE_PATTERN_MARKERS:
        ...
        expected_scope_pattern_issue = f"workflow_scope_pattern:missing_marker:{marker}"
    ...
    with tempfile.TemporaryDirectory(prefix="phase2_cross_alignment_selftest_") as tmp_dir_str:
        ...
        non_object_manifest_path.write_text("[]", encoding="utf-8")
        try:
            load_json_object(non_object_manifest_path, label="targets")
        except SystemExit as exc:
            if str(exc) != "targets:expected_object":
                raise SystemExit(
                    "phase2-cross-alignment:self-test:json_expected_object_shape"
                ) from exc
        else:
            raise SystemExit("phase2-cross-alignment:self-test:json_expected_object_missing")
        checks_run += 1
