...
INDEFINITE_C_POLICY_MARKERS = (
    ...
    "parity scorecard link or blocker record, explicit non-goals, and written rationale",
)
...
- freeze-map status-change requests must route through `Documentation/zigux/phase15-architecture-council-review-process.md`, keep the required approver set, evidence archive path, rollback threshold, `retired_from_active_discussion` state, trigger-specific evidence refresh, and indefinite-C policy link or non-applicability note explicit
""",
    )
    _write(
