...
test "runGenksymsCrc keeps visible plus bytes before a hidden NUL tail" {
    const visible = "plus+visible+record";
    const hidden = "hidden+plus+tail";
    const later = "+later+visible";
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"plus+visible+record\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, capture.list.items, "\"input\":\"+later+visible\"") != null);
