...
test "runFixdep preserves spaces in target output" {
    ...
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "zigux_fixdep_target_space_source.c",
        .data = "CONFIG_ZIGUX_TARGET_SPACE_SOURCE\n",
    });

    const source_path = try std.fmt.allocPrint(
        std.testing.allocator,
        ".zig-cache/tmp/{s}/zigux_fixdep_target_space_source.c",
        .{tmp.sub_path[0..]},
    );
    defer std.testing.allocator.free(source_path);

    const target = "module/sample target.o";
    const escaped_target = "module/sample\\ target.o";

    const depfile_bytes = try std.fmt.allocPrint(
        std.testing.allocator,
        "{s}: {s}\n",
        .{ escaped_target, source_path },
    );
    defer std.testing.allocator.free(depfile_bytes);

    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = "target_space.d",
        .data = depfile_bytes,
    });

    const depfile_path = try std.fmt.allocPrint(
        std.testing.allocator,
        ".zig-cache/tmp/{s}/target_space.d",
        .{tmp.sub_path[0..]},
    );
    ...
    const cmdline = try std.fmt.allocPrint(
        std.testing.allocator,
        "cc -DZIGUX_TARGET_SPACE=1 -c {s} -o {s}",
        .{ source_path, target },
    );
    ...
    const expected = try std.fmt.allocPrint(
        ...
        "savedcmd_{s} := {s}\n\n" ++
            "source_{s} := {s}\n\n" ++
            "deps_{s} := \\\n" ++
            "    $(wildcard include/config/ZIGUX_TARGET_SPACE_SOURCE) \\\n" ++
            "\n" ++
            "{s}: $(deps_{s})\n\n" ++
            "$(deps_{s}):\n",
