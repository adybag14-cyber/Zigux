...
def review_key_issues(helper: str, fixture_section: dict[str, object], review_anchor: dict[str, object]) -> list[str]:
    issues: list[str] = []
...
def collect_issues(root: Path) -> list[str]:
    ...
    if not fixture_path.exists():
        issues.append(f"missing:{FIXTURE_REL.as_posix()}")
    if not manifest_path.exists():
        issues.append(f"missing:{MANIFEST_REL.as_posix()}")
    if not blockers_path.exists():
        issues.append(f"missing:{BLOCKERS_REL.as_posix()}")
    if issues:
        return issues
