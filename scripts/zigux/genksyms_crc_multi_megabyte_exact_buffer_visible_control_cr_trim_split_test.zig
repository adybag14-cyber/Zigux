...
test "lane19 multi-megabyte exact-buffer split keeps a visible control record with trimmed carriage returns and no tail" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + gen.c_line_payload_len + 6,
    );
