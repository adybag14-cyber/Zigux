...
def collect_issues(root: Path) -> list[tuple[str, str]]:
    ...
    issues.extend(collect_marker_issues(workflow_text, (VALIDATOR_REQUIRED_MARKERS[-1],), "MISSING_WORKFLOW_MARKER"))
...
def build_self_test_root(root: Path) -> None:
    ...
    write_text(root, WORKFLOW, "\n".join(("name: zigux-bootstrap", VALIDATOR_REQUIRED_MARKERS[-1])) + "\n")
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase2_tool_manifest_packets_") as tmp_dir:
        ...
        assert ("MISSING_WORKFLOW_MARKER", VALIDATOR_REQUIRED_MARKERS[-1]) in collect_issues(root)
        checks += 1
