...
test "lane19 multi-megabyte exact-buffer split preserves visible low control bytes, truncates at NUL, and keeps the following line" {
    ...
    var input = try std.ArrayList(u8).initCapacity(
        ...
        skipped_record_count * skipped_record_len + c_line_payload_len + 8,
    );
