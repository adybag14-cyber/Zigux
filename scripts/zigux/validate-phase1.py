...
EXPECTED_RBTREE_NEXT_SAFE_STEP_NOTE = (
    "If this helper lane reopens, the smallest shared-replay expansion is now the dedicated cached-root leftmost-return fixture key; until then, cached-root leftmost-return and singleton-erase behavior stay owned by direct helper-local anchors."
)
...
def make_fixture_root(root: Path) -> None:
    ...
    manifest["review_anchors"]["tools/lib/rbtree.zig"] = {
        ...
        "parity_fixture_keys": ["find_found_key", "match_iterator_serials"],
...
def run_self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="zigux_phase1_validator_") as tmp_dir:
        ...
        readme.write_text(
            original.replace(DOC_MARKERS["docs_root_phase1_packet"][2] + "\n", "", 1),
            encoding="utf-8",
        )
        assert (
            f'docs_root_phase1_packet:{DOC_MARKERS["docs_root_phase1_packet"][2]}:expected=1:actual=0'
