...
test "question-prefixed positionals remain delayed argv data" {
...
test "question-prefixed required values are consumed as data" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "question-prefixed terminator tails do not change parser state" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
...
test "question-prefixed bridge JSON preserves live rendered argv order" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
