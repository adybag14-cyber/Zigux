...
def collect_failures(root: Path) -> list[str]:
    ...
    required_paths = (
        ...
        VALIDATOR_PATH,
        BUILD_ZIG_PATH,
        LANE_OWNER_TEST_PATH,
        MAKEFILE_PATH,
        PHASE14_RCU_NOTE_PATH,
        PHASE14_SKBUFF_NOTE_PATH,
        PHASE14_TRACEABILITY_PATH,
        SELF_PATH,
    )
    ...
    if manifest.get("surveyed_commit_mode_reason", "").find("readiness-gate survey") == -1:
    ...
    for entry in blocker_ownership:
        ...
        snippet = (
            f"- `{anchor}`: owner `{entry.get('owner')}`; phase `{entry.get('phase')}`; "
            f"status bucket `{entry.get('status_bucket')}`; required approver set "
            f"`{entry.get('required_approver_set')}`; validation gate "
            f"`{entry.get('validation_gate')}`; rollback owner `{entry.get('rollback_owner')}`"
        )
    ...
    for gap_id in (
        "phase15-readiness-gate-note-readback",
        "phase15-readiness-gate-checker-readback",
        "phase15-shared-lane-owner-readback",
        "phase15-review-checklist-study-only-boundary-guard",
        "phase15-tests-readme-alignment-guard",
        ...
        "blocker_ownership": [
            {
                ...
                "latest_blocker_disposition": EXPECTED_BLOCKERS["kernel/sched/core.c"],
            },
            {
                ...
                "replay_command": "zig test zigux/tests/phase15_freeze_map_governance.zig",
                "latest_blocker_disposition": EXPECTED_BLOCKERS["mm/page_alloc.c"],
            },
            {
                ...
                "evidence_archive_path": "Documentation/zigux/phase15-evidence-archives/kernel-rcu-tree.md",
                "benchmark_notes": "pending_until_rcu_followup_is_narrower_than_freeze_boundary",
                "replay_command": "zig test zigux/tests/phase15_freeze_map_governance.zig",
                "latest_blocker_disposition": EXPECTED_BLOCKERS["kernel/rcu/tree.c"],
            },
            {
                ...
                "evidence_archive_path": "Documentation/zigux/phase15-evidence-archives/net-core-skbuff.md",
                "benchmark_notes": "pending_until_skbuff_followup_is_narrower_than_lifetime_boundary",
                "replay_command": "zig test zigux/tests/phase15_freeze_map_governance.zig",
                "latest_blocker_disposition": EXPECTED_BLOCKERS["net/core/skbuff.c"],
            },
        ],
        "deep_core_blocker_survey": [
            {"anchor": anchor, "roadmap_basis": "basis", "repo_reality": "reality", "current_blocker": EXPECTED_BLOCKERS[anchor]}
            for anchor in EXPECTED_FREEZE_TARGETS
        ],
        "maintenance_handoff": {
            "current_lane_posture": "maintenance_mode",
            "replay_before_trusting": list(EXPECTED_REPLAY_COMMANDS),
            "reopen_conditions": [
                "a freeze-map anchor changes status bucket, blocker disposition, or required approver set",
                "the freeze-in-C or study-only anchor set changes in Documentation/zigux/freeze-map.md",
                "the checker-backed shared reminder packet or an adjacent Phase 15 governance packet drifts enough to change the per-anchor evidence archive, replay command, stay-in-C, or no-silent-exception posture recorded here",
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase15_freeze_map_packet_") as tmp_dir:
        ...
        _seed_repo(case_root)
        _write(
            case_root / GOVERNANCE_NOTE_PATH,
            _sample_governance_note().replace(
                "- materialized_in_contents_readback `phase15-shared-build-route-readback`\n",
                "",
                1,
            ),
        ...
        failures = collect_failures(case_root)
        expected = [
            "missing_note_marker:materialized_in_contents_readback `phase15-shared-build-route-readback`"
        ]
        ...
        _seed_repo(case_root)
        _write(
            case_root / FREEZE_MAP_PATH,
            _sample_freeze_map().replace("rollback threshold, ", "", 1),
        )
        failures = collect_failures(case_root)
        expected = ["missing_freeze_map_term:rollback threshold"]
        if failures != expected:
        ...
        failures = collect_failures(case_root)
        expected = ["unexpected_make_target:phase15"]
        if failures != expected:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify that the Phase 15 freeze-map governance packet stays aligned."
    )
    parser.add_argument("--root", type=Path, default=Path.cwd(), help="repository root")
    parser.add_argument(
        "--self-test",
        action="store_true",
        help="run the synthetic fixture coverage for this checker",
    )
