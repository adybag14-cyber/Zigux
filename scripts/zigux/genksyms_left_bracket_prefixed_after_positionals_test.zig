...
test "genksyms bridge treats left-bracket-prefixed values after positionals as request input" {
    ...
    try testing.expectEqualStrings("[input.o", request.rendered_args[2]);
}

test "genksyms bridge accepts left-bracket-prefixed required option arguments after positionals" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    ...
    try testing.expectEqualStrings("[types.sym", request.rendered_args[4]);
    try testing.expectEqualStrings("-d", request.rendered_args[5]);
    try testing.expectEqualStrings("input.c", request.rendered_args[6]);
}

test "genksyms bridge keeps left-bracket-prefixed terminator tail out of option parsing" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    ...
    try testing.expectEqualStrings("[tail", request.rendered_args[3]);
    try testing.expectEqualStrings("--reference", request.rendered_args[4]);
    try testing.expectEqualStrings("[ref", request.rendered_args[5]);
}

test "genksyms bridge renders left-bracket-prefixed terminator tail in bridge json" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    ...
    try testing.expectEqualStrings(
        "{\"tool\":\"scripts/genksyms/genksyms\",\"stdin\":\"cpp-stream\",\"stdout\":\"symversions\",\"argv\":[\"scripts/genksyms/genksyms\",\"-p\",\"input.c\",\"--\",\"[tail\",\"-V\"],\"options\":{\"debug_level\":0,\"warnings\":false,\"dump_defs\":false,\"preserve\":true,\"reference_files\":[],\"dump_types_file\":null}}\n",
        output.written(),
    );
}
