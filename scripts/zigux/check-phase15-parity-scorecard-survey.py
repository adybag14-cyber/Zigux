...
EXPECTED_SCORECARD_COMMIT = "current-master-readback-2026-05-21"
EXPECTED_ANCHORS = (
    "kernel/sched/core.c",
    "mm/page_alloc.c",
    "kernel/rcu/tree.c",
...
FORBIDDEN_SURVEY_MARKERS = (
    ...
    "phase15-parity-scorecard-survey-exact-handoff-readback",
    "phase15-parity-scorecard-dedicated-route-alignment-readback",
...
def collect_failures(root: Path) -> list[str]:
    ...
    if EXPECTED_SURVEYED_COMMIT not in scorecard_zig:
...
def _sample_scorecard_zig() -> str:
    return f"""test \"phase15 scorecard\" {{
    _ = \"{EXPECTED_SURVEYED_COMMIT}\";
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase15_parity_scorecard_survey_") as tmp_dir:
        ...
        _write(
            ...
            _sample_survey().replace("blocked `phase15-parity-scorecard-reminder-route-wording-drift`\n", "", 1),
        )
        failures = collect_failures(missing_marker_root)
        expected = [
            "survey:missing_marker:blocked `phase15-parity-scorecard-reminder-route-wording-drift`"
        ]
        ...
        expected = [
            ...
            "survey:forbidden_marker_present:That means the survey-local truthfulness gap is now closed.",
        ]
        if failures != expected:
        ...
        drifted["surveyed_commit"] = "current-master-readback-2026-05-20"
