...
EXPECTED_MARKERS = {
    ROOT / "scripts" / "zigux" / "mk_elfconfig_fd_trailing_elf32_hides_elf64_test.zig": {
        "label": "elf32_trailing_hides_elf64",
        "marker": 'test "fd-backed trailing 32-bit ELF input keeps a later 64-bit ELF header hidden behind the trailing bytes" {',
    },
    ROOT / "scripts" / "zigux" / "mk_elfconfig_fd_trailing_elf64_hides_elf32_test.zig": {
        "label": "elf64_trailing_hides_elf32",
        "marker": 'test "fd-backed trailing 64-bit ELF input keeps a later 32-bit ELF header hidden behind the trailing bytes" {',
    },
}
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="lane14-fd-trailing-hide-cluster-") as tmp_dir:
        ...
        try:
            ...
            if "missing_marker:elf64_trailing_hides_elf32" not in str(exc):
                raise
        else:
        ...
        duplicate_path = (
            duplicate_root / "scripts" / "zigux" / "mk_elfconfig_fd_trailing_elf32_hides_elf64_test.zig"
        )
        ...
        try:
            ...
            if "duplicate_marker:elf32_trailing_hides_elf64" not in str(exc):
                raise
        else:
            raise AssertionError("expected duplicate-marker self-test failure")

    print("MK_ELFCONFIG_FD_TRAILING_HIDE_CLUSTER_MARKERS_SELF_TEST=pass")
    print("MK_ELFCONFIG_FD_TRAILING_HIDE_CLUSTER_MARKERS_SELF_TEST_CASE_COUNT=3")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check that the mk_elfconfig trailing mixed-class hide replay cluster stays present."
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    print("MK_ELFCONFIG_FD_TRAILING_HIDE_CLUSTER_MARKERS=pass")
    print(f"MK_ELFCONFIG_FD_TRAILING_HIDE_CLUSTER_MARKERS_COUNT={len(EXPECTED_MARKERS)}")
    return 0
