...
RELEASE_MARKERS = [
    ...
    "PHASE13_SHARED_REPLAY_STEP_COUNT=13",
...
DOCS_ROOT_MARKERS = [
    ...
    "`Documentation/zigux/README.md` now also keeps the dedicated `zigux/tests/phase13_landlock_syscalls_reviewability.zig` gate, the adjacent notifier-list manifest `zigux/tests/phase13_notifier_list_manifest.json`, and the roadmap-adjacent notifier evidence (`zigux/tests/phase13_notifier_list_reviewability.zig`, `zigux/bindings/notifier_abi.zig`, `include/zigux/notifier_abi.h`, and `zigux/helpers/notifier_chain_view.zig`) visible from the docs root, so the top-level Phase 13 summary does not undercount the actual ten-step shared replay on current `master`.",
...
DOCS_ROOT_EXACT_COUNT_MARKERS = {
    "`Documentation/zigux/README.md` now also keeps the dedicated `zigux/tests/phase13_landlock_syscalls_reviewability.zig` gate, the adjacent notifier-list manifest `zigux/tests/phase13_notifier_list_manifest.json`, and the roadmap-adjacent notifier evidence (`zigux/tests/phase13_notifier_list_reviewability.zig`, `zigux/bindings/notifier_abi.zig`, `include/zigux/notifier_abi.h`, and `zigux/helpers/notifier_chain_view.zig`) visible from the docs root, so the top-level Phase 13 summary does not undercount the actual ten-step shared replay on current `master`.": 1,
}
