...
CLOSURE_EVIDENCE_MARKERS = [
    ...
    "directly re-readable helper, verify, build, and route-surface anchors now include `drivers/virtio/virtio.zig`, `drivers/virtio/virtio_ring.zig`, `drivers/virtio/virtio_input.zig`, `drivers/virtio/virtio_input_verify.zig`, `drivers/virtio/virtio_mmio.zig`, `drivers/virtio/virtio_mmio_verify.zig`, `zigux/tests/phase10_build.zig`, `zigux/tests/phase10_virtio_mmio_survey.zig`, and `zigux/Makefile`",
    ...
    "`zigux/Makefile` itself now rematerializes on current `master`, but its live body still exposes only the Phase 2 toolchain and kbuild routes, so `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10` should stay framed as repo-reality-gap vocabulary rather than freshly direct re-read proof here.",
