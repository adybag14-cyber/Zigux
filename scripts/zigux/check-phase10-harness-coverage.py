...
LANE_SEQUENCING_FORBIDDEN_MARKERS = [
    ...
    "validator-first routes, or Linux-style Phase 10 make routes that current readback still does not materialize.",
]
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase10_harness_coverage_") as tmp_dir:
        ...
        expect_missing_marker(root, "Documentation/zigux/phase10-virtio-driver-lane-sequencing.md", "Use the directly re-readable ring, input, and MMIO anchors before widening shared wording", "Use the directly re-readable ring, input, and MMIO anchors before widening shared wording back into direct claims about the missing core-side companions, the missing MMIO slice companion, the missing MMIO manifest surface, validator-first routes, or Linux-style Phase 10 make routes that current readback still does not materialize.", "phase10_lane_sequencing:forbidden:validator-first routes, or Linux-style Phase 10 make routes that current readback still does not materialize.")
