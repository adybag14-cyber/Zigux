...
LANE_SEQUENCING_MARKERS = [
    ...
    "`zigux/Makefile` now rematerializes and its live body exposes `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10`",
    "Use the directly re-readable ring, input, and MMIO anchors before widening shared wording back into direct claims about the missing core-side companions, the missing MMIO slice companion, the missing MMIO manifest surface, or the still-missing dedicated `scripts/zigux/validate-phase10.py` and `scripts/zigux/validate-phase10-closure.py` surfaces.",
...
LANE_SEQUENCING_FORBIDDEN_MARKERS = [
    ...
    "`zigux/Makefile` now rematerializes but still exposes only the Phase 2 toolchain and kbuild routes, so keep those validator-first names and the Linux-style Phase 10 make-route names framed as last-known packet members or repo-reality-gap vocabulary until a fresh reread proves the Phase 10 route stack is back",
    "so keep those validator-first names framed as last-known packet members or repo-reality gaps while treating the returned Makefile-backed route stack as the shared build gate",
    "Use the directly re-readable ring, input, and MMIO anchors before widening shared wording back into direct claims about the missing core-side companions, the missing MMIO slice companion, the missing MMIO manifest surface, or the validator-first routes that current readback still does not materialize.",
]
...
MAKEFILE_MARKERS = [
    ...
    "$(PYTHON) scripts/zigux/check-phase10-input-packet.py",
    "$(PYTHON) scripts/zigux/check-phase10-mmio-packet.py",
...
def validate(root: Path) -> tuple[list[str], list[str]]:
    ...
    bootstrap_route = read_text(root, "scripts/zigux/check-phase10-bootstrap-route.py")
    ...
    check_markers(missing_markers, "phase10_makefile", makefile, MAKEFILE_MARKERS)
    ...
    check_markers(missing_markers, "phase10_bootstrap_route", bootstrap_route, BOOTSTRAP_ROUTE_MARKERS)
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="zigux_phase10_harness_coverage_") as tmp_dir:
        ...
        expect_missing_marker(root, "zigux/Makefile", "$(PYTHON) scripts/zigux/check-phase10-bootstrap-route.py", "$(PYTHON) scripts/zigux/check-phase10-bootstrap-route-missing.py", "phase10_makefile:$(PYTHON) scripts/zigux/check-phase10-bootstrap-route.py")
        expect_missing_marker(root, "scripts/zigux/check-phase10-bootstrap-route.py", 'VALIDATE_CMD = "make -C zigux phase10-validate"', 'VALIDATE_CMD = "make -C zigux phase10-validate-missing"', 'phase10_bootstrap_route:VALIDATE_CMD = "make -C zigux phase10-validate"')
        ...
        expect_missing_marker(root, "Documentation/zigux/phase10-virtio-driver-lane-sequencing.md", "current `master` still does not materialize `scripts/zigux/validate-phase10.py` or `scripts/zigux/validate-phase10-closure.py` through the direct readback available in this lane, while `zigux/Makefile` now rematerializes and its live body exposes `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10`, so keep those still-missing dedicated validator-script names framed as last-known packet members or repo-reality gaps while treating the returned Makefile-backed route stack as the shared build gate", "current `master` still does not materialize `scripts/zigux/validate-phase10.py` or `scripts/zigux/validate-phase10-closure.py` through the direct readback available in this lane, while `zigux/Makefile` now rematerializes and its live body exposes `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10`, so keep those validator-first names framed as last-known packet members or repo-reality gaps while treating the returned Makefile-backed route stack as the shared build gate", "phase10_lane_sequencing:forbidden:so keep those validator-first names framed as last-known packet members or repo-reality gaps while treating the returned Makefile-backed route stack as the shared build gate")
        expect_missing_marker(root, "Documentation/zigux/phase10-virtio-driver-lane-sequencing.md", "`zigux/Makefile` now rematerializes and its live body exposes `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10`", "`zigux/Makefile` now rematerializes but still exposes only the Phase 2 toolchain and kbuild routes", "phase10_lane_sequencing:`zigux/Makefile` now rematerializes and its live body exposes `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10`")
        expect_missing_marker(root, "Documentation/zigux/phase10-virtio-driver-lane-sequencing.md", "Keep the returned `zigux/Makefile` body together with `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10` explicit as the shared build gate rather than restating them as gaps.", "Keep the returned shared build gate implicit.", "phase10_lane_sequencing:Keep the returned `zigux/Makefile` body together with `make -C zigux phase10-validate`, `make -C zigux phase10-test`, and `make -C zigux phase10` explicit as the shared build gate rather than restating them as gaps.")
        expect_missing_marker(root, "Documentation/zigux/phase10-virtio-driver-lane-sequencing.md", "Use the directly re-readable ring, input, and MMIO anchors before widening shared wording back into direct claims about the missing core-side companions, the missing MMIO slice companion, the missing MMIO manifest surface, or the still-missing dedicated `scripts/zigux/validate-phase10.py` and `scripts/zigux/validate-phase10-closure.py` surfaces.", "Use the directly re-readable ring, input, and MMIO anchors before widening shared wording back into direct claims about the missing core-side companions, the missing MMIO slice companion, the missing MMIO manifest surface, or the validator-first routes that current readback still does not materialize.", "phase10_lane_sequencing:forbidden:Use the directly re-readable ring, input, and MMIO anchors before widening shared wording back into direct claims about the missing core-side companions, the missing MMIO slice companion, the missing MMIO manifest surface, or the validator-first routes that current readback still does not materialize.")
...
def main() -> int:
    ...
    print("PHASE10_HARNESS_COVERAGE_REQUIRED_MARKER_COUNT=" f"{len(DOCS_ROOT_MARKERS) + len(REVIEW_CHECKLIST_MARKERS) + len(COMPANION_MARKERS) + len(LANE_SEQUENCING_MARKERS) + len(CLOSURE_EVIDENCE_MARKERS) + len(MODULE_SLICE_MARKERS) + len(BUILD_MARKERS) + len(MAKEFILE_MARKERS) + len(REGISTRATION_HELPER_MARKERS) + len(VERIFY_MARKERS) + len(BOOTSTRAP_ROUTE_MARKERS) + len(SHARED_FREEZE_BOUNDARY_MARKERS) + len(SCRIPTS_README_MARKERS)}")
