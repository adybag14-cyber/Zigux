...
- `PHASE10_LEDGER_REPO_REALITY_GAPS=Documentation/zigux/phase10-virtio-core-slice.md,Documentation/zigux/phase10-virtio-mmio-slice.md`
...
The two older Phase 10 slice-note paths that still stay absent on current `master` are `Documentation/zigux/phase10-virtio-core-slice.md` and `Documentation/zigux/phase10-virtio-mmio-slice.md`, so this ledger keeps those note companions separate from readback-path flakiness instead of folding them together with authenticated contents misses.
