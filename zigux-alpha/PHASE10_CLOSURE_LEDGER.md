...
The same closure packet also keeps the landed ring and input helper packets explicit: the ring survey still owns the queue-discipline ladder through broken-queue recovery, and the dedicated input packet still owns the capability, multitouch-slot, teardown-observation, registration-preflight, queue-callback-preflight, probe-preflight, and registration-blocker helpers. Mirroring those helper ladders here keeps the active virtio tranche self-contained instead of making later closure checks reconstruct that state from prose and per-driver manifests.
