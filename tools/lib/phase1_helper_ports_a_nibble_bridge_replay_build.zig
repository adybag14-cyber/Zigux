...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_nibble_bridge_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-nibble-bridge-replay-tests",
    ...
    const named_step = b.step(
        "phase1-helper-ports-a-nibble-bridge-replay",
        "Run the Lane 06 Phase 1 helper ports A nibble bridge replay",
    );
    named_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 Phase 1 helper ports A nibble bridge replay");
