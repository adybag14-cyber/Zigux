...
pub fn fill(dst: []Word, nbits: usize) void {
    ...
    @memset(dst[0..bitsToWords(nbits)], ~@as(Word, 0));
}
...
test "bitmap fill rounds a partial tail up to the active word window and zero clears only the active word window" {
