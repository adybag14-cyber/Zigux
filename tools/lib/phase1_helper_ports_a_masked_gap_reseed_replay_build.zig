...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_masked_gap_reseed_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named_step = b.step("phase1-helper-ports-a-masked-gap-reseed-replay", "Run the Lane 06 masked gap reseed replay");
    named_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 masked gap reseed replay tests");
