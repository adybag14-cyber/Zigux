...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_helix_gate_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const unit_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-helix-gate-replay-tests",
        .root_module = root_module,
    });
    const run_unit_tests = b.addRunArtifact(unit_tests);

    const replay_step = b.step("phase1-helper-ports-a-helix-gate-replay", "Run the Lane 06 helper ports A helix gate replay");
    replay_step.dependOn(&run_unit_tests.step);

    const test_step = b.step("test", "Run the Lane 06 helper ports A helix gate replay");
