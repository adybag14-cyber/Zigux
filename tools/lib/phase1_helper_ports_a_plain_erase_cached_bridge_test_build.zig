...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_plain_erase_cached_bridge_test.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-plain-erase-cached-bridge-test",
        .root_module = root_module,
    });

    const run_tests = b.addRunArtifact(tests);
    const route = b.step(
        "phase1-helper-ports-a-plain-erase-cached-bridge-test",
        "Run the Lane 06 helper ports A plain erase cached bridge contract",
    );
    route.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 helper ports A plain erase cached bridge contract");
