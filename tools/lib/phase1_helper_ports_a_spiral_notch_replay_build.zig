...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_spiral_notch_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const test_step = b.step("phase1-helper-ports-a-spiral-notch-replay", "Run Lane 06 Phase 1 helper ports A spiral notch replay");
    const unit_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-spiral-notch-replay",
        .root_module = root_module,
    });
    const run_tests = b.addRunArtifact(unit_tests);
    test_step.dependOn(&run_tests.step);

    const default_test_step = b.step("test", "Run Lane 06 Phase 1 helper ports A spiral notch replay");
