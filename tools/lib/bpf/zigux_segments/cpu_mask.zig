...
test "parseCpuMaskString rejects empty and malformed ranges" {
    ...
    try std.testing.expectError(error.InvalidCpuRange, parseCpuMaskString(std.testing.allocator, "0\r\n1"));
}
...
test "parseCpuMaskFromReader rejects carriage-return-delimited input that libbpf would reject" {
    const ReaderState = struct {
        chunks: []const []const u8,
        index: usize = 0,
