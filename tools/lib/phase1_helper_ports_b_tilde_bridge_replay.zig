...
test "argv split preserves tilde packet tokens" {
    var args = try argv_split.argvSplit(
        std.testing.allocator,
        " root=~/disk tilde~bridge key=value~more plain ",
    );
    ...
    try std.testing.expectEqualStrings("tilde~bridge", splitArg(args, 1));
...
test "argv split tilde tokens are duplicated and writable" {
...
test "cmdline memparse leaves tilde tail exact" {
    const hex = cmdline.memparse("0x7e~mask");
    try std.testing.expectEqual(@as(u64, 0x7e), hex.value);
    try std.testing.expectEqualStrings("~mask", hex.rest);

    const scaled = cmdline.memparse("126K~boot");
    try std.testing.expectEqual(@as(u64, 126 << 10), scaled.value);
    try std.testing.expectEqualStrings("~boot", scaled.rest);
}

test "cmdline option list treats tilde as option data" {
    try std.testing.expect(cmdline.parseOptionStr("quiet,debug~trace,nohlt", "debug~trace"));
    try std.testing.expect(cmdline.parse_option_str("quiet,tilde~bridge,nohlt", "tilde~bridge"));
...
test "cmdline nextArg keeps tilde within tokens when present" {
...
test "ctype classifies tilde as punctuation graph print data" {
    try std.testing.expectEqual(ctype._P, ctype.mask(tilde));
    try std.testing.expect(ctype.ispunct(tilde));
    try std.testing.expect(ctype.isgraph(tilde));
    try std.testing.expect(ctype.isprint(tilde));
    try std.testing.expect(!ctype.isspace(tilde));
    try std.testing.expect(!ctype.isalnum(tilde));
    try std.testing.expectEqual(tilde, ctype.tolower(tilde));
    try std.testing.expectEqual(tilde, ctype.toupper(tilde));
    try std.testing.expectEqual(tilde, ctype.fastTolower(tilde));
}

test "hweight counts tilde byte lanes consistently" {
    try std.testing.expectEqual(@as(u32, 6), hweight8Count(tilde));
    try std.testing.expectEqual(@as(u32, 12), hweight16Count(0x7e7e));
    try std.testing.expectEqual(@as(u32, 24), hweight32Count(0x7e7e_7e7e));
    try std.testing.expectEqual(@as(u64, 48), hweight64Count(0x7e7e_7e7e_7e7e_7e7e));
}

test "tilde bridge keeps mask tail visible across helpers" {
    var args = try argv_split.argvSplit(std.testing.allocator, "mask=0x7e~flags tail");
    defer freeSplit(std.testing.allocator, &args);
    ...
    try std.testing.expectEqual(@as(u64, 0x7e), parsed.value);
