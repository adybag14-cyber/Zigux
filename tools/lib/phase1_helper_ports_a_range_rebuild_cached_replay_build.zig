...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_range_rebuild_cached_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-range-rebuild-cached-replay-tests",
        .root_module = replay_module,
    });

    const run_replay_tests = b.addRunArtifact(replay_tests);
    const route = b.step("phase1-helper-ports-a-range-rebuild-cached-replay", "Run the Lane 06 range rebuild cached replay");
    route.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run the Lane 06 range rebuild cached replay");
