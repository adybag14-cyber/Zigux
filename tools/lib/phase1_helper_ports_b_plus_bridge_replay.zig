...
test "argv split preserves plus packet tokens" {
    var args = try argv_split.argvSplit(
        std.testing.allocator,
        " root=/dev+disk plus+bridge key=value+more plain ",
    );
    ...
    try std.testing.expectEqualStrings("plus+bridge", splitArg(args, 1));
...
test "argv split plus tokens are duplicated and writable" {
...
test "cmdline memparse leaves plus tail exact" {
    const hex = cmdline.memparse("0x2b+mask");
    try std.testing.expectEqual(@as(u64, 0x2b), hex.value);
    try std.testing.expectEqualStrings("+mask", hex.rest);

    const signed = cmdline.memparse("+64K+boot");
    try std.testing.expectEqual(@as(u64, 64 << 10), signed.value);
    try std.testing.expectEqualStrings("+boot", signed.rest);
}

test "cmdline option list treats plus as option data" {
    try std.testing.expect(cmdline.parseOptionStr("quiet,debug+trace,nohlt", "debug+trace"));
    try std.testing.expect(cmdline.parse_option_str("quiet,plus+bridge,nohlt", "plus+bridge"));
...
test "cmdline nextArg keeps plus within tokens when present" {
...
test "ctype classifies plus as punctuation graph print data" {
    try std.testing.expectEqual(ctype._P, ctype.mask(plus));
    try std.testing.expect(ctype.ispunct(plus));
    try std.testing.expect(ctype.isgraph(plus));
    try std.testing.expect(ctype.isprint(plus));
    try std.testing.expect(!ctype.isspace(plus));
    try std.testing.expect(!ctype.isalnum(plus));
    try std.testing.expectEqual(plus, ctype.tolower(plus));
    try std.testing.expectEqual(plus, ctype.toupper(plus));
    try std.testing.expectEqual(plus, ctype.fastTolower(plus));
}

test "hweight counts plus byte lanes consistently" {
    try std.testing.expectEqual(@as(u32, 4), hweight8Count(plus));
    try std.testing.expectEqual(@as(u32, 8), hweight16Count(0x2b2b));
    try std.testing.expectEqual(@as(u32, 16), hweight32Count(0x2b2b_2b2b));
    try std.testing.expectEqual(@as(u64, 32), hweight64Count(0x2b2b_2b2b_2b2b_2b2b));
}

test "plus bridge keeps signed mask tail visible across helpers" {
    var args = try argv_split.argvSplit(std.testing.allocator, "mask=+0x2b+flags tail");
    defer freeSplit(std.testing.allocator, &args);
    ...
    try std.testing.expectEqual(@as(u64, 0x2b), parsed.value);
    try std.testing.expectEqualStrings("+flags", parsed.rest);
    try std.testing.expect(ctype.ispunct(parsed.rest[0]));
    try std.testing.expectEqual(@as(u32, 4), hweight8Count(parsed.rest[0]));
}
