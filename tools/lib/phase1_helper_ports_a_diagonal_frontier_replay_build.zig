...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_diagonal_frontier_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step("phase1-helper-ports-a-diagonal-frontier-replay", "Run Lane 06 diagonal frontier helper replay");
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run Lane 06 diagonal frontier helper replay tests");
