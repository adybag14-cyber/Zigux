...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_complement_or_prev_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step("phase1-helper-ports-a-complement-or-prev-replay", "Run the Lane 06 helper ports A complement/OR/prev replay");
    replay_step.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run the Lane 06 helper ports A complement/OR/prev replay");
