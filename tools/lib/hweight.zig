...
test "hweight helpers satisfy overlap inclusion exclusion identities" {
    const left8: u32 = 0b1110_0110;
    const right8: u32 = 0b1011_1010;
    try std.testing.expectEqual(swHweight8(left8) + swHweight8(right8), swHweight8(left8 ^ right8) + (2 * swHweight8(left8 & right8)));
    try std.testing.expectEqual(__sw_hweight8(left8) + __sw_hweight8(right8), __sw_hweight8(left8 ^ right8) + (2 * __sw_hweight8(left8 & right8)));

    const left16: u32 = 0xe6b5;
    const right16: u32 = 0xbad3;
    try std.testing.expectEqual(swHweight16(left16) + swHweight16(right16), swHweight16(left16 ^ right16) + (2 * swHweight16(left16 & right16)));
    try std.testing.expectEqual(__sw_hweight16(left16) + __sw_hweight16(right16), __sw_hweight16(left16 ^ right16) + (2 * __sw_hweight16(left16 & right16)));

    const left32: u32 = 0xe6b5_93ad;
    const right32: u32 = 0xbad3_f16c;
    try std.testing.expectEqual(swHweight32(left32) + swHweight32(right32), swHweight32(left32 ^ right32) + (2 * swHweight32(left32 & right32)));
    try std.testing.expectEqual(__sw_hweight32(left32) + __sw_hweight32(right32), __sw_hweight32(left32 ^ right32) + (2 * __sw_hweight32(left32 & right32)));

    const left64: u64 = 0xe6b5_93ad_7c4f_2811;
    const right64: u64 = 0xbad3_f16c_95a0_72ce;
    try std.testing.expectEqual(swHweight64(left64) + swHweight64(right64), swHweight64(left64 ^ right64) + (2 * swHweight64(left64 & right64)));
    try std.testing.expectEqual(__sw_hweight64(left64) + __sw_hweight64(right64), __sw_hweight64(left64 ^ right64) + (2 * __sw_hweight64(left64 & right64)));

    const left_long: usize = if (@sizeOf(usize) == 4) 0xe6b5_93ad else 0xe6b5_93ad_7c4f_2811;
    const right_long: usize = if (@sizeOf(usize) == 4) 0xbad3_f16c else 0xbad3_f16c_95a0_72ce;
    try std.testing.expectEqual(hweightLong(left_long) + hweightLong(right_long), hweightLong(left_long ^ right_long) + (2 * hweightLong(left_long & right_long)));
    try std.testing.expectEqual(hweight_long(left_long) + hweight_long(right_long), hweight_long(left_long ^ right_long) + (2 * hweight_long(left_long & right_long)));
}
