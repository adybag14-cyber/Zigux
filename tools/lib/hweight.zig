...
test "hweight helpers complement to the width bit count" {
    const value8: u32 = 0b1010_0101;
    try std.testing.expectEqual(@as(u32, 8), swHweight8(value8) + swHweight8((~value8) & 0xff));
    try std.testing.expectEqual(@as(u32, 8), __sw_hweight8(value8) + __sw_hweight8((~value8) & 0xff));

    const value16: u32 = 0xa55a;
    try std.testing.expectEqual(@as(u32, 16), swHweight16(value16) + swHweight16((~value16) & 0xffff));
    try std.testing.expectEqual(@as(u32, 16), __sw_hweight16(value16) + __sw_hweight16((~value16) & 0xffff));

    const value32: u32 = 0xa55a_c33c;
    ...
    const value64: u64 = 0xa55a_c33c_0f0f_f00f;
    ...
    const value_long: usize = if (@sizeOf(usize) == 4) 0xa55a_c33c else 0xa55a_c33c_0f0f_f00f;
