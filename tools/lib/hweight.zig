...
test "hweight helpers pair each value with its in-width complement" {
    const value8: u32 = 0b0011_0101;
    const mask8: u32 = 0xff;
    try std.testing.expectEqual(@as(u32, 8), swHweight8(value8) + swHweight8((~value8) & mask8));
    try std.testing.expectEqual(@as(u32, 8), __sw_hweight8(value8) + __sw_hweight8((~value8) & mask8));

    const value16: u32 = 0x1357;
    const mask16: u32 = 0xffff;
    try std.testing.expectEqual(@as(u32, 16), swHweight16(value16) + swHweight16((~value16) & mask16));
    try std.testing.expectEqual(@as(u32, 16), __sw_hweight16(value16) + __sw_hweight16((~value16) & mask16));

    const value32: u32 = 0x1357_9bdf;
    const mask32: u32 = 0xffff_ffff;
    try std.testing.expectEqual(@as(u32, 32), swHweight32(value32) + swHweight32((~value32) & mask32));
    try std.testing.expectEqual(@as(u32, 32), __sw_hweight32(value32) + __sw_hweight32((~value32) & mask32));

    const value64: u64 = 0x0123_4567_89ab_cdef;
    const mask64: u64 = 0xffff_ffff_ffff_ffff;
    try std.testing.expectEqual(@as(u64, 64), swHweight64(value64) + swHweight64((~value64) & mask64));
    try std.testing.expectEqual(@as(u64, 64), __sw_hweight64(value64) + __sw_hweight64((~value64) & mask64));

    const long_mask: usize = std.math.maxInt(usize);
    const long_value: usize = if (@sizeOf(usize) == 4) 0x89ab_cdef else 0x0123_4567_89ab_cdef;
    try std.testing.expectEqual(@bitSizeOf(usize), hweightLong(long_value) + hweightLong((~long_value) & long_mask));
    try std.testing.expectEqual(@bitSizeOf(usize), hweight_long(long_value) + hweight_long((~long_value) & long_mask));
}
