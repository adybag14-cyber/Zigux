...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_stride_halo_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const tests = b.addTest(.{
        .name = "phase1-helper-ports-a-stride-halo-replay-tests",
    ...
    const named_step = b.step(
        "phase1-helper-ports-a-stride-halo-replay",
        "Run the Lane 06 Phase 1 helper ports A stride halo replay",
    );
    named_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 Phase 1 helper ports A stride halo replay");
