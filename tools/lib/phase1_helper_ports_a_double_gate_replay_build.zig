...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_double_gate_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named_step = b.step("phase1-helper-ports-a-double-gate-replay", "Run the Lane 06 Phase 1 helper ports A double-gate replay");
    named_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 Phase 1 helper ports A double-gate replay");
