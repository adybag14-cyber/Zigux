...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_offset_mirror_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_tests = b.addTest(.{
        .name = "phase1-helper-ports-a-offset-mirror-replay-tests",
        .root_module = replay_module,
    });
    const run_replay_tests = b.addRunArtifact(replay_tests);

    const route = b.step("phase1-helper-ports-a-offset-mirror-replay", "Run the Lane 06 offset mirror helper replay");
    route.dependOn(&run_replay_tests.step);

    const test_step = b.step("test", "Run the Lane 06 offset mirror helper replay");
