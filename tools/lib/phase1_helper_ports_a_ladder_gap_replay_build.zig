...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_ladder_gap_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named = b.step("phase1-helper-ports-a-ladder-gap-replay", "Run the Lane 06 helper ports A ladder-gap replay");
    named.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 helper ports A ladder-gap replay");
