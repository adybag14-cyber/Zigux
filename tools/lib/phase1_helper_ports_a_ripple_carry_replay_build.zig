...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_ripple_carry_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const named_step = b.step("phase1-helper-ports-a-ripple-carry-replay", "Run the Lane 06 ripple-carry helper ports A replay");
    named_step.dependOn(&run_tests.step);

    const test_step = b.step("test", "Run the Lane 06 ripple-carry helper ports A replay tests");
