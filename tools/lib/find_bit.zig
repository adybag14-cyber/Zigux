...
test "clump8 next scans ignore out-of-range tail bits once the in-range tail bit is behind start" {
    const nbits = bits_per_long + 5;
    const bitmap = [_]Word{ 0, (@as(Word, 1) << 3) | (@as(Word, 1) << 6) };

    var clump: u8 = 0;
    try std.testing.expectEqual(@as(usize, bits_per_long), findNextClump8(&clump, &bitmap, nbits, bits_per_long + 2));
    try std.testing.expectEqual(@as(u8, 0b0000_1000), clump);
