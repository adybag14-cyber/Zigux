...
test "scnprintfPad reports the visible padded width" {
    var buffer: [9]u8 = undefined;
    const written = scnprintfPad(&buffer, buffer.len - 1, "id={d}", .{7});
    try std.testing.expectEqual(@as(usize, 8), buffer.len - 1);
    try std.testing.expectEqual(@as(usize, 8), std.mem.indexOfScalar(u8, &buffer, 0).?);
    try std.testing.expectEqual(@as(usize, 8), written);
}

test "scnprintfPad with zero logical size leaves an empty C string" {
    var buffer = [_]u8{ 0xaa, 0xbb };
    const written = scnprintfPad(&buffer, 0, "{s}", .{"ignored"});
