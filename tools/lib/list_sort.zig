...
test "list sort preserves current order when every later comparison ties" {
    const Entry = struct {
        key: i32,
        ordinal: usize,
        node: ListHead = .{},
    ...
    var mode = SortMode.descending;
    listSort(&mode, &head, directional_cmp);
    listSort(null, &head, all_ties_cmp);
