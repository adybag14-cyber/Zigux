...
test "list sort reuses signed subtractive comparator across repeated reordering" {
    const Entry = struct {
        key: i32,
        ordinal: usize,
        node: ListHead = .{},
    ...
    while (current != &head) : (current = current.?.next) {
        const entry: *const Entry = @fieldParentPtr("node", current.?);
        descending_keys[idx] = entry.key;
        descending_ordinals[idx] = entry.ordinal;
        try std.testing.expect(current.?.next.?.prev == current.?);
        try std.testing.expect(current.?.prev.?.next == current.?);
        idx += 1;
    }

    try std.testing.expectEqualSlices(i32, &.{ 5, 5, 2, 0, -1, -3, -3 }, descending_keys[0..idx]);
    try std.testing.expectEqualSlices(usize, &.{ 1, 5, 4, 2, 6, 0, 3 }, descending_ordinals[0..idx]);
    try std.testing.expect(head.next == &entries[1].node);
    try std.testing.expect(head.prev == &entries[3].node);
    ...
    while (current != &head) : (current = current.?.next) {
        const entry: *const Entry = @fieldParentPtr("node", current.?);
        ascending_keys[idx] = entry.key;
        ascending_ordinals[idx] = entry.ordinal;
        try std.testing.expect(current.?.next.?.prev == current.?);
        try std.testing.expect(current.?.prev.?.next == current.?);
        idx += 1;
    }

    try std.testing.expectEqualSlices(i32, &.{ -3, -3, -1, 0, 2, 5, 5 }, ascending_keys[0..idx]);
    try std.testing.expectEqualSlices(usize, &.{ 0, 3, 6, 2, 4, 1, 5 }, ascending_ordinals[0..idx]);
    try std.testing.expect(head.next == &entries[0].node);
    try std.testing.expect(head.prev == &entries[5].node);
}
