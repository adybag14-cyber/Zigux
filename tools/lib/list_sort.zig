...
test "list sort reuses boolean-style comparator context across repeated reordering" {
    const Entry = struct {
        key: i32,
        ordinal: usize,
        node: ListHead = .{},
...
test "list sort preserves current boolean-style order when a later pass ties everything" {
    const Entry = struct {
        key: i32,
        ordinal: usize,
        node: ListHead = .{},
    ...
    var mode = SortMode.descending;
    listSort(&mode, &head, boolean_cmp);
    listSort(null, &head, ties_cmp);
...
test "list sort reuses signed subtractive comparator context across repeated reordering" {
    ...
    try std.testing.expectEqualSlices(i32, &.{ -3, -3, -1, 0, 5, 7, 7 }, keys[0..idx]);
    try std.testing.expectEqualSlices(usize, &.{ 0, 5, 2, 4, 6, 1, 3 }, ordinals[0..idx]);
    try std.testing.expect(head.next == &entries[0].node);
    try std.testing.expect(head.prev == &entries[3].node);
}
