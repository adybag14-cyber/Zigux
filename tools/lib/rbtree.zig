...
pub const PostorderSafeIterator = struct {
    ...
    pub fn next(self: *PostorderSafeIterator) ?*Node {
        const node = self.current orelse return null;
        // Cache exactly one step ahead before the caller mutates `node`.
        // Caching farther ahead can go stale after erase-triggered rebalancing.
        self.current = nextPostorder(node);
        return node;
    }
};
...
test "rbtree iteratePostorderSafe caches the next node before node invalidation" {
    const Entry = struct {
        key: i32,
        node: Node = Node.init(),
    };
...
test "rbtree findFirst returns the leftmost duplicate match" {
    const Entry = struct {
        key: i32,
        serial: usize,
        node: Node = Node.init(),
    ...
    const found_entry: *const Entry = @fieldParentPtr("node", found);
    try std.testing.expectEqual(@as(i32, 10), found_entry.key);
    try std.testing.expectEqual(@as(usize, 0), found_entry.serial);
...
test "rbtree iterateMatches streams only the duplicate range" {
    ...
    const wanted = @as(i32, 10);
    var iterator = iterateMatches(&wanted, &root, cmpKey);
    var serials: [3]usize = undefined;
    var count: usize = 0;
    while (iterator.next()) |node| {
        const entry: *const Entry = @fieldParentPtr("node", node);
