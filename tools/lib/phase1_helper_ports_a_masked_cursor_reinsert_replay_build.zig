...
pub fn build(b: *std.Build) void {
    ...
    const replay_module = b.createModule(.{
        .root_source_file = b.path("phase1_helper_ports_a_masked_cursor_reinsert_replay.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const replay_step = b.step("phase1-helper-ports-a-masked-cursor-reinsert-replay", "Run the Lane 06 masked cursor reinsert replay");
    replay_step.dependOn(&replay_tests.step);

    const test_step = b.step("test", "Run all tests");
    test_step.dependOn(&replay_tests.step);
}
